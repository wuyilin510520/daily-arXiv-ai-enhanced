...
Motivation: Existing radiography report generation methods lack sufficient quality for real-world clinical deployment, while large visual language models show promise through alignment techniques adapted from LLMs.

Method: Uses Direct Preference Optimization with random contrastive sampling to construct training pairs, eliminating need for reward models or human preference annotations. Framework is model-agnostic and supplements existing state-of-the-art models.

Result: Improves clinical performance metrics by up to 5% when applied to three state-of-the-art models, without requiring additional training data.

Conclusion: The proposed Random DPO framework effectively enhances RRG accuracy and can be readily integrated with existing models to improve clinical applicability.
...
<details>
  <summary>Details</summary>
Motivation: Current computer-aided autism diagnosis methods have poor gaze feature performance and lack real-world generalizability, requiring better multimodal approaches.

Method: Analyzed 325 participants using multimodal features (facial expressions, voice prosody, head motion, HRV, gaze) with novel statistical descriptors for gaze variability and late fusion integration.

Result: Improved gaze-based classification from 64% to 69% using novel descriptors, and achieved 74% overall accuracy with multimodal fusion.

Conclusion: Multimodal behavioral analysis enables scalable video-based autism screening tools that align with clinical findings on gaze aversion.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models face scalability issues due to quadratic attention costs and unbounded KV memory growth during long-horizon inference, which hinders real-time deployment despite improved generalization through scaling.

Method: Partitions KV cache into fixed-size chunks and uses a recurrent gating module to selectively retain high-utility context by summarizing and filtering historical information based on learned utility scores while maintaining causality.
...
<details>
  <summary>Details</summary>
Motivation: Large-scale vision language models can generate realistic radiology reports but suffer from factual errors and hallucinations, limiting their clinical translation. There's a need for automated error detection in generated reports.

Method: Created synthetic dataset by perturbing findings and locations in ground truth reports, then trained a multi-label cross-modal contrastive regression network to detect errors in findings and their indicated locations.

Result: Achieved robust performance in finding veracity prediction and localization across multiple X-ray datasets, with 0.997 concordance correlation coefficient with ground truth-based verification for error detection in state-of-the-art report generators.

Conclusion: The proposed fact-checking model shows high utility for clinical inference in radiology workflows by effectively detecting errors in automatically generated reports.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal large language models struggle to capture low-level spatial details critical for diagnosing retinal diseases like glaucoma, diabetic retinopathy, and retinitis pigmentosa, limiting their accuracy in medical image analysis.

Method: The MDF-MLLM integrates skip features from four U-Net encoder layers into cross-attention blocks within LLaMA 3.2 11B MLLM. Vision features are patch-wise projected and fused using scaled cross-attention and FiLM-based U-Net modulation. Both U-Net and MLLM components were fully fine-tuned during training on 1,305 fundus image-text pairs from three public datasets.

Result: MDF-MLLM achieved 94% accuracy on dual-type disease classification, representing 56% improvement over baseline MLLM (60% accuracy). Recall and F1-scores improved by 67% and 35% respectively. The model showed substantial gains in spatial reasoning, particularly for inherited diseases with rich clinical text.

Conclusion: MDF-MLLM presents a generalizable, interpretable, and modular framework for fundus image classification that outperforms traditional MLLM baselines through multi-scale feature fusion. The architecture shows promise for clinical decision support systems, with future work planned for synchronized training, expanding disease coverage, and segmentation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-based jailbreak methods struggle to bypass T2I model safety filters, leaving image-based vulnerabilities unexplored.

Method: LLM decouples unsafe prompts into pseudo-safe and harmful prompts, rewrites harmful prompts into adversarial prompts, and uses visual language model for iterative refinement to maintain semantic consistency.

Result: Successfully bypasses safety filters to generate NSFW content while maintaining alignment with original unsafe prompts.

Conclusion: MPDA demonstrates effective multimodal jailbreak attacks on T2I models, highlighting vulnerabilities in current safety mechanisms.
...
Method: Proposes intertwined training using three tasks: salient object detection, foreground contour detection, and edge detection, with a novel mutual learning module (MLM) containing multiple network branches trained in mutual learning manner.
...
Conclusion: The proposed multi-task mutual learning framework effectively addresses incomplete predictions and boundary inaccuracies in saliency detection, delivering superior performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal relevance metrics are limited to bimodal analysis (like CLIP) and cannot properly evaluate similarity across three or more modalities, restricting comprehensive multimodal evaluation.

Method: Uses multimodal joint representation to integrate multiple modalities into the same latent space, enabling fair relevance scoring across N modalities at one consistent scale.
...
Conclusion: MAJORScore provides a more reliable metric for evaluating similarity on large-scale multimodal datasets and multimodal model performance, overcoming limitations of bimodal-only evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Current scaffolding inspections are primarily visual, time-intensive, and prone to human errors, which can lead to unsafe conditions on construction sites. There is a need for more accurate and efficient inspection methods.

Method: Developed a cloud-based AI platform that processes point cloud data of scaffolding structures. The system compares and evaluates certified reference data with recent point cloud data to detect structural modifications.

Result: The proposed approach enables automated monitoring of scaffolding structures, detecting deviations from design specifications that could compromise structural integrity.

Conclusion: AI and digitization can significantly enhance scaffolding inspection accuracy, reduce manual inspection time and effort, and improve overall construction site safety through automated structural monitoring.
...
<details>
  <summary>Details</summary>
Motivation: Fine-grained controllability remains a critical challenge in text-to-image generation, particularly counterfactual controllability - the ability to generate images that deliberately contradict common-sense patterns, which is crucial for creativity and exploratory applications.

Method: Proposed framework with three components: image evaluator for dataset construction, supervised prompt rewriter for revised prompts, and DPO-trained ranker for optimal prompt selection. Created first counterfactual size text-image dataset and enhanced Grounded SAM with refinements.

Result: Method outperforms state-of-the-art baselines and ChatGPT-4o, with image evaluator achieving 114% improvement over its backbone. Establishes foundation for future counterfactual controllability research.

Conclusion: The proposed automatic prompt engineering framework successfully addresses counterfactual size controllability in text-to-image generation, demonstrating superior performance over existing methods and providing a solid basis for advancing counterfactual controllability research.
...
<details>
  <summary>Details</summary>
Motivation: Optical microscopy has resolution limitations (~200nm) that typically require expensive specialized equipment or techniques. This study aims to provide an economical alternative for super-resolution microscopy using standard phase-contrast techniques combined with deep learning.

Method: The study assessed two previously developed DNN architectures (O-Net and Theta-Net) on their ability to super-resolve images from custom-fabricated test targets with nanoscale features calibrated via atomic force microscopy. The evaluation focused on non-fluorescent phase-modulated microscopy modalities including Zernike phase contrast and differential interference contrast microscopy.

Result: Both O-Net and Theta-Net performed well for super-resolution but were complementary rather than competing approaches. O-Net models performed better with high image signal-to-noise ratios, while Theta-Net models were more effective with low SNRs.

Conclusion: The choice of DNN architecture significantly impacts super-resolution performance in non-fluorescent optical nanoscopy, with model selection depending on image SNR conditions. This demonstrates the importance of architecture selection even when using the same training data and training epochs.
...
<details>
  <summary>Details</summary>
Motivation: Prior audio-visual navigation methods overlook deeper perceptual context and struggle with effective multimodal fusion, limiting their ability to guide robots to audio sources efficiently.

Method: Proposes Dynamic Multi-Target Fusion (DMTF-AVN) with multi-target architecture and refined Transformer mechanism to filter and selectively fuse cross-modal visual and audio information.
...
Conclusion: DMTF-AVN demonstrates strong scalability and generalizability, advancing multimodal fusion strategies for robotic navigation applications.
...
Method: Supervised sparse autoencoder training with systematic concept labeling that promotes one-to-one concept-neuron mappings, mitigating feature splitting and promoting feature centralization through cross-entropy computation.
...
Conclusion: SAEmnesia provides an efficient and scalable approach for concept unlearning with minimal computational overhead and significant performance improvements over current state-of-the-art methods.
...
<details>
  <summary>Details</summary>
Motivation: Deep learning models require substantial computational resources for training and hyperparameter search, especially with large datasets. Random sampling for coreset selection fails to capture intra-class diversity and is biased towards dominant classes in imbalanced datasets.

Method: Introduces a lightweight mechanism that extracts intra-class diversity by forming per-class clusters, which are then utilized for final sampling to create representative coresets from the original dataset.
...
Conclusion: The intelligent coreset selection method effectively addresses the limitations of random sampling by capturing intra-class diversity, providing a more representative subset for training and hyperparameter search while reducing computational demands.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning models use single or limited prior mammograms and lack adaptation for real-world settings with imbalanced outcomes and heterogeneous follow-up.

Method: Developed LongiMam - an end-to-end deep learning model combining convolutional and recurrent neural networks to capture spatial and temporal patterns from current and up to four prior mammograms.

Result: LongiMam consistently improved prediction when prior mammograms were included, outperforming single-visit models. Performed best in women with observed changes in mammographic density over time and across key risk groups.
...
Method: Correlation analysis between CNN architectures and human behavioral/fMRI data, plus Object2Brain framework combining GradCAM and object detection at CNN-filter level.

Result: CNNs struggle to go beyond simple visual processing and don't reflect higher-order brain processing for social cognition tasks, though different architectures show varying object class sensitivities.

Conclusion: CNN-brain correspondences are limited for complex social cognition tasks, with CNNs primarily capturing basic visual processing rather than higher-order cognitive functions.
...
<details>
  <summary>Details</summary>
Motivation: Address systemic misalignment between CBMs and expert reasoning when models learn shortcuts from biased data, going beyond simple concept interventions.
...
Method: Developed a deep learning model trained on shipwreck data from Great Lakes and Irish coasts, enhanced with synthetic data generation, integrated into QGIS for automated preprocessing, inference, and shipwreck detection.
...
<details>
  <summary>Details</summary>
Motivation: To understand the impact of neural network pruning on model interpretability, as prior works focused on performance preservation but overlooked interpretability effects.
...
Result: Light-to-moderate pruning improved saliency-map focus and faithfulness while retaining meaningful concepts. Aggressive pruning merged features, reducing sparsity and concept coherence despite maintaining accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Most existing approaches rely on point cloud input, which limits practical applications since consumer cameras typically lack depth sensors and visual-only data is more common. There's a need for methods that can perform holistic scene understanding from multi-view images alone.
...
Conclusion: TUN3D sets a new benchmark in holistic indoor scene understanding by enabling joint layout estimation and 3D object detection from multi-view images without requiring depth sensors or ground-truth camera poses.
...
Method: Segments images into key regions (processed with image-oriented semantic encoder) and non-key regions (compressed via image-to-text modeling), with lightweight deployment using model quantization and low-rank adaptation fine-tuning.

Result: Outperforms traditional methods in both semantic fidelity and visual quality for image transmission tasks.

Conclusion: The proposed system effectively enhances semantic communication by refining semantic granularity and optimizing resource utilization without sacrificing performance.
...
<details>
  <summary>Details</summary>
Motivation: To provide comprehensive datasets that enable research and development in mmWave Integrated Sensing and Communication systems for various human sensing applications.

Method: Created labeled mmWave datasets using a testbed with specific experimental settings and signal features, and demonstrated utility through validation on downstream tasks using parameter-efficient fine-tuning.

Result: Successfully developed mmHSense datasets that support multiple human sensing applications and showed that parameter-efficient fine-tuning can adapt ISAC models to different tasks while reducing computational complexity.

Conclusion: mmHSense provides valuable open datasets for advancing mmWave ISAC research, and parameter-efficient fine-tuning offers an effective approach for adapting models across different sensing tasks with reduced computational requirements.
...
<details>
  <summary>Details</summary>
Motivation: The medial axis (Hamilton-Jacobi skeleton) is sensitive to noise, where minor boundary variations cause disproportionate skeletal expansions. Classical pruning methods help but lack formal scale-space properties.

Method: Proposes skeletonisation scale-spaces that leverage sparsification of the medial axis, providing both continuous and discrete formulations with densification for inverse progression from coarse to fine scales.

Result: The framework satisfies key scale-space properties (hierarchical architecture, controllable simplification, geometric equivariance) and enables overcomplete shape representations beyond original skeletons.

Conclusion: The approach effectively addresses noise sensitivity in skeletonisation and demonstrates practical utility for robust skeletonisation, shape compression, and stiffness enhancement in additive manufacturing.
...
<details>
  <summary>Details</summary>
Motivation: High-resolution climate projections are needed for local decision-making, but current projections have low spatial resolution (12.5 km) that limits their usability.
...
Conclusion: Single-image super-resolution models are effective for statistically downscaling climate projections to higher resolution (1 km) when high-resolution training data is unavailable.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models are vulnerable to image-based jailbreaking attacks that can bypass safety alignments, creating security risks that need to be addressed.

Method: Proposes JaiLIP - a jailbreaking attack that minimizes a joint objective combining MSE loss between clean/adversarial images and the model's harmful-output loss to create imperceptible adversarial images.

Result: JaiLIP generates highly effective and imperceptible adversarial images that outperform existing methods in producing toxic outputs, as measured by Perspective API and Detoxify metrics. Also demonstrated effectiveness in transportation domain.
...
<details>
  <summary>Details</summary>
Motivation: To quantify the gap between automated species identification systems and human expert performance, and compare AI uncertainty with expert disagreement in biological identification tasks.

Method: Organized a challenge (ExpertCLEF) evaluating 19 deep-learning systems from 4 research teams against 9 expert botanists specializing in French flora, using visual observations with inherent uncertainty.
...
Conclusion: Automated species identification using deep learning has reached near-human expert performance, demonstrating significant advancement in the field of automated biological classification.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods generate quad meshes by first creating triangle meshes and then merging them, which produces poor topology quality. There's a need for native quad-mesh generation that directly creates quadrilateral meshes with better topology.

Method: QuadGPT uses an autoregressive framework with two key innovations: 1) unified tokenization method to handle mixed triangle and quadrilateral topologies, 2) specialized Reinforcement Learning fine-tuning method called tDPO for improved generation quality.

Result: Extensive experiments show QuadGPT significantly surpasses previous triangle-to-quad conversion pipelines in both geometric accuracy and topological quality.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models reproduce copyrighted content, but existing concept erasure methods don't scale well for multiple conflicting concepts and use rigid static approaches that degrade performance.
...
Conclusion: DyME provides an effective on-demand erasure framework that enables flexible multi-concept removal while maintaining image quality, addressing key limitations of static erasure approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing metrics like BLEU and ROUGE fail to capture human judgment nuances for video understanding, while manual evaluation is costly. LLM/MLLM evaluators for video tasks remain underexplored.

Method: Train VideoJudge using generator-evaluator interplay: generator produces responses conditioned on target ratings, and mismatched responses are discarded. Create 3B and 7B MLLM models specialized for video evaluation.

Result: VideoJudge-7B outperforms larger MLLM baselines (Qwen2.5-VL 32B/72B) on 3 out of 4 meta-evaluation benchmarks. LLM judges perform worse than MLLM judges, and chain-of-thought reasoning doesn't help, confirming video inputs are essential.

Conclusion: VideoJudge demonstrates that specialized MLLM judges can effectively evaluate video understanding models, with video inputs being crucial for accurate assessment rather than text-only LLMs or complex reasoning chains.
...
<details>
  <summary>Details</summary>
Motivation: To enable scalable multi-agent collaborative perception by overcoming communication bandwidth constraints that limit information sharing between connected agents like autonomous vehicles and robots.

Method: End-to-end method using a simple bottleneck network followed by multi-stage residual vector quantization (RVQ) to compress feature dimensions, transmitting only per-pixel code indices instead of full features.

Result: Achieves 273x compression at 30 bpp to 1365x compression at 6 bpp on DAIR-V2X dataset. At 18 bpp (455x), matches or outperforms raw-feature CP, and enables ultra-low-bandwidth operation with graceful degradation at 6-12 bpp.

Conclusion: ReVQom enables efficient and accurate multi-agent collaborative perception, representing a step toward practical V2X deployment by significantly reducing communication bandwidth requirements.
...
<details>
  <summary>Details</summary>
Motivation: To investigate how contemporary Text-to-Image AI models perpetuate gender stereotypes and cultural inaccuracies when generating depictions of professionals in Saudi Arabia.

Method: Analyzed 1,006 images from ImageFX, DALL-E V3, and Grok for 56 Saudi professions using neutral prompts. Two Saudi annotators evaluated images on five dimensions (gender, clothing, background, activities, age) with a third researcher adjudicating disagreements, resulting in 10,100 judgments.

Result: Strong gender imbalance: ImageFX 85% male, Grok 86.6% male, DALL-E V3 96% male. Cultural inaccuracies in clothing, settings, and activities were frequent. Counter-stereotypical images often resulted from cultural misinterpretations rather than progressive portrayals.

Conclusion: Current models mirror societal biases in training data and offer limited reflection of Saudi labor market's gender dynamics and cultural nuances. Need for more diverse training data, fairer algorithms, and culturally sensitive evaluation frameworks.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches train separate small models for each type of inappropriate content, requiring extensive human-labeled data and lacking cross-issue generalization capabilities.

Method: Three targeted pretraining tasks: Caption to enhance video detail perception, Visual Question Answering to understand issue definitions and guidelines, and Chain-of-Thought to improve reasoning capability.

Result: The pretraining approach significantly improves MLLM performance in both zero-shot and supervised fine-tuning settings, with strong generalization to emergent, previously unseen issues.

Conclusion: The proposed reasoning-enhanced MLLM pretraining paradigm effectively addresses the challenges of inappropriate content detection in short videos and demonstrates superior generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with accurate numeric coordinate prediction for GUI grounding due to high-resolution images and complex layouts, leading to poor performance.

Method: Proposes GUI-Cursor that treats GUI grounding as interactive search - model generates actions to move cursor toward target UI elements using spatial relations and movement history, trained with multi-step online RL and dense trajectory-based rewards.

Result: Achieves SOTA results: ScreenSpot-v2 (88.8% → 93.9%) and ScreenSpot-Pro (26.8% → 56.5%). Solves 95% of instances within 2 steps and adapts step count based on difficulty.

Conclusion: Interactive search approach with cursor movement significantly improves GUI grounding accuracy over direct coordinate prediction, with efficient step usage and adaptability to varying task difficulties.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-video retrieval systems have two main limitations: they are vulnerable to low-quality text-video data pairs which are hard to identify, and cosine similarity alone provides no explanation for ranking results, limiting interpretability.

Method: Proposes X-CoT framework that uses LLM Chain-of-Thought reasoning instead of embedding models. Expands benchmarks with additional video annotations, and designs a retrieval CoT with pairwise comparison steps that generates detailed reasoning and complete ranking.
...
Conclusion: The X-CoT framework successfully addresses interpretability issues in text-to-video retrieval by providing explainable rankings through CoT reasoning, while also enhancing retrieval performance and enabling better model and data analysis.
...
<details>
  <summary>Details</summary>
Motivation: Manual inspection of surgical instruments is error-prone, and existing automated defect detection methods trained on natural/industrial images fail to transfer effectively to the surgical domain, leading to false positives, poor sensitivity to small defects, and inadequate feature capture due to domain shift.
...
Conclusion: The proposed approach provides an effective solution for automated defect detection in surgical instruments by specifically addressing domain-specific challenges through targeted adaptations.
...
<details>
  <summary>Details</summary>
Motivation: Current alignment-based approaches require computationally expensive pretrained encoders for representation alignment, which is inefficient and costly.

Method: Introduces Linear SEParability (LSEP) regularization that promotes linear separability of intermediate layer representations, eliminating the need for auxiliary encoders and incorporating linear probing directly into learning dynamics.
...
Conclusion: LSEP provides an effective alternative to encoder-based alignment methods, improving representation quality while being more computationally efficient.
...
Method: Integrates semivariograms - a geostatistical tool - into contrastive learning to model how spatial correlation changes with distance, relating feature space distance to geographical distance to capture expected visual content in spatial correlation.

Result: Evaluation on OSV5M dataset shows improved geo-localization performance, especially at finer granularity, by explicitly modeling spatial priors.

Conclusion: Explicitly incorporating spatial dependencies through semivariogram-based regularization enhances contrastive learning for image-based geo-localization, effectively addressing false negatives and hard negatives.
...
<details>
  <summary>Details</summary>
Motivation: To build digital human agents capable of infinite multimodal interactions from a single portrait, enabling persistent and intelligent audiovisual interactions in real-time video calls.

Method: Uses a Thinker-Actor dual-transformer architecture: Thinker module (pretrained large language-speech model) for perception and reasoning, Actor module (chunk-wise autoregressive diffusion model) for synchronized multimodal generation with cross-attention to Thinker's hidden states. Includes inter/intra-chunk attentions with time-aligned multimodal positional embeddings, chunk-wise diffusion forcing, and global identity referencing.

Result: X-Streamer runs in real-time on two A100 GPUs, sustaining hours-long consistent video chat experiences from arbitrary portraits with synchronized multimodal responses.

Conclusion: The framework paves the way toward unified world modeling of interactive digital humans by enabling persistent, intelligent multimodal interactions from static portraits.
...
<details>
  <summary>Details</summary>
Motivation: Current video generators struggle with motion forecasting from single images even in simple physical scenarios, as they focus on pixel generation rather than directly modeling motion dynamics.
...
Result: The approach captures scene-wide dynamics and uncertainty, yielding more accurate and diverse predictions than prior regressors and generators, and shows effectiveness on robotics applications and real-world physics datasets.

Conclusion: Directly modeling motion trajectories is more effective for motion forecasting than pixel-based video generation, as it avoids the overhead of pixel generation and better captures physical dynamics.
...
<details>
  <summary>Details</summary>
Motivation: To understand the fundamental architectural trade-offs between spatial feature extraction (DINOv3) and temporal modeling (V-JEPA2) approaches for video action recognition, providing empirical guidance for method selection.

Method: Evaluated both architectures on UCF Sports dataset using multiple metrics: classification accuracy, clustering performance (Silhouette score), intra-class consistency, and inter-class discrimination. Conducted action-specific analysis to examine performance across different action types.

Result: DINOv3 achieved superior clustering (Silhouette: 0.31 vs 0.21) and exceptional discrimination (6.16x separation ratio), especially for pose-identifiable actions. V-JEPA2 showed consistent reliability with lower variance (0.094 vs 0.288) across all action types. DINOv3 excels at static pose recognition but degrades on motion-dependent actions, while V-JEPA2 provides balanced performance.

Conclusion: Architectural choices create distinct trade-offs: spatial processing excels for pose-based recognition but lacks motion robustness, while temporal modeling offers balanced reliability. Selection should consider task requirements and reliability constraints.
...
<details>
  <summary>Details</summary>
Motivation: Traditional damage assessment methods are slow and dangerous, while current computer vision approaches only provide limited outputs like classification labels or segmentation masks, lacking comprehensive situational understanding.
...
<details>
  <summary>Details</summary>
Motivation: To understand which aspects of vision model representations are shared across different architectures and training methods versus which aspects reflect distinctive computational strategies.

Method: Used multiple representational similarity metrics (RSA, Soft Matching, Linear Predictivity) capturing different facets of representations, then integrated them using Similarity Network Fusion (SNF) inspired by multi-omics integration.

Result: Geometry and tuning metrics strongly discriminate between model families, while linear decodability shows weaker separation. SNF achieved substantially sharper family separation than individual metrics. Clustering revealed supervised ResNets and ViTs form distinct clusters, while self-supervised models group together across architectures, and hybrid architectures cluster with masked autoencoders.

Conclusion: The framework provides a principled typology showing that emergent computational strategies, shaped jointly by architecture and training objective, define representational structure beyond surface design categories.
...
Method: Introduces a framework with cross-branch supervision where geometry cues guide video generation and video priors regularize 3D prediction, enabling joint modeling of video latents and implicit 3D field in a single forward pass.
...
<details>
  <summary>Details</summary>
Motivation: To create a flexible foundation model that can handle the heterogeneous and multimodal nature of scientific observations in PDEs, enabling scalable and data-efficient scientific machine learning.
...
Conclusion: MORPH presents a powerful backbone for learning from heterogeneous scientific data, charting a path toward scalable and data-efficient scientific machine learning.
...
<details>
  <summary>Details</summary>
Motivation: Infrared imaging is effective for low-light urban detection but faces challenges like class imbalance, thermal noise, and computational constraints that hinder practical deployment.

Method: Selected YOLOv8 as baseline, replaced CSPDarknet with MobileNetV4 for efficiency, and introduced SlideLoss to dynamically emphasize underrepresented and occluded samples.

Result: MS-YOLO reduces computational overhead by 1.5% while sustaining high accuracy, achieves competitive mAP and superior precision with only 6.7 GFLOPs on FLIR ADAS V2 benchmark.
...
<details>
  <summary>Details</summary>
Motivation: Existing DETR-based MOT frameworks process detection and tracking queries jointly in single Transformer layers, causing conflicts and degraded association accuracy.
...
Result: Achieves 71.3 HOTA on DanceTrack (9+ point improvement), 72.2 HOTA on SportsMOT, and 54.7 mTETA/41.6 mHOTA on BDD100k without external data - all state-of-the-art results.
...
Method: Proposes DeLiVR with two components: 1) rotation-bounded Lie relative bias for geometry-consistent alignment using in-plane angle prediction, and 2) differential group displacement that computes angular differences between frames with temporal decay and attention masks.

Result: Extensive experiments show effectiveness on publicly available benchmarks.

Conclusion: Lie groups provide a principled approach for enforcing spatial and temporal consistency in video modeling, enabling efficient video deraining with improved performance.
...
Method: The method involves: 1) Testing current visual foundational models (DINOv2, DINOv3, PE-Core) on SAR data, 2) Self-supervised finetuning of SSL models with SAR data to create AFRL-DINOv2 models, 3) Analyzing performance trade-offs with different backbones and downstream adaptation recipes.

Result: The results show that self-supervised finetuning with SAR data creates state-of-the-art SAR foundation models (AFRL-DINOv2) that significantly outperform the current best SAR-domain model SARATR-X. The models demonstrate improved performance in challenging downstream environments.

Conclusion: While the work shows positive results and sets new state-of-the-art performance, the authors conclude that there is still a long way to go in developing SAR foundation models, hoping this work will inspire future research in this direction.
...
<details>
  <summary>Details</summary>
Motivation: Existing UI testing methods rely on physical devices or static screenshot analysis, which limit scalability and hinder development of intelligent UI agents.
...
Conclusion: UISim enables realistic UI simulation for testing, prototyping, synthetic data generation, and AI agent training, streamlining UI development.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in retinal vessel segmentation including small vessel segmentation difficulties and high computational costs, especially for real-world clinical environments with limited resources.
...
Conclusion: LFA-Net provides an efficient and lightweight solution for retinal vessel segmentation that is ideal for resource-constrained clinical environments while maintaining high segmentation accuracy.
...
Conclusion: The proposed framework effectively incorporates both visual scene context and label-guided semantic information to improve group-level emotion recognition performance.
...
<details>
  <summary>Details</summary>
Motivation: Direct application of SAM to medical imaging faces challenges like ambiguous boundaries, lack of anatomical relationship modeling, and absence of uncertainty quantification, which are critical for clinical use.

Method: KG-SAM integrates three components: (1) medical knowledge graph for anatomical relationships, (2) energy-based CRF for anatomically consistent predictions, and (3) uncertainty-aware fusion module for reliability.

Result: Achieved 82.69% Dice score on prostate segmentation and substantial gains in abdominal segmentation (78.05% on MRI, 79.68% on CT) across multi-center datasets.

Conclusion: KG-SAM establishes itself as a robust and generalizable framework that advances medical image segmentation by addressing key limitations of SAM in clinical settings.
...
TL;DR: UniVid is a framework that fine-tunes a pre-trained video diffusion transformer to handle diverse vision tasks without task-specific modifications, using visual sentences to represent tasks and enabling cross-modal and cross-source generalization.


<details>
  <summary>Details</summary>
Motivation: To explore whether pre-trained video generation models can adapt to diverse image and video tasks in a unified way, avoiding costly task-specific pre-training across modalities and sources.
...
Result: UniVid generalizes well to cross-modal inference (images and videos) and cross-source tasks (natural to annotated data) despite being trained only on natural video data.

Conclusion: Pre-trained video generation models have the potential to serve as scalable and unified foundations for vision modeling, enabling flexible task switching and broad generalization.
...
TL;DR: A token merging method that maintains spatial integrity for ViT backbones with spatial architectures, achieving speedups with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Existing token reduction methods fail to preserve spatial structure required by modern ViT architectures with window attention, decomposed relative positional embeddings, and RoPE.
...
Result: 1.25x speedup on SAM-H with only 0.7% mIOU drop on COCO, and 1.15x speedup on DeiT-B with no top-1 accuracy drop on ImageNet after one epoch of fine-tuning.

Conclusion: The method enables seamless compatibility with spatial architectures while maintaining strong performance across various vision tasks.
...
TL;DR: GASP-ICL is a training-free framework for multimodal deepfake detection that uses guided adaptive scoring and propagation in-context learning to enhance large vision-language models without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models have strong multimodal reasoning but struggle with detecting subtle forgery cues, resolving cross-modal inconsistencies, and performing task-aligned retrieval in multimodal deepfake detection.

Method: Proposes GASP-ICL framework with MDD-adapted feature extractor for aligned image-text pair retrieval, and Graph-Structured Taylor Adaptive Scorer (GSTAS) to capture cross-sample relations and propagate query-aligned signals for discriminative exemplar selection.
...
Conclusion: GASP-ICL effectively enhances large vision-language models for robust multimodal deepfake detection through semantic alignment and task-relevant demonstration selection in a training-free manner.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use unified features for all actions in videos, making it challenging to model interactions between different objects in multi-action scenarios.

Method: Uses Spatio-temporal Scene Graphs (SSGs) with Dynamic Prompt Module (DPM) to guide Graph Parsing Neural Network (GPNN) in generating action-specific representations with dynamic weight aggregation.
...
Result: Created a multimodal dataset for hate identification and developed DeHater vision-language model for dehatification tasks, setting new standards in AI-driven hate detection.
...
Result: Achieves SOTA performance with 64.82% on cross-image reasoning tasks, exceeding previous best method by 1%. Demonstrates effectiveness in multi-image grounding benchmarks.

Conclusion: MIRG-RL successfully addresses cross-image reasoning challenges through its unified framework and training paradigm, setting new benchmarks for multi-image understanding in LVLMs.
...
Method: Hybrid framework combining intra-chunk diffusion denoising with inter-chunk autoregressive causal generation, featuring action-guided variable-length chunking that partitions video based on robotic action semantics, and a Context-aware Mixture-of-Experts framework that adaptively activates specialized experts for each chunk.

Result: Extensive experiments demonstrate the method achieves stable and consistent long-horizon generation over extended rollouts, overcoming the limitations of existing approaches.

Conclusion: LongScape successfully addresses the challenge of stable long-horizon video generation through its hybrid approach and action-guided chunking mechanism, enabling high-quality embodied manipulation data generation with both visual fidelity and temporal consistency.
...
<details>
  <summary>Details</summary>
Motivation: Existing world models have limitations - pixel-based models introduce visual redundancies that hinder action decoding, while latent models overlook fine-grained details needed for precise manipulation.

Method: Proposes MoWM framework that uses motion-aware representations from latent model as high-level prior to guide extraction of fine-grained visual features from pixel space model, highlighting informative visual details for action decoding.
...
Conclusion: The hybrid approach effectively combines the strengths of different world model representations for embodied action planning, providing valuable insights for future research.
...
TL;DR: DiTraj is a training-free framework for trajectory control in text-to-video generation using Diffusion Transformers (DiT), featuring foreground-background separation guidance and inter-frame Spatial-Temporal Decoupled 3D-RoPE to enhance trajectory controllability.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory control methods either require substantial training resources or are designed for U-Net, failing to leverage DiT's superior performance. There's a need for efficient trajectory control specifically tailored for DiT-based video generation models.

Method: Proposes DiTraj with two key components: 1) Foreground-background separation guidance using LLM to convert prompts, 2) Inter-frame Spatial-Temporal Decoupled 3D-RoPE that modifies foreground tokens' position embedding to eliminate cross-frame spatial discrepancies and strengthen cross-frame attention.

Result: Extensive experiments show the method outperforms previous approaches in both video quality and trajectory controllability, achieving effective trajectory control without requiring additional training.

Conclusion: DiTraj provides a simple yet effective training-free solution for trajectory control in DiT-based video generation, successfully addressing the limitations of existing methods while maintaining high video quality.
...
TL;DR: Hybrid retrieval method combining dense embeddings with lexical features significantly outperforms baseline methods for multi-hop QA, achieving 50% improvement in exact match and 47% in F1 score on HotpotQA dataset.


<details>
  <summary>Details</summary>
Motivation: Multi-hop reasoning requiring evidence combination across multiple passages remains challenging for transformer-based QA systems, necessitating better retrieval strategies.

Method: Comprehensive evaluation of retrieval strategies including cosine similarity, maximal marginal relevance, and a hybrid method integrating dense embeddings with lexical overlap and re-ranking. Adapted EfficientRAG pipeline with token labeling and iterative refinement.
...
Conclusion: Hybrid retrieval-augmented generation provides a practical zero-shot solution for multi-hop question answering, balancing accuracy, efficiency, and interpretability, though limitations remain in handling distractors and temporal reasoning.
...
TL;DR: HDR-4DGS enables High Dynamic Range Dynamic Novel View Synthesis for scenes with temporal variations by using Gaussian Splatting with dynamic tone-mapping to maintain radiance coherence across time.


<details>
  <summary>Details</summary>
Motivation: Current HDR NVS methods assume static scenes, but real-world scenarios often contain dynamic elements like moving objects and changing lighting, creating a more challenging problem that needs joint modeling of temporal radiance variations.

Method: Proposed HDR-4DGS architecture based on Gaussian Splatting with a dynamic tone-mapping module that explicitly connects HDR and LDR domains, adapting tone-mapping functions according to evolving radiance distributions across time.

Result: HDR-4DGS achieves both temporal radiance consistency and spatially accurate color translation, enabling photorealistic HDR renderings from arbitrary viewpoints and time instances, outperforming state-of-the-art methods.

Conclusion: The proposed HDR DNVS problem and HDR-4DGS solution effectively address the challenge of dynamic scenes in HDR novel view synthesis, demonstrating superior performance in both quantitative metrics and visual quality.
...
TL;DR: SRHand is a method that reconstructs detailed 3D hand geometry and textures from low-resolution images by combining implicit image representation with explicit hand meshes, achieving superior results compared to existing approaches.


<details>
  <summary>Details</summary>
Motivation: Prior methods for hand reconstruction rely on high-resolution multi-view images and fail on low-resolution inputs. Existing super-resolution methods work only for static objects with fixed resolutions, not for articulated deformable hands.

Method: Proposes SRHand with geometric-aware implicit image function (GIIF) that learns detailed hand priors by upsampling coarse input images. Jointly optimizes implicit image function and explicit 3D hand shapes to maintain multi-view and pose consistency.

Result: Significantly outperforms state-of-the-art image upsampling methods adapted to hand datasets and 3D hand reconstruction methods on InterHand2.6M and Goliath datasets, achieving fine-detailed reconstruction (wrinkles, nails) both quantitatively and qualitatively.

Conclusion: SRHand successfully reconstructs detailed 3D hand geometry and textures from low-resolution images by leveraging implicit image representation with explicit hand meshes, preserving multi-view consistency and achieving fine details.
...
TL;DR: The paper argues that current fake image detection research focuses too much on generative algorithms while neglecting proper definition and data collection of "real" images, suggesting we need to rethink what constitutes a real image in the smartphone era.


<details>
  <summary>Details</summary>
Motivation: To highlight the shortcomings in current fake image detection research, which relies on outdated definitions of "real" images and old datasets like ImageNet, while modern smartphone photography uses neural network-based image formation algorithms similar to fake image generators.

Method: Position paper analyzing the current state of fake image detection research, examining how "real" images are defined and collected, and comparing modern smartphone image formation processes with fake image generation techniques.

Result: The analysis reveals that current fake detection methods rely on outdated low-resolution datasets and fail to account for modern smartphone photography where images are computationally generated from multiple sensor inputs using neural networks.

Conclusion: The detection of fake images may not be a sound objective without clear technical definitions of "real" images and new benchmark datasets that reflect modern image acquisition technologies.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs can capture high-level aesthetic features but struggle with accurate aesthetic judgments due to scarcity of multimodal aesthetic reasoning data and the subjective nature of aesthetic judgment.

Method: Proposes Aes-R1 framework with AesCoT pipeline for constructing high-quality chain-of-thought aesthetic reasoning data, and Relative-Absolute Policy Optimization (RAPO) RL algorithm that jointly optimizes absolute score regression and relative ranking order.

Result: Improves backbone's average PLCC/SRCC by 47.9%/34.8%, surpassing state-of-the-art baselines of similar size. Shows robust generalization under limited supervision and out-of-distribution scenarios.
...
TL;DR: StableDub is a novel visual dubbing framework that addresses two key limitations: speaker-specific lip habit modeling and occlusion robustness, using Stable-Diffusion backbone with lip-habit-modulated mechanism and occlusion-aware training.


<details>
  <summary>Details</summary>
Motivation: Current visual dubbing methods have two critical deficiencies: audio-only driving fails to capture speaker-specific lip habits, and conventional blind-inpainting produces visual artifacts when handling obstructions like microphones or hands.

Method: Built on Stable-Diffusion backbone with lip-habit-modulated mechanism for joint phonemic audio-visual synchronization and speaker-specific orofacial dynamics. Uses occlusion-aware training by explicitly exposing occlusion objects to inpainting, and hybrid Mamba-Transformer architecture for efficiency.

Result: Achieves superior performance in lip habit resemblance and occlusion robustness. Surpasses other methods in audio-lip sync, video quality, and resolution consistency. Eliminates need for cost-intensive priors and shows superior training efficiency.

Conclusion: StableDub expands the applicability of visual dubbing methods by addressing key limitations in lip habit modeling and occlusion handling, enabling more practical deployment with improved efficiency and quality.
...
<details>
  <summary>Details</summary>
Motivation: To enable interactive control of object motion within text-driven 3D scene generation, allowing users to define 3D trajectories for objects and seamlessly integrate them into generated 3D environments.

Method: Three-stage pipeline: 1) Enhanced text-to-3D background generation using 2D Gaussian Splatting with panoramic images, 2) 3D copy-and-paste approach with physics-aware object positioning, 3) Temporal animation using part-augmented motion-conditioned video diffusion model for view-consistent motion.

Result: The framework successfully generates harmonized 3D scenes with user-controlled object motion, demonstrating effective spatial and temporal alignment of animated objects within high-quality 3D backgrounds.

Conclusion: Drag4D provides a unified architecture for interactive 3D scene generation with object motion control, achieving high-quality results with precise spatial alignment and view-consistent temporal animation.
...
Method: Builds on pre-trained video backbone with two key components: (1) Motion-aware Loss that emphasizes learning at high-motion regions, and (2) Audio Sync Guidance that uses a visually aligned off-sync model without audio layers to better exploit audio cues during inference while maintaining visual quality.
...
Conclusion: Syncphony successfully addresses the synchronization challenge in audio-to-video generation through motion-aware training and audio guidance techniques, achieving superior performance in temporal alignment and visual quality compared to existing approaches.
...
TL;DR: A novel Language-Guided Change Detection model (LG-CD) that uses natural language prompts to improve remote sensing change detection accuracy by integrating visual and textual information through multi-modal fusion.


<details>
  <summary>Details</summary>
Motivation: Most deep learning-based remote sensing change detection methods focus only on unimodal visual information and neglect the rich semantic information from multimodal data like text, limiting their accuracy and robustness.

Method: LG-CD uses SAM2 visual foundational model for multi-scale feature extraction, multi-layer adapters for fine-tuning, Text Fusion Attention Module (TFAM) for visual-textual alignment, and Vision-Semantic Fusion Decoder (V-SFD) with cross-attention for final change detection masks.

Result: Experiments on LEVIR-CD, WHU-CD, and SYSU-CD datasets show LG-CD consistently outperforms state-of-the-art change detection methods.

Conclusion: The approach demonstrates the effectiveness of leveraging multimodal information for generalized change detection and provides new insights for the field.
...
Method: Train CNN-based representation extractor via contrastive learning on unannotated data, then sideload it to frozen YOLO11n backbone. Compare various fusion methods and granularity.

Result: Improves detection performance by 3.8% to 9.5% in mAP50 on NVD dataset.

Conclusion: The proposed sideload-CL-adaptation framework effectively leverages unannotated data to enhance vehicle detection performance in challenging Nordic conditions.
...
TL;DR: IF-V2V is an inversion-free video editing method that adapts flow-matching-based I2V models for video editing without computational overhead, using vector field rectification and structure-preserving initialization.


<details>
  <summary>Details</summary>
Motivation: Existing video editing methods require inversion with model-specific design or extensive optimization, limiting their ability to leverage up-to-date I2V models for transferring image editing capabilities to video.

Method: Proposes Vector Field Rectification with Sample Deviation to incorporate source video information, Structure-and-Motion-Preserving Initialization for motion-aware noise generation, and Deviation Caching to minimize computational cost.

Result: Achieves superior editing quality and consistency over existing approaches, offering a lightweight plug-and-play solution for video editing.

Conclusion: IF-V2V provides an effective inversion-free method for video editing that leverages existing I2V models while maintaining computational efficiency and editing quality.
...
<details>
  <summary>Details</summary>
Motivation: Most multi-view counting methods rely on fully supervised learning requiring large annotated datasets, which is expensive and time-consuming to obtain.

Method: Uses self-supervised learning with neural volumetric rendering to learn implicit scene representations, reconstructing continuous geometry and view-dependent appearance through differential neural rendering.
...
Conclusion: SSLCounter provides an effective SSL framework for MVC that reduces annotation dependency while maintaining high performance, with flexible integration into existing frameworks.
...
TL;DR: A benchmark for object-centric spatial reasoning in vision foundation models reveals a trade-off: detectors excel at precise localization but lack relational reasoning, while VLMs provide coarse layouts but struggle with fine-grained spatial context.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks focus on localization accuracy rather than true spatial understanding, which requires reasoning about object arrangements, relative positions, and depth within scenes.
...
Result: Detectors (GroundingDINO, OWLv2) deliver precise bounding boxes but limited relational reasoning; VLMs (SmolVLM, GPT-4o) provide coarse layout cues and fluent captions but struggle with fine-grained spatial context.

Conclusion: There's a significant gap between localization and true spatial understanding, highlighting the need for spatially-aware foundation models.
...
Method: PAtch-based k-Nearest neighbor visual In-Context Learning (PANICL) leverages multiple in-context pairs and smooths assignment scores across pairs without requiring additional training.

Result: PANICL demonstrates consistent improvements across various tasks including foreground segmentation, object detection, colorization, multi-object segmentation, and keypoint detection. It shows strong robustness to domain shifts and generalizes well to other VICL models.

Conclusion: PANICL is a versatile and broadly applicable framework that effectively mitigates bias in visual in-context learning through multi-patch neighbor smoothing.
...
TL;DR: SingRef6D is a lightweight 6D pose estimation pipeline that uses only a single RGB reference image, eliminating the need for depth sensors or multi-view setups. It enhances depth prediction for challenging surfaces and integrates depth-aware matching to handle difficult materials and lighting conditions.
...
Method: Uses token-scaler-based fine-tuning on Depth-Anything v2 for improved depth prediction, and introduces depth-aware matching within LoFTR to integrate spatial relationships for robust matching in challenging conditions.

Result: Achieves 14.41% improvement in depth prediction on REAL275 and 6.1% improvement in average recall for pose estimation across REAL275, ClearPose, and Toyota-Light datasets compared to state-of-the-art methods.

Conclusion: SingRef6D provides a robust, resource-efficient solution for 6D pose estimation that works well with challenging materials and lighting conditions using only single RGB images.
...
TL;DR: DynaNav is a dynamic visual navigation framework that adapts feature and layer selection based on scene complexity, achieving significant efficiency improvements over ViNT while maintaining or improving navigation performance.
...
Method: Proposes DynaNav with trainable hard feature selector for sparse operations and integrates feature selection into early-exit mechanism using Bayesian Optimization for optimal exit thresholds.
...
Conclusion: DynaNav effectively addresses computational efficiency and interpretability issues in visual navigation, making it suitable for resource-tight scenarios.
...
TL;DR: SemanticControl is a training-free method that enables text-to-image diffusion models to effectively use loosely aligned visual conditions by adaptively suppressing conflicting visual guidance and strengthening text guidance using attention masks from surrogate prompts.


<details>
  <summary>Details</summary>
Motivation: ControlNet's effectiveness depends on precisely aligned visual conditions, which are often unavailable for uncommon or imaginative scenes. Loosely aligned conditions (e.g., human poses for animal actions) are more available but existing methods struggle with them, causing low text fidelity or artifacts.

Method: Run auxiliary denoising with surrogate prompt aligned with visual condition to extract attention masks, then use these masks during actual target prompt denoising to suppress conflicting visual guidance and strengthen text guidance.

Result: Improves performance under loosely aligned conditions across various condition types (depth maps, edge maps, human skeletons), outperforming existing baselines.

Conclusion: SemanticControl effectively addresses the limitation of requiring precisely aligned visual conditions in ControlNet, enabling better use of semantically relevant but misaligned visual guides.
...
TL;DR: Proposes an Emotion Statement Judgment task and automated pipeline to evaluate Multimodal Large Language Models' emotion perception capabilities, revealing performance gaps compared to humans.


<details>
  <summary>Details</summary>
Motivation: Address inconsistencies in evaluating MLLMs' emotion perception from images, overcoming limitations of existing methods like oversight of plausible responses, limited emotional taxonomies, and labor-intensive annotations.

Method: Develops an Emotion Statement Judgment task and an automated pipeline to construct emotion-centric statements with minimal human effort, enabling systematic evaluation of MLLMs.

Result: MLLMs show stronger performance in emotion interpretation and context-based judgment but have limitations in comprehending perception subjectivity. Top models like GPT4o still show significant performance gaps compared to humans.

Conclusion: The work provides a fundamental evaluation framework that contributes to advancing emotional intelligence in MLLMs by identifying key areas for future improvement.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from attribute leakage that compromises subject fidelity and fail to align with nuanced human preferences due to reliance on simple reconstruction-based objectives.

Method: Introduces explicit positional supervision to separate attention regions, employs Mixture-of-Experts architecture for scenario planning, and designs an online reinforcement learning framework with scoring mechanism for preference alignment.
...
Conclusion: MultiCrafter effectively addresses key challenges in multi-subject image generation through attention separation, enhanced model capacity, and preference alignment.
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations of existing open-world part segmentation methods that transfer 2D supervision and fail to capture intrinsic 3D geometry, leading to surface-only understanding and limited generalization.

Method: Uses a triplane-based dual-branch encoder in an encoder-decoder architecture for scalable part-aware representation learning, trained on over 5 million 3D shape-part pairs curated through a model-in-the-loop annotation pipeline.

Result: Achieves highly accurate part identification with single prompts and automatically decomposes shapes into both surface and internal structures in Segment-Every-Part mode, outperforming state-of-the-art methods by large margins.

Conclusion: PartSAM marks a decisive step toward foundation models for 3D part understanding by combining scalable architecture with diverse 3D data, enabling emergent open-world capabilities.
...
TL;DR: Proposed a deep learning framework using customized pre-trained models (EfficientNet B0, ResNet18, MobileNetV2) for blind contrast quality assessment, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Most no-reference image quality assessment models struggle to accurately evaluate contrast distortions under diverse real-world conditions, despite contrast being fundamental to visual perception and image quality.

Method: Customized and fine-tuned three pre-trained architectures (EfficientNet B0, ResNet18, MobileNetV2) with contrast-aware regression heads, trained end-to-end using targeted data augmentations on CID2013 and CCID2014 datasets containing synthetic and authentic contrast distortions.

Result: EfficientNet B0 achieved state-of-the-art performance with PLCC=0.9286/SRCC=0.9178 on CCID2014 and PLCC=0.9581/SRCC=0.9369 on CID2013, surpassing traditional methods and other deep baselines.
...
Method: Proposes Geo-R1 with a "reason first, then act" process: generates explicit, interpretable reasoning chains to decompose referring expressions, then uses these rationales to localize target objects through reinforcement fine-tuning.

Result: Consistently and substantially outperforms supervised fine-tuning baselines on three few-shot geospatial referring benchmarks, demonstrating strong cross-dataset generalization and robustness.

Conclusion: Geo-R1's reasoning-centric approach enables more effective use of limited annotations, enhances generalization, and provides interpretability for geospatial referring tasks.
...
TL;DR: This paper evaluates sycophantic behavior in clinical vision-language models and proposes VIPER, a lightweight mitigation strategy that filters non-evidentiary content to generate evidence-first answers, reducing sycophancy while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Vision language models (VLMs) are increasingly used in clinical workflows but often exhibit sycophantic behavior prioritizing alignment with user phrasing and social cues over evidence-based reasoning, which poses risks in medical decision-making.

Method: Created a medical sycophancy benchmark from PathVQA, SLAKE, and VQA-RAD datasets, using psychologically motivated pressure templates. Proposed VIPER - Visual Information Purification for Evidence-based Response - which filters non-evidentiary content and generates constrained evidence-first answers.

Result: VLMs are generally vulnerable to sycophancy, with significant variations in adversarial responses. Imitation and expert-provided corrections were the most effective triggers, suggesting bias mechanisms independent of visual evidence. VIPER framework reduces sycophancy by an average amount while outperforming baselines.

Conclusion: The benchmark analysis and VIPER mitigation framework lay groundwork for robust deployment of medical VLMs in real-world clinician interactions, emphasizing the need for evidence-anchored defenses against sycophantic behavior.
...
TL;DR: GLARIFY is a novel method that leverages spatiotemporal gaze information to enhance Vision-Language Models' effectiveness in handling ambiguous queries from smart glasses users, addressing challenges of noisy gaze patterns and contextual ambiguity.


<details>
  <summary>Details</summary>
Motivation: With smart glasses becoming popular, users' attention integration into VLMs faces ambiguity challenges: verbal questions become ambiguous using pronouns or skipping context, and human gaze patterns are noisy with complex spatiotemporal relationships. Previous works only use single images, failing to capture dynamic attention.

Method: Analyzed gaze modality samples, used GPT-4o to create automatic data synthesis pipeline generating GLARIFY-Ambi dataset with chain-of-thought process for noisy gaze patterns, and designed heatmap module to incorporate gaze information into VLMs while preserving pretrained knowledge.

Result: GLARIFY significantly outperforms baselines on hold-out test set, demonstrating robust alignment of VLMs with human attention.

Conclusion: GLARIFY paves the way for a usable and intuitive interaction paradigm with visual assistants by robustly aligning VLMs with human attention.
...
TL;DR: LVLMs exhibit spatial bias where identical visual information placed at different image locations produces inconsistent outputs. This stems from unbalanced position embeddings in the language model, not the vision encoder. The proposed BaPA method assigns identical position embeddings to all image tokens, improving spatial robustness without retraining.


<details>
  <summary>Details</summary>
Motivation: To systematically study and address the spatial bias in Large Vision-Language Models, where models produce inconsistent outputs when identical visual information is placed at different spatial locations, revealing limitations in spatial-semantic understanding.

Method: Developed Balanced Position Assignment (BaPA) - a mechanism that assigns identical position embeddings to all image tokens to promote balanced integration of visual information. Used a carefully designed probing dataset to analyze spatial bias and conducted extensive experiments.

Result: BaPA enhances spatial robustness of LVLMs without retraining and further boosts performance across diverse multimodal benchmarks when combined with lightweight fine-tuning. Analysis shows BaPA yields balanced attention and more holistic visual understanding.

Conclusion: Current LVLMs suffer from spatial bias due to unbalanced position embeddings in the language model component. BaPA effectively mitigates this issue by equalizing position embeddings for image tokens, improving both spatial robustness and overall multimodal performance.
...
TL;DR: A novel approach to disentangle visual and semantic features from pre-trained diffusion models, enabling visual correspondence analysis and introducing a new metric (VSM) to quantify visual inconsistencies in subject-driven image generation.


<details>
  <summary>Details</summary>
Motivation: Diffusion model backbones contain both semantic and visual features, but isolating visual features is challenging due to lack of annotated datasets. The paper aims to enable visual correspondence analysis similar to semantic correspondence.

Method: An automated pipeline constructs image pairs with annotated semantic and visual correspondences from existing datasets, and uses a contrastive architecture to separate visual and semantic features.

Result: The proposed VSM metric outperforms CLIP, DINO, and vision-language models in quantifying visual inconsistencies while enabling spatial localization of inconsistent regions in subject-driven generation.
...
TL;DR: WAVE introduces unified audio-visual embeddings from multimodal LLMs that enable any-to-any cross-modal retrieval and prompt-aware embedding generation, achieving state-of-the-art performance on video benchmarks and superior results in audio/video retrieval.


<details>
  <summary>Details</summary>
Motivation: While multimodal LLM embeddings work well as general-purpose representations, their application to dynamic modalities like audio and video remains underexplored, creating a need for unified representations across text, audio, and video.

Method: Uses hierarchical feature fusion strategy and joint multi-modal, multi-task training to create unified embedding space for text, audio, and video, enabling any-to-any cross-modal retrieval and prompt-aware embeddings.

Result: Sets new SOTA on MMEB-v2 video benchmark, achieves superior results in audio and video-to-audio retrieval, and shows remarkable performance in multimodal QA, significantly outperforming existing embedding models.

Conclusion: WAVE opens up broad possibilities for cross-modal, any-to-any applications through its unified representation space and prompt-aware capabilities, with ablation studies validating the joint training approach.
...
TL;DR: ERGO is an efficient vision-language model that uses a two-stage coarse-to-fine reasoning pipeline to reduce computational costs while maintaining accuracy by focusing only on relevant image regions at full resolution.


<details>
  <summary>Details</summary>
Motivation: Existing LVLMs incur substantial computational overhead due to processing large numbers of vision tokens, and current methods often fail after image downsampling due to perception-driven reasoning that requires clear visual information.

Method: Proposes ERGO with a two-stage reasoning pipeline: first analyzes downsampled image to identify task-relevant regions, then crops only these regions at full resolution for processing. Uses reasoning-driven perception with multimodal context and reinforcement learning for coarse-to-fine perception.

Result: ERGO achieves higher accuracy than original models and competitive methods with greater efficiency. For example, surpasses Qwen2.5-VL-7B on V* benchmark by 4.7 points while using only 23% of vision tokens and achieving 3x inference speedup.

Conclusion: The proposed ERGO approach effectively reduces computational costs while preserving accuracy by focusing on relevant regions, demonstrating significant efficiency improvements across multiple datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing learning-based DFF methods struggle in complex scenes where focus cues become ambiguous due to fine textures or abrupt depth changes, leading to misleading depth estimates.

Method: Uses a variational formulation with dual constraints: spatial constraints analyze gradient pattern changes across focus levels to distinguish depth edges from texture artifacts, and focal constraints enforce unimodal, monotonic focus probabilities aligned with physical focus behavior.
...
Conclusion: The dual constraint approach with proper inductive biases improves robustness and accuracy in challenging regions, making DFF more reliable for complex scenes.
...
TL;DR: The paper introduces RDcomm, a communication-efficient collaborative perception framework that uses information theory to optimize performance-communication trade-offs in multi-agent systems.


<details>
  <summary>Details</summary>
Motivation: To address the lack of theoretical foundation for performance-communication trade-offs in collaborative perception systems and provide optimal communication strategies for multi-agent collaboration.

Method: Proposes RDcomm framework with two key innovations: task entropy discrete coding for efficient task-relevant information supply, and mutual-information-driven message selection for redundancy-less transmission.

Result: Achieves state-of-the-art accuracy on DAIR-V2X and OPV2V datasets while reducing communication volume by up to 108 times compared to previous methods.

Conclusion: The proposed rate-distortion theory and RDcomm framework successfully bridge the theoretical gap in collaborative perception, enabling optimal communication strategies that maximize performance while minimizing bandwidth usage.
...
TL;DR: FailureAtlas is a framework that autonomously explores and maps failure landscapes of Text-to-Image models through active exploration, discovering systematic failures and linking them to training data scarcity.


<details>
  <summary>Details</summary>
Motivation: Static benchmarks have limited diagnostic power for uncovering systematic failures in T2I models and identifying root causes. There's a need for active exploration to better understand model weaknesses.
...
Conclusion: FailureAtlas establishes a new diagnostic-first methodology for deep model auditing to guide development of more robust generative AI.
...
TL;DR: A training-free method reduces hallucinations in multimodal LLMs by projecting captions to visual space for hallucination detection and editing decoder hidden states using positive/negative anchors.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from hallucinations that persist even in larger models, and existing mitigation approaches require additional training or compromise scalability and informativeness.

Method: Uses hallucination amplification: projects captions via text-to-image model to reveal implicit hallucinations as negative anchor, uses original image as positive anchor, then edits decoder hidden states by pulling toward faithful semantics and pushing away from hallucination directions.

Result: Significantly reduces hallucinations at object, attribute, and relation levels while preserving recall and caption richness, achieving over 5% hallucination reduction on CHAIR with LLaVA-v1.5-7B, with strong cross-architecture generalization.

Conclusion: The method is effective, efficient, training-free, and introduces minimal side effects on hallucination-free captions, demonstrating robust plug-and-play applicability.
...
TL;DR: CoFFT is a training-free approach that enhances Vision Language Models' visual reasoning by emulating human visual cognition through iterative foresight-focus cycles.
...
Method: Three-stage iterative process: (1) Diverse Sample Generation for exploring reasoning paths, (2) Dual Foresight Decoding to evaluate samples based on visual focus and reasoning progression, (3) Visual Focus Adjustment to precisely adjust focus toward beneficial regions.

Result: Consistent performance improvements of 3.1-5.8% across multiple benchmarks using Qwen2.5-VL, InternVL-2.5, and Llava-Next models with controllable computational overhead.

Conclusion: CoFFT effectively enhances VLM reasoning by creating an interdependent cycle where reasoning guides visual focus and visual focus informs subsequent reasoning, addressing limitations of current VLMs.
...
TL;DR: This paper introduces a new egocentric video dataset for face-to-face instruction analysis and benchmarks multimodal large language models (MLLMs) against conventional models for procedural step segmentation and conversation-state classification tasks.


<details>
  <summary>Details</summary>
Motivation: Face-to-face instructional interactions have not been systematically studied in computer vision due to lack of suitable datasets and limited analytical techniques, despite being critical for educational support and skill transfer.

Method: The authors created a new egocentric video dataset with ground-truth annotations and benchmarked multimodal large language models (MLLMs) that jointly process images, audio, and text against conventional task-specific models.

Result: MLLMs outperformed specialized baselines even without task-specific fine-tuning, demonstrating their effectiveness in understanding face-to-face instructional scenes that involve multiple modalities including speech content/prosody, gaze/body motion, and visual context.

Conclusion: Multimodal large language models show promise for holistic understanding of instructional interactions by effectively handling both verbal and nonverbal communication in an integrated manner.
...
TL;DR: DAVIS is a diffusion-based audio-visual separation framework that uses generative models (DDPM and Flow Matching) to directly synthesize separated sound spectrograms from noise, conditioned on mixed audio and visual inputs, achieving superior separation quality.


<details>
  <summary>Details</summary>
Motivation: Existing mask-based regression methods for audio-visual sound separation face limitations in capturing complex data distributions needed for high-quality separation across diverse sound categories.

Method: Leverages Denoising Diffusion Probabilistic Models (DDPM) and Flow Matching within a Separation U-Net architecture to synthesize separated sound spectrograms directly from noise distribution, conditioned on mixed audio and visual information.

Result: Both DDPM and Flow Matching variants of DAVIS surpass existing methods in separation quality on AVE and MUSIC datasets, demonstrating superior performance in audio-visual source separation.

Conclusion: The generative framework of DAVIS proves highly effective for audio-visual sound source separation, with both diffusion and flow matching approaches outperforming state-of-the-art methods.
...
Method: Proposes SpecXNet with Dual-Domain Feature Coupler (local spatial branch + global spectral branch using FFT) and Dual Fourier Attention module for dynamic feature fusion, built on modified XceptionNet backbone.
...
Conclusion: Unified spatial-spectral learning is effective for robust and generalizable deepfake detection, with code available on GitHub for reproducibility.
...
TL;DR: MGM is a novel framework that generates 3D content with PBR materials (albedo, roughness, metallic) using multiview material diffusion and Gaussian material representation, enabling dynamic relighting.
...
Method: Fine-tunes multiview material diffusion model conditioned on depth/normal maps, then uses Gaussian material representation to model PBR channels from generated multiview images.
...
<details>
  <summary>Details</summary>
Motivation: Current point cloud completion methods have limitations: supervised methods rely on ground truth with domain gap issues, unsupervised methods need complete point clouds, weakly-supervised methods require multi-view observations, and existing self-supervised methods produce unsatisfactory results due to limited self-supervised signals.
...
Conclusion: The proposed self-supervised method with multi-view augmentations and Mamba architecture effectively addresses limitations of existing approaches and demonstrates superior performance in point cloud completion.
...
<details>
  <summary>Details</summary>
Motivation: To address the high resource demands of conditional image generation models and scarcity of well-annotated data, which hinder deployment on edge devices and raise privacy concerns when user data is sent to third parties.
...
Result: Achieves significant reductions in computational cost and latency while maintaining high-fidelity generation capabilities and controllability, as demonstrated by comparative metrics.

Conclusion: Refine-Control effectively enables efficient deployment of conditional image generation models on edge devices by addressing resource constraints and data scarcity through semi-supervised distillation.
...
<details>
  <summary>Details</summary>
Motivation: Traditional classification assumes IID data, which destroys class correlations in 3D point clouds. This paper addresses the need to handle non-IID data while preserving inter-class relationships.
...
Result: Extensive experiments on ScanObject, ModelNet40, ScanntV2_cls and ModelNet-C datasets show competitive performance and improved robustness against corruption.

Conclusion: JGEKD effectively handles non-IID 3D point cloud data by preserving class correlations through joint graph entropy knowledge distillation, achieving competitive results and enhanced corruption robustness.
...
<details>
  <summary>Details</summary>
Motivation: Current neural program synthesis methods only use textual representations, failing to capture the visual-spatial nature of material node graphs that makes them intuitive for humans. There's a need to bridge this gap between visual accessibility and automated generation.

Method: Developed MultiMat framework using large multimodal models trained on production-quality procedural materials dataset. Combined with constrained tree search inference algorithm to ensure syntactic validity while efficiently navigating program space.

Result: Multimodal program synthesis is more efficient in both unconditional and conditional graph synthesis, achieving higher visual quality and fidelity than text-only baselines, establishing new state-of-the-art performance.

Conclusion: Multimodal approaches that leverage both visual and textual representations significantly improve procedural material graph generation compared to text-only methods, better capturing the intuitive visual-spatial nature of node graphs.
...
TL;DR: This paper integrates StyleGAN, DragGAN, and PCA to enhance latent space efficiency and controllability for GAN-generated images, achieving faster optimization while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: To improve the efficiency and interpretability of latent space exploration in GANs by combining structured latent spaces (StyleGAN), intuitive manipulation (DragGAN), and dimensionality reduction (PCA).

Method: Integrates PCA-based dimensionality reduction with DragGAN framework for image manipulation, applying PCA to latent W+ layers of DragGAN to reduce optimization time while maintaining quality.

Result: PCA integration consistently reduces total optimization time while maintaining good visual quality and even boosting SSIM, particularly in shallower latent spaces (W+ layers = 3). Successfully aligned images from AFHQ-Dog and AFHQ-Cat domains for cross-model manipulation.

Conclusion: The approach enables efficient and interpretable latent space control for image synthesis and editing applications, demonstrating potential for wide-ranging applications.
...
TL;DR: MinerU2.5 is a 1.2B-parameter document parsing vision-language model that uses a two-stage coarse-to-fine parsing strategy to achieve state-of-the-art accuracy with low computational cost.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient document parsing model that can handle high-resolution inputs without computational overhead while maintaining fine-grained detail recognition.

Method: Two-stage parsing: first performs layout analysis on downsampled images, then performs targeted content recognition on native-resolution crops using a comprehensive data engine for training.
...
Conclusion: MinerU2.5 demonstrates strong document parsing ability through its efficient two-stage approach, balancing accuracy and computational efficiency.
...
Result: RSThinker model achieves dominant performance, significantly outperforming state-of-the-art models across comprehensive tasks while providing verifiable analytical traces.

Conclusion: Geo-CoT framework provides concrete pathway from opaque perception to structured, verifiable reasoning in Earth Observation, with public release of dataset and model.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from three key flaws: reliance on costly per-scene retraining, restrictive monosemous design that fails to represent multi-concept semantics, and vulnerability to cross-view semantic inconsistencies.
...
Conclusion: MUSplat provides an efficient training-free solution for 3D open-vocabulary understanding that overcomes key limitations of existing methods while achieving superior performance.
...
TL;DR: UrbanFeel is a comprehensive benchmark with 14.3K visual questions to evaluate MLLMs' urban development understanding across static perception, temporal change understanding, and subjective environmental perception.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for MLLMs in urban environments are limited, lacking systematic exploration of temporal evolution and subjective perception that aligns with human perception.
...
Result: Gemini-2.5 Pro achieved the best overall performance, approaching human expert levels with only 1.5% gap. Models perform well on scene understanding tasks but struggle with temporal reasoning. Some models surpass humans in pixel-level change detection and reach human-level consistency in subjective perception dimensions.

Conclusion: MLLMs show strong capabilities in urban scene understanding and subjective perception, but need improvement in temporal reasoning for urban development tasks.
...
TL;DR: EXCL proposes a source-free unsupervised domain adaptation method using dual experts framework and retrieval-augmentation-interaction pipeline to adapt models without source data access.


<details>
  <summary>Details</summary>
Motivation: Address privacy and cost concerns in domain adaptation by eliminating need for source data access, while overcoming limitations of existing methods that neglect complementary insights and target data structure.

Method: Dual Experts framework with frozen source-domain model (plus Conv-Adapter) and pretrained vision-language model (with trainable text prompt), trained via three-stage RAIN pipeline: collaborative retrieval of pseudo-source/complex samples, separate expert fine-tuning, and learning object consistency enforcement.

Result: Extensive experiments on four benchmark datasets demonstrate state-of-the-art matching performance.

Conclusion: EXCL effectively addresses SFUDA challenges by leveraging dual expert cooperation and retrieval-augmented interaction, achieving competitive performance without source data access.
...
TL;DR: FlashEdit enables real-time text-guided image editing with diffusion models through three innovations: one-step inversion-and-editing pipeline, background shield technique, and sparsified spatial cross-attention mechanism, achieving 150x speedup while maintaining high fidelity.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based image editing methods suffer from prohibitive latency that hinders real-world applications, despite achieving remarkable quality.

Method: Three key innovations: (1) One-Step Inversion-and-Editing (OSIE) pipeline bypasses costly iterative processes; (2) Background Shield (BG-Shield) technique selectively modifies features only within edit region; (3) Sparsified Spatial Cross-Attention (SSCA) mechanism suppresses semantic leakage to background.
...
Conclusion: FlashEdit enables high-fidelity, real-time image editing with significant speed improvements while preserving background quality.
...
TL;DR: Neural-MedBench is a new benchmark for evaluating clinical reasoning in neurology, revealing that current vision-language models perform poorly on diagnostic reasoning tasks despite good performance on standard medical benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing medical AI benchmarks focus too much on classification accuracy, creating an illusion of proficiency while models still fail at high-stakes diagnostic reasoning. There's a need for benchmarks that specifically test clinical reasoning capabilities.

Method: Created Neural-MedBench with multi-sequence MRI scans, EHR data, and clinical notes covering differential diagnosis, lesion recognition, and rationale generation. Used hybrid scoring combining LLM graders, clinician validation, and semantic similarity metrics.

Result: State-of-the-art VLMs (GPT-4o, Claude-4, MedGemma) showed sharp performance drops on Neural-MedBench compared to conventional datasets. Error analysis revealed reasoning failures dominate model shortcomings rather than perceptual errors.

Conclusion: Proposes a Two-Axis Evaluation Framework: breadth-oriented datasets for statistical generalization and depth-oriented benchmarks like Neural-MedBench for reasoning fidelity. Neural-MedBench serves as an open diagnostic testbed for rigorous clinical AI assessment.
...
TL;DR: UniMapGen is a generative framework for large-scale map construction that uses discrete sequence representation of lane lines with multi-modal inputs and state update strategy to overcome satellite data limitations and achieve smooth, complete maps.


<details>
  <summary>Details</summary>
Motivation: Traditional map construction methods are costly and inefficient, while existing satellite-based approaches suffer from data limitations (occlusions, outdatedness) and produce rough, discontinuous roads requiring extensive post-processing.
...
Conclusion: UniMapGen provides an efficient generative framework for large-scale map construction that overcomes satellite data limitations and produces high-quality, complete maps with better continuity than traditional methods.
...
TL;DR: GS-2M is a unified framework for mesh reconstruction and material decomposition from multi-view images using 3D Gaussian Splatting, achieving robust performance on reflective surfaces without relying on external priors.


<details>
  <summary>Details</summary>
Motivation: Previous methods handle mesh reconstruction and material decomposition separately, struggle with reflective surfaces, and often require external model priors. Current joint approaches use complex neural components that limit scalability.
...
Result: Produces reconstruction results comparable to state-of-the-art methods, delivering triangle meshes and associated material components for downstream tasks, validated on widely used datasets.

Conclusion: The unified GS-2M framework effectively addresses both mesh reconstruction and material decomposition simultaneously, maintaining geometric details while being resilient to reflective surfaces without complex neural components.
...
TL;DR: This paper introduces MesaTask, an LLM-based framework for task-oriented tabletop scene generation that creates realistic 3D layouts aligned with human task instructions, addressing the limitations of manual or randomized scene creation methods.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for creating tabletop scenes for robot training rely on time-consuming manual layout design or purely randomized layouts, which lack plausibility and task alignment. There's a significant gap between high-level task instructions and the generated tabletop scenes.

Method: Proposed a Spatial Reasoning Chain that decomposes generation into object inference, spatial interrelation reasoning, and scene graph construction. Developed MesaTask, an LLM-based framework enhanced with DPO algorithms to generate physically plausible tabletop scenes.

Result: Created MesaTask-10K dataset with ~10,700 synthetic tabletop scenes with manually crafted realistic layouts. Exhaustive experiments showed superior performance compared to baselines in generating task-conforming tabletop scenes with realistic layouts.

Conclusion: MesaTask effectively bridges the gap between task instructions and tabletop scene generation, producing physically plausible scenes that align well with given task descriptions, addressing limitations of traditional scene creation methods.
...
TL;DR: Rule-based reinforcement learning improves generalization in document image classification, particularly for out-of-distribution data across different scenarios.


<details>
  <summary>Details</summary>
Motivation: To explore the benefits of rule-based reinforcement learning in document analysis tasks, leveraging its enhanced reasoning capabilities and better generalization properties.

Method: Applied rule-based reinforcement learning to document image classification task, testing generalization across out-of-distribution images, unseen classes, and different modalities.

Result: Reinforcement learning demonstrated better generalization capabilities compared to traditional methods, especially for out-of-distribution data scenarios.
...
TL;DR: SceneSplit is a black-box jailbreak method for Text-to-Video models that fragments harmful narratives into multiple benign scenes, manipulating the generative output space to bypass safety filters and achieve high attack success rates.
...
Method: SceneSplit fragments harmful narratives into multiple individually benign scenes, uses sequential scene combination to constrain the output space to unsafe regions, employs iterative scene manipulation to bypass safety filters, and utilizes a strategy library for reusing successful attack patterns.

Result: Achieved high average Attack Success Rates: 77.2% on Luma Ray2, 84.1% on Hailuo, and 78.2% on Veo2 across 11 safety categories, significantly outperforming existing baselines.
...
TL;DR: HiGS is a momentum-based sampling technique that improves diffusion model outputs by integrating past predictions, achieving better image quality with fewer steps and no extra computation.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models often produce unrealistic outputs with poor details when using fewer neural function evaluations or lower guidance scales.
...
Result: HiGS improves image quality across diverse models and achieves state-of-the-art FID of 1.61 for unguided ImageNet generation at 256×256 with only 30 steps instead of 250.

Conclusion: HiGS serves as a plug-and-play enhancement that enables faster diffusion sampling with higher fidelity, requiring no additional training or fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: Lightweight 3D medical image segmentation faces an "efficiency/robustness conflict" when processing complex anatomical structures and heterogeneous modalities, requiring better representation capabilities for low computational budgets.

Method: VeloxSeg uses a dual-stream CNN-Transformer architecture with Paired Window Attention (PWA) for multi-scale information retrieval and Johnson-Lindenstrauss lemma-guided convolution (JLC) for robust local feature extraction. It incorporates modal interaction and Spatially Decoupled Knowledge Transfer (SDKT) via Gram matrices to inject texture priors from self-supervised networks.
...
Conclusion: The proposed VeloxSeg framework successfully overcomes the efficiency/robustness conflict in lightweight 3D medical image segmentation, providing superior performance with significant computational efficiency gains for complex anatomical structures and heterogeneous modalities.
...
TL;DR: NIFTY is a hybrid texture synthesis framework combining diffusion models with patch-based optimization, using non-parametric flow-matching without neural network training.


<details>
  <summary>Details</summary>
Motivation: To address limitations of patch-based texture synthesis methods like poor initialization and visual artifacts while avoiding neural network training requirements.

Method: Hybrid framework combining diffusion models with classical patch-based texture optimization, using non-parametric flow-matching built on non-local patch matching.
...
Conclusion: NIFTY provides an effective texture synthesis approach that combines strengths of diffusion models and patch-based methods while avoiding neural training.
...
TL;DR: RAPID3 introduces a reinforcement learning framework that achieves 3x faster sampling for Diffusion Transformers without modifying the base generator, using three lightweight policy heads for adaptive acceleration.


<details>
  <summary>Details</summary>
Motivation: Existing acceleration methods for Diffusion Transformers use uniform heuristics that sacrifice quality, while dynamic networks require expensive fine-tuning. RAPID3 aims to provide per-image adaptive acceleration with zero updates to the base model.

Method: Uses three lightweight policy heads (Step-Skip, Cache-Reuse, Sparse-Attention) that observe denoising states and make independent acceleration decisions. Trained via Group Relative Policy Optimization with frozen generator, plus an adversarial discriminator to prevent reward hacking.

Result: Achieves nearly 3x faster sampling across state-of-the-art DiT backbones (Stable Diffusion 3, FLUX) while maintaining competitive generation quality compared to original models.

Conclusion: RAPID3 successfully demonstrates that reinforcement learning can enable adaptive, high-quality acceleration of Diffusion Transformers without requiring modifications to the base generator architecture.
...
<details>
  <summary>Details</summary>
Motivation: Current PAR methods fail to fully exploit attribute knowledge and contextual information. Existing approaches that use attribute text as additional input are still in their infancy and don't adequately model the complex relationships between visual features and semantic attributes.

Method: The paper constructs a multi-modal knowledge graph to mine relationships between local visual features and text, and between attributes and visual context samples. It introduces a knowledge graph-guided cross-modal hypergraph learning framework to enhance standard PAR frameworks.
...
TL;DR: CircuitSense is a benchmark for evaluating circuit understanding in MLLMs across hierarchical engineering design workflows, revealing major gaps in visual-to-mathematical reasoning despite strong visual perception capabilities.


<details>
  <summary>Details</summary>
Motivation: While MLLMs excel at natural image tasks, their ability to extract mathematical models from technical diagrams remains unexplored, particularly in engineering design workflows that require hierarchical abstraction from system specifications to component implementations.
...
Result: Closed-source models achieved over 85% accuracy on perception tasks (component recognition, topology identification) but performed below 19% on symbolic derivation and analytical reasoning. Models with stronger symbolic reasoning capabilities consistently achieved higher design task accuracy.

Conclusion: There is a critical gap between visual parsing and symbolic reasoning in MLLMs. Symbolic reasoning is identified as the key metric for engineering competence, with mathematical understanding being fundamental to circuit synthesis and design tasks.
...
TL;DR: HierLight-YOLO is a hierarchical feature fusion and lightweight model based on YOLOv8 that enhances real-time small object detection in drone imagery through multi-scale feature fusion and lightweight modules.


<details>
  <summary>Details</summary>
Motivation: Real-time detection of small objects in drone photography faces challenges of detecting small targets (<32 pixels) while maintaining efficiency on resource-constrained platforms. YOLO detectors have high false negative rates for small objects in drone-based detection.

Method: Proposes Hierarchical Extended Path Aggregation Network (HEPAN) for multi-scale feature fusion, plus two lightweight modules: Inverted Residual Depthwise Convolution Block (IRDCB) and Lightweight Downsample (LDown) module. Includes small object detection head for tiny object (4 pixels) detection.
...
Conclusion: HierLight-YOLO effectively addresses the dual challenges of small object detection and real-time efficiency in drone-based applications through hierarchical feature fusion and lightweight design.
...
<details>
  <summary>Details</summary>
Motivation: The proliferation of multimodal disinformation combining text and images presents significant challenges across digital platforms, requiring effective detection methods.

Method: Proposed GPT-4o-based approach with optimized prompts, structured multimodal analysis framework, preprocessing for token limitations, six evaluation criteria with self-assessment, and confidence-level analysis.

Result: Comprehensive performance analysis across multiple datasets (Gossipcop, Politifact, Fakeddit, MMFakeBench, AMMEBA) highlighting GPT-4o's strengths and limitations in disinformation detection.

Conclusion: The study provides a robust and reproducible methodological framework for automated multimodal disinformation analysis, including confidence-level and variability-based evaluation methods.
...
TL;DR: Using GPT-4 with specialized prompts to predict occlusion order relationships between objects in images without training data, achieving better accuracy through semantic context and commonsense reasoning.


<details>
  <summary>Details</summary>
Motivation: Occlusion is a major challenge for vision models in interpreting complex real-world scenes, requiring robust methods to predict object occlusion order relationships.
...
TL;DR: A multi-focus image fusion method that enhances boundary quality using gradient-domain modeling and Tenengrad gradient detection to preserve sharp focus-defocus transitions and focused details.


<details>
  <summary>Details</summary>
Motivation: Existing multi-focus image fusion methods struggle with preserving sharp focus-defocus boundaries, often resulting in blurred transitions and loss of focused details, which limits their effectiveness in applications like surveillance, microscopy, and computational photography.

Method: Proposes a gradient-domain-based model for initial fusion with complete boundaries, uses Tenengrad gradient detection to extract salient features and generate saliency maps, and develops a focus metric integrating gradient and complementary information for boundary refinement.
...
Conclusion: The proposed method effectively generates high-quality fused boundaries while preserving boundary details and focused regions, providing superior multi-focus image fusion performance compared to existing approaches.
...
TL;DR: TDOA is a novel text adversarial attack method that addresses dynamic-output scenarios by converting them to static single-output scenarios using clustering-based surrogate model training, achieving high attack success rates with minimal queries.


<details>
  <summary>Details</summary>
Motivation: Existing text adversarial attack methods are designed for static scenarios with fixed output labels and predefined label spaces, creating a gap for dynamic-output scenarios where labels can change.

Method: Uses clustering-based surrogate model training to convert dynamic-output scenarios to static single-output scenarios, and employs farthest-label targeted attack strategy to select adversarial vectors that deviate most from model's coarse-grained labels.

Result: Achieves maximum attack success rate of 50.81% with single query per text on dynamic outputs, and 82.68% on static outputs. Also extends to generative settings, surpassing prior results by up to 0.64 RDBLEU and 0.62 RDchrF.

Conclusion: TDOA effectively addresses dynamic-output text adversarial attacks, demonstrates strong performance in compromising large language models with limited access, and shows state-of-the-art results in both static and generative settings.
...
TL;DR: This paper introduces Logic Tensor Networks (LTNs) to incorporate medical background knowledge into semantic segmentation models, showing improved performance especially with limited training data.
...
Method: Use Logic Tensor Networks (LTNs) to encode medical background knowledge using first-order logic rules, applied in an end-to-end framework with SwinUNETR for semantic segmentation of hippocampus in brain MRI scans.

Result: LTNs improve baseline segmentation performance, especially when training data is scarce, demonstrating the effectiveness of incorporating medical knowledge.

Conclusion: Neuro-symbolic methods like LTNs are general enough to be adapted and applied to other medical semantic segmentation tasks, showing promise despite being in preliminary stages.
...
TL;DR: VARE and S-VARE are novel concept erasure frameworks for visual autoregressive models that enable stable and precise removal of unsafe concepts while maintaining generation quality, addressing safety gaps in text-to-image generation.


<details>
  <summary>Details</summary>
Motivation: Existing concept erasure techniques designed for diffusion models fail to work with visual autoregressive models due to their different next-scale token prediction paradigm, creating safety concerns in text-to-image generation.

Method: VARE uses auxiliary visual tokens to reduce fine-tuning intensity, while S-VARE incorporates filtered cross entropy loss to precisely identify unsafe visual tokens and preservation loss to maintain semantic fidelity, preventing language drift and reduced diversity.
...
Conclusion: The proposed methods successfully enable stable concept erasure in VAR models, addressing the limitations of existing techniques and enhancing safety in visual autoregressive text-to-image generation.
...
Method: RAU combines VLM-based spatial reasoning between reference and target images with SAM2's fine-grained segmentation. The VLM learns anatomical region identification through relative spatial reasoning trained on moderately sized datasets, then provides spatial cues to SAM2 for pixel-level segmentation.

Result: RAU consistently outperforms SAM2 fine-tuning baselines across both in-distribution and out-of-distribution datasets, achieving more accurate segmentations and reliable localization. It shows strong generalization ability crucial for medical applications.

Conclusion: RAU demonstrates the potential of VLM-driven approaches for anatomical understanding in clinical workflows, being the first framework to explore VLMs for reference-based identification, localization and segmentation of anatomical structures in medical images.
...
TL;DR: FreqDebias addresses spectral bias in deepfake detectors by using frequency debiasing with Fo-Mixup augmentation and dual consistency regularization to improve cross-domain generalization.
...
Conclusion: The proposed frequency debiasing framework effectively mitigates spectral bias and improves deepfake detector generalization across diverse forgery types.
...
<details>
  <summary>Details</summary>
Motivation: Universal image restoration faces challenges with unknown degradation mixtures where existing methods oversmooth, hallucinate, or drift semantics. Current approaches rely on text prompts or MLLM captions which introduce latency and instability.

Method: Uses lightweight dual-branch conditioner to inject degraded input and restored proxy signals, timestep/layer-adaptive modulation schedule for hierarchical routing, and enforces caption-free semantic alignment via SigLIP features from proxy.

Result: Outperforms strong open-source and commercial baselines across synthetic and in-the-wild benchmarks. Ablation studies confirm necessity of each component.

Conclusion: For large diffusion transformers, the key to robust caption-free universal image restoration is controlling when, where, and what to condition on, rather than adding parameters or relying on text prompts.
...
Method: For visual modality: Multi-Scale Explanation Aggregation (MSEA) aggregates attributions over multi-scale inputs to dynamically adjust receptive fields. For textual modality: Activation Ranking Correlation (ARC) measures token relevance via top-k prediction ranking alignment to suppress spurious activations.

Result: Extensive experiments on state-of-the-art MLLMs and benchmark datasets show the approach consistently outperforms existing interpretability methods, producing more faithful and fine-grained explanations.

Conclusion: The proposed intra-modal interaction enhancement methods significantly improve MLLM interpretability by addressing limitations in both visual and textual attribution, yielding more holistic and coherent explanations of model behavior.
...
TL;DR: U-MAN enhances U-Net for medical image segmentation by replacing simple skip connections with Progressive Attention-Guided Feature Fusion and adding Multi-scale Adaptive KAN modules to handle multi-scale features, achieving superior boundary accuracy and detail preservation.


<details>
  <summary>Details</summary>
Motivation: Conventional U-Net architectures have limitations in preserving fine-grained details and precise boundaries due to semantic gaps in skip connections and lack of multi-scale feature extraction in deep layers.
...
Conclusion: U-MAN effectively addresses U-Net limitations through attention-guided feature fusion and multi-scale adaptive processing, achieving superior medical image segmentation performance.
...
TL;DR: The paper proposes γ-Quant, a method for learning non-linear quantization for pattern recognition in low-bandwidth, energy-constrained settings using raw data with as few as 4-bits.


<details>
  <summary>Details</summary>
Motivation: Current pattern recognition models use pre-processed data optimized for human perception, but this is inefficient for automated analysis in low-bandwidth scenarios and affects battery life in wearable devices.

Method: γ-Quant learns task-specific non-linear quantization for raw data, demonstrated on raw-image object detection and human activity recognition from wearable sensor data.

Result: Raw data with learnable quantization using only 4-bits performs on par with using raw 12-bit data.

Conclusion: Task-specific learned quantization enables efficient pattern recognition in low-bandwidth, energy-constrained environments without sacrificing performance.
...
<details>
  <summary>Details</summary>
Motivation: Application-oriented VIF methods require labeled datasets for downstream tasks like segmentation, which are labor-intensive to acquire. This paper addresses the data acquisition bottleneck by developing a self-supervised approach.

Method: Uses cross-segmentation consistency between feature-level and pixel-level fusion as a self-supervised task, with a two-stage training strategy and dynamic weight adjustment for effective joint learning without segmentation labels.
...
Conclusion: SSVIF provides an effective self-supervised solution for segmentation-oriented VIF that eliminates the need for labeled segmentation data while achieving competitive performance with supervised methods.
...
TL;DR: Proposes B\'ezier Meets Diffusion framework for cross-domain medical image generation, combining B\'ezier-curve style transfer and conditional diffusion models to address domain gaps and generate realistic labeled target-domain images.


<details>
  <summary>Details</summary>
Motivation: Training robust learning algorithms across medical imaging modalities is challenging due to large domain gaps. Unsupervised domain adaptation (UDA) helps but existing GAN-based approaches struggle with high variability regions.

Method: 1) B\'ezier-curve-based style transfer to reduce domain gap; 2) Train segmentation model on transferred images; 3) Use pseudo-labels to train conditional diffusion model; 4) Uncertainty-guided score matching to handle noisy pseudo-labels.
...
Conclusion: The unified framework effectively addresses domain adaptation challenges in medical imaging by combining B\'ezier style transfer and diffusion models, producing high-quality synthetic data that enhances segmentation performance.
...
TL;DR: PSTTS is a plug-and-play module that reduces computational overhead in event-based vision by identifying and removing spatio-temporal redundant tokens from event data without additional parameters.


<details>
  <summary>Details</summary>
Motivation: Existing event-based methods convert event streams to event frames but neglect spatial sparsity and inter-frame motion redundancy, causing computational inefficiency. Token sparsification methods for RGB videos are ineffective for event data due to unreliable intermediate representations and event noise.

Method: PSTTS uses two stages: Spatial Token Purification (removes noise and non-event regions via spatio-temporal consistency) and Temporal Token Selection (removes redundant temporal information by evaluating motion pattern similarity between adjacent frames).
...
Conclusion: PSTTS effectively addresses computational inefficiency in event-based vision by exploiting raw event data characteristics to identify and remove redundant tokens, achieving optimal accuracy-efficiency trade-off.
...
TL;DR: GCPO is a method that identifies critical tokens in autoregressive visual generation and applies token-wise policy optimization, achieving better performance with only 30% of tokens compared to full-token approaches.


<details>
  <summary>Details</summary>
Motivation: Existing RLVR methods apply uniform optimization across all image tokens, ignoring varying token contributions. The challenge is identifying critical tokens during AR generation and implementing effective token-wise optimization.

Method: GCPO identifies critical tokens from three perspectives: causal dependency (early tokens determine later ones), entropy-induced spatial structure (high entropy gradient tokens), and RLVR-focused token diversity (low similarity tokens). It uses dynamic token-wise advantage weights based on confidence divergence.
...
Conclusion: GCPO successfully addresses the challenge of identifying and optimizing critical tokens in RLVR-based AR visual generation, providing more efficient and effective training through token-wise policy optimization.
...
Conclusion: EAGLE is an effective and practical framework for advancing MLLM interpretability, providing fine-grained insights into model decisions.
...
TL;DR: VLMs show good color naming for prototypical colors but struggle with non-prototypical ones, with language model architecture significantly influencing naming patterns and severe cross-linguistic training imbalances favoring English and Chinese.


<details>
  <summary>Details</summary>
Motivation: To understand whether vision-language models name colors like humans for effective human-AI interaction, as color is fundamental to human visual perception and communication.

Method: Systematic evaluation of 5 VLMs using 957 color samples, replicating classic color naming methodologies, with cross-linguistic analysis across 9 languages and ablation studies.

Result: VLMs achieve high accuracy on prototypical colors but performance drops significantly on expanded non-prototypical sets. 21 common color terms emerge across models, with constrained vs expansive naming approaches. Severe training imbalances favor English and Chinese, with hue as primary driver of color naming decisions.

Conclusion: Language model architecture significantly influences color naming independent of visual processing capabilities, revealing systematic differences in how VLMs approach color naming compared to humans.
...
TL;DR: EfficientDepth is a novel monocular depth estimation system that combines transformer architecture with lightweight convolutional decoder and bimodal density head to achieve geometric consistency, fine details, and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing monocular depth estimation methods fail to meet key requirements for 3D reconstruction and view synthesis, including geometric consistency, fine details, robustness to real-world challenges, and efficiency for edge devices.
...
Result: EfficientDepth achieves performance comparable to or better than state-of-the-art models with significantly reduced computational resources.

Conclusion: The proposed EfficientDepth system successfully addresses key challenges in monocular depth estimation while maintaining high performance and computational efficiency.
...
TL;DR: This survey paper comprehensively reviews category discovery (CD) methods, categorizing them into novel category discovery (NCD) and generalized category discovery (GCD) settings, and analyzes key components including representation learning, label assignment, and class number estimation.


<details>
  <summary>Details</summary>
Motivation: Category discovery is an emerging open-world learning task that aims to automatically categorize unlabeled data containing instances from unseen classes, given some labeled data from seen classes. The field has seen significant growth but lacks a comprehensive survey.

Method: The survey introduces a taxonomy with two base settings (NCD and GCD) and several derived settings for real-world challenges. It analyzes methods across three fundamental components: representation learning, label assignment, and estimation of class number. The authors also benchmark existing methods.

Result: Key insights show that large-scale pretrained backbones, hierarchical and auxiliary cues, and curriculum-style training benefit category discovery. Challenges remain in label assignment design, class number estimation, and scaling to complex multi-object scenarios.

Conclusion: The paper provides a comprehensive review of category discovery literature, identifies key insights from existing methods, and points out promising future research directions. A living survey is maintained on GitHub for ongoing updates.
...
<details>
  <summary>Details</summary>
Motivation: Current SSL methods struggle with semantic understanding of complex anomalies involving multiple entities and temporal dependencies, while LLMs are computationally expensive for frame-by-frame analysis and lack spatial localization.

Method: Hybrid approach with multi-task SSL temporal analyzer (using nnFormer backbone) to identify suspicious frames, followed by LLM validator that applies structured rule-based reasoning for semantic context enrichment.

Result: Achieves 72.5% frame-level AUC on ComplexVAD dataset, outperforming existing baselines by 12.5% while reducing LLM computation.

Conclusion: The hybrid SSL-LLM approach effectively addresses complex video anomaly detection by combining temporal pattern learning with semantic reasoning, providing a balanced solution for computational efficiency and detection accuracy.
...
TL;DR: JanusVLN proposes a dual implicit neural memory framework for Vision-and-Language Navigation that models spatial-geometric and visual-semantic memory as compact neural representations, achieving state-of-the-art performance by avoiding computational redundancy and memory bloat.


<details>
  <summary>Details</summary>
Motivation: Current VLN methods relying on explicit semantic memory (textual cognitive maps or stored visual frames) suffer from spatial information loss, computational redundancy, and memory bloat, which impede efficient navigation.

Method: Extends MLLM with 3D prior knowledge from spatial-geometric encoder, constructs dual implicit memory from historical key-value caches of both encoders, and uses sliding window approach to avoid redundant computation while enabling efficient incremental updates.
...
Conclusion: The dual implicit neural memory represents a novel paradigm that explores promising new directions for future VLN research by mimicking human navigation's implicit scene representation.
...
Method: Leverages temporal dynamics through SNN leakage factors for diverse pseudo-labeling in a co-training framework. Uses agreement among multiple predictions from a single SNN to generate pseudo-labels from weakly-augmented samples for training on strongly-augmented ones.

Result: SpikeMatch outperforms existing SSL methods adapted to SNN backbones across various standard benchmarks.

Conclusion: The framework effectively mitigates confirmation bias by capturing discriminative features with limited labels, demonstrating the viability of SSL for SNNs.
...
TL;DR: The paper proposes using 2D Gaussian Splatting (2DGS) as an alternative visual representation to RGB images for vision-language pipelines, achieving significant compression (3-20x) and faster processing while maintaining semantic capabilities.


<details>
  <summary>Details</summary>
Motivation: To address inefficiencies in current vision-language pipelines: (i) high energy costs of transmitting dense RGB images from edge devices to cloud, and (ii) sequence length explosion from patch-based tokenization that stresses attention mechanisms.

Method: Developed scalable 2DGS pipeline with structured initialization, luminance-aware pruning, and batched CUDA kernels. Adapted CLIP training by reusing frozen RGB transformer backbone with lightweight splat-aware input stem and perceiver resampler, training only 7% of parameters.

Result: Achieved 90x faster fitting and 97% GPU utilization compared to prior implementations. GS encoders yield meaningful zero-shot ImageNet-1K performance while compressing inputs 3-20x relative to pixels. Accuracy currently trails RGB encoders but establishes viability.

Conclusion: 2DGS is established as a viable multimodal substrate that opens a path toward representations that are both semantically powerful and transmission-efficient for edge-cloud learning, though further work is needed to close the accuracy gap with RGB encoders.
...
TL;DR: LongLive is an autoregressive framework for real-time long video generation that addresses efficiency and quality challenges through KV-recache mechanism, streaming long tuning, and frame-level attention sink.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing methods: diffusion models are inefficient due to bidirectional attention, while causal AR models degrade in quality on long videos and lack interactive capabilities for dynamic content creation.

Method: Uses frame-level autoregressive design with KV-recache mechanism for prompt switching, streaming long tuning for long video training, and short window attention with frame sink for consistency and speed.

Result: Achieves 20.7 FPS on single H100 GPU, supports up to 240-second videos, fine-tunes 1.3B model in 32 GPU-days, maintains strong VBench performance, and supports INT8 quantization with minimal quality loss.

Conclusion: LongLive successfully enables efficient, high-quality long video generation with real-time interactive capabilities, addressing key challenges in both training and inference for dynamic content creation.
...
TL;DR: SPARK is a synergistic framework that recycles RLVR rollouts and correctness data to co-evolve both policy and reward models, eliminating the need for separate reward models and human preference data while achieving significant performance gains.


<details>
  <summary>Details</summary>
Motivation: To address the high costs and potential reward-policy mismatch in RLHF, and the wasted supervision in RLVR that discards valuable rollout and correctness data after each update.

Method: SPARK recycles rollouts and correctness data from RLVR to simultaneously train the model as a generative reward model using multiple objectives (pointwise reward score, pairwise comparison, evaluation conditioned on further-reflection responses), creating a co-evolving feedback loop between policy and reward.
...
Conclusion: SPARK provides an efficient, on-policy, and stable framework that eliminates the need for separate reward models and costly human preference data while supporting test-time scaling via self-reflection, demonstrating robustness and broad generalization.
...
<details>
  <summary>Details</summary>
Motivation: Depth estimation is crucial for robotics, autonomous vehicles, and AR applications that operate under computational constraints. Stereo image pairs provide an effective solution, but acquiring reliable ground-truth depth data is difficult, making self-supervised approaches necessary.
...
Result: Achieves competitive metrics on KITTI Eigen Split test data while being 10.18× faster than current best model. Achieves state-of-the-art results on KITTI Eigen Split Improved Ground Truth and Driving Stereo datasets.

Conclusion: CCNeXt provides an efficient and effective self-supervised solution for depth estimation that balances computational cost with performance, outperforming existing methods across multiple benchmarks.
...
TL;DR: UML-CoT is a structured reasoning framework that uses Unified Modeling Language to create symbolic chain-of-thought reasoning and executable action plans, outperforming traditional CoT in interpretability and execution success.
...
Method: UML-CoT leverages UML class diagrams for compositional object semantics and activity diagrams for procedural control flow. Uses a three-stage training pipeline combining supervised fine-tuning with Group Relative Policy Optimization, including reward learning from answer-only data.
...
Conclusion: UML provides a more expressive and actionable structured reasoning formalism compared to traditional CoT approaches, enabling better interpretability and executability in embodied tasks.
...
<details>
  <summary>Details</summary>
Motivation: Curating high-quality, domain-specific datasets is a major bottleneck for deploying robust vision systems, requiring complex trade-offs between data quality, diversity, and cost.

Method: A central orchestrator agent uses multi-step reasoning to execute specialized tools: Calibrated Discovery for relevant data sourcing, Controllable Synthesis for generating rare scenario data, and Consensus Annotation with a novel consensus mechanism using non-maximum suppression and voting.

Result: Consensus Annotation achieved 14.2 candidate proposals per image (vs 7.4 ground-truth) on COCO with 37.1% mAP, discovered 903 new categories on Open Images. Calibrated Discovery was 40x more computationally efficient at 10-million sample scale.
...
<details>
  <summary>Details</summary>
Motivation: Few-shot image classification is challenging due to limited labeled examples, and existing methods often require extensive fine-tuning or external information sources.
...
TL;DR: This paper shows that VAR (Visual Autoregressive Generation) with Markovian attention mask is mathematically equivalent to discrete diffusion, establishing a bridge between AR transformers and diffusion models.


<details>
  <summary>Details</summary>
Motivation: To uncover the theoretical connection between autoregressive transformers and diffusion models in visual generation, and leverage diffusion advantages to improve VAR's efficiency and performance.

Method: Reinterpret VAR with Markovian attention mask as Scalable Visual Refinement with Discrete Diffusion (SRDD), importing diffusion advantages like iterative refinement into the VAR framework.
...
Conclusion: The mathematical equivalence between VAR with Markovian attention and discrete diffusion provides a principled bridge that enables importing diffusion advantages into AR transformers, yielding significant improvements in visual generation.
...
<details>
  <summary>Details</summary>
Motivation: Current CLIP-based CIL approaches use simplistic templates like "a photo of a [CLASS]" that ignore the hierarchical nature of visual concepts. They also only use the last layer representation, missing hierarchical information from earlier layers.

Method: Leverages LLMs to recursively generate discriminative textual descriptors that augment semantic space with hierarchical cues. These descriptors are matched to different semantic hierarchy levels and adaptively routed based on task requirements.

Result: Extensive experiments on multiple benchmarks show the method consistently achieves state-of-the-art performance in class-incremental learning.

Conclusion: HERMAN enables precise discrimination while alleviating catastrophic forgetting in incremental tasks by explicitly modeling hierarchical visual concepts through multi-level representation matching.
...
TL;DR: DeeptraceReward is a benchmark for fine-grained detection of AI-generated video artifacts, with 4.3K annotations across 3.3K videos that include explanations, spatial regions, and temporal timestamps for fake traces.


<details>
  <summary>Details</summary>
Motivation: To address the gap in detecting spatiotemporal grounded visual artifacts that reveal videos as AI-generated, moving beyond simple binary classification to detailed human-perceived fake trace analysis.

Method: Created a dataset with detailed annotations including natural-language explanations, bounding-box regions, and precise timestamps for fake traces. Trained multimodal language models as reward models to mimic human judgments and localizations.

Result: The 7B reward model outperformed GPT-5 by 34.7% on average across fake clue identification, grounding, and explanation. Found consistent difficulty gradient: binary classification easiest, then explanations, spatial grounding, and temporal labeling hardest.

Conclusion: DeeptraceReward provides a rigorous testbed for socially aware video generation by foregrounding human-perceived deepfake traces, enabling better detection and training of trustworthy AI systems.
...
TL;DR: CapRL applies Reinforcement Learning with Verifiable Rewards to image captioning, using caption utility as quality metric - a good caption should enable a vision-free LLM to answer questions about the image accurately.
...
Method: Two-stage pipeline: LVLM generates caption, then vision-free LLM answers multiple-choice questions using only the caption. Reward is based on answer accuracy, defining caption quality by its utility.

Result: Significant gains across 12 benchmarks with CapRL-5M dataset. Achieves performance comparable to Qwen2.5-VL-72B and exceeds baseline by 8.4% in Prism Framework evaluation.

Conclusion: RLVR paradigm successfully addresses limitations of SFT for subjective tasks like image captioning. CapRL demonstrates that utility-based quality metrics enable scalable, high-performance caption generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing referring segmentation methods require fine-tuning or multiple pre-trained models with additional training and modifications. Diffusion models contain rich semantic information that can be leveraged as general-purpose feature extractors.

Method: Extracts features and attention scores from diffusion transformers without modifications. Uses stop words as attention magnets to filter noise, identifies and handles global attention sinks, and implements attention redistribution strategy where stop words partition background activations into smaller clusters.
...
Conclusion: The proposed RefAM framework demonstrates that diffusion transformer features can be effectively utilized for grounding tasks through proper attention handling strategies, achieving superior performance in a training-free manner.
