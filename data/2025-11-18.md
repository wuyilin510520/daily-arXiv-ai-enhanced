...
TL;DR: AI system analyzes handwritten exam scripts to quantify student stress levels using OCR and sentiment analysis.


<details>
  <summary>Details</summary>
Motivation: To go beyond traditional grading and provide insights into students' cognitive and emotional states during exams.

Method: Uses high-resolution image processing, TrOCR for text recognition, and RoBERTa-based sentiment analysis with entropy fusion to create a Stress Index. Employs five-model voting and unsupervised anomaly detection for robustness.

Result: Developed an innovative framework for academic forensics that quantifies psychological stress from handwriting.

Conclusion: The system successfully bridges graphology and AI to provide data-driven stress assessment in educational contexts.
...
TL;DR: Real-time pothole detection using SVM classifier with 98.1% accuracy from vehicle sensors for large-scale road condition monitoring.


<details>
  <summary>Details</summary>
Motivation: Road conditions significantly impact daily commute and traffic flow. Small cracks can develop into large potholes due to temperature changes and vehicle pressure, requiring frequent monitoring to maintain road quality and ensure smooth traffic.

Method: Used SVM classifier with onboard vehicle sensors to detect potholes in real-time from data collected over 2km of local roads containing 26 potholes.

Result: Achieved 98.1% accuracy in pothole detection based on the collected dataset.

Conclusion: The approach enables effective real-time pothole identification using vehicle sensors, providing valuable data for large-scale road condition analysis and management.
...
<details>
  <summary>Details</summary>
Motivation: Lack of standardized habitat classification system, incomplete coverage, and inability of existing models to effectively integrate semantic and texture features for multi-scale habitats in cultivated land ecosystems.

Method: Developed annotated dataset with 15 habitat categories, proposed DWFF-Net using frozen DINOv3 encoder, data-level adaptive dynamic weighting for feature fusion, dynamic weight computation network in decoder, and hybrid loss function.

Result: Achieved mIoU of 0.6979 and F1-score of 0.8049, outperforming baseline by 0.021 and 0.0161 respectively. Improved IoU for micro-habitat categories like field ridges.

Conclusion: Established habitat identification framework enabling sub-meter precision mapping at low cost, providing technical support for fine-grained habitat monitoring in cultivated landscapes.
...
TL;DR: AGENet is a few-shot medical image segmentation framework that uses edge-aware geodesic distance learning to improve boundary delineation with limited training data.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation requires large annotated datasets, creating bottlenecks for clinical applications. Existing few-shot methods have suboptimal boundary delineation, especially for anatomically similar regions without sufficient spatial context.
...
Result: Extensive experiments show improvements over state-of-the-art methods, reducing boundary errors while maintaining computational efficiency.
...
TL;DR: EPSegFZ is a pre-training-free network for few-shot and zero-shot 3D point cloud semantic segmentation that uses prototype-enhanced attention and language-guided embeddings to improve performance without relying on pre-training.


<details>
  <summary>Details</summary>
Motivation: Current few-shot 3D point cloud segmentation methods rely heavily on pre-training, limiting flexibility, and fail to fully utilize textual information from support sets, which restricts zero-shot capabilities.

Method: Uses Prototype-Enhanced Registers Attention (ProERA), Dual Relative Positional Encoding (DRPE) cross-attention for feature extraction, and Language-Guided Prototype Embedding (LGPE) to leverage textual information.

Result: Outperforms state-of-the-art by 5.68% on S3DIS and 3.82% on ScanNet benchmarks.

Conclusion: EPSegFZ achieves superior performance in few-shot and zero-shot scenarios without pre-training by effectively combining visual and textual information from support sets.
...
TL;DR: TASA is a novel geometry-optimized framework for 3D scene-level affordance segmentation that combines 2D semantic cues with 3D geometric reasoning in a coarse-to-fine manner, achieving superior accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene-level affordance understanding are limited by focusing on object-level affordances or inefficiently lifting 2D predictions to 3D, neglecting rich geometric structure and incurring high computational costs.

Method: TASA features a task-aware 2D affordance detection module to identify manipulable points from language and visual inputs, guiding task-relevant view selection, and a 3D affordance refinement module that integrates 2D semantic priors with local 3D geometry.
...
Conclusion: The proposed TASA framework effectively addresses limitations of existing methods by jointly leveraging 2D semantic cues and 3D geometric reasoning, providing accurate and spatially coherent 3D affordance masks while maintaining computational efficiency.
...
TL;DR: LE-CapsNet is a lightweight, enhanced version of Capsule Network that achieves higher accuracy (76.73% on CIFAR-10) while being 4x faster and more robust to affine transformations.


<details>
  <summary>Details</summary>
Motivation: Capsule Networks (CapsNet) have advantages over CNNs in detecting overlapping categories and transformed images, but they are slow, resource-intensive, and lag in accuracy compared to CNNs.

Method: Proposed LE-CapsNet as a light, enhanced variant of CapsNet with reduced parameters (3.8M weights) and optimized structure.

Result: Achieved 76.73% accuracy on CIFAR-10 (4x faster inference than CapsNet) and 94.3% accuracy on AffNIST (vs CapsNet's 90.52%), demonstrating better robustness to affine transformations.

Conclusion: LE-CapsNet successfully addresses CapsNet's limitations by providing a more efficient, faster, and accurate alternative that maintains the benefits of capsule networks while overcoming their computational drawbacks.
...
TL;DR: TBSD is a new method that resolves the trade-off between texture quality and shape distortion in 3D asset generation using Score Distillation Sampling by formulating it as a multi-objective optimization problem.


<details>
  <summary>Details</summary>
Motivation: Vanilla SDS suffers from over-saturation and over-smoothing, and recent variants using negative prompts face a critical trade-off between limited texture optimization or texture gains with shape distortion.

Method: Target-Balanced Score Distillation (TBSD) formulates generation as a multi-objective optimization problem and introduces an adaptive strategy that effectively resolves the texture-shape trade-off.
...
Conclusion: TBSD successfully resolves the fundamental trade-off in SDS-based 3D generation, enabling high-quality 3D assets with both realistic textures and accurate geometry.
...
TL;DR: CompressNAS is a neural architecture search framework that globally optimizes rank selection for tensor decomposition to compress CNNs for deployment on resource-constrained devices, achieving significant compression with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Deep CNNs are difficult to deploy on microcontrollers and lightweight NPUs due to their size and computational demands. Existing rank selection methods for tensor decomposition are local and ignore global trade-offs between compression and accuracy.

Method: CompressNAS treats rank selection as a global search problem using a MicroNAS-inspired framework with a fast accuracy estimator to efficiently evaluate candidate decompositions under memory and accuracy constraints.

Result: Achieved 8x compression of ResNet-18 with <4% accuracy drop on ImageNet; 2x compression of YOLOv5s without accuracy drop and 2x compression of YOLOv5n with 2.5% drop on COCO. Introduced STResNet family with competitive performance.

Conclusion: CompressNAS enables efficient CNN compression through global rank optimization, making deep networks more deployable on resource-constrained devices while maintaining competitive accuracy.
...
TL;DR: AdaptFly is a prompt-guided test-time adaptation framework for UAV semantic segmentation that enables robust performance under weather, lighting, and viewpoint changes without weight updates, using lightweight token-prompt retrieval for resource-limited UAVs and gradient-free visual prompt optimization for resource-massive UAVs.


<details>
  <summary>Details</summary>
Motivation: Segmentation foundation models deteriorate under environmental variations, resource-limited UAVs cannot run gradient-based adaptation, and resource-massive UAVs adapt independently without sharing experience, leading to wasted knowledge and inefficiency.

Method: Two complementary adaptation modes: lightweight token-prompt retrieval from shared global memory for resource-limited UAVs, and gradient-free sparse visual prompt optimization using Covariance Matrix Adaptation Evolution Strategy for resource-massive UAVs, with activation-statistic detection and cross-UAV knowledge pooling.
...
Conclusion: AdaptFly provides a practical path to resilient, communication-efficient perception for low-altitude UAV networks, enabling fleet-wide collaboration with negligible bandwidth overhead.
...
TL;DR: A biologically plausible masked autoencoder approach that mimics human visual blind spot filling for learning word-referent mappings from child development data.


<details>
  <summary>Details</summary>
Motivation: To develop a biologically plausible learning strategy that mimics how children naturally learn word-object associations from their visual environment, addressing the challenge of learning from limited data without prior knowledge.

Method: Uses a masked autoencoder-based visual backbone with a novel masking strategy that incorporates knowledge about the blind spot in human eyes, combined with contrastive learning for video-text modeling to acquire word-referent mappings.

Result: The proposed biologically plausible masking strategy performs at least as effectively as random masking for learning word-referent mappings from cross-situational and temporally extended episodes.

Conclusion: Biologically inspired masking strategies can be as effective as standard approaches while being more biologically plausible, offering potential for more natural language acquisition models.
...
<details>
  <summary>Details</summary>
Motivation: Spatial omics data (transcriptomics, proteomics, epigenomics) lack pathological morphological context from histopathological images, requiring integration for comprehensive tissue analysis. However, heterogeneity across modalities, resolution mismatches, and biological perturbations during sample preparation pose significant integration challenges.

Method: GROVER uses a Graph Convolutional Network encoder based on Kolmogorov-Arnold Networks to capture nonlinear dependencies between modalities and spatial structure. It employs spot-feature-pair contrastive learning for cross-modal alignment and a dynamic expert routing mechanism to adaptively select informative modalities while suppressing noisy inputs.

Result: Experiments on real-world spatial omics datasets demonstrate that GROVER outperforms state-of-the-art baselines in multimodal integration performance.

Conclusion: GROVER provides a robust and reliable solution for adaptive integration of spatial multi-omics data, effectively addressing the challenges of multimodal heterogeneity and resolution mismatches.
...
TL;DR: HSI-Detect uses hyperspectral imaging reconstruction from RGB inputs to improve Deepfake detection by amplifying manipulation artifacts in specific frequency bands.


<details>
  <summary>Details</summary>
Motivation: Current Deepfake detection methods operate in RGB space with only three spectral channels, limiting their ability to detect subtle manipulation artifacts that may be more visible in denser spectral bands.
...
Conclusion: Spectral-domain mapping through hyperspectral imaging reconstruction shows promise for enhancing Deepfake detection capabilities by revealing artifacts that are weak or invisible in the RGB domain.
...
<details>
  <summary>Details</summary>
Motivation: Recent work in Equivariant learning literature highlights advantages of bi-Lipschitz models, motivating examination of whether invariant point cloud networks preserve natural symmetry-aware distances.
...
Conclusion: Modified bi-Lipschitz invariant networks outperform standard invariant models for finding correspondences between 3D point clouds.
...
Method: Uses a multi-agent system: 1) multimodal concept generator mines visual concepts from training images, 2) symbols are discovered based on visual concepts, 3) LLM reasoner composes symbols into first-order rules, 4) vision verifier quantifies symbol presence during inference.

Result: Improves state-of-the-art neurosymbolic baselines by average 5% across five benchmarks (including medical imaging and underrepresented datasets), reduces hallucinated symbols in rules by up to 50%.

Conclusion: Concept-RuleNet successfully reinstates visual grounding in neurosymbolic reasoning while maintaining transparency, achieving better performance and reduced hallucinations compared to existing methods.
...
TL;DR: Proposes Batch Transformers with implicit sparse attention to important dimensions, reducing bottleneck size in encoder-decoder architectures for improved face recognition with makeup/occlusion datasets.


<details>
  <summary>Details</summary>
Motivation: To address the computational bottleneck in traditional Transformers by focusing attention on important dimensions rather than entire sequences, enabling more efficient processing while maintaining performance.

Method: Uses implicit sparse attention mechanism that selectively attends to primary components/dimensions, implemented in Batch Transformers architecture for encoder-decoder neural networks.

Result: Achieved significant reduction in bottleneck size while improving performance on synthetic image generation for face recognition with makeup and occlusion datasets, increasing variability of limited original data.

Conclusion: Batch Transformers with dimension-selective attention provide an efficient alternative to traditional Transformers, enabling better performance with reduced computational requirements for face recognition tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current single-shot text-to-image models struggle with long, compositional prompts typical of creative workflows, requiring a system that can reliably execute complex multi-step image generation tasks.

Method: Uses reflective reinforcement learning to orchestrate a registry of pretrained experts, handles long-form prompts via dynamic task decomposition, and supervises alignment through structured feedback from a vision-language model critic, framing image synthesis as a Markov Decision Process.
...
Method: Uses a ground-truth-primed memory and burn-in anchor loss for stable identity propagation, with a single lightweight temporal-attention layer for embedding refinement across frames.

Result: Achieves 76.3 AUC and 53.7 FPS on Mini-LaSOT benchmark, outperforming transformer baselines like TrackFormer and MOTRv2 under fast motion, scale change, and occlusion.

Conclusion: SOTFormer demonstrates effective unified tracking and forecasting with constant memory usage and real-time performance, superior to existing transformer approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing dynamic graph learning approaches in machining process planning fail to incorporate 3D geometric information of parts, lacking domain awareness in predicting machining operation sequences despite their ability to capture spatio-temporal dependencies.

Method: Proposes MP-GFormer, a dynamic graph transformer that integrates evolving 3D geometric representations through attention mechanism, using StereoLithography surface meshes for 3D geometry after each operation and boundary representation for initial designs.

Result: Achieves 24% improvement in main operation prediction accuracy and 36% improvement in sub-operation prediction accuracy compared to state-of-the-art approaches on a synthesized dataset.

Conclusion: The integration of 3D geometric information into dynamic graph learning significantly enhances machining operation sequence prediction, demonstrating the importance of domain-aware geometric representations in machining process planning.
...
<details>
  <summary>Details</summary>
Motivation: To address the intellectual property risks of unauthorized model merging where free-riders combine fine-tuned models without authorization, violating IP rights and undermining model ownership.
...
Result: Reduces merged model accuracy by up to 90% with less than 1.5% performance loss on the protected model, validated on vision (ViT-L-14) and language (Llama2, Gemma2, Mistral) models.

Conclusion: MergeGuard effectively protects models from unauthorized merging by reshaping parameter geometry to cause destructive interference in merged models while preserving original model functionality.
...
<details>
  <summary>Details</summary>
Motivation: Most medical image segmentation models don't explicitly encode boundary information, making boundary preservation a persistent challenge that affects diagnostic precision.
...
Result: Extensive evaluations across diverse datasets (cerebral aneurysm, stroke, liver, breast tumor) and multiple imaging modalities show FocusSDF consistently outperforms existing distance transform based loss functions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional wildlife monitoring methods are resource-intensive and limited by logistical challenges. Deep learning models for detecting sparsely distributed species like muskoxen are constrained by small datasets.

Method: Compared baseline model trained on real imagery with 5 zero-shot and 5 few-shot models that incorporated progressively more synthetic imagery. Zero-shot models used only synthetic data, while few-shot models combined real and synthetic data.

Result: Zero-shot models showed improved detection performance with synthetic imagery, with performance plateauing when synthetic data exceeded 100% of baseline training set. Few-shot models achieved better recall and slightly higher accuracy than real-only models, though improvements weren't statistically significant.

Conclusion: Synthetic imagery shows potential for training accurate object detection models when real data is scarce, enabling monitoring of rare or inaccessible species and increasing monitoring frequency. This approach can initiate models without real data and refine them over time.
...
TL;DR: Harpia is a CUDA-based library integrated into Annotat3D that enables scalable, interactive segmentation of large 3D datasets in HPC environments with strict memory control and GPU acceleration.


<details>
  <summary>Details</summary>
Motivation: High-resolution volumetric imaging techniques generate large datasets that challenge existing tools for efficient processing, segmentation, and interactive exploration.

Method: Developed Harpia with strict memory control, native chunked execution, and GPU-accelerated filtering, annotation, and quantification tools to handle datasets exceeding single-GPU memory capacity.
...
Conclusion: The system's interactive interface with efficient GPU resource management makes it suitable for collaborative scientific imaging workflows in shared HPC infrastructures.
...
TL;DR: Automated prompt optimization using DSPy framework significantly improves medical vision-language model performance, achieving 53% median improvement over zero-shot baselines without model finetuning.


<details>
  <summary>Details</summary>
Motivation: VLMs underperform on medical tasks, and existing solutions like finetuning require large datasets/compute while manual prompt engineering is hard to generalize and inaccessible to medical institutions.

Method: Adapted DSPy framework for automated prompt optimization, testing 10 open-source VLMs across 5 medical imaging tasks with 4 optimization techniques.

Result: 53% median relative improvement over zero-shot baselines, with largest gains of 300-3400% on tasks with poor zero-shot performance. Scalable, privacy-preserving approach.

Conclusion: Automated prompt optimization enables significant performance gains for medical AI systems, reduces dependence on manual prompt design, and allows clinicians to focus on patient care.
...
TL;DR: PI-NAIM is a dual-path architecture that dynamically routes medical data samples to optimal imputation methods based on missingness complexity, combining statistical and neural approaches with cross-path attention fusion.


<details>
  <summary>Details</summary>
Motivation: Medical imaging and multi-modal clinical settings often face missing modality challenges, and existing imputation methods lack either representational capacity or computational efficiency.
...
Conclusion: PI-NAIM provides a unified solution for real-world scenarios with missing data, enabling seamless integration into vision pipelines handling incomplete sensor measurements, missing modalities, or corrupted inputs.
...
TL;DR: QTSplus is a lightweight visual token selection module that dynamically selects important video tokens based on text queries, reducing attention cost and improving long video understanding in MLLMs.
...
Result: Compresses vision stream by 89%, reduces end-to-end latency by 28% on long videos, achieves near-parity accuracy overall, and outperforms original model by +20.5 and +5.6 points on TempCompass direction and order accuracies.
...
TL;DR: First use of event cameras for image dehazing, leveraging their high dynamic range (HDR) capabilities to guide diffusion models for reconstructing clear images from hazy inputs.


<details>
  <summary>Details</summary>
Motivation: Traditional RGB-based dehazing methods suffer from limited dynamic range, making dehazing ill-posed and causing loss of structure and illumination details. Event cameras offer superior HDR (120dB vs 60dB) and microsecond latency, making them ideal for hazy scenes.

Method: Proposed an event-guided diffusion model that transfers HDR information from events to frames. Designed an event-guided module that maps sparse HDR event features (edges, corners) into diffusion latent space to provide precise structural guidance during generation.

Result: Achieved state-of-the-art results on two benchmarks and a newly collected drone dataset in heavy haze (AQI=341) with synchronized RGB and event sensors. The method improves visual realism and reduces semantic drift.

Conclusion: Event cameras combined with diffusion models provide an effective solution for image dehazing by leveraging HDR cues from events to reconstruct clear images while preserving structural details.
...
TL;DR: Comparative analysis of three U-Net variants for semantic segmentation of rock art petroglyphs, showing that attention-enhanced architectures outperform baseline with 2.5-2.9% Dice Score improvements.
...
Method: Three U-Net-based architectures were compared: (1) BEGL-UNet with Border-Enhanced Gaussian Loss, (2) Attention-Residual BEGL-UNet with residual blocks and gated attention, and (3) Spatial Channel Attention BEGL-UNet with CBAM-based attention modules. All used BEGL loss combining binary cross-entropy with Gaussian edge enhancement. Evaluation used 5-fold cross-validation on images from Poço da Bebidinha Archaeological Complex.

Result: Attention-Residual BEGL-UNet achieved best performance with Dice Score 0.710, validation loss 0.067, and recall 0.854. Spatial Channel Attention BEGL-UNet had comparable results (DSC 0.707, recall 0.857). Baseline BEGL-UNet scored DSC 0.690. Attention mechanisms improved Dice Score by 2.5-2.9% over baseline.

Conclusion: Attention mechanisms significantly enhance semantic segmentation performance for archaeological heritage preservation, with Attention-Residual BEGL-UNet emerging as the most effective architecture for rock art petroglyph segmentation.
...
TL;DR: This paper evaluates vision-language models for fine-grained glomerular subtyping under few-shot learning conditions, finding that pathology-specialized VLMs with vanilla fine-tuning perform best even with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Fine-grained glomerular subtyping is clinically important but suffers from scarce labels, and it's unclear how VLMs should be adapted for this task under data constraints.

Method: Systematically evaluated pathology-specialized and general-purpose VLMs for few-shot glomerular subtyping, analyzing classification performance and representation geometry including feature alignment and subtype separability.

Result: Pathology-specialized VLMs with vanilla fine-tuning are most effective, achieving substantial discrimination gains with only 4-8 labeled examples per subtype, though additional supervision provides incremental improvements.

Conclusion: Supervision level and adaptation strategy jointly shape diagnostic performance and multimodal structure, providing guidance for model selection and annotation investment in clinical settings.
...
TL;DR: This paper presents an IPPG method that breaks the constraint of facial close-ups by achieving synergistic optimization of identity fidelity and scene semantic creation through a dual-line inference pipeline and adaptive fusion strategies.
...
Method: The method uses a Dual-Line Inference pipeline with identity-semantic separation, Identity Adaptive Fusion strategy that defers ID-semantic fusion to noise prediction stage, and Identity Aggregation Prepending module to aggregate ID information and replace random initializations.

Result: Experimental results validate stable and effective performance in IPPG tasks beyond facial close-ups, enabling efficient generation without manual masking or fine-tuning.

Conclusion: The proposed method addresses over-reliance on facial close-ups, facilitates film-level character-scene creation, and provides richer personalized generation capabilities as a plug-and-play component for existing IPPG frameworks.
...
<details>
  <summary>Details</summary>
Motivation: Existing transformation-based attacks have three key limitations: (1) misleading performance characterization based only on low-iteration settings, (2) uniform parameter usage across different models/iterations/tasks that impairs transferability, and (3) computationally expensive grid search for parameter optimization.

Method: The authors conducted empirical studies revealing dynamic patterns of transferability, proposed a Concentric Decay Model (CDM) to explain these patterns, and developed Dynamic Parameter Optimization (DPO) based on the rise-then-fall pattern.

Result: Comprehensive experiments across different surrogate models, iterations, and tasks demonstrate that DPO significantly improves transferability of transformation-based attacks while reducing computational complexity.

Conclusion: The proposed DPO method effectively addresses the limitations of existing transformation-based attacks by providing an efficient parameter optimization approach that enhances transferability across various settings.
...
<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of lithography SEM images is crucial for semiconductor process control and yield optimization, but existing methods lack precision and robustness across diverse pattern geometries.

Method: Two-stage approach: coarse stage uses Human-in-the-Loop Bootstrapping with SAM for robust segmentation; fine stage converts 2D segmentation to 1D regression using groove-normal profiles and lightweight MLP for point-wise refinement.

Result: LithoSeg outperforms previous approaches in segmentation accuracy and metrology precision while requiring less supervision.
...
TL;DR: SIT-ADDA-Auto adapts only early convolutional layers for microscopy image domain adaptation, using adversarial alignment and uncertainty to automatically select adaptation depth without target labels.


<details>
  <summary>Details</summary>
Motivation: Deep learning models often fail when applied to microscopy images from new instruments or settings, and conventional domain adaptation methods disrupt learned semantic representations by retraining entire networks.

Method: SIT-ADDA-Auto freezes deeper layers and adapts only the earliest convolutional layers using adversarial alignment with automatic depth selection based on predictive uncertainty, requiring no target labels.
...
Conclusion: Adapting only early layers provides reliable transfer for microscopy domain adaptation, offering a design rule for label-free adaptation and practical deployment in field settings.
...
TL;DR: A multi-camera computer vision framework for real-time traffic safety assessment using Post-Encroachment Time (PET) computation at intersections, achieving sub-second precision and real-time processing on edge devices.
...
Method: Four synchronized cameras provide continuous coverage, with YOLOv11 segmentation for vehicle detection and homography matrices to transform vehicle polygons into a unified bird's-eye map. A novel pixel-level PET algorithm measures vehicle positions without fixed cells.

Result: The framework achieves fine-grained hazard visualization with 3.3 sq-cm accuracy, processes data at 2.68 FPS for 800x800 pixel heatmaps, and identifies high-risk regions with sub-second precision on edge devices.

Conclusion: The study validates decentralized vision-based PET analysis as feasible for intelligent transportation systems, offering a replicable methodology for high-resolution, real-time, and scalable intersection safety evaluation.
...
TL;DR: Proposes LIHE framework for Weakly-Supervised Generalized Referring Expression Comprehension (WGREC) that handles expressions with variable numbers of targets using a two-stage approach with hybrid hyperbolic-Euclidean similarity.


<details>
  <summary>Details</summary>
Motivation: Existing WREC methods are limited by one-to-one mapping assumption and cannot handle expressions with zero or multiple targets in realistic scenarios. The paper aims to bridge this gap with a more practical WGREC paradigm.

Method: LIHE framework with two stages: Referential Decoupling (predicts target count and decomposes expressions) and Referent Grounding (localizes sub-expressions using HEMix - hybrid hyperbolic-Euclidean similarity module).

Result: LIHE establishes the first effective weakly supervised WGREC baseline on gRefCOCO and Ref-ZOM datasets. HEMix improves IoU@0.5 by up to 2.5% on standard REC benchmarks.

Conclusion: The proposed LIHE framework successfully addresses the challenges of WGREC through referential decoupling and hybrid similarity modeling, providing a practical solution for handling expressions with variable numbers of referents.
...
TL;DR: NSDD distills iterative diffusion solvers into a single-pass student model for fast, photorealistic lensless imaging without paired supervision, achieving near-teacher quality with measurement consistency.


<details>
  <summary>Details</summary>
Motivation: To overcome biases from paired lensless-lensed supervision and instability of generic diffusion priors in noisy, ill-posed lensless deconvolution settings.

Method: Separates range-space enforcement from null-space diffusion-prior updates, distilling iterative DDNM+ solver into a single-pass student model conditioned on lensless measurement and range-space anchor.

Result: Second fastest method behind Wiener, achieves near-teacher perceptual quality (second-best LPIPS), outperforms DPS and classical convex baselines on Lensless-FFHQ and PhlatCam datasets.

Conclusion: Provides a practical path toward fast, ground-truth-free, photorealistic lensless imaging with preserved measurement consistency at reduced runtime and memory requirements.
...
TL;DR: VL-SurgPT is the first multimodal surgical tracking dataset that combines visual tracking with textual descriptions of point status, enabling context-aware tracking systems that perform better in challenging surgical conditions.
...
Method: Created VL-SurgPT dataset with 908 in vivo video clips (754 for tissue tracking, 154 for instrument tracking) and proposed TG-SurgPT, a text-guided tracking approach that leverages semantic descriptions to improve robustness.
...
Conclusion: VL-SurgPT bridges visual and linguistic modalities to enable development of context-aware tracking systems crucial for advancing computer-assisted surgery applications under challenging intraoperative conditions.
...
TL;DR: GCAgent is a novel framework for long-video understanding that uses structured episodic memory to capture global context and temporal dependencies, achieving state-of-the-art performance on Video-MME benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with long-video understanding due to token limitations and inability to capture long-term temporal dependencies and complex event relationships.

Method: Proposes GCAgent with Schematic and Narrative Episodic Memory that structurally models events and their relations, operating in a multi-stage Perception-Action-Reflection cycle with Memory Manager for context retrieval.

Result: Achieves 23.5% accuracy improvement on Video-MME Long split over baseline, 73.4% accuracy on Long split, and 71.9% overall average on Video-MME benchmark - state-of-the-art among 7B-scale MLLMs.

Conclusion: The agent-based reasoning paradigm with structured episodic memory effectively addresses long-video understanding challenges and enables cognitively-inspired video comprehension.
...
TL;DR: Proposes a novel framework that jointly integrates visual and physical cues for 3D hand-object pose estimation, overcoming limitations of existing methods that rely solely on visual cues and produce physically implausible results.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D hand-object pose estimation rely mainly on visual cues, often producing results that violate physical constraints like interpenetration or non-contact. Current physics integration approaches use post-optimization or non-differentiable physics engines, compromising visual consistency and end-to-end trainability.

Method: Two key ideas: 1) Joint visual-physical cue learning: training model to extract both 2D visual cues and 3D physical cues for comprehensive representation learning; 2) Candidate pose aggregation: novel refinement process that aggregates multiple diffusion-generated candidate poses using both visual and physical predictions.

Result: Extensive experiments show the method significantly outperforms existing state-of-the-art approaches in both pose accuracy and physical plausibility.

Conclusion: The proposed framework successfully integrates visual and physical cues for hand-object pose estimation, producing results that are both visually consistent and physically plausible, addressing key limitations of previous methods.
...
TL;DR: KA-MIG enhances masked image generation by incorporating explicit token-level semantic dependency knowledge from training data through three knowledge graphs, improving generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing masked image generation methods struggle to learn semantic dependencies directly from data because visual tokens lack clear semantic meaning and sequences are long, limiting performance.

Method: Proposes KA-MIG framework that uses three knowledge graphs (co-occurrence, semantic similarity, position-token incompatibility) and a graph-aware encoder to learn enriched token representations, integrated via lightweight fusion into existing MIG methods.
...
Conclusion: Incorporating explicit token-level semantic dependency knowledge as priors effectively enhances the model's ability to capture semantic dependencies and improves masked image generation quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal learning methods require all modalities to be present, making it challenging to use datasets with missing modalities. The paper identifies an 'anchor shift' problem where missing modalities cause deviations from optimal alignment.

Method: Proposes CalMRL which leverages modality priors and connections to model missing modality imputation at representation level. Uses bi-step learning with closed-form posterior distribution solution for shared latents.

Result: Theoretical validation shows CalMRL mitigates anchor shift and ensures convergence. Extensive experiments demonstrate superiority over existing methods, enabling flexible use of data with missing modalities.

Conclusion: CalMRL provides an effective solution for multimodal representation learning with incomplete data, offering new flexibility to handle missing modalities while maintaining alignment quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D reconstruction from sparse, low-resolution images often fail to recover fine texture details due to inherent lack of high-frequency information in LR inputs.

Method: Uses scene-specific reference gallery generated with MLLMs and diffusion models, Reference-Guided Feature Enhancement module to align/fuse features, and Texture-Aware Density Control to adaptively adjust Gaussian density based on texture richness.

Result: Outperforms existing methods on RealEstate10K, ACID, and DTU datasets, and shows strong cross-dataset and cross-resolution generalization capabilities.

Conclusion: SRSplat effectively addresses texture detail recovery in 3D reconstruction from sparse LR images through joint external reference and internal texture optimization.
...
<details>
  <summary>Details</summary>
Motivation: Non-IID data poses a major challenge for federated learning, especially in medical imaging where data distribution shifts are common. Histopathological images with feature distribution shifts need effective solutions that don't compromise privacy.

Method: Proposed Federated Stain Distribution Alignment (FedSDA) method that uses diffusion models to fit data distributions and stain separation to extract key features. Aligns stain distribution of each client with target distribution in FL framework while avoiding privacy risks.

Result: Extensive experiments show FedSDA effectively improves baselines focusing on mitigating client disparities and outperforms other non-IID data distribution methods. Provides practical insights for computational pathology.

Conclusion: FedSDA successfully addresses non-IID histopathological image challenges through stain distribution alignment, demonstrating effectiveness in federated learning while maintaining privacy protection.
...
TL;DR: DCMM-Transformer incorporates anatomical groupings in medical images using a Degree-Corrected Mixed-Membership model as additive bias in self-attention, overcoming limitations of previous methods like SBM-Transformer.


<details>
  <summary>Details</summary>
Motivation: Standard Vision Transformers fail to exploit latent anatomical groupings in medical images, and existing approaches like SBM-Transformer suffer from non-differentiability, training instability, and inability to model complex community structure.

Method: Proposes DCMM-Transformer that uses Degree-Corrected Mixed-Membership model as additive bias in self-attention, enabling differentiable and interpretable incorporation of community structure and degree heterogeneity without binary sampling.

Result: Superior performance and generalizability demonstrated across diverse medical imaging datasets (brain, chest, breast, ocular), with learned group structure producing anatomically meaningful and semantically coherent attention maps.

Conclusion: DCMM-Transformer effectively incorporates anatomical structure in medical image analysis through differentiable community modeling, enhancing both performance and interpretability compared to existing approaches.
...
TL;DR: DeiTFake is a transformer-based deepfake detection method using DeiT with a two-stage progressive training strategy that achieves 99.22% accuracy and 0.9997 AUROC on OpenForensics dataset.


<details>
  <summary>Details</summary>
Motivation: Deepfakes pose major threats to digital media integrity, requiring robust detection methods to identify subtle manipulation artifacts.

Method: Proposes DeiT-based transformer with two-stage progressive training: initial transfer learning with standard augmentations followed by fine-tuning with advanced affine and deepfake-specific augmentations. Uses knowledge distillation to capture subtle manipulation artifacts.
...
TL;DR: UniABG is an unsupervised cross-view geo-localization framework that uses adversarial view bridging and graph-based calibration to achieve state-of-the-art performance without pairwise annotations.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of supervised methods requiring extensive pairwise annotations and unsupervised methods suffering from noisy pseudo-labels due to cross-view domain gaps.

Method: Dual-stage framework: 1) View-Aware Adversarial Bridging (VAAB) for view-invariant features and robust pseudo-labels, 2) Heterogeneous Graph Filtering Calibration (HGFC) with dual inter-view structure graphs for reliable cross-view correspondence.
...
Conclusion: UniABG effectively addresses cross-view domain gaps in unsupervised geo-localization through adversarial bridging and graph-based calibration, demonstrating superior performance without annotation requirements.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion transformer (DiT) based video generation models suffer from slow inference speeds and high memory consumption, hindering their practical deployment.

Method: Three main innovations: 1) PipeSP pipelining algorithm for sequence parallelism, 2) DeDiVAE to decouple diffusion and VAE modules into separate GPU groups, 3) Attention co-processing (Aco) to optimize GPU resource utilization in VAE group.

Result: PipeDiT achieves 1.06x to 4.02x speedups over OpenSoraPlan and HunyuanVideo across various resolution and timestep configurations on 8-GPU systems.

Conclusion: The proposed PipeDiT framework effectively addresses the performance bottlenecks in video generation models, enabling faster and more efficient deployment of DiT-based video generation systems.
...
TL;DR: MovSemCL is a movement-semantics contrastive learning framework that addresses limitations in trajectory similarity computation by modeling trajectory semantics and hierarchy more effectively while reducing computational costs.
...
Method: MovSemCL transforms GPS trajectories into movement-semantics features, segments them into patches, and uses intra- and inter-patch attentions to encode local and global trajectory patterns. It also employs a curvature-guided augmentation strategy that preserves informative segments while masking redundant ones.

Result: Experiments show MovSemCL outperforms state-of-the-art methods, achieving mean ranks close to the ideal value of 1 at similarity search tasks, improvements by up to 20.3% at heuristic approximation, and reducing inference latency by up to 43.4%.

Conclusion: MovSemCL effectively addresses key limitations in trajectory similarity computation through hierarchical representation and physically plausible augmentations, demonstrating superior performance and efficiency compared to existing methods.
...
TL;DR: DCA-LUT is a deep learning framework for purple fringing removal that uses a Chromatic-Aware Coordinate Transformation module to isolate fringing into a dedicated dimension and a 5D LUT for color correction.


<details>
  <summary>Details</summary>
Motivation: Purple fringing caused by Longitudinal Chromatic Aberration degrades image quality, and traditional solutions rely on expensive hardware and handcrafted features, ignoring data-driven approaches.

Method: Uses Chromatic-Aware Coordinate Transformation to create an image-adaptive color space that isolates fringing into a dedicated dimension, then applies a 5D Look-Up Table for color correction. Trained on a large-scale synthetic purple fringing dataset (PF-Synth).

Result: Achieves state-of-the-art performance in purple fringing removal on both synthetic and real-world datasets.

Conclusion: The proposed DCA-LUT framework effectively removes purple fringing through targeted separation and learned color mapping, outperforming existing methods.
...
TL;DR: VAEmotionLLM is a two-stage framework that teaches visual language models to hear with limited audio pretraining and understand emotion across modalities, achieving state-of-the-art results on art-centric emotion understanding.


<details>
  <summary>Details</summary>
Motivation: Current audio-visual language models require large-scale audio pretraining, limiting scalability, and most prior work overlooks the emotion intentionally expressed by artworks that convey emotion through joint visual and auditory elements.

Method: Two-stage framework: 1) Vision-Guided Audio Alignment (VG-Align) distills visual pathway into audio pathway by aligning next-token distributions; 2) Cross-Modal Emotion Adapter (EmoAdapter) injects emotion-sensitive residuals and applies emotion supervision.
...
Conclusion: VAEmotionLLM enables effective cross-modal emotion understanding with limited audio pretraining, demonstrating the framework's effectiveness for art-centric emotion analysis across modalities.
...
TL;DR: A multimodal prompting-driven quantization framework for point cloud analysis that uses text embeddings as prototype priors and integrates visual and semantic information through dual-constrained quantization.


<details>
  <summary>Details</summary>
Motivation: Current vector quantization methods using trainable vectors or clustered centroids lack representativeness and interpretability, despite the promise of multimodal alignment in vision-language models.

Method: Uses text embeddings from pre-trained models as robust prototype priors, employs multimodal prompts for adaptive refinement, introduces dual-constrained quantization space with compactness and separation regularization, and applies Gumbel-Softmax for differentiable discretization.
...
Conclusion: The proposed framework effectively addresses limitations of current quantization methods by leveraging multimodal alignment and creating hybrid representations that encode both geometric and semantic information.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in multilabel image categorization by modeling label dependencies and uncertainties, which are common in real-world computer vision applications.
...
Result: Achieved 0.794 mAP on COCO-2014, outperforming ResNet-SRN (0.771) and Vision Transformer baselines (0.785), demonstrating improved multilabel classification accuracy.
...
TL;DR: SemanticStitch is a deep learning framework that uses semantic priors to preserve foreground object integrity in image stitching, addressing misalignments and visual discrepancies caused by varying capture conditions.


<details>
  <summary>Details</summary>
Motivation: Traditional image stitching methods struggle with misalignments from varying angles, positions, and object movements, and seam carving approaches ignore semantic information, disrupting foreground continuity.

Method: A deep learning-based framework incorporating semantic priors of foreground objects with a novel loss function that emphasizes semantic integrity of salient objects.

Result: Experimental results show substantial improvements over traditional techniques, with the method evaluated on two specialized real-world datasets.

Conclusion: The approach provides robust support for practical applications by significantly enhancing stitching quality and visual coherence through semantic-aware processing.
...
<details>
  <summary>Details</summary>
Motivation: Existing prompt-based continual learning methods attach independent task-specific prompts to each layer, which provides flexibility but makes layers susceptible to unnecessary updates and increases catastrophic forgetting risk when prompts are aggregated.

Method: Hierarchical layer-grouped prompt tuning where: (i) layers in same group share similar prompts adjusted by position encoding to preserve pre-trained model relationships, (ii) single task-specific root prompt generates sub-prompts for each layer group to enhance synergy.
...
Conclusion: The proposed hierarchical prompt tuning method effectively reduces catastrophic forgetting by improving model stability through shared layer-group prompts and root-prompt conditioned sub-prompt generation.
...
<details>
  <summary>Details</summary>
Motivation: SNNs are energy-efficient alternatives to conventional vision systems and align well with event cameras, but they remain costly to train due to temporal coding, limiting their practical deployment.
...
Conclusion: PACE enables minute-scale SNN training and efficient edge deployment through compact synthetic datasets that maintain high performance while dramatically reducing computational and storage requirements.
...
TL;DR: SpikeNM is the first semi-structured N:M pruning framework for SNNs that learns sparse networks from scratch with at most N non-zeros per M-weight block, achieving hardware-friendly patterns while maintaining accuracy.
...
Method: SpikeNM uses M-way basis-logit parameterization with differentiable top-k sampler to linearize complexity, and eligibility-inspired distillation (EID) to align mask probabilities with spiking dynamics for stable training under high sparsity.

Result: At 2:4 sparsity, SpikeNM maintains or even improves accuracy across mainstream datasets while producing hardware-amenable patterns that complement intrinsic spike sparsity.

Conclusion: SpikeNM successfully bridges the gap between unstructured and structured pruning for SNNs, enabling efficient edge deployment through semi-structured sparsity patterns that balance hardware efficiency and model accuracy.
...
TL;DR: Proposed DGCF framework combines frozen DINOv3 Transformer with trainable CNN for CT synthesis from CBCT/MRI, achieving SOTA performance via cross fusion and perceptual loss.


<details>
  <summary>Details</summary>
Motivation: Existing CNN models lack global semantic understanding while Transformers overfit small medical datasets; need balanced local and contextual representation for medical image translation.

Method: DINOv3-Guided Cross Fusion (DGCF) integrates frozen self-supervised DINOv3 Transformer with trainable CNN encoder-decoder, using learnable cross fusion module and Multi-Level DINOv3 Perceptual (MLDP) loss.

Result: Achieved state-of-the-art performance on SynthRAD2023 pelvic dataset for both MRI→CT and CBCT→CT translation tasks in terms of MS-SSIM, PSNR and segmentation-based metrics.
...
TL;DR: Proposes Adaptive Begin-of-Video Tokens (ada-BOV) to improve long video generation in autoregressive video diffusion models, addressing consistency and motion quality issues in existing stream denoising approaches.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive video diffusion models struggle with generating coherent long videos due to denoising latency, error accumulation in chunk-based methods, and poor consistency/motion dynamics in stream denoising approaches.

Method: Introduces ada-BOV tokens as learnable embeddings that adaptively absorb denoised preceding frames via adaptive-layer-norm modulation, plus refinement strategy for stream denoising that decouples sampling trajectory length from attention window constraints, and disturbance-augmented training noise schedule.
...
Conclusion: The proposed ada-BOV framework effectively addresses key limitations in autoregressive video diffusion models, enabling higher-quality long video generation with better consistency and motion dynamics.
...
TL;DR: SS-CA integrates counterfactual explanations into training to address models' incomplete causal learning by using attribution methods to identify critical regions and augmenting data with natural background replacements.


<details>
  <summary>Details</summary>
Motivation: Models often rely on limited sufficient causes for predictions, making them sensitive to distribution shifts. Attribution methods show that masking critical regions causes model misclassification while humans still recognize targets, revealing insufficient causal dependencies.

Method: Propose Subset-Selected Counterfactual Augmentation (SS-CA) using Counterfactual LIMA to identify minimal spatial region sets whose removal alters predictions. Replace identified regions with natural background and train models jointly on augmented and original samples.

Result: Extensive experiments show SS-CA improves generalization on in-distribution test data and achieves superior performance on out-of-distribution benchmarks like ImageNet-R and ImageNet-S. Also enhances robustness under perturbations including noise.
...
TL;DR: BdSL-SPOTER is a pose-based transformer framework for Bengali Sign Language recognition that achieves 97.92% accuracy with improved efficiency and lower computational costs.


<details>
  <summary>Details</summary>
Motivation: To develop an accurate and efficient recognition system for Bengali Sign Language (BdSL) that can work with limited data and provide practical accessibility solutions for real-world applications.
...
TL;DR: TEMPO is a global, quarterly dataset of building density and height at 37.6m resolution derived from satellite imagery using deep learning, covering 2018-2025.


<details>
  <summary>Details</summary>
Motivation: To enable large-scale monitoring of development patterns and climate impacts for global resilience and adaptation efforts at lower computational cost than existing approaches.

Method: Multi-task deep learning model trained on building footprint/height data paired with quarterly PlanetScope satellite images to predict building density and height.

Result: Achieves 85-88% F1 score on validation, 0.96 five-year trend-consistency score, and captures quarterly changes in built settlements efficiently.

Conclusion: TEMPO provides temporally stable, computationally efficient monitoring of global building development, essential for climate resilience and adaptation planning.
...
TL;DR: DFF-Adapter is a parameter-efficient fine-tuning method for DINOv2 that uses lightweight multi-head LoRA modules to simultaneously perform deepfake detection and fine-grained manipulation type classification, achieving state-of-the-art performance with only 3.5M trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods treat DINOv2 as generic binary classification, overlooking distinct artifacts from different manipulation methods, which limits detection effectiveness.

Method: Proposes DeepFake Fine-Grained Adapter (DFF-Adapter) that incorporates lightweight multi-head LoRA modules into every transformer block of DINOv2, with a shared branch that propagates fine-grained manipulation cues to the authenticity head for multi-task cooperative optimization.
...
Conclusion: The proposed DFF-Adapter effectively enhances deepfake detection by leveraging fine-grained manipulation type classification to improve artifact sensitivity, providing an efficient and powerful solution for deepfake detection.
...
TL;DR: MEMR-Seg introduces multi-round entity-level reasoning for medical image segmentation, addressing limitations of single-round dialogue methods.


<details>
  <summary>Details</summary>
Motivation: Existing medical segmentation methods lack interactivity and multi-round reasoning capabilities, being confined to single-round dialogues.

Method: Proposed MediRound baseline model with Judgment & Correction Mechanism to mitigate error propagation in multi-round segmentation pipeline.

Result: Method effectively addresses MEMR-Seg task and outperforms conventional medical referring segmentation methods.

Conclusion: Multi-round entity-level reasoning segmentation enables more interactive and reasoning-based medical image analysis.
...
TL;DR: RadarMP is a unified method for 3D scene motion perception using 4D mmWave radar that jointly performs radar target detection and motion estimation, achieving reliable performance across adverse weather conditions.


<details>
  <summary>Details</summary>
Motivation: 4D mmWave radar provides all-weather operational capability for autonomous driving, but sparse and noisy radar points lead to imprecise motion perception when optical sensors degrade under adverse conditions.

Method: Jointly models radar target detection and motion estimation in a unified architecture using low-level radar echo signals from consecutive frames, with self-supervised loss functions guided by Doppler shifts and echo intensity.
...
Conclusion: RadarMP enhances perception capabilities for full-scenario autonomous driving systems by providing consistent radar point cloud generation and pointwise 3D scene flow prediction.
...
TL;DR: OAD-Promoter is a novel approach that enhances LLM-based Visual Question Answering by mitigating language bias and improving out-of-distribution generalization through object attribute descriptions, memory knowledge assistance, and optimized prompting.
...
Method: Three-component framework: Object-concentrated Example Generation (OEG) for global/regional visual cues, Memory Knowledge Assistance (MKA) for OOD knowledge retrieval, and OAD Prompt for optimized LLM inference.

Result: Significantly improves LLM-based VQA performance in few-shot/zero-shot settings, achieving new state-of-the-art results.

Conclusion: OAD-Promoter effectively addresses language bias and domain-shift challenges in LLM-based VQA, demonstrating robust performance improvements across different settings.
...
TL;DR: A lightweight C-based runtime for spiking neural network inference on edge devices with optimizations that reduce latency and memory while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: SNNs offer advantages for temporal processing and energy efficiency on resource-constrained hardware, but training and deployment remain challenging.
...
Result: Achieved ~10x speedups on desktop CPU with additional gains from pruning, large memory reductions enabling microcontroller deployment (Arduino Portenta H7), while maintaining functional parity with Python baseline on N-MNIST and ST-MNIST datasets.
...
TL;DR: MAVIS is the first benchmark for evaluating multimodal source attribution systems that handle visual questions, retrieve multimodal evidence, and generate long-form answers with citations.


<details>
  <summary>Details</summary>
Motivation: Existing source attribution work has focused on text-only scenarios and overlooked multimodality, creating a need for comprehensive evaluation of multimodal source attribution systems.

Method: Developed a dataset with 157K visual QA instances with fact-level citations to multimodal documents, created fine-grained automatic metrics for informativeness, groundedness, and fluency, and evaluated various multimodal RAG approaches.

Result: LVLMs with multimodal RAG generate more informative and fluent answers than unimodal RAG, but show weaker groundedness for image documents vs text documents. There's a trade-off between informativeness and groundedness across prompting methods.

Conclusion: Mitigating contextual bias in interpreting image documents is a crucial direction for future research in multimodal source attribution systems.
...
TL;DR: TMKT is a cross-modal training framework that uses Time-step Mixup to bridge the gap between RGB and DVS modalities for spiking neural networks, achieving superior performance in spiking image classification.


<details>
  <summary>Details</summary>
Motivation: Event cameras and SNNs promise energy-efficient vision but face challenges with scarce event data and sparse DVS outputs. Direct knowledge transfer from RGB to DVS underperforms due to substantial modality distribution gaps.

Method: Proposes TMKT with probabilistic Time-step Mixup (TSM) that interpolates RGB and DVS inputs at various time steps. Uses two modality-aware objectives: Modality Aware Guidance (MAG) for per-frame supervision and Mixup Ratio Perception (MRP) for sequence-level mix ratio estimation.

Result: Achieves superior performance in spiking image classification tasks across diverse benchmarks and multiple SNN backbones. Enables smoother knowledge transfer and mitigates modality mismatch during training.

Conclusion: TMKT effectively bridges the modality gap between RGB and DVS for SNNs through temporal interpolation and explicit feature alignment, demonstrating strong performance in energy-efficient visual intelligence tasks.
...
TL;DR: FIA-Edit is an inversion-free image editing framework that uses Frequency-Interactive Attention to achieve high-fidelity edits while preserving source image structure and semantics efficiently.


<details>
  <summary>Details</summary>
Motivation: Existing flow-based inversion-free methods fail to effectively integrate source information, leading to poor background preservation, spatial inconsistencies, and over-editing.

Method: Uses Frequency-Interactive Attention with two components: Frequency Representation Interaction (FRI) for cross-domain alignment via frequency exchange, and Feature Injection (FIJ) to incorporate source-side queries, keys, values, and text embeddings into target cross-attention.

Result: Achieves high-fidelity editing at low computational cost (~6s per 512*512 image), outperforms existing methods in visual quality, background fidelity, and controllability, and successfully extends to clinical applications for medical data augmentation.

Conclusion: FIA-Edit provides an efficient and effective solution for text-guided image editing with superior performance and novel applications in medical imaging.
...
<details>
  <summary>Details</summary>
Motivation: Existing hash center-based methods suffer from random center initialization that ignores inter-class semantic relationships, and two-stage approaches introduce complexity, computational overhead, and performance gaps due to stage-wise discrepancies.
...
Conclusion: CRH effectively addresses limitations of existing hash center methods by enabling end-to-end learning with dynamic center reassignment, achieving superior performance in image retrieval.
...
TL;DR: Proposes Completion-by-Correction paradigm for point cloud completion using topologically complete shape priors and feature-space correction, achieving superior results over traditional Completion-by-Inpainting methods.


<details>
  <summary>Details</summary>
Motivation: Traditional Completion-by-Inpainting methods suffer from structural inconsistencies and topological artifacts due to limited geometric and semantic constraints when synthesizing missing structures from fused latent features.

Method: PGNet framework with multi-stage approach: dual-feature encoding to ground generative prior, synthesizing structurally aligned scaffold, and hierarchical correction for progressive geometric refinement.

Result: Achieves state-of-the-art performance on ShapeNetViPC dataset with 23.5% improvement in Chamfer Distance and 7.1% improvement in F-score compared to baselines.

Conclusion: Completion-by-Correction paradigm provides more robust and structurally consistent point cloud completion by shifting from unconstrained synthesis to guided refinement using shape priors.
...
TL;DR: MixAR introduces a novel framework using mixture training paradigms that leverage discrete tokens as prior guidance for continuous autoregressive modeling to address challenges in continuous latent spaces.


<details>
  <summary>Details</summary>
Motivation: Autoregressive approaches in discrete token spaces discard fine-grained information due to quantization and limited codebook size, while continuous latent spaces offer higher quality but face challenges in efficient modeling due to their vast and unstructured nature.
...
Conclusion: MixAR successfully addresses challenges in continuous autoregressive modeling by leveraging discrete tokens as guidance, with DC-Mix and TI-Mix strategies proving effective for balancing efficiency and quality.
...
TL;DR: MMRINet is a lightweight brain tumor segmentation model using Mamba state-space models for efficient 3D MRI analysis, achieving competitive performance with only ~2.5M parameters.


<details>
  <summary>Details</summary>
Motivation: Address computational limitations in resource-constrained clinical settings where deep 3D networks are too expensive for brain tumor segmentation in multi-parametric MRI.
...
Conclusion: MMRINet provides efficient and accurate brain tumor segmentation suitable for low-resource clinical environments while maintaining strong performance.
...
TL;DR: A novel two-phase cross-view, cross-modal unsupervised domain adaptation framework for driver activity recognition that addresses viewpoint variations and domain shifts, improving accuracy by 50% compared to supervised cross-view methods.


<details>
  <summary>Details</summary>
Motivation: Driver distraction causes traffic accidents, but current deep learning methods struggle with real-world deployment due to camera viewpoint variations and domain shifts across different vehicle configurations.

Method: Two-phase framework: (1) learn view-invariant features using contrastive learning on multi-view data, (2) perform domain adaptation to new modalities using information bottleneck loss without labeled target data.

Result: Improves top-1 accuracy on RGB video data by almost 50% compared to supervised contrastive learning-based cross-view methods, and outperforms unsupervised domain adaptation-only methods by up to 5%.

Conclusion: The joint framework effectively addresses both cross-view generalization and domain adaptation challenges, enabling robust deployment of driver monitoring systems across diverse vehicle environments.
...
TL;DR: Proposes Hierarchical Semantic Learning (HSL) framework for Cross-domain Few-shot Segmentation to address segmentation granularity gaps, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing CD-FSS methods focus on style gaps but ignore segmentation granularity gaps, leading to insufficient semantic discriminability for novel classes in target domains.
...
Conclusion: The proposed HSL framework effectively addresses segmentation granularity gaps in CD-FSS tasks through hierarchical semantic learning.
...
TL;DR: OmniSparse is a training-aware fine-grained sparse attention framework for long-video MLLMs that achieves 2.7x speedup and 2.4x memory reduction while matching full attention performance.


<details>
  <summary>Details</summary>
Motivation: Existing sparse attention methods fail to bridge training-inference gap and lack fine-grained token selection across queries, KV, and heads, leading to suboptimal performance and limited acceleration.

Method: Three adaptive mechanisms: query selection via lazy-active classification, KV selection with head-level dynamic budget allocation, and KV cache slimming based on head-level decoding query patterns.
...
Conclusion: OmniSparse effectively bridges training-inference gap and enables fine-grained sparse attention for long-video MLLMs with significant acceleration and memory efficiency gains.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-view diffusion-based 3D generation methods suffer from shape and texture misalignment across generated views, leading to incomplete geometric details and textural ghosting, with poor robustness to oblique perspective inputs.
...
Result: Achieves leading results in both geometric and texture evaluation metrics across flexible input viewpoints, with more complete geometric details and clean textures.
...
TL;DR: A geometry-guided multi-view diffusion model that uses depth maps, normal maps, and segmentation masks to ensure cross-view consistency and generate high-quality multi-view images with rich details.


<details>
  <summary>Details</summary>
Motivation: To address computational challenges in maintaining cross-view consistency and generating high-resolution outputs in multi-view image generation, which has applications in 3D reconstruction, VR, and AR.

Method: Proposes a multi-view geometry extraction module using depth/normal maps and segmentation masks, decoupled geometry-enhanced attention mechanism, adaptive learning strategy, iterative refinement process, and dynamic geometry intensity adjustment.

Result: Generates images that are consistent across views and rich in detail, improving overall image quality and detail preservation while ensuring realistic results.

Conclusion: The proposed model effectively addresses cross-view consistency and high-resolution generation challenges in multi-view image generation through geometry-guided approaches.
...
TL;DR: AI system for automated traffic violation detection using YOLOv8 and EasyOCR to identify helmet non-compliance, missing rear-view mirrors, and extract license plates with high precision (0.9147) and recall (0.886).


<details>
  <summary>Details</summary>
Motivation: Manual enforcement of helmet laws and vehicle safety standards is resource-intensive and inconsistent, requiring automated solutions to enhance road safety and enforcement efficiency.

Method: Uses YOLOv8 for object detection and EasyOCR for license plate recognition, trained on custom annotated dataset with data augmentation. Includes Streamlit interface for real-time monitoring and advanced image preprocessing for challenging conditions.
...
Conclusion: Presents a practical and effective solution for automated traffic rule enforcement with considerations for real-world deployment, significantly improving road safety through AI-powered monitoring.
...
TL;DR: MoS is a novel fusion paradigm for multimodal diffusion models that uses a learnable token-wise router to create flexible, state-based interactions between modalities, achieving state-of-the-art results with 3B-5B parameters.


<details>
  <summary>Details</summary>
Motivation: To develop a more efficient and flexible fusion method for multimodal diffusion models that can precisely align token-level features with the diffusion trajectory while minimizing computational overhead.

Method: Uses a learnable token-wise router that creates denoising timestep- and input-dependent interactions between modalities' hidden states, employing sparse top-k selection and ε-greedy training strategy.

Result: Achieves state-of-the-art results in text-to-image generation and editing, with 3B-5B parameter models matching or surpassing counterparts up to 4× larger.

Conclusion: MoS establishes a flexible and compute-efficient paradigm for scaling multimodal diffusion models with minimal learnable parameters and negligible computational overhead.
...
TL;DR: FaNe is a semantic-enhanced vision-language pre-training framework that addresses false negatives from semantically similar texts and improves fine-grained cross-modal alignment in medical imaging.


<details>
  <summary>Details</summary>
Motivation: Existing medical VLP methods suffer from false negatives caused by semantically similar texts and lack sufficient fine-grained cross-modal alignment, limiting their effectiveness.

Method: Proposes semantic-aware positive pair mining with adaptive normalization, text-conditioned sparse attention pooling for localized visual-text alignment, and hard-negative aware contrastive loss for intra-modal discrimination.

Result: Achieves state-of-the-art performance on five medical imaging benchmarks across image classification, object detection, and semantic segmentation tasks.
...
TL;DR: Spectral Representation Filtering (SRF) is a training-free method that reduces hallucinations in vision-language models by analyzing and filtering hallucination modes in feature representations without inference overhead.
...
Method: SRF identifies low-rank hallucination modes through eigendecomposition of covariance differences between truthful and hallucinatory caption features, then applies a soft spectral filter to attenuate these modes in deeper VLM layers' projection weights.
...
Conclusion: SRF provides an effective post-hoc solution to suppress hallucinations in VLMs that requires no training, incurs zero inference overhead, and works without architectural modifications.
...
TL;DR: DHMI is a diffusion-based model inversion attack framework that reconstructs high-quality training images from deep hashing models, revealing serious privacy risks in binary code retrieval systems.


<details>
  <summary>Details</summary>
Motivation: Deep hashing introduces severe privacy risks by enabling potential reconstruction of original training data from hash codes, but model inversion attacks specifically targeting deep hashing models remain unexplored due to the inaccessibility of genuine training hash codes and the discrete Hamming space.
...
Result: Experiments show DHMI successfully reconstructs high-resolution, high-quality images even under the most challenging black-box setting where no training hash codes are available, outperforming existing state-of-the-art model inversion attacks in black-box scenarios.

Conclusion: DHMI demonstrates practical efficacy and confirms critical privacy risks inherent in deep hashing systems, highlighting the need for privacy protection in binary code retrieval applications.
...
TL;DR: Fusionista2.0 is a streamlined video retrieval system optimized for speed and usability, reducing retrieval time by up to 75% while improving accuracy and user satisfaction.


<details>
  <summary>Details</summary>
Motivation: To meet the Video Browser Showdown's demand for accurate results under strict time constraints by creating a fast and user-friendly video retrieval system.

Method: Re-engineered core modules: ffmpeg for keyframe extraction, Vintern-1B-v3.5 for multilingual OCR, faster-whisper for real-time ASR, and lightweight vision-language models for QA. Also redesigned UI for better responsiveness and workflow.

Result: Retrieval time reduced by up to 75% while accuracy and user satisfaction both increased.

Conclusion: Fusionista2.0 is confirmed as a competitive and user-friendly system for large-scale video search.
...
TL;DR: A prompt-conditioned framework using MedSigLIP with FiLM and multi-scale pooling for LDCT image quality assessment, achieving state-of-the-art results on LDCTIQA2023 challenge.


<details>
  <summary>Details</summary>
Motivation: To enable data-efficient learning and rapid adaptation for medical image quality assessment by conditioning patch-token features on clinical intent through text prompts.

Method: Built on MedSigLIP with Feature-wise Linear Modulation (FiLM) to inject textual priors, combined with multi-scale pooling (global, local, texture-aware) through separate regression heads fused by lightweight MLP, trained with pairwise ranking loss.
...
Conclusion: The prompt-guided approach effectively enables data-efficient learning and demonstrates superior performance in LDCT image quality assessment.
...
TL;DR: A dual-stage disease-aware framework for chest X-ray report generation that learns disease-aware semantic tokens and integrates them with visual features to improve clinical accuracy and linguistic quality.


<details>
  <summary>Details</summary>
Motivation: Existing approaches lack sufficient disease-awareness in visual representations and adequate vision-language alignment for medical image analysis, often overlooking critical pathological features and struggling to generate clinically accurate reports.

Method: Stage 1: Learns Disease-Aware Semantic Tokens (DASTs) through cross-attention and multi-label classification, while aligning vision and language via contrastive learning. Stage 2: Uses Disease-Visual Attention Fusion (DVAF) to integrate disease-aware representations with visual features, and Dual-Modal Similarity Retrieval (DMSR) to retrieve relevant exemplars for contextual guidance.

Result: Achieves state-of-the-art performance on benchmark datasets (CheXpert Plus, IU X-ray, MIMIC-CXR) with significant improvements in clinical accuracy and linguistic quality.

Conclusion: The proposed disease-aware framework effectively addresses limitations of existing approaches by incorporating disease-specific awareness and better vision-language alignment, leading to more clinically accurate chest X-ray report generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing video benchmarks focus on single-video analysis and cannot assess models' ability to reason across multiple videos simultaneously, which is crucial for real-world video understanding scenarios.

Method: Created CrossVid benchmark with 4 high-level dimensions and 10 specific tasks, providing 5,331 videos and 9,015 challenging question-answering pairs in single-choice, multiple-choice, and open-ended formats.

Result: Gemini-2.5-Pro performed best with 50.4% average accuracy, but most MLLMs struggle with cross-video reasoning due to inability to integrate and compare evidence across multiple videos.

Conclusion: CrossVid effectively reveals limitations in current MLLMs' cross-video reasoning capabilities and provides a foundation for future improvements in multimodal video understanding.
...
TL;DR: ZoomEarth is an adaptive cropping-zooming framework for ultra-high-resolution remote sensing images that uses active perception to revisit information-rich regions, achieving state-of-the-art performance and demonstrating versatility across multiple downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for ultra-high-resolution remote sensing images suffer from redundancy when processing finer visual inputs due to passive perception paradigms, limiting their effectiveness in extracting meaningful information from rich fine-grained data.

Method: Proposed ZoomEarth framework with adaptive cropping-zooming and Region-Guided reward, trained via supervised fine-tuning and Group Relative Policy Optimization on the LRS-GRO benchmark dataset containing 17 question types across global, region, and object levels.

Result: Achieves state-of-the-art performance on LRS-GRO and zero-shot performance on three public UHR remote sensing benchmarks. Successfully integrates with downstream models for cloud removal, denoising, segmentation, and image editing tasks.

Conclusion: The active perception paradigm with adaptive cropping-zooming enables effective processing of ultra-high-resolution remote sensing images, demonstrating strong versatility and extensibility across various applications through simple tool interfaces.
...
TL;DR: TM-UNet is a lightweight medical image segmentation framework that uses token sequence modeling with memory mechanisms to achieve efficient global reasoning with linear complexity, outperforming state-of-the-art methods with reduced computation.


<details>
  <summary>Details</summary>
Motivation: Transformer-based methods achieve good results in medical image segmentation but have high computational costs that hinder clinical deployment, necessitating more efficient alternatives.

Method: Proposes TM-UNet with multi-scale token-memory (MSTM) blocks that transform 2D spatial features into token sequences using spatial scanning, matrix memory cells for selective information retention, exponential gating for token effectiveness, and parallel pooling for multi-scale contextual extraction.
...
Conclusion: TM-UNet provides an efficient solution for medical image segmentation by integrating token sequence modeling with memory mechanisms, enabling effective global reasoning with linear complexity suitable for clinical deployment.
...
TL;DR: D³ToM is a dynamic token merging method that accelerates Diffusion MLLMs by selectively merging redundant visual tokens during denoising steps, reducing computational complexity while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Diffusion MLLMs suffer from slow inference due to full bidirectional self-attention over thousands of visual tokens, resulting in cubic decoding complexity that becomes impractical for real-world applications.

Method: Uses decider tokens from previous denoising steps to create importance maps, maintains salient tokens and merges redundant ones through similarity-based aggregation. Features dynamic merge ratios that vary with each denoising step and integrates as a plug-and-play module without altering model parameters.

Result: Extensive experiments demonstrate that D³ToM significantly accelerates inference while preserving competitive performance compared to standard Diffusion MLLMs.

Conclusion: D³ToM effectively addresses the computational bottleneck in Diffusion MLLMs through dynamic token merging, enabling faster inference without compromising model performance, making diffusion-based multimodal models more practical for real-world deployment.
...
TL;DR: A multi-modal extrinsic calibration framework for event cameras, LiDARs, and RGB cameras using a novel 3D calibration target that enables one-shot joint calibration across all three sensing modalities.


<details>
  <summary>Details</summary>
Motivation: To address the challenging calibration of event cameras and enable precise multi-sensor alignment for autonomous driving systems, overcoming limitations of existing pairwise calibration approaches.

Method: Uses a novel 3D calibration target with features specifically designed for each modality (planes for LiDARs, ChArUco for RGB cameras, active LED patterns for event cameras) to enable simultaneous perception and joint extrinsic calibration.

Result: Validated through extensive experiments on a custom autonomous driving dataset, demonstrating accurate and robust calibration performance across all three sensor types.

Conclusion: The proposed framework provides an effective one-shot solution for multi-modal extrinsic calibration, particularly addressing the challenging event camera calibration problem in autonomous driving applications.
...
TL;DR: FreRec addresses frequency misalignment in generative data augmentation for medical AI by recalibrating high-frequency components to improve downstream classification performance.


<details>
  <summary>Details</summary>
Motivation: Generative data augmentation in medical AI suffers from frequency misalignment between real and synthesized images, introducing detrimental features that harm downstream tasks.

Method: FreRec uses Statistical High-frequency Replacement (SHR) for rough alignment and Reconstructive High-frequency Mapping (RHM) to enhance image quality and reconstruct high-frequency details.
...
Conclusion: FreRec is a standalone post-processing method compatible with any generative model that effectively reduces frequency distributional discrepancy in medical GDA pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Current GS-based LiDAR rendering methods suffer from artifacts in extrapolated novel views due to incomplete reconstruction from single traversal scans.

Method: Uses controllable LiDAR generation model with diffusion priors conditioned on coarsely extrapolated rendering to produce geometry-consistent scans, with effective distillation for expansive reconstruction.
...
Conclusion: LiDAR-GS++ successfully addresses extrapolation artifacts while preserving detailed scene surfaces, enabling high-fidelity LiDAR reconstruction with global geometric consistency.
...
TL;DR: A framework that adds temporal reasoning to standard classifiers without architectural changes, using a Support-Exemplar-Query learning paradigm and soft-DTW loss for temporal alignment.


<details>
  <summary>Details</summary>
Motivation: Real-world visual data evolves gradually over time, but conventional classifiers assume temporal independence, limiting their ability to capture temporal dynamics.

Method: Support-Exemplar-Query (SEQ) learning paradigm structures training data into temporally coherent trajectories, learns class-specific temporal prototypes, and uses differentiable soft-DTW loss for sequence alignment with multi-term objective for semantic consistency and temporal smoothness.

Result: Enhances performance on fine-grained and ultra-fine-grained image classification, and delivers precise, temporally consistent predictions in video anomaly detection.
...
TL;DR: A training-free framework that models negation as a subspace in VLM embedding space, improving negation understanding by 30% while preserving zero-shot performance.
...
Method: Divide VLM embedding space into semantically consistent subspaces, model negation as subspace rather than single point, construct spherical caps around embeddings of A and N, score images by central direction close to A and far from N.

Result: Improves negation understanding by about 30% on average over prior methods, closes gap between affirmative and negated prompts while preserving zero-shot performance.

Conclusion: Proposed training-free framework effectively handles negation in VLMs without compromising zero-shot capabilities.
...
TL;DR: Back-projecting vehicle detections from infrastructure cameras to ground plane improves turning movement count accuracy compared to image plane analysis, with multi-camera fusion providing even better results.


<details>
  <summary>Details</summary>
Motivation: Accurate turning movement counts at intersections are crucial for signal control, traffic management and urban planning, but current computer vision systems rely on image plane analysis which may be suboptimal.

Method: Back-projecting vehicle detections from one or more infrastructure cameras to the ground plane for analysis in real-world 3D coordinates, with weak fusion of detections from multiple cameras.

Result: Single-camera back-projection yields more accurate trajectory classification and turning movement counts than image plane analysis, and multi-camera fusion achieves even higher accuracy.

Conclusion: Traffic analysis should be performed on the ground plane rather than the image plane for better accuracy in turning movement counting.
...
TL;DR: CLAReSNet is a hybrid CNN-transformer architecture for hyperspectral image classification that integrates multi-scale convolutional feature extraction with transformer-style attention via an adaptive latent bottleneck, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral image classification faces challenges including high spectral dimensionality, complex spectral-spatial correlations, limited training samples, and severe class imbalance. Existing methods using isolated CNNs or transformers yield suboptimal results due to quadratic complexity and insufficient inductive biases.

Method: Proposes CLAReSNet with multi-scale convolutional stem with deep residual blocks and enhanced Convolutional Block Attention Module for spatial features, followed by spectral encoder layers combining bidirectional RNNs with Multi-Scale Spectral Latent Attention (MSLA) that reduces complexity from O(T²D) to O(Tlog(T)D) via adaptive latent token allocation.

Result: Achieved state-of-the-art performance with overall accuracies of 99.71% on Indian Pines and 99.96% on Salinas datasets, significantly surpassing HybridSN, SSRN, and SpectralFormer. Learned embeddings show superior inter-class separability and compact intra-class clustering.

Conclusion: CLAReSNet effectively addresses hyperspectral image classification challenges under limited samples and severe class imbalance through its hybrid architecture that combines the strengths of CNNs and transformers while mitigating their individual limitations.
...
<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detection methods lack explainability, and while MLLMs are being used to provide explanations, their ability to evaluate these explanations hasn't been properly studied.
...
Result: Best reward model scored 88.76% on the benchmark, while human inter-annotator agreement reached 98.30%, showing a significant performance gap.

Conclusion: There remains a visible gap between current MLLMs' reasoning abilities and human-level performance in judging explanations for AI-generated image detection.
...
TL;DR: DT-R1 is a reinforcement learning framework that trains LLMs to create digital twin representations of visual inputs for unified visual reasoning across multiple tasks and modalities.


<details>
  <summary>Details</summary>
Motivation: Existing visual reasoning approaches require task-specific architectures and training, preventing unified solutions and limiting cross-task generalization.

Method: Uses reinforcement learning (GRPO) with a novel reward function to train LLMs to construct digital twin representations of visual inputs and reason over them.

Result: Achieves consistent improvements over state-of-the-art task-specific models across six visual reasoning benchmarks covering two modalities and four task types.
...
TL;DR: FastReasonSeg is a distillation method that compresses reasoning segmentation models using digital twin representations, achieving SOTA performance with 0.6B parameters while outperforming 20x larger models and enabling real-time deployment.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning segmentation methods require large multimodal models that exceed computational capabilities of edge devices where embodied AI systems deploy. Current distillation approaches fail to transfer multi-step reasoning capabilities needed for reasoning segmentation.

Method: Uses digital twin representations to decouple perception from reasoning. Employs two-stage distillation: supervised fine-tuning on teacher-generated reasoning chains, followed by reinforcement fine-tuning with joint rewards for segmentation accuracy and reasoning quality alignment.

Result: Achieves state-of-the-art reasoning segmentation performance on four benchmarks (JiTBench, RVTBench, ReasonSeg, LLM-Seg40K). The 0.6B variant outperforms models with 20x more parameters while achieving 7.79 FPS throughput with only 2.1GB memory consumption.

Conclusion: FastReasonSeg enables efficient deployment of reasoning segmentation in resource-constrained environments for real-time applications, making embodied AI systems more practical for edge devices.
...
<details>
  <summary>Details</summary>
Motivation: Existing online SCD methods are significantly less accurate than offline approaches and need improvements in accuracy and efficiency.

Method: Self-supervised fusion loss for multi-cue change inference, PnP-based fast pose estimation, and change-guided update strategy for 3D Gaussian Splatting scene representation.

Result: Achieves state-of-the-art performance, surpassing even the best offline approaches, with over 10 FPS operation on complex real-world datasets.

Conclusion: The approach successfully addresses the challenges of online SCD by combining efficient pose estimation, self-supervised learning, and optimized scene representation updates.
...
TL;DR: The paper introduces a reasoning-based text-to-video retrieval method that handles implicit queries requiring reasoning, using digital twin representations and large language models to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-video retrieval methods fail with implicit queries that require reasoning, as they only handle explicit descriptions of visual content.
...
Result: Achieves 81.2% R@1 on ReasonT2VBench-135 (outperforming strongest baseline by >50 percentage points) and 81.7% R@1 on ReasonT2VBench-1000, while establishing SOTA results on MSR-VTT, MSVD, and VATEX benchmarks.

Conclusion: The proposed reasoning text-to-video retrieval paradigm successfully handles implicit queries through structured digital twin representations and LLM reasoning, significantly outperforming existing methods.
...
TL;DR: AGGRNet framework improves medical image classification by extracting both informative and non-informative features to better distinguish subtle classes and handle inter-class similarity and intra-class variability.


<details>
  <summary>Details</summary>
Motivation: Existing attention-based models struggle with complex medical image analysis tasks due to difficulties in distinguishing subtle classes, capturing inter-class similarity, and handling intra-class variability, leading to incorrect diagnoses.
...
Conclusion: AGGRNet effectively addresses challenges in complex medical image analysis by capturing fine-grained visual patterns through dual feature extraction, leading to improved classification performance.
...
TL;DR: Hybrid quantum-classical framework using ResNet50 encoder and Quantum CNN achieves 99% accuracy in diagnosing kidney stones, cysts, and tumors from CT images.


<details>
  <summary>Details</summary>
Motivation: To improve medical diagnosis of kidney conditions (stones, cysts, tumors) using quantum computing advantages combined with classical deep learning for enhanced feature extraction and classification.

Method: Combines pretrained ResNet50 encoder with Quantum CNN, using denoising and histogram equalization for preprocessing, angle encoding for qubit transformation, and addressing class imbalance through data augmentation.

Result: Both 8-qubit and 12-qubit configurations achieved 0.99 test accuracy, with 12-qubit model showing perfect recall for cysts and 0.9956 F1-score for tumors, demonstrating reliable classification across all classes.

Conclusion: Integration of classical preprocessing and deep feature extraction with quantum circuits significantly enhances medical diagnostic performance for kidney condition classification.
...
TL;DR: A framework that disentangles aleatoric and epistemic uncertainty in deep feature space to guide inference-time model selection, reducing compute by ~60% with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Most estimators collapse uncertainty modes into a single score, preventing reliable reasoning about when to allocate more compute or adjust inference.

Method: Uncertainty-Guided Inference-Time Selection framework using regularized global density model for aleatoric uncertainty, and three complementary components for epistemic uncertainty (local support deficiency, manifold spectral collapse, cross-layer feature inconsistency) without sampling, ensembling or extra forward passes.

Result: 60% compute reduction on MOT17 with negligible accuracy loss; tighter prediction intervals at matched coverage; 13.6 percentage points improvement in computational savings over total-uncertainty baseline.

Conclusion: The orthogonal uncertainty decomposition enables practical self-regulating visual inference with significant computational savings while maintaining accuracy.
...
TL;DR: MSLoRA is a parameter-efficient adapter that reweights feature responses in vision backbones using low-rank projections and multi-scale nonlinear transformations, working across both CNNs and ViTs with less than 5% parameter overhead.
...
Method: Combines low-rank linear projection with multi-scale nonlinear transformation to jointly modulate spatial and channel attention, fused through pointwise multiplication and residual connection while keeping pretrained weights frozen.

Result: Consistently improves transfer performance on classification, detection, and segmentation tasks with less than 5% of backbone parameters, enabling stable optimization and fast convergence.
...
Method: Integrates frozen vision-language models for open-world detection/segmentation, uses Q-Former bottleneck to bridge perception-action domains, and employs vision-action contrastive learning to align embeddings for action retrieval.
...
Conclusion: The proposed VLA-R framework effectively addresses open-world autonomous driving challenges by combining vision-language perception with vision-action retrieval, enabling robust performance in unfamiliar environments.
...
TL;DR: PROBE is a self-supervised framework for automated pavement defect detection that uses visual probing of target domains without labels, achieving robust zero-shot transfer and improved domain adaptation.


<details>
  <summary>Details</summary>
Motivation: Automated pavement defect detection faces poor cross-domain generalization issues, with supervised methods requiring costly re-annotation and standard self-supervised methods being vulnerable to domain shifts.

Method: Introduces Self-supervised Prompt Enhancement Module (SPEM) to derive defect-aware prompts from unlabeled target data, and Domain-Aware Prompt Alignment (DAPA) objective to align prompt-conditioned source and target representations.
...
TL;DR: A rotation-only optimization framework for Structure from Motion that represents translation in terms of rotation, enabling pose optimization on the rotation manifold with improved accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: To explore the relationship between scene structures, rotation and translation in SfM, building on pose-only imaging geometry that has shown better performance through pose adjustment.

Method: Proposes a rotation-only optimization framework based on reprojection error that condenses imaging geometry representation onto the rotation manifold, working for both two-view and multi-view scenarios.

Result: Demonstrates superior accuracy and robustness over current state-of-the-art rotation estimation methods, comparable to multiple bundle adjustment iteration results.

Conclusion: This work contributes to more accurate, efficient and reliable 3D visual computing by leveraging the rotation-only perspective in Structure from Motion.
...
<details>
  <summary>Details</summary>
Motivation: Real-world images are often degraded by weather, and existing restoration methods sacrifice high-frequency details crucial for small object detection. Cascading restoration and super-resolution creates conflicts between noise removal and detail enhancement.
...
Result: Extensive experiments show DHGM achieves superior performance over existing methods with lower computational costs.

Conclusion: DHGM successfully bridges the conflict between weather removal and super-resolution, generating clean high-resolution images while preserving critical high-frequency details for visual tasks.
...
TL;DR: MFI-ResNet improves ResNet's parameter efficiency and accuracy by replacing multi-layer residual blocks with MeanFlow modules in a compression-expansion strategy.


<details>
  <summary>Details</summary>
Motivation: To leverage generative flow-fields from MeanFlow to characterize ResNet's feature transformations, bridging generative modeling and discriminative learning for better efficiency.

Method: Compression phase simplifies ResNet stages to 1-2 MeanFlow modules; expansion phase selectively incubates first three stages to match baseline while keeping last stage in MeanFlow form, followed by fine-tuning.

Result: Reduces parameters by 46.28% on CIFAR-10 and 45.59% on CIFAR-100 vs ResNet-50, while improving accuracy by 0.23% and 0.17% respectively.

Conclusion: Generative flow-fields can effectively characterize ResNet's feature transformations, providing new insights into the relationship between generative and discriminative learning.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models have high computational demands, and while visual token pruning has been studied for autoregressive VLMs, it remains unexplored for Diffusion VLMs which enable parallel token decoding but suffer from large visual token overhead.

Method: RedVTP estimates visual token importance using attention from masked response tokens and prunes less important tokens after the first inference step, leveraging consistent importance scores across steps.

Result: RedVTP improves token generation throughput by up to 186% for LLaDA-V and 28.05% for LaViDa, and reduces inference latency by up to 64.97% and 21.87% respectively, without compromising accuracy.

Conclusion: The proposed RedVTP strategy effectively improves inference efficiency of Diffusion VLMs through response-driven visual token pruning, demonstrating significant speed improvements while maintaining or even improving model accuracy.
...
TL;DR: UP-Fusion is a unified multi-modality image fusion framework that uses channel perturbation and pre-trained knowledge to overcome gradient conflicts and improve generalization across different fusion tasks.


<details>
  <summary>Details</summary>
Motivation: Existing unified models for multi-modality image fusion suffer from gradient conflicts due to large modality differences, while modality-specific encoders reduce generalization across different fusion tasks.

Method: Proposes three modules: Semantic-Aware Channel Pruning Module (SCPM) to filter and enhance features using pre-trained knowledge, Geometric Affine Modulation Module (GAM) to maintain modal discriminability, and Text-Guided Channel Perturbation Module (TCPM) to reshape channel distribution during decoding.
...
Conclusion: UP-Fusion effectively addresses gradient conflicts and generalization issues in multi-modality image fusion through channel perturbation and pre-trained knowledge integration, achieving superior performance.
...
TL;DR: Real-time driver drowsiness detection system using deep convolutional neural networks and OpenCV to analyze facial landmarks like eye openings and mouth movements, achieving high accuracy on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Long drives can cause driver fatigue and drowsiness, which poses serious safety risks. A real-time detection system is needed to prevent accidents caused by drowsy driving.

Method: Uses OpenCV for real-time facial image capture and landmark detection (eyes and mouth), combined with pre-trained DCNNs to analyze facial characteristics for drowsiness detection.
...
Conclusion: The proposed system provides a non-invasive, cost-effective solution for real-time driver drowsiness detection that can be integrated into smart car technology to enhance road safety.
...
TL;DR: CoTBox-TTT is a test-time training method that adapts vision-language models at inference by updating soft prompts, using visual chain-of-thought to identify relevant regions and ensure answer consistency, improving medical VQA performance without retraining.


<details>
  <summary>Details</summary>
Motivation: Current medical VQA systems fail under domain shift and produce weakly grounded answers due to attending to spurious regions, especially when retraining or additional labels are impractical at deployment.

Method: Updates only continuous soft prompts while keeping all backbones frozen. Uses visual chain-of-thought to identify question-relevant regions and encourages answer consistency between original image and localized crop. Label-free and plug-and-play.

Result: Increases closed-ended accuracy by 12.3% on pathVQA when added to LLaVA. Practical for real deployments and works with diverse backbones.

Conclusion: CoTBox-TTT provides an effective test-time training approach that improves medical VQA reliability under domain shift without requiring retraining or additional labels.
...
TL;DR: MOON2.0 is a dynamic modality-balanced multimodal framework that addresses modality imbalance, underutilized alignment relationships, and data noise in e-commerce product understanding through Modality-driven MoE, Dual-level Alignment, and MLLM-based Image-text Co-augmentation.


<details>
  <summary>Details</summary>
Motivation: Address three key challenges in e-commerce multimodal understanding: modality imbalance from mixed training, underutilization of intrinsic visual-textual alignment within products, and limited handling of noisy e-commerce multimodal data.

Method: Proposes MOON2.0 framework with: (1) Modality-driven Mixture-of-Experts for adaptive processing by modality composition; (2) Dual-level Alignment to leverage semantic alignment; (3) MLLM-based Image-text Co-augmentation with Dynamic Sample Filtering for data quality improvement.
...
TL;DR: DenseAnnotate is an audio-driven annotation platform that enables efficient creation of dense, fine-grained annotations for images and 3D assets through voice narration, addressing limitations of traditional text-based annotation methods.


<details>
  <summary>Details</summary>
Motivation: Current training datasets for multimodal LLMs rely on sparse annotations from internet mining or manual typing, which capture only a fraction of visual content. Dense annotations are more valuable but scarce, especially for specialized domains like multicultural imagery and 3D assets.

Method: An audio-driven online platform where annotators narrate observations aloud while synchronously linking spoken phrases to image regions or 3D scene parts, incorporating speech-to-text transcription and region-of-attention marking.

Result: Created a dataset of 3,531 images, 898 3D scenes, and 7,460 3D objects with audio-aligned dense annotations in 20 languages. Models trained on this dataset showed 5% improvement in multilingual, 47% in cultural alignment, and 54% in 3D spatial capabilities.

Conclusion: The platform offers a feasible approach for future vision-language research and can be applied to various tasks and diverse data types, demonstrating significant improvements in model performance across multiple domains.
...
TL;DR: A framework combining LLMs and grid-based integer programming for automated interior design that jointly optimizes room layout and furniture placement from textual prompts.


<details>
  <summary>Details</summary>
Motivation: To create an automated interior design system that can interpret natural language prompts and generate optimal room layouts and furniture arrangements while considering various design constraints.

Method: Uses LLMs to extract design constraints from text prompts, encodes them into a grid-based representation, and applies integer programming with a coarse-to-fine optimization strategy for computational efficiency.

Result: The approach significantly outperforms existing two-stage design pipelines in solution quality and achieves notable computational efficiency through the coarse-to-fine strategy.

Conclusion: The joint optimization framework effectively combines LLM-driven constraint extraction with mathematical optimization to produce high-quality interior designs from textual descriptions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional image masking underutilizes discarded pixels and may remove critical features, while MIM shows masked regions contain valuable contextual information and semantic diversity.

Method: Proposes MaskAnyNet with a relearning mechanism that adds an additional branch to any model to jointly learn from recomposed masked regions, treating masked content as auxiliary knowledge.

Result: Experiments on CNN and Transformer backbones show consistent performance gains across multiple benchmarks, with improved semantic diversity through masked content reuse.

Conclusion: Masked regions should be treated as valuable auxiliary knowledge rather than discarded, and the proposed approach effectively leverages semantic diversity to enhance feature learning and preserve fine details.
...
TL;DR: C3DFusion is a novel temporal fusion module for 3D semantic scene completion that enhances reconstruction of out-of-frame areas by aligning 3D features from current and historical frames using context blurring and feature densification techniques.


<details>
  <summary>Details</summary>
Motivation: Existing camera-based 3D SSC methods struggle to reconstruct critical out-of-frame areas near ego-vehicle sides, despite historical frames containing valuable contextual information about these unseen regions.
...
Conclusion: C3DFusion effectively addresses the limitation of reconstructing out-of-frame areas in 3D semantic scene completion through enhanced temporal fusion, achieving superior performance and strong generalization capabilities.
...
TL;DR: A neural network-based approach for camera relocalization that directly maps images to visible 3D structure points, eliminating the need for image retrieval or search heuristics.


<details>
  <summary>Details</summary>
Motivation: Existing structure-based relocalization methods require elaborate pipelines with search heuristics or image retrieval, leading to computational complexity and storage requirements that grow with scene size.

Method: Proposes a visible structure retrieval network that learns a direct mapping from query images to the subset of visible 3D structure points in the map, reducing 2D-3D correspondence search space.
...
Conclusion: The neural network-based paradigm enables efficient structure-based relocalization by directly retrieving visible scene structure, offering a more compact and scalable alternative to traditional approaches.
...
<details>
  <summary>Details</summary>
Motivation: Address performance degradation of AIGI detectors under real-world motion blur, which distorts textures and suppresses artifacts in handheld photography and video.

Method: Teacher-student knowledge distillation with frozen DINOv3 teacher trained on clean images, distilling features and logits to student trained on blurred images for consistent representations under motion degradation.
...
Conclusion: The proposed framework effectively addresses motion blur challenges in AIGI detection through knowledge distillation, enabling robust performance in real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail to address visible image degradation under adverse weather conditions and use fixed architectures that lack adaptability to diverse degradation scenarios.

Method: Uses mixture-of-experts system with pre-trained vision-language model for semantic priors, degradation-aware channel attention module with prototype decomposition, and expert routing guided by semantic priors and channel features.

Result: Extensive experiments show superior performance over state-of-the-art methods in complex degradation scenarios.

Conclusion: MdaIF framework effectively handles multi-degradation scenarios through degradation-aware fusion guided by semantic priors and adaptive expert routing.
...
TL;DR: D²-VPR is a distillation- and deformable-based framework for Visual Place Recognition that reduces model complexity while maintaining competitive performance through knowledge distillation and adaptive feature aggregation.


<details>
  <summary>Details</summary>
Motivation: To address the high computational overhead and model complexity of visual foundation models like DINOv2 in VPR applications, especially for deployment on resource-constrained devices.
...
Conclusion: The proposed framework successfully balances performance and efficiency, making powerful VPR models more deployable on resource-constrained devices.
...
TL;DR: ReaSon is a framework that uses reinforcement learning to select keyframes for video understanding by optimizing for both predictive sufficiency and causal necessity through a novel Causal Information Bottleneck approach.


<details>
  <summary>Details</summary>
Motivation: Keyframe selection is crucial for video understanding with VLMs due to limited input tokens and temporal sparsity of relevant information across video frames. Current methods need frames that are both informative and causally decisive.

Method: Proposes Reinforced Causal Search with Information Bottleneck (ReaSon) - uses a learnable policy network to select keyframes from candidate frames for predictive sufficiency, then assesses causal necessity via counterfactual interventions, guided by a composite reward through reinforcement learning.

Result: Extensive experiments on NExT-QA, EgoSchema, and Video-MME show ReaSon consistently outperforms existing state-of-the-art methods under limited-frame settings.

Conclusion: ReaSon effectively addresses keyframe selection for video understanding by combining causal reasoning with reinforcement learning, demonstrating strong generalization ability across multiple benchmarks.
...
TL;DR: HiGFA is a hierarchical diffusion-based data augmentation method that uses strong text and contour guidance early, then fine-grained classifier guidance with dynamic strength modulation in final stages to generate faithful synthetic images for fine-grained tasks.
...
Method: Hierarchical guidance: early stages use strong text and contour guidance for scene/style/structure; final stages activate fine-grained classifier guidance with dynamic strength modulation based on prediction confidence.

Result: Experiments on multiple FGVC datasets show HiGFA effectively generates diverse yet faithful synthetic images that improve fine-grained classification performance.

Conclusion: HiGFA's hierarchical, confidence-driven guidance enables intelligent balance between global structure formation and precise detail refinement, making diffusion models more suitable for fine-grained data augmentation.
...
TL;DR: EmoVerse is a large-scale open-source dataset for interpretable visual emotion analysis, featuring multi-layered annotations, dual emotion representations, and a novel annotation pipeline.


<details>
  <summary>Details</summary>
Motivation: To address limitations in visual emotion analysis caused by lack of interpretable datasets and single-label approaches that don't explain how visual elements contribute to emotions.

Method: Created EmoVerse dataset with 219k+ images using Background-Attribute-Subject triplets grounded to visual regions, dual CES/DES annotations, and a multi-stage annotation pipeline for reliability.

Result: Developed a comprehensive dataset enabling word-level and subject-level emotional reasoning, plus an interpretable model that maps visual cues to dimensional emotion space with attribution explanations.

Conclusion: EmoVerse provides a foundation for advancing explainable high-level emotion understanding through its dataset, pipeline, and model components.
...
TL;DR: SEMC is a Structure-Enhanced Mixture-of-Experts Contrastive learning framework that improves ultrasound standard plane recognition by combining structure-aware feature fusion with expert-guided contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to effectively exploit shallow structural information and struggle to capture fine-grained semantic differences through contrastive samples generated by image augmentations, resulting in suboptimal recognition of both structural and discriminative details in ultrasound standard planes.

Method: Proposes SEMC with two key modules: 1) Semantic-Structure Fusion Module (SSFM) to exploit multi-scale structural information and align shallow/deep features, and 2) Mixture-of-Experts Contrastive Recognition Module (MCRM) for hierarchical contrastive learning and classification using MoE mechanism.

Result: Extensive experiments on in-house liver ultrasound dataset (6 standard planes) and two public datasets show SEMC outperforms recent state-of-the-art methods across various metrics.
...
TL;DR: A novel method combining signal processing and machine learning to reconstruct surface temperatures through forest vegetation, enabling automated aerial wildfire monitoring with drones for early fire detection before visible signs appear.


<details>
  <summary>Details</summary>
Motivation: To enable fully automated aerial wildfire monitoring using autonomous drones for early detection of ground fires before smoke or flames are visible, overcoming the limitations of synthetic aperture sensing which introduces thermal blur that obscures actual surface temperatures.

Method: Train a visual state space model to recover subtle thermal signals from blurred synthetic aperture data, using a latent diffusion model integrated with vector quantization to generate realistic surface temperature simulations from real wildfire recordings, expanded through temperature augmentation and procedural thermal forest simulation.

Result: On simulated data across varied conditions, the method reduced RMSE by 2-2.5x compared to conventional thermal and uncorrected SA imaging. In field experiments on high-temperature hotspots, achieved 12.8x RMSE gain over conventional thermal and 2.6x gain over uncorrected SA images. Also demonstrated generalization to other thermal signals like human signatures.

Conclusion: The method successfully reconstructs complete morphology of fire and human signatures that conventional imaging fails due to partial occlusion, showing significant improvements in thermal signal recovery and classification accuracy beyond simple thresholding approaches.
...
TL;DR: Typographical attacks using deceptive text extensions can effectively protect geo-privacy against LVLMs while preserving image quality.


<details>
  <summary>Details</summary>
Motivation: Large Visual Language Models pose serious privacy threats by inferring users' geolocations from shared images, requiring protection methods that don't degrade visual quality.

Method: Two-stage, semantics-aware typographical attack that generates deceptive text extensions outside visual content to disrupt geolocation inference.
...
Conclusion: Establishes a practical and visually-preserving protection strategy against emerging geo-privacy threats from LVLMs.
...
TL;DR: TempoMaster is a novel framework for long video generation that formulates the task as next-frame-rate prediction, using progressive frame rate refinement to achieve temporal coherence and efficient synthesis.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generating long videos with temporal coherence while maintaining efficiency, overcoming limitations in existing video generation methods.

Method: Generate low-frame-rate clip as blueprint, then progressively increase frame rate using bidirectional attention within each level and autoregression across frame rates.

Result: Establishes new state-of-the-art in long video generation, excelling in both visual and temporal quality.

Conclusion: TempoMaster effectively solves long video generation through progressive frame rate refinement, achieving superior coherence and efficiency.
...
TL;DR: CountIHC is a framework for multi-class cell counting in IHC images that uses rank-aware knowledge distillation from foundation models and vision-language alignment for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: Current regression-based counting methods struggle with multi-class counting and don't effectively leverage foundation models, while dealing with challenges like chromogen overlap and variable staining in IHC images.

Method: Proposes rank-aware agglomeration with RATS strategy for teacher selection, and vision-language alignment using semantic anchors from text prompts to guide class-specific density map regression.

Result: Outperforms state-of-the-art methods across 12 biomarkers and 5 tissue types, shows high agreement with pathologists, and demonstrates scalability on H&E-stained data.

Conclusion: The framework effectively addresses multi-class cell counting challenges in IHC images through selective knowledge distillation and vision-language alignment, showing superior performance and clinical relevance.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with complex lane structures and unreliable topology predictions, which are critical for autonomous driving navigation and control decisions.

Method: Three-phase framework: Hierarchical Prior Extractor (HPE) for spatial and sequential priors, Region-Focused Decoder (RFD) for fine-grained query construction, and Robust Boundary-Point Topology Reasoning (RBTR) with denoising strategy.

Result: Achieves state-of-the-art performance on OpenLane-V2 benchmark with OLS of 48.0% on subsetA and 45.4% on subsetB.

Conclusion: Integrating spatial and sequential priors into fine-grained queries with topological denoising enables precise modeling of complex lane structures and trustworthy topology predictions.
...
TL;DR: Seg-VAR is a novel framework that rethinks segmentation as a conditional autoregressive mask generation problem, outperforming previous methods on various segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Visual autoregressive modeling strategies have shown promise in image generation but remain unexplored for segmentation tasks that require precise spatial perception.

Method: Uses three components: image encoder for latent priors, spatial-aware seglat encoder mapping masks to discrete tokens, and decoder reconstructing masks. Employs multi-stage training strategy.

Result: Experiments show Seg-VAR outperforms previous discriminative and generative methods on various segmentation tasks and validation benchmarks.

Conclusion: By framing segmentation as sequential hierarchical prediction, Seg-VAR opens new avenues for integrating autoregressive reasoning into spatial-aware vision systems.
...
TL;DR: A teacher-student framework combining CNN and ViT with LoRA fine-tuning achieves efficient and accurate single-image morphing attack detection, outperforming six state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Face Recognition Systems are vulnerable to morphing attacks that blend biometric features from multiple individuals, creating security risks that need robust detection methods.

Method: Proposed a teacher-student framework where a CNN-based teacher model refines a ViT-based student model, integrated with Low-Rank Adaptation (LoRA) for efficient fine-tuning to reduce computational costs.

Result: Extensive experiments on a morphing dataset with ten different generation algorithms show superior detection performance and computational efficiency compared to six state-of-the-art S-MAD techniques.

Conclusion: The proposed approach effectively addresses morphing attack detection with high accuracy and efficiency, making it suitable for practical security applications.
...
TL;DR: SoccerNet-GAR is a multimodal dataset for group activity recognition with synchronized video and tracking data from World Cup 2022, showing tracking-based methods outperform video-based approaches with better accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: To enable apples-to-apples comparison between video and tracking modalities for group activity recognition, as no standardized benchmark currently exists that aligns both modalities for the same activities.

Method: Introduced SoccerNet-GAR dataset with 94,285 synchronized group activities from 64 World Cup matches, and proposed a novel role-aware graph neural network for tracking-based GAR that encodes tactical structure through positional edges and temporal attention.

Result: Tracking-based model achieved 67.2% balanced accuracy vs 58.1% for best video baseline, while training 4.25x faster with 438x fewer parameters (197K vs 86.3M).

Conclusion: Tracking modality is more effective than video for group activity recognition, highlighting the importance of modality choice and role-aware modeling in GAR.
...
TL;DR: Proposes a Hierarchical Ladder Network for test-time adaptation that improves OOD detection and domain shift robustness through attention refinement and weighted entropy mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods suffer performance drops when encountering OOD samples, as misclassifying them as ID classes degrades accuracy and impairs adaptation. Need to address OOD detection and domain shift challenges.

Method: Uses Hierarchical Ladder Network to extract OOD features from Transformer class tokens, combines predictions via weighted probability fusion. Adds Attention Affine Network to refine self-attention for domain shift, and employs weighted entropy to suppress low-confidence samples.

Result: Experimental results show significant performance improvements on benchmark classification datasets compared to existing TTA methods.

Conclusion: The proposed method effectively handles OOD samples and domain shift in test-time adaptation, achieving superior performance on standard classification benchmarks.
...
TL;DR: Unified framework combining object detection and pose estimation with flexible onboarding from CAD models or neural reconstruction, using CNOS detector and OPFormer transformer for accurate 6D pose estimation.


<details>
  <summary>Details</summary>
Motivation: To create a seamless end-to-end system that handles both object detection and pose estimation while supporting both traditional CAD model onboarding and model-free neural reconstruction scenarios.

Method: Onboarding stage generates object representations from CAD models or NeRF reconstruction from multi-view images. CNOS detector localizes objects, then OPFormer transformer-based module extracts features using foundation models, encodes multiple template views with NOCS geometric priors, and establishes 2D-3D correspondences for pose estimation.

Result: The integrated system achieves strong balance between accuracy and efficiency on BOP benchmarks, demonstrating practical applicability in both model-based and model-free scenarios.

Conclusion: The framework provides a versatile solution for 6D pose estimation that works effectively whether traditional CAD models are available or need to be reconstructed from images.
...
TL;DR: C3Net is a novel architecture for camouflaged object detection that addresses six fundamental challenges through a dual-pathway decoder with edge refinement and contextual localization, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Traditional segmentation methods and modern foundation models fail dramatically on camouflaged objects due to intrinsic similarity with surroundings, edge disruption, scale variations, environmental complexities, contextual dependencies, and saliency-camouflage disambiguation.

Method: C3Net uses a dual-pathway decoder: Edge Refinement Pathway with gradient-initialized Edge Enhancement Modules for boundary recovery, and Contextual Localization Pathway with Image-based Context Guidance for intrinsic saliency suppression, combined via Attentive Fusion Module with spatial gating.

Result: Achieves S-measures of 0.898 on COD10K, 0.904 on CAMO, and 0.913 on NC4K datasets, demonstrating state-of-the-art performance while maintaining efficient processing.

Conclusion: Complex camouflaged object detection requires architectural innovation with specialized components working synergistically, showing comprehensive coverage beyond isolated improvements is necessary for this challenging task.
...
TL;DR: MDiTFace is a diffusion transformer framework that enables effective multimodal feature interaction between semantic masks and text for facial generation through unified tokenization and decoupled attention mechanisms.
...
Method: Uses unified tokenization strategy for semantic mask and text inputs, stacked multivariate transformer blocks for synchronous processing, and a novel decoupled attention mechanism that separates mask tokens from temporal embeddings into dynamic and static pathways.

Result: Reduces computational overhead introduced by mask condition by over 94% while maintaining performance. Significantly outperforms competing methods in facial fidelity and conditional consistency.

Conclusion: MDiTFace effectively addresses cross-modal interaction challenges in facial generation through its transformer-based architecture and decoupled attention mechanism, achieving superior performance with dramatically reduced computational costs.
...
TL;DR: Denoising-VAE addresses the optimization dilemma in high-dimensional latent spaces by suppressing redundant high-frequency noise, enabling faster convergence and better performance in diffusion models.


<details>
  <summary>Details</summary>
Motivation: High-dimensional VAE latent spaces improve reconstruction but degrade generative performance due to redundant high-frequency components that hinder diffusion model training convergence.

Method: Proposes spectral self-regularization to suppress high-frequency noise while preserving reconstruction quality, and spectral alignment to optimize Denoising-VAE-based generative models.

Result: Achieves 2× faster convergence than SD-VAE, state-of-the-art reconstruction (rFID=0.28, PSNR=27.26), and competitive generation (gFID=1.82) on ImageNet 256×256.

Conclusion: Denoising-VAE effectively resolves the optimization dilemma in high-dimensional latent spaces without relying on external vision foundation models, improving both reconstruction and generation quality.
...
TL;DR: CILMP is a novel prompt tuning method that incorporates Large Language Models (LLMs) to transfer medical knowledge into Vision-Language Models (VLMs) for medical image classification, achieving superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning VLMs for medical tasks is resource-intensive, and existing prompt tuning methods fail to distinguish different medical concepts and disease-specific features across imaging modalities. LLMs possess specialized medical knowledge that can enhance prompt tuning.

Method: CILMP extracts disease-specific representations from LLMs, intervenes within a low-rank linear subspace, and creates disease-specific prompts. It includes a conditional mechanism to generate instance-adaptive prompts based on individual medical images.
...
Conclusion: CILMP effectively bridges LLMs and VLMs to transfer medical knowledge into prompts, demonstrating superior performance in medical image classification tasks while maintaining computational efficiency.
...
TL;DR: DPVO-QAT++ is a hierarchical quantization optimization framework that combines heterogeneous precision design and GPU kernel fusion to significantly improve computational efficiency of deep visual SLAM systems while maintaining trajectory accuracy.
...
Result: Achieved 52.1% FPS increase, 29.1% latency reduction, and 64.9% GPU memory reduction on TartanAir; 30.1% FPS increase, 23.1% latency reduction, and 37.7% memory reduction on EuRoC, while maintaining comparable trajectory accuracy.

Conclusion: The framework effectively bridges the gap between high-precision deep visual odometry and efficiency requirements for practical deployment on embedded platforms.
...
Method: Collects 16,750 real tampering instances, analyzes editing traces, and models tampering parameters using Fourier series-inspired hierarchical framework with basis functions and learned weights.
...
Conclusion: FSTS provides an interpretable framework for synthesizing realistic training data that better reflects real-world forgery traces, improving model generalization.
...
TL;DR: A real-time conversational digital human system combining realistic 3D avatars, expressive speech synthesis, and knowledge-grounded dialogue with minimal latency.


<details>
  <summary>Details</summary>
Motivation: Achieving both visual realism and real-time responsiveness in digital humans for interactive applications remains challenging.

Method: Asynchronous execution pipeline coordinating multi-modal components, retrieval-augmented methods including history augmentation and intent-based routing for knowledge access.

Result: System enables responsive and believable digital humans with wake word detection, emotionally expressive prosody, and context-aware response generation.

Conclusion: Integrated system suitable for immersive applications in communication, education, and entertainment.
...
<details>
  <summary>Details</summary>
Motivation: To develop an accurate and real-time model for optical flow and disparity estimation that efficiently handles constraints in real-time applications.

Method: Uses a non-causal Mamba block-based model with pairwise input image fusion in non-causal selective state space for dense perception.

Result: Achieves reduced inference times while maintaining high accuracy and low GPU usage for optical flow and disparity map generation.
...
TL;DR: Proposes PRISM, a task-aware evaluation framework for novel view synthesis that uses Zero123 features and two new metrics to reliably assess synthesis quality and rank models according to human preferences.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation metrics for novel view synthesis struggle to assess whether generated images are both realistic and faithful to source views and viewpoint transformations, often mis-ranking incorrect results.

Method: Leverages features from Zero123 foundation model with lightweight tuning, introducing two metrics: reference-based D_PRISM and reference-free MMD_PRISM to evaluate synthesis quality.

Result: Both metrics reliably identify incorrect generations and rank models in agreement with human preference studies. MMD_PRISM produces clear, stable rankings across three benchmarks (Toys4K, GSO, OmniObject3D) with lower scores indicating stronger models.

Conclusion: The framework provides a principled and practical approach to assessing synthesis quality, addressing a fundamental gap in NVS evaluation and enabling more reliable progress in novel view synthesis.
...
TL;DR: BridgeEQA is a new benchmark for embodied question answering focused on infrastructure inspection, featuring 2,200 open-vocabulary QA pairs across 200 real bridge scenes with professional inspection reports and NBI condition ratings.


<details>
  <summary>Details</summary>
Motivation: Current embodied agents struggle with real-world question answering due to lack of benchmarks that capture practical conditions. Infrastructure inspection naturally requires multi-scale reasoning, spatial understanding, and complex semantic relationships.

Method: Proposed Embodied Memory Visual Reasoning (EMVR) formulates inspection as sequential navigation over image-based scene graphs, where agents traverse views, compare evidence, and reason within a Markov decision process.

Result: Evaluations show substantial performance gaps in state-of-the-art vision-language models under episodic memory EQA settings. EMVR demonstrates strong performance improvements over baselines on the BridgeEQA benchmark.

Conclusion: BridgeEQA provides a faithful testbed for embodied question answering in practical settings, and EMVR offers an effective approach for multi-image reasoning in inspection tasks. Both dataset and code are publicly released.
...
TL;DR: R²Seg is a training-free framework for robust out-of-distribution tumor segmentation that uses LLM-guided anatomical reasoning and statistical rejection to suppress false positives without parameter updates.
...
Method: Two-stage Reason-and-Reject process: 1) LLM-guided anatomical reasoning to localize organs and generate ROIs, 2) Statistical testing to reject candidates not significantly different from normal tissue using a frozen foundation model.
...
Conclusion: R²Seg provides effective OOD tumor segmentation without parameter updates, avoiding catastrophic forgetting while maintaining compatibility with test-time augmentation.
...
TL;DR: HEDGE is a unified framework for detecting hallucinations in vision-language models using controlled visual perturbations, semantic clustering, and uncertainty metrics.


<details>
  <summary>Details</summary>
Motivation: Vision-language models are prone to hallucinations despite enabling open-ended visual question answering, requiring robust detection methods.

Method: Combines sampling, distortion synthesis, semantic clustering (entailment- and embedding-based), and metric computation into a reproducible pipeline.

Result: Hallucination detectability varies by architecture and prompt design, with Qwen2.5-VL performing best and Med-Gemma worst. VASE metric with embedding clustering and moderate sampling (n~10-15) provides most robust signal.
...
TL;DR: A controllability-based interpretability framework for State Space Models (SSMs) that quantifies how input tokens influence internal state dynamics, enabling analysis of hierarchical feature refinement in vision tasks.


<details>
  <summary>Details</summary>
Motivation: To address the lack of transparent, attention-like mechanisms in SSMs like Mamba, making it challenging to understand how these models process spatial information despite their competitive performance and linear computational complexity.

Method: Two complementary formulations: Jacobian-based method for any SSM architecture measuring influence through state propagation chain, and Gramian-based approach for diagonal SSMs with closed-form analytical solutions. Both operate in single forward pass with linear complexity.

Result: SSMs naturally implement hierarchical feature refinement from diffuse low-level textures in early layers to focused, clinically meaningful patterns in deeper layers. Reveals domain-specific controllability signatures, progressive spatial selectivity, and influence of scanning strategies on attention patterns.

Conclusion: Controllability analysis establishes a unified, foundational interpretability paradigm for SSMs across domains including computer vision, NLP, and cross-domain tasks.
...
TL;DR: CountOCC is an amodal counting framework that reconstructs occluded object features using multimodal guidance and visual equivalence objectives, achieving state-of-the-art performance on occlusion-augmented datasets.


<details>
  <summary>Details</summary>
Motivation: Current object counting methods fail under occlusion because backbone networks encode occluding surfaces instead of target objects, corrupting feature representations needed for accurate counting.

Method: CountOCC reconstructs occluded object features through hierarchical multimodal guidance, integrating spatial context from visible fragments with semantic priors from text and visual embeddings. It also uses a visual equivalence objective to enforce consistency in attention space between occluded and unoccluded views.

Result: CountOCC achieves 26.72% and 20.80% MAE reduction on FSC 147 validation and test sets under occlusion, 49.89% MAE reduction on CARPK, and 28.79% MAE reduction on CAPTUREReal, demonstrating robust performance across diverse domains.

Conclusion: CountOCC effectively addresses occlusion challenges in object counting by reconstructing complete object features and enforcing visual consistency, setting new state-of-the-art results across multiple datasets.
...
TL;DR: FSDAM is a few-shot framework for joint driver attention prediction and caption generation using only ~100 annotated examples, achieving competitive performance with minimal supervision.


<details>
  <summary>Details</summary>
Motivation: Existing driver attention models require large-scale gaze datasets that are labor-intensive to collect, limiting practical deployment in data-constrained scenarios.

Method: Dual-pathway architecture with separate modules for spatial attention prediction and caption generation, using cross-modal alignment to maintain semantic consistency.
...
TL;DR: First study of backdoor attacks on open-vocabulary object detectors using prompt tuning, proposing TrAP method that jointly optimizes image/text prompts with visual triggers for stealthy backdoor injection.


<details>
  <summary>Details</summary>
Motivation: As OVODs gain traction in high-stakes applications like robotics and autonomous driving, understanding their security risks becomes crucial, particularly the new attack surface introduced by prompt tuning.

Method: Propose TrAP (Trigger-Aware Prompt tuning) - a multi-modal backdoor injection strategy that jointly optimizes prompt parameters in both image and text modalities along with visual triggers, using curriculum-based training to progressively shrink trigger size.

Result: Achieves high attack success rates for both object misclassification and disappearance attacks while improving clean image performance on downstream datasets compared to zero-shot setting.

Conclusion: Demonstrates significant security vulnerabilities in OVODs through prompt tuning backdoors, highlighting the need for robust security measures in multi-modal vision-language systems.
...
TL;DR: VLMs fail to properly attend to relevant visual tokens when answering questions, leading to wrong answers. The paper proposes a KL attention loss to directly supervise visual attention, improving performance on visual tasks.


<details>
  <summary>Details</summary>
Motivation: Standard next-token prediction loss provides insufficient signal for directing attention to visual tokens in VLMs, causing them to ignore relevant visual information when answering questions.

Method: Proposes KL attention loss (KLAL) that aligns attention distribution of visual tokens to ground truth attention maps using KL divergence, combined with standard next-token prediction loss.

Result: Notable improvements across geometric tasks, pointing, and referring expression comprehension on both synthetic and real-world data. Also created a new line tracing dataset where even commercial VLMs perform poorly.

Conclusion: Direct supervision of visual attention through KL attention loss significantly improves VLM performance on visual reasoning tasks by ensuring answer tokens properly attend to relevant visual information.
...
TL;DR: This study explores inferring low-level voxel content (target occupancy percentages) from high-level voxelized LiDAR data using a multi-target regression approach with KPConv, addressing class imbalance through cost-sensitive learning and evaluating voxel size sensitivity.


<details>
  <summary>Details</summary>
Motivation: Voxelization reduces LiDAR computational cost but loses fine-scale structural information. The research aims to determine if target occupancy percentages (bark, leaf, soil, miscellaneous) can be recovered from voxelized data, addressing gaps in deep imbalance learning for multi-target regression with forest LiDAR data.

Method: Multi-target regression using Kernel Point Convolutions (KPConv) with cost-sensitive learning (density-based relevance) to handle class imbalance. Methods include weighted MSE, Focal Regression, and regularization. Sensitivity analysis on voxel sizes from 0.25 to 2 meters using DIRSIG-simulated forest data.

Result: Larger voxel sizes (2m) yield lower errors due to reduced variability, while smaller sizes (0.25-0.5m) show higher errors, especially in canopy areas where variability is greatest. Bark and leaf targets exhibit significantly higher errors at fine resolutions, indicating difficulty in accurate within-canopy content estimation.

Conclusion: Voxel size choice is application-dependent. The work fills gaps in deep imbalance learning for multi-target regression and provides simulated datasets for 3D LiDAR forest point clouds, demonstrating the trade-off between resolution and estimation accuracy in voxelized LiDAR processing.
...
TL;DR: SAGE is a saliency-guided training method that uses contrastive embeddings in latent space rather than image space, improving model generalization and performance across various tasks.


<details>
  <summary>Details</summary>
Motivation: Existing saliency-guided training methods rely on unreliable internal model mechanisms in image space, limiting their effectiveness for integrating human perceptual priors.

Method: Uses salient-preserving and saliency-degrading augmentations with contrastive triplet loss in latent space to guide models toward salient features and away from non-salient features, plus logit distribution sanity checks.

Result: Demonstrates improved classification performance in both open- and closed-set scenarios compared to state-of-the-art saliency-based methods across various backbones.

Conclusion: SAGE effectively integrates human saliency into network training through latent space guidance, showing wide generalization across tasks and superior performance to existing approaches.
...
TL;DR: Stable Diffusion embeddings can be viewed as point clouds in Wasserstein space rather than Euclidean matrices, enabling optimal transport-based interpolation that produces smoother image transitions.


<details>
  <summary>Details</summary>
Motivation: The permutation-invariance property of Stable Diffusion with respect to CLIP embeddings suggests these embeddings are better understood as point clouds in Wasserstein space rather than Euclidean matrices.

Method: Reframe interpolation between embeddings as an optimal transport problem, computing geodesics (shortest paths) in Wasserstein space rather than using standard Euclidean interpolation methods.
...
Conclusion: Viewing embeddings as point clouds in Wasserstein space better captures the geometry of embedding space and enables more natural interpolations through optimal transport.
...
TL;DR: Introduces RoCoISLR, the first large-scale Romanian Isolated Sign Language Recognition dataset with 9,000+ video samples across 6,000 glosses, and benchmarks 7 state-of-the-art models showing transformer-based architectures perform best.


<details>
  <summary>Details</summary>
Motivation: Address the lack of large-scale, standardized datasets for Romanian Sign Language recognition, which limits research progress compared to American Sign Language resources.

Method: Created RoCoISLR dataset with 9,000+ video samples spanning 6,000 standardized glosses from multiple sources. Evaluated 7 video recognition models (I3D, SlowFast, Swin Transformer, TimeSformer, Uniformer, VideoMAE, PoseConv3D) under consistent experimental setups.

Result: Transformer-based architectures outperformed convolutional baselines; Swin Transformer achieved the best performance with 34.1% Top-1 accuracy. Results highlight challenges of long-tail class distributions in low-resource sign languages.

Conclusion: RoCoISLR provides the foundational dataset for systematic Romanian Isolated Sign Language Recognition research, establishing initial benchmarks and demonstrating the superiority of transformer-based models for this task.
...
TL;DR: A lightweight color harmonization method for AR using optimal transport theory, enabling real-time on-device inference with competitive performance.


<details>
  <summary>Details</summary>
Motivation: Color harmonization is essential for seamless AR composites but current methods are too slow for real-time AR pipelines, requiring lightweight on-device solutions.

Method: Train a compact encoder to predict the Monge-Kantorovich transport map from classical optimal transport theory for color transformation.

Result: Achieves best aggregated score on real composite AR images compared to state-of-the-art methods, with real-time performance suitable for AR applications.

Conclusion: Proposed MKL-Harmonizer enables practical color harmonization for AR with on-device inference, and releases dedicated AR dataset and toolkit for community use.
...
TL;DR: An uncertainty-aware brain tumor segmentation framework that combines tumor localization with anatomical context and provides voxel-wise uncertainty estimates to aid clinical decision-making.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods lack uncertainty estimates for errors and fail to segment healthy brain structures around tumors, limiting clinical adoption despite good benchmark performance.

Method: Augments nnUNet with a channel for voxel-wise uncertainty, trained on BraTS2023. Uses a unified model combining normal and cancer datasets to segment both tumors and healthy brain structures in one pass without extra networks.

Result: Achieved correlation of 0.750 and RMSD of 0.047 for uncertainty without compromising tumor accuracy. Unified model achieved DSC of 0.81 for brain structures and 0.86 for tumor, with robust key-region performance.

Conclusion: First model to output tumor segmentation in natural surroundings with overlaid uncertainty maps, providing key insights for evaluating predictions and fixing errors to support informed surgical decisions.
...
TL;DR: Proposes Multi-Scale Recursive Network (MSRNet) for camouflaged object detection using pyramid vision transformer and recursive-feedback decoding to handle small/multiple objects in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with precisely detecting camouflaged objects in complex scenarios, especially with small and multiple objects, indicating need for improved approaches.

Method: Uses Pyramid Vision Transformer backbone for multi-scale feature extraction, Attention-Based Scale Integration Units for selective feature merging, and recursive-feedback decoding with Multi-Granularity Fusion Units.

Result: Achieves state-of-the-art results on two benchmark datasets and ranks second on two others, successfully detecting small and multiple camouflaged objects.

Conclusion: Joint multi-scale learning and recursive feature optimization enables effective camouflaged object detection in challenging scenarios.
...
TL;DR: SAGA is a comprehensive framework for AI-generated video source attribution that identifies the specific generative model used, offering multi-granular attribution across five levels and achieving state-of-the-art performance with minimal labeled data.


<details>
  <summary>Details</summary>
Motivation: The proliferation of hyper-realistic synthetic videos created by generative AI has escalated misuse risks and outstripped traditional binary real/fake detectors, creating an urgent need for more sophisticated attribution methods.

Method: SAGA uses a novel video transformer architecture with features from a robust vision foundation model to capture spatio-temporal artifacts. It employs a data-efficient pretrain-and-attribute strategy requiring only 0.5% source-labeled data per class, and introduces Temporal Attention Signatures (T-Sigs) for interpretability.

Result: Extensive experiments show SAGA sets a new benchmark for synthetic video provenance, achieving state-of-the-art attribution performance that matches fully supervised methods while using minimal labeled data, with strong cross-domain generalization.

Conclusion: SAGA provides crucial, interpretable insights for forensic and regulatory applications by enabling comprehensive AI-generated video source attribution at scale, offering far richer forensic insights than traditional detection methods.
...
<details>
  <summary>Details</summary>
Motivation: To understand what video finetuning brings to multimodal LLMs, as current approaches often naively concatenate frame tokens without proper temporal reasoning.

Method: Proposed Visual Chain-of-Thought (vCoT) - an explicit reasoning process that generates transitional event descriptions between consecutive frames, and systematically compared image-only vs video-finetuned models.

Result: vCoT significantly improves image-only models on long-form video QA, but yields only marginal gains for video-finetuned models. Video models also transfer temporal reasoning to static settings, outperforming image models on relational visual reasoning.

Conclusion: Video-finetuned models already capture frame-to-frame transitions implicitly, demonstrating effective temporal reasoning that transfers to static visual tasks.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of partially overlapping multi-view setups where actions are only visible in subset of views, especially in real-world systems with limited input modalities and sequence-level annotations instead of dense frame-level labels.
...
Result: Outperforms competitive distillation methods on MultiSensor-Home dataset across multiple backbones and environments, achieving significant gains and surpassing teacher model under limited conditions.

Conclusion: ViCoKD effectively addresses partial view overlap and modality limitations in multi-view action recognition through knowledge distillation and view-aware consistency mechanisms.
...
TL;DR: EgoLoc is a zero-shot method for temporal interaction localization that identifies hand-object contact and separation moments in egocentric videos without needing object masks or verb-noun taxonomies.


<details>
  <summary>Details</summary>
Motivation: Existing methods focus on 'how to interact' but struggle with 'when to interact' - precisely localizing contact/separation moments, which is crucial for VR/AR applications and robotic motion planning.

Method: Uses hand-dynamics-guided sampling to generate visual prompts, exploits vision-language models to identify contact attributes and localize timestamps, and provides closed-loop feedback for refinement.
...
Conclusion: EgoLoc provides a generalizable zero-shot solution for fine-grained temporal interaction localization without requiring object masks or category annotations during training.
...
TL;DR: A data-driven framework for automatic and interpretable creativity assessment from drawings using multi-modal, multi-task learning that analyzes both content and style dimensions.


<details>
  <summary>Details</summary>
Motivation: Current creativity assessment relies on subjective expert scoring which is labor-intensive and inconsistent. There's a need for automated, objective methods that can assess creativity through both what is drawn (content) and how it's drawn (style).

Method: Multi-modal, multi-task learning framework that simultaneously predicts creativity scores, categorizes content types, and extracts stylistic features. Uses conditional learning to adapt visual feature extraction based on creativity-relevant signals from stylistic and semantic cues.

Result: Achieves state-of-the-art performance compared to existing regression-based approaches and provides interpretable visualizations that align well with human judgments.

Conclusion: The proposed framework successfully automates creativity assessment from drawings while maintaining interpretability, bridging the gap between data-driven methods and human cognitive understanding of creativity.
...
TL;DR: ActVAR introduces dynamic activation with dual sparsity in weights and tokens to reduce computational costs in Visual Autoregressive models while maintaining performance through expert subnetworks and token selection.


<details>
  <summary>Details</summary>
Motivation: VAR models face escalating computational costs as sequence length grows, and existing static pruning methods degrade performance by permanently removing weights or tokens, disrupting pretrained dependencies.

Method: Decomposes FFNs into lightweight expert sub-networks with learnable router for dynamic token-specific expert selection, plus gated token selector for high-update-potential tokens while reconstructing unselected tokens. Uses two-stage knowledge distillation supervised by original VAR model.

Result: Achieves up to 21.2% FLOPs reduction on ImageNet 256×256 benchmark with minimal performance degradation.

Conclusion: ActVAR provides an effective dynamic activation framework that enhances efficiency in VAR models without sacrificing capacity through dual sparsity and knowledge-preserving training.
...
TL;DR: First method for 3D scene reconstruction that directly models native HDR observations using 3D Gaussian Splatting, preserving full dynamic range throughout the pipeline.


<details>
  <summary>Details</summary>
Motivation: Professional digital media creation requires HDR imaging, but existing 3D reconstruction methods focus on LDR data, limiting professional workflow applicability. Direct HDR capture cameras have emerged, enabling native HDR scene reconstruction.

Method: Proposes Native High dynamic range 3D Gaussian Splatting (NH-3DGS) with novel luminance-chromaticity decomposition of color representation for direct optimization from native HDR camera data.

Result: Significantly outperforms existing methods in reconstruction quality and dynamic range preservation on both synthetic and real multi-view HDR datasets.

Conclusion: Enables professional-grade 3D reconstruction directly from native HDR captures, advancing the field beyond LDR-based approaches.
...
TL;DR: FDP is a frequency-decomposition preprocessing framework that enhances unsupervised anomaly detection in brain MRI by leveraging frequency-domain analysis to suppress pathology while preserving normal anatomy, achieving significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Current UAD methods use artificial noise perturbations that lack biophysical fidelity and morphological complexity of real clinical lesions, limiting their effectiveness in brain MRI anomaly detection.

Method: Frequency-Decomposition Preprocessing (FDP) framework that analyzes pathological signatures in frequency domain, distinguishing anomaly patterns from normal anatomy and leveraging consistent low-frequency representations in healthy scans for reconstruction.

Result: FDP consistently improves anomaly detection performance, achieving 17.63% increase in DICE score with LDM while maintaining robust improvements across multiple baselines.

Conclusion: FDP represents the first UAD method using frequency-domain reconstruction for simultaneous pathology suppression and anatomical preservation, demonstrating significant enhancements to existing anomaly detection approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current sports video understanding approaches are limited by being single-sport centric, task-specific, or relying on training-free paradigms without robust learned reasoning processes.

Method: Proposes DeepSport with data distillation pipeline synthesizing CoT trajectories from 10 data sources, two-stage training (SFT + RL with gated tool-use reward), and active frame interrogation via specialized extraction tool.
...
Conclusion: Establishes a new foundation for domain-specific video reasoning to handle the complexities of diverse sports through active, iterative reasoning paradigms.
...
TL;DR: Proposes CASL, a curvature-augmented self-supervised learning framework for 3D anomaly detection that achieves state-of-the-art performance while maintaining generalizability to other 3D understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D anomaly detection methods are task-specific and lack generalizability, while classical self-supervised models perform suboptimally for anomaly detection under unified fine-tuning.

Method: Uses multi-scale curvature prompts in a U-Net architecture to guide point coordinate prediction through reconstruction, enabling effective anomaly detection via simple fine-tuning without task-specific designs.

Result: Achieves leading anomaly detection performance and generalizes well to standard 3D understanding tasks like point cloud classification, outperforming both dedicated anomaly detection and classical self-supervised models.

Conclusion: Curvature plays a critical role in 3D anomaly detection, and the proposed CASL framework demonstrates that general-purpose models can effectively handle anomaly detection without specialized mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Existing fine-tuning methods ignore cross-modal heterogeneity, limiting MLLMs' potential. Need for efficient fine-tuning that addresses cross-modal relationships.

Method: Multimodal Noise Generator (MuNG) that analyzes cross-modal relationships in image-text pairs to generate task-adaptive beneficial noise, injected into frozen MLLMs from a variational inference perspective.

Result: Outperforms full-parameter fine-tuning and other methods on QwenVL and LLaVA models, achieving better cross-modal alignment with only 1-2% additional parameters.

Conclusion: MuNG provides an efficient fine-tuning strategy that effectively addresses cross-modal heterogeneity through beneficial noise injection, significantly improving MLLM performance with minimal parameter overhead.
...
TL;DR: CoordAR is an autoregressive framework for 6D pose estimation of unseen objects using only one reference view, addressing limitations of existing methods through probabilistic correspondence modeling and modality-decoupled encoding.
...
Method: Formulates 3D-3D correspondences as discrete tokens using coordinate map tokenization, modality-decoupled encoding of RGB and coordinate features, and autoregressive transformer decoder conditioned on query features and generated tokens.
...
Conclusion: CoordAR provides an effective probabilistic framework for one-reference 6D pose estimation that overcomes limitations of coordinate regression methods through autoregressive modeling and uncertainty handling.
...
<details>
  <summary>Details</summary>
Motivation: Current generative video models struggle with controlling photographic elements like depth of field and exposure, which are crucial for cinematic storytelling and mood creation, as existing methods are mostly limited to camera motion control.

Method: Introduces a decoupled cross-attention mechanism to disentangle camera motion from photographic inputs, enabling independent control. Uses a comprehensive data generation strategy combining simulated photographic effects with real-world collection to build a large-scale training dataset.

Result: The model generates high-fidelity videos with precisely controlled, user-specified photographic camera effects, demonstrating effective fine-grained control without compromising scene consistency.

Conclusion: CineCtrl successfully addresses the challenge of controlling professional camera parameters in generative video models, enabling cinematic editing capabilities that were previously unavailable in existing video generation frameworks.
...
TL;DR: A unified text-driven framework for traffic scene image generation and editing that addresses semantic richness, viewpoint diversity, visual fidelity, and text-image alignment challenges.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations in current text-driven image generation for traffic scenes, including insufficient semantic richness, limited camera viewpoints, low visual fidelity, and poor text-image alignment.

Method: Uses a controllable mask mechanism to unify generation and editing tasks, incorporates multi-view data from vehicle and roadside perspectives, employs two-stage training with coarse-to-fine data, and implements mask-region-weighted loss for small element fidelity.

Result: Extensive experiments show the method achieves leading performance in text-based image generation and editing for traffic scenes.

Conclusion: The proposed framework successfully addresses key challenges in traffic scene generation and editing, demonstrating superior performance through unified architecture, multi-view data integration, and specialized training strategies.
...
TL;DR: PFAvatar reconstructs high-quality 3D avatars from OOTD photos using a two-stage approach: fine-tuning a pose-aware diffusion model and distilling a NeRF-based avatar, achieving 48x speed-up and superior detail preservation.


<details>
  <summary>Details</summary>
Motivation: To create practical 3D avatars from real-world "Outfit of the Day" photos that exhibit diverse poses, occlusions, and complex backgrounds, overcoming limitations of previous methods that use image segmentation and suffer from inconsistency.

Method: Two-stage approach: (1) Fine-tune pose-aware diffusion model using ControlNet for pose estimation and Condition Prior Preservation Loss to prevent language drift; (2) Distill 3D avatar using NeRF representation with canonical SMPL-X space sampling and Multi-Resolution 3D-SDS.

Result: Achieves 48x speed-up (5 minutes personalization), outperforms state-of-the-art in reconstruction fidelity, detail preservation, and robustness to occlusions/truncations. Preserves high-frequency textures and handles occlusions correctly.

Conclusion: PFAvatar advances practical 3D avatar generation from real-world OOTD albums, supporting downstream applications like virtual try-on, animation, and human video reenactment, demonstrating versatility and practical value.
...
<details>
  <summary>Details</summary>
Motivation: Existing industrial anomaly detection methods only detect presence of anomalies but cannot discover and classify multiple anomaly types, while current approaches don't sufficiently exploit image priors and perform poorly on semantically subtle industrial anomalies.

Method: Uses Grounded SAM with text prompts to localize object regions, introduces Anomaly-Map-Guided Attention with Region Guidance Factor to distinguish background/object/anomalous regions, and employs prototype learning for clustering unseen anomaly classes and multi-type classification.
...
Conclusion: ProtoAnomalyNCD effectively discovers and classifies multiple unseen anomaly types by leveraging object localization and anomaly-map priors within a unified prototype-learning framework.
...
TL;DR: Semi-supervised HDR image reconstruction using uncertainty-based masking to address confirmation bias, achieving comparable performance to fully-supervised methods with only 6.7% HDR ground truths.


<details>
  <summary>Details</summary>
Motivation: HDR image reconstruction typically requires LDR-HDR image pairs which are hard to obtain, motivating the need for annotation-efficient methods that work with limited ground truth data.

Method: Uses teacher-student semi-supervised learning with uncertainty-based masking at pixel and patch levels to discard unreliable parts of pseudo HDR ground truths generated by the teacher model, preventing confirmation bias.

Result: Outperforms previous annotation-efficient algorithms and achieves comparable performance to state-of-the-art fully-supervised methods using only 6.7% of HDR ground truths.

Conclusion: The proposed uncertainty-based masking approach effectively addresses confirmation bias in semi-supervised HDR reconstruction, enabling high performance with minimal ground truth data.
...
TL;DR: The paper introduces RAD, a recurrent autoregressive diffusion framework that integrates LSTM with attention into diffusion transformers to enhance long-term video generation by maintaining consistent memory updates across training and inference.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models lack effective memory compression and retrieval for long-term generation beyond window sizes, leading to forgetting and spatiotemporal inconsistencies.

Method: Proposes RAD framework with LSTM integration for frame-wise autoregression, ensuring consistent memory update and retrieval across training and inference phases.

Result: RAD achieves superior performance on Memory Maze and Minecraft datasets, demonstrating efficient long video generation with LSTM's sequence modeling capabilities.

Conclusion: The RAD framework effectively addresses memory limitations in long video generation, showing that LSTM integration in diffusion models provides robust memory retention and consistent performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing adversarial attacks on TSR systems have limitations - pixel-level methods lack stealthiness and transferability, while T2I diffusion models show limited effectiveness and poor generalization to out-of-distribution sign types.

Method: Proposes a T2I-based attack pipeline with CLIP-based loss and masked prompts for better focus and controllability, plus two novel style customization methods to guide visual appearance and improve stealthiness and out-of-domain generalization.

Result: Achieves 83.3% average physical-world attack success rate with high transferability across varied real-world conditions including different distances, angles, lighting, and sign categories.
...
TL;DR: EndoSight AI is a deep learning system for real-time gastrointestinal polyp detection and segmentation during endoscopy, achieving 88.3% mAP for detection and 69% Dice coefficient for segmentation at 35+ FPS.
...
Method: Deep learning architecture using Hyper-Kvasir dataset, incorporating clinically relevant performance metrics and a novel thermal-aware procedure for model robustness and efficiency.

Result: Achieves 88.3% mean Average Precision for polyp detection and 69% Dice coefficient for segmentation, with real-time inference speeds exceeding 35 frames per second on GPU hardware.

Conclusion: This integrated AI solution is designed for seamless deployment in endoscopy workflows, promising to advance diagnostic accuracy and clinical decision-making in gastrointestinal healthcare.
...
TL;DR: CalibrateMix improves semi-supervised learning model calibration using targeted mixup of easy and hard samples, reducing overconfidence while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing SSL methods suffer from poor calibration with overconfident predictions, and while mixup helps in supervised settings, it's under-explored in SSL where pseudolabels are unreliable.

Method: Leverages training dynamics to identify easy-to-learn and hard-to-learn samples, then performs targeted mixup between these categories to improve calibration.
...
Conclusion: Targeted mixup based on training dynamics effectively improves SSL model calibration while maintaining or improving classification performance.
...
TL;DR: GrOCE is a training-free framework for precise concept erasure in diffusion models using graph-based semantic reasoning, achieving SOTA performance without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing concept erasure methods require costly fine-tuning or use coarse semantic separation, degrading unrelated concepts and lacking adaptability to evolving concept sets.

Method: Models concepts as dynamic semantic graph with three components: Dynamic Topological Graph Construction, Adaptive Cluster Identification with multi-hop traversal, and Selective Edge Severing for targeted removal.

Result: Achieves state-of-the-art performance on Concept Similarity and Fréchet Inception Distance metrics, offering efficient and stable concept erasure.

Conclusion: GrOCE provides precise, adaptive concept removal through graph-based reasoning without requiring model retraining.
...
TL;DR: HiFusion is a deep learning framework that predicts spatial transcriptomics gene expression from H&E-stained whole-slide images by modeling both fine-grained intra-spot morphology and biologically relevant regional context through hierarchical multi-resolution analysis and cross-attention fusion.


<details>
  <summary>Details</summary>
Motivation: Existing methods for predicting gene expression from histopathology images fail to capture biological heterogeneity within spots and are susceptible to morphological noise when integrating contextual tissue information, limiting clinical adoption of spatial transcriptomics due to technical complexity and high costs.

Method: HiFusion integrates two components: (1) Hierarchical Intra-Spot Modeling module that extracts fine-grained morphological representations through multi-resolution sub-patch decomposition with feature alignment loss, and (2) Context-aware Cross-scale Fusion module that uses cross-attention to selectively incorporate biologically relevant regional context.
...
Conclusion: HiFusion demonstrates potential as a robust, accurate, and scalable solution for spatial transcriptomics inference from routine histopathology, bridging the gap between gene expression and tissue morphology.
...
TL;DR: MCAQ-YOLO introduces spatially adaptive quantization for object detection using morphological complexity metrics to guide bit allocation, achieving better accuracy than uniform quantization with minimal runtime overhead.


<details>
  <summary>Details</summary>
Motivation: Most neural network quantization methods use uniform bit precision across spatial regions, ignoring the heterogeneous structural and textural complexity of visual data, which leads to suboptimal performance.
...
Result: Achieves 85.6% mAP@0.5 with 4.2 average bits and 7.6x compression ratio, outperforming uniform 4-bit quantization by 3.5 percentage points with only 1.8ms additional runtime overhead per image.

Conclusion: Morphology-driven spatial quantization enhances efficiency and robustness for computationally constrained, safety-critical visual recognition tasks, with consistent performance gains across datasets.
...
TL;DR: ArtiWorld is a pipeline that automatically converts rigid 3D objects into articulated URDF models using LLM knowledge and 3D point clouds, enabling scalable creation of interactive simulation environments.


<details>
  <summary>Details</summary>
Motivation: Manual conversion of rigid 3D assets into articulated objects for robot learning environments is labor-intensive and costly, creating a need for automated solutions.

Method: Uses Arti4URDF with 3D point clouds, LLM prior knowledge, and URDF-oriented prompts to identify articulable objects from scene descriptions and reconstruct executable URDF models while preserving original geometry.
...
Conclusion: Provides a practical path for building interactive, robot-ready simulation environments directly from existing 3D assets without manual articulation.
...
TL;DR: CCI is a new interpretability method for CLIP that uses patch clustering to identify spurious correlations, achieving SOTA faithfulness and enabling automatic foreground/background diagnosis. The COVAR benchmark is introduced to systematically evaluate foreground/background effects.


<details>
  <summary>Details</summary>
Motivation: CLIP models suffer from spurious correlations, particularly background over-reliance, which limits their robustness and interpretability. Existing benchmarks like CounterAnimals have limitations in properly attributing performance degradation.

Method: CCI clusters CLIP's patch embeddings into semantic groups, masks them, and evaluates prediction changes. Combined with GroundedSAM for automatic foreground/background categorization. COVAR benchmark systematically varies object foregrounds and backgrounds.

Result: CCI achieves more than twofold improvement on deletion-AUC metric for MS COCO retrieval. Comprehensive evaluation of 18 CLIP variants reveals that many errors come from viewpoint variation, scale shifts, and fine-grained confusions, not just background correlations.

Conclusion: CCI provides superior interpretability and diagnostic capabilities for VLMs. The findings challenge the assumption that all performance degradation stems from background correlations, highlighting the need for more nuanced evaluation through benchmarks like COVAR.
...
TL;DR: UNSEEN is a plug-and-play dataset pruning framework that addresses sample scoring limitations by using generalization-based scoring from models not exposed to training data, outperforming SOTA methods with 30% data reduction on ImageNet-1K.


<details>
  <summary>Details</summary>
Motivation: Existing dataset pruning methods rely on fitting-centric scoring from models trained on full datasets, which causes dense score distributions and reduces sample distinction, hindering effective selection.

Method: Proposes UNSEEN framework that scores samples based on generalization using models not exposed to them during training, with multi-step incremental selection through models trained on varying coresets.

Result: Significantly outperforms SOTA methods on CIFAR-10, CIFAR-100, and ImageNet-1K, achieving lossless performance with 30% data reduction on ImageNet-1K.

Conclusion: Generalization-based scoring and multi-step incremental selection effectively address limitations of fitting-centric approaches, enabling more effective dataset pruning.
...
TL;DR: WSAE-Net introduces weighted semantic maps and auto-adaptive candidate editing sequences to improve visual counterfactual explanations by ensuring semantic relevance and computational efficiency.
...
Method: Proposes WSAE-Net with two key components: weighted semantic maps to reduce non-semantic feature computations, and auto-adaptive candidate editing sequences to optimize computational order while maintaining semantic relevance.

Result: The methodology demonstrates superior performance in generating counterfactual explanations while maintaining computational efficiency.

Conclusion: WSAE-Net contributes to clearer and deeper understanding of visual counterfactual explanations by addressing semantic relevance and computational optimization challenges.
...
TL;DR: PerTouch is a diffusion-based image retouching framework that balances controllability and subjectivity by using parameter maps for semantic-level editing while maintaining global aesthetics.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of balancing controllability and subjectivity in image retouching, enabling personalized aesthetic preferences while maintaining fine-grained control.

Method: Uses parameter maps with attribute values in semantic regions as input, constructs explicit parameter-to-image mapping, employs semantic replacement and parameter perturbation for boundary perception, and integrates a VLM-driven agent with feedback-driven rethinking and scene-aware memory.
...
TL;DR: Medal S is a medical segmentation foundation model that supports native-resolution spatial and textual prompts in an end-to-end framework, achieving superior multi-class segmentation across CT, MRI, PET, ultrasound, and microscopy modalities with 90% faster inference than sequential approaches.
...
Method: Uses channel-wise alignment between volumetric prompts and text embeddings, lightweight 3D convolutional module for voxel-space refinement, dynamic resampling for target-patch ratio balance, and supports text-only and hybrid prompting modes with optimized preprocessing and two-stage inference.

Result: Outperforms SAT with DSC 75.44 vs 69.83, NSD 77.34 vs 71.06, F1 38.24 vs 24.88, and DSC TP 65.46 vs 46.97 on five-modality average, with 90% faster inference time for 24-class segmentation compared to sequential prompting.

Conclusion: Medal S successfully harmonizes spatial precision with semantic textual guidance, demonstrating superior efficiency and accuracy in multi-class medical segmentation tasks across multiple imaging modalities.
...
TL;DR: Infinite-Story is a training-free framework for consistent text-to-image generation in multi-prompt storytelling, addressing identity and style inconsistency through novel techniques while achieving 6x faster inference than existing methods.


<details>
  <summary>Details</summary>
Motivation: To solve key challenges in consistent T2I generation for storytelling scenarios, specifically identity inconsistency and style inconsistency across multiple prompts, without requiring fine-tuning or suffering from slow inference speeds.

Method: Uses scale-wise autoregressive model with three complementary techniques: Identity Prompt Replacement to mitigate context bias in text encoders, and a unified attention guidance mechanism with Adaptive Style Injection and Synchronized Guidance Adaptation to enforce global style and identity consistency while preserving prompt fidelity.

Result: Achieves state-of-the-art generation performance with high identity and style consistency across diverse prompts, delivering over 6x faster inference (1.72 seconds per image) than existing fastest consistent T2I models.

Conclusion: Infinite-Story is an effective and practical framework for real-world visual storytelling that operates entirely at test time without fine-tuning, offering superior consistency and significantly faster inference speeds.
...
TL;DR: SAGE is a training-free method that mitigates multimodal spurious bias in CLIP models by selecting prompts that maximize semantic separation between classes, improving zero-shot robustness without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: CLIP models develop spurious biases where they rely on background correlations rather than object features, harming out-of-distribution performance. Existing methods require fine-tuning or bias knowledge, limiting CLIP's out-of-the-box usability.

Method: SAGE explores prompt templates and selects those that induce the largest semantic separation between classes, guided by theoretical analysis of multimodal spurious bias in zero-shot classification.

Result: Extensive experiments on 4 benchmark datasets and 5 backbone models show SAGE consistently improves zero-shot performance and generalization, outperforming previous zero-shot approaches without external knowledge or model updates.

Conclusion: SAGE effectively mitigates multimodal spurious bias in CLIP through guided prompt selection, maintaining out-of-the-box usability while improving robustness on out-of-distribution data.
...
TL;DR: DTGS integrates Retinex-based illumination decomposition with thermal-guided 3D Gaussian Splatting to enable robust novel view synthesis under extreme low-light conditions, overcoming illumination inconsistencies and geometric distortions.


<details>
  <summary>Details</summary>
Motivation: Standard 3DGS fails with underexposed inputs due to independent enhancement causing illumination inconsistencies and geometric distortion in low-light novel view synthesis.

Method: Joint optimization framework combining Retinex-inspired illumination decomposition with thermal-guided 3DGS through cyclic enhancement-reconstruction mechanism and thermal supervisory branch.

Result: DTGS significantly outperforms existing baselines on RGBT-LOW dataset, achieving superior radiometric consistency, geometric fidelity, and color stability under extreme illumination.

Conclusion: The unified DTGS framework successfully addresses low-light NVS challenges through tight coupling of enhancement and reconstruction with thermal guidance.
...
TL;DR: BP-FPN is a novel feature pyramid architecture for moving infrared small target detection that addresses fundamental bottlenecks in per-frame feature representation through backpropagation-driven design.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods focus on spatio-temporal feature aggregation but have limited gains, revealing that the fundamental bottleneck lies in ambiguous per-frame feature representations rather than spatio-temporal modeling itself.
...
TL;DR: GeoUniPS is a universal photometric stereo network that combines synthetic supervision with geometric priors from 3D reconstruction models to handle complex real-world scenes where traditional methods fail due to unreliable lighting cues.


<details>
  <summary>Details</summary>
Motivation: Universal photometric stereo struggles in complex real-world scenes with biased lighting, shadows, and self-occlusions where multi-illumination cues become unreliable.

Method: Uses a Light-Geometry Dual-Branch Encoder to extract both multi-illumination cues and geometric priors from frozen 3D reconstruction models, and introduces PS-Perp dataset with perspective projection to overcome orthographic projection limitations.

Result: Achieves state-of-the-art performance across multiple datasets, especially in complex in-the-wild scenes, with both quantitative and qualitative improvements.

Conclusion: Integrating geometric priors from large-scale 3D reconstruction models with photometric stereo enables robust surface normal estimation in challenging real-world conditions.
...
TL;DR: Efficient training and sampling scheme for MeanFlow in Representation Autoencoder latent space, achieving better performance with reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: MeanFlow training is computationally demanding and unstable, with SD-VAE decoder dominating generation costs and requiring complex guidance hyperparameters for class-conditional generation.

Method: Use RAE latent space with pre-trained vision encoder, apply Consistency Mid-Training for initialization, two-stage training with distillation from pre-trained flow matching teacher and optional bootstrapping with one-point velocity estimator.
...
Conclusion: The method stabilizes training, removes need for guidance, simplifies configurations, and significantly reduces computational costs while improving performance.
...
TL;DR: SpectralAdapt is a semi-supervised domain adaptation framework that bridges the domain gap between general and human-centered HSI datasets for hyperspectral image reconstruction from RGB images, addressing data scarcity in medical applications.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging has great potential for healthcare but is costly and technically demanding. The scarcity of human HSI data limits progress in medical applications, while general domain datasets are abundant.

Method: Proposes SpectralAdapt with two key components: Spectral Density Masking (SDM) that adaptively masks RGB channels based on spectral complexity, and Spectral Endmember Representation Alignment (SERA) that uses physically interpretable endmembers as domain-invariant anchors to guide unlabeled predictions.

Result: Experiments on benchmark datasets demonstrate consistent improvements in spectral fidelity, cross-domain generalization, and training stability.

Conclusion: SpectralAdapt effectively mitigates domain shift, spectral degradation, and data scarcity in HSI reconstruction, highlighting SSDA as an efficient solution for hyperspectral imaging in healthcare.
...
TL;DR: REVISOR is a novel multimodal reflection framework that enables MLLMs to collaboratively reflect across text and visual modalities, significantly enhancing long-form video understanding without requiring additional fine-tuning or external models.


<details>
  <summary>Details</summary>
Motivation: Text-only reflection mechanisms are insufficient for long-form video understanding due to rich dynamic visual content and lack of cross-modal interaction capabilities during reflection.

Method: Proposed REVISOR framework with Dual Attribution Decoupled Reward (DADR) mechanism integrated into GRPO training strategy, enabling accurate review of relevant video segments and enforcing causal alignment between reasoning and video evidence.

Result: Achieved impressive results on four benchmarks: VideoMME, LongVideoBench, MLVU, and LVBench, significantly enhancing long-form video understanding capability.

Conclusion: REVISOR successfully addresses limitations of text-only reflection for long-form video understanding by enabling cross-modal collaborative reflection, demonstrating substantial improvements across multiple benchmarks.
...
TL;DR: Ocean is an object-centric 3D semantic scene completion framework that decomposes scenes into object instances using MobileSAM segmentation, then refines features through 3D semantic group attention and instance-aware diffusion to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing ego-centric approaches for 3D semantic scene completion overlook fine-grained object-level details, leading to semantic and geometric ambiguities in complex environments like autonomous driving scenarios.

Method: 1) Use MobileSAM for instance mask extraction from images; 2) 3D Semantic Group Attention with linear attention for object-centric feature aggregation; 3) Global Similarity-Guided Attention to handle segmentation errors; 4) Instance-aware Local Diffusion for feature refinement in BEV space.

Result: Achieved state-of-the-art performance on SemanticKITTI (17.40 mIoU) and SSCBench-KITTI360 (20.28 mIoU) benchmarks.

Conclusion: Object-centric decomposition with instance-level processing significantly improves 3D semantic scene completion accuracy compared to ego-centric approaches, especially in complex autonomous driving environments.
...
TL;DR: Uni-Inter is a unified framework for human motion generation that handles human-human, human-object, and human-scene interactions within a single architecture using a shared volumetric representation.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on task-specific designs with limited generalization, needing a unified approach for diverse interaction scenarios.

Method: Introduces Unified Interactive Volume (UIV) to encode heterogeneous interactive entities into a shared spatial field, with joint-wise probabilistic prediction over UIV for motion generation.

Result: Achieves competitive performance across three interaction tasks and generalizes well to novel entity combinations.
...
TL;DR: A lightweight framework for multilingual vision-language alignment that uses English as semantic anchor, requiring no multilingual image-text pairs and only training a small projection module.


<details>
  <summary>Details</summary>
Motivation: CLIP models perform poorly on low-resource languages due to scarce multilingual image-text data, with existing models showing low retrieval performance in underrepresented languages like Czech, Finnish, Croatian, Hungarian, and Romanian.

Method: Freezes pretrained image and multilingual text encoders, trains only a compact 1.7M-parameter projection module using contrastive loss over English representations as semantic anchors, requiring no image-text or text-text pairs.

Result: Significant performance gains in five underrepresented languages on multilingual retrieval benchmarks, achieving robust multilingual alignment even with limited supervision.

Conclusion: The pivot-based, parameter-efficient alignment strategy effectively enables inclusive multimodal learning for low-resource languages without requiring extensive multilingual training data.
...
TL;DR: Proposes MGCA-Net for open-vocabulary temporal action localization, using multi-grained category awareness to improve recognition of both base and novel action categories without requiring training data for all categories.


<details>
  <summary>Details</summary>
Motivation: Existing methods recognize action categories at a single granularity, which degrades accuracy for both base and novel categories in open-vocabulary settings.

Method: Multi-Grained Category-Aware Network with localizer, action presence predictor, conventional classifier for base actions, and coarse-to-fine classifier for novel actions that identifies action presence at video granularity and assigns categories at proposal granularity.
...
TL;DR: DiffPixelFormer is a Transformer-based model for RGB-D indoor semantic segmentation that enhances intra-modal representations and models inter-modal interactions using a Differential-Shared Inter-Modal module for fine-grained pixel-level alignment.


<details>
  <summary>Details</summary>
Motivation: Existing RGB-D fusion methods rely on computationally intensive cross-attention and insufficiently model intra- and inter-modal feature relationships, leading to imprecise feature alignment and limited discriminative representation.

Method: Proposes Intra-Inter Modal Interaction Block (IIMIB) with self-attention for intra-modal dependencies and Differential-Shared Inter-Modal (DSIM) module to disentangle modality-specific/shared cues. Uses dynamic fusion strategy to balance modality contributions.
...
Conclusion: DiffPixelFormer effectively enhances RGB-D indoor semantic segmentation through improved intra-modal representations and fine-grained inter-modal interactions, demonstrating state-of-the-art performance.
...
TL;DR: The paper introduces Pretext-GRPO and ViSS-R1 frameworks that integrate self-supervised learning into MLLMs to enhance visual-centric video reasoning by forcing models to process transformed visual inputs and reconstruct original videos.


<details>
  <summary>Details</summary>
Motivation: Current R1-based MLLMs for video reasoning often underutilize rich visual information, leading to shortcut learning and increased hallucination, necessitating more robust visual-centric approaches.

Method: Proposes Pretext-GRPO (self-supervised reinforcement learning algorithm) and ViSS-R1 framework that integrates pretext-task-based learning into MLLM's R1 post-training, requiring models to identify transformations and reconstruct original videos while answering queries.

Result: Comprehensive evaluations on six video reasoning benchmarks demonstrate the effectiveness and superiority of the proposed methods for complex video reasoning tasks.

Conclusion: The proposed Pretext-GRPO and ViSS-R1 frameworks successfully address visual underutilization in video reasoning by integrating self-supervised learning, leading to more robust and accurate video understanding.
...
TL;DR: MonoUnc is a BEV-free 3D lane detector that explicitly models aleatoric uncertainty using local lane structures, outperforming previous SoTA methods on major benchmarks.


<details>
  <summary>Details</summary>
Motivation: Address aleatoric uncertainty in monocular 3D lane detection caused by inherent observation noise, overcoming limitations of existing methods that rely on simplified geometric assumptions and fail to capture structural variations.

Method: Projects 3D lanes to front-view space as parametric curves, generates curve-point query embeddings for 3D point predictions, models each lane segment as 3D Gaussian parameterized by local structure and uncertainty, and uses novel 3D Gaussian matching loss.

Result: Outperforms previous state-of-the-art methods across all benchmarks on ONCE-3DLanes and OpenLane datasets under stricter evaluation criteria.

Conclusion: MonoUnc effectively addresses aleatoric uncertainty in 3D lane detection through local structure modeling and achieves superior performance, with proposed comprehensive evaluation metrics for better error quantification.
...
TL;DR: A novel framework that transfers knowledge from Segment Anything 2 (SAM2) to electron microscopy (EM) neuron segmentation, using feature-guided attention and dual-affinity decoding to bridge domain gaps and achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: EM image segmentation faces challenges from complex neuron morphologies, low signal-to-noise ratios, and limited annotations, which hinder accuracy and generalization of existing methods.

Method: Extract SAM2 features, use Feature-Guided Attention module to guide a lightweight Fine-Grained Encoder with semantic cues, and employ dual-affinity decoder for coarse and refined affinity maps.
...
Conclusion: Transferring natural image pre-trained representations with domain-adaptive guidance effectively addresses EM neuron segmentation challenges.
...
<details>
  <summary>Details</summary>
Motivation: Systematic evaluation of gait recognition robustness to real-world corruptions and silhouette variability is lacking, despite strong performance on controlled datasets.

Method: Developed RobustGait framework evaluating 15 corruption types at 5 severity levels across multiple datasets, testing six state-of-the-art gait systems across perturbation types, silhouette extraction methods, architectural capacities, and deployment scenarios.

Result: Key findings: RGB-level noise better reflects real degradation; gait accuracy is sensitive to silhouette extractor biases; robustness depends on perturbation type and architectural design; noise-aware training and knowledge distillation improve performance.

Conclusion: RobustGait provides comprehensive robustness evaluation revealing critical vulnerabilities in gait recognition systems and demonstrates that robustness-enhancing strategies can improve deployment readiness.
...
TL;DR: AdaptiveAD addresses over-reliance on ego status in autonomous driving systems by decoupling scene perception from ego status using a dual-branch architecture with adaptive fusion.


<details>
  <summary>Details</summary>
Motivation: Existing modular autonomous driving architectures suffer from over-reliance on ego status, which hinders generalization and robust scene understanding by allowing ego status to be used as a shortcut.

Method: Proposes AdaptiveAD with dual-branch structure: one branch for scene-driven reasoning without ego status in BEV encoder, another for ego-driven planning. Uses scene-aware fusion module to integrate decisions, with path attention mechanism and auxiliary tasks (BEV unidirectional distillation, autoregressive online mapping).
...
Conclusion: The architectural-level solution effectively addresses ego status dependency through explicit decoupling and adaptive fusion, enhancing generalization and robustness in autonomous driving planning.
...
TL;DR: The paper introduces the RFxG taxonomy to organize saliency explanations along Reference-Frame (pointwise vs contrastive) and Granularity (class-level vs group-level) axes, revealing limitations in current evaluation metrics and proposing new faithfulness metrics for comprehensive assessment.
...
Method: Propose the Reference-Frame × Granularity (RFxG) taxonomy framework, develop four novel faithfulness metrics to systematically assess explanation quality across both dimensions, and conduct comprehensive evaluation on ten saliency methods, four model architectures, and three datasets.

Result: Demonstrate critical limitations in existing evaluation metrics that prioritize pointwise faithfulness while neglecting contrastive reasoning and semantic granularity. The RFxG framework provides a principled way to organize and evaluate saliency explanations.

Conclusion: Advocate for a shift toward user-intent-driven evaluation, providing both conceptual foundation and practical tools to develop visual explanations that are faithful to model behavior and meaningfully aligned with human understanding and inquiry.
...
TL;DR: MergeSlide is a lifelong learning framework for Whole Slide Images that treats continual learning as a model merging problem using vision-language pathology foundation models, achieving state-of-the-art performance on TCGA datasets.


<details>
  <summary>Details</summary>
Motivation: To reduce the resources and effort required for data transfer and processing of gigabyte-scale Whole Slide Images in cancer-related tasks through lifelong learning.

Method: Uses class-aware prompts for task definition, fine-tunes with MLP-free backbone, employs orthogonal continual merging strategy, and introduces Task-to-Class Prompt-aligned inference for unknown task identity.

Result: Outperforms both rehearsal-based continual learning and vision-language zero-shot baselines on six TCGA datasets.

Conclusion: MergeSlide provides an effective solution for lifelong learning on WSIs by leveraging model merging and vision-language foundation models, demonstrating superior performance while mitigating catastrophic forgetting.
...
TL;DR: CapeNext overcomes limitations of static joint embeddings in Category-Agnostic Pose Estimation by integrating hierarchical cross-modal interaction and dual-stream feature refinement, achieving state-of-the-art performance on MP-100 dataset.


<details>
  <summary>Details</summary>
Motivation: Static textual keypoint descriptions in CAPE suffer from polysemy-induced cross-category ambiguity (e.g., 'leg' meaning different things for humans vs furniture) and insufficient discriminability for fine-grained intra-category variations (e.g., posture differences between cats).

Method: Proposes a framework that integrates hierarchical cross-modal interaction with dual-stream feature refinement, enhancing joint embeddings with both class-level and instance-specific cues from textual descriptions and specific images.
...
Conclusion: The proposed hierarchical cross-modal interaction and dual-stream refinement effectively address the limitations of static joint embeddings in CAPE, significantly improving pose estimation performance across diverse categories.
...
<details>
  <summary>Details</summary>
Motivation: Real-world tracking scenarios involve both linear and non-linear motion patterns, but current approaches either use computationally efficient Kalman filters (which fail on non-linear motion) or data-driven predictors (which suffer from limited generalization and computational overhead).

Method: Proposes PlugTrack framework that employs multi-perceptive motion analysis to generate adaptive blending factors, fusing Kalman filter and data-driven motion predictors adaptively based on motion patterns.

Result: Achieves significant performance gains on MOT17/MOT20 datasets and state-of-the-art performance on DanceTrack, demonstrating that Kalman filters outperform data-driven predictors in up to 34% of cases even in non-linear motion datasets.

Conclusion: PlugTrack successfully bridges classical and modern motion prediction paradigms through adaptive fusion, being the first framework to achieve this in multi-object tracking while maintaining computational efficiency and generalization.
...
TL;DR: Proposes the first dataset distillation method for medical image enhancement that addresses the many-to-many mapping challenge in low-level tasks by leveraging anatomical priors and personalized generation.


<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods focus on high-level tasks with many-to-one mappings, but low-level medical image enhancement requires pixel-level fidelity, making it an underdetermined problem that current methods cannot solve effectively.

Method: Uses shared anatomical priors from representative patients as initialization, then personalizes for each patient using Structure-Preserving Personalized Generation (SPG) module. Aligns gradients between distilled data and raw patient data while preserving privacy by excluding patient-specific details.

Result: Enables creation of distilled datasets for medical image enhancement that maintain pixel-level fidelity while significantly reducing training and storage costs, with privacy preservation.

Conclusion: The proposed method successfully addresses the underdetermined problem of low-level dataset distillation for medical images, enabling practical deployment of enhancement methods while preserving patient privacy.
...
TL;DR: DGS-Net is a novel framework that preserves transferable pre-trained priors in CLIP models while suppressing task-irrelevant components through gradient-space decomposition, achieving superior AI-generated image detection performance and cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Address concerns about misinformation and trust erosion from AI-generated images, while solving the catastrophic forgetting problem that occurs when fine-tuning large multimodal models like CLIP for synthetic content detection.

Method: Proposes Distillation-guided Gradient Surgery Network (DGS-Net) with gradient-space decomposition that separates harmful and beneficial descent directions, projecting task gradients onto orthogonal complement of harmful directions and aligning with beneficial ones from frozen CLIP encoder.
...
Conclusion: DGS-Net effectively preserves transferable pre-trained priors while suppressing irrelevant components, providing a robust solution for AI-generated image detection with strong cross-domain generalization capabilities.
...
TL;DR: Proposes an unsupervised image dehazing method using implicit neural degradation representation that combines channel-independent and channel-dependent mechanisms for better haze removal in complex scenes.
...
Conclusion: The proposed unsupervised method effectively handles complex haze distributions and achieves high-quality image restoration without explicit feature extraction or physical models.
...
<details>
  <summary>Details</summary>
Motivation: Existing deraining methods struggle with preserving semantic and spatial details, which is critical for applications like autonomous driving and video surveillance.

Method: Multi-Prior Hierarchical Mamba network combining macro-semantic (CLIP) and micro-structural (DINOv2) priors with progressive fusion injection and Fourier-enhanced dual-path Mamba modules.

Result: Achieves 0.57 dB PSNR gain on Rain200H dataset and superior generalization on real-world rainy scenarios.

Conclusion: MPHM effectively addresses semantic and structural detail preservation in image deraining through synergistic multi-prior integration and hierarchical Mamba architecture.
...
TL;DR: Proposes a Rotationally Invariant Features (RIF) framework for 3D anomaly detection that addresses orientation and position variations through coordinate mapping and lightweight feature extraction.


<details>
  <summary>Details</summary>
Motivation: Existing 3D anomaly detection methods struggle with point clouds that have changes in orientation and position, causing significant feature variations that hinder detection performance.

Method: Uses Point Coordinate Mapping (PCM) to map points into rotationally invariant space, and a lightweight Convolutional Transform Feature Network (CTF-Net) with transfer learning pre-training using 3D data augmentation.

Result: Achieves 17.7% average P-AUROC improvement on Anomaly-ShapeNet and 1.6% improvement on Real3D-AD dataset, demonstrating strong generalization ability when combined with traditional methods.

Conclusion: The RIF framework provides robust rotationally invariant features that significantly improve 3D anomaly detection performance and shows great potential for industrial applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle with fine-grained details in close-up scenarios due to severe input limitations and modest viewpoint variations, making sparse-view 3D scene reconstruction challenging.
...
Conclusion: The CloseUpShot framework successfully addresses the challenges of close-up novel view synthesis from sparse inputs through improved conditioning techniques and global geometric constraints.
...
Method: Proposes RePo which: 1) Maps GPS trajectories to grid sequences for region-wise representation with structural and semantic features, 2) Uses three expert networks to extract local, correlation, and continuous movement patterns for point-wise representation, 3) Employs a router network to fuse point-wise features and cross-attention to combine with region-wise features, 4) Trains with contrastive loss using hard negative samples.
...
Conclusion: Jointly encoding region-wise and point-wise features effectively captures comprehensive trajectory information for similarity modeling, significantly outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Prior jailbreak attacks on T2V models use obvious adversarial perturbations that are easy to detect, while benign-looking prompts with rich implicit cues can better circumvent safety measures while preserving malicious intent.

Method: VEIL uses modular prompt design with three components: neutral scene anchors for plausibility, latent auditory triggers that exploit audio-visual associations, and stylistic modulators to amplify effects. Attack generation is formulated as constrained optimization solved via guided search.
...
Conclusion: VEIL demonstrates that implicit cues in seemingly benign prompts can effectively jailbreak T2V models, revealing critical vulnerabilities in current safety mechanisms that rely on explicit content detection.
...
TL;DR: ILA is a black-box adversarial attack framework that manipulates indoor lighting to disrupt VLN agents, revealing vulnerabilities to realistic lighting variations.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial evaluations use unrealistic perturbations, while indoor lighting is an intrinsic but overlooked scene attribute that strongly influences navigation in real-world environments.

Method: Proposed Indoor Lighting-based Adversarial Attack (ILA) with two modes: Static (SILA) with constant lighting intensity and Dynamic (DILA) with lights switched on/off at critical moments.
...
TL;DR: MedGEN-Bench is a comprehensive medical multimodal benchmark with 6,422 expert-validated image-text pairs across multiple modalities and clinical tasks, designed to evaluate AI systems on contextually intertwined instructions requiring cross-modal reasoning and open-ended generation.


<details>
  <summary>Details</summary>
Motivation: Address limitations in existing medical visual benchmarks that use ambiguous queries, oversimplify diagnostic reasoning, and overlook image generation capabilities, while meeting clinicians' needs for AI systems that generate both textual diagnoses and corresponding medical images.

Method: Created MedGEN-Bench with three formats (Visual Question Answering, Image Editing, Contextual Multimodal Generation) and employed a three-tier assessment framework integrating pixel-level metrics, semantic text analysis, and expert-guided clinical relevance scoring.

Result: Systematically evaluated 10 compositional frameworks, 3 unified models, and 5 VLMs using the benchmark, demonstrating its effectiveness in assessing medical AI systems.

Conclusion: MedGEN-Bench provides a comprehensive evaluation framework that advances medical AI research by focusing on contextually intertwined instructions and cross-modal reasoning, moving beyond traditional multiple-choice formats.
...
TL;DR: WinMamba is a novel Mamba-based 3D object detection backbone that addresses spatial information loss in existing methods through window-scale-adaptive modules and window-shift strategies, achieving superior performance on autonomous driving datasets.


<details>
  <summary>Details</summary>
Motivation: Current 3D object detection methods struggle to balance computational efficiency and long-range dependency capture. Mamba-based models offer efficiency advantages but discard spatial information through fixed-window axis-aligned scanning.

Method: Proposes WinMamba blocks with window-scale-adaptive modules for multi-scale voxel feature compensation and WinMamba layers with learnable positional encoding and window-shift strategy for rich contextual cues in linear state space.

Result: Extensive experiments on KITTI and Waymo datasets show WinMamba significantly outperforms baselines. Ablation studies confirm the individual contributions of WSF and AWF modules in improving detection accuracy.

Conclusion: WinMamba effectively addresses spatial information loss in Mamba-based 3D detection, achieving better balance between efficiency and accuracy through adaptive window scaling and shift strategies.
...
TL;DR: This paper explores computer vision methods for road distress segmentation, evaluating GAN-generated synthetic data and comparing CNN vs transformer-based models, with MaskFormer showing superior performance.


<details>
  <summary>Details</summary>
Motivation: America's road infrastructure is graded poorly (D), with inefficient manual inspection methods. The availability of real-time visual data from autonomous vehicles presents an opportunity to apply computer vision for more efficient road monitoring and maintenance.

Method: Evaluated synthetic data generated with GANs for model training, applied CNNs for road distress segmentation, and examined the transformer-based model MaskFormer.

Result: GAN-generated data improved model performance. MaskFormer outperformed the CNN model in two key metrics: mAP50 and IoU.

Conclusion: Computer vision methods, particularly transformer-based models like MaskFormer with synthetic data augmentation, offer promising solutions for automated road distress detection and infrastructure monitoring.
...
TL;DR: CSIP-ReID is a novel skeleton-driven pretraining framework for video person re-identification that replaces text-based approaches with skeleton sequences to capture fine-grained temporal motion cues.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal pretraining for video ReID relies on video-text pairs, which lack genuine multimodal learning and fail to capture essential fine-grained temporal motion information needed for identity distinction.

Method: Two-stage approach: (1) Contrastive learning to align skeleton and visual features at sequence level, (2) Dynamic Prototype Fusion Updater (PFU) to refine multimodal identity prototypes, and Skeleton Guided Temporal Modeling (SGTM) module to distill temporal cues from skeletons into visual features.

Result: Achieves state-of-the-art results on standard video ReID benchmarks (MARS, LS-VID, iLIDS-VID) and shows strong generalization to skeleton-only ReID tasks (BIWI, IAS), significantly outperforming previous methods.
...
TL;DR: SOMA is a dense registration framework that integrates structural gradient priors into deep features for SAR-optical image matching, achieving significant improvements in registration precision.


<details>
  <summary>Details</summary>
Motivation: SAR and optical images have different imaging mechanisms making pixel-level registration challenging. Deep learning hasn't effectively leveraged gradient cues that are crucial in traditional methods for highlighting structural differences.

Method: Proposes Feature Gradient Enhancer (FGE) that embeds multi-scale, multi-directional gradient filters into feature space using attention and reconstruction. Uses Global-Local Affine-Flow Matcher (GLAM) combining affine transformation and flow-based refinement in coarse-to-fine architecture.

Result: Significantly improves registration precision: increases CMR@1px by 12.29% on SEN1-2 dataset and 18.50% on GFGE_SO dataset. Exhibits strong robustness and generalization across diverse scenes and resolutions.

Conclusion: SOMA effectively bridges the gap between traditional gradient-based methods and deep learning for SAR-optical registration, demonstrating superior performance and generalization capabilities.
...
TL;DR: THIR is an unsupervised medical image retrieval framework that uses topological data analysis (Betti numbers from persistent homology) to extract structural patterns from histopathological images without training, outperforming supervised methods on breast cancer datasets.


<details>
  <summary>Details</summary>
Motivation: Breast cancer caused 685,000 deaths in 2020, highlighting the need for early diagnosis and accurate clinical decision making through efficient image retrieval systems.

Method: Uses cubical persistence to extract topological fingerprints (Betti numbers) from RGB histopathological images, encoding loop evolution as compact feature vectors, then performs similarity retrieval by computing distances between these topological descriptors.

Result: Outperforms state-of-the-art supervised and unsupervised methods on BreaKHis dataset, processes entire dataset in under 20 minutes on standard CPU.
...
TL;DR: HDW-SR is a diffusion-based super-resolution method that uses wavelet decomposition to focus on high-frequency details, replacing standard U-Net with frequency-aware architecture for better fine detail recovery.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based SISR methods produce blurred fine details due to insufficient guidance in the high-frequency domain, limiting their ability to restore sharp image features.

Method: Proposes HDW-SR with wavelet-based downsampling for multi-scale frequency decomposition, diffusion only on residual maps, sparse cross-attention between frequency subbands, and Dynamic Thresholding Block for high-frequency refinement.

Result: Experiments on synthetic and real-world datasets show competitive super-resolution performance with superior recovery of fine-grained image details compared to existing methods.

Conclusion: HDW-SR effectively addresses high-frequency detail blurring in diffusion-based super-resolution through frequency-aware architecture and wavelet decomposition, achieving improved detail restoration.
...
TL;DR: GenTract is the first generative model for global tractography that learns direct mapping from dMRI to complete, anatomically plausible streamlines, achieving significantly higher precision than existing methods.


<details>
  <summary>Details</summary>
Motivation: Local tractography methods suffer from error accumulation and high false positive rates, while global methods are computationally expensive. There's a need for a better approach that combines the benefits of both.

Method: Frames tractography as a generative task using both diffusion-based and flow matching paradigms to learn direct mapping from dMRI data to complete streamlines.

Result: GenTract achieves precision 2.1x higher than the next-best method (TractOracle), with even more pronounced advantages in challenging low-resolution and noisy settings where it outperforms competitors by an order of magnitude.
...
TL;DR: ViXML framework integrates decoder-only models and visual information for Extreme Multi-label Classification, achieving state-of-the-art performance while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: To harness larger decoder-only models and exploit visual information in Extreme Multi-label Classification (XMC) while balancing efficiency and performance, as current approaches mainly use small encoder-only transformers.

Method: Proposes Vision-enhanced eXtreme Multi-label Learning (ViXML) framework that efficiently integrates foundation vision models by pooling a single embedding per image, and uses decoder-only models with manageable computational overhead.

Result: ViXML with small encoders outperforms text-only decoders in most cases, showing substantial improvements (+8.21% in P@1 on largest dataset) while limiting computational growth.

Conclusion: Visual information and decoder-only models play critical roles in XMC separately and can be combined for improved performance, with images providing benefits equivalent to billions of parameters.
...
TL;DR: Proposes Object-Centric 3D Rollout (OCR) to improve video spatial reasoning in MLLMs by introducing structured perturbations to object geometry during training, forcing holistic scene reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs suffer from query-locked reasoning - focusing only on objects mentioned in prompts while ignoring contextual cues, limiting robust video spatial reasoning capabilities.
...
Conclusion: OCR effectively addresses query-locked reasoning by compelling models to reason holistically across entire scenes, demonstrating superior performance over prior rollout strategies.
...
TL;DR: A differentiable stroke reconstruction framework that unifies painting, stylized texturing, and smudging to reproduce the human painting-smudging loop, producing realistic stroke reconstructions with smooth shading.


<details>
  <summary>Details</summary>
Motivation: Existing generative models focus on final image generation or patch-based process simulation, lacking explicit stroke structure and realistic shading in painting synthesis.

Method: Optimizes single- and dual-color Bezier strokes through parallel differentiable paint renderer, adds style generation for geometry-conditioned textures, and introduces differentiable smudge operator for color blending and shading with coarse-to-fine optimization.
...
Conclusion: Offers a unified model for expressive digital painting creation that faithfully reproduces the human painting-smudging process with explicit stroke structure and realistic shading.
...
TL;DR: MonoDLGD is a difficulty-aware label-guided denoising framework for monocular 3D object detection that adaptively perturbs ground-truth labels based on detection uncertainty to improve geometric supervision and handle varying object complexity.


<details>
  <summary>Details</summary>
Motivation: Monocular 3D object detection suffers from ambiguous depth cues and existing DETR-based methods struggle with inaccurate depth estimates while ignoring instance-level detection difficulty factors like occlusion, distance, and truncation.

Method: Proposes a difficulty-aware label-guided denoising framework that adaptively perturbs ground-truth labels (stronger perturbations for easier instances, weaker for harder cases) and reconstructs them to provide explicit geometric supervision, jointly optimizing label reconstruction and 3D object detection.

Result: Extensive experiments on KITTI benchmark demonstrate state-of-the-art performance across all difficulty levels.

Conclusion: MonoDLGD effectively improves geometry-aware representation learning and robustness to varying object complexity through adaptive label perturbation and reconstruction.
...
TL;DR: Self-supervised pipeline extracts ultrasound images from monitor photos, bypassing DICOM transfer bottlenecks for rapid algorithm testing.


<details>
  <summary>Details</summary>
Motivation: Ultrasound machines display images on built-in monitors but require DICOM for hospital system transfer, creating a bottleneck that slows down testing and prototyping of new algorithms.

Method: Proposed a self-supervised pipeline to extract and rectify ultrasound images from photographs of the monitor display.

Result: In proof-of-concept study, rectified images maintained sufficient visual fidelity to classify cardiac views with 0.79 balanced accuracy compared to native DICOM images.

Conclusion: The method successfully bypasses DICOM transfer requirements and enables rapid testing and prototyping of ultrasound algorithms using monitor photographs.
...
TL;DR: RefineVAD is a weakly-supervised video anomaly detection framework that jointly models temporal motion patterns and semantic structures of different anomaly types through motion-aware temporal attention and category-oriented refinement.


<details>
  <summary>Details</summary>
Motivation: Existing methods oversimplify anomalies as a single category, ignoring the diverse semantic and temporal characteristics of real-world anomalies. Humans perceive anomalies by jointly interpreting temporal motion patterns and semantic structures.

Method: Two core modules: Motion-aware Temporal Attention and Recalibration (MoTAR) for motion salience estimation and dynamic temporal focus adjustment, and Category-Oriented Refinement (CORE) for injecting soft anomaly category priors through cross-attention with learnable prototypes.

Result: Extensive experiments on WVAD benchmark validate RefineVAD's effectiveness and highlight the importance of integrating semantic context to guide feature refinement toward anomaly-relevant patterns.

Conclusion: RefineVAD successfully models both "how" motion evolves and "what" semantic category it resembles by jointly leveraging temporal dynamics and semantic structure, improving weakly-supervised video anomaly detection.
...
TL;DR: PAVE-Net is the first fully end-to-end framework for multi-person 2D pose estimation in videos, eliminating heuristic operations like detection, RoI cropping, and NMS through a novel pose-aware attention mechanism and spatiotemporal modeling.


<details>
  <summary>Details</summary>
Motivation: Existing two-stage methods rely on heuristic operations (detection, RoI cropping, NMS) that limit accuracy and efficiency. The goal is to create a fully end-to-end approach that eliminates these limitations.

Method: Proposes PAVE-Net with spatial encoder for intra-frame relations and spatiotemporal pose decoder for global dependencies. Uses pose-aware attention mechanism for temporal association and explicitly models spatiotemporal dependencies among pose keypoints.
...
Conclusion: PAVE-Net demonstrates that fully end-to-end multi-person video pose estimation is feasible and outperforms existing methods in both accuracy and efficiency, setting a new state-of-the-art approach.
...
TL;DR: 3DAlign-DAER is a unified framework for fine-grained text-3D geometry alignment using dynamic attention policy and efficient retrieval strategy, achieving superior performance on cross-modal tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to align fine-grained textual semantics with detailed geometric structures and degrade when scaling to large 3D databases.

Method: Proposes dynamic attention policy with Hierarchical Attention Fusion and Monte Carlo tree search for attention calibration, plus efficient hierarchical retrieval strategy for inference.

Result: Extensive experiments show superior performance on diverse benchmarks, with the framework outperforming traditional methods in both accuracy and efficiency.

Conclusion: 3DAlign-DAER effectively addresses fine-grained text-3D alignment challenges and scales well to large datasets, with codes, models, and the new Align3D-2M dataset to be released.
...
TL;DR: HARL is a hybrid-domain adaptive learning framework for robust gaze estimation that disentangles gaze-relevant features from low-quality facial images by aligning with high-quality near-eye images and incorporates head-pose geometric constraints.


<details>
  <summary>Details</summary>
Motivation: Current appearance-based gaze estimation methods suffer significant performance degradation in cross-domain evaluation due to interference from gaze-irrelevant factors like expressions, wearables, and image quality.

Method: Proposes Hybrid-domain Adaptive Representation Learning (HARL) that: 1) Disentangles gaze-relevant representation by aligning features from high-quality near-eye images in unsupervised domain adaptation, 2) Uses sparse graph fusion module to explore geometric constraints between gaze direction and head-pose.
...
Conclusion: The proposed HARL framework effectively learns robust gaze representation by combining hybrid-domain adaptation and geometric constraints, demonstrating superior performance across multiple datasets.
...
TL;DR: MRIQT is a 3D diffusion model that enhances portable ultra-low-field MRI (0.064T) to high-field quality for neonatal brain imaging, achieving superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Portable ultra-low-field MRI offers accessible neonatal care but suffers from low signal-to-noise ratio and poor diagnostic quality compared to high-field MRI.
...
Result: Surpasses recent GAN and CNN baselines by 15.3% in PSNR, with physicians rating 85% of outputs as good quality with clear pathology present.

Conclusion: MRIQT enables high-fidelity, diffusion-based enhancement of portable ultra-low-field MRI for reliable neonatal brain assessment.
...
TL;DR: MMD-Thinker is a two-stage framework that uses adaptive multi-dimensional thinking for multimodal misinformation detection, addressing limitations of general-purpose MLLMs through task-specific instruction tuning and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Multimodal misinformation with low creation cost and high deception poses significant threats, while current MLLM-based detectors suffer from insufficient reasoning and biases due to uniform thinking paradigms.

Method: Develop tailor-designed thinking modes, use task-specific instruction tuning to inject these modes into MLLMs, and apply reinforcement learning with mixed advantage function to enhance reasoning capabilities. Also created MMR dataset with 8K+ image-text pairs.
...
TL;DR: RFMNet improves camouflaged object detection by fusing multi-stage features from reference salient images with camouflage features using overlapped windows cross-attention and referring feature aggregation.


<details>
  <summary>Details</summary>
Motivation: To enhance camouflaged object detection performance by leveraging rich salient image features through multi-context fusion with camouflaged object features, focusing on local information matching.
...
Conclusion: The proposed RFMNet with multi-context fusion and local attention mechanisms effectively improves referring camouflaged object detection performance.
...
TL;DR: GeoX-Bench is a comprehensive benchmark for evaluating large multimodal models on cross-view geo-localization and pose estimation tasks using 10,859 panoramic-satellite image pairs across 128 cities.


<details>
  <summary>Details</summary>
Motivation: To explore and evaluate LMMs' capabilities in cross-view geo-localization and pose estimation domains, which remain unexplored despite potential benefits for navigation, autonomous driving, and outdoor robotics.

Method: Created GeoX-Bench with 10,859 panoramic-satellite image pairs spanning 128 cities in 49 countries and 755,976 QA pairs, then evaluated 25 state-of-the-art LMMs and explored instruction-tuning capabilities.
...
Conclusion: GeoX-Bench highlights a critical area for future improvement in LMMs' pose estimation capabilities and demonstrates that instruction-tuning can enhance cross-view geo-sense abilities.
...
<details>
  <summary>Details</summary>
Motivation: To address the gap in AI assistants for step-by-step procedural support in egocentric (first-person) perspectives, driven by advances in vision language models and egocentric perception research.

Method: Comprehensive review of current techniques, datasets, and evaluation metrics; novel experiments evaluating representative VLM-based methods; technical analysis and gap identification.

Result: Established a new taxonomy for egocentric procedural AI assistants; identified limitations of existing VLM-based approaches; provided comprehensive evaluation framework.

Conclusion: Identified research challenges and future directions for developing effective egocentric procedural AI assistants, with ongoing repository for latest work collection.
...
<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting has high memory footprint that scales with scene complexity, and existing compression methods have limitations in exploiting symmetry-based redundancies.

Method: Proposes a plug-and-play compression framework that introduces learnable mirrors to detect and eliminate local and global reflective redundancies in 3DGS scenes.

Result: Achieves 1.66× compression over HAC (up to 3× on large scenes) and 108× overall compression of 3DGS scenes while maintaining rendering quality.

Conclusion: SymGS effectively surpasses existing compression limits by incorporating symmetry-aware techniques, providing significant memory reduction for 3D Gaussian Splatting scenes.
...
TL;DR: SpatialSky-Bench benchmark reveals VLMs' poor spatial intelligence in UAV navigation. Sky-VLM model achieves SOTA performance using new SpatialSky-Dataset.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs lack spatial intelligence capabilities for UAV tasks, raising concerns about their effectiveness in dynamic aerial environments.

Method: Created SpatialSky-Bench benchmark with 13 subcategories, developed SpatialSky-Dataset with 1M samples, and built Sky-VLM specialized for UAV spatial reasoning.

Result: Mainstream VLMs performed unsatisfactorily in complex UAV scenarios. Sky-VLM achieved state-of-the-art performance across all benchmark tasks.

Conclusion: Sky-VLM paves the way for developing VLMs suitable for UAV scenarios by addressing spatial intelligence gaps.
...
TL;DR: A dual-backbone framework combining CNN and transformer features with top-k pooling achieves 90.7% AUC for video anomaly detection using only video-level supervision.
...
Conclusion: The proposed framework effectively detects diverse anomalies in surveillance videos using only video-level supervision.
...
<details>
  <summary>Details</summary>
Motivation: Conventional multi-view geometry pipelines are cumbersome and quality-sensitive due to dense reconstruction, meshing, and simplification steps. There's a need for direct lightweight building surface reconstruction.

Method: Train 3D Gaussian Splatting field, use normal-gradient-guided Gaussian optimization to select primitives aligned with building structures, apply multi-view edge-consistency pruning, and perform depth-constrained Delaunay triangulation.

Result: Achieves substantially fewer faces and vertices while maintaining computational efficiency, directly producing lightweight building models from multi-view imagery.

Conclusion: SF-Recon provides an effective method for direct lightweight building surface reconstruction without post-processing simplification, demonstrating improved efficiency and structural fidelity.
...
TL;DR: DTO-Humans: A novel method for generating scene-consistent multi-person human mesh pseudo-ground-truth using depth-conditioned optimization, enabling metric-scale human mesh recovery with state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current multi-person human mesh recovery methods lack scene-level consistency due to single-person-centric pseudo-ground-truth generation, resulting in conflicting depths and scales among individuals in the same image.

Method: Proposes Depth-conditioned Translation Optimization (DTO) that jointly refines camera-space translations using anthropometric priors and monocular depth cues within a MAP framework. Also introduces Metric-Aware HMR with relative metric loss for direct metric-scale estimation.

Result: Created DTO-Humans dataset with 0.56M high-quality, scene-consistent multi-person images (avg 4.8 persons/image). Achieved SOTA performance on relative depth reasoning and human mesh recovery.

Conclusion: The proposed DTO method successfully addresses scene-level consistency in multi-person mesh recovery, and Metric-Aware HMR enables direct metric-scale estimation, advancing the field of in-the-wild human mesh reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Table images have unique challenges including redundant background regions and the need for question-specific focus. Existing MLLMs overlook these characteristics, resulting in uninformative and redundant visual representations.

Method: Three key approaches: 1) Progressive question conditioning that injects questions into Vision Transformer layers with increasing frequency, 2) Token pruning strategy to discard background tokens, 3) Token focusing training to concentrate essential information in retained tokens.
...
Conclusion: The proposed methods effectively address table understanding challenges by generating informative and compact visual features, demonstrating that efficiency and performance can be improved simultaneously in MLLMs for table understanding.
...
TL;DR: SkyReels-Text is a font-controllable framework for precise poster text editing that enables simultaneous editing of multiple text regions with different typographic styles while preserving non-edited areas, without requiring font labels or fine-tuning during inference.
...
Method: The framework uses cropped glyph patches as input to control typography, allowing users to provide desired font styles even if not in standard libraries. It enables simultaneous editing of multiple text regions with distinct typographic styles while preserving non-edited regions.

Result: Extensive experiments on multiple datasets including handwritten text benchmarks show state-of-the-art performance in both text fidelity and visual realism, with unprecedented control over font families and stylistic nuances.

Conclusion: SkyReels-Text bridges the gap between general-purpose image editing and professional-grade typographic design, offering practical utility for artistic design workflows.
...
TL;DR: DriveSora, a diffusion-based video generation model, enables automated correction of failure cases in autonomous driving by generating high-fidelity, spatiotemporally consistent videos aligned with 3D layouts, integrated into the CorrectAD agentic system.


<details>
  <summary>Details</summary>
Motivation: Address the robustness issues in end-to-end autonomous driving planning methods caused by the long-tail problem (rare but safety-critical failure cases) through automated self-correction.

Method: Propose CorrectAD system with PM-Agent for data requirement formulation and DriveSora for generating spatiotemporally consistent videos conditioned on 3D layouts, creating an end-to-end model-agnostic pipeline.

Result: CorrectAD corrects 62.5% and 49.8% of failure cases on nuScenes and in-house datasets respectively, reducing collision rates by 39% and 27% across multiple end-to-end planners.

Conclusion: The proposed agentic system successfully addresses long-tail failure cases in autonomous driving through automated data generation and self-correction, demonstrating significant improvement in safety metrics.
...
TL;DR: DriveLiDAR4D is a novel LiDAR generation pipeline that produces temporally consistent LiDAR scenes with controllable foreground objects and realistic backgrounds, achieving state-of-the-art performance on autonomous driving datasets.


<details>
  <summary>Details</summary>
Motivation: Existing 3D LiDAR point cloud generation methods lack sequential generation capabilities and struggle to produce accurately positioned foreground objects and realistic backgrounds, limiting their practical applicability for autonomous driving systems.

Method: The approach uses multimodal conditions and a novel sequential noise prediction model called LiDAR4DNet to generate temporally consistent LiDAR scenes with full scene manipulation capability in an end-to-end manner.

Result: On nuScenes dataset, DriveLiDAR4D achieved FRD score of 743.13 and FVD score of 16.96, outperforming state-of-the-art method UniScene by 37.2% in FRD and 24.1% in FVD respectively.

Conclusion: DriveLiDAR4D is the first work to address sequential generation of LiDAR scenes with full scene manipulation capability, demonstrating superior performance over existing methods for autonomous driving applications.
...
TL;DR: A computer vision framework for group activity recognition using Mask R-CNN for actor localization, feature extraction with multiple backbones, and Actor Relation Graphs with Graph Convolutional Networks to model interactions.
...
Method: Uses Mask R-CNN for actor localization, multiple backbone networks (Inception V3, MobileNet, VGG16) for feature extraction, RoIAlign for spatial alignment, mask fusion for refined features, Actor Relation Graphs for interaction modeling, and Graph Convolutional Networks for reasoning.
...
TL;DR: Novel Mixture-of-Experts framework for object detection using adaptive routing among multiple YOLOv9-T experts to improve performance metrics.
...
Method: Mixture-of-Experts framework with adaptive routing mechanism that dynamically selects among multiple YOLOv9-T expert models for specialized feature processing.

Result: Achieved higher mean Average Precision (mAP) and Average Recall (AR) compared to a single YOLOv9-T model.

Conclusion: The Mixture-of-Experts approach with adaptive routing successfully improves object detection performance by leveraging multiple specialized experts for dynamic feature specialization.
...
TL;DR: Hybrid semi-supervised learning approach for retinal image quality assessment that combines manual overall quality labels with pseudo-labels for quality details, improving interpretability without extra manual annotation costs.


<details>
  <summary>Details</summary>
Motivation: Existing retinal image quality assessment tools only classify overall image quality without indicating specific acquisition defects to guide recapture, mainly due to the high cost of detailed annotations.

Method: Multi-task framework using ResNet-18 backbone that combines manual labels for overall quality with pseudo-labels generated by a Teacher model trained on small dataset, then fine-tunes pre-trained model in multi-task setting.

Result: Outperforms single-task baselines (F1: 0.875 vs. 0.863 on EyeQ, 0.778 vs. 0.763 on DeepDRiD), matches or surpasses existing methods, achieves performance comparable to Teacher model for detail prediction, and performs similarly to experts on newly annotated EyeQ subset.

Conclusion: Semi-supervised approach improves overall quality assessment while providing interpretable feedback on capture conditions (illumination, clarity, contrast), enhancing interpretability at no extra labeling cost and offering clinically actionable outputs to guide image recapture.
...
TL;DR: Proposes gDDCM, a generalized version of DDCM that extends image compression to various diffusion models including DDPM, Score-Based Models, Consistency Models, and Rectified Flow.


<details>
  <summary>Details</summary>
Motivation: DDCM was limited to DDPM only and couldn't be applied to other diffusion models, creating a need for a more generalized compression approach.

Method: Extends DDCM by replacing random noise in backward process with noise from specific sets according to predefined rules, making it compatible with multiple diffusion model variants.
...
<details>
  <summary>Details</summary>
Motivation: To enable trustworthy application of Vision-Language Models in automated driving by ensuring robust perception capabilities, especially for critical objects at long distances (30+ meters) in complex traffic scenes.

Method: Created a VQA benchmark with two parts: DTP-Synthetic using a simulator and DTP-Real using existing real traffic images, with distance annotations for each object to analyze performance degradation with distance.

Result: Provides a comprehensive dataset with images, questions, ground truth answers, and distance annotations, enabling systematic evaluation of VLM perception capabilities in traffic scenarios.

Conclusion: DTPQA enables isolated evaluation of perception capabilities in VLMs for automated driving applications, with distance-based performance analysis and tools for generating additional data.
...
TL;DR: TripleFDS is a novel Scene Text Editing framework that achieves triple feature disentanglement (text style, content, background) using SCB Groups and contrastive regularization, achieving state-of-the-art performance with flexible editing capabilities.
...
Method: Proposes TripleFDS framework with SCB Synthesis dataset using SCB Groups (triple attributes per image) for training, employs inter-group contrastive regularization and intra-sample multi-feature orthogonality for disentanglement, and feature remapping to prevent shortcuts during reconstruction.

Result: Achieves state-of-the-art image fidelity (SSIM 44.54) and text accuracy (ACC 93.58%) on mainstream STE benchmarks, trained on 125,000 SCB Groups, and supports new operations like style replacement and background transfer.

Conclusion: TripleFDS overcomes limitations of previous methods by providing complete triple feature disentanglement, enabling more flexible and consistent scene text editing with superior performance.
...
TL;DR: The paper introduces a 'What Color Is It' dataset to test color perception hallucinations in Multimodal Large Models (MLMs) and investigates causes and solutions.


<details>
  <summary>Details</summary>
Motivation: MLMs are susceptible to visual perception interference, especially in color perception, which increases hallucination risks.

Method: Created a novel benchmark dataset using a simple method to trigger single-modality visual hallucination in MLMs.

Result: The dataset successfully triggers visual hallucinations, revealing vulnerabilities in MLMs' color perception.

Conclusion: Identifies underlying causes of visual modality hallucination and proposes potential solutions to enhance MLM robustness.
...
TL;DR: DelAnyFlow is a resolution-agnostic methodology for large-scale agricultural field boundary mapping that combines the DelAny instance segmentation model with structured post-processing to generate topologically consistent vector boundaries, achieving state-of-the-art accuracy and scalability.
...
Method: DelAnyFlow combines the DelAny instance segmentation model (based on YOLOv11 backbone trained on FBIS 22M dataset) with a structured post-processing, merging, and vectorization sequence. The FBIS 22M dataset contains 672,909 multi-resolution image patches and 22.9 million validated field instances.

Result: DelAny model achieves over 100% higher mAP and 400x faster inference than SAM2. DelAnyFlow generated a complete field boundary layer for Ukraine (603,000km²) in under 6 hours, delineating 3.75M fields at 5m and 5.15M at 2.5m resolution, significantly outperforming existing operational products.

Conclusion: DelAnyFlow delivers a scalable, cost-effective methodology for field delineation in regions lacking digital cadastral data, with strong zero-shot generalization capabilities and support for national-scale applications.
...
TL;DR: VOPE is a new method to evaluate hallucinations in LVLMs during voluntary imagination tasks (like story writing) by checking consistency between imagined objects and their actual presence in images.


<details>
  <summary>Details</summary>
Motivation: Current hallucination research focuses on factual tasks where any novel content is considered hallucination, but this approach is inappropriate for voluntary imagination tasks where models are expected to create novel content.

Method: VOPE uses recheck-based questions to evaluate how LVLMs interpret the presence of imagined objects in their own responses, comparing consistency with actual image content.

Result: Most LVLMs hallucinate heavily during voluntary imagination and perform poorly on presence evaluation for imagined objects; existing mitigation methods show limited effectiveness.

Conclusion: Voluntary imagination tasks present distinct hallucination challenges that require new approaches, as current methods are insufficient for this domain.
...
<details>
  <summary>Details</summary>
Motivation: To create a computationally efficient method for mapping between 3D shapes that works across different representations (point clouds, meshes, SDFs, volumetric data) without requiring extensive training or data-driven procedures.

Method: Represent 3D shapes as probability distributions via continuous invertible flow mappings from a fixed anchor distribution. Use composition of inverse flow (source to anchor) and forward flow (anchor to target) to map points between surfaces, enhanced with pointwise task-tailored embeddings.

Result: Achieves high coverage and accuracy across diverse benchmarks and challenging shape matching settings. Also shows promising results in UV mapping and registration of raw point cloud scans of human bodies.

Conclusion: The flow-matching based representation provides an efficient, invertible, and modality-agnostic framework for 3D shape mapping that works across multiple representations and extends to various applications beyond shape matching.
...
<details>
  <summary>Details</summary>
Motivation: Existing IFDL methods struggle with generalization across datasets and offer limited interpretability, while MLLMs show strong generalization potential but current approaches require large-scale training and fail to leverage vanilla MLLMs' inherent capabilities.

Method: Proposes Foresee pipeline with type-prior-driven strategy and Flexible Feature Detector (FFD) module specifically for copy-move manipulations, enabling training-free operation and lightweight inference.

Result: Extensive experiments show superior localization accuracy, richer textual explanations, and stronger generalization across various tampering types including copy-move, splicing, removal, local enhancement, deepfake, and AIGC-based editing.

Conclusion: Foresee effectively unleashes vanilla MLLMs' potential in forensic domain without additional training, outperforming existing methods in both accuracy and explanatory richness while demonstrating robust generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Multimedia documents are often distributed as static raster images, limiting editability. Existing raster-vectorization methods fail to preserve high-level semantic structure and distinction between text and image elements.
...
Result: Achieves reconstruction LPIPS of 0.069 and is preferred by human evaluators in 82.9% of cases compared to strongest zero-shot VLM baseline. Also introduces Slide2SVG dataset for future research.

Conclusion: SliDer effectively restores editability to raster slide documents by preserving semantic structure through VLM-based derendering, outperforming existing methods and enabling practical document customization.
...
TL;DR: InterMoE: A Dynamic Temporal-Selective Mixture of Experts framework for generating high-fidelity 3D human interactions that preserves individual characteristics while adhering to textual descriptions.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to preserve unique individual characteristics and fully adhere to textual descriptions in human interaction generation, which is crucial for applications like virtual reality and robotics.

Method: Uses a Dynamic Temporal-Selective Mixture of Experts with a routing mechanism that combines high-level text semantics and low-level motion context to dispatch temporal motion features to specialized experts, allowing dynamic capacity allocation and focus on critical temporal features.
...
Conclusion: InterMoE effectively preserves specific individual characteristic identities while ensuring high semantic fidelity in human interaction generation through its temporal-selective expert routing approach.
...
TL;DR: LGIP benchmark evaluates VLMs' invariance to paraphrases and sensitivity to semantic flips, revealing that EVA02-CLIP and large OpenCLIP variants perform well while SigLIP models often prefer flipped captions over original ones.


<details>
  <summary>Details</summary>
Motivation: To assess how reliably vision-language models respond to controlled linguistic perturbations beyond standard retrieval metrics, as current VLMs achieve strong zero-shot performance but their linguistic robustness is unclear.

Method: Language-Guided Invariance Probing (LGIP) benchmark using 40k MS COCO images with five human captions each, automatically generating paraphrases and rule-based semantic flips that alter object category, color, or count.

Result: EVA02-CLIP and large OpenCLIP variants show favorable invariance-sensitivity balance with low paraphrase variance and consistent preference for original captions. SigLIP models exhibit large invariance errors and often prefer flipped captions, especially for object and color edits.

Conclusion: LGIP provides a model-agnostic diagnostic for VLM linguistic robustness that reveals failures invisible to standard retrieval metrics, highlighting the need for better linguistic understanding in VLMs.
...
TL;DR: A deep learning framework monitors urban village redevelopment in China, revealing prolonged processes and three transformation pathways, highlighting the need for context-sensitive planning.


<details>
  <summary>Details</summary>
Motivation: There is a lack of systematic evaluation of whether demolished urban villages in China have been effectively reused, raising concerns about redevelopment efficacy and sustainability.

Method: Semantic segmentation of multi-temporal remote sensing imagery to map UV boundaries, followed by classification of post-demolition land use into six categories across four representative Chinese cities.

Result: UV redevelopment processes were frequently prolonged; transitions primarily occurred in peripheral areas; three spatiotemporal transformation pathways identified: synchronized, delayed, and gradual optimization.

Conclusion: The study reveals fragmented, complex, nonlinear nature of UV redevelopment, underscoring need for tiered and context-sensitive planning strategies for more inclusive, efficient, and sustainable urban renewal.
...
Method: Developed an asymptotically minimax approach to multi-target conformal prediction that ensures joint marginal coverage while providing tight prediction intervals. Applied to multi-metric blind image quality assessment, multi-task uncertainty quantification, and multi-round measurement acquisition.

Result: Numerical demonstrations using synthetic and MRI data show benefits over existing multi-target conformal prediction methods, with tighter prediction intervals while maintaining coverage guarantees.

Conclusion: The proposed minimax approach effectively addresses multi-target uncertainty quantification in ill-posed imaging problems, outperforming existing methods and enabling practical applications across various imaging domains.
...
TL;DR: A new class of attacks that compromises model interpretability without affecting accuracy by using small color perturbations in federated learning to shift saliency maps away from meaningful regions.


<details>
  <summary>Details</summary>
Motivation: To reveal vulnerabilities in model interpretability systems, challenging the assumption that correct predictions imply faithful explanations, and demonstrating that interpretability itself can be an attack surface.

Method: Proposes a saliency-aware attack framework called Chromatic Perturbation Module that systematically crafts adversarial examples by altering color contrast between foreground and background to disrupt explanation fidelity, with perturbations accumulating across federated learning rounds.

Result: The attack reduces peak activation overlap in Grad-CAM explanations by up to 35% while preserving classification accuracy above 96% across multiple datasets, showing standard training pipelines are insufficient to detect explanation degradation.

Conclusion: Interpretability can be compromised independently of prediction accuracy, highlighting a new security vulnerability in machine learning systems where subtle color perturbations can poison model explanations in a stealthy and persistent manner.
...
<details>
  <summary>Details</summary>
Motivation: Existing OOD detectors struggle when OOD samples are semantically similar to in-distribution classes, which is critical for safety-sensitive applications.
...
Result: BootOOD outperforms prior post-hoc methods, surpasses training-based methods without outlier exposure, and is competitive with state-of-the-art outlier-exposure approaches on CIFAR-10, CIFAR-100, and ImageNet-200.

Conclusion: BootOOD provides an effective self-supervised approach for OOD detection that handles semantically challenging cases while maintaining or improving ID accuracy.
...
TL;DR: Proposes a method to defend CLIP models against backdoor attacks by identifying triggers and victim samples using an image segmentation oracle, then fine-tuning with a curated dataset.


<details>
  <summary>Details</summary>
Motivation: Multimodal models like CLIP are vulnerable to backdoor attacks, and existing defenses require extensive retraining without targeting specific affected labels.

Method: Uses an image segmentation oracle to supervise CLIP outputs, develops algorithms to identify backdoor triggers and victim samples/labels, then curates a compact dataset for fine-tuning.

Result: Extensive experiments show the strategy effectively defends CLIP models against backdoor attacks on visual recognition benchmarks.

Conclusion: The approach successfully enhances CLIP's robustness against backdoor attacks by efficiently identifying and mitigating specific vulnerabilities.
...
<details>
  <summary>Details</summary>
Motivation: Monocular height estimation is cost-effective but limited by expensive labeled data. The scarcity of high-quality annotations hinders model generalization and performance.

Method: Proposes TSE-Net with teacher, student, and exam networks. Teacher generates pseudo-labels via joint regression and classification with hierarchical bi-cut strategy and Plackett-Luce calibration. Student learns from filtered pseudo-labels, while exam provides temporal ensemble.

Result: Evaluated on three datasets spanning different resolutions and imaging modalities, showing improved performance through semi-supervised learning with unlabeled data.

Conclusion: The semi-supervised approach effectively leverages unlabeled data to enhance monocular height estimation, overcoming data scarcity limitations through the TSE-Net framework with pseudo-label filtering.
...
TL;DR: Opt3DGS improves 3D Gaussian Splatting optimization through a two-stage process combining adaptive exploration and curvature-guided exploitation to escape local optima and achieve better convergence.


<details>
  <summary>Details</summary>
Motivation: 3DGS faces optimization challenges including entrapment in suboptimal local optima and insufficient convergence quality, limiting its rendering performance.
...
Result: State-of-the-art rendering quality on diverse benchmark datasets, refining 3DGS optimization without changing its underlying representation.

Conclusion: Opt3DGS provides a robust framework that significantly enhances 3DGS optimization, achieving superior novel view synthesis results through improved optimization strategies.
...
TL;DR: HPL is a unified framework for person re-identification that uses hierarchical prompt learning to jointly optimize both image-to-image and text-to-image retrieval tasks through task-aware prompt modeling and cross-modal regularization.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat image-to-image and text-to-image ReID tasks separately, leading to representation entanglement and suboptimal performance. The authors aim to create a unified framework that addresses both tasks simultaneously.

Method: Proposes Hierarchical Prompt Learning (HPL) with: 1) Task-Routed Transformer using dual classification tokens, 2) Hierarchical prompt generation with identity-level learnable tokens and instance-level pseudo-text tokens, 3) Cross-Modal Prompt Regularization for semantic alignment.

Result: Achieves state-of-the-art performance on multiple ReID benchmarks for both I2I and T2I tasks, validating the effectiveness of the unified approach.

Conclusion: The proposed HPL framework successfully unifies I2I and T2I ReID tasks through hierarchical prompt learning, demonstrating improved performance and better cross-modal alignment compared to separate task approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing tile-based models fail to incorporate broader tissue context that influences cell identity, and available human annotations are coarse-grained and unevenly distributed, making fine-grained subtype-level supervision difficult.

Method: NuClass uses two paths: Path local (224x224 crops for nuclear morphology) and Path global (1024x1024 neighborhood for tissue context), with a learnable gating module and uncertainty-guided objective that directs global path to prioritize regions where local path is uncertain.

Result: Evaluated on three held-out cohorts, NuClass achieves up to 96% F1 for best-performing class, outperforming strong baselines on a dataset of over two million cells across eight organs and 16 classes.
...
TL;DR: VVS accelerates visual autoregressive models by skipping partial verification steps in speculative decoding, reducing target model forward passes by 2.8× while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Visual AR models have high inference latency due to next-token-prediction. Existing speculative decoding has limited acceleration because it requires verification after each draft step.

Method: Proposes VVS framework with three modules: verification-free token selector with dynamic truncation, token-level feature caching/reuse, and fine-grained skipped step scheduling.

Result: Achieves 2.8× reduction in target model forward passes compared to vanilla AR decoding while maintaining competitive generation quality.

Conclusion: VVS offers superior speed-quality trade-off over conventional SD frameworks and has strong potential to reshape the speculative decoding paradigm.
...
TL;DR: Proposes ICLR framework with DIEM and CCL for low-light image enhancement, addressing chrominance-luminance interaction issues and achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Address limitations in HVI color space for LLIE: distributional differences between chrominance/luminance branches limit complementary feature extraction, luminance errors propagate to chrominance, and weak correlation in homogeneous-color regions causes gradient conflicts with traditional losses.

Method: ICLR framework with Dual-stream Interaction Enhancement Module (DIEM) for complementary information extraction from fusion and enhancement dimensions, and Covariance Correction Loss (CCL) that uses luminance residual statistics to penalize chrominance errors and balance gradient conflicts via chrominance covariance constraints.

Result: Experimental results on multiple datasets demonstrate that ICLR framework outperforms state-of-the-art methods in low-light image enhancement.

Conclusion: The proposed ICLR framework effectively addresses chrominance-luminance interaction challenges in low-light image enhancement and achieves superior performance compared to existing methods.
...
TL;DR: A machine learning framework that generates customizable anatomical templates conditioned on subject attributes like age and sex, using convolutional neural networks for efficient template creation and registration.


<details>
  <summary>Details</summary>
Motivation: Traditional template creation is computationally expensive, leading to limited template availability and sub-optimal templates that don't represent diverse populations well, especially with large anatomical variations.

Method: Uses convolutional registration neural networks to learn a function that outputs templates conditioned on subject-specific attributes (age, sex), leveraging available segmentations to produce anatomical label maps, and enabling subject-to-template registration.

Result: Demonstrated on 3D brain MRI datasets, the method learns high-quality templates representative of populations, with annotated conditional templates enabling better registration than unlabeled unconditional templates and outperforming other template construction methods.

Conclusion: The proposed framework efficiently creates population-representative conditional templates that improve registration accuracy and outperform existing template construction approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for nuclei detection and classification are hindered by reliance on detailed expert annotations and insufficient use of tissue context, creating a need for methods that reduce annotation burden while improving accuracy.
...
TL;DR: A real-time driver drowsiness detection system using webcam, facial landmark tracking with MediaPipe Face Mesh, and Eye Aspect Ratio (EAR) to monitor eye movements and detect drowsiness through prolonged eye closures or low blinking rates.


<details>
  <summary>Details</summary>
Motivation: Driver fatigue causes thousands of road accidents, fatalities, and injuries annually, necessitating systems to improve road safety by alerting drowsy drivers.

Method: Uses standard webcam with MediaPipe Face Mesh for facial landmark detection and Eye Aspect Ratio (EAR) method to analyze eye movements, detecting drowsiness through prolonged eye closures and low blinking rates, with OpenCV for image processing.

Result: Experimental tests show the system achieves high accuracy and fast response times, making it suitable for integration into Advanced Driving Assistance Systems (ADAS).

Conclusion: The system provides a high-performance, low-cost driver monitoring solution that effectively detects drowsiness and can enhance road safety when integrated into existing ADAS.
...
TL;DR: The paper introduces two novel margin-based α-divergence losses (Q-Margin and A3M) for face and speaker verification, addressing training instability in A3M and achieving significant performance gains on challenging benchmarks.


<details>
  <summary>Details</summary>
Motivation: To integrate angular margin capabilities into α-divergence loss functions for verification tasks, which can induce sparse solutions but lack straightforward margin integration.

Method: Developed two approaches: Q-Margin (margin in reference measure) and A3M (margin in logits), with a prototype re-initialization strategy to address A3M training instability.

Result: Achieved significant performance gains on IJB-B and IJB-C face verification benchmarks and strong performance on VoxCeleb speaker verification, particularly excelling at low false acceptance rates.

Conclusion: The proposed margin-based α-divergence losses effectively combine sparsity benefits with angular margin capabilities, making them particularly suitable for high-security applications requiring minimal false authentications.
...
TL;DR: CacheFlow enables efficient long-form video QA by combining dynamic token dropping with compressive long-term memory, reducing token processing by up to 87% while maintaining answer quality.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with long-form video QA due to growing attention and KV caches that force expensive inference or limited sliding windows, making them impractical for live streaming scenarios.

Method: Training-free pipeline with Dynamic Token Dropping (prunes tokens via cosine similarity) and compressive long-term memory (summarizes keys for retrieval, offloads full KV pairs for later rehydration). Uses consensus-based retrieval of Top-K relevant blocks.
...
Conclusion: CacheFlow provides a practical solution for long-form video understanding that is architecture-agnostic, requires no fine-tuning, and works well for both offline and live streaming scenarios.
...
TL;DR: Part-X-MLLM is a 3D multimodal LLM that unifies diverse 3D tasks by generating structured programs from RGB point clouds and language prompts, enabling part-level manipulation through a single interface.


<details>
  <summary>Details</summary>
Motivation: To create a unified framework for 3D tasks that decouples symbolic planning from geometric synthesis, allowing any compatible geometry engine to be controlled through language.

Method: Uses a dual-encoder architecture pre-trained to disentangle structure from semantics, instruction-tuned on part-centric data, and generates structured token sequences encoding bounding boxes, semantic descriptions, and edit commands.
...
Conclusion: The model successfully demonstrates that symbolic planning can be decoupled from geometric synthesis, enabling versatile 3D manipulation through language-native interfaces.
...
TL;DR: PhysX-Anything is the first simulation-ready physical 3D generative framework that creates high-quality 3D assets with geometry, articulation, and physical attributes from single images, enabling direct use in embodied AI applications.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generation methods overlook physical and articulation properties, limiting their utility in embodied AI. The paper aims to bridge this gap by creating simulation-ready 3D assets.

Method: Proposes a VLM-based physical 3D generative model with a new 3D representation that tokenizes geometry efficiently (193x reduction in tokens), and constructs PhysX-Mobility dataset with 2K+ objects and rich physical annotations.

Result: Strong generative performance and robust generalization demonstrated on PhysX-Mobility and in-the-wild images. Sim-ready assets can be directly used for contact-rich robotic policy learning in MuJoCo-style environments.

Conclusion: PhysX-Anything substantially empowers downstream applications in embodied AI and physics-based simulation by providing high-quality, simulation-ready 3D assets with physical properties.
...
TL;DR: DMDR combines reinforcement learning with diffusion model distillation to create efficient few-step generators that can outperform their multi-step teachers.


<details>
  <summary>Details</summary>
Motivation: To overcome the performance limitation where distilled few-step diffusion models are capped by their multi-step teachers, and to improve inference efficiency while maintaining or exceeding original quality.
...
TL;DR: OlmoEarth is a multimodal spatio-temporal foundation model for Earth observation data that achieves state-of-the-art performance across various benchmarks and real-world tasks.
...
Method: The model employs a novel self-supervised learning formulation with specialized masking strategy and loss functions designed specifically for the Earth observation domain.

Result: OlmoEarth outperforms 12 other foundation models, achieving best performance on 15 out of 24 tasks with embeddings and 19 out of 29 tasks with full fine-tuning. It's deployed as part of an end-to-end platform for data collection, labeling, training, and inference.

Conclusion: OlmoEarth provides state-of-the-art performance for Earth observation tasks and is made available as an open-source platform to help non-profits and NGOs solve global challenges using Earth observation data.
...
TL;DR: GS-Light is an efficient pipeline for text-guided 3D scene relighting using Gaussian Splatting, leveraging LVLM for lighting priors and multi-view diffusion without training.


<details>
  <summary>Details</summary>
Motivation: To enable intuitive text-based control over 3D scene lighting (direction, color, intensity) while maintaining multi-view consistency and high fidelity.

Method: Uses LVLM to parse text prompts into lighting priors, fuses with geometry/semantic constraints to compute illumination maps, generates initial latents for multi-view diffusion, and fine-tunes 3DGS with relit appearance.

Result: Outperforms state-of-the-art baselines in quantitative metrics (multi-view consistency, quality, aesthetics) and qualitative user studies on indoor/outdoor scenes.

Conclusion: GS-Light provides an effective training-free solution for high-quality text-guided 3D scene relighting with improved lighting direction accuracy and multi-view consistency.
...
TL;DR: TiViBench is a hierarchical benchmark for evaluating reasoning capabilities in image-to-video generation models across four dimensions, revealing that commercial models show stronger reasoning potential while open-source models have untapped potential hindered by limited training.


<details>
  <summary>Details</summary>
Motivation: Existing video generation benchmarks focus on visual fidelity and temporal coherence but fail to capture higher-order reasoning abilities similar to LLMs, creating a gap in evaluating physical plausibility and logical consistency.

Method: Proposed TiViBench benchmark with 4 reasoning dimensions (Structural Reasoning, Spatial & Visual Pattern Reasoning, Symbolic & Logical Reasoning, Action Planning) across 24 task scenarios and 3 difficulty levels, plus VideoTPO test-time strategy using LLM self-analysis for preference optimization.

Result: Commercial models (Sora 2, Veo 3.1) demonstrate stronger reasoning potential, while open-source models show untapped potential limited by training scale and data diversity. VideoTPO significantly enhances reasoning performance without additional training or data.

Conclusion: TiViBench and VideoTPO provide foundations for evaluating and advancing reasoning in video generation models, setting the stage for future research in this emerging field.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models excel at semantic editing but lack 3D-aware object manipulation capabilities, often operating in 2D space or requiring slow 3D reconstruction.
...
Result: Extensive experiments show FFSE significantly outperforms existing methods in both single-round and multi-round 3D-aware editing scenarios, preserving realistic background effects and maintaining scene consistency.

Conclusion: FFSE enables intuitive 3D-aware object manipulation with physical consistency, supporting arbitrary operations like translation, scaling, and rotation while maintaining realistic scene effects across multiple editing rounds.
...
TL;DR: UnSAMv2 enables segment anything at any granularity without human annotations by extending UnSAM's divide-and-conquer strategy with granularity control embedding, achieving significant improvements across multiple benchmarks with minimal data and parameters.


<details>
  <summary>Details</summary>
Motivation: SAM family models have limited ability to control segmentation granularity, requiring manual refinement through additional prompts or mask selection, which is ambiguous and expensive to collect dense annotations across all granularities.

Method: Extends UnSAM's divide-and-conquer strategy by discovering abundant mask-granularity pairs and introducing a novel granularity control embedding that enables precise, continuous control over segmentation scale.

Result: With only 6K unlabeled images and 0.02% additional parameters, UnSAMv2 substantially enhances SAM-2, improving NoC90 (5.69 → 4.75), 1-IoU (58.0 → 73.1), and AR1000 (49.6 → 68.3) across over 11 benchmarks.
...
TL;DR: Proposes SAAS model for multi-shot video object segmentation with transition mimicking augmentation and new Cut-VOS benchmark, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing VOS methods focus on single-shot videos and struggle with shot discontinuities, limiting real-world applicability.

Method: Transition mimicking data augmentation (TMA) for cross-shot generalization, and SAAS model that detects and comprehends shot transitions.

Result: Achieves state-of-the-art performance on YouMVOS and Cut-VOS benchmarks by effectively handling complex transitions.

Conclusion: SAAS enables effective multi-shot video object segmentation through transition understanding and cross-shot generalization.
...
TL;DR: The SenseNova-SI family scales up multimodal foundation models to improve spatial intelligence using 8 million diverse data samples, achieving state-of-the-art performance on multiple spatial benchmarks while maintaining strong general multimodal understanding.


<details>
  <summary>Details</summary>
Motivation: Despite progress in multimodal foundation models, they still exhibit significant deficiencies in spatial intelligence, which this work aims to address through systematic scaling and data curation.
...
Conclusion: The work demonstrates successful cultivation of spatial intelligence through systematic data scaling, shows early signs of emergent generalization capabilities, and releases all models publicly to facilitate further research in spatial intelligence for multimodal foundation models.
...
TL;DR: The paper proposes JiT (Just image Transformers), a diffusion model that directly predicts clean images rather than noise, based on the manifold assumption that natural data lies on low-dimensional manifolds while noised quantities do not.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models predict noise or noised quantities instead of clean images, which goes against the manifold assumption that natural data exists on low-dimensional manifolds. The authors argue that predicting clean data is fundamentally different and more effective.

Method: Use simple, large-patch Transformers on pixels without tokenizers, pre-training, or extra losses. The approach directly predicts clean images using Transformers with patch sizes of 16 and 32 on ImageNet at 256x256 and 512x512 resolutions.

Result: Competitive results on ImageNet at 256 and 512 resolutions, showing that predicting high-dimensional noised quantities can fail catastrophically while their clean prediction approach works effectively.

Conclusion: Directly predicting clean data using Transformers (JiT) provides a self-contained paradigm for diffusion models that aligns with the manifold assumption, enabling effective generation even with apparently under-capacity networks in high-dimensional spaces.
