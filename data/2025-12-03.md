...
TL;DR: Fine-tuning TerraMind GFM on multimodal FloodsNet dataset improves flood mapping accuracy and recall compared to Sen1Floods11-trained models, with base-unfrozen configuration offering best balance of performance and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Floods are devastating global hazards, and while EO satellites provide critical coverage, operational accuracy depends on labeled datasets and model generalization. Recent GFMs like TerraMind offer improved generalizability but their performance on diverse global flood events remains poorly understood.

Method: Fine-tuned TerraMind GFM for flood extent mapping using FloodsNet dataset (85 global flood events with co-located Sentinel-1 SAR and Sentinel-2 optical imagery). Tested four configurations: base vs. large models, frozen vs. unfrozen backbones. Compared against TerraMind Sen1Floods11 example and U-Net trained on both datasets.
...
Conclusion: Integrating multimodal optical and SAR data and fine-tuning GFMs can enhance near-real-time flood mapping. This study provides one of the first global-scale evaluations of GFMs for flood segmentation, highlighting both potential and current limitations for climate adaptation and disaster resilience.
...
TL;DR: Proposes a context-enriched contrastive loss function with two convergence targets to address information distortion in contrastive learning while improving learning effectiveness and convergence speed.


<details>
  <summary>Details</summary>
Motivation: Standard contrastive learning can introduce information distortion as models over-rely on samples with identical labels while neglecting positive pairs from the same original image, especially in large datasets.

Method: A context-enriched contrastive loss with two components: 1) label-sensitive component differentiating features of identical vs. distinct classes, and 2) component drawing closer augmented samples from same source image while distancing all others.

Result: Achieves improvements over 16 state-of-the-art contrastive learning methods on 8 benchmark datasets (CIFAR10, CIFAR100, Caltech-101, Caltech-256, ImageNet, BiasedMNIST, UTKFace, CelebA) in both generalization and convergence speed. Shows 22.9% improvement on BiasedMNIST for systematic distortion tasks.

Conclusion: The proposed method effectively addresses information distortion in contrastive learning, leading to more efficient and equitable downstream training with better generalization and faster convergence.
...
TL;DR: Proposes a joint evaluation framework for text-to-image models and vision-language models using 27 specific failure modes, with a dataset of prompts, generated images, and VLM annotations.


<details>
  <summary>Details</summary>
Motivation: T2I models often fail to capture specific attributes in prompts, and current VLM benchmarks don't match the complexity of scenes they need to evaluate. There's a need for better evaluation frameworks to assess prompt adherence and model reliability.

Method: Developed a structured methodology to jointly evaluate T2I models and VLMs by testing whether VLMs can identify 27 specific failure modes in images generated by T2I models. Created a dataset with prompts, images from 5 T2I models (Flux, SD3 variants), and annotations from 3 VLMs (Molmo, InternVL3, Pixtral) validated by Llama3.

Result: Analysis revealed systematic errors in attribute fidelity and object representation. Found that current metrics are insufficient to capture nuanced errors in T2I model outputs.

Conclusion: Targeted benchmarks are crucial for advancing generative model reliability and interpretability, as current evaluation methods fail to capture the specific failure modes in text-to-image generation.
...
TL;DR: LLOST: A deep latent variable model that predicts somatic mutation profiles from medical images using dual variational autoencoders with a shared latent space and conditional normalizing flow priors.


<details>
  <summary>Details</summary>
Motivation: Early cancer diagnosis and treatment are crucial for efficacy. Medical imaging is used initially, followed by genetic information for treatment selection. The paper aims to bridge these domains by predicting somatic mutations from medical images to enable earlier genetic insights.

Method: 1) Convert lesion images to point clouds for modality invariance; 2) Propose LLOST with dual variational autoencoders for lesion point clouds and somatic mutation counts; 3) Use a shared latent space to unify features from both domains; 4) Employ conditional normalizing flow priors for each of the three latent spaces to handle diverse distributions.

Result: The model shows predictive performance on specific mutation counts and accurate prediction of mutation occurrence. It identifies shared patterns between imaging and somatic mutation domains that reflect cancer type. Experiments use TCIA medical images and Pan Cancer dataset somatic mutations.

Conclusion: The model successfully links medical imaging with genetic information. Future improvements could include expanding to other genetic domains and enhancing model architecture.
...
TL;DR: SplatSuRe selectively applies super-resolution only to undersampled regions in 3D Gaussian Splatting, using camera geometry to identify where to add high-frequency content, resulting in sharper and more consistent renders than uniform SR approaches.


<details>
  <summary>Details</summary>
Motivation: Standard super-resolution applied uniformly to all low-resolution views in 3DGS introduces multi-view inconsistencies and blurry renders. Existing methods don't account for the fact that close-up views may already contain high-frequency information for regions captured in distant views.

Method: SplatSuRe uses camera pose relative to scene geometry to identify undersampled regions lacking high-frequency supervision, then selectively applies super-resolution content only to those regions rather than uniformly across all images.

Result: The approach surpasses baselines in fidelity and perceptual quality across Tanks & Temples, Deep Blending and Mip-NeRF 360 datasets. Gains are most significant in localized foreground regions where higher detail is desired.

Conclusion: Selective super-resolution based on camera geometry and scene coverage is more effective than uniform SR application for achieving high-quality, consistent renders in 3D Gaussian Splatting.
...
TL;DR: RobustSurg improves surgical scene segmentation generalization across centers and modalities by exploiting style-content separation with instance normalization and feature covariance mapping, plus a restitution module to preserve task-relevant features.


<details>
  <summary>Details</summary>
Motivation: Current surgical scene segmentation methods fail to generalize to unseen distributions (different centers) and modalities. While domain generalization methods exist for natural scenes, they don't work well for surgical scenes due to limited visual cues and extreme diversity (blood, artifacts).

Method: 1) Exploit style and content information using instance normalization and feature covariance mapping for robust feature representations. 2) Introduce a restitution module within ResNet backbone to retain useful task-relevant features that might be lost during normalization. 3) Provide new multiclass, multicentre surgical segmentation dataset.

Result: RobustSurg achieved ~23% improvement over baseline DeepLabv3+ and 10-32% improvement over SOTA on unseen HeiCholSeg dataset when trained on CholecSeg8K. Also achieved ~22% improvement over baseline and ~11% improvement over recent SOTA on EndoUDA polyp dataset.

Conclusion: Style-content separation with instance normalization and feature covariance mapping, combined with a restitution module, effectively improves surgical scene segmentation generalization across centers and modalities, addressing key challenges in surgical computer vision.
...
TL;DR: The paper introduces Monofractal and Multifractal Recalibration methods that use multifractal analysis to create statistical descriptions of encoder embeddings as channel-attention functions in CNNs, showing substantial gains over other channel-attention mechanisms on medical imaging datasets.


<details>
  <summary>Details</summary>
Motivation: Multifractal analysis has proven useful for self-seeding phenomena but is underutilized in deep learning. Existing multifractal methods rely on heavy pooling or feature-space decimation that constrain tasks like semantic segmentation. The authors aim to overcome these limitations.

Method: Introduce two inductive priors: Monofractal and Multifractal Recalibration. These leverage relationships between probability mass of exponents and multifractal spectrum to form statistical descriptions of encoder embeddings, implemented as channel-attention functions in convolutional networks using a U-Net-based framework.

Result: Multifractal recalibration yields substantial gains over baselines with other channel-attention mechanisms using higher-order statistics. Validated on three medical imaging datasets: ISIC18 (dermoscopy), Kvasir-SEG (endoscopy), and BUSI (ultrasound). Also found that excitation responses don't become increasingly specialized with encoder depth in U-Nets due to skip connections, and effectiveness relates to global statistics of instance variability.

Conclusion: Multifractal recalibration is an effective channel-attention mechanism that outperforms existing methods, particularly valuable for medical imaging tasks where multifractal analysis can capture pathological regularities. The approach provides insights into attention layer behavior in U-Net architectures.
...
<details>
  <summary>Details</summary>
Motivation: Current VQA methods have limitations: they provide only single quality scores without diagnostic feedback, are format-specific rather than truly generic, and learn compromised representations from disparate perceptual domains.

Method: Proposes Unified-VQA framework using Diagnostic Mixture-of-Experts with multiple perceptual experts for distinct domains, multi-proxy expert training with ranking-inspired loss, and diagnostic multi-task head for global scores and artifact vectors using weakly-supervised learning.

Result: Outperforms 18 benchmark methods for both generic VQA and diagnostic artifact detection across 17 databases with diverse streaming artifacts in HD, UHD, HDR, and HFR formats, using static parameters without retraining.

Conclusion: Represents an important step toward practical, actionable, and interpretable video quality assessment by providing unified, diagnostic quality evaluation across multiple formats.
...
TL;DR: AV-SpeakerBench is a new benchmark for evaluating multimodal LLMs on fine-grained speaker-centric audiovisual reasoning in real-world videos, focusing on aligning who speaks, what is said, and when it occurs.


<details>
  <summary>Details</summary>
Motivation: Existing video benchmarks rarely assess fine-grained reasoning about human speech, often being visually solvable or only coarsely evaluating speech, offering limited insight into whether models can properly align speaker identity, speech content, and timing.

Method: Created a curated benchmark of 3,212 multiple-choice questions with three key features: (1) speaker-centered formulation treating speakers as the core reasoning unit, (2) fusion-grounded question design embedding audiovisual dependencies into question semantics, and (3) expert-curated annotations ensuring temporal precision and cross-modal validity.
...
Conclusion: AV-SpeakerBench establishes a rigorous foundation for advancing fine-grained audiovisual reasoning in future multimodal systems, highlighting current limitations in audiovisual fusion capabilities of existing models.
...
TL;DR: Proposes Visual LIF (VLIF) neuron and spiking decomposition modules for energy-efficient image deraining using SNNs, achieving SOTA performance with 13% energy consumption.


<details>
  <summary>Details</summary>
Motivation: SNNs are biologically plausible and energy-efficient but underexplored for low-level vision tasks like image deraining. Traditional spiking neurons lack spatial contextual understanding and have frequency-domain saturation limitations.

Method: Introduces Visual LIF (VLIF) neuron to address spatial context limitations, plus Spiking Decomposition and Enhancement Module and lightweight Spiking Multi-scale Unit for hierarchical multi-scale representation learning.

Result: Outperforms state-of-the-art SNN-based deraining methods across five benchmark datasets while using only 13% of their energy consumption.

Conclusion: Establishes foundation for deploying SNNs in high-performance, energy-efficient low-level vision tasks, demonstrating practical viability of spiking neural networks for real-world computer vision applications.
...
TL;DR: SPF is a new flow matching approach for climate emulation that uses a spatiotemporal pyramid to enable efficient, parallel sampling at multiple timescales, outperforming baselines on ClimateBench and scaling with the new ClimateSuite dataset.


<details>
  <summary>Details</summary>
Motivation: Current generative approaches for climate emulation rely on slow weather-scale autoregression that struggles with long climate horizons and nonstationary forcings, limiting efficient climate modeling.

Method: Spatiotemporal Pyramid Flows (SPF) partition the generative trajectory into a spatiotemporal pyramid, progressively increasing spatial resolution while coupling each stage with specific timescales, enabling direct sampling at any temporal level with conditioning on physical forcings.
...
Conclusion: SPF and ClimateSuite provide a foundation for accurate, efficient, probabilistic climate emulation across temporal scales and realistic future scenarios, with publicly available data and code.
...
TL;DR: Repurposing CogVideo text-to-video model for progressive image restoration by fine-tuning it to generate restoration trajectories instead of natural motion.


<details>
  <summary>Details</summary>
Motivation: Text-to-video models have strong temporal generation capabilities but their potential for image restoration remains underexplored. The authors want to leverage these temporal capabilities for progressive visual restoration tasks.

Method: Fine-tune CogVideo to generate restoration trajectories rather than natural video motion. Construct synthetic datasets for super-resolution, deblurring, and low-light enhancement where each sample shows gradual transition from degraded to clean frames. Compare two prompting strategies: uniform text prompt across all samples vs. scene-specific prompting using LLaVA multi-modal LLM refined with ChatGPT.
...
Conclusion: CogVideo can be successfully repurposed for progressive visual restoration tasks, learning to generate restoration trajectories that improve image quality over time while maintaining temporal coherence, with strong generalization to real-world scenarios without additional training.
...
Result: Models pretrained on Mediterranean data degrade from 67.8% to 51.8% mIoU on Peruvian domain; MORP-Synth improves performance up to +6 mIoU and boosts minority-class IoU (+10.8 oil, +14.6 look-alike). Compiled Peruvian dataset of 2112 labeled patches from 40 Sentinel-1 scenes (2014-2024).

Conclusion: MORP-Synth effectively addresses domain adaptation challenges for SAR oil spill segmentation, significantly improving transfer learning performance from Mediterranean to Peruvian conditions through synthetic data augmentation.
...
TL;DR: Self-supervised tracker using pre-trained video diffusion models' motion representations achieves 6-point improvement on benchmarks for tracking visually similar objects.


<details>
  <summary>Details</summary>
Motivation: Current self-supervised trackers struggle with visually similar objects when visual cues become ambiguous, limiting scalability without extensive labeled data. There's a need for better motion-based tracking without task-specific training.

Method: Leverages pre-trained video diffusion models which inherently learn motion representations suitable for tracking. The denoising process isolates motion in early, high-noise stages, distinct from later appearance refinement. Uses these diffusion-derived motion representations for robust tracking.
...
Conclusion: Pre-trained video diffusion models contain inherent motion representations suitable for tracking without task-specific training, offering a promising approach for self-supervised tracking of visually similar objects where traditional methods fail.
...
TL;DR: TALO: A plug-and-play framework for temporal alignment of 3D foundation model predictions using Thin Plate Spline with globally propagated control points and point-agnostic submap registration for robustness.


<details>
  <summary>Details</summary>
Motivation: 3D vision foundation models struggle with temporal consistency in online settings like driving scenarios, where predictions across time windows need alignment. Existing methods using global transformation have limitations in assumption validity, local alignment scope, and robustness to noisy geometry.
...
Conclusion: The proposed TALO framework demonstrates superior temporal alignment capabilities, robustness, and generality for 3D foundation models in online settings, with publicly available code.
...
TL;DR: Proposes MS-CAM, a visual explanation method for CNN interpretability in SAR tasks that combines channel-wise and element-wise weights to highlight network regions of interest.


<details>
  <summary>Details</summary>
Motivation: CNN opacity hinders high-reliability requirements in SAR applications, limiting deployment. Improving interpretability is crucial for SAR CNN development.

Method: Multi-weight self-matching class activation mapping (MS-CAM) matches SAR images with CNN feature maps and gradients, combining channel-wise and element-wise weights for visualization.

Result: MS-CAM more accurately highlights network's regions of interest and captures detailed target features on SAR classification dataset, enhancing interpretability. Also validated for weakly-supervised object localization.
...
TL;DR: BAGEL: A unified multimodal model that addresses inference inefficiencies through systematic pruning analysis and Mixture-of-Experts adaptation, achieving full model performance with only half the parameters activated.
...
Method: 1) Systematic analysis using training-free pruning as probing methodology (depth pruning and width reduction) to study component compressibility. 2) Mixture-of-Experts (MoE) Adaptation that partitions generation modules into multiple experts with sparse activation to restore generation quality. 3) Expert-frozen tuning and fully trainable adaptation approaches.

Result: Understanding components show notable compressibility, especially in generation tasks. Generation components are highly sensitive to compression. MoE adaptation enables BAGEL to achieve performance comparable to the full model while activating only about half of its parameters.

Conclusion: The proposed MoE adaptation effectively addresses inference inefficiencies in unified multimodal models, demonstrating that sparse activation can maintain performance while significantly reducing computational requirements, making unified models more practical for deployment.
...
TL;DR: WSCF-MVCC is a weakly-supervised calibration-free multi-view crowd counting method that uses only crowd counts as supervision instead of expensive density maps, leveraging self-supervised ranking loss and semantic information for better view matching.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view crowd counting methods require costly camera calibrations and crowd annotations (either scene-level or image-level). Calibration-free methods still need expensive image-level annotations for single-view modules. There's a need for more practical, annotation-efficient approaches suitable for real-world deployment.

Method: Proposes WSCF-MVCC with: 1) Weak supervision using only crowd counts instead of density maps for single-view counting module; 2) Self-supervised ranking loss with multi-scale priors to enhance perceptual ability without extra annotations; 3) Semantic information utilization for more accurate view matching and scene-level crowd estimation.
...
Conclusion: WSCF-MVCC is more suitable for practical deployment than calibrated methods, offering effective multi-view crowd counting without requiring costly camera calibrations or extensive crowd annotations.
...
TL;DR: VACoT is a framework that dynamically applies image augmentations during VLM inference to improve robustness on challenging inputs, especially OCR-related adversarial scenarios, without costly retraining.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with basic perception tasks that conventional models handle well, and retraining on augmented data is costly with diminishing returns. Current approaches are limited to local cropping and don't address the need for robust inference-time augmentation.

Method: VACoT dynamically invokes image augmentations during inference, integrates a structured collection of general visual augmentations (not just cropping), uses efficient agentic reinforcement learning with conditional rewards to encourage necessary augmentations while penalizing verbose responses.
...
Conclusion: VACoT provides an effective framework for improving VLM robustness through inference-time visual augmentations, reducing training complexity while broadening query image views, with particular strength in adversarial OCR scenarios.
...
TL;DR: SqueezeNet outperforms ResNet-50 for TB detection from chest X-rays, achieving 89% accuracy vs 73%, suggesting lightweight models may be better suited for resource-limited settings.


<details>
  <summary>Details</summary>
Motivation: Traditional TB diagnostic methods like sputum smear microscopy are inefficient, especially in resource-limited settings, prompting exploration of deep learning approaches for faster, more accessible detection.

Method: Used Kaggle dataset of 4,200 chest X-rays; compared pretrained ResNet-50 with SqueezeNet; applied data splitting, augmentation, and resizing; evaluated with accuracy, precision, recall, F1 score, and confusion matrix.

Result: SqueezeNet achieved 89% accuracy, 98% precision, 80% recall, 87% F1 score with 32% loss; ResNet-50 had 73% accuracy, 88% precision, 52% recall, 65% F1 score with 54% loss.

Conclusion: Machine learning shows promise for TB detection, with SqueezeNet performing better than ResNet-50; lightweight models could be deployed on mobile devices for resource-limited areas, but further development of faster, smaller, more accurate models is needed.
...
TL;DR: Proposed a map-free trajectory prediction algorithm using MoE for frequency selection, selective attention for redundancy filtering, and multimodal decoder with patch/point-level losses, showing superior performance on NuScenes.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory prediction methods struggle with extracting valuable scene information from redundant data, reducing computational efficiency and accuracy in complex interactive scenarios, especially with intricate agent interactions.

Method: Map-free algorithm operating across temporal, spatial, and frequency domains: 1) MoE mechanism for adaptive selection of critical frequency components and multi-scale temporal feature integration, 2) Selective attention module to filter redundant information in temporal sequences and spatial interactions, 3) Multimodal decoder supervised by patch-level and point-level losses.

Result: Experiments on NuScenes datasets demonstrate the algorithm's superiority, validating its effectiveness in handling complex interactive scenarios.

Conclusion: The proposed map-free trajectory prediction algorithm effectively addresses challenges in complex interactive scenarios by efficiently extracting valuable information and filtering redundancies across multiple domains.
...
TL;DR: SAGE is a style-adaptive generalization framework that synthesizes visual prompts to improve frozen model generalization for semantic segmentation under privacy constraints, without modifying model weights.


<details>
  <summary>Details</summary>
Motivation: Traditional fine-tuning is impossible when model parameters are inaccessible due to privacy/security constraints, creating need for input-level strategies that enhance generalization without weight modification.

Method: Uses style transfer to create diverse source domain style representations, then adaptively fuses these style cues based on visual context to form dynamic prompts that harmonize image appearance without touching model internals.

Result: Extensive experiments on five benchmark datasets show SAGE achieves competitive/superior performance vs state-of-the-art methods under privacy constraints and outperforms full fine-tuning baselines in all settings.

Conclusion: SAGE effectively bridges the gap between frozen model invariance and unseen domain diversity through closed-loop design, enabling practical domain generalization under privacy constraints.
...
TL;DR: Real-time UAV photogrammetry framework with explore-and-exploit approach that provides quality feedback and guided image acquisition for time-critical applications.


<details>
  <summary>Details</summary>
Motivation: Existing real-time UAV photogrammetry methods lack quality assessment of 3D reconstruction and guided feedback for image acquisition, which is essential for time-critical applications like disaster response and digital-twin maintenance.
...
Result: Achieves in-situ reconstruction and evaluation in near real time, reduces coverage gaps and re-flight costs through actionable feedback, and enables transition from passive to intelligent adaptive exploration workflow.

Conclusion: The framework integrates data collection, processing, 3D reconstruction, assessment, and online feedback to create an intelligent, adaptive exploration workflow for real-time UAV photogrammetry applications.
...
TL;DR: FDTA introduces a feature refinement framework with three adapters (spatial, temporal, identity) to enhance object discriminativeness in end-to-end MOT, addressing low association accuracy caused by high inter-object similarity in DETR embeddings.


<details>
  <summary>Details</summary>
Motivation: End-to-end MOT methods have strong detection but low association accuracy due to object embeddings from DETR having excessively high inter-object similarity. These embeddings focus only on category-level discrimination within single frames, while tracking requires instance-level distinction across frames with spatial and temporal continuity.

Method: FDTA (From Detection to Association) is an explicit feature refinement framework with three complementary adapters: Spatial Adapter (SA) integrates depth-aware cues for spatial continuity, Temporal Adapter (TA) aggregates historical information for temporal dependencies, and Identity Adapter (IA) uses quality-aware contrastive learning for instance-level separability.
...
Conclusion: The proposed FDTA framework successfully addresses the association accuracy limitations in end-to-end MOT by enhancing object discriminativeness through complementary spatial, temporal, and identity perspectives, leading to superior tracking performance on challenging benchmarks.
...
TL;DR: Researchers developed a camera-guided pipeline to generate accurate radar semantic segmentation labels without human annotation, addressing the scarcity of labeled 4D radar datasets and improving reproducibility.


<details>
  <summary>Details</summary>
Motivation: 4D radar shows promise for robust perception in adverse conditions, but radar semantic segmentation progress is limited by scarce open-source datasets and labels. Existing datasets like RaDelft only provide LiDAR annotations with no public code for radar label generation, hindering reproducibility and research.

Method: Developed a camera-guided radar labeling pipeline that projects radar point clouds into camera-based semantic segmentation results, then applies spatial clustering to generate accurate radar labels without human annotation.

Result: Successfully reproduced RaDelft group's numerical results and demonstrated that the camera-guided pipeline generates accurate radar labels that significantly enhance radar segmentation accuracy. Also studied and quantified how different fog levels affect radar labeling performance.

Conclusion: Established a reproducible framework that enables the research community to train and evaluate labeled 4D radar data, addressing the critical bottleneck of label scarcity in radar semantic segmentation research.
...
TL;DR: Skywork-R1V4 is a 30B parameter multimodal agent that unifies image manipulation, web search, and planning through supervised fine-tuning on 30K trajectories, achieving SOTA results without reinforcement learning.
...
Method: Developed a 30B parameter multimodal agent that unifies multimodal planning, active image manipulation ("thinking with images"), deep multimodal search, and interleaved reasoning. Trained solely via supervised fine-tuning on fewer than 30,000 high-quality, planning-execution-consistent trajectories with stepwise consistency filtering.
...
Conclusion: Sophisticated agentic multimodal intelligence can be achieved through carefully curated supervised learning alone, without any reliance on reinforcement learning, demonstrating the effectiveness of unified multimodal planning and execution.
...
TL;DR: Nav-R²: A framework for open-vocabulary object-goal navigation using structured Chain-of-Thought reasoning and Similarity-Aware Memory to improve decision-making and success rates for unseen objects.
...
Method: Explicitly models target-environment relationships and environment-action planning through structured Chain-of-Thought reasoning, coupled with a Similarity-Aware Memory that preserves target-relevant and observation-relevant features from temporal and semantic perspectives by compressing video frames and fusing historical observations.

Result: Achieves state-of-the-art performance in localizing unseen objects through a streamlined pipeline, avoids overfitting to seen object categories, and maintains real-time inference at 2Hz.

Conclusion: Nav-R² addresses key challenges in open-vocabulary object-goal navigation by providing transparent reasoning and improved performance on novel objects while maintaining efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Multi-agent debate (MAD) has shown promise for leveraging complementary strengths of diverse LLMs, but its application has been limited to language-only tasks, leaving multimodal reasoning problems underexplored.

Method: WISE (Weighted Iterative Society-of-Experts) partitions agents into Solvers (generate solutions) and Reflectors (verify correctness, assign weights, provide feedback). Uses a modified Dawid-Skene algorithm to aggregate solutions across debate rounds while accounting for response variance and feedback weights.

Result: WISE consistently improves accuracy by 2-7% over state-of-the-art MAD setups and aggregation methods across diverse multimodal tasks (SMART-840, VisualPuzzles, EvoChart-QA, SMART-840++) and various LLM configurations.

Conclusion: The WISE framework successfully generalizes multi-agent debate to multimodal reasoning problems, demonstrating that heterogeneous expert agents with specialized roles and weighted aggregation can significantly improve performance on vision-and-language tasks.
...
TL;DR: MitUNet is a hybrid neural network for precise wall segmentation from 2D floor plans, combining Mix-Transformer encoder with U-Net decoder and Tversky loss optimization to improve boundary accuracy and thin structure detection for 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for wall segmentation from floor plans struggle with detecting thin structural components and produce masks with irregular boundaries, lacking the geometric precision needed for subsequent vectorization in 3D reconstruction pipelines.

Method: MitUNet uses a hierarchical Mix-Transformer encoder to capture global context and a U-Net decoder enhanced with scSE attention blocks for boundary recovery. It employs Tversky loss function optimization with fine-tuned hyperparameters to balance precision and recall, suppressing false positives while maintaining sensitivity to thin structures.

Result: Experiments on the public CubiCasa5k dataset and a proprietary regional dataset show that MitUNet generates structurally correct masks with high boundary accuracy, outperforming standard single-task models.
...
TL;DR: GuiDG framework improves domain generalization in vision-language models by training domain experts instead of a universal model, using prompt tuning and cross-modal attention for adaptive expert integration.


<details>
  <summary>Details</summary>
Motivation: Current fine-tuning methods for vision-language models face a trade-off between domain specificity and generalization ability. Fine-tuning a universal model on entire datasets compromises generalization to unseen domains, creating a need for better approaches that maintain both specificity and generalization.
...
Result: Extensive experiments on standard DG benchmarks and newly constructed ImageNet-DG demonstrate that GuiDG improves upon state-of-the-art fine-tuning methods while maintaining efficiency.

Conclusion: Training multiple parameter-efficient expert models on partitioned source domains leads to better generalization than fine-tuning a universal model. The proposed GuiDG framework effectively addresses the domain specificity vs. generalization trade-off in VLM fine-tuning.
...
Conclusion: Reinforcement learning approaches show advantages in GUI navigation, and the findings offer practical guidance for building more capable and generalizable GUI agents. The GUI Exploration Lab addresses the limitation of inaccessible real-world GUI environments for systematic agent training and evaluation.
...
TL;DR: WorldMM is a multimodal memory agent for long video understanding that uses episodic, semantic, and visual memories with adaptive retrieval across multiple temporal scales, achieving state-of-the-art performance on long video QA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing video LLMs struggle with hours-long videos due to limited context capacity and loss of visual details. Current memory-augmented methods rely too heavily on text summaries and fixed temporal scales, failing to utilize visual evidence and handle events of variable durations.

Method: WorldMM constructs three complementary memory types: episodic memory for factual events across multiple temporal scales, semantic memory for high-level conceptual knowledge, and visual memory for detailed scene information. An adaptive retrieval agent iteratively selects relevant memory sources and temporal granularities based on queries.
...
Conclusion: WorldMM effectively addresses long video reasoning challenges through its multimodal memory architecture and adaptive retrieval mechanism, demonstrating superior performance in understanding complex, long-duration videos.
...
TL;DR: LightHCG is a lightweight causal representation learning model for glaucoma detection that uses only 1-7% of parameters compared to existing vision models while achieving high classification performance and enabling intervention analysis.


<details>
  <summary>Details</summary>
Motivation: Current AI-driven glaucoma detection models have limitations in reliability, parameter efficiency, potential spurious correlations, and lack capability for intervention analysis or clinical simulations. There's a need for more efficient and causally-aware models.
...
Result: The model achieves high glaucoma classification performance while using 93-99% fewer parameters (weights) compared to existing advanced vision models like InceptionV3, MobileNetV2, and VGG16.

Conclusion: LightHCG provides a lightweight, causally-aware approach to glaucoma detection that not only improves classification efficiency but also enables AI-driven intervention analysis, addressing key limitations of current vision models.
...
TL;DR: A momentum self-distillation method for efficient vision-language model training in healthcare, achieving competitive performance with single GPU while reducing computational demands.


<details>
  <summary>Details</summary>
Motivation: Medical healthcare faces challenges in obtaining detailed annotations, requiring robust VLMs. Contrastive learning typically needs large batch sizes which are computationally demanding and limited to well-resourced institutions. With limited healthcare data, it's important to extract knowledge from both data and models efficiently.

Method: Leverages momentum self-distillation to enhance multimodal learning and integrates momentum mechanisms with gradient accumulation to enlarge effective batch size without increasing resource consumption. Uses momentum method combined with distillation for computational efficiency and knowledge exploitation.
...
TL;DR: BOLT extracts orthogonal spectral bases from existing fine-tuned models to enable efficient transfer to new tasks with minimal training.


<details>
  <summary>Details</summary>
Motivation: Meta-learning requires expensive meta-training and can be unstable, while many task-specific pre-trained models exist but lack efficient transfer methods to new tasks with limited data/compute.
...
Result: BOLT provides strong training-free initialization and achieves robust performance compared to PEFT baselines and meta-learned initialization with very few trainable parameters.

Conclusion: Constraining adaptation to task-informed orthogonal subspaces offers an effective alternative for unseen-task transfer, reusing existing models efficiently.
...
TL;DR: Temporal Dynamics Enhancer (TDE) improves SNN object detection by generating diverse temporal stimuli and using spike-driven attention, achieving state-of-the-art performance with reduced energy consumption.


<details>
  <summary>Details</summary>
Motivation: Current SNNs have limited expressive power because they replicate or aggregate inputs at fixed intervals, leading to neurons receiving nearly identical stimuli across time steps, which is particularly problematic for complex tasks like object detection.

Method: Proposes Temporal Dynamics Enhancer (TDE) with two modules: Spiking Encoder (SE) generates diverse input stimuli across time steps, and Attention Gating Module (AGM) guides SE generation based on inter-temporal dependencies. Also introduces Spike-Driven Attention (SDA) to eliminate high-energy multiplication operations from attention modules.

Result: TDE achieves mAP50-95 scores of 57.7% on PASCAL VOC and 47.6% on neuromorphic EvDET200K dataset, outperforming state-of-the-art methods. SDA consumes only 0.240 times the energy of conventional attention modules.

Conclusion: TDE effectively enhances SNNs' temporal modeling capacity for object detection while maintaining energy efficiency through spike-driven attention mechanisms.
...
TL;DR: A comprehensive analysis of the nuScenes dataset's creation, impact, and legacy in autonomous driving research, revealing previously undisclosed technical details and tracing its influence on subsequent datasets and standards.


<details>
  <summary>Details</summary>
Motivation: To provide unprecedented insights into the creation and technical details of the widely-used nuScenes autonomous driving dataset, which had many implementation details not previously revealed in academic publications, and to trace its substantial influence on the field.

Method: Revisiting the nuScenes dataset and its extensions (nuImages and Panoptic nuScenes), revealing technical implementation details, analyzing its impact on subsequent datasets and community standards, and surveying both official and unofficial tasks using the dataset.

Result: Provides comprehensive documentation of nuScenes' creation process, demonstrates its pioneering role as the first dataset with radar data, diverse urban scenes from two continents, and collected using fully autonomous vehicles, and shows how it defined numerous standards and influenced many subsequent datasets.

Conclusion: nuScenes has been foundational to autonomous driving research, serving as a benchmark dataset that promoted multi-modal sensor fusion, standardized evaluation, and broad task coverage, while significantly shaping the field's development through its technical innovations and community standards.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D layout estimation models are trained on synthetic datasets with simple single-room/single-floor environments, requiring scene splitting that removes global spatial context essential for reasoning about multi-level structures like staircases.

Method: Introduces HouseLayout3D benchmark for real-world multi-floor building layout estimation, and presents MultiFloor3D - a simple training-free baseline that leverages recent scene understanding methods.
...
Conclusion: The work addresses a critical gap in 3D layout estimation by introducing a real-world benchmark for full building-scale analysis and demonstrating that even simple training-free methods can outperform existing approaches, indicating significant room for improvement in this area.
...
TL;DR: ClusterStyle: A clustering-based framework for stylized motion generation that models intra-style diversity using global and local prototype embeddings, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing stylized motion generation models struggle with capturing intra-style diversity - the ability for a single style to correspond to diverse motion variations. Current approaches learn unstructured embeddings that fail to model the rich variations within style categories.

Method: Proposes ClusterStyle framework with: 1) Global and local prototype-based style embedding spaces to capture style diversity at both category and temporal levels, 2) Non-learnable prototype anchors for structured optimization, 3) Stylistic Modulation Adapter (SMA) to integrate style features into pretrained text-to-motion models.

Result: Extensive experiments show ClusterStyle outperforms existing state-of-the-art models in both stylized motion generation and motion style transfer tasks.

Conclusion: The clustering-based approach with structured prototype embeddings effectively addresses the intra-style diversity challenge in motion generation, enabling more varied and realistic stylized motions.
...
TL;DR: STL is a self-training framework that improves VLM reasoning by generating structured rationales (see-then-think) and learning from both positive and negative explanations.


<details>
  <summary>Details</summary>
Motivation: Current VLM reasoning enhancement methods rely on expensive human annotations, proprietary models, or self-training that ignores perception. There's a need for cost-effective joint improvement of both perception and reasoning.

Method: STL uses a structured reasoning template: first extract visual attributes (see), then use them for reasoning (think). The framework employs self-training where models generate and learn from their own structured rationales, augmented with negative rationales that explain why wrong answers are incorrect.

Result: STL consistently outperforms baselines trained only on answers or self-generated reasoning across diverse domains. Qualitative analysis confirms high-quality rationales.

Conclusion: STL provides a cost-effective solution to enhance multimodal reasoning in VLMs by jointly improving perception and reasoning through structured self-training with positive and negative rationales.
...
TL;DR: Audio-video joint denoising training improves video generation quality even when only video quality matters, by using audio as a privileged signal that helps model physical causal relationships.
...
Method: Introduces AVFullDiT architecture that leverages pre-trained text-to-video and text-to-audio modules for joint denoising. Trains both a T2AV model with audio-video joint denoising and a T2V-only counterpart under identical settings for comparison.

Result: Audio-video joint denoising consistently improves video generation on challenging subsets with large and object contact motions. The audio acts as a privileged signal that helps the model internalize causal relationships between visual events and acoustic consequences.

Conclusion: Cross-modal co-training is a promising approach for developing stronger, more physically grounded world models, as audio-video joint denoising provides benefits beyond synchrony by regularizing video dynamics through physical causal relationships.
...
TL;DR: SEER-Bench introduces a sequential embodied AI benchmark combining EQA and EMN tasks, with 3DSPMR approach using 3D spatial memory to enhance MLLMs for sequential reasoning.


<details>
  <summary>Details</summary>
Motivation: Real-world embodied agents face sequential tasks where they must reuse spatial knowledge from previous explorations, but existing research focuses on single-task settings. The paper addresses the challenge of leveraging accumulated spatial knowledge for subsequent reasoning in sequential embodied tasks.

Method: Proposes 3DSPMR (3D Spatial Memory Reasoning) that incorporates relational, visual, and geometric cues from explored regions to augment Multi-Modal Large Language Models. This is the first work to explicitly integrate geometric information into MLLM-based spatial understanding.
...
Conclusion: The work successfully addresses the sequential embodied AI challenge by developing a benchmark and a novel 3D spatial memory reasoning approach that significantly improves performance on sequential embodied tasks.
...
TL;DR: TGDD improves dataset distillation by aligning synthetic data distributions with evolving model representations during training, achieving better downstream performance without extra computational cost.


<details>
  <summary>Details</summary>
Motivation: Current distribution matching methods for dataset distillation overlook how feature representations evolve during model training, limiting synthetic data expressiveness and downstream task performance.

Method: TGDD reformulates distribution matching as dynamic alignment along the model's training trajectory, capturing evolving semantics at each stage while using distribution constraint regularization to reduce class overlap.

Result: TGDD achieves state-of-the-art performance on ten datasets, with 5.0% accuracy gain on high-resolution benchmarks, while maintaining efficiency without additional optimization overhead.

Conclusion: TGDD effectively preserves semantic diversity and representativeness in synthetic data by aligning with model training trajectories, achieving superior performance-efficiency balance for dataset distillation.
...
TL;DR: WorldPack is a video world model with compressed memory that improves long-term spatial consistency and fidelity in video generation using trajectory packing and memory retrieval techniques.


<details>
  <summary>Details</summary>
Motivation: Existing video world models struggle with temporally- and spatially-consistent long-term world modeling due to prohibitively expensive computational costs for long-context inputs, which remains unresolved even with recent state-of-the-art models.

Method: WorldPack uses efficient compressed memory consisting of two key components: trajectory packing (for high context efficiency) and memory retrieval (to maintain consistency in rollouts and support long-term generations requiring spatial reasoning).

Result: WorldPack significantly outperforms strong state-of-the-art models on the LoopNav benchmark in Minecraft, demonstrating notable improvements in spatial consistency, fidelity, and quality in long-term generation despite using much shorter context length.

Conclusion: WorldPack successfully addresses the long-standing problem of consistent long-term world modeling through efficient compressed memory architecture, achieving superior performance in spatial consistency and generation quality compared to existing approaches.
...
TL;DR: G-SHARP is a real-time surgical scene reconstruction framework using Gaussian splatting for minimally invasive procedures, built on commercial-friendly GSplat for practical deployment.


<details>
  <summary>Details</summary>
Motivation: Existing real-time endoscopic reconstruction methods often rely on non-commercial derivatives, limiting practical deployability in clinical settings. There's a need for a commercially compatible solution that can provide fast and accurate 3D modeling of deformable tissue for minimally invasive surgeries.

Method: G-SHARP is built natively on the GSplat (Apache-2.0) differentiable Gaussian rasterizer, enabling principled deformation modeling and robust occlusion handling. It's designed as the first surgical pipeline using this commercial-friendly framework.

Result: Achieves state-of-the-art reconstruction quality on the EndoNeRF pulling benchmark with strong speed-accuracy trade-offs suitable for intra-operative use. Successfully deployed via Holoscan SDK on NVIDIA IGX Orin and Thor edge hardware for real-time surgical visualization.

Conclusion: G-SHARP provides a commercially deployable, real-time surgical reconstruction framework that overcomes limitations of previous non-commercial approaches, enabling practical operating-room applications with high-fidelity tissue modeling.
...
TL;DR: UCAgents is a hierarchical multi-agent framework for medical VQA that enforces unidirectional convergence through structured evidence auditing, suppressing rhetorical drift while amplifying visual signal extraction with 87.7% lower token cost.


<details>
  <summary>Details</summary>
Motivation: Current VLMs in medical diagnosis suffer from reasoning detachment where explanations drift from verifiable image evidence, undermining clinical trust. Existing multi-agent frameworks amplify textual noise and computational cost while failing to anchor reasoning to visual evidence.

Method: UCAgents uses a hierarchical multi-agent framework with structured evidence auditing that forbids position changes and limits agent interactions to targeted evidence verification. It introduces one-round inquiry discussion to uncover visual-textual misalignment risks, constraining both visual ambiguity and textual noise.

Result: Achieves 71.3% accuracy on PathVQA (+6.0% over SOTA) with 87.7% lower token cost. Strikes balance between uncovering more visual evidence and avoiding confusing textual interference across four medical VQA benchmarks.

Conclusion: UCAgents demonstrates both diagnostic reliability and computational efficiency critical for real-world clinical deployment by enforcing evidence-based convergence while minimizing computational overhead.
...
TL;DR: 3D-SLIM replaces causal attention masks with adaptive spatial masks for better 3D scene-language understanding, addressing sequential bias and restricted instruction attention without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Existing 3D scene-language methods use standard LLM decoders with causal attention masks, which create two problems: 1) sequential bias among order-agnostic 3D objects, and 2) restricted object-instruction attention that hinders task-specific reasoning.
...
Result: Substantial performance improvements across diverse 3D scene-language tasks. Extensive experiments across multiple benchmarks and LLM baselines validate effectiveness, showing the critical role of decoder design in 3D multi-modal reasoning.

Conclusion: 3D-SLIM effectively addresses fundamental conflicts in 3D scene understanding by replacing causal masks with adaptive spatial attention masks, demonstrating that simple masking strategy improvements can significantly enhance 3D multi-modal reasoning without architectural changes.
...
TL;DR: YingVideo-MV is a cascaded framework for generating music-performance videos with camera motions from audio signals, addressing the gap in existing methods that lack explicit camera control for music videos.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models for audio-driven avatar video generation have made progress in natural audio-visual synchronization, but music-performance videos with camera motions remain unexplored. Existing long-video generation methods lack explicit camera motion control.
...
Result: The method achieves outstanding performance in generating coherent and expressive music videos with precise music-motion-camera synchronization, demonstrated through comprehensive benchmark tests.

Conclusion: YingVideo-MV is the first framework for music-driven long-video generation that successfully integrates camera motion control and achieves high-quality music performance video synthesis from audio signals.
...
TL;DR: The paper analyzes limitations of deep-learning-based registration methods using GMMs for partial-to-partial point set registration, introduces an attention-based reference point shifting (ARPS) layer to achieve transformation-invariant features, and shows significant performance improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: To investigate problems with deep-learning-based registration methods using Gaussian mixture models (GMMs) for partial-to-partial point set registration, particularly focusing on the limitations of DeepGMR and similar approaches in handling transformation invariance of feature vectors.

Method: Introduces an attention-based reference point shifting (ARPS) layer that uses an attention module to robustly identify a common reference point between two partial point sets, enabling transformation-invariant feature extraction without relying on overlap region detection.

Result: The ARPS layer significantly enhances performance of DeepGMR and its variant UGMMReg, with these extended models outperforming prior deep learning methods that use attention blocks and Transformers for overlap region or reference point extraction.

Conclusion: The findings provide deeper insights into registration methods using deep learning and GMMs, demonstrating that focusing on common reference point identification rather than overlap regions leads to more robust transformation-invariant features and better registration performance.
...
TL;DR: MedSeg-TTA is a comprehensive benchmark evaluating 20 test-time adaptation methods across 7 medical imaging modalities with standardized protocols, revealing that no single method works best across all conditions and highlighting modality-specific performance patterns.


<details>
  <summary>Details</summary>
Motivation: Current test-time adaptation evaluations in medical image segmentation are limited in modality coverage, task diversity, and methodological consistency, creating a need for a standardized benchmark to enable fair comparisons and guide clinical deployment.

Method: Created MedSeg-TTA benchmark with unified data preprocessing, backbone configuration, and test protocols across 7 imaging modalities (MRI, CT, ultrasound, pathology, dermoscopy, OCT, chest X-ray). Evaluated 20 adaptation methods covering four paradigms: Input-level Transformation, Feature-level Alignment, Output-level Regularization, and Prior Estimation.

Result: No single paradigm performs best in all conditions. Input-level methods are more stable under mild appearance shifts. Feature-level and Output-level methods excel in boundary-related metrics. Prior-based methods show strong modality dependence. Several methods degrade significantly under large inter-center and inter-device shifts.

Conclusion: MedSeg-TTA provides a rigorous foundation for future research on robust test-time adaptation in medical imaging, emphasizing the importance of principled method selection for clinical deployment based on modality characteristics and domain shift severity.
...
<details>
  <summary>Details</summary>
Motivation: Current document layout parsing methods use fragmented, multi-stage pipelines that suffer from error propagation and fail to leverage the synergies of joint training, limiting AI's ability to access structured knowledge from documents.

Method: Introduces dots.ocr, a single Vision-Language Model that jointly learns three core document parsing tasks within a unified, end-to-end framework, enabled by a scalable data engine that synthesizes a vast multilingual corpus.

Result: Achieves state-of-the-art performance on OmniDocBench and establishes a new baseline on XDocParse (126 languages), outperforming the next-best competitor by +7.4 points, demonstrating robust multilingual capabilities.

Conclusion: The unified end-to-end paradigm for document layout parsing proves superior to fragmented pipelines, with dots.ocr showing strong performance across diverse languages, layouts, and domains, advancing global document intelligence research.
...
<details>
  <summary>Details</summary>
Motivation: Autoregressive models are structurally misaligned with geospatial understanding because they force sequential narratives onto inherently parallel spatial scenes, hindering structured and coherent output generation.
...
Conclusion: Aligning generative process with data's intrinsic structure is key to unlocking superior performance in complex geospatial analysis, validating the parallel refinement approach over sequential autoregressive methods.
...
TL;DR: ViT-SR uses a two-stage training approach with self-supervised pre-training on colorization followed by fine-tuning for 4x super-resolution, achieving strong results on DIV2K benchmark.


<details>
  <summary>Details</summary>
Motivation: Single Image Super-Resolution (SISR) remains a challenging problem in computer vision, and the paper aims to improve performance by leveraging self-supervised pre-training to learn rich visual representations before fine-tuning for the super-resolution task.

Method: Two-stage training strategy: 1) Self-supervised pretraining on a colorization task to learn generalizable visual representations, 2) Fine-tuning for 4x super-resolution by predicting high-frequency residual images added to initial bicubic interpolation, simplifying residual learning.

Result: Achieved impressive results on DIV2K benchmark: SSIM of 0.712 and PSNR of 22.90 dB, demonstrating the effectiveness of the two-stage approach.

Conclusion: The two-stage self-supervised pre-training approach is effective for complex image restoration tasks like super-resolution, and further improvements could be achieved with larger ViT architectures or alternative pretext tasks.
...
<details>
  <summary>Details</summary>
Motivation: General-purpose vision-language models are suboptimal for remote sensing tasks. Existing geospatial VLMs struggle to differentiate between task types and interpretation granularities, limiting their ability to balance local detail perception and global contextual understanding.

Method: SkyMoE employs an adaptive router that generates task- and granularity-aware routing instructions to specialized LLM experts. It introduces context-disentangled augmentation to create contrastive pairs between local and global features, promoting expert decoupling and granularity sensitivity. Also constructs MGRS-Bench benchmark for evaluation.
...
Conclusion: SkyMoE effectively addresses the limitations of existing geospatial VLMs by enabling specialized expert handling of diverse sub-tasks with different granularity requirements, achieving superior performance in multimodal, multi-task remote sensing interpretation.
...
TL;DR: This dissertation addresses zero-shot anomaly classification/segmentation, introducing CoDeGraph to handle consistent anomalies, extending to 3D medical imaging, and bridging batch/text-based methods.


<details>
  <summary>Details</summary>
Motivation: Zero-shot anomaly detection is crucial for industrial inspection and medical imaging where training data is unavailable. The work addresses the challenge of "consistent anomalies" - recurring similar anomalies that bias distance-based methods.

Method: 1) Formalizes consistent anomaly problem and analyzes patch representations from pre-trained Vision Transformers, identifying similarity scaling and neighbor-burnout phenomena. 2) Introduces CoDeGraph framework using multi-stage graph construction, community detection, and structured refinement to filter consistent anomalies. 3) Extends to 3D medical imaging with training-free volumetric tokenization for MRI. 4) Bridges batch/text-based methods using CoDeGraph pseudo-masks to supervise vision-language models.

Result: The dissertation demonstrates: 1) Understanding of statistical/geometric behavior of patch representations with consistent anomalies. 2) CoDeGraph effectively suppresses consistent anomaly influence. 3) Zero-shot 3D anomaly segmentation is achievable without 3D training samples. 4) Pseudo-masks can supervise prompt-driven vision-language models.

Conclusion: This work provides both theoretical understanding and practical solutions for zero-shot anomaly classification/segmentation, addressing core challenges through principled approaches that handle consistent anomalies, scale to 3D medical imaging, and integrate different zero-shot paradigms.
...
TL;DR: Proposes Noisy Query Tokens method to bridge Vision-Language Models with Diffusion Models, addressing task generalization collapse through distributed representation learning and VAE branch for detail recovery.


<details>
  <summary>Details</summary>
Motivation: Existing methods using fixed learnable query tokens for connecting VLMs and Diffusion Models suffer from task generalization collapse - they fail to adapt to new tasks distant from pre-training tasks, limiting their flexibility and continual learning capabilities.

Method: 1) Noisy Query Tokens: Learn a distributed representation space between VLM and Diffusion Model via end-to-end optimization to enhance continual learning. 2) VAE branch with linear projection: Recovers fine-grained image details that might be lost in the bridging process.

Result: Experimental results confirm the approach mitigates generalization collapse and enables stable continual learning across diverse tasks, demonstrating improved adaptability to new tasks beyond pre-training scope.

Conclusion: The proposed method successfully addresses the generalization collapse problem in bridging VLMs with Diffusion Models, enabling more flexible and stable continual learning through distributed representation learning and detail-preserving mechanisms.
...
TL;DR: Training-free acceleration method for multi-view 3D models that achieves 8-10× speedup while maintaining accuracy by converting early global layers to frame attention and subsampling global attention with diagonal preservation.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view 3D models like VGGT and π³ rely heavily on global self-attention, resulting in high computational costs. While sparse-attention variants offer partial speedups, there's no systematic analysis of how global attention contributes to multi-view reasoning.
...
Conclusion: The systematic analysis of global attention roles enables effective training-free acceleration that maintains performance while dramatically reducing computational costs, making multi-view 3D models more practical for real-world applications.
...
TL;DR: OmniPerson is a unified identity-preserving pedestrian generation pipeline for visible/infrared image/video ReID tasks, offering holistic control over pedestrian attributes with multi-reference identity preservation.


<details>
  <summary>Details</summary>
Motivation: Person re-identification suffers from limited training data due to privacy and annotation costs. Existing pedestrian generation methods fail to ensure identity consistency and lack controllability, limiting their effectiveness for dataset augmentation.

Method: 1) OmniPerson unified generation model with control over all key pedestrian attributes (RGB/IR modality, image/video, any number of reference images, poses, text). 2) Multi-Refer Fuser for robust identity preservation from multiple reference images. 3) PersonSyn dataset with automated curation pipeline transforming public ReID benchmarks into richly annotated resources.

Result: OmniPerson achieves state-of-the-art in pedestrian generation with high visual fidelity and identity consistency. Augmenting existing datasets with generated data consistently improves ReID model performance.

Conclusion: OmniPerson provides an effective solution for generating high-quality pedestrian data for ReID tasks, addressing data scarcity while maintaining identity consistency and controllability. The method, dataset, and code will be open-sourced.
...
TL;DR: Panel2Patch is a novel biomedical vision-language pretraining pipeline that extracts hierarchical supervision from scientific figures by parsing layouts, panels, and markers to create multi-granular vision-language pairs, enabling more effective pretraining with less data.


<details>
  <summary>Details</summary>
Motivation: Current biomedical vision-language pretraining compresses rich scientific figures into coarse figure-level pairs, discarding fine-grained correspondences that clinicians actually use when examining local structures. There's a need to preserve the hierarchical structure and local semantics in biomedical literature.

Method: Panel2Patch parses layouts, panels, and visual markers from scientific figures and captions, then constructs hierarchical aligned vision-language pairs at figure, panel, and patch levels. It uses a granularity-aware pretraining strategy that unifies heterogeneous objectives from coarse descriptions to fine region-focused phrases.
...
Conclusion: Panel2Patch addresses the limitation of coarse figure-level pretraining by mining hierarchical structure from biomedical literature, preserving local semantics, and enabling more efficient and effective biomedical vision-language model development.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for co-speech gesture synthesis use motion graphs but rely on one-to-one mapping between audio and gestures, which is insufficient due to the inherent many-to-many nature of audio-gesture relationships. Current approaches using feature distances or shared embedding spaces cannot adequately handle this complexity.

Method: 1) Use a diffusion model to generate gesture motions by learning the joint distribution of audio and motion, enabling contextually appropriate gesture generation. 2) Extract both low-level and high-level audio features to enrich diffusion model training. 3) Apply a motion-based retrieval algorithm to find optimal paths in the motion graph using global and local similarity assessment. 4) Stitch retrieved motion segments into coherent video output.

Result: Experimental results show significant improvement over prior approaches in both synchronization accuracy and naturalness of generated gestures.

Conclusion: The proposed framework effectively addresses the many-to-many mapping challenge in audio-gesture synthesis by combining diffusion models for gesture generation with sophisticated motion graph retrieval and stitching techniques, resulting in more synchronized and natural co-speech gestures.
...
TL;DR: Texture-mapped Gaussian Splatting reduces parameter count by using adaptive texture maps for appearance details instead of many small Gaussian primitives.


<details>
  <summary>Details</summary>
Motivation: Standard Gaussian Splatting wastes parameters by using many small Gaussian primitives for fine appearance details, especially when geometry and appearance have different frequency characteristics. Texture mapping offers a more efficient alternative for representing detailed appearance.

Method: Proposes per-primitive texture maps for 2D Gaussian primitives with adaptive texture resolution. Texture texel size is bounded by image sampling frequency and adapts to input image content through upscaling/downscaling during optimization. Also enables control of primitive count based on texture resolution.
...
Conclusion: Incorporating adaptive texture mapping into Gaussian Splatting provides a principled way to represent detailed appearance more efficiently than using many small Gaussian primitives, improving parameter efficiency while maintaining quality.
...
TL;DR: RULER-Bench is a new benchmark for evaluating reasoning capabilities of video generation models, focusing on rule-based cognitive reasoning across 40 tasks and 6 rule categories.


<details>
  <summary>Details</summary>
Motivation: Existing video generation benchmarks focus on visual perception and understanding, but lack evaluation of rule-based reasoning capabilities. There's a need for fine-grained decomposition and comprehensive evaluation of reasoning in video models.

Method: Built RULER-Bench covering 40 tasks across 6 rule categories with 622 annotated instances. Uses text-to-video and image-to-video paradigms. Evaluates with checklist of 4 metrics and GPT-o3 scoring (85% alignment with human judgments).

Result: State-of-the-art video generation models achieve only 48.87% on rule coherence metric, showing significant room for improvement in reasoning capabilities.

Conclusion: RULER-Bench reveals substantial gaps in reasoning capabilities of current video generation models and provides a framework to advance reasoning-aware video generation toward vision foundation intelligence.
...
TL;DR: PPTBench is a comprehensive multimodal benchmark for evaluating LLMs on PowerPoint tasks, revealing current models struggle with visual-layout reasoning despite good semantic understanding.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks overlook layout-centric challenges in PowerPoint presentations, which are central to real-world slide creation and editing. PowerPoint presentations combine rich textual content with structured visual layouts, making them ideal for evaluating multimodal reasoning and layout understanding abilities of MLLMs.

Method: Introduces PPTBench, a comprehensive multimodal benchmark leveraging 958 PPTX files with 4,439 samples across four categories: Detection, Understanding, Modification, and Generation. The benchmark evaluates models' ability to handle both semantic content and visual-layout reasoning in PowerPoint scenarios.

Result: Experiments reveal substantial gap between semantic understanding and visual-layout reasoning in current MLLMs. Models can interpret slide content but fail to produce coherent spatial arrangements. Current MLLMs struggle to combine visual cues with JSON-based layout structures and fail to integrate visual information into API planning ability. Case studies show systematic layout errors like misalignment and element overlap.

Conclusion: PPTBench provides new perspective for evaluating VLLMs in PPT scenarios, highlighting challenges and directions for future research on visual-structural reasoning and coherent slide generation. The benchmark and code are fully released to support reproducibility and future research.
...
TL;DR: A novel pretraining strategy using large-scale simulated datasets improves generalization of remote sensing image fusion models across different satellite sensors.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for remote sensing image fusion suffer from poor generalization due to limited real training data and domain gaps between different satellite sensors.
...
Result: Pretrained models significantly improve generalization across six satellite datasets (WorldView-2/3/4, IKONOS, QuickBird, GaoFen-2) in zero-shot and one-shot scenarios, with superior performance across CNN, Transformer, and Mamba architectures.

Conclusion: The work provides a practical solution for cross-domain pansharpening, establishes a new benchmark for generalization in remote sensing image fusion, and demonstrates the potential of foundation models through advanced training strategies.
...
TL;DR: PoreTrack3D is the first benchmark for dynamic 3D Gaussian splatting in pore-scale, non-rigid facial trajectory tracking, featuring over 440,000 facial trajectories with 68 manually reviewed full-length sequences.


<details>
  <summary>Details</summary>
Motivation: There's a need for benchmark datasets that capture both traditional facial landmarks and pore-scale keypoints to advance the study of fine-grained facial expressions through analysis of subtle skin-surface motion.

Method: Created a comprehensive benchmark dataset with over 440,000 facial trajectories (including 52,000+ longer than 10 frames and 68 manually reviewed 150-frame sequences). Developed a pipeline for high-fidelity facial motion capture and dynamic 3D reconstruction.

Result: Established the first performance baseline in this domain by systematically evaluating state-of-the-art dynamic 3D Gaussian splatting methods on PoreTrack3D. The dataset is publicly available.

Conclusion: PoreTrack3D advances fine-grained facial expression analysis and establishes a new framework for high-fidelity facial motion capture and dynamic 3D reconstruction, providing the first benchmark for pore-scale facial trajectory tracking.
...
TL;DR: SelVA introduces text-conditioned selective video-to-audio generation that produces only user-intended sounds from multi-object videos, addressing limitations of current mixed-sound approaches.


<details>
  <summary>Details</summary>
Motivation: Current V2A methods generate single source-mixed sounds at once, making precise audio editing difficult for multimedia production where individual sound tracks are needed for creative control.

Method: SelVA treats text prompts as explicit source selectors, modulates video encoder to extract prompt-relevant features, uses supplementary tokens to suppress text-irrelevant activations, and employs self-augmentation for mono audio supervision.

Result: Extensive experiments on VGG-MONOAUDIO benchmark show SelVA's effectiveness in audio quality, semantic alignment, and temporal synchronization, outperforming existing approaches.

Conclusion: SelVA successfully addresses the selective V2A generation problem, enabling precise audio generation from multi-object videos with text conditioning, advancing multimedia production capabilities.
...
TL;DR: Hybrid document retrieval system combining ColPali's patch-level visual similarity with OCR spatial coordinates to enable precise region retrieval instead of entire pages.


<details>
  <summary>Details</summary>
Motivation: VLMs like ColPali retrieve entire pages rather than specific regions, limiting utility for RAG where precise context is needed. OCR systems extract structured text with coordinates but lack semantic relevance assessment.

Method: Unified hybrid architecture using ColPali's patch-level similarity scores as spatial relevance filters over OCR-extracted regions. Formalizes coordinate mapping between vision transformer patch grids and OCR bounding boxes, introduces intersection metrics for relevance propagation, and establishes theoretical bounds on retrieval precision.

Result: Approach operates at inference time without additional training. Snappy implementation released as open-source with empirical evaluation ongoing.

Conclusion: Proposed hybrid system bridges the gap between semantic visual retrieval and precise spatial localization, enabling more accurate region-level document retrieval for RAG applications.
...
TL;DR: PolarGuide-GSDR integrates polarization guidance with 3D Gaussian Splatting to achieve real-time, high-fidelity reconstruction of specular scenes without environment maps or restrictive material assumptions.


<details>
  <summary>Details</summary>
Motivation: Existing methods like polarization-aware NeRF have slow training and rendering, while 3DGS struggles with reflection-geometry entanglement. Current approaches either require environment maps or make restrictive material/viewpoint assumptions.

Method: Proposes a bidirectional coupling mechanism: first uses 3DGS geometric priors to resolve polarization ambiguity, then uses refined polarization cues to guide 3DGS's normal and spherical harmonic representation for reflection separation.
...
Conclusion: First framework embedding polarization priors directly into 3DGS optimization, offering superior interpretability and real-time performance for modeling complex reflective scenes without environment maps or restrictive assumptions.
...
TL;DR: UAUTrack is a unified single-target tracking framework that integrates multiple modalities for Anti-UAV tracking using text prior prompts to focus on UAVs across scenarios.


<details>
  <summary>Details</summary>
Motivation: Current Anti-UAV tracking lacks unified cross-modal collaboration frameworks, with existing approaches focusing on independent models and struggling with effective multimodal data fusion. The field is still in its infancy with limited information sharing between modalities.

Method: Proposes UAUTrack: a single-stream, single-stage, end-to-end architecture that integrates multiple modalities (RGB, TIR, RGB-T fusion). Introduces a text prior prompt strategy to direct model attention to UAVs across various scenarios.

Result: Achieves state-of-the-art performance on Anti-UAV and DUT Anti-UAV datasets. Maintains favorable accuracy-speed trade-off on Anti-UAV410 dataset, demonstrating both high accuracy and practical efficiency across diverse Anti-UAV scenarios.

Conclusion: UAUTrack provides an effective unified framework for cross-modal collaboration in Anti-UAV tracking, addressing current limitations in multimodal data fusion and achieving superior performance with practical efficiency.
...
TL;DR: PGP-DiffSR is a lightweight diffusion method for image super-resolution that reduces computational costs by pruning redundant blocks and using phase information guidance.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based super-resolution models rely on large backbones like SDXL and DiT, which have excessive computational and memory costs during training and inference.

Method: 1) Progressive pruning approach to remove intra-block redundancy while preserving restoration capability; 2) Phase-exchange adapter module that uses phase information of inputs to guide the pruned diffusion model for better restoration.

Result: Achieves competitive restoration quality while significantly reducing computational load and memory consumption compared to existing diffusion-based super-resolution methods.

Conclusion: PGP-DiffSR provides an efficient alternative to large diffusion models for image super-resolution by combining progressive pruning with phase-guided adaptation, making diffusion-based SR more practical for resource-constrained applications.
...
TL;DR: FASA is a two-stage foreground-aware slot attention method that explicitly separates foreground from background using dual-slot competition and pseudo-mask guidance to improve object discovery.


<details>
  <summary>Details</summary>
Motivation: Existing slot attention methods process foreground and background regions indiscriminately, leading to background interference and suboptimal instance discovery on real-world data. There's a need for explicit foreground-background separation for better object discovery.
...
Conclusion: FASA demonstrates that explicit foreground modeling with pseudo-mask guidance enables precise object discovery and object-coherent representation learning, addressing limitations of existing slot attention approaches.
...
TL;DR: ClimaDrive is a semantics-guided image-to-image framework that synthesizes weather-diverse, physically plausible out-of-distribution driving data to improve anomaly segmentation for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Anomaly segmentation is crucial for safe autonomous driving but suffers from limited anomaly data diversity. Existing synthetic data generation methods lack contextual coherence and physical realism, creating domain gaps between synthetic and real data.

Method: ClimaDrive unifies structure-guided multi-weather generation with prompt-driven anomaly inpainting to create semantically coherent and physically plausible OoD driving data. The framework produces ClimaOoD, a large-scale benchmark spanning six driving scenarios under clear and adverse weather conditions.
...
Conclusion: ClimaOoD enhances model robustness and offers valuable training data for better generalization in open-world anomaly detection, demonstrating the effectiveness of semantically coherent, weather-diverse synthetic data generation.
...
TL;DR: ALDI++ domain adaptation framework with ViTDet backbone achieves SOTA performance for object detection in security X-ray imagery across multiple domain adaptation scenarios.


<details>
  <summary>Details</summary>
Motivation: Domain adaptation is crucial for object detection in real-world applications where distribution shifts degrade model performance. Security X-ray imaging presents unique challenges due to variations in scanning devices and environmental conditions, causing significant domain discrepancies that need to be addressed.

Method: ALDI++ domain adaptation framework integrates self-distillation, feature alignment, and enhanced training strategies to mitigate domain shift. The approach uses Vision Transformer for Detection (ViTDet) as backbone architecture.

Result: ALDI++ surpasses state-of-the-art domain adaptation methods across multiple adaptation scenarios on the EDS dataset. With ViTDet backbone, it achieves the highest mean average precision (mAP). Category-wise analysis shows consistent improvements in detection accuracy across diverse object classes.
...
TL;DR: GeoBridge is a foundation model for cross-view geo-localization that uses semantic anchors to bridge multi-view features through textual descriptions, enabling bidirectional matching across drone, street, and satellite views with language-to-image retrieval support.


<details>
  <summary>Details</summary>
Motivation: Traditional satellite-centric geo-localization has limitations: it's fragile when high-resolution/up-to-date satellite imagery is unavailable, and it fails to exploit complementary cues across different views (drone, satellite, street) and modalities (language, image).

Method: Proposes GeoBridge with a novel semantic-anchor mechanism that bridges multi-view features through textual descriptions. Also introduces GeoLoc dataset - first large-scale cross-modal multi-view aligned dataset with 50,000+ pairs of drone, street-view panorama, and satellite images with textual descriptions from 36 countries.
...
Conclusion: GeoBridge addresses limitations of traditional satellite-centric geo-localization by leveraging multi-view and multi-modal data through semantic anchors, with the GeoLoc dataset enabling robust foundation model training for flexible localization tasks.
...
TL;DR: VLM-Pruner is a training-free token pruning algorithm for vision-language models that balances redundancy and spatial sparsity to reduce computational costs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Vision-language models have high computational costs due to many visual tokens, hindering mobile deployment. Existing pruning methods either ignore inter-token redundancy or spatial relationships, leading to inefficient pruning or poor object coverage.

Method: Proposes VLM-Pruner with: 1) centrifugal token pruning paradigm for near-to-far selection preserving object details, 2) Buffering for Spatial Sparsity (BSS) criterion to defer selection of distant tokens, 3) parallel greedy strategy for efficient selection, and 4) selective fusion of salient information from discarded tokens to retained ones.
...
Conclusion: VLM-Pruner effectively addresses limitations of existing pruning methods by balancing redundancy and spatial sparsity, enabling efficient deployment of VLMs on resource-constrained devices without training.
...
TL;DR: Sex-stratified whole-body MR image registration method using adipose tissue and muscle masks to augment intensity-based registration, achieving better performance than existing methods on UK Biobank data.


<details>
  <summary>Details</summary>
Motivation: To enable robust inter-subject registration of whole-body MR images from UK Biobank for body-wide spatial standardization and correlation analysis between non-imaging data and image-derived parameters.

Method: Sex-stratified approach using subcutaneous adipose tissue and muscle masks from VIBESegmentator to augment intensity-based graph-cut registration, compared against intensity-only method, uniGradICON, and MIRTK.

Result: Proposed method achieved mean Dice scores of 0.77/0.75 (males/females), outperforming intensity-only by 6pp, uniGradICON by 9pp/8pp, and MIRTK by 12pp/13pp. Age-correlation maps were less noisy and better aligned.

Conclusion: Using two tissue masks improves whole-body registration of UK Biobank MR images, enabling more accurate spatial standardization for medical research applications.
...
<details>
  <summary>Details</summary>
Motivation: Transferring multimodal LLM capabilities to remote sensing imagery is challenging due to extremely small targets in kilometer-scale scenes and complex geospatial relations involving relative positions, spatial hierarchies, and contextual dependencies across distant objects.
...
TL;DR: DF-Mamba: A deformable state scanning Mamba framework for 3D hand pose estimation that addresses occlusion challenges by capturing global context better than CNN-based methods.


<details>
  <summary>Details</summary>
Motivation: Current 3D hand pose estimation methods struggle with severe occlusions (like overlapping hands) because CNN-based feature extraction (e.g., ResNet) has limited capability to model global context needed to relate local occluded features to global cues.

Method: Proposes Deformable Mamba (DF-Mamba) using state space modeling (Mamba) with deformable state scanning. It aggregates local features after convolution while selectively preserving useful global context cues through selective state modeling and deformable scanning.

Result: DF-Mamba outperforms latest image backbones (VMamba, Spatial-Mamba) on all five divergent datasets covering single-hand/two-hand scenarios, hand-only/hand-object interactions, and RGB/depth-based estimation. Achieves state-of-the-art performance with comparable inference speed to ResNet-50.

Conclusion: DF-Mamba effectively addresses occlusion challenges in 3D hand pose estimation by capturing global context through Mamba's selective state modeling and deformable scanning, significantly improving accuracy while maintaining efficiency.
...
TL;DR: CAEVL enables UAV localization without paired UAV-satellite training data by learning from satellite images only, using domain shift simulation.


<details>
  <summary>Details</summary>
Motivation: Current UAV localization methods require expensive paired UAV-satellite datasets that are hard to acquire, limiting practical deployment in GNSS-denied environments.

Method: Uses satellite-only training with dedicated augmentation strategy to simulate domain shift between satellite and UAV views, introducing CAEVL model architecture.

Result: Achieves competitive performance to paired-data approaches on new ViLD dataset, demonstrating strong generalization without UAV training data.

Conclusion: Eliminating need for paired UAV-satellite datasets makes image-based localization more practical and accessible for real-world UAV applications.
...
TL;DR: RAMF framework improves hate speech detection in videos using multimodal fusion and adversarial reasoning, achieving 3-7% gains over SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Hate speech in online videos is a growing threat, but existing methods struggle with complex multimodal semantic relationships and nuanced hateful content understanding.

Method: Proposes Reasoning-Aware Multimodal Fusion (RAMF) with two components: 1) Local-Global Context Fusion (LGCF) and Semantic Cross Attention (SCA) for multimodal fusion, and 2) adversarial reasoning using a three-stage vision-language model process generating objective descriptions, hate-assumed inferences, and non-hate-assumed inferences.

Result: Achieves robust generalization performance on two real-world hateful video datasets, improving state-of-the-art methods by 3% in Macro-F1 and 7% in hate class recall.

Conclusion: RAMF effectively addresses multimodal fusion challenges and nuanced hate speech understanding through innovative fusion techniques and adversarial reasoning, demonstrating significant performance improvements.
...
TL;DR: AttMetNet is an attention-enhanced deep learning framework for methane plume detection using Sentinel-2 satellite imagery, combining NDMI with attention mechanisms to reduce false positives and improve detection accuracy.


<details>
  <summary>Details</summary>
Motivation: Methane is a powerful greenhouse gas contributing to global warming, and accurate detection is crucial for timely action. Current methods (traditional band differences/ratios and CNN-based approaches) suffer from high false positive rates and lack mechanisms to prioritize methane-specific features, requiring expert verification.

Method: AttMetNet introduces a methane-aware architecture that fuses the Normalized Difference Methane Index (NDMI) with an attention-enhanced U-Net. It uses attention mechanisms to selectively amplify methane absorption features while suppressing background noise, and employs focal loss to address severe class imbalance from limited positive samples and sparse plume pixels.

Result: Extensive experiments show AttMetNet surpasses recent methods in methane plume detection with lower false positive rate, better precision-recall balance, and higher IoU. The model is trained on real methane plume datasets, making it robust to practical scenarios.
...
TL;DR: STANet is the first smoke-type-aware laparoscopic video desmoking network that distinguishes between Diffusion Smoke and Ambient Smoke, using a coarse-to-fine disentanglement module and attention mechanisms to improve surgical visibility.


<details>
  <summary>Details</summary>
Motivation: Surgical smoke from electrocautery/lasers obstructs visual guidance in laparoscopic videos. Existing desmoking methods don't account for different smoke types with distinct motion patterns and spatio-temporal characteristics.

Method: Proposes STANet with two sub-networks: 1) smoke mask segmentation with attention-weighted mask aggregation and smoke type prediction, 2) smokeless video reconstruction guided by smoke masks. Includes coarse-to-fine disentanglement module with smoke-type-aware cross attention to separate entangled smoke types.

Result: Outperforms state-of-the-art approaches in quality evaluations and shows superior generalization across multiple downstream surgical tasks. Also created first large-scale synthetic video desmoking dataset with smoke type annotations.

Conclusion: STANet effectively addresses smoke-type-specific distinctions in laparoscopic videos, improving surgical visibility and demonstrating practical value for various surgical applications through better generalization.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-intrinsic generation methods lack structural consistency and physical coherence when generating multiple scene properties simultaneously. There's a need for a unified framework that can generate comprehensive intrinsic maps while maintaining physical relationships between different scene properties.

Method: Two key innovations: 1) Query-Broadcast Attention ensures structural consistency by sharing queries across all intrinsic maps in self-attention blocks. 2) Tensor LoRA enables parameter-efficient modeling of cross-map relations for efficient joint training. Together these enable stable joint diffusion training of multiple intrinsic properties.

Result: LumiX achieves 23% higher alignment and better preference score (0.19 vs -0.41) compared to state-of-the-art methods. It produces coherent, physically meaningful results and can also perform image-conditioned intrinsic decomposition within the same framework.

Conclusion: LumiX provides a structured diffusion framework for coherent text-to-intrinsic generation that maintains physical consistency across multiple scene properties through innovative attention and adaptation mechanisms, outperforming existing methods.
...
TL;DR: TrackNetV5 introduces Motion Direction Decoupling and Residual-Driven Spatio-Temporal Refinement to overcome occlusion and directional ambiguity limitations in previous versions, achieving state-of-the-art performance with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Previous TrackNet versions (V1-V3) struggle with occlusions due to reliance on purely visual cues, while TrackNetV4 suffers from directional ambiguity because its absolute difference method discards motion polarity. These limitations hinder robust tracking of fast-moving small objects in sports.

Method: Two novel mechanisms: 1) Motion Direction Decoupling (MDD) module decomposes temporal dynamics into signed polarity fields to explicitly encode both movement occurrence and trajectory direction, recovering lost directional priors. 2) Residual-Driven Spatio-Temporal Refinement (R-STR) head uses a Transformer-based coarse-to-fine approach with factorized spatio-temporal contexts to estimate corrective residuals for recovering occluded targets.
...
Conclusion: TrackNetV5 successfully addresses key limitations of previous versions through innovative directional encoding and occlusion handling mechanisms, establishing a robust architecture for fast-moving small object tracking in sports with superior precision and computational efficiency.
...
Method: 1) A lightweight data pipeline replacing multi-toolchains with an end-to-end model and unified post-verification; 2) Training a 7B dual-task expert model (Qwen-Verify) for efficient failure detection and instruction recaptioning; 3) Creating UnicEdit-10M dataset spanning diverse editing tasks; 4) Proposing UnicBench benchmark with novel metrics including Non-edit Consistency and Reasoning Accuracy.

Result: The pipeline yields UnicEdit-10M, a 10M-scale dataset covering diverse basic and complex editing tasks. UnicBench extends beyond basic edits to assess spatial and knowledge-driven reasoning, enabling fine-grained diagnosis of model limitations.

Conclusion: The analysis of mainstream models on UnicBench reveals their limitations and provides clear directions for future research, addressing the critical data and evaluation gaps in multimodal image editing.
...
<details>
  <summary>Details</summary>
Motivation: Previous CVR approaches overlook the disparity in information density between video (rich semantics) and text (sparser) modalities, leading to modification subject referring ambiguity and limited detailed semantic focus, which degrade retrieval performance.

Method: HUD leverages modality density disparity with three components: Holistic Pronoun Disambiguation for cross-modal interaction, Atomistic Uncertainty Modeling for fine-grained semantics, and Holistic-to-Atomistic Alignment for precise composed feature learning.
...
Conclusion: By addressing modality density disparity through hierarchical uncertainty-aware disambiguation, HUD enables effective object disambiguation and detailed semantic focus, advancing multi-modal query understanding in composed retrieval tasks.
...
TL;DR: IC-World is a novel framework for shared world modeling that generates multiple videos from input images showing the same world from different camera poses, using in-context generation with large video models and reinforcement learning for consistency.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of shared world modeling, where a model needs to generate multiple videos from input images that represent the same underlying world but from different camera perspectives. This requires maintaining consistency across generated videos in terms of both scene geometry and object motion.

Method: IC-World leverages the inherent in-context generation capability of large video models for parallel generation from input images. The framework is further fine-tuned using reinforcement learning (Group Relative Policy Optimization) with two novel reward models: one for scene-level geometry consistency and another for object-level motion consistency across generated videos.

Result: Extensive experiments show that IC-World substantially outperforms state-of-the-art methods in both geometry and motion consistency metrics for shared world modeling tasks.

Conclusion: IC-World represents the first systematic exploration of shared world modeling with video-based world models, demonstrating superior performance in maintaining consistency across multiple generated videos from different camera perspectives.
...
TL;DR: PhyCustom: A fine-tuning framework with novel regularization losses to enable diffusion models to accurately customize physical concepts, addressing limitations of current methods that fail to reflect physical properties in generated images.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based text-to-image customization methods excel at concrete concepts (styles, shapes) but fail at physical concepts due to lack of explicit physical knowledge during training. Even with physics-related prompts, existing methods cannot accurately reflect physical properties in generated results.

Method: PhyCustom framework with two novel regularization losses: 1) isometric loss to activate diffusion models to learn physical concepts, and 2) decouple loss to eliminate mixture learning of independent concepts.

Result: Outperforms previous state-of-the-art and popular methods in physical customization both quantitatively and qualitatively on diverse datasets.

Conclusion: PhyCustom successfully enables diffusion models to perform accurate physical customization by explicitly incorporating physical knowledge through specialized regularization losses.
...
TL;DR: Adversarial training unexpectedly increases transferability of adversarial attacks, creating a paradox where robust models produce more transferable adversarial examples.


<details>
  <summary>Details</summary>
Motivation: While adversarial training improves model robustness against attacks, its effect on attack transferability remains underexplored. The paper investigates whether adversarial training unintentionally makes adversarial examples more transferable between models.

Method: Trained a diverse zoo of 36 models (CNNs and ViTs), conducted comprehensive transferability experiments to compare adversarial examples from adversarially trained vs standard models.

Result: Adversarially trained models produce perturbations that transfer more effectively than those from standard models, revealing a paradox and introducing new ecosystem risks.

Conclusion: Robustness evaluations should assess both model resistance to transferred attacks AND the model's propensity to produce transferable adversarial examples. All models, code, and scripts are released for reproducibility.
...
TL;DR: ReVSeg introduces explicit multi-step reasoning for video object segmentation using pretrained VLMs, decomposing the task into semantic interpretation, temporal evidence selection, and spatial grounding, optimized with reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Existing video object segmentation methods collapse complex reasoning about dynamics, causality, and temporal interactions into simplified latent embeddings, making the reasoning chain opaque and intractable. There's a need for more explicit and interpretable reasoning approaches.

Method: ReVSeg decomposes reasoning into three explicit sequential operations: semantics interpretation, temporal evidence selection, and spatial grounding, using pretrained vision language models. It employs reinforcement learning to optimize the multi-step reasoning chain, allowing self-refinement from outcome-driven signals.

Result: ReVSeg achieves state-of-the-art performance on standard video object segmentation benchmarks and produces interpretable reasoning trajectories, demonstrating the effectiveness of explicit decomposition and reinforcement learning optimization.

Conclusion: Explicit decomposition of reasoning into sequential operations, combined with reinforcement learning optimization, enables more interpretable and effective video object segmentation, outperforming existing methods that collapse complex reasoning into opaque latent embeddings.
...
TL;DR: AAG is a method for action anticipation using only a single frame, combining RGB features with depth cues and prior action context, achieving competitive performance against video-based methods on instructional datasets.


<details>
  <summary>Details</summary>
Motivation: Humans can anticipate actions from single moments with sufficient context, but conventional methods rely on temporal video aggregation. The paper investigates whether models can achieve similar competence by replacing video aggregation with alternative modalities.

Method: AAG (Action Anticipation at a Glimpse) combines RGB features with depth cues from a single frame for spatial reasoning, and incorporates prior action context obtained either from textual summaries generated by Vision-Language Models or from predictions by a single-frame action recognizer.
...
Conclusion: Multimodal single-frame action anticipation can effectively replace video aggregation for action prediction tasks, demonstrating that models can achieve human-like competence in anticipating actions from glimpses when provided with sufficient contextual information.
...
TL;DR: First systematic study of identity-preserving AI-generated content (IP-AIGC) detection for Indian/South-Asian faces, showing fine-tuned detectors overfit to training generators and fail to generalize to held-out IP-AIGC despite strong in-domain performance.


<details>
  <summary>Details</summary>
Motivation: Modern image editors create identity-preserving AIGC where the same person appears with new attributes, but detector robustness and fairness for under-represented populations like Indian/South-Asian faces remains unclear.

Method: Assembled Indian-focused training splits from FairFD and HAV-DF datasets, created two held-out IP-AIGC test sets using commercial generators (Gemini, ChatGPT) with identity-preserving prompts. Evaluated SOTA detectors (AIDE, Effort) under pretrained and fine-tuned regimes using AUC, AP, EER, and accuracy metrics.

Result: Fine-tuning yields strong in-domain gains but consistently degrades performance on held-out IP-AIGC for Indian cohorts (e.g., AIDE AUC drops from 0.923 to 0.563), indicating overfitting to training-generator cues. Pretrained performance remains high on non-IP images, suggesting specific brittleness to identity-preserving edits.

Conclusion: IP-AIGC-Indian is a challenging, practically relevant scenario that motivates representation-preserving adaptation and India-aware benchmark curation to address generalization gaps in AIGC detection.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of face-voice association in multilingual scenarios, specifically for English-German pairs in the FAME 2026 challenge.
...
Conclusion: The proposed method performs favorably in multilingual face-voice association, demonstrating effectiveness in cross-modal semantic information extraction.
...
TL;DR: STSR 2025 Challenge benchmarked semi-supervised learning for CBCT/IOS dental imaging, with tasks for segmentation and registration using limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Annotated data scarcity in digital dentistry (CBCT and IOS) limits automated solutions for pulp canal segmentation and cross-modal registration, necessitating semi-supervised learning approaches.

Method: Organized challenge with two tasks: 1) semi-supervised segmentation of teeth/pulp canals in CBCT, 2) semi-supervised rigid registration of CBCT and IOS. Provided 60 labeled/640 unlabeled IOS samples and 30 labeled/250 unlabeled CBCT scans with varying resolutions/FOVs.

Result: Top teams achieved Dice score 0.967 and Instance Affinity 0.738 for segmentation using nnU-Net/Mamba-like models with pseudo-labeling/consistency. Registration used PointNetLK with differentiable SVD and geometric augmentation, with hybrid neural-classical refinement for accurate alignment.

Conclusion: The challenge demonstrated effective semi-supervised learning solutions for dental imaging tasks, with all data and code publicly available to ensure reproducibility and advance the field.
...
TL;DR: Online RL post-training framework for video diffusion models that improves camera control accuracy using geometry-based rewards and segment-level feedback.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models rely mainly on supervised fine-tuning for camera control, leaving reinforcement learning post-training underexplored despite its potential for better optimization.

Method: Designs verifiable geometry reward that estimates 3D camera trajectories, divides them into segments, computes segment-wise relative poses, and compares generated-reference pairs for dense feedback. Also constructs dataset with diverse camera motions and subject dynamics.

Result: Outperforms SFT baselines across multiple aspects including camera-control accuracy, geometric consistency, and visual quality, demonstrating superiority in camera-controlled video generation.

Conclusion: Online RL post-training with geometry-based rewards effectively improves camera control in video generation, addressing reward sparsity and optimization efficiency challenges.
...
TL;DR: MindGPT-4ov is a multimodal LLM with a novel post-training framework featuring automated data generation, curriculum fine-tuning, and hybrid reinforcement learning, achieving SOTA performance at low cost.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient post-training paradigm for MLLMs that enhances foundational capabilities and generalization while reducing costs, enabling seamless transition from research to industrial deployment.
...
Result: Outperforms state-of-the-art models on benchmarks including MMBench, MMStar, MathVision, and MathVista, with superior performance in vertical domain tasks and efficient deployment capabilities.

Conclusion: MindGPT-4ov provides a general post-training paradigm applicable to various MLLMs, demonstrating strong performance and deployment efficiency, with plans to open-source model weights, datasets, and code for community development.
...
TL;DR: Systematic study of how LiDAR-to-image projection methods affect place recognition performance when combined with vision foundation models, identifying optimal projection characteristics for real-world autonomy.


<details>
  <summary>Details</summary>
Motivation: To understand how different 2D projection methods from 3D LiDAR data influence metric place recognition performance, particularly when using vision foundation models, and to determine which projection characteristics work best for practical autonomous systems.

Method: Developed a modular retrieval pipeline that isolates the effect of 2D projections by controlling for backbone models, aggregation methods, and evaluation protocols. Used consistent geometric and structural channels across multiple datasets and deployment scenarios to analyze projection characteristics.

Result: Identified specific projection characteristics that most strongly determine discriminative power, robustness to environmental variation, and suitability for real-time autonomy. Experiments across different datasets and integration into operational place recognition policies validated that well-designed projections can effectively substitute for end-to-end 3D learning.

Conclusion: Carefully designed LiDAR-to-image projections can serve as effective surrogates for end-to-end 3D learning in LiDAR place recognition, with specific projection characteristics being crucial for achieving optimal performance in real-world autonomous systems.
...
TL;DR: Phase-aware acceleration of diffusion models using specialized LoRA adapters for different denoising phases, achieving 5× speedup with minimal training cost.


<details>
  <summary>Details</summary>
Motivation: Diffusion models have high computational costs and require many inference steps. Existing distillation methods suffer from heavy retraining costs and degraded generalization.

Method: Propose phase-aware acceleration with two specialized LoRA adapters: Slow-LoRA for early semantic stages (smaller speedups) and Fast-LoRA for later redundant phases (larger speedups). Lightweight adapters are added to the base model instead of retraining student models.
...
Conclusion: Phase-aware acceleration with specialized LoRA adapters provides efficient speedup with minimal training cost and strong generalization, offering a practical solution for deploying diffusion models.
...
TL;DR: MRD is a training-free framework that improves high-resolution image understanding in MLLMs by using multi-resolution semantic fusion to preserve object integrity and open-vocabulary detection for global object localization.


<details>
  <summary>Details</summary>
Motivation: Current crop-based approaches for high-resolution image understanding in MLLMs fragment objects across multiple crops, disrupting semantic similarity computation and failing to preserve object integrity.

Method: Proposes Multi-resolution Retrieval-Detection (MRD) with: 1) multi-resolution semantic fusion that integrates similarity maps from different resolutions to preserve object integrity, and 2) open-vocabulary object detection using sliding-window approach for global object localization.
...
Conclusion: MRD addresses fragmentation issues in crop-based high-resolution image understanding by preserving object integrity through multi-resolution fusion and enabling direct global localization through OVD.
...
TL;DR: DiverseAR addresses sample diversity limitations in bitwise autoregressive image generation by introducing adaptive logits scaling and energy-based path search to enhance diversity while maintaining visual quality.
...
Method: DiverseAR introduces two key components: (1) an adaptive logits distribution scaling mechanism that dynamically adjusts binary output distribution sharpness during sampling for smoother predictions, and (2) an energy-based generation path search algorithm that avoids sampling low-confidence tokens to preserve visual quality.
...
Conclusion: The proposed DiverseAR method effectively addresses diversity limitations in bitwise AR models through principled distribution scaling and quality-preserving path search, offering a balanced solution for diverse yet high-quality image generation.
...
TL;DR: EGGS proposes a hybrid 2D/3D Gaussian splatting method that balances appearance fidelity and geometric consistency for novel view synthesis.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting provides high appearance fidelity but suffers from multi-view inconsistencies, while 2D Gaussian Splatting ensures multi-view consistency but loses texture details. There's a need to balance both appearance and geometry quality.

Method: Exchangeable Gaussian Splatting (EGGS) integrates 2D and 3D Gaussians with three key components: Hybrid Gaussian Rasterization for unified rendering, Adaptive Type Exchange for dynamic adaptation between 2D/3D Gaussians, and Frequency-Decoupled Optimization to exploit strengths of each representation type.

Result: EGGS outperforms existing methods in rendering quality, geometric accuracy, and efficiency, with CUDA-accelerated implementation ensuring practical training and inference.

Conclusion: EGGS provides a practical solution for high-quality novel view synthesis by effectively balancing appearance fidelity and geometric consistency through hybrid 2D/3D Gaussian representation.
...
TL;DR: LoVoRA is a mask-free video editing framework for object removal and addition using object-aware localization, eliminating need for external masks or reference images.
...
Method: Uses dataset construction pipeline with image-to-video translation, optical flow-based mask propagation, and video inpainting. Core innovation is learnable object-aware localization mechanism with Diffusion Mask Predictor for end-to-end editing without external control signals.

Result: Extensive experiments and human evaluation demonstrate effective and high-quality performance for both object removal and addition tasks with temporal consistency.
...
Method: Created VideoScience-Bench with 200 prompts spanning 14 topics and 103 concepts in physics/chemistry, each encoding composite scientific scenarios requiring multi-concept reasoning. Evaluated 7 SOTA video models using expert annotations across 5 dimensions and validated VLM-as-a-Judge against human assessments.

Result: Strong correlation found between VLM-as-a-Judge assessments and human expert evaluations. The benchmark reveals current video models' limitations in scientific reasoning and provides first systematic evaluation of video models as scientific reasoners.

Conclusion: VideoScience-Bench is the first benchmark to evaluate video models as scientific reasoners, not just generators, requiring demonstrations of scientific understanding consistent with expected physical/chemical phenomena. The benchmark and evaluation code are publicly available.
...
TL;DR: Layout Anything: A transformer-based framework for indoor layout estimation that adapts OneFormer's universal segmentation to geometric structure prediction, achieving state-of-the-art performance with fast inference.


<details>
  <summary>Details</summary>
Motivation: To create an efficient indoor layout estimation framework that eliminates complex post-processing while maintaining geometric consistency, suitable for AR applications and large-scale 3D scene reconstruction.

Method: Adapts OneFormer's universal segmentation architecture with task-conditioned queries and contrastive learning, plus two key modules: (1) layout degeneration strategy for data augmentation preserving Manhattan-world constraints, and (2) differentiable geometric losses for planar consistency and sharp boundaries.

Result: Achieves state-of-the-art performance: 5.43% PE / 4.02% CE on LSUN, 7.04% PE / 5.17% CE on Hedau, and 4.03% PE / 3.15% CE on Matterport3D-Layout, with fast 114ms inference time.

Conclusion: The framework successfully combines geometric awareness with computational efficiency, eliminating post-processing while achieving top performance, making it particularly suitable for AR and large-scale 3D reconstruction applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing low-light image enhancement algorithms are too computationally intensive for real-time vehicular applications like nighttime driving, requiring lightweight solutions that can operate with limited resources.

Method: Proposes UltraFast-LieNET with Dynamic Shifted Convolution (DSConv) kernels (12 parameters each), Multi-scale Shifted Residual Blocks (MSRB) for expanded receptive field, residual structures, and multi-level gradient-aware loss function for stability.

Result: Achieves 26.51 dB PSNR on LOLI-Street dataset, outperforming state-of-the-art by 4.6 dB with only 180 parameters. Minimum configuration uses only 36 parameters while maintaining real-time performance across four benchmark datasets.

Conclusion: UltraFast-LieNET provides an effective lightweight solution for real-time low-light image enhancement in resource-constrained environments like vehicle cameras, balancing performance and computational efficiency.
...
TL;DR: BEVDilation is a LiDAR-centric fusion framework that uses image BEV features as implicit guidance rather than naive concatenation to address spatial misalignment from depth errors and enhance LiDAR features for 3D object detection.


<details>
  <summary>Details</summary>
Motivation: Previous LiDAR-camera fusion methods suffer from degraded performance due to indiscriminate fusion of geometrically disparate sensors. The fundamental disparity in geometric accuracy between LiDAR and cameras causes spatial misalignment from image depth estimation errors.

Method: Proposes BEVDilation with two key blocks: 1) Sparse Voxel Dilation Block that densifies foreground voxels using image priors to mitigate point cloud sparsity, and 2) Semantic-Guided BEV Dilation Block that enhances LiDAR feature diffusion with image semantic guidance and long-range context capture. Formulates image BEV features as implicit guidance rather than naive concatenation.
...
Conclusion: BEVDilation's LiDAR-centric strategy with image guidance effectively addresses spatial misalignment, point cloud sparsity, and semantic limitations, leading to superior 3D object detection performance with improved robustness to depth errors.
...
TL;DR: CIA is a new image-centric jailbreak attack method that embeds harmful queries into benign visual contexts using multi-agent systems and visualization strategies, outperforming previous text-image interplay attacks on MLLMs.


<details>
  <summary>Details</summary>
Motivation: Existing jailbreak attacks on Multimodal Large Language Models (MLLMs) focus primarily on text-image interplay and treat visual modality as secondary prompts, underutilizing images' potential to carry complex contextual information for attacks.

Method: Proposes Contextual Image Attack (CIA) using a multi-agent system to embed harmful queries into seemingly benign visual contexts through four distinct visualization strategies, enhanced with contextual element enhancement and automatic toxicity obfuscation techniques.

Result: On MMSafetyBench-tiny dataset, CIA achieves toxicity scores of 4.73 and 4.83 against GPT-4o and Qwen2.5-VL-72B models, with Attack Success Rates of 86.31% and 91.07%, significantly outperforming prior work.

Conclusion: The visual modality itself is a potent vector for jailbreaking advanced MLLMs, demonstrating that image-centric attacks can be highly effective when leveraging complex visual contexts and sophisticated embedding strategies.
...
TL;DR: InEx is a training-free multi-agent framework that autonomously mitigates hallucination in multimodal LLMs through internal introspective reasoning and external cross-modal verification.


<details>
  <summary>Details</summary>
Motivation: Hallucination remains a critical challenge in LLMs, hindering reliable MLLM development. Existing solutions rely too much on human intervention and underutilize agents' autonomous mitigation abilities. The authors draw inspiration from human cognitive processes where people use introspective reasoning followed by external verification from diverse perspectives.

Method: InEx uses a multi-agent framework with: 1) Internal introspective reasoning guided by entropy-based uncertainty estimation to improve decision agent reliability, 2) External cross-modal multi-agent collaboration where the agent's response is iteratively verified and refined through editing and self-reflection agents, 3) Training-free approach that leverages existing model capabilities.
...
Conclusion: InEx effectively addresses hallucination in MLLMs by mimicking human cognitive processes through autonomous introspective reasoning and external verification, providing a training-free solution that significantly improves reliability.
...
Result: Extensive experiments show U4D produces geometrically faithful and temporally consistent LiDAR sequences, advancing the reliability of 4D world modeling for autonomous perception and simulation.

Conclusion: U4D presents an effective uncertainty-aware framework that addresses spatial uncertainty variations in 4D LiDAR world modeling, improving geometric fidelity and temporal stability for more reliable autonomous systems.
...
TL;DR: GraphFusion3D: A unified 3D object detection framework using adaptive cross-modal fusion and graph reasoning to address sparse point cloud challenges.


<details>
  <summary>Details</summary>
Motivation: Point clouds present challenges for 3D object detection due to sparse data, incomplete structures, limited semantic information, and difficulty capturing contextual relationships between distant objects.

Method: Proposes GraphFusion3D with Adaptive Cross-Modal Transformer (ACMT) for image-point feature integration, Graph Reasoning Module (GRM) for neighborhood relationship modeling using multi-scale graph attention, and cascade decoder for progressive refinement.

Result: Achieves 70.6% AP25 and 51.2% AP50 on SUN RGB-D, and 75.1% AP25 and 60.8% AP50 on ScanNetV2, demonstrating substantial improvement over existing approaches.

Conclusion: GraphFusion3D effectively addresses point cloud challenges through adaptive multi-modal fusion and graph-based reasoning, achieving state-of-the-art performance on major 3D detection benchmarks.
...
TL;DR: TEXTRIX is a native 3D attribute generation framework that directly colors 3D models in volumetric space using a latent 3D attribute grid and Diffusion Transformer with sparse attention, avoiding multi-view fusion limitations and enabling both high-fidelity texture synthesis and precise 3D part segmentation.


<details>
  <summary>Details</summary>
Motivation: Current 3D texture generation methods rely on multi-view fusion, which suffers from inter-view inconsistencies and incomplete coverage of complex surfaces, limiting fidelity and completeness of generated content.
...
Conclusion: TEXTRIX provides a native 3D approach that overcomes limitations of multi-view fusion methods, enabling high-fidelity texture generation and precise 3D segmentation through a unified framework.
...
TL;DR: DynamicVerse is a physical-scale multimodal 4D world modeling framework that creates comprehensive 4D representations from real-world videos, addressing limitations of existing datasets by providing metric-scale geometry, real-world motion, instance masks, and descriptive captions.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for understanding dynamic physical worlds are limited - they come from constrained simulators or use traditional Structure-from-Motion with limited descriptive captioning. This restricts foundation models' ability to accurately interpret real-world dynamics from monocular internet videos, which is crucial for human-agent interaction and embodied AI.

Method: The framework employs large vision, geometric, and multimodal models to interpret metric-scale static geometry, real-world dynamic motion, instance-level masks, and holistic descriptive captions. It integrates window-based Bundle Adjustment with global optimization to convert long real-world video sequences into comprehensive 4D multimodal format.

Result: Created a large-scale dataset with 100K+ videos, 800K+ annotated masks, and 10M+ frames from internet videos. Experimental evaluations on video depth estimation, camera pose estimation, and camera intrinsics estimation demonstrate superior performance in capturing physical-scale measurements with greater global accuracy than existing methods.

Conclusion: DynamicVerse bridges the gap in existing datasets by providing physical-scale, multimodal 4D world modeling for dynamic real-world videos, enabling better interpretation of real-world dynamics and supporting foundation models for embodied AI applications.
...
TL;DR: DGGT is a pose-free dynamic scene reconstruction framework that jointly predicts 3D Gaussian maps and camera parameters from sparse, unposed images, enabling fast 4D reconstruction for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Current methods for dynamic driving scene reconstruction rely on per-scene optimization, known camera calibration, or short frame windows, making them slow and impractical for scalable autonomous driving training and evaluation.

Method: A unified framework that reformulates camera pose as model output rather than input, jointly predicts per-frame 3D Gaussian maps and camera parameters, uses lightweight dynamic head for disentanglement, lifespan head for temporal consistency, and diffusion-based rendering refinement.

Result: Achieves state-of-the-art performance and speed on large-scale driving benchmarks (Waymo, nuScenes, Argoverse2), outperforms prior work in both dataset-specific training and zero-shot transfer, and scales well with increasing input frames.

Conclusion: DGGT provides a practical, scalable solution for 4D reconstruction in autonomous driving by eliminating pose dependency and enabling fast, single-pass reconstruction from sparse, unposed images.
...
TL;DR: SurfFill combines LiDAR and photogrammetry using Gaussian surfels to complete LiDAR scans by identifying ambiguous areas and growing points from them.


<details>
  <summary>Details</summary>
Motivation: LiDAR excels in flat regions but misses small structures and dark materials, while photogrammetry captures details but lacks LiDAR's accuracy in featureless areas. Combining both can leverage their complementary strengths.

Method: Uses Gaussian surfel-based completion with ambiguity heuristic from density changes to identify missed areas, then constrains Gaussian surfel reconstruction to focus optimization on ambiguous regions, extracting and sampling primitives to complete point clouds. Includes divide-and-conquer for large-scale scenes.

Result: Outperforms previous reconstruction methods on both synthetic and real-world LiDAR point cloud completion tasks.

Conclusion: SurfFill successfully combines LiDAR and camera-based capture strengths, addressing LiDAR limitations through Gaussian surfel completion with ambiguity detection, enabling more complete 3D reconstructions.
...
TL;DR: Sync-LoRA is a method for editing portrait videos that maintains frame-accurate synchronization while allowing diverse visual modifications like appearance changes, expression edits, and object additions.


<details>
  <summary>Details</summary>
Motivation: Editing portrait videos requires flexible control over modifications while preserving the subject's original temporal behavior and maintaining precise frame synchronization, which is challenging with existing methods.

Method: Uses an image-to-video diffusion model where edits start from the first frame and propagate through the sequence. Trains an in-context LoRA using automatically generated paired videos with identical motion trajectories but different appearances, filtered through synchronization-based selection to ensure temporal alignment.

Result: Achieves high visual fidelity and strong temporal coherence, generalizing to unseen identities and diverse edits (appearance modifications, object additions, background changes) while robustly handling pose and expression variations.

Conclusion: Sync-LoRA successfully balances edit fidelity with precise motion preservation, enabling high-quality portrait video editing with frame-accurate synchronization and identity consistency.
...
TL;DR: A method to adapt frame-based neural networks for stable, robust video inference using stability adapters and a unified accuracy-stability-robustness loss.


<details>
  <summary>Details</summary>
Motivation: Frame-based networks applied to video often suffer from temporal inconsistency (flickering), especially when inputs contain time-varying corruptions. There's a need for general adaptation methods that can stabilize existing models without full retraining.

Method: Introduces stability adapters that can be inserted into virtually any architecture, with resource-efficient training using frozen base networks. Proposes a unified conceptual framework centered on an accuracy-stability-robustness loss, with theoretical analysis of its properties.

Result: Method improves temporal stability and robustness against various image corruptions (compression artifacts, noise, adverse weather) while preserving or improving prediction quality. Validated on multiple vision tasks including denoising, image enhancement, depth estimation, and semantic segmentation.

Conclusion: The approach provides a general, effective solution for adapting frame-based models to achieve stable and robust video inference without requiring full model retraining, addressing both temporal consistency and corruption robustness simultaneously.
...
TL;DR: AutoBrep is a Transformer model that autoregressively generates high-quality, watertight B-Rep CAD models using a unified tokenization scheme for geometry and topology.


<details>
  <summary>Details</summary>
Motivation: Directly generating B-Reps end-to-end with precise geometry and watertight topology remains challenging despite recent progress in CAD modeling.

Method: Uses unified tokenization encoding geometric primitives as latent geometry tokens and structural relationships as topological reference tokens. Sequence follows B-Rep face adjacency graph breadth-first traversal. Transformer model generates neighboring faces/edges progressively.

Result: Outperforms baselines with better quality and watertightness, scalable to complex solids with good fidelity and inference speed. Supports autocompletion for user-controllable CAD generation.

Conclusion: AutoBrep demonstrates effective B-Rep generation through unified representation and next-token prediction, enabling high-quality, valid CAD model generation with autocompletion capabilities.
...
TL;DR: FLAT connects unrolled networks to flow ODEs for MRI reconstruction, improving stability and convergence with fewer iterations than diffusion models.


<details>
  <summary>Details</summary>
Motivation: MRI acquisition is slow, and existing deep learning methods have trade-offs: unrolled networks are efficient but unstable due to freely-learnable parameters, while diffusion models are stable but computationally expensive.

Method: Theoretical proof that unrolled networks are discrete implementations of conditional probability flow ODEs, leading to Flow-Aligned Training (FLAT) that derives unrolled parameters from ODE discretization and aligns intermediate reconstructions with ideal ODE trajectory.

Result: FLAT achieves high-quality MRI reconstructions with up to 3× fewer iterations than diffusion-based models and significantly greater stability than unrolled networks on three MRI datasets.

Conclusion: Connecting unrolled networks to flow ODEs provides theoretical grounding and practical improvements for accelerated MRI reconstruction, offering both stability and computational efficiency.
...
TL;DR: MAViD is a multimodal framework for audio-visual dialogue that uses a Conductor-Creator architecture to generate synchronized long-duration conversational content with consistent identity and quality.


<details>
  <summary>Details</summary>
Motivation: Existing audio-visual dialogue systems are limited to non-interactive approaches that produce constrained and unnatural human speech, lacking effective multimodal fusion and long-duration generation capabilities.

Method: Proposes a Conductor-Creator architecture: Conductor handles understanding, reasoning, and generating motion/speech instructions; Creator uses AR-diffusion hybrid for audio-video generation with novel fusion module for clip continuity.

Result: The framework successfully generates vivid, contextually coherent long-duration dialogue interactions and accurately interprets multimodal queries, demonstrating superior performance in experiments.

Conclusion: MAViD effectively addresses multimodal audio-visual dialogue challenges through its dual-component architecture and hybrid generation approach, enabling natural, synchronized long-duration conversational content.
...
TL;DR: ViSAudio introduces end-to-end binaural spatial audio generation from silent video, overcoming limitations of two-stage approaches through conditional flow matching with dual-branch architecture.


<details>
  <summary>Details</summary>
Motivation: Current video-to-audio generation focuses on mono output, lacking spatial immersion. Existing binaural approaches use two-stage pipelines (mono generation + spatialization) that cause error accumulation and spatio-temporal inconsistencies.

Method: Proposes ViSAudio framework with conditional flow matching and dual-branch audio generation architecture. Two dedicated branches model audio latent flows, integrated with conditional spacetime module to balance channel consistency while preserving spatial characteristics. Also introduces BiAudio dataset with ~97K video-binaural audio pairs.

Result: ViSAudio outperforms state-of-the-art methods in both objective metrics and subjective evaluations, generating high-quality binaural audio with spatial immersion that adapts to viewpoint changes, sound-source motion, and diverse acoustic environments.

Conclusion: The work successfully demonstrates end-to-end binaural spatial audio generation from silent video, providing a more immersive audio experience with better spatio-temporal alignment compared to existing approaches.
...
TL;DR: Video4Spatial framework shows video diffusion models can perform complex spatial tasks (navigation and object grounding) using only visual data, demonstrating visuospatial intelligence without auxiliary modalities.


<details>
  <summary>Details</summary>
Motivation: To investigate whether video generative models can exhibit visuospatial intelligence - a core human cognitive capability - using only visual data, without relying on auxiliary modalities like depth or poses.
...
Result: The model successfully performs scene navigation (following camera-pose instructions while maintaining 3D geometric consistency) and object grounding (semantic localization, instruction following, and planning). It demonstrates strong spatial understanding, maintains spatial consistency, and generalizes to long contexts and out-of-domain environments.

Conclusion: Video generative models can exhibit visuospatial reasoning capabilities using only visual data, advancing the field toward general visuospatial intelligence without requiring auxiliary modalities.
...
TL;DR: MultiShotMaster is a framework for generating narrative multi-shot videos with flexible shot arrangement, coherent narrative, and enhanced controllability beyond text prompts.


<details>
  <summary>Details</summary>
Motivation: Current video generation techniques excel at single-shot clips but struggle with narrative multi-shot videos that require flexible shot arrangement, coherent narrative, and better controllability beyond simple text prompts.

Method: Extends pretrained single-shot model with two novel RoPE variants: Multi-Shot Narrative RoPE for explicit phase shift at shot transitions, and Spatiotemporal Position-Aware RoPE for reference token and grounding signal incorporation. Also includes automated data annotation pipeline for multi-shot video data extraction.

Result: The framework supports multi-shot video generation with text-driven inter-shot consistency, customized subject with motion control, and background-driven customized scenes. Both shot count and duration are flexibly configurable, with extensive experiments demonstrating superior performance and outstanding controllability.

Conclusion: MultiShotMaster successfully addresses the challenges of multi-shot video generation by enabling flexible shot arrangement, coherent narrative flow, and enhanced controllability through novel architectural extensions and automated data collection.
...
TL;DR: PPTArena is a benchmark for PowerPoint editing that tests reliable slide modifications via natural language instructions, with PPTPilot as a structure-aware editing agent that outperforms existing systems.


<details>
  <summary>Details</summary>
Motivation: Current PowerPoint editing benchmarks use image-PDF renderings or text-to-slide generation, but lack focus on in-place editing of real slides with natural language instructions. There's a need for reliable editing across complex slide elements like text, charts, tables, animations, and styles.

Method: PPTArena: 100 decks, 2125 slides, 800+ targeted edits with ground-truth decks and dual VLM-as-judge pipeline scoring instruction following and visual quality. PPTPilot: structure-aware agent that plans semantic edit sequences, routes between programmatic tools and XML operations, and uses iterative plan-edit-check loops.

Result: PPTPilot outperforms proprietary agents and frontier VLM systems by over 10 percentage points on compound, layout-sensitive, and cross-slide edits, with significant gains in visual fidelity and deck-wide consistency.

Conclusion: PPTPilot demonstrates strong performance on PowerPoint editing tasks, but existing agents still struggle with long-horizon, document-scale editing, highlighting remaining challenges in reliable PPT editing despite improvements.
...
TL;DR: OneThinker is a unified multimodal reasoning model that combines image and video understanding across 10 fundamental visual tasks using a multi-task RL approach with EMA-GRPO to handle reward heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches train separate models for different tasks and treat image/video reasoning as disjoint domains, limiting scalability toward a multimodal reasoning generalist and hindering knowledge sharing across tasks and modalities.

Method: Constructed OneThinker-600k training corpus covering diverse visual tasks, used commercial models for CoT annotation to create OneThinker-SFT-340k for SFT cold start, and proposed EMA-GRPO (Exponential Moving Average - Generalized Reward Policy Optimization) to handle reward heterogeneity in multi-task RL by tracking task-wise moving averages of reward standard deviations.
...
Conclusion: OneThinker marks a step toward a unified multimodal reasoning generalist by unifying image and video understanding across diverse visual tasks, with all code, model, and data released.
...
TL;DR: CAMEO is a training technique that supervises attention maps in multi-view diffusion models using geometric correspondence to improve view consistency and training efficiency.


<details>
  <summary>Details</summary>
Motivation: Multi-view diffusion models show view consistency but the underlying mechanism is unclear. The authors discovered that attention maps acquire geometric correspondence during training, but this signal is incomplete and degrades with large viewpoint changes.

Method: CAMEO directly supervises attention maps using geometric correspondence during training. Surprisingly, supervising just a single attention layer is sufficient to guide the model toward learning precise correspondences.

Result: CAMEO reduces training iterations by half while achieving superior performance. It preserves geometry and structure of reference images, accelerates convergence, and improves novel view synthesis performance.

Conclusion: CAMEO is a simple yet effective, model-agnostic training technique that enhances multi-view diffusion models by supervising attention maps with geometric correspondence, improving both training efficiency and generation quality.
...
TL;DR: MagicQuill V2 introduces layered composition for generative image editing, bridging diffusion model semantics with traditional graphics control through content, spatial, structural, and color layers.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models use monolithic prompts that fail to disentangle distinct user intentions for content, position, and appearance, creating a gap between semantic power and granular control.

Method: Deconstructs creative intent into four controllable visual cue layers: content (what), spatial (where), structural (shape), and color (palette). Includes specialized data generation pipeline, unified control module, and fine-tuned spatial branch for precise local editing.

Result: Extensive experiments validate that the layered approach effectively resolves the user intention gap, granting creators direct, intuitive control over the generative process.

Conclusion: MagicQuill V2 successfully bridges the gap between diffusion model semantic power and traditional graphics software granular control through its novel layered composition paradigm.
