...
Motivation: Prior work has focused on 2D image/video generation while overlooking native 3D/4D representations like RGB-D, occupancy grids, and LiDAR point clouds. The absence of standardized definitions has led to fragmented research.

Method: The survey establishes precise definitions, introduces a structured taxonomy (VideoGen, OccGen, LiDARGen), systematically summarizes datasets and evaluation metrics, and provides a comprehensive literature review.

Result: The paper presents a foundational framework for 3D/4D world modeling, categorizing approaches and providing standardized evaluation methods for the field.

Conclusion: This survey aims to provide a coherent reference for advancing 3D/4D world modeling, addressing practical applications, open challenges, and promising research directions in the field.
...
<details>
  <summary>Details</summary>
Motivation: Traditional flood detection methods rely on unimodal data and static rule-based systems that fail to capture dynamic flood relationships. Existing approaches have limitations in hierarchical refinement, cross-modal integration, and adaptability to noisy environments.

Method: XFloodNet combines three components: 1) Hierarchical Cross-Modal Gated Attention for visual-textual feature alignment, 2) Heterogeneous Convolutional Adaptive Multi-Scale Attention for spectral/spatial feature prioritization, and 3) Cascading Convolutional Transformer Feature Refinement for noise-resistant detection.

Result: Achieved state-of-the-art F1-scores of 93.33% on Chennai Floods, 82.24% on Rhine18 Floods, and 88.60% on Harz17 Floods, significantly outperforming existing methods.

Conclusion: XFloodNet provides an effective solution for urban flood classification by addressing limitations of traditional methods through advanced deep learning techniques and cross-modal integration, demonstrating superior performance across multiple benchmark datasets.
...
<details>
  <summary>Details</summary>
Motivation: VideoLLMs face computational bottlenecks when increasing input frames for better temporal detail, leading to prohibitive costs and performance degradation from long context lengths.

Method: VPS runs multiple parallel inference streams, each processing disjoint subsets of video frames, then aggregates output probabilities to integrate richer visual information without additional training.
...
Conclusion: VPS provides a memory-efficient, robust framework that effectively contracts the Chinchilla scaling law by leveraging uncorrelated visual evidence, enhancing VideoLLMs' temporal reasoning capabilities without computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: Address the gap in existing stance detection methods that focus only on textual data, while real-world social media content increasingly combines text with visual elements requiring advanced multimodal approaches.

Method: Uses Large Language Model for stance-relevant text summaries, domain-aware image caption generator for visual content interpretation, and specialized transformer module for joint modeling of text-image interactions through hierarchical fusion.
...
Conclusion: The proposed multimodal framework effectively integrates textual and visual information for robust stance detection, demonstrating superior performance compared to text-only methods.
...
<details>
  <summary>Details</summary>
Motivation: To improve vehicle detection accuracy in UAV images by leveraging ensemble learning and transfer learning techniques to overcome limitations of existing methods.

Method: Combines three pre-trained Faster R-CNN feature extractors (InceptionV3, ResNet50, GoogLeNet) with five transfer classifiers (KNN, SVM, MLP, C4.5, Naive Bayes) to create 15 base learners, aggregated via weighted averaging and optimized with whale optimization algorithm.

Result: Outperforms existing methods on the AU-AIR UAV dataset, achieving better accuracy, precision, and recall for detecting multiple vehicle types (Car, Van, Truck, Bus).

Conclusion: SI-EDTL demonstrates that ensemble deep transfer learning with swarm intelligence optimization can significantly improve vehicle detection performance in UAV imagery compared to conventional approaches.
...
<details>
  <summary>Details</summary>
Motivation: To create a comprehensive machine learning-ready dataset for Martian digital elevation model prediction, addressing issues with artefacts and missing data in existing large-scale DEM processing pipelines.

Method: Developed processing pipeline for Mars orthoimage and DEM pairs from Mars Reconnaissance Orbiter CTX data, created 80,898 samples with optical image patches, DEM patches, and mask patches. Trained small U-Net architecture on the dataset.

Result: Even a very small U-Net architecture trained specifically on MCTED dataset outperforms the zero-shot performance of DepthAnythingV2 foundation model for elevation prediction tasks.

Conclusion: The MCTED dataset provides a valuable resource for Martian DEM prediction research, demonstrating that specialized training on domain-specific data yields better performance than general foundation models in zero-shot settings. Dataset and code are open source.
...
Method: APML uses Sinkhorn iterations on a temperature-scaled similarity matrix from pairwise distances, with analytically computed temperature to guarantee minimum assignment probability, making it fully differentiable.

Result: APML achieves faster convergence, superior spatial distribution (especially in low-density regions), and improved or comparable quantitative performance on ShapeNet benchmarks and WiFi-based 3D human point cloud generation.

Conclusion: APML provides an efficient, differentiable alternative to traditional point cloud losses, enabling better optimization and performance without additional hyperparameter search.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep unfolding networks for infrared small target detection face challenges with parameter lightweightness and noise robustness, limiting their practical deployment in real-world applications.
...
Result: Extensive experiments on ISTD datasets show superior performance compared to state-of-the-art methods including RPCANet, DRPCANet, and RPCANet++, demonstrating both lightweightness and robustness.

Conclusion: L-RPCANet achieves excellent performance while maintaining lightweight parameter design and strong noise robustness, making it suitable for practical infrared small target detection applications.
...
<details>
  <summary>Details</summary>
Motivation: Current digital camera power consumption is unsustainable for gigapixel cameras at high frame rates (100-1000 fps), requiring 10-100X power reduction through compressive measurement techniques.

Method: Proposes Ultra-Sparse Sampling (USS) where only one sub-frame is set to 1 per spatial location, builds DMD encoding system, and develops BSTFormer transformer with local block, global sparse, and temporal attention to handle measurement mismatch.

Result: Extensive experiments on simulated and real-world data show significant performance improvement over previous state-of-the-art algorithms, with USS providing higher dynamic range than random sampling.

Conclusion: USS strategy is effective for video compressive imaging, offers fixed exposure time advantages, and is suitable for on-chip implementation of complete video SCI systems.
...
<details>
  <summary>Details</summary>
Motivation: Detecting fatal incidents like shootings and stabbings in surveillance videos is difficult due to rarity and ethical data collection issues.

Method: Created GTA-Crime dataset using Grand Theft Auto 5 footage, developed generation framework, and proposed snippet-level domain adaptation with Wasserstein adversarial training.

Result: Experimental results validate the dataset and show that incorporating GTA-Crime with domain adaptation enhances real-world fatal violence detection accuracy.

Conclusion: GTA-Crime provides an ethical solution for fatal incident detection research and the framework successfully bridges synthetic-to-real domain gaps for improved performance.
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers are pretrained on 3-channel natural images but chest X-rays are grayscale (single-channel), creating a compatibility gap that needs addressing for optimal medical image analysis.
...
Conclusion: Simple channel replication enables ViTs to leverage full representational power on grayscale medical images, establishing new SOTA for TB/pneumonia detection with clinical deployment potential.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods often ignore the asymmetrical characteristics caused by brain disorders in Alzheimer's disease, and most rely on pretraining without addressing the specific asymmetric patterns of brain atrophy.

Method: Proposed network with 3D CNN Encoder and Symmetry Interactive Transformer (SIT) that uses inter-equal grid block fetch operation to align left and right hemisphere features, enabling focused analysis on asymmetric regions caused by structural changes.
...
Conclusion: The method demonstrates improved diagnostic performance and interpretability by specifically addressing asymmetric brain changes in Alzheimer's disease, providing an effective end-to-end solution for sMRI-based AD diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: Frame-based cameras with long exposure times cause visual blurring and information loss between frames, degrading video quality. Event cameras offer high temporal resolution to address this problem.

Method: Uses Learnable Double Integral (LDI) network to map reference frames to sharp images, learning-based division reconstruction for varying exposure intervals, and adaptive parameter-free fusion strategy with event confidence.

Result: Achieves state-of-the-art performance on both synthetic and real-world datasets for video deblurring and interpolation tasks, demonstrated with a real-world dataset using DAVIS346c camera.

Conclusion: EVDI++ provides an effective unified framework that leverages event camera capabilities to significantly improve video quality through self-supervised learning with real-world blurry videos and events.
...
Method: Proposes HyMamba network with Spectral State Integration (SSI) module for progressive refinement of spectral features, and Hyperspectral Mamba (HSM) module with three directional scanning SSMs to learn spatial and spectral information synchronously.
...
<details>
  <summary>Details</summary>
Motivation: VLMs often disregard visual evidence and rely on inherent biases when answering specific questions about images, leading to inaccurate responses that need systematic investigation.

Method: Developed a multi-dimensional examination framework using open-source VLMs to analyze how attention values change with varying input parameters like image size, object count, background color, and prompt specificity.

Result: Minor modifications in image characteristics and prompt specificity cause large changes in how VLMs formulate answers and their overall performance.

Conclusion: VLMs are highly sensitive to input variations, and systematic characterization of these sensitivity patterns is crucial for understanding and improving model behavior.
...
<details>
  <summary>Details</summary>
Motivation: Address the problem of biased predictions in 3D point cloud segmentation where models tend to favor seen classes due to smaller training data scales compared to image-based tasks.
...
Conclusion: The proposed E3DPC-GZSL effectively reduces overconfident predictions toward seen classes and improves uncertainty estimation, demonstrating superior performance in 3D point cloud segmentation tasks.
...
Conclusion: The proposed framework effectively addresses key limitations in WSOD through innovative heatmap guidance, enhanced network architecture, and improved optimization, demonstrating superior performance on standard benchmarks.
...
Method: Expert labeling of high-resolution satellite imagery through wall-to-wall digitization over large grids, with quality ensured through multi-interpreter consensus and field validation. Includes hierarchical typology distinguishing oil palm planting stages and similar perennial crops.

Result: Comprehensive polygon-based dataset covering various agro-ecological zones, suitable for training both conventional CNNs and newer geospatial foundation models. Released under CC-BY license following FAIR data principles.

Conclusion: The dataset fills a key gap in training data for remote sensing, aims to improve land cover mapping accuracy, and supports global deforestation reduction goals through transparent monitoring of oil palm expansion.
...
<details>
  <summary>Details</summary>
Motivation: CT scans have spatially sparse lesion distributions with intricate structures, and complex implicit relationships exist between pathological descriptions in reports and their corresponding sub-regions in radiographs, making medical vision-language pre-training challenging.

Method: Multi-modal masked modeling for low-level semantics, similarity-driven alignment to select and align patches with report sentences, and cross-granularity fusion integrating instance-level and word-patch-level multimodal information.
...
Conclusion: SimCroP effectively captures key pathology structures in sparse radiographs and improves performance for multi-scale downstream tasks in medical imaging.
...
Method: Boosted Training Scheme for Early Exits (BTS-EE) sequentially trains and calibrates each branch before the next, uses lightweight 1D convolution branch architecture, and implements Class Precision Margin (CPM) calibration for per-class threshold tuning.

Result: Experiments on CINIC-10 with ResNet18 show BTS-EE outperforms non-boosted training across 64 configurations, achieving up to 45% computation reduction with only 2% accuracy degradation.
...
<details>
  <summary>Details</summary>
Motivation: Current CNNs neglect clinical metadata and require extensive preprocessing, while general VLMs lack clinical specificity for accurate melanoma diagnosis.

Method: Proposed a retrieval-augmented VLM framework that incorporates semantically similar patient cases into diagnostic prompts without requiring fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: To address the problem of noise and error accumulation that occurs during basic feature extraction, perspective transformation, and feature fusion processes in 3D object detection systems.
...
Result: Achieves new state-of-the-art performance on the nuScenes dataset and demonstrates compatibility with various advanced baseline methods.
...
<details>
  <summary>Details</summary>
Motivation: To develop a more general and less biased approach for disentangling video representations into dynamic motion and static content components without heavy assumptions.

Method: Transformer-based architecture with low-bitrate vector quantization as information bottleneck, combined with denoising diffusion model for self-supervised representation learning.

Result: Validated on talking head videos for motion transfer and auto-regressive generation, and generalized to 2D cartoon character videos, demonstrating effective disentanglement.

Conclusion: Presents a novel perspective for self-supervised learning of disentangled video representations that contributes to video analysis and generation with broad applicability.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use modular pipelines optimized independently, leading to cascading errors and lack of holistic supervision in 3D semantic occupancy prediction.

Method: Semantic Causality-Aware 2D-to-3D Transformation with three components: Channel-Grouped Lifting, Learnable Camera Offsets, and Normalized Convolution, all guided by a novel causal loss that enables end-to-end gradient flow.
...
Conclusion: The proposed causal loss enables holistic end-to-end supervision, making previously non-trainable components learnable and improving overall performance in 3D semantic occupancy prediction.
...
<details>
  <summary>Details</summary>
Motivation: Vehicle images in traffic surveillance often suffer from severe noise and blur due to adverse weather, poor lighting, or high-speed motion, which reduces license plate recognition accuracy, especially when plates occupy small regions.
...
Conclusion: VRAE is an effective pre-processing solution for real-time image enhancement in traffic surveillance, significantly improving license plate recognition performance while maintaining computational efficiency.
...
Method: Proposes three techniques: 1) Sparse transformation to avoid unnatural interpolation during projection, 2) Density-aware weighting to adaptively fuse features based on spatial confidence and camera distance, 3) Multi-view consistency loss to encourage each camera to learn discriminative features independently before fusion.
...
Conclusion: SCFusion effectively mitigates limitations of conventional BEV projection and provides a robust and accurate solution for multi-view object detection and tracking.
...
<details>
  <summary>Details</summary>
Motivation: Video-based AI systems in safety-critical domains like autonomous driving and healthcare lack interpretable explanations due to spatiotemporal complexity and model opacity. Existing methods suffer from poor temporal coherence, limited robustness, and lack actionable causal insights.
...
Result: LD-ViCE outperforms state-of-the-art methods with up to 68% increase in R2 score while reducing inference time by half. It generates semantically meaningful and temporally coherent explanations across three diverse video datasets (EchoNet-Dynamic, FERV39k, Something-Something V2).

Conclusion: LD-ViCE represents a significant advancement toward trustworthy AI deployment in safety-critical domains by providing efficient, realistic, and interpretable counterfactual explanations for video-based models.
...
Conclusion: HyperTTA provides a comprehensive solution for robust HSI classification under diverse degradation conditions through dataset construction, improved classifier design, and lightweight test-time adaptation without requiring source data or target annotations.
...
<details>
  <summary>Details</summary>
Motivation: Accurate forecasting of individualized cortical thickness trajectories is essential for detecting subtle neurodegenerative changes and enabling earlier interventions, but challenging due to the cortex's non-Euclidean geometry and need for multi-modal data integration.

Method: Proposes bidirectional conditional Brownian bridge diffusion process with conditional spherical U-Net (CoS-UNet) that combines spherical convolutions and dense cross-attention to integrate cortical surfaces and tabular data.

Result: Achieves significantly reduced prediction errors compared to previous approaches on ADNI and OASIS datasets, and can generate both factual and counterfactual cortical thickness trajectories.

Conclusion: SBDM provides a novel framework for exploring hypothetical cortical development scenarios and enables more precise forecasting of neurodegenerative processes.
...
<details>
  <summary>Details</summary>
Motivation: Most Mamba-based vision models focus on architecture and scan paths while neglecting the SSM module itself. The authors want to explore SSM's potential for lightweight super-resolution tasks.

Method: Introduce First-order State Space Model (FSSM) that applies first-order hold condition in SSMs, derives new discretized form, incorporates token correlations, and analyzes cumulative error without increasing parameters.

Result: FSSM improves MambaIR performance on five benchmark datasets, surpasses current lightweight SR methods, and achieves state-of-the-art results without additional parameters.

Conclusion: The proposed FSSM successfully enhances SSM module performance for vision tasks, demonstrating that optimizing the core SSM calculation process can significantly improve results in lightweight super-resolution applications.
...
<details>
  <summary>Details</summary>
Motivation: Contrastive self-supervised learning works well on balanced datasets but fails on imbalanced ones. The paper addresses this limitation by extending the multi-view framework beyond two views to better handle class imbalance.

Method: Theoretical justification based on mutual information for more than two views, introducing a loss function that segregates intra and inter discriminatory characteristics and filters out extreme features to learn better representations for tail classes.

Result: Achieved 2% improvement on Cifar10-LT, 5% on Cifar100-LT, and 3% on Imagenet-LT using ResNet architectures, setting new state-of-the-art accuracy for self-supervised imbalanced dataset classification.

Conclusion: The multi-view objective with more than two views effectively addresses dataset imbalance in self-supervised learning by extracting better representations for tail classes through improved feature discrimination.
...
<details>
  <summary>Details</summary>
Motivation: To create a practical, end-to-end system that converts natural language instructions into multiple image processing steps while maintaining transparency and reliability.

Method: Combines open-vocabulary detection, promptable segmentation, text-conditioned inpainting, and vision-language description into a single workflow with threshold adjustments, mask inspection, and resource-aware defaults.

Result: Achieved over 90% usable masks with 85% accuracy in single-word prompts, identified inpainting as the main runtime bottleneck (60-75% of total time), and established reliable operational practices.

Conclusion: Provides a transparent, reliable pattern for assembling vision and multimodal models with clear guardrails that improve reliability in object replacement, scene augmentation, and removal tasks.
...
<details>
  <summary>Details</summary>
Motivation: Underwater object detection faces significant challenges that compromise performance in marine applications, and existing methods fail to fully capture the complexities of underwater environments.

Method: Systematically categorizes UOD challenges into five key areas, analyzes progression from traditional to modern approaches, explores LVLMs' potential, and presents case studies including synthetic dataset generation with DALL-E 3 and fine-tuning Florence-2 LVLM.

Result: Identifies three key insights: current UOD methods are insufficient for dynamic underwater environments, synthetic data generation shows potential but needs refinement, and LVLMs hold promise but real-time application requires further research.

Conclusion: LVLMs represent a promising direction for advancing underwater object detection, though significant research is needed to optimize them for real-time applications and ensure synthetic data realism.
...
<details>
  <summary>Details</summary>
Motivation: Current video embeddings poorly represent temporally opposite actions (chiral pairs) that require understanding of visual changes over time, such as opening/closing doors or approaching/moving away.

Method: Self-supervised adaptation recipe using auto-encoder with perceptual straightening-inspired latent space to inject time-sensitivity into frozen image feature sequences.
...
Conclusion: The proposed method successfully creates compact, time-sensitive video representations that effectively distinguish temporally opposite actions and enhances performance of standard video models.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to effectively coordinate heterogeneous modalities (text, image, audio) due to scarce training data with paired triplet conditions and difficulties in collaborating sub-tasks of subject preservation and audio-visual synchronization.

Method: Two-stage progressive multimodal training: 1) Minimal-invasive image injection for subject preservation, 2) Focus-by-predicting strategy for audio-visual sync that implicitly guides audio-facial association, plus time-adaptive Classifier-Free Guidance for flexible control.
...
Conclusion: HuMo successfully addresses multimodal coordination challenges through its progressive training paradigm and specialized strategies, demonstrating superior performance in human-centric video generation from multimodal inputs.
...
<details>
  <summary>Details</summary>
Motivation: Current video hallucination benchmarks rely on manual categorization and neglect human perception processes. LVMs produce inaccurate descriptions despite progress in understanding dynamic video content.

Method: MESH uses a bottom-up Question-Answering framework with target and trap instances, evaluating basic objects, coarse-to-fine subject features, and subject-action pairs to align with human video understanding.

Result: LVMs perform well on basic object recognition but show significantly increased hallucinations when handling fine details or aligning multiple actions involving various subjects in longer videos.

Conclusion: MESH provides an effective and comprehensive approach for systematically identifying hallucinations in video models, revealing specific weaknesses in LVMs' ability to process complex video content accurately.
...
Method: Multi-view approach with 24 views (selection vector) in random selection to learn view-invariant embeddings. Also experimented with randomized view selection across all five height levels (120 views total, selection matrices).

Result: ViewSparsifier approach won both tasks: Plant Age Prediction and Leaf Count Estimation in the GroMo Grand Challenge.

Conclusion: Multi-view approach with view-invariant embeddings significantly improves plant phenotyping accuracy over single-view methods, with potential for further improvement through expanded view selection.
...
Result: Method consistently outperforms existing SOTA approaches across multiple domain generalization benchmarks on cardiac and fundus datasets.
...
<details>
  <summary>Details</summary>
Motivation: Accurate maps of Greenland's subglacial bed are crucial for sea-level projections, but current radar observations are sparse and unevenly distributed, limiting reliability.

Method: Uses graph-learning with Monte Carlo dropout for uncertainty modeling, spatial graphs from surface observables (elevation, velocity, mass balance) augmented with gradient features and polynomial trends, and hybrid loss combining confidence-weighted radar supervision with dynamic regularization.
...
<details>
  <summary>Details</summary>
Motivation: To alleviate the manual workload for medical professionals in complex 3D segmentation tasks that cannot be fully automated, particularly in radiotherapy planning (organ at risk identification) and diagnosis of degenerative diseases like sarcopenia (muscle volume measurements).

Method: Proposes an implicit shape prior to segment volumes from sparse slice manual annotations generalized for multi-organ cases, along with a framework for automatically selecting the most informative slices to guide and minimize subsequent interactions.

Result: Experimental validation shows the method's effectiveness on two medical use cases: assisted segmentation of at-risk organs for brain cancer patients, and acceleration of database creation for patients with sarcopenia with unseen muscle shapes.

Conclusion: The approach successfully reduces the manual segmentation burden in medical imaging tasks while maintaining accuracy, making it valuable for clinical applications requiring precise 3D segmentation from limited annotations.
...
Conclusion: Suitable for real-time forensic applications due to superior efficiency and performance on new high-resolution SIF dataset.
...
<details>
  <summary>Details</summary>
Motivation: Current medical image segmentation methods face modality ambiguity in text descriptions, reliance on manual prompting for SAM workflows, and lack of unified frameworks that work across different imaging modalities and tasks.

Method: Pre-train CLIP-based image encoder on multi-modal retinal data, use GroundingDINO for automatic spatial prompt generation via lesion detection, employ text prompts with modality signatures, and guide SAM for automated segmentation.
...
Conclusion: CLAPS demonstrates broad generalizability as a foundation model, providing a fully automated and unified segmentation pipeline for diverse retinal imaging tasks and modalities.
...
<details>
  <summary>Details</summary>
Motivation: To extend LLMs' specialized applications by using ad videos as a challenging test-bed that contains rich marketing logic, persuasive strategies, and audience engagement elements beyond typical visual content.

Method: Created AdsQA benchmark with 1,544 ad videos (10,962 clips, 22.7 hours) with 5 challenging tasks. Proposed ReAd-R, a Deepseek-R1 styled RL model that reflects on questions and generates answers via reward-driven optimization.
...
Conclusion: Ad videos provide a valuable and challenging domain for evaluating LLMs' advanced perception capabilities, and the proposed ReAd-R model demonstrates superior performance in handling these complex marketing-oriented tasks.
...
<details>
  <summary>Details</summary>
Motivation: Acquiring paired multimodal ophthalmic images (fundus + OCT) is expensive and limited. Existing methods using only fundus or text features fail to capture fine-grained spatial information from different imaging modalities.

Method: UOPSL uses contrastive learning on unpaired OCT and fundus images to learn a predilection sites matrix in OCT latent space. This matrix captures lesion localization patterns and is then used to assist fundus-only classification when OCT data is unavailable.

Result: Extensive experiments on 9 diverse datasets across 28 categories show the framework outperforms existing benchmarks.

Conclusion: The proposed approach successfully bridges unpaired fundus and OCT data using spatial priors, enabling improved disease recognition from fundus images alone while overcoming modality imbalance issues.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models struggle in data-scarce domains where exhaustive retraining or costly paired data are required. Existing methods either lack controllability (unpaired) or need large domain-specific datasets (fully paired).

Method: Aligns source and target distributions in shared latent space, integrating pretrained source diffusion models with target-domain Latent Aligned Diffusion Model trained on partially paired latent representations.

Result: Superior performance in depth-to-image translation under partial supervision, successfully extended to multi-source translation (depth + segmentation) and multi-target class-conditioned style transfer.

Conclusion: LADB provides scalable and versatile solution for real-world domain translation, particularly effective when data annotation is costly or incomplete.
...
<details>
  <summary>Details</summary>
Motivation: Existing ISLR methods struggle with morphologically similar but semantically distinct gestures due to geometric ambiguity from using single reference frames, which fail to properly model the complex interplay between hand shape and motion trajectory.
...
Conclusion: DSLNet successfully addresses geometric ambiguity in ISLR by decoupling gesture morphology and trajectory modeling in complementary coordinate systems, demonstrating superior performance and efficiency compared to existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: To develop an optical flow estimation method that doesn't require ground truth annotations, can handle high-resolution data, and captures both fine details and long-range motion patterns through fractal-inspired architecture.

Method: Uses Fractal Deformation Network (FDN) - a recursive encoder-decoder with skip connections inspired by fractal geometry. Minimizes energy functional with L1/L2 data fidelity terms for brightness constancy and TV regularization for spatial smoothness.
...
Conclusion: The fractal-inspired architecture combined with variational formulation provides an effective unsupervised approach for optical flow estimation that works well without ground truth data, especially for high-resolution applications.
...
<details>
  <summary>Details</summary>
Motivation: Traditional RGB-based approaches suffer from training instability and poor generalization in complex coastal environments with irregular boundaries and spectral characteristics.
...
Conclusion: The proposed Robust U-Net framework significantly enhances coastal water segmentation performance while maintaining computational efficiency, with code available for reproducibility.
...
Result: The survey analyzes relationships between CI methods and their practical contributions to CV applications, highlighting emerging opportunities and challenges.

Conclusion: Task-specific, adaptive imaging pipelines have potential to improve robustness, accuracy, and efficiency in real-world scenarios like autonomous navigation, surveillance, AR, and robotics.
...
<details>
  <summary>Details</summary>
Motivation: Address deployment challenges of large MLLMs in resource-constrained environments, focusing on energy efficiency, computational scalability, and environmental sustainability for real-world applications.

Method: Develops BreezeCLIP-enhanced Q-Gated Multimodal Language Model (BcQLM) - a compact vision-language encoder optimized for efficient multimodal understanding with modular and extensible design.

Result: Achieves performance comparable to standard-size MLLMs while significantly reducing computational costs. Validated on multiple datasets, effectively balancing accuracy and efficiency.

Conclusion: BcQLM offers a promising path toward deployable MLLMs under practical hardware constraints, enabling generalization to broader multimodal tasks with improved efficiency.
...
Method: Developed CQ module that predicts and embeds object density maps, then integrates this density information into decoder queries. The method extends density definitions to include bounding box dimensions and creates density-guided queries for better crowd detection.

Result: Significant performance improvements on STCrowd dataset for both 2D and 3D detection, outperforming most state-of-the-art methods. Also improves performance on CrowdHuman dataset when integrated with existing detectors.
...
Method: Employs a bijective mapping framework that breaks down large-scale modifications into fine-grained atomic changes at map element level for interpretability, preserving unchanged elements with high fidelity.
...
<details>
  <summary>Details</summary>
Motivation: MLLM judge models suffer from biases, overconfidence, and inconsistent performance across diverse image domains, and standard prompt ensembling methods fail for TTI tasks.

Method: Multimodal Mixture-of-Bayesian Prompt Ensembles (MMB) using Bayesian prompt ensemble approach augmented by image clustering to dynamically assign prompt weights based on visual characteristics.

Result: MMB improves accuracy in pairwise preference judgments and enhances calibration, outperforming baselines on HPSv2 and MJBench benchmarks in alignment with human annotations.

Conclusion: Multimodal-specific strategies are crucial for judge calibration, and MMB provides a promising path for reliable large-scale TTI evaluation.
...
TL;DR: Deep learning framework using mobile phone images achieves 86% accuracy for arsenicosis diagnosis, with transformer models outperforming CNNs and interpretability tools providing clinical transparency.


<details>
  <summary>Details</summary>
Motivation: Arsenicosis is a serious public health concern in Asia caused by arsenic-contaminated water, with early cutaneous manifestations often underdiagnosed in rural areas with limited dermatologist access. Automated image-based solutions can support early detection.

Method: End-to-end framework using mobile phone-captured skin images with 20 classes and over 11,000 images. Multiple deep learning architectures (CNNs and Transformers) benchmarked, with interpretability via LIME and Grad-CAM. Web-based diagnostic tool demonstrated deployment feasibility.

Result: Transformer-based models significantly outperformed CNNs, with Swin Transformer achieving 86% accuracy. Interpretability visualizations confirmed models attended to lesion-relevant regions. Strong performance on external validation samples demonstrated generalization capability.
...
TL;DR: Event-based cameras achieve 18.47 arcsecond accuracy in star tracking using Earth's rotation as ground truth, demonstrating potential for low-cost, low-latency attitude determination systems.
...
Method: Static event camera mounted on ground-based telescope captures night sky events. Earth's rotation provides known motion reference. Event stream processed for orientation estimates compared against IERS Earth orientation measurements.

Result: Achieved RMS error of 18.47 arcseconds and about error of 78.84 arcseconds. Performance combined with EBC advantages: sparse data computation, high dynamic range, low energy consumption, and fast update rates.

Conclusion: Event cameras provide sufficient accuracy for practical star tracking applications, offering cost-effective and efficient alternative to traditional framing sensors with demonstrated utility for low-cost, low-latency attitude determination systems.
...
TL;DR: BEAMER introduces a novel dense image matching approach that predicts multiple correspondent hypotheses per pixel at each scale using beam search and cross-attention, improving robustness in challenging cases like depth discontinuities and strong zoom-ins.


<details>
  <summary>Details</summary>
Motivation: Traditional dense matching methods produce only single correspondent hypotheses per pixel at each scale, which leads to erroneous matches in challenging scenarios such as depth discontinuities and strong zoom-in cases where neighboring correspondences are widely spread.

Method: Proposes a beam search strategy to propagate multiple hypotheses at each scale and integrates these multiple hypotheses into cross-attention layers, creating the BEAMER architecture that learns to preserve and propagate multiple correspondence hypotheses across scales.

Result: BEAMER demonstrates significantly improved robustness compared to state-of-the-art methods, particularly excelling at handling depth discontinuities and cases where the target image is a strong zoom-in of the source image.

Conclusion: Predicting multiple correspondent hypotheses per source location at each scale through beam search and cross-attention integration is an effective approach that substantially enhances the robustness of dense image matching in challenging scenarios.
...
Method: Created a large-scale artifact dataset (GeneVA) with rich human annotations specifically focused on spatio-temporal artifacts in videos generated from natural text prompts.

Result: Provides a comprehensive benchmark dataset that enables systematic evaluation of text-to-video generation models and identification of various artifact types.
...
<details>
  <summary>Details</summary>
Motivation: Existing reward models for visual generation have fundamental limitations - CLIP-based RMs suffer from architectural constraints and Bradley-Terry losses are misaligned with VLM architectures, while RLHF optimization suffers from reward hacking where models exploit reward signal flaws without improving true quality.

Method: RewardDance reformulates reward scoring as the model's probability of predicting a "yes" token indicating generated image outperforms reference image according to specific criteria. This aligns reward objectives with VLM architectures and enables scaling across model size (up to 26B parameters) and context (instructions, references, chain-of-thought reasoning).

Result: Extensive experiments show RewardDance significantly surpasses state-of-the-art methods in text-to-image, text-to-video, and image-to-video generation. Large-scale RMs exhibit and maintain high reward variance during RL fine-tuning, proving resistance to hacking and ability to produce diverse, high-quality outputs.

Conclusion: RewardDance resolves the persistent reward hacking challenge and greatly relieves mode collapse problems that plague smaller models, providing a scalable framework for effective reward modeling in visual generation tasks.
...
TL;DR: A novel method for 3D dynamic scene reconstruction of fabrics using only monocular RGB video, combining 3D geometry reconstruction and appearance estimation with physical simulation and differentiable rendering.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of reconstructing 3D dynamic scenes, particularly fabrics, using only monocular RGB video input while achieving realistic deformations and renderings.

Method: Proposes a system that combines 3D geometry reconstruction and appearance estimation using physical simulation of cloth geometry and differentiable rendering. Introduces two novel regularization terms to address depth ambiguity in monocular video.
...
Conclusion: The approach successfully combines 3D reconstruction and appearance estimation for fabrics using monocular video, with significant error reduction and high-quality results for both geometry and appearance recovery.
