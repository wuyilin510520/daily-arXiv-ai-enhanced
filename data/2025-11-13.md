...
TL;DR: AI-based system for automating gas plant digitization using OCR, Vision LLM, Object Detection, and a novel Transformer architecture to extract design data and hierarchical plant structure from P&ID documents with high accuracy.


<details>
  <summary>Details</summary>
Motivation: To automate the digitization of SNAM's gas infrastructure plants by extracting information from P&ID documents, streamlining daily work processes and overcoming data standardization challenges.

Method: Uses OCR, Vision LLM, Object Detection, Relational Reasoning and optimization algorithms with a novel Transformer architecture for Scene Graph Generation to analyze complex relations between plant components.

Result: Achieved 91% accuracy in textual design data extraction, 93% component identification accuracy, and ~80% accuracy in hierarchical structure extraction.

Conclusion: The synergistic AI approach successfully automates plant digitization, overcoming data variety challenges and achieving high accuracy rates for practical industrial application.
...
TL;DR: Proposes a systematic evaluation framework to detect and quantify lip leakage in audio-driven talking face generation models, where generated lips are influenced by reference images rather than audio.


<details>
  <summary>Details</summary>
Motivation: Current inpainting-based talking face generation methods can introduce lip leaking where generated lips are influenced by identity reference images rather than solely by driving audio, which is difficult to detect with standard metrics.

Method: Uses three complementary test setups: silent-input generation, mismatched audio-video pairing, and matched audio-video synthesis, along with derived metrics like lip-sync discrepancy and silent-audio-based lip-sync scores.

Result: The framework provides a systematic way to analyze and quantify lip leakage, studies how different identity reference selections affect leakage, and offers insights into reference design.

Conclusion: The proposed methodology is model-agnostic and establishes a more reliable benchmark for future research in talking face generation by addressing the lip leakage problem.
...
Result: Achieves ~90% detection and tracking accuracy, successfully tracks ~80% of trajectories under challenging conditions, and shows strong generalization through transfer learning and graceful performance degradation during camera failures.
...
TL;DR: A topology-driven multi-subspace fusion framework on Grassmannian manifolds that enables adaptive subspace collaboration for geometric representation learning, outperforming static single-subspace approaches.


<details>
  <summary>Details</summary>
Motivation: Existing Grassmannian approaches use static single-subspace representations, failing to capture the dynamic interplay between multiple subspaces needed for complex geometric structures.
...
Conclusion: Successfully adapts multi-channel interaction philosophy from Euclidean networks to non-Euclidean domains, advancing geometric deep learning with better discriminability and robustness.
...
TL;DR: TTM is a training-free framework for motion-controlled video generation that uses crude reference animations as motion cues and dual-clock denoising to balance motion fidelity with natural dynamics.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based video generation lacks precise motion control, and prior methods require expensive model-specific fine-tuning, which is restrictive.

Method: Uses crude reference animations from user-friendly manipulations as coarse motion cues, adapts SDEdit mechanism to video domain, employs dual-clock denoising for region-dependent alignment, and preserves appearance with image conditioning.

Result: TTM matches or exceeds training-based baselines in realism and motion control on object and camera motion benchmarks, while enabling precise appearance control through pixel-level conditioning.

Conclusion: TTM provides a lightweight, plug-and-play solution for motion- and appearance-controlled video generation without additional training costs, compatible with any backbone model.
...
TL;DR: Proposes a unified memory bank framework for continual anomaly detection that eliminates task-specific memory fragmentation, achieving state-of-the-art performance on benchmark datasets.
...
Method: Uses a unified memory bank shared across all tasks with incremental embedding updates in a fixed-size coreset, and computes anomaly scores via nearest-neighbor matching.
...
Conclusion: The proposed framework enables continuous knowledge acquisition without task-specific memory fragmentation and demonstrates robust performance in practical scenarios.
...
TL;DR: A unified framework for accident anticipation that combines diffusion-based denoising with time-aware actor-critic modeling to handle noisy inputs and balance early detection with false-alarm suppression.
...
Method: Integrates diffusion-based denoising with a time-aware actor-critic model. The diffusion module reconstructs noise-resilient image and object features through iterative refinement. The actor-critic architecture uses long-horizon temporal reasoning and time-weighted rewards to determine optimal alert timing.

Result: Achieves state-of-the-art accuracy on three benchmark datasets (DAD, CCD, A3D) with significant gains in mean time-to-accident. Maintains robust performance under Gaussian and impulse noise. Produces earlier, more stable, and human-aligned predictions in both routine and complex traffic scenarios.

Conclusion: The proposed framework demonstrates strong potential for real-world, safety-critical deployment by effectively handling sensor degradation while providing reliable early accident warnings.
...
TL;DR: RS-Net is a modular framework for Dynamic Scene Graph Generation that scores object pair relations using spatial and temporal context, improving relation prediction without changing existing model architectures.
...
Method: Proposes Relation Scoring Network (RS-Net) with spatial context encoder using learnable tokens and temporal encoder for video-level aggregation, integrated into unified triplet scoring mechanism.

Result: On Action Genome dataset, RS-Net consistently improves Recall and Precision across baselines, with notable gains in mean Recall, addressing long-tailed relation distribution while maintaining competitive efficiency.
...
TL;DR: A lightweight Anonymizing Adapter Module (AAM) that removes private information from video features while maintaining task utility, achieving 35% privacy reduction with near-baseline performance.


<details>
  <summary>Details</summary>
Motivation: Current privacy methods focus on pixel-level anonymization requiring retraining, making them unsuitable for video foundation models. Extracted visual features inadvertently reveal sensitive personal information.

Method: Plug-and-play AAM module with three objectives: clip-level self-supervised privacy, co-training for utility retention, and latent consistency for generalization. Applied to frozen video encoders.

Result: 35% reduction in privacy leakage while maintaining near-baseline utility across Action Recognition, Temporal Action Detection, and Anomaly Detection tasks. Effectively mitigates gender bias.

Conclusion: The proposed method provides effective privacy preservation for video foundation models without retraining, balancing privacy protection with utility maintenance across diverse tasks.
...
TL;DR: Autoregressive next-pixel prediction scaling analysis reveals task-dependent optimal strategies and projects feasibility of pixel-by-pixel image modeling within 5 years.


<details>
  <summary>Details</summary>
Motivation: To investigate scaling properties of autoregressive next-pixel prediction as a simple, end-to-end framework for unified vision models, which is currently under-explored.

Method: Train Transformers using IsoFlops profiles across compute budgets up to 7e19 FLOPs at resolutions from 32x32, evaluating next-pixel prediction objective, ImageNet classification accuracy, and generation quality via Fr'echet Distance.

Result: Optimal scaling is task-dependent: generation requires 3-5x faster data growth than classification. Higher resolutions require model size to grow faster than data size. Compute is the primary bottleneck, not training data.

Conclusion: Pixel-by-pixel image modeling is projected to be feasible within the next 5 years given current compute growth rates of 4-5x annually.
...
TL;DR: The paper explores diffusion model finetuning techniques (LoRA and DreamBooth) to generate balanced training data that preserves original group distributions, addressing dataset bias in image classification.


<details>
  <summary>Details</summary>
Motivation: Address biases in image classification systems caused by uneven group representation in training data, where certain attributes (like blond hair) become disproportionately associated with specific groups, reinforcing stereotypes.

Method: Use LoRA and DreamBooth for diffusion model finetuning, with a clustering approach within groups to handle intra-group variations (training one DreamBooth model per cluster). Generate balanced data for pretraining followed by fine-tuning on real data.

Result: The finetuning approaches outperform vanilla Stable Diffusion and achieve comparable results to SOTA debiasing techniques like Group-DRO, with superior performance as dataset bias severity increases.

Conclusion: Diffusion model finetuning techniques effectively generate balanced training data that preserves group distributions, offering a robust solution for dataset bias in image classification systems.
...
TL;DR: Overview report of the Women in Computer Vision Workshop (WiCV@CVPR 2025), documenting program details, participation statistics, mentorship outcomes, and historical trends to support future diversity initiatives.
...
Method: Organized the 16th edition workshop featuring paper presentations (14 accepted papers from 32 submissions), mentoring program (80 mentees with 37 mentors), networking events, and travel grants/scholarships.

Result: Successful workshop with over 100 onsite participants, 14 accepted papers (5 oral presentations), 36 extended abstract posters, and $44,000 USD in travel grants and diversity awards distributed.

Conclusion: WiCV continues to effectively increase visibility, inclusion, and professional growth of women and underrepresented minorities in computer vision, serving as a model for diversity initiatives in the field.
...
TL;DR: PoseKAN introduces an adaptive graph Kolmogorov-Arnold Network framework for 3D human pose estimation that overcomes GCN limitations by using learnable activation functions and multi-hop feature aggregation.


<details>
  <summary>Details</summary>
Motivation: GCN-based methods have limited receptive fields that struggle with long-range dependencies needed for handling occlusions and depth ambiguities, and exhibit spectral bias that prioritizes low-frequency components over high-frequency details.
...
Conclusion: PoseKAN provides a more expressive and adaptive framework for 3D pose estimation that effectively captures complex pose variations and improves spatial awareness compared to traditional GCN approaches.
...
TL;DR: SIFT-Graph is a multimodal defense framework that combines SIFT keypoints with Graph Attention Networks to enhance vision model robustness against adversarial attacks while maintaining clean accuracy.
...
Conclusion: The framework provides a unified, structure-aware defense mechanism that leverages multimodal feature aggregation for enhanced adversarial robustness.
...
TL;DR: DT-NVS is a 3D diffusion model for generalized novel view synthesis from single images, using transformer architecture and novel training strategies to handle real-world scenes.


<details>
  <summary>Details</summary>
Motivation: Existing methods focus on small camera movements or unnatural object-centric scenes, limiting real-world applications. This work aims to generate novel views of everyday scenes from single images.

Method: Proposes DT-NVS with transformer backbone, novel camera conditioning for unaligned datasets, and training paradigm swapping reference frame roles between conditioning image and noisy input.
...
Conclusion: The approach successfully handles real-world, multi-category scenes and demonstrates superior performance in single-image novel view synthesis compared to existing methods.
...
TL;DR: The paper introduces Shadow-informed Pose Feature (SiPF) and Rotation-invariant Attention Convolution (RIAttnConv) to address the limitation of existing rotation-invariant methods in distinguishing geometrically similar but spatially distinct structures in 3D point clouds.


<details>
  <summary>Details</summary>
Motivation: Existing rotation-invariant methods for 3D point clouds lose global pose information, making them unable to distinguish geometrically similar but spatially distinct structures (e.g., left vs right airplane wings), which is identified as 'Wing-tip feature collapse'.

Method: Proposes SiPF which augments local rotation-invariant descriptors with a globally consistent reference point ('shadow') from learned shared rotation. Also introduces RIAttnConv operator that integrates SiPFs into feature aggregation, and a task-adaptive shadow locating module using Bingham distribution over unit quaternions.

Result: Extensive experiments on 3D classification and part segmentation benchmarks show the approach substantially outperforms existing rotation-invariant methods, especially in tasks requiring fine-grained spatial discrimination under arbitrary rotations.

Conclusion: The proposed method successfully preserves global pose awareness while maintaining rotation invariance, overcoming the limitations of existing rotation-invariant approaches in distinguishing structurally similar components.
...
TL;DR: SAS-SSM introduces a skeleton structure-aware approach for 3D human pose estimation using State Space Models, addressing limitations of existing SSM methods that disrupt spatial structure and entangle spatiotemporal features.


<details>
  <summary>Details</summary>
Motivation: Existing SSM-based methods flatten 2D pose sequences using manual scan operations, which disrupts inherent spatial structure and entangles spatial and temporal features, making it difficult to capture complex pose dependencies.

Method: Proposes SAS-SSM with structure-aware spatiotemporal convolution to capture local joint interactions, followed by stride-based scan strategy to construct multi-scale global structural representations while maintaining linear computational complexity.
...
Conclusion: The proposed SAS-SSM enables flexible modeling of both local and global pose information while maintaining computational efficiency, providing an effective solution for 3D human pose estimation.
...
TL;DR: Proposes MFAVBs (Multiple Fusing-Augmenting ViT Blocks) for image clustering, using Vision Transformers to explicitly fuse features from positive pairs in contrastive learning, outperforming state-of-the-art methods on seven datasets.
...
Method: Uses shared-weight ViTs to process positive pairs, fuses their features into a larger ViT, splits features into new augmented samples for multiple fusion through MFAVBs, and projects features to instance-level and clustering-level spaces with CLIP-based preprocessing.

Result: Experiments on seven public datasets show MFAVBs outperform state-of-the-art clustering techniques.

Conclusion: MFAVBs effectively enhance contrastive clustering by explicitly fusing positive pair features through multiple fusion and augmentation operations.
...
TL;DR: A deep learning approach using EfficientNet architectures achieved high performance in classifying six histopathological regions of glioblastoma on training data (F1=0.98), but showed reduced generalization on validation (F1=0.546) and testing (F1=0.517) data, highlighting challenges in clinical model deployment.


<details>
  <summary>Details</summary>
Motivation: Glioblastoma has poor prognosis despite diagnostic advances. Automated identification of histological sub-regions could help understand the disease morphology at scale and improve clinical diagnostics.

Method: Four-step deep learning approach using EfficientNet architectures (B0-B4) to classify six histopathological regions in H&E stained GBM tissue sections, evaluated on BraTS-Path 2024 dataset with 5-fold cross-validation.

Result: EfficientNet-B1 and B4 achieved F1=0.98 on training data, but performance dropped to F1=0.546 on validation and F1=0.517 on testing data, showing generalization challenges.

Conclusion: While the approach shows promise for automated histopathological analysis of GBM, the performance gap between training and testing data underscores the critical need for models that generalize well to new data for clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: To reproduce human-like visual recognition efficiency in artificial systems by enhancing biologically inspired neural networks for better object recognition and symmetry detection.
...
Result: Enhanced VisNet variants substantially improved recognition accuracy compared to baseline model across MNIST, CIFAR10, and custom symmetric object datasets.

Conclusion: VisNet-inspired architectures offer a powerful and interpretable framework for visual recognition, demonstrating adaptability and biological relevance for both neuroscience and AI applications.
...
TL;DR: Proposes SemBridge framework for Asymmetric Cross-modal Knowledge Distillation (ACKD) to handle modalities with weak semantic consistency, achieving SOTA performance on remote sensing scene classification.


<details>
  <summary>Details</summary>
Motivation: Symmetric Cross-modal Knowledge Distillation (SCKD) is constrained in real-world scenarios due to limited paired modalities, requiring methods for modalities with weak semantic connections.

Method: SemBridge framework with Student-Friendly Matching module (uses self-supervised learning for semantic-based knowledge) and Semantic-aware Knowledge Alignment module (employs Lagrangian optimization for optimal transport path).

Result: Achieves state-of-the-art performance compared with 7 existing approaches on 6 different model architectures across various datasets, particularly on remote sensing scene classification with multi-spectral and RGB images.

Conclusion: The proposed ACKD approach effectively bridges modalities with limited semantic overlap, and SemBridge framework successfully addresses the challenges of knowledge transmission costs in asymmetric cross-modal scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Document layout understanding remains data-intensive despite advances in semi-supervised learning, requiring a more efficient approach that leverages structural knowledge from LLMs.

Method: OCR-LLM pipeline infers hierarchical regions which are combined with teacher detector outputs through inverse-variance fusion to generate refined pseudo-labels, using learned instance-adaptive gating.

Result: Achieves 88.2±0.3 AP with lightweight backbone using only 5% labels, and 89.7±0.4 AP with LayoutLMv3, surpassing standard semi-supervised learning and matching UDOP which requires 100M+ pages of pretraining.

Conclusion: LLM structural priors are complementary to both lightweight and pretrained architectures, providing targeted semantic disambiguation and enabling privacy-preserving deployment with minimal performance loss.
...
TL;DR: Proposes CCDF framework with Cycle Consistency and Semantic Consistency modules to improve unsupervised change detection in remote sensing by reducing generator overfitting and enabling detail reconstruction.


<details>
  <summary>Details</summary>
Motivation: Previous unsupervised methods using generator networks for style transfer often suffer from poor performance due to generator overfitting, leading to inaccurate change detection.
...
TL;DR: HitoMi-Cam is a lightweight, shape-agnostic person detection method using spectral reflectance of clothing, achieving real-time performance on edge devices and outperforming CNNs in unpredictable shape scenarios like disaster rescue.


<details>
  <summary>Details</summary>
Motivation: CNN-based object detection has shape dependency that degrades performance for postures not in training data, limiting effectiveness in real-world scenarios like disaster rescue where human shapes are unpredictable.

Method: Spectral-based approach using clothing reflectance properties, implemented on resource-constrained edge devices without GPU. The HitoMi-Cam system is designed to be lightweight and shape-agnostic.

Result: Achieved 23.2 fps processing speed (253x190 pixels) on edge device. In search and rescue scenarios, achieved 93.5% AP vs CNN's best 53.8% AP, with minimal false positives throughout all evaluations.

Conclusion: HitoMi-Cam serves as a complementary tool to CNNs, proving spectral-based person detection is viable for real-time edge device operation in environments with unpredictable shapes like disaster rescue.
...
TL;DR: Negative Entity Suppression (NES) method improves zero-shot image captioning by reducing hallucinations through synthetic images, filtering negative entities from retrieved content, and attention-level suppression.


<details>
  <summary>Details</summary>
Motivation: Text-only training for zero-shot image captioning suffers from poor cross-domain generalization and hallucination, while retrieval-based methods can exacerbate these issues when irrelevant entities are retrieved.
...
Conclusion: The proposed NES method effectively addresses hallucination challenges in zero-shot image captioning through negative entity suppression, demonstrating superior cross-domain generalization capabilities.
...
TL;DR: SPEED-Q is a novel framework for low-bit weight-only quantization of Vision-Language Models (1B-2B parameters) for edge devices, addressing modality sensitivity differences and training instability through staged processing and enhanced distillation.


<details>
  <summary>Details</summary>
Motivation: Enable deployment of VLMs on resource-constrained edge devices through aggressive quantization, as existing research rarely explores low-bit quantization for billion-parameter VLMs suitable for edge deployment.
...
TL;DR: Diff-FCHM is a novel collaborative compression method that uses machine-vision-oriented compression as the basis for human vision, employing diffusion priors to restore high-fidelity details.


<details>
  <summary>Details</summary>
Motivation: Existing collaborative compression methods built on human-vision pipelines are inefficient for machine vision, which only needs core region information. Machine vision requires much less information than human vision compression.

Method: Proposes Diff-FCHM using machine-vision-oriented compression as basis, progressively aggregates semantics from machine-vision compression, and uses diffusion prior to restore high-fidelity details for human vision. Includes plug-and-play variable bit-rate strategy for machine tasks.

Result: Experimental results show consistently superior performance on both machine-vision and human-vision compression with remarkable margins.

Conclusion: Diff-FCHM successfully demonstrates that machine-vision-oriented compression can effectively serve as the basis for collaborative compression, achieving excellent results for both human and machine vision tasks.
...
TL;DR: Hierarchical Distillation (HD) framework synergizes trajectory-based and distribution-based distillation to achieve state-of-the-art single-step diffusion models with high fidelity and structural preservation.


<details>
  <summary>Details</summary>
Motivation: To overcome the inference latency of diffusion models while addressing the trade-off between trajectory-based methods (preserving global structure but losing details) and distribution-based methods (achieving higher fidelity but suffering from mode collapse and unstable training).

Method: Proposes Hierarchical Distillation (HD) framework that uses trajectory distillation to create a structural "sketch" as initialization for distribution-based refinement. Introduces Adaptive Weighted Discriminator (AWD) to dynamically allocate token weights for focusing on local imperfections during adversarial training.

Result: Achieves FID of 2.26 on ImageNet 256×256, rivaling the 250-step teacher model. Also shows promising results on high-resolution text-to-image MJHQ benchmark, demonstrating generalizability.

Conclusion: Establishes a robust new paradigm for high-fidelity, single-step diffusion models by synergizing trajectory and distribution distillation approaches.
...
Conclusion: First attempt to explore ensemble non-attention for boosting cross-architecture transferability, providing new insights into launching ensemble attacks.
...
TL;DR: Proposes BRHVC, a novel neural B-frame video compression method that harmonizes bi-directional references using Bi-directional Motion Converge and Bi-directional Contextual Fusion to optimize reference information utilization.


<details>
  <summary>Details</summary>
Motivation: Neural B-frame video compression remains underexplored compared to P-frame compression, and hierarchical coding in NBVC can cause unbalanced contribution from reference frames due to large frame spans.

Method: Uses Bi-directional Motion Converge (BMC) to converge multiple optical flows for more accurate motion compensation, and Bi-directional Contextual Fusion (BCF) to explicitly model reference context weights guided by motion compensation accuracy.

Result: Outperforms previous state-of-the-art neural video compression methods and surpasses traditional VTM-RA coding on HEVC datasets under random access configuration.

Conclusion: BRHVC effectively harmonizes bi-directional references through more efficient motions and contexts, achieving superior compression performance in neural B-frame video coding.
...
Conclusion: First work to introduce Hausdorff Dimension into FGM, effectively enhancing diversity while providing principled theoretical contribution to FGM development.
...
TL;DR: AuthSig is a static electronic signature framework that uses generative models and watermarking to bind authentication information to signature images, enabling reliable verification and preventing malicious reuse.


<details>
  <summary>Details</summary>
Motivation: Static scanned signatures are widely used but lack authentication attributes, making them vulnerable to copying and reuse. Current solutions like dynamic pressure-sensitive and PKI-based signatures are not always practical.

Method: Uses generative models to modulate style embeddings during signature generation to encode watermark bits, implementing a One Signature, One Use policy. Introduces keypoint-driven data augmentation to enhance style diversity for robust watermark embedding.
...
Conclusion: AuthSig provides a practical solution for authenticating static electronic signatures by embedding invisible watermarks that enable reliable verification while maintaining signature convenience.
...
TL;DR: Proposes CoDR framework for efficient and effective demonstration selection in in-context learning using coreset-based dual retrieval with cluster-pruning.


<details>
  <summary>Details</summary>
Motivation: Traditional demonstration selection methods for in-context learning are inefficient or suboptimal, struggling to balance efficiency and effectiveness in this NP-hard problem.

Method: Coreset-based Dual Retrieval (CoDR) with cluster-pruning to construct diverse coresets and dual retrieval mechanism for global selection while maintaining efficiency.

Result: Experimental results show significant improvement in ICL performance compared to existing strategies like random, similarity-based, and infoscore-based sampling.

Conclusion: CoDR provides a robust solution for effective and efficient demonstration selection in in-context learning for Large Visual Language Models.
...
TL;DR: WDT-MD is a novel Wavelet Diffusion Transformer framework that addresses limitations in current diffusion-based microaneurysm detection methods by preventing identity mapping, distinguishing MAs from other anomalies, and improving normal feature reconstruction.
...
Conclusion: WDT-MD represents a significant advancement for improving early diabetic retinopathy screening by addressing fundamental limitations of existing diffusion-based approaches.
...
TL;DR: A variational segmentation model combining denoising (I-divergence + adaptive TV) with bias field estimation to handle Gamma/Poisson noise and intensity inhomogeneity, optimized via ICTM-RMSAV.


<details>
  <summary>Details</summary>
Motivation: Existing segmentation methods degrade under heavy noise and intensity inhomogeneity, particularly with Gamma-distributed multiplicative noise and Poisson noise.

Method: Integrates denoising (I-divergence + adaptive TV regularizer) with bias field estimation within ICTM framework, using spatially adaptive weights and RMSAV optimization.

Result: Superior accuracy and robustness on synthetic/real images with intensity inhomogeneity and diverse noise types compared to competing methods.

Conclusion: The proposed model effectively handles challenging segmentation scenarios with noise and intensity inhomogeneity through integrated denoising and bias correction.
...
TL;DR: T-Rex-Omni introduces negative visual prompts to improve open-set object detection by suppressing hard negative distractors, using a unified encoder, training-free NNC module, and NNH loss for better discrimination.


<details>
  <summary>Details</summary>
Motivation: Current open-set object detectors rely only on positive indicators and are vulnerable to visually similar but semantically different distractors, limiting their effectiveness.

Method: Proposes T-Rex-Omni with unified visual prompt encoder for positive/negative prompts, training-free Negating Negative Computing (NNC) module, and Negating Negative Hinge (NNH) loss for fine-tuning.

Result: Achieves remarkable zero-shot detection performance, narrows gap between visual-prompted and text-prompted methods, and excels in long-tailed scenarios (51.2 AP_r on LVIS-minival).

Conclusion: Negative prompts are established as a crucial new dimension for advancing open-set visual recognition systems, with flexible deployment options for both positive-only and joint positive-negative inference.
...
TL;DR: Owl is a causally-grounded framework that mitigates object hallucination in LVLMs by modeling the hallucination process via structural causal graphs and using VTACR to quantify modality imbalance, with attention intervention and dual-path contrastive decoding.


<details>
  <summary>Details</summary>
Motivation: Existing approaches independently regulate visual or textual attention, overlooking their interaction as key causal factors in object hallucination.

Method: Proposes VTACR metric to quantify modality imbalance, uses structural causal graphs to model hallucination, implements fine-grained attention intervention, and employs dual-path contrastive decoding.

Result: Achieves significant hallucination reduction on POPE and CHAIR benchmarks, setting new SOTA in faithfulness while preserving vision-language understanding.

Conclusion: Owl effectively mitigates object hallucination by addressing the core issue of modality imbalance through causally-grounded attention reweighting and contrastive decoding strategies.
...
TL;DR: Proposes a dense cross-scale image alignment model that improves accuracy and efficiency by leveraging cross-scale feature correlations and incorporating a fully spatial correlation module with just noticeable difference weighting.
...
Method: Uses dense cross-scale alignment considering feature correlations across scales, includes a fully spatial correlation module, and incorporates just noticeable difference to focus on distortion-sensitive regions.
...
Conclusion: The proposed model successfully addresses accuracy and efficiency challenges in unsupervised image alignment through cross-scale feature correlation and distortion-aware optimization.
...
<details>
  <summary>Details</summary>
Motivation: Existing cloud image extrapolation methods have three key limitations: lack of adaptive feature extraction mechanisms, insufficient temporal guidance for long-range dependencies, and inefficient quadratic computational costs from attention mechanisms.

Method: USF-Net integrates adaptive large-kernel convolutions with low-complexity attention in an encoder-decoder framework. It features USTM with SiB (multi-scale contextual capture) and TiB (temporal dependency modeling), DSM with TGM for unified spatiotemporal modeling, and DUM to prevent ghosting effects.

Result: Extensive experiments on the ASI-CIS dataset show USF-Net significantly outperforms state-of-the-art methods, achieving superior balance between prediction accuracy and computational efficiency for cloud extrapolation.

Conclusion: USF-Net effectively addresses key challenges in cloud image sequence extrapolation through its unified spatiotemporal fusion approach and introduces a new benchmark dataset (ASI-CIS) for the field.
...
TL;DR: 4KDehazeFlow is a novel UHD image dehazing method using Flow Matching and Haze-Aware vector field that provides efficient adaptive nonlinear color transformation, achieving superior performance over state-of-the-art methods.
...
Method: Models dehazing as progressive optimization of continuous vector field flow using Flow Matching, learnable 3D LUT for compact haze transformation encoding, and fourth-order Runge-Kutta ODE solver for stable artifact suppression.

Result: Exceeds seven state-of-the-art methods with 2dB PSNR increase and better performance in dense haze and color fidelity.
...
TL;DR: PAN is a general world model that predicts future states through high-quality video simulation using language-conditioned actions and long-horizon consistency.


<details>
  <summary>Details</summary>
Motivation: Current video generation models lack causal control and interactivity needed for reasoning, while existing world models are limited to restricted domains with poor generalization.

Method: Uses Generative Latent Prediction (GLP) architecture combining autoregressive latent dynamics based on LLM with video diffusion decoder, trained on large-scale video-action pairs.

Result: Achieves strong performance in action-conditioned world simulation, long-horizon forecasting, and simulative reasoning compared to other models.

Conclusion: PAN represents progress toward general world models that enable predictive simulation for reasoning and acting across diverse environments.
...
Method: Integrates cultural object detection backbone with structured program generation, uses curated Vietnamese cultural knowledge base, and dual-modality explanation module combining visual attention with textual rationales.

Result: Developed a Vietnamese Cultural VQA dataset and demonstrated practicality of programming-based methodologies for cultural AI with transparent explanations.

Conclusion: The system provides interpretable cultural understanding supporting education and cultural preservation with emphasis on cultural sensitivity.
...
<details>
  <summary>Details</summary>
Motivation: Existing counterattack methods like TTC are limited by narrow search spaces that overfit to specific adversarial patterns, lacking the diversity needed to neutralize a broad range of perturbations effectively.

Method: Proposes Directional Orthogonal Counterattack (DOC) that incorporates orthogonal gradient directions and momentum-based updates to expand counterattack space exploration and increase perturbation diversity, along with a directional sensitivity score for adaptive counterattack strength modulation.
...
TL;DR: The paper introduces Composition-Incremental Learning for Compositional Generalization (CompIL) to address the challenge of continually learning new compositions in compositional zero-shot learning tasks.


<details>
  <summary>Details</summary>
Motivation: Real-world data continually emerges with nearly infinite, long-tailed compositions that are not fully visible, requiring models to gradually improve compositional generalization capabilities incrementally.

Method: Proposes a pseudo-replay framework with a visual synthesizer to generate representations of learned compositions and a linguistic primitive distillation mechanism to maintain aligned primitive representations across learning stages.

Result: Developed MIT-States-CompIL and C-GQA-CompIL benchmarks and demonstrated the effectiveness of the proposed framework through extensive experiments.

Conclusion: The proposed CompIL framework successfully enables models to progressively improve compositional generalization capabilities in an incremental learning setting.
...
TL;DR: UL-TTA is a training-free test-time adaptation method for vision-language models that adapts only logit-level parameters (prototypes, priors, temperature) using online EM-style updates, achieving improved accuracy and calibration under domain shift without backbone updates.


<details>
  <summary>Details</summary>
Motivation: Vision-language models suffer from feature drift, class-prior mismatch, and miscalibration under domain shift, while existing test-time adaptation methods require computationally expensive operations like backpropagation through large backbones or heavy memory usage.
...
Result: Consistently improves top-1 accuracy (+4.7 points over zero-shot CLIP) while reducing ECE by 20-30% across large-scale benchmarks, with <8% latency overhead and no collapse in long streams up to 200K samples.

Conclusion: Logit-level Bayesian adaptation is sufficient for state-of-the-art accuracy-calibration trade-offs in VLMs under domain shift without updating backbone parameters.
...
TL;DR: The paper introduces DKDS, a new dataset for degraded Kuzushiji documents with seals, addressing OCR challenges in noisy pre-modern Japanese cursive script documents.


<details>
  <summary>Details</summary>
Motivation: Existing OCR methods perform well on clean Kuzushiji documents but fail on degraded documents with noise like document degradation and seals, with no existing dataset specifically addressing these challenges.
...
Conclusion: The DKDS dataset serves as a new benchmark for handling noisy Kuzushiji documents, with publicly available dataset and implementation code to advance research in this area.
...
Method: Built on an image-to-image generative framework, PIFF maps Digital Elevation Models to flood depth predictions using a simplified inundation model (SPM) for hydrodynamic priors and a transformer-based rainfall encoder for temporal dependencies.

Result: Tested on a 26 km study area in Tainan, Taiwan with 182 rainfall scenarios (24-720 mm over 24 hours), PIFF demonstrated effectiveness as a data-driven alternative for flood prediction.

Conclusion: PIFF provides an accurate, real-time flood mapping solution that captures causal relationships between rainfall, topography, and flooding, offering a practical replacement for costly traditional simulations.
...
TL;DR: MACEval is a multi-agent continual evaluation network that provides dynamic, automated evaluation of large language models through role assignment, in-process data generation, and evaluation routing, addressing issues of data contamination and scalability in traditional benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional benchmarks for large models are closed-ended, prone to data contamination, difficult to maintain, and heavily dependent on human curation, which undermines evaluation credibility and cannot keep pace with rapidly advancing model capabilities.

Method: MACEval employs a multi-agent network with role assignment, in-process data generation, and evaluation routing through cascaded agents. It uses an interactive and autonomous evaluation mode with customized evaluation topologies.

Result: Experiments on 9 open-ended tasks with 23 large models show MACEval is human-free, automatic, efficient, economical (reducing data and overhead), and flexible/scalable for integrating existing benchmarks.

Conclusion: MACEval provides a sustainable, longitudinal evaluation framework that can broaden future directions for large model evaluation by addressing key limitations of traditional benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional vision-based human mesh recovery faces limitations due to occlusions, lighting issues, and privacy concerns. Pressure-based sensing offers an occlusion-free, privacy-friendly alternative, but existing methods struggle with distinguishing intermingled pressure signals from multiple people walking simultaneously.

Method: A top-down pipeline using tracking-by-detection strategy to first identify and segment each individual's pressure signal from raw pressure data, then perform human mesh recovery for each extracted individual signal. Also created a multi-person interaction pressure dataset (MIP).

Result: Achieved 89.2mm MPJPE and 112.6mm WA-MPJPE100, demonstrating excellent performance in multi-person HMR using pressure data.
...
TL;DR: HOTFLoc++ is an end-to-end framework for LiDAR place recognition and 6-DoF localization in forests using octree-based transformers and multi-scale geometric verification.


<details>
  <summary>Details</summary>
Motivation: To address challenges in LiDAR place recognition in forest environments, including clutter, self-similarity, and viewpoint changes, particularly for ground-to-ground and ground-to-aerial scenarios.

Method: Uses octree-based transformer for hierarchical local descriptors, learnable multi-scale geometric verification module, and coarse-to-fine registration approach.

Result: Achieves 90.7% Recall@1 on CS-Wild-Places (29.6% improvement over baselines), 91.7% on Wild-Places, 96.0% on MulRan. Reduces localization errors by ~2× with multi-scale re-ranking, with 97.2% of registrations having <2m and <5° error.

Conclusion: The method significantly outperforms state-of-the-art approaches in challenging forest environments while being two orders of magnitude faster than RANSAC for dense point clouds.
...
Method: Proposes DBINDS that uses diffusion model inversion to recover initial noise sequences, forms Initial Noise Difference Sequence (INDS), extracts multi-domain multi-scale features, and uses feature optimization with LightGBM classifier tuned by Bayesian search.
...
Conclusion: Analyzing latent-space dynamics through diffusion inversion provides a more effective approach for detecting AI-generated videos than pixel-level methods, enabling better generalization across different generators.
...
TL;DR: A framework combining DermBench benchmark and DermEval evaluator for reliable assessment of multimodal LLMs in dermatology diagnosis from images, achieving close alignment with expert ratings.


<details>
  <summary>Details</summary>
Motivation: Reliable evaluation remains the primary bottleneck for responsible clinical deployment of multimodal LLMs in dermatology diagnosis.

Method: Created DermBench with 4,000 real-world dermatology images paired with expert-certified diagnostic narratives, and trained DermEval as a reference-free multimodal evaluator that produces structured critiques with scores.

Result: Achieved close alignment with expert ratings - mean deviations of 0.251 (DermBench) and 0.117 (DermEval) out of 5 on 4,500 cases, providing reliable measurement across different multimodal LLMs.

Conclusion: The framework enables clinically meaningful, reproducible, and scalable assessment of multimodal LLMs for dermatology diagnosis, critical for identifying model limitations and biases.
...
TL;DR: TACO is a framework that reduces hallucinations in Multimodal Large Language Models through self-verification and confidence calibration without external vision experts.


<details>
  <summary>Details</summary>
Motivation: MLLMs often suffer from hallucinations (errors in object existence, attributes, or relations) which undermine their reliability.

Method: Decomposes responses into atomic queries, paraphrases them to reduce wording sensitivity, estimates confidence using self-consistency or self-confidence aggregation, and refines answers with a language model.

Result: Outperforms direct prompting and Visual Contrastive Decoding on five benchmarks, reduces systematic biases, and improves confidence calibration with two MLLMs.

Conclusion: TACO effectively enhances the faithfulness of MLLMs by mitigating hallucinations through self-verification and confidence calibration.
...
TL;DR: Spatial Information Bottleneck (S-IB) framework improves neural network interpretability by spatially disentangling information flow, optimizing gradient-based attributions during training to enhance explanation quality across multiple methods.
...
Method: Proposes Spatial Information Bottleneck (S-IB) that maximizes mutual information between foreground Vector-Jacobian Products (VJP) and inputs while minimizing mutual information in background regions, based on information-theoretic analysis showing VJP forms minimal sufficient statistics.
...
Conclusion: Directly optimizing VJP's spatial structure during training improves visualization quality across diverse explanation paradigms, providing a principled approach to enhance neural network interpretability.
...
TL;DR: Proposes FPGA-based deployment of Animation-based Generative Codec for edge devices, achieving 24.9× and 4.1× higher energy efficiency compared to CPU and GPU respectively.


<details>
  <summary>Details</summary>
Motivation: AGC deployment on resource-constrained edge devices faces challenges due to high parameter count, algorithm inflexibility, and power consumption from extensive computations and data transmission.

Method: Network compression via post-training static quantization and layer fusion, overlapped accelerator using co-processor paradigm with hardware engines (convolution, grid sampling, upsample), and parallelization optimization with double-buffered pipelines and loop unrolling.
...
Conclusion: FPGA-oriented deployment scheme enables efficient AGC implementation on edge devices with significantly improved energy efficiency compared to traditional computing platforms.
...
TL;DR: Systematic evaluation of deep learning vs classical methods for energy expenditure estimation from physiological signals, showing transformers with minute ventilation achieve best performance (0.87 W/kg RMSE), with significant inter-subject variability requiring adaptive modeling.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches rarely disentangle neural architecture effects from signal choice effects in energy expenditure estimation, and there's a need for systematic comparison across different signal combinations and activity types.

Method: Compared classical baselines with neural architectures (CNN, ResNet with attention, transformers) across single signals, signal pairs, and grouped sensor inputs from Hexoskin smart shirt for diverse physical activities.

Result: Minute ventilation was most predictive individual signal (transformer achieved 0.87 W/kg RMSE). Better results in low-intensity activities (RMSE down to 0.29 W/kg; NRMSE = 0.04), while higher-intensity tasks had larger RMSE but comparable normalized errors. Strong inter-individual variability observed.

Conclusion: Transformers with minute ventilation provide best overall performance, but subject-level analysis shows need for adaptive modeling strategies due to strong inter-individual variability.
...
TL;DR: RichKD enhances multi-teacher knowledge distillation by incorporating CLIP's vision-language knowledge alongside conventional visual teachers, improving accuracy, prediction confidence, and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing multi-teacher KD methods lack knowledge diversity as they rely only on unimodal visual information, overlooking the potential of cross-modal representations from vision-language models like CLIP.

Method: Proposes a framework that fuses logits and features from conventional visual teachers with those from CLIP, incorporating CLIP's multi-prompt textual guidance to capture both dataset-specific and semantically enriched visual cues.

Result: Consistently outperforms most existing baselines across multiple benchmarks, yields more confident and reliable predictions, increases confident-correct cases while reducing confidently wrong ones, and shows stronger robustness under distribution shifts and input corruptions.

Conclusion: Incorporating CLIP's vision-language knowledge as complementary supervision significantly enhances knowledge distillation quality, improving both accuracy and semantic understanding of the student model.
...
Method: Optimizes continuous density fields from Trellis-generated voxel grids using differentiable, physically constrained loss terms including mass regularization and restricted optimization domain, without requiring architectural changes to pretrained models.

Result: Achieves up to 43% material mass reduction in text-to-3D tasks, improves stability compared to baselines, and real-world 3D-printing confirms reliable fabrication of self-supporting hollow designs.

Conclusion: DensiCrafter successfully bridges the gap between 3D generative models and practical manufacturability by producing lightweight, self-supporting structures that maintain high geometric fidelity while being suitable for real-world fabrication.
...
TL;DR: Introduces a feedback mechanism in teacher-student semi-supervised learning for medical image segmentation to counter error propagation by enabling student feedback on teacher's pseudo-labels.


<details>
  <summary>Details</summary>
Motivation: Addresses self-reinforcing bias in teacher-student paradigm for medical image segmentation, where inherent image ambiguities make the framework vulnerable to erroneous supervision that gets iteratively reconfirmed.

Method: Proposes a feedback mechanism with two key components: feedback attributor (identifies pseudo-labels triggering student updates) and feedback receiver (determines where to apply feedback). Also introduces dual-teacher feedback model for more dynamic feedback loops and cross-teacher supervision.
...
Conclusion: The feedback mechanism successfully counters error reconfirmations in teacher-student framework for medical image segmentation, with dual-teacher model providing additional gains through dynamic feedback and cross-teacher supervision.
...
Method: Proposes FQ-PETR with three key components: (1) QFPE using LiDAR-prior-guided single-point sampling to align feature scales, (2) DULUT with cascaded linear LUTs for non-linear function approximation, (3) QANS performing quantization after softmax numerical stabilization.

Result: Achieves near-floating-point accuracy (only 1% degradation) under W8A8 quantization while reducing latency by up to 75%, significantly outperforming existing PTQ and QAT baselines across PETR variants.

Conclusion: FQ-PETR successfully enables efficient deployment of PETR-based 3D detection models through targeted quantization innovations, maintaining high accuracy while dramatically reducing computational requirements.
...
TL;DR: Proposes TDCNet, a novel moving infrared small target detection network that combines temporal difference and 3D convolution through re-parameterized TDC blocks to capture multi-scale motion contextual features while suppressing background clutter.


<details>
  <summary>Details</summary>
Motivation: Moving infrared small target detection is challenging due to weak target features and complex background interference. Existing methods using temporal differences or 3D convolutions have limitations: temporal differences lack spatial feature extraction capability, while 3D convolutions lack explicit motion awareness.

Method: Introduces temporal difference convolution (TDC) re-parameterization module with three parallel TDC blocks that fuse temporal difference and 3D convolution into unified spatio-temporal representations. Also proposes TDC-guided spatio-temporal attention mechanism for cross-attention between TDC-based and 3D backbone features.
...
TL;DR: Flora is a zero-shot skeleton action recognition method that addresses fragile alignment and rigid classifiers through flexible neighbor-aware semantic attunement and distribution-aware flow classification.


<details>
  <summary>Details</summary>
Motivation: Existing methods face two fundamental issues: fragile point-to-point alignment from imperfect semantics, and rigid classifiers with static decision boundaries and coarse-grained anchors.

Method: Uses flexible neighbor-aware semantic attunement with cross-modal geometric consistency for robust point-to-region alignment, and employs noise-free flow matching with condition-free contrastive regularization for distribution-aware classification.
...
Conclusion: Flora effectively addresses the limitations of existing zero-shot skeleton action recognition methods through its novel alignment and classification approach, demonstrating superior performance across multiple datasets.
...
TL;DR: OUGS introduces an object-aware uncertainty formulation for 3D Gaussian Splatting that uses physical Gaussian parameters and semantic segmentation to improve reconstruction efficiency and quality for specific objects in complex scenes.
...
Method: Derives uncertainty directly from explicit physical parameters of 3D Gaussian primitives (position, scale, rotation), propagates covariance through rendering Jacobian, and integrates semantic segmentation masks to create object-aware uncertainty scores.

Result: Significantly improves 3DGS reconstruction efficiency and achieves higher quality for targeted objects compared to state-of-the-art methods, while also serving as a robust global scene uncertainty estimator.
...
<details>
  <summary>Details</summary>
Motivation: Accurate bronchoscope localization is challenging due to respiratory motion, anatomical variability, and CT-to-body divergence, causing misalignment between intra-operative views and pre-operative CT. Existing methods fail to generalize across domains and patients.

Method: Proposes a vision-based pose optimization framework with a fine-tuned modality- and domain-invariant encoder for similarity computation between real endoscopic RGB frames and CT-rendered depth maps. Uses differentiable rendering for iterative camera pose refinement through depth consistency.

Result: Achieves average translational error of 2.65 mm and rotational error of 0.19 rad when trained on synthetic data. Shows strong cross-domain generalization on real patient data without domain-specific adaptation.

Conclusion: The framework enables robust, domain-invariant localization through iterative vision-based optimization, while the new benchmark provides standardized evaluation for vision-based bronchoscopy navigation.
...
Method: Constructed dedicated detection/tracking dataset for American football players, evaluated various methods, fine-tuned detection models, and integrated with re-identification models in tracking systems.

Result: Fine-tuned detection models outperformed pre-trained models. Integration with re-identification models significantly improved tracking accuracy in crowded scenarios compared to existing approaches.
...
TL;DR: Dual-Teacher Knowledge Distillation framework that combines ViT and CNN teachers to improve lightweight CNN performance in video action recognition, addressing architectural mismatch in cross-architecture distillation.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers (ViTs) achieve strong video action recognition but are computationally expensive, while lightweight CNNs are efficient but less accurate. Existing cross-architecture knowledge distillation methods struggle with architectural mismatch and overlook the value of stronger homogeneous CNN teachers.
...
Result: Extensive experiments on HMDB51, EPIC-KITCHENS-100, and Kinetics-400 show consistent outperformance over state-of-the-art distillation approaches, achieving up to 5.95% accuracy gain on HMDB51.

Conclusion: The dual-teacher approach effectively leverages both heterogeneous ViT and homogeneous CNN teachers to guide lightweight CNN students, overcoming architectural mismatch and improving video action recognition performance while maintaining efficiency.
...
TL;DR: DreamPose3D is a diffusion-based framework for 3D human pose estimation that combines action-aware reasoning with temporal imagination, achieving state-of-the-art performance by addressing temporal coherence and joint relationship modeling.


<details>
  <summary>Details</summary>
Motivation: Existing 3D pose estimation methods rely solely on geometric cues and predict poses independently, limiting their ability to resolve ambiguous motions and generalize to real-world scenarios. The paper is inspired by how humans understand and anticipate motion through action awareness and temporal reasoning.

Method: DreamPose3D uses a diffusion-based framework that dynamically conditions denoising with action prompts extracted from 2D pose sequences. It incorporates kinematic joint affinity into attention mechanisms and uses a hallucinative pose decoder to predict temporally coherent 3D pose sequences.

Result: Extensive experiments on Human3.6M and MPI-3DHP datasets show state-of-the-art performance across all metrics. The method also demonstrated strong robustness on a broadcast baseball dataset, handling ambiguous and noisy 2D inputs while maintaining temporal consistency.

Conclusion: DreamPose3D effectively addresses the limitations of existing methods by combining action-aware reasoning with temporal imagination, providing a robust solution for 3D human pose estimation that can handle real-world scenarios with ambiguous inputs.
...
<details>
  <summary>Details</summary>
Motivation: Current prompt learning methods face challenges with semantic misalignment between LLMs and CLIP models due to divergent training, lack scalability across evolving foundation models, and conventional Euclidean-space optimization amplifies modality gaps in biomedical imaging.

Method: Proposes vMFCoOp framework that inversely estimates von Mises-Fisher distributions on a shared hyperspherical manifold, aligning semantic biases via Unified Semantic Anchors with three complementary constraints.
...
Conclusion: The framework achieves robust biomedical prompting and superior few-shot classification, with plans for continuous expansion to more downstream applications and resource sharing through GitHub.
...
TL;DR: RF-DETR introduces a lightweight specialist detection transformer that uses neural architecture search to find optimal accuracy-latency tradeoffs for target datasets, achieving state-of-the-art performance on COCO and Roboflow100-VL.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary detectors often fail to generalize to real-world datasets with out-of-distribution classes, and fine-tuning heavy VLMs for new domains is inefficient.

Method: Fine-tunes a pre-trained base network on target datasets and evaluates thousands of network configurations with different accuracy-latency tradeoffs using weight-sharing NAS without retraining.

Result: RF-DETR (nano) achieves 48.0 AP on COCO (5.3 AP better than D-FINE at similar latency), and RF-DETR (2x-large) outperforms GroundingDINO by 1.2 AP on Roboflow100-VL while running 20x faster. RF-DETR (2x-large) is the first real-time detector to surpass 60 AP on COCO.

Conclusion: RF-DETR provides an efficient approach for discovering optimal accuracy-latency Pareto curves for any target dataset, significantly improving real-time detection performance across diverse domains.
