...
Motivation: To enhance multilingual capabilities of Vision Language Models (VLMs) by integrating OCR technology, particularly focusing on Korean and English languages where existing VQA benchmarks are limited.

Method: Trained KLOCR, a bilingual OCR baseline on 100M instances, curated KOCRBench for Korean VQA evaluation, and analyzed various prompting methods for OCR-augmented generation.

Result: OCR-extracted text significantly boosts performance across both open source and commercial VLMs in bilingual visual question answering tasks.

Conclusion: OCR-augmented generation provides substantial benefits for multilingual VQA, with the released KLOCR model, code, and KOCRBench dataset supporting further research in this domain.
...
<details>
  <summary>Details</summary>
Motivation: To address the high cost of gathering human feedback for large video-language models and the expense of training specialized reward models, making fine-tuning more cost-effective and flexible.

Method: Proposes Oracle-RLAIF framework with a general Oracle ranker that ranks candidate responses instead of scoring them, and introduces $GRPO_{rank}$ rank-based loss function for ordinal feedback optimization with rank-aware advantages.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use kinematics-based pose estimation followed by physics post-processing, which causes error accumulation and limits reconstruction quality. There's a need for unified approach that combines physical constraints with visual alignment.

Method: Uses pixel-as-ray strategy to lift 2D keypoints into 3D spatial rays for global pose guidance. Combines local visual features with reinforcement learning, and employs distillation scheme to transfer motion knowledge from mocap-trained expert to vision-conditioned policy.
...
<details>
  <summary>Details</summary>
Motivation: To understand when combining human and machine face identification decisions improves accuracy, given individual differences between people and machines affect collaborative outcomes.

Method: Used data from expert and non-expert face identifiers to examine human-human and human-machine collaborations. Applied Proximal Accuracy Rule to predict collaborative benefits and established critical fusion zones. Implemented intelligent human-machine fusion by selecting people who could improve high-performing machine accuracy.

Result: Found surprisingly large critical fusion zone where humans less accurate than machines can still improve system accuracy through fusion. Intelligent fusion was more accurate than machine alone or combining all human-machine judgments. Human-only partnerships achieved similar average performance but intelligent human-machine collaboration better minimized impact of low-performing humans.

Conclusion: Both humans and machines have meaningful roles in assuring accurate face identification. Provides evidence-based roadmap for intelligent AI use in face identification through optimized human-machine collaboration.
...
<details>
  <summary>Details</summary>
Motivation: Video generation models hallucinate like LLMs but lack uncertainty quantification methods, creating safety concerns for real-world applications.

Method: Proposes S-QUBED framework with: (i) calibration metric using robust rank correlation, (ii) black-box UQ method decomposing uncertainty into aleatoric and epistemic components via latent modeling, (iii) UQ benchmark dataset.
...
Conclusion: First successful uncertainty quantification framework for video models that addresses critical safety gaps by providing reliable uncertainty estimates and disentangling uncertainty sources.
...
Method: Prompt Embedding Optimization (PEO) uses a tripartite objective function that optimizes text embeddings for aesthetic fidelity, embedding adherence, and prompt preservation with minimal divergence from original prompt.

Result: Quantitative and qualitative evaluations show PEO exceeds or equals state-of-the-art text-to-image and prompt adaptation methods.

Conclusion: PEO effectively improves aesthetic quality in diffusion models through text embedding optimization while maintaining prompt fidelity, offering a training-free and backbone-independent solution.
...
<details>
  <summary>Details</summary>
Motivation: Existing hand tracking datasets are predominantly captured in controlled lab settings, limiting environmental diversity and model generalization for real-world applications.

Method: Developed a lightweight back-mounted capture rig with eight exocentric cameras and a Meta Quest 3 headset providing two egocentric views, combined with an ego-exo tracking pipeline for generating accurate 3D hand pose ground truth.
...
Conclusion: The proposed system enables nearly unconstrained mobility in genuinely in-the-wild conditions while maintaining precise 3D hand tracking capabilities, addressing limitations of existing controlled lab setups.
...
TL;DR: This paper presents a distillation method to convert slow diffusion-based co-speech video generation models into real-time capable models by using input human pose conditioning for attention and loss functions.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based co-speech video generation methods are too slow for real-time applications due to numerous denoising steps and costly attention mechanisms.

Method: Distill many-step diffusion video model into few-step student model using input human pose conditioning for attention and loss functions, including input-aware sparse attention and input-aware distillation loss.
...
Conclusion: The proposed input-aware distillation method successfully enables real-time co-speech video generation while maintaining and even improving visual quality through pose-guided attention and loss functions.
...
<details>
  <summary>Details</summary>
Motivation: To address catastrophic forgetting in continual adaptation of deep generative models, overcoming limitations of retraining on synthetic data which leads to intractable training time and performance degradation.

Method: Designs a novel conditioning mechanism using low rank adaptation (LoRA) with exclusively rank 1 matrices, whose matrix rank is functionally increased using carefully selected functions (FunLoRA).
...
Conclusion: FunLoRA provides an effective parameter-efficient fine-tuning method for continual learning that avoids catastrophic forgetting and enables efficient adaptation with minimal computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: Traffic light and sign recognition is critical for AV safety, but current models are vulnerable to both digital adversarial attacks and natural perturbations (glare, rain, dirt, graffiti), lacking consideration of temporal continuity and multi-static field-of-view sensing.

Method: Built a multi-source dataset from aiMotive, Udacity, Waymo, and self-recorded Texas videos. Used mid and long-term RGB sequences temporally aligned for four operational domains. Implemented a unified three-layer defense stack with feature squeezing, defensive distillation, entropy-based anomaly detection, and sequence-wise temporal voting.

Result: Unified Defense Stack achieved 79.8mAP, reduced attack success rate to 18.2% (superior to YOLOv8, YOLOv9, BEVFormer), and reduced high-risk misclassification to 32%. Physical transferability confirmed via recapture probes.

Conclusion: The proposed framework effectively enhances robustness against both digital and natural perturbations in traffic light and sign recognition, improving safety for autonomous vehicles through temporal sequence preservation and multi-layer defense mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Flow-matching models are deterministic and poorly suited for reinforcement learning, which is crucial for improving image quality and human alignment. Existing methods that add random noise perturbations are inefficient and unstable.
...
Conclusion: Smart-GRPO provides a practical path for reinforcement learning in flow-matching frameworks, bridging the gap between efficient training and human-aligned generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing Gaussian Splatting methods require dense, calibrated views and perform poorly with free sparse images due to limited overlap and overfitting.
...
Conclusion: FSFSplatter effectively addresses the challenges of sparse image reconstruction through integrated dense initialization and geometry-enhanced optimization.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-driven motion generation methods lack causal logic of action execution and human intention understanding, and are limited by the absence of visual grounding for fine-grained spatiotemporal details.

Method: Jointly optimizes multimodal-conditioned motion generation and intention prediction, uses mixture-of-attention mechanism with adaptive scope for local alignment between conditional tokens and motion subsequences, and leverages visual priors.

Result: Reduces FID by 38.6% on HumanML3D and 34.6% on Mo440H benchmark, surpasses LLM-based methods in motion captioning, and enables intention prediction and vision-conditioned generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing sparse control methods for dynamic 3D reconstruction suffer from static redundancy and dynamic insufficiency because they allocate control points purely based on geometry rather than motion complexity.

Method: Uses vision foundation models to establish patch-token-node correspondences, applies motion-adaptive compression to concentrate points in dynamic regions, and introduces spline-based trajectory parameterization initialized by 2D tracklets instead of MLP deformation fields.

Result: Achieves significant improvements in reconstruction quality and efficiency over state-of-the-art methods through better motion representation and optimization stability.

Conclusion: The motion-adaptive framework successfully addresses the fundamental mismatch between control point allocation and motion complexity, enabling more efficient and higher-quality dynamic 3D reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Traditional denoising methods rely on handcrafted priors and struggle with real noise complexity, while deep learning methods need extensive labeled data and may not generalize well across different noise types and imaging conditions.

Method: Hybrid framework combining untrained DIP network (adapts to unique noise characteristics without labeled data) and pre-trained DRUNet network (leverages learned representations from large datasets) using regularization by denoising (RED).

Result: Extensive experiments on benchmark datasets demonstrate superior performance for real-world noise removal, with enhanced generalization across varying noise patterns and improved performance in limited training data scenarios.

Conclusion: Net2Net effectively addresses real-world noise removal challenges by combining the adaptability of untrained networks with the robustness of pre-trained networks, achieving better generalization and performance without requiring extensive labeled data.
...
<details>
  <summary>Details</summary>
Motivation: To enhance LLMs' reasoning capabilities for retrieval tasks by adapting DeepSeek-R1's RL approach, but finding direct application infeasible due to high computational costs and training instability.

Method: Introduces information compression with details inspection mechanism to reduce token consumption, and a new training paradigm with activation stage using retrieval-tailored synthetic CoT dataset followed by RL with curriculum reward.
...
Conclusion: The proposed approach successfully enables effective RL-based training for retrieval tasks while maintaining computational efficiency and achieving superior performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing test-time adaptation methods either rely on computationally expensive backpropagation or focus only on likelihood adaptation while ignoring the importance of adaptive priors, limiting real-time deployment and performance under distribution shifts.

Method: BCA+ introduces a dynamic cache that stores and updates class embeddings, spatial scales (for detection), and adaptive class priors from historical predictions. It formulates adaptation as Bayesian inference, fusing initial VLM outputs with cache-based predictions that combine dynamically updated likelihood (feature/scale similarity) and prior (evolving class distribution).
...
Conclusion: BCA+ provides a unified, training-free framework that successfully corrects both semantic understanding and contextual confidence in vision-language models through dual-adaptation mechanism and uncertainty-guided fusion.
...
<details>
  <summary>Details</summary>
Motivation: Existing brain tumor classification methods are limited to predefined classes and cannot identify novel tumor types. Unsupervised learning lacks labeled data integration, while semi-supervised methods assume all classes are represented in training data.

Method: HGCD-BT integrates hierarchical clustering with contrastive learning, extending GCD with a novel semi-supervised hierarchical clustering loss to capture the hierarchical structure of brain tumor taxonomies.

Result: +28% improvement in accuracy over state-of-the-art GCD methods for patch-level classification on OpenSRH dataset, particularly in identifying unseen tumor categories. Also demonstrated generalizability on slide-level classification using Digital Brain Tumor Atlas.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for long-form video understanding either rely on uniform sampling (missing critical moments) or rigid temporal spacing (missing fine-grained cues), while diversity-focused methods neglect query relevance.

Method: Proposes AdaRD-Key that maximizes Relevance-Diversity Max-Volume objective combining query-conditioned relevance scores with log-determinant diversity, plus lightweight relevance-aware gating for weak alignment cases.
...
Conclusion: AdaRD-Key provides an effective training-free solution for query-driven long-form video understanding that balances relevance and diversity while handling weak query-video alignment through adaptive gating.
...
<details>
  <summary>Details</summary>
Motivation: To understand how VLMs approach complex lateral thinking challenges like rebus puzzles, moving beyond performance metrics to explore underlying reasoning processes and failure patterns.
...
Result: Reasoning quality varies dramatically across puzzle categories - models show strengths in visual composition but fundamental limitations in absence interpretation and cultural symbolism. Prompting strategy significantly influences both cognitive approach and problem-solving effectiveness.

Conclusion: Explainability is an integral component of model performance rather than a post-hoc consideration, revealing systematic patterns in VLM cognitive processes for complex lateral thinking tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current text removal datasets have issues with ground truth artifacts, simplistic backgrounds, and poor evaluation metrics, limiting out-of-domain generalization and accurate assessment.

Method: Developed a synthetic text removal benchmark using object-aware text placement and vision-language model-generated content on complex backgrounds.

Result: Created a dataset with clean ground truth and challenging text removal scenarios, available at https://huggingface.co/datasets/cyberagent/OTR.

Conclusion: The proposed benchmark addresses key limitations in existing text removal datasets and enables more accurate evaluation across different domains.
...
<details>
  <summary>Details</summary>
Motivation: Medical object detection struggles when training on mixed modalities due to heterogeneous statistics and disjoint representation spaces, requiring methods to align features from different sources.

Method: Proposes modality tokens (text-derived embeddings encoding imaging modality) integrated via Multimodality Context Attention (MoCA) and QueryREPA pretraining with contrastive objective to align object queries with modality context.

Result: Consistently improves AP across diverse medical modalities with minimal overhead, producing modality-aware queries that transfer effectively to downstream training.

Conclusion: Offers a practical approach for robust multimodality medical object detection that preserves DETR architectures while injecting modality cues into object queries.
...
Method: Proposes Masked Contrastive Decoding (MaskCD) which utilizes the "image heads" in LVLMs, masking them to construct contrastive samples for contrastive decoding.

Result: Evaluated on LLaVA-1.5-7b and Qwen-VL-7b using benchmarks like CHAIR, POPE, AMBER and MME. Results show MaskCD effectively alleviates hallucinations and retains general capabilities of LVLMs.

Conclusion: MaskCD provides an effective solution for reducing hallucinations in large vision-language models while maintaining their overall performance capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Pose estimation at small scales remains challenging, with few solutions capable of capturing 6 degrees of freedom with nanometric and microradian resolutions over large ranges.

Method: Uses phase-based local thresholding algorithm and phase processing of pseudo-periodic patterns, with different pattern designs for various microscopy applications.

Result: The software provides fast and reliable pose measurement with robustness to noise, defocus, and occlusion, achieving nanometer resolutions through phase processing.

Conclusion: Guidelines are provided for selecting appropriate pattern designs and microscope magnification lenses based on desired performance requirements.
...
Method: Treats multi-modal medical data as sequential frames with quality-driven selection using three modules: PreWeightNet for global contribution assessment, ThresholdNet for adaptive filtering, and EffiWeightNet for effective weight computation. Includes Causal Modality Identity Module (CMIM) for modality consistency.

Result: Extensive experiments show Med-K2N outperforms state-of-the-art methods by significant margins on multiple benchmarks.

Conclusion: The proposed framework effectively addresses K-to-N medical generation challenges through adaptive weighting mechanisms and causal modality identity constraints, demonstrating superior performance in cross-modal medical image synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Need for accurate, non-destructive egg quality assessment to ensure food safety, maintain standards, and improve efficiency in poultry production without internal measurements.

Method: Uses ensemble learning with multimodal features: deep features from pre-trained CNNs (ResNet152, DenseNet169, ResNet152V2) on external images combined with shape and weight data, followed by PCA, SMOTE augmentation, and ensemble voting.
...
Conclusion: First study to use machine learning for internal egg quality assessment using only external features, with publicly available dataset and code to promote reproducibility and further research.
...
<details>
  <summary>Details</summary>
Motivation: Current zero-shot captioners only use global image representations and can only generate whole-image captions, limiting their ability to describe specific regions or non-contiguous areas.
...
Result: Achieves state-of-the-art performance on zero-shot dense, region-set, and trace captioning tasks. Demonstrates better performance compared to other baselines and competitors.

Conclusion: Patch-wise semantic representations are effective for scalable caption generation, enabling flexible region-based captioning without requiring region-level supervision.
...
<details>
  <summary>Details</summary>
Motivation: Detecting unknown objects in semantic segmentation is crucial for safety-critical applications like autonomous driving, but the capability of foundation models to detect out-of-distribution regions remains underexplored.

Method: A training-free approach using features from InternImage backbone with K-Means clustering and confidence thresholding on raw decoder logits to identify OoD clusters.

Result: Achieved 50.02 Average Precision on RoadAnomaly and 48.77 on ADE-OoD with InternImage-L, surpassing several supervised and unsupervised baselines.
...
Method: HoloV rethinks token retention from a holistic perspective by adaptively distributing pruning budget across spatial crops to capture global visual context rather than isolated salient features.

Result: HoloV achieves superior performance across tasks, MLLM architectures, and pruning ratios. LLaVA1.5 with HoloV preserves 95.8% performance after pruning 88.9% of visual tokens.

Conclusion: HoloV provides an effective solution for efficient MLLM inference by minimizing representational collapse and maintaining task-relevant information through holistic token pruning strategy.
...
Method: CAW consists of two components: (1) Confidence-Aware loss that prioritizes uncertain adversarial examples by scaling KL divergence, and (2) feature alignment regularization that preserves semantic consistency between frozen and fine-tuned image encoder features.
...
Conclusion: CAW effectively enhances zero-shot robustness in vision-language models through its dual-component approach, achieving superior performance against adversarial attacks while maintaining efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Reliable risk assessment for carotid atheromatous disease requires integrating diverse clinical and imaging information in a transparent and interpretable way for clinicians.
...
Result: Zero-shot experiments showed LVLMs struggle with imaging modality/anatomy identification and risk classification. Domain adaptation with LoRA substantially improved stroke risk stratification. Multimodal data integration enhanced specificity and balanced accuracy, achieving competitive performance compared to CNN baselines.
...
Conclusion: The proposed FDA-VAE provides an efficient and interpretable solution for multi-phase CE MRI synthesis with better parameter efficiency and synthesis performance.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with long and detailed prompts, showing inconsistent generation. Existing evaluation metrics also fail to align well with human preferences for long-prompt-based image generation.

Method: Created LPG-Bench with 200 long prompts, generated 2,600 images from 13 models, and performed human annotations. Proposed TIT metric that compares consistency between original prompt and LMM-generated description of the image, with two instantiations: TIT-Score and TIT-Score-LLM.

Result: TIT framework achieves superior alignment with human judgment compared to existing metrics. TIT-Score-LLM achieves 7.31% absolute improvement in pairwise accuracy over the strongest baseline.

Conclusion: LPG-Bench and TIT methods provide a comprehensive framework to benchmark and advance text-to-image models, especially for long-prompt generation scenarios.
...
<details>
  <summary>Details</summary>
Motivation: 3D editing is challenging due to cross-view consistency, structural fidelity, and fine-grained controllability requirements. Existing approaches are slow, prone to distortions, or dependent on error-prone manual 3D masks.

Method: Proposes 3DEditVerse benchmark with 116,309 training pairs built through pose-driven geometric edits and foundation model-guided appearance edits. Also introduces 3DEditFormer transformer with dual-guidance attention and time-adaptive gating to disentangle editable regions from preserved structure.
...
Conclusion: The proposed approach addresses key challenges in 3D editing by advancing both data (through 3DEditVerse benchmark) and model (through 3DEditFormer transformer) fronts, enabling precise and consistent edits without auxiliary 3D masks.
...
<details>
  <summary>Details</summary>
Motivation: Most existing Sentinel-2 datasets are cloud-free, limiting their usefulness in tropical regions where clouds are common, and deep networks lose spatial/spectral details during downsampling.

Method: Developed cloud injection algorithm to simulate realistic cloud cover, and proposed lightweight method that injects Normalized Difference Indices (NDIs) into final decoding layers to retain key spatial features.

Result: NDI injection enhanced land cover segmentation by 1.99% for U-Net and 2.78% for DeepLabV3 on cloud-free imagery. Under cloud-covered conditions, incorporating Sentinel-1 data led to significant performance gains across all models.

Conclusion: Radar-optical fusion is effective in challenging atmospheric scenarios, and lightweight NDI injection helps retain spatial features with minimal computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: Existing RealSR methods using large generative models have high computational costs and latency, making them impractical for edge deployment. There's a need for lightweight models that can run efficiently on edge devices.

Method: 1) LiteED - highly efficient VAE alternative reducing parameters by 97.5% while preserving encoding/decoding quality. 2) Online annealing pruning for U-Net that progressively shifts generative priors from heavy to lightweight modules. 3) Multi-layer feature distillation loss to mitigate knowledge loss during pruning.

Result: PocketSR achieves 146M parameters, processes 4K images in 0.8 seconds, with performance on par with state-of-the-art single-step and multi-step RealSR models.

Conclusion: PocketSR provides a highly practical solution for edge-device applications, demonstrating that lightweight models can achieve competitive performance while significantly reducing computational requirements.
...
TL;DR: MEve is a self-curated prompt suite for evaluating multi-event text-to-video generation, used to study when and where multi-event prompts control event transitions in models like OpenSora and CogVideoX.


<details>
  <summary>Details</summary>
Motivation: Existing multi-event video generation methods overlook intrinsic factors in event shifting, and the paper aims to understand when and where multi-event prompts control event transitions during T2V generation.

Method: Introduces MEve evaluation suite and conducts systematic study of OpenSora and CogVideoX models through extensive experiments analyzing denoising steps and block-wise model layers.
...
Conclusion: Reveals essential factors for multi-event video generation and highlights possibilities for multi-event conditioning in future models.
...
<details>
  <summary>Details</summary>
Motivation: FER remains challenging due to occlusions, illumination/pose variations, subtle intra-class differences, and dataset imbalance that hinders minority emotion recognition.

Method: Built on EfficientNetV2-S with transfer learning, strong data augmentation, stratified splitting, and fine-tuning with class-weighted loss to handle imbalanced distributions.
...
Conclusion: Hybrid approaches combining discrete models like MaskGIT with continuous objectives can improve compositional generalization in visual generation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Address privacy and IP concerns from unauthorized usage of personalization techniques in text-to-image diffusion models by creating imperceptible "unlearnable" training samples.

Method: Model-based perturbation in latent space using trajectory-shifted sampling that alternates between denoising and inversion while modifying the starting point of denoising trajectories.
...
Conclusion: The method provides effective and imperceptible defense against unauthorized model adaptation by integrating unlearnability into Latent Diffusion Models framework.
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether geometry-grounded semantic features provide advantages over visual-only features in distilled radiance fields for spatial tasks like pose estimation.
...
Result: Geometry-grounded features contain finer structural details but surprisingly decrease pose estimation accuracy, while visual-only features perform better for broader downstream tasks.

Conclusion: Visual-only features offer greater versatility than geometry-grounded features, highlighting the need for future research on effective geometry-grounding strategies to enhance pretrained semantic features.
...
Method: Uses dual-branch diffusion architecture: one branch synthesizes missing regions from masked target, another extracts geometric features from projected point clouds. Joint self-attention ensures coherence. Target-aware masking identifies occluded areas in references to focus on relevant cues.
...
<details>
  <summary>Details</summary>
Motivation: Current T2SV methods face modal interference from shared text captions and lack clear mechanisms for effective cross-modal feature interaction, limiting audio-video synchronization and semantic alignment.

Method: Proposes HVGC framework for generating disentangled video and audio captions, and BridgeDiT with Dual CrossAttention mechanism for bidirectional cross-modal feature exchange in a dual-tower diffusion transformer architecture.

Result: Achieves state-of-the-art performance on three benchmark datasets with comprehensive human evaluations and ablation studies validating the effectiveness of the proposed components.

Conclusion: The method successfully addresses key challenges in T2SV generation and provides important insights for future research, with all code and checkpoints to be publicly released.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with reconstructing complex visual stimuli from brain activity due to heterogeneity in low-level features and semantic entanglement in high-level features of natural scenes.

Method: Separates visual cortex into two hierarchical regions: Structural Generator extracts structural info from spatial processing voxels as latent diffusion priors, and Semantic Extractor converts semantic processing voxels into CLIP embeddings, integrated via Versatile Diffusion model.

Result: HAVIR enhances both structural and semantic quality of reconstructions, performs well even in complex scenes, and outperforms existing models.

Conclusion: The hierarchical approach inspired by visual cortex representation theory effectively addresses challenges in brain-to-image reconstruction, demonstrating improved performance for complex visual stimuli.
...
TL;DR: Mask2IV is a two-stage framework for generating interaction-centric videos without needing dense mask annotations, using motion trajectory prediction and conditional video generation.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with complex dynamic interactions in videos, and obtaining precise mask annotations is challenging for real-world use.
...
Conclusion: Mask2IV provides an effective solution for interaction-centric video generation with versatile control options while eliminating the need for dense mask inputs.
...
Method: Combines individual- and population-level mobility structures within a probabilistic topological model called Markovian Reeb Graphs to generate spatiotemporal trajectories.

Result: Evaluations on Urban Anomalies dataset (Atlanta and Berlin) using Jensen-Shannon Divergence show strong fidelity in population- and agent-level metrics while being data- and compute-efficient.
...
<details>
  <summary>Details</summary>
Motivation: Spine disorders affect 619 million people globally and are a leading cause of disability, but AI-assisted diagnosis is limited by lack of level-aware multimodal datasets and standardized benchmarks for vertebral-level reasoning across X-ray, CT, and MRI.

Method: Developed SpineMed ecosystem co-designed with spine surgeons, featuring SpineMed-450k dataset curated from textbooks, guidelines, open datasets, and ~1,000 hospital cases using clinician-in-the-loop pipeline with two-stage LLM generation (draft and revision).

Result: Evaluation on SpineBench reveals systematic weaknesses in existing LVLMs for fine-grained level-specific reasoning, while model fine-tuned on SpineMed-450k shows consistent significant improvements across all tasks, with clinician assessments confirming diagnostic clarity and practical utility.

Conclusion: SpineMed addresses critical gaps in spine AI diagnosis by providing traceable multimodal data and standardized benchmarks, enabling improved vertebral-level reasoning and demonstrating superior performance over existing models through clinician-validated results.
...
Method: Multi-agent system with perception agent that analyzes image features to dynamically select detection models, and detection agent that consolidates expert detectors into unified framework.

Result: Achieves state-of-the-art results, surpassing both unified approaches and domain-specific detectors in practicality, adaptiveness, and scalability.

Conclusion: UniShield provides an effective unified solution for forgery detection across diverse domains with superior performance and practical applicability.
...
<details>
  <summary>Details</summary>
Motivation: To create relightable 3D models that can realistically simulate object appearance under novel lighting conditions without requiring complex light transport simulations or per-illumination optimization.

Method: Uses a generative relighting model to sample object appearances under multiple lighting environments, then trains a lighting-conditioned NeRF with dual-branch architecture that separately encodes general lighting effects and specularities.

Result: Improves upon state-of-the-art on most metrics on TensoIR and Stanford-ORB datasets, and works effectively on real-world object captures.

Conclusion: ROGR provides an efficient feed-forward approach for relightable 3D reconstruction that outperforms existing methods and eliminates the need for per-illumination optimization.
...
Method: Training strategy combining dynamic volumetric prompt generation with content-aware adaptive cropping, initialized from nnInteractive segmentation model weights.

Result: Achieved average Dice score of 0.6385, normalized surface distance of 0.6614, and area-under-the-curve metrics of 2.4799 (Dice) and 2.5671 (NSD) on competition benchmark.

Conclusion: Proposed method effectively addresses computational challenges and enables efficient interactive 3D biomedical image segmentation with strong performance.
...
<details>
  <summary>Details</summary>
Motivation: Product quantization has been underutilized for latent representations in high-fidelity image generation despite being a classical method for scalable vector encoding.

Method: Integrates product quantization into the VQGAN framework, analyzing the interaction between codebook size, embedding dimensionality, and subspace factorization to optimize performance.

Result: Achieves PSNR of 37dB (vs prior 27dB), reduces FID, LPIPS, and CMMD scores by up to 96%, and enables faster diffusion model generation or doubled output resolution at no additional cost.

Conclusion: PQGAN demonstrates that product quantization is a strong extension for discrete latent representation in image synthesis, with novel findings about scaling behavior and optimal hyperparameter selection.
...
<details>
  <summary>Details</summary>
Motivation: Current autoregressive video diffusion models face a trade-off between generating natural new content during exploration and maintaining spatial consistency when revisiting areas. Temporal-only memory lacks long-term consistency, while adding spatial memory can degrade new scene generation quality when the model over-relies on insufficient spatial context.

Method: Memory Forcing pairs training protocols with a geometry-indexed spatial memory. Hybrid Training exposes distinct gameplay regimes, Chained Forward Training extends autoregressive training with model rollouts, Point-to-Frame Retrieval efficiently maps visible points to source frames, and Incremental 3D Reconstruction maintains an explicit 3D cache.

Result: Extensive experiments show that Memory Forcing achieves superior long-term spatial consistency and generative quality across diverse environments while maintaining computational efficiency for extended sequences.

Conclusion: The proposed Memory Forcing framework successfully addresses the trade-off between exploration and consistency in video diffusion models, enabling faithful simulation of gameplay with both natural content generation and reliable spatial preservation.
...
Result: MonSTeR outperforms trimodal models that rely solely on unimodal representations, and user studies validate that retrieval scores align with human preferences. The model demonstrates versatility in zero-shot in-Scene Object Placement and Motion Captioning.

Conclusion: MonSTeR successfully addresses the gap in evaluating motion-text-scene alignment through a unified latent space approach, enabling robust cross-modal retrieval and zero-shot applications while aligning with human judgment.
...
TL;DR: A training-free, architecture-agnostic test-time defense against adversarial attacks that uses stochastic resonance by introducing small translational perturbations to combat noise without information loss.


<details>
  <summary>Details</summary>
Motivation: Existing defense methods rely on feature filtering or smoothing which can cause information loss, while adversarial attacks use imperceptible perturbations to alter model predictions.
...
Result: Achieves state-of-the-art robustness: recovers up to 68.1% accuracy loss on image classification, 71.9% on stereo matching, and 29.2% on optical flow under various adversarial attacks.

Conclusion: The method provides a versatile, practical defense that works across diverse network architectures and tasks without training, establishing the first generic test-time defense for dense prediction tasks.
...
TL;DR: Mixer is a novel randomized neural network for texture representation learning that uses hyperspherical random embeddings and a dual-branch module to capture intra- and inter-channel relationships through a new optimization formulation.
...
Method: Proposes Mixer with hyperspherical random embeddings and a dual-branch learning module to capture both intra- and inter-channel relationships, enhanced by a newly formulated optimization problem for building rich texture representations.

Result: Experimental results show promising performance across several pure texture benchmarks with distinct characteristics and challenges.

Conclusion: The proposed Mixer approach demonstrates effective texture representation learning capabilities through its novel architecture combining hyperspherical embeddings and dual-branch learning with optimized formulation.
...
TL;DR: The paper proposes RULER tokens and Interleaved MRoPE to improve GUI grounding accuracy by providing explicit spatial guidance instead of relying on implicit coordinate generation, especially for high-resolution displays.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with GUI grounding due to unreliable patch-to-pixel mapping, particularly when extrapolating to high-resolution displays unseen during training. The implicit coordinate generation approach leads to accuracy degradation and failures on new resolutions.

Method: Two complementary innovations: 1) RULER tokens serve as explicit coordinate markers for position referencing, 2) Interleaved MRoPE improves spatial encoding by ensuring equal representation of width and height dimensions, addressing asymmetry in standard positional schemes.

Result: Experiments on ScreenSpot, ScreenSpot-V2, and ScreenSpot-Pro show consistent gains in grounding accuracy, with the largest improvements on high-resolution interfaces.
...
Method: Generates domain-relevant pseudo question-answer pairs for unlabeled data using a QA generator regularized by caption distillation, with selective updates of only the most relevant neurons for question-answering.
...
Conclusion: LEAML framework effectively adapts MLLMs to specialized domains with limited labeled data through pseudo QA generation and selective parameter updates.
