...
TL;DR: The paper presents an interactive AI model for creative dance interaction that generates artificial dance partners by mimicking and enhancing human movement sequences using motion capture data and diffusion models.


<details>
  <summary>Details</summary>
Motivation: To explore creative human-AI interaction through dance, complementing verbal AI interactions with embodied experiences, as dance is a primal form of human expression.

Method: Uses motion capture data with diffusion models, motion inpainting, and motion style transfer to generate temporally coherent movement representations that respond to human movement references, leveraging single-person motion data and high-level features.

Result: The model successfully generates diverse and realistic dance movements that show various deviations from the human partner while maintaining realism, with quantitative assessment showing convergence between generated samples and test set distributions.

Conclusion: The generations represent first steps toward creative dancing with AI, producing both realistic and creatively enhanced movements that deviate from human input while maintaining coherence.
...
<details>
  <summary>Details</summary>
Motivation: Coral reefs are vital marine ecosystems facing increasing threats from pollution, ocean acidification, and temperature anomalies, making efficient monitoring and protection urgent.

Method: Developed a coral bleaching classification system using three state-of-the-art models (ResNet, ViT, CNN) on a diverse global dataset of healthy and bleached corals from various environments, with comprehensive hyperparameter tuning.

Result: The CNN model achieved the highest accuracy of 88%, outperforming existing benchmarks and other tested models.

Conclusion: The study provides important insights for autonomous coral monitoring and offers a comprehensive analysis of widely used computer vision models for coral health assessment.
...
TL;DR: YOLOv8-based deep learning pipeline automates family-level fish identification from video transects in Kenya and Tanzania, achieving mAP@0.5 of 0.52 and providing the first region-specific benchmark for automated reef fish monitoring in the Western Indian Ocean.


<details>
  <summary>Details</summary>
Motivation: Coral reef monitoring in the Western Indian Ocean is limited by labor demands of underwater visual censuses, creating need for automated solutions.

Method: Developed and evaluated a YOLOv8-based deep learning pipeline using a curated dataset of 24 fish families from video transects collected in Kenya and Tanzania.
...
Conclusion: Deep learning shows potential as a scalable complement to traditional monitoring methods for coral reef fish assessment.
...
Method: Select patches with high mutual information under natural perturbations (color changes, motion) as positive samples for contrastive learning. This mutual-information-informed data augmentation replaces human-engineered approaches.

Result: The method was evaluated on multiple benchmarks across state-of-the-art representation learning frameworks and demonstrated effectiveness in improving learned features.

Conclusion: Mutual-information-informed data augmentation is a promising direction for future research in representation learning, providing better generalization than human-engineered data selection methods.
...
TL;DR: Benchmark study comparing three federated learning frameworks (NVIDIA FLARE, Flower, Owkin Substra) for medical imaging applications using PathMNIST dataset.


<details>
  <summary>Details</summary>
Motivation: Evaluate FL frameworks' suitability for real-world medical imaging applications to enable collaborative model training without direct data sharing.

Method: Used PathMNIST dataset to assess model performance, convergence efficiency, communication overhead, scalability, and developer experience across three FL frameworks.

Result: NVIDIA FLARE offers superior production scalability, Flower provides flexibility for prototyping, and Owkin Substra demonstrates exceptional privacy and compliance features.

Conclusion: Each framework has distinct strengths optimized for different use cases, making them all relevant for practical deployment in healthcare environments.
...
Method: Extensive comparative study of well-known image denoising methods combined with Contrast Limited Adaptive Histogram Equalization (CLAHE) for efficient denoising of rice leaf images. Experiments were performed on a rice leaf image dataset using various metrics to comprehensively test enhancement methods.

Result: The study provides a strong basis for assessing the effectiveness of different denoising methodologies in digital image processing, with results examined using various performance metrics.

Conclusion: The approach reveals insights useful for future adaptation in agricultural research and other domains, demonstrating the importance of proper image preprocessing for reliable agricultural analysis.
...
TL;DR: This paper introduces InfraLiDARs' Benchmark to study how different LiDAR scanning patterns (repetitive vs non-repetitive) affect roadside perception performance in ITS, finding that non-repetitive LiDAR offers cost-effective performance comparable to high-end repetitive LiDAR.


<details>
  <summary>Details</summary>
Motivation: The impact of different LiDAR scanning patterns on perceptual performance in infrastructure-based ITS remains under-investigated, despite their distinct point cloud distributions affecting object detection efficacy.

Method: Created InfraLiDARs' Benchmark dataset in CARLA simulation with concurrent infrastructure-based LiDARs using both scanning paradigms, then conducted statistical analysis and evaluated various 3D object detection algorithms.
...
Conclusion: Provides insights for optimal LiDAR scanning pattern selection and compatible algorithms for roadside perception systems, and releases the benchmark dataset for further research.
...
TL;DR: Cosmos-Predict2.5 is a new flow-based Physical AI model that unifies Text2World, Image2World, and Video2World generation, with improved video quality and instruction alignment over previous versions.


<details>
  <summary>Details</summary>
Motivation: To create more reliable synthetic data generation, policy evaluation, and closed-loop simulation for robotics and autonomous systems by advancing Physical AI capabilities.

Method: Built on flow-based architecture, trained on 200M curated video clips with reinforcement learning-based post-training, and integrated with Cosmos-Reason1 for richer text grounding and world simulation control.

Result: Achieved substantial improvements over Cosmos-Predict1 in video quality and instruction alignment, with models at 2B and 14B scales. Cosmos-Transfer2.5 delivers higher fidelity and robust long-horizon video generation despite being 3.5x smaller than previous version.

Conclusion: Cosmos-Predict2.5 and Cosmos-Transfer2.5 establish versatile tools for scaling embodied intelligence, with open-source release to accelerate Physical AI research and deployment.
...
TL;DR: Deep learning models were applied to detect habitat changes in complex alpine environments using geospatial foundation models (GFMs) and compared post-classification vs direct change detection approaches.
...
TL;DR: LeMiCa is a training-free acceleration framework for diffusion-based video generation that uses lexicographic minimax path optimization to reduce global error accumulation, achieving both speed improvements and better generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing caching strategies focus on local heuristic errors but overlook global error accumulation, leading to content degradation between accelerated and original videos.

Method: Formulates cache scheduling as a directed graph with error-weighted edges and introduces Lexicographic Minimax Path Optimization to bound worst-case path error.
...
Conclusion: LeMiCa provides a robust and generalizable paradigm for accelerating diffusion-based video generation with dual improvements in speed and quality.
...
TL;DR: PLD is a three-stage framework that improves vision-language-action models through residual RL and distribution-aware data collection, achieving near-perfect task success rates across multiple environments.
...
TL;DR: SpinalSAM-R1 is a multimodal vision-language system that integrates fine-tuned SAM with DeepSeek-R1 for spine CT image segmentation, featuring anatomy-guided attention and natural language interaction.


<details>
  <summary>Details</summary>
Motivation: Spine CT segmentation faces challenges from low contrast and complex vertebral boundaries, while existing models like SAM have poor domain adaptability and high annotation requirements for spinal imaging.

Method: Proposes SpinalSAM-R1 with anatomy-guided attention mechanism and semantics-driven interaction using DeepSeek-R1, fine-tuned with LoRA for efficient adaptation to spine CT segmentation.

Result: Achieves superior segmentation performance on spine CT images, with 94.3% parsing accuracy for 11 clinical operations and sub-800ms response times in the developed interactive software.

Conclusion: SpinalSAM-R1 effectively addresses domain adaptation challenges in spine CT segmentation through multimodal integration and natural language interaction, with released open-source software.
...
TL;DR: Proposes a filter for confocal laser endomicroscopy (CLE) video sequences to reduce redundancy in self-supervised learning (SSL), improving training efficiency and performance on tumor classification tasks.


<details>
  <summary>Details</summary>
Motivation: CLE images are hard to interpret for non-experts, and machine learning models suffer from overfitting due to limited histopathology-correlated data. SSL can help but is hindered by high inter-frame correlation in CLE videos.

Method: Developed a filter to reduce dataset redundancy in CLE videos for SSL training. Used four baseline networks and a SSL teacher-student network with vision transformer backbone, evaluated on sinonasal tumor and skin squamous cell carcinoma datasets.

Result: Filtered SSL-pretrained models achieved highest test accuracy: 67.48% on sinonasal tumor and 73.52% on skin carcinoma, outperforming non-SSL baselines. Training time reduced by 67%.
...
TL;DR: FreeSliders enables training-free, modality-agnostic fine-grained control in diffusion models by partially estimating Concept Sliders formula during inference, extending control to video and audio with improved evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Current fine-grained controllable generation methods like Concept Sliders require per-concept training and architecture-specific fine-tuning, limiting scalability to new modalities and concepts.

Method: Partially estimate the Concept Sliders formula during inference without training, extend benchmark to video and audio, introduce automatic scale selection and non-linear traversal detection for perceptually uniform edits.

Result: Enables plug-and-play concept control across modalities, improves over existing baselines, and provides tools for principled controllable generation without training requirements.

Conclusion: FreeSliders provides a simple yet effective training-free approach for fine-grained concept control that works across multiple modalities, establishing new benchmarks and evaluation methods for controllable generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-to-video generation approaches struggle with maintaining temporal consistency, compositional understanding, and fine-grained control over visual narratives, creating a need for improved methods that can handle complex multi-object scenes with realistic dynamics.

Method: MOVAI introduces three key components: 1) Compositional Scene Parser (CSP) that decomposes text into hierarchical scene graphs with temporal annotations, 2) Temporal-Spatial Attention Mechanism (TSAM) for coherent motion dynamics across frames while preserving spatial details, and 3) Progressive Video Refinement (PVR) module for iterative quality enhancement through multi-scale temporal reasoning.

Result: Extensive experiments show MOVAI achieves state-of-the-art performance with improvements of 15.3% in LPIPS, 12.7% in FVD, and 18.9% in user preference studies compared to existing methods. The framework excels at generating complex multi-object scenes with realistic temporal dynamics and fine-grained semantic control.

Conclusion: MOVAI presents an effective hierarchical framework that successfully addresses key challenges in text-to-video generation through compositional scene understanding and temporal-aware modeling, demonstrating superior performance in generating high-fidelity videos with complex temporal dynamics and semantic control.
...
TL;DR: Chain of Time method improves physical simulation in vision-language models by generating intermediate images during simulation, requiring no fine-tuning and enhancing performance on various physical properties.


<details>
  <summary>Details</summary>
Motivation: Inspired by in-context reasoning in ML and mental simulation in humans, to improve physical reasoning capabilities in vision-language models.

Method: Chain of Time generates series of intermediate images during simulation at inference time without additional training, applied to 2-D graphics and 3-D videos testing velocity, acceleration, fluid dynamics, and conservation of momentum.

Result: Substantially improves performance of state-of-the-art image generation model, reveals hidden physical reasoning capabilities like simulating velocity, gravity, and collisions over time, but also identifies limitations in inferring physical parameters from input images.

Conclusion: Chain of Time provides both performance improvements and interpretability insights into physical reasoning capabilities of vision-language models, uncovering both strengths and weaknesses in temporal physical simulation.
...
TL;DR: First end-to-end framework combining generative AI and deep reinforcement learning for autonomous cardiac ultrasound scanning, addressing reproducibility issues in existing approaches.


<details>
  <summary>Details</summary>
Motivation: Cardiac ultrasound effectiveness is limited by operator dependence, time constraints, human error, and shortage of trained professionals, especially in remote areas. Existing AI approaches lack reproducibility, rely on proprietary data, and use simplified models.

Method: Two-component framework: (1) conditional generative simulator combining GANs with VAEs to produce realistic action-conditioned US images, (2) DRL module that learns autonomous scanning policies using the simulator. Includes expert-validated models for image classification and quality assessment.

Result: Framework delivers AI-driven guidance, supports conditional generation of realistic US images, and establishes reproducible foundation extendable to other organs. Public dataset released for reproducibility. VAE-GAN benchmarked against existing GAN variants with qualitative/quantitative assessment, DRL system evaluated under varying configurations.

Conclusion: Proposed framework enables autonomous and reproducible cardiac US scanning, addressing key limitations in existing approaches through integration of generative AI and DRL, with validation demonstrating effectiveness.
...
TL;DR: VLM6D introduces a dual-stream architecture using RGB-D input with Vision Transformer and PointNet++ encoders for robust 6D object pose estimation, achieving state-of-the-art performance on Occluded-LineMOD.


<details>
  <summary>Details</summary>
Motivation: Current 6D pose estimation methods struggle with generalization from synthetic to real-world data due to lighting variations, textureless objects, and severe occlusions.

Method: Dual-stream architecture with self-supervised Vision Transformer (DINOv2) for RGB processing and PointNet++ for 3D point cloud processing, with complementary feature fusion.
...
Conclusion: VLM6D effectively addresses limitations of current approaches by leveraging complementary visual and geometric data streams for robust 6D pose estimation in challenging real-world conditions.
...
TL;DR: A hybrid ConvNeXt-ViT architecture for age estimation that combines CNN's local feature extraction with Transformer's global attention, achieving superior performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Age estimation from facial images is complex, and existing methods can benefit from combining the complementary strengths of CNNs and Transformers for better feature extraction.

Method: Proposed a hybrid ConvNeXt-ViT architecture using pre-trained models, linear layers, and regularization techniques. Explored different configurations and adapted attention mechanisms in CNN framework.

Result: Achieved superior performance in terms of mean absolute error (MAE) on benchmark datasets MORPH II, CACD, and AFAD, outperforming traditional methods.

Conclusion: Hybrid architectures combining CNNs and Transformers show transformative potential for age estimation and represent a promising direction for complex computer vision challenges.
...
TL;DR: FLoC is an efficient visual token compression framework that uses facility location function and lazy greedy algorithm to select a compact, representative subset of visual tokens for long video understanding, achieving near-optimal performance with significant efficiency gains.


<details>
  <summary>Details</summary>
Motivation: The scalability of video-LMMs for long video understanding is limited by the overwhelming volume of visual tokens generated from extended video sequences, creating a need for efficient token compression.

Method: Proposes FLoC framework based on facility location function that selects a compact, representative, and diverse subset of visual tokens within a predefined budget using lazy greedy algorithm. The approach is training-free, model-agnostic, and query-agnostic.

Result: Extensive evaluations on Video-MME, MLVU, and LongVideoBench benchmarks show FLoC consistently surpasses recent compression techniques, demonstrating effectiveness, robustness, and efficiency in processing speed while drastically reducing visual tokens.

Conclusion: FLoC provides a versatile, efficient solution for visual token compression in long video understanding that seamlessly integrates with diverse video-LLMs and existing workflows while maintaining near-optimal performance.
...
TL;DR: The paper proposes BlurGuard, a method that applies adaptive Gaussian blur to adversarial noise to make image protection more robust against reversal techniques like JPEG compression, improving protection performance while reducing quality degradation.


<details>
  <summary>Details</summary>
Motivation: To address the vulnerability of existing adversarial noise-based image protection methods that can be easily reversed by simple techniques like JPEG compression, making them impractical for real-world use.

Method: Applies an adaptive per-region Gaussian blur on the adversarial noise to adjust the overall frequency spectrum, making the noise more difficult to detect and reverse while maintaining imperceptibility.

Result: The method consistently improves per-sample worst-case protection performance against various reversal techniques across diverse image editing scenarios, while reducing quality degradation in terms of perceptual metrics.

Conclusion: Adversarial noise for image protection should be both imperceptible and irreversible, and the proposed BlurGuard method effectively enhances robustness against noise reversal techniques while maintaining image quality.
...
TL;DR: CompAgent is an agentic framework for visual compliance verification that augments MLLMs with visual tools and dynamic planning to outperform existing methods on policy compliance tasks.


<details>
  <summary>Details</summary>
Motivation: Visual compliance verification is critical in media and advertising but existing methods are costly and limited. MLLMs have policy knowledge but struggle with fine-grained visual reasoning.

Method: Augments MLLMs with visual tools (object detectors, face analyzers, etc.) and uses a planning agent to dynamically select tools based on compliance policies, plus a verification agent for multi-modal reasoning.

Result: Outperforms specialized classifiers, direct MLLM prompting, and routing baselines, achieving 76% F1 score and 10% improvement over state-of-the-art on UnsafeBench dataset.
...
TL;DR: AIFo is a training-free multi-agent framework that emulates human forensic investigation to detect AI-generated images, achieving 97.05% accuracy by coordinating various forensic tools through LLM-based agents and structured debate mechanisms.
...
Method: Uses multi-agent collaboration with forensic tools (reverse image search, metadata extraction, classifiers, VLM analysis) coordinated by specialized LLM-based agents. Includes structured multi-agent debate for conflicting evidence and memory-augmented reasoning from historical cases.

Result: Achieved 97.05% accuracy on 6,000 images across controlled and real-world scenarios, substantially outperforming traditional classifiers and state-of-the-art VLMs.
...
TL;DR: This paper proposes EMPL, an Energy-based Multi-prompt Learning method that generates multiple prompt embeddings from an energy-based distribution defined by VLMs, achieving better balance between in-domain and out-of-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Existing research focuses on single-prompt paradigms, while the potential of multi-prompt learning for Vision-Language Models remains largely unexplored. The paper aims to provide a principled framework for vision-language multi-prompt learning.

Method: Extends the constant modality gap phenomenon to learnable prompts, proposes EMPL that generates multiple prompt embeddings by drawing instances from an energy-based distribution implicitly defined by VLMs.

Result: Comprehensive experiments justify the claims and demonstrate the excellence of EMPL, showing superiority in vision-language transfer with multi-prompt augmentation.

Conclusion: EMPL is both parameter-efficient and rigorously leads to better balance between in-domain and out-of-domain open-vocabulary generalization, providing a principled approach to multi-prompt learning for VLMs.
...
TL;DR: Proposes an efficient transfer learning method for detecting weather-related conditions on satellite ground terminals, outperforming traditional deep learning approaches.


<details>
  <summary>Details</summary>
Motivation: Weather events significantly impact satellite Internet reliability, but existing solutions lack fine-grained detection capability for ground terminal components.

Method: Uses transfer learning to enable local detection of weather conditions (snow, wet, etc.) on ground components.

Result: Shows superior performance compared to YOLOv7, YOLOv9, Faster R-CNN, and R-YOLO, with good generalization across scenarios.

Conclusion: The transfer learning method provides effective and generalizable weather condition detection for reliable satellite Internet operations.
...
Conclusion: Modality-specific encoding with learnable fusion effectively exploits the complementary nature of polarized intensity and phase information in ssQPM for improved cell segmentation performance.
...
TL;DR: A VQ-VAE-based compression framework for electron microscopy data that enables flexible decoding options and ROI-driven reconstruction from highly compressed latents.
...
Method: Vector-quantized variational autoencoder (VQ-VAE) compression spanning 16x to 1024x, with pay-as-you-decode: top-only decoding for extreme compression, optional Transformer prior for texture restoration via FiLM and concatenation, and ROI-driven selective high-resolution reconstruction.

Result: The framework enables extreme compression (up to 1024x) while maintaining flexible decoding options and selective reconstruction capabilities.

Conclusion: This compression approach addresses the challenges of large-scale EM datasets by providing scalable compression with flexible reconstruction workflows.
...
TL;DR: This paper proposes a novel algorithm for computing optimal transport maps in hyperbolic space using geometric variational techniques, extending methods from Euclidean and spherical geometry.
...
Result: The proposed method was validated through experiments on synthetic data and multi-genus surface models, demonstrating its efficacy.

Conclusion: The paper successfully develops and validates an efficient algorithm for computing optimal transport maps in hyperbolic space, addressing a gap in existing methods that were limited to Euclidean and spherical geometries.
...
TL;DR: MSDI framework generates realistic 4D human motions by combining 3D Gaussian representations with motion diffusion priors and LLM semantic guidance, achieving physically plausible interactions without retraining.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models produce unrealistic deformations and physical inconsistencies due to lack of 3D physical priors, requiring better spatial-aware motion generation.

Method: Uses Motion Score Distilled Interaction (MSDI) with Motion Diffusion Score Distillation Sampling (MSDS) to distill gradients from pre-trained motion diffusion models, guided by LLM spatial and semantic information for motion optimization.
...
Conclusion: MSDI offers a scalable zero-shot solution for realistic 4D human motion generation by integrating 3D representations, motion diffusion, and semantic constraints.
...
TL;DR: The paper introduces L48, a real-world fine-grained multi-label dataset for single-positive multi-label learning, addressing limitations of synthetic benchmarks and revealing performance gaps in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing SPML methods are benchmarked on synthetic datasets created from fully-annotated data, which don't reflect real-world complexities and fine-grained challenges that cause difficult misclassifications.

Method: Created L48 dataset from bird sound recordings, providing natural SPML setting with single-positive annotations in fine-grained domain, plus two extended settings with domain priors for additional negative labels.

Result: Benchmarked existing SPML methods on L48 and observed significant performance differences compared to synthetic datasets, revealing method weaknesses and the need for more realistic benchmarks.

Conclusion: The L48 dataset provides a more realistic and challenging benchmark for SPML learning, highlighting limitations of current methods and the importance of real-world fine-grained evaluation.
...
TL;DR: A 3-stage automated pipeline for processing beetle images: detection using transformer-based models, sorting/cropping individual beetles, and morphological segmentation using fine-tuned transformer models.


<details>
  <summary>Details</summary>
Motivation: Biologists need to process large-scale beetle image data efficiently for entomology and ecology research, as manual processing of thousands of beetle trays is time-consuming.

Method: 3-stage pipeline: 1) Detection using transformer-based open-vocabulary object detector and vision-language model, 2) Sorting and cropping individual beetles, 3) Morphological segmentation using fine-tuned transformer-based segmentation models trained on 670 manually labeled beetle images.

Result: Developed a specialized pipeline that integrates multiple deep learning methods for automated beetle image processing with relatively high accuracy in segmentation.

Conclusion: The pipeline significantly improves efficiency in processing large-scale beetle data and accelerates biological research by automating the detection and segmentation tasks.
...
TL;DR: MambaNetLK is a novel correspondence-free 3D registration framework that integrates Mamba State Space Model with PointNetLK architecture, achieving superior performance on clinical endoscopic data with significant error reduction over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Accurate 3D point cloud registration is crucial for reliable image-guided colonoscopy, but biological tissues present challenges including repetitive textures, homogeneous geometry causing feature degeneracy, and domain shifts between pre-operative and intra-operative data.

Method: Proposes MambaNetLK framework that enhances PointNetLK by integrating Mamba State Space Model as cross-modal feature extractor to capture long-range dependencies efficiently with linear-time complexity, using Lucas-Kanade algorithm for iterative alignment. Also introduces C3VD-Raycasting-10k clinical dataset with 10,014 aligned point cloud pairs.

Result: On C3VD-Raycasting-10k dataset, MambaNetLK achieves best performance, reducing median rotation error by 56.04% and RMSE translation error by 26.19% over second-best method. Also demonstrates strong generalization on ModelNet40 and superior robustness to initial pose perturbations.

Conclusion: MambaNetLK provides robust foundation for 3D registration in surgical navigation, enabling more accurate and reliable guidance systems in minimally invasive procedures through combination of globally expressive SSM-based feature extractor and large-scale clinical dataset.
...
TL;DR: The paper introduces Spot The Ball benchmark to evaluate visual social inference in VLMs, showing humans outperform models 2-3x by using social cues like gaze and pose, while models rely on superficial spatial heuristics.


<details>
  <summary>Details</summary>
Motivation: To address the gap in visual social reasoning between humans and AI, as humans excel at inferring hidden scene elements from behavioral cues like gaze and pose, which is critical for developing human-like AI agents.

Method: Created Spot The Ball benchmark using sports images with removed balls, evaluated four state-of-the-art VLMs (Gemini, GPT, LLaMA, Qwen) with three prompting strategies, and compared against human baselines.

Result: Humans consistently achieved 20-34% accuracy across all sports, while models performed at ≤17% accuracy. Models relied on spatial heuristics like guessing near image center or players, whereas humans leveraged social cues.

Conclusion: There's a persistent human-model gap in visual social reasoning, highlighting the need for architectures that explicitly encode structured behavioral cues for robust, human-like inference.
...
TL;DR: A federated learning framework combining frozen CLIP ViT with lightweight transformer classifier for agricultural classification, addressing privacy concerns and non-IID data issues while achieving 86.6% accuracy.


<details>
  <summary>Details</summary>
Motivation: To overcome privacy concerns in centralized training and address challenges of non-IID data and high communication costs in standard federated learning for agricultural classification tasks.

Method: Integrates frozen CLIP vision transformer for feature extraction with lightweight transformer classifier, shares 1% of CLIP-extracted features across clients to align class representations while preserving privacy.
...
TL;DR: PersonalView is a lightweight adaptation method that enables existing generative models to produce multi-view consistent images of a person using only 100 training samples, outperforming baselines trained on large multi-view datasets.
...
Method: Uses a conditioning architecture leveraging pre-trained diffusion transformer's in-context learning ability, combined with Semantic Correspondence Alignment Loss to preserve original generative capabilities.

Result: Significantly outperforms baselines trained on large multi-view corpora in multi-view consistency, text alignment, identity similarity, and visual quality.

Conclusion: PersonalView provides an efficient solution for multi-view generation with minimal training data, demonstrating superior performance over existing methods.
...
TL;DR: LITHOS is the largest public dataset for automated petrography, featuring 211,604 RGB patches and 105,802 expert-annotated mineral grains across 25 categories, with a dual-encoder transformer model that outperforms single-polarization approaches.


<details>
  <summary>Details</summary>
Motivation: Petrography is labor-intensive and requires expert visual analysis, limiting scalability. There's a need for automated techniques to reduce manual effort and enable large-scale analysis.

Method: Created LITHOS dataset with polarized light images and expert annotations. Proposed a dual-encoder transformer architecture that integrates both polarization modalities for mineral classification.
...
Conclusion: LITHOS provides a comprehensive benchmark for automated petrographic analysis, with the dual-polarization approach showing superior performance, and the dataset is publicly available to foster further research.
...
TL;DR: Systematic evaluation of 11 lightweight vision models across 7 diverse datasets reveals ImageNet accuracy doesn't predict cross-domain performance, introduces Cross-Dataset Score (xScore) for robustness measurement, and identifies key architectural components for better generalization.


<details>
  <summary>Details</summary>
Motivation: Lightweight vision models are widely deployed in mobile systems but primarily benchmarked on ImageNet, raising questions about their cross-domain generalization and how to systematically quantify robustness across diverse datasets.

Method: Evaluated 11 lightweight vision models (2.5M parameters) trained under fixed 100-epoch schedule across 7 diverse datasets, introduced Cross-Dataset Score (xScore) metric to quantify performance consistency and robustness across domains.
...
TL;DR: A hybrid DeepONet-NTK framework for solving complex inverse problems with physics-informed constraints and regularization.


<details>
  <summary>Details</summary>
Motivation: To address challenges in solving inverse problems involving nonlinearity, sparsity, and noisy data in computational physics and imaging sciences.
...
Result: Validated on synthetic and real datasets, demonstrating robustness, scalability, and precision in tasks like source localization governed by Navier-Stokes equations and image reconstruction.
...
TL;DR: FedDISC is a federated learning framework that integrates diffusion models for missing-modality recovery in multimodal emotion recognition, addressing performance degradation from unpredictable modality absence in real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: Real-world multimodal emotion recognition suffers from unpredictable modality absence, which degrades performance of existing methods. Conventional recovery approaches relying on complete multimodal data training suffer from semantic distortion under extreme data distributions like fixed-modality absence.

Method: FedDISC integrates federated learning with modality-specific diffusion models. It uses DISC-Diffusion module with Dialogue Graph Network for conversational dependencies and Semantic Conditioning Network for semantic alignment. Also employs Alternating Frozen Aggregation strategy that cyclically freezes recovery and classifier modules.

Result: Extensive experiments on IEMOCAP, CMUMOSI, and CMUMOSEI datasets show FedDISC achieves superior emotion classification performance across diverse missing modality patterns, outperforming existing approaches.

Conclusion: FedDISC successfully addresses missing-modality challenges in multimodal emotion recognition through federated diffusion model integration and semantic consistency mechanisms, demonstrating robust performance across various modality absence scenarios.
...
TL;DR: OSMGen is a generative framework that creates realistic satellite imagery from OpenStreetMap data, enabling generation of consistent before-after image pairs for urban monitoring and training data.


<details>
  <summary>Details</summary>
Motivation: Automating urban monitoring is challenging due to scarce curated datasets of urban features and their changes. There's a need for accurate geospatial data for urban planning and infrastructure monitoring.

Method: Uses raw OpenStreetMap JSON data including vector geometries, semantic tags, location, and time. Generates satellite imagery directly from OSM data, allowing user edits to OSM inputs to create targeted visual changes while preserving the rest of the scene.

Result: Produces realistic satellite imagery and consistent before-after image pairs. Enables generation of training data to address scarcity and class imbalance, and allows planners to preview proposed interventions by editing map data.

Conclusion: OSMGen provides a framework for creating paired (JSON, image) data for both static and changed states, paving the way toward a closed-loop system where satellite imagery can automatically drive structured OSM updates.
...
Method: Leverages diffusion snap-back - multi-strength image reconstruction dynamics to extract interpretable manifold-based features by analyzing how reconstruction metrics (LPIPS, SSIM, PSNR) evolve across varying noise strengths.

Result: Achieves 0.993 AUROC under cross-validation on balanced dataset of 4,000 images, remains robust to common distortions like compression and noise, and demonstrates strong generalization despite limited data and single diffusion backbone (Stable Diffusion v1.5).

Conclusion: The method offers a foundation for scalable, model-agnostic synthetic media forensics with strong generalization and interpretability.
...
TL;DR: Transfer learning with UNet and MobileNet encoder enables efficient thermal cloud segmentation for CubeSats, achieving 0.877 F1 score and real-time inference on edge hardware.


<details>
  <summary>Details</summary>
Motivation: Onboard cloud segmentation is critical for thermal Earth observation CubeSats, which face hardware constraints, limited spectral data (single thermal band), and insufficient labeled data, making conventional cloud masking infeasible.

Method: Use transfer learning with UNet and lightweight MobileNet encoder, pretrained on Landsat-7 Cloud Cover Assessment Dataset and fine-tuned with mission-specific samples in joint-training setup, then convert to TensorRT engine for edge deployment.

Result: Improved macro F1 from 0.850 to 0.877 over FOREST-2-only baselines, with full-image inference in under 5 seconds on NVIDIA Jetson Nano.

Conclusion: Leveraging public datasets and lightweight architectures enables accurate, efficient thermal-only cloud masking on-orbit, supporting real-time decision-making in data-limited Earth observation missions.
...
TL;DR: Oitijjo-3D is a free generative AI framework that uses Google Street View imagery to create 3D models of cultural heritage sites in Bangladesh, overcoming resource limitations of traditional methods.


<details>
  <summary>Details</summary>
Motivation: Cultural heritage restoration in Bangladesh faces challenges of limited resources and technical expertise, making traditional 3D digitization methods (photogrammetry, LiDAR) infeasible due to cost, hardware requirements, and need for expert operators.

Method: Two-stage pipeline: 1) multimodal visual reasoning with Gemini 2.5 Flash Image for structure-texture synthesis, 2) neural image-to-3D generation through Hexagen for geometry recovery, using publicly available Google Street View imagery.

Result: Produces photorealistic, metrically coherent 3D reconstructions in seconds with significant speedups compared to conventional Structure-from-Motion pipelines, preserving both visual and structural fidelity for landmarks like Ahsan Manzil, Choto Sona Mosque, and Paharpur.

Conclusion: Oitijjo-3D democratizes cultural preservation by turning open imagery into digital heritage, reframing preservation as a community-driven, AI-assisted act of cultural continuity for resource-limited nations.
...
<details>
  <summary>Details</summary>
Motivation: Current video moment retrieval solutions don't handle conflicts between different models' location results, preventing proper integration of multiple models for better performance.

Method: Proposed a reinforcement learning model that scans entire videos to find moment boundaries while producing locational evidence, and a multi-agent system framework using evidential learning to resolve conflicts between agents' localization outputs.
...
Conclusion: Modeling competition and conflict in multi-agent systems improves RL performance in moment retrieval, and evidential learning plays a new role in multi-agent frameworks for conflict resolution.
...
TL;DR: VisionCAD is a vision-based radiological assistance framework that captures medical images from displays using cameras, enabling AI-assisted diagnosis without requiring integration with existing hospital IT infrastructure.
...
Method: Uses a camera system to capture on-screen medical images, then employs an automated pipeline that detects, restores, and analyzes the images to transform camera-captured data into diagnostic-quality images suitable for automated analysis and report generation.

Result: Achieves diagnostic performance comparable to conventional CAD systems with F1-score degradation typically less than 2% across classification tasks, and automated report quality within 1% of those from original images.

Conclusion: VisionCAD offers an accessible approach for AI-assisted diagnosis that can be deployed in diverse clinical settings without requiring modifications to existing infrastructure, using only a camera device and standard computing resources.
...
TL;DR: FERBench systematically benchmarks 20 MLLMs on FER tasks, revealing limitations in reasoning and interpretability. The authors then develop UniFER-7B using post-training strategies that outperforms major MLLMs.


<details>
  <summary>Details</summary>
Motivation: Despite MLLMs' success in various tasks, their performance on facial expression recognition remains unexplored. The authors aim to unify FER tasks using MLLMs and address their reasoning limitations.

Method: Created FERBench benchmark with 20 MLLMs across 4 FER datasets. Developed post-training strategies using two datasets: UniFER-CoT-230K for cold-start and UniFER-RLVR-360K for RL with verifiable rewards, leading to UniFER-7B model.

Result: MLLMs show good classification performance but significant limitations in reasoning and interpretability. UniFER-7B outperforms many open-source and closed-source MLLMs including Gemini-2.5-Pro and Qwen2.5-VL-72B.

Conclusion: The study demonstrates that while current MLLMs have limitations in FER reasoning, targeted post-training strategies can significantly enhance their capabilities, resulting in a unified FER foundation model that surpasses major generalist MLLMs.
...
TL;DR: VinciCoder is a unified multimodal code generation model that achieves state-of-the-art performance through a two-stage training framework combining supervised finetuning with visual reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models rely on single-task training, which limits the development of generalized visual code intelligence. The paper aims to overcome this narrow paradigm.

Method: Two-stage training: 1) Large-scale SFT with 1.6M image-code pairs for direct code generation and visual-based code refinement; 2) Visual Reinforcement Learning (ViRL) with coarse-to-fine reward mechanism using visual similarity across local and global image patches.
...
Conclusion: The proposed VinciCoder model with its two-stage training framework and ViRL strategy successfully addresses the limitations of single-task training and advances multimodal code generation capabilities.
...
Result: Achieves state-of-the-art performance across all three tasks, particularly 0.899 S-measure on CoCA for CoSOD (8.0% improvement over prior best), using significantly less training data than specialized methods.

Conclusion: The proposed unified framework with CoT reasoning and CGPO effectively handles multiple saliency tasks, outperforming specialized approaches while being more data-efficient.
...
TL;DR: LGCA is a framework that improves zero-shot image classification by capturing local features, selecting salient regions for expansion, and combining local-global similarities to reduce misinformation from random crops.


<details>
  <summary>Details</summary>
Motivation: Address the issue of misinformation and bias introduced by random image crops in CLIP models, as many images share similar features at small scales.

Method: Proposes Localized-Globalized Cross-Alignment (LGCA) that captures local features, repeatedly selects the most salient regions and expands them, and uses similarity scores that incorporate both original and expanded images.

Result: Substantially improves zero-shot performance across diverse datasets, outperforming state-of-the-art baselines.

Conclusion: LGCA effectively captures both local and global features while minimizing misinformation, with theoretical analysis showing it maintains the same time complexity as the original model, making it efficient and scalable.
...
TL;DR: ITEM is a fake image detector that uses image-text misalignment in CLIP's joint space as discriminative clues, outperforming existing methods with better generalization across generative models.


<details>
  <summary>Details</summary>
Motivation: Existing fake image detection methods focus only on visual clues and suffer from overfitting, lacking generalization to unseen generative models. The paper addresses this by leveraging multi-modal misalignment between images and captions.

Method: Proposes ITEM detector that measures image-text misalignment in CLIP's space using a hierarchical approach: global image-level misalignment followed by fine-grained object-level misalignment from captions, then tunes an MLP head for detection.
...
Conclusion: Leveraging image-text misalignment in multi-modal space provides more robust and generalizable fake image detection compared to visual-only methods, with the hierarchical approach capturing both global and local semantic inconsistencies.
...
TL;DR: A frequency-based detection method for identifying diffusion-generated images by enhancing discriminative frequency clues across all bands, achieving superior generalization to unseen models and robustness to perturbations.


<details>
  <summary>Details</summary>
Motivation: Diffusion models create high-quality images that raise concerns about malicious use, and existing detectors struggle with generalization across different models and robustness to various perturbations.

Method: Proposes Frequency Forgery Clue (F^2C) representation using a frequency-selective function as a weighted filter to Fourier spectrum, suppressing less discriminative bands while enhancing informative ones based on observed frequency differences between real and diffusion-generated images.
...
Conclusion: The frequency-based approach provides an effective solution for detecting diffusion-generated images with strong generalization capabilities and robustness across different settings.
...
<details>
  <summary>Details</summary>
Motivation: CLIP's reliance on uncurated web data makes it vulnerable to data poisoning and backdoor attacks, but existing research has focused mainly on image-based attacks, leaving text modality underexplored.

Method: The framework uses iterative background-aware text selection and background-driven augmentation to generate poisoned text samples that maintain semantic coherence with target classes.
...
Conclusion: ToxicTextCLIP demonstrates significant vulnerabilities in CLIP's text modality and highlights the need for more robust defense mechanisms against text-based poisoning attacks.
...
TL;DR: A weakly supervised deep learning framework for pneumonia classification and localization from chest X-rays using Grad-CAM explanations, achieving high accuracy without pixel-level annotations.


<details>
  <summary>Details</summary>
Motivation: To develop an interpretable AI system for pneumonia screening that doesn't require costly pixel-level annotations and enhances clinical trust through transparent heatmap visualizations.

Method: Used seven ImageNet-pretrained architectures (ResNet-18/50, DenseNet-121, EfficientNet-B0, MobileNet-V2/V3, ViT-B16) with focal loss and patient-wise splits. Employed Grad-CAM for generating clinically meaningful heatmaps from image-level labels only.

Result: ResNet-18 and EfficientNet-B0 achieved best overall test accuracy of 98%, ROC-AUC = 0.997, and F1 = 0.987. MobileNet-V2 provided optimal trade-off between accuracy and computational cost. Grad-CAM visualizations confirmed focus on clinically relevant lung regions.

Conclusion: The work demonstrates the potential of weakly supervised explainable models to enhance pneumonia screening transparency and clinical trust in AI-assisted medical imaging.
...
<details>
  <summary>Details</summary>
Motivation: Current generative models achieve high-fidelity 3D human reconstruction but lack utility for specific tasks like human-part segmentation, and there's scarcity of labeled 3D human datasets.
...
Conclusion: The unified framework successfully enables joint modeling of appearance and human-part semantics with superior performance in both reconstruction and segmentation tasks.
...
TL;DR: A human-in-the-loop deep learning framework for automated vestibular schwannoma segmentation in MRI that combines multi-center data with expert consensus, achieving high accuracy (DSC 0.9670) and 37.4% efficiency improvement over manual annotation.


<details>
  <summary>Details</summary>
Motivation: Manual segmentation of vestibular schwannoma on MRI is time-consuming and requires expert annotation. Current deep learning methods lack robustness across diverse datasets and complex clinical cases.

Method: Bootstrapped DL-based framework with iterative segmentation and quality refinement, using multi-center data and expert consensus for trustworthy annotations. Human-in-the-loop model training approach.

Result: Significant improvement in segmentation accuracy (DSC increased from 0.9125 to 0.9670), stable performance on external datasets, and 37.4% efficiency gain over manual annotation. Dataset includes 190 patients with 534 T1CE scans.

Conclusion: The human-in-the-loop approach achieves high segmentation accuracy and is clinically adaptable for automated VS segmentation in diverse settings. Dataset is publicly available on TCIA.
...
TL;DR: FedMGP introduces personalized federated prompt learning with multiple prompt groups, dynamic aggregation, and achieves SOTA performance with minimal communication parameters.
...
<details>
  <summary>Details</summary>
Motivation: To enable efficient synthesis of controllable 4D scenes from single images by unifying generative priors with geometry and motion constraints, eliminating the need for time-consuming optimization or refinement processes.

Method: Uses a video latent transformer to augment video diffusion models for joint spatio-temporal modeling, predicting deformable 3D Gaussian fields that encode appearance, geometry, and motion in a single forward pass.
...
Conclusion: Diff4Splat provides an efficient feed-forward approach for 4D scene synthesis that combines the strengths of video diffusion models with explicit 3D representations, enabling fast generation of controllable dynamic scenes from single images.
...
<details>
  <summary>Details</summary>
Motivation: To advance explainable Medical Visual Question Answering (Med-VQA) with spatial grounding capabilities for chest X-ray analysis, addressing the need for clinically grounded and reproducible research.

Method: Created a dataset with 17,597 question-answer pairs across 4,394 chest X-ray images, annotated with radiologist-verified bounding boxes and clinical reasoning explanations. Used a balanced distribution (41.7% positive, 58.3% negative samples) to mitigate hallucinations.

Result: Benchmarking with MedGemma-4B-it showed improved performance (F1 = 0.624, +11.8% over baseline) while enabling lesion localization. The dataset supports six diagnostic question types.

Conclusion: VinDr-CXR-VQA successfully advances reproducible and clinically grounded Med-VQA research, with publicly available dataset and evaluation tools.
...
TL;DR: OmniTrack++ is a feedback-driven framework for Multi-Object Tracking in panoramic imagery that addresses unique challenges like 360° FoV, resolution dilution, and view-dependent distortions through progressive refinement with trajectory cues.


<details>
  <summary>Details</summary>
Motivation: Conventional MOT methods designed for narrow-FoV pinhole cameras perform poorly in panoramic imagery due to 360° FoV, resolution dilution, and severe view-dependent distortions, requiring specialized solutions.

Method: Uses DynamicSSM block to stabilize panoramic features, FlexiTrack Instances for flexible localization with trajectory feedback, ExpertTrack Memory with Mixture-of-Experts for appearance consolidation, and adaptive Tracklet Management switching between tracking modes.

Result: Achieves state-of-the-art performance with substantial HOTA improvements: +25.5% on JRDB and +43.07% on QuadTrack over original OmniTrack, as demonstrated on JRDB and EmboTrack benchmarks.

Conclusion: OmniTrack++ provides an effective solution for panoramic MOT with robust handling of distortions and identity ambiguity, supported by the comprehensive EmboTrack benchmark for real-world panoramic perception evaluation.
...
TL;DR: ID-Composer is a novel framework for multi-subject video generation from text prompts and reference images, using hierarchical identity-preserving attention, VLM semantic understanding, and reinforcement learning to improve identity preservation and video quality.


<details>
  <summary>Details</summary>
Motivation: Existing video generative models are limited to text or single image conditioning, lacking controllability for multi-subject scenarios where preserving multiple subject identities and maintaining temporal consistency are challenging.

Method: Uses hierarchical identity-preserving attention to aggregate features across subjects and modalities, leverages pretrained VLM for semantic understanding, and employs online reinforcement learning (RLVR) to align critical concepts like subject ID.
...
Conclusion: ID-Composer effectively addresses multi-subject video generation challenges through its hierarchical attention mechanism, VLM integration, and reinforcement learning approach, achieving superior performance in preserving subject identities and maintaining video quality.
...
TL;DR: A test-time debiasing method for CLIP models that uses segmentation to isolate target attributes and adjust non-target regions, requiring no training data or bias annotations.


<details>
  <summary>Details</summary>
Motivation: Existing debiasing methods require training data and group labels, limiting practicality. Test-time methods often need prior knowledge of dataset-specific biases, reducing generalizability in open settings.
...
<details>
  <summary>Details</summary>
Motivation: Traditional VAD methods are semi-automated, require human assessment, and only provide binary normal/anomalous outputs, lacking fine-grained detection and interactivity.

Method: Proposes T-VAD with Anomaly Heatmap Decoder for pixel-wise visual-textual feature alignment to generate anomaly heatmaps, and Region-aware Anomaly Encoder to transform heatmaps into learnable textual embeddings for LVLM guidance.

Result: Achieves SOTA performance with 94.8% AUC on UBnormal dataset, 67.8%/76.7% accuracy in anomaly heatmaps, and high BLEU-4 scores (62.67-88.84) and Yes/No accuracy (89.73%-97.67%) for textual descriptions on multiple datasets.

Conclusion: T-VAD significantly enhances granularity and interactivity of video anomaly detection by combining fine-grained heatmap generation with detailed textual anomaly descriptions using vision-language models.
...
TL;DR: Real-IAD Variety is introduced as the largest and most diverse industrial anomaly detection benchmark with 198,960 high-resolution images across 160 object categories, 28 industries, 24 materials, and 22 colors, addressing limitations of existing datasets.


<details>
  <summary>Details</summary>
Motivation: Current IAD algorithms are constrained by limited public benchmarks with restricted category diversity and insufficient scale, causing metric saturation and poor real-world transferability.

Method: Created a comprehensive benchmark with extensive coverage across industries, materials, and colors, and conducted experimental analysis comparing state-of-the-art methods across different category scales.

Result: Multi-class unsupervised methods show significant performance degradation when scaled from 30 to 160 categories, while vision-language models demonstrate remarkable robustness with minimal performance variation across category counts.

Conclusion: Real-IAD Variety's unprecedented scale and complexity make it essential for training next-generation foundation models, enabling development of scalable, general-purpose anomaly detection systems beyond domain-specific constraints.
...
TL;DR: The paper proposes a method for learning and synthesizing multi-instance semantics from a single image using penalty-based attention optimization and box control to handle similar semantics and ensure precise layout control.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in learning multi-instance semantics from limited training data, especially when instances have similar semantics or appearance, which can lead to semantic leakage and poor disentanglement.

Method: Uses penalty-based attention optimization during learning to disentangle similar semantics, and introduces box control optimization in attention layers during synthesis to mitigate semantic leakage while precisely controlling output layout.
...
Conclusion: The proposed approach effectively addresses the challenges of multi-instance semantic learning from single images, providing robust performance even with similar semantics and limited data.
...
Method: Uses a compact set of neural voxels with learned deformation fields to model temporal dynamics instead of separate Gaussian sets per timestamp, plus a novel view refinement stage for challenging viewpoints.
...
Conclusion: 4D-NVS enables efficient dynamic scene modeling with reduced memory consumption and accelerated training while preserving high image quality.
...
TL;DR: FREE framework addresses Domain-Shifted Generalized Category Discovery by using frequency-domain analysis to separate known/unknown domains and apply perturbation strategies, improving category discovery under distribution shifts.


<details>
  <summary>Details</summary>
Motivation: Existing GCD methods perform poorly under distribution shifts, and real-world unlabeled data often comes from unknown domains with different distributions from labeled data.

Method: Frequency-based domain separation, cross-domain and intra-domain frequency perturbation strategies, extended self-supervised contrastive learning, semantic clustering loss, and clustering-difficulty-aware resampling.

Result: Extensive experiments show FREE effectively mitigates distribution shift impacts and achieves superior performance in discovering both known and unknown categories across various benchmarks.

Conclusion: Frequency-domain information is effective for handling distribution shifts in GCD, and the proposed FREE framework provides a robust solution for the challenging DS_GCD task.
...
TL;DR: The paper presents a context-aware zero-shot anomaly detection method that uses memory-augmented pipeline with cross-attention temporal fusion and contextual similarity scoring, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Video anomalies depend on contextual information and temporal evolution, but most detectors don't consider context, limiting their generalization to real-life situations. The work addresses context-aware zero-shot anomaly detection challenge.

Method: Memory-augmented pipeline correlating temporal signals with visual embeddings using cross-attention, and real-time zero-shot anomaly classification by contextual similarity scoring.

Result: Achieved 90.4% AUC on UCF-Crime and 83.67% AP on XD-Violence, setting new state-of-the-art among zero-shot models. Model achieves real-time inference with high precision and explainability.

Conclusion: By fusing cross-attention temporal fusion and contextual memory, the method achieves high fidelity anomaly detection, advancing zero-shot models for real-world surveillance and infrastructure monitoring applications.
...
TL;DR: CueBench is a new benchmark for context-aware video anomaly understanding that evaluates models on recognizing subtle anomalies defined by complex contexts and semantics across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: Current video anomaly detection methods have superficial understanding of real-world anomalies, lacking the ability to comprehend complex contextual differences that distinguish anomalies from normal events (e.g., climbing with vs. without safety gear).

Method: Created CueBench benchmark with event-centric hierarchical taxonomy covering 14 conditional and 18 absolute anomaly events across 174 scenes and 198 attributes. Proposed Cue-R1 model using R1-style reinforcement fine-tuning with verifiable, task-aligned, and hierarchy-refined rewards in a unified generative framework.

Result: Extensive evaluation shows existing vision-language models are far from satisfactory real-world anomaly understanding, while Cue-R1 surpasses state-of-the-art approaches by over 24% on average across various tasks.

Conclusion: The paper establishes that current deep models have limited capability for real-world video anomaly understanding, and demonstrates that the proposed CueBench benchmark and Cue-R1 model provide significant improvements in context-aware anomaly comprehension.
...
TL;DR: The paper proposes triplet segmentation - a new task that spatially grounds surgical action triplets (<instrument, verb, target>) using instrument instance segmentation, addressing limitations of existing frame-level classification methods.


<details>
  <summary>Details</summary>
Motivation: Existing surgical action recognition methods fail to reliably link actions to specific instrument instances and lack spatial precision for detailed instrument-tissue interaction analysis, creating a gap in surgical scene understanding.

Method: Proposes TargetFusionNet architecture that extends Mask2Former with target-aware fusion mechanism, fusing weak anatomy priors with instrument instance queries to improve anatomical target prediction accuracy.

Result: TargetFusionNet consistently improves performance across recognition, detection, and triplet segmentation metrics, demonstrating that strong instance supervision combined with weak target priors significantly enhances surgical action understanding accuracy and robustness.
...
TL;DR: This paper introduces FGI-EMIT, the first large-scale multispectral LiDAR benchmark dataset for individual tree segmentation, and benchmarks both unsupervised and deep learning methods, showing DL approaches significantly outperform traditional methods, especially for understory trees.


<details>
  <summary>Details</summary>
Motivation: The lack of large-scale multispectral LiDAR benchmark datasets has hindered progress in individual tree segmentation, despite evidence that multispectral reflectance can improve accuracy. There's a need for standardized evaluation and comparison of methods.
...
Result: DL methods significantly outperformed unsupervised approaches - ForestFormer3D achieved best F1-score of 73.3% vs Treeiso's 52.7%. The biggest gap was in understory trees (25.9 percentage point difference). DL methods maintained superiority even at low point densities (10 points/m²).

Conclusion: Deep learning approaches are superior for individual tree segmentation, particularly for challenging cases like understory trees. However, current DL methods fail to effectively leverage multispectral reflectance information, suggesting room for improvement in feature utilization.
...
TL;DR: MR-CLIP is a framework that learns MRI contrast representations by aligning images with DICOM acquisition parameters, enabling sequence classification and quality control without manual annotations.


<details>
  <summary>Details</summary>
Motivation: MRI suffers from data heterogeneity and lack of standardized contrast labels across different scanners and protocols, limiting large-scale automated analysis.

Method: Aligns volumetric MRI images with their DICOM acquisition parameters to learn unified contrast representations through metadata-guided learning.

Result: Produces embeddings that form distinct MRI sequence clusters, outperforms supervised 3D baselines in few-shot classification, and enables unsupervised quality control by detecting metadata inconsistencies.

Conclusion: MR-CLIP provides a scalable foundation for label-efficient MRI analysis by leveraging routinely available acquisition metadata as supervisory signal.
...
TL;DR: The paper proposes a dual-region quantization strategy and sensitivity-aware finetuning for post-training quantization of image super-resolution networks, achieving performance comparable to quantization-aware training with significant speedup.


<details>
  <summary>Details</summary>
Motivation: Existing PTQ methods for SR networks fail to achieve satisfactory performance because they overlook the impact of activation outliers, which are strongly correlated with image color information and cannot be directly removed without performance degradation.

Method: Proposes a dual-region quantization strategy that partitions activations into outlier and dense regions, applying uniform quantization to each region independently. Also introduces sensitivity-aware finetuning that focuses more on highly sensitive layers.

Result: Extensive experiments show the method outperforms existing PTQ approaches across various SR networks and datasets, achieving performance comparable to QAT methods in most scenarios with at least 75x speedup.

Conclusion: The proposed dual-region quantization with sensitivity-aware finetuning effectively addresses activation outliers and layer sensitivity issues in PTQ for SR networks, providing a practical alternative to QAT with significant inference acceleration.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models often produce limited output diversity, which restricts their usefulness for creative exploration and ideation tasks where varied visual outputs are needed.

Method: Uses LLM for semantic evolution of prompts, CLIP embeddings to measure novelty, and emitters to guide search into distinct regions of prompt space to boost diversity.

Result: WANDER significantly outperforms existing evolutionary prompt optimization baselines in diversity metrics when evaluated using FLUX-DEV for generation and GPT-4o-mini for mutation.

Conclusion: The approach successfully addresses diversity limitations in text-to-image generation, with ablation studies confirming the effectiveness of emitters in enhancing output variety.
...
TL;DR: Analysis of loss functions for LDCT image enhancement reveals inconsistencies between loss functions and quality metrics, highlighting the need to consider perceptual quality metrics when developing new loss functions.


<details>
  <summary>Details</summary>
Motivation: LDCT imaging reduces radiation exposure but suffers from noise and artifacts. While deep learning models enhance LDCT images, current loss functions focus on PSNR/SSIM which don't adequately reflect perceptual quality, especially for medical images.
...
Result: Found inconsistencies between loss functions and quality metrics, showing that commonly used metrics like PSNR and SSIM are limited in reflecting perceptual quality for medical images.

Conclusion: There is a critical need to consider image quality metrics when developing new loss functions for LDCT image enhancement to ensure better perceptual quality in medical imaging.
...
TL;DR: Deep learning models trained on North American ADNI data show poor generalization to Latin American FLENI cohort, with performance dropping from .96-.97 AUC to .82-.80 AUC due to domain shift.


<details>
  <summary>Details</summary>
Motivation: To assess generalization of Alzheimer's disease diagnostic models from dominant North American datasets to underrepresented Latin American populations.

Method: Benchmark convolutional and Transformer-based models on ADNI dataset and evaluate generalization on FLENI cohort. Conduct ablation studies on normalization and sampling, plus occlusion sensitivity analysis.

Result: Significant performance drop (AUC from .96-.97 to .82-.80) reveals domain shift. No clear advantage for transformers. Per-image normalization and proper sampling are key for generalization.

Conclusion: Population-aware validation is crucial for diagnostic AI models. Future work needed on domain adaptation and cohort diversification.
...
<details>
  <summary>Details</summary>
Motivation: Most existing place recognition methods rely on contrastive learning, but an alternative classification approach could offer advantages in training efficiency and stability.
...
Result: Achieves competitive performance compared to contrastive learning-based methods on the NuScenes dataset.

Conclusion: Multi-class classification is a viable alternative to contrastive learning for place recognition, offering training efficiency and stability benefits.
...
TL;DR: This study investigates how generative AI models encode Western beauty standards and erase 'ugliness', finding significant demographic biases including light skin tone preference (86.5%), younger age depiction (74%), and troubling intersectional effects where non-binary individuals are portrayed as younger and more hypersexualized.


<details>
  <summary>Details</summary>
Motivation: Social media exacerbates Western beauty norms causing negative self-image and body dysmorphia, particularly in women and girls. With increasing AI-generated content, there are concerns these harmful beauty standards are being exaggerated and perpetuated by generative models.

Method: Created two image generation pipelines (text-to-image and text-to-language-to-image), developed a structured beauty taxonomy, prompted three language models and two text-to-image models to generate 5984 images, and conducted a Likert-scale study with women and non-binary social media users evaluating 1200 images.

Result: 86.5% of generated images depicted lighter skin tones, 22% contained explicit content despite SFW training, 74% were rated as younger demographic. Non-binary individuals were rated as younger and more hypersexualized. Prompts with 'negative' beauty traits consistently produced higher NSFW ratings regardless of gender.

Conclusion: Generative AI models contain pervasive demographic biases related to beauty standards that are actively perpetuated by model developers through practices like negative prompting. This leads to pollution of data streams and erasure of features outside stereotypical beauty norms, with significant societal implications.
...
TL;DR: IoT-based animal detection system for durian plantations using combined YOLOv5 and SSD algorithms with Telegram notifications and automated sound deterrents.


<details>
  <summary>Details</summary>
Motivation: Traditional farming practices are ineffective against animal intrusions in durian plantations, causing crop damage and financial losses. Current systems have limitations including single algorithm dependency, poor notification platforms, and limited deterrent mechanisms.

Method: Integrated YOLOv5 and SSD object detection algorithms for improved accuracy, real-time monitoring with Telegram notifications, and automated sound deterrent mechanism (e.g., tiger roar) triggered upon animal detection.

Result: YOLO+SSD model achieved accuracy rates: elephant 90%, boar 85%, monkey 70%. System performs best during daytime and decreases at night, with consistent performance on both still images and videos.

Conclusion: The study provides a comprehensive framework combining detection, notification, and deterrence, offering practical automated farming solutions with potential for future innovations.
...
TL;DR: A method for 3D instance segmentation that uses granularity-consistent 2D mask tracking to eliminate conflicting pseudo labels from foundation models, combined with progressive curriculum learning from fragmented to coherent supervision.


<details>
  <summary>Details</summary>
Motivation: To avoid costly manual 3D annotations by using 2D masks from foundation models, but existing methods process frames independently causing inconsistent segmentation granularity and conflicting 3D pseudo labels that degrade accuracy.

Method: Granularity-Consistent automatic 2D Mask Tracking to maintain temporal correspondences across frames, combined with three-stage curriculum learning that progressively trains from fragmented single-view data to unified multi-view annotations and globally coherent full-scene supervision.
...
Conclusion: The structured learning pipeline enables robust distillation of consistent 3D representations from initially fragmented 2D priors, solving the inconsistency problem in existing pseudo-label generation approaches.
...
TL;DR: FedOnco-Bench is a reproducible benchmark for privacy-aware federated learning using synthetic oncologic CT scans, evaluating segmentation performance and privacy leakage across FL methods with distinct privacy-utility trade-offs.


<details>
  <summary>Details</summary>
Motivation: FL systems remain vulnerable to membership-inference attacks and data heterogeneity, especially in privacy-sensitive medical environments where sensitive data must be protected.
...
Conclusion: FedOnco-Bench serves as a standardized, open-source platform for benchmarking and developing privacy-preserving FL methods for medical image segmentation, demonstrating clear privacy-utility trade-offs in federated learning.
...
TL;DR: Med-Banana-50K is a 50K-image dataset for medical image editing across three modalities and 23 diseases, featuring bidirectional edits generated by Gemini-2.5-Flash-Image with medical quality control through LLM-as-Judge evaluation.


<details>
  <summary>Details</summary>
Motivation: The research community lacks large-scale, high-quality, openly accessible datasets specifically designed for medical image editing with anatomical and clinical constraints, which hinders progress in multimodal medical AI.

Method: Dataset construction using Gemini-2.5-Flash-Image to generate bidirectional edits (lesion addition/removal) from real medical images, with systematic medical quality control via LLM-as-Judge evaluation and iterative refinement up to five rounds.

Result: Created a comprehensive 50K-image dataset spanning chest X-ray, brain MRI, and fundus photography with 23 disease types, including 37K failed attempts for preference learning, all medically validated and publicly available.

Conclusion: Med-Banana-50K establishes a foundational resource for training and evaluating next-generation medical image editing models, addressing the critical gap in medically-constrained editing datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based GUI grounding approaches directly generate coordinates from visual inputs, which is challenging and computationally intensive. The authors propose a more intuitive approach by first selecting relevant visual patches and then determining click locations within those patches.

Method: GUI-AIMA uses an attention-based supervised fine-tuning framework that aligns MLLMs' multimodal attention with patch-wise grounding signals calculated adaptively through multi-head aggregation on simplified query-visual attention matrices. It operates in a coordinate-free manner and can integrate a plug-and-play zoom-in stage.

Result: GUI-AIMA-3B trained with only 85k screenshots achieves state-of-the-art performance among 3B models, with 58.6% average accuracy on ScreenSpot-Pro and 62.2% on OSWorld-G, demonstrating exceptional data efficiency.

Conclusion: Light training can effectively trigger the native grounding capability of MLLMs, and GUI-AIMA provides an efficient coordinate-free approach for GUI grounding that outperforms existing methods while requiring significantly less training data.
...
TL;DR: TA-LSDiff combines topology-aware diffusion probabilistic models with level set energy for pancreas segmentation, achieving state-of-the-art accuracy without explicit geometric evolution.
...
Method: Proposes TA-LSDiff model combining topology-aware diffusion probabilistic model and level set energy with four complementary terms. Includes pixel-adaptive refinement module using affinity weighting for boundary precision.

Result: Achieves state-of-the-art accuracy on four public pancreas datasets, outperforming existing methods. Ablation studies confirm contribution of each component.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs use unified position encoding strategies that treat text and visual tokens uniformly, failing to account for their distinct structural properties - sequential continuity for text and spatial coherence for vision.
...
Conclusion: The proposed OMEGA framework effectively addresses the limitations of unified position encoding in VLMs by providing modality-specific handling and adaptive scaling, leading to significant performance improvements across various model architectures and tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal adversarial attack methods suffer from overfitting due to lack of input diversity, relying too much on adversarial examples from one modality when attacking another.
...
Result: LSSA significantly improves transferability of multimodal adversarial examples across diverse VLP models and downstream tasks, and outperforms other advanced attacks on Large Vision-Language Models.

Conclusion: The proposed LSSA method effectively addresses overfitting in multimodal adversarial attacks by increasing input diversity through local shuffling and sampling, achieving superior transferability performance.
...
TL;DR: VCA is a drop-in replacement for MHSA in Vision Transformers that reduces computational complexity from O(N²C) to O(NnC) by using visual-contrast tokens and positive/negative streams for discriminative attention.


<details>
  <summary>Details</summary>
Motivation: Current Vision Transformers spend most computation on quadratic query-key interactions for all token pairs, including visually weak or redundant correlations, which is inefficient.

Method: VCA distills dense query fields into pooled visual-contrast tokens, splits them into positive and negative streams, and uses their differential interaction to highlight discriminative regions.

Result: VCA improves DeiT-Tiny accuracy on ImageNet-1K from 72.2% to 75.6% (+3.4%), enhances hierarchical ViTs by up to 3.1%, and reduces FID in image generation by 2.1-5.2 points across diffusion and flow models.

Conclusion: VCA provides a simple, efficient way to build faster and more accurate Vision Transformers with explicit discrimination mechanisms and reduced computational complexity.
...
TL;DR: PIAT is a novel adversarial training framework that reduces oscillations and overfitting by interpolating model parameters between epochs and using NMSE to align logit magnitudes between clean and adversarial examples.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial training methods suffer from oscillations and overfitting during training, which degrades model robustness against adversarial attacks.

Method: Parameter Interpolation Adversarial Training (PIAT) interpolates model parameters between epochs to create smoother decision boundaries, and uses Normalized Mean Square Error (NMSE) to align relative logit magnitudes between clean and adversarial examples.

Result: Extensive experiments show PIAT significantly improves robustness for both CNNs and Vision Transformers across multiple benchmark datasets.
...
TL;DR: OmniBrainBench is the first comprehensive multimodal VQA benchmark for brain imaging analysis, covering 15 imaging modalities and 15 clinical tasks across 9,527 VQA pairs and 31,706 images, revealing significant performance gaps between MLLMs and physicians.


<details>
  <summary>Details</summary>
Motivation: Current brain imaging VQA benchmarks are limited in modality coverage and pathological granularity, hindering comprehensive assessment of MLLMs throughout the full clinical continuum.

Method: Created OmniBrainBench with 15 brain imaging modalities from 30 medical sources, featuring 9,527 validated VQA pairs and 31,706 images that simulate clinical workflows across 15 multi-stage clinical tasks validated by a professional radiologist.

Result: Evaluation of 24 state-of-the-art models shows: (1) proprietary MLLMs outperform open-source and medical models but lag physicians; (2) medical MLLMs show wide performance variation; (3) open-source models trail overall but excel in specific tasks; (4) MLLs underperform significantly in complex preoperative tasks, revealing a visual-to-clinical reasoning gap.

Conclusion: OmniBrainBench sets a new standard for evaluating MLLMs in brain imaging analysis, highlighting substantial gaps compared to expert clinical reasoning and providing a comprehensive benchmark for advancing multimodal AI in medical imaging.
...
TL;DR: Proposes an Occlusion-Aware Diffusion Model (ODM) for pedestrian crossing intention prediction that handles occlusion scenarios by reconstructing occluded motion patterns and using occlusion-aware transformers.
...
Method: Uses an occlusion-aware diffusion transformer architecture to estimate noise features of occluded patterns, and introduces an occlusion mask-guided reverse process to utilize observation information and reduce prediction error accumulation.

Result: Achieves more robust performance than existing methods on PIE and JAAD benchmarks under various occlusion scenarios.

Conclusion: The proposed ODM method effectively handles occlusion scenarios in pedestrian crossing intention prediction and outperforms existing approaches.
...
TL;DR: LMD is a post-hoc interpretability method that disentangles modality-specific contributions in sensor fusion models for autonomous driving, enabling attribution of predictions to individual sensors like cameras, radar, and LiDAR.


<details>
  <summary>Details</summary>
Motivation: Transparency in autonomous driving perception models is critical for safety, but multi-sensor fusion makes it difficult to understand how each modality contributes to predictions due to information entanglement.
...
Conclusion: LMD provides practical interpretability for high-capacity multimodal architectures in autonomous driving, enabling better understanding of sensor fusion decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Traditional retrieval methods are limited by database coverage, while individual LVLMs struggle with diverse geographic regions and complex scenes. Existing multi-agent systems lack mechanisms to handle conflicting predictions effectively.

Method: Uses heterogeneous graph neural networks with typed edges to model supportive collaboration, competitive argumentation, and knowledge transfer. Features dual-level debate mechanism combining node-level refinement and edge-level argumentation modeling, plus cross-level topology refinement.
...
TL;DR: Fleming-VL is a unified end-to-end framework for medical visual understanding across heterogeneous modalities (2D, 3D, video) that addresses domain gaps through data-centric strategies including pretraining scaling, rare data fine-tuning, and extended evaluation benchmarks.
...
Method: Three key data-centric strategies: (1) scaling up pretraining with long-context natural and medical data, (2) complementing fine-tuning with rare medical data including video analysis and underrepresented 2D modalities, (3) extending evaluation frameworks to include 3D and video benchmarks. Uses supervised fine-tuning (SFT) and group relative policy optimization (GRPO).
...
Conclusion: The framework enables comprehensive medical visual understanding across heterogeneous modalities and is publicly released to promote transparent, reproducible progress in medical AI.
...
Conclusion: The proposed approach effectively addresses modality imbalance and information inconsistency in ZS-SBIR, achieving state-of-the-art results.
...
TL;DR: EVTAR is an end-to-end virtual try-on model that directly fits garments onto person images using additional reference images, eliminating the need for complex inputs like masks or pose data.
...
Method: Two-stage training strategy with simple inference using only source image and target garment inputs. Leverages additional reference images of different people wearing the same clothes to preserve garment texture and details. Enriches training data with supplementary references and unpaired person images.
...
Conclusion: EVTAR provides a more practical and realistic virtual try-on solution by simulating how humans consider reference models when choosing outfits, generating high-quality dressing effects without complex preprocessing requirements.
...
<details>
  <summary>Details</summary>
Motivation: Current video anomaly methods lack explainability, providing only frame-wise scores without spatial or semantic context, and remain data-dependent and task-specific.

Method: Chained test-time reasoning process that sequentially connects temporal detection, spatial localization, and textual explanation using intra-task reasoning and inter-task chaining with careful prompt design.
...
<details>
  <summary>Details</summary>
Motivation: Foundation models like SAM perform poorly on vascular structures due to thin, branching anatomy and low texture contrast, creating a need for specialized vessel segmentation methods.

Method: Integrates convolutional adapter for local texture features, multi-prompt encoder with anatomical prompts (skeletons, bifurcation points, midpoints) via hierarchical cross-attention, and lightweight mask decoder to reduce artifacts.

Result: Outperforms PEFT-based SAM variants by over 10% Dice and 13% IoU, achieves competitive performance with fully fine-tuned methods using fewer parameters, and generalizes well to out-of-distribution settings.

Conclusion: VesSAM provides an efficient and powerful solution for vessel segmentation that addresses the limitations of generic foundation models while maintaining parameter efficiency and strong generalization.
...
TL;DR: A self-supervised multimodal iterative denoising (MID) framework that models noisy data as states in a continuous noise accumulation process, learning to estimate noise steps and remove noise iteratively without requiring clean-noisy paired datasets.


<details>
  <summary>Details</summary>
Motivation: Real-world data is often corrupted by complex, non-linear noise that traditional rule-based denoising methods cannot effectively handle, necessitating more robust and adaptable approaches.

Method: MID models noisy data as states in continuous non-linear noise accumulation, learns two neural networks to estimate current noise step and predict/subtract noise increments, and uses first-order Taylor expansion to linearize complex non-linear noise for iterative removal.
...
Conclusion: MID provides an effective self-supervised framework for denoising complex non-linear noise without requiring paired datasets, showing broad applicability across multiple domains including computer vision, biomedical, and bioinformatics.
...
TL;DR: A multimodal machine learning framework using identical feature extraction on both visual and X-ray images achieves 97.8% accuracy in authenticating Goya paintings, outperforming single-modal approaches.


<details>
  <summary>Details</summary>
Motivation: Art authentication of Goya's works is challenging due to his heterogeneous stylistic evolution and extensive forgery history, requiring advanced computational methods.

Method: Unified feature extraction (GLCM, LBP, entropy, energy, color analysis) applied to both visual and X-ray images, processed through optimized One-Class SVM with hyperparameter tuning on 24 authenticated paintings.

Result: 97.8% classification accuracy with 0.022 false positive rate; case study on "Un Gigante" achieved 92.3% authentication confidence.

Conclusion: Multimodal approach with identical computational methods applied to both visual and radiographic imagery significantly improves art authentication performance over single-modal methods.
...
TL;DR: HyFormer-Net is a hybrid CNN-Transformer model for breast ultrasound that simultaneously performs segmentation and classification with intrinsic interpretability, achieving superior performance and demonstrating strong cross-dataset generalization through progressive fine-tuning.
...
Method: Proposed HyFormer-Net with dual-branch encoder integrating EfficientNet-B3 and Swin Transformer via multi-scale hierarchical fusion blocks. Uses attention-gated decoder for precision and explainability. Introduces dual-pipeline interpretability with intrinsic attention validation and Grad-CAM for classification reasoning.

Result: On BUSI dataset: Dice Score 0.761 +/- 0.072, accuracy 93.2%, malignant recall 92.1 +/- 2.2%. Ensemble modeling achieves exceptional Dice 90.2%, accuracy 99.5%, and perfect 100% malignant recall. Cross-dataset study shows progressive fine-tuning with 10% target data recovers 92.5% performance, and with 50% data achieves 77.3% Dice, exceeding source-domain performance.

Conclusion: HyFormer-Net outperforms existing methods and demonstrates the first successful cross-dataset generalization for hybrid CNN-Transformers in breast ultrasound. The model's interpretability features and ability to achieve strong performance with minimal target-domain data make it clinically viable.
...
TL;DR: FastBoost is a parameter-efficient neural architecture using Dynamically Scaled Progressive Attention (DSPA) that achieves state-of-the-art performance on CIFAR benchmarks with significantly reduced parameters.


<details>
  <summary>Details</summary>
Motivation: To create a highly parameter-efficient neural network that maintains high accuracy while being suitable for resource-constrained edge devices, addressing the trade-off between model size and performance.

Method: Uses Dynamically Scaled Progressive Attention (DSPA) with three innovations: Adaptive Fusion (channel-spatial attention blending), Phase Scaling (training-stage-aware intensity modulation), and Residual Adaptation (self-optimized skip connections). Integrated with enhanced MBConv blocks and dual attention pathways.

Result: Achieved CIFAR-10: 95.57% accuracy (0.85M parameters) and 93.80% (0.37M parameters); CIFAR-100: 81.37% accuracy (0.92M parameters) and 74.85% (0.44M parameters). 2.1x parameter reduction over MobileNetV3 with +3.2% accuracy improvement on CIFAR-10.

Conclusion: FastBoost demonstrates unprecedented parameter-accuracy trade-offs through co-optimization of dynamic attention and efficient convolution, enabling deployment in edge devices without accuracy degradation.
...
TL;DR: T-MLA is a targeted multiscale log-exponential attack framework that exploits vulnerabilities in neural image compression systems by crafting adversarial perturbations in the wavelet domain, causing significant reconstruction quality degradation while remaining visually imperceptible.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attacks on neural image compression (NIC) are naive adaptations of pixel-space methods, overlooking the structured nature of compression pipelines. There's a need for more advanced vulnerability analysis specific to NIC systems.

Method: Proposed T-MLA framework crafts adversarial perturbations directly in the wavelet domain, strategically confining perturbations to specific wavelet subbands to maximize distortion while ensuring perceptual stealth. This is a principled, offline attack approach.
...
Conclusion: The findings expose critical security vulnerabilities in neural image compression systems that could impact generative and content delivery pipelines, highlighting the need for more robust defense mechanisms.
...
Method: Uses S2 cells (nested multiresolution global grid) and sequentially predicts finer-level cells conditioned on visual inputs and previous predictions, similar to autoregressive text generation. Incorporates beam search and multi-sample inference with various selection strategies.
...
Conclusion: The hierarchical sequence prediction approach with autoregressive sampling and test-time compute scaling techniques effectively addresses image geolocalization challenges, setting new state-of-the-art performance benchmarks.
...
TL;DR: SliceVision-F2I is a synthetic dataset that transforms network KPI vectors into visual representations using four encoding methods, designed for visual learning and ML applications in network slicing.
...
Method: Created dataset by transforming multivariate KPI vectors into RGB images using four encoding methods: physically inspired mappings, Perlin noise, neural wallpapering, and fractal branching, with 30,000 samples per method.

Result: Generated a comprehensive dataset with raw KPI vectors and corresponding low-resolution RGB images, simulating realistic network conditions with operational uncertainties.

Conclusion: SliceVision-F2I is publicly available and suitable for visual learning, network state classification, anomaly detection, and benchmarking image-based ML techniques in network data analysis.
...
TL;DR: Novel method combining Epanechnikov kernel density estimation with bimodal logistic regression for respiratory disease diagnosis from chest X-rays, achieving moderate performance with 70.14% accuracy.


<details>
  <summary>Details</summary>
Motivation: To develop a flexible statistical approach for medical image analysis that doesn't rely on specific distribution assumptions and can adapt to pixel intensity variations in chest X-rays.
...
Result: Tested on 13,808 chest X-rays from COVID-19 Radiography Dataset, achieving 70.14% accuracy, 59.26% sensitivity, and 74.18% specificity, showing moderate performance with room for sensitivity improvement.
...
TL;DR: ViACT integrates anatomical priors into video transformers for echocardiogram analysis, focusing representation learning on anatomical regions like the myocardium to avoid spurious correlations from non-diagnostic areas.


<details>
  <summary>Details</summary>
Motivation: Video transformers for echocardiogram analysis tend to learn spurious correlations from non-diagnostic regions like image backgrounds, limiting their diagnostic accuracy and interpretability.

Method: ViACT represents deforming anatomical structures as point sets and encodes spatial geometry with image patches into transformer tokens. It uses masked autoencoding that only reconstructs anatomical patches during pre-training, enforcing focus on anatomical regions.

Result: ViACT produces interpretable attention maps aligned with known pathology regions for cardiac amyloidosis detection, generalizes to myocardium point tracking without specialized components, and improves performance on tasks like left ventricular ejection fraction regression.

Conclusion: Integrating anatomical constraints into video transformers effectively focuses learning on diagnostically relevant regions, improving interpretability and generalization while avoiding spurious correlations from non-diagnostic areas.
...
TL;DR: Using GPU-equipped embedded devices in edge computing to improve performance of computer vision applications, especially for AR on mobile devices.
...
Method: Proposes using embedded devices with GPUs in edge computing infrastructure to offload intensive computer vision tasks from mobile devices.

Result: Experiments showed GPUs achieve significant performance gains compared to CPUs alone, ensuring better user experience for computer vision applications.

Conclusion: GPU-enabled embedded devices in edge computing effectively overcome resource limitations and improve performance for computer vision and AR applications on mobile platforms.
...
TL;DR: PCP is a weakly supervised framework for medical imaging that predicts clinical concepts without requiring concept annotations, using class-level priors and refinement mechanisms to achieve better performance than zero-shot methods.


<details>
  <summary>Details</summary>
Motivation: Current interpretable-by-design frameworks need costly concept annotations, and zero-shot methods fail to capture domain-specific medical features, creating reliability issues in clinical applications.

Method: Uses Prior-guided Concept Predictor (PCP) with class-level concept priors as weak supervision, incorporating KL divergence and entropy regularization to align predictions with clinical reasoning.

Result: Improves concept-level F1-score by over 33% vs zero-shot baselines on PH2 and WBCatt datasets, achieves competitive classification performance on four medical datasets compared to fully supervised CBMs and V-IP.

Conclusion: PCP provides effective interpretable predictions for medical imaging without requiring concept annotations, bridging the gap between annotation-free methods and fully supervised approaches.
...
<details>
  <summary>Details</summary>
Motivation: To systematically encode temporal, amplitude, and structural symmetries in HAR data to improve robustness and generalization without increasing model capacity.
...
Result: On UCI-HAR dataset under out-of-distribution perturbations, CatEquiv achieves markedly higher robustness compared to circularly padded CNNs and plain CNNs.

Conclusion: Enforcing categorical symmetries yields strong invariance and generalization benefits without requiring additional model capacity.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning polyp segmentation models either provide ambiguous polyp margins that compromise clinical decision-making or use heavy architectures with high computational complexity, making them unsuitable for real-time endoscopic applications.

Method: Proposed MicroAUNet combines depthwise-separable dilated convolutions with a single-path, parameter-shared channel-spatial attention block to strengthen multi-scale boundary features, and uses a progressive two-stage knowledge-distillation scheme to transfer semantic and boundary cues from a high-capacity teacher network.

Result: Extensive experiments on benchmarks demonstrate state-of-the-art accuracy under extremely low model complexity, making MicroAUNet suitable for real-time clinical polyp segmentation.

Conclusion: MicroAUNet provides an effective solution for real-time colorectal polyp segmentation by balancing accuracy and computational efficiency through lightweight architecture design and knowledge distillation techniques.
...
TL;DR: ROVER is a benchmark for evaluating reciprocal cross-modal reasoning in unified multimodal models, testing how models use one modality to guide outputs in the other modality across 1312 tasks grounded in 1876 images.


<details>
  <summary>Details</summary>
Motivation: Current evaluations treat multimodal abilities in isolation, focusing on unimodal reasoning rather than testing the crucial ability of reciprocal cross-modal reasoning where one modality guides, verifies, or refines outputs in the other.
...
Conclusion: Reciprocal cross-modal reasoning is a critical frontier for enabling true omnimodal generation, highlighting the importance of testing how models integrate and leverage information across different modalities.
...
TL;DR: The paper introduces an automated pipeline for mining YouTube videos to create a large-scale animal video dataset (30K videos, 2M frames), presents the Animal-in-Motion benchmark for 4D quadruped reconstruction, and establishes baseline methods showing a gap between 2D metrics and realistic 3D reconstructions.


<details>
  <summary>Details</summary>
Motivation: Current animal video datasets are limited in scale and lack processing for animal-centric 3D/4D tasks, while existing collection methods rely on controlled capture setups rather than in-the-wild videos.

Method: Developed an automated pipeline to mine YouTube videos and process them into object-centric clips with auxiliary annotations. Created Animal-in-Motion benchmark with 230 manually filtered sequences (11K frames) for 4D reconstruction. Evaluated state-of-the-art model-based and model-free methods, and enhanced a model-free approach with sequence-level optimization.

Result: Collected 30K videos (2M frames) - an order of magnitude more than prior works. Found that 2D metrics favor model-based methods despite unrealistic 3D shapes, while model-free methods yield more natural reconstructions but score lower, revealing an evaluation gap. Established the first 4D animal reconstruction baseline.

Conclusion: The pipeline, benchmark, and baseline advance large-scale, markerless 4D animal reconstruction and related tasks from in-the-wild videos, addressing limitations in current datasets and evaluation methods.
...
Method: Uses Multi-level Discrete Wavelet Transform to decompose images into wavelet spectra, pyramid tokenization for transformer input, and a dual-decoder to handle low-frequency and high-frequency sub-bands while maintaining alignment.
...
TL;DR: Proposes GCN-PSN, a topology-aware Graph Convolutional Network that models human skeleton as a graph to learn discriminative pose embeddings for Action Quality Assessment, outperforming coordinate-based methods.


<details>
  <summary>Details</summary>
Motivation: Action Quality Assessment requires fine-grained understanding of human motion and precise evaluation of pose similarity, which can be better captured by modeling skeletal topology rather than just coordinates.

Method: Uses a topology-aware Graph Convolutional Network (GCN) framework that models human skeleton as a graph, with Siamese architecture trained using contrastive regression objective to learn pose embeddings.
...
Conclusion: Modeling skeletal topology through GCNs is effective for pose similarity and action quality assessment, providing better performance than coordinate-based approaches.
...
TL;DR: MoSa is a hierarchical motion generation framework that uses a coarse-to-fine scalable process with Multi-scale Token Preservation Strategy and hierarchical RQ-VAE to generate 3D human motions from text, achieving state-of-the-art performance with only 10 inference steps.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional motion generation methods that require many inference steps and struggle with maintaining multi-scale motion details, aiming to improve both generation quality and efficiency.

Method: Proposes Multi-scale Token Preservation Strategy (MTPS) with hierarchical RQ-VAE, Scalable Autoregressive modeling that predicts scale tokens, and CAQ-VAE - a convolution-attention hybrid VQ-VAE to prevent reconstruction degradation from interpolation.

Result: Achieves FID of 0.06 on Motion-X dataset (vs MoMask's 0.20), reduces inference time by 27%, and generalizes well to downstream tasks like motion editing without additional fine-tuning.

Conclusion: MoSa demonstrates superior motion generation quality and efficiency through its hierarchical coarse-to-fine approach, setting new state-of-the-art benchmarks while maintaining strong generalization capabilities.
...
TL;DR: OmniVLA is an omni-modality vision-language-action model that integrates multiple sensing modalities (infrared camera, mmWave radar, microphone array) using sensor-masked images to enhance robotic manipulation capabilities beyond RGB-only perception.
...
Conclusion: Integrating multiple sensing modalities through sensor-masked images significantly enhances VLA model performance for robotic manipulation tasks, demonstrating the value of omni-modality perception beyond RGB-only approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing VQA systems are biased towards Western foods and don't adequately handle the cultural and culinary diversity of Indian cuisines. Current Indian food VQA approaches use a two-step process that doesn't capture the complex reasoning needed for accurate food identification.
...
Conclusion: Multi-step reasoning chains significantly enhance performance for Indian Food VQA by enabling better understanding of complex culinary contexts and relationships between food items.
...
Method: Four training methods were evaluated: baseline model trained on US right-hand driving data; model trained on flipped US data; model pretrained on US data then fine-tuned on Australian highways; and model pretrained on flipped US data then fine-tuned on Australian highways. Saliency-based analysis was used to measure attention shifts.

Result: Pretraining on flipped data alone worsens prediction stability due to misaligned feature representations, but significantly improves adaptation when followed by fine-tuning, leading to lower prediction error and stronger focus on left-side cues. Similar trends were confirmed with ResNet architecture.
...
TL;DR: The paper identifies flaws in human evaluation practices for speech-driven 3D gesture generation and introduces a standardized evaluation protocol for the BEAT2 dataset. Through large-scale crowdsourced evaluation of six models, it reveals that newer models don't consistently outperform older ones, published claims may not hold under rigorous testing, and the field needs disentangled assessments of motion quality and multimodal alignment.


<details>
  <summary>Details</summary>
Motivation: To address the lack of standardization and flawed experimental setups in human evaluation of automated speech-driven 3D gesture generation, which makes it impossible to compare different methods or determine state-of-the-art performance.

Method: Introduced a detailed human evaluation protocol for the BEAT2 motion-capture dataset and conducted large-scale crowdsourced evaluation to rank six recent gesture-generation models across motion realism and speech-gesture alignment dimensions.

Result: The evaluation showed that: 1) newer models don't consistently outperform earlier approaches; 2) published claims of high motion realism or speech-gesture alignment may not hold under rigorous evaluation; 3) the field requires disentangled assessments of motion quality and multimodal alignment for accurate benchmarking.

Conclusion: The field must adopt standardized evaluation protocols with disentangled assessments of motion quality and multimodal alignment to enable meaningful progress. The authors will release synthetic motion data, rendered video stimuli, open-source rendering scripts, and human preference votes to drive standardization and enable new evaluation research.
...
<details>
  <summary>Details</summary>
Motivation: Human gaze provides valuable supervisory signals for understanding visual attention in complex environments, particularly in egocentric videos where viewer attention is crucial for task assessment.

Method: Inject gaze-derived features into Vision Transformer attention mechanism to bias spatial feature selection toward human-attended regions, using a depth-aware gaze-guided framework for egocentric videos.

Result: Demonstrated consistent gains in detection accuracy over gaze-agnostic baselines on custom simulator dataset and public benchmarks (Ego4D Ego-Motion and Ego-CH-Gaze datasets), with ablation studies validating effectiveness.

Conclusion: The gaze-integrated approach effectively enhances object detection by emphasizing viewer-prioritized areas, with introduced gaze-aware attention head importance metric revealing how gaze cues modulate transformer attention dynamics.
...
TL;DR: The paper introduces Adversarial Flatness Attack (AFA) to address deceptive flatness in transferable adversarial attacks, using dual-order information and MonteCarlo Adversarial Sampling to improve transferability across models.


<details>
  <summary>Details</summary>
Motivation: Current transferable attacks focus on flat losses but fall into suboptimal deceptive flatness regions, limiting their effectiveness against unknown victim models.

Method: Proposes Adversarial Flatness (AF) with theoretical assurance, implements AFA attack addressing gradient sign issues, and enhances sampling efficiency with MonteCarlo Adversarial Sampling (MCAS).

Result: Superior performance on ImageNet dataset, outperforming six baselines with flatter adversarial regions and improved transferability across architectures and input transformations.
...
TL;DR: CenterMamba-SAM is an end-to-end framework for brain lesion segmentation that uses a frozen pretrained backbone with lightweight adapters, featuring a novel 3x3 corner-axis-center scanning strategy and memory-driven prompts for improved boundary sensitivity and inter-slice coherence.
...
Method: Proposes CenterMamba-SAM with: 1) CenterMamba encoder using 3x3 corner-axis-center short-sequence scanning for center-prioritized information aggregation, 2) Memory-driven structural prompt generator with prototype bank for automatic prompt synthesis, 3) Memory-augmented multi-scale decoder with attention modules and progressive refinement.
...
Conclusion: The framework effectively addresses brain lesion segmentation challenges through its novel scanning strategy and memory-driven components, achieving superior performance while maintaining computational efficiency.
...
TL;DR: A Light Geometry-aware adapter improves LiDAR semantic segmentation in adverse weather by preserving neighbor continuity and applying region-aware regularization to structurally fragile areas, achieving significant performance gains without target labels.
...
Method: The adapter aligns azimuth and applies horizontal circular padding to preserve neighbor continuity. It uses local-window K-Nearest Neighbors to compute local statistics, compressing them into geometry-aware cues that drive region-aware regularization during training.
...
Conclusion: Geometry-driven regularization is a key direction for all-weather LiDAR segmentation, with the plug-and-play adapter providing significant improvements at negligible inference cost.
...
TL;DR: MotionStream enables real-time motion-conditioned video generation with sub-second latency and up to 29 FPS streaming, addressing the limitations of current methods that suffer from minutes-long latency and non-causal processing.


<details>
  <summary>Details</summary>
Motivation: Current motion-conditioned video generation methods have prohibitive latency (minutes per video) and non-causal processing that prevents real-time interaction, making them unsuitable for interactive applications.

Method: Distill a bidirectional text-to-video teacher model into a causal student using Self Forcing with Distribution Matching Distillation. Introduce sliding-window causal attention with attention sinks and KV cache rolling to handle infinite-length streaming while maintaining constant computational cost.

Result: Achieves state-of-the-art results in motion following and video quality while being two orders of magnitude faster than existing methods. Enables real-time generation at up to 29 FPS with sub-second latency, allowing infinite-length streaming video generation.

Conclusion: MotionStream uniquely enables real-time interactive video generation where users can paint trajectories, control cameras, or transfer motion and see results unfold immediately, delivering a truly interactive experience.
...
TL;DR: PRevivor is a prior-guided color transformer that restores color-degraded ancient Chinese paintings by learning from recent paintings, achieving superior performance through luminance enhancement and hue correction.


<details>
  <summary>Details</summary>
Motivation: Ancient Chinese paintings suffer from irreversible color degradation due to complex chemistry mechanisms, and the lack of comprehensive datasets hampers the development of digital restoration tools.

Method: Decomposes color restoration into luminance enhancement (using variational U-Nets and multi-scale mapping) and hue correction (using dual-branch color query module guided by localized hue priors).

Result: Extensive experiments show PRevivor outperforms state-of-the-art colorization methods both quantitatively and qualitatively.

Conclusion: PRevivor successfully revives color-degraded ancient paintings through a novel two-stage approach that combines localized hue correction with global reasoning capabilities.
...
TL;DR: This review paper analyzes adaptation strategies for foundation models in medical imaging, addressing challenges like domain shifts, data scarcity, and privacy concerns, while proposing future research directions.


<details>
  <summary>Details</summary>
Motivation: Foundation models offer transformative potential for medical image analysis but face challenges in clinical adaptation including domain shifts, limited annotated data, computational demands, and privacy requirements.
...
Result: Evaluation of performance gains, clinical applicability, and limitations of various adaptation approaches, identifying trade-offs and unresolved challenges in medical imaging applications.

Conclusion: The review provides a roadmap for developing adaptive, trustworthy, and clinically integrated foundation models through emerging directions like continual learning, federated approaches, hybrid self-supervised learning, and systematic benchmarking.
...
<details>
  <summary>Details</summary>
Motivation: Increasing realism of generated images raises concerns about misuse, requiring robust detection methods that don't heavily depend on large quantities of generated images like current binary classifiers.

Method: Uses pairs of functions that yield consistent outputs for natural images but divergent outputs for generated ones, leveraging orthogonal gradient subspaces. Detects generated images when transformations along data manifolds cause significant loss changes in self-supervised models pre-trained on natural images. Employs normalizing flows to amplify differences by extruding generated images away from natural image manifold.

Result: Extensive experiments demonstrate the efficacy of the method in detecting generated images effectively.

Conclusion: The proposed framework provides an effective approach for generated image detection by exploiting geometric manifold differences and amplifying them through normalizing flows, addressing limitations of current binary classifier-based methods.
...
TL;DR: UniREditBench is a new benchmark for evaluating reasoning-based image editing models, addressing limitations of existing benchmarks by covering multi-object interactions, game-world scenarios, and using multimodal dual-reference evaluation.
...
Method: Created UniREditBench with 2,700 curated samples across real- and game-world scenarios in 8 primary dimensions and 18 sub-dimensions. Introduced multimodal dual-reference evaluation (textual + ground-truth images). Built UniREdit-Data-100K synthetic dataset with CoT reasoning annotations and fine-tuned Bagel model to create UniREdit-Bagel.

Result: UniREdit-Bagel demonstrated substantial improvements in both in-domain and out-of-distribution settings. Benchmarking revealed strengths and weaknesses of various open-source and closed-source image editing models across different aspects.

Conclusion: UniREditBench provides a comprehensive framework for evaluating reasoning-based image editing, addressing key limitations of existing benchmarks and enabling better assessment of model capabilities in complex reasoning scenarios.
...
TL;DR: REASON is a two-stage framework for automated gastric content assessment from ultrasound that uses probability maps and dual-branch fusion of RLD and SUP views to improve aspiration risk stratification.


<details>
  <summary>Details</summary>
Motivation: Traditional manual tracing methods for gastric content assessment are inefficient and inaccurate, creating a need for automated solutions to improve preoperative aspiration risk assessment.

Method: Two-stage framework: Stage 1 uses segmentation to generate probability maps that suppress artifacts and highlight gastric anatomy. Stage 2 employs a dual-branch classifier that fuses information from RLD and SUP ultrasound views.

Result: Outperforms current state-of-the-art approaches by a significant margin on a self-collected dataset.

Conclusion: The framework shows great promise for automated preoperative aspiration risk assessment, offering more robust, efficient, and accurate clinical solutions.
...
TL;DR: A novel MIL framework with latent factor grouping and cluster-reasoning instance disentangling for interpretable WSI representation, addressing spatial, semantic, and decision entanglements.


<details>
  <summary>Details</summary>
Motivation: Spatial, semantic, and decision entanglements among instances limit representation and interpretability in multiple instance learning for whole-slide pathology images.

Method: Three-phase approach: 1) Positive semi-definite latent factor grouping for spatial entanglement, 2) Instance probability counterfactual inference via cluster-reasoning for semantic entanglement, 3) Generalized linear weighted decision with instance effect re-weighting for decision entanglement.
...
Conclusion: The proposed framework effectively addresses entanglements in MIL for WSI representation, achieving superior performance and interpretability aligned with pathologist reasoning.
...
<details>
  <summary>Details</summary>
Motivation: To address privacy risks from malicious misuse of personalization techniques in diffusion models, as existing adversarial perturbation methods fail against clean images or simple transformations.

Method: Proposes Direct Protective Optimization loss function and dual-path Learning to Protect strategy that alternates between personalization and protection paths to simulate future personalization trajectories.

Result: Outperforms existing methods, achieving state-of-the-art performance in preventing unauthorized personalization without compromising generative quality.

Conclusion: APDM effectively disrupts subject personalization in target models and provides robust protection against unauthorized content generation.
...
TL;DR: MVSMamba is the first Mamba-based Multi-View Stereo network that achieves superior performance and efficiency through a Dynamic Mamba module with reference-centered dynamic scanning for global feature aggregation.
...
Method: Proposes MVSMamba with Dynamic Mamba module using reference-centered dynamic scanning strategy for efficient intra-/inter-view feature interaction, omnidirectional multi-view representations, and multi-scale global feature aggregation.

Result: Outperforms state-of-the-art MVS methods on DTU dataset and Tanks-and-Temples benchmark with superior performance and efficiency.

Conclusion: MVSMamba demonstrates that Mamba architecture can effectively replace Transformers in MVS, achieving better performance with linear computational complexity.
...
<details>
  <summary>Details</summary>
Motivation: Address the vulnerability of deep learning models to adversarial attacks, particularly the need for attacks that are both effective and visually imperceptible across multi-object environments.

Method: Integrates CLIP model's text-image alignment with guided loss to incorporate natural language semantics, combining concentrated perturbation strategy from SSAE with dissimilar text embeddings from GAMA.

Result: Achieves competitive or superior performance compared to existing techniques across diverse black-box victim models while preserving greater visual similarity to original images.

Conclusion: The proposed method successfully creates effective adversarial examples that maintain high structural similarity to original inputs, demonstrating the power of integrating semantic guidance with perturbation strategies.
...
TL;DR: The paper introduces a benchmark of 1,333 English Rebus puzzles and proposes RebusDescProgICE, a framework that improves Vision-Language Models' performance on these puzzles using combined unstructured descriptions and structured reasoning.


<details>
  <summary>Details</summary>
Motivation: Rebus puzzles require complex skills like image recognition, cognitive reasoning, and commonsense understanding, making them challenging for current Vision-Language Models.

Method: Proposed RebusDescProgICE framework using unstructured descriptions and code-based structured reasoning with improved reasoning-based in-context example selection.

Result: Improved performance by 2.1-4.1% for closed-source models and 20-30% for open-source models compared to Chain-of-Thought Reasoning.

Conclusion: The benchmark and proposed framework effectively address the challenges of Rebus puzzle solving in Vision-Language Models.
...
Method: Uses prompt-conditioned instance-query generator to transform single point prompts into multiple specialized queries. Features hybrid CNN-Transformer encoder that injects CNN-derived boundary saliency into ViT self-attention via spatial gating. Employs competitively optimized query decoder for end-to-end parallel multi-instance prediction.

Result: Achieved comparable performance on LiTS17 and KiTS21 datasets. Exhibits strong robustness to prompts and provides practical solution for efficient annotation of clinically relevant multi-lesion cases.

Conclusion: MIQ-SAM3D provides an effective framework for multi-instance 3D medical image segmentation, overcoming limitations of single-object approaches while maintaining strong generalization and robustness.
...
<details>
  <summary>Details</summary>
Motivation: Previous text-to-image diffusion models only assessed content similarity under single style intensity, but experiments show that increasing style intensity causes significant content feature loss, leading to suboptimal content-style trade-offs.

Method: Proposes Content-Style Subspace Blending and a Content-Style Balance loss to expand the content-style frontier and improve content similarity across different style intensities.
...
Conclusion: The proposed method successfully broadens the content-style frontier and improves content preservation across varying style intensities in text-to-image diffusion models.
...
TL;DR: CMI-MTL is a Cross-Mamba Interaction based Multi-Task Learning framework for medical visual question answering that addresses cross-modal alignment challenges and free-form answer diversity through three key modules: fine-grained visual-text alignment, cross-modal feature representation, and answer-enhanced multi-task learning.


<details>
  <summary>Details</summary>
Motivation: Existing self-attention methods struggle with cross-modal semantic alignment between vision and language, and classification-based approaches can't handle free-form answer diversity, limiting their effectiveness in medical VQA tasks.

Method: Three main modules: FVTA for fine-grained visual-text feature alignment, CIFR for cross-modal interleaved feature representation, and FFAE for free-form answer-enhanced multi-task learning that leverages auxiliary knowledge from open-ended questions.
...
Conclusion: CMI-MTL effectively addresses cross-modal alignment challenges and free-form answer diversity in medical VQA, showing superior performance over existing methods while providing interpretable results.
...
TL;DR: A pipeline for generating realistic synthetic data of event-based cameras on AUVs in underwater environments to train vision models, demonstrated for rock detection in poor visibility conditions.


<details>
  <summary>Details</summary>
Motivation: Underwater environments pose challenges like poor lighting and high dynamic range where traditional vision techniques struggle. Event-based cameras offer a solution by tracking frame-by-frame changes, but need realistic training data.

Method: Developed a pipeline to generate synthetic data simulating event-based cameras mounted on Autonomous Underwater Vehicles in underwater environments with poor visibility and suspended particles.

Result: The pipeline successfully generates realistic synthetic data that can be used to train vision models for underwater tasks like rock detection.

Conclusion: The proposed synthetic data generation pipeline enables effective training of vision models for event-based cameras in challenging underwater conditions, with potential for generalization to other underwater applications.
...
<details>
  <summary>Details</summary>
Motivation: Current approaches struggle with patch redundancy and ambiguity due to information density disparities between vision and language modalities, and MLLMs' dense outputs may conflict with sparse captions.

Method: Two-stage mechanism that integrates unified semantics from both dense and sparse texts to identify salient visual patches, and uses relevance-aware selection with mean value computation to highlight crucial patch-word correspondences.
...
Conclusion: SEPS effectively addresses patch redundancy and ambiguity in cross-modal alignment through semantic integration and relevance-aware selection, demonstrating superior performance over existing approaches.
...
TL;DR: This paper introduces Fire-ART dataset and a panoramic image-based reconstruction approach for automated firefighting asset recognition and BIM integration, achieving 73-88% F1-scores and sub-meter localization accuracy.


<details>
  <summary>Details</summary>
Motivation: Conventional firefighting asset management methods are inefficient due to limited automated recognition and reconstruction capabilities, hindering emergency preparedness and risk assessment.

Method: Developed Fire-ART dataset with 15 asset types (2,626 images, 6,627 instances) and integrated modified cube-map conversion with radius-based spherical camera projection for enhanced recognition and localization.

Result: Achieved F1-scores of 73% and 88% with localization errors of 0.620m and 0.428m respectively in two real-world case studies.

Conclusion: The Fire-ART dataset and reconstruction approach provide valuable resources and robust technical solutions for accurate digital management of fire safety equipment.
...
TL;DR: Training-free explanation method using smooth tunable contours instead of dense masks, achieving compact, interpretable regions with high fidelity and improved consistency.


<details>
  <summary>Details</summary>
Motivation: Dense perturbation masks for vision model explanations are often fragmented, overfitted, and require post-processing, needing a more compact and robust alternative.

Method: Parameterizes star-convex regions using truncated Fourier series, optimized under extremal preserve/delete objective using classifier gradients, with single connected masks and explicit area control.

Result: Matches dense mask fidelity while producing compact regions with improved consistency, achieves higher relevance mass and lower complexity than baselines, especially strong on DINO models with 15%+ relevance mass improvement.

Conclusion: Smooth contour-based explanations provide faithful, compact, and interpretable alternatives to dense masks, with robust performance across benchmarks and models.
...
TL;DR: A distillation framework for efficient causal video generation that enables high-quality synthesis with extremely limited denoising steps (1-2 steps) through adversarial self-distillation and first-frame enhancement strategies.


<details>
  <summary>Details</summary>
Motivation: Current hybrid video generation models suffer from error accumulation and long inference times due to their sequential, iterative nature. There's a need for more efficient video synthesis with minimal denoising steps.
...
Result: Extensive experiments on VBench show the method surpasses state-of-the-art approaches in both one-step and two-step video generation, producing high-quality videos with extremely limited denoising steps.

Conclusion: The framework produces a single distilled model that flexibly supports multiple inference-step settings, eliminating the need for repeated re-distillation and enabling efficient, high-quality video synthesis.
...
TL;DR: UniSOT is a unified tracker that handles three reference modalities (bounding box, natural language, or both) across four video modalities (RGB, RGB+Depth, RGB+Thermal, RGB+Event) with uniform parameters.


<details>
  <summary>Details</summary>
Motivation: Existing trackers are designed for specific reference and video modality combinations, leading to separate model designs that limit practical applications. A unified tracker is needed to handle various requirements across different modalities.

Method: Developed UniSOT, a unified tracker that can perform tracking with three reference modalities (bounding box, natural language, or both) across four video modalities (RGB, RGB+Depth, RGB+Thermal, RGB+Event) using uniform parameters.

Result: Extensive experiments on 18 benchmarks show superior performance against modality-specific counterparts. UniSOT outperforms previous counterparts by over 3.0% AUC on TNL2K across all three reference modalities and outperforms Un-Track by over 2.0% main metric across all three RGB+X video modalities.

Conclusion: UniSOT demonstrates that a unified approach can effectively handle diverse reference and video modality combinations, showing superior performance compared to modality-specific trackers and enabling more practical applications.
...
Method: Uses a resolution-aware token decoder with global self-attention and lightweight dilated depthwise refinement for local coherence. Incorporates gated cross-attention to integrate fine-scale features without amplifying noise, and class-aware point refinement for residual ambiguities. Adds boundary-band consistency regularization during training.

Result: Achieves competitive performance and improved stability across transitions in off-road semantic segmentation.

Conclusion: The proposed method effectively balances computational efficiency with boundary fidelity and robustness to imperfect supervision in challenging off-road segmentation scenarios.
...
TL;DR: A training-only method to improve thermal-infrared object detection by sharpening instance boundaries and injecting RGB semantic priors, achieving SOTA without needing RGB input at test time.


<details>
  <summary>Details</summary>
Motivation: Thermal-infrared detection suffers from low contrast and weak high-frequency cues, causing duplicate boxes, missed small objects, and class confusion. Existing methods either translate TIR to RGB (fragile to artifacts) or fuse RGB-TIR at test time (requires extra sensors and calibration).

Method: Training-only objectives that: 1) sharpen instance boundaries via feature clustering (same-class pull, different-class push), and 2) inject cross-modal semantic priors by aligning student's thermal features with RGB-trained teacher's features.

Result: Outperformed prior approaches and achieved state-of-the-art performance in thermal-infrared object detection.

Conclusion: The proposed training-only method effectively improves thermal representation without requiring RGB input at test time, addressing root causes of poor thermal detection performance.
...
TL;DR: A Model-Agnostic Ordinal Meta-Learning (MAOML) algorithm is introduced to train smaller VLMs for fruit freshness classification, achieving 92.71% accuracy by addressing data scarcity through meta-learning and leveraging label ordinality.


<details>
  <summary>Details</summary>
Motivation: To manage perishable fruit wastage by predicting freshness using non-invasive visual methods, while overcoming challenges of costly expert labeling, data scarcity, and limitations of proprietary VLMs (data privacy concerns) and sub-optimal open-source VLMs.

Method: Proposed Model-Agnostic Ordinal Meta-Learning (MAOML) algorithm that uses meta-learning to handle data sparsity and incorporates label ordinality to train smaller Vision Language Models for fruit freshness classification.

Result: Achieved state-of-the-art performance with 92.71% accuracy averaged across all fruits in both zero-shot and few-shot settings, outperforming existing open-source VLMs and matching proprietary model performance.

Conclusion: MAOML successfully addresses data scarcity and privacy concerns by enabling effective training of smaller VLMs for fruit freshness classification, achieving industry-standard accuracy without relying on proprietary models.
...
TL;DR: The paper introduces Reg-DPO, a method that improves video generation quality using automated preference pairs and regularization, achieving better performance than existing approaches.
...
Conclusion: The proposed approach effectively addresses video generation challenges through automated preference pairs, regularization for stability, and memory optimization, delivering state-of-the-art performance.
...
TL;DR: QA-SNNE is a black box uncertainty estimator that incorporates question semantics into prediction confidence for safer surgical VQA, improving AUROC by 15-38% for zero-shot models.


<details>
  <summary>Details</summary>
Motivation: Safety and reliability are essential for surgical VQA deployment, but most research overlooks safety behaviors like ambiguity awareness and referral to human experts. Uncertainty estimation is needed to enable safer decision making.
...
Result: PEFT models degrade under mild paraphrasing while LVLMs are more resilient. QA-SNNE improves AUROC in most in-template settings and enhances hallucination detection, with 15-38% AUROC increase for zero-shot models. Gains maintained under out-of-template stress.

Conclusion: QA-SNNE offers a practical and interpretable step toward automatic failure detection in surgical VQA by linking semantic uncertainty to question context. Combining LVLM backbones with question-aligned uncertainty estimation can improve safety and clinician trust.
...
TL;DR: The paper proposes a framework for post-training quantization of vision transformers by modeling quantization errors as Gaussian noise and using noise injection to achieve flat minima, enabling effective low-bit quantization.
...
Method: Proposes modeling Activation Quantization Error (AQE) and Weight Quantization Error (WQE) as independent Gaussian noises, and uses noise injection optimization methods to achieve flat minima in the full-precision neural network.
...
Conclusion: The method opens novel pathways for obtaining low-bit PTQ models by proactively pre-conditioning models through error source measurement and disentangling, achieving flat minima crucial for quantization.
...
TL;DR: HMVLM is a unified framework that integrates 3D human motion with foundation models using MoE LoRA strategy to address catastrophic forgetting and develop autoregressive-compatible pose representations.


<details>
  <summary>Details</summary>
Motivation: To bridge the modality gap between human motion and text, prevent catastrophic forgetting during integration, and create generalizable pose representations for heterogeneous downstream tasks.

Method: Uses Mixture of Expert Low-Rank Adaptation (MoE LoRA) with dynamic gating network for task allocation, introduces zero expert to preserve pre-trained parameters, and implements body-part-specific tokenization for enhanced spatial resolution.
...
TL;DR: SecDiff is a diffusion-aided decoding framework that enhances security and robustness of deep joint source-channel coding against physical-layer attacks like pilot spoofing and subcarrier jamming, achieving better reconstruction quality with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing deep JSCC frameworks are vulnerable to physical-layer adversarial threats that compromise semantic fidelity, creating a need for more secure and robust semantic communication systems.

Method: Uses pseudoinverse-guided sampling and adaptive guidance weighting for flexible step-size control. For jamming attacks: power-based subcarrier masking recasts recovery as masked inpainting. For pilot spoofing: formulates channel estimation as blind inverse problem with EM-driven reconstruction algorithm that alternates between pilot recovery and channel estimation.

Result: Extensive experiments on OFDM channels show SecDiff outperforms existing secure and generative JSCC baselines, achieving favorable trade-off between reconstruction quality and computational cost.

Conclusion: SecDiff represents a promising step toward practical, low-latency, and attack-resilient semantic communications by balancing security, robustness, and efficiency.
...
TL;DR: EPAN is a dual-branch network for person re-identification that achieves 90.09% Rank-1 accuracy and 78.82% mAP on the Inspection-Personnel dataset, designed for robust performance across diverse IoT surveillance conditions.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of person re-identification in IoT-enabled smart environments, particularly dealing with perspective and environmental changes in surveillance systems.

Method: EPAN employs a dual-branch architecture to extract alignment information under varying scales and viewpoints, mitigating the impact of perspective and environmental changes.

Result: Achieved outstanding performance with 90.09% Rank-1 accuracy and 78.82% mean Average Precision on the Inspection-Personnel dataset.
...
TL;DR: A probabilistic framework using flow matching on SE(3) manifold for 6D object pose estimation that captures multi-modal pose distributions instead of single deterministic outputs.


<details>
  <summary>Details</summary>
Motivation: Address challenges in object pose estimation including partial observability, occlusions, and object symmetries that lead to pose ambiguity and multiple hypotheses. Deterministic methods fail to capture multi-modality and are overconfident.
...
Result: Achieves state-of-the-art results on Real275, YCB-V, and LM-O datasets. Demonstrates practical applications in robotic manipulation tasks like active perception and uncertainty-aware grasp synthesis.

Conclusion: The proposed probabilistic framework effectively handles pose ambiguity and enables uncertainty-aware reasoning, outperforming deterministic approaches and showing value in downstream robotic applications.
...
TL;DR: The paper introduces DiMoDE, a framework that discriminatively treats motion components in unsupervised depth and ego-motion learning, leveraging geometric regularities of rigid flows to improve both tasks through targeted constraints and reformulated joint learning.


<details>
  <summary>Details</summary>
Motivation: Most existing methods treat ego-motion as auxiliary, mixing motion types or excluding depth-independent rotations, which limits geometric constraints and reduces reliability under diverse conditions.

Method: Network outputs align optical axes and imaging planes between frames, transform optical flows through these alignments, quantify deviations to impose geometric constraints on each ego-motion component, and reformulate joint learning into coaxial and coplanar forms with closed-form geometric relationships.
...
Conclusion: The discriminative treatment of motion components and geometric reformulation enables more targeted refinement and complementary constraints, significantly improving depth and ego-motion estimation robustness.
...
<details>
  <summary>Details</summary>
Motivation: Existing LLIE methods rely on deterministic pixel mappings between paired low/normal-light images, which fail when normal-light references are unavailable and neglect the continuous physical process of luminance transitions.

Method: Proposes Luminance-Aware Statistical Quantification (LASQ) that models luminance transition as power-law distribution and uses diffusion forward process to discover optimal transition paths between luminance layers without normal-light references.

Result: Achieves superior performance on domain-specific datasets with normal-light references and better generalization across non-reference datasets, enabling more adaptable light restoration in practical situations.

Conclusion: LASQ provides a probabilistic framework for LLIE that overcomes limitations of deterministic approaches, offering improved generalization and practical applicability without requiring normal-light references.
...
<details>
  <summary>Details</summary>
Motivation: Realistic textures require including natural alterations like stains and tears that are ubiquitous in nature, but current methods lack automated ways to handle these features without manual annotation.

Method: Learning-based approach using unsupervised anomaly detection to identify appearance-altering features, automatic clustering into semantic groups, and conditional generation with diffusion-based editing for interactive texture creation.

Result: A complete pipeline that transforms small image collections into versatile generative models enabling interactive painting of features on arbitrary-sized textures.

Conclusion: The system successfully automates texture editing with natural blemishes using unsupervised methods, and the introduced algorithms for diffusion-based editing and infinite texture generation are broadly applicable.
...
TL;DR: NSYNC introduces a contrastive learning framework using synthetic negative images to improve text-to-image diffusion models' stylization capabilities by orthogonal gradient updates.
...
Method: Uses synthetic negative image sets in contrastive training, computes orthogonal gradients by subtracting positive gradient's projection onto negative gradient to eliminate trivial shared attributes.
...
Conclusion: The proposed contrastive learning framework with synthetic negative data effectively enhances stylization capabilities of text-to-image diffusion models.
...
TL;DR: Proposes a structured five-layer model for generating and evaluating rare driving scenarios using generative models and data augmentation.


<details>
  <summary>Details</summary>
Motivation: Rare driving scenarios are critical for autonomous vehicle development but difficult to encounter, necessitating simulation and generation methods.

Method: Uses a five-layer model with subclasses and characteristics for agents, alongside large foundational models for scenario generation and embedding-based comparison.

Result: Develops diversity and originality metrics for evaluating synthetic datasets, with qualitative evaluation of generated scenarios.

Conclusion: The structured approach improves rare scenario generation and evaluation for autonomous vehicle development.
...
<details>
  <summary>Details</summary>
Motivation: Traditional ResNet-based ReID algorithms fail to handle occlusions effectively in surveillance scenarios, where occlusions obscure key body features and increase identification complexity.

Method: Proposed PCD-ReID algorithm with Transformer-based network to extract shared component features (helmets, uniforms), trained on new real-world patrol surveillance dataset covering 6 months, 10,000 individuals, and 50,000+ images.

Result: Achieved 79.0% mAP and 82.7% Rank-1 accuracy, representing 15.9% Rank-1 improvement over ResNet50-based methods in occlusion-aware pedestrian re-identification.

Conclusion: PCD-ReID effectively addresses occlusion challenges in tower inspection scenarios and shows strong potential for practical deployment in surveillance and security applications.
...
TL;DR: NOA is a graphical user interface plugin for napari that simplifies AI-based organoid analysis by integrating detection, segmentation, tracking, feature extraction, and ML prediction tools in an accessible framework.


<details>
  <summary>Details</summary>
Motivation: AI tools for organoid microscopy image analysis are not easily accessible to biologists without programming experience, leading to labor-intensive manual workflows, and existing tools are narrowly focused on specific tasks.

Method: Developed Napari Organoid Analyzer (NOA) as an open-source napari plugin that integrates multiple state-of-the-art algorithms for detection, segmentation, tracking, feature extraction, custom annotation, and ML-based prediction.
...
Conclusion: NOA enables comprehensive, AI-driven organoid image analysis within an accessible and extensible framework, overcoming the accessibility barrier for biologists.
...
<details>
  <summary>Details</summary>
Motivation: Current VLAs struggle with pixel-level scene understanding and rely heavily on textual prompts, limiting their flexibility in real-world robot control settings.

Method: Built on a visuomotor instruction tuning framework that integrates a multiscale pixel-aware encoder with a visual prompting encoder, trained using a two-stage automated annotation pipeline that generates Pixel-160K dataset with pixel-level annotations.
...
TL;DR: Proposes using DC-GAN to generate synthetic MRI data to address limited medical imaging data, and validates quality using CNN classification on both real and synthetic images.


<details>
  <summary>Details</summary>
Motivation: Limited availability of original MRI data makes synthetic data generation crucial for medical imaging tasks, particularly for brain tumor analysis.

Method: Uses Deep Convolutional Generative Adversarial Network (DC-GAN) for synthetic MRI generation and Convolutional Neural Network (CNN) classifier to evaluate synthetic data quality through brain tumor classification.

Result: Classification performance on synthetic images is comparable to real images, demonstrating GAN-generated images are effective for downstream medical tasks.

Conclusion: GAN-based synthetic medical image generation is a viable solution for data scarcity in medical imaging, with synthetic data performing comparably to real data in classification tasks.
...
TL;DR: CDD-VT proposes a dualistic visual tokenizer that adaptively combines continuous and discrete tokenization approaches based on image complexity, achieving superior performance with a more streamlined architecture.


<details>
  <summary>Details</summary>
Motivation: To resolve the dichotomy between continuous tokenizers (complex pipelines but strong performance) and discrete tokenizers (elegant but with information loss) in multimodal large models.

Method: Uses adaptive primitive allocation where simple images use few primitives (like discrete tokenization) and complex images use many primitives (like continuous tokenization), with Diverse Quantitative Primitives and Dynamic Primitive Allocator components.

Result: Achieves superior performance over specialized continuous and discrete tokenizers in reconstruction, retrieval and classification tasks.

Conclusion: CDD-VT effectively bridges the continuous-discrete tokenization gap, enabling strong performance within a concise and scalable multimodal large model architecture.
...
<details>
  <summary>Details</summary>
Motivation: Current RECIST v1.1 standard relies on 2D diameter measurements, but volumetric measurements provide more reliable treatment assessment. Manual volumetric annotation is labor-intensive, limiting clinical adoption.

Method: Lite ENSAM - a lightweight adaptation of ENSAM architecture designed for efficient volumetric tumor segmentation from CT scans annotated with RECIST annotations.

Result: Achieved Dice Similarity Coefficient (DSC) of 60.7% and Normalized Surface Dice (NSD) of 63.6% on hidden test set, with average total RAM time of 50.6 GBs and average inference time of 14.4s on CPU.

Conclusion: Lite ENSAM provides an efficient solution for volumetric tumor segmentation that could help overcome the limitations of manual volumetric annotation in clinical practice.
...
TL;DR: DINO-MX is a modular training framework that unifies DINO, DINOv2, and DINOv3 principles, offering flexible training strategies, reduced computational costs, and compatibility with Hugging Face ecosystem.
...
TL;DR: The paper introduces PSPooling, a non-learnable mesh pooling operator for 3D anatomical shape analysis, and MedShapeNet19 benchmark dataset, showing improved reconstruction and classification performance.
...
Conclusion: The work establishes a strong baseline for medical 3D shape learning and provides MedShapeNet19 as a standardized benchmark for anatomical shape classification research.
...
<details>
  <summary>Details</summary>
Motivation: Existing fusion techniques for heterogeneous retrievers rely only on rank or score signals, ignoring candidates' representations, which is problematic for complex multi-modal data like videos.

Method: ViC serializes both content evidence and retriever metadata directly in VLM prompts, using S-Grid (compact image grids with optional subtitles) to represent videos for list-wise reasoning.
...
Method: Two-stage fine-tuning: first SFT on Viewpoint-100K dataset for foundational knowledge, then GRPO for generalization; plus hybrid cold-start initialization for viewpoint representation learning.
...
TL;DR: This paper proposes an adaptive RL framework for diffusion-based image restoration models that uses MLLM-based IQA models as rewards, focusing on challenging samples and dynamically combining RL with supervised fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Directly applying existing RL methods to diffusion-based image restoration is suboptimal because restoration emphasizes fidelity over pure generation, requiring different optimization objectives.

Method: Uses IQA models as rewards instead of ground-truth supervision, focuses RL on challenging samples distant from ground truth, implements RL with MLLM-based IQA models for distribution alignment, and adaptively combines RL with SFT using automatic weighting based on sample difficulty.

Result: The proposed RL framework effectively boosts performance across various restoration tasks, as demonstrated through extensive experiments on multiple benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models for relighting often produce unrealistic results due to optimization in semantic latent space, leading to issues like overexposed highlights and misaligned shadows that lack physical correctness.

Method: Proposes a unified framework that brings RGB-space geometry feedback into a flow matching backbone, supervised by depth and normal maps extracted from outputs. Uses path consistency learning for efficient training and introduces a structured 6D annotation protocol for fine-grained control.

Result: Achieves state-of-the-art relighting quality with significantly improved physical consistency, while delivering 20x speedup for both image and video relighting compared to previous methods.

Conclusion: UniLumos successfully addresses physical plausibility issues in relighting by incorporating geometry feedback and efficient training methods, enabling high-quality controllable lighting effects with substantial performance improvements.
...
TL;DR: A progressive network architecture for synthesizing IHC-equivalent images from H&E slides that addresses limitations in existing stain translation methods by decoupling structural, color, and cell boundary optimization.
...
Method: Progressive network architecture with stage-wise optimization of visual aspects, incorporating color and cell border generation logic, built on ASP framework with additional DAB chromogen concentration and image gradient loss functions.

Result: Experiments on HER2 and ER datasets showed significant improvement in visual quality and finer structural details in generated IHC images compared to baseline methods.
...
TL;DR: LFRD2 is a hybrid framework combining neural networks with physical modeling to address depth quality degradation in under-display ToF imaging caused by TOLED layers, using time-fractional reaction-diffusion dynamics and continuous convolution for iterative depth refinement.
...
Conclusion: LFRD2 successfully combines neural network expressiveness with physical model interpretability to address the challenges of under-display ToF imaging through TOLED layers, with publicly available code.
...
<details>
  <summary>Details</summary>
Motivation: While probabilistic robustness is a practical complement to adversarial robustness, dedicated training methods for improving PR are underexplored with limitations in evaluation protocols, comparisons to strong AT baselines, and unified generalization analysis.

Method: Introduced PRBench benchmark that empirically compares common AT and PR-targeted training methods using comprehensive metrics including clean accuracy, PR and AR performance, training efficiency, and generalization error across 222 trained models on 7 datasets and 10 architectures.

Result: AT methods are more versatile in improving both AR and PR performance across diverse hyperparameter settings, while PR-targeted training methods consistently yield lower generalization error and higher clean accuracy.

Conclusion: PRBench provides the first comprehensive benchmark for PR evaluation, revealing trade-offs between AT and PR-targeted methods, with AT offering broader robustness improvements and PR-targeted methods providing better generalization and clean accuracy.
...
TL;DR: Developed a task explorer pipeline using clustering, factor analysis, and string edit distance to automatically identify global/local strategies and hierarchical subtasks in human-machine interaction.
...
Method: Task explorer pipeline using clustering techniques paired with factor analysis and string edit distance to identify global strategies (generalized action sets) and local strategies (similar action compositions), plus hierarchical subtask identification.

Result: Successfully identified key strategies for task completion and encoded user runs with hierarchical subtask structures. Developed Task Explorer application for reviewing pipeline results.

Conclusion: The pipeline is adaptable to any action-based time-series data and helps inform both humans and machines about user knowledge, skill, and behavior.
...
<details>
  <summary>Details</summary>
Motivation: Pneumonia remains a leading cause of morbidity and mortality worldwide, necessitating accurate automated detection systems. While transformer-based detectors like RT-DETR show promise in object detection, their application to medical imaging for pneumonia detection remains underexplored.

Method: Proposes CGF-DETR with three key modules: XFABlock in backbone for multi-scale feature extraction using convolutional attention with CSP architecture; SPGA module replacing standard multi-head attention with dynamic gating and single-head self-attention; GCFC3 in neck for feature representation enhancement via multi-path convolution fusion with structural re-parameterization.

Result: Achieves 82.2% mAP@0.5 on RSNA Pneumonia Detection dataset, outperforming baseline RT-DETR-l by 3.7% while maintaining comparable inference speed at 48.1 FPS. Complete model achieves 50.4% mAP@[0.5:0.95]. Ablation studies confirm each module contributes meaningfully to performance improvement.

Conclusion: CGF-DETR demonstrates superior performance for pneumonia detection in chest X-rays compared to baseline transformer detectors, with proposed architectural improvements effectively enhancing detection accuracy while maintaining real-time inference capabilities.
...
TL;DR: 3EED is a large-scale 3D visual grounding benchmark with multi-platform data (vehicle, drone, quadruped) featuring RGB and LiDAR, 10x larger than existing datasets, enabling cross-platform evaluation of language-referred object localization.


<details>
  <summary>Details</summary>
Motivation: Existing 3D grounding benchmarks are limited to indoor environments, single platforms, and small scale, creating a need for comprehensive outdoor multi-platform datasets to advance embodied AI.

Method: Developed scalable annotation pipeline combining vision-language model prompting with human verification; proposed platform-aware normalization and cross-modal alignment techniques for cross-platform learning.

Result: Created dataset with 128,000+ objects and 22,000+ validated referring expressions across diverse outdoor scenes; established performance gaps revealing challenges in generalizable 3D grounding.

Conclusion: 3EED enables research in language-driven 3D embodied perception and highlights the need for improved cross-platform generalization in visual grounding systems.
...
<details>
  <summary>Details</summary>
Motivation: Previous methods for 2D-to-3D human pose lifting suffer from depth ambiguity and temporal incoherence due to errors in 2D pose estimation, focusing mainly on local temporal constraints while neglecting global spatial-temporal correlations.

Method: Proposes HGFreNet with hop-hybrid graph attention (HGA) module to group k-hop neighbors and apply attention globally, plus Transformer encoder for temporal correlations and frequency-domain trajectory consistency constraints.

Result: Extensive experiments on Human3.6M and MPI-INF-3DHP datasets show HGFreNet outperforms state-of-the-art methods in both positional accuracy and temporal consistency.

Conclusion: The proposed approach effectively addresses depth ambiguity and temporal incoherence in 3D human pose estimation through global spatial-temporal modeling and frequency-domain constraints.
...
TL;DR: Wonder3D++ is a novel method for efficient high-fidelity textured mesh generation from single-view images using cross-domain diffusion and multi-view attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: To address limitations of existing methods: SDS-based approaches are time-consuming with inconsistent geometry, while fast network inference methods produce low-quality results lacking geometric details.

Method: Uses cross-domain diffusion model to generate multi-view normal maps and color images, employs multi-view cross-domain attention for consistency, and implements cascaded 3D mesh extraction in coarse-to-fine manner.
...
Conclusion: The proposed method holistically improves quality, consistency, and efficiency of single-view 3D reconstruction tasks.
...
TL;DR: UniLION is a unified autonomous driving model that efficiently handles LiDAR point clouds, multi-view images, and temporal sequences using linear group RNN operators, eliminating quadratic attention overhead and supporting multiple configurations without explicit fusion modules.


<details>
  <summary>Details</summary>
Motivation: Transformers have quadratic attention mechanisms that introduce significant computational overhead when processing long-sequence data, which is problematic for autonomous driving applications that require processing large-scale LiDAR point clouds and temporal sequences.

Method: Uses linear group RNN operator (performs linear RNN for grouped features) to efficiently handle large-scale LiDAR point clouds, high-resolution multi-view images, and temporal sequences. Serves as a single versatile architecture supporting multiple specialized variants without requiring explicit temporal or multi-modal fusion modules.

Result: Consistently delivers competitive and state-of-the-art performance across 3D perception (object detection, tracking, occupancy prediction, BEV segmentation), prediction (motion prediction), and planning (end-to-end planning) tasks.

Conclusion: UniLION offers a unified paradigm that simplifies multi-modal and multi-task autonomous driving system design while maintaining superior performance, providing a fresh perspective on 3D foundation model development for autonomous driving.
...
<details>
  <summary>Details</summary>
Motivation: Foundation models show promise as world simulators but their application in high-stakes surgical domains remains unexplored, requiring specialized causal knowledge beyond general physical rules.

Method: Created SurgVeo benchmark and Surgical Plausibility Pyramid (SPP) - a four-tiered framework to assess model outputs from basic appearance to complex surgical strategy. Evaluated Veo-3 model on zero-shot prediction tasks using surgical clips, with assessment by four board-certified surgeons.

Result: Veo-3 achieved exceptional Visual Perceptual Plausibility but failed critically at higher SPP levels including Instrument Operation, Environment Feedback, and Surgical Intent Plausibility, revealing a distinct "plausibility gap".

Conclusion: This work provides first quantitative evidence of the gap between visual mimicry and causal understanding in surgical AI, establishing a foundation for developing models capable of handling specialized healthcare complexities.
...
TL;DR: A prompt-driven GraphRAG framework that improves multi-hop question answering through strategic prompt design for entity extraction, fact selection, and passage reranking, achieving state-of-the-art results on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Current graph-based RAG approaches for complex reasoning overlook the critical role of prompt design in enhancing retrieval and reasoning processes, creating a research gap in prompt-aware graph reasoning.

Method: Creates symbolic knowledge graphs from text data using entity and fact triple encoding, employs LLMs for semantic filtering and answer generation, and uses entity-guided graph traversal with Personalized PageRank for efficient retrieval.

Result: Achieved state-of-the-art performance with F1 scores of 80.7% on HotpotQA and 78.9% on 2WikiMultiHopQA, and Recall@5 scores of 97.1% and 98.1% respectively.

Conclusion: Prompt design is crucial for improving retrieval accuracy and response quality in multi-hop question answering, establishing foundations for more efficient and interpretable reasoning systems through prompt-aware graph reasoning.
...
TL;DR: The Scitextures dataset contains 100,000+ images from 1,200+ scientific models across multiple domains, enabling AI evaluation on linking visual patterns to their generating mechanisms.


<details>
  <summary>Details</summary>
Motivation: To explore the connection between visual patterns and the underlying mechanisms that produce them, bridging visual understanding with scientific processes.

Method: Created an agentic AI pipeline to autonomously collect and implement models in standardized form, then used this dataset to evaluate AI models' ability to link patterns to generating mechanisms and infer mechanisms from real-world patterns.

Result: Vision-language models demonstrated capability to understand and simulate physical systems beyond visual patterns, successfully identifying, modeling, and coding mechanisms that generate patterns.

Conclusion: The Scitextures dataset provides a comprehensive benchmark for evaluating AI's ability to connect visual patterns with their generating processes, showing promising results in mechanistic understanding.
...
Result: TIR-Bench is universally challenging for all tested models. Strong performance requires genuine thinking-with-images capabilities. Pilot study compares direct versus agentic fine-tuning approaches.

Conclusion: The benchmark successfully captures advanced thinking-with-images capabilities that existing benchmarks miss, highlighting the need for models to develop genuine tool-use reasoning for image transformation tasks.
