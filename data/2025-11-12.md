...
Method: Three-stage framework: (1) transform video content into fine-grained captions using vision-language model, (2) construct structured knowledge by organizing captions into four semantic slots (action, object, context, environment), (3) generate slot-wise explanations revealing which semantic factors contribute most to anomaly decisions.
...
Conclusion: Text-based reasoning enables interpretable video anomaly detection that is both effective and explainable, making it suitable for real-world surveillance applications where understanding why an event is anomalous is crucial.
...
TL;DR: MoDM framework reconstructs molecular structures from second-order moments of cryo-EM images under different orientation distributions using convex optimization, enabling accurate recovery without full orientation information.


<details>
  <summary>Details</summary>
Motivation: To improve cryo-EM reconstruction by leveraging multiple datasets under different experimental conditions, overcoming limitations of single dataset approaches and demonstrating the value of dataset diversity.

Method: Method of double moments (MoDM) uses second-order statistics from two datasets: one with uniform orientation distribution and another with unknown non-uniform distribution, combined with convex-relaxation-based optimization.

Result: The method generically uniquely determines molecular structures (up to global rotation/reflection) and achieves accurate reconstruction using only second-order statistics from multiple datasets.

Conclusion: Leveraging dataset diversity through multiple experimental conditions significantly enhances reconstruction quality in computational imaging, with MoDM providing a practical framework for cryo-EM structure determination.
...
TL;DR: SSViT is a novel Transformer-based framework for modulo video recovery that uses selective token attention to efficiently reconstruct high-quality videos from 8-bit folded inputs.


<details>
  <summary>Details</summary>
Motivation: Conventional image sensors have limited dynamic range causing saturation in HDR scenes. Modulo cameras fold irradiance to avoid saturation but require specialized unwrapping algorithms. Existing HDR methods are unsuitable for modulo recovery, and deep learning progress in this area has been slow despite the decade-old concept.

Method: Proposed Selective Spatiotemporal Vision Transformer (SSViT) that captures global dependencies and spatiotemporal relationships using a token selection strategy to focus on critical regions and improve efficiency for modulo video reconstruction.

Result: SSViT produces high-quality reconstructions from 8-bit folded videos and achieves state-of-the-art performance in modulo video recovery, demonstrating superior performance over conventional HDR methods.

Conclusion: SSViT is the first successful deep learning framework for modulo video reconstruction, effectively addressing the challenges of modulo recovery through Transformer architecture with selective spatiotemporal attention.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models suffer from hallucinations that create incoherent samples due to mode interpolation and score smoothening, but existing methods lack ways to prevent this during sampling.

Method: Uses Laplacian (sharpness) of the score function with efficient approximation via finite-difference Hutchinson trace estimator for higher dimensions, applied as post-hoc adjustment during inference.

Result: Significantly reduces hallucinated samples across 1D, 2D distributions and high-dimensional image datasets.

Conclusion: The Laplacian-based correction effectively reduces mode interpolation hallucinations and reveals relationship between Laplacian and score uncertainty.
...
TL;DR: ASAG is a novel guidance method for diffusion models that uses optimal transport theory to adversarially disrupt attention scores via Sinkhorn algorithm, improving sample quality without model retraining.
...
Method: ASAG injects adversarial cost within self-attention layers using Sinkhorn algorithm to reduce pixel-wise similarity between queries and keys, weakening misleading attention alignments.
...
Conclusion: The method is lightweight, plug-and-play, and improves reliability without requiring model retraining.
...
TL;DR: LiveNeRF enables real-time face replacement at 33 FPS with high visual quality for applications like live streaming and video conferencing, while addressing ethical concerns through consent verification and detection systems.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of existing face replacement methods by achieving real-time performance for practical deployment in entertainment, education, and communication applications, while ensuring responsible use.

Method: LiveNeRF framework that enables real-time face replacement with superior visual quality, specifically designed for live applications.

Result: Achieves real-time performance at 33 FPS with high visual quality, enabling practical deployment in live streaming, video conferencing, and interactive media.

Conclusion: The framework successfully addresses key limitations of existing methods and enables practical applications while advocating for responsible deployment through consent verification and integration with detection systems to minimize misuse risks.
...
TL;DR: TrackStudio is a GUI-based markerless motion tracking toolkit that combines existing open-source tools into an accessible pipeline requiring no programming skills, validated to work reliably across diverse environments.
...
Method: Combined established open-source tools into a single modular GUI pipeline providing automatic 2D/3D tracking, calibration, preprocessing, feature extraction, and visualization. Tested across three environments with low-cost webcams and high-resolution cameras in challenging conditions.

Result: Across 76 participants, achieved average inter-frame correlations >0.98 and average triangulation errors <13.6mm for hand tracking, demonstrating stable and consistent performance. The pipeline can be extended to other body and face regions.
...
TL;DR: A deep learning CNN model was developed to automatically classify coronary artery calcium scores into six clinical categories from cardiac CT scans, achieving high accuracy comparable to current semiautomatic methods.


<details>
  <summary>Details</summary>
Motivation: Cardiovascular disease causes high mortality worldwide, and current CAC scoring practices require time-intensive semiautomatic analysis by trained professionals, creating a need for automated solutions.

Method: Used a deep learning convolutional neural network trained on 68 patient scans with semiautomatic CAC scores as reference labels. Dataset was divided into training, validation and test sets from ECG-gated GE Discovery 570 Cardiac SPECT/CT camera.

Result: Model achieved high performance with Cohen's kappa of 0.962, overall accuracy of 96.5%, and high generalizability. Misclassified 32 cases with tendency to overestimate CAC in 26 out of 32 misclassifications.

Conclusion: The CNN model demonstrates viability for automated CAC scoring into six clinical categories, showing accurate and consistent results comparable to current semiautomatic practice with good generalizability.
...
TL;DR: FlowFeat is a high-resolution, multi-task feature representation that uses motion profile distillation from optical flow networks to enhance dense prediction tasks like video object segmentation, depth estimation, and semantic segmentation.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art networks like transformers produce low-resolution feature grids that are suboptimal for dense prediction tasks, creating a need for high-resolution representations with better geometric and semantic cues.

Method: Novel distillation technique that embeds distribution of plausible apparent motions (motion profiles) using optical flow networks and diverse video data through self-supervised training framework.

Result: Significantly enhances representational power of five state-of-the-art encoders across three dense tasks; computationally inexpensive and robust to inaccurate flow estimation.

Conclusion: FlowFeat advances reliable and versatile dense image representations by encoding high spatial detail with geometric and semantic cues while maintaining temporal consistency.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches lack robust intra-modal mechanisms to assess token significance and fail to model fine-grained uncertainty, leading to poor generalization in complex scenes and inability to capture one-to-many/many-to-one region-word correspondences.

Method: Incorporates significance-aware and granularity-aware modeling using modality-specific biases to identify salient features, and represents region features as a mixture of Gaussian distributions to capture fine-grained uncertainty.

Result: Achieves state-of-the-art performance on Flickr30K and MS-COCO datasets across various backbone architectures, significantly enhancing robustness and interpretability.

Conclusion: The proposed unified approach effectively addresses fundamental limitations in fine-grained image-text alignment and demonstrates superior performance and interpretability compared to existing methods.
...
TL;DR: UltraGS is a Gaussian Splatting framework for ultrasound imaging that enables novel view synthesis with real-time performance and state-of-the-art image quality.


<details>
  <summary>Details</summary>
Motivation: Limited field of view in ultrasound imaging complicates novel view synthesis, which is crucial for clinical diagnostics and surgical planning.

Method: Proposes depth-aware Gaussian splatting with learnable field of view and SH-DARS rendering function combining spherical harmonics with ultrasound physics (depth attenuation, reflection, scattering).

Result: Achieves SOTA results: PSNR up to 29.55, SSIM up to 0.89, MSE as low as 0.002, with real-time synthesis at 64.69 fps on three datasets including their Clinical Ultrasound Examination Dataset.

Conclusion: UltraGS provides an effective solution for ultrasound novel view synthesis with superior performance and real-time capability, benefiting clinical applications.
...
TL;DR: VectorSynth is a diffusion-based framework for generating pixel-accurate satellite images from polygonal geographic annotations with semantic attributes, enabling fine-grained spatial edits and map-informed content generation.


<details>
  <summary>Details</summary>
Motivation: Prior text- or layout-conditioned models lack dense cross-modal correspondences between imagery and semantic vector geometry, limiting their ability to perform fine-grained, spatially grounded edits in satellite image synthesis.
...
Conclusion: VectorSynth enables rapid what-if simulations, spatial edits, and map-informed content generation through its ability to align imagery and semantic vector geometry, representing a significant advancement in satellite image synthesis.
...
TL;DR: Auto-US is an AI system that combines ultrasound videos with clinical text for automated diagnosis, achieving 86.73% accuracy on a new CUV dataset of 495 videos across 5 categories and 3 organs.


<details>
  <summary>Details</summary>
Motivation: Existing AI-assisted ultrasound diagnosis research has limitations in dataset diversity, diagnostic performance, and clinical applicability.

Method: Developed Auto-US system integrating ultrasound video data with clinical diagnostic text, built CUV dataset of 495 videos, created CTU-Net for video classification, and incorporated large language models for diagnostic suggestions.

Result: CTU-Net achieved 86.73% accuracy in ultrasound video classification, diagnostic suggestions received scores above 3/5 from professional clinicians.

Conclusion: Auto-US demonstrates effectiveness and clinical potential for real-world ultrasound applications through its integrated approach combining video analysis with LLM-generated diagnostic insights.
...
TL;DR: Proposes PGCD and DAPD methods for class incremental medical image segmentation that address limitations of existing approaches by using prototype-guided calibration and dual-aligned prototype distillation to better preserve old knowledge while learning new classes.


<details>
  <summary>Details</summary>
Motivation: Existing CIMIS methods either use one-size-fits-all strategies that treat all regions equally (hindering old knowledge preservation) or focus only on global prototype alignment while ignoring local representations in new data (causing knowledge degradation).

Method: PGCD uses prototype-to-feature similarity to calibrate class-specific distillation intensity across spatial regions, while DAPD aligns local prototypes of old classes with both global and local prototypes to enhance old category segmentation.

Result: Comprehensive evaluations on two multi-organ segmentation benchmarks show the method outperforms state-of-the-art approaches, demonstrating robustness and generalization capabilities.

Conclusion: The proposed PGCD and DAPD framework effectively addresses knowledge preservation challenges in CIMIS by combining calibrated distillation and dual-aligned prototype learning, achieving superior performance compared to existing methods.
...
Method: Integrates SMART Vector Median Filtering (SMART-VMF) - a spatially adaptive, multi-scale, robustness-aware mechanism that selectively suppresses corrupted regions while preserving semantic details.

Result: Achieves 79.8% clean accuracy and 46.3% robust accuracy on ImageNet with LaVAN multi-patch attacks (four 1% patches), outperforming existing defenses. Also effective on real medical imagery against natural artifacts.
...
<details>
  <summary>Details</summary>
Motivation: Challenge the conventional view that initial noise in diffusion models is purely random, showing it actually contains rich semantic information that can be analyzed and manipulated.

Method: A two-step training-free process: 1) Semantic erasure - remove unwanted semantics from noise using information theory, 2) Semantic injection - inject desired semantics by leveraging the equivalence between diffusion generation and semantic injection.

Result: The method is consistently effective across various text-to-content models based on both DiT and UNet architectures, providing universal noise modulation capabilities.

Conclusion: The approach presents a novel perspective for optimizing diffusion model generation and provides a universal tool for consistent content generation by revealing and manipulating the semantic patterns hidden in initial noise.
...
TL;DR: SCBCH is a cross-modal hashing framework that addresses noisy multi-label data through semantic-consistent classification and bidirectional soft contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing cross-modal hashing methods rely on fully annotated datasets and are vulnerable to label noise, while ignoring semantic overlaps in multi-label data.

Method: Proposes two modules: Cross-modal Semantic-Consistent Classification (CSCC) to estimate sample reliability and reduce noise impact, and Bidirectional Soft Contrastive Hashing (BSCH) for adaptive contrastive learning based on semantic overlap.

Result: Extensive experiments on four benchmarks show SCBCH consistently outperforms state-of-the-art methods under noisy multi-label conditions.

Conclusion: SCBCH effectively handles noisy multi-label data in cross-modal retrieval through semantic consistency and adaptive contrastive learning, demonstrating superior robustness and performance.
...
TL;DR: DCDNet addresses cross-domain few-shot segmentation by decoupling domain-relevant and category-relevant features using adversarial-contrastive learning, then dynamically fusing them with matrix guidance for better generalization and adaptation.
...
Method: Proposes DCDNet with three modules: Adversarial-Contrastive Feature Decomposition (ACFD) to decouple features, Matrix-Guided Dynamic Fusion (MGDF) to adaptively integrate features, and Cross-Adaptive Modulation (CAM) for enhanced generalization during fine-tuning.
...
Conclusion: DCDNet effectively addresses feature entanglement in cross-domain few-shot segmentation through feature decomposition and adaptive fusion, achieving superior performance across multiple domains.
...
TL;DR: A strong multilabel chest X-ray classification pipeline using SE-ResNeXt101 with Label-Graph Refinement module that improves performance through learned inter-label relationships.


<details>
  <summary>Details</summary>
Motivation: To develop an effective multilabel classification system for chest X-rays that addresses class imbalance and leverages label co-occurrence patterns for improved performance.

Method: Fine-tuned SE-ResNeXt101 backbone with sigmoid head, trained using Multilabel Iterative Stratification, Asymmetric Loss, mixed-precision training, and a novel Label-Graph Refinement module that learns sparse inter-label coupling.

Result: The baseline SE-ResNeXt101 achieved 92.64% macro AUC, and adding Label-Graph Refinement consistently improved validation macro AUC across folds with minimal computational overhead.

Conclusion: The proposed method provides a reproducible, hardware-friendly approach for stronger multilabel CXR classifiers without requiring extra annotations.
...
TL;DR: PC-Diffusion is a lightweight framework that uses a preference classifier to align diffusion models with human preferences, avoiding the computational cost and instability of full model fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Address limitations of DPO methods in diffusion models: high computational cost from full model fine-tuning and sensitivity to reference model quality causing instability.

Method: Uses a trainable Preference Classifier to model relative preferences between samples, decoupling preference alignment from the generative model without needing full fine-tuning or reference models.

Result: Achieves comparable preference consistency to DPO while significantly reducing training costs and enabling efficient, stable preference-guided generation.

Conclusion: PC-Diffusion provides an effective alternative to DPO for preference alignment in diffusion models with theoretical guarantees and practical efficiency benefits.
...
TL;DR: A foundation model for SAR land-cover classification using Dynamic Instance and Contour Consistency Contrastive Learning (DI3CL) that outperforms existing methods across multiple tasks.
...
Method: Proposed DI3CL pre-training framework with Dynamic Instance module for global contextual awareness and Contour Consistency module for geometric contour focus, trained on large-scale SARSense dataset (460,532 images).

Result: Extensive experiments across SAR land-cover mapping, water body detection, and road extraction tasks consistently demonstrate superior performance over existing methods.

Conclusion: The foundation model serves as a robust cornerstone for accelerating downstream SAR land-cover classification model development and deployment.
...
<details>
  <summary>Details</summary>
Motivation: The inherent mismatch between MLLMs' discrete token outputs and IQA's continuous score requirements causes conversion errors and semantic confusion, limiting MLLM-based IQA methods' performance.

Method: Proposes Q-Scorer framework with a lightweight regression module and IQA-specific score tokens integrated into the MLLM pipeline to enable continuous quality scoring.
...
Conclusion: Q-Scorer effectively bridges the gap between discrete MLLM outputs and continuous IQA requirements, significantly improving performance while maintaining MLLMs' original capabilities.
...
Method: Proposes a hierarchical plane-enhanced scene graph using dominant planar structures as spatial anchors, and a task-adaptive subgraph extraction method to filter irrelevant information dynamically.
...
Conclusion: Sparse3DPR demonstrates robustness and generalization capability, providing an effective training-free solution for open-ended 3D scene understanding.
...
TL;DR: Synthetic correlated diffusion imaging (CDI$^s$) enhances prostate cancer lesion segmentation, achieving up to 72.5% improvement over standard diffusion methods without requiring additional scan time.
...
Result: CDI$^s$ integration reliably enhances or preserves segmentation performance in 94% of configurations, with CDI$^s$ + DWI achieving significant improvements in half of architectures with zero degradation.
...
TL;DR: SSOMotion: A human motion synthesis framework that uses unified Scene Semantic Occupancy (SSO) for scene representation, incorporating semantic understanding through CLIP encoding and bi-directional tri-plane decomposition for efficient computation.
...
Result: Extensive experiments on ShapeNet furniture, PROX, and Replica datasets demonstrate cutting-edge performance with effective scene semantic understanding and generalization ability.

Conclusion: SSOMotion effectively integrates semantic scene understanding with motion synthesis, achieving superior performance while reducing computational redundancy through efficient scene representation.
...
TL;DR: CloudMamba is an SSM-based point cloud network that addresses Mamba's limitations in point cloud analysis through sequence expanding/merging, chainedMamba for bidirectional scanning, and grouped selective state space model to reduce overfitting.


<details>
  <summary>Details</summary>
Motivation: To overcome Mamba's limitations in point cloud analysis including imperfect point cloud serialization, insufficient high-level geometric perception, and overfitting of the selective state space model (S6).

Method: Proposes sequence expanding (serializing points along each axis separately) and sequence merging (fusing higher-order features from different sequences), chainedMamba for bidirectional scanning to capture geometric information, and grouped selective state space model (GS6) via parameter sharing to reduce overfitting.

Result: Experiments on various point cloud tasks validate CloudMamba's ability to achieve state-of-the-art results with significantly less complexity.

Conclusion: CloudMamba effectively addresses Mamba's limitations in point cloud analysis and achieves superior performance with reduced computational complexity.
...
TL;DR: MonoCLUE enhances monocular 3D object detection by combining local clustering of visual features with generalized scene memory to address geometric ambiguity and improve detection of occluded objects.
...
Method: 1) K-means clustering on visual features to capture object-level appearance parts; 2) Construct generalized scene memory by aggregating clustered features across images; 3) Integrate both local cluster features and scene memory into object queries to guide attention.
...
Conclusion: MonoCLUE's unified local clustering and generalized scene memory strategy provides consistent representations that generalize across scenes, improving object-level feature consistency for stable detection in varying environments.
...
TL;DR: A universal visual perception framework using flow matching that generates diverse visual representations across multiple tasks, bridging the gap between heterogeneous tasks through a universal velocity field.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models are limited by single-task-single-model paradigm, restricting generalizability and scalability in multi-task scenarios. Inspired by cross-domain generalization of large language models.

Method: Formulates visual perception as universal flow-matching from image patch tokens to task-specific representations. Uses self-supervised foundation model as anchor with multi-scale circular task embedding mechanism to learn universal velocity field.
...
Conclusion: Significant step towards general-purpose visual perception, providing solid foundation for future universal vision modeling research with demonstrated robustness, scalability, and generalization.
...
TL;DR: Proposes a hierarchical auto-regressive sketch generation method that allows flexible stroke-level manipulation during the drawing process, unlike previous methods that require all conditions to be set before generation starts.


<details>
  <summary>Details</summary>
Motivation: Existing sketch generation methods require all stroke-level conditions to be set simultaneously before generation begins, preventing further manipulation during the drawing process. The authors aim to enable more flexible sketch manipulation by allowing real-time adjustments.

Method: Uses a three-stage hierarchical auto-regressive process: 1) predict stroke embeddings, 2) anchor strokes on canvas, 3) translate embeddings to drawing actions. Each stroke is generated considering previously generated strokes and their positions, enabling real-time manipulation through editable stroke embeddings.

Result: The proposed method allows flexible manipulation of sketch drawing at any time during generation by adjusting stroke embeddings, providing more control over the drawing process compared to previous approaches.

Conclusion: The hierarchical auto-regressive approach enables more flexible sketch manipulation during generation, overcoming limitations of previous methods that required all conditions to be set before generation starts.
...
TL;DR: Theoretical analysis of text polarity detection in images for binarization preprocessing.


<details>
  <summary>Details</summary>
Motivation: Text polarity detection is crucial for image binarization in applications like license plate recognition and character recognition. Existing intuitive approaches based on power-law transformation show interesting histogram statistics but lack theoretical foundation.

Method: Theoretical analysis of the phenomenon where maximum between-class variance increases for dark text on bright background and decreases for bright text on dark background after power-law transformation.

Result: Provides theoretical justification for the empirical observations in existing literature about text polarity detection using power-law transformations.

Conclusion: The paper establishes theoretical understanding of text polarity detection mechanisms that were previously only empirically observed, enhancing reliability for image binarization preprocessing tasks.
...
TL;DR: AquaOV255 is the first large-scale underwater segmentation dataset with 255 categories. Earth2Ocean is a training-free framework that transfers terrestrial vision-language models to underwater domains using geometric priors and semantic alignment.
...
Method: Earth2Ocean framework with two components: Geometric-guided Visual Mask Generator (GMG) for local structure perception using self-similarity geometric priors, and Category-visual Semantic Alignment (CSA) for enhancing text embeddings via multimodal reasoning and scene-aware templates.

Result: Extensive experiments on UOVSBench benchmark show Earth2Ocean achieves significant performance improvement while maintaining efficient inference.

Conclusion: The proposed dataset and framework successfully bridge the gap in underwater segmentation, enabling effective transfer of terrestrial vision-language models to marine environments without additional training.
...
TL;DR: HD²-SSC framework addresses dimension and density gaps in camera-based 3D semantic scene completion for autonomous driving by expanding 2D image features to 3D and refining voxel occupancies.
...
Method: Two modules: 1) High-dimension Semantic Decoupling expands 2D features along pseudo third dimension and identifies focal regions; 2) High-density Occupancy Refinement uses detect-and-refine architecture to complete missing voxels and correct erroneous ones.
...
Conclusion: HD²-SSC successfully bridges dimension and density gaps in camera-based 3D semantic scene completion, enhancing autonomous driving scene perception.
...
TL;DR: Novel image-based path planning algorithm using computer vision and disparity maps from UAVs, compared with A* and PRM algorithms.


<details>
  <summary>Details</summary>
Motivation: Terrain depth significantly impacts path safety, and 2D images cannot distinguish craters and hills, requiring depth-aware planning.

Method: Uses disparity maps from UAVs, applies computer vision techniques (edge/line/corner detection, stereo depth reconstruction) to define way-points, and automatically detects start/end points using ArUco markers and circle detection.

Result: Algorithm tested in V-REP simulations and physical lab setup, showing promising results and effectiveness compared to A* and PRM.

Conclusion: Proposed algorithm demonstrates successful path planning using computer vision techniques with depth information, outperforming traditional methods in terrain-aware navigation.
...
<details>
  <summary>Details</summary>
Motivation: Privacy concerns limit deep model training in medical imaging, and existing FL approaches face challenges with data heterogeneity and high resource costs, especially with vision-language models.

Method: Uses masked feature adaptation module for communication, frozen CLIP encoders, masked MLP local classifier, KL divergence-based distillation, and model compression with ensemble predictions.

Result: Achieves 8% higher performance than second best baseline on ISIC2019 dataset and 120x faster than FedAVG with reasonable resource costs.

Conclusion: FedMedCLIP provides an effective FL solution for medical imaging that balances performance, privacy, and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing layout-to-image methods often degrade visual quality and stylistic consistency due to loss of pretrained knowledge when integrating layout conditions.

Method: Uses Layout Synthesis dataset from base model images, Laytrol Network with inherited MM-DiT parameters, zero-initialized outputs, layout encoder initialized as text encoder, and Object-level Rotary Position Embedding.

Result: Qualitative and quantitative experiments show improved effectiveness in generating spatially consistent images while maintaining visual quality.

Conclusion: The proposed approach successfully addresses the challenge of spatial controllability in text-to-image generation while preserving pretrained model knowledge.
...
TL;DR: DiffRegCD is a unified framework that combines dense registration and change detection in a single model, using diffusion features and classification-based correspondence estimation to handle misaligned real-world imagery.


<details>
  <summary>Details</summary>
Motivation: Real-world change detection faces challenges with misaligned inputs due to parallax, viewpoint shifts, and temporal gaps. Existing methods struggle with large displacements and rely on limited registration techniques.
...
Result: Extensive experiments on aerial and ground-level datasets show DiffRegCD consistently outperforms recent baselines and remains reliable under wide temporal and geometric variations.

Conclusion: Diffusion features and classification-based correspondence provide a strong foundation for unified change detection, establishing DiffRegCD as an effective solution for handling misaligned real-world imagery.
...
TL;DR: ISExplore is a segment selection strategy that identifies informative 5-second video segments for talking face generation, achieving 5x faster processing while maintaining high-fidelity output.


<details>
  <summary>Details</summary>
Motivation: Current TFG methods require processing several minutes of reference video which takes hours, limiting practical applications. Exploratory studies showed that short informative segments can achieve comparable or better performance than full videos.
...
Result: Extensive experiments show 5x faster data processing and training speed for NeRF and 3DGS methods while maintaining high-fidelity output quality.

Conclusion: Video informative quality is more important than length. ISExplore enables efficient talking face generation by selecting optimal short segments, making TFG methods more practical for real-world applications.
...
TL;DR: Proposes a multimodal prototype-based MIL approach that uses bidirectional interaction between vision and text modalities to improve generalization and interpretability in computational pathology.


<details>
  <summary>Details</summary>
Motivation: Address challenges in computational pathology where LLMs lack fine-grained medical knowledge for instance-level descriptions and existing MIL methods use unidirectional guidance limiting cross-modal synergy.

Method: Uses frozen LLM to generate task-specific pathological entity descriptions as text prototypes, learns instance-level vision prototypes, and employs Stereoscopic Optimal Transport for fusion to enable broader semantic alignment.
...
Conclusion: The proposed bidirectional multimodal prototype-based MIL approach effectively enhances model generalization and interpretability in computational pathology tasks.
...
TL;DR: ReIDMamba is a pure Mamba-based person re-identification framework that overcomes Transformer scalability issues by using state space models, achieving SOTA performance with only one-third the parameters of TransReID.


<details>
  <summary>Details</summary>
Motivation: To address the scalability limitations of Transformer-based ReID methods which suffer from quadratic computational complexity, while maintaining the ability to extract robust discriminative features for person re-identification.

Method: Proposes a Mamba-based baseline with multiple class tokens, plus two novel techniques: multi-granularity feature extractor (MGFE) with multi-branch architecture and class token fusion, and ranking-aware triplet regularization (RATR) for feature diversity.
...
Conclusion: ReIDMamba successfully demonstrates that pure Mamba-based approaches can effectively address ReID challenges while being more computationally efficient than Transformer-based methods.
...
TL;DR: Proposes Burst Image Quality Assessment (BuIQA) to evaluate frame quality in burst sequences, introduces a benchmark dataset, and develops a unified framework with task-driven prompts for efficient adaptation across downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Redundancy in burst images increases storage/transmission demands and reduces downstream task efficiency, necessitating quality assessment for optimal frame selection.

Method: Establishes benchmark dataset with 7,346 burst sequences and 191,572 quality scores. Proposes unified framework with task-driven prompt generation using knowledge distillation and task-aware quality assessment network.

Result: Outperforms state-of-the-art across 10 downstream scenarios. Achieves 0.33 dB PSNR improvement in denoising and super-resolution tasks by selecting high-quality frames.

Conclusion: BuIQA effectively addresses burst image redundancy by providing quality assessment for frame selection, improving downstream task performance while reducing storage/transmission overhead.
...
Method: Projects labels to images for 2D boxes, extracts image features from pre-trained vision backbone, uses LVLM for text descriptions and text encoder for text features, aligns 3D features with image/text features, fuses features with learned weights, and enhances pseudo labels with 2D detector.
...
Conclusion: MMAssist effectively leverages multi-modal assistance (image and text) to improve 3D UDA performance, demonstrating the value of incorporating additional modalities in domain adaptation for 3D object detection.
...
TL;DR: A modular pipeline using diffusion-based semantic morphing and flow refinement to improve spatial alignment for change detection without retraining existing networks.


<details>
  <summary>Details</summary>
Motivation: Address spatial misalignment in bi-temporal remote sensing images caused by seasonal/multi-year gaps, overcoming limitations of existing models that rely on precise co-registration.

Method: Integrates diffusion-based semantic morphing to create intermediate frames, dense registration, and residual flow refinement with a lightweight U-Net to co-register image pairs.

Result: Extensive experiments on LEVIR-CD, WHU-CD, and DSIFN-CD show consistent gains in registration accuracy and downstream change detection across multiple backbones.

Conclusion: The proposed approach demonstrates generality and effectiveness in improving spatial and temporal robustness for change detection without modifying existing networks.
...
TL;DR: DANCE is a density-agnostic and class-aware network for point cloud completion that preserves observed geometry while completing missing regions using ray-based sampling and transformer refinement.
...
Method: Uses ray-based sampling from multiple viewpoints to generate candidate points, then refines positions and predicts opacity scores via transformer decoder. Includes lightweight classification head trained on geometric features for semantic guidance.
...
TL;DR: Specialized vision-language models for fracture detection and description in chest X-rays outperform general-purpose models in generating accurate fracture reports.
...
Result: Significant improvements over general-purpose models in generating accurate fracture descriptions, with analysis revealing strengths and limitations by fracture type, location, and age.
...
TL;DR: A semantic-guided framework using transformer-based fusion to improve large-mask image inpainting by leveraging structure-aware candidates from pretrained amodal completion models.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of large-mask image inpainting where essential visual content is missing and contextual cues are limited, leading to poor inpainting quality.

Method: Propose Context-Semantic Fusion Network (CSF-Net) - a transformer-based framework that fuses structure-aware candidates from pretrained AC models with contextual features to generate semantic guidance for inpainting.

Result: CSF-Net effectively reduces object hallucination while enhancing visual realism and semantic alignment across diverse masking conditions on Places365 and COCOA datasets.

Conclusion: The framework can be seamlessly integrated into existing inpainting models without architectural changes and consistently enhances performance, providing improved structural accuracy and semantic consistency.
...
<details>
  <summary>Details</summary>
Motivation: Traditional weed management methods using chemical herbicides cause environmental contamination and herbicide resistance. Precision weeding with computer vision offers eco-friendly alternatives but faces limitations from high-power computational requirements.

Method: Deployed YOLOv8n object detector on STM32U575ZI microcontroller using compression techniques including structured pruning, integer quantization, and input image resolution scaling to meet hardware constraints. Trained and evaluated on CropAndWeed dataset with 74 plant species.

Result: Achieved real-time in-situ weed detection with minimal energy consumption of 51.8mJ per inference, enabling scalable deployment in power-constrained agricultural environments.

Conclusion: The system successfully balances detection accuracy and efficiency, providing a practical solution for sustainable weed management in agricultural settings with limited power resources.
...
TL;DR: SharpV is an efficient method for adaptive pruning of visual tokens and KV cache in VideoLLMs, addressing quadratic complexity issues through dynamic spatial-temporal pruning and self-calibrated cache reduction.


<details>
  <summary>Details</summary>
Motivation: Current VideoLLMs suffer from quadratic computational complexity and KV cache scaling issues due to processing excessive redundant visual tokens, requiring efficient compression methods.

Method: Two-stage pruning: (1) Adaptive visual token pruning based on spatial-temporal information, (2) KV cache pruning via self-calibration using similarity to original features, following information bottleneck principles.

Result: Achieves performance gains over dense models on multiple benchmarks, with full compatibility with hardware acceleration techniques like Flash Attention.

Conclusion: SharpV provides an effective framework for reducing computational complexity in VideoLLMs through adaptive pruning while maintaining or improving performance, offering new insights into information flow management.
...
Method: Leverages episodic memory consolidation inspired by avian memory, integrating segmentation with appearance-aware meta-learning memory (AMM) and tracking with geometry-aware localization memory (GLM), then uses visual geometry grounded Transformer (VGGT) to unify 2D-3D tasks.
...
TL;DR: AdvRoad generates natural-looking road-style adversarial posters that can stealthily attack 3D object detectors in autonomous driving systems by making them perceive non-existent objects.
...
Method: Two-stage approach: Road-Style Adversary Generation and Scenario-Associated Adaptation to create natural-looking road-style adversarial patterns that blend with the environment while maximizing attack effectiveness.

Result: AdvRoad successfully attacks various 3D object detectors across different scenes and locations, with physical experiments confirming real-world threats.

Conclusion: The proposed AdvRoad demonstrates practical security threats to autonomous driving systems through stealthy, natural-looking adversarial attacks that bypass human detection while effectively compromising object detectors.
...
TL;DR: Cascade HQP-DETR addresses limitations in Imaginary Supervised Object Detection by introducing high-quality synthetic datasets, proposal-guided query encoding, and cascade denoising, achieving state-of-the-art performance on PASCAL VOC with only synthetic training.


<details>
  <summary>Details</summary>
Motivation: Object detection requires expensive annotated datasets. ISOD trains on synthetic images but faces issues with dataset quality, DETR convergence problems, and denoising overfitting to noise.

Method: Three key innovations: (1) High-quality data pipeline using LLaMA-3, Flux, and Grounding DINO; (2) Proposal-guided query encoding with SAM-generated proposals; (3) Cascade denoising with progressive IoU thresholds across decoder layers.
...
Conclusion: Cascade HQP-DETR successfully addresses ISOD limitations and shows the architecture's universal applicability, advancing from weak to full supervision in synthetic training.
...
TL;DR: A multi-modal deepfake detection framework using FPN-Transformer achieves 0.7535 score on IJCAI'25 DDL-AV benchmark by leveraging cross-modal correlations and precise temporal localization.


<details>
  <summary>Details</summary>
Motivation: Address limitations of unimodal detection methods that can't leverage cross-modal correlations or precisely localize forged segments against sophisticated deepfake manipulations.

Method: Uses pre-trained self-supervised models (WavLM for audio, CLIP for video) with FPN-Transformer architecture, R-TLM blocks with localized attention, and dual-branch prediction for forgery probability and temporal offset refinement.

Result: Achieved 0.7535 final score on IJCAI'25 DDL-AV benchmark test set for cross-modal deepfake detection and localization in challenging environments.
...
TL;DR: 3DGS-QA is the first subjective quality assessment dataset for 3D Gaussian Splatting, with 225 degraded reconstructions across 15 object types, enabling systematic study of distortion factors and introducing a no-reference quality prediction model that operates directly on 3D Gaussian primitives.


<details>
  <summary>Details</summary>
Motivation: The perceptual quality of 3DGS-rendered content under varying reconstruction conditions remains largely underexplored, with factors like viewpoint sparsity, limited training iterations, point downsampling, noise, and color distortions significantly degrading visual quality without systematic perceptual impact studies.

Method: Created 3DGS-QA dataset with controlled distortion factors, developed a no-reference quality prediction model that extracts spatial and photometric cues directly from 3D Gaussian primitives without requiring rendered images or ground-truth references, and benchmarked existing quality assessment methods.

Result: The proposed method consistently achieves superior performance compared to existing quality assessment approaches, demonstrating robustness and effectiveness for 3DGS content evaluation.

Conclusion: 3DGS-QA fills a critical gap in 3DGS quality assessment research, providing the first comprehensive dataset and an effective no-reference quality prediction model that operates directly on Gaussian representations, with publicly available dataset and code to facilitate future research.
...
TL;DR: WEDepth adapts Vision Foundation Models for monocular depth estimation without modifying their structure or pretrained weights, achieving state-of-the-art performance through multi-level feature enhancement.


<details>
  <summary>Details</summary>
Motivation: Monocular depth estimation is challenging due to its ill-posed nature. Vision Foundation Models have shown strong world understanding capabilities that could benefit depth estimation tasks.

Method: Uses VFMs as multi-level feature enhancers to systematically inject prior knowledge at different representation levels, without modifying model structure or pretrained weights.

Result: Establishes new SOTA on NYU-Depth v2 and KITTI datasets, achieving competitive results compared to diffusion-based approaches and methods pre-trained on relative depth. Shows strong zero-shot transfer capability.

Conclusion: WEDepth effectively adapts VFMs for monocular depth estimation while preserving their inherent priors, demonstrating superior performance and generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Address high inter-observer variability in medical image segmentation by capturing diverse expert annotation styles rather than collapsing them into consensus masks.

Method: Uses probabilistic U-Net backbone to capture expert hypotheses, prompt-guided projection for latent space navigation, and multi-level contrastive learning to align textual and visual representations.

Result: Reduces Generalized Energy Distance by 17% and improves mean Dice by more than one point compared to DPersona on lung nodule and prostate MRI datasets.
...
TL;DR: GECO2 is a novel few-shot counting and detection method that addresses object scale issues through dense query representation across scales, achieving superior performance in counting and detection with faster speed and lower memory usage.
...
Result: GECO2 surpasses state-of-the-art few-shot counters by 10% in counting and detection accuracy, while running 3x faster with smaller GPU memory footprint.

Conclusion: The proposed dense query representation effectively addresses object scale issues in few-shot counting and detection, delivering significant improvements in accuracy, speed, and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To address the fundamental trade-off between strong identity consistency and high prompt diversity in subject-driven image generation.

Method: LoRA fine-tuned diffusion model with latent concatenation strategy, masked Conditional Flow Matching objective, and two-stage Distilled Data Curation Framework for training data preparation.
...
Conclusion: The proposed approach achieves effective subject-driven image generation with improved identity consistency and diversity, validated through the CHARIS evaluation framework.
...
TL;DR: I2E converts static images to event streams for SNN training, achieving 300x speedup and state-of-the-art accuracy on ImageNet (60.50%) and CIFAR10-DVS (92.5%).


<details>
  <summary>Details</summary>
Motivation: Address the critical scarcity of event-stream data that hinders adoption of energy-efficient spiking neural networks.

Method: Simulate microsaccadic eye movements using highly parallelized convolution to convert static images into high-fidelity event streams.

Result: Achieved 60.50% accuracy on ImageNet and 92.5% on CIFAR10-DVS through sim-to-real pre-training and fine-tuning, with 300x faster conversion than prior methods.

Conclusion: I2E provides a scalable solution to the event data problem and establishes synthetic event data as a high-fidelity proxy for real sensor data, bridging a key gap in neuromorphic engineering.
...
<details>
  <summary>Details</summary>
Motivation: Traditional radar-based heartbeat sensing suffers from noise degradation, while learning-based methods require costly labeled data. There's a need for unsupervised approaches that maintain noise robustness without expensive ground-truth signals.

Method: Uses heartbeat and noise ranges in radar range matrix to create positive/negative samples. Implements Noise-Contrastive Triplet (NCT) loss with pseudo-labels from traditional radar methods. Includes pseudo-label augmentation with adaptive noise-aware label selection.

Result: Extensive experiments on Equipleth dataset and collected radar dataset show performance comparable to state-of-the-art supervised methods.

Conclusion: Radar-APLANC provides an effective unsupervised solution for radar heartbeat sensing that eliminates dependency on expensive ground-truth data while maintaining noise robustness and performance comparable to supervised approaches.
...
Method: Probing Stable Diffusion with regression layers to predict semantic attributes from internal representations, comparing predictions against human annotations, and analyzing different model components.

Result: Semantic understanding primarily comes from CLIP text encoding rather than the reverse diffusion process; attributes are represented to varying degrees and become harder to disambiguate during diffusion.
...
TL;DR: The paper introduces Beyond the Pixels, a hierarchical evaluation framework that uses structured reasoning with VLMs to assess fine-grained identity preservation in generative models, outperforming existing metrics and aligning better with human judgments.
...
Method: A hierarchical framework that decomposes identity assessment into feature-level transformations by (1) hierarchically decomposing subjects into (type, style) -> attribute -> feature decision tree, and (2) prompting for concrete transformations rather than abstract similarity scores.

Result: The framework demonstrates strong alignment with human judgments across four state-of-the-art generative models and introduces a new benchmark with 1,078 image-prompt pairs spanning diverse subject types including underrepresented categories.

Conclusion: The hierarchical decomposition grounds VLM analysis in verifiable visual evidence, reducing hallucinations and improving consistency in identity preservation evaluation.
...
Conclusion: StableMorph improves biometric security evaluation by creating more realistic attacks and supports development of more robust detection systems, setting new standards for morph quality.
...
TL;DR: This paper introduces a new dataset for face recognition security focusing on Nylon Face Masks (NFMs) - a novel 3D spoofing attack that mimics facial geometry with elastic, photorealistic masks.


<details>
  <summary>Details</summary>
Motivation: Face recognition systems are widely deployed but vulnerable to presentation attacks, particularly concerning new threats like NFMs that can closely simulate victims' facial features.

Method: Created a dataset using iPhone 11 Pro with 3,760 genuine samples from 100 subjects and 51,281 NFM attack samples across four presentation scenarios (humans/mannequins), then benchmarked with five state-of-the-art PAD methods.

Result: Results showed significant performance variability across PAD methods, indicating challenges in detecting NFM attacks and poor generalization to unseen attack conditions.

Conclusion: NFMs pose serious security threats to face recognition systems, highlighting the need for PAD techniques that can effectively generalize to emerging spoofing methods.
...
<details>
  <summary>Details</summary>
Motivation: Latent fingerprint identification is challenging due to low image quality, background noise, and partial impressions that make traditional methods less effective.

Method: Combines EfficientNet-B0 (CNN) and Swin Tiny (Transformer) backbones with spatial attention module, fuses features into 512D embeddings, and uses cosine similarity for matching in closed-set identification.
...
Conclusion: The proposed LatentPrintFormer demonstrates superior performance for latent fingerprint identification by effectively combining local and global feature representations with noise suppression.
...
TL;DR: Proposes a federated learning framework combining SAM2 with Flower framework for privacy-preserving foam segmentation in wastewater treatment plants, enabling collaborative training without data sharing.


<details>
  <summary>Details</summary>
Motivation: Foam formation in WTPs reduces efficiency and increases costs, but standard ML requires large labeled datasets and faces data scarcity, heterogeneity, and privacy concerns that prevent data sharing between plants.
...
Result: Framework accelerates training convergence and improves segmentation performance even with limited local datasets by leveraging SAM2's pre-trained weights, validated on real-world and synthetic datasets.

Conclusion: Provides a practical, scalable, privacy-aware solution for automatic foam tracking, demonstrating potential of integrating foundational models into FL systems for industrial challenges with distributed sensitive data.
...
<details>
  <summary>Details</summary>
Motivation: Existing STR frameworks suffer from decoupled visual-linguistic optimization that causes cross-modal misalignment, with visual encoders biased toward background distractors and decoders misaligned with geometrically deformed text.

Method: Three-stage architecture: (1) Dual Attention Macaron Encoder (DAME) for refined visual features, (2) Position-Aware Module and Semantic Quantizer for spatial-semantic integration, (3) Multi-Modal Collaborative Verifier for cross-modal self-correction.

Result: Achieves 83.5% average accuracy on Union14M-L benchmark and 79.1% on heavily occluded OST dataset, setting new records in 9 out of 14 evaluation scenarios.

Conclusion: OTSNet's neurocognitive-inspired unified approach effectively addresses cross-modal misalignment in STR, demonstrating superior performance for irregular text patterns through hierarchical cognitive processing.
...
<details>
  <summary>Details</summary>
Motivation: Existing Event-RGB datasets are limited by sparse extreme condition coverage and low spatial resolution (≤640x480), preventing comprehensive evaluation of detectors in challenging scenarios.

Method: Created a large-scale pixel-aligned dataset with 130+ spatiotemporal-aligned sequences and 340k manual bounding boxes, then benchmarked 14 methods across Event-based, RGB-based, and Event-RGB fusion configurations.

Result: Fusion-based models performed best on full test set and normal subset, but in illumination challenge subset, top event-based model outperformed all fusion models, while fusion models still beat RGB-based counterparts.

Conclusion: PEOD establishes a realistic benchmark for multimodal perception, revealing limitations of existing fusion methods when frame modality is severely degraded, and facilitates future research in robust object detection.
...
TL;DR: Proposes Boomda, a balanced multi-objective optimization method for multimodal domain adaptation that addresses varying domain shifts across different modalities through information bottleneck and correlation alignment.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of expensive manual annotation in multimodal learning and the scarcity of annotated data, particularly the under-explored area of heterogeneous multimodal domain adaptation where different modalities experience varying domain shifts.
...
Result: Extensive empirical results demonstrate that Boomda outperforms competing schemes and showcases effectiveness of the proposed approach.

Conclusion: Boomda provides an efficient modality-balanced multimodal domain adaptation algorithm that effectively handles varying domain shifts across different modalities through balanced multi-objective optimization.
...
TL;DR: Proposes a Non-Aligned Reference (NAR-IQA) framework for evaluating perceptual quality of Novel View Synthesis images without pixel-level alignment, using contrastive learning with LoRA-enhanced DINOv2 embeddings trained on synthetic distortions.


<details>
  <summary>Details</summary>
Motivation: Existing FR-IQA methods fail with misaligned references in NVS, while NR-IQA methods lack generalization. There's a need for quality assessment that handles partial scene content without pixel-level alignment.

Method: Built a large-scale dataset with synthetic distortions targeting Temporal Regions of Interest. Uses contrastive learning framework with LoRA-enhanced DINOv2 embeddings, supervised by existing IQA methods. Trained exclusively on synthetic distortions to avoid overfitting.
...
Conclusion: The proposed NAR-IQA framework effectively addresses the challenge of quality assessment for NVS images with non-aligned references, demonstrating robust performance and generalization capability.
...
TL;DR: LandSegmenter is a Land Use and Land Cover foundation model framework that addresses data scarcity through weak supervision, multi-modal integration, and zero-shot transfer learning.


<details>
  <summary>Details</summary>
Motivation: Current LULC models are limited by modality specificity and fixed taxonomies, while foundation models require expensive fine-tuning or massive labeled data that's impractical in remote sensing.

Method: Proposes three-stage solution: 1) LAS dataset with weak labels from existing products, 2) RS-specific adapter with text encoder for cross-modal features, 3) confidence-guided fusion for zero-shot performance.

Result: Achieves competitive/superior performance on six LULC datasets, particularly excelling in zero-shot transfer to unseen datasets, demonstrating effective weak supervision for task-specific FMs.

Conclusion: LandSegmenter framework effectively addresses LULC mapping challenges through scalable weak supervision and multi-modal integration, enabling practical foundation model development in remote sensing.
...
TL;DR: Proposes Mg-MRN, a multi-granularity mutual refinement network for zero-shot learning that enhances visual features through decoupled multi-granularity feature learning and cross-granularity interactions.
...
Method: Uses multi-granularity feature extraction module for decoupled region feature mining, followed by cross-granularity feature fusion module to strengthen interactions between region features of varying granularities.
...
Conclusion: Mg-MRN effectively refines discriminative and transferable visual features through multi-granularity learning and cross-granularity interactions, improving ZSL performance.
...
TL;DR: Proposes a shadow generation framework using Keypoints Linear Model and Shadow Triangle Algorithm to create realistic shadows in image composition, outperforming existing methods in geometric accuracy and visual realism.
...
Method: Uses Keypoints Linear Model (KPLM) with 9 keypoints and bounding block for physically plausible shadow projection, combined with Shadow Triangle Algorithm (STA) for geometric calculations of shadow angles, lengths, and positions.
...
Conclusion: The proposed framework successfully addresses limitations in shadow generation by combining physical modeling with geometric formulations, producing more realistic and geometrically accurate shadows in image composition.
...
TL;DR: Proposes a Distributed Zero-Shot Learning (DistZSL) framework that leverages decentralized data for unseen classes using cross-node attribute regularization and global attribute-to-visual consensus to handle data heterogeneity.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning effective zero-shot learning models from distributed data while handling data heterogeneity issues across different nodes.

Method: Introduces two key components: 1) cross-node attribute regularizer that enforces similar attribute feature distances across nodes, and 2) global attribute-to-visual consensus that ensures consistent bilateral mapping between attribute and visual feature distributions.
...
Conclusion: The proposed DistZSL framework effectively handles data heterogeneity in distributed settings and significantly enhances zero-shot learning performance across different nodes.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion-based anomaly detection methods rely on synthetic noise generation and require per-class model training, limiting their generalization and scalability for multi-class real-world images.

Method: Integrates pre-trained Vision-Language Model with Latent Diffusion Model, using VLM-generated image descriptions as conditioning for LDM training to learn robust normal image representations without manual annotations.

Result: Achieves competitive performance with up to 25-point improvement in pixel-level PRO metric on Real-IAD dataset and 8-point improvement on COCO-AD dataset, outperforming state-of-the-art diffusion-based approaches.

Conclusion: The proposed VLMDiff framework effectively leverages vision-language models to enhance unsupervised multi-class visual anomaly detection, demonstrating superior performance and scalability compared to existing methods.
...
TL;DR: WarpGAN improves 3D GAN inversion for single-shot novel view synthesis by using a warping-and-inpainting strategy to better handle occluded regions, overcoming limitations of existing methods that rely solely on generative priors.


<details>
  <summary>Details</summary>
Motivation: Existing 3D GAN inversion methods focus on reconstructing visible regions but generate poor-quality occluded regions due to information loss from low bit-rate latent codes, limiting the realism and multi-view consistency of novel view synthesis.

Method: Proposed WarpGAN with warping-and-inpainting strategy: 1) Use 3D GAN inversion encoder to get latent code, 2) Warp to novel view using depth map from 3D GAN, 3) Use SVINet with symmetry prior and multi-view correspondence to inpaint occluded regions.

Result: Quantitative and qualitative experiments show WarpGAN consistently outperforms state-of-the-art methods in novel view synthesis quality, particularly for occluded regions.

Conclusion: The warping-and-inpainting strategy effectively addresses the limitation of existing 3D GAN inversion methods by incorporating image inpainting, resulting in improved quality and realism for both visible and occluded regions in single-shot novel view synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Box-level IoU prediction suffers from structural coupling where predicted IoU can be overestimated for poorly localized boxes due to inherent similarity between predicted and ground-truth boxes.

Method: Proposes Pixel-level Quality Assessment (PQA) framework that measures alignment between pixel positions relative to predicted and ground-truth boxes, avoiding direct box comparison.

Result: Extensive experiments on HRSC2016 and DOTA show consistent improvements: +5.96% AP50:95 on Rotated RetinaNet and +2.32% on STD.

Conclusion: PQA effectively eliminates similarity bias in quality estimation and can be seamlessly integrated into various oriented object detectors to improve performance.
...
<details>
  <summary>Details</summary>
Motivation: Current UI programming approaches face limitations in multimodal coding capabilities and underutilize iterative visual feedback, making UI development complex and inefficient.

Method: Proposed UI2Code^N model trained through staged pretraining, fine-tuning, and reinforcement learning, unifying UI-to-code generation, UI editing, and UI polishing with test-time scaling for interactive generation.

Result: Establishes new state-of-the-art on UI-to-code and UI polishing benchmarks, achieving performance comparable to leading closed-source models like Claude-4-Sonnet and GPT-5.

Conclusion: The interactive UI-to-code paradigm with UI2Code^N significantly improves multimodal coding capabilities and demonstrates the value of iterative visual feedback in UI development workflows.
...
Method: Introduces a loss function that penalizes open space regions using auxiliary datasets, leveraging the observation that deep network features cluster around class means arranged as vertices of a regular simplex.
...
Conclusion: The proposed open-set recognition method effectively addresses the challenge of identifying unknown medical conditions using auxiliary data and geometric properties of feature representations.
...
TL;DR: Large image-to-3D models have strong canonical view bias, struggling with rotated inputs. A lightweight CNN can detect and correct orientation to restore performance.


<details>
  <summary>Details</summary>
Motivation: To understand the inductive biases and limitations of large-scale image-to-3D generative models, particularly their inability to generalize across viewpoints.

Method: Conducted controlled experiments using 2D rotations on Hunyuan3D 2.0 model, and developed a lightweight CNN for orientation detection and correction.

Result: State-of-the-art models show degraded performance under rotated inputs, but this can be mitigated by the proposed CNN correction method without modifying the main generative model.

Conclusion: Scale alone may not be sufficient for robust 3D generation; modular, symmetry-aware designs should be pursued to address viewpoint generalization issues.
...
TL;DR: The paper evaluates a decoupled multimodal pipeline for food recognition, combining EfficientNet-B4 with Google's Gemini LLM, and analyzes trade-offs between visual accuracy, efficiency, and generative output quality.


<details>
  <summary>Details</summary>
Motivation: To address the need for robust automated nutritional analysis and culinary guidance in digital food applications, while evaluating the impact of visual classification errors on generative output quality.

Method: Comparative evaluation of a multimodal pipeline with different vision backbones (EfficientNet-B4, VGG-16, ResNet-50, YOLOv8) and LLMs (Gemini, Gemma), using a new Custom Chinese Food Dataset (CCFD) to address cultural bias. Introduces Semantic Error Propagation (SEP) analysis.

Result: EfficientNet-B4 achieved 89.0% Top-1 accuracy (best balance), Gemini achieved 9.2/10 factual accuracy (superior generative quality), but system utility is bottlenecked by visual front-end accuracy. High semantic similarity identified as critical failure mode.

Conclusion: The overall system performance is fundamentally limited by the visual classification accuracy, with semantic error propagation from visual to generative modules being a key challenge.
...
TL;DR: 2Dto3D-SR is a real-time single-view 3D super-resolution framework that converts 3D data into structured 2D representations using PNCC, enabling use of existing 2D super-resolution models without needing high-resolution RGB guidance.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing 3D super-resolution methods that require complex 3D point processing or high-resolution RGB guidance, making real-time 3D super-resolution more practical and accessible.

Method: Encodes 3D data from single viewpoint into 2D representation using Projected Normalized Coordinate Code (PNCC), allowing direct application of 2D image super-resolution architectures. Implemented with Swin Transformers for accuracy and Vision Mamba for efficiency.

Result: Swin Transformer model achieves state-of-the-art accuracy on benchmarks, while Vision Mamba model delivers competitive results at real-time speeds, demonstrating the framework's effectiveness across different performance requirements.

Conclusion: The geometry-guided pipeline provides a simple yet practical solution for real-world 3D super-resolution scenarios, particularly when high-resolution RGB data is unavailable, with both high-accuracy and real-time efficient implementations available.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use uniform local regions with fixed sizes, limiting adaptability to geometric complexity variations in point cloud surface reconstruction.

Method: Adaptively modulates spacing and size of local regions based on input point cloud curvature.

Result: Improved reconstruction accuracy and efficiency compared to fixed-size uniform approaches.

Conclusion: Curvature-based adaptive local region modulation enhances surface reconstruction performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing vision-language fine-tuning methods overlook semantic relationships within images that are highlighted in textual context, leading to suboptimal performance in multimodal alignment.

Method: Extract multilevel semantic features from vision encoders, project vision features to group related semantics, and fuse visual-textual features using inheritable cross-attention that removes redundant visual relationships by discarding low-correlation feature pairs.

Result: The method outperforms all existing methods when evaluated on eight foundation models and two downstream tasks (visual question answering and image captioning).

Conclusion: The proposed approach effectively improves multimodal alignment and fusion by considering both semantics and relationships, demonstrating superior performance across multiple foundation models and tasks.
...
TL;DR: DiPVNet is a direction-perceptive vector network that addresses rotational variations in point clouds through atomic dot-product operators that simultaneously encode directional selectivity and rotation invariance at both local and global levels.


<details>
  <summary>Details</summary>
Motivation: Point cloud processing faces challenges from arbitrary rotations that disrupt intrinsic directional characteristics. Existing methods fail to fully exploit multiscale directional nature of point clouds for enhanced feature representations.

Method: Proposes DiPVNet with Learnable Local Dot-Product (L2DP) Operator for adaptive local structure capture and global directional response spectrum based on direction-aware spherical Fourier transform (DASFT) for holistic directional modeling.

Result: Extensive experiments show DiPVNet achieves state-of-the-art performance on point cloud classification and segmentation tasks under challenging scenarios with noise and large-angle rotations.

Conclusion: DiPVNet effectively models rotational symmetry while maintaining adaptive directional perception through its novel dot-product operators, demonstrating superior performance in handling rotational variations in point clouds.
...
TL;DR: NERVE is a training-free open-vocabulary semantic segmentation method that uses neighborhood structure from stable diffusion self-attention, entropy-based attention selection, and stochastic random walk refinement to achieve state-of-the-art performance without conventional post-processing.


<details>
  <summary>Details</summary>
Motivation: Existing training-free OVSS methods have limitations including expensive affinity refinement, ineffective attention map fusion, and reliance on fixed-size Gaussian kernels that enforce isotropic neighborhoods.
...
Conclusion: NERVE provides an effective training-free approach for open-vocabulary semantic segmentation that effectively delineates objects with arbitrary shapes through spatial diffusion and selective attention mechanisms.
...
TL;DR: LayerEdit is a training-free multi-object image editing framework that addresses attention entanglements in inter-object conflict regions through object-layered decomposition and coherent fusion, enabling conflict-free multi-object editing.


<details>
  <summary>Details</summary>
Motivation: Existing text-driven multi-object image editing methods follow localize-editing paradigm but neglect critical inter-object interactions, leading to editing leakage or constraints due to attention entanglements in conflict regions.
...
Conclusion: LayerEdit successfully addresses inter-object attention entanglements through precise layer decomposition and coherent fusion, enabling effective disentangled multi-object editing without training requirements.
...
TL;DR: Top2Ground is a diffusion-based method that directly generates photorealistic ground-view images from aerial images without intermediate representations, using joint spatial and semantic conditioning.


<details>
  <summary>Details</summary>
Motivation: Generate ground-level images from aerial views is challenging due to extreme viewpoint differences, occlusions, and limited field of view. Existing methods often rely on intermediate representations like depth maps or 3D voxels.

Method: Uses diffusion-based approach conditioned on joint VAE-encoded spatial features (from aerial RGB and height maps) and CLIP-based semantic embeddings to ensure geometric constraints and semantic consistency.

Result: Achieves 7.3% average improvement in SSIM across CVUSA, CVACT, and Auto Arborist datasets, handling both wide and narrow fields of view with strong generalization.

Conclusion: Top2Ground demonstrates robust performance in directly generating ground-level images from aerial views without intermediate representations, showing strong generalization across diverse datasets.
...
TL;DR: ImageBindDC is a novel multimodal data condensation framework that uses Characteristic Function loss in ImageBind's unified feature space to preserve inter-modal dependencies, achieving lossless performance with just 5 condensed datapoints per class.


<details>
  <summary>Details</summary>
Motivation: Existing data condensation techniques fail in multimodal scenarios where preserving intricate inter-modal dependencies is crucial for effective model training.

Method: Uses Characteristic Function loss in Fourier domain for statistical alignment, enforcing three levels of distributional consistency: uni-modal, cross-modal, and joint-modal alignment within ImageBind's unified feature space.

Result: On NYU-v2 dataset, achieves lossless performance comparable to full dataset training with just 5 condensed datapoints per class - 8.2% absolute improvement over previous best method and 4x faster condensation time.

Conclusion: ImageBindDC effectively addresses multimodal data condensation by preserving complex inter-modal relationships through unified feature space and characteristic function-based alignment.
...
TL;DR: Proposes Edge-awareness Semantic Concordance framework for resilient fusion of event and RGB modalities in semantic segmentation under extreme conditions, addressing feature-level mismatch through edge-aware latent re-coding and uncertainty optimization.


<details>
  <summary>Details</summary>
Motivation: Existing semantic segmentation methods suffer from significant information loss in extreme conditions (insufficient light, camera motion), and current multi-modality approaches using event data face feature-level mismatch between heterogeneous event and RGB modalities.

Method: Edge-awareness Semantic Concordance framework with Edge-awareness Latent Re-coding that realigns event-RGB features using uncertainty indicators and edge dictionary, followed by Re-coded Consolidation and Uncertainty Optimization for heterogeneous fusion.

Result: Outperforms state-of-the-art by 2.55% mIoU on proposed DERS-XS dataset, with superior resilience under spatial occlusion.

Conclusion: The proposed framework effectively unifies multi-modality heterogeneous features using latent edge cues, demonstrating robust performance in extreme conditions for semantic segmentation.
...
TL;DR: SWAN is a swipe-based web app that speeds up histopathology image annotation while maintaining quality comparable to traditional folder-based methods.
...
Method: Developed SWAN (SWipeable ANnotations), an open-source web application that enables intuitive image patch classification using swiping gestures on both desktop and mobile platforms, with real-time metadata capture and flexible gesture-to-class mapping.

Result: In a pilot study with 4 pathologists annotating 600 mitotic figure patches, SWAN achieved pairwise percent agreement of 86.52%-93.68% (Cohen's Kappa = 0.61-0.80), comparable to folder-based method's 86.98%-91.32% (Cohen's Kappa = 0.63-0.75). Participants rated it highly usable and appreciated mobile annotation capability.

Conclusion: SWAN can accelerate image annotation while maintaining annotation quality, offering a scalable and user-friendly alternative to conventional workflows for histopathology image analysis.
...
TL;DR: Proposes MAUGIF, an unsupervised general image fusion method that classifies fusion tasks into additive and multiplicative types, using dual cross-image autoencoders with mechanism-aware decoder architectures.
...
Method: Uses dual cross-image autoencoders with shared latent space, dual encoders capture common content while isolating modality-specific details, and mechanism-aware dual decoders selectively reintegrate unique characteristics during fusion.
...
TL;DR: SynWeather is the first dataset for unified multi-region, multi-variable weather data synthesis, and SynWeatherDiff is a probabilistic diffusion transformer model that addresses over-smoothing in weather synthesis.


<details>
  <summary>Details</summary>
Motivation: Current weather data approaches are limited to single-variable, single-region tasks using deterministic modeling, which restricts unified synthesis across variables and regions, overlooks cross-variable complementarity, and causes over-smoothed results.

Method: Introduced SynWeather dataset covering four regions (Continental US, Europe, East Asia, Tropical Cyclone regions) with high-resolution observations of key weather variables. Developed SynWeatherDiff, a probabilistic weather synthesis model based on Diffusion Transformer framework.
...
Conclusion: The proposed SynWeather dataset and SynWeatherDiff model successfully address the limitations of current approaches by enabling unified multi-region, multi-variable weather data synthesis while overcoming the over-smoothing problem through probabilistic modeling.
...
Conclusion: SkelSplat provides an effective framework for multi-view 3D human pose estimation that generalizes well across different scenarios and is robust to occlusions, without requiring 3D ground-truth supervision.
...
TL;DR: NeuSpring is a neural spring field method that creates physical digital twins of deformable objects from videos, achieving better reconstruction and simulation for current state modeling and future prediction.


<details>
  <summary>Details</summary>
Motivation: Existing methods focus on current state modeling but generalize poorly to future prediction because they ignore intrinsic physical properties of deformable objects.

Method: Uses neural spring field with two innovations: 1) piecewise topology solution for multi-region spring connections using zero-order optimization, 2) neural spring field representing spring properties across frames with canonical coordinate-based neural network.

Result: Achieves superior performance on real-world datasets with Chamfer distance improved by 20% for current state modeling and 25% for future prediction.

Conclusion: NeuSpring effectively models deformable objects' physical properties and enables better reconstruction and simulation for both current states and future predictions.
...
<details>
  <summary>Details</summary>
Motivation: Minimizing inconsistencies across successive AI versions is crucial, especially in image classification where negative flips occur when updated models misclassify previously correct samples. This problem worsens with more training classes over time.

Method: Preserves original model margins while learning improved models using explicit margin-calibration on logits and integrates double-source focal distillation loss with previous and new independently trained models.
...
Conclusion: The proposed approach effectively mitigates negative flips in model updates by balancing margin preservation and new class learning through calibrated constraints and distillation techniques.
...
<details>
  <summary>Details</summary>
Motivation: Accurate spatial alignment across time points in mammography is crucial for detecting meaningful tissue changes and improving deep learning-based breast cancer risk prediction models.
...
Result: Image-based registration consistently outperformed other approaches across all metrics (predictive accuracy, precision, recall, deformation field quality), with regularized deformation fields improving quality but reducing risk prediction performance.
...
TL;DR: DiveSeg is a novel underwater instance segmentation framework that leverages DINO pretrained models with two key components: AquaStyle Aligner for underwater domain adaptation and ObjectPrior Prompter for object-level guidance, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Underwater instance segmentation is crucial for marine exploration and ecological protection, but existing methods struggle with underwater domain adaptation and require both object- and instance-level reasoning.

Method: Built on DINO foundation model with two components: AquaStyle Aligner embeds underwater color style features during fine-tuning, and ObjectPrior Prompter uses binary segmentation prompts to provide object-level priors.

Result: Achieves state-of-the-art performance on UIIS and USIS10K datasets, demonstrating effectiveness in underwater instance segmentation tasks.

Conclusion: DiveSeg effectively adapts pretrained foundation models to underwater domains and provides a robust framework for underwater instance segmentation with superior performance.
...
TL;DR: Proposes SASG-DA, a diffusion-based data augmentation method for sEMG gesture recognition that enhances faithfulness through semantic guidance and improves diversity via sparse-aware sampling.


<details>
  <summary>Details</summary>
Motivation: sEMG-based gesture recognition suffers from limited training data causing overfitting and poor generalization in deep learning models. Existing data augmentation methods struggle with balancing faithfulness and diversity.

Method: SASG-DA uses Semantic Representation Guidance for faithful generation, Gaussian Modeling Semantic Modeling for flexible sampling, and Sparse-Aware Semantic Sampling to explore underrepresented regions.
...
Conclusion: The proposed approach effectively mitigates overfitting and improves gesture recognition by providing both faithful and diverse augmented samples.
...
TL;DR: VideoChain is a novel Multi-hop Video Question Generation framework that generates reasoning-intensive questions requiring analysis across multiple video segments, outperforming existing methods on standard metrics.


<details>
  <summary>Details</summary>
Motivation: Existing multi-hop question generation is limited to text, while video question generation only handles zero-hop questions over single segments. There's a need for systems that can generate questions requiring reasoning across multiple video segments.

Method: Modular architecture built on modified BART backbone enhanced with video embeddings to capture textual and visual dependencies. Uses TVQA+ dataset to automatically construct MVQ-60 dataset by merging zero-hop QA pairs.
...
Conclusion: VideoChain effectively generates coherent, contextually grounded, and reasoning-intensive questions that span multiple video segments, demonstrating the framework's capability for multi-hop video reasoning.
...
TL;DR: SLOPE is a unified framework that effectively combines structured sparsity and low-bit quantization to compress deep neural networks while maintaining high accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks are computationally expensive and large, making them difficult to deploy on resource-constrained devices. While weight quantization and structured sparsity are effective individually, their naive combination severely harms model accuracy.

Method: SLOPE uses a training-time regularization strategy that minimizes discrepancy between full-precision weights and their sparse, quantized counterparts by promoting angular alignment rather than direct matching.

Result: On ResNet-18, SLOPE achieves ~20× model size reduction while retaining ~99% of original accuracy. It outperforms state-of-the-art quantization and structured sparsity methods across classification, detection, and segmentation tasks on various models.

Conclusion: SLOPE provides a principled way to combine structured sparsity and quantization, enabling significant model compression without substantial accuracy loss across multiple computer vision tasks.
...
TL;DR: A hierarchical VQ-VAE model that learns disentangled motion-to-clean image features for generalized MRI motion correction without requiring artifact-specific training.


<details>
  <summary>Details</summary>
Motivation: Existing ML-based MRI motion correction methods struggle to generalize across different motion types and body regions, as they are often tailored to specific applications and datasets.

Method: Proposed a hierarchical vector-quantized variational auto-encoder with a codebook to capture finite motion patterns at multiple resolutions, enabling coarse-to-fine correction. An auto-regressive model learns the prior distribution of motion-free images to guide correction.

Result: The method demonstrated robust correction across varying motion severity on simulated whole-body motion artifacts and effectively disentangled physical motion features.

Conclusion: The approach improves generalizability of ML-based MRI motion correction and shows potential for application across anatomical regions and motion types through motion feature disentanglement.
...
TL;DR: The paper questions the importance of strict positional equivariance in Rotary Positional Encodings (RoPE), shows RoPE is a general solution for 1D data, proposes Spherical RoPE with non-commutative generators that performs equally well or better than equivariant alternatives.


<details>
  <summary>Details</summary>
Motivation: To challenge the conventional belief that relative positional embeddings (specifically RoPE's positional equivariance) are crucial for performance, and to explore whether removing the equivariance constraint could lead to better positional encodings for vision tasks.

Method: Mathematically analyzed RoPE as a general solution for equivariant positional embedding in 1D data, proposed Mixed RoPE for M-dimensional data with commutative generators, and introduced Spherical RoPE with non-commutative generators that doesn't enforce strict equivariance.

Result: Spherical RoPE with non-commutative generators demonstrated equivalent or better learning behavior compared to equivariant analogues like Mixed RoPE, suggesting relative positional embeddings may not be as important as commonly believed in computer vision.

Conclusion: Relative positional embeddings might not be essential for good performance in computer vision, and removing the requirement for strict equivariance could enable faster and better-generalizing positional encodings for vision applications.
...
TL;DR: Introduces Text-based Aerial-Ground Person Retrieval (TAG-PR) for retrieving person images from aerial and ground views using text descriptions, along with a new dataset and framework to handle viewpoint discrepancies.


<details>
  <summary>Details</summary>
Motivation: Traditional Text-based Person Retrieval focuses only on ground-view images, lacking practical significance for real-world scenarios involving aerial views. The large viewpoint discrepancy between aerial and ground images presents unique challenges.

Method: Created TAG-PEDES dataset with automatically generated text descriptions using diversified text generation. Developed TAG-CLIP framework with hierarchically-routed mixture of experts for view-specific/agnostic features and viewpoint decoupling strategy for cross-modal alignment.

Result: TAG-CLIP was evaluated on the proposed TAG-PEDES dataset and existing T-PR benchmarks, demonstrating effectiveness in handling view heterogeneity.

Conclusion: The work successfully addresses the new task of aerial-ground person retrieval with text descriptions, providing both a dataset and framework that handle viewpoint discrepancies effectively.
...
TL;DR: RAPTR is a weakly supervised radar-based 3D human pose estimation method that uses only 3D bounding boxes and 2D keypoints instead of costly 3D keypoint labels, achieving state-of-the-art performance on indoor radar datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional radar-based indoor 3D human pose estimation requires expensive 3D keypoint labels that are difficult to obtain in complex indoor environments with clutter, occlusions, or multiple people.
...
Conclusion: RAPTR enables scalable 3D human pose estimation from radar using only weak supervision (3D BBox and 2D keypoints), making it practical for real-world indoor applications.
...
TL;DR: Anatomy-VLM is a fine-grained vision-language model that incorporates multi-scale anatomical information and structured knowledge to achieve expert-level disease diagnosis from radiology images.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models treat images holistically and overlook fine-grained details crucial for medical diagnosis, while clinicians analyze specific anatomical regions using prior medical knowledge.
...
Result: Anatomy-VLM achieves outstanding performance on both in- and out-of-distribution datasets, validates on downstream segmentation tasks, and enables zero-shot anatomy-wise interpretation.

Conclusion: The model's fine-grained alignment captures anatomical and pathology-related knowledge, providing strong expert-level clinical interpretation capabilities.
...
TL;DR: OmniAID is a novel AIGI detection framework that uses a decoupled Mixture-of-Experts architecture to separate content-specific flaws from universal artifacts, achieving superior generalization across diverse generative models and content domains.
...
Method: A decoupled Mixture-of-Experts architecture with Routable Specialized Semantic Experts for different content domains and a Fixed Universal Artifact Expert, trained using a two-stage strategy with domain-specific hard-sampling followed by gating network training.
...
TL;DR: Proposes DBPNet with Cross-Pyramid Consistency Regularization (CPCR) for semi-supervised medical image segmentation, achieving state-of-the-art performance by leveraging consistency learning between dual decoders.


<details>
  <summary>Details</summary>
Motivation: To effectively exploit unlabeled data for semi-supervised medical image segmentation by combining consistency learning and uncertainty minimization approaches.

Method: Designs a hybrid Dual Branch Pyramid Network (DBPNet) with encoder and two slightly different decoders, and introduces CPCR learning strategy that extends soft-labeling to pyramid predictions across decoders for knowledge distillation.

Result: DBPNet with CPCR outperforms five state-of-the-art self-supervised learning methods and achieves comparable performance with recent methods on a public benchmark dataset.

Conclusion: The proposed hybrid consistency learning approach effectively leverages unlabeled data through cross-pyramid consistency regularization between dual decoders, demonstrating superior performance in semi-supervised medical image segmentation.
...
TL;DR: Contrastive Integrated Gradients (CIG) is a novel attribution method for Whole Slide Image analysis that enhances interpretability by computing contrastive gradients in logit space, providing sharper differentiation between tumor subtypes while maintaining theoretical soundness.
...
Method: CIG computes contrastive gradients in logit space by comparing feature importance relative to a reference class, highlights class-discriminative regions, satisfies integrated attribution axioms, and introduces MIL-AIC and MIL-SIC metrics to measure attribution quality under weak supervision.

Result: CIG yields more informative attributions across three cancer datasets (CAMELYON16, TCGA-RCC, TCGA-Lung), showing quantitative improvements in MIL-AIC/MIL-SIC metrics and qualitative alignment with ground truth tumor regions.

Conclusion: CIG demonstrates strong potential for interpretable and trustworthy WSI-based diagnostics by providing sharper tumor differentiation while maintaining theoretical consistency.
...
TL;DR: Automated classification and object detection of peripheral blood cells using Faster R-CNN with ResNet-50-FPN backbone, comparing baseline training vs transfer learning from COCO dataset.


<details>
  <summary>Details</summary>
Motivation: Address data scarcity and heterogeneity in peripheral blood cell analysis by creating a unified dataset from four public sources and developing robust automated detection systems.

Method: Developed data pipeline to merge four datasets (PBC, BCCD, Chula, Sickle Cell), used Faster R-CNN with ResNet-50-FPN backbone, compared randomly initialized baseline vs transfer learning from COCO.
...
TL;DR: CoMa introduces a compressed pre-training phase that transforms vision-language models into competitive embedding models through contrastive learning, achieving state-of-the-art results with minimal data.


<details>
  <summary>Details</summary>
Motivation: Current VLMs require large-scale contrastive learning for embedding adaptation, but comprehensive input understanding can facilitate better downstream task performance through decoupled optimization objectives.

Method: Proposes CoMa - a compressed pre-training phase that serves as a warm-up stage for contrastive learning, enabling transformation of VLMs into embedding models with minimal data.

Result: Achieves new state-of-the-art results on MMEB benchmark among VLMs of comparable size, demonstrating optimization in both efficiency and effectiveness.

Conclusion: The compressed pre-training approach successfully decouples comprehensive understanding from discriminative feature learning, enabling efficient adaptation of VLMs into high-performance embedding models.
...
TL;DR: A fast multi-organ segmentation framework using hierarchical sparse sampling and Residual Transformer to reduce computation time while maintaining accuracy, achieving ~2.24 seconds on CPU.
...
Result: Improved qualitative and quantitative segmentation performance on 10,253 CT images and TotalSegmentator dataset, achieving ~2.24 seconds processing time on CPU hardware.

Conclusion: The method demonstrates potential for real-time fine organ segmentation with fast speed and maintained accuracy compared to current fast organ classifiers.
...
TL;DR: CleverBirds is a large-scale benchmark for knowledge tracing in fine-grained bird species recognition, featuring over 17M questions from 40K+ participants across 10K+ species, enabling study of visual expertise development.


<details>
  <summary>Details</summary>
Motivation: Understanding how humans develop fine-grained visual expertise through learning progression is challenging but essential for modeling visual learning and inferring knowledge states.

Method: Created a benchmark using data from eBird citizen-science platform, containing long-range learning patterns with ~400 questions per participant across multiple-choice questions about bird species.

Result: Tracking learners' knowledge is challenging across participant subgroups and question types, with contextual information providing varying predictive benefits. The dataset supports development of knowledge tracing methods.

Conclusion: CleverBirds enables new research avenues for studying visual expertise development over time and across individuals, being one of the largest benchmarks with high concept diversity.
...
<details>
  <summary>Details</summary>
Motivation: Real-world applications require complex, iterative video workflows that combine multiple capabilities, but current specialized AI models only excel at isolated tasks, creating a gap for unified video processing systems.

Method: Uses Plan-and-Act dual-agent architecture: planner agent interprets user intentions and decomposes tasks, executor agents execute through modular MCP-based tool servers, with hierarchical multi-level memory for long-horizon reasoning.

Result: Enables iterative any-conditioned video workflows (text/image/video-conditioned generation → multi-round editing → object segmentation → compositional synthesis) and introduces UniVA-Bench benchmark for evaluation.

Conclusion: UniVA and UniVA-Bench are open-sourced to catalyze research on interactive, agentic, and general-purpose video intelligence for next-generation multimodal AI systems.
...
TL;DR: LSLM is a framework that uses LLMs to translate 3D American Sign Language, capturing spatial and depth information for more accurate ASL-to-text translation and improving accessibility for hearing-impaired individuals.


<details>
  <summary>Details</summary>
Motivation: To enhance digital communication accessibility for hearing-impaired individuals by moving beyond 2D video-based sign language recognition to leverage 3D data, and to explore how LLMs can process embodied multimodal languages beyond text.

Method: Leverages LLMs as backbone for 3D ASL translation, using 3D sign language data to capture spatial, gestural, and depth information. Investigates both direct translation from 3D gesture features to text and instruction-guided translation modulated by external prompts.

Result: Enables more accurate and resilient ASL translation compared to 2D methods, with flexible translation capabilities through prompt modulation.

Conclusion: Provides foundational step toward inclusive multimodal intelligent systems capable of understanding diverse forms of language, broadening LLMs' understanding beyond text-based inputs.
...
TL;DR: 3D4D is an interactive 4D visualization framework that uses WebGL and Supersplat rendering to convert static images and text into coherent 4D scenes, featuring real-time multi-modal interaction through foveated rendering.


<details>
  <summary>Details</summary>
Motivation: To enable adaptive, user-driven exploration of complex 4D environments by transforming static visual and textual data into interactive 4D scenes.
...
Conclusion: 3D4D provides an effective solution for interactive 4D visualization, enabling user-driven exploration of complex environments through its integrated rendering approach.
...
TL;DR: A robust framework for 3D mesh reconstruction from multi-view images with noisy camera poses, combining pose refinement with neural implicit scene representation to produce editable meshes compatible with standard 3D tools.


<details>
  <summary>Details</summary>
Motivation: Accurate 3D reconstruction is essential for robotics but challenging due to noisy camera poses in real-world settings. Existing NeRF methods rely on precise extrinsics and use implicit representations incompatible with standard 3D software.

Method: Jointly refines camera poses while learning an implicit scene representation that captures geometry and appearance, producing editable 3D meshes.

Result: Achieves accurate and robust 3D reconstruction under pose uncertainty on standard benchmarks, producing high-quality meshes compatible with common graphics and robotics tools.
...
TL;DR: Proposed attention-based Vision Transformer for UE positioning using Angle Delay Profile from CSI, achieving 38% improvement over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing DL models for UE positioning have limitations: equal attention to entire input and poor performance with non-sequential data like instantaneous CSI.

Method: Attention-based Vision Transformer architecture focusing on Angle Delay Profile from CSI matrix, validated on DeepMIMO and ViWi ray-tracing datasets.

Result: Achieved RMSE of 0.55m indoors, 13.59m outdoors in DeepMIMO, and 3.45m in ViWi's outdoor blockage scenario. Outperformed state-of-the-art by ~38%.
...
<details>
  <summary>Details</summary>
Motivation: Current methods either prioritize spatial transcriptomics (losing histopathology structural information) or use vanilla contrastive learning that overly smooths functional information. There's a need to preserve both structural and functional features for better tumor substructure identification.

Method: SENCA-st (Shared Encoder with Neighborhood Cross Attention) uses cross-attention mechanisms to emphasize regions that are structurally similar in histopathology but functionally different in spatial transcriptomics, preserving features from both modalities.
...
Conclusion: SENCA-st effectively integrates histopathology and spatial transcriptomics data, overcoming limitations of existing methods by preserving both structural and functional information through cross-attention mechanisms.
