...
Method: Proposed ESCA framework with SGClip model trained on 87K+ videos using neurosymbolic learning pipeline with model-driven self-supervision from video-caption pairs.

Result: SGClip excels in scene graph generation and action localization benchmarks. ESCA consistently improves MLLMs, achieving SOTA performance in embodied environments and reducing perception errors.

Conclusion: ESC A enables open-source models to surpass proprietary baselines by providing structured spatial-temporal understanding through scene graph generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing sparse detectors overlook token representation quality, leading to sub-optimal foreground quality and limited performance. The paper identifies geometric structure preservation and class distribution as key factors for improving sparse detector performance.

Method: Proposes Sparse Selector (SS) with Ray-Aware Supervision (RAS) to preserve geometric information, Class-Balanced Supervision to adaptively reweight class semantics, and Ray Positional Encoding to address LiDAR-image modality distribution differences. Integrated into CrossRay3D end-to-end detector.

Result: Achieves state-of-the-art performance on nuScenes benchmark with 72.4 mAP and 74.7 NDS, running 1.84× faster than other leading methods. Demonstrates strong robustness even with partial or complete LiDAR/camera data missing.

Conclusion: CrossRay3D successfully addresses token representation quality issues in sparse detectors through geometric structure preservation and class-balanced supervision, achieving superior performance and efficiency while maintaining robustness to sensor failures.
...
<details>
  <summary>Details</summary>
Motivation: Manual infrastructure inspection is costly and hazardous, while existing automatic systems are limited to individual defects or provide unstructured outputs that don't guide maintenance effectively.

Method: Leverages CCTV streams with YOLO object detectors for multi-defect detection and segmentation, then passes detections to vision language models (VLMs) for scene-aware summarization and structured JSON output generation.

Result: The system accurately identifies diverse defects and produces coherent, structured summaries including incident descriptions, recommended tools, dimensions, repair plans, and urgent alerts.

Conclusion: The pipeline shows promise for automated infrastructure monitoring but faces challenges in scaling to city-wide deployments.
...
<details>
  <summary>Details</summary>
Motivation: Traditional IAD methods lack multi-turn dialogues and detailed anomaly descriptions, while existing large model approaches haven't fully leveraged MLLMs' capabilities for anomaly detection tasks.
...
Conclusion: IAD-GPT effectively combines MLLMs with visual grounding techniques to enable robust industrial anomaly detection with detailed descriptive capabilities.
...
Result: AI-SR achieved highest diagnostic accuracy (κ=0.71 vs 0.58-0.60), fastest reporting times (25±9s vs 37±18s vs 88±38s), reduced eye movements, and was the preferred mode. SR guided visual attention toward images.

Conclusion: Structured reporting improves efficiency by directing visual attention to images, while AI-prefilled structured reporting further enhances both diagnostic accuracy and user satisfaction.
...
<details>
  <summary>Details</summary>
Motivation: Machine learning models trained on imbalanced datasets often exhibit systematic errors from interactions of multiple attributes like object class and environmental conditions.

Method: Intersectional Fairness Evaluation Framework (IFEF) combines quantitative fairness metrics with interpretability tools, plus Bias-Weighted Augmentation (BWA) that adapts transformation intensities based on subgroup statistics.
...
<details>
  <summary>Details</summary>
Motivation: Previous quantization approaches were non-differentiable and required manual derivative setting in backpropagation, making learning questionable. Also, prior shift/logarithmic quantization methods either avoided activation quantization or achieved lower accuracy.

Method: A differentiable quantization approach that can scale to n-bit quantization, learning logarithmic quantized values of form 2^n. The method provides proof of convergence to optimal neural network.

Result: With ResNet18 on ImageNet, weight-only quantization achieves less than 1% accuracy drop compared to full precision. Both weight and activation quantization achieve SOTA-comparable accuracy in 15 training epochs with slightly higher CPU instructions but no higher precision multiplication required.

Conclusion: The proposed differentiable quantization method provides convergence guarantees, supports n-bit quantization, and achieves near-full-precision accuracy with minimal training epochs and reasonable inference costs.
...
<details>
  <summary>Details</summary>
Motivation: Road surface damage threatens traffic safety and hinders sustainable urban development, but accurate detection is challenging due to diverse damage shapes, difficulty capturing slender cracks with high aspect ratios, and high error rates in small-scale damage recognition.

Method: StripRFNet comprises three modules: (1) Shape Perception Module (SPM) with large separable kernel attention for shape discrimination; (2) Strip Receptive Field Module (SRFM) using large strip convolutions and pooling for slender cracks; (3) Small-Scale Enhancement Module (SSEM) with high-resolution P2 feature map, dedicated detection head, and dynamic upsampling for small-object detection.

Result: On RDD2022 benchmark, StripRFNet achieves highest F1-score of 80.33% compared with CRDDC'2022 participants and ORDDC'2024 Phase 2 results. On Chinese subset, it improves F1-score, mAP50, and mAP50:95 by 4.4, 2.9, and 3.4 percentage points over baseline, while maintaining competitive inference speed.
...
Method: Uses color space perturbations on individual objects during training for robustness, generates diverse pedestrian instances with diffusion models, and applies object perturbations at inference to quantify uncertainty through detection score variance.

Result: Experiments on NuImages 10K dataset with YOLOv8 show accuracy improvements, uncertainty reduction across all object classes, and higher uncertainty values for false positives compared to true positives.

Conclusion: ObjectTransforms is an effective lightweight mechanism for reducing and quantifying uncertainty in vision-based perception during both training and inference phases.
...
<details>
  <summary>Details</summary>
Motivation: To provide timely access to a comprehensive egocentric multimodal dataset for research, capturing real-world interactions between wearers and their environment using state-of-the-art wearable technology.

Method: Dataset collection using Aria Gen 2 glasses worn by primary subject Dia'ane and friends during daily activities across five scenarios (cleaning, cooking, eating, playing, outdoor walking), with incremental release strategy.

Result: Created a publicly available dataset with comprehensive raw sensor data and machine perception algorithm outputs, demonstrating robust performance across diverse users and conditions.

Conclusion: A2PD provides valuable multimodal egocentric data for research, available at projectaria.com with open-source tools and usage examples in Project Aria Tools.
...
<details>
  <summary>Details</summary>
Motivation: Current methods fail when geometry between input and appearance objects differs significantly. Direct application of 3D generative models produces poor results, requiring a more principled approach for appearance transfer.

Method: Uses pretrained rectified flow models conditioned on image/text, with training-free guidance added periodically during sampling. Guidance includes part-aware losses for appearance and self-similarity modeled as differentiable loss functions.

Result: Successfully transfers texture and geometric details to 3D assets, outperforming baselines qualitatively and quantitatively. Traditional metrics are unsuitable, so evaluation uses GPT-based ranking system confirmed by user study.

Conclusion: The proposed method effectively transfers appearance to 3D assets and is generalizable to different diffusion models and guidance functions.
...
<details>
  <summary>Details</summary>
Motivation: To enhance efficiency and safety of thrombectomy for ischemic stroke by automating critical aspects of the procedure, as current methods are resource and personnel-intensive.

Method: Self-supervised framework that classifies various skeletal landmarks using a regression-based pretext task, with the positional pretext task enhancing downstream classification performance.

Result: The model outperforms existing methods in both regression and classification tasks, with positional pretext task significantly improving downstream classification performance.

Conclusion: The framework shows promise for extending toward fully autonomous C-arm control to optimize trajectories from pelvis to head during stroke thrombectomy procedures.
...
Method: Proposes DuetMatch with dual-branch asynchronous optimization (encoder and decoder trained separately), Decoupled Dropout Perturbation for regularization, Pair-wise CutMix Cross-Guidance for diversity, and Consistency Matching to reduce confirmation bias from noisy pseudo-labels.
...
Conclusion: DuetMatch demonstrates effective and robust performance for medical image segmentation by addressing optimization stability and noise issues through its novel dual-branch architecture and regularization techniques.
...
Method: Predicts 3D displacement vectors from arbitrary starting X-ray images, captures aleatoric and epistemic uncertainties, uses conformal prediction for calibration, and combines probabilistic loss with skeletal pose regularization.
...
Conclusion: The pipeline shows potential as a safe and reliable component for autonomous C-arm systems in clinical workflows.
...
<details>
  <summary>Details</summary>
Motivation: Current deep generative models don't meet traditional photographic quality standards, requiring manual IQA which is slow and expensive due to low yield of acceptable generated images.

Method: Developed a formula to estimate cost savings based on precision and pass yield of IQA engines, and applied it to background inpainting use case with AutoML solution.

Result: Achieved significant cost saving of 51.61% through automatic pre-filtering that increases overall quality of images sent for manual review.

Conclusion: Automatic IQA pre-filtering can substantially reduce costs in production pipelines using generated images, with the proposed formula providing a framework for estimating savings.
...
<details>
  <summary>Details</summary>
Motivation: To understand how the brain encodes visual information by reconstructing images from fMRI signals, and to determine which latent space best supports this transformation while capturing the compositional nature of visual stimuli.

Method: Projects fMRI signals into a structured text space as intermediate representation, uses object-centric diffusion module to generate images by composing individual objects, and includes attribute relationship search module to identify key attributes and relationships aligned with neural activity.

Result: Extensive experiments show PRISM outperforms existing methods, achieving up to 8% reduction in perceptual loss on real-world datasets.
...
<details>
  <summary>Details</summary>
Motivation: Traditional model-centric approaches struggle in tropical agriculture due to high cloudiness, diverse crop calendars, and limited datasets. Remote sensing in these areas faces unique challenges that require a shift in focus from models to data quality.

Method: The paper advocates a Data-Centric AI pipeline that prioritizes techniques like confident learning, core-set selection, data augmentation, and active learning. It reviews 25 strategies and proposes a practical pipeline using the 9 most mature methods.

Result: The paper identifies ready-to-use data-centric strategies suitable for large-scale agricultural mapping pipelines, with particular emphasis on their applicability in tropical contexts where traditional methods fail.

Conclusion: A data-centric approach focusing on data curation and quality is essential for developing robust and scalable AI models that can handle the dynamic realities of tropical agriculture, offering practical solutions through a curated pipeline of mature methods.
...
<details>
  <summary>Details</summary>
Motivation: Current video training methods are tailored for transformers and fail to leverage SSMs' unique attributes, causing spatio-temporal inflexibility where models degrade on unseen resolutions, limiting performance across short- and long-form videos.
...
Result: Outperforms transformer and SSM baselines by up to 28% on short-action (UCF-101, HMDB-51) and long-action (COIN, Breakfast) benchmarks, with strong adaptability to fine-grained actions (SSV2, Diving-48).

Conclusion: Provides a simple drop-in training recipe that makes video SSMs more robust, resolution-agnostic, and efficient across diverse action recognition scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning models struggle to capture global facial features critical to human aesthetic perception. Vision Transformers model long-range relationships but have quadratic complexity, while Mamba models offer linear complexity for sequential dependencies.

Method: A heterogeneous ensemble architecture that fuses Vision Transformer (for global facial structure and symmetry) with Mamba-based Vision model (for efficient long-range dependency modeling with linear complexity). Uses Grad-CAM for interpretability analysis.

Result: Achieved state-of-the-art performance on SCUT-FBP5500: Pearson Correlation of 0.9212, MAE of 0.2085, RMSE of 0.2698. Visualizations confirmed complementary feature extraction between the two backbones.

Conclusion: VM-BeautyNet presents a powerful new architectural paradigm for computational aesthetics, effectively combining global structure modeling with efficient sequential feature extraction for facial beauty prediction.
...
<details>
  <summary>Details</summary>
Motivation: OCSCC often goes undetected due to subtle early stages and hidden development areas, leading to preventable deaths. Early detection using CNNs could save lives.

Method: Trained a CNN on 4293 images of benign/malignant tumors and negative samples. Tested on images at 5 different resolutions and developed hardware for image capture and processing.

Result: Higher resolution images led to more accurate predictions with diminishing returns. The system achieved measurable precision, recall, and mAP scores for OCSCC detection.

Conclusion: CNNs combined with proper image capture hardware can effectively detect OCSCC, with image resolution playing a crucial role in detection accuracy.
...
Method: Collected data from 439 participants in a multi-camera stage, capturing over 54 million frames of 3D motion tracking including hand tracking and body shape, with text annotations and separate audio tracks.
...
<details>
  <summary>Details</summary>
Motivation: Human behaviors contain rich cues about scene dynamics and can help address ambiguities in static object-level reconstruction by leveraging intentional interactions.

Method: Online approach that iteratively disassembles and reconstructs the environment using human-object interaction cues, integrating camera/object pose estimation, instance decomposition, and online map updating with Gaussian splatting for efficient rendering.

Result: Achieves accurate and consistent dynamic scene modeling with photorealistic rendering, validated in multiple real-world scenarios with promising advantages.

Conclusion: The proposed system provides a flexible, progressive alternative to conventional object-level reconstruction methods by capitalizing on human-object interaction cues in dynamic environments.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models (VLMs) offer superior zero-shot detection for video anomaly detection but have high computational costs and unstable visual grounding, hindering real-time deployment.

Method: Two-stage cascaded system with offline learning of normal behavioral rules, motion mask prompting to direct VLM attention to motion regions, and rule-based deviation detection that identifies anomalies as deviations from learned norms.

Result: Achieves 57.68 fps on NVIDIA L40S GPU (151.79× speedup) with 97.2% accuracy comparable to state-of-the-art VLM-based VAD methods across four datasets.

Conclusion: Cerberus establishes itself as a practical solution for real-time video analytics by balancing efficiency and accuracy through its innovative two-stage approach.
...
<details>
  <summary>Details</summary>
Motivation: To address fundamental challenges in evaluating membership inference attacks against large vision-language models, where prior results were inflated by distributional bias rather than true membership detection.

Method: Created a controlled benchmark with 6,000 images where member and non-member distributions are carefully balanced, with ground-truth labels across three training stages.

Result: State-of-the-art MIA methods converged to random chance performance when evaluated under unbiased conditions in OpenLVLM-MIA.

Conclusion: The benchmark clarifies current limitations of MIA research on LVLMs and provides a foundation for developing stronger privacy-preserving techniques.
...
<details>
  <summary>Details</summary>
Motivation: To enable precise transfer of stroke attributes (line thickness, deformation, texture sparsity) from reference styles to content images while maintaining semantic structure and content fidelity.

Method: Introduces cross-image stroke attention mechanism embedded in self-attention layers for fine-grained semantic correspondences, plus adaptive contrast enhancement and semantic-focused attention for content preservation.
...
Conclusion: Stroke2Sketch provides an effective training-free solution for sketch style transfer with superior stroke attribute control and semantic preservation compared to existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: To systematically analyze how deepfake detection performance scales with data quantity and diversity, and to understand the scaling laws in this domain.

Method: Constructed ScaleDF dataset with 5.8M real images from 51 domains and 8.8M fake images from 102 methods, then analyzed performance scaling patterns.

Result: Observed power-law scaling where detection error decreases predictably as number of real domains or deepfake methods increases, enabling performance forecasting.

Conclusion: Scaling laws apply to deepfake detection, allowing data-driven performance prediction and suggesting data-centric approaches to counter evolving deepfake technology.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models are limited to sub-1K resolutions due to quadratic attention complexity and lack of native 4K training data, creating a need for scalable ultra-high-resolution text-to-image generation.

Method: Uses hierarchical local attention with low-resolution global guidance, dividing high-res latents into local windows and injecting global semantics via scaled positional anchors with LoRA adaptation for consistency.

Result: Achieves 2x faster inference, lower memory usage, and reliable 4K resolution generation with superior global coherence and local detail compared to dense attention baselines and state-of-the-art methods.

Conclusion: Hierarchical local attention with guided low-resolution anchors is an effective approach for advancing ultra-high-resolution image generation without requiring additional high-resolution training data.
...
TL;DR: DiffusionX is a cloud-edge collaborative framework that reduces diffusion model generation time by 15.8% while maintaining image quality, using lightweight on-device previews and cloud-based final refinements.


<details>
  <summary>Details</summary>
Motivation: To address the computational intensity of diffusion models and the iterative prompt refinement process that increases latency and burdens cloud resources.

Method: A cloud-edge collaborative framework with lightweight on-device diffusion model for rapid previews and high-capacity cloud model for final refinements, plus a noise level predictor to balance computation load.

Result: Reduces average generation time by 15.8% compared to Stable Diffusion v1.5 while maintaining comparable image quality, and only 0.9% slower than Tiny-SD with significantly improved image quality.

Conclusion: DiffusionX demonstrates efficiency and scalability with minimal overhead, providing an effective solution for prompt-based image generation workflows.
...
Method: Three-part token-level enhancement: 1) Token Index Embedding clusters tokens for same reference images, 2) Instruct Token Injection provides extra visual feature container with detailed priors, 3) Identity-Token Disentanglement (ITD) strategy explicitly guides tokens to independently represent each identity's features.

Result: Significantly enhances existing AR methods in conditional image generation, achieving good identity consistency while preserving high-quality background reconstruction. Surpasses current state-of-the-art models in multiple reference image generation tasks.

Conclusion: TokenAR framework effectively solves reference identity confusion in multi-subject generation, enabling high-quality and high-diversity outputs. The authors also introduce InstructAR Dataset, the first large-scale open-source dataset for multi-reference image generation training and evaluation.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of understanding about how training strategies reshape vision encoders in MLLMs, particularly given the shift from SFT to RL training paradigms.
...
Result: RL produces stronger and more precisely localized visual representations than SFT, leading to better performance on vision-related VQA benchmarks. The proposed PIVOT method outperforms larger vision encoders with only 1% computational cost.

Conclusion: Vision encoder training strategy significantly impacts MLLM performance, and RL-based optimization (PIVOT) provides an efficient path for advancing MLLM vision backbones.
...
<details>
  <summary>Details</summary>
Motivation: Existing noun filtering strategies in Language-assisted Image Clustering lack theoretical foundation despite being intuitive, creating a need for a more rigorous approach.

Method: GradNorm measures noun positiveness using gradient magnitudes from cross-entropy between predicted target distribution and softmax output, with theoretical error bounds.

Result: Extensive experiments show GradNorm achieves state-of-the-art clustering performance on various benchmarks and subsumes existing filtering strategies as special cases.

Conclusion: GradNorm provides a theoretically grounded and empirically effective solution for positive noun filtering in Language-assisted Image Clustering.
...
Method: Created MIRAD dataset capturing three key dimensions: diverse individualized products with large intra-class variation, data from six geographically dispersed manufacturing nodes, and substantial imaging heterogeneity.

Result: Evaluation of SOTA anomaly detection methods shows significant performance drop compared to conventional benchmarks, highlighting unresolved complexities in real-world individualized production.
...
<details>
  <summary>Details</summary>
Motivation: Current cataract surgery datasets lack the diversity and annotation depth needed for training generalizable deep-learning models in computer-assisted surgery systems.

Method: Collected 3,000 cataract surgery videos from two surgical centers with surgeons of varying experience levels, enriched with four annotation layers: temporal surgical phases, instance segmentation, instrument-tissue interaction tracking, and quantitative skill scores based on ICO-OSCAR rubrics.
...
Conclusion: The dataset addresses the gap in diverse, deeply annotated cataract surgery resources and supports development of generalizable deep-learning models for computer-assisted surgery systems.
...
Result: Developed end-to-end platform with real-time detection, automated alerts to contractors, and actionable analytics for evidence-driven repair planning and budget allocation.

Conclusion: iWatchRoadv2 enables data-driven smart city management, transparent governance, and sustainable improvements in road infrastructure maintenance through complete automation of pothole monitoring lifecycle.
...
<details>
  <summary>Details</summary>
Motivation: While powerful 3D parametric models exist for humans and animals, equally expressive approaches for modeling plants are lacking. The work aims to address this gap in plant modeling.

Method: The authors developed Demeter, a data-driven parametric model that encodes plant morphology factors including topology, shape, articulation, and deformation. They collected a large-scale ground-truthed dataset from a soybean farm as a testbed.

Result: Experiments show that Demeter effectively synthesizes shapes, reconstructs structures, and simulates biophysical processes.

Conclusion: Demeter provides a compact learned representation for plant morphology that handles varying topology and multiple sources of shape variation, advancing crop plant modeling capabilities.
...
Result: 42% end-to-end efficiency improvement, 3.1x frame rate boost on Raspberry Pi 5, 2.98x overall speed-up, with minimal accuracy loss (PA-MPJPE increases from 9.0mm to 9.1mm on FreiHAND).

Conclusion: The framework achieves comparable accuracy to state-of-the-art methods while significantly enhancing computational efficiency for edge device deployment.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D segmentation methods struggle with ambiguous, reasoning-based instructions, while 2D vision-language models lack intrinsic 3D spatial understanding, creating a gap between complex human instructions and precise 3D object grounding.

Method: Uses 3D Gaussian Splatting representations to render photorealistic views for MLLM comprehension. Proposes Global-to-Local Spatial Grounding: multiple global views for coarse localization, then close-up novel views for fine-grained segmentation.
...
Conclusion: REALM demonstrates practical utility and versatility in bridging complex human instructions with precise 3D object grounding without requiring extensive 3D-specific post-training.
...
<details>
  <summary>Details</summary>
Motivation: Vision-language models often fail to adequately utilize visual evidence, relying instead on linguistic priors or textual shortcuts. Reinforcement learning could help but lacks scalable reward mechanisms.

Method: Proposes SSL4RL framework that reformulates self-supervised learning objectives (like predicting image rotation or reconstructing masked patches) into dense, automatic reward signals for RL-based fine-tuning.

Result: Substantially improves performance on both vision-centric and vision-language reasoning benchmarks. Also shows generality by applying to graph learning with significant gains.

Conclusion: SSL4RL establishes a versatile paradigm for aligning multimodal models using verifiable, self-supervised objectives, with identified key factors like task difficulty and semantic alignment influencing effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: Traditional point and line matching are treated separately, and while GlueStick combined them, its heavy architecture prevented real-time use on edge devices.

Method: Uses Attentional Line Message Passing (ALMP) to explicitly expose line connectivity to the network, enabling efficient communication between nodes in a lightweight architecture.

Result: Establishes new state-of-the-art performance across different benchmarks while maintaining computational efficiency.

Conclusion: LightGlueStick successfully combines point and line matching in a lightweight framework suitable for real-time applications and edge devices.
...
<details>
  <summary>Details</summary>
Motivation: Traditional deepfake detection lacks transparency and generalization, requiring detectors that provide verifiable reasoning explanations alongside detection results.

Method: Uses Spatio-Temporal Subtle Information Tokenization (ST-SIT) for feature extraction and Fine-grained Multimodal Chain-of-Thought (Fg-MCoT) with facial feature constraints for reasoning. Built on ER-FF++ benchmark dataset.

Result: Achieves outstanding performance in detection accuracy, explainability, and robustness across cross-forgery methods and cross-dataset scenarios.

Conclusion: EDVD-LLaMA provides a more explainable and superior solution compared to previous deepfake video detection methods.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing methods in precisely localizing target persons and predicting fine-grained actions in language-guided atomic video action recognition, particularly in complex multi-person scenarios.

Method: RefAtomNet++ framework with multi-hierarchical semantic-aligned cross-attention mechanism and multi-trajectory Mamba modeling at partial-keyword, scene-attribute, and holistic-sentence levels, using dynamic scanning trajectories for visual spatial tokens.
...
Conclusion: The proposed multi-hierarchical semantic-aligned cross-attention and multi-trajectory Mamba modeling effectively improve cross-modal alignment and fine-grained action recognition in RAVAR tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional object detection frameworks underperform for rotated objects due to limitations in capturing orientation variations, especially in applications like aerial imagery and autonomous driving.

Method: Proposes rotation-invariant loss function using Gaussian bounding box representation and Bhattacharyya distance, with anisotropic Gaussian representation to address isotropic variance issues in square-like objects.
...
Conclusion: The approach shows potential to establish new benchmarks in rotated object detection for applications requiring precise object localization regardless of orientation.
...
<details>
  <summary>Details</summary>
Motivation: Mitochondria segmentation from EM images is valuable but annotation-intensive. Unsupervised domain adaptation methods have low practical performance, so weakly supervised approaches with minimal point annotations are needed.

Method: Multi-task learning framework with joint segmentation and center detection, cross-teaching mechanism, class-focused cross-domain contrastive learning, and instance-aware pseudo-label selection strategy.

Result: Outperforms existing UDA and WDA methods, significantly narrowing performance gap with supervised upper bound. Also achieves substantial improvements over other UDA techniques.

Conclusion: The proposed weakly supervised domain adaptation approach effectively leverages sparse point annotations to achieve high-performance mitochondria segmentation with minimal annotation effort.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLN methods focus on historical information, ignoring future implications of actions, leading to suboptimal long-term navigation decisions.

Method: Trains Q-model with unlabeled trajectory data to generate Q-features for candidate actions, then integrates these with navigation instructions using cross-modal future encoder for A*-style search.
...
Conclusion: The proposed foresighted approach with Q-learning and future-aware features significantly enhances goal-oriented VLN by considering long-term outcomes.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D Gaussian Splatting compression methods lack human priors, leading to suboptimal bitrate efficiency and reconstruction quality, which limits their use in streamable 3D avatar systems.

Method: Disentangles Gaussian representation into structural layer (StyleUNet-based generator mapping poses to Gaussians) and motion layer (SMPL-X model for compact pose variations). Uses facial attention mechanism to preserve identity and expression details under low-bitrate constraints.
...
Conclusion: HGC-Avatar offers an effective hierarchical compression framework that enables efficient transmission and high-quality rendering of dynamic avatars while supporting progressive decoding and controllable rendering.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks overlook multimodal inconsistencies in scientific papers, either isolating single modalities or using synthetic errors that fail to capture real-world complexity, undermining clarity, reproducibility, and trust in scientific research.

Method: Created PRISMM-Bench through a multi-stage pipeline: review mining, LLM-assisted filtering, and human verification to curate 262 inconsistencies from 242 papers. Designed three tasks: inconsistency identification, remedy, and pair matching. Introduced structured JSON-based answer representations to minimize linguistic biases.

Result: Benchmarked 21 leading LMMs including large open-weight models (GLM-4.5V 106B, InternVL3 78B) and proprietary models (Gemini 2.5 Pro, GPT-5). Results show strikingly low performance (26.1-54.2%), highlighting the challenge of multimodal scientific reasoning.

Conclusion: Current LMMs struggle significantly with detecting and resolving multimodal inconsistencies in scientific papers, motivating the need for progress towards more trustworthy scientific assistants capable of reliable multimodal reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Out-of-stock detection is crucial for retail verification to identify product unavailability on shelves, and existing methods need improvement in accuracy.

Method: Extends YOLOv8 with additional convolutional branches for OOS detection, product segmentation, and depth estimation. Uses pseudo-labeled depth data from Depth Anything V2 with a proposed depth normalization procedure for training stability.

Result: Outperformed state-of-the-art OOS detection methods by 1.8% mAP. Ablation studies showed auxiliary learning improved mAP by 3.7% and depth normalization by 4.2%.

Conclusion: The proposed OOS-DSD method successfully advances OOS detection through auxiliary learning and depth normalization, achieving superior performance over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: To improve image categorization and retrieval by leveraging graph structures and attention mechanisms to capture important features and relationships between images.

Method: Uses GAT-based autoencoder on graphs where nodes represent images and edges capture similarity. Creates context-aware latent representations and category representatives for comparison.

Result: The approach effectively categorizes query images by comparing their representatives to category representatives, then retrieves similar images within identified categories.

Conclusion: The representative-centric approach with GAT autoencoders demonstrates effectiveness for image categorization and retrieval tasks.
...
Method: READ adds two auxiliary objectives to contrastive learning: (1) token-level reconstruction where a frozen decoder reconstructs alternative captions from original caption embeddings, and (2) sentence-level alignment that explicitly aligns paraphrased sentences in embedding space.
...
Conclusion: The reconstruction and alignment objectives offer complementary benefits: reconstruction encourages capturing relationships between words within captions, while alignment ensures consistent representations for paraphrases with different wording, effectively enhancing compositional reasoning capabilities.
...
TL;DR: This paper introduces the first systematic benchmark for political deepfake detection using real-world examples from social media, revealing that current detectors perform poorly on authentic political deepfakes and are vulnerable to simple manipulations.
...
Method: Created the first systematic benchmark based on the Political Deepfakes Incident Database - a curated collection of real-world political deepfakes from social media since 2018, and evaluated state-of-the-art deepfake detectors from academia, government, and industry.

Result: Academic and government detectors performed relatively poorly. Paid detection tools achieved higher performance than free-access models, but all detectors struggled to generalize to authentic political deepfakes and were vulnerable to simple manipulations, especially in video domain.

Conclusion: There is an urgent need for politically contextualized deepfake detection frameworks to better safeguard the public in real-world settings, as current detectors are insufficient for handling authentic political deepfakes.
...
TL;DR: SHIELD is a training-free framework that mitigates object hallucinations in Large Vision-Language Models by addressing three key issues in visual encoders: statistical bias, inherent bias, and vulnerability.


<details>
  <summary>Details</summary>
Motivation: Object hallucination in LVLMs, where models produce plausible but inaccurate object descriptions, remains a significant challenge. Previous work focused on LLM components, but this paper identifies visual encoders as the primary source of hallucinations.

Method: SHIELD uses three strategies: re-weighting visual tokens to reduce statistical bias, introducing noise-derived tokens to counter inherent bias, and applying adversarial attacks with contrastive decoding to address vulnerability.

Result: SHIELD effectively mitigates object hallucinations across diverse benchmarks and LVLM families, and achieves strong performance on general LVLM benchmarks, demonstrating broad applicability.

Conclusion: SHIELD successfully addresses object hallucinations in LVLMs by targeting visual encoder issues, providing a training-free solution with wide applicability across different model families and benchmarks.
...
TL;DR: VisionSelector is a lightweight plug-and-play framework for compressing visual tokens in MLLMs using a learnable scorer with differentiable Top-K mechanism and curriculum annealing, achieving superior performance across various compression ratios.


<details>
  <summary>Details</summary>
Motivation: MLLMs face computational and memory bottlenecks from massive visual tokens in high-resolution images, and existing compression methods risk discarding critical information due to heuristic rules and attention biases.

Method: Proposes VisionSelector - a decoupled scorer module with differentiable Top-K mechanism and curriculum annealing strategy for end-to-end learnable token compression that adapts to various compression rates.

Result: Achieves 100% accuracy on MME with 30% retention budget, outperforms prior methods by 12.14% at 10% retention budget, doubles prefill speed, with only 12.85M trainable parameters.

Conclusion: VisionSelector provides an efficient and adaptive token selection framework that generalizes well across compression rates while preserving critical information and improving computational efficiency.
...
Method: Combines advanced neural networks (U-Net, EfficientNet, Transformer) with real-time optimization strategies including model pruning, quantization, and GPU acceleration. Supports flexible deployment on edge devices, servers, and cloud systems with PACS/EHR integration.

Result: Achieved state-of-the-art performance: >92% classification accuracy, >91% segmentation Dice scores, and <80ms inference times. Visual explanation tools (Grad-CAM, segmentation overlays) provided enhanced transparency and interpretability.

Conclusion: The framework can significantly accelerate diagnostic workflows, reduce clinician workload, and enable trustworthy AI integration in time-critical healthcare environments through high-performance real-time medical image analysis.
...
<details>
  <summary>Details</summary>
Motivation: Enable autonomous drone navigation without GPS or expensive sensors like LiDAR for indoor operations, addressing challenges in metric depth estimation and computational efficiency.

Method: Combines semantic segmentation with monocular depth estimation using adaptive scale factor algorithm for metric distance conversion. Uses knowledge distillation with SVM teacher to train lightweight U-Net student network for real-time segmentation. Also employs end-to-end learning for flight policies.

Result: Achieved 14.4 cm mean distance error, 100% success rate in 30 real-world and 100 digital-twin flight tests, increased surveillance distance, reduced mission time, and 87.5% autonomous mission success rate with end-to-end learning.

Conclusion: Successfully demonstrates practical vision-based drone navigation for structured environments, solving metric depth estimation and computational efficiency challenges for resource-constrained platforms.
...
TL;DR: MultiVerse is a challenging multi-turn conversation benchmark with 647 dialogues covering 484 tasks across 12 VLM evaluation benchmarks, showing that even top models like GPT-4o only achieve 50% success rate in complex multi-turn conversations.


<details>
  <summary>Details</summary>
Motivation: Real-world applications require multi-turn dialogues, but existing datasets only partially capture the breadth and depth of conversational scenarios encountered by users.

Method: Created MultiVerse benchmark with 647 dialogues averaging 4 turns each, derived from 12 popular VLM evaluation benchmarks. Used checklist-based evaluation with GPT-4o as automated evaluator across 37 key aspects.

Result: Evaluated 18 VLMs and found even strongest models achieve only 50% success rate in complex multi-turn conversations. Providing full dialogue context significantly enhances performance for smaller/weaker models.

Conclusion: MultiVerse serves as a comprehensive landscape for evaluating multi-turn interaction abilities in VLMs, highlighting the importance of in-context learning and revealing significant performance gaps in current models.
...
TL;DR: Using Retrieval Augmented Generation with Cypher query language to efficiently ground natural language in large 3D scene graphs, overcoming scalability limitations of traditional LLM context window methods.


<details>
  <summary>Details</summary>
Motivation: To enable robots to understand natural language by connecting it to their world representations, addressing the challenge that existing methods don't scale well to large or rich 3D scene graphs when using LLMs.

Method: Proposed using Retrieval Augmented Generation with a graph database encoding of 3DSG and providing Cypher query language as a tool for LLMs to retrieve relevant data, rather than encoding entire scene graphs in the context window.

Result: The approach scales significantly better to large, rich graphs on both local and cloud-based models, leading to large performance improvements in grounded language tasks while substantially reducing token count of scene graph content.

Conclusion: Using Cypher as an interface to 3D scene graphs is an effective solution for scalable natural language grounding in robotics applications.
...
TL;DR: UTAP is a universal adversarial attack method that uses fixed noise patterns to systematically degrade pathology foundation models' performance across diverse datasets and models, revealing critical vulnerabilities in AI systems for medical pathology.


<details>
  <summary>Details</summary>
Motivation: To reveal critical vulnerabilities in pathology foundation models and establish a high-standard benchmark for evaluating model robustness in medical AI applications.

Method: Optimized deep learning approach that generates fixed, weak noise patterns (UTAP) which, when added to pathology images, systematically disrupts feature representations of multiple foundation models.

Result: UTAP successfully degraded performance of various state-of-the-art pathology foundation models across multiple datasets, causing significant performance drops with visually imperceptible modifications, demonstrating both universality across datasets and transferability across different models.

Conclusion: UTAP constitutes a broad threat to pathology foundation models and establishes a critical benchmark for robustness evaluation, highlighting the need for advanced defense mechanisms and adversarial training to ensure safe AI deployment in pathology.
...
TL;DR: HYDRA introduces a hybrid knowledge distillation approach for spectral reconstruction from RGB images, achieving state-of-the-art performance with 18% accuracy improvement and faster inference.


<details>
  <summary>Details</summary>
Motivation: Previous multi-scale attention methods only work well for sparse spectra, while modern hyperspectral sensors have hundreds of channels, requiring better generalizable spectral reconstruction methods.

Method: Uses a Teacher model that encodes latent hyperspectral data and a Student model that learns mappings from natural images to the Teacher's encoded domain, with a novel training approach.

Result: Achieves SOTA performance across all metrics with 18% accuracy boost and faster inference times at various channel depths compared to current SOTA models.

Conclusion: HYDRA successfully addresses limitations of prior spectral reconstruction models and provides high-quality reconstruction for modern hyperspectral imaging applications.
...
TL;DR: MSSR is a dual-agent framework that improves spatial reasoning in VLMs by first extracting sufficient 3D information using expert models and then iteratively refining it to achieve minimality, addressing bottlenecks in 3D understanding and redundant information.


<details>
  <summary>Details</summary>
Motivation: Spatial reasoning remains challenging for VLMs due to inadequate 3D understanding from 2D-centric pre-training and reasoning failures from redundant 3D information.

Method: A dual-agent framework with Perception Agent that queries 3D scenes using perception toolbox (including novel SOG module) to extract sufficient information, and Reasoning Agent that iteratively refines information to achieve minimality by pruning redundancy and requesting missing details.
...
Conclusion: The explicit pursuit of both sufficiency and minimality in spatial reasoning effectively addresses fundamental bottlenecks, offering high-quality training data for future models.
...
TL;DR: SDPA++ is a self-supervised denoising framework for OCT images that uses only noisy images to generate pseudo-ground-truth and train denoising models through patch aggregation.


<details>
  <summary>Details</summary>
Motivation: Acquiring paired clean/noisy OCT datasets is challenging due to intrinsic speckle noise and clinical constraints, creating a need for self-supervised approaches that work with only noisy images.

Method: Generate pseudo-ground-truth images through self-fusion and self-supervised denoising, then use these as targets to train an ensemble of denoising models with patch-based strategy.

Result: Validated on real-world IEEE SPS VIP Cup dataset showing improvements in CNR, MSR, Texture Preservation, and Edge Preservation metrics.
...
TL;DR: The paper proposes DCCL, a domain-connecting contrastive learning method that improves domain generalization by enhancing intra-class connectivity across domains through data augmentation and model anchoring.


<details>
  <summary>Details</summary>
Motivation: Direct application of contrastive learning to domain generalization deteriorates performance due to lack of intra-class connectivity across domains, despite contrastive learning's class-separated representations theoretically being beneficial.

Method: DCCL introduces aggressive data augmentation and cross-domain positive samples to improve intra-class connectivity on the data side, and uses model anchoring with generative transformation loss on the model side to embed unseen test domains.
...
Conclusion: DCCL successfully addresses the intra-class connectivity problem in domain generalization through a novel contrastive learning paradigm that connects domains and produces more generalizable representations.
...
<details>
  <summary>Details</summary>
Motivation: To overcome the inefficiency of multi-step denoising in diffusion-based human motion prediction methods by enabling single-step generation.
...
TL;DR: A novel framework called SCENECOT that introduces grounded Chain-of-Thought reasoning for 3D scene understanding, achieving strong performance on complex 3D reasoning tasks.
...
Method: Introduces SCENECOT framework with grounded Chain-of-Thought reasoning that decouples complex tasks into simpler problems using multimodal expert modules, and creates SCENECOT-185K dataset with 185K high-quality reasoning instances.
...
TL;DR: IR-WM is an Implicit Residual World Model for autonomous driving that focuses on modeling scene changes rather than full reconstruction, using BEV representations and residual prediction to improve efficiency and accuracy.
...
Method: IR-WM establishes BEV representations of current state, uses previous timestep BEV features as temporal prior, predicts only residual changes conditioned on ego-vehicle actions and scene context, and applies alignment module to correct semantic/dynamic misalignments.

Result: Achieves top performance on nuScenes benchmark in both 4D occupancy forecasting and trajectory planning, with implicit future state generation substantially improving planning accuracy.

Conclusion: The implicit residual approach focusing on modeling world evolution rather than full scene reconstruction is effective for autonomous driving systems, demonstrating superior performance in forecasting and planning tasks.
...
TL;DR: UKANFormer is a semantic segmentation model that achieves high-precision coral reef mapping using noisy supervision from Allen Coral Atlas, outperforming conventional methods and producing more accurate predictions than the training labels themselves.


<details>
  <summary>Details</summary>
Motivation: Global coral reef mapping products like Allen Coral Atlas have limited spatial precision and semantic consistency, especially for fine-grained boundary delineation, creating a need for more accurate mapping methods.

Method: Proposed UKANFormer model based on UKAN architecture with Global-Local Transformer (GL-Trans) block in decoder to extract both global semantic structures and local boundary details from noisy supervision.

Result: Achieved coral-class IoU of 67.00% and pixel accuracy of 83.98%, outperforming conventional baselines under same noisy labels setting. Model predictions were visually and structurally more accurate than the noisy training labels.

Conclusion: Architectural design can mitigate label noise and support scalable mapping under imperfect supervision, challenging the notion that data quality directly limits model performance. Provides foundation for ecological monitoring where reliable labels are scarce.
...
TL;DR: This survey provides a unified framework for world models in embodied AI, proposing a three-axis taxonomy covering functionality, temporal modeling, and spatial representation, while systematizing data resources, metrics, and identifying key challenges in the field.


<details>
  <summary>Details</summary>
Motivation: Embodied AI requires agents that can perceive, act, and anticipate how actions reshape future world states, with world models serving as internal simulators that capture environment dynamics to support perception, prediction, and decision making.

Method: The authors formalize the problem setting and learning objectives, and propose a three-axis taxonomy: (1) Functionality (Decision-Coupled vs. General-Purpose), (2) Temporal Modeling (Sequential Simulation vs. Global Difference Prediction), (3) Spatial Representation (Global Latent Vector, Token Feature Sequence, Spatial Latent Grid, Decomposed Rendering).
...
<details>
  <summary>Details</summary>
Motivation: While inference-time scaling through search has transformed LLMs, similar gains in image generation have been difficult to achieve, with search strategies showing limited benefits in continuous diffusion models.
...
Conclusion: Model architecture, particularly discrete token spaces in autoregressive models, is critical for effective inference-time optimization in visual generation, not just model scale.
...
TL;DR: The paper introduces a dataset of SR artifacts with human-rated prominence scores and trains a regressor to detect prominent artifacts, aiming for prominence-aware evaluation rather than treating artifacts as uniform defects.


<details>
  <summary>Details</summary>
Motivation: Current SR models produce artifacts with varying perceptual impact, but existing methods treat artifacts as uniform binary defects rather than considering their prominence to human observers.
...
Conclusion: The dataset and code are released to facilitate prominence-aware evaluation and mitigation of SR artifacts, moving beyond uniform artifact treatment.
...
TL;DR: WaMaIR is a CNN-based image restoration framework that improves texture detail reconstruction using large receptive field perception, channel-aware modeling, and texture enhancement loss.
...
Method: Proposes three key components: Global Multiscale Wavelet Transform Convolutions (GMWTConvs) for large receptive field feature extraction, Mamba-Based Channel-Aware Module (MCAM) for capturing long-range dependencies in feature channels, and Multiscale Texture Enhancement Loss (MTELoss) for preserving detailed texture structures.
...
Conclusion: WaMaIR effectively addresses texture detail restoration challenges in image restoration through its novel large receptive field architecture and channel-aware modeling.
...
TL;DR: Region in Context is a novel framework for text-conditioned image editing that performs multilevel semantic alignment between vision and language, enabling precise and harmonized changes by understanding regions in their global image context.


<details>
  <summary>Details</summary>
Motivation: Most existing approaches treat image regions in isolation, relying solely on local cues without accounting for how each part contributes to the overall visual and semantic composition, resulting in inconsistent edits, unnatural transitions, or loss of coherence across the image.

Method: The framework introduces a dual-level guidance mechanism: regions are represented with full-image context and aligned with detailed region-level descriptions, while the entire image is simultaneously matched to a comprehensive scene-level description generated by a large vision-language model.

Result: Experiments show that it produces more coherent and instruction-aligned results compared to existing methods.

Conclusion: The proposed Region in Context framework enables precise and harmonized image editing by performing multilevel semantic alignment between vision and language, with regions understanding their role within the global image context.
...
<details>
  <summary>Details</summary>
Motivation: Existing MRG models rely heavily on LLMs with limited exploration of pre-trained vision foundation models or advanced fine-tuning techniques. Non-Transformer architectures like Mamba networks remain underexplored for medical report generation.
...
Result: Extensive experiments on three benchmark datasets fully validated the effectiveness of the proposed strategies for X-ray MRG.

Conclusion: The proposed EMRRG framework demonstrates strong performance in X-ray medical report generation by leveraging Mamba networks and parameter-efficient fine-tuning methods.
...
TL;DR: GS2POSE is a novel 6D object pose estimation method that uses Bundle Adjustment principles with 3D Gaussian Splatting (3DGS) to create a pose-differentiable rendering pipeline, achieving improved accuracy on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Current 6D pose estimation methods struggle with textureless objects and varying illumination conditions, as they rely on 2D-3D feature correspondences.

Method: The approach formulates pose regression using Bundle Adjustment principles, leverages Lie algebra to extend 3DGS for pose-differentiable rendering, and iteratively optimizes pose by comparing input and rendered images while updating color parameters for illumination adaptation.

Result: GS2POSE achieves accuracy improvements of 1.4%, 2.8% and 2.5% on T-LESS, LineMod-Occlusion and LineMod datasets respectively compared to previous models.

Conclusion: GS2POSE effectively addresses limitations in textureless object pose estimation and illumination variations through its novel pose-differentiable rendering approach based on 3DGS and Bundle Adjustment principles.
...
TL;DR: Training-free video understanding framework using pre-trained VLMs and machine learning for zero-shot structural analysis without end-to-end training.


<details>
  <summary>Details</summary>
Motivation: Current video understanding models require extensive task-specific training on annotated datasets, which is costly and lacks scalability. There's a need to translate the zero-shot reasoning capabilities of VLMs from static images to videos.

Method: Reframe video understanding as self-supervised spatio-temporal clustering. Transform video into semantic feature trajectory using frozen VLM encoder, apply Kernel Temporal Segmentation (KTS) for event segmentation, then use unsupervised density-based clustering to identify recurring scenes and themes.

Result: The framework automatically produces structured, multi-modal video summaries by selecting representative keyframes from discovered clusters and leveraging VLM's generative capabilities for textual descriptions.

Conclusion: Provides an effective, interpretable, and model-agnostic pathway for zero-shot automated structural analysis of video content without requiring end-to-end training.
...
TL;DR: LENS is a plug-and-play method that enables Multimodal Large Language Models (MLLMs) to perform pixel-level segmentation without finetuning, preserving the model's generalization capabilities.
...
Method: LENS attaches a lightweight, trainable head to a frozen MLLM, refines spatial cues from attention maps to extract keypoints, and generates point-wise features compatible with mask decoders.
...
Conclusion: The attachable design of LENS establishes an efficient paradigm for extending MLLMs without compromising their generalization, paving the way for truly multi-talented unified models.
...
Result: Achieves Intersection-over-Union (IoU) of 0.82 on Cityscapes dataset, demonstrating high accuracy with simple design.
...
TL;DR: PIF is a Personalized Image Filter that learns photographic styles from reference images using textual inversion on a pretrained diffusion model, enabling effective style transfer while preserving content.


<details>
  <summary>Details</summary>
Motivation: Previous methods fail to learn meaningful photographic concepts from references or preserve content image integrity. Photographic style involves understanding how photos are edited from their original appearance.

Method: Uses pretrained text-to-image diffusion model with textual inversion technique to learn photographic concepts from reference images by optimizing prompts for these concepts.

Result: PIF shows outstanding performance in extracting and transferring various kinds of photographic style while preserving content.

Conclusion: The proposed PIF method successfully addresses limitations of previous approaches by leveraging diffusion model generative prior and textual inversion for effective photographic style learning and transfer.
...
TL;DR: Created a comprehensive lychee dataset with 11,414 images (RGB and depth) for fruit detection and maturity classification, featuring multiple varieties and ripeness stages, with consistent annotations verified by multiple reviewers.
...
Method: Collected color (RGB) images under diverse conditions across multiple lychee varieties, applied data augmentation, used depth images, and implemented a multi-reviewer annotation process with three independent labelers and a fourth verifier to ensure consistency.

Result: Produced a dataset with 11,414 images (878 raw RGB, 8,780 augmented RGB, 1,756 depth) annotated with 9,658 label pairs for detection and maturity classification, and validated it using three representative deep learning models.

Conclusion: The dataset successfully addresses the gap in lychee research data, provides comprehensive annotations for detection and maturity classification, and is publicly available for academic use to advance vision-based harvesting robot development.
...
<details>
  <summary>Details</summary>
Motivation: Coral reefs are declining rapidly due to climate change, creating urgent need for scalable automated monitoring systems. Existing datasets are limited by size, geography, or coarse labels and are not machine learning ready.

Method: Aggregated imagery from 76 curated CoralNet sources and Al Wajh in Red Sea, totaling ~925,000 genus-level hard coral annotations with expert-verified labels mapped to WoRMS. Proposed two evaluation settings: within-source and cross-source benchmarks.

Result: Supervised within-source performance is promising but drops sharply across domains. Zero-shot models perform poorly across the board, especially for rare and visually similar genera. The dataset provides a challenging benchmark for domain generalization.

Conclusion: ReefNet offers a comprehensive benchmark to catalyze advances in domain generalization and fine-grained coral classification, with plans to release dataset, code, and models to advance global coral reef monitoring and conservation.
...
<details>
  <summary>Details</summary>
Motivation: Current moisture prediction methods for wood chips are either slow/destructive (oven drying) or inaccurate when dealing with variable source materials. There's a need for robust approaches that handle source variability effectively.

Method: Comprehensive analysis of five texture feature types from wood chip images, combined into a feature set. Proposed AdaptMoist domain adaptation method that transfers knowledge between different wood chip sources using texture features, with model saving criterion based on adjusted mutual information.
...
TL;DR: M2HVideo is a framework for generating photorealistic human videos from mannequin footage, addressing head-body misalignment and identity drift through pose-aware encoding and distribution alignment.
...
Method: Proposes M2HVideo framework with dynamic pose-aware head encoder, mirror loss in pixel space using DDIM-based denoising, and distribution-aware adapter for feature alignment to enhance temporal coherence.
...
Conclusion: M2HVideo effectively addresses key challenges in mannequin-to-human video generation, producing identity-controllable and photorealistic human videos from mannequin footage.
...
TL;DR: 2DGS-R improves 3D Gaussian Splatting by using hierarchical training to achieve both high-quality rendering and accurate geometric structures, with minimal storage and training time overhead.


<details>
  <summary>Details</summary>
Motivation: Current 3DGS methods struggle to balance high-quality rendering with accurate surface representation. 2DGS improves geometry but compromises rendering quality, showing the need for a method that optimizes both aspects effectively.

Method: Hierarchical training approach: 1) Train original 2D Gaussians with normal consistency regularization, 2) Select poorly-rendering 2D Gaussians and apply in-place cloning to enhance them, 3) Fine-tune the model with frozen opacity.

Result: Achieves high-quality rendering while preserving fine geometric structures, requiring only 1% more storage and minimal additional training time compared to original 2DGS.

Conclusion: 2DGS-R effectively balances efficiency with performance, leading to improvements in both visual fidelity and geometric reconstruction accuracy without significant overhead.
...
TL;DR: ArmFormer is a lightweight transformer-based semantic segmentation framework that achieves pixel-level weapon detection with high accuracy and computational efficiency suitable for edge devices.


<details>
  <summary>Details</summary>
Motivation: Traditional weapon detection systems provide only bounding box localizations, lacking fine-grained segmentation needed for comprehensive threat analysis. Existing segmentation models either sacrifice accuracy for speed or require excessive computational resources incompatible with edge deployment.
...
Result: Achieves state-of-the-art performance with 80.64% mIoU and 89.13% mFscore while maintaining real-time inference at 82.26 FPS. With only 4.886G FLOPs and 3.66M parameters, outperforms heavyweight models requiring up to 48x more computation.

Conclusion: ArmFormer establishes itself as the optimal solution for deployment on portable security cameras, surveillance drones, and embedded AI accelerators in distributed security infrastructure.
...
TL;DR: BARL is a semi-supervised medical image segmentation framework that achieves label-space and representation-space alignment through dual-branch collaboration, outperforming state-of-the-art methods on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current semi-supervised medical image segmentation methods focus only on label-space consistency while ignoring representation-space alignment, leading to suboptimal discriminative and spatially coherent feature learning.

Method: BARL uses bilateral alignment with two collaborative branches: Dual-Path Regularization and Progressively Cognitive Bias Correction for label-space alignment, plus region-level and lesion-instance matching for representation-space alignment.

Result: Extensive experiments on four public benchmarks and a proprietary CBCT dataset show BARL consistently surpasses state-of-the-art SSMIS methods.

Conclusion: BARL effectively addresses both label-space and representation-space alignment, demonstrating superior performance in semi-supervised medical image segmentation.
...
<details>
  <summary>Details</summary>
Motivation: Current memory bank methods suffer from inconsistent feature transformations and limited discriminative capacity, especially in capturing local geometric details and rotation invariance, particularly when registration fails.
...
Conclusion: The integration of registration and anomaly detection enables acquisition of features that are both robust to rotations and highly effective for anomaly detection.
...
TL;DR: This paper proposes a neuron-level analysis framework to study multimodal information processing in vision-language models (VLMs) by comparing artificial neurons with human brain activity, revealing shared representational mechanisms and brain-like hierarchical processing.


<details>
  <summary>Details</summary>
Motivation: Current understanding of parallels between ANNs and human brain processing is limited: unimodal ANN studies miss multimodal capabilities, and multimodal research focuses on high-level outputs rather than individual neurons.

Method: A novel neuron-level analysis framework combining fine-grained artificial neuron analysis with fMRI-based voxel encoding, applied to two architecturally distinct VLMs: CLIP and METER.

Result: Four key findings: (1) ANs predict BN activities across multiple functional networks; (2) Both show functional redundancy; (3) ANs exhibit polarity patterns paralleling BNs; (4) Different VLM architectures drive distinct BN patterns.
...
TL;DR: Class-N-Diff is a classification-induced diffusion model that integrates a classifier within a diffusion model to simultaneously generate and classify dermoscopic images, improving both generation quality and classification performance for medical applications.
...
Result: Generates more realistic and diverse dermoscopic images while the classifier demonstrates improved performance for downstream diagnostic tasks.
...
TL;DR: Edit-R1 is a post-training framework using policy optimization with DiffusionNFT and MLLM-based reward modeling to improve instruction-based image editing generalization beyond supervised training patterns.
...
Method: Uses Diffusion Negative-aware Finetuning (DiffusionNFT) for likelihood-free policy optimization, employs MLLM as training-free reward model with fine-grained feedback, and implements low-variance group filtering to reduce scoring noise.

Result: UniWorld-V2 achieves state-of-the-art results on ImgEdit (4.49) and GEdit-Bench (7.83) benchmarks, with framework being model-agnostic and delivering substantial gains across diverse base models.

Conclusion: The Edit-R1 framework effectively addresses overfitting in instruction-based image editing through policy optimization and MLLM-based rewards, demonstrating wide applicability and superior performance across multiple benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Aviation's non-CO2 effects, particularly contrails, significantly impact climate through radiative forcing. Validating contrail models requires linking observed contrails to source flights, which is challenging with satellite data due to limited resolution and contrail drift/deformation.

Method: Uses ground-based cameras that capture contrails shortly after formation when they remain thin and distinct. Introduces a modular framework with geometric representations, distance metrics, temporal smoothing, and probability-based assignment strategies.

Result: Developed a framework that can attribute contrails to source flights using ground camera data, providing a strong baseline for contrail-to-flight attribution research.

Conclusion: This work establishes an effective approach for contrail attribution using ground-based cameras and provides a modular framework for future research in linking contrails to their source flights.
...
TL;DR: This paper evaluates four transformer-based architectures for thermal weapon segmentation, showing significant improvements over traditional CNNs with SegFormer-b5 achieving the best performance (94.15% mIoU) and SegFormer-b0 providing the fastest inference (98.32 FPS).
...
Method: Adapted and evaluated four transformer architectures (SegFormer, DeepLabV3+, SegNeXt, Swin Transformer) on a custom thermal dataset of 9,711 images from real surveillance videos. Used MMSegmentation framework with standard augmentation for robust training and fair comparison.
...
Conclusion: Transformer architectures demonstrate robust generalization for weapon detection in low-light thermal environments, offering flexible accuracy-speed trade-offs suitable for diverse real-time security applications, significantly outperforming traditional CNN approaches.
...
TL;DR: This paper introduces Res-Bench, a benchmark for evaluating resolution robustness in Multimodal Large Language Models (MLLMs), focusing on performance stability across different input resolutions rather than just semantic accuracy.


<details>
  <summary>Details</summary>
Motivation: Current MLLM evaluations primarily assess semantic performance but overlook resolution robustness - whether performance remains stable across varying input resolutions, creating a critical gap in understanding model behavior.

Method: Created Res-Bench with 14,400 samples across 12 resolution levels and 6 capability dimensions. Designed novel evaluation framework with robustness metrics: Spearman's correlation for resolution-performance trends, and Absolute/Relative Continuous Error (ACE/RCE) for performance volatility. Conducted large-scale evaluation of leading MLLMs.

Result: The study provides comprehensive analysis including: (1) model-centric and task-centric robustness examination, (2) investigation of preprocessing strategies (padding and super-resolution), and (3) exploration of fine-tuning for stability enhancement.

Conclusion: Res-Bench addresses the critical gap in evaluating resolution robustness in MLLMs, providing a comprehensive framework and metrics to assess performance stability across varying input resolutions, which is essential for understanding and improving model behavior in real-world applications.
...
TL;DR: This review paper provides a comprehensive analysis of foundation models in medical image analysis, categorizing them by architecture, training strategies, and clinical applications, while addressing challenges and future directions.
...
Method: Systematic categorization of studies into vision-only and vision-language FMs based on architectural foundations, training strategies, and downstream clinical tasks, along with quantitative meta-analysis of temporal trends in dataset utilization and application domains.

Result: The review provides a structured analysis of FMs in medical image analysis, identifying trends in dataset usage and application domains, and critically discussing persistent challenges and emerging solutions.

Conclusion: The paper identifies key future research directions to enhance robustness, explainability, and clinical integration of foundation models, accelerating their translation into real-world medical practice.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion and flow models achieve high generative quality but are computationally expensive due to slow multi-step sampling. Existing distillation methods lack a unified theoretical foundation.
...
<details>
  <summary>Details</summary>
Motivation: Existing ADL recognition methods have limitations: sequence-based approaches lack spatial awareness and are noise-sensitive, while image-based approaches lose fine-grained temporal dynamics. Naive fusion methods fail to properly align these complementary representations.

Method: Proposes CARE framework with Sequence-Image Contrastive Alignment (SICA) that jointly optimizes representation learning via contrastive alignment and classification via cross-entropy. Integrates time-aware sequence encoding with spatially-informed image representations.
...
<details>
  <summary>Details</summary>
Motivation: Standard VSG approaches require labeled training data and process full videos offline, which is costly and limits online applications. The authors aim to perform VSG online without training.

Method: The method uses LMMs to predict steps from limited frames and incorporates Bayesian filtering with step transitions modeled via dependency matrices from LLMs and step progress estimation.

Result: BaGLM outperforms state-of-the-art training-based offline methods on three datasets, showing that online zero-shot VSG can achieve superior performance.

Conclusion: The work demonstrates that online VSG without training is feasible and effective using LMMs and Bayesian filtering, opening possibilities for real-time video understanding applications.
...
TL;DR: An empirical study investigating how different video feature representations affect temporal video grounding performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Research in temporal video grounding has focused on a limited selection of video representations, potentially leading to architectural overfitting. The study aims to explore the impact of diverse video features on model performance.

Method: Extracted features from three benchmarks (Charades-STA, ActivityNet-Captions, YouCookII) using various video encoders including CNN-based, temporal reasoning, and transformer architectures, then evaluated their impact on a classical grounding model.

Result: Significant performance differences were observed by simply changing the video encoder, revealing clear patterns and errors that indicate potential feature complementarity between different representation types.

Conclusion: Video feature selection significantly impacts temporal video grounding performance, and different feature types show complementary strengths that could be leveraged for improved models.
...
Method: Created a benchmark for measuring generalization to lower-resolution images, and trained iBOT on MillionAID with remote sensing-specific modifications.

Result: No consistent improvements were found from specialized pretrained models compared to general-purpose baselines at ViT-B scale.

Conclusion: Specialized remote sensing foundation models may not be necessary when working at smaller scales, as general-purpose models perform equally well.
...
TL;DR: ED-VTG is a two-stage method for video temporal grounding that uses multimodal LLMs to enrich language queries and then ground them with a lightweight decoder, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: To leverage multimodal LLMs for fine-grained video temporal grounding by addressing the challenge of directly grounding natural language queries in videos, which often lack sufficient context and details.

Method: Two-stage approach: 1) Transform language queries into enriched sentences with additional details and cues, 2) Use lightweight decoder to ground enriched queries with multiple-instance-learning training to select optimal query versions.

Result: State-of-the-art performance across various benchmarks in temporal video grounding and paragraph grounding, significantly outperforming previous LLM-based approaches and comparable to specialized models.

Conclusion: ED-VTG demonstrates that enriching queries before grounding with multimodal LLMs is effective for temporal video grounding, showing strong performance especially in zero-shot scenarios while maintaining advantages over specialized models.
...
TL;DR: W2R2 is a training framework that addresses 2D semantic bias in multimodal 3D grounding by disentangling 2D semantic and 3D spatial features, using dual-objective loss functions to improve 3D localization without changing inference architecture.


<details>
  <summary>Details</summary>
Motivation: Current multimodal 3D grounding models suffer from severe "2D semantic bias" - they over-rely on 2D image features for localization while largely disregarding 3D geometric inputs, leading to suboptimal fusion performance.
...
Conclusion: W2R2 effectively addresses 2D semantic bias through disentangled representation learning and targeted shortcut suppression, improving 3D grounding performance without architectural changes.
...
TL;DR: This paper presents a novel approach for generating synthetic fingerprint images using conditional StyleGAN2-ADA and StyleGAN3 architectures for live fingerprints and CycleGANs for spoof fingerprints, creating privacy-preserving datasets for biometric research.


<details>
  <summary>Details</summary>
Motivation: Large fingerprint datasets are time-consuming, expensive to collect, and require strict privacy measures. Synthetic fingerprint data can address privacy, cost, and accessibility issues in biometric data collection.

Method: Uses conditional StyleGAN2-ADA and StyleGAN3 to generate high-resolution synthetic live fingerprints conditioned on finger identities. Employs CycleGANs to translate these into realistic spoof fingerprints simulating various presentation attack materials.

Result: Created two synthetic datasets (DB2 and DB3) with 1,500 fingerprint images each. StyleGAN3 achieved FID as low as 5, TAR of 99.47% at 0.01% FAR. StyleGAN2-ADA achieved TAR of 98.67% at same FAR. No significant identity leakage detected, confirming strong privacy preservation.

Conclusion: The approach successfully generates high-quality synthetic fingerprint datasets that preserve privacy while maintaining biometric utility, enabling robust spoof detection system development without compromising privacy.
...
<details>
  <summary>Details</summary>
Motivation: Lung cancer is the leading cause of cancer mortality, with CT imaging being central to screening and treatment. Manual segmentation is variable and time-intensive, while deep learning faces barriers to clinical adoption. The study aims to enhance reproducibility, prognostic accuracy, and clinical trust through a clinician-in-the-loop approach.

Method: Used multi-center CT data from 999 patients across 12 public datasets with five DL models (3D Attention U-Net, ResUNet, VNet, ReconNet, SAM-Med3D). Assessed segmentation reproducibility using 497 radiomic features via statistical tests. Compared supervised and semi-supervised learning across 38 dimensionality reduction strategies and 24 classifiers. Six physicians qualitatively evaluated masks across seven clinical domains.
...
Conclusion: Integrating VNet with SSL yields accurate, reproducible, and clinically trusted CT-based lung cancer prognosis, highlighting a feasible path toward physician-centered AI translation in clinical practice.
...
TL;DR: The paper proposes a generalized selection method for class representations in person re-identification that improves performance beyond state-of-the-art by choosing better representations beyond just class centroids.


<details>
  <summary>Details</summary>
Motivation: Current person re-identification research focuses on feature extraction and objective functions, but selecting better class representatives is an underexplored area that can significantly improve performance. Prior methods using class centroids yield suboptimal results.

Method: A generalized selection method that chooses representations not limited to class centroids, allowing adjustment of the number of representations per class to meet specific application requirements. Applied on top of multiple re-identification embeddings.

Result: The approach substantially improves upon contemporary results in all cases, achieving better balance between accuracy and mean average precision beyond state-of-the-art performance.

Conclusion: The proposed generalized representation selection method effectively addresses limitations of prior centroid-based approaches and leads to significant improvements in person re-identification metrics.
...
TL;DR: V-Reason improves video reasoning in Large Multimodal Models by optimizing inference-time behavior using entropy-based control, achieving near-RL performance without training while reducing computational overhead.


<details>
  <summary>Details</summary>
Motivation: Current video reasoning methods using LMMs require expensive RL training and verbose chain-of-thought, causing high computational costs and limited control over the thinking process.

Method: Uses entropy of model outputs to guide micro-exploration/exploitation phases, then adapts LMM's value cache during inference via a small trainable controller with entropy-based objective, requiring no dataset supervision or RL.

Result: Achieves significant improvements over base models across video reasoning datasets, narrowing gap with RL-trained models to within 0.6% average accuracy while reducing output tokens by 58.6%.

Conclusion: Entropy-based inference-time optimization effectively improves video reasoning performance without expensive training, offering substantial efficiency benefits over RL-based approaches.
...
TL;DR: This paper analyzes the trade-off between general-purpose foundation vision models (Hiera) and specialized models (SAM2) for segmentation, quantifying the information-theoretic cost of specialization and revealing performance differences across various downstream tasks.
...
TL;DR: ProDAT enables progressive point cloud coding using density-aware tail-drop mechanism, achieving superior compression efficiency with single-model multi-bitrate decoding.
...
Conclusion: ProDAT successfully bridges the gap for progressive point cloud coding while maintaining superior compression performance.
...
TL;DR: FMCAF is a preprocessing architecture that enhances RGB and infrared fusion using frequency-domain filtering and cross-attention, achieving improved performance on multimodal object detection tasks without dataset-specific tuning.


<details>
  <summary>Details</summary>
Motivation: To improve multimodal object detection robustness by leveraging complementary cues from RGB and infrared sensors, addressing challenges in low-light and aerial scenarios.

Method: Combines frequency-domain filtering block (Freq-Filter) to suppress redundant spectral features with cross-attention-based fusion module (MCAF) for better intermodal feature sharing.
...
TL;DR: GSPlane enhances Gaussian Splatting by incorporating planar priors to improve geometric accuracy and mesh structure in 3D scene reconstruction, particularly for planar regions in man-made environments.


<details>
  <summary>Details</summary>
Motivation: Current Gaussian Splatting methods struggle with reconstructing planar regions smoothly and precisely, which is crucial for scene editing and physical simulations in man-made environments.

Method: Leverages segmentation and normal prediction models to extract planar priors, uses structured representations for planar Gaussian coordinates, introduces Dynamic Gaussian Re-classifier for robust training, and refines mesh layouts using optimized planar priors.

Result: Significantly improves geometric accuracy of extracted meshes across various baselines without sacrificing rendering quality, produces clean mesh connectivity with reduced vertices and faces.

Conclusion: GSPlane effectively addresses planar reconstruction challenges in Gaussian Splatting, enabling more accurate geometry and structured representations suitable for downstream applications like scene editing and object manipulation.
...
TL;DR: Proposes a novel optimization strategy for pre-trained diffusion models to enhance fidelity in low-light image restoration while preserving realism, addressing content fidelity loss through latent refinement and bidirectional interaction.


<details>
  <summary>Details</summary>
Motivation: Pre-trained diffusion-based methods sacrifice content fidelity for perceptual realism, especially in low-light scenarios where degraded information limits effective control. Two main causes are identified: lack of suitable conditional latent modeling and absence of bidirectional interaction between conditional and noisy latents.

Method: Introduces a latent refinement pipeline to recover spatial details lost during VAE encoding, incorporating generative priors. Enables dynamic bidirectional interaction between refined latent conditions and noisy latents during diffusion process. The approach is plug-and-play and integrates with existing diffusion networks.
...
Conclusion: The proposed optimization strategy effectively addresses fidelity loss in diffusion-based methods, providing more effective control for low-level vision tasks, particularly in challenging low-light scenarios.
...
<details>
  <summary>Details</summary>
Motivation: To develop imperceptible watermarks for privacy protection and content verification that work with standard consumer cameras without being visible to human observers.
...
Result: Successfully embeds 128 bits within 10-second video clips, providing sufficient capacity for essential metadata while maintaining human imperceptibility.

Conclusion: The approach enables practical invisible watermarking for consumer applications, balancing imperceptibility with detection capability using spectral modulation techniques.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for generating OOD samples using text-to-image diffusion models suffer from semantic instability and insufficient shift diversity, limiting their generalization to realistic OOD scenarios.

Method: GOOD uses off-the-shelf ID classifiers to guide diffusion sampling trajectories: (1) Image-level guidance reduces input likelihood via gradient of log partition, (2) Feature-level guidance uses k-NN distance in classifier's latent space to promote sampling in feature-sparse regions, plus a unified OOD score combining both discrepancies.
...
Conclusion: GOOD's dual-guidance design enables more controllable and diverse OOD sample generation, leading to improved OOD detection robustness.
...
<details>
  <summary>Details</summary>
Motivation: Articulated objects like laptops and drawers pose challenges for 3D reconstruction and pose estimation due to their multi-part geometries and variable joint configurations, which create structural diversity across different states.

Method: The approach uses a Kinematic-Aware VAE to encode geometry, joint angles, and part segmentation into a structured latent space, then employs two conditional diffusion models for regressing global pose/joint parameters and generating kinematic-aware latent codes from partial observations, with an iterative optimization module for bidirectional refinement.
...
Conclusion: KineDiff3D provides an effective unified solution for articulated object reconstruction and kinematic property estimation from single-view inputs, addressing the challenges posed by structural diversity in multi-part objects.
...
TL;DR: GACO-CAD is a two-stage post-training framework that improves geometric accuracy and modeling conciseness in generating editable CAD models from single images using multi-modal LLMs.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with accurately inferring 3D geometry from 2D images due to limited spatial reasoning capabilities, which hinders generating editable parametric CAD models from single images.

Method: Two-stage framework: 1) Supervised fine-tuning using depth and surface normal maps as dense geometric priors combined with RGB images, 2) Reinforcement learning with group length reward to promote compact parametric modeling sequences and dynamic weighting for training stability.

Result: Achieves state-of-the-art performance on DeepCAD and Fusion360 datasets, outperforming existing methods in code validity, geometric accuracy, and modeling conciseness under the same MLLM backbone.

Conclusion: GACO-CAD successfully addresses the spatial reasoning limitations of MLLMs for CAD generation by leveraging geometric priors and promoting concise modeling procedures, enabling more accurate and efficient single-view CAD reconstruction.
...
TL;DR: Face recognition systems are vulnerable to adversarial attacks, but face preprocessing techniques significantly impact attack success rates. Different face detection models can reduce attack success by up to 78%, while interpolation methods have minimal effect. A preprocessing-invariant method using input transformations improves transferability by up to 27%.


<details>
  <summary>Details</summary>
Motivation: Face preprocessing is a critical but often overlooked component in face recognition systems when evaluating adversarial attacks, especially in blackbox settings where the preprocessing pipeline is unknown.

Method: Investigated transferability of state-of-the-art adversarial attacks against different preprocessing techniques in blackbox settings. Proposed a preprocessing-invariant method using input transformations to improve attack transferability.

Result: Face detection model choice degrades attack success rate by up to 78%, while interpolation methods have minimal impact. The proposed preprocessing-invariant method improves attack transferability by up to 27%. Facial preprocessing degrades attack strength even in whitebox settings.

Conclusion: Preprocessing is crucial in face recognition systems and must be considered to improve adversarial generalization of facial adversarial examples.
...
<details>
  <summary>Details</summary>
Motivation: Masked autoregressive models have constrained acceleration potential due to the modeling complexity of spatially correlated visual tokens in a single step.

Method: Generation then Reconstruction (GtR) with two stages: structure generation for global semantic scaffolding, followed by detail reconstruction for completing remaining tokens. Also includes Frequency-Weighted Token Selection (FTS) to allocate more computation to detail tokens based on high-frequency energy.
...
Conclusion: GtR effectively accelerates masked autoregressive models through hierarchical sampling and intelligent token selection, demonstrating significant speed improvements without quality degradation.
...
TL;DR: This paper presents the first large-scale systematic evaluation of Out-of-Distribution (OoD) detection methods for automated plankton recognition, identifying ViM as the top-performing approach.


<details>
  <summary>Details</summary>
Motivation: Automated plankton recognition faces challenges from distribution shifts between training and test data due to plankton's complex morphologies, species diversity, and continuous discovery of novel species, leading to unpredictable errors during deployment.

Method: The study designed OoD benchmarks simulating various distribution shift scenarios using the DYB-PlanktonNet dataset and systematically evaluated twenty-two OoD detection methods.
...
Conclusion: This comprehensive evaluation provides a reliable reference for algorithm selection in automated plankton recognition and lays a foundation for future research in plankton OoD detection.
...
TL;DR: A framework for jointly learning photorealistic 3D head avatars with hand-face interactions, addressing challenges in pose tracking and hand-induced facial deformations through depth order loss, contact regularization, and PCA-based deformation modeling.
...
Method: Combines depth order loss with contact regularization for pose tracking, learns PCA basis for hand-induced facial deformations from interaction dataset, and incorporates contact loss inspired by physics-based simulation to reduce interpenetration artifacts.

Result: Evaluated on iPhone RGB(D) videos and synthetic dataset, the method captures better appearance and more accurate deforming geometry than state-of-the-art surface reconstruction methods.

Conclusion: The proposed framework successfully models photorealistic 3D head avatars with realistic hand-face interactions, overcoming challenges in pose tracking and deformation learning through novel regularization and PCA-based approaches.
...
Conclusion: HIDISC provides an efficient and effective framework for DG-GCD that avoids the computational costs of episodic training while achieving superior generalization across domains and categories through hyperbolic geometry and novel domain augmentation techniques.
...
Conclusion: The proposed prompt-aware token pruning method effectively balances task relevance and information diversity, enabling substantial computational savings while maintaining model performance.
...
TL;DR: Adapted Segment Anything Model (SAM) to track riverbank erosion in Bangladesh using historical Google Earth imagery, achieving 86.30% mIoU and 92.60% Dice score for detecting vanished settlements.


<details>
  <summary>Details</summary>
Motivation: Bangladesh's rivers cause massive destruction by swallowing villages and farmland annually, displacing thousands, but tracking this erosion has been extremely difficult for human analysts.

Method: Used color-channel analysis for rough land/water segmentation, then fine-tuned SAM's mask decoder to recognize riverbank erosion signatures using a new dataset of historical Google Earth imagery with manual annotations of disappeared settlements.
...
Conclusion: Provides the first annotated dataset of disappeared settlements, a specialized AI model for erosion detection, and a method for quantifying land loss, enabling policymakers to monitor erosion and protect vulnerable communities.
...
TL;DR: A round outcome prediction model for VALORANT using TimeSformer video recognition to analyze minimap information and tactical features, achieving 81% accuracy from middle phases onward.


<details>
  <summary>Details</summary>
Motivation: Existing esports match prediction research relies mainly on match logs and statistics, but VALORANT requires complex strategies that can be better captured through visual analysis of match footage.

Method: Based on TimeSformer video recognition model, incorporating detailed tactical features extracted from minimap information including character positions and in-game events, using dataset augmented with tactical event labels.

Result: Model achieved approximately 81% prediction accuracy, especially from middle phases of rounds onward, significantly outperforming models trained only on minimap information itself.

Conclusion: Leveraging tactical features from match footage is highly effective for predicting round outcomes in VALORANT, demonstrating the value of visual analysis over traditional statistical approaches.
...
Method: EndoCIL incorporates three components: 1) MDBR for diverse exemplar selection using distribution alignment, 2) PRCBL to handle inter-phase and intra-phase class imbalance with prior distributions, and 3) CFG to adjust classifier gradients and reduce new-class bias.
...
TL;DR: A DINOv2-based method with registers is proposed for face spoofing attack detection, focusing on extracting generalizable features and suppressing attention perturbations to identify minute differences between live and spoofed face images.


<details>
  <summary>Details</summary>
Motivation: Face recognition systems are vulnerable to spoofing attacks where malicious actors use photos to bypass authentication. Current systems need robust detection methods to identify these attacks before face recognition occurs.

Method: Uses DINOv2 with registers to extract generalizable features and suppress perturbations in the attention mechanism, enabling focused attention on essential minute features for distinguishing live vs spoofed faces.

Result: The method was tested on datasets from The 6th Face Anti-Spoofing Workshop and SiW dataset, demonstrating effectiveness in detecting spoofing attacks.

Conclusion: The proposed DINOv2-based approach with registers provides an effective solution for face spoofing attack detection by leveraging attention mechanisms to identify subtle differences between authentic and spoofed face images.
...
TL;DR: VisiPruner is a training-free pruning framework for MLLMs that reduces up to 99% of vision-related attention computations by leveraging insights from a three-stage cross-modal interaction process discovered through systematic analysis.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from significant computational overhead due to quadratic growth of attention computations with multimodal tokens, and existing pruning methods lack understanding of how MLLMs process and fuse multimodal information.
...
Conclusion: The discovered layer-wise processing dynamics provide actionable guidelines for training efficient MLLMs by aligning model architecture with intrinsic cross-modal interaction patterns.
...
TL;DR: The paper introduces QV-M^2 dataset for multi-moment retrieval (MMR) and proposes FlashMMR framework with multi-moment post-verification to improve moment boundary refinement and achieve robust multi-moment alignment.


<details>
  <summary>Details</summary>
Motivation: Existing moment retrieval methods focus on single-moment retrieval, but real-world applications often have queries corresponding to multiple relevant moments, making current datasets and methods insufficient for video temporal grounding.

Method: Proposed FlashMMR framework with Multi-moment Post-verification module using constrained temporal adjustment and verification to re-evaluate candidate segments, pruning low-confidence proposals and achieving robust multi-moment alignment.
...
TL;DR: A fairness-aware deepfake detection framework that combines temporal feature learning and demographic-aware data augmentation to address bias, improve transparency, and enhance detection reliability across demographic groups.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods suffer from bias, lack transparency, and fail to capture temporal information, leading to biased decisions and unreliable results across different demographic groups.

Method: Integrates temporal feature learning using sequence-based clustering for deepfake videos and concept extraction for interpretability. Introduces demographic-aware data augmentation that balances underrepresented groups and applies frequency-domain transformations to preserve deepfake artifacts.
...
Conclusion: The proposed framework effectively mitigates bias in deepfake detection while maintaining high accuracy, providing a more reliable and interpretable solution that works well across different demographic groups.
...
TL;DR: FineVision is a meticulously curated 24M-sample vision-language corpus that unifies 200+ sources through a semi-automated pipeline with human oversight, featuring rigorous de-duplication and decontamination, leading to superior model performance.


<details>
  <summary>Details</summary>
Motivation: The fragmented landscape of inconsistent and contaminated public datasets hampers vision-language model advancement, necessitating a unified, high-quality corpus.

Method: Semi-automated human-in-the-loop pipeline that unifies 200+ sources into 185 subsets, with automation for ingestion/mapping and human reviewers for auditing, plus rigorous de-duplication and decontamination against 66 benchmarks.

Result: Models trained on FineVision consistently outperform those trained on existing open mixtures across a broad evaluation suite, demonstrating benefits of scale, data hygiene, and balanced automation.

Conclusion: FineVision's curated corpus and tools accelerate data-centric VLM research, showing the importance of data quality through scale, hygiene, and human oversight.
...
TL;DR: Plug-and-Forecast (PnF) is a plug-and-play approach that enhances existing motion forecasting models using multimodal large language models (MLLMs) to improve generalization in autonomous driving without requiring fine-tuning.
...
Result: The approach achieves significant improvements in motion prediction performance on both Waymo Open Motion Dataset and nuScenes Dataset, demonstrating consistent performance gains across benchmarks.

Conclusion: PnF provides a practical, plug-and-play solution that enhances motion forecasting in autonomous driving by leveraging MLLMs' natural language capabilities for better scene understanding and adaptation, without requiring model fine-tuning.
...
TL;DR: SG-CLDFF is a saliency-guided cross-layer deep feature fusion framework that improves white blood cell segmentation and classification by integrating saliency preprocessing with multi-scale feature aggregation.


<details>
  <summary>Details</summary>
Motivation: Accurate WBC analysis is crucial for hematological diagnosis but challenging due to staining variability, complex backgrounds, and class imbalance in microscopic images.

Method: Uses saliency priors to highlight WBC regions, EfficientSwin-style backbone for multi-resolution features, ResNeXt-CC-inspired cross-layer fusion, multi-task training with segmentation and classification heads, and class-aware weighted losses with saliency-alignment regularization.

Result: Consistent improvements in IoU, F1, and classification accuracy on BCCD, LISC, ALL-IDB benchmarks compared to CNN and transformer baselines. Ablation study confirms contributions of saliency preprocessing and cross-layer fusion.

Conclusion: SG-CLDFF provides a practical and explainable framework for reliable automated WBC analysis in clinical workflows, with enhanced robustness and interpretability through saliency guidance and multi-scale feature fusion.
...
TL;DR: A surgical lighting system using YOLOv11 object detection to automatically track a blue marker and direct LED lighting via servomotors, achieving 96.7% mAP@50 accuracy.
...
Method: Uses YOLOv11 object detection to identify a blue marker placed above surgical site, then directs high-power LED lighting using two servomotors with tilt-pan brackets.

Result: YOLO model achieves 96.7% mAP@50 on validation set with annotated images simulating surgical scenes containing blue spherical marker.

Conclusion: The automated machine vision-based system reduces surgeon physical strain, improves illumination consistency, and supports better surgical outcomes.
...
TL;DR: Longer training in self-supervised learning can degrade performance on dense prediction tasks like semantic segmentation, a phenomenon called Self-supervised Dense Degradation (SDD). The paper introduces DSE metric for model selection and regularization to address this issue.


<details>
  <summary>Details</summary>
Motivation: To address the counterintuitive phenomenon where longer self-supervised learning training impairs dense prediction performance, and to solve the challenge of evaluating dense performance without annotations during training.

Method: Proposes Dense representation Structure Estimator (DSE) with class-relevance and effective dimensionality measures, plus model selection strategy and DSE-based regularization method.

Result: Experiments on 16 SSL methods across 4 benchmarks show model selection improves mIoU by 3.0% on average with negligible cost, and DSE regularization consistently mitigates dense degradation.

Conclusion: SDD is a consistent phenomenon across SSL methods, and the proposed DSE metric and methods effectively address it through better model selection and regularization.
...
TL;DR: LongInsightBench is the first benchmark for evaluating long video understanding across visual, audio, and text modalities, focusing on human language, viewpoints, actions, and contextual elements.


<details>
  <summary>Details</summary>
Motivation: To assess models' ability to understand long videos with rich multimodal content, particularly in scenarios requiring temporal localization and long-range causal inference.

Method: Created a benchmark with 1,000 long-duration, information-dense videos from FineVideo dataset, featuring six challenging task scenarios (Intra-Event and Inter-Event Tasks) and implemented a three-step semi-automated quality assurance pipeline.

Result: Experimental results show omni-modal models struggle with precise temporal localization and long-range causal inference tasks, revealing information loss and processing bias in multi-modal fusion.

Conclusion: Current omni-modal models face significant challenges in long video understanding tasks, highlighting the need for improved temporal reasoning and multimodal fusion capabilities.
...
TL;DR: CausalMamba is a scalable framework that overcomes limitations in fMRI-based causal inference by decomposing the problem into BOLD deconvolution and causal graph inference, achieving superior accuracy and revealing hidden neural dynamics.
...
Result: On simulated data, achieves 37% higher accuracy than DCM. On real task fMRI data, recovers well-established neural pathways with 88% fidelity, while conventional approaches fail in over 99% of subjects. Reveals strategic shifts in brain's primary causal hub depending on stimulus.
...
TL;DR: Large adversarial clothes can bypass existing defense methods against adversarial patches, revealing vulnerabilities in current adversarial defense approaches.


<details>
  <summary>Details</summary>
Motivation: To test the robustness of existing adversarial defense methods against large adversarial clothes, which provide a more natural and challenging test case than traditional adversarial patches.

Method: Evaluated various defense methods against adversarial clothes with large coverage over the human body, both in digital and physical worlds, and crafted a single set of clothes that could break multiple defense methods on Faster R-CNN.

Result: All defense methods performed poorly against adversarial clothes. A single set of clothes achieved 96.06% ASR against undefended detector and over 64.84% ASRs against nine defended models in the physical world.

Conclusion: Existing adversarial defense methods have common vulnerabilities against adversarial clothes, highlighting the need for more robust defense strategies against large-scale physical attacks.
...
TL;DR: CharDiff is a diffusion-based framework that uses character-level guidance to restore and recognize severely degraded license plate images, achieving superior performance over baseline models.


<details>
  <summary>Details</summary>
Motivation: License plate image restoration is important not just for LPR systems but also for increasing evidential value, enhancing visual clarity, and facilitating further utilization of license plate images.

Method: Uses diffusion-based framework with character-level guidance, leveraging fine-grained character priors from segmentation and OCR modules. Incorporates CHARM module for region-wise masking to ensure character guidance is restricted to individual character regions.
...
<details>
  <summary>Details</summary>
Motivation: To address the emerging challenge of detailed and explainable Image Quality Assessment (IQA) by moving beyond scalar quality prediction to more interpretable, human-aligned evaluation paradigms.

Method: Proposed iDETEX - a unified multimodal large language model that simultaneously performs three key tasks: quality grounding, perception, and description. Used task-specific offline augmentation modules, data mixing strategy, and online enhancement strategies to exploit multi-sourced supervision.
...
Conclusion: iDETEX demonstrates effectiveness and robustness in delivering accurate and interpretable quality assessments, advancing the field of detailed and explainable IQA.
...
TL;DR: A post-processing Open-set Recognition method that measures agreement between feature-based NCM distribution and softmax probabilities to identify unknown classes without retraining existing models.


<details>
  <summary>Details</summary>
Motivation: Current OSR methods require retraining pre-trained models, and existing approaches perform inconsistently across different datasets.

Method: Proposes measuring agreement between NCM-based probability distribution (from feature space) and softmax probabilities (from logit space) to detect unknown samples.

Result: Achieves AUROC of 93.41 and 95.35 on African and Swedish animal datasets, ranking top three consistently across both datasets.

Conclusion: The method provides consistent OSR performance across datasets without requiring model retraining, outperforming state-of-the-art methods that excel on only single datasets.
...
TL;DR: Semantic-E2VID is an event-to-video reconstruction framework that incorporates semantic information from vision foundation models to enhance video reconstruction quality by addressing the semantic gap in event camera data.


<details>
  <summary>Details</summary>
Motivation: Event cameras capture only intensity changes, missing static objects and semantic information, which limits the quality of event-to-video reconstruction. Existing methods overlook semantic information that is crucial for high-quality video reconstruction.

Method: Proposes cross-modal feature alignment to transfer semantics from Segment Anything Model to event encoder, semantic-aware feature fusion to integrate learned semantics, and semantic perceptual supervision using SAM-generated labels.

Result: Significantly enhances frame quality and outperforms state-of-the-art E2V methods across multiple benchmarks.

Conclusion: Incorporating semantic information from vision foundation models effectively bridges the semantic gap in event-to-video reconstruction, leading to improved reconstruction quality.
...
TL;DR: M2H is a multi-task learning framework for semantic segmentation, depth, edge, and surface normal estimation from monocular images, using a Window-Based Cross-Task Attention Module for structured feature exchange while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: To enable real-time spatial perception on edge devices by developing efficient multi-task models that leverage complementary task information while minimizing computational overhead.

Method: Uses a lightweight ViT-based DINOv2 backbone with a novel Window-Based Cross-Task Attention Module that enables structured feature exchange while preserving task-specific details.

Result: Outperforms state-of-the-art multi-task models on NYUDv2, surpasses single-task depth and semantic baselines on Hypersim, achieves superior performance on Cityscapes, and maintains computational efficiency on laptop hardware.

Conclusion: M2H provides an effective foundation for monocular spatial perception systems supporting 3D scene graph construction in dynamic environments, with practical validation on real-world data.
...
TL;DR: A training-free method for Video-LLMs that enables efficient streaming video processing by selecting important visual tokens, using recurrent processing for temporal coherence, and caption-based QA for lightweight responses.
...
Method: Uses three key concepts: 1) LLM-informed selection of visual tokens to discard ~95% unimportant tokens, 2) Recurrent processing of past selected tokens for temporal coherence, 3) Caption-based question answering for lightweight responses.

Result: Achieves state-of-the-art performance on streaming video benchmarks with minimal performance loss while significantly improving efficiency.

Conclusion: The proposed method effectively balances efficiency and effectiveness for streaming video understanding, making Video-LLMs practical for real-time applications without requiring retraining.
...
TL;DR: Synthetic facial recognition datasets can achieve comparable or better accuracy than real datasets while preserving privacy, with best-performing synthetic datasets reaching 95.67% accuracy and surpassing established real datasets.


<details>
  <summary>Details</summary>
Motivation: Real facial recognition datasets raise ethical concerns due to collection without consent, leading to dataset retractions and legal liabilities under GDPR. Synthetic data offers a privacy-preserving alternative but lacks comprehensive empirical validation.

Method: Systematic literature review of 25 synthetic facial recognition datasets (2018-2025) combined with experimental validation examining 7 key privacy requirements. Experiments involved over 10 million synthetic samples and comparison on 5 standard benchmarks.

Result: Best-performing synthetic datasets (VariFace 95.67%, VIGFace 94.91%) surpassed CASIA-WebFace (94.70%). Public alternatives Vec2Face (93.52%) and CemiFace (93.22%) performed well. Synthetic data maintains intra-class variability and identity separability while offering unprecedented bias mitigation control.

Conclusion: Synthetic facial data is scientifically viable and ethically imperative for facial recognition research, providing comparable performance to real datasets while addressing privacy concerns and enabling bias mitigation.
...
Method: Integrates multiple facial expression features through attention-based feature fusion and uses adaptive class balancing that dynamically adjusts training sample contributions based on class distribution and classification difficulty.
...
Conclusion: The proposed method effectively addresses limitations of existing approaches by leveraging multiple facial expressions and handling class imbalance, providing a reliable solution for PD severity diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: Previous weakly-supervised affordance grounding methods only transfer knowledge one-way from exocentric to egocentric images, limiting applicability in complex scenarios where object interaction regions may be occluded.
...
Conclusion: The bidirectional closed-loop framework enables more robust affordance grounding by enhancing knowledge transfer between different viewpoints.
...
TL;DR: A vision-based monitoring system using YOLOv11 and BoT-SORT to automatically detect and track horses and people in stables, distinguishing five event types while accounting for camera blind spots.


<details>
  <summary>Details</summary>
Motivation: Current monitoring of stalled horses is labor-intensive and time-consuming, requiring automated solutions for early detection of health and welfare issues.

Method: Uses object detection (YOLOv11) and multi-object tracking (BoT-SORT) to detect/track horses and people, infers event states from object trajectories and spatial relations, and employs foundation models (CLIP and GroundingDINO) for dataset annotation.

Result: Qualitative evaluation showed reliable performance for horse-related events but limitations in detecting people due to data scarcity. The system successfully distinguishes five event types and handles camera blind spots.

Conclusion: Provides a foundation for real-time behavioral monitoring in equine facilities with implications for animal welfare and stable management, though needs improvement in person detection capabilities.
...
TL;DR: DeepDetect is a dense keypoint detector that combines classical detector outputs with deep learning to achieve high density, repeatability, and matching performance across challenging conditions.
...
Method: Fuse outputs from 7 keypoint and 2 edge detectors to create ground-truth masks, then train a lightweight ESPNet model using these masks as labels to produce dense keypoints that focus semantically on images.

Result: On Oxford Affine Covariant Regions dataset: achieves 0.5143 average keypoint density, 0.9582 average repeatability, and 59,003 correct matches - surpassing other detectors in all metrics.

Conclusion: DeepDetect successfully unifies classical detector strengths with deep learning to create an intelligent, dense keypoint detector that performs well across diverse and visually degraded conditions.
...
<details>
  <summary>Details</summary>
Motivation: To create a resource-efficient compressed-domain front end for computer vision tasks by leveraging existing motion vector data from video compression.

Method: Using AV1 motion vectors to generate dense sub-pixel correspondences and short tracks filtered by cosine consistency, operating directly in the compressed domain.

Result: On short videos, performs comparably to sequential SIFT with far less CPU usage, yields denser matches with competitive pairwise geometry. In a 117-frame demo: registered all images, reconstructed 0.46-0.62M points at 0.51-0.53px reprojection error.
...
TL;DR: Proposes HQ-NightRain benchmark and CST-Net for nighttime image deraining, using color space transformation and illumination guidance to handle complex rain effects in low-light conditions.


<details>
  <summary>Details</summary>
Motivation: Nighttime image deraining is challenging due to complex rain-illumination coupling effects and lack of high-quality datasets that accurately represent these nighttime scenarios.

Method: Developed CST-Net with learnable color space converter to facilitate rain removal in Y channel where rain is more pronounced, and introduced implicit illumination guidance to improve robustness in complex nighttime scenarios.

Result: Extensive experiments demonstrate the value of the HQ-NightRain dataset and the effectiveness of the proposed CST-Net method for nighttime image deraining.

Conclusion: The proposed HQ-NightRain benchmark and CST-Net method provide effective solutions for nighttime image deraining, addressing the challenges of rain-illumination coupling in low-light conditions through color space transformation and illumination guidance.
...
TL;DR: Sparse-view 3D Gaussian Splatting suffers from overfitting to training views. The paper shows initialization is the key factor, and proposes three methods to improve SfM-based initialization: frequency-aware SfM, 3DGS self-initialization, and point-cloud regularization.


<details>
  <summary>Details</summary>
Motivation: Sparse-view 3D Gaussian Splatting often overfits to training views, causing blurring in novel views. Previous approaches either enhance initialization or add training-time constraints, but the authors found initialization is the decisive factor that determines performance limits.

Method: Three initialization improvements: (1) frequency-aware SfM using low-frequency view augmentation and relaxed multi-view correspondences for better low-texture coverage; (2) 3DGS self-initialization that lifts photometric supervision into additional points; (3) point-cloud regularization with geometric/visibility priors for multi-view consistency and uniform coverage.

Result: Experiments on LLFF and Mip-NeRF360 datasets show consistent gains in sparse-view settings, establishing the approach as a stronger initialization strategy.

Conclusion: Initialization is the decisive factor in sparse-view 3DGS performance. The proposed three-pronged initialization strategy effectively addresses SfM's limitations in sparse-view scenarios and achieves superior results.
...
TL;DR: SparseWorld is a novel 4D occupancy world model that uses sparse dynamic queries for flexible and adaptive perception, achieving state-of-the-art performance in perception, forecasting, and planning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing occupancy world models rely on static embeddings/grids that limit perception flexibility and exhibit misalignment with dynamic real scenarios due to in-place classification over grids.

Method: Uses Range-Adaptive Perception module with learnable queries modulated by ego vehicle states and temporal-spatial associations, State-Conditioned Forecasting with regression-guided formulation instead of classification, and Temporal-Aware Self-Scheduling training strategy.

Result: Achieves state-of-the-art performance across perception, forecasting, and planning tasks, with advantages in flexibility, adaptability, and efficiency validated through extensive experiments and visualizations.

Conclusion: SparseWorld effectively addresses limitations of static occupancy models by introducing dynamic queries and regression-based forecasting, providing a more flexible and adaptive solution for 4D environment modeling.
...
<details>
  <summary>Details</summary>
Motivation: Salient object detection can achieve near-supervised accuracy without pixel-level labels if reliable pseudo-masks are available. Current prototype-based methods underutilize global consistency from optimal transport when prototype quality is weak, and boundary/interior pixels follow different geometric patterns.

Method: POTNet adapts Prototypical Optimal Transport with an entropy-guided dual-clustering head: spectral clustering for high-entropy pixels, k-means for low-entropy pixels, and optimal transport alignment between the two prototype sets. This generates sharp, part-aware pseudo-masks in one forward pass without handcrafted priors.
...
Conclusion: The proposed split-fuse-transport design with entropy-guided dual clustering and optimal transport alignment enables unsupervised salient object detection to achieve performance close to fully supervised models, demonstrating that reliable pseudo-masks can substitute for pixel-level labels.
...
TL;DR: A rubric-guided pseudo-labeling framework for zero-shot video summarization that uses ground-truth annotations to create structured scoring rubrics, enabling LLMs to evaluate scenes with contextual prompting for better narrative coherence.


<details>
  <summary>Details</summary>
Motivation: Existing supervised methods have high labeling costs and limited generalization, while unsupervised approaches fail to capture high-level semantics. Zero-shot methods are sensitive to prompt templates and dataset-specific normalization.

Method: Transform ground-truth annotations into pseudo labels aggregated into dataset-adaptive scoring rubrics. Use contextual prompting where first/last segments are scored by descriptions alone, while intermediate ones include adjacent scene summaries to assess narrative progression.

Result: Achieves F1 scores of 57.58 on SumMe and 63.05 on TVSum, surpassing unsupervised and prior zero-shot baselines while approaching supervised performance.
...
Method: Optimized four pillars: data processing, model architecture, training strategy, and infrastructure. Used data preprocessing, video compression, parameter scaling, curriculum-based pretraining, and alignment-focused post-training.

Result: MUG-V 10B model matches state-of-the-art video generators overall and surpasses leading open-source baselines on e-commerce video generation tasks in human evaluations. Achieved significant efficiency gains and performance improvements.

Conclusion: Successfully developed and open-sourced a complete large-scale video generation training stack including model weights, Megatron-Core-based training code, and inference pipelines, achieving high training efficiency and near-linear multi-node scaling.
...
TL;DR: MambaX-Net is a semi-supervised 3D segmentation model for longitudinal prostate cancer surveillance that leverages temporal MRI data and previous segmentation masks to improve prostate zone segmentation with limited expert annotations.


<details>
  <summary>Details</summary>
Motivation: Active Surveillance for prostate cancer requires accurate longitudinal monitoring, but existing deep-learning models are unsuitable due to being trained on single-time-point data and requiring extensive expert annotations, which are scarce in longitudinal settings.

Method: Proposed MambaX-Net with two key components: Mamba-enhanced Cross-Attention Module for capturing temporal evolution and long-range dependencies, and Shape Extractor Module for encoding previous segmentation masks into anatomical representations. Also uses semi-supervised self-training with pseudo-labels from pre-trained nnU-Net.

Result: MambaX-Net significantly outperforms state-of-the-art U-Net and Transformer-based models, achieving superior prostate zone segmentation even when trained on limited and noisy longitudinal data.

Conclusion: The proposed architecture effectively addresses challenges in longitudinal Active Surveillance by leveraging temporal information and semi-supervised learning, enabling accurate prostate segmentation without extensive expert annotations.
...
TL;DR: WP-CrackNet is a weakly-supervised road crack detection method that uses only image-level labels for pixel-wise detection, achieving comparable performance to supervised methods through adversarial learning between classifier, reconstructor, and detector components.


<details>
  <summary>Details</summary>
Motivation: To reduce reliance on costly pixel-level annotations for road crack detection in smart city infrastructure maintenance by developing a weakly-supervised approach that requires only image-level labels.

Method: Integrates three components: classifier generating CAMs, reconstructor measuring feature inferability, and detector producing pixel-wise results. Uses adversarial learning between classifier and reconstructor, with detector learning from pseudo labels from post-processed CAMs. Includes path-aware attention module (PAAM) and center-enhanced CAM consistency module (CECCM) for improved performance.
...
Conclusion: WP-CrackNet demonstrates that weakly-supervised learning with only image-level labels can achieve effective pixel-wise road crack detection, making infrastructure inspection more scalable and cost-effective.
...
TL;DR: PAGE-4D extends VGGT for dynamic scenes, enabling camera pose estimation, depth prediction, and point cloud reconstruction without post-processing by using a dynamics-aware aggregator to resolve task conflicts.


<details>
  <summary>Details</summary>
Motivation: Existing 3D feed-forward models like VGGT struggle with dynamic elements in real-world scenarios because they're trained on static datasets, limiting their performance with moving humans or deformable objects.

Method: Proposes a dynamics-aware aggregator that disentangles static and dynamic information by predicting a dynamics-aware mask, suppressing motion for pose estimation while amplifying it for geometry reconstruction.
...
Conclusion: The dynamics-aware approach effectively resolves the inherent conflict between camera pose estimation and geometry reconstruction in dynamic scenes, enabling robust 4D reconstruction without post-processing.
...
TL;DR: This paper introduces UCIS4K, the first underwater camouflaged instance segmentation dataset with 3,953 images, and proposes UCIS-SAM network with three modules to address underwater vision challenges like color distortion and low contrast.


<details>
  <summary>Details</summary>
Motivation: Underwater vision tasks face challenges due to degraded environments with color distortion, low contrast, and blurring. Traditional methods trained on terrestrial datasets perform poorly in underwater scenes for camouflaged instance segmentation.

Method: Proposed UCIS-SAM network based on Segment Anything Model with three key modules: Channel Balance Optimization Module for underwater feature learning, Frequency Domain True Integration Module to reduce camouflage interference, and Multi-scale Feature Frequency Aggregation Module for boundary enhancement.

Result: Extensive experiments on UCIS4K dataset and public benchmarks show that UCIS-SAM outperforms state-of-the-art approaches in underwater camouflaged instance segmentation.

Conclusion: The proposed UCIS-SAM network effectively addresses underwater camouflaged instance segmentation challenges through specialized modules for underwater feature enhancement, camouflage pattern reduction, and boundary strengthening.
...
TL;DR: ShapeCraft is a multi-agent framework that generates structured, interactive 3D assets from natural language using Graph-based Procedural Shape (GPS) representation, outperforming existing LLM-based methods.
...
Result: Qualitative and quantitative experiments show superior performance in generating geometrically accurate and semantically rich 3D assets compared to existing LLM-based agents.

Conclusion: ShapeCraft demonstrates versatility through animated and user-customized editing examples, highlighting potential for broader interactive applications in 3D generation.
...
TL;DR: Proposes a machine learning framework using UAV-scanned point clouds and BIM synthetic data for automated segmentation of 3D infrastructure models, reducing manual labeling needs.


<details>
  <summary>Details</summary>
Motivation: To overcome time-consuming and error-prone manual labeling in segmenting structural components from UAV-captured 3D infrastructure models.

Method: Combines real-world UAV-scanned point clouds with synthetic data from Building Information Modeling (BIM) using a machine learning framework for automated segmentation.

Result: High accuracy in identifying rails and crossties on railroad track dataset; reduced training time while maintaining reasonable segmentation accuracy using smaller datasets supplemented with BIM data.

Conclusion: The automated approach improves precision and efficiency of 3D infrastructure model segmentation and advances integration of UAV and BIM technologies for structural health monitoring.
...
TL;DR: Dinomaly2 is a unified framework for unsupervised anomaly detection that bridges performance gaps in multi-class models while extending across diverse data modalities and task settings using a minimalistic reconstruction-based approach.


<details>
  <summary>Details</summary>
Motivation: Existing multi-class anomaly detection models significantly underperform specialized single-class counterparts, and the field has fragmented into scenario-specific methods, creating deployment barriers that highlight the need for a unified solution.
...
Result: Achieves unprecedented 99.9% and 99.3% image-level AUROC on MVTec-AD and VisA respectively for multi-class models, state-of-the-art performance in multi-view/multi-modal inspection, and surpasses previous full-shot models using only 8 normal examples per class (98.7% and 97.4% I-AUROC on MVTec-AD and VisA).
...
TL;DR: CaMiT dataset tracks car model evolution over time (2007-2023) to study AI adaptation to changing visual environments, proposing time-incremental learning methods for improved temporal robustness.
...
Method: Created CaMiT dataset with 787K labeled and 5.1M unlabeled car samples; proposed time-incremental classification with two strategies: updating backbone or only final layer; explored time-aware image generation.
...
TL;DR: DINO-CV is a self-supervised segmentation framework using Airborne LiDAR-derived DEMs to automatically map dry-stone walls, overcoming vegetation occlusion and data scarcity challenges.


<details>
  <summary>Details</summary>
Motivation: Dry-stone walls have heritage and environmental value but remain unmapped due to inaccessibility and high manual mapping costs. Visual occlusion by vegetation and limited labeled data hinder automated mapping.

Method: Uses high-resolution Airborne LiDAR-derived DEMs to capture terrain structures beneath vegetation. Introduces self-supervised cross-view pre-training with knowledge distillation across multiple DEM derivatives, supporting various vision backbones including ResNet, Wide ResNet, and Vision Transformers.

Result: Achieved 68.6% mIoU on test areas and maintained 63.8% mIoU when fine-tuned with only 10% labeled data. Successfully identified dense colonial dry-stone walls in Budj Bim UNESCO World Heritage site.
...
TL;DR: This paper compares two frugal federated learning approaches for violence detection: zero-shot/fine-tuned vision-language models (VLMs) and personalized 3D CNNs, evaluating accuracy, calibration, and energy usage in non-IID settings.


<details>
  <summary>Details</summary>
Motivation: To develop energy-efficient and environmentally sustainable AI solutions for violence detection in video surveillance, addressing the need for responsible and resource-aware deployment.

Method: Compared two strategies: (1) zero-shot and federated fine-tuning of vision-language models (LLaVA-7B) using LoRA, and (2) personalized training of a compact 65.8M parameter 3D CNN. Evaluated under realistic non-IID settings with metrics including accuracy, calibration, energy usage, and CO2 emissions.
...
TL;DR: 4DSegStreamer is a novel framework for real-time 4D panoptic segmentation using a Dual-Thread System that enables efficient streaming processing and can be integrated into existing 3D/4D segmentation methods.


<details>
  <summary>Details</summary>
Motivation: Real-time, fine-grained perception is critical for dynamic environments like crowd evacuation and autonomous driving, requiring processing within constrained time budgets.
...
Result: Demonstrates superior robustness under high FPS conditions, effectively predicts dynamic objects in complex scenes across indoor (HOI4D) and outdoor (SemanticKITTI, nuScenes) datasets.

Conclusion: 4DSegStreamer provides an effective framework for real-time 4D panoptic segmentation that can be seamlessly integrated into existing methods and shows strong performance in dynamic environments.
...
<details>
  <summary>Details</summary>
Motivation: Current image editing models focus on completing editing instructions but overlook accompanying physical effects like shadows, reflections, and object interactions, which are crucial for realism.

Method: Proposed PICABench benchmark with eight physical sub-dimensions, PICAEval evaluation protocol using VLM-as-a-judge with human annotations, and PICA-100K dataset from video learning.

Result: Evaluation of mainstream models shows physical realism remains challenging with significant room for improvement, as models fail to maintain physical consistency.

Conclusion: Physical realism is a key challenge in image editing, and the proposed benchmark and solutions provide foundation for moving from basic content editing to physically consistent realism.
...
<details>
  <summary>Details</summary>
Motivation: Existing fine-tuning methods for medical image segmentation have limitations: insufficient high-level feature representation and disruption of pretrained weight structural integrity during fine-tuning.

Method: Proposes IC-MoE with: 1) Three expert types (basic, semantic, adaptive) with pixel probability adaptive voting for expert selection/fusion, and 2) Semantic-guided contrastive learning to address weak supervision in contrastive learning.
...
TL;DR: Proposes Bi-IRRA framework for multilingual text-to-image person retrieval using bidirectional implicit relation reasoning and multi-dimensional global alignment to address modality heterogeneity across languages.


<details>
  <summary>Details</summary>
Motivation: Address limitations in current TIPR methods: global methods overlook fine-grained differences, local methods need prior information for part alignments, and existing approaches are English-centric, restricting multilingual applications.

Method: Develops Bi-IRRA framework with bidirectional implicit relation reasoning module for masked image/text prediction to model local relations, and multi-dimensional global alignment module to bridge modality heterogeneity. Also creates multilingual TIPR benchmark using LLM translations refined with domain knowledge.

Result: Achieves new state-of-the-art results on all multilingual TIPR datasets.

Conclusion: Bi-IRRA effectively addresses multilingual TIPR challenges through implicit relation reasoning and global alignment, demonstrating superior performance across multilingual datasets.
...
<details>
  <summary>Details</summary>
Motivation: Traditional closed-set 3D detectors struggle with open-world scenarios, and existing 3D open-vocabulary models face challenges with vocabulary expansion and semantic overlap. Research on generalized 3D objectness remains insufficient.

Method: Proposes OP3Det using 2D foundation models for semantic and geometric priors, with cross-modal mixture of experts that dynamically routes uni-modal and multi-modal features from point clouds and RGB images.

Result: Significantly outperforms existing open-world 3D detectors by up to 16.0% in AR and achieves 13.5% improvement over closed-world 3D detectors.

Conclusion: OP3Det demonstrates extraordinary performance in generalized 3D object discovery, effectively bridging the gap between closed-set detection and open-world scenarios through cross-modal fusion and foundation model integration.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models suffer from computationally expensive sampling, and existing gradient-based optimization methods for distillation rely on intricate training techniques and don't explicitly preserve fine-grained details.

Method: Proposes Generalized Solver - a simple ODE sampler parameterization without complex training tricks, combined with adversarial training to mitigate artifacts and enhance detail fidelity.

Result: Demonstrates superior performance compared to existing solver training methods under similar resource constraints, reducing function evaluations from dozens to just a few.

Conclusion: The Generalized Adversarial Solver provides an effective approach for efficient diffusion model sampling while maintaining high quality and detail preservation.
...
<details>
  <summary>Details</summary>
Motivation: Vision foundation models come in limited pre-determined sizes, forcing sub-optimal deployment choices under real-world constraints, creating need for elastic models that can adapt to various computational budgets.

Method: Combines gradient information with cross-network structure correlations approximated via evolutionary algorithm, uses self-supervised importance scoring, and is retraining-free without requiring classification heads or labeled data.

Result: Superior performance over state-of-the-art methods across various sparsities, generates elastic models in under 5 minutes on single A100 GPU that can be adjusted to any computational budget.

Conclusion: SnapViT provides an efficient pruning strategy for pretrained Vision Transformers with novel evolutionary Hessian approximation and self-supervised scoring, enabling flexible deployment without performance degradation.
...
Method: Two-stage approach: first classifies drawing types (circle, meander, spiral), then extracts features using 2x2 chunking strategy where each chunk is processed separately, with ensemble method combining decisions from all chunks.

Result: Achieved 97.08% accuracy for seen patients and 94.91% for unseen patients on NewHandPD dataset, maintaining only 2.17 percentage points gap compared to 4.76-point drop in prior work.

Conclusion: The proposed chunking and ensemble approach effectively addresses dataset limitations and significantly improves robustness for Parkinson's disease detection, particularly on unseen patient data.
...
TL;DR: A new computational framework using YOLOv11 for automated analysis of circulating blood cell clusters (CCCs) from flow cytometry images, achieving over 95% accuracy in cluster classification and cell type identification.


<details>
  <summary>Details</summary>
Motivation: There is a lack of automated tools for analyzing cell clusters in flow cytometry images, which have irregular shapes, heterogeneous cell types, and require multi-channel staining for accurate identification.

Method: Two-step approach: 1) Fine-tuned YOLOv11 model to classify images into cell cluster vs non-cluster groups, outperforming CNNs and ViT; 2) Cell type identification by overlaying cluster contours with multi-channel fluorescence stain regions to handle cell debris and staining artifacts.
...
Conclusion: The automated framework successfully analyzes CCC images from flow cytometry using bright-field and fluorescence data, with potential for broader applications in immune and tumor cell cluster analysis across various diseases.
...
TL;DR: RaindropGS is a benchmark for evaluating 3D Gaussian Splatting under real-world raindrop conditions, addressing limitations of existing synthetic benchmarks by providing unconstrained raindrop-corrupted images and analyzing the full pipeline from pose estimation to reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS benchmarks use synthetic raindrop images with known camera poses, but real-world raindrops interfere with pose estimation and create domain gaps, degrading reconstruction quality.

Method: Created RaindropGS benchmark with three components: data preparation (real-world dataset with raindrop-focused, background-focused, and rain-free images), data processing, and raindrop-aware 3DGS evaluation including pose estimation, rain removal, and Gaussian training comparison.

Result: Revealed critical insights: camera focus position significantly impacts 3DGS performance, and inaccurate pose/point cloud initialization severely interferes with reconstruction under raindrop conditions.

Conclusion: The benchmark establishes clear directions for developing more robust 3DGS methods by addressing the challenges of unconstrained raindrop images and providing comprehensive evaluation metrics.
...
<details>
  <summary>Details</summary>
Motivation: Existing MLLM evaluation benchmarks are limited to single-turn question answering, failing to capture the complexity of real-world multi-turn dialogues in video understanding scenarios.

Method: Created MT-Video-Bench with 987 multi-turn dialogues across diverse domains, assessing six core competencies focused on perceptivity and interactivity, aligned with real-world applications like sports analysis and video-based tutoring.
...
Conclusion: MT-Video-Bench fills a critical gap in MLLM evaluation and will be publicly available to advance research in multi-turn video dialogue understanding.
...
TL;DR: This paper investigates feature learning strategies for offline signature verification, focusing on improving cross-dataset generalization. It compares raw signature images vs shell preprocessing across three public benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for signature verification struggle with cross-dataset generalization due to variations in handwriting styles and acquisition protocols, limiting real-world applicability.

Method: Developed two experimental pipelines: one using raw signature images and another using shell preprocessing. Tested on three public benchmarks (CEDAR, ICDAR, GPDS Synthetic) to evaluate cross-dataset performance.
...
Conclusion: While raw images performed better currently, shell preprocessing shows promise for future refinement toward robust cross-domain signature verification systems.
...
TL;DR: The paper investigates whether image-to-video (I2V) models can generate realistic pedestrian movement patterns in crowded scenes by conditioning on trajectory keyframes and evaluating using pedestrian dynamics metrics.


<details>
  <summary>Details</summary>
Motivation: To explore the world-modeling capabilities of diffusion transformer-based I2V models for generating realistic pedestrian movement in crowded public scenes.

Method: Conditions I2V models on keyframes extracted from pedestrian trajectory benchmarks and evaluates trajectory prediction performance.

Result: The paper presents quantitative evaluation of pedestrian dynamics, but specific results are not detailed in the abstract.

Conclusion: The research demonstrates the potential of I2V models for pedestrian trajectory prediction in crowded environments.
...
TL;DR: Training-free multi-reference VPR method using matrix decomposition for basis representations and residual matching, achieving up to 18% improvement in Recall@1 over single-reference methods.
...
Conclusion: The method demonstrates strong generalization while remaining lightweight, providing effective multi-reference VPR without training requirements.
...
Method: Uses Deep-UNet with Dual Attention Gates and ASPP for lesion segmentation, then two DenseNet201 encoders (original image + segmented lesion) fused via multi-head cross-attention, plus transformer-based module for patient metadata integration.

Result: Achieves state-of-the-art segmentation performance and significantly improves classification accuracy and average AUC on HAM10000, ISIC 2018 and 2019 datasets. Grad-CAM visualizations confirm model focuses on lesion areas.
...
Method: SparseVILA decouples visual sparsity by pruning redundant visual tokens during prefill and retrieving only query-relevant tokens during decoding, using an AWQ-optimized inference pipeline.
...
Conclusion: SparseVILA establishes a training-free, architecture-agnostic framework for accelerating large VLMs without sacrificing capability by decoupling query-agnostic pruning and query-aware retrieval.
...
TL;DR: UltraCUA is a foundation model that bridges multimodal computer-use agents with programmatic tool capabilities through hybrid actions combining GUI primitives and high-level tool calls.
...
Method: Four-component approach: automated pipeline for scaling programmatic tools from documentation and code, synthetic data engine producing 17,000+ verifiable tasks, large-scale hybrid action trajectory collection, and two-stage training with supervised fine-tuning plus online reinforcement learning.

Result: 7B and 32B models show 22% relative improvement on OSWorld, 11% faster execution, and 21.7% success rate on WindowsAgentArena out-of-domain evaluation, outperforming baselines trained on Windows data.

Conclusion: Hybrid action mechanism successfully reduces error propagation while maintaining execution efficiency, bridging the gap between primitive GUI actions and programmatic tool capabilities in computer-use agents.
...
<details>
  <summary>Details</summary>
Motivation: Scaling context windows to million-token levels in LLMs brings prohibitive computational and memory costs, limiting the practicality of long-context LLMs.

Method: Renders long texts into images and processes them with vision-language models, using LLM-driven genetic search to identify optimal visual rendering configurations for balancing accuracy and compression.

Result: Achieves 3-4x token compression while maintaining comparable accuracy to Qwen3-8B on long-context benchmarks, with 4x faster prefilling/decoding and 2x faster SFT training. A 128K-context VLM can scale to handle 1M-token-level tasks.

Conclusion: Visual context scaling through Glyph provides an effective alternative to token-based sequence extension, enabling efficient processing of long-context tasks while benefiting multimodal applications like document understanding.
...
TL;DR: ConsistEdit is a novel attention control method for MM-DiT models that enables consistent text-guided editing across images and videos by incorporating vision-only attention control, mask-guided pre-attention fusion, and differentiated manipulation of query, key, and value tokens.
...
Method: Proposed ConsistEdit method specifically designed for MM-DiT architecture, featuring: 1) vision-only attention control, 2) mask-guided pre-attention fusion, and 3) differentiated manipulation of query, key, and value tokens.
...
Conclusion: ConsistEdit is the first approach to perform editing across all inference steps and attention layers without handcraft, significantly enhancing reliability and consistency for text-guided editing tasks.
