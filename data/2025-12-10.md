...
TL;DR: Researchers collected a GIF dataset from Twitter and used VGG16 deep learning model to detect cyberbullying with 97% accuracy.


<details>
  <summary>Details</summary>
Motivation: Cyberbullying is escalating on social media, but most research focuses on text detection with very few studies on GIFs/stickers, creating a research gap.

Method: 1. Extracted cyberbullying-related hashtags from Twitter, 2. Used GIPHY API to download GIFs (4100 total, both cyberbullying and non-cyberbullying), 3. Applied pre-trained VGG16 deep learning model for detection.

Result: The VGG16 model achieved 97% accuracy in detecting cyberbullying in GIFs, and the researchers created a valuable GIF dataset for future research.

Conclusion: The study successfully addresses the gap in cyberbullying detection for GIFs/stickers, provides a dataset for researchers, and demonstrates high accuracy using deep learning models.
...
TL;DR: Deep learning with 4-band satellite imagery enables near real-time monitoring of fire damage in Sudan's armed conflicts, outperforming baseline methods with minimal delay.
...
Method: Uses 4-band imagery from Planet Labs combined with a deep learning model to monitor fire damage in armed conflicts. The approach is tested on five case studies in Sudan and compared against baseline methods.

Result: The automated deep learning method captures active fires and charred areas more accurately than baseline methods. Surprisingly, using 8-band imagery or time series of imagery provides only marginal improvements over the 4-band approach.

Conclusion: Fire damage in armed conflicts can be effectively monitored with minimal delay using 4-band satellite imagery and deep learning, offering a practical solution for rapid conflict monitoring in Sudan and similar contexts.
...
<details>
  <summary>Details</summary>
Motivation: Video face swapping in film production needs high fidelity and temporal consistency over long sequences, but current methods struggle with these challenges. The authors explore leveraging rich visual attributes from source videos to enhance both fidelity and temporal coherence.

Method: LivingSwap uses keyframes as conditioning signals to inject target identity, combining keyframe conditioning with video reference guidance for temporal stitching. To address data scarcity, they construct a paired face-swapping dataset called Face2Face and reverse data pairs for reliable ground-truth supervision.

Result: Extensive experiments show state-of-the-art results, seamlessly integrating target identity with source video's expressions, lighting, and motion while significantly reducing manual effort in production workflows.

Conclusion: LivingSwap presents an effective video reference-guided approach for face swapping that achieves superior temporal consistency and fidelity compared to existing methods, making it valuable for film and entertainment production.
...
TL;DR: A hierarchical semantic segmentation framework for dental imaging that embeds anatomical structure through recurrent level-wise predictions with feature conditioning, improving performance on fine-grained anatomies.
...
Method: Recurrent level-wise prediction scheme with restrictive output heads and top-down feature conditioning using Feature-wise Linear Modulation, plus probabilistic composition rules and hierarchical loss combining Dice, cross entropy, and consistency terms.

Result: Hierarchical variants consistently increased IoU, Dice, and recall (especially for fine-grained anatomies) and produced more anatomically coherent masks, though with increased false positives (higher recall over precision).

Conclusion: Explicit hierarchical structuring improves both performance and clinical plausibility in dental imaging, particularly valuable in low-data regimes.
...
TL;DR: FRIEDA is a new benchmark for evaluating complex cartographic reasoning in large vision-language models, testing spatial intelligence through multi-step inference on real map images across topological, metric, and directional relations.


<details>
  <summary>Details</summary>
Motivation: Current map VQA evaluations treat maps as simple charts, missing the complex spatial reasoning needed for real-world applications like disaster response and urban planning. There's a gap in assessing layered symbology comprehension and spatial relations that span multiple maps.

Method: Created FRIEDA benchmark using real map images from documents across various domains and geographical areas. Questions target all three GIS spatial relation categories: topological (border, equal, intersect, within), metric (distance), and directional (orientation). All questions require multi-step inference, with many needing cross-map grounding.

Result: Evaluated 11 state-of-the-art LVLMs in direct and contextual settings. Best models (Gemini-2.5-Pro and GPT-5-Think) achieved only 38.20% and 37.20% accuracy respectively, far below human performance of 84.87%. Shows persistent gap in multi-step cartographic reasoning.

Conclusion: FRIEDA reveals significant limitations in current LVLMs' spatial intelligence and provides a rigorous benchmark to drive progress in cartographic reasoning capabilities essential for real-world applications.
...
TL;DR: SSplain is a novel explainer method for medical image classification that generates pixel-wise explanations while preserving image smoothness and sparsity, outperforming existing methods on ROP diagnosis tasks.


<details>
  <summary>Details</summary>
Motivation: Neural networks are black-box models in medical diagnosis, requiring explainers to build clinician trust. Existing explainers fail to preserve important image structures like smoothness and sparsity in their explanations.
...
Result: SSplain outperforms common explainers in post-hoc accuracy and smoothness analyses, identifies clinically relevant features for ROP diagnosis, and generalizes well to other public datasets.

Conclusion: SSplain provides realistic, structure-preserving explanations for medical image classification that enhance model interpretability and align with clinical understanding, with code publicly available.
...
TL;DR: A unified model for sign language understanding that performs both sign language translation (SLT) and sign-subtitle alignment (SSA) using privacy-preserving visual features, sliding perceiver mapping, and multi-task training, achieving SOTA results on BSL and strong ASL performance.


<details>
  <summary>Details</summary>
Motivation: To develop a unified model for practical sign language communication, corpus construction, and educational applications by enabling both translation of signing videos to spoken language text and temporal alignment of signing with subtitles.

Method: Three-component approach: (1) lightweight visual backbone using human keypoints and lip-region images for privacy preservation, (2) Sliding Perceiver mapping network to aggregate visual features into word-level embeddings, (3) multi-task scalable training jointly optimizing SLT and SSA with multilingual pretraining on BSL and ASL datasets.

Result: Achieved state-of-the-art results on BOBSL (BSL) dataset for both SLT and SSA, with robust zero-shot generalization and fine-tuned performance on How2Sign (ASL), demonstrating scalable translation across different sign languages.

Conclusion: The unified model successfully bridges vision-text gap for sign language understanding, showing strong cross-linguistic generalization potential through multilingual pretraining and effective multi-task architecture design.
...
Result: Achieves state-of-the-art generalization on GAN- and diffusion-generated datasets and maintains performance under significant model pruning.
...
TL;DR: M2A is a simple continual test-time adaptation method using random masking with consistency and entropy losses, achieving strong performance on corrupted datasets without complex uncertainty or attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods rely on calibrated uncertainty or stable attention scores with complex masking designs. The paper questions whether simple random masking can be sufficient under strong corruption, aiming to simplify adaptation without performance degradation.

Method: M2A generates random masked views (spatial or frequency) and adapts using two objectives: mask consistency loss aligning predictions across different views, and entropy minimization loss encouraging confident outputs. Explores spatial (patch/pixel) and frequency (all/low/high) masking subtypes.
...
Conclusion: Simple random masking with consistency and entropy objectives is sufficient for effective test-time adaptation, eliminating the need for complex uncertainty or attention-based masking designs.
...
TL;DR: This paper evaluates various change detection models for deforestation monitoring in the Amazon using PRODES data, addressing limitations of existing approaches by testing modern architectures and preprocessing techniques to achieve improved performance.


<details>
  <summary>Details</summary>
Motivation: The preservation of the Amazon Rainforest is crucial for climate change, biodiversity, and indigenous cultures. Existing machine learning approaches for deforestation detection using PRODES data have significant limitations: unsatisfactory effectiveness, lack of modern architectures like self-attention mechanisms, and absence of methodological standardization for direct comparisons between studies.

Method: The authors evaluate various change detection models in a unified dataset, including fully convolutional models and Transformer-based networks with self-attention mechanisms. They investigate the impact of different pre- and post-processing techniques such as filtering deforested areas based on connected component size, texture replacement, and image enhancements. They also test different strategies for combining evaluated models.

Result: The approaches significantly improve individual model effectiveness. By combining models, they achieve an F1-score of 80.41%, which is comparable to other recent works in the literature.

Conclusion: The work addresses key gaps in deforestation detection by providing standardized evaluation of modern architectures and demonstrating that preprocessing techniques and model combination strategies can substantially improve performance for Amazon deforestation monitoring using PRODES data.
...
TL;DR: CVP is a multimodal spatial reasoning framework inspired by human central/peripheral vision, using target-affinity tokens and allocentric grids to achieve state-of-the-art 3D scene understanding.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal models for spatial reasoning rely on unstructured representations (point clouds, voxels, patch features) with implicit scene context injection via coordinate embeddings, leading to limited spatial reasoning capabilities due to lack of explicit high-level structural understanding.
...
Result: CVP achieves state-of-the-art performance across a range of 3D scene understanding benchmarks, demonstrating superior spatial reasoning capabilities compared to existing approaches.

Conclusion: The biologically-inspired central-peripheral vision framework provides an effective approach for structured, context-aware understanding of complex 3D environments, addressing limitations of unstructured representations in multimodal spatial reasoning.
...
TL;DR: Fourier-RWKV: A linear-complexity Transformer-based dehazing framework using Multi-State Perception (spatial, frequency, semantic) for efficient non-uniform haze removal.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based dehazing methods have quadratic computational complexity, making them impractical for real-time deployment despite their ability to capture global context needed for non-uniform haze conditions.
...
Result: State-of-the-art performance on multiple benchmarks across diverse haze scenarios with significantly reduced computational overhead, achieving favorable trade-off between restoration quality and practical efficiency.

Conclusion: Fourier-RWKV successfully addresses the computational bottleneck of Transformer-based dehazing while maintaining high performance through synergistic integration of spatial, frequency, and semantic perception states, enabling practical real-world deployment.
...
TL;DR: Monocular depth estimation models show trade-off between accuracy and human similarity - improving accuracy doesn't necessarily make models more human-like, requiring human-centric evaluation beyond traditional metrics.


<details>
  <summary>Details</summary>
Motivation: To investigate whether there's a trade-off between model accuracy and human-like behavior in monocular depth estimation, similar to what's observed in object recognition, particularly for natural outdoor scenes where benchmarks use sensor-based ground truth rather than human perceptual estimates.

Method: Systematically analyzed 69 monocular depth estimators using KITTI dataset. Applied affine fitting to decompose prediction errors into interpretable components on a factor-by-factor basis to dissect error patterns.

Result: While humans and DNNs share certain estimation biases (positive error correlations), distinct trade-off relationships exist between model accuracy and human similarity. Improving accuracy does not necessarily lead to more human-like behavior.

Conclusion: Necessity of developing multifaceted, human-centric evaluations beyond traditional accuracy metrics for monocular depth estimation models, as accuracy improvements don't guarantee more human-like behavior.
...
TL;DR: GeoLoom is a framework for text-to-geometric-diagram generation using formal language translation and Monte Carlo optimization for precise spatial accuracy.


<details>
  <summary>Details</summary>
Motivation: Geometric diagram generation requires strict spatial accuracy while offering well-defined constraints, presenting both challenge and opportunity. Current methods lack the structural fidelity needed for precise geometric representations.

Method: Two-component framework: 1) Autoformalization module translating natural language to generation-oriented formal language GeoLingua, 2) Coordinate solver using Monte Carlo optimization to map formal constraints to precise coordinates. Also introduces GeoNF dataset and constraint-based evaluation metric.
...
Conclusion: GeoLoom offers a principled foundation for interpretable and scalable diagram generation in geometric domains, combining formal language translation with efficient optimization for enhanced correctness.
...
TL;DR: On-device Animal Re-ID framework for MCU-class devices achieves competitive accuracy with 100x smaller models, enabling wildlife monitoring in low-connectivity environments.


<details>
  <summary>Details</summary>
Motivation: Camera-based animal re-identification is needed for wildlife monitoring and livestock management in outdoor environments with limited wireless connectivity, but existing models are too large for microcontroller-based edge devices with memory and resolution constraints.

Method: 1) Characterized gap between SOTA Animal Re-ID models and MCU hardware, showing limited benefit from knowledge distillation under constraints. 2) Designed compact architecture by systematically scaling MobileNetV2 backbone for low-resolution inputs. 3) Introduced data-efficient fine-tuning strategy requiring only 3 images per animal identity for fast adaptation to new sites.

Result: Achieved competitive retrieval accuracy across 6 public datasets while reducing model size by over 100x. On self-collected cattle dataset, deployed model performed fully on-device inference with minimal accuracy drop and unchanged Top-1 accuracy compared to cluster version.
...
TL;DR: Blur2Sharp is a novel framework that integrates 3D-aware neural rendering and diffusion models to generate sharp, geometrically consistent novel-view images from a single reference view, addressing challenges in lifelike human avatar creation.


<details>
  <summary>Details</summary>
Motivation: Current approaches for human avatar creation either produce geometrically inconsistent multi-view images or sacrifice photorealism, resulting in blurry outputs under diverse viewing angles and complex motions. There's a need for methods that can generate sharp, geometrically consistent novel views from minimal input.

Method: Blur2Sharp uses a dual-conditioning architecture: 1) Human NeRF model generates geometrically coherent multi-view renderings for target poses with explicit 3D structural guidance, 2) Diffusion model conditioned on these renderings refines images while preserving details. Hierarchical feature fusion incorporates texture, normal, and semantic priors from parametric SMPL models to improve global coherence and local detail accuracy.

Result: Extensive experiments show Blur2Sharp consistently surpasses state-of-the-art techniques in both novel pose and view generation tasks, particularly excelling under challenging scenarios involving loose clothing and occlusions.

Conclusion: Blur2Sharp successfully addresses the fundamental challenge of creating lifelike human avatars with realistic pose variation and viewpoint flexibility by integrating 3D-aware neural rendering and diffusion models, producing sharp, geometrically consistent novel-view images from only a single reference view.
...
TL;DR: VisKnow framework constructs structured visual knowledge bases (like AnimalKB) with multi-modal object knowledge graphs to advance comprehensive object understanding beyond simple recognition.


<details>
  <summary>Details</summary>
Motivation: Current object understanding is limited to simple category labels, lacking comprehensive perception of components, attributes, relationships, and contextual knowledge. Existing multi-modal data is task-oriented and not systematically organized for holistic object understanding.

Method: Proposes VisKnow framework that structures multi-modal object knowledge as graphs, integrating enriched aligned text and image-source knowledge with region annotations at object and part levels through expert design and large-scale model application.

Result: Constructed AnimalKB covering 406 animal categories with 22K textual knowledge triplets, 420K images, and region annotations. Experiments show enhanced performance on zero-shot recognition, fine-grained VQA, and serves as benchmark for knowledge graph completion and part segmentation.

Conclusion: Automatically constructed visual knowledge bases like AnimalKB demonstrate significant potential to advance visual understanding and practical applications by providing structured, comprehensive multi-modal object knowledge.
...
TL;DR: The paper explores prompt tuning methods for 3D object detection, investigating whether models trained on Waymo dataset can serve as foundation models and adapt to other scenarios, proposing a Scene-Oriented Prompt Pool (SOP²).


<details>
  <summary>Details</summary>
Motivation: To explore the effectiveness of prompt tuning methods (inspired by NLP success with LLMs) in 3D object detection, investigating transfer learning capabilities from large-scale datasets to other scenarios.

Method: Investigates impact of prompt tokens and prompt generators, proposes Scene-Oriented Prompt Pool (SOP²) for 3D object detection adaptation.

Result: Demonstrates effectiveness of prompt pools in 3D object detection, showing that models trained on Waymo dataset can adapt to other scenarios using prompt tuning techniques.

Conclusion: Prompt tuning methods are effective for 3D object detection adaptation, and the proposed SOP² approach shows promise for inspiring further research into prompt-based techniques in 3D vision.
...
<details>
  <summary>Details</summary>
Motivation: Traditional video codecs are optimized for perceptual quality based on human visual system models, but these assumptions don't apply to intermediate neural network features in split inference systems. Features are abstract, sparse, and task-specific, making perceptual fidelity irrelevant.

Method: The authors investigate using Versatile Video Coding (VVC) for compressing neural features under the MPEG-AI Feature Coding for Machines (FCM) standard. They perform tool-level analysis to understand individual coding components' impact on compression efficiency and downstream task accuracy.

Result: Three lightweight VVC profiles were developed: Fast (2.96% BD-Rate gain, 21.8% encoding time reduction), Faster (1.85% BD-Rate gain, 51.5% speedup), and Fastest (95.6% encoding time reduction with only 1.71% BD-Rate loss).

Conclusion: The proposed VVC profiles effectively compress neural features for split inference systems, providing significant encoding speed improvements while maintaining compression efficiency, demonstrating that traditional perceptual quality optimizations can be streamlined for machine-oriented feature compression.
...
TL;DR: MM-CoT is a diagnostic benchmark that evaluates multimodal models' Chain-of-Thought reasoning by testing visual grounding and logical coherence through constrained event chain selection with adversarial distractors.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on generating explanations but neglect verifying whether multimodal models' reasoning is genuinely grounded in visual evidence and logically coherent. There's a gap in assessing true reasoning fidelity beyond generative fluency.

Method: Instead of free-form explanation generation, models must select the sole event chain satisfying two constraints: visual consistency (anchored in observable evidence) and logical coherence (causal and commonsense validity). Adversarial distractors are engineered to violate one constraint to expose distinct reasoning failures.
...
<details>
  <summary>Details</summary>
Motivation: Current depth completion methods use unrealistic uniform random sampling of sparse depth points from ground truth, ignoring that real sensors have geometry-dependent and spatially nonuniform reliability patterns.

Method: Uses PCA-based surface normal estimation on RGB-D point clouds to compute per-pixel depth reliability measures, then samples sparse depth points according to this reliability distribution rather than uniformly at random.

Result: Improves accuracy, reduces artifacts near edges/discontinuities, and creates more realistic training conditions that better reflect real sensor behavior on NYU Depth v2 dataset.

Conclusion: Geometry-aware sparse depth sampling produces better depth completion results by more accurately simulating real sensor characteristics compared to uniform random sampling.
...
<details>
  <summary>Details</summary>
Motivation: Current camera-only BEV perception systems face a fundamental tension between achieving state-of-the-art performance and maintaining practical deployment tractability on vehicles. This bottleneck comes from dependencies on computationally expensive view transformations and platform-specific custom kernels.

Method: The framework follows two principles: "Deployable by Design" via a novel view transformation that decomposes projection into standard Index-Gather-Reshape pipeline using deterministic pre-sorting, and "Fast by Algorithm" through end-to-end depth-aware fusion with temporal aggregation and robust data augmentation.

Result: FastBEV++ achieves new state-of-the-art 0.359 NDS on nuScenes benchmark while maintaining exceptional real-time performance exceeding 134 FPS on automotive-grade hardware (Tesla T4).

Conclusion: FastBEV++ presents a mature and scalable design philosophy for production autonomous systems by offering high accuracy without custom plugins, reconciling the tension between performance and deployment efficiency.
...
TL;DR: HTC-VLM introduces a hybrid framework with dual channels (continuous for fine details, discrete for symbolic anchors) that compresses 580 visual tokens into 1 token while maintaining 87.2% performance across 7 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Vision-language models face quadratic computational costs from processing hundreds of visual patch tokens. Traditional compression methods face a trade-off: continuous compression dilutes high-level semantics like object identities, while discrete quantization loses fine-grained details like textures.

Method: Hybrid framework with dual channels: continuous pathway for fine-grained details via ViT patches, and discrete pathway for symbolic anchors using MGVQ quantization projected to four tokens. These are fused into a 580-token hybrid sequence and compressed into a single voco token via disentanglement attention mask and bottleneck.

Result: Achieves average performance retention of 87.2% across seven benchmarks (GQA, VQAv2, MMBench, MME, POPE, SEED-Bench, ScienceQA-Image), outperforming leading continuous baseline at 81.0% with 580-to-1 compression ratio. Attention analyses show compressed token prioritizes discrete anchor, validating semantic guidance.

Conclusion: A minimalist hybrid design can resolve the efficiency-fidelity dilemma and advance scalable VLMs by disentangling semantics and appearance through dual channels.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in breast ultrasound lesion segmentation by extracting robust local features and learning global dependencies, while handling ultrasound noise and maintaining morphological integrity of malignant lesions.

Method: Proposes a hybrid framework with: 1) Residual CNN modules for local features, 2) Customized Swin Transformer blocks with internal residual pathways for global dependencies, 3) Laplacian-of-Gaussian operator for tissue continuity and noise suppression, 4) Boundary-oriented operator for morphological integrity, 5) Stage-wise contraction strategy for scale invariance, 6) Multi-Scale Channel Attention and Squeezing (MSCAS) module for selective feature emphasis, 7) Pixel-Attention module for adaptive weighting of lesion pixels.
...
Conclusion: The Residual-SwinCA-Net framework significantly improves breast lesion diagnostic performance and supports timely clinical decision-making through superior segmentation accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Existing knowledge distillation methods for temporal 3D object detection overlook future frames, focusing only on spatial features or temporal relations, making it hard for online models to learn future knowledge from offline teachers.
...
Result: Achieves up to 1.3 mAP and 1.3 NDS gains on nuScenes dataset, plus most accurate velocity estimation, without increasing inference cost when applied to two high-performing 3D object detection baselines.
...
TL;DR: QHP introduces query-aware hub prototypes for few-shot 3D segmentation, addressing prototype bias by modeling support-query correlations and optimizing prototype distributions.
...
Method: Proposes Query-aware Hub Prototype (QHP) learning with two modules: 1) Hub Prototype Generation (HPG) constructs bipartite graph connecting query and support points, identifies frequently linked support hubs to generate query-relevant prototypes; 2) Prototype Distribution Optimization (PDO) uses purity-reweighted contrastive loss to refine prototypes by pulling bad hubs and outlier prototypes toward class centers.
...
Conclusion: QHP successfully addresses prototype bias in few-shot 3D segmentation by explicitly modeling semantic correlations between support and query sets and optimizing prototype distributions.
...
TL;DR: SFP proposes spatial and frequency priors for real-world scene recovery, using transmission map estimation from inverse degraded images and adaptive frequency enhancement with novel priors, integrated via weighted fusion.
...
Method: 1) Spatial prior: Estimate transmission map from inverse degraded image's spectral projection. 2) Frequency prior: Construct adaptive frequency enhancement mask using two novel priors - DC component mean intensity approximation and low radial frequency magnitude ratio (1% of total spectrum). 3) Weighted fusion strategy integrates spatial restoration, frequency enhancement, and salient input features.

Result: Extensive evaluations demonstrate effectiveness and superiority of SFP for scene recovery under various degradation conditions compared to existing methods.

Conclusion: SFP provides an effective approach for real-world scene recovery by combining spatial and frequency priors, overcoming limitations of single-prior methods and synthetic-trained networks, with strong generalization to diverse real-world scenarios.
...
TL;DR: RLCNet: An end-to-end deep learning framework for simultaneous online calibration of LiDAR, RADAR, and camera sensors in autonomous vehicles, featuring real-time operation with weighted moving average and outlier rejection.
...
Method: RLCNet: A novel end-to-end trainable deep learning framework for simultaneous online calibration of multimodal sensors. Incorporates an online calibration framework with weighted moving average and outlier rejection for real-time operation and dynamic parameter adjustment.

Result: Validated on real-world datasets, RLCNet demonstrates robust performance under diverse conditions with superior accuracy and robustness compared to existing methods. The framework reduces prediction noise and improves resilience to drift.

Conclusion: RLCNet provides a practical solution for online sensor calibration in autonomous vehicles, enabling dynamic adjustment of calibration parameters with improved accuracy and robustness against sensor drift and environmental challenges.
...
TL;DR: EgoX is a framework that converts third-person (exocentric) videos into first-person (egocentric) videos using adapted video diffusion models with geometric consistency mechanisms.


<details>
  <summary>Details</summary>
Motivation: Egocentric perception provides direct, immersive understanding of the world from one's own viewpoint. Converting exocentric to egocentric videos enables new immersive applications but is extremely challenging due to major camera pose differences and minimal view overlap between perspectives.

Method: EgoX leverages pretrained video diffusion models through lightweight LoRA adaptation. It uses a unified conditioning strategy combining exocentric and egocentric priors via width and channel concatenation. A geometry-guided self-attention mechanism selectively attends to spatially relevant regions for geometric coherence.
...
Conclusion: EgoX successfully addresses the challenging task of exocentric-to-egocentric video translation by combining adapted diffusion models with geometric consistency mechanisms, enabling immersive video generation from third-person perspectives.
...
TL;DR: PAVAS introduces physics-aware video-to-audio generation using physical parameters (mass, velocity) to synthesize more realistic sounds that reflect underlying physical factors, outperforming appearance-driven V2A models.


<details>
  <summary>Details</summary>
Motivation: Current V2A models are appearance-driven and capture visual-acoustic correlations without considering the physical factors that shape real-world sounds, limiting their physical realism.

Method: Uses Physics-Driven Audio Adapter (Phy-Adapter) in latent diffusion framework, with Physical Parameter Estimator (PPE) that employs VLM for mass estimation and segmentation-based 3D reconstruction for motion trajectory/velocity computation.

Result: Outperforms existing V2A models in quantitative/qualitative evaluations, produces physically plausible and perceptually coherent audio, validated on new VGG-Impact benchmark with Audio-Physics Correlation Coefficient (APCC) metric.

Conclusion: Incorporating physical reasoning into V2A generation enables more realistic sound synthesis that reflects underlying physical factors, advancing beyond appearance-driven approaches.
...
TL;DR: OpenSubject is a large-scale video-derived dataset (2.5M samples, 4.35M images) for subject-driven image generation and manipulation, featuring a four-stage pipeline that leverages cross-frame identity priors to improve identity preservation in complex scenes.


<details>
  <summary>Details</summary>
Motivation: Current subject-driven image generation models often fail to maintain reference identities and struggle with complex multi-subject scenes, creating a need for better training data that preserves identity consistency.
...
Result: Training with OpenSubject improves generation and manipulation performance, particularly in complex scenes, as demonstrated through extensive experiments and a comprehensive benchmark evaluating identity fidelity, prompt adherence, manipulation consistency, and background consistency.

Conclusion: OpenSubject provides a high-quality, large-scale dataset that addresses identity preservation challenges in subject-driven generation, enabling better performance in complex multi-subject scenarios through its carefully constructed video-derived corpus and verification pipeline.
...
TL;DR: Terrain Diffusion: AI-powered successor to Perlin noise that uses diffusion models to generate infinite, seamless, coherent planetary landscapes with real-time synthesis capabilities.


<details>
  <summary>Details</summary>
Motivation: Procedural noise functions like Perlin noise have been limited in realism and large-scale coherence despite being fast and infinite. The paper aims to bridge the fidelity of modern diffusion models with the essential properties of procedural noise for next-generation world generation.
...
Result: Establishes diffusion models as practical foundation for procedural world generation, capable of synthesizing entire planets coherently, controllably, and without limits while maintaining seamless infinite extent, seed-consistency, and constant-time random access.

Conclusion: Terrain Diffusion successfully bridges the gap between diffusion model fidelity and procedural noise properties, creating an AI-era successor to Perlin noise that enables realistic, coherent, and infinite planetary-scale terrain generation.
...
TL;DR: GeoDM: A geometry-aware distribution-matching framework for dataset distillation that operates in Cartesian product of Euclidean, hyperbolic, and spherical manifolds to capture flat, hierarchical, and cyclical data structures.


<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods are confined to Euclidean spaces, which only capture linear structures and overlook the intrinsic geometry of real data (e.g., curvature). High-dimensional data often lie on low-dimensional manifolds, so distilled data should align with the original data manifold geometry.

Method: Proposes GeoDM framework operating in Cartesian product of Euclidean, hyperbolic, and spherical manifolds. Introduces learnable curvature and weight parameters for three geometries. Designs optimal transport loss to enhance distribution fidelity. Theoretical analysis shows geometry-aware distribution matching yields smaller generalization error bound.
...
Conclusion: Geometry-aware distribution matching in product spaces is superior to Euclidean-only approaches for dataset distillation, as it better captures the intrinsic manifold structures of real-world data.
...
<details>
  <summary>Details</summary>
Motivation: Precise teeth landmark detection is crucial for advanced orthodontic diagnostics, personalized treatment planning, and treatment monitoring, but faces challenges due to complex tooth geometry and individual variations.

Method: The paper presents the 3DTeethLand challenge organized with MICCAI 2024, which introduces the first publicly available dataset for 3D teeth landmark detection and calls for algorithm development using deep learning techniques.

Result: The challenge establishes a benchmark dataset and framework to assess state-of-the-art methods for 3D teeth landmark detection, encouraging community participation and methodological contributions.

Conclusion: The 3DTeethLand challenge provides a valuable resource to advance 3D teeth landmark detection technology, addressing significant clinical needs in orthodontics through collaborative research and standardized evaluation.
...
TL;DR: GeoDiffMM is a diffusion-based Lagrangian video motion magnification framework that uses optical flow as geometric conditioning to amplify subtle motions while suppressing noise and preserving structural consistency.
...
Conclusion: GeoDiffMM provides a novel diffusion-based Lagrangian approach that effectively separates true micro-motion from noise through geometric conditioning, enabling structurally consistent motion magnification with better generalization.
...
TL;DR: Proposes LSQMM, a quaternion-based SVM for color image classification that treats RGB channels as pure quaternions to preserve channel correlations, with quaternion nuclear norm regularization for low-rank structure.


<details>
  <summary>Details</summary>
Motivation: Conventional methods represent color images as vectors/matrices/tensors in real field, losing intrinsic coupling relationships among RGB channels. Quaternion algebra can better preserve these relationships, as shown in image recovery tasks.

Method: LSQMM treats RGB channels as pure quaternions, adds quaternion nuclear norm regularization to hinge loss to promote low-rank structures from correlated channels, and uses ADMM-based iterative algorithm to solve the quaternion optimization model.

Result: Experimental results on multiple color image datasets show LSQMM has advantages in classification accuracy, robustness, and computational efficiency compared to SVM, support matrix machines, and support tensor machines.

Conclusion: Quaternion-based approach effectively preserves channel correlations in color images, leading to improved classification performance through low-rank regularization and quaternion algebra.
...
TL;DR: This paper provides a systematic explainable AI analysis of image protection mechanisms (Glaze, Nightshade), revealing they operate as structured, low-entropy perturbations tightly coupled to image content across representational, spatial, and spectral domains.


<details>
  <summary>Details</summary>
Motivation: While empirical effectiveness of image protection mechanisms like Glaze and Nightshade is known, their internal structure, detectability, and representational behavior remain poorly understood. The study aims to systematically analyze these adversarial perturbations using explainable AI techniques.

Method: The study uses a unified framework integrating white-box feature-space inspection and black-box signal-level probing. Specific techniques include latent-space clustering, feature-channel activation analysis, occlusion-based spatial sensitivity mapping, and frequency-domain characterization.

Result: Protection mechanisms operate as structured, low-entropy perturbations tightly coupled to underlying image content. Protected images preserve content-driven feature organization with protection-specific substructure rather than inducing global representational drift. Frequency analysis shows Glaze and Nightshade redistribute energy along dominant image-aligned frequency axes rather than introducing diffuse noise.

Conclusion: Contemporary image protection operates through structured feature-level deformation rather than semantic dislocation, explaining why protection signals remain visually subtle yet consistently detectable. This work advances interpretability of adversarial image protection and informs future defense and detection strategies for generative AI systems.
...
TL;DR: PointDico is a novel 3D representation learning model that combines diffusion and contrastive learning through knowledge distillation to overcome limitations of existing methods on unordered point clouds.
...
Conclusion: The proposed PointDico model successfully combines diffusion and contrastive learning to overcome limitations of existing 3D representation learning methods, achieving superior performance on benchmark datasets.
...
TL;DR: Bi^2MAC: A bimodal bi-adaptive mask-aware convolution for pansharpening that uses soft/hard masks to route features to appropriate processing branches, achieving SOTA performance with lower computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning pansharpening methods struggle with regional heterogeneity in remote sensing images. Adaptive convolution methods address this but suffer from high computational costs and limited ability to effectively capture heterogeneous regions.

Method: Proposes Bimodal Bi-Adaptive Mask-Aware Convolution (Bi^2MAC) with a lightweight mask generation module that produces both soft and hard masks. Soft masks modulate input features preliminarily, while hard masks guide different region types into separate processing branches: redundant features go to a compact branch for low-cost global processing, and heterogeneous features go to a focused branch for fine-grained modeling with more computational resources.

Result: Extensive experiments on multiple benchmark datasets show Bi^2MAC achieves state-of-the-art performance while requiring substantially lower training time and parameter counts, with minimal computational cost among adaptive convolution models.

Conclusion: Bi^2MAC effectively exploits information from different region types while intelligently allocating computational resources, overcoming limitations of conventional adaptive convolution methods for pansharpening tasks.
...
TL;DR: HybridSplat: A hybrid splatting framework that accelerates rendering of complex reflective scenes using reflection-baked Gaussian tracing and achieves 7x speedup with 4x fewer primitives.
...
Method: Proposes HybridSplat with reflection-baked Gaussian tracing that bakes view-dependent reflection within each Gaussian primitive, uses tile-based Gaussian splatting for reflection rendering, integrates reflective and base primitives in unified framework, and adds pipeline-level acceleration with reflection-sensitive Gaussian pruning.

Result: Achieves ~7x rendering speed acceleration across complex reflective scenes from Ref-NeRF and NeRF-Casting datasets with 4x fewer Gaussian primitives than ray-tracing based baselines while preserving reflection quality.

Conclusion: HybridSplat establishes a new state-of-the-art method for complex reflective scene rendering, offering significantly faster rendering, lower memory usage, and high-fidelity reconstruction.
...
TL;DR: DINO-BOLDNet generates BOLD fMRI images from T1w structural MRI using DINOv3-guided multi-slice attention, achieving better performance than GAN baselines.


<details>
  <summary>Details</summary>
Motivation: To recover missing BOLD fMRI information when images are corrupted or unavailable, enabling downstream tasks that require functional brain data.

Method: Uses frozen self-supervised DINOv3 encoder for within-slice structural features, slice-attention module for cross-slice context, multi-scale decoder for functional contrast restoration, and DINO-based perceptual loss for structural/textural consistency.

Result: Outperforms conditional GAN baseline in PSNR and MS-SSIM metrics on clinical dataset of 248 subjects. First framework capable of generating mean BOLD images directly from T1w images.

Conclusion: Demonstrates potential of self-supervised transformer guidance for structural-to-functional mapping in neuroimaging, offering solution for missing BOLD data scenarios.
...
TL;DR: TrackingWorld: A novel pipeline for dense 3D tracking of almost all pixels in world-centric coordinates from monocular video, addressing camera motion separation and tracking newly emerging objects.
...
Conclusion: TrackingWorld successfully addresses the limitations of existing methods by providing a comprehensive solution for dense 3D tracking of almost all pixels within a world-centric coordinate system from monocular video.
...
<details>
  <summary>Details</summary>
Motivation: Fire detection is crucial for safety, but limited fire datasets constrain detection model performance. Home IoT systems need better training data for effective fire detection.

Method: Proposed SCU-CGAN integrates U-Net, CBAM (Convolutional Block Attention Module), and an additional discriminator to generate realistic fire images from nonfire images for dataset augmentation.

Result: SCU-CGAN achieved 41.5% improvement in KID score over CycleGAN, and the augmented dataset improved YOLOv5 nano's mAP@0.5:0.95 by 56.5% without model structure changes.

Conclusion: SCU-CGAN effectively generates high-quality fire images for dataset augmentation, significantly enhancing fire detection model accuracy while addressing data scarcity issues.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs using Pre-Norm architecture suffer from severe norm disparity between high-norm visual tokens and low-norm text tokens. This imbalance causes asymmetric update dynamics where visual tokens update much slower than text tokens, fundamentally impairing effective cross-modal feature fusion.
...
TL;DR: A novel framework for simultaneous image enhancement and noise suppression under complex lighting conditions using gradient-domain weighted guided filtering, Retinex decomposition, and multi-exposure fusion.


<details>
  <summary>Details</summary>
Motivation: Existing image enhancement methods either amplify noise excessively or only work well under specific illumination conditions, limiting their effectiveness in challenging real-world scenarios with complex lighting.

Method: 1) Use gradient-domain weighted guided filter (GDWGIF) for illumination estimation and initial quality improvement. 2) Apply Retinex model to decompose image into illumination and reflection layers. 3) Process layers in parallel - correct illumination layer and enhance reflection layer. 4) Optimize dynamic range through multi-exposure fusion and linear stretching.

Result: Experimental evaluation on real-world datasets shows the proposed method outperforms state-of-the-art methods in both contrast enhancement and noise suppression.

Conclusion: The proposed framework effectively addresses the limitations of existing methods by providing simultaneous enhancement and noise suppression under complex illumination conditions, making it suitable for practical vision-based applications.
...
TL;DR: This paper addresses the challenge of detecting facial retouching in biometric systems, proposing to use beauty assessment algorithms and AI-based feature extraction to improve detection rates.


<details>
  <summary>Details</summary>
Motivation: Facial retouching is widely used in social media and professional photography to enhance beauty, but when retouched images are used as biometric samples, it becomes problematic for face recognition systems. Since retouching poses challenges for biometric systems, detecting facial retouching becomes increasingly necessary.

Method: The study analyzes changes in beauty assessment algorithms of retouched images, assesses different AI-based feature extraction methods for retouching detection, and evaluates whether face beauty can be exploited to enhance detection rates.

Result: In scenarios where the attacking retouching algorithm is unknown, the proposed approach achieved 1.1% D-EER (Detection Equal Error Rate) on single image detection.

Conclusion: The research demonstrates that analyzing beauty assessment algorithms and using AI-based feature extraction can effectively detect facial retouching, achieving promising results even when the specific retouching algorithm is unknown.
...
TL;DR: The paper develops an optimized deep learning pipeline for automated fish re-identification using the AutoFish dataset, showing that Vision Transformers with hard triplet mining and custom image transformations outperform CNNs, achieving 90.43% Rank-1 accuracy.
...
Method: Developed a deep learning pipeline using the AutoFish dataset (simulating EM systems with conveyor belts and six similar fish species). Used hard triplet mining with custom image transformations including dataset-specific normalization. Compared Vision Transformer-based Swin-T architecture against CNN-based ResNet-50.

Result: Swin-T consistently outperformed ResNet-50, achieving peak performance of 41.65% mAP@k and 90.43% Rank-1 accuracy. Analysis revealed the main challenge is distinguishing visually similar individuals of the same species (intra-species errors), where viewpoint inconsistency is more problematic than partial occlusion.

Conclusion: The optimized pipeline with hard triplet mining and custom transformations effectively addresses fish re-identification challenges in EM systems, with Vision Transformers showing superior performance over traditional CNNs for this task.
...
TL;DR: SAM-Body4D is a training-free framework for temporally consistent and occlusion-robust human mesh recovery from videos, leveraging video segmentation and occlusion-aware refinement without retraining.
...
Method: 1) Generate identity-consistent masklets using promptable video segmentation; 2) Refine masklets with Occlusion-Aware module to recover missing regions; 3) Use refined masklets to guide SAM 3D Body for consistent mesh trajectories; 4) Padding-based parallel strategy for efficient multi-human inference.

Result: SAM-Body4D achieves improved temporal stability and robustness in challenging in-the-wild videos without any retraining, demonstrating effectiveness in handling occlusions and maintaining consistency.

Conclusion: The proposed training-free framework successfully addresses temporal inconsistency and occlusion issues in video-based human mesh recovery by leveraging human continuity and video segmentation, offering a practical solution for real-world applications.
...
TL;DR: OneClip-RAG: A one-shot video-clip based retrieval augmentation method that enables MLLMs to process long videos efficiently by using query-guided video chunking and progressive training.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) have excessive memory overhead and can only process videos with limited frames, creating a need for efficient long-video understanding methods.

Method: Proposes OneClip-RAG with: 1) Query-guided video chunking algorithm that unifies clip chunking and cross-modal retrieval in one step, 2) SynLongVideo dataset creation for instruction following, 3) Progressive training regime, 4) Plug-and-play integration with existing MLLMs.

Result: Boosts InternLV2 8B and Qwen2-VL 7B to GPT-4o level on MLVU benchmark; enables LLaVA-Video to understand hour-long videos in <2.2 minutes on single 4090 GPU; shows superior efficiency and performance gains over baseline MLLMs.

Conclusion: OneClip-RAG provides an effective and efficient solution for long-video understanding in MLLMs, addressing memory limitations while maintaining knowledge integrity and semantic coherence through innovative retrieval augmentation techniques.
...
<details>
  <summary>Details</summary>
Motivation: 6D pose estimation in industrial bin-picking is challenging due to occlusions, reflections, and textureless parts in densely packed environments. Existing methods struggle with these conditions, creating a need for robust depth-only approaches.

Method: Holistic depth-only approach that fuses multi-view depth maps into fine-grained 3D point clouds or sparse TSDF. Uses staged heatmap mechanism for scene-adaptive attention priors and density-aware sparse transformer blocks to handle occlusions and non-uniform 3D data distribution. Operates fully sparse with per-voxel voting strategy for simultaneous pose predictions.

Result: Validated on IPD and MV-YCB datasets, demonstrating competitive performance in heavily cluttered industrial and household bin picking scenarios. Enables high-resolution volumetric representations for fine geometric details.

Conclusion: The proposed sparse 3D approach effectively addresses challenges in close-range robotic applications, providing accurate 6D pose estimation in densely packed environments through scene-adaptive attention and sparse volumetric representations.
...
TL;DR: LapFM is a surgical foundation model for laparoscopic segmentation that uses hierarchical concept evolving pre-training on massive unlabeled images, achieving superior generalization across diverse surgical targets.


<details>
  <summary>Details</summary>
Motivation: Surgical segmentation faces challenges of annotation scarcity and semantic inconsistency across diverse procedures. Existing approaches are limited as domain adapters rather than true surgical foundation models, struggling to generalize across surgical variability.

Method: 1) Hierarchical Concept Evolving Pre-training with Laparoscopic Concept Hierarchy (LCH) using hierarchical mask decoder and parent-child query embeddings. 2) Confidence-driven Evolving Labeling that iteratively generates and filters pseudo-labels based on hierarchical consistency, creating LapBench-114K dataset from unlabeled images.

Result: LapFM significantly outperforms state-of-the-art methods, establishing new standards for granularity-adaptive generalization in universal laparoscopic segmentation. The model creates a benchmark of 114K image-mask pairs.

Conclusion: LapFM successfully evolves robust segmentation capabilities from massive unlabeled surgical images, addressing annotation scarcity and semantic inconsistency while achieving superior generalization across diverse surgical procedures.
...
Method: A unified learning-based framework that performs end-to-end color correction by jointly leveraging data from high-resolution RGB and auxiliary low-resolution MS sensors within a single model, refactored from two state-of-the-art image-to-image architectures.

Result: The approach improves color accuracy and stability, reducing error by up to 50% compared to RGB-only and MS-driven baselines, demonstrated through extensive experiments on a dedicated dataset created from public spectral datasets.

Conclusion: The proposed unified framework effectively integrates multispectral information throughout the color correction pipeline, producing coherent and color-accurate outputs while demonstrating flexibility across different architectures.
...
TL;DR: Subset selection methods for explaining vision models degrade under OOD conditions. The paper introduces an uncertainty-aware submodular optimization framework that improves explanation robustness in both ID and OOD settings.


<details>
  <summary>Details</summary>
Motivation: Current subset selection methods for explaining deep vision models work well in-distribution but degrade significantly under out-of-distribution conditions, producing unreliable explanations. There's a need for more robust explanation methods that maintain fidelity across different data distributions.

Method: Combines submodular subset selection with layer-wise gradient-based uncertainty estimation. Uses adaptive weight perturbations to estimate uncertainty, then guides submodular optimization with these estimates to ensure diverse and informative subset selection. No additional training or auxiliary models required.

Result: Empirical evaluations show the framework mitigates weaknesses of existing methods under OOD scenarios (redundant, unstable, uncertainty-sensitive explanations) and also yields improvements in ID settings. Demonstrates enhanced attribution and object-level interpretability.

Conclusion: Current subset-based explanation approaches have limitations under OOD conditions. Uncertainty-driven optimization can enhance attribution robustness and interpretability, paving the way for more transparent and trustworthy AI in real-world vision applications.
...
TL;DR: A lightweight football player tracking method combining SAM with CSRT trackers and jersey color-based re-identification achieves 7.6-7.7 FPS with stable memory usage, maintaining high tracking success in crowded scenes but struggling with long-term occlusions.


<details>
  <summary>Details</summary>
Motivation: Football player tracking faces challenges from frequent occlusions, similar player appearances, and rapid motion in crowded scenes, requiring robust yet efficient tracking solutions.

Method: A team-aware tracking system combining Segment Anything Model (SAM) for precise initialization, CSRT trackers for continuous tracking, and HSV histogram-based appearance models for jersey color-based re-identification to improve occlusion recovery.

Result: Achieves 7.6-7.7 FPS with stable memory usage (~1880 MB), maintains 100% tracking success in light occlusions and 90% in crowded penalty-box scenarios, recovers 50% of heavy occlusions, but struggles with long-term occlusions (only 8.66% re-acquisition success).

Conclusion: Classical tracker-based methods work well with continuous visibility but require stronger re-identification mechanisms for extended absences; the approach offers practical guidelines for deploying football tracking systems under resource constraints.
...
TL;DR: ContextDrag is a new drag-based image editing method that leverages contextual information from reference images to preserve fine-grained texture details and improve editing coherence without needing finetuning or inversion.
...
Method: ContextDrag introduces two key innovations: 1) Context-preserving Token Injection (CTI) that injects noise-free reference features via Latent-space Reverse Mapping algorithm, and 2) Position-Consistent Attention (PCA) that re-encodes reference tokens with overlap-aware masking to eliminate irrelevant feature interference.
...
Conclusion: ContextDrag provides a new paradigm for drag-based editing that effectively leverages contextual modeling capabilities of editing models to achieve precise drag control while preserving semantic and texture consistency.
...
TL;DR: Visionary is a web-native platform for real-time 3D Gaussian Splatting rendering with dynamic neural processing, offering plug-and-play algorithms and superior efficiency through GPU-based sorting.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS viewer solutions are fragmented, heavy, or constrained by legacy pipelines, resulting in high deployment friction and limited support for dynamic content and generative models.
...
Result: Achieves superior rendering efficiency compared to current Web viewers due to GPU-based primitive sorting, supports multiple variants (MLP-based 3DGS, 4DGS, neural avatars, style transformation), and enables "click-to-run" browser experience.
...
TL;DR: PCI is a training-free framework that analyzes when specific concepts form during diffusion model generation by measuring concept insertion success across timesteps.


<details>
  <summary>Details</summary>
Motivation: Current evaluation focuses on final outputs, but understanding the dynamic generation process is crucial for model reliability, predictability, and controllability. The paper aims to determine when noise transforms into specific concepts during denoising.

Method: Proposes Prompt-Conditioned Intervention (PCI) - a model-agnostic framework that analyzes Concept Insertion Success (CIS), defined as the probability that a concept inserted at a given timestep is preserved in the final image.

Result: PCI reveals diverse temporal behaviors across diffusion models, showing certain phases are more favorable to specific concepts. Provides actionable insights for text-driven image editing with stronger edits balancing semantic accuracy and content preservation.

Conclusion: PCI offers a novel way to analyze concept dynamics in diffusion models without model access or training, enabling better understanding of generation processes and more effective image editing interventions.
...
<details>
  <summary>Details</summary>
Motivation: Existing on-the-fly 3DGS methods using monocular RGB streams fail to achieve complete 3D coverage due to limited field of view. Multi-camera rigs address this limitation but lack real-time reconstruction frameworks.
...
Conclusion: The method enables efficient, real-time 3D reconstruction from multi-camera rigs with complete 3D coverage, addressing limitations of monocular approaches while maintaining high fidelity and speed.
...
<details>
  <summary>Details</summary>
Motivation: Multi-modal large reasoning models (MLRMs) pose serious privacy risks by inferring precise geographic locations from personal images through hierarchical reasoning. Existing privacy protection techniques designed for perception-based models are ineffective against MLRMs' sophisticated multi-step reasoning processes.
...
TL;DR: NoisyCLIP enables early detection of text-image misalignment during diffusion model generation by measuring semantic alignment in noisy latent space, reducing computational cost by 50% while maintaining 98% of CLIP alignment performance.


<details>
  <summary>Details</summary>
Motivation: Current conditional diffusion models suffer from misalignment and hallucinations, requiring expensive post-generation alignment checks. The paper hypothesizes that misalignments can be detected early in the denoising process, enabling real-time assessment without waiting for complete generation.

Method: Proposes NoisyCLIP, which measures semantic alignment in the noisy latent space during the reverse diffusion process. It's the first method to explore prompt-to-latent misalignment detection using dual encoders during image generation.
...
Conclusion: Early misalignment detection in noisy latent space is feasible and efficient, providing significant computational savings while maintaining alignment quality, enabling practical real-time assessment during diffusion model generation.
...
TL;DR: OCCDiff uses latent diffusion in occupancy function space for robust 3D building reconstruction from LiDAR point clouds with varying densities and noise.


<details>
  <summary>Details</summary>
Motivation: Building reconstruction from LiDAR point clouds faces challenges with varying point densities and noise interference, making it difficult to accurately capture building surfaces.

Method: Proposes OCCDiff combining latent diffusion with function autoencoder for continuous occupancy functions. Includes point encoder for conditioning, multi-modal features, and multi-task training for robust feature learning.

Result: Generates physically consistent samples with high fidelity to target distribution and exhibits robustness to noisy data.

Conclusion: OCCDiff effectively addresses building reconstruction challenges by leveraging latent diffusion in occupancy function space, providing flexible high-quality 3D profiles under diverse resolution conditions.
...
TL;DR: Self-Calling Chain-of-Thought (sCoT) reformulates multimodal reasoning as language-only CoT with self-calling to avoid explicit modality interleaving, improving training efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Optimizing interleaved multimodal Chain-of-Thought (iMCoT) through reinforcement learning is challenging due to reliance on scarce high-quality reasoning data, and explicit modality interleaving reduces training efficiency.

Method: Proposes sCoT where a main agent decomposes visual reasoning tasks into atomic subtasks and invokes parameter-sharing subagents (virtual replicas) to solve them in isolated context, using group-relative policy optimization to reinforce effective reasoning behavior.

Result: On HR-Bench 4K, sCoT improves overall reasoning performance by up to 1.9% with ~75% fewer GPU hours compared to strong baseline approaches.

Conclusion: sCoT provides an effective and efficient visual reasoning paradigm that avoids explicit modality interleaving while maintaining strong performance through self-calling architecture and group-relative policy optimization.
...
Method: Introduces progressive reparameterization strategy that gradually replaces dense feed-forward network blocks with compact Parameterized Hypercomplex Multiplication (PHM) layers, using residual interpolation schedule with lightweight reconstruction and knowledge distillation losses.
...
Result: MVP significantly boosts performance across various models and benchmarks: UI-TARS-1.5-7B to 56.1%, GTA1-7B to 61.7%, Qwen3VL-8B-Instruct to 65.3%, and Qwen3VL-32B-Instruct to 74.0% on ScreenSpot-Pro benchmark.

Conclusion: MVP effectively addresses coordinate prediction instability in GUI grounding by leveraging multi-view inference and clustering, demonstrating substantial performance improvements across different models without requiring additional training.
...
TL;DR: Photo3D is a framework that enhances photorealistic 3D generation by using GPT-4o-Image generated images to overcome limitations in real-world 3D data, with structure-aligned multi-view synthesis and detail enhancement techniques.


<details>
  <summary>Details</summary>
Motivation: Current 3D-native generators produce reliable geometry but lack realistic appearances due to insufficient diverse, high-quality real-world 3D assets with rich texture details. This limitation stems from difficulties in capturing real-world data at different scales, with non-rigid motions, and scanner precision constraints.

Method: 1) Uses GPT-4o-Image generated images as data source; 2) Designs structure-aligned multi-view synthesis pipeline to address multi-view inconsistency; 3) Constructs detail-enhanced multi-view dataset paired with 3D geometry; 4) Implements realistic detail enhancement scheme with perceptual feature adaptation and semantic structure matching; 5) Provides dedicated training strategies for both geometry-texture coupled and decoupled 3D-native generation paradigms.

Result: Photo3D generalizes well across diverse 3D-native generation paradigms and achieves state-of-the-art photorealistic 3D generation performance, demonstrating effectiveness in enhancing appearance realism while preserving structural consistency.

Conclusion: Photo3D successfully bridges the gap between 3D geometry generation and photorealistic appearance by leveraging AI-generated image data and novel consistency-preserving techniques, offering a generalizable solution for advancing photorealistic 3D generation across different paradigms.
...
TL;DR: Fast-ARDiff is a unified AR-diffusion framework that accelerates both AR speculative decoding and diffusion decoding through entropy-informed strategies and joint optimization.


<details>
  <summary>Details</summary>
Motivation: Current AR-diffusion hybrid models suffer from high latency due to sequential AR generation and iterative denoising, creating a bottleneck that needs to be addressed.

Method: Uses entropy-informed speculative strategy to align draft and target model entropy characteristics, integrates diffusion into end-to-end framework with dynamic scheduler, and employs joint distillation with trajectory and distribution matching for stable training.
...
Conclusion: Fast-ARDiff successfully addresses the latency bottleneck in AR-diffusion hybrids through unified optimization, achieving significant speed improvements while maintaining quality.
...
TL;DR: Proposes a novel quaternion Wasserstein distance and Wasserstein quaternion GAN for color image generation, addressing chromatic aberration by modeling color channel correlations.


<details>
  <summary>Details</summary>
Motivation: Existing color image generation models ignore correlations among color channels (leading to chromatic aberration), and lack systematic theory for measuring color image data distributions.

Method: Defines new quaternion Wasserstein distance with dual theory; develops quaternion linear programming with strong duality using quaternion convex set separation theorem and Farkas lemma; proposes Wasserstein quaternion GAN.

Result: The proposed model surpasses both standard/quaternion GANs and Wasserstein GAN in generation efficiency and image quality.

Conclusion: Quaternion Wasserstein distance effectively models color channel correlations, leading to improved color image generation with better efficiency and quality.
...
Method: Derives explicit expression of fixed point from ideal inversion step, then introduces error approximation using calculable error from previous step to approximate unknown error at current step, creating iteration-free unbiased estimator.
...
Conclusion: Proposed iteration-free fixed-point estimator provides efficient and effective solution for diffusion inversion, overcoming computational limitations of iterative methods while maintaining reconstruction quality.
...
TL;DR: SSCATeR: A sparse scatter-based convolution algorithm with temporal data recycling that reduces LiDAR processing time by up to 6.61x by focusing only on changing regions between frames.


<details>
  <summary>Details</summary>
Motivation: To reduce computational overhead in LiDAR-based object detection by exploiting the temporal continuity of LiDAR scanning and focusing processing only on regions where point cloud data changes between frames.

Method: Uses sliding time windows with short strides, stores convolution results between passes, extends scatter-based convolutions to allow data reuse, and treats LiDAR data as a continuous stream to process only changing parts of point clouds.

Result: Achieves identical feature maps to traditional sparse convolution techniques while reducing processing time by up to 6.61-fold, with significant reduction in convolution operations per forward pass.

Conclusion: SSCATeR enables efficient real-time LiDAR processing by exploiting temporal sparsity and data recycling, maintaining accuracy while dramatically improving computational efficiency for object detection.
...
<details>
  <summary>Details</summary>
Motivation: Understanding how the human brain represents visual concepts remains challenging due to complex brain signals, vast concept space, and limitations of small-scale studies relying on manual inspection without systematic validation.

Method: Two-stage framework: 1) Discover interpretable patterns in fMRI activity through unsupervised data-driven decomposition methods; 2) Explain patterns by identifying natural images that elicit them and generating natural-language descriptions. Automated pipeline tests multiple candidate explanations, assigns reliability scores, and selects most consistent descriptions.
...
Conclusion: The automated, large-scale approach enables systematic discovery and explanation of visual representations across human cortex, overcoming limitations of traditional small-scale studies.
...
TL;DR: A modular neural ISP framework that processes raw images with full control over intermediate stages, enabling high-quality rendering, editing flexibility, and generalization to different cameras.


<details>
  <summary>Details</summary>
Motivation: Prior neural ISP designs lack modularity and control over intermediate stages, limiting scalability, debuggability, generalization to different cameras, and flexibility for user-preference styles.

Method: Modular neural ISP framework with intermediate stage control, learning-based approach with variants of different capacities (0.5M to 3.9M parameters), and user-interactive photo-editing tool integration.

Result: Achieves competitive qualitative and quantitative results across multiple test sets, enables diverse editing operations and picture styles, supports unlimited post-editable re-rendering, and generalizes well to unseen cameras.

Conclusion: The modular neural ISP design provides superior control, scalability, and flexibility compared to prior approaches, making it suitable for practical photo-editing applications while maintaining high rendering quality.
...
TL;DR: Proposes an adaptive pseudo-labeling method for continual test-time adaptation in semantic segmentation that dynamically adjusts thresholds based on class and instance confidence distributions, outperforming SOTA methods across multiple scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods use fixed or batch-level thresholds that don't account for varying difficulty across classes and instances, which is especially problematic for dense, multi-class semantic segmentation where each image requires complex predictions.

Method: Adaptively adjusts pseudo labels to reflect confidence distribution within each image and dynamically balances learning toward classes most affected by domain shifts, using fine-grained, class- and instance-aware adaptation.

Result: Extensive experiments across eight CTTA and TTA scenarios (including synthetic-to-real and long-term shifts) show consistent outperformance of state-of-the-art techniques, setting new standards for semantic segmentation under evolving conditions.

Conclusion: The proposed adaptive approach produces more reliable supervision and mitigates error accumulation throughout continual adaptation, demonstrating superior performance for semantic segmentation in continuously evolving domains.
...
TL;DR: HiGINE is a hierarchical graph-based approach that uses multiplex immunofluorescence images of tumor microenvironment to predict lung cancer patient survival and improve risk stratification.


<details>
  <summary>Details</summary>
Motivation: The tumor microenvironment contains valuable prognostic biomarkers, but current methods fail to capture complex cellular interactions needed for accurate survival prediction and risk stratification in lung cancer.
...
Result: The model demonstrates improved risk stratification, robustness, and generalizability when validated on two public datasets, showing enhanced performance in predicting patient survival (short vs. long).

Conclusion: HiGINE effectively leverages complex cellular interactions in the tumor microenvironment through hierarchical graph-based analysis and multimodal fusion, providing a promising approach for enhanced prognostic assessment in lung cancer.
...
TL;DR: Automated system for generating unobstructed surgical videos by aligning multiple camera views and selecting the least occluded perspective, eliminating manual post-processing.


<details>
  <summary>Details</summary>
Motivation: Surgical videos are valuable for education/research but often obstructed by surgeons blocking the view. Current multi-camera setups require manual alignment when lighting systems are moved, making the process labor-intensive.

Method: Proposed method automatically detects when lighting system moves, realigns camera frames, and selects the camera with minimal occlusion to generate consistent surgical field videos from a fixed perspective.

Result: Surgeon user study showed videos from proposed method were superior for confirming surgical area and viewing comfort. Also demonstrated improved video quality over existing techniques and assessed surgeon preferences for different synthesis options.

Conclusion: Fully automated alignment and view synthesis system successfully generates high-quality, unobstructed surgical videos, addressing occlusion issues and eliminating manual post-processing labor.
...
TL;DR: Deep learning models (YOLOv8s & MobileNetV3L) applied to pollen detection/classification in optical & holographic microscopy, achieving 91.3% mAP50 detection & 97% classification accuracy on optical images, with performance improvements on holographic images through dataset expansion techniques.
...
Method: Used YOLOv8s for object detection and MobileNetV3L for classification, evaluated across optical and holographic microscopy images. Applied dataset expansion techniques including automated labeling and bounding box area enlargement to improve performance on holographic images.

Result: On optical images: 91.3% mAP50 for detection and 97% overall accuracy for classification. On holographic images: initial performance was low (2.49% mAP50 detection, 42% classification), improved to 13.3% mAP50 detection and 54% classification accuracy after dataset expansion techniques.

Conclusion: Deep learning techniques can be successfully paired with cost-effective lensless digital holographic microscopy devices for image classification tasks, demonstrating potential for automated pollen analysis in veterinary cytology applications.
...
TL;DR: CLIP's template-sample similarity introduces bias that reduces classification accuracy. The paper proposes using empty prompts to capture unbiased template features and offset this bias through pre-training and fine-tuning stages.


<details>
  <summary>Details</summary>
Motivation: The authors identify that CLIP's few-shot learning performance is compromised by template-sample similarity bias, where the model relies on template proximity rather than true sample-to-category alignment, reducing both accuracy and robustness.

Method: A two-stage framework: 1) Pre-training uses empty prompts (textual inputs conveying "emptiness" without category info) to reveal and reduce template-induced bias within CLIP encoder. 2) Few-shot fine-tuning employs a bias calibration loss to enforce correct alignment between images and categories.

Result: Experiments across multiple benchmarks show the template correction method significantly reduces performance fluctuations caused by TSS bias, yielding higher classification accuracy and stronger robustness.

Conclusion: The proposed framework effectively addresses template-sample similarity bias in CLIP, improving few-shot learning performance by decoupling template bias and ensuring the model focuses on relevant visual cues rather than template proximity.
...
TL;DR: OpenMonoGS-SLAM: First monocular SLAM framework combining 3D Gaussian Splatting with open-set semantic understanding using visual foundation models, requiring no depth sensors or semantic ground truth.
...
Method: Unifies 3D Gaussian Splatting with open-set semantics using Visual Foundation Models: MASt3R for visual geometry, SAM and CLIP for open-vocabulary semantics. Uses self-supervised learning without depth input or 3D semantic ground truth. Includes a memory mechanism for managing high-dimensional semantic features to construct Gaussian semantic feature maps.

Result: Achieves performance comparable to or surpassing existing baselines in both closed-set and open-set segmentation tasks, without relying on depth maps or semantic annotations.

Conclusion: OpenMonoGS-SLAM successfully demonstrates the feasibility of combining monocular SLAM with open-set semantic understanding using foundation models, enabling intelligent perception in open-world environments without specialized sensors or annotations.
...
TL;DR: A method that estimates metric depth from video blur patterns and dense trajectories using joint optical design, leveraging vision encoders, point trackers, and vision-language models for depth estimation and trajectory densification.


<details>
  <summary>Details</summary>
Motivation: Camera rotational dynamics during long-exposure capture cause perspective-based blur that varies with object depth, creating an opportunity to estimate depth from blur patterns in video streams.

Method: Uses off-the-shelf vision encoder and point tracker to extract video information, estimates depth via windowed embedding and multi-window aggregation, and densifies sparse optical trajectories using a vision-language model through joint optical design algorithm.
...
Conclusion: The proposed method effectively leverages depth-dependent blur patterns in video to estimate metric depth and reconstruct dense trajectories, demonstrating robust performance in real-world handheld shooting scenarios.
...
TL;DR: A unified aerial VLN framework using only monocular RGB images and language instructions, treating navigation as next-token prediction with multi-task learning, keyframe selection, and action merging to address redundancy and supervision imbalance.


<details>
  <summary>Details</summary>
Motivation: Existing aerial VLN methods require panoramic images, depth inputs, or odometry, increasing system cost and complexity, which hinders practical deployment on lightweight UAVs. There's a need for simpler, more practical solutions using only monocular RGB observations.

Method: 1) Formulates navigation as next-token prediction problem; 2) Uses prompt-guided multi-task learning to jointly optimize spatial perception, trajectory reasoning, and action prediction; 3) Keyframe selection strategy to reduce visual redundancy; 4) Action merging and label reweighting to mitigate long-tailed supervision imbalance.

Result: Achieves strong results on Aerial VLN benchmark under monocular RGB-only setting, significantly outperforming existing RGB-only baselines and narrowing performance gap with state-of-the-art panoramic RGB-D methods. Works well in both seen and unseen environments.

Conclusion: The proposed framework demonstrates that effective aerial VLN is possible with only monocular RGB observations, reducing system complexity and cost while maintaining competitive performance, enabling more practical deployment on lightweight UAVs.
...
TL;DR: CoIG framework makes image generation transparent by decomposing prompts into step-by-step instructions, enabling monitoring and improving reliability.


<details>
  <summary>Details</summary>
Motivation: Current image generation models are opaque "black boxes" that limit human observation, intervention, and control, making it difficult to ensure reliability and safety. Their non-human-like workflows also hinder interpretability.
...
Result: CoIG substantially enhances quantitative monitorability while achieving competitive compositional robustness compared to baseline models. It introduces two novel metrics: CoIG Readability (clarity of intermediate steps) and Causal Relevance (impact of each step on final image).

Conclusion: CoIG brings transparency and interpretability to image generation similar to how Chain-of-Thought improved LLMs, making the process monitorable, reliable, and more human-like while being model-agnostic.
...
TL;DR: C-DIRA is a lightweight driver behavior recognition framework that uses dynamic ROI routing and adversarial learning to achieve efficient, accurate, and domain-invariant performance on edge devices.


<details>
  <summary>Details</summary>
Motivation: Existing lightweight models for driver distraction recognition fail to capture fine-grained behavioral cues and have poor generalization to unseen drivers/conditions, while ROI-based methods increase computational costs, creating a need for efficient yet accurate solutions.
...
Conclusion: C-DIRA effectively balances compactness, efficiency, and generalization for real-time driver behavior recognition on edge devices, addressing key challenges in lightweight model design for in-vehicle applications.
...
TL;DR: MNAME is a plug-and-play training framework for denoising generative models that uses a memory bank mechanism to provide abundant negative samples for contrastive learning without external encoders, achieving state-of-the-art FID of 2.40 on ImageNet-256.


<details>
  <summary>Details</summary>
Motivation: Denoising generative models (diffusion, flow-matching) have high training costs and inefficient representation learning. Existing approaches that inject discriminative representations via auxiliary alignment rely on external pre-trained encoders, introducing overhead and domain shift issues.

Method: Proposes MNAME framework with memory bank mechanism that maintains a large, dynamically updated queue of negative samples across training iterations, decoupling negative sample count from mini-batch size. Uses low-dimensional projection head to minimize memory/bandwidth overhead. No external encoders needed.
...
Conclusion: MNAME offers self-contained training without dependency on pretrained vision foundation models, introduces no additional parameters/computational cost during inference, and enables faster convergence with superior generative quality.
...
TL;DR: Proposes CSCon, a dual-branch center-surrounding contrastive learning framework for 3D point clouds that addresses limitations of existing generative SSL methods by better capturing local geometric details through patch-level contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing 3D SSL is dominated by generative MAE methods which struggle to capture high-level discriminative features, while contrastive methods effective in 2D images are scarce for 3D data. Direct application of 2D contrastive methods fails to learn 3D local details effectively.

Method: Dual-branch center-surrounding contrast framework: applies masking to center and surrounding parts separately to create center-biased and surrounding-biased representations, combined with patch-level contrastive loss to enhance both high-level information and local sensitivity.

Result: Under FULL and ALL protocols, achieves comparable performance to generative methods; under MLP-LINEAR, MLP-3, and ONLY-NEW protocols, attains SOTA results, surpassing cross-modal approaches. Specifically, under MLP-LINEAR protocol, outperforms Point-MAE baseline by 7.9%, 6.7%, and 10.3% on three ScanObjectNN variants.

Conclusion: CSCon effectively addresses the limitations of existing 3D SSL methods by combining dual-branch center-surrounding masking with patch-level contrastive learning, achieving superior discriminative feature learning and local detail capture for 3D point clouds.
...
TL;DR: MoSAIC-ReID is a Mixture-of-Experts framework that quantifies the importance of pedestrian attributes for person re-identification, providing interpretability through attribute-specific experts and controlled attribution analysis.


<details>
  <summary>Details</summary>
Motivation: Current person re-identification methods achieve high accuracy but lack transparency about which semantic attributes (clothing colors, accessories, etc.) the models actually rely on for identification.

Method: Proposes MoSAIC-ReID using LoRA-based experts (each linked to a single attribute) with an oracle router for controlled attribution analysis, enabling systematic quantification of attribute importance.

Result: Achieves competitive performance on Market-1501 and DukeMTMC datasets when attribute annotations are available; provides large-scale quantitative analysis showing clothing colors and intrinsic characteristics contribute most strongly, while infrequent cues like accessories have limited effect.

Conclusion: Offers a principled framework for interpretable ReID and highlights requirements for integrating explicit semantic knowledge in practice, advancing transparency in person re-identification systems.
...
<details>
  <summary>Details</summary>
Motivation: Foundation models show strong generalization in monocular depth estimation but face two challenges when applied to Full Surround Monocular Depth Estimation (FSMDE): high computational cost limiting real-time performance, and difficulty estimating metric-scale depth since they typically predict only relative depth.

Method: Uses knowledge distillation to transfer depth knowledge from foundation models to lightweight FSMDE networks. Introduces: 1) Cross-interaction knowledge distillation that distills scale-invariant depth bin probabilities while guiding metric-scale depth bin center inference from ground truth, and 2) View-relational knowledge distillation that encodes structural relationships among adjacent camera views to enhance cross-view depth consistency.

Result: Experiments on DDAD and nuScenes datasets demonstrate effectiveness compared to conventional supervised methods and existing knowledge distillation approaches. Achieves favorable trade-off between performance and efficiency, meeting real-time requirements.

Conclusion: The proposed knowledge distillation strategy successfully addresses computational cost and metric-scale depth challenges in FSMDE, enabling real-time performance while maintaining accuracy through effective knowledge transfer from foundation models.
...
TL;DR: This paper explores using SAM 3 for training-free Open-Vocabulary Semantic Segmentation in remote sensing, achieving promising results through mask fusion and presence score filtering.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based methods for Open-Vocabulary Semantic Segmentation face challenges in precise localization and require complex pipelines, especially in remote sensing with dense small targets. SAM 3's unified segmentation and recognition framework offers potential for this task.

Method: 1) Implement mask fusion strategy combining SAM 3's semantic segmentation head and Transformer decoder outputs for better land coverage. 2) Use presence score from presence head to filter out non-existent categories, reducing false positives from large vocabularies and patch-level processing.

Result: The simple adaptation achieves promising performance on extensive remote sensing datasets, demonstrating SAM 3's potential for remote sensing Open-Vocabulary Semantic Segmentation without any training.

Conclusion: SAM 3 shows strong potential for training-free Open-Vocabulary Semantic Segmentation in remote sensing applications, with mask fusion and presence filtering effectively addressing challenges in this domain.
...
TL;DR: The paper proposes a distribution-based framework for individual fairness in skin lesion classification, treating skin tone as continuous rather than categorical, using KDE and statistical distance metrics to quantify disparities, and introducing distance-based reweighting to address underrepresentation.


<details>
  <summary>Details</summary>
Motivation: Current fairness research in medical imaging uses coarse subgroup categories that overlook individual-level variations, risking obscured biases for outliers within subgroups. Skin color discrimination is historically significant but current approaches fail to capture continuous nature of skin tone.

Method: Treats skin tone as continuous attribute using kernel density estimation (KDE) to model distribution. Compares 12 statistical distance metrics to quantify disparities between skin tone distributions. Proposes distance-based reweighting (DRW) loss function to correct underrepresentation in minority tones.

Result: Experiments with CNN and Transformer models show: (1) categorical reweighting limitations in capturing individual-level disparities, (2) superior performance of distribution-based reweighting, particularly with Fidelity Similarity (FS), Wasserstein Distance (WD), Hellinger Metric (HM), and Harmonic Mean Similarity (HS).

Conclusion: Establishes robust methodology for advancing individual-level fairness in dermatological AI systems, with broader implications for handling sensitive continuous attributes in medical image analysis beyond categorical approaches.
...
TL;DR: A novel Sign Language Spotting task is introduced for detecting query signs within continuous sign sequences using pose keypoints, achieving 61.88% accuracy on WSLP 2025 dataset.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored problem of sign-to-sign retrieval (detecting specific signs within continuous sign sequences), which is crucial for bridging communication gaps between deaf and hearing communities.

Method: End-to-end model using pose keypoints extracted from sign videos with encoder-only backbone and binary classification head, avoiding intermediate gloss recognition or text-based matching.
...
Conclusion: Pose-based framework provides effective solution for Sign Language Spotting with reduced computational cost and visual noise, establishing foundation for future sign language retrieval research.
...
TL;DR: A novel workflow combining 3D rendering in Blender with constrained diffusion models to generate photorealistic synthetic mushroom images for computer vision training, achieving state-of-the-art segmentation performance with only synthetic data.


<details>
  <summary>Details</summary>
Motivation: Industrial mushroom cultivation needs computer vision for monitoring and harvesting, but creating large annotated real-world datasets is expensive. Synthetic data offers scalability but often lacks realism for real-world generalization.

Method: Integrates 3D rendering in Blender with a constrained diffusion model to automatically generate high-quality annotated, photorealistic synthetic images of Agaricus Bisporus mushrooms. Preserves full control over 3D scene configuration and annotations while achieving photorealism without specialized graphics expertise.

Result: Released two synthetic datasets (6,000 images each, depicting over 250k mushroom instances). Mask R-CNN models trained on synthetic data achieved state-of-the-art segmentation performance in zero-shot testing on two independent real-world datasets (F1 = 0.859 on M18K benchmark).

Conclusion: The pipeline successfully generates photorealistic synthetic training data that generalizes well to real-world scenarios. Although demonstrated on Agaricus Bisporus mushrooms, the method can be adapted to other mushroom species or agricultural domains like fruit and leaf detection.
...
TL;DR: Proposes skewness-guided pruning for multimodal Swin Transformers in federated learning, achieving 36% model size reduction without accuracy loss for medical imaging on edge devices.


<details>
  <summary>Details</summary>
Motivation: Medical AI models are too large for edge devices, and privacy constraints prevent centralized data management, requiring both model compression and federated learning solutions.

Method: Skewness-guided pruning method that selectively prunes Multi-Head Self-Attention and Multi-Layer Perceptron layers based on statistical skewness of their output distributions in a multimodal Swin Transformer, validated in horizontal federated learning environment.

Result: Approximately 36% model size reduction with no loss in accuracy on the compact Swin Transformer, demonstrating feasibility of efficient model compression and privacy-preserving distributed learning.

Conclusion: The proposed approach enables deployment of multimodal medical AI on edge devices while maintaining performance and addressing privacy concerns through federated learning.
...
TL;DR: Wan-Move is a scalable framework for precise motion control in video generation using dense point trajectories as motion guidance, integrated into existing image-to-video models without architecture changes.
...
Method: Represents object motions with dense point trajectories for fine-grained control, projects trajectories into latent space, propagates first frame features along trajectories to create aligned spatiotemporal feature maps, and integrates these as motion guidance into off-the-shelf image-to-video models without architectural changes.

Result: Generates 5-second, 480p videos with motion controllability rivaling commercial systems like Kling 1.5 Pro's Motion Brush. Introduces MoveBench benchmark with diverse content, larger data volume, longer durations, and high-quality annotations. Extensive experiments show superior motion quality.

Conclusion: Wan-Move provides a simple, scalable solution for precise motion control in video generation by making original condition features motion-aware, eliminating need for auxiliary motion encoders, and enabling easy fine-tuning of base models while achieving commercial-grade performance.
...
TL;DR: Self-refining diffusion framework uses XAI-based flaw detection to identify artifacts in generated images, then amplifies noise in flawed regions during forward process and focuses on them during reverse process to improve image quality.


<details>
  <summary>Details</summary>
Motivation: Despite remarkable success of diffusion models in image synthesis, addressing artifacts and unrealistic regions remains a critical challenge that needs to be solved to improve generation quality.

Method: Proposes self-refining diffusion framework with XAI-based flaw highlighter that produces flaw activation maps (FAMs) to identify artifacts and unrealistic regions. Uses FAMs to amplify noise in flawed regions during forward process and focus on these regions during reverse process.

Result: Achieves up to 27.3% improvement in Fréchet inception distance across various diffusion-based models, with consistent strong performance on diverse datasets. Shows robust effectiveness across different tasks including image generation, text-to-image generation, and inpainting.

Conclusion: Explainable AI techniques can extend beyond interpretability to actively contribute to image refinement. The framework offers versatile and effective approach applicable to various diffusion models and tasks, significantly advancing image synthesis.
...
TL;DR: LoFA is a hypernetwork framework that predicts personalized LoRA weights in seconds by learning structured distribution patterns in parameter changes, outperforming traditional LoRA optimization.
...
Result: LoFA consistently predicts high-quality personalized priors within seconds across multiple tasks and user prompts, outperforming conventional LoRA that requires hours of processing.

Conclusion: LoFA bridges the gap between efficient personalization and quality adaptation by leveraging structured distribution patterns in LoRA parameter changes, enabling practical real-time model personalization.
...
TL;DR: MatteViT is a document shadow removal framework that uses spatial and frequency-domain information with high-frequency amplification and continuous luminance-based shadow matte to preserve text details while removing shadows.


<details>
  <summary>Details</summary>
Motivation: Document shadow removal is essential for digitized document clarity, and preserving high-frequency details (text edges, lines) is critical since shadows often obscure fine structures.

Method: Proposes MatteViT with two key strategies: 1) Lightweight high-frequency amplification module (HFAM) that decomposes and adaptively amplifies high-frequency components, and 2) Continuous luminance-based shadow matte generated using custom dataset and matte generator for precise spatial guidance from early stages.

Result: Achieves state-of-the-art performance on public benchmarks (RDD and Kligler), provides robust practical solution, and better preserves text-level details in downstream OCR tasks, improving recognition performance over prior methods.

Conclusion: MatteViT offers an effective document shadow removal framework that preserves fine-grained structural details while eliminating shadows, demonstrating practical value for real-world applications and downstream tasks like OCR.
...
TL;DR: Training-free Dual Hyperbolic Adapters (T-DHA) improves vision-language model adaptation by using hyperbolic space for hierarchical semantic relationships, achieving better few-shot recognition and domain generalization without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models suffer from performance degradation with domain changes and require substantial computational resources for fine-tuning in new domains, creating a need for more efficient adaptation methods.

Method: T-DHA characterizes vision-language relationships in hyperbolic space using the Poincaré ball model, leveraging exponential volume growth for hierarchical data structures, and combines this with negative learning for improved classification.
...
Conclusion: Hyperbolic space representation with negative learning provides more accurate and robust classification with fewer feature dimensions, offering an effective training-free adaptation method for vision-language models.
...
TL;DR: InfiniteVL combines sliding window attention with Gated DeltaNet to create a linear-complexity VLM that overcomes limitations of both window attention and linear attention, achieving competitive performance with 3.6× speedup and constant memory footprint.


<details>
  <summary>Details</summary>
Motivation: Window attention suffers performance degradation when sequence length exceeds window size, while linear attention underperforms on information-intensive tasks like OCR and document understanding. Need a solution that maintains competitive performance while addressing quadratic complexity and KV cache growth.

Method: Proposes InfiniteVL architecture that synergizes sliding window attention (SWA) with Gated DeltaNet for linear complexity. Uses three-stage training: distillation pretraining, instruction tuning, and long-sequence SFT. Achieves efficiency with less than 2% of training data compared to leading VLMs.

Result: Substantially outperforms previous linear-complexity VLMs and matches performance of leading Transformer-based VLMs. Achieves over 3.6× inference speedup vs FlashAttention-2 accelerated Transformers with constant latency/memory. Maintains 24 FPS real-time prefill in streaming video with long-term memory retention.

Conclusion: InfiniteVL successfully addresses limitations of both window and linear attention approaches, providing an efficient linear-complexity VLM that maintains competitive multimodal performance while enabling practical long-sequence applications with constant memory footprint.
...
Method: Theoretical analysis formalizing inductive biases needed for compositional generalization in both decoder-based (generative) and encoder-based (non-generative) methods. Empirical evaluation training various generative and non-generative methods on photorealistic image datasets, comparing their compositional generalization capabilities.

Result: Generative methods can enforce necessary inductive biases through decoder constraints and inversion, enabling compositional generalization. Non-generative methods generally cannot enforce these biases via regularization or architecture, often failing to generalize compositionally without large-scale pretraining or added supervision.

Conclusion: Generative approaches with decoder inversion are better suited for achieving compositional generalization in visual perception, supporting the hypothesis that human-level perception may require generative mechanisms.
...
TL;DR: Tri-Bench benchmark reveals VLMs struggle with geometric reasoning, especially with camera tilt and minority triangle types, despite explicit frame-of-reference hints.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models often fail under realistic scene changes, highlighting the need for verifiable geometric reasoning in trustworthy agentic AI. Current VLMs lack reliable spatial reasoning capabilities that are critical for real-world deployment.

Method: Created Tri-Bench - a compact benchmark of planar triangle problems testing relative geometric reasoning. Evaluated four recent VLMs with a single fixed prompt containing explicit guardrail describing surrounding square border (enabling correct answers via homography). Tested six simple tasks over binary and continuous targets, varying camera pose (planar vs. tilted) and scene context via object interference (10 everyday objects).

Result: Overall accuracy modest (~69% average, best ~75%, worst ~64%). Responses align better with 2D projections (~72% accuracy). All VLMs fail completely (~0% accuracy) on recognizing minority shape classes (equilateral, isosceles, right-angled triangles). Accuracy degrades by ~4.1% under camera tilt. Object interference has no significant effect on accuracy.

Conclusion: VLMs fail to correctly utilize explicit frame-of-reference hints and default to 2D image plane cues, demonstrating fundamental limitations in geometric reasoning capabilities that hinder trustworthy agentic AI deployment.
...
TL;DR: SOLI uses Siamese network architecture to optimize latent embeddings for lightweight, low-resolution image captioning, reducing computational overhead while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Image captioning is important for assisting visually impaired individuals and improving content management, but current transformer-based models are too heavyweight for low-resolution images and require significant computational resources, making retraining challenging.

Method: SOLI employs a Siamese network architecture with dual-pathway neural network structure to optimize latent embeddings for low-resolution images, enhancing efficiency and accuracy of image-to-text translation while minimizing computational overhead.

Result: The approach provides an efficient solution for lightweight, low-resolution image captioning that maintains performance while reducing computational demands, making it suitable for resource-constrained scenarios.

Conclusion: SOLI presents an effective solution for low-resolution image captioning that balances performance with computational efficiency, addressing the limitations of heavyweight transformer models in resource-constrained environments.
...
TL;DR: Enhanced VLM-based visual grounding in satellite imagery using structured localization mechanism with control tokens


<details>
  <summary>Details</summary>
Motivation: Vision-language models show promise for remote sensing but need improved spatial reasoning for precise object localization in complex satellite scenes

Method: Fine-tune pretrained VLM on instruction-following tasks, interface with dedicated grounding module using specialized control tokens for localization

Result: 24.8% relative improvement over previous methods on visual grounding benchmarks, consistently outperforms state-of-the-art on remote sensing tasks

Conclusion: Structured spatial reasoning integration into VLMs enables more reliable real-world satellite data analysis
...
TL;DR: A GPU-optimized rotation-invariant convolution framework that eliminates im2col overhead, reduces memory traffic via structured data sharing among rotated filters, and achieves significant speedups and energy savings while maintaining segmentation accuracy.


<details>
  <summary>Details</summary>
Motivation: Rotation invariance is crucial for precise object-level segmentation in UAV aerial imagery where targets have arbitrary orientations, but conventional convolution architectures lack rotation invariance, and existing rotation-invariant methods are computationally expensive with high memory traffic.

Method: Introduces a GPU-optimized rotation-invariant convolution framework that eliminates the traditional im2col step and exploits structured data sharing among symmetrically rotated filters to reduce memory traffic and computational redundancy, with generalization to arbitrary rotation angles.

Result: Achieves 20-55% faster training and 15-45% lower energy consumption than CUDNN, with up to 45% speedup and 41% energy savings on 256×256 inputs, and 32% speedup with 23% lower energy on 1024×1024 inputs. Integrated into U-Net, yields up to 6% accuracy improvement over non-rotation-aware baseline.

Conclusion: The proposed method provides an effective and highly efficient alternative to existing rotation-invariant CNN frameworks, enabling rotation-invariant segmentation with significantly reduced computational cost and energy consumption while maintaining accuracy.
...
TL;DR: VALOR is an annotation-free training framework that improves visual reasoning by combining LLM reasoning refinement via RL with VLM visual grounding enhancement through automated hard-negative mining, outperforming existing methods without ground truth labels.


<details>
  <summary>Details</summary>
Motivation: Visual reasoning requires both precise object grounding and understanding complex spatial relationships. Existing methods have limitations: language-only chain-of-thought approaches need large-scale supervised data, while program-synthesis approaches suffer from flawed logic and erroneous grounding without training.

Method: Uses AI-powered verifiers: (1) LLM verifier refines LLM reasoning via reinforcement learning, (2) VLM verifier strengthens visual grounding through automated hard-negative mining. Combines strengths of modern AI systems: language-only reasoning models for decomposing spatial queries, and vision specialist models improved via VLM critics.
...
Conclusion: VALOR provides an effective annotation-free training framework that enhances both reasoning and grounding in visual reasoning tasks by leveraging AI-powered verifiers, eliminating the need for ground truth labels while achieving superior performance.
...
TL;DR: UniLayDiff: A unified diffusion transformer model that handles all content-aware layout generation tasks (unconditional and various conditional) with a single end-to-end trainable framework, achieving SOTA performance.
...
Method: Treats layout constraints as a distinct modality and uses Multi-Modal Diffusion Transformer to capture interactions between background images, layout elements, and constraints. Integrates relation constraints through LoRA fine-tuning after pretraining on other tasks.
...
Conclusion: UniLayDiff provides a unified solution for diverse layout generation tasks with a single model, enhancing layout quality while addressing the challenge of handling multiple constraint types in graphic design automation.
...
<details>
  <summary>Details</summary>
Motivation: Existing image-to-3D generators have object-centric training that limits generalization to complex, large-scale scenes with faithful structure and texture. There's a need for methods that can reconstruct complete 3D scenes from single images with both good geometry and texture quality.

Method: EvoScene uses a self-evolving, training-free framework with three iterative stages: 1) Spatial Prior Initialization, 2) Visual-guided 3D Scene Mesh Generation, and 3) Spatial-guided Novel View Generation. It alternates between 2D and 3D domains, combining geometric reasoning from 3D generation models with visual knowledge from video generation models.

Result: Experiments show EvoScene achieves superior geometric stability, view-consistent textures, and unseen-region completion compared to strong baselines. It produces ready-to-use 3D meshes for practical applications across diverse scenes.

Conclusion: EvoScene successfully addresses the challenge of generating high-quality, textured 3D scenes from single images by leveraging complementary strengths of existing models through iterative refinement, enabling practical 3D scene reconstruction without additional training.
...
TL;DR: LiDAS is an active illumination system that dynamically optimizes headlight patterns to improve nighttime perception for autonomous vehicles, enabling daytime-trained models to work at night without retraining.


<details>
  <summary>Details</summary>
Motivation: Nighttime environments challenge camera-based perception systems because they passively rely on existing scene lighting. Current methods uniformly brighten scenes, which is inefficient and doesn't optimize for perception performance.

Method: LiDAS combines off-the-shelf visual perception models with high-definition headlights in a closed-loop system. It dynamically predicts optimal illumination fields that maximize downstream perception performance by decreasing light on empty areas and reallocating it to object regions.

Result: In real-world closed-loop driving scenarios, LiDAS achieved +18.7% mAP50 and +5.0% mIoU over standard low-beam at equal power. It maintained performance while reducing energy use by 40%. The system enables zero-shot nighttime generalization of daytime-trained models.

Conclusion: LiDAS offers a cost-effective solution for robust nighttime perception by turning existing headlights into active vision actuators. It complements domain-generalization methods and strengthens robustness without requiring model retraining.
...
TL;DR: UniT is a unified text restoration framework that integrates Diffusion Transformer, Vision-Language Model, and Text Spotting Module to recover high-quality images from degraded text inputs while reducing text hallucinations.


<details>
  <summary>Details</summary>
Motivation: Diffusion models provide strong generative priors for image restoration but often produce text hallucinations in text-centric tasks due to lack of explicit linguistic knowledge. Current methods need better integration of textual understanding for accurate text restoration.

Method: UniT combines three components: 1) Diffusion Transformer (DiT) backbone for strong representational power, 2) Vision-Language Model (VLM) to extract textual content from degraded images, and 3) Text Spotting Module (TSM) trained on diffusion features to generate intermediate OCR predictions. These components work iteratively during denoising - VLM provides textual guidance while TSM enables iterative refinement of this guidance.
...
Conclusion: UniT successfully addresses text hallucinations in diffusion-based image restoration by integrating explicit linguistic knowledge through iterative refinement between VLM and TSM, demonstrating superior text restoration capabilities compared to existing methods.
...
TL;DR: D4RT is a feedforward transformer model that jointly infers depth, spatio-temporal correspondence, and camera parameters from single videos for efficient 4D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Reconstructing complex geometry and motion of dynamic scenes from video is challenging; existing methods often involve heavy computation or multiple specialized decoders.

Method: Uses unified transformer architecture with novel querying mechanism that avoids dense per-frame decoding and multiple task-specific decoders, enabling flexible probing of 3D positions across space and time.

Result: Sets new state-of-the-art performance across wide spectrum of 4D reconstruction tasks; achieves lightweight, scalable design with remarkably efficient training and inference.

Conclusion: D4RT provides a simple yet powerful solution for dynamic scene reconstruction that outperforms previous methods while being computationally efficient and scalable.
...
TL;DR: Selfi improves VGGT's 3D reconstruction by adding geometric consistency through feature alignment, achieving SOTA in novel view synthesis and camera pose estimation.


<details>
  <summary>Details</summary>
Motivation: VGGT models lack explicit multi-view geometric consistency in their features, which limits their performance in 3D reconstruction tasks like novel view synthesis and pose estimation.

Method: Selfi trains a lightweight feature adapter using reprojection-based consistency loss, distilling VGGT outputs into a geometrically-aligned feature space that captures 3D spatial proximity.

Result: Achieves state-of-the-art performance in both novel view synthesis and camera pose estimation, demonstrating improved 3D feature consistency.

Conclusion: Feature alignment is a highly beneficial step for downstream 3D reasoning, transforming VGGT backbones into high-fidelity 3D reconstruction engines.
...
TL;DR: Astra is an interactive general world model that generates real-world futures for diverse scenarios with precise action interactions, using an autoregressive denoising architecture with temporal causal attention and action-aware adapters.


<details>
  <summary>Details</summary>
Motivation: While diffusion transformers have advanced video generation, world models capable of predicting long-horizon futures from past observations and actions remain underexplored, especially for general-purpose scenarios and various action forms.

Method: Proposes Astra with: 1) autoregressive denoising architecture with temporal causal attention, 2) noise-augmented history memory to balance responsiveness and coherence, 3) action-aware adapter for precise action control, and 4) mixture of action experts for heterogeneous action modalities.
