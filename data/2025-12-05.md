...
TL;DR: A novel video deepfake detection method that trains on synthetically generated videos with manipulated facial motion dependencies to improve generalization to unseen manipulations.


<details>
  <summary>Details</summary>
Motivation: Current deepfake detection methods struggle with generalization to unseen manipulations, especially in videos. Existing video methods focus on frame-to-frame instabilities but overlook the violation of natural motion dependencies between different facial regions.

Method: Proposes a synthetic video generation method that creates training data with subtle kinematic inconsistencies. Uses an autoencoder to decompose facial landmark configurations into motion bases, then manipulates these bases to break natural correlations in facial movements. These artifacts are introduced into pristine videos via face morphing.

Result: A network trained on this synthetic data learns to spot sophisticated biomechanical flaws and achieves state-of-the-art generalization results on several popular benchmarks.

Conclusion: The proposed approach effectively addresses the generalization challenge in video deepfake detection by focusing on biomechanical inconsistencies in facial motion, outperforming existing methods that only consider frame-to-frame instabilities.
...
TL;DR: OnSight Pathology is a platform-agnostic computer vision software that provides real-time AI inferences for digital pathology using continuous screen captures, enabling easy deployment on consumer PCs without complex integration.
...
Method: Developed OnSight Pathology as a platform-agnostic computer vision software that uses continuous custom screen captures to provide real-time AI inferences. It operates locally as a single executable file on consumer-grade PCs without complex software integration.

Result: Demonstrated utility using over 2,500 publicly available whole slide images across different slide viewers and clinical cases. Showed robustness across routine histopathological tasks including brain tumor classification, mitosis detection, and immunohistochemical stain quantification. Also demonstrated compatibility with live microscope camera feeds including personal smartphones.

Conclusion: OnSight Pathology can deliver real-time AI inferences across broad pathology pipelines, removing key barriers to AI adoption in histopathology by enabling cost-effective, secure deployment without complex integration requirements.
...
TL;DR: LAPA is an end-to-end transformer architecture for multi-camera point tracking that integrates appearance matching with geometric constraints using attention mechanisms, outperforming traditional decoupled pipelines.
...
Method: Uses transformer-based architecture with cross-view attention enhanced by geometric priors to establish soft correspondences, constructs 3D point representations via attention-weighted aggregation (instead of classical triangulation), and maintains temporal consistency through transformer decoder modeling long-range dependencies.
...
Conclusion: LAPA's unified end-to-end approach effectively addresses limitations of traditional decoupled pipelines by jointly reasoning across views and time, demonstrating superior performance in challenging multi-camera tracking scenarios.
...
TL;DR: PARSE is an unsupervised framework that learns hierarchical event structure from streaming video using multiscale recurrent predictors, where event boundaries emerge from prediction errors, achieving state-of-the-art performance on action segmentation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Humans naturally perceive continuous experience as a hierarchy of temporally nested events (fine-grained actions within coarser routines), but current computer vision models lack this ability to segment video both predictively and hierarchically without supervision.

Method: PARSE uses a unified framework with a hierarchy of recurrent predictors operating at different temporal granularities - lower layers model short-term dynamics while higher layers integrate longer-term context through attention-based feedback. Event boundaries emerge naturally as transient peaks in prediction error.

Result: PARSE achieves state-of-the-art performance among streaming methods on three benchmarks (Breakfast Actions, 50 Salads, Assembly 101) and rivals offline baselines in both temporal alignment (H-GEBD) and structural consistency (TED, hF1).

Conclusion: Predictive learning under uncertainty provides a scalable path toward human-like temporal abstraction and compositional event understanding, demonstrating that unsupervised hierarchical event segmentation can match human perception structures.
...
TL;DR: MoReGen is a physics-grounded text-to-video framework that integrates LLMs, physics simulators, and renderers to generate physically accurate videos, with MoReSet benchmark for evaluation.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video models generate photorealistic videos but struggle with physical validity and obeying physics principles, especially Newtonian motion. There's a need for models that can generate intent-aligned videos with proper physical coherence.

Method: MoReGen framework uses multi-agent LLMs, physics simulators, and renderers to generate reproducible, physically accurate videos in the code domain. Also introduces MoReSet benchmark with 1,275 human-annotated videos across 9 Newtonian phenomena classes, and proposes object-trajectory correspondence as evaluation metric.

Result: State-of-the-art T2V models struggle to maintain physical validity, while MoReGen establishes a principled approach toward physically coherent video synthesis. The MoReSet benchmark enables quantitative assessment of physical validity.

Conclusion: MoReGen provides a systematic framework for physics-grounded video generation, addressing the core challenge of physical coherence in T2V models. The work establishes foundations for physically accurate video synthesis through integrated simulation and evaluation approaches.
...
TL;DR: ReasonX uses MLLM as perceptual judge for relative comparisons, uses these as GRPO rewards to fine-tune intrinsic decomposition models on unlabeled real-world images, achieving significant improvements across architectures.


<details>
  <summary>Details</summary>
Motivation: Current intrinsic image decomposition models rely on synthetic datasets with paired supervision, limiting generalization to diverse real-world scenarios. There's a need for methods that can effectively learn from unlabeled, in-the-wild images.

Method: Proposes ReasonX framework that uses multimodal large language model (MLLM) as perceptual judge to provide relative intrinsic comparisons. These comparisons serve as GRPO (Group Relative Preference Optimization) rewards for fine-tuning intrinsic decomposition models on unlabeled real-world images. The framework aligns conditional intrinsic predictors by rewarding agreement between the judge's relational assessments and analytically derived relations from model outputs.

Result: Achieves significant improvements across multiple base architectures and modalities: 9-25% WHDR reduction on IIW albedo dataset and up to 46% depth accuracy gains on ETH3D dataset.

Conclusion: ReasonX demonstrates the promise of MLLM-guided comparative supervision to bridge low- and high-level vision reasoning, offering a model-agnostic approach that can be applied to different intrinsic predictors for improved generalization to real-world scenarios.
...
TL;DR: AdversarialAnatomyBench is a new benchmark for evaluating vision-language models on rare anatomical variants, revealing significant performance drops (74% to 29%) and persistent anatomical biases that current scaling and interventions cannot resolve.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for clinical vision-language models focus on common anatomical presentations, missing the challenges posed by rare anatomical variants that violate learned priors about "typical" human anatomy.

Method: Created AdversarialAnatomyBench - the first benchmark comprising naturally occurring rare anatomical variants across diverse imaging modalities and anatomical regions. Tested 22 state-of-the-art VLMs on basic medical perception tasks comparing typical vs. atypical anatomy.

Result: Mean accuracy dropped from 74% on typical anatomy to 29% on atypical anatomy. Even top models (GPT-5, Gemini 2.5 Pro, Llama 4 Maverick) showed 41-51% performance drops. Model errors mirrored anatomical biases, and neither model scaling nor interventions (bias-aware prompting, test-time reasoning) resolved these issues.

Conclusion: Current VLMs have poor generalization to rare anatomical presentations, highlighting a critical limitation. AdversarialAnatomyBench provides a foundation for systematically measuring and mitigating anatomical bias in multimodal medical AI systems.
...
TL;DR: MVRoom is a controllable novel view synthesis pipeline for 3D indoor scenes using multi-view diffusion conditioned on coarse 3D layouts, featuring a two-stage design with layout-aware epipolar attention and iterative scene generation.


<details>
  <summary>Details</summary>
Motivation: To achieve high-fidelity and controllable 3D scene generation for novel view synthesis in indoor environments, addressing the challenge of maintaining multi-view consistency while enabling text-to-scene generation capabilities.

Method: Two-stage pipeline: 1) Novel representations bridge 3D layout with consistent image-based condition signals, 2) Image-conditioned multi-view generation with layout-aware epipolar attention mechanism. Also includes iterative framework for recursive scene generation with varying object counts and complexities.

Result: Outperforms state-of-the-art baseline methods both quantitatively and qualitatively in 3D scene generation for novel view synthesis. Ablation studies validate the effectiveness of key components in the generation pipeline.

Conclusion: MVRoom successfully achieves high-fidelity, controllable 3D scene generation for novel view synthesis through its two-stage layout-conditioned diffusion approach with enhanced multi-view consistency mechanisms, supporting text-to-scene generation capabilities.
...
TL;DR: UniLight creates a unified latent space for lighting representation that bridges multiple modalities (text, images, irradiance, environment maps) through contrastive learning and spherical harmonics prediction, enabling cross-modal lighting transfer and manipulation.


<details>
  <summary>Details</summary>
Motivation: Lighting is crucial for visual appearance but difficult to represent and understand in images. Existing lighting representations (environment maps, irradiance, spherical harmonics, text) are incompatible, limiting cross-modal transfer between different lighting modalities.

Method: Proposes UniLight, a joint latent space that unifies multiple lighting modalities. Uses modality-specific encoders for text, images, irradiance, and environment maps trained contrastively to align representations. Includes auxiliary spherical-harmonics prediction task to reinforce directional understanding. Employs multi-modal data pipeline for large-scale training and evaluation.

Result: The representation captures consistent and transferable lighting features, enabling flexible manipulation across modalities. Supports three tasks: lighting-based retrieval, environment-map generation, and lighting control in diffusion-based image synthesis.

Conclusion: UniLight successfully creates a unified lighting representation that bridges multiple incompatible modalities, enabling cross-modal lighting transfer and manipulation for various applications in computer vision and graphics.
...
TL;DR: The paper proposes GRU-SNF, a novel inference-time refinement technique that combines GRU-Normalizing Flows with stochastic sampling methods to improve diversity in video motion transfer predictions without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Real-time video motion transfer applications need accurate yet diverse future predictions for realistic synthesis and robust decision-making under uncertainty. Current deterministic transformation structures in GRU-NF limit expressivity and multimodal distribution capture.

Method: Introduces Markov Chain Monte Carlo (MCMC) steps during GRU-NF inference, inspired by Stochastic Normalizing Flows (SNF). This enables exploration of richer output space and better approximation of true data distribution without retraining.

Result: GRU-SNF outperforms GRU-NF in generating diverse outputs without sacrificing accuracy, even under longer prediction horizons. It captures multimodal behavior more effectively in keypoint-based video motion transfer pipelines.

Conclusion: Integrating stochastic dynamics with flow-based sequence models shows strong potential for generative time series forecasting, enabling better multimodal behavior capture while maintaining temporal coherence and perceptual diversity.
...
TL;DR: Theoretical analysis of plug-and-play flow matching (PnP-Flow) for image restoration via continuous limit SDE model, leading to improved step scheduling, regularization, and acceleration techniques.


<details>
  <summary>Details</summary>
Motivation: While PnP-Flow has shown empirical success in image restoration, there's a lack of theoretical understanding behind its performance, creating a gap between practice and theory.

Method: Derived a continuous limit for PnP-Flow resulting in a stochastic differential equation (SDE) surrogate model. This SDE model provides insights for: (1) quantifying restoration error to improve step scheduling and regularize neural network vector fields, and (2) accelerating existing PnP-Flow models via extrapolation techniques.

Result: The SDE-informed improved PnP-Flow significantly outperforms baseline PnP-Flow and other state-of-the-art methods across multiple image restoration tasks (denoising, deblurring, super-resolution, inpainting) with superior evaluation metrics.

Conclusion: Theoretical SDE analysis bridges the gap between empirical success and theoretical understanding of PnP-Flow, providing practical improvements for step scheduling, regularization, and acceleration that lead to state-of-the-art performance in image restoration.
...
<details>
  <summary>Details</summary>
Motivation: Data loading has become a major bottleneck in deep learning despite hardware advances, especially with growing model complexity and input data sizes. The computational overhead of full JPEG decoding limits training and inference speed.

Method: Proposes a lightweight super-resolution pipeline that operates directly on JPEG discrete cosine transform (DCT) coefficients in the frequency domain, bypassing the need for full JPEG decoding and reducing computational overhead.

Result: Achieves 2.6x speedup in data loading and 2.5x speedup in training while preserving visual quality comparable to standard single-image super-resolution approaches.

Conclusion: Training models directly on encoded JPEG features is an effective approach for super-resolution tasks, significantly improving data loading efficiency and training speed without compromising visual quality.
...
TL;DR: GfM is a lightweight monocular geometry estimation method that decouples global and local road structure to accurately recover fine-scale road geometry like bumps and slopes, using only camera height above ground.


<details>
  <summary>Details</summary>
Motivation: Conventional monocular depth estimation oversmooths fine-scale road geometry features (bumps, slopes, surface irregularities), losing critical information needed for safe vehicle motion planning and stability control.

Method: GfM decouples global and local structure by predicting a dominant road surface plane plus residual variations expressed by gamma (dimensionless vertical deviation ratio). Using only camera height above ground, it deterministically recovers metric depth via closed-form solution based on planar parallax geometry.

Result: Achieves state-of-the-art near-field accuracy in both depth and gamma estimation on KITTI and RSRD datasets, maintains competitive global depth performance with only 8.88M parameters, and adapts robustly across diverse camera setups.

Conclusion: GfM provides physically interpretable, self-supervised monocular geometry estimation that accurately captures fine-scale road details essential for autonomous vehicle control, without requiring full extrinsic calibration or large annotated datasets.
...
TL;DR: FL²oRA: A LoRA-based approach that improves CLIP calibration in federated learning settings without explicit calibration procedures.


<details>
  <summary>Details</summary>
Motivation: While CLIP calibration has been studied in offline settings, its behavior under federated learning (FL) remains unexplored. The paper investigates how FL affects CLIP calibration and aims to improve reliability in distributed settings.

Method: Analyzes Textual Prompt Tuning approaches under FL, evaluates existing in-training calibration techniques across aggregation methods, and proposes FL²oRA - a straightforward LoRA-based approach that naturally improves calibration in FL.

Result: Textual Prompt Tuning degrades calibration under FL, existing calibration techniques provide limited improvements, and FL²oRA consistently produces well-calibrated models across multiple benchmarks, reducing the need for explicit calibration procedures.

Conclusion: The key challenge in FL calibration is not just aggregation or calibration techniques, but which components to fine-tune. FL²oRA effectively addresses this by naturally improving calibration through LoRA-based fine-tuning in federated settings.
...
TL;DR: TOMCap is a text-only training method for image captioning that uses CLIP representations and retrieval-augmented prompting without needing aligned image-caption pairs.


<details>
  <summary>Details</summary>
Motivation: To reduce reliance on curated data for image captioning by developing a method that doesn't require humanly-annotated image-text pairs, addressing the performance gap between unsupervised and fully supervised approaches.

Method: Uses pre-trained language model decoder prompted with CLIP-derived information after modality gap reduction. Combines retrieved caption examples and latent vector representations to guide generation without aligned image-caption pairs.

Result: TOMCap outperforms other training-free and text-only methods in extensive experiments. Analysis shows impact of different retrieval-augmentation and modality gap reduction configurations.

Conclusion: TOMCap demonstrates effective text-only training for image captioning by bridging modality gaps and using retrieval-augmented prompting, achieving competitive performance without aligned image-caption data.
...
TL;DR: Low-cost automated system for sex-based sorting of house crickets using computer vision and physical actuation achieves 86.8% accuracy.


<details>
  <summary>Details</summary>
Motivation: Global demand for sustainable protein sources drives edible insect farming, but current cricket farming lacks automated sex sorting despite benefits for selective breeding, optimized reproduction, and nutritional differentiation.

Method: Combines computer vision and physical actuation using Raspberry Pi 5 with Raspberry AI Camera, custom YOLOv8 nano object detection model, and servo-actuated sorting arm for real-time automated sex sorting.

Result: Model achieved mAP@0.5 of 0.977 in testing, and real-world experiments with cricket groups achieved overall sorting accuracy of 86.8%.

Conclusion: Demonstrates feasibility of deploying lightweight deep learning models on resource-constrained devices for insect farming, offering practical solution to improve efficiency and sustainability in cricket production.
...
TL;DR: Mind-to-Face is the first framework that decodes EEG signals directly into high-fidelity facial expressions, enabling neural-driven avatars without visual cues.
...
Method: Uses dual-modality recording setup with synchronized EEG and multi-view facial video. Employs CNN-Transformer encoder to map EEG signals into dense 3D position maps (65k+ vertices), then renders through modified 3D Gaussian Splatting pipeline for photorealistic results.

Result: EEG alone can reliably predict dynamic, subject-specific facial expressions including subtle emotional responses. Demonstrates neural signals contain richer affective and geometric information than previously assumed.

Conclusion: Establishes new paradigm for neural-driven avatars, enabling personalized, emotion-aware telepresence and cognitive interaction in immersive environments.
...
TL;DR: DisentangleFormer is a vision transformer architecture that decouples spatial and channel processing to address the limitation of standard self-attention's entangled representations, achieving SOTA performance on hyperspectral imaging tasks while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Standard self-attention in Vision Transformers jointly processes spatial and channel dimensions, creating entangled representations that prevent independent modeling of structural and semantic dependencies. This is especially problematic in hyperspectral imaging where channels capture distinct biophysical/biochemical cues.

Method: The architecture features: (1) Parallel Disentanglement for independent spatial-token and channel-token stream processing, (2) Squeezed Token Enhancer for adaptive calibration and fusion of spatial and channel streams, and (3) Multi-Scale FFN to complement global attention with multi-scale local context.

Result: Achieves state-of-the-art performance on hyperspectral benchmarks (Indian Pine, Pavia University, Houston, BigEarthNet, and infrared pathology dataset). Also retains competitive accuracy on ImageNet while reducing computational cost by 17.8% in FLOPs.

Conclusion: DisentangleFormer successfully addresses the fundamental limitation of standard self-attention through principled spatial-channel decoupling, enabling robust multi-channel vision representation with improved performance and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Modeling dynamic 3D scenes from multiple views is challenging due to high-dimensional nature and the need to aggregate information from multiple views to reconstruct time-evolving geometry and motion, especially with unsynchronized video sets.

Method: 1) Compute dense per-video 4D feature tracks and cross-video correspondences using Fused Gromov-Wasserstein optimal transport. 2) Perform global frame-level temporal alignment to maximize overlapping motion of matched 4D tracks. 3) Achieve sub-frame synchronization through multi-video 4D Gaussian splatting built on motion-spline scaffold representation.

Result: Achieves sub-frame synchronization accuracy with average temporal error below 0.26 frames, and high-fidelity 4D reconstruction reaching 26.3 PSNR scores on Panoptic Studio dataset. First general 4D Gaussian Splatting approach for unsynchronized video sets without assuming predefined scene objects or prior models.

Conclusion: SyncTrack4D successfully addresses the challenge of reconstructing dynamic 3D scenes from unsynchronized video sets by leveraging dense 4D track representations for simultaneous synchronization and reconstruction, achieving state-of-the-art performance.
...
TL;DR: This paper presents a B-spline-based method for generating realistic DIC datasets and introduces Bayes-DIC Net, a Bayesian neural network architecture for displacement field prediction with uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: Deep learning-based DIC algorithms require large-scale, realistic datasets for training and generalization, but existing datasets may not capture real-world displacement field complexities. There's also a need for networks that can provide confidence estimates in predictions for practical applications.

Method: 1) Generate DIC datasets using non-uniform B-spline surfaces with randomly generated control points to create realistic displacement fields and speckle patterns. 2) Propose Bayes-DIC Net architecture with multi-level information extraction during down-sampling and single skip connection aggregation during up-sampling. 3) Incorporate lightweight convolutional blocks for expanded receptive fields and contextual information. 4) Add dropout modules to transform the network into a Bayesian neural network for uncertainty estimation.

Result: The method enables generation of large-scale DIC datasets capturing real-world displacement scenarios. Bayes-DIC Net provides both predictive results and confidence levels when processing unlabeled data, enhancing practicality and reliability for displacement field prediction tasks.

Conclusion: This work offers novel approaches for DIC dataset generation and algorithm enhancement, addressing both data scarcity and uncertainty quantification challenges in deep learning-based DIC applications.
...
TL;DR: NN-RAG is a retrieval-augmented generation system that extracts, validates, and makes searchable reusable neural modules from PyTorch codebases, enabling cross-repository architecture migration and expanding executable neural architecture diversity.
...
Method: NN-RAG performs scope-aware dependency resolution, import-preserving reconstruction, and validator-gated promotion to extract neural modules from PyTorch codebases. It uses multi-level de-duplication (exact, lexical, structural) and enables cross-repository migration by automatically identifying reusable modules and regenerating them with complete dependencies.

Result: Applied to 19 major repositories, NN-RAG extracted 1,289 candidate blocks, validated 941 (73.0%), with over 80% structurally unique. It contributes approximately 72% of all novel network structures to the LEMUR dataset and enables cross-repository migration of architectural patterns.

Conclusion: NN-RAG transforms fragmented vision code into a reproducible, provenance-tracked substrate for algorithmic discovery, offering the first open-source solution that both quantifies and expands the diversity of executable neural architectures across repositories.
...
TL;DR: This paper proposes DLED, a dual-level evidential approach for open set face forgery detection that can identify novel fake categories using uncertainty estimation from spatial and frequency features.


<details>
  <summary>Details</summary>
Motivation: Face forgery generation algorithms are rapidly evolving, creating new fake categories that existing detection methods cannot identify. Current methods only handle binary Real-vs-Fake classification or known fake categories, making them vulnerable to novel forgeries.

Method: Proposes Dual-Level Evidential face forgery Detection (DLED) that collects and fuses category-specific evidence on both spatial and frequency levels to estimate prediction uncertainty for detecting novel fake categories.

Result: DLED achieves state-of-the-art performance, outperforming baseline models by an average of 20% in detecting forgeries from novel fake categories, while maintaining competitive performance on traditional Real-vs-Fake detection tasks.

Conclusion: The proposed DLED approach effectively addresses the open set face forgery detection problem by leveraging uncertainty estimation from dual-level evidence, making it robust to emerging novel fake categories in real-world scenarios.
...
TL;DR: SANTA is a framework that reduces factual hallucinations in video captioning by using self-augmented contrastive alignment to improve object and action faithfulness.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs for video captioning suffer from factual inaccuracies (hallucinations), especially for dynamic videos where both visual object and temporal action hallucinations need to be addressed simultaneously, which remains an unsolved challenge.

Method: SANTA uses a hallucinative self-augmentation scheme to identify potential hallucinations in MLLMs and transform original captions into contrasted negatives. It also employs tracklet-phrase contrastive alignment to match regional objects and relation-guided actions with corresponding visual and temporal phrases.

Result: Extensive experiments show SANTA outperforms existing methods in alleviating both object and action hallucinations, achieving superior performance on hallucination examination benchmarks.

Conclusion: SANTA effectively addresses the challenging problem of joint object and action hallucination mitigation in video captioning through self-augmented contrastive alignment, improving factual faithfulness in generated descriptions.
...
TL;DR: MAFNet is a lightweight stereo matching network using only 2D convolutions with frequency-domain decomposition and adaptive fusion for efficient real-time disparity estimation.


<details>
  <summary>Details</summary>
Motivation: Existing stereo matching methods have limitations: 3D convolution-based approaches are computationally expensive for cost aggregation, while iterative optimization methods lack non-local context modeling. Both are poorly suited for resource-constrained mobile devices and real-time applications.

Method: Proposes Multi-frequency Adaptive Fusion Network (MAFNet) with: 1) Adaptive frequency-domain filtering attention module that decomposes cost volume into high-frequency and low-frequency volumes for separate feature aggregation, and 2) Linformer-based low-rank attention mechanism to adaptively fuse high- and low-frequency information.
...
Conclusion: MAFNet provides an efficient stereo matching solution using only 2D convolutions, enabling high-quality disparity estimation suitable for resource-constrained mobile devices and real-time applications.
...
TL;DR: FMA-Net++ is a framework for joint video super-resolution and deblurring that explicitly models the coupled effect of motion and dynamically varying exposure, achieving state-of-the-art results with improved efficiency.


<details>
  <summary>Details</summary>
Motivation: Real-world video restoration faces complex degradations from motion coupled with dynamically varying exposure, which is a common artifact in auto-exposure or low-light capture but largely overlooked by prior works.
...
Result: Achieves state-of-the-art accuracy and temporal consistency on new REDS-ME and REDS-RE benchmarks and GoPro, outperforming recent methods in both restoration quality and inference speed. Generalizes well to challenging real-world videos despite being trained solely on synthetic data.

Conclusion: FMA-Net++ effectively addresses the coupled motion-exposure degradation problem in real-world video restoration through explicit modeling of exposure variations, achieving superior performance with efficient inference and good generalization to real-world scenarios.
...
TL;DR: FARL uses Fourier analysis to disentangle structural and stylistic visual features, injecting them into VLMs via dual cross-attention for better few-shot generalization.
...
Method: FARL uses Fourier analysis to separate structural features (from phase spectrum) and stylistic features (from amplitude spectrum). A dual cross-attention mechanism learns separate representation tokens for each, which are asymmetrically injected deep into VLM encoders to guide adaptation.
...
Conclusion: Explicitly disentangling structural and stylistic visual representations through Fourier analysis and asymmetric injection enhances VLM generalization capabilities in few-shot learning scenarios.
...
TL;DR: This paper presents a comprehensive analysis of transfer learning techniques for medical image classification using deep convolutional neural networks, evaluating six pre-trained models on a custom chest X-ray dataset for disease detection.


<details>
  <summary>Details</summary>
Motivation: Medical image classification is vital for disease identification, but training large deep learning models from scratch is often infeasible. Transfer learning offers a solution by reusing pre-trained models for new medical imaging tasks.

Method: The study evaluates six pre-trained CNN models (AlexNet, VGG16, ResNet18, ResNet34, ResNet50, and InceptionV3) on a custom chest X-ray dataset for disease detection. The analysis includes performance evaluation, uncertainty analysis, and runtime comparison to assess robustness and computational efficiency.

Result: InceptionV3 consistently outperforms other models across all standard metrics. The ResNet family shows progressively better performance with increasing depth, while VGG16 and AlexNet perform reasonably well but with lower accuracy. Transfer learning is beneficial, especially with limited data, but improvement depends on model architecture, dataset size, and domain similarity.

Conclusion: Transfer learning is effective for medical image classification, with InceptionV3 showing superior performance. A well-trained feature extractor combined with a lightweight feedforward model can provide efficient predictions. The study provides insights for selecting appropriate models based on specific requirements in medical imaging applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing distillation methods for remote sensing object detection struggle with mixed features in RSIs and neglect subtle feature variations, leading to entangled knowledge confusion and poor performance on dense and small objects.

Method: Proposes DS2D2 with explicit and implicit distillation using spectral decomposition. Uses first-order wavelet transform for spatial preservation, designs Density-Independent Scale Weight (DISW) for dense/small objects, and extracts implicit knowledge via full-frequency and high-frequency amplifiers mapping feature differences to prediction deviations.

Result: Achieves 4.2% AP50 improvement for RetinaNet and 3.8% AP50 improvement for Faster R-CNN on DIOR dataset, outperforming existing distillation approaches. Also validated on DOTA dataset.

Conclusion: DS2D2 effectively addresses mixed features and subtle discrepancies in remote sensing object detection through spectral decomposition-based distillation, demonstrating superior performance and architecture-agnostic applicability.
...
TL;DR: Proposes differentiable triangle-based ray tracing pipeline that directly uses triangles as rendering primitives without proxy geometry, achieving higher quality than existing ray tracing methods while maintaining real-time performance.


<details>
  <summary>Details</summary>
Motivation: Existing ray tracing methods for 3D Gaussian particles require constructing complex intermediate meshes and performing costly intersection tests through proxy geometry, which is inefficient and limits rendering quality.

Method: Differentiable triangle-based ray tracing pipeline that directly treats triangles as rendering primitives without relying on any proxy geometry. Unifies primitives by rendering triangles optimized by rasterization-based Triangle Splatting method.

Result: Achieves significantly higher rendering quality than existing ray tracing approaches while maintaining real-time rendering performance. Successfully unifies primitives used in novel-view synthesis.

Conclusion: Triangle-based ray tracing provides a unified primitive approach that overcomes limitations of proxy geometry methods, enabling high-quality real-time rendering with direct triangle primitives.
...
TL;DR: An explainable multimodal framework using RGB-D data and LLMs for Parkinson's disease gait analysis with improved accuracy and clinical interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing gait analysis approaches for Parkinson's disease detection are limited by single-modality inputs, low robustness, lack of clinical transparency, and difficulty capturing fine-grained motion patterns in realistic conditions.

Method: Uses dual YOLOv11-based encoders for RGB-D feature extraction, Multi-Scale Local-Global Extraction module, Cross-Spatial Neck Fusion mechanism, and frozen LLM to translate visual embeddings into clinical explanations.

Result: Achieves higher recognition accuracy, improved robustness to environmental variations (lighting, occlusion), and provides clear visual-linguistic reasoning compared to single-input baselines.

Conclusion: The framework bridges visual recognition with clinical understanding through multimodal feature learning and language-based interpretability, offering a novel vision-language paradigm for reliable and explainable Parkinson's disease gait analysis.
...
TL;DR: SSMP introduces a self-paced, self-corrective masked prediction method for automatic movie trailer generation that outperforms traditional selection-then-ranking approaches through bi-directional contextual modeling and progressive correction.


<details>
  <summary>Details</summary>
Motivation: Current automatic trailer generation methods use a "selection-then-ranking" paradigm that suffers from error propagation and limits trailer quality. The authors aim to move beyond this paradigm to create higher-quality trailers.

Method: SSMP uses a Transformer encoder trained via masked prediction, reconstructing trailer shot sequences from randomly masked counterparts. It features self-paced masking (adapting difficulty to model capability) and progressive self-correction during generation (filling high-confidence positions first and re-masking others for iterative refinement).
...
Conclusion: The proposed SSMP method successfully overcomes limitations of traditional selection-then-ranking approaches through its self-paced, self-corrective masked prediction framework, producing higher-quality movie trailers that better mimic human editing processes.
...
TL;DR: MindDrive is an end-to-end autonomous driving framework that harmonizes trajectory generation with comprehensive decision reasoning through a structured "context simulation - candidate generation - multi-objective trade-off" paradigm.


<details>
  <summary>Details</summary>
Motivation: Existing E2E-AD approaches have limitations: trajectory generation methods produce high-quality trajectories but use simple decision mechanisms, while trajectory selection methods perform multi-dimensional evaluation but lack sufficient generative capability. There's a need to integrate both high-quality trajectory generation with comprehensive decision reasoning.

Method: MindDrive uses a structured reasoning paradigm with two key components: 1) Future-aware Trajectory Generator (FaTG) based on a World Action Model (WaM) that performs ego-conditioned "what-if" simulations to predict future scenes and generate foresighted trajectory candidates, and 2) VLM-oriented Evaluator (VLoE) that leverages large vision-language models to conduct multi-objective evaluations across safety, comfort, and efficiency dimensions for human-aligned decision making.

Result: Extensive experiments on NAVSIM-v1 and NAVSIM-v2 benchmarks show that MindDrive achieves state-of-the-art performance across multi-dimensional driving metrics, significantly enhancing safety, compliance, and generalization capabilities.

Conclusion: MindDrive provides a promising path toward interpretable and cognitively guided autonomous driving by harmonizing high-quality trajectory generation with comprehensive decision reasoning through a structured reasoning paradigm.
...
TL;DR: StreamEQA is the first benchmark for streaming video question answering in embodied scenarios, evaluating models on embodied (perception, interaction, planning) and streaming (backward, real-time, forward reasoning) dimensions across 156 videos with 21K QA pairs.


<details>
  <summary>Details</summary>
Motivation: As embodied intelligence advances toward real-world deployment, agents need continuous perception and reasoning over streaming visual inputs to maintain situational awareness, comprehend interactions, and plan actions dynamically based on past, present, and anticipated future events.

Method: Created StreamEQA benchmark with 156 independent long videos, defining 42 tasks and generating ~21K question-answer pairs with precise timestamps through a hybrid pipeline combining automated generation and human refinement. Evaluates models on two dimensions: Embodied (perception, interaction, planning) and Streaming (backward, real-time, forward reasoning).

Result: Evaluation of 13 state-of-the-art video-LLMs reveals that despite strong performance on conventional benchmarks, these models still struggle with streaming video understanding in embodied scenarios.

Conclusion: StreamEQA is introduced as the first benchmark for streaming video QA in embodied scenarios, designed to catalyze research on streaming video understanding for embodied applications, addressing current limitations in existing models.
...
TL;DR: GuidNoise: A diffusion-based method that synthesizes realistic noise using only a single noisy/clean image pair as guidance, enabling data augmentation for image denoising without camera metadata or extensive training pairs.


<details>
  <summary>Details</summary>
Motivation: Current generative models for real noise synthesis require camera metadata and extensive noisy-clean image pairs, which are costly to acquire and show limited generalization between different settings. The paper aims to reduce these prerequisites.
...
Result: GuidNoise synthesizes high-quality noisy images under diverse noise environments without additional metadata. It enables efficient generation of noisy-clean pairs at inference time, significantly improving denoising performance, especially with lightweight models and limited training data.

Conclusion: GuidNoise provides a practical solution for noise synthesis using minimal guidance (single pair), eliminating the need for camera metadata and extensive training data. The self-augmentation capability enhances denoising performance in real-world scenarios with limited resources.
...
TL;DR: dVLM-AD: A diffusion-based vision-language model for end-to-end autonomous driving that addresses consistency issues in autoregressive VLMs by using iterative denoising for better controllability between reasoning and planning.


<details>
  <summary>Details</summary>
Motivation: Existing AR-based vision-language models for autonomous driving suffer from inconsistency between high-level reasoning and low-level planning due to causal attention and sequential token generation limitations. The paper aims to improve controllability and reliability in end-to-end driving systems.

Method: Proposes dVLM-AD, a diffusion-based vision-language model that unifies perception, structured reasoning, and low-level planning. Uses discrete diffusion with bidirectional attention and iterative denoising for better consistency between reasoning and action planning.

Result: dVLM-AD achieves 9% improvement in behavior-trajectory consistency and 6% increase in RFS on long-tail WOD-E2E scenarios compared to AR-based baselines. Performs comparably to existing driving VLM/VLA systems despite using a modest backbone.

Conclusion: Diffusion-based VLMs offer a more controllable and reliable pathway for scalable end-to-end autonomous driving by addressing consistency issues in AR-based approaches, particularly for out-of-distribution scenarios.
...
TL;DR: UniTS is a unified generative model for multiple satellite time series tasks (reconstruction, cloud removal, change detection, forecasting) using flow matching and diffusion transformers with adaptive conditioning.


<details>
  <summary>Details</summary>
Motivation: Existing satellite remote sensing methods require specialized models for different tasks, lacking unified spatiotemporal modeling across multiple time series applications.

Method: UniTS uses flow matching generative paradigm with diffusion transformer architecture, featuring Adaptive Condition Injector (ACor) for multimodal input conditioning and Spatiotemporal-aware Modulator (STM) for capturing complex dependencies.

Result: UniTS outperforms existing methods, especially for severe cloud contamination, modality absence, and forecasting phenological variations. Two new datasets (TS-S12 and TS-S12CR) were created for benchmarking.

Conclusion: UniTS demonstrates exceptional generative and cognitive capabilities for both low-level and high-level time series tasks, providing a unified solution for diverse satellite remote sensing applications.
...
TL;DR: DeRA is a novel 1D video tokenizer that decouples spatial-temporal representation learning, using appearance and motion streams aligned with pretrained vision models, plus a SACP module to handle gradient conflicts.


<details>
  <summary>Details</summary>
Motivation: To improve training efficiency and performance in video tokenization by decoupling spatial-temporal representation learning, addressing the limitations of previous approaches that don't separate appearance and motion learning effectively.

Method: DeRA maintains a compact 1D latent space while factorizing video encoding into appearance and motion streams aligned with pretrained vision foundation models. It introduces Symmetric Alignment-Conflict Projection (SACP) module to proactively reformulate gradients by suppressing components along conflicting directions caused by heterogeneous supervision.

Result: DeRA outperforms previous state-of-the-art video tokenizer LARP by 25% on UCF-101 in terms of rFVD. Achieves new state-of-the-art results on both UCF-101 class-conditional generation and K600 frame prediction when used for autoregressive video generation.

Conclusion: DeRA's decoupled spatial-temporal representation learning approach with appearance/motion streams and gradient conflict handling via SACP enables more efficient and effective video tokenization, setting new benchmarks in video generation tasks.
...
TL;DR: The paper introduces NABLA, a bird image dataset for evaluating identity-preserving generation, showing current models fail on birds and that training with species/age/sex grouping improves performance.


<details>
  <summary>Details</summary>
Motivation: Current identity-preserving image generation models work well for humans and rigid objects but fail for non-rigid, fine-grained categories like birds, which lack accessible high-quality data (especially videos/multi-view observations). Birds are an ideal test domain due to their diversity, fine-grained identification cues, and varied poses.

Method: Created the NABirds Look-Alikes (NABLA) dataset with 4,759 expert-curated image pairs, plus 1,073 pairs from iNaturalist multi-image observations and some videos. Used species, age, and sex grouping as a proxy for identity during training to improve identity preservation.

Result: State-of-the-art baselines fail to maintain bird identity on the NABLA dataset. Training with images grouped by species, age, and sex substantially improves identity-preserving generation performance on both seen and unseen species.

Conclusion: The NABLA dataset provides a valuable benchmark for evaluating identity-preserving generation in challenging domains like birds. The proposed training approach using species/age/sex grouping effectively improves identity preservation, advancing beyond human/rigid object applications toward more demanding domains requiring accuracy and fine detail.
...
TL;DR: COMET is a real-time autoregressive framework for generating stable, controllable character motion with fine-grained joint control and long-horizon synthesis using a Transformer-based conditional VAE with reference-guided feedback.


<details>
  <summary>Details</summary>
Motivation: Existing methods for character motion generation lack fine-grained control and suffer from motion degradation over long sequences, limiting their use in interactive applications that require stable, controllable motion in real-time.

Method: COMET uses an autoregressive framework with an efficient Transformer-based conditional VAE for precise interactive control over arbitrary joints. It introduces a novel reference-guided feedback mechanism to prevent error accumulation and ensure long-term stability, which also functions as a plug-and-play stylization module for real-time style transfer.

Result: COMET robustly generates high-quality motion at real-time speeds, significantly outperforming state-of-the-art approaches in complex motion control tasks like goal-reaching and in-betweening, demonstrating readiness for demanding interactive applications.

Conclusion: COMET provides a practical solution for real-time character animation with fine-grained control and long-term stability, addressing key limitations of existing methods and enabling versatile interactive applications through its innovative reference-guided feedback mechanism.
...
TL;DR: MM-SHR: A multi-model architecture for specular highlight removal that combines CNN's local detail extraction with transformer's global dependency modeling using novel attention mechanisms for efficient and accurate highlight removal across different scales.


<details>
  <summary>Details</summary>
Motivation: Specular highlights in practical environments impair visual performance and degrade task effectiveness. Existing single-type models (CNN or transformer) struggle to balance local fine-grained details and global long-range dependencies, especially for specular highlights of different scales.

Method: Proposes MM-SHR architecture that uses convolution operations in shallow layers to extract local details and attention mechanisms in deep layers to capture global features. Introduces OAIBlock (Omni-Directional Attention Integration Block) and HDDAConv (Adaptive Region-Aware Hybrid-Domain Dual Attention Convolutional Network) with omni-directional pixel-shifting and window-dividing operations in a coarse-to-fine manner.
...
Conclusion: MM-SHR effectively addresses the scale variation problem in specular highlight removal by combining local and global modeling capabilities, achieving superior performance compared to existing methods while maintaining computational efficiency.
...
TL;DR: The paper investigates fundamental design choices in motion diffusion models, comparing different motion representations and training configurations to understand their impact on performance and training efficiency.


<details>
  <summary>Details</summary>
Motivation: While diffusion models have become successful for human motion synthesis, there's limited understanding of how fundamental design choices like motion representations and loss functions affect performance. The authors aim to provide systematic insights into these decisions to improve conditional motion diffusion models.

Method: The study uses a proxy motion diffusion model (MDM) with v-loss (weighted sum of motion data and noise) called vMDM. They conduct empirical studies comparing six common motion representations, evaluate training time under various configurations, and analyze performance on large motion datasets using quality and diversity metrics.

Result: Results show clear performance differences across motion representations in diverse datasets. Different configurations significantly impact model training efficiency, demonstrating the importance of these design decisions on motion diffusion model outcomes.

Conclusion: The study provides foundational insights into motion representation choices and training configurations for diffusion models, offering guidance for improving conditional motion synthesis systems through better understanding of latent data distributions and workflow decisions.
...
TL;DR: UltraImage is a framework that enables image diffusion transformers to generate high-quality images beyond their training resolution by addressing content repetition and quality degradation through frequency correction and adaptive attention concentration.


<details>
  <summary>Details</summary>
Motivation: Recent image diffusion transformers achieve high-fidelity generation at training scales but struggle with larger resolutions, suffering from content repetition and quality degradation when extrapolating beyond training sizes.

Method: 1) Frequency-wise analysis identifies repetition arises from periodicity of dominant frequency aligned with training resolution; introduces recursive dominant frequency correction. 2) Quality degradation stems from diluted attention; proposes entropy-guided adaptive attention concentration to sharpen local attention for details while preserving global structural consistency.

Result: Outperforms prior methods on Qwen-Image and Flux (around 4K) across three generation scenarios, reducing repetition and improving visual fidelity. Can generate images up to 6K*6K without low-resolution guidance from 1328p training resolution, demonstrating extreme extrapolation capability.

Conclusion: UltraImage provides a principled framework that successfully addresses key limitations of image diffusion transformers for high-resolution generation, enabling extreme resolution extrapolation while maintaining visual quality and coherence.
...
TL;DR: DuGI-MAE: A dual-domain guided infrared foundation model using token entropy masking and global noise filtering, pretrained on Inf-590K dataset for superior infrared image interpretation.


<details>
  <summary>Details</summary>
Motivation: Existing foundation models like MAE trained on visible data perform poorly on infrared images due to their distinct characteristics. While InfMAE was developed for infrared, it still has limitations including omission of informative tokens, insufficient global modeling, and neglect of non-uniform noise common in infrared imagery.

Method: 1) Deterministic masking strategy based on token entropy to preserve only high-entropy tokens for reconstruction. 2) Dual-Domain Guidance (DDG) module that simultaneously captures global token relationships and adaptively filters non-uniform background noise. 3) Construction of Inf-590K dataset with diverse scenes, target types, and resolutions for large-scale pretraining.
...
Conclusion: The proposed DuGI-MAE effectively addresses limitations of existing infrared foundation models by incorporating entropy-based masking and dual-domain guidance for better infrared image interpretation, with code made available for reproducibility.
...
TL;DR: EgoLCD is an end-to-end framework for generating long, coherent egocentric videos by treating long video synthesis as a memory management problem, combining long-term sparse KV cache with short-term memory and memory regulation to prevent content drift.


<details>
  <summary>Details</summary>
Motivation: Generating long, coherent egocentric videos is challenging due to content drift in autoregressive models where object identity and scene semantics degrade over time during hand-object interactions and procedural tasks.

Method: EgoLCD combines a Long-Term Sparse KV Cache for stable global context with attention-based short-term memory, extended by LoRA for local adaptation. It uses Memory Regulation Loss for consistent memory usage and Structured Narrative Prompting for explicit temporal guidance.

Result: Extensive experiments on EgoVid-5M benchmark show EgoLCD achieves state-of-the-art performance in both perceptual quality and temporal consistency, effectively mitigating generative forgetting.

Conclusion: EgoLCD represents a significant step toward building scalable world models for embodied AI by addressing the memory management challenge in long-context video generation.
...
TL;DR: VideoSSM combines autoregressive diffusion with state-space memory for coherent long-video generation, addressing motion drift and repetition through hybrid short/long-term context management.


<details>
  <summary>Details</summary>
Motivation: Autoregressive diffusion struggles with long-video coherence due to accumulated errors, motion drift, and content repetition over minute-scale horizons, requiring better memory mechanisms for maintaining consistency.

Method: VideoSSM unifies AR diffusion with hybrid state-space memory: SSM serves as evolving global memory for scene dynamics across entire sequence, while context window provides local memory for motion cues and fine details.

Result: Achieves state-of-the-art temporal consistency and motion stability on short- and long-range benchmarks, especially at minute-scale horizons, enabling content diversity and interactive prompt-based control.

Conclusion: Establishes scalable, memory-aware framework for long video generation with linear time scaling, preserving global consistency without repetitive patterns and supporting prompt-adaptive interaction.
...
<details>
  <summary>Details</summary>
Motivation: Medical image segmentation faces challenges due to scarce annotated data and high computational costs. While foundation models like SAM show promise, they struggle with domain shifts in medical datasets. Zero-shot enhancement without task-specific training is urgently needed.

Method: Proposes BA-TTA-SAM, a task-agnostic test-time adaptation framework with two key mechanisms: (1) encoder-level Gaussian prompt injection for explicit guidance in initial representation learning, and (2) cross-layer boundary-aware attention alignment that exploits hierarchical ViT features to align deep semantic responses with shallow boundary cues.

Result: Experiments on ISIC, Kvasir, BUSI, and REFUGE datasets show 12.4% average DICE score improvement compared to SAM's zero-shot segmentation. The method consistently outperforms state-of-the-art models without requiring source-domain training data.

Conclusion: BA-TTA-SAM significantly enhances SAM's generalization ability for medical image segmentation through efficient test-time adaptation. The framework demonstrates strong performance across multiple medical datasets while maintaining zero-shot capabilities without task-specific training.
...
TL;DR: WiFi-based gesture recognition using Doppler spectra from CSI with attention mechanisms for cross-domain robustness, achieving 99.72% in-domain and 97.61% cross-domain accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing WiFi-based gesture sensing solutions work well in trained environments but lack cross-domain capabilities when deployed in untrained environments, limiting practical applications.

Method: Extract Doppler spectra from CSI, fuse multi-angle information into images, propose attention-based network combining multi-semantic spatial attention with self-attention channel mechanism, using ResNet18 as backbone.

Result: Achieves 99.72% in-domain accuracy and 97.61% cross-domain accuracy on Widar3 dataset, significantly outperforming existing solutions.

Conclusion: The proposed attention-based network effectively extracts domain-independent features for robust cross-domain WiFi gesture recognition while maintaining high in-domain performance.
...
TL;DR: ICRE network mines modality-specific identity clues for VI-ReID, using MPFR to capture overlooked attributes, SDCE to distill identity knowledge, and ICG Loss to reduce modality gaps, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Current VI-ReID methods focus only on modality-invariant features while ignoring modality-specific identity-aware knowledge, which is crucial for discriminative feature learning. There's a need to bridge this gap by utilizing implicit discriminative knowledge in modality-specific attributes.

Method: Proposes ICRE network with three components: 1) MPFR module aggregates shallow features from shared branches to capture overlooked modality-specific attributes; 2) SDCE module distills identity-aware knowledge from aggregated shallow features to guide modality-invariant feature learning; 3) ICG Loss reduces modality discrepancies in enhanced features and promotes diverse representation space.

Result: Extensive experiments across multiple public datasets show ICRE outperforms existing state-of-the-art methods in Visible-Infrared Person Re-Identification.

Conclusion: The proposed ICRE network effectively mines and utilizes modality-specific identity clues to enhance discriminative feature learning in cross-modal person re-identification, addressing limitations of existing methods that overlook modality-specific identity-aware knowledge.
...
<details>
  <summary>Details</summary>
Motivation: Traditional 3D scanning requires manual planning, but with the rise of embodied systems like drones and robots, there's a need for fully automated, accurate 3D scanning and reconstruction, especially for challenging materials like non-lambertian and specular surfaces.

Method: Auto3R uses a data-driven uncertainty quantification model that operates in an iterative 3D reconstruction and scanning process. It predicts uncertainty distributions over potential scanning viewpoints without requiring ground truth geometry or appearance information.

Result: Auto3R achieves superior performance, outperforming state-of-the-art methods by a large margin in extensive experiments. It was successfully deployed on a robot arm with a camera to effectively digitize real-world 3D objects, producing ready-to-use, photorealistic digital assets.

Conclusion: Auto3R provides an effective solution for automating 3D scanning and reconstruction, enabling embodied systems to autonomously create high-quality digital assets from real-world objects, including those with challenging material properties.
...
<details>
  <summary>Details</summary>
Motivation: Current Video LLMs rely on appearance-based matching and fail in scenarios requiring deeper understanding of physical dynamics. Motion signals are entangled with appearance, continuous-time motion modeling is needed, and physical annotations are costly to obtain.

Method: Proposes PhyVLLM with dual-branch encoder to disentangle appearance and motion, Neural ODE module for differentiable physical dynamic representations, and self-supervised learning to avoid explicit physical labels. Motion-aware representations are projected into pretrained LLM token space.
...
Conclusion: Explicit incorporation of physical motion modeling enhances Video LLMs' capabilities for physics reasoning while maintaining original multimodal performance, demonstrating the importance of disentangling appearance and motion for deeper video understanding.
...
TL;DR: Refaçade enables precise object retexturing by transferring local textures from reference to target objects in images/videos, using texture removal and jigsaw permutation to avoid structural interference.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models struggle with object retexturing because ControlNet approaches conditioned on raw reference images introduce unwanted structural information and fail to disentangle texture from structure.
...
Result: Superior visual quality, precise editing, and controllability compared to baselines, demonstrated through extensive experiments with both quantitative and human evaluations.

Conclusion: Refaçade effectively addresses the object retexturing task by separating texture from structure, enabling precise and controllable texture transfer in images and videos.
...
TL;DR: Novel video-based facial analysis method for alcohol intoxication detection using Graph Attention Network and 3D ResNet fusion, achieving 95.82% accuracy on a new curated dataset.


<details>
  <summary>Details</summary>
Motivation: Alcohol consumption is a major public health concern and cause of accidents/fatalities worldwide, creating need for non-invasive, reliable detection methods.

Method: Integrates facial landmark analysis via Graph Attention Network (GAT) with spatiotemporal visual features from 3D ResNet, dynamically fused with adaptive prioritization. Uses curated dataset of 3,542 video segments from 202 individuals.

Result: Achieves 95.82% accuracy, 0.977 precision, and 0.97 recall, outperforming custom 3D-CNN and VGGFace+LSTM baselines.

Conclusion: Model demonstrates potential for practical deployment in public safety systems for non-invasive, reliable alcohol intoxication detection.
...
TL;DR: X-Humanoid is a generative video editing approach that converts human videos into humanoid robot videos using fine-tuned diffusion models, creating large-scale training data for embodied AI.


<details>
  <summary>Details</summary>
Motivation: Progress in Vision-Language-Action models and world models is limited by scarce training data. Existing methods that overlay robot arms on egocentric videos can't handle complex full-body motions and scene occlusions in third-person videos, making them unsuitable for human-to-humanoid conversion.

Method: Adapts Wan 2.2 model into video-to-video structure and fine-tunes it for human-to-humanoid translation. Creates paired training data (17+ hours) using Unreal Engine pipeline from community assets, then applies to 60 hours of Ego-Exo4D videos to generate 3.6M robotized frames.

Result: Generated large-scale dataset of over 3.6 million robotized humanoid video frames. Quantitative analysis and user studies show superiority: 69% of users rated it best for motion consistency, and 62.1% for embodiment correctness.

Conclusion: X-Humanoid successfully bridges the data scarcity gap for embodied AI by providing a scalable method to convert human videos into humanoid robot videos, enabling better training of VLA models and world models.
...
TL;DR: VideoMem: A novel framework for ultra-long video understanding using adaptive memory management and progressive reinforcement learning, outperforming existing models on long video benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing vision language models struggle with ultra-long videos due to limited context length and inefficient long-term memory retention. Current RAG-based approaches have high storage and computational overhead.

Method: VideoMem frames long video understanding as sequential generation with adaptive memory management. It dynamically updates a global memory buffer to retain critical information while discarding redundancy. Uses Progressive Grouped Relative Policy Optimization (PRPO) with Progressive State Propagation (PSP) to retain valid states and Temporal Cascading Reward (TCR) to alleviate reward sparsity.
...
Conclusion: VideoMem provides an effective solution for ultra-long video understanding through adaptive memory management and progressive reinforcement learning, addressing limitations of existing approaches.
...
TL;DR: Gaussian Entropy Field (GEF) improves 3D Gaussian Splatting by minimizing configurational entropy to enhance surface reconstruction quality while maintaining rendering efficiency.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting excels at rendering but struggles with surface reconstruction quality. The authors propose that well-reconstructed surfaces exhibit low configurational entropy, where dominant primitives define geometry clearly while suppressing redundant components.

Method: Three complementary techniques: (1) entropy-driven surface modeling via entropy minimization, (2) adaptive spatial regularization using Surface Neighborhood Redundancy Index (SNRI) and image entropy-guided weighting, (3) multi-scale geometric preservation through competitive cross-scale entropy alignment.

Result: Achieves superior Chamfer Distance (0.64) on DTU and F1 score (0.44) on T&T benchmarks, along with best SSIM (0.855) and LPIPS (0.136) on Mip-NeRF 360, demonstrating competitive geometric precision and superior rendering quality.

Conclusion: GEF framework successfully enhances surface reconstruction accuracy in 3D Gaussian Splatting without compromising photometric fidelity, validating the entropy-based approach for improving geometric quality.
...
TL;DR: The paper introduces a novel adversarial attack on Document Visual Question Answering (DocVQA) systems that forges document content in visually imperceptible ways to induce specific incorrect answers, demonstrating vulnerabilities in state-of-the-art models like Pix2Struct and Donut.


<details>
  <summary>Details</summary>
Motivation: While recent DocVQA models show impressive reasoning capabilities, they remain vulnerable to adversarial attacks. The authors aim to expose critical security vulnerabilities by creating attacks that can forge document content in ways that are visually imperceptible to humans but can manipulate model outputs for malicious purposes.

Method: The authors develop specialized attack algorithms that produce adversarially forged documents tailored to different attacker goals. These attacks can range from targeted misinformation (inducing specific wrong answers) to systematic model failure scenarios. The approach is tested against two state-of-the-art end-to-end models: Pix2Struct (vision-language transformer) and Donut (transformer-based document image understanding model).

Result: The attacks successfully demonstrate vulnerabilities in current DocVQA systems. The adversarial forged documents can induce specific or generally incorrect answers from both Pix2Struct and Donut models, highlighting that these state-of-the-art systems are not robust against visually imperceptible document manipulations.

Conclusion: The findings reveal critical security vulnerabilities in current DocVQA systems that could be exploited for malicious purposes like spreading misinformation. The work calls for the development of more robust defenses and security measures to protect document understanding systems from such adversarial attacks.
...
TL;DR: COOPER is a unified MLLM that integrates depth and segmentation as auxiliary modalities with adaptive interleaved reasoning to significantly improve 3D spatial understanding while maintaining general performance.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with 3D-aware spatial reasoning because they treat perception enhancement (using auxiliary modalities like depth/segmentation) and reasoning enhancement (training on spatial VQA datasets) in isolation, rather than developing an intrinsic ability to integrate both aspects.

Method: COOPER is a unified MLLM that leverages depth and segmentation as auxiliary modalities, trained in two stages: first for auxiliary modality generation, then for adaptive interleaved reasoning to integrate perception and reasoning capabilities.

Result: COOPER achieves 6.91% average improvement in spatial reasoning while maintaining general performance. Even the variant trained only for auxiliary modality generation achieves 7.92% gain on distance and size estimation.

Conclusion: Learning to generate auxiliary modalities helps internalize spatial knowledge and strengthen spatial understanding, and a unified approach with adaptive interleaved reasoning can significantly enhance MLLMs' 3D spatial intelligence.
...
<details>
  <summary>Details</summary>
Motivation: Supervised deep learning for microscopic image analysis requires high-quality datasets, but creation is hindered by resource constraints, domain variability, and bias risks. There's a need for standardized guidance to improve dataset quality and availability.

Method: Review article providing a comprehensive guide covering: 1) image acquisition, 2) annotation software selection, 3) annotation creation with quality criteria (correctness, completeness, consistency), and methods to enhance annotation quality using advanced techniques beyond single annotators.

Result: Provides a standard operating procedure (SOP) as supplemental material outlining best practices for dataset development, emphasizes addressing domain shifts in training data, and highlights the importance of open datasets for innovation and reproducibility.

Conclusion: By addressing dataset creation challenges and offering practical recommendations, this review aims to advance the availability of high-quality datasets, ultimately contributing to the development of generalizable and robust deep learning models for pathology applications.
...
TL;DR: A robotic assembly task inspired by traditional crafts where robots build target objects using available objects that don't directly match the target's parts, focusing on object selection from RGB images.


<details>
  <summary>Details</summary>
Motivation: Inspired by traditional handmade crafts where people improvise assemblies using available objects, the paper aims to develop robotic systems that can similarly assemble target objects using non-matching available parts.

Method: Uses mask segmentation neural network to identify visible parts, retrieves labeled template meshes, performs pose optimization, simplifies parts to primitive shapes (cuboids/cylinders), and designs search algorithm for correspondences based on local/global proportions.
...
Conclusion: The proposed approach successfully addresses the Craft Assembly Task by selecting appropriate object subsets from available objects to approximate target objects, demonstrating feasibility in real-world scenarios.
...
TL;DR: TARDis is a physics-aware framework for tumor segmentation/diagnosis in CT that treats missing contrast phases as missing time points on continuous enhancement curves, using disentangled static (anatomy) and dynamic (perfusion) representations to hallucinate missing hemodynamic features.


<details>
  <summary>Details</summary>
Motivation: Multi-phase CT imaging is often incomplete due to radiation concerns or scanning limitations, creating a "missing modality" problem. Existing methods treat missing phases as absent channels, ignoring the temporal continuity of contrast enhancement dynamics.

Method: TARDis redefines missing modalities as missing sample points on Time-Attenuation Curves. It uses a dual-path architecture: (1) quantization-based path with learnable embedding dictionary for time-invariant anatomical features, and (2) probabilistic path with Conditional Variational Autoencoder to model dynamic enhancement conditioned on scan time.

Result: Extensive experiments on large-scale private abdominal CT dataset (2,282 cases) and two public datasets show TARDis significantly outperforms state-of-the-art incomplete modality frameworks, maintaining robust diagnostic performance even in extreme data-sparsity scenarios.

Conclusion: TARDis demonstrates potential for reducing radiation exposure while maintaining diagnostic precision by effectively hallucinating missing hemodynamic features through physics-aware modeling of contrast enhancement dynamics.
...
TL;DR: SiamDFF is a novel Siamese network for infrared UAV tracking that uses dynamic feature fusion, feature enhancement, and knowledge distillation to handle weak features and complex backgrounds in real-time.


<details>
  <summary>Details</summary>
Motivation: Infrared UAV targets often have weak features and appear in complex backgrounds, making accurate tracking challenging for anti-UAV applications. Existing methods struggle with these difficult conditions.

Method: Proposes SiamDFF with three key components: 1) Selective Target Enhancement Network (STEN) using intensity-aware multi-head cross-attention, 2) Dynamic Spatial Feature Aggregation Module (DSFAM) for multi-scale feature integration, and 3) Dynamic Channel Feature Aggregation Module (DCFAM) for template feature fusion. Also introduces a target-aware contextual attention knowledge distiller to transfer target priors from teacher to student network.

Result: Extensive experiments on real infrared UAV datasets show the approach outperforms state-of-the-art trackers under complex backgrounds while achieving real-time tracking speed.

Conclusion: SiamDFF effectively addresses the challenges of infrared UAV tracking through dynamic feature fusion and knowledge distillation, providing superior performance in complex scenarios with real-time capability.
...
TL;DR: SAM3-I extends SAM3's concept segmentation to handle complex natural language instructions with attributes, relations, and reasoning, while preserving original concept capabilities.


<details>
  <summary>Details</summary>
Motivation: SAM3 only supports short noun-phrase prompts, but real-world applications require richer expressions with attributes, spatial relations, functionalities, actions, states, and implicit reasoning. Current workarounds using external agents and iterative filtering are inefficient and imprecise.

Method: SAM3-I introduces an instruction-aware cascaded adaptation mechanism that progressively aligns expressive instruction semantics with SAM3's vision-language representations. It also develops a structured instruction taxonomy (concept, simple, complex levels) and a scalable data engine to create diverse instruction-mask pairs.

Result: SAM3-I delivers appealing performance, demonstrating that SAM3 can be effectively extended to follow natural-language instructions while preserving its strong concept grounding. The framework is open-sourced with practical fine-tuning workflows.
...
TL;DR: The paper introduces AbstainEQA, a new benchmark for evaluating when embodied question answering agents should abstain from answering ambiguous or unanswerable questions, showing current models perform poorly compared to humans.


<details>
  <summary>Details</summary>
Motivation: Existing EQA benchmarks assume all questions must be answered, but real-world embodied agents need to know when they lack sufficient information to answer questions reliably. Human analysis shows 32.4% of queries contain missing or underspecified context.

Method: Derived five abstention categories from human communication error theories: actionability limitation, referential underspecification, preference dependence, information unavailability, and false presupposition. Created AbstainEQA by transforming well-posed OpenEQA questions into ambiguous variants across these categories (1,636 abstention cases paired with 1,636 original instances).
...
Conclusion: Abstention is a fundamental prerequisite for reliable embodied interaction and necessary basis for effective clarification. Current models significantly underperform humans in knowing when to withhold answers, highlighting a critical gap in embodied AI capabilities.
...
TL;DR: Zero-shot pipeline for detecting, identifying, and localizing harmful content in images with pixel-accurate masks, achieving 85.8% recall and 78.1% precision while being robust to adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: Current NSFW flags are insufficient for content moderation - moderators need to know exactly what objects make an image illegal and where they're located for effective enforcement.

Method: Uses foundation segmentation model (SAM) to generate object masks, refines them into larger regions, scores each region for malicious relevance using vision-language model with open-vocabulary prompts, fuses scores into consolidated malicious object map, and employs ensemble across multiple segmenters for robustness.
...
Conclusion: The pipeline provides the first practical tool for fine-grained, explainable malicious-image moderation that processes images in seconds and integrates seamlessly with existing VLM workflows while being robust to adaptive attacks.
...
TL;DR: HeFT is a zero-shot point tracking framework that uses pretrained video diffusion models' attention heads and frequency components for feature selection, achieving state-of-the-art performance without training data.
...
Method: Analyzes VDiT attention heads as functional units, identifies low-frequency components as crucial for correspondences, proposes head- and frequency-aware feature selection, extracts features via single-step denoising, and uses soft-argmax localization with forward-backward consistency checks.
...
Conclusion: Video diffusion models serve as powerful foundation models for downstream tasks, with HeFT demonstrating their potential for zero-shot point tracking and paving the way toward unified visual foundation models.
...
TL;DR: SEASON is a training-free method that enhances temporal and spatial faithfulness in VideoLLMs by dynamically diagnosing hallucination tendencies and applying adaptive contrastive decoding against temporal and spatial negatives.


<details>
  <summary>Details</summary>
Motivation: VideoLLMs struggle with temporal reasoning, generating temporally inconsistent or causally implausible descriptions, causing severe hallucination issues. While spatial hallucinations have been studied, temporal reasoning remains underexplored.

Method: Self-Diagnostic Contrastive Decoding (SEASON) - a training-free method that adaptively enhances temporal and spatial faithfulness by dynamically diagnosing each token's hallucination tendency and applying adaptive contrastive decoding against corresponding temporal and spatial negatives.
...
Conclusion: SEASON effectively addresses temporal hallucination issues in VideoLLMs through adaptive contrastive decoding without requiring additional training, demonstrating superior performance on both hallucination-specific and general video understanding tasks.
...
TL;DR: I2I-Bench is a comprehensive benchmark for image-to-image editing models addressing limitations of existing benchmarks through diverse tasks, automated multi-dimensional evaluation, and alignment with human preferences.


<details>
  <summary>Details</summary>
Motivation: Existing image editing benchmarks have limited task scopes, insufficient evaluation dimensions, and heavy reliance on manual annotations, which constrain scalability and practical applicability.

Method: Proposes I2I-Bench featuring: (1) 10 task categories across single-image and multi-image editing, (2) 30 decoupled evaluation dimensions with automated hybrid evaluation using specialized tools and LMMs, (3) rigorous alignment validation with human preferences.

Result: Benchmarked numerous mainstream image editing models, investigating gaps and trade-offs across various dimensions. All components will be open-sourced.

Conclusion: I2I-Bench provides a comprehensive, scalable benchmark for evaluating image editing models, addressing limitations of existing approaches and facilitating future research through open-source availability.
...
TL;DR: Live Avatar: A real-time streaming avatar synthesis system using 14B parameter diffusion model with novel parallelization and consistency mechanisms to achieve 20 FPS infinite-length generation.
...
Result: Achieves state-of-the-art performance with 20 FPS end-to-end generation on 5 H800 GPUs, enabling practical real-time high-fidelity avatar generation at scale - the first system to accomplish this.

Conclusion: Establishes a new paradigm for deploying advanced diffusion models in industrial long-form video synthesis applications, overcoming previous limitations of sequential computation and inconsistency for real-time streaming avatar generation.
...
TL;DR: Reward Forcing: A novel framework for streaming video generation that prevents initial frame copying and enhances motion dynamics through EMA-Sink tokens and Rewarded Distribution Matching Distillation.
...
Conclusion: Reward Forcing effectively addresses the limitations of existing streaming video generation methods by preventing initial frame copying while maintaining long-horizon consistency and significantly enhancing motion quality.
...
TL;DR: The paper introduces CrossPoint-Bench, a benchmark for evaluating cross-view point correspondence in VLMs, reveals a 54.65% performance gap between state-of-the-art models and humans, and proposes CrossPoint-378K dataset and CroPond model that achieves 39.7% improvement over Gemini-2.5-Pro.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models lack precise point-level cross-view correspondence capability, which is crucial for spatial understanding and embodied AI applications requiring precise affordance interactions.

Method: 1) Proposes Cross-View Point Correspondence (CVPC) task and CrossPoint-Bench benchmark with hierarchical design based on human cognitive process; 2) Constructs CrossPoint-378K dataset with 378K QA pairs across 900 scenes focused on actionable affordance regions; 3) Develops CroPond model trained on this dataset.

Result: State-of-the-art models (e.g., Gemini-2.5-Pro) show 54.65% accuracy gap compared to humans on CrossPoint-Bench. CroPond achieves state-of-the-art performance, surpassing Gemini-2.5-Pro by 39.7% accuracy on the benchmark.

Conclusion: The work exposes a significant challenge in VLMs' fine-grained spatial reasoning, provides a comprehensive benchmark and dataset, and demonstrates that specialized training (CroPond) can substantially improve cross-view point correspondence, offering foundation for future embodied AI research.
...
TL;DR: OmniScaleSR is a diffusion-based framework for realistic arbitrary-scale super-resolution that achieves both high fidelity and realism through explicit scale control mechanisms and multi-domain fidelity enhancement.


<details>
  <summary>Details</summary>
Motivation: Existing arbitrary-scale SR methods using implicit neural representation (INR) lack realism in fine details, while diffusion-based Real-ISR models can achieve ASSR but suffer from excessive hallucination or blurry outputs without explicit scale control, especially at ultra-high scales.

Method: Proposes OmniScaleSR with diffusion-native scale control mechanisms that work synergistically with implicit scale adaptation, enabling scale-aware and content-aware modulation of the diffusion process, plus multi-domain fidelity enhancement designs.

Result: Extensive experiments show OmniScaleSR surpasses state-of-the-art methods in both fidelity and perceptual realism, with particularly strong performance at large magnification factors on bicubic degradation benchmarks and real-world datasets.

Conclusion: OmniScaleSR effectively addresses the limitations of both INR-based and diffusion-based approaches for arbitrary-scale SR by combining explicit scale control with diffusion priors, achieving superior results across various scales.
...
TL;DR: The paper proposes MIND, a hierarchical visual encoder with a Status Judgment module to suppress ambiguous lip features, along with ConvoInsight-DB dataset and PRISM evaluation metric for generative psychological analysis of conversations.
...
Method: Proposes MIND (Multilevel Insight Network for Disentanglement) with Status Judgment module to suppress ambiguous lip features based on temporal feature variance; constructs ConvoInsight-DB dataset with expert annotations; designs PRISM (Mental Reasoning Insight Rating Metric) using expert-guided LLM for multidimensional evaluation.

Result: MIND significantly outperforms all baselines on PRISM benchmark, achieving +86.95% gain in micro-expression detection over prior SOTA. Ablation studies confirm Status Judgment module is most critical component.

Conclusion: The paper presents a complete ecosystem addressing both visual disentanglement and evaluation challenges in conversation analysis, with MIND's Status Judgment module proving essential for performance improvements. Code is open-sourced.
...
TL;DR: E3AD is an emotion-aware vision-language-action framework for autonomous driving that incorporates passenger emotion understanding and free-form natural language commands to improve human-aligned trajectory planning.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving systems using vision-language-action models ignore passenger emotional states, which are crucial for comfort and AD acceptance. There's a need for systems that can interpret free-form commands, infer emotion, and plan trajectories accordingly.

Method: Proposes E3AD framework with: 1) continuous Valence-Arousal-Dominance (VAD) emotion model to capture tone/urgency from language, 2) dual-pathway spatial reasoning module fusing egocentric and allocentric views for human-like spatial cognition, and 3) consistency-oriented training combining modality pretraining with preference-based alignment.

Result: Achieves state-of-the-art VAD correlation for emotion estimation, improves visual grounding and waypoint planning across real-world datasets, demonstrating better human-aligned grounding, planning, and human-centric feedback.

Conclusion: Injecting emotion into VLA-style driving yields more human-aligned grounding, planning, and human-centric feedback, advancing the field toward Open-Domain End-to-End autonomous driving that considers passenger emotional states.
...
TL;DR: Instance-aware depth completion framework using binary instance masks as spatial priors to refine depth predictions, achieving lower RMSE than baseline methods.


<details>
  <summary>Details</summary>
Motivation: Existing depth completion methods often rely on semantic segmentation but overlook object-level understanding. The paper aims to leverage instance-aware cues to improve depth accuracy, especially near object boundaries, occlusions, and thin structures.

Method: Four-component framework: 1) frozen YOLO V11 instance segmentation branch for binary instance masks, 2) U-Net-based depth completion backbone, 3) cross-attention fusion module, and 4) attention-guided prediction head. Instance masks guide depth branch via cross-attention to focus on object-centric regions.

Result: Validated on Virtual KITTI 2 dataset, achieves lower RMSE compared to U-Net-only baseline and previous semantic-guided methods while maintaining competitive MAE. Effectively enhances depth accuracy near object boundaries, occlusions, and thin structures.

Conclusion: Incorporating instance-aware cues offers a promising direction for improving depth completion without relying on dense semantic labels, providing better object-level understanding for 3D perception systems.
...
TL;DR: VRSketch2Shape: A framework for generating 3D shapes from sequential VR sketches using temporal stroke ordering, outperforming previous methods in geometric fidelity and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing sketch-to-shape models ignore the temporal ordering of strokes in VR sketching, discarding crucial structural and design intent cues that could improve 3D shape generation.

Method: Three main contributions: (1) automated pipeline for generating sequential VR sketches from arbitrary shapes, (2) dataset of 20k+ synthetic and 900 hand-drawn sketch-shape pairs across four categories, and (3) order-aware sketch encoder coupled with diffusion-based 3D generator.
...
Conclusion: VRSketch2Shape demonstrates the importance of temporal stroke ordering for VR sketch-to-shape generation, offering improved performance and generalization while releasing all data and models open-source.
...
<details>
  <summary>Details</summary>
Motivation: Supervised training struggles with consistent image generation due to lack of large-scale datasets capturing visual consistency and the complexity of modeling human perceptual preferences. RL offers a promising data-free alternative to learn complex visual criteria.

Method: Two-component framework: 1) PaCo-Reward - pairwise consistency evaluator trained on automated sub-figure pairing dataset, using generative autoregressive scoring with task-aware instructions and CoT reasoning. 2) PaCo-GRPO - RL algorithm with resolution-decoupled optimization to reduce cost and log-tamed multi-reward aggregation for balanced optimization.

Result: PaCo-Reward significantly improves alignment with human perceptions of visual consistency. PaCo-GRPO achieves state-of-the-art consistency performance with improved training efficiency and stability across representative subtasks.

Conclusion: PaCo-RL demonstrates promise as a practical and scalable solution for consistent image generation, addressing key challenges in preserving identities, styles, and logical coherence across multiple images for applications like storytelling and character design.
...
TL;DR: LaFiTe introduces a 3D-native texturing framework using generative sparse latent color fields to overcome limitations of UV-based methods, achieving state-of-the-art fidelity and enabling flexible texture generation applications.
...
Method: LaFiTe uses a variational autoencoder (VAE) to encode surface appearance into a sparse, structured latent space, then decodes it into a continuous color field. A conditional rectified-flow model synthesizes textures from this representation.

Result: LaFiTe exceeds state-of-the-art methods by >10 dB PSNR in reconstruction, effectively disentangles texture appearance from mesh topology/UV parameterization, and enables high-quality texture synthesis across diverse styles and geometries.

Conclusion: LaFiTe sets a new benchmark for 3D-native texturing and enables flexible downstream applications like material synthesis and texture super-resolution, advancing 3D content creation workflows.
...
TL;DR: EMMA is an efficient unified multimodal architecture that handles understanding, generation, and editing tasks with a 32x compression autoencoder, channel-wise token concatenation, shared-decoupled networks, and mixture-of-experts mechanisms.


<details>
  <summary>Details</summary>
Motivation: To create a more efficient and unified multimodal architecture that can handle understanding, generation, and editing tasks simultaneously, addressing the token explosion problem in multimodal models and improving training balance between different tasks.

Method: Four key components: 1) Efficient autoencoder with 32x compression ratio, 2) Channel-wise concatenation instead of token-wise concatenation, 3) Shared-and-decoupled network for task-specific modeling, 4) Mixture-of-experts mechanism for visual understanding encoder.
...
Conclusion: EMMA provides an efficient foundation for unified multimodal architectures that can handle multiple tasks simultaneously while maintaining high performance and computational efficiency.
...
TL;DR: RobustSplat++ improves 3D Gaussian Splatting for in-the-wild scenes by delaying Gaussian growth and using scale-cascaded mask bootstrapping to handle transient objects and illumination variations.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods struggle with in-the-wild scenes containing transient objects and illumination variations, causing artifacts in rendered images. The Gaussian densification process inadvertently models these transient disturbances, leading to poor quality reconstructions.

Method: Three key designs: 1) Delayed Gaussian growth strategy that optimizes static structure before allowing splitting/cloning to prevent overfitting to transients; 2) Scale-cascaded mask bootstrapping that starts with low-resolution feature similarity for robust initial transient mask estimation, then refines with high-resolution supervision; 3) Integration of both strategies with appearance modeling for comprehensive scene handling.
...
Conclusion: RobustSplat++ provides a robust solution for 3D Gaussian Splatting in challenging real-world scenarios by addressing the limitations of Gaussian densification through delayed growth strategies and multi-scale mask bootstrapping, enabling higher quality novel-view synthesis and 3D modeling.
...
TL;DR: LatentFM is a flow-based generative model for medical image segmentation that operates in latent space, using VAEs to encode images/masks and flow matching to generate diverse segmentation outputs with uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: Flow matching has shown strong generative capabilities for learning exact data densities. The authors aim to leverage this for medical image segmentation to generate diverse outputs and quantify uncertainty, providing clinicians with richer information.

Method: 1) Design two VAEs to encode medical images and masks into lower-dimensional latent space. 2) Estimate conditional velocity field based on input image to guide the flow. 3) Sample multiple latent representations to synthesize diverse segmentation outputs. 4) Generate confidence maps from pixel-wise variance to quantify model certainty.

Result: Experiments on ISIC-2018 and CVC-Clinic datasets show superior segmentation accuracy compared to deterministic and generative baselines. The method achieves uncertainty-aware predictions and remains highly efficient in latent space.

Conclusion: LatentFM successfully applies flow matching to medical image segmentation, enabling accurate, diverse segmentation with uncertainty quantification while maintaining efficiency through latent space operations.
...
TL;DR: FreeGen is a co-training framework for free-viewpoint driving scene synthesis that combines reconstruction and generation models to achieve both interpolation consistency and extrapolation realism without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing datasets and generative models for autonomous driving simulation struggle with providing consistent off-trajectory observations, limiting large-scale evaluation and training. Current approaches either lack interpolation consistency or extrapolation realism, often requiring per-scene optimization.

Method: FreeGen uses a feed-forward reconstruction-generation co-training framework. The reconstruction model provides stable geometric representations for interpolation consistency, while the generation model performs geometry-aware enhancement for realism at unseen viewpoints. Through co-training, generative priors are distilled into reconstruction for better off-trajectory rendering, and refined geometry provides stronger structural guidance for generation.

Result: Experiments show that FreeGen achieves state-of-the-art performance for free-viewpoint driving scene synthesis, effectively addressing the joint challenge of interpolation consistency and extrapolation realism.

Conclusion: FreeGen successfully enables scalable free-viewpoint driving scene synthesis through reconstruction-generation co-training, providing a solution for large-scale autonomous driving simulation and pre-training without per-scene optimization.
...
TL;DR: SBM is a Transformer-based model for BIM layout synthesis that tokenizes buildings by unifying heterogeneous architectural features, learns joint representations, and operates in two modes: encoder-only for room embeddings and encoder-decoder for autoregressive layout generation.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of tokenizing buildings in BIM scenes, specifically how to unify heterogeneous feature sets of architectural elements while preserving compositional structure for effective layout synthesis.

Method: SBM uses a sparse attribute-feature matrix to represent room properties, a unified embedding module for joint categorical/continuous feature representations, and a Transformer backbone with two modes: encoder-only for room embeddings and encoder-decoder for autoregressive Data-Driven Entity Prediction (DDEP).

Result: SBM learns compact room embeddings that reliably cluster by type and topology, enabling strong semantic retrieval. In DDEP mode, it produces functionally sound layouts with fewer collisions, boundary violations, and improved navigability.

Conclusion: The Small Building Model successfully addresses building tokenization challenges and demonstrates effectiveness in both retrieval and generative layout synthesis tasks for BIM scenes.
...
TL;DR: Proposes DevDet framework for multi-domain face forgery detection that amplifies real/fake differences to overcome domain dominance in feature space, enabling better single-image real/fake judgments in domain-unspecified conditions.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detectors struggle with single-image real/fake judgments in real-world scenarios where inputs are domain-unspecified, because domain differences dominate the feature space more than subtle real/fake distinctions, despite having high AUC within each domain.

Method: Introduces Multi-In-Domain Face Forgery Detection (MID-FFD) paradigm with sufficient real-fake domains for training, and proposes DevDet framework with Face Forgery Developer (FFDev) to amplify real/fake differences and Dose-Adaptive Fine-Tuning (DAFT) strategy to make real/fake distinctions dominant in feature space.

Result: DevDet demonstrates superiority in predicting real/fake under MID-FFD scenario while maintaining original generalization ability to unseen data, effectively addressing the domain-dominant issue in multi-domain deepfake detection.

Conclusion: The proposed MID-FFD paradigm and DevDet framework provide an effective solution for real-world deepfake detection where domain-unspecified inputs require definitive real/fake judgments, overcoming the limitation of domain differences dominating feature space in multi-domain scenarios.
...
TL;DR: LineAR is a training-free KV cache compression method for autoregressive image generation that reduces memory usage and increases throughput while maintaining or improving image quality.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive image generation suffers from severe memory bottlenecks due to caching all previously generated visual tokens, leading to high storage requirements and low throughput.

Method: LineAR uses a progressive KV cache compression pipeline that manages cache at line level using a 2D view, preserving visual dependency regions while evicting less-informative tokens guided by inter-line attention.

Result: LineAR achieves significant improvements: reduces KV cache to 1/6-1/8 while improving FID scores (ImageNet FID from 2.77 to 2.68, COCO FID from 23.85 to 22.86), with up to 67.61% memory reduction and 7.57x speedup.

Conclusion: LineAR enables efficient autoregressive image generation with substantial memory savings and throughput speedup while maintaining or improving generation quality, validated across multiple models.
...
<details>
  <summary>Details</summary>
Motivation: Video-based pose estimation often fails in self-contact scenarios, while bioimpedance sensing can cheaply measure ground-truth skin-to-skin contact. Combining both could provide better 3D pose capture in the wild.

Method: Initialize pose using off-the-shelf estimator, then perform contact-aware pose optimization during measured self-contact. Minimize reprojection error and deviations from input estimate while enforcing vertex proximity constraints based on bioimpedance measurements.

Result: 11.7% average improvement in reconstruction accuracy across three input pose estimators, validated on synchronized RGB video, bioimpedance measurements, and 3D motion capture dataset.

Conclusion: BioTUCH effectively combines visual pose estimation with bioimpedance sensing to improve 3D human pose reconstruction, especially for self-contact scenarios. The miniature wearable bioimpedance sensor enables large-scale data collection for training better pose estimation models.
...
TL;DR: SP-Det: A self-prompted detection framework that automatically generates textual prompts for multi-label lesion detection in chest X-rays, eliminating the need for manual expert annotations.


<details>
  <summary>Details</summary>
Motivation: Existing promptable detection frameworks for chest X-ray lesion detection require manual expert annotations as prompts, which are labor-intensive and impractical for clinical applications. There's a need to eliminate this dependency on human annotations while maintaining or improving detection accuracy.

Method: Proposes SP-Det with two key components: 1) Expert-free dual-text prompt generator (DTPG) that creates semantic context prompts (global pathological patterns) and disease beacon prompts (disease-specific manifestations), and 2) Bidirectional feature enhancer (BFE) that integrates diagnostic context with disease-specific embeddings to improve feature representation.

Result: Extensive experiments on two chest X-ray datasets with diverse thoracic disease categories show that SP-Det outperforms state-of-the-art detection methods while completely eliminating dependency on expert-annotated prompts compared to existing promptable architectures.

Conclusion: SP-Det provides an effective solution for automated lesion detection in chest X-rays by generating its own rich textual context prompts, making the system more practical for clinical applications by removing the need for labor-intensive manual annotations.
...
TL;DR: SDG-Track is a real-time UAV tracking system for edge devices that uses an Observer-Follower architecture to resolve the resolution-speed conflict, achieving 35.1 FPS while maintaining 97.2% detection precision on NVIDIA Jetson Orin Nano.


<details>
  <summary>Details</summary>
Motivation: Real-time tracking of small UAVs on edge devices faces a fundamental resolution-speed conflict: downsampling high-resolution imagery causes small target features to collapse below detectable thresholds, while processing native 1080p frames yields insufficient throughput for smooth gimbal control.
...
Conclusion: SDG-Track effectively reconciles the resolution-speed conflict for real-time UAV tracking on edge devices through its Observer-Follower architecture and training-free recovery mechanism, enabling practical deployment on resource-constrained platforms.
...
<details>
  <summary>Details</summary>
Motivation: Address catastrophic forgetting in object detection where models need complete retraining when new products are added, leading to high training costs and time consumption, especially problematic in retail with frequent product introductions.

Method: YOTO integrates YOLO11n for object localization, DeIT for feature extraction, and Proxy Anchor Loss for metric learning. Classification uses cosine similarity between target product embeddings and a Qdrant vector database.

Result: Achieved encouraging accuracy for both new and existing products in retail case study with 140 products. Training time efficiency improved 3x compared to classical approaches, with average inference time of 580ms per image on edge device.

Conclusion: YOTO framework effectively addresses catastrophic forgetting in object detection, enabling efficient addition of new products without retraining, making it practical for real-world retail applications with frequent product updates.
...
TL;DR: E(3)-Pose is a fast pose estimation method that jointly models rotation equivariance and object symmetry for robust fetal head pose estimation in MRI scans, enabling automatic adaptive prescription of 2D diagnostic slices.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenging problem of accounting for fetal head motion during diagnostic MRI scans. The goal is to enable automatic adaptive prescription of 2D diagnostic MRI slices with 6-DoF head pose estimation, supported by rapidly acquired 3D MRI volumes before each 2D slice. Existing methods struggle with clinical volumes due to pose ambiguities from anatomical symmetries, low resolution, noise, and artifacts.

Method: E(3)-Pose is a novel pose estimation method that jointly and explicitly models rotation equivariance and object symmetry. The method captures anatomical symmetries and rigid pose equivariance by construction, making it robust to the challenges in clinical MRI volumes.
...
Conclusion: E(3)-Pose provides a robust solution for fetal head pose estimation in clinical MRI settings by explicitly modeling rotation equivariance and object symmetry, addressing key limitations of existing methods and enabling practical clinical applications.
...
Method: ReflexFlow introduces two components: 1) Anti-Drift Rectification (ADR) that reflexively adjusts prediction targets for biased inputs using redesigned loss with training-time scheduled sampling, and 2) Frequency Compensation (FC) that reflects on missing low-frequency components and compensates them by reweighting the loss using exposure bias.

Result: Experiments on CIFAR-10, CelebA-64, and ImageNet-256 show ReflexFlow outperforms prior approaches in mitigating exposure bias, achieving a 35.65% reduction in FID on CelebA-64.

Conclusion: ReflexFlow is a model-agnostic, simple and effective solution that improves Flow Matching generation quality by dynamically correcting exposure bias through reflexive refinement of the learning objective.
...
TL;DR: SFD introduces asynchronous semantic-first diffusion where semantic latents denoise before texture latents, enabling faster convergence and better image generation.


<details>
  <summary>Details</summary>
Motivation: Current LDMs denoise semantic and texture information synchronously, ignoring the natural coarse-to-fine generation process where semantics form before textures. This misses the opportunity for semantic guidance to improve texture generation.

Method: SFD constructs composite latents combining compact semantic latents (from pretrained visual encoder via Semantic VAE) with texture latents. It uses separate noise schedules with temporal offset - semantics denoise first, providing clearer high-level guidance for subsequent texture refinement.
...
Conclusion: Asynchronous, semantics-led modeling is effective for latent diffusion, enabling natural coarse-to-fine generation with superior performance and faster convergence.
...
TL;DR: First automated top-down method for virtually unrolling severely damaged Herculaneum Papyri using explicit parametric surface modeling on CT scans


<details>
  <summary>Details</summary>
Motivation: Herculaneum Papyri contain unseen Greek/Latin texts but are too fragile to physically unroll; current virtual unrolling methods are laborious and need automation for gigavoxel CT scans

Method: Global fitting of explicit parametric model to neural network predictions of papyrus locations in CT scans, ensuring continuous 2D sheet even through undetectable regions

Result: Successfully unrolls large regions of two scrolls in high-resolution CT scans, outperforming existing automated methods for this data

Conclusion: Presents first effective automated top-down approach for virtual unrolling of severely damaged ancient scrolls, enabling access to previously inaccessible texts
...
TL;DR: LiteVGGT achieves 10x speedup and substantial memory reduction for 3D vision foundation models, enabling efficient processing of 1000-image scenes through geometry-aware cached token merging.


<details>
  <summary>Details</summary>
Motivation: Current 3D vision foundation models like VGGT are time-consuming and memory-intensive for long sequences, limiting their application to large-scale scenes beyond hundreds of images.

Method: Proposes geometry-aware cached token merging based on two insights: (1) tokens from local image regions have inherent geometric correlations leading to computational redundancy, and (2) token similarity across adjacent layers remains stable allowing reusable merge decisions. Analyzes each token's geometric importance for optimal anchor selection and caches/reuses merge indices across layers.

Result: Achieves up to 10x speedup and substantial memory reduction, enabling efficient processing of 1000-image scenes while retaining VGGT's core performance. Also enables efficient fine-tuning and FP8 quantization for further gains.

Conclusion: LiteVGGT provides an effective, scalable, and robust solution for efficient large-scale 3D reconstruction, validated through extensive experiments.
...
TL;DR: A novel multimodal action recognition method using deep neural networks with gating mechanisms for adaptive fusion of RGB, optical flow, audio, and depth data, achieving improved accuracy over unimodal approaches.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of traditional unimodal action recognition methods by leveraging multiple modalities (RGB, optical flow, audio, depth) for more comprehensive and robust action understanding, with applications in surveillance, human-computer interaction, and active assisted living.

Method: Deep neural network architecture with gating mechanisms and adaptive weighting-based fusion strategies that selectively integrate relevant information from multiple modalities. The approach explores various gated fusion strategies to identify optimal multimodal integration.

Result: Demonstrated promising advancements in accuracy across human action recognition, violence action detection, and self-supervised learning tasks on benchmark datasets. The method shows superiority over conventional unimodal approaches through more holistic action representations.

Conclusion: The gating-based multimodal fusion approach represents a significant advancement in action recognition, with potential to revolutionize systems across diverse fields including surveillance and human-computer interaction, particularly for active assisted living applications.
...
TL;DR: FASTer is a unified framework for efficient robot learning that combines a learnable tokenizer (FASTerVQ) with an autoregressive policy (FASTerVLA) to improve both inference speed and task performance while maintaining strong generalization.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive VLA models face a trade-off between reconstruction fidelity and inference efficiency in their action tokenization process. There's a need for a framework that can achieve both high performance and fast inference in robotic manipulation tasks.

Method: FASTerVQ encodes action chunks as single-channel images to capture global spatio-temporal dependencies with high compression. FASTerVLA then builds on this tokenizer with block-wise autoregressive decoding and a lightweight action expert for efficient policy execution.

Result: FASTerVQ achieves superior reconstruction quality, high token utilization, and strong cross-task/cross-embodiment generalization. FASTerVLA surpasses previous state-of-the-art VLA models in both inference speed and task performance across simulated and real-world benchmarks.

Conclusion: The FASTer framework successfully addresses the efficiency-performance trade-off in VLA models, providing a unified solution that delivers both faster inference and higher task performance while maintaining strong generalization capabilities.
...
TL;DR: GeoPE extends RoPE to 2D vision by using quaternions and Lie algebra to create geometrically coupled positional embeddings that preserve spatial topology.


<details>
  <summary>Details</summary>
Motivation: Standard ViTs flatten 2D images into 1D sequences, disrupting spatial topology. Existing 2D RoPE variants treat spatial axes independently, failing to decouple false sequential proximity from true spatial distance.

Method: Extends rotations to 3D Euclidean space using quaternions. Overcomes non-commutativity by constructing unified rotational operator via geometric mean in Lie algebra, creating geometrically coupled encoding that separates spatial dimensions.

Result: Extensive experiments on image classification, object detection, and 3D semantic segmentation show GeoPE consistently outperforms existing 2D RoPE variants and significantly enhances shape bias.

Conclusion: GeoPE effectively captures true geometric structure by restoring the 2D spatial manifold, confirming its ability to preserve spatial topology in vision transformers.
...
TL;DR: A balanced few-shot learning framework for retinal disease diagnosis that addresses class imbalance through balanced episodic sampling, targeted augmentation (CLAHE), and ResNet-50 encoding, achieving improved accuracy and reduced bias for underrepresented diseases.


<details>
  <summary>Details</summary>
Motivation: Automated retinal disease diagnosis faces challenges with conventional deep learning requiring large annotated datasets that are costly and often imbalanced across disease categories, limiting practical reliability. Few-shot learning offers a solution by enabling generalization from few samples.

Method: Proposes a balanced few-shot episodic learning framework with three key components: (1) balanced episodic sampling ensuring equal class participation in 5-way 5-shot episodes, (2) targeted augmentation including CLAHE and color/geometry transformations to improve minority-class diversity, and (3) ResNet-50 encoder pretrained on ImageNet for fine-grained feature capture. Uses prototype computation in embedding space with cosine similarity classification.

Result: The framework achieves substantial accuracy gains and reduces bias toward majority classes, with notable improvements for underrepresented diseases. Trained on 100 episodes and evaluated on 1,000 test episodes, it demonstrates robust performance on the RFMiD dataset focusing on ten most represented classes.

Conclusion: Dataset-aware few-shot pipelines combined with balanced sampling and CLAHE-enhanced preprocessing can deliver more robust and clinically fair retinal disease diagnosis under data-constrained conditions, addressing the class imbalance problem in medical imaging.
...
TL;DR: MoLD: A novel adaptive method that dynamically integrates features from multiple ViT layers using gating mechanism for AI-generated image detection, outperforming final-layer-only approaches.


<details>
  <summary>Details</summary>
Motivation: Existing AI-generated image detection methods primarily use final-layer CLIP-ViT features, but systematic analysis reveals earlier layers provide more localized and generalizable features that can improve detection performance.

Method: MoLD (Multi-layer Dynamic integration) - an adaptive method that dynamically integrates features from multiple ViT layers using a gating-based mechanism to leverage distinct aspects captured by different layers.

Result: Extensive experiments show MoLD significantly improves detection performance on both GAN- and diffusion-generated images, enhances generalization across diverse generative models, and exhibits robustness in real-world scenarios.

Conclusion: The approach demonstrates scalability and versatility by successfully applying to other pre-trained ViTs like DINOv2, showing that adaptive multi-layer feature integration is superior to final-layer-only methods for AI-generated image detection.
...
TL;DR: A family of stable contrastive losses for learning pixel-level representations that capture both semantic and geometric information, enabling precise point-correspondence across images without momentum-based teacher-student training.


<details>
  <summary>Details</summary>
Motivation: To develop pixel-level representations that jointly capture semantic and geometric information in a stable manner, addressing the need for precise point-correspondence across images without complex momentum-based training architectures.

Method: Proposes stable contrastive losses that map each pixel to overcomplete descriptors that are both view-invariant and semantically meaningful. The approach avoids momentum-based teacher-student training while maintaining stability.

Result: Demonstrated through experiments in synthetic 2D and 3D environments, showing the properties of the proposed loss and the effectiveness of the resulting overcomplete representations for point-correspondence tasks.

Conclusion: The proposed stable contrastive loss family successfully learns pixel-level representations that capture both semantic and geometric information, enabling precise point-correspondence across images without requiring momentum-based teacher-student training.
...
TL;DR: LVLM-based text-to-image systems amplify social biases more than non-LVLM models, with system prompts identified as the primary driver. The paper introduces FairPro, a training-free meta-prompting framework that reduces bias while maintaining text-image alignment.


<details>
  <summary>Details</summary>
Motivation: While large vision-language models dominate text-to-image generation, their tendency to amplify social biases remains insufficiently understood. The authors aim to investigate this bias amplification and develop practical solutions.

Method: Created a 1,024-prompt benchmark across four linguistic complexity levels. Used decoded intermediate representations, token-probability diagnostics, and embedding-association analyses to trace bias propagation. Proposed FairPro, a training-free meta-prompting framework that enables LVLMs to self-audit and construct fairness-aware system prompts at test time.

Result: LVLM-based models produce significantly more socially biased images than non-LVLM models. System prompts were identified as the primary driver of biased behavior. FairPro substantially reduces demographic bias while preserving text-image alignment in experiments with SANA and Qwen-Image models.

Conclusion: System prompts play a central role in bias propagation in LVLM-based T2I systems. FairPro offers a practical, deployable approach for building more socially responsible text-to-image systems without requiring model retraining.
...
TL;DR: Memory-optimized version of VANICP point cloud registration algorithm for embedded GPUs, reducing memory usage by over 97% while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: VANICP is computationally efficient but memory-intensive, limiting deployment on resource-constrained embedded systems with limited hardware resources.

Method: Proposes GPU-oriented dynamic memory assignment strategy to optimize memory usage of dilation operation, creating enhanced VANICP framework.

Result: Achieves over 97% reduction in memory consumption while preserving original performance, enabling lightweight execution on embedded GPUs.

Conclusion: Successfully addresses memory constraints for VANICP deployment in embedded systems through optimized memory management strategy.
...
TL;DR: DiT-based framework for single-image reflection removal using pre-trained diffusion transformers conditioned on reflection inputs and guided to clean outputs, with synthetic PBR data and LoRA adaptation achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: To leverage the generalization capabilities of foundation diffusion models for reflection removal, addressing the limitations of task-specific architectures and insufficient realistic training data.

Method: Repurpose pre-trained DiT foundation model by conditioning on reflection-contaminated inputs and guiding toward clean transmission layers; create PBR pipeline in Blender for realistic synthetic data; use LoRA-based adaptation for efficient fine-tuning.
...
Conclusion: Pretrained diffusion transformers combined with physically grounded data synthesis and efficient adaptation provide a scalable, high-fidelity solution for single-image reflection removal.
...
<details>
  <summary>Details</summary>
Motivation: Transparent objects pose challenges for depth sensing due to refraction/reflection. Previous supervised methods require costly depth annotation data.

Method: Proposes self-supervised training by simulating depth deficits of transparent objects within non-transparent regions, using original depth maps as ground truth for supervision.

Result: Achieves performance comparable to supervised approaches. Pre-training with this method improves model performance when training samples are limited.

Conclusion: The self-supervised method effectively addresses the data annotation challenge for transparent object depth completion while maintaining competitive performance.
...
TL;DR: GNVC-VD is the first DiT-based generative neural video compression framework that unifies spatio-temporal latent compression with sequence-level generative refinement using a video diffusion transformer to eliminate perceptual flickering artifacts.


<details>
  <summary>Details</summary>
Motivation: Existing perceptual codecs rely on frame-wise image generative priors that lack temporal modeling, causing perceptual flickering artifacts. There's a need for video-native generative priors that can ensure temporal coherence while maintaining high perceptual quality, especially at extreme low bitrates.

Method: GNVC-VD introduces a unified flow-matching latent refinement module using a video diffusion transformer to jointly enhance intra- and inter-frame latents through sequence-level denoising. Instead of starting from pure Gaussian noise, it initializes refinement from decoded latents and learns a correction term for compression-induced degradation. A conditioning adaptor injects compression-aware cues into DiT layers for artifact removal while maintaining temporal coherence.

Result: Extensive experiments show GNVC-VD surpasses both traditional and learned codecs in perceptual quality, significantly reduces flickering artifacts that persist in prior generative approaches, and performs well even below 0.01 bpp.
...
TL;DR: HTR-ConvText: A hybrid CNN-ViT model with stroke-level feature extraction and textual context injection for improved handwritten text recognition, especially with limited data and complex scripts.


<details>
  <summary>Details</summary>
Motivation: Handwritten Text Recognition faces challenges due to limited data, high writing style variance, and complex scripts with diacritics. Existing approaches struggle to generalize without massive synthetic data.

Method: Proposes HTR-ConvText with: 1) Residual CNN backbone + MobileViT with Positional Encoding for stroke-level local features and global context, 2) ConvText encoder combining global context and local features in hierarchical structure, 3) Auxiliary module injecting textual context to mitigate CTC weaknesses.
...
Conclusion: HTR-ConvText effectively addresses HTR challenges by capturing fine-grained stroke-level features while preserving global context, achieving superior performance in data-limited scenarios with diverse handwriting styles.
...
TL;DR: RAMEN is a resolution-adjustable multimodal encoder for Earth observation data that learns sensor-agnostic representations across diverse spatial, spectral, and temporal resolutions, enabling users to control output resolution for precision-computation trade-offs.


<details>
  <summary>Details</summary>
Motivation: Current foundation models for Earth observation data often require fixed input resolutions or use sensor-specific encoders, limiting their ability to generalize across the wide range of heterogeneous EO modalities with varying spatial, spectral, and temporal resolutions.

Method: RAMEN treats modality, spatial, and temporal resolutions as key input features and defines spatial resolution as a controllable output parameter. It uses a single unified transformer encoder trained on masked multimodal EO data reconstruction from diverse sources, enabling sensor-agnostic representation learning.

Result: RAMEN effectively transfers to both known and unseen sensor configurations and outperforms larger state-of-the-art models on the PANGAEA benchmark, which contains various multi-sensor and multi-resolution downstream tasks.

Conclusion: RAMEN provides a unified approach for Earth observation data analysis by enabling coherent analysis across modalities within a shared latent space while giving users direct control over spatial resolution for precision-computation trade-offs, demonstrating superior performance on standard benchmarks.
...
TL;DR: A novel semantic-guided hierarchical synthesis architecture for facial image inpainting that handles large irregular masks better than existing methods by combining CNN and Vision Transformer features for semantic layout generation, followed by multi-modal texture refinement.
...
Method: Two-stage semantic-guided hierarchical synthesis: 1) Semantic layout generation using CNN (local features) and Vision Transformer (global features) fusion, 2) Multi-Modal Texture Generator for texture refinement with multi-scale information. Architecture handles arbitrary masks through dynamic attention without mask-specific training.

Result: Outperforms state-of-the-art methods on CelebA-HQ and FFHQ datasets, showing improvements in LPIPS, PSNR, and SSIM metrics. Produces visually striking results with better semantic preservation in challenging large-area inpainting scenarios.

Conclusion: The proposed semantic-guided hierarchical synthesis approach effectively addresses challenges in facial image inpainting, particularly for large irregular masks, by leveraging semantic understanding before texture generation, resulting in improved structural consistency and photorealistic quality.
...
TL;DR: MoRe4D is a novel framework for generating interactive 4D scenes from single static images by jointly performing motion generation and geometric reconstruction, addressing spatiotemporal inconsistencies in existing methods.
...
Method: 1) Created TrajScene-60K dataset with 60K video samples and dense point trajectories; 2) Proposed 4D-STraG diffusion model to generate geometrically consistent 4D point trajectories; 3) Designed depth-guided motion normalization and motion-aware module for geometry-dynamics integration; 4) Developed 4D-ViSM to render videos from 4D point track representations.
...
Conclusion: The proposed joint motion generation and geometric reconstruction framework effectively addresses spatiotemporal inconsistencies in 4D scene synthesis from single images, enabled by a new large-scale dataset and novel diffusion-based trajectory generation approach.
...
<details>
  <summary>Details</summary>
Motivation: Existing 4D semantic field construction methods rely on scene-specific Gaussian splatting, which requires per-scene optimization, has limited generalization, and is difficult to scale to real-world applications.

Method: Proposes 4DLangVGGT with two key components: 4D Visual Geometry Transformer (StreamVGGT) for spatio-temporal geometric representations, and Semantic Bridging Decoder (SBD) that projects geometry-aware features into language-aligned semantic space.

Result: Achieves state-of-the-art performance on HyperNeRF and Neu3D datasets with up to 2% gains under per-scene training and 1% improvements under multi-scene training, demonstrating strong generalization and deployment efficiency.

Conclusion: 4DLangVGGT establishes a new paradigm for open-vocabulary 4D scene understanding by enabling joint training across multiple dynamic scenes and direct inference application, significantly improving practicality for large-scale deployment.
...
TL;DR: A 4D-controllable video diffusion framework that decouples scene dynamics from camera motion, enabling independent control of both through continuous world-time sequences and camera trajectories as conditioning inputs.
...
Method: Introduces a framework with continuous world-time sequences and camera trajectories as conditioning inputs, using 4D positional encoding in attention layers and adaptive normalizations for feature modulation. Trained on a unique dataset with independently parameterized temporal and camera variations.

Result: The model achieves robust real-world 4D control across diverse timing patterns and camera trajectories while preserving high generation quality, outperforming prior work in controllability.

Conclusion: The proposed framework successfully decouples scene dynamics from camera pose, enabling precise 4D control in video generation with superior controllability compared to existing methods.
...
TL;DR: Contact-guided 3D generation for object reconstruction using shape priors from generative models and contact information from videos/interactions to reduce ambiguity in partial observations.


<details>
  <summary>Details</summary>
Motivation: Object reconstruction is challenging due to partial observations from cameras, especially with occlusions. Vision signals alone are ambiguous for complete geometry reconstruction.

Method: Combines two information sources: 1) generative models for shape priors to guess unseen geometry, and 2) contact information from videos/physical interactions for sparse boundary constraints. Uses contact-guided 3D generation inspired by drag-based editing in generative models.

Result: Experiments on synthetic and real-world data show improved reconstruction compared to pure 3D generation and contact-based optimization approaches.

Conclusion: Leveraging both generative shape priors and contact information through guided 3D generation effectively reduces ambiguity and improves object geometry reconstruction for robot manipulation.
...
TL;DR: Deep Forcing enables training-free long-video generation with 12x extrapolation using attention sink optimization and KV cache pruning.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive video diffusion models suffer from temporal repetition, drift, and motion deceleration during long rollouts. StreamingLLM-style attention sinks degrade video quality when applied naively to video diffusion.

Method: Two training-free mechanisms: 1) Deep Sink dedicates half sliding window to persistent sink tokens with temporal RoPE phase realignment; 2) Participative Compression performs importance-aware KV cache pruning to preserve only actively participating tokens while discarding redundant history.

Result: Enables over 12x extrapolation (e.g., 5s-trained to 60s+ generation) with better imaging quality than LongLive, better aesthetic quality than RollingForcing, maintains overall consistency, and improves dynamic degree while maintaining real-time generation.

Conclusion: Training-free KV-cache management can match or exceed training-based approaches for autoregressively streaming long-video generation, demonstrating effective solutions for temporal stability in video diffusion models.
...
TL;DR: The paper introduces Visual Reasoning Tracer (VRT) to make multimodal LLMs reveal their reasoning steps, creates VRT-Bench for evaluation and VRT-80k for training, showing current models struggle with intermediate reasoning but training on VRT-80k improves trace quality.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs produce only final predictions without revealing intermediate reasoning steps or fine-grained evidence, unlike human visual reasoning which operates through chains of reasoning. This opacity limits understanding and trust in model decisions.

Method: Introduces VRT task requiring models to localize target objects AND predict intermediate objects forming reasoning paths. Creates VRT-Bench (human-annotated evaluation benchmark), new metric for reasoning trace quality, and VRT-80k (large-scale training dataset).

Result: Existing models often produce correct final outputs but struggle to ground intermediate reasoning. Models trained on VRT-80k achieve substantial improvements in tracing reasoning paths and generating interpretable reasoning traces.

Conclusion: The VRT framework addresses the opacity of MLLM reasoning by making intermediate reasoning steps explicit, enabling better understanding, evaluation, and improvement of visual reasoning capabilities in multimodal models.
...
TL;DR: The paper introduces Spatial Aesthetics (SA-IQA), a new paradigm for assessing aesthetic quality of AI-generated interior scenes across four dimensions, with a benchmark dataset and applications to improve AIGC generation.


<details>
  <summary>Details</summary>
Motivation: Existing Image Quality Assessment (IQA) methods for AI-generated images focus mainly on portraits and artistic images, lacking systematic evaluation of interior scenes. There's a need for comprehensive assessment of spatial aesthetics in interior design images.

Method: 1) Introduced Spatial Aesthetics paradigm with four dimensions: layout, harmony, lighting, and distortion. 2) Built SA-BENCH benchmark with 18,000 images and 50,000 annotations. 3) Developed SA-IQA through MLLM fine-tuning and multidimensional fusion approach. 4) Applied SA-IQA to downstream tasks: GRPO reinforcement learning optimization and Best-of-N selection filtering.

Result: SA-IQA significantly outperforms existing methods on SA-BENCH benchmark. The framework sets a new standard for spatial aesthetics evaluation and effectively improves AIGC generation quality through reward optimization and filtering.

Conclusion: The paper establishes a comprehensive framework for assessing spatial aesthetics in AI-generated interior scenes, providing both benchmark and practical tools to advance research and applications in this domain. Code and dataset will be open-sourced.
...
TL;DR: EvoIR is an All-in-One Image Restoration framework that uses evolutionary frequency modulation to dynamically adapt to diverse degradations, outperforming state-of-the-art methods.
...
Method: Proposes EvoIR with two key components: 1) Frequency-Modulated Module (FMM) that explicitly decomposes features into high/low-frequency branches and adaptively modulates them, and 2) Evolutionary Optimization Strategy (EOS) that iteratively adjusts frequency-aware objectives through population-based evolutionary process to balance structural accuracy and perceptual fidelity.

Result: EvoIR outperforms state-of-the-art AiOIR methods on multiple benchmarks, with the combination of FMM and EOS showing greater improvements than using either component alone.

Conclusion: EvoIR provides a robust and versatile framework for AiOIR tasks through evolutionary frequency modulation, effectively addressing diverse degradation types with better generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To bridge algorithmic design with artistic storytelling by creating a practical pipeline for shadow-drawing compositional art, expanding computational visual art possibilities.

Method: Optimizes scene parameters (object pose, lighting) to reveal meaningful shadows, uses shadow strokes to guide line drawing generation, and employs automatic evaluation for shadow-drawing coherence and visual quality.

Result: Produces compelling results across diverse inputs including real-world scans, curated datasets, and generative assets, with extensions to multi-object scenes, animations, and physical deployments.

Conclusion: Provides a practical pipeline for creating shadow-drawing art and broadens computational visual art design space, successfully bridging algorithmic design and artistic storytelling.
...
TL;DR: ARM-Thinker is an agentic multimodal reward model that uses external tools for verification, improving visual grounding and reliability over static reward models.
...
Result: Achieves +16.2% average improvement on reward modeling benchmarks, +9.6% on tool-use tasks, and outperforms baselines on multimodal math and logical reasoning benchmarks.

Conclusion: Agentic capabilities significantly enhance both accuracy and interpretability of reward models for vision-language systems.
...
TL;DR: DraCo introduces an interleaved reasoning paradigm that generates low-resolution draft images as visual planning, verifies semantic alignment with prompts, and refines through selective corrections, addressing limitations of purely textual CoT reasoning in multimodal LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal LLMs with chain-of-thought reasoning are limited by either treating models as standalone generators or relying on abstract textual planning, lacking concrete visual guidance and verification mechanisms.

Method: 1. Generate low-resolution draft image as visual preview for concrete structural planning. 2. Use model's understanding to verify semantic alignment between draft and prompt. 3. Perform refinement through selective corrections with super-resolution. 4. Introduce DraCo-240K dataset for training three atomic capabilities. 5. Develop DraCo-CFG, a specialized classifier-free guidance strategy for interleaved reasoning.

Result: DraCo achieves significant improvements: +8% on GenEval, +0.91 on Imagine-Bench, and +3% on GenEval++, outperforming direct generation and other CoT-empowered generation methods.

Conclusion: The interleaved reasoning paradigm with draft-as-CoT effectively addresses coarse-grained textual planning limitations and rare attribute combination challenges, demonstrating superior performance through visual planning and verification mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Synthesizing high-fidelity frozen 3D scenes from monocular Mannequin-Challenge videos presents unique challenges distinct from standard dynamic scene reconstruction. The goal is to create frozen scenes while preserving subtle dynamics for user-controlled instant selection, but current methods suffer from artifacts like ghosting and blur due to sparse temporal supervision.

Method: The paper introduces Splannequin, an architecture-agnostic regularization method that detects two states of Gaussian primitives (hidden and defective) and applies temporal anchoring. Under forward camera motion, hidden states are anchored to recent well-observed past states, while defective states are anchored to future states with stronger supervision. The method integrates into existing dynamic Gaussian pipelines via simple loss terms without architectural changes.

Result: The method achieves markedly improved visual quality, enabling high-fidelity, user-selectable frozen-time renderings. It's validated by 96% user preference and adds zero inference overhead to existing pipelines.

Conclusion: Splannequin effectively addresses artifacts in frozen scene synthesis from monocular videos through temporal anchoring of Gaussian states, providing a practical solution that integrates seamlessly with existing dynamic Gaussian splatting methods while significantly improving visual quality.
...
TL;DR: Light-X is a video generation framework that enables joint control of both camera viewpoint and illumination from monocular videos, addressing the trade-off between lighting fidelity and temporal consistency.
...
Method: 1) Disentangled design that decouples geometry and lighting signals: geometry/motion captured via dynamic point clouds projected along user-defined camera trajectories, while illumination cues from relit frames consistently projected into same geometry. 2) Light-Syn pipeline: degradation-based synthesis with inverse-mapping to create training pairs from in-the-wild monocular footage, covering static, dynamic, and AI-generated scenes.
...
Conclusion: Light-X enables controllable video rendering with both viewpoint and illumination control, representing a key step toward generative modeling of real-world scenes through effective disentanglement of geometry and lighting signals.
