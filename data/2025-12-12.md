...
TL;DR: This paper presents a neuromorphic approach to eye tracking using spiking neural networks (SNNs) that achieves high accuracy with dramatically reduced model size and computational cost compared to conventional ANN approaches.


<details>
  <summary>Details</summary>
Motivation: Wearable eye tracking systems for AR/VR require low latency and milliwatt-level power, but conventional frame-based pipelines suffer from motion blur, high computational cost, and limited temporal resolution. While neuromorphic sensors and SNNs offer promise, existing approaches are either too specialized or underperform compared to modern ANN architectures.

Method: The authors create neuromorphic versions of top-performing event-based eye-tracking models by replacing recurrent and attention modules with lightweight Leaky Integrate-and-Fire (LIF) layers and exploiting depth-wise separable convolutions to reduce model complexity.

Result: The models achieve 3.7-4.1px mean error, approaching the accuracy of the specialized neuromorphic system Retina (3.24px), while reducing model size by 20x and theoretical compute by 850x compared to the closest ANN variant. The efficient variants are projected to operate at 3.9-4.9 mW with 3 ms latency at 1 kHz.
...
<details>
  <summary>Details</summary>
Motivation: Previous HBox-supervised OOD methods have limitations in scale estimation by directly comparing GT HBoxes with minimum circumscribed rectangles of predicted RBoxes, leading to inaccurate scale predictions. There's also a need to address learning collapse issues in orientation prediction.

Method: Proposes two key components: 1) Adaptive Bounding Box Scaling (ABBS) that appropriately scales GT HBoxes to optimize for each predicted RBox size, and 2) Symmetric Prior Angle (SPA) loss that exploits inherent symmetry of aerial objects for self-supervised learning to prevent learning collapse.
...
Conclusion: ABBSPO effectively addresses limitations in previous HBox-supervised OOD methods through adaptive scaling and symmetry-based orientation prediction, establishing a new state-of-the-art in weakly supervised oriented object detection.
...
TL;DR: SST combines diffusion models with autoregressive generation for image captioning, achieving SOTA results on COCO without RL.


<details>
  <summary>Details</summary>
Motivation: Diffusion models underperform autoregressive models in discrete domains like language generation. The paper aims to combine strengths of both approaches - diffusion's bidirectional/refining capabilities with autoregressive models' linguistic structure.

Method: Proposes Show, Suggest and Tell (SST) - a hybrid approach where diffusion models provide suggestions to guide autoregressive generation rather than replacing it entirely.
...
Conclusion: The hybrid approach combining diffusion suggestions with autoregressive generation is a promising underexplored direction that yields state-of-the-art results in image captioning.
...
TL;DR: MetaVoxel is a joint diffusion model that learns the complete distribution over brain MRI scans and clinical metadata, enabling unified zero-shot inference for multiple tasks without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Current medical AI models are typically trained for specific conditional tasks (e.g., disease classification, biomarker estimation, image generation) requiring separate models for each task. This limits flexibility and clinical applicability.

Method: MetaVoxel uses a generative joint diffusion modeling framework that learns a single diffusion process spanning all variables (imaging data and clinical metadata). It models the joint distribution rather than conditional distributions.
...
Conclusion: Joint multimodal diffusion modeling offers a promising direction for unifying medical AI models, enabling flexible zero-shot inference using arbitrary subsets of inputs without task-specific retraining, thus broadening clinical applicability.
...
TL;DR: Proposes Independent Density Estimation (IDE) method to improve compositional generalization in vision-language models by learning connections between individual words and corresponding image features.


<details>
  <summary>Details</summary>
Motivation: Large-scale vision-language models achieve good results but struggle with human-like compositional generalization - understanding novel combinations of known concepts.
...
Conclusion: IDE effectively addresses compositional generalization challenges in vision-language models by learning word-image feature connections and using entropy-based inference.
...
TL;DR: TraceFlow is a framework for high-fidelity rendering of dynamic specular scenes using a novel Gaussian splatting representation and hybrid rendering pipeline that combines rasterization and ray tracing for physically accurate reflections.


<details>
  <summary>Details</summary>
Motivation: The paper addresses two key challenges in rendering dynamic specular scenes: precise reflection direction estimation and physically accurate reflection modeling. Existing methods struggle with high-fidelity rendering of specular reflections in complex dynamic environments.

Method: 1) Residual Material-Augmented 2D Gaussian Splatting representation for modeling dynamic geometry and material properties; 2) Dynamic Environment Gaussian and hybrid rendering pipeline that decomposes rendering into diffuse and specular components; 3) Coarse-to-fine training strategy for optimization stability and physically meaningful decomposition.

Result: TraceFlow outperforms prior methods on dynamic scene benchmarks both quantitatively and qualitatively, producing sharper and more realistic specular reflections in complex dynamic environments.

Conclusion: The proposed framework successfully addresses the challenges of dynamic specular scene rendering through novel representations and rendering techniques, achieving state-of-the-art performance in generating physically accurate reflections.
...
Method: Introduces a new benchmark dataset with 2,765 unique entities tracked in 552 videos belonging to 40 categories (objects and parts). Evaluates seven variants of four existing models adapted to the hierarchical tracking task.

Result: The new dataset proves challenging for existing models, with seven variants of four models struggling to perform well on the hierarchical instance tracking task.

Conclusion: Hierarchical instance tracking is a novel and challenging task that requires new approaches beyond current tracking methods, with the introduced dataset serving as a benchmark for future research.
...
TL;DR: A wavelet-based persistent homology method improves breast cancer detection model generalization across different mammography scanners and populations by adding topological image structure features.


<details>
  <summary>Details</summary>
Motivation: Breast cancer screening mammography suffers from false positives/negatives and poor generalization across different scanners, modalities, and patient populations, requiring methods that improve external performance.

Method: Uses topological data analysis (persistent homology) to create wavelet-based vectorizations of image structure that persist across intensity thresholds, generating stable spatial multi-scale maps. These maps are integrated into a two-stage detection pipeline via channel concatenation.

Result: On the INbreast dataset, augmenting ConvNeXt Tiny with wavelet persistence channels increased patient-level AUC from 0.55 to 0.75 under limited training budget, showing significant improvement in cross-population generalization.

Conclusion: Wavelet-based persistent homology provides stable, multi-scale topological features that significantly improve breast cancer detection model generalization across different scanners and populations, addressing a key limitation in current screening systems.
...
TL;DR: MPEG's Feature Coding for Machines (FCM) standard enables efficient edge-cloud inference by compressing intermediate DNN features, achieving ~85% bitrate reduction while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Deploying DNNs on edge devices is challenging due to high compute demands. Traditional approaches (full on-device or cloud offloading) face latency, bandwidth, and privacy trade-offs. Edge-cloud splitting offers balance but introduces bandwidth issues from transmitting intermediate features.

Method: MPEG initiated the Feature Coding for Machines (FCM) standard, creating a bitstream syntax and codec pipeline specifically for compressing intermediate DNN features. The paper presents the design and performance of the Feature Coding Test Model (FCTM).

Result: FCTM achieves significant bitrate reductions averaging 85.14% across multiple vision tasks while preserving model accuracy. The standard provides a scalable solution for bandwidth-limited and privacy-sensitive applications.

Conclusion: FCM offers a scalable path for efficient and interoperable deployment of intelligent features in consumer applications with bandwidth constraints and privacy concerns, enabling practical edge-cloud inference splitting.
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models for autonomous driving use natural language for chain-of-thought reasoning, but text may not be the most efficient representation for reasoning about driving actions and outcomes.

Method: Proposes Latent-CoT-Drive (LCDrive) that uses latent language with two token types: action-proposal tokens (same vocabulary as output actions) and world model tokens (learned latent world model for future outcomes). Cold-started with ground-truth future rollouts supervision, then post-trained with closed-loop reinforcement learning.
...
Conclusion: Latent chain-of-thought reasoning in action-aligned space is more efficient than natural language reasoning for autonomous driving, unifying reasoning and decision making while improving performance and safety.
...
TL;DR: FCM (Feature Coding for Machines) compresses neural features for machine-to-machine communication, reducing bandwidth while maintaining accuracy close to edge inference, with existing codecs like HEVC performing well for tracking tasks.


<details>
  <summary>Details</summary>
Motivation: Current machine-to-machine systems rely on pixel-based video streaming optimized for humans, which is bandwidth-intensive, scales poorly, and exposes raw images to third parties. There's a need for more efficient, private, and scalable communication for machine vision tasks.

Method: Two approaches: Video Coding for Machines (VCM) applies task-aware coding in pixel domain, and Feature Coding for Machines (FCM) compresses intermediate neural features. FCM reduces bitrate while preserving privacy and supporting compute offload. The paper analyzes H.26X codecs (AVC, HEVC, VVC) as inner codecs in FCM.

Result: FCM maintains accuracy close to edge inference while significantly reducing bitrate. HEVC and VVC achieve almost identical machine task performance (1.39% BD-Rate increase when replacing VVC with HEVC). AVC yields 32.28% BD-Rate increase compared to VVC. For tracking tasks, codec choice has minimal impact, with HEVC outperforming VVC and achieving -1.81% BD Rate.

Conclusion: Existing hardware for deployed codecs (particularly HEVC) can support machine-to-machine communication without degrading performance, especially for tracking tasks. FCM offers an efficient alternative to pixel-based streaming for machine vision applications.
...
TL;DR: MCDPO addresses reward conflicts in RLHF for diffusion models by introducing a disentangled Bradley-Terry objective with conditional preference vectors, enabling independent optimization of multiple reward axes within a single network.
...
Method: Proposes Multi Reward Conditional DPO (MCDPO) with disentangled Bradley-Terry objective. Introduces preference outcome vectors as conditions during training, allowing independent optimization of each reward axis. Uses dimensional reward dropout for balanced optimization across dimensions.

Result: Extensive experiments on Stable Diffusion 1.5 and SDXL show superior performance on benchmarks. The conditional framework enables dynamic multi-axis control at inference using Classifier Free Guidance to amplify specific reward dimensions without additional training or external reward models.

Conclusion: MCDPO effectively resolves reward conflicts in RLHF for diffusion models, enabling better alignment across multiple evaluation dimensions while maintaining inference-time flexibility for controlling specific reward axes.
...
TL;DR: SWIFT enables effective semi-supervised few-shot learning by finetuning VLMs with simple techniques (classifier initialization & temperature tuning) to overcome flat probability distributions, achieving performance close to supervised learning.


<details>
  <summary>Details</summary>
Motivation: Semi-supervised few-shot learning (SSFSL) has neglected powerful open-source Vision-Language Models (VLMs) despite their availability, while related FSL areas have exploited them. Real-world auto-annotation applications need to leverage these resources, but existing SSL methods fail when applied to VLMs.

Method: SWIFT (Stage-Wise Finetuning with Temperature Tuning) uses two simple techniques: 1) classifier initialization to provide better starting points, and 2) temperature tuning to adjust softmax probabilities. This enables effective finetuning of VLMs on limited labeled data, abundant unlabeled data, and noisy task-relevant data from VLM pretraining sets.

Result: SWIFT outperforms recent FSL and SSL methods by ~5 accuracy points on five SSFSL benchmarks. Remarkably, it even rivals supervised learning that uses ground truth labels for all unlabeled data.

Conclusion: Simple techniques can overcome the flat probability distribution problem in VLMs, enabling effective semi-supervised few-shot learning. SWIFT successfully leverages open-source VLMs for real-world auto-annotation applications, bridging the gap between SSFSL and available resources.
...
TL;DR: RobustSora benchmark evaluates how digital watermarks affect AI-generated video detection, showing detectors partially rely on watermarks with 2-8% performance variations when watermarks are manipulated.
...
Result: Performance variations of 2-8 percentage points under watermark manipulation. Transformer-based models show consistent moderate dependency (6-8pp), while MLLMs exhibit diverse patterns (2-8pp), indicating partial watermark dependency in current detectors.

Conclusion: Current AIGC video detectors partially rely on watermarks, creating vulnerabilities. RobustSora provides essential tools for developing watermark-aware training strategies to advance robust AIGC detection research.
...
TL;DR: THE-Pose introduces a novel category-level 6D pose estimation framework that uses topological priors and hybrid graph fusion to overcome limitations of 3D graph convolution methods, achieving significant performance improvements.
...
Result: Extensive experiments on the REAL275 dataset show THE-Pose achieves a 35.8% improvement over the 3D-GC baseline (HS-Pose) and surpasses previous state-of-the-art by 7.2% across all key metrics. The fused features ensure stability for unseen or complicated objects even under significant occlusions.

Conclusion: THE-Pose effectively addresses limitations of existing 3D-GC methods by incorporating topological priors and hybrid fusion, demonstrating superior performance in category-level 6D pose estimation through better integration of 2D image context and 3D geometric structure.
...
TL;DR: GDKVM is a novel architecture for echocardiography video segmentation that uses Linear Key-Value Association for inter-frame correlations, Gated Delta Rule for memory storage, and Key-Pixel Feature Fusion for multi-scale feature integration, achieving state-of-the-art performance with real-time capability.
...
Method: GDKVM architecture with three key components: 1) Linear Key-Value Association (LKVA) to model inter-frame correlations, 2) Gated Delta Rule (GDR) to efficiently store intermediate memory states, and 3) Key-Pixel Feature Fusion (KPFF) module to integrate local and global features at multiple scales for robustness against boundary blurring and noise.

Result: GDKVM outperforms state-of-the-art methods on two mainstream echocardiography datasets (CAMUS and EchoNet-Dynamic) in terms of segmentation accuracy and robustness while maintaining real-time performance.

Conclusion: GDKVM effectively addresses the trade-off between capturing long-range spatiotemporal dependencies and maintaining computational efficiency with fine-grained feature representation, providing a superior solution for echocardiography video segmentation with practical clinical applicability.
...
<details>
  <summary>Details</summary>
Motivation: Existing NCD methods rely solely on visual features, which suffer from insufficient discriminability and are vulnerable to long-tail data distributions, creating a bottleneck in discovering novel classes effectively.
...
Result: Achieves up to 25.3% improvement in accuracy for unknown classes on CIFAR-100 dataset compared to current methods. Shows unique resilience to long-tail distributions, which is a first in NCD literature.

Conclusion: LLM-NCD successfully overcomes limitations of visual-only NCD methods by leveraging multimodal fusion and prototype-guided clustering, demonstrating significant performance gains and robustness to challenging data distributions.
...
TL;DR: Long-LRM++ improves upon generalizable Gaussian splatting by combining semi-explicit scene representation with lightweight decoder, achieving real-time 14 FPS rendering while matching quality of slower implicit methods and scaling to 64 input views.


<details>
  <summary>Details</summary>
Motivation: Current generalizable Gaussian splatting methods (like Long-LRM) are error-sensitive and produce blurry fine details, while implicit representation methods (like LVSM and LaCT) have high fidelity but are too slow for real-time rendering due to computationally intensive decompression processes.

Method: Long-LRM++ adopts a semi-explicit scene representation combined with a lightweight decoder, avoiding the deep sequential decompression of implicit methods while maintaining their quality benefits. The approach scales to 64 input views at 950×540 resolution.

Result: Achieves real-time 14 FPS rendering on A100 GPU while matching LaCT's rendering quality on DL3DV, scales to 64 input views, and delivers superior novel-view depth prediction on ScanNetv2 compared to direct depth rendering from Gaussians.

Conclusion: The semi-explicit representation with lightweight decoder successfully bridges the gap between quality and speed, enabling real-time performance while maintaining the fidelity benefits of implicit representations, with validated effectiveness through extensive ablation studies.
...
TL;DR: Stronger robust teachers don't always produce more robust students due to robust saturation. The paper identifies adversarial transferability as key and proposes SAAD, which reweights training examples by transferability without extra computation.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial distillation methods often fail to incorporate state-of-the-art robust teachers, and stronger teachers don't necessarily yield more robust students (robust saturation). Current explanations focusing on capacity gaps are incomplete.

Method: Proposes Sample-wise Adaptive Adversarial Distillation (SAAD) that identifies adversarial transferability (fraction of student-crafted adversarial examples effective against teacher) as key factor. SAAD reweights training examples by their measured transferability without additional computational cost.

Result: Experiments on CIFAR-10, CIFAR-100, and Tiny-ImageNet show SAAD consistently improves AutoAttack robustness over prior methods.

Conclusion: Adversarial transferability is crucial for successful robustness transfer in adversarial distillation. SAAD effectively addresses robust saturation by adaptively reweighting examples based on transferability, outperforming existing methods.
...
TL;DR: MotionEdit introduces a new dataset and benchmark for motion-centric image editing, where existing models struggle. The authors propose MotionNFT, a fine-tuning framework that improves motion fidelity without harming general editing capabilities.


<details>
  <summary>Details</summary>
Motivation: Current image editing datasets focus on static appearance changes and lack high-quality motion editing examples. Motion-centric editing is scientifically challenging and practically important for applications like video synthesis and animation.

Method: 1) Created MotionEdit dataset with high-fidelity image pairs from continuous videos showing realistic motion transformations. 2) Developed MotionEdit-Bench benchmark with generative, discriminative, and preference-based metrics. 3) Proposed MotionNFT framework that computes motion alignment rewards based on how well motion flow matches ground truth, guiding models toward accurate motion transformations.

Result: Existing state-of-the-art diffusion models perform poorly on motion editing tasks. MotionNFT consistently improves editing quality and motion fidelity for FLUX.1 Kontext and Qwen-Image-Edit models without sacrificing their general editing abilities.

Conclusion: Motion editing is a challenging but important task. The MotionEdit dataset and benchmark enable proper evaluation, while MotionNFT provides an effective solution for improving motion fidelity in image editing models.
...
TL;DR: ShotDirector is a framework for controllable multi-shot video generation that integrates camera parameter control and editing-pattern-aware prompting to create film-like shot transitions.


<details>
  <summary>Details</summary>
Motivation: Current video generation methods focus on visual consistency but neglect intentional film-editing patterns and cinematographic language, resulting in mere sequential shot changes without coherent narrative expression.
...
Conclusion: ShotDirector effectively achieves film-like controllable shot transitions by integrating precise camera control with editing-pattern-aware prompting, addressing the limitations of current video generation approaches.
...
TL;DR: Disentangled360 is a 3D-aware framework that combines direction disentangled volume rendering with single-image 360° view synthesis for medical imaging and natural scene reconstruction, outperforming existing methods without requiring scene-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current techniques either oversimplify anisotropic light behavior or lack generalizability across different contexts (medical imaging vs. natural scenes). There's a need for a unified framework that can handle both isotropic and anisotropic contributions while maintaining structural realism.

Method: The framework uses a Gaussian Splatting backbone with distinct differentiation between isotropic and anisotropic contributions. It implements a dual-branch conditioning framework: one for CT intensity driven scattering in volumetric data, and another for real-world RGB scenes using normalized camera embeddings. A hybrid pose agnostic anchoring method addresses scale ambiguity by adaptively sampling scene depth and material transitions.

Result: Superior SSIM and LPIPS performance on Mip-NeRF 360, RealEstate10K, and DeepDRR datasets. Runtime assessments confirm viability for interactive applications. The system enables rapid, photorealistic view synthesis with inherent directionality.

Conclusion: Disentangled360 facilitates mixed-reality medical supervision, robotic perception, and immersive content creation by eliminating the need for scene-specific fine-tuning or expensive photon simulations. It integrates preoperative radiography simulation and consumer-grade 360° rendering into a single inference pipeline.
...
TL;DR: Efficient-VLN reduces training overhead in Vision-Language Navigation by addressing quadratic token processing and exploration-efficiency trade-offs through progressive memory, recursive memory, and dynamic mixed policy.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs show promise in VLN but face prohibitive training costs from processing long observation sequences as tokens and the exploration-efficiency trade-off in DAgger data collection.

Method: Two memory mechanisms: progressive memory (dynamic token allocation to recent observations) and learnable recursive memory (key-value cache as memory state). Plus dynamic mixed policy to balance exploration vs efficiency.

Result: Achieves SOTA on R2R-CE (64.2% SR) and RxR-CE (67.0% SR) with only 282 H800 GPU hours - dramatic reduction in training overhead compared to existing methods.

Conclusion: Efficient-VLN successfully addresses VLN training efficiency challenges while maintaining high performance, making MLLM-based VLN more practical for real-world development.
...
Method: Proposes a prototype-driven framework with vision-language alignment: 1) CoOp-style learnable prompt tuning for text-based prototypes, 2) learnable image prototypes, 3) dual-modal prototype bank combining semantic and appearance cues, 4) multi-scale pyramid module to address ViT oversmoothing and improve spatial precision.
...
TL;DR: A prototype learning framework for weakly supervised semantic segmentation in histopathology that combines CONCH's morphology-aware representations with SegFormer's spatial cues and text guidance to generate high-quality pseudo masks without dense annotations.
...
Conclusion: The proposed framework successfully integrates complementary strengths of vision-language models and segmentation backbones for WSSS in histopathology, achieving better performance while maintaining computational efficiency through strategic use of frozen models and lightweight adapters.
...
Conclusion: Point2Pose effectively addresses 3D human pose estimation challenges through generative modeling and multi-modal data, showing promising results with the new MVPose3D dataset.
...
TL;DR: EchoingPixels is a token reduction framework for Audio-Visual LLMs that uses cross-modal interaction to dynamically reduce tokens from a joint audio-visual pool, achieving 2-3x speedup with only 5-20% of original tokens while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: AV-LLMs face prohibitive computational overhead from massive audio and video tokens. Existing unimodal token reduction methods cannot leverage audio-visual cross-modal synergies, and static per-modality budgets are suboptimal due to distinct and dynamic information densities of audio and video.

Method: Introduces Cross-Modal Semantic Sieve (CS2) that co-attends to joint multimodal streams and reduces tokens from a combined pool rather than using fixed per-modality budgets. Also includes Synchronization-Augmented RoPE (Sync-RoPE) to maintain temporal relationships for sparsely selected tokens.
...
Conclusion: EchoingPixels effectively addresses the token reduction bottleneck for AV-LLMs by enabling early audio-visual interaction and adaptive token allocation, making multimodal processing more efficient while preserving critical temporal modeling capabilities.
...
TL;DR: StainNet is a pathology foundation model specialized for special stain images (like immunohistochemistry) using self-distillation SSL on 1.4M patches from 20K+ WSIs, addressing limitations of existing H&E-focused PFMs.
...
Method: Proposes StainNet, a vision transformer-based foundation model using self-distillation self-supervised learning, trained on over 1.4 million patch images from 20,231 publicly available special staining WSIs in the HISTAI database.

Result: Demonstrates strong performance on slide-level liver malignancy classification and two public ROI-level datasets, shows effectiveness in few-ratio learning and retrieval tasks, and compares favorably with larger pathology foundation models.

Conclusion: StainNet addresses the gap in special stain pathology foundation models, providing a specialized solution that outperforms existing H&E-focused models for applications involving special staining techniques.
...
TL;DR: ISMVC proposes selective imputation for incomplete multi-view clustering, evaluating missing positions' informativeness and only imputing when sufficient support exists, combined with variational autoencoder for robust representation learning.
...
Conclusion: ISMVC provides a lightweight, data-driven, model-agnostic selective imputation approach that can be integrated as a plug-in module into existing incomplete multi-view clustering models, offering robust performance by balancing imputation benefits with noise prevention.
...
TL;DR: Proposes a conditional generative framework using Pix2Pix to generate realistic filament microscopy images from binary masks, with a filament-aware structural loss to address data shortage in filament segmentation.


<details>
  <summary>Details</summary>
Motivation: Filament segmentation is crucial for biological analysis but suffers from data shortage due to the extreme difficulty of manual pixel-level annotation for dense, elongated filamentous structures like microtubules and actin filaments.
...
TL;DR: HD-CLIP is a plug-and-play module for real-world image super-resolution that decomposes low-quality images into semantic and ordinal degradation embeddings, enabling better guidance for diffusion models without requiring known degradation severity.
...
Conclusion: HD-CLIP addresses limitations of existing methods by providing richer degradation guidance to diffusion models, enabling better generalization to complex real-world super-resolution tasks without requiring known degradation severity.
...
TL;DR: The paper introduces CoSPlan benchmark to evaluate VLMs on error-prone visual sequential planning tasks, proposes SGI method to improve VLM performance on corrective planning, and shows SGI generalizes to traditional planning benchmarks.


<details>
  <summary>Details</summary>
Motivation: VLMs have strong reasoning capabilities but are unexplored in visual sequential planning, especially in handling non-optimal/erroneous steps. Practical planning often involves mistakes that need detection and correction, which current VLMs struggle with.

Method: 1) Created CoSPlan benchmark with 4 domains (maze navigation, block rearrangement, image reconstruction, object reorganization) to evaluate Error Detection and Step Completion. 2) Proposed Scene Graph Incremental updates (SGI) - a training-free method that introduces intermediate reasoning steps between initial and goal states to help VLMs reason about sequences.

Result: State-of-the-art VLMs (Intern-VLM, Qwen2) struggle on CoSPlan even with Chain-of-Thought and Scene Graphs. SGI method yields average 5.2% performance gain on CoSPlan. SGI also generalizes to traditional planning tasks like Plan-Bench and VQA.

Conclusion: VLMs need improvement in corrective sequential planning. SGI provides effective intermediate reasoning for sequence planning without training. The approach enhances reliability in error-prone planning and generalizes to traditional planning benchmarks.
...
TL;DR: OmniZoo introduces a large-scale animal motion dataset and a generalized autoregressive framework for text-driven motion generation across arbitrary skeletal topologies, enabling cross-species motion style transfer.


<details>
  <summary>Details</summary>
Motivation: Current motion generation methods rely on fixed skeletal templates, preventing generalization to skeletons with different or perturbed topologies. There's a lack of large-scale heterogeneous animal motion data and unified generative frameworks capable of jointly modeling arbitrary skeletal topologies and textual conditions.

Method: Introduces OmniZoo dataset (140 species, 32,979 sequences with multimodal annotations) and a generalized autoregressive motion generation framework. Key innovation is a Topology-aware Skeleton Embedding Module that encodes geometric and structural properties of any skeleton into a shared token space for seamless fusion with textual semantics.
...
Conclusion: OmniZoo addresses the core limitations of current motion generation by providing both the necessary large-scale heterogeneous dataset and a unified framework that generalizes across skeletal topologies while maintaining text-conditioned motion generation capabilities.
...
Method: Proposes TranSamba, which augments a standard Vision Transformer backbone with Cross-Plane Mamba blocks that leverage state space models for efficient information exchange across neighboring slices with linear complexity.

Result: TranSamba achieves new state-of-the-art performance on three datasets, consistently outperforming existing methods across diverse modalities and pathologies while maintaining linear time complexity and constant memory usage.

Conclusion: TranSamba effectively addresses the limitations of 2D encoders for volumetric medical segmentation by combining Transformer self-attention within slices with efficient cross-slice modeling using Mamba blocks, establishing superior performance for weakly supervised volumetric segmentation.
...
<details>
  <summary>Details</summary>
Motivation: mmWave radars struggle with detecting static people in dense groups due to spatial resolution limits and reliance on movement. Existing breathing rate estimation methods assume known person count, creating a gap for practical applications.

Method: Uses novel multi-stage signal processing to extract ultra-low frequency (<1 Hz) signals from breathing and micro-scale body movements, differentiates these from background noise, maps spatial information to individuals, and counts people without prior knowledge.

Result: Achieves 87% average F1 score and 0.6 mean absolute error in familiar environments, 60% F1 score and 1.1 MAE in untested environments. Can count up to 7 people in 3 square meters with minimal spacing.

Conclusion: mmCounter successfully addresses the challenge of counting static people in dense indoor spaces using mmWave radar, offering practical solution for scenarios where movement-based detection fails and prior person count is unknown.
...
TL;DR: The paper introduces a Video Toolkit and Spatiotemporal Reasoning Framework (STAR) to enhance MLLMs' video question answering capabilities by addressing spatial-temporal reasoning challenges.
...
Method: Proposes a comprehensive Video Toolkit for MLLMs and a Spatiotemporal Reasoning Framework (STAR) that strategically schedules temporal and spatial tools to progressively localize key areas in videos.
...
TL;DR: Visual Funnel addresses "Contextual Blindness" in MLLMs by using a training-free, two-step approach with contextual anchoring and entropy-scaled hierarchical crops to preserve both fine details and global context.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with fine-grained visual perception despite good reasoning capabilities. Current crop-based methods cause "Contextual Blindness" - a structural disconnect between high-fidelity details from crops and the broader global context, limiting performance in precision-demanding tasks.

Method: Visual Funnel: 1) Contextual Anchoring identifies region of interest in a single forward pass; 2) Entropy-Scaled Portfolio constructs hierarchical crops preserving context from focal detail to broader surroundings by dynamically determining crop sizes based on attention entropy and refining crop centers.

Result: Significantly outperforms naive single-crop and unstructured multi-crop baselines. Shows that simply adding more unstructured crops provides limited or detrimental benefits, confirming hierarchical structure is key to resolving Contextual Blindness.

Conclusion: The limitation is not information quantity but structural diversity in input. Visual Funnel's hierarchical portfolio structure effectively resolves Contextual Blindness in MLLMs without requiring training.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for long video moment retrieval face limitations: supervised learning has poor scalability and generalization despite high resource consumption, while zero-shot methods suffer from candidate explosion in search phases and require expensive VLM verification in refine phases.

Method: P2S introduces two key innovations: (1) Adaptive Span Generator to prevent candidate explosion in the search phase, and (2) Query Decomposition to refine candidates without relying on high-cost VLM verification, making it a fully training-free framework.
...
Conclusion: P2S successfully addresses the dual challenges of inefficient search and costly refinement in zero-shot long video moment retrieval, achieving superior performance without task-specific training while being computationally efficient.
...
<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) requires dense, high-quality input images, but real-world applications often have sparse and motion-blurred data. These issues create a vicious cycle where sparse views can't resolve motion blur, and motion blur erases details needed for view alignment, leading to catastrophic reconstruction failures with fragmented views and low-frequency bias.

Method: CoherentGS uses a dual-prior strategy combining two pre-trained generative models: a specialized deblurring network for restoring sharp details and photometric guidance, and a diffusion model providing geometric priors to fill unobserved regions. Key techniques include a consistency-guided camera exploration module that adaptively guides the generative process, and depth regularization loss ensuring geometric plausibility.

Result: CoherentGS significantly outperforms existing methods in both quantitative and qualitative experiments on synthetic and real-world scenes, using as few as 3, 6, and 9 input views, setting a new state-of-the-art for this challenging task.

Conclusion: The proposed CoherentGS framework successfully addresses the compound degradation problem of sparse and blurry images in 3D reconstruction, breaking the vicious cycle through a dual-prior strategy and achieving high-fidelity results with very limited input views.
...
TL;DR: First joint scene flow learning framework (RaLiFlow) for 4D radar and LiDAR fusion, addressing radar noise/sparsity challenges with novel fusion module and loss functions.


<details>
  <summary>Details</summary>
Motivation: Radar complements LiDAR with lower cost, weather robustness, and velocity detection, but radar-LiDAR fusion for scene flow remains unexplored due to radar noise/sparsity and lack of suitable datasets.

Method: Constructs Radar-LiDAR scene flow dataset from public automotive data with preprocessing for radar denoising and label generation. Proposes RaLiFlow framework with Dynamic-aware Bidirectional Cross-modal Fusion (DBCF) module integrating radar dynamic cues via cross-attention, plus specialized loss functions for handling unreliable radar data and ensuring instance-level consistency.
...
Conclusion: First successful radar-LiDAR fusion for scene flow estimation, demonstrating radar's value as complementary sensor despite its challenges, with novel fusion approach and dataset enabling effective multimodal learning.
...
<details>
  <summary>Details</summary>
Motivation: Surgical endoscopy requires precise pixel-level correspondences for 3D reconstruction and camera tracking, but faces challenges like weak perspective cues, tissue reflections, and deformable anatomy that degrade conventional computer vision techniques. Deep learning models need domain-specific adaptation for surgical images.

Method: Proposes a self-supervised pipeline using novel-view synthesis to generate ground-truth correspondences, then uses contrastive learning with triplet mining. Augments DINOv2 backbone with additional Transformer layer optimized to produce embeddings for direct matching via cosine similarity thresholding.

Result: Outperforms state-of-the-art methods on SCARED datasets with improved matching precision and lower epipolar error compared to related work.

Conclusion: The framework enables more accurate high-level computer vision applications in surgical endoscopy by providing better feature correspondences through domain-adapted deep learning.
...
TL;DR: FROW benchmark evaluates LVLMs on fine-grained recognition using GPT-4o, with data construction and training optimizations showing significant accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: Existing LVLM benchmarks focus on reasoning tasks but neglect fine-grained recognition, which is crucial for practical applications. There's a need for comprehensive evaluation of detailed visual recognition capabilities.

Method: Introduces FROW benchmark with two optimization strategies: 1) Data construction using mosaic data (combined short-answer responses) and open-world data (real-world Q&A from GPT-4o), 2) Training process improvements by incorporating fine-grained data in pre-training.

Result: Mosaic data improves category recognition by 1%; open-world data boosts FROW benchmark accuracy by 10-20% and content accuracy by 6-12%; fine-grained pre-training improves category recognition by up to 10%.

Conclusion: The FROW benchmark addresses the gap in fine-grained recognition evaluation for LVLMs, demonstrating that optimized data construction and training strategies significantly improve performance on detailed visual recognition tasks.
...
TL;DR: Proposes an adaptive dual-weight gravitational-based point cloud denoising method that achieves high accuracy, strong edge preservation, and real-time performance through octree spatial partitioning, adaptive voxel-based noise removal, and gravitational scoring with density and distance weights.
...
TL;DR: MultiHateLoc: A weakly-supervised framework for temporal localization of multimodal hate speech in videos using only video-level labels.


<details>
  <summary>Details</summary>
Motivation: Existing research focuses on video-level hate speech classification but neglects the crucial task of temporal localization (identifying when hateful segments occur), especially under weak supervision where only video-level labels are available. Current static fusion or classification-based architectures fail to capture cross-modal and temporal dynamics in multimodal hate speech.

Method: MultiHateLoc incorporates: (1) modality-aware temporal encoders to model heterogeneous sequential patterns with text-based preprocessing for feature enhancement; (2) dynamic cross-modal fusion to adaptively emphasize informative modalities at each moment, plus cross-modal contrastive alignment for feature consistency; (3) modality-aware MIL objective to identify discriminative segments under video-level supervision.

Result: Despite using only coarse video-level labels, MultiHateLoc produces fine-grained, interpretable frame-level predictions and achieves state-of-the-art performance in localization tasks on HateMM and MultiHateClip datasets.

Conclusion: MultiHateLoc is the first framework for weakly-supervised multimodal hate localization that effectively addresses the challenges of temporal localization under weak supervision, producing accurate frame-level predictions while only requiring video-level labels.
...
TL;DR: This paper introduces WildRoad dataset and MaGRoad model for off-road road network extraction, addressing domain gap issues in wild terrains where current methods fail.
...
Method: Two complementary approaches: 1) Release WildRoad - a global off-road road network dataset created with a dedicated interactive annotation tool; 2) Introduce MaGRoad - a path-centric framework that aggregates multi-scale visual evidence along candidate paths to infer connectivity robustly.
...
Conclusion: The WildRoad dataset and path-centric MaGRoad paradigm provide a stronger foundation for mapping roads in wild terrains, addressing limitations of current urban-focused methods.
...
TL;DR: TransLocNet is a cross-modal attention framework for aerial-ground localization that fuses LiDAR geometry with aerial imagery, achieving sub-meter, sub-degree accuracy with 63% error reduction.


<details>
  <summary>Details</summary>
Motivation: Aerial-ground localization is challenging due to large viewpoint and modality gaps between ground-level LiDAR scans and overhead aerial imagery. Existing methods struggle with aligning these fundamentally different data representations.

Method: TransLocNet uses a cross-modal attention framework: LiDAR scans are projected into bird's-eye-view, aligned with aerial features via bidirectional attention, processed through a likelihood map decoder for spatial probability distributions, and enhanced with contrastive learning for shared embedding space.

Result: Experiments on CARLA (synthetic) and KITTI (real-world) show TransLocNet outperforms state-of-the-art baselines, reducing localization error by up to 63% and achieving sub-meter, sub-degree accuracy.

Conclusion: TransLocNet provides robust and generalizable aerial-ground localization that works effectively in both synthetic and real-world settings, demonstrating the effectiveness of cross-modal attention for bridging viewpoint and modality gaps.
...
TL;DR: The paper proposes NCTTA, a novel test-time adaptation method that addresses performance degradation under domain shifts by realigning feature embeddings with classifier weights using hybrid targets to mitigate unreliable pseudo-labels.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods lack theoretical insights into why performance degrades under domain shifts. The authors extend Neural Collapse theory to sample-wise level and discover Sample-wise Alignment Collapse (NC3+), revealing that performance degradation stems from sample-wise misalignment between feature embeddings and classifier weights during adaptation.

Method: NCTTA is a feature-classifier alignment method with hybrid targets that blends geometric proximity (based on NC3+ theory) with predictive confidence. This approach mitigates the impact of unreliable pseudo-labels under domain shifts by realigning feature embeddings with their corresponding classifier weights.

Result: Extensive experiments show NCTTA significantly enhances robustness to domain shifts, outperforming Tent by 14.52% on ImageNet-C benchmark.

Conclusion: The paper provides theoretical insights into TTA performance degradation through NC3+ phenomenon and proposes an effective solution (NCTTA) that addresses sample-wise misalignment issues, demonstrating substantial improvements in domain shift robustness.
...
TL;DR: Efficient real-time framework for mobile-based physiotherapy exercise analysis using pose estimation, angle features, and dynamic programming for sequence matching.


<details>
  <summary>Details</summary>
Motivation: To enable remote physiotherapy supervision and m-health applications by providing real-time identification, classification, and evaluation of human physiotherapy exercises on mobile devices.

Method: 1. Interpret movements as sequences of static poses using pose-estimation neural network from camera input. 2. Transform body keypoints into trigonometric angle-based features. 3. Use lightweight supervised models for frame-level pose predictions and accuracy scores. 4. Employ dynamic programming with modified Levenshtein distance algorithm for full exercise recognition and deviation detection.

Result: Experimental evaluation demonstrates effectiveness of the methodology, highlighting its applicability to remote physiotherapy supervision and m-health applications with real-time performance and client-side operation.

Conclusion: The proposed framework provides an efficient, scalable solution for real-time physiotherapy exercise analysis on mobile devices, enabling remote supervision and m-health applications through robust sequence matching and inaccuracy localization.
...
Conclusion: The proposed unified-transform framework with dual-domain progressive temporal alignment and QCMoE effectively addresses the trade-off between temporal alignment accuracy and error propagation, offering a promising solution for learned video compression with consistent quality control.
...
TL;DR: Hybrid Shape-from-Focus method combines traditional multi-scale DDL kernels for focus volume extraction with lightweight GRU-based iterative refinement and learned upsampling for high-quality depth estimation.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning SFF methods use heavy feature encoders and simple one-step aggregation that introduces artifacts and amplifies noise in depth maps. Need more robust and efficient approach.
...
Conclusion: Hybrid approach combining traditional focus volume computation with lightweight deep learning refinement provides more robust, accurate, and efficient Shape-from-Focus depth estimation compared to existing methods.
...
TL;DR: Pulse3DFace is the first dataset for estimating 3D blood pulsation maps, enabling research on dynamic facial blood flow modeling and remote pulse estimation methods.


<details>
  <summary>Details</summary>
Motivation: To address the need for better datasets to improve remote photoplethysmography imaging methods and enable research into illumination-robust blood pulsation analysis.

Method: Collected raw videos from 15 subjects at 30Hz from 23 viewpoints, with blood pulse reference measurements and facial 3D scans using monocular structure-from-motion. Processed 3D pulsation maps compatible with FLAME 3D head model texture space.

Result: Created comprehensive dataset including SNR, local pulse amplitude, phase information, and supplementary data. Evaluated illumination conditions, map consistency, and physiological feature capture in facial and neck regions.

Conclusion: Pulse3DFace enables development of dynamic facial blood pulsation models, synthetic video generation for method validation, and novel multi-view approaches for illumination-robust pulse analysis.
...
TL;DR: TaP enhances few-shot semantic segmentation by using LoRA to adaptively fine-tune encoders on support sets, improving performance on novel classes with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Prior FSS research focused mainly on decoder improvements, but the encoder's limited ability to extract meaningful features for unseen classes remains a key bottleneck. The paper addresses this limitation to enable better generalization to novel classes.

Method: TaP (Take a Peek) uses Low-Rank Adaptation (LoRA) to fine-tune the encoder on the support set, enabling fast adaptation to novel classes while preventing catastrophic forgetting. The method is model-agnostic and can be integrated into existing FSS pipelines.

Result: Extensive experiments on COCO 20^i, Pascal 5^i, and cross-domain datasets (DeepGlobe, ISIC, Chest X-ray) show consistent performance improvements across diverse models and shot settings. Significant gains in complex multi-class scenarios demonstrate practical effectiveness.

Conclusion: TaP addresses a critical limitation in FSS by enhancing encoder generalization to novel classes, paving the way for more robust, efficient, and generalizable segmentation systems. Strong performance can be achieved even with low-rank adaptations, ensuring computational efficiency.
...
TL;DR: Blink: A dynamic visual token resolution framework for MLLMs that emulates human "blink-like" perception by selectively allocating computation to salient regions across layers.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs have limited visual perception compared to humans who efficiently scan and focus on salient regions. The authors investigate whether MLLMs exhibit similar behavior and find they naturally attend to different regions across layers, suggesting selective computation allocation could enhance perception.

Method: Blink framework with two modules: 1) Saliency-guided scanning that estimates token importance using attention maps, and 2) Dynamic token resolution that extends important tokens via plug-and-play TokenSR module, then drops them when they lose focus in subsequent layers.

Result: Extensive experiments validate Blink's effectiveness in enhancing visual perception and multimodal understanding, demonstrating improved performance through adaptive computation allocation.

Conclusion: Blink successfully emulates human-inspired visual perception by dynamically balancing broad exploration and fine-grained focus, making MLLMs more efficient and effective in visual understanding tasks.
...
TL;DR: GETok introduces learnable spatial tokens to improve object grounding in MLLMs without changing the autoregressive architecture, achieving state-of-the-art performance on referring tasks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with accurate object grounding in 2D space because their autoregressive Transformer architecture tokenizes images, limiting spatial reasoning capabilities. The paper addresses how to improve sequential language tokens for better 2D spatial grounding.

Method: GETok integrates specialized learnable tokens into MLLMs: grid tokens partition the image into structured spatial anchors, and offset tokens enable precise iterative refinement of localization predictions. This embeds spatial relationships directly into tokens while maintaining the original autoregressive architecture.
...
Conclusion: GETok significantly advances MLLMs' native 2D space reasoning capabilities by embedding spatial relationships directly into tokens, enabling better object grounding without modifying the core autoregressive architecture.
...
TL;DR: Proposed Few-Shot Prototypical Network with ST-GCN and Multi-Scale Temporal Aggregation for skeleton-based sign language recognition, achieving 43.75% Top-1 accuracy on WLASL and strong zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Address data scarcity and long-tail distribution in isolated sign language recognition, where standard classifiers overfit frequent classes and fail on rare signs due to insufficient training data.

Method: Few-Shot Prototypical Network framework with skeleton-based encoder using ST-GCN and novel Multi-Scale Temporal Aggregation module. Episodic training learns semantic metric space where signs are classified by proximity to dynamic class prototypes rather than fixed decision boundaries.
...
<details>
  <summary>Details</summary>
Motivation: Existing video editing methods overlook audio-visual synchronization and lack fine-grained spatial and temporal controllability needed for precise instance-level edits, despite realistic audio-visual synchronization being crucial for engaging content creation.

Method: Proposes a granularity-aware mask refiner that iteratively refines coarse user-provided masks into precise instance-level regions, and a self-feedback audio agent to curate high-quality audio guidance for fine-grained temporal control. Also constructs a large-scale dataset with instance-centric correspondence and comprehensive annotations.
...
Conclusion: AVI-Edit provides an effective framework for audio-sync video instance editing with improved spatial and temporal control, addressing key limitations in current video editing approaches.
...
TL;DR: Symmetric U-Net architecture (SymUNet) achieves state-of-the-art all-in-one image restoration with simpler design than complex existing methods, using symmetric feature alignment and cross-scale propagation. Enhanced variant SE-SymUNet adds semantic injection from CLIP features.


<details>
  <summary>Details</summary>
Motivation: Existing all-in-one image restoration methods rely on complex architectures (Mixture-of-Experts, diffusion models) and elaborate degradation prompt strategies. The authors reveal that well-crafted feature extraction inherently encodes degradation information, and a symmetric U-Net can effectively unleash these cues without complex designs.

Method: SymUNet: symmetric U-Net architecture with aligned feature scales across encoder-decoder and streamlined cross-scale propagation. Simple additive fusion in skip connections preserves intrinsic degradation signals. SE-SymUNet variant integrates direct semantic injection from frozen CLIP features via cross-attention to amplify degradation priors.
...
Conclusion: A symmetric U-Net architecture is sufficient for state-of-the-art all-in-one image restoration, challenging the need for complex architectures. The approach preserves intrinsic degradation signals through symmetric design and cross-scale propagation, offering simpler yet more effective solutions.
...
<details>
  <summary>Details</summary>
Motivation: Most SOD methods assume ideal visual conditions and fail to handle weather-induced noise in real-world scenarios, which degrades segmentation accuracy.

Method: Proposes a SOD framework with specific encoder and replaceable decoder, introduces noise indicator vector for different weather types, and designs Noise Indicator Fusion Module (NIFM) that embeds weather-aware priors via adaptive feature modulation.

Result: Extensive experiments on WXSOD dataset show the framework (especially NIFM-enhanced encoder) improves segmentation accuracy under complex weather conditions compared to vanilla encoder, tested with varying training data scales and multiple encoder/decoder configurations.

Conclusion: The proposed weather-robust SOD framework effectively handles weather-induced noise while maintaining compatibility with mainstream SOD decoders, advancing real-world salient object detection.
...
TL;DR: Training-free text-only retrieval method (TRSLLaVA) for remote sensing images using VLM-generated captions, achieving competitive results with supervised models without any training.


<details>
  <summary>Details</summary>
Motivation: Address the semantic gap in remote sensing image retrieval and the lack of benchmarks for evaluating VLM-generated text in zero-shot retrieval contexts.

Method: Introduce RSRT dataset with structured captions, then propose TRSLLaVA - a training-free approach that reformulates cross-modal retrieval as text-to-text matching using VLM-generated captions in unified textual embedding space.

Result: Achieves 42.62% mean Recall on RSITMD (nearly double the CLIP baseline's 23.86%) and competitive performance with state-of-the-art supervised models on RSITMD and RSICD benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Video understanding in challenging conditions (occlusion, camouflage, rapid movement) depends more on motion dynamics than static appearance. Current methods often require user queries, but TCAM aims to autonomously discover and describe motion patterns.

Method: TCAM uses a motion-field attention mechanism to identify multiple motion activities and spatially ground natural language descriptions to corresponding trajectories. It combines global video-text alignment with fine-grained spatial correspondence through unified training, enabling query-free discovery of multiple motion expressions via multi-head cross-attention.

Result: On the MeViS benchmark, TCAM achieves 58.4% video-to-text retrieval, 64.9 JF for spatial grounding, and discovers 4.8 relevant expressions per video with 84.7% precision, demonstrating strong cross-task generalization.

Conclusion: Motion patterns aligned with contrastive vision-language representations provide powerful semantic signals for recognizing and describing actions, enabling autonomous video understanding without user queries.
...
<details>
  <summary>Details</summary>
Motivation: Rising incidence of vision-threatening eye diseases requires scalable, accurate screening solutions. Need to address "black-box" limitations of standard CNNs and bridge gap between algorithmic output and expert medical validation for clinical deployment.
...
Conclusion: Interpretable deep learning approach using auxiliary segmentation tasks can enhance clinical trust and viability of automated ocular disease diagnosis systems by making predictions more transparent and grounded in medical features.
...
TL;DR: Lang2Motion generates object motion trajectories from text by aligning motion manifolds with CLIP embedding spaces, using point tracking from real videos and dual supervision from text descriptions and visualizations.


<details>
  <summary>Details</summary>
Motivation: Previous work focused on human motion or video synthesis, but there's a need for explicit trajectory generation for arbitrary objects from language descriptions, using motion extracted from real-world videos.

Method: Transformer-based auto-encoder learns trajectory representations through dual supervision: textual motion descriptions and rendered trajectory visualizations, both mapped through CLIP's frozen encoders. Uses point tracking from real-world videos to extract motion.
...
Conclusion: Lang2Motion effectively generates object motion trajectories from language, demonstrates cross-domain transfer capabilities, and supports style transfer, semantic interpolation, and latent-space editing through CLIP-aligned representations.
...
TL;DR: DOCR-Inspector is a VLM-based system that assesses document parsing quality through fine-grained error detection and classification into 28 error types, outperforming commercial models on real-world benchmarks.
...
Method: Uses VLM-as-a-Judge to analyze document images and parsed outputs, identifies errors, classifies them into 28 predefined types, and employs Chain-of-Checklist reasoning for hierarchical quality assessment. Trained on DOCRcase-200K dataset.

Result: DOCR-Inspector-7B outperforms commercial models like Gemini 2.5 Pro and leading open-source models on DOCRcaseBench (882 real-world cases with manual annotations). Quality assessments effectively guide parsing refinement.

Conclusion: DOCR-Inspector provides reliable, comprehensive document parsing assessment in real-world scenarios, serving as both a practical evaluator and a driver for advancing document parsing systems at scale.
...
TL;DR: K-Track is a Kalman-enhanced tracking framework that accelerates deep learning point trackers 5-10X while maintaining >85% accuracy by combining sparse keyframe updates with lightweight Kalman filtering for intermediate frames.


<details>
  <summary>Details</summary>
Motivation: Deep learning point trackers achieve state-of-the-art accuracy but require per-frame GPU inference, making them impractical for resource-constrained edge devices where compute, power, and connectivity are limited.

Method: K-Track uses a hybrid approach: sparse deep learning updates on keyframes combined with lightweight Kalman filtering for intermediate frames, with principled Bayesian uncertainty propagation to maintain temporal coherence. The framework is tracker-agnostic and works with existing point trackers.
...
Conclusion: K-Track provides a practical path to deploy high-quality point tracking on resource-limited edge devices, bridging the gap between modern tracking algorithms and deployable vision systems for robotics, autonomous systems, AR, and video analysis.
...
<details>
  <summary>Details</summary>
Motivation: The increasing ease of fabricating realistic portrayals of individuals using generative models creates serious risks for security, communication, and public trust. Current systems need to not only detect manipulated content but also provide clear and reliable reasoning.
...
TL;DR: NaviHydra: A controllable navigation-guided end-to-end model for autonomous driving that interprets high-level commands and generates safe trajectories, outperforming baselines on NAVSIM benchmark.


<details>
  <summary>Details</summary>
Motivation: Traditional rule-based systems struggle in dynamic environments, while end-to-end methods have difficulty complying with explicit navigation commands. There's a need for models that can robustly interpret high-level navigation instructions and generate safe, compliant trajectories in complex autonomous driving scenarios.
...
Result: The method significantly outperforms baseline models and achieves state-of-the-art results in the NAVSIM benchmark. A comprehensive test evaluating response to various navigation commands demonstrates improved controllability and navigation safety.

Conclusion: NaviHydra effectively advances autonomous driving by providing a framework that combines the interpretability of rule-based systems with the adaptability of end-to-end methods, enabling robust compliance with navigation commands while maintaining safety in dynamic environments.
...
<details>
  <summary>Details</summary>
Motivation: Current AI models capture object geometry and appearance but neglect internal physical properties like volumetric density, which are crucial for predicting center of mass, stability, and interaction dynamics in robotics and simulation applications. The main limitation has been lack of large-scale real-world data.

Method: 1) Created XDen-1K dataset with 1,000 real-world objects across 148 categories, providing high-resolution 3D geometric models with part-level annotations and corresponding biplanar X-ray scans. 2) Developed optimization framework to recover volumetric density fields from sparse X-ray views. 3) Used X-ray images as conditioning signal for volumetric segmentation. 4) Applied dataset to downstream robotics tasks.

Result: The dataset enables accurate recovery of volumetric density fields. When used as conditioning for segmentation networks, it improves volumetric segmentation. In robotics applications, it enhances center-of-mass estimation accuracy and increases robotic manipulation success rates.

Conclusion: XDen-1K serves as a foundational resource and benchmark for physically grounded visual inference and embodied AI research, bridging the gap in physical property estimation data and enabling better prediction of object behavior in real-world applications.
...
TL;DR: Geo6DPose is a lightweight, training-free pipeline for zero-shot 6D object pose estimation that runs locally on commodity hardware, achieving comparable performance to larger cloud-based models while addressing practical deployment constraints in robotics.


<details>
  <summary>Details</summary>
Motivation: Current zero-shot 6D pose estimation methods rely on large-scale models and cloud inference, which cause high latency, energy consumption, and deployment risks (connectivity, cost, data governance). These conflict with real-world robotics needs for on-device inference with limited compute.

Method: Combines foundation model visual features with geometric filtering: computes similarity maps between template DINO descriptors and scene patches, establishes mutual correspondences by projecting scene patch centers to 3D and template descriptors to object coordinates, recovers poses via correspondence-driven RANSAC, and ranks poses using a weighted geometric alignment metric considering reprojection consistency and spatial support.

Result: Achieves 53.7 AR (average recall) at 1.08 FPS with sub-second inference on a single commodity GPU, matching performance of significantly larger zero-shot baselines while being fully local and training-free.

Conclusion: Geo6DPose advances practical, fully local 6D perception for robotic deployment by eliminating training, fine-tuning, and network access requirements while maintaining compatibility with evolving foundation backbones, offering a viable solution for real-world robotics with limited compute resources.
...
TL;DR: The paper proposes a new deep learning approach for single-molecule localization microscopy that reformulates the training objective as a set-matching problem using optimal transport, eliminating the need for non-differentiable NMS layers and enabling end-to-end training.


<details>
  <summary>Details</summary>
Motivation: Current SMLM methods require non-overlapping fluorophore emissions, leading to long acquisition times that hinder live-cell imaging. Existing deep learning approaches rely on non-differentiable NMS layers that may discard true positives.

Method: Reformulate SMLM training as a set-matching problem with an optimal-transport loss, eliminating NMS during inference. Propose an iterative neural network that integrates knowledge of the microscope's optical system.

Result: Experiments on synthetic benchmarks and real biological data show that both the new loss function and architecture surpass state-of-the-art performance at moderate and high emitter densities.

Conclusion: The proposed approach enables more efficient SMLM with denser emissions, potentially accelerating live-cell imaging by reducing acquisition times while maintaining accuracy.
...
TL;DR: SHARP enables photorealistic view synthesis from a single image using 3D Gaussian representation, achieving real-time rendering and state-of-the-art performance with significant quality improvements and speed gains.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of generating photorealistic novel views from a single image, which has applications in VR/AR, content creation, and 3D reconstruction. Current methods often require multiple images, complex optimization, or produce low-quality results with slow rendering.

Method: SHARP uses a neural network to regress parameters of a 3D Gaussian representation from a single input image in a single feedforward pass (<1 second). The 3D Gaussians form a metric representation with absolute scale, enabling real-time rendering of high-resolution novel views.

Result: SHARP achieves state-of-the-art performance across multiple datasets, reducing LPIPS by 25-34% and DISTS by 21-43% compared to prior models. It also reduces synthesis time by three orders of magnitude while maintaining robust zero-shot generalization.

Conclusion: SHARP demonstrates that efficient single-image view synthesis with photorealistic quality is achievable through 3D Gaussian representations, offering practical real-time applications with significant improvements in both quality and speed over existing methods.
...
TL;DR: The paper introduces uncertainty estimation methods for landmark-based chest X-ray segmentation, releasing a large dataset with per-node uncertainty estimates to enhance robustness and safe clinical deployment.


<details>
  <summary>Details</summary>
Motivation: Uncertainty estimation is crucial for safe clinical deployment of medical image segmentation systems, but prior work focused on pixel-level uncertainty while landmark-based segmentation with topological guarantees remains underexplored from an uncertainty perspective.

Method: Uses hybrid neural network architectures combining standard image convolutional encoders with graph-based generative decoders, deriving two complementary uncertainty measures: (1) latent uncertainty from learned distribution parameters, and (2) predictive uncertainty from multiple stochastic output predictions generated from latent samples.
...
Conclusion: Uncertainty estimation enhances robustness and safe deployment of landmark-based anatomical segmentation in chest X-ray. The authors release CheXmask-U, a large-scale dataset of 657,566 chest X-ray landmark segmentations with per-node uncertainty estimates, enabling researchers to account for spatial variations in segmentation quality.
...
TL;DR: SpaceDrive is a spatial-aware vision language model framework for autonomous driving that treats 3D spatial information as explicit positional encodings rather than textual digits, enabling better spatial reasoning and more accurate trajectory planning.


<details>
  <summary>Details</summary>
Motivation: Current vision language models (VLMs) struggle to understand fine-grained 3D spatial relationships, which is fundamental for autonomous driving systems that interact with the physical world. This limitation hinders their ability to perform accurate spatial reasoning for driving tasks.

Method: SpaceDrive uses a universal positional encoder for all 3D coordinates derived from multi-view depth estimation, historical ego-states, and text prompts. These 3D positional encodings are superimposed on corresponding 2D visual tokens and serve as task-agnostic coordinate representations, replacing digit-wise numerical tokens as both inputs and outputs for the VLM.
...
Conclusion: By treating spatial information as explicit positional encodings rather than textual digits, SpaceDrive enables joint reasoning over semantic and spatial representations, improving spatial understanding and planning accuracy for autonomous driving tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing video depth estimation methods either suffer from temporal inconsistencies (frame-by-frame monocular models) or are computationally demanding for real-time applications, limiting their practical applicability.
...
TL;DR: IRMoGen introduces a novel paradigm that interleaves motion generation with assessment and refinement through iterative text-motion dialogue, achieving better text-motion alignment than separate approaches.


<details>
  <summary>Details</summary>
Motivation: Current motion-aware LLMs treat understanding and generation separately, limiting mutual benefits from interactive feedback between tasks. The paper reveals that motion assessment and refinement can bridge this gap for bidirectional knowledge flow.

Method: Proposes IRMoGen paradigm with IRG-MotionLLM model that interleaves motion generation, assessment, and refinement. Uses three-stage training scheme and automated data engine to synthesize interleaved reasoning annotations from existing datasets.

Result: Assessment/refinement tasks significantly improve text-motion alignment; interleaving yields consistent performance gains; IRG-MotionLLM outperforms baselines and achieves advanced performance on text-to-motion benchmarks.

Conclusion: Tightly coupling motion generation with assessment and refinement through iterative dialogue enables bidirectional knowledge flow, leading to superior motion generation performance compared to separate approaches.
...
<details>
  <summary>Details</summary>
Motivation: Traditional automated polyp reporting suffers from inconsistencies and hallucinations due to scarce high-quality multimodal medical data, creating a need for more reliable, clinically-aligned diagnostic systems.

Method: Proposes LDP framework with: 1) MMEndo dataset curation (expert-annotated colonoscopy image-text pairs), 2) Fine-tuning Qwen2-VL-7B using LoRA for parameter efficiency, and 3) Clinical alignment via Direct Preference Optimization (DPO).

Result: LDP outperforms baselines on automated metrics and clinical evaluations (Physician Score 7.2/10), reduces training costs by 833x compared to full fine-tuning, and shows robustness on external validation with IU-XRay dataset.
...
TL;DR: A pipeline automates coronary geometry extraction from CCTA, streamlines CFD simulation, and uses diffusion models to predict coronary blood pressure directly from imaging features, bypassing slow CFD computations for faster CAD diagnosis.


<details>
  <summary>Details</summary>
Motivation: CFD-based simulation of coronary blood flow provides valuable hemodynamic markers for CAD diagnosis but is computationally expensive, time-consuming, and difficult to integrate into clinical workflows, limiting availability of labeled data for AI models and hindering broad adoption of non-invasive physiology-based CAD assessment.
...
Conclusion: This work provides a scalable and accessible framework for rapid, non-invasive blood pressure prediction to support CAD diagnosis, reducing manual burden and computational costs while enabling efficient AI model training for clinical applications.
...
TL;DR: The paper challenges conventional wisdom that global semantic performance of vision encoders drives generation quality in representation alignment (REPA), finding instead that spatial structure (patch token similarity) is more important. The authors propose iREPA with simple convolutional and spatial normalization layers that improve convergence.


<details>
  <summary>Details</summary>
Motivation: To investigate what aspect of target representations matters most for generative training via representation alignment: whether it's global semantic information (ImageNet accuracy) or spatial structure (patch token similarity). The conventional belief that stronger global performance leads to better generation is questioned.

Method: 1) Large-scale empirical analysis across 27 vision encoders and different model scales to study correlation between representation properties and generation performance. 2) Proposed iREPA with two modifications: replacing MLP projection layer with simple convolution layer, and introducing spatial normalization layer for external representations to accentuate spatial information transfer.

Result: Surprising finding: spatial structure, not global semantic performance, drives generation performance of target representations. iREPA (implemented in <4 lines of code) consistently improves convergence speed across diverse vision encoders, model sizes, and training variants (REPA, REPA-E, Meanflow, JiT, etc.).

Conclusion: The work challenges fundamental assumptions about representation alignment mechanisms, showing spatial structure is more critical than global semantics for generation. Simple architectural modifications that emphasize spatial information transfer can significantly improve training efficiency, motivating re-examination of how representation alignment works for generative models.
...
TL;DR: GLAT with IRM improves prostate cancer grading by iteratively selecting relevant patches and modeling tissue connectivity via graph Laplacian constraints, outperforming SOTA methods.
...
Method: Proposes Graph Laplacian Attention-Based Transformer (GLAT) with Iterative Refinement Module (IRM). IRM iteratively refines patch selection using pretrained ResNet50 for local features and foundation model for importance scoring. GLAT constructs patch graph with nodes as patches, applies graph Laplacian constraints for spatial consistency, and uses learnable filtering to enhance discriminative features. Convex aggregation dynamically adjusts patch importance for WSI-level representation.
...
Conclusion: The proposed GLAT with IRM effectively addresses prostate cancer grading challenges by combining iterative patch refinement with graph-based spatial modeling, resulting in superior performance and diagnostic relevance.
...
TL;DR: SEPL (Self-Ensemble Post Learning) addresses domain generalization with noisy labels by leveraging intermediate features and training multiple probing classifiers, then ensembling their predictions.


<details>
  <summary>Details</summary>
Motivation: Domain generalization faces challenges when encountering noisy labels, which exacerbate spurious feature enlargement in deep layers, degrading existing algorithm performance. The paper explores how to make existing methods work effectively in noisy conditions.

Method: SEPL consists of feature probing training and prediction ensemble inference. It leverages intermediate feature representations to train multiple probing classifiers using semi-supervised algorithms (to handle noisy labels), then integrates predictions using crowdsourcing inference approach since different classifiers focus on different image areas.

Result: Extensive experiments show the method enhances robustness of existing methods and exhibits significant potential for real-world applications with high flexibility.

Conclusion: SEPL effectively addresses domain generalization with noisy labels by diversifying features through self-ensemble post learning, leveraging intermediate representations and ensembling predictions from multiple probing classifiers.
...
TL;DR: PoseGAM is a geometry-aware multi-view framework for 6D object pose estimation that directly predicts poses without explicit feature matching, achieving state-of-the-art performance on unseen objects.
...
Method: Proposes PoseGAM, a multi-view framework built on foundation model architectures that integrates object geometry through explicit point-based geometry and learned features from geometry representation networks, using a large synthetic dataset for training.

Result: Achieves state-of-the-art performance with average AR improvement of 5.1% over prior methods, with up to 17.6% gains on individual datasets, demonstrating strong generalization to unseen objects.

Conclusion: PoseGAM effectively addresses unseen object pose estimation by eliminating explicit matching and integrating geometry awareness through multi-view learning, showing significant performance improvements across benchmarks.
...
TL;DR: SWiT-4D converts monocular videos into animated 3D assets with 4D meshes using a sliding-window transformer that integrates with existing image-to-3D generators, requiring minimal 4D supervision.


<details>
  <summary>Details</summary>
Motivation: Current 4D content generation faces challenges in converting monocular videos to high-quality animated 3D assets with explicit 4D meshes. There's a scarcity of large-scale 4D mesh datasets for training generalizable video-to-4D models from scratch, while image-to-3D generation has made significant progress with extensive datasets.

Method: SWiT-4D is a Sliding-Window Transformer for lossless, parameter-free temporal 4D mesh generation. It integrates with any Diffusion Transformer (DiT)-based image-to-3D generator, adding spatial-temporal modeling across video frames while preserving the original single-image forward process. It also includes an optimization-based trajectory module for recovering global translation in static-camera monocular videos.

Result: SWiT-4D demonstrates strong data efficiency, achieving high-fidelity geometry and stable temporal consistency with only a single short (<10s) video for fine-tuning. It outperforms existing baselines in temporal smoothness on both in-domain zoo-test sets and challenging out-of-domain benchmarks (C4D, Objaverse, and in-the-wild videos).

Conclusion: SWiT-4D provides a practical solution for 4D mesh reconstruction from videos of arbitrary length by leveraging existing image-to-3D priors while minimizing reliance on 4D supervision, making it deployable under extremely limited 4D supervision conditions.
...
Method: Created a four-level framework (Perception, Planning, Prediction, Cross-Video Reasoning) with 1,106 human-annotated questions grounded in 1,278 video clips from 25 existing datasets and in-house videos. Each item was designed and reviewed by 3DV experts with explanatory rationales.

Result: Evaluation of 25 MLLMs revealed a large human-AI gap - many models perform near chance, best reasoning model lags humans by nearly 60%. Spatially fine-tuned models fail to generalize. Systematic failures found in geometric reasoning, motion grounding, long-horizon prediction, and cross-video correspondence.

Conclusion: The benchmark establishes a solid testbed for advancing video-based spatial intelligence, revealing current limitations in MLLMs' spatial understanding and providing insights for future improvements in this crucial capability.
...
TL;DR: MiSI-Bench: A benchmark for evaluating Vision-Language Models' Microscopic Spatial Intelligence with 163K QA pairs and 587K images covering 9 tasks, showing current VLMs lag behind humans but fine-tuned 7B models show promise in spatial transformations.


<details>
  <summary>Details</summary>
Motivation: To assess Vision-Language Models' capability in Microscopic Spatial Intelligence - perceiving and reasoning about spatial relationships of invisible microscopic entities, which is fundamental to scientific discovery.
...
Conclusion: The benchmark reveals limitations of current VLMs in microscopic spatial reasoning and highlights the necessity of integrating explicit domain knowledge for progress toward scientific AGI.
...
Method: Created PubTables-v2 dataset supporting multiple table extraction tasks, evaluated domain-specialized vision-language models, and developed POTATR - an image-to-graph extension of Table Transformer for page-level table extraction.

Result: PubTables-v2 enables benchmarking of multi-page table structure recognition and comprehensive page-level table extraction, demonstrating current progress through VLM evaluations and POTATR model development.

Conclusion: PubTables-v2 addresses the data scarcity problem in table extraction research, enables new benchmarks for challenging tasks, and facilitates development of improved models like POTATR for comprehensive document understanding.
...
TL;DR: DuetSVG is a multimodal model that jointly generates image and SVG tokens to create better SVG graphics from text descriptions, using visual guidance to improve geometric coherence and visual appeal.
...
Conclusion: Joint generation of visual and SVG tokens with test-time visual guidance significantly improves SVG generation quality compared to text-only approaches.
...
TL;DR: FoundationMotion introduces an automated pipeline for creating large-scale motion datasets using object tracking and LLMs, enabling fine-tuning of models that outperform state-of-the-art baselines on motion understanding tasks.
...
Method: Automated pipeline that detects/tracks objects in videos to extract trajectories, then uses these trajectories and video frames with LLMs to generate fine-grained captions and diverse QA pairs about motion and spatial reasoning.

Result: Fine-tuned open-source models (NVILA-Video-15B and Qwen2.5-7B) achieve substantial improvements in motion understanding without compromising other tasks, outperforming strong baselines like Gemini-2.5 Flash and Qwen2.5-VL-72B.
...
TL;DR: BabyVLM-V2 is a developmentally grounded vision-language framework that improves upon V1 with longitudinal infant-centric pretraining data and DevCV Toolbox for cognitive evaluation, enabling compact models to achieve competitive performance on early childhood cognitive tasks.


<details>
  <summary>Details</summary>
Motivation: Early children's developmental trajectories provide a natural goal for sample-efficient pretraining of vision foundation models, aiming to create more developmentally plausible AI systems that mimic infant learning processes.

Method: 1) Longitudinal, multifaceted pretraining set covering infant-centric audiovisual corpus (video-utterance, image-utterance, multi-turn conversational data); 2) DevCV Toolbox adapting NIH Baby Toolbox vision measures into 10 multimodal tasks for spatial reasoning, memory, and vocabulary; 3) Compact model pretrained from scratch.

Result: Compact models pretrained from scratch achieve competitive performance on DevCV Toolbox, outperforming GPT-4o on some tasks, demonstrating sample efficiency and developmental plausibility.

Conclusion: BabyVLM-V2 provides a principled, unified framework to accelerate research in developmentally plausible pretraining of vision foundation models, showing that infant-inspired learning can lead to efficient and capable AI systems.
...
TL;DR: Any4D is a scalable multi-view transformer for dense 4D reconstruction that directly predicts per-pixel motion and geometry for multiple frames, supporting multiple modalities (RGB, RGB-D, IMU, Radar) with a modular scene representation.


<details>
  <summary>Details</summary>
Motivation: Prior work focuses on limited scenarios: either 2-view dense scene flow or sparse 3D point tracking. There's a need for a more comprehensive 4D reconstruction method that can handle multiple frames and diverse sensor modalities for real-world applications.

Method: Uses a scalable multi-view transformer architecture with modular 4D scene representation: egocentric factors (depthmaps, camera intrinsics) in local camera coordinates and allocentric factors (camera extrinsics, scene flow) in global world coordinates. Processes N frames directly and supports RGB, RGB-D, IMU, and Radar Doppler inputs.
...
Conclusion: Any4D provides a flexible, efficient framework for 4D reconstruction that outperforms existing methods and enables multiple downstream applications by supporting various sensor modalities and direct multi-frame processing.
...
<details>
  <summary>Details</summary>
Motivation: Current speech-driven talking head methods either achieve high visual fidelity but are too slow, or are fast but temporally unstable with video artifacts, limiting real-world applications.

Method: Map Gaussian Splatting to 3D Morphable Models to create person-specific avatars, and use transformer-based prediction of model parameters directly from audio to ensure temporal consistency.

Result: Enables real-time talking head video generation from monocular video and independent audio inputs, achieving competitive quantitative and qualitative performance.

Conclusion: The proposed method addresses the speed-stability trade-off in talking head generation by combining Gaussian Splatting with 3D Morphable Models and transformer-based audio parameter prediction for real-time, temporally consistent results.
...
TL;DR: OmniView is a unified diffusion model framework that generalizes across diverse 4D consistency tasks including novel view synthesis, video generation with camera control, and image-to-video, outperforming specialized models across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Prior camera control methods are fragmented, focusing on specific 4D tasks like novel view synthesis or text-to-video separately, requiring training on disjoint datasets. There's a need for a unified approach that can handle multiple 4D consistency tasks with a single model.

Method: OmniView separately represents space, time, and view conditions, enabling flexible combinations of these inputs. This unified framework allows the model to handle various tasks including novel view synthesis from static/dynamic/multiview inputs, temporal extrapolation, and video generation with camera control.
...
Conclusion: OmniView demonstrates strong generalizability across 4D tasks in a single model, showing the feasibility of a generalist 4D video model that can unify previously fragmented approaches to camera control and 4D consistency.
...
TL;DR: Mull-Tokens introduces modality-agnostic latent tokens that can hold intermediate reasoning information in either image or text formats, enabling multimodal reasoning without specialized tools or costly image generation.


<details>
  <summary>Details</summary>
Motivation: Real-world reasoning requires multimodal thinking about space, time, and affordances that text alone cannot convey. Existing multimodal models are brittle, don't scale, and rely on expensive specialist tools, image generation, or handcrafted data for modality switching.

Method: Develop Mull-Tokens - modality-agnostic latent tokens pre-trained to hold intermediate information in either image or text modalities. Train using supervision from interleaved text-image traces, then fine-tune without supervision using only final answers, inspired by latent reasoning frameworks.

Result: Across four challenging spatial reasoning benchmarks (puzzle solving, perspective-taking), Mull-Tokens outperforms text-only and interleaved image-text baselines with +3% average improvement and up to +16% on reasoning-heavy puzzle solving tasks.

Conclusion: Mull-Tokens offers a simple solution for abstract multimodal reasoning, enabling models to think free-form across modalities without expensive tools or generation, advancing conversations about grounding textual and visual reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Traditional vision-language models autoregressively generate tokens, which can be inefficient and focus on surface-level linguistic details rather than task-relevant semantics. There's a need for more efficient models that work in abstract representation spaces.

Method: VL-JEPA uses a Joint Embedding Predictive Architecture to predict continuous embeddings of target texts rather than generating tokens. It learns in an abstract representation space, and at inference time uses a lightweight text decoder only when needed to translate embeddings into text.

Result: VL-JEPA achieves stronger performance than standard token-space VLM training with 50% fewer trainable parameters. It supports selective decoding (2.85x fewer decoding operations), outperforms CLIP, SigLIP2, and Perception Encoder on video tasks, and matches classical VLMs on VQA datasets despite having only 1.6B parameters.

Conclusion: VL-JEPA demonstrates that predicting continuous embeddings in a joint vision-language space is more efficient and effective than traditional token generation, enabling better performance with fewer parameters and supporting multiple vision-language tasks without architecture modifications.
...
TL;DR: AlcheMinT introduces explicit timestamp conditioning for subject-driven video generation, enabling precise temporal control over subject appearance/disappearance in multi-subject videos.
...
Conclusion: The framework successfully introduces explicit timestamp conditioning for subject-driven video generation, enabling applications requiring precise temporal control over multiple subjects in videos.
...
TL;DR: MeViS: A large-scale multi-modal dataset for referring motion expression video segmentation, focusing on motion-based object tracking in videos using language descriptions of object motions.


<details>
  <summary>Details</summary>
Motivation: Existing referring video segmentation datasets focus on static attributes and salient objects, allowing target identification from single frames, which underemphasizes motion's role in video understanding. There's a need to explore motion expressions and motion reasoning for pixel-level video understanding.

Method: Created MeViS dataset with 33,072 human-annotated motion expressions (text and audio) covering 8,171 objects in 2,006 complex videos. Benchmarked 15 existing methods across 4 tasks: RVOS, AVOS, RMOT, and new RMEG task. Proposed LMPM++ approach for RVOS/AVOS/RMOT.

Result: Dataset reveals weaknesses in existing methods for motion expression-guided video understanding. LMPM++ achieves new state-of-the-art results on RVOS/AVOS/RMOT tasks. Dataset provides platform for developing motion expression-guided video understanding algorithms.

Conclusion: MeViS addresses the gap in motion-focused video understanding datasets and demonstrates the importance of motion expressions for complex video scene analysis. The dataset and proposed method advance the field of motion expression-guided video understanding.
...
TL;DR: Flex is a geometry-agnostic scene encoder that uses learnable scene tokens to jointly encode multi-camera, multi-timestep data for autonomous driving, achieving 2.2x faster inference with better performance than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems face computational bottlenecks when processing high-volume multi-camera data. Existing approaches rely on explicit 3D inductive biases (BEV, occupancy, tri-plane representations) which may not be optimal for scalability and efficiency.

Method: Flex employs a small set of learnable scene tokens to jointly encode information from all image tokens across different cameras and timesteps. It's geometry-agnostic, learning compact scene representations directly from data without explicit 3D priors, aggressively compressing visual input for downstream LLM-based policy models.
...
<details>
  <summary>Details</summary>
Motivation: Existing All-in-One Image Restoration (AiOIR) methods fail to explicitly model degradation types and struggle to adapt restoration behavior to complex or mixed degradations, limiting their effectiveness in handling diverse degradation scenarios.
...
Conclusion: ClusIR successfully bridges semantic degradation cues with frequency-domain modulation through cluster-guided synergy, enabling adaptive and high-fidelity image restoration for diverse degradation types within a unified framework.
...
<details>
  <summary>Details</summary>
Motivation: While RL has been successful in 2D image generation and other domains, applying it to 3D generation remains unexplored due to higher spatial complexity requiring globally consistent geometry and fine-grained local textures, making 3D generation sensitive to reward designs and RL algorithms.

Method: Systematic study across four dimensions: (1) Evaluating reward dimensions and model choices, (2) Studying GRPO variants with token-level optimization, (3) Introducing MME-3DR benchmark to measure implicit reasoning, (4) Proposing Hi-GRPO for hierarchical global-to-local optimization with dedicated reward ensembles.

Result: Developed AR3D-R1, the first RL-enhanced text-to-3D model that excels from coarse shape to texture refinement. Key findings show alignment with human preference is crucial, general multi-modal models provide robust 3D attribute signals, and token-level optimization is effective.

Conclusion: This pioneering study provides insights into RL-driven reasoning for 3D generation, demonstrating that systematic approaches to reward design, algorithm selection, and hierarchical optimization can successfully extend RL to the challenging domain of 3D autoregressive generation.
...
<details>
  <summary>Details</summary>
Motivation: Previous diffusion models generate images independently at inference, missing potential benefits from collaborative generation. The authors explore whether samples can be generated together to leverage inter-image relationships.

Method: Propose Group Diffusion that unlocks attention mechanism to be shared across images rather than just within patches of a single image. This enables joint denoising of multiple images at inference time, learning both intra and inter-image correspondence.

Result: Observe clear scaling effect - larger group sizes yield stronger cross-sample attention and better generation quality. Achieve up to 32.2% FID improvement on ImageNet-256x256. Introduce qualitative measure showing correlation between cross-sample attention strength and FID.

Conclusion: Cross-sample inference is an effective, previously unexplored mechanism for generative modeling. GroupDiff reveals the potential of collaborative generation through shared attention across images during diffusion model inference.
...
TL;DR: Omni-Attribute is an open-vocabulary image attribute encoder that learns disentangled, attribute-specific representations for visual concept personalization, addressing information leakage in existing methods.
...
Method: Joint data-model design: (i) curating semantically linked image pairs annotated with positive/negative attributes to teach the encoder what to preserve or suppress; (ii) dual-objective training paradigm balancing generative fidelity with contrastive disentanglement.
...
Conclusion: Omni-Attribute successfully addresses the limitation of entangled representations in existing methods by learning high-fidelity, attribute-specific representations for better visual concept personalization.
...
TL;DR: StereoWalker improves robot navigation foundation models by incorporating stereo vision and explicit mid-level vision modules (depth estimation, tracking), achieving better performance with far less training data compared to monocular-only approaches.


<details>
  <summary>Details</summary>
Motivation: Current navigation foundation models rely solely on monocular vision and assume vision capabilities will emerge implicitly, but this requires massive pixel-to-action supervision and struggles with depth-scale ambiguity and dynamic scenes. Mid-level vision priors are ignored despite their potential benefits.

Method: StereoWalker augments NFMs with stereo inputs to resolve depth-scale ambiguity and incorporates explicit mid-level vision modules including depth estimation and dense pixel tracking. The authors also curate a large stereo navigation dataset with automatic action annotation from Internet stereo videos.

Result: StereoWalker achieves comparable performance to state-of-the-art using only 1.5% of training data, surpasses state-of-the-art with full data, and shows that stereo vision yields higher navigation performance than monocular input.

Conclusion: Relying solely on monocular vision and ignoring mid-level vision priors is inefficient for robot navigation. Incorporating stereo inputs and explicit mid-level vision modules significantly improves performance and data efficiency in navigation foundation models.
...
TL;DR: SceneMaker is a decoupled 3D scene generation framework that separates de-occlusion from object generation and uses unified pose estimation to handle severe occlusion and open-set scenarios.
...
Method: 1) Decouple de-occlusion model from 3D object generation, enhanced with image datasets and collected de-occlusion datasets. 2) Propose unified pose estimation model integrating global and local mechanisms for attention. 3) Construct open-set 3D scene dataset for generalization.

Result: Comprehensive experiments demonstrate superiority on both indoor and open-set scenes. Code and datasets are publicly released.

Conclusion: The decoupled framework effectively addresses occlusion and open-set challenges in 3D scene generation through separate de-occlusion and unified pose estimation components.
...
Method: WorldLens spans five evaluation aspects: Generation, Reconstruction, Action-Following, Downstream Task, and Human Preference. The authors also create WorldLens-26K (human-annotated video dataset with scores and rationales) and WorldLens-Agent (evaluation model distilled from human annotations for scalable, explainable scoring).
...
TL;DR: StereoSpace: A diffusion-based framework for monocular-to-stereo synthesis that uses viewpoint conditioning instead of explicit depth or warping, achieving state-of-the-art results with strong geometric consistency and perceptual comfort.


<details>
  <summary>Details</summary>
Motivation: Current stereo generation methods often rely on explicit depth estimation and warping operations, which can be limited in handling complex scenes with disocclusions, layered structures, and non-Lambertian materials. The authors aim to develop a more robust approach that doesn't require explicit geometric proxies.
...
Result: StereoSpace outperforms other methods across warp & inpaint, latent-warping, and warped-conditioning categories. It achieves sharp parallax and strong robustness on challenging scenes including layered and non-Lambertian materials. The method establishes viewpoint-conditioned diffusion as a scalable, depth-free solution for stereo generation.

Conclusion: Viewpoint-conditioned diffusion provides an effective, scalable approach to stereo generation that avoids the limitations of explicit depth estimation and warping, offering strong geometric consistency and perceptual comfort without requiring geometric proxies at test time.
