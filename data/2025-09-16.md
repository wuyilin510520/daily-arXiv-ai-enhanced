...
Motivation: To enable privacy control in shared-space mixed reality meetings by allowing users to selectively remove personal or sensitive items from their environment so they're not visible to other participants.

Method: Uses semantic segmentation and precise object selection followed by real-time inpainting from secondary observer's viewpoint. Implements YOLOv11 for object detection and modified Decoupled Spatial-Temporal Transformer (DSTT) model for video inpainting. Uses mobile ZED 2i depth camera without requiring fixed viewpoint or prior 3D scanning.

Result: The system achieves frame rates exceeding 20 fps at 720p resolution, demonstrating real-time performance for practical privacy-preserving MR applications.

Conclusion: The research demonstrates the feasibility of real-time diminished reality for privacy control in mixed reality environments, providing a portable and robust solution that doesn't require fixed viewpoints or pre-scanned environments.
...
<details>
  <summary>Details</summary>
Motivation: Progress in surgical vision-language pre-training is limited by small, low-quality datasets lacking procedural diversity and hierarchical structure. Existing datasets are insufficient for developing robust surgical foundation models.

Method: Created SurgLaVi dataset using automated pipeline that generates fine-grained transcriptions, segments videos into procedural units, and applies dual-modality filtering. Developed SurgCLIP - a CLIP-style video-text contrastive framework with dual encoders.

Result: SurgCLIP achieves consistent improvements across phase, step, action, and tool recognition, surpassing prior state-of-the-art methods by large margins. Released SurgLaVi-β with 113k public clip-caption pairs (4x larger than existing datasets).

Conclusion: Large-scale, semantically rich, and hierarchically structured datasets directly translate into stronger surgical representations. SurgLaVi establishes a key resource for developing surgical foundation models with superior generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning models for 3D brain MRI are task-specific and lack generalization. While SSL has shown success in 2D medical imaging, few accessible foundation models exist for 3D brain MRI that can handle diverse neurological conditions.

Method: Developed a general SimCLR-based SSL foundation model pre-trained on 18,759 patients (44,958 scans) from 11 public datasets spanning diverse neurological diseases. Compared against Masked Autoencoders and supervised baselines on four downstream tasks.
...
Method: Formulates RGB-to-HSI as physics-grounded inverse problem with nuclear norm regularization, estimates CSS/illumination for forward operator, introduces data-adaptive low-rank subspace SVT to avoid full SVD costs, and develops USCTNet deep unfolding solver with parameter estimation and learnable proximal updates.
...
Conclusion: The proposed USCTNet framework effectively addresses physical consistency and computational efficiency in hyperspectral image reconstruction from single RGB images.
...
<details>
  <summary>Details</summary>
Motivation: To investigate the effectiveness of LLMs for medical imaging tasks since their utility in image-based healthcare applications remains unexplored despite strong performance in text-based tasks.

Method: Evaluated general-purpose vision-language LLM (LLaMA 3.2 Instruct) on BraTS 2020 dataset for glioma classification and segmentation, comparing performance before/after fine-tuning against custom 3D CNNs.

Result: CNNs outperformed LLMs in both tasks: classification (80% vs 76% accuracy), segmentation (accurate localization vs center-clustered predictions). Fine-tuning provided minimal improvement for LLMs.

Conclusion: Current LLMs are not well-suited for image-based medical tasks; more rigorous fine-tuning or alternative training strategies are needed for better performance and robustness.
...
<details>
  <summary>Details</summary>
Motivation: Traditional part segmentation methods rely on appearance-based semantic cues, which don't align well with object articulation. There's a need for kinematic parts that are consistent across views and time for animation and motion tasks.

Method: Uses dual-branch diffusion model with spatial color encoding to map part masks to RGB-like images, sharing VAE between branches. Includes BiDiFuse module for cross-branch consistency and contrastive part consistency loss for spatial-temporal alignment.

Result: SP4D generalizes well to diverse scenarios including real-world videos, novel objects, and rare poses. Generated 2D part maps can be lifted to 3D for skeletal structures and skinning weights with few manual adjustments.

Conclusion: The framework produces kinematic-aware outputs suitable for downstream animation tasks, demonstrating strong generalization across various scenarios while maintaining consistency across views and time.
...
<details>
  <summary>Details</summary>
Motivation: Current ISLR approaches lose crucial hand shape and orientation details when combining RGB and pose modalities due to imprecise representations like bounding boxes.

Method: Uses pose information for rough hand/body localization, then applies promptable zero-shot video segmentation to precisely segment these parts, focusing RGB processing on relevant body areas.

Result: Outperforms state-of-the-art methods on ChaLearn249 IsoGD dataset, with ablation studies showing strong benefits from focusing on signer's body and hands.

Conclusion: SegSLR effectively combines RGB and pose information through segmentation, preserving critical shape details and improving ISLR performance.
...
<details>
  <summary>Details</summary>
Motivation: Current surgical segmentation models require labeled data and domain-specific adaptation, limiting their flexibility in dynamic operating room environments where manual input is impractical.

Method: Speech-guided collaborative perception framework combining LLM reasoning with open-set VFM perception, using clinician speech feedback to guide segmentation and instrument-based labeling.

Result: Evaluated on Cataract1k and in-house skull-base datasets, demonstrating real-time segmentation and tracking capabilities in surgical scenes through live mock experiments.

Conclusion: The human-AI collaboration paradigm enables adaptable, hands-free surgical tools for dynamic operating rooms, overcoming limitations of traditional supervised models.
...
<details>
  <summary>Details</summary>
Motivation: Current computer vision systems fail with real-world camera effects due to lack of training data with such effects. Existing data generation methods are costly, have sim-to-real gaps, or fail to accurately model camera effects.

Method: A two-stage pipeline: 1) Reconstructs dynamic scenes from multi-view videos using 4D Gaussian Splatting, 2) Applies physically-based ray tracing to generate videos with controllable, physically accurate camera effects.
...
Conclusion: 4D-GRT provides an effective solution for generating training data with realistic camera effects, bridging the gap between ideal pinhole camera assumptions and real-world camera behavior in computer vision systems.
...
Method: Two-agent approach: Editor agent processes video clips with natural language instructions using standard editing tools, Critic agent provides feedback or renders final output. Uses learning-based communication between specialized agents.

Result: System significantly outperforms existing approaches in coverage, time constraint satisfaction, and human preference based on user study evaluations.

Conclusion: The multi-agent framework with Editor-Critic architecture effectively automates video editing through natural language interaction, demonstrating superior performance over previous methods.
...
Result: Cross-validation accuracy: 0.91-0.96; External testing: average 0.93 (range 0.87-0.98). F1, precision, recall were stable (0.87-0.96), ROC-AUC varied more (0.50-0.82). MI with ETr pipeline consistently performed best.
...
Method: Integrated three scene sources (real-world scans, procedural generation, designer-created scenes) with comprehensive processing including real-to-sim replication, interactive object addition, and collision resolution via physical simulations.
...
Conclusion: InternScenes enables new challenges in complex scene generation and navigation, paves way for scaling model training, and will be open-sourced to benefit the research community.
...
<details>
  <summary>Details</summary>
Motivation: To optimize mathematical handwriting representation by finding the best balance between basis choice and polynomial degree for accurate modeling with low computational cost.

Method: Examined condition numbers for polynomial evaluation in Legendre, Legendre-Sobolev, Chebyshev, and Chebyshev-Sobolev bases, and bounded how different inner products provide norms for symbol variations.

Result: The study provides insights into the trade-offs between different polynomial bases and their computational characteristics for handwriting representation.

Conclusion: Different polynomial bases offer varying computational efficiencies and accuracy trade-offs, with condition numbers and inner product norms being key factors in selecting optimal basis-degree combinations for mathematical handwriting modeling.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in glioma evolution prediction caused by sparse, irregular longitudinal MRI data in clinical practice, where incomplete follow-up sequences create data imbalances and make reliable modeling difficult.
...
Result: Achieved promising results when trained on public dataset and evaluated on private internal dataset. Framework produces flexible time-dependent probability maps from just two early follow-up scans, enabling clinicians to interrogate tumor progression risks at any future milestone.

Conclusion: The proposed method effectively addresses clinical limitations of data scarcity and irregular follow-ups, providing a flexible tool for glioma progression prediction with uncertainty quantification and clinical relevance through radiation dose integration.
...
<details>
  <summary>Details</summary>
Motivation: Existing LiDAR segmentation methods often require large datasets and suffer from high computational complexity, limiting their generalization in data-scarce scenarios. The paper aims to improve performance using only LiDAR data without external sensors.

Method: Uses point-plane projections to learn features from 2D representations of 3D point clouds, and introduces a geometry-aware data augmentation technique that aligns with LiDAR sensor properties to address class imbalance.

Result: Significant improvements in limited-data scenarios and competitive results on SemanticKITTI and PandaSet datasets.

Conclusion: The proposed approach effectively enhances point-based methods by leveraging 2D representations through point-plane projections, providing a computationally efficient solution that works well with limited training data.
...
Method: Generates semantic features directly from raw point clouds through multi-view, multi-granularity rendering, mask-level vision-language feature extraction, sample-balanced fusion, and distillation into a 3D backbone model.
...
Method: Uses dual-camera setup for frontal and workspace views, integrates Face Module (ArcFace for identity verification, head pose, gaze tracking, mouth movement) and Hand Module (YOLOv11 for prohibited item detection), with features aggregated into LSTM network for temporal pattern analysis and real-time cheating probability scoring.

Result: Achieves 90.7% accuracy in suspicious activity classification, mAP@.5 of 0.57 for prohibited item detection, and processes video at 2.4 FPS without GPU on custom dataset simulating diverse exam conditions.

Conclusion: AutoOEP is an effective, resource-efficient automated proctoring solution that significantly reduces human intervention needs and enhances online assessment integrity.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations in compressive spectral imaging where high compression ratios create ill-posed reconstruction problems, and traditional methods lack performance while deep learning lacks interpretability.

Method: Dual-camera CASSI with total variation subgradient theory, dynamic regularization using RGB/panchromatic reference images, and adaptive reference generation mechanism.

Result: Effectively preserves spatial-spectral structural consistency with robust performance across diverse reconstruction scenarios.

Conclusion: Provides mathematically well-founded, interpretable framework for computational spectral imaging with improved reconstruction quality.
...
<details>
  <summary>Details</summary>
Motivation: Large-scale Earth Observation foundation models are computationally expensive, limiting accessibility and reuse for downstream tasks. The authors investigate compact architectures as a practical pathway toward smaller general-purpose EO models.

Method: Proposed Metadata-aware Mixture-of-Experts Masked Autoencoder (MoE-MAE) with sparse expert routing and geo-temporal conditioning (latitude/longitude, seasonal/daily cyclic encodings). Pretrained on BigEarthNet-Landsat dataset and evaluated using linear probes with frozen encoder.

Result: Despite only 2.5M parameters, the model competes with much larger architectures. Metadata-aware pretraining improves transfer and label efficiency. On EuroSAT-Landsat dataset (without explicit metadata), it still shows competitive performance compared to models with hundreds of millions of parameters.

Conclusion: Compact, metadata-aware MoE-MAEs are an efficient and scalable step toward future Earth Observation foundation models, demonstrating that small models can achieve competitive performance through effective architecture design and metadata integration.
...
Method: Optimized sinogram-based method to simulate motion artifacts, creating paired datasets. Proposed Edge-enhanced Self-attention WGAN-GP with edge-enhancing skip connections, self-attention mechanisms, and VGG-based perceptual loss.

Result: Achieved mean SNR of 26.78, SSIM of 0.81, VIF of 0.76 for source dataset; SNR of 29.31, SSIM of 0.87, VIF of 0.81 for target dataset.

Conclusion: The methods represent an important initial step toward implementing deep learning-based motion correction in HR-pQCT, though they address a simplified representation of real-world motion artifacts.
...
<details>
  <summary>Details</summary>
Motivation: To identify key factors that influence the performance of state-of-the-art gaze-based authentication systems, particularly focusing on eye tracking signal quality, calibration methods, and filtering techniques.

Method: Conducted experiments on a large-scale dataset of 8,849 subjects using Meta Quest Pro equivalent hardware with 72Hz video oculography-driven gaze estimation. Employed state-of-the-art neural network architecture to study calibration target depth, fusion of calibrated/non-calibrated gaze, signal quality, and moving average filtering effects.
...
Conclusion: Careful consideration of calibration depth consistency, gaze data fusion strategies, and signal quality improvement are crucial for optimizing gaze-based authentication performance, while simple filtering may have negative impacts.
...
Method: Created TrueSkin dataset with 7,299 images systematically categorized into 6 classes under diverse conditions. Used this to benchmark existing recognition and generation models, then trained recognition models and fine-tuned generation models on the dataset.

Result: LMMs misclassify intermediate skin tones as lighter ones; generative models struggle with accurate skin tone production due to prompt biases. Training on TrueSkin improved classification accuracy by over 20% compared to LMMs and conventional approaches, and significantly improved skin tone fidelity in generation models.

Conclusion: TrueSkin serves as both a benchmark for evaluation and valuable training resource to enhance fairness and accuracy in skin tone recognition and generation tasks, highlighting the need for comprehensive datasets in this domain.
...
Result: Achieved higher detection rate while requiring significantly less computational time compared to traditional transfer learning methods.

Conclusion: The proposed RL-based framework provides an automated and efficient solution for model selection in transfer learning, particularly beneficial for researchers with limited data and expertise in deep learning.
...
<details>
  <summary>Details</summary>
Motivation: To develop accurate automatic fungal species recognition tools that can handle highly imbalanced class distributions and reliably identify rare, under-documented species with very few training samples, supporting mycologists and citizen scientists in biodiversity monitoring.

Method: Proposed a robust deep learning method based on prototypical networks that enhances prototype representations for few-shot fungal classification, specifically designed to work with limited training data for rare species.

Result: The prototypical network approach exceeded the competition baseline by more than 30 percentage points in Recall@5 on both public (PB) and private (PR) leaderboards, demonstrating strong performance for both common and rare fungal species.

Conclusion: The method shows strong potential for accurately identifying fungal species, including rare and under-documented taxa, supporting the main objectives of FungiCLEF 2025 and enabling more effective large-scale biodiversity monitoring.
...
Method: Performs global clustering across all bags to establish cluster centers, then local clustering within each bag. Computes attention scores within clusters and applies sparse regularization to cluster weights for selective retention of relevant clusters.

Result: Achieves state-of-the-art performance on CAMELYON16 and TCGA-NSCLC benchmarks. Theoretical analysis shows csMIL requires O(s log K) bags to recover s relevant clusters.

Conclusion: csMIL enhances robustness to noisy instances, improves interpretability by identifying critical regions, reduces computational complexity, and aligns with compressed sensing principles.
...
<details>
  <summary>Details</summary>
Motivation: ZS-VAD is crucial for practical applications like data privacy and new surveillance deployments. Skeleton-based approaches have generalizable advantages but existing methods only learn low-level representations and rely on domain-limited normality boundaries.

Method: Two modules: 1) Language-guided semantic typicality modeling that projects skeleton snippets into action semantic space using LLM knowledge, 2) Test-time context uniqueness analysis to derive scene-adaptive boundaries by analyzing spatio-temporal differences.

Result: Achieves state-of-the-art results on four large-scale VAD datasets (ShanghaiTech, UBnormal, NWPU, UCF-Crime) featuring over 100 unseen surveillance scenes without using target domain training samples.

Conclusion: The framework successfully unlocks skeleton data potential through action typicality and uniqueness learning, demonstrating strong generalization to new scenes with different normal/abnormal behavior patterns.
...
Method: Developed a modular GUI platform built on Segment Anything Model 2 (SAM2) for zero-shot segmentation and automated analysis of spatial-temporal microscopy videos.

Result: The platform quantifies key metrics including cyst formation rate, growth velocity, and morphological changes while generating comprehensive reports.

Conclusion: Organoid Tracker provides an extensible, open-source framework that accelerates kidney development research, PKD modeling, and therapeutic discovery without requiring programming expertise.
...
<details>
  <summary>Details</summary>
Motivation: To develop an effective vision language model system for the Driving with Language track of CVPR 2024 Autonomous Grand Challenge using the DriveLM-nuScenes dataset.

Method: Enhanced LLaVA models through fine-tuning with LoRA and DoRA methods, integrated depth information from open-source depth estimation models, and used Chain-of-Thought reasoning for multiple-choice and yes/no questions during inference.

Result: Achieved a top score of 0.7799 on the validation set leaderboard, ranking 1st place.

Conclusion: The comprehensive methodology combining fine-tuned LLaVA models, depth integration, and Chain-of-Thought reasoning proved highly effective for autonomous driving language tasks, resulting in state-of-the-art performance.
...
<details>
  <summary>Details</summary>
Motivation: To develop a reliable traversability costmap prediction system for planetary rovers that can effectively fuse multi-modal sensor data and operate in self-supervised manner without manual labeling.

Method: Fuses camera and LiDAR data to produce bird's-eye-view terrain costmaps using DINOv3 image encoder and FiLM-based sensor fusion, trained self-supervised with IMU-derived labels and optimized with Huber+smoothness loss.

Result: Highly robust performance with minor changes in MAE/MSE (e.g., MAE increases from ~0.0775 to 0.0915 when LiDAR is sparsified), indicating geometry dominates learned cost and model is resilient to input variations.

Conclusion: The framework provides strong multi-modal BEV costmap prediction with contributions including high-fidelity simulation, self-supervised labeling pipeline, and robust model architecture, though limited by data diversity and requiring future work on domain generalization.
...
Conclusion: The proposed CAA mechanism enables control signals to guide visual attention learning, leading to more robust and generalizable policies for precise parking tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional perspective video generation models struggle with panoramic content due to fundamental projection differences. Existing solutions are complex and inefficient, while LoRA has shown success in style transfer tasks, suggesting it could effectively model the transformation between perspective and panoramic projections.

Method: Treats panoramic video generation as an adaptation problem using Low-Rank Adaptation (LoRA) to fine-tune a pretrained video diffusion model. Theoretical analysis shows LoRA can model the projection transformation when its rank exceeds the task's degrees of freedom.

Result: The method efficiently fine-tunes with only approximately 1,000 videos while achieving high-quality panoramic generation. Experimental results show it maintains proper projection geometry and surpasses previous state-of-the-art in visual quality, left-right consistency, and motion diversity.

Conclusion: LoRA-based adaptation provides an efficient and effective solution for panoramic video generation, successfully bridging the gap between perspective and panoramic projections while requiring minimal training data and outperforming existing approaches.
...
Result: Experiments on synthetic and real datasets substantiate the usefulness of the proposed method, demonstrating improved hyperspectral unmixing performance.

Conclusion: SMILE provides progressive theoretical support and a new framework that successfully integrates super-resolution guidance with hyperspectral unmixing, addressing both task affinity validation and convergence guarantees.
...
Method: Develops a mathematical model incorporating copula theory, constructs a copula-guided framework with two key modules (copula function estimation and temporal dependency guidance), and provides theoretical support for validity.
...
Conclusion: The copula-guided approach successfully addresses temporal dependency modeling in multitemporal hyperspectral unmixing, providing a novel framework with theoretical foundation and practical effectiveness.
...
Method: Dataset collected using low-cost UAVs over hurricane regions, reconstructed via Structure-from-Motion and Multi-View Stereo techniques, with semantic annotations created through manual 2D labeling projected into 3D space.

Result: Created a large-scale outdoor 3D dataset capturing fine-grained structural damage in real disaster contexts, enabling evaluation of state-of-the-art 3D segmentation models.

Conclusion: 3DAeroRelief serves as a valuable resource for advancing robust 3D vision systems in post-disaster scenarios, highlighting both challenges and opportunities for 3D scene understanding in emergency response.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal signals in real-world scenarios are prone to missing due to sensor failures and adverse weather conditions, which severely degrades model performance. Existing generative approaches inadequately handle the heterogeneity of multimodal remote sensing data and suffer from heavy dependence on dominant modalities.
...
Conclusion: The proposed GEMMNet effectively addresses the limitations of existing generative approaches in handling multimodal remote sensing data heterogeneity and modality dominance bias, demonstrating superior performance in semantic segmentation under missing modality conditions.
...
<details>
  <summary>Details</summary>
Motivation: Current fluid reconstruction methods rely on controlled lab environments, leaving real-world videos largely unexplored despite significant advances in 3D vision and fluid dynamics rendering.
...
Result: Outperforms previous reconstruction and generation methods with +2.22 average PSNR improvement on wild videos, and enables diverse realistic editing through fluid dynamics simulation.
...
<details>
  <summary>Details</summary>
Motivation: Existing mask-based pruning methods like MaskGS use global mean regularization, which is misaligned with the local per-pixel reconstruction loss that determines image quality along individual camera rays.

Method: Proposes SVR-GS with spatially variant regularizer that renders per-pixel spatial masks from each Gaussian's effective contribution along rays, applying sparsity pressure on low-importance Gaussians. Implements three spatial-mask aggregation strategies in CUDA with gradient analysis.

Result: Reduces Gaussians by 1.79x vs MaskGS and 5.63x vs 3DGS with only 0.50 dB and 0.40 dB PSNR drops respectively on Tanks&Temples, Deep Blending, and Mip-NeRF360 datasets.

Conclusion: SVR-GS produces significantly smaller, faster, and more memory-efficient models suitable for real-time applications in robotics, AR/VR, and mobile perception.
...
Method: Uses pre-trained VGGT to extract dense point maps from multi-view images, merges them into unified point clouds with confidence scores, then processes through parallel DGCNN decoder heads with composite Gaussian loss for volume and surface area prediction.

Result: Achieves competitive accuracy and generalizes well to unseen coral morphologies.

Conclusion: Provides efficient and scalable coral geometry estimation directly from sparse image sets, enabling better coral growth analysis and reef monitoring.
...
<details>
  <summary>Details</summary>
Motivation: Existing traffic video analysis approaches struggle with accurately modeling spatiotemporal causality and integrating domain-specific knowledge, limiting effectiveness in complex traffic scenarios.

Method: Built on Qwen2.5-VL backbone with LoRA fine-tuning, plus a knowledge prompting module combining Chain-of-Thought reasoning with Retrieval-Augmented Generation for precise traffic regulation injection.

Result: Achieves state-of-the-art performance on TrafficQA and DriveQA benchmarks, with strong zero-shot reasoning and cross-scenario generalization capabilities.

Conclusion: Traffic-MLLM demonstrates superior multimodal traffic data processing through effective spatiotemporal modeling and domain knowledge integration, advancing intelligent transportation systems.
...
<details>
  <summary>Details</summary>
Motivation: Traditional 3D reconstruction relies on RGB data only, while existing multispectral methods have high costs, low accuracy, and poor geometric features. Current NeRF models can't handle multi-band information.

Method: Modified NeRF architecture with three key changes: expanded hidden layers for 6-band inputs, redesigned residual functions for spectral discrepancy optimization, and adapted data compression for higher bit-depth multispectral imagery.
...
Conclusion: The proposed Multispectral-NeRF effectively addresses the limitations of current multispectral 3D reconstruction methods by extending NeRF to handle multi-band spectral data with improved accuracy and quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing voxel-based and plane-based SSC methods struggle with realistic geometric details, while neural reconstruction methods like NeRF and 3DGS suffer from high computational costs and poor semantic accuracy in large-scale autonomous driving scenes.

Method: Proposes SPHERE with two modules: Semantic-guided Gaussian Initialization (SGI) that uses dual-branch 3D representations to locate focal voxels as anchors for efficient Gaussian initialization, and Physical-aware Harmonics Enhancement (PHE) that incorporates semantic spherical harmonics for physical-aware contextual details and semantic-geometry consistency.

Result: Extensive experiments on SemanticKITTI and SSCBench-KITTI-360 benchmarks validate the effectiveness of SPHERE, achieving superior performance in 3D semantic scene completion.

Conclusion: SPHERE successfully integrates voxel and Gaussian representations to address both semantic accuracy and realistic geometric detail challenges in camera-based 3D semantic scene completion for autonomous driving.
...
<details>
  <summary>Details</summary>
Motivation: Existing GAN and VAE-based steganography models suffer from mode collapse, which causes information imbalance between cover and secret images in the stego image and affects extraction quality.

Method: Proposes StegOT model with multiple channel optimal transport (MCOT) module that transforms multi-peak feature distributions into single-peak distributions to achieve information trade-off between cover and secret images.
...
Conclusion: StegOT effectively addresses mode collapse in steganography using optimal transport theory, improving information balance and image quality in both encoding and decoding processes.
...
<details>
  <summary>Details</summary>
Motivation: AI models for skin lesion classification have shown promising performance but are susceptible to bias based on skin tone representation. The Fitzpatrick Skin Tone scale has been criticized for having greater granularity for lighter-skinned subjects, raising concerns about fairness in AI dermatology applications.

Method: Trained multiple AI models to classify benign vs. malignant lesions using FST-specific data with different granularity levels. Compared models trained on FST-specific data (three groups: FST 1/2, 3/4 and 5/6) against general models trained on FST-balanced data.

Result: Models trained on FST-specific data (three groups) generally performed better than general models trained on FST-balanced data. Reducing the granularity of FST scale information (from 1/2 and 3/4 to 1/2/3/4) had a detrimental effect on performance.

Conclusion: The granularity of FST groups significantly impacts AI model performance for skin lesion classification. Given concerns about human biases in FST scale categories, researchers should move away from FST scale and transition to alternative scales that better represent the diversity of human skin tones in fair AI research.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D forest datasets are too small for robust tree segmentation systems, and real data collection is expensive. Synthetic data has shown success in other domains like self-driving, suggesting potential for forest vision tasks.

Method: Developed a synthetic data generation pipeline integrating game engines with physics-based LiDAR simulation to create large-scale annotated 3D forest datasets. Used synthetic data for pretraining and minimal real forest plot annotation for fine-tuning.

Result: Pretrained model fine-tuned on just 0.1 hectare of real data achieves segmentation performance competitive with models trained on full-scale real datasets. Critical success factors identified: physics, diversity, and scale.

Conclusion: Synthetic data can substantially reduce the need for labeled real data in tree segmentation, paving the way for more robust 3D forest vision systems. The pipeline and dataset are publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Existing concept sliders struggle with no-AIGC images from real-world settings, creating a need for better methods that can handle diverse image categories while maintaining realism and quality.
...
Conclusion: Beyond Sliders successfully bridges the gap in image manipulation for real-world images, offering superior performance compared to existing concept slider methods through its innovative adversarial refinement approach.
...
<details>
  <summary>Details</summary>
Motivation: Fine-grained visual classification is sensitive to geometric variability (orientations, scales, distortions). Existing equivariant architectures are computationally expensive and restrictive. STNs offer flexibility but need improved robustness.

Method: Decompose affine transformations into rotation, scaling, and shearing components. Use Gaussian variational posteriors to model uncertainty. Employ component-wise alignment loss guided by augmentation parameters. Shared localization encoder with geometric constraints.

Result: Consistent improvement in robustness on challenging moth classification benchmarks compared to other STN approaches.

Conclusion: The probabilistic component-wise extension of STNs provides effective canonicalization for transformer-based vision pipelines, enhancing geometric robustness without architectural constraints.
...
<details>
  <summary>Details</summary>
Motivation: RFID-based cattle identification systems are unreliable due to tag loss, damage, tampering, and security vulnerabilities. Biometric identification using unique cattle muzzle patterns offers a more robust alternative, but deep learning approaches face challenges with limited data, collection disruptions, and dynamic herd compositions requiring frequent retraining.

Method: Developed a Cooperative Model-Agnostic Meta-Learning (CCoMAML) framework with Multi-Head Attention Feature Fusion (MHAFF) as feature extractor. This few-shot learning approach enables efficient learning from minimal data samples without requiring model retraining.
...
Method: Implements adversarially and naturally robust Hellinger distance-based feature class aggregation with attention mechanisms and a novel Hellinger Similarity contrastive loss function.

Result: Achieves resilience to adversarial perturbations up to ε=0.30 and Gaussian noise up to σ=0.30, with performance gains of 1.20% (1-shot) and 1.40% (5-shot) on miniImageNet, and superior image reconstruction with FID score of 2.75.

Conclusion: ANROT-HELANet establishes new state-of-the-art performance in few-shot learning while maintaining robustness against both adversarial and natural perturbations through its Hellinger-based approach.
...
TL;DR: CMLReID is a CLIP-based framework that addresses both same-cloth and clothing-change person re-identification in continual learning settings using adaptive prompts and knowledge fusion techniques.


<details>
  <summary>Details</summary>
Motivation: Real-world surveillance systems need person re-identification models that handle clothing changes while maintaining continual learning capabilities, but existing methods either focus only on same-cloth settings or treat clothing-change re-ID as a separate problem.

Method: CMLReID uses two novel components: Context-Aware Semantic Prompt (CASP) for generating adaptive prompts that align multi-grained visual cues with semantic text space, and Adaptive Knowledge Fusion and Projection (AKFP) that creates robust prototypes through dual-path learning with Clothing-State-Aware Projection Loss.

Result: Experiments across multiple datasets show CMLReID outperforms all state-of-the-art methods with strong robustness and generalization despite clothing variations and sequential learning challenges.

Conclusion: The proposed framework successfully addresses the hybrid continual learning problem for person re-identification, effectively handling both same-cloth and clothing-change scenarios while mitigating representation mismatches and forgetting issues.
...
Method: Extract class-agnostic attributes using CLIP, represent them as key-value pairs (visual prototype + textual prompt), maintain domain-specific attribute dictionaries, and perform cross-domain alignment through visual attention consistency and prediction consistency.
...
Conclusion: The proposed attribute-based framework provides an effective rehearsal-free solution for CI-UDA by preserving domain-invariant knowledge and enabling better cross-domain alignment, demonstrating superior performance over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation studies for synthetic datasets lack a universally accepted standard framework, making it difficult to robustly assess synthetic data quality and guide optimization efforts.

Method: Integrates generalized cross-validation experiments and domain transfer learning principles. Trains task-specific models on both synthetic and real datasets, constructs a GCV Matrix to quantify domain transferability, and introduces two metrics for simulation quality and transfer quality.
...
TL;DR: ROSGS is a two-stage pipeline that efficiently reconstructs relightable outdoor scenes using Gaussian Splatting representation, achieving state-of-the-art relighting accuracy and rendering efficiency.
...
Method: Two-stage pipeline: 1) Reconstructs scene geometry using 2D Gaussian Splatting with monocular normal priors, 2) Decomposes texture and lighting through a hybrid lighting model using spherical Gaussian for directional sunlight and Spherical Harmonic coefficients for low-frequency skylight.

Result: ROSGS achieves state-of-the-art performance in relighting outdoor scenes, demonstrating superior relighting accuracy and rendering efficiency compared to existing methods.

Conclusion: The proposed ROSGS framework effectively addresses the limitations of previous approaches by combining efficient geometric reconstruction with a hybrid lighting model, making it suitable for high-quality relighting of complex outdoor scenes.
...
<details>
  <summary>Details</summary>
Motivation: Existing hallucination mitigation methods require expensive external human annotations or auxiliary models for preference data collection, which increases costs and limits sustainable improvement of LVLMs.

Method: APASI leverages the target LVLM to self-inject hallucinations into generated responses, creating preference pairs based on three key observations of hallucinations. It uses iterative alignment training with curriculum learning to periodically update preference data with increasing challenge levels.

Result: Extensive experiments across six benchmarks show APASI effectively mitigates hallucinations for three baseline models and achieves comparable or superior performance to external dependency methods.

Conclusion: APASI provides an effective and generalizable solution for hallucination mitigation without external dependencies, enabling stable and continuous enhancement of LVLMs through autonomous preference alignment.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for unaligned scene change detection rely solely on 2D visual cues, which fail under large viewpoint changes and lack explicit geometric reasoning, limiting their ability to handle visual overlaps and occlusions reliably.

Method: Proposes a training-free framework that integrates geometric priors from a Geometric Foundation Model with visual foundation model representations to enable reliable change detection under viewpoint misalignment.

Result: The approach achieves superior and robust performance on PSCD, ChangeSim, and PASLCD datasets, demonstrating effectiveness in handling viewpoint variations.

Conclusion: Leveraging geometric priors from foundation models provides a powerful solution for unaligned scene change detection, addressing core challenges like visual overlap identification, robust correspondence, and occlusion detection.
...
TL;DR: UnLoc is an efficient data-driven method for camera localization using floorplans that introduces probabilistic uncertainty modeling and leverages pre-trained depth models, achieving significant performance improvements over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Floorplan data is widely available and robust to visual changes, but existing methods lack uncertainty modeling in depth predictions and require custom depth networks trained for each specific environment, limiting generalization.

Method: Proposes a novel probabilistic model that treats depth predictions as explicit probability distributions and uses off-the-shelf pre-trained monocular depth models instead of environment-specific custom networks.

Result: Achieves 2.7x higher localization recall on long sequences (100 frames) and 16.7x higher on short sequences (15 frames) than state-of-the-art on the challenging LaMAR HGE dataset, demonstrating superior accuracy and robustness.

Conclusion: UnLoc provides an efficient and generalizable solution for sequential camera localization by incorporating uncertainty estimation and leveraging pre-trained models, significantly outperforming existing approaches while eliminating the need for per-environment training.
...
TL;DR: SIKNet is a novel learning-aided Kalman filter that uses semantic-independent encoding to improve motion estimation in multi-object tracking, outperforming traditional Kalman filters and other learning-based methods.
...
Method: Proposed Semantic-Independent KalmanNet (SIKNet) with Semantic-Independent Encoder (SIE) that uses 1D convolution along homogeneous-semantic elements and fully-connected layers with nonlinear activation for heterogeneous-semantic elements.

Result: SIKNet outperforms traditional Kalman filters and achieves superior robustness and accuracy compared to existing learning-aided filters, as demonstrated on a large-scale semi-simulated dataset from multiple MOT datasets.

Conclusion: The proposed SIKNet effectively handles motion estimation challenges in MOT by combining semantic-independent encoding with learning techniques, providing improved performance over traditional and existing learning-based filtering methods.
...
TL;DR: This doctoral research demonstrates that MLP-based models outperform transformers and CNNs for medical image analysis by enabling fine-grained long-range dependency modeling on high-resolution features containing detailed anatomical/pathological information.


<details>
  <summary>Details</summary>
Motivation: Medical image computing requires models that capture both global context and local subtle visual characteristics, but existing approaches (CNNs limited by locality, transformers limited by computational constraints) struggle with fine-grained long-range dependency modeling in high-resolution medical images.

Method: The research investigates transformers for pixel- and image-wise medical vision tasks, then pioneers MLP-based visual models specifically designed to capture fine-grained long-range visual dependency in medical images through extensive experimentation.

Result: Extensive experiments confirm the critical role of long-range dependency modeling in MIC and reveal that MLPs provide feasibility in modeling finer-grained long-range dependency among higher-resolution medical features containing enriched anatomical/pathological details.

Conclusion: MLPs establish as a superior paradigm over transformers/CNNs, consistently enhancing performance across various medical vision tasks and paving the way for next-generation medical vision backbones due to their ability to handle high-resolution features efficiently.
...
<details>
  <summary>Details</summary>
Motivation: In near-ambient conditions relevant to computer vision, both reflected environmental light and emitted surface light are comparable and vary over time, but previous methods either assume one component dominates or treat the second as constant. Accurate separation is essential for understanding object properties like emissivity, temperature, reflectance and shape.
...
Result: Quantitatively evaluated using carefully calibrated emissivities for various materials. Showed qualitative results on complex everyday scenes including glass with hot liquid and people moving in background. Successfully separated reflected and emitted components.

Conclusion: The introduced method provides the first effective solution for separating reflected and emitted light components in thermal imaging under near-ambient conditions, enabling more accurate analysis of material properties and scene dynamics in computer vision applications.
...
TL;DR: SSL can learn meaningful representations even without strict instance consistency, with moderate view diversity improving performance while excessive diversity reduces effectiveness.
...
Result: SSL still learns meaningful representations without strict instance consistency. Moderate view diversity (enforced through zero overlapping or smaller crop scales) enhances downstream performance on classification and dense prediction tasks, while excessive diversity reduces effectiveness.

Conclusion: There is an optimal range for view diversity in SSL, with moderate EMD values correlating with improved learning. These findings provide insights for future SSL framework design and are robust across diverse data sources.
...
TL;DR: Two regularization methods to make CNNs more robust to image corruptions by encouraging shape-based rather than texture-based representations.
...
Method: Proposed two complementary regularization strategies: 1) auxiliary loss enforcing feature consistency between original and low-frequency filtered inputs to reduce high-frequency texture dependence, 2) supervised contrastive learning to structure feature space around class-consistent shape representations.

Result: Both methods improved corruption robustness on CIFAR-10-C benchmark without degrading clean accuracy.

Conclusion: Loss-level regularization can effectively steer CNNs toward more shape-aware and resilient representations, making them more robust to common image corruptions.
...
TL;DR: GLaVE-Cap is a novel video captioning framework that addresses limitations in current local-to-global approaches by introducing TrackFusion for comprehensive local caption generation and CaptionBridge for effective local-global interaction, achieving state-of-the-art performance.
...
Conclusion: GLaVE-Cap successfully addresses the limitations of current video captioning approaches through its innovative global-local alignment framework and vision expert integration, providing comprehensive and coherent video descriptions.
...
TL;DR: A portable 3D skin imaging probe using GelSight technology achieves micron-level wrinkle measurement accuracy and demonstrates measurable wrinkle reduction after moisturizer application.


<details>
  <summary>Details</summary>
Motivation: There is no existing portable, high-resolution 3D skin reconstruction device that has been validated for depth measurement across various body locations, limiting objective dermatological assessment.
...
TL;DR: MixANT introduces a mixture of experts approach to dynamically select contextually relevant forget-gate matrices in State Space Models, improving long-term human activity anticipation without sacrificing efficiency.
...
Method: Proposes a mixture of experts architecture that dynamically selects appropriate A matrices (forget-gates) based on input features, enhancing representational capacity while maintaining computational efficiency.

Result: Outperforms state-of-the-art methods on 50Salads, Breakfast, and Assembly101 datasets across all evaluation settings, demonstrating superior performance in human activity anticipation.

Conclusion: Input-dependent forget-gate mechanisms are crucial for reliable prediction of human behavior in diverse real-world scenarios, and MixANT effectively addresses this need with dynamic parameter selection.
...
TL;DR: Hypernetwork-based method for handling missing modalities in medical imaging, dynamically generating task-specific models that outperform traditional approaches by up to 8% accuracy.


<details>
  <summary>Details</summary>
Motivation: Real-world clinical environments often have incomplete multi-modal medical imaging data, and existing approaches (discarding samples, imputation, or dropout methods) limit robustness and generalizability.

Method: A hypernetwork that learns to predict parameters of task-specific classification models conditioned on available modalities, enabling training and inference on all samples regardless of data completeness.

Result: Outperformed state-of-the-art approaches with absolute accuracy increase up to 8% when trained on dataset with 25% completeness (75% missing modalities).

Conclusion: The approach provides an efficient solution for real-world multi-modal medical data analysis by enabling a single model to generalize across all modality configurations.
...
TL;DR: Proposes weighted rotation blending using quaternion averaging to address Gaussian deformation artifacts in human avatar animation, enabling simpler vertex-based Gaussians that work with standard Gaussian rasterizers.


<details>
  <summary>Details</summary>
Motivation: Linear blend skinning causes artifacts when deforming Gaussians due to their non-linear rotation properties. Existing solutions require complex mesh properties or corrective models, which are inefficient.

Method: Weighted rotation blending approach leveraging quaternion averaging to properly handle Gaussian rotations during deformation, requiring only modification of linear blend skinning technique.

Result: Enables efficient animation of vertex-based Gaussians that can be integrated into any engine using standard Gaussian rasterizers without complex additional components.

Conclusion: The proposed quaternion-based rotation blending provides a simpler and more effective solution for Gaussian deformation in human avatar animation compared to existing mesh-based or corrective approaches.
...
TL;DR: A method to disentangle biological and technical factors in medical imaging data using latent space rotation, improving cluster consistency and enabling better biomarker discovery across multi-center data.
...
Method: Active learning approach via post-hoc rotation of data latent space to disentangle biological and technical factors, creating stable clusters across different acquisition settings.

Result: Cluster consistency improved by +19.01% (ARI), +16.85% (NMI), and +12.39% (Dice) compared to entangled representations, outperforming four state-of-the-art harmonization methods. Enhanced Cox survival prediction for idiopathic pulmonary fibrosis patients.

Conclusion: The label-free framework facilitates biomarker discovery in multi-center routine imaging data by effectively separating biological from technical variations.
...
TL;DR: A masked autoencoder approach for handling missing MRI sequences in 3D medical imaging, using multi-modal transformer architecture that can infer missing data and improve downstream task performance.


<details>
  <summary>Details</summary>
Motivation: Missing input sequences are common in medical imaging data, posing challenges for deep learning models that require complete input data for optimal performance.

Method: Developed a masked autoencoder (MAE) paradigm inspired by MultiMAE, treating each MRI sequence as separate modality with late-fusion transformer encoder and individual decoder streams for multi-task reconstruction.

Result: Achieved absolute improvement of 10.1 overall Dice score and 0.46 MCC over baseline methods when handling missing input sequences in downstream segmentation and classification tasks.

Conclusion: The method provides a flexible and generalizable encoder for brain MRIs that can infer missing sequences and be adapted to various downstream applications, demonstrating strong performance and robustness.
...
TL;DR: TrajTrack is a lightweight LiDAR-based 3D single object tracking framework that combines explicit motion proposals with implicit trajectory modeling, achieving state-of-the-art performance while maintaining high speed.
...
Method: Proposes a trajectory-based paradigm where a base two-frame tracker is enhanced by learning motion continuity from historical bounding box trajectories. Uses explicit motion proposals followed by implicit motion modeling to predict future trajectories and refine initial proposals.
...
Conclusion: TrajTrack successfully resolves the efficiency-robustness trade-off in 3D SOT by leveraging trajectory-based learning without requiring additional point cloud inputs, making it both effective and computationally efficient.
...
<details>
  <summary>Details</summary>
Motivation: Infrared and visible image fusion aims to combine complementary structural and textural information from different spectral domains, but existing methods need better modeling of inter-modality interactions and semantic preservation in task-critical regions.

Method: Proposes FusionNet with modality-aware attention mechanism, pixel-wise alpha blending module for adaptive content-aware fusion, and target-aware loss using weak ROI supervision to maintain semantic consistency in important object regions.
...
TL;DR: MHIM-MIL is a novel Multiple Instance Learning framework that uses masked hard instance mining to improve computational pathology by focusing on challenging tissue instances rather than just easy-to-classify ones.


<details>
  <summary>Details</summary>
Motivation: Existing MIL methods in computational pathology focus on identifying salient instances via attention mechanisms, but this creates bias towards easy-to-classify instances while neglecting challenging ones that are crucial for accurate discriminative boundaries.

Method: Uses a Siamese structure with consistency constraint, class-aware instance probability, and momentum teacher to mask salient instances and implicitly mine hard instances. Employs large-scale random masking for diversity and global recycle network to preserve key features. Student updates teacher via exponential moving average.

Result: Outperforms latest methods in performance and efficiency on cancer diagnosis, subtyping, survival analysis tasks across 12 benchmarks.

Conclusion: MHIM-MIL effectively addresses the bias problem in MIL for computational pathology by focusing on hard instances, leading to improved performance and efficiency in various cancer-related tasks.
...
TL;DR: SFGNet is a novel network for camouflaged object detection that integrates semantic prompts and frequency-domain features with specialized modules for handling complex backgrounds and preserving boundary details.


<details>
  <summary>Details</summary>
Motivation: Most existing camouflaged object detection methods overlook semantic differences in textual prompts and fine-grained frequency features, limiting their ability to accurately segment objects that blend into surroundings.

Method: Proposes SFGNet with Multi-Band Fourier Module (MBFM) to handle complex backgrounds and blurred boundaries, and Interactive Structure Enhancement Block (ISEB) to ensure structural integrity and boundary details in predictions.

Result: Extensive experiments on three COD benchmark datasets demonstrate that the method significantly outperforms state-of-the-art approaches.

Conclusion: The integration of semantic prompts and frequency-domain features with specialized modules effectively improves camouflaged object detection and boundary perception capabilities.
...
TL;DR: The paper proposes three zero-shot auxiliary reasoning methods to improve GUI grounding performance in vision-language models by providing explicit spatial cues like axes, grids and labeled intersections, without requiring fine-tuning.


<details>
  <summary>Details</summary>
Motivation: General vision-language models struggle with GUI grounding tasks despite having latent spatial understanding capabilities. There's a gap between their implicit spatial knowledge (measured by Pointing Game) and explicit coordinate output performance, and current fine-tuning approaches require high data/annotation costs.

Method: Three zero-shot auxiliary reasoning methods that provide explicit spatial cues (axes, grids, and labeled intersections) as part of the input image to help VLMs articulate their implicit spatial understanding without fine-tuning.

Result: Evaluation on four GUI grounding benchmarks across seven open-source and proprietary VLMs shows that the proposed methods substantially improve GUI grounding performance.

Conclusion: The proposed zero-shot auxiliary reasoning methods effectively bridge the gap between VLMs' latent spatial understanding and explicit coordinate output capabilities for GUI grounding, providing a cost-effective alternative to fine-tuning approaches.
...
TL;DR: GPS-SLAM combines Gaussian and SDF representations to achieve real-time 3D reconstruction at 150+ fps, providing 10x speedup over current Gaussian-based SLAM methods while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Current Gaussian-based SLAM methods are computationally slow (<20 fps) compared to geometry-centric approaches (hundreds of fps), due to the heavy computational burden of modeling scenes with numerous Gaussians and complex iterative optimization.

Method: Proposes a Gaussian-SDF hybrid representation: colorized SDF for smooth geometry and appearance, plus 3D Gaussians for underrepresented details. SDF is efficiently constructed via RGB-D fusion, while Gaussians undergo iterative optimization, enabling 50% fewer Gaussians and 75% fewer iterations.
...
Conclusion: The hybrid representation enables real-time performance for photorealistic 3D reconstruction, bridging the performance gap between Gaussian-based and geometry-centric SLAM methods.
...
TL;DR: Proposes Hierarchical Identity Learning (HIL) framework with multi-center contrastive learning and bidirectional matching to address fine-grained variations in unsupervised visible-infrared person re-identification.


<details>
  <summary>Details</summary>
Motivation: Existing cluster-based methods focus on commonality within clusters but neglect finer-grained differences among images, limiting performance in unsupervised cross-modal person re-identification.

Method: HIL framework with secondary clustering to generate multiple memories per cluster, Multi-Center Contrastive Learning (MCCL) to refine representations, and Bidirectional Reverse Selection Transmission (BRST) for reliable cross-modal matching.
...
Conclusion: The proposed HIL framework effectively addresses fine-grained variations and cross-modal discrepancies in unsupervised visible-infrared person re-identification, achieving state-of-the-art results.
...
TL;DR: BiCDO is an iterative data-centric framework that finds optimal class distributions for multi-class image classification, allowing performance prioritization for specific classes and improving model reliability with minimal code changes.


<details>
  <summary>Details</summary>
Motivation: Addresses the need for performance prioritization in safety-critical scenarios (e.g., prioritizing 'Human' over 'Dog') and aims to minimize bias and variance in multi-class classification by optimizing class distributions rather than using uniform distributions.

Method: Iterative framework that identifies Pareto optimized class distributions, determines optimal number of images per class, and can be integrated into existing training pipelines with minimal code changes. Validated using EfficientNet, ResNet and ConvNeXt on CIFAR-10 and iNaturalist21 datasets.

Result: Demonstrated improved and balanced model performance through optimized data distribution across different architectures and datasets.

Conclusion: BiCDO provides an effective data-centric approach to enhance reliability in multi-class image classification by optimizing class distributions, particularly valuable for safety-critical applications where certain classes require higher performance prioritization.
...
TL;DR: MVQA-68K is a novel 68,000+ video dataset with multi-dimensional quality annotations across 7 dimensions including aesthetics, camera movement, and factual consistency, featuring detailed reasoning chains to improve VQA interpretability and performance.


<details>
  <summary>Details</summary>
Motivation: Traditional video quality assessment methods produce single numerical scores that lack comprehensiveness and interpretability, which is inadequate for selecting high-quality videos from large-scale datasets used in pre-training modern video generation models.

Method: Created MVQA-68K dataset with over 68,000 carefully annotated videos covering seven quality dimensions, each annotation includes detailed chain-of-thought reasoning. Used this dataset to train multimodal large language models for enhanced VQA performance.

Result: MVQA-68K significantly enhances MLLM performance on VQA tasks, achieving state-of-the-art results on internal test set and public benchmarks (LSVQ-test, LSVQ-1080p, LIVE-VQC). Incorporating explicit reasoning process substantially boosts zero-shot generalization.

Conclusion: The multi-dimensional approach with detailed reasoning annotations provides more comprehensive and interpretable video quality assessment, enabling better selection of high-quality videos for training advanced video generation models.
...
Method: HyGDL uses a single encoder and analytically defines style as the component orthogonal to style-invariant content via vector projection. It follows the Invariance Pre-training Principle by systematically varying style biases while keeping supervision constant to force learning of invariant essence.

Result: The framework achieves explicit content-style disentanglement, addressing shortcut learning at its core rather than just surface-level feature alignment or separation.

Conclusion: HyGDL provides a fundamental solution to shortcut learning in SSL by altering the learning mechanism itself through hybrid generative-discriminative approach and analytical style-content separation.
...
TL;DR: This paper introduces a new softmax-based frame allocation strategy for video anomaly detection that prioritizes anomaly-dense segments while maintaining full-video coverage, and constructs two complementary benchmarks for comprehensive evaluation.
...
Method: Proposes a softmax-based frame allocation strategy that balances sampling across temporal scales by focusing on anomaly-dense segments while ensuring full-video coverage. Builds two benchmarks: image-based for frame-level reasoning and video-based for temporally localized segments with abnormality scoring.
...
Conclusion: The proposed approach provides a more comprehensive evaluation framework for video anomaly detection that bridges the gap between frame-level and video-level analysis, demonstrating superior performance through anomaly-focused temporal sampling.
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations of 2D deepfake methods that suffer from geometric inconsistencies and poor generalization to novel views, enabling more realistic and controllable 3D visual forgeries.
...
Conclusion: Bridges 3D modeling with deepfake synthesis, enabling scene-aware and immersive visual forgeries, while highlighting the potential threat of 3D Gaussian Splatting for manipulation attacks.
...
<details>
  <summary>Details</summary>
Motivation: Random noise initialization in vanilla diffusion models can introduce mismatched semantic information in masked regions, leading to biased inpainting results with low consistency and coherence with surrounding areas.
...
TL;DR: A real-world benchmark dataset for all-in-one adverse weather image restoration containing aligned degraded-clean image pairs across rain, snow, and haze conditions with diverse scenes and lighting.


<details>
  <summary>Details</summary>
Motivation: Existing methods use synthetic datasets with significant domain gaps in resolution, style, and characteristics, hindering unified model development and fair evaluation. Lack of large-scale real-world datasets is a critical bottleneck.

Method: Created a real-world dataset with precisely aligned degraded and clean images captured under various weather conditions (rain, snow, haze) across diverse outdoor scenes and illumination settings to enable supervised learning.

Result: Provides a comprehensive benchmark for evaluating task-specific, task-general, and all-in-one restoration methods. The dataset enables rigorous evaluation and advancement of robust all-in-one image restoration.

Conclusion: The dataset offers a valuable foundation for advancing practical all-in-one image restoration in real-world scenarios and has been publicly released to support research community development.
...
TL;DR: RVMamba and Joint-OCTAMamba frameworks using Mamba state-space model for improved retinal vessel and foveal avascular zone segmentation in OCTA imaging, outperforming existing methods on OCTA-500 dataset.


<details>
  <summary>Details</summary>
Motivation: Current 2D-based methods for retinal vessel segmentation in OCTA imaging provide insufficient accuracy, and existing joint segmentation models suffer from performance imbalance between different tasks like FAZ segmentation.
...
TL;DR: DTGen is a few-shot data augmentation method using diffusion models for fine-grained dirty tableware recognition, enabling high-quality synthetic data generation and improved classification performance with minimal real samples.


<details>
  <summary>Details</summary>
Motivation: Existing methods for intelligent tableware cleaning suffer from coarse-grained classification and scarcity of few-shot data, making them inadequate for industrialization requirements in food safety and smart home applications.

Method: DTGen uses generative diffusion models with LoRA for efficient domain specialization, structured prompts for diverse dirty image generation, and CLIP-based cross-modal filtering to ensure data quality. It also includes lightweight deployment strategies for embedded systems.

Result: Under extremely limited real few-shot conditions, DTGen can synthesize virtually unlimited high-quality samples, significantly improving classifier performance and enabling fine-grained dirty tableware recognition.

Conclusion: DTGen validates the value of generative AI in few-shot industrial vision and provides a feasible deployment path for automated tableware cleaning and food safety monitoring, with potential integration into embedded dishwashers for intelligent energy and detergent regulation.
...
TL;DR: MindVL is a multimodal LLM trained on Ascend NPUs that achieves Qwen2.5-VL level performance using only 1/10 of training data, featuring native-resolution vision processing and optimized training framework.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient multimodal language model that can process images at native variable resolutions without degradation, while being optimized for training on Ascend NPUs with reduced data requirements.

Method: Uses native-resolution Vision Transformers, three-phase training (warm-up, multitask training, instruction tuning), multimodal data packaging, hybrid parallelism, test-time resolution search, and weight averaging. Developed Mindspeed-MLLM framework for Ascend NPU optimization.

Result: Achieves performance on par with Qwen2.5-VL in general multimodal understanding and document/table comprehension despite using only 1/10 of training data. Delivers leading performance in OCR assessments.

Conclusion: MindVL demonstrates that optimized training frameworks and native-resolution processing can achieve state-of-the-art multimodal performance with significantly reduced data requirements, making it highly efficient for complex visual content analysis.
...
TL;DR: A pipeline to extract navigable trails from scanned paper maps for GPS navigation, combining georeferencing, segmentation, graph construction, and iterative refinement.
...
TL;DR: A new industrial dataset (IMD) for 6D pose estimation featuring metallic, texture-less objects to address limitations of existing benchmarks that use everyday textured objects.


<details>
  <summary>Details</summary>
Motivation: Existing 6D pose estimation benchmarks use everyday objects with rich textures and low reflectivity, which don't generalize well to industrial settings where objects are metallic, texture-less, and highly reflective.

Method: Created a dataset of 45 true-to-scale industrial components captured with RGB-D camera under natural lighting and varied arrangements. Evaluated state-of-the-art models (XMem, SAM2 for segmentation; BundleTrack, BundleSDF for pose estimation) on three tasks: video segmentation, 6D pose tracking, and one-shot pose estimation.
...
TL;DR: Depthwise separable CNNs naturally converge to just 8 universal master key filters that are linear transformations of each other, achieving strong performance even when frozen.


<details>
  <summary>Details</summary>
Motivation: To extend the Master Key Filters Hypothesis by showing that DS-CNNs inherently converge to a minimal set of fundamental filters regardless of architecture or dataset.
...
Conclusion: Depthwise convolutional layers naturally converge to fundamental spatial operators (DoGs, Gaussians, derivatives) that mirror classical image processing and biological visual systems, providing insights for generalization and transfer learning.
...
Conclusion: Established best practices for training and deploying document layout detectors, with all models and code released openly to benefit the document conversion community.
...
TL;DR: MISRA is a segmentation framework for microsurgical instruments that addresses thin structure segmentation challenges through luminance augmentation, skip attention, and iterative feedback, achieving 5.37% improvement in mean IoU.
...
Method: Proposes MISRA framework that augments RGB input with luminance channels, integrates skip attention to preserve elongated features, and employs an Iterative Feedback Module (IFM) for continuity restoration across multiple passes.
...
Conclusion: MISRA represents a promising step toward reliable scene parsing for computer-assisted and robotic microsurgery, with the framework and new benchmark dataset advancing the field.
...
<details>
  <summary>Details</summary>
Motivation: Uneven trajectory density causes fragmented roads in sparse areas and redundant segments in dense regions, posing challenges for existing map inference methods that need to handle both scenarios effectively.

Method: Proposes DGMap with Multi-scale Grid Encoding, Mask-enhanced Keypoint Extraction, and Global Context-aware Relation Prediction to integrate global semantic context with local geometric features for improved keypoint detection and false connection suppression.
...
Conclusion: DGMap effectively addresses trajectory density challenges through global context integration, demonstrating superior performance in reducing road fragmentation and suppressing redundant segments across varying density conditions.
...
TL;DR: EchoCare is a novel ultrasound foundation model developed through self-supervised learning on a large-scale multi-source dataset (4.5M images from 23+ countries), featuring a hierarchical classifier for joint pixel-level and representation-level learning, achieving SOTA performance across 10 ultrasound benchmarks.


<details>
  <summary>Details</summary>
Motivation: Address the scarcity of large labeled ultrasound datasets and limited generalizability of task-specific models in clinical AI by developing a generalizable foundation model for diverse ultrasound applications.

Method: Self-supervised learning on EchoCareData (4.5M ultrasound images from diverse global sources) with a hierarchical classifier architecture for joint learning of pixel-level and representation-level features to capture both global anatomical contexts and local ultrasound characteristics.
...
<details>
  <summary>Details</summary>
Motivation: Existing learning-based 3D face reconstruction methods require large 3D facial datasets that are difficult to obtain, and struggle with capturing detailed multi-scale features under diverse facial attributes and conditions, leading to incomplete reconstructions.

Method: Multi-Scale Feature Fusion with Multi-Attribute (MSMA) framework that integrates multi-scale feature fusion with multi-attribute learning and uses a large-kernel attention module to enhance precision of feature extraction across scales for accurate 3D facial parameter estimation.

Result: Comprehensive experiments on MICC Florence, Facewarehouse and custom datasets show the approach achieves results on par with current state-of-the-art methods and in some instances surpasses SOTA performance across challenging conditions.

Conclusion: The proposed MSMA framework effectively addresses the challenges of 3D face reconstruction from unconstrained images by improving feature extraction and multi-attribute learning, demonstrating competitive performance with existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current foundation models like SAM2 have strong zero-shot video segmentation but lack proper identity management and memory efficiency for MOTS applications.

Method: Integrates pre-trained object detectors with SAM2 and a novel Seg2Track module for track initialization, management, and reinforcement without fine-tuning. Uses sliding-window memory strategy.

Result: Achieves 4th overall rank on KITTI MOTS for car and pedestrian classes, sets new benchmark in association accuracy (AssA), and reduces memory usage by 75% with minimal performance loss.

Conclusion: Seg2Track-SAM2 advances MOTS by combining robust zero-shot tracking, enhanced identity preservation, and efficient memory utilization without requiring fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: Retinal vessel segmentation is crucial for early disease diagnosis, but existing methods like SA-UNet underuse attention in skip connections and fail to address severe foreground-background class imbalance in retinal images.

Method: Proposed SA-UNetv2 with cross-scale spatial attention injected into all skip connections to enhance multi-scale feature fusion, and a combined weighted Binary Cross-Entropy plus Matthews Correlation Coefficient loss function to improve robustness to class imbalance.

Result: Achieved state-of-the-art performance on DRIVE and STARE datasets with only 1.2MB memory and 0.26M parameters (less than 50% of SA-UNet), and 1 second CPU inference on 592x592x3 images.

Conclusion: SA-UNetv2 demonstrates strong efficiency and deployability in resource-constrained, CPU-only settings, making it suitable for practical medical applications with limited computational resources.
...
TL;DR: FineQuest is a training-free framework for sports VideoQA that uses dual-mode reasoning (reactive and deliberative) with a multimodal sports knowledge scene graph (SSGraph) to achieve state-of-the-art performance on new benchmarks.


<details>
  <summary>Details</summary>
Motivation: VideoQA based on LLMs struggles with complex sports videos due to the domain-specific knowledge gap between general models and sports understanding requirements.

Method: Proposes FineQuest framework with dual-mode reasoning: reactive for simple queries, deliberative for complex ones. Uses SSGraph - a multimodal sports knowledge scene graph spanning 9 sports to encode visual instances and domain terminology.
...
Conclusion: FineQuest demonstrates effective training-free approach for sports VideoQA by combining cognitive-inspired reasoning with domain-specific knowledge representation through scene graphs.
...
Result: Validated on echocardiography classification benchmark, demonstrating superior performance over specialized baselines in calibration, selective classification, and multi-view fusion.

Conclusion: The proposed framework is architecture-agnostic and can enhance uncertainty estimation and robustness in any supervised learning pipeline for medical image analysis.
...
TL;DR: SpecVLM accelerates vision-language models using speculative decoding with an elastic visual compressor and online distillation, achieving 2.5-2.9x speedups while maintaining lossless decoding.


<details>
  <summary>Details</summary>
Motivation: Direct speculative decoding for VLMs faces challenges due to visual token dominance in prefill stage, causing compute and memory issues with high-resolution images and videos.

Method: Combines EAGLE-2-style speculative decoding with elastic visual compressor (pruning/pooling/convolution/resampler) and online-logit distillation using cross-entropy + Smooth L1 objective.

Result: Achieves 2.5-2.9x end-to-end speedups across LLaVA and MMMU models within 5 epochs, maintaining target model output distribution (lossless) across resolutions and task difficulties.

Conclusion: SpecVLM provides practical acceleration for VLMs through adaptive visual compression and efficient online training, demonstrating scalable performance improvements without offline data requirements.
...
TL;DR: MAFS is a unified network that jointly performs infrared-visible image fusion and semantic segmentation using parallel sub-networks with mutual enhancement between tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods don't explore the reciprocal promotion between pixel-wise image fusion and cross-modal feature fusion from a task-level perspective.

Method: Parallel structure with fusion and segmentation sub-networks, heterogeneous feature fusion strategy, multi-stage Transformer decoder, and dynamic factor for adaptive task weighting.

Result: Achieves competitive results compared with state-of-the-art methods in both image fusion and semantic segmentation tasks.

Conclusion: The proposed framework successfully demonstrates mutual enhancement between image fusion and semantic segmentation tasks through a unified multi-task approach.
...
<details>
  <summary>Details</summary>
Motivation: Existing probabilistic verification approaches struggle with the complexity and dimensionality of modern semantic segmentation tasks, often producing overly conservative guarantees that limit practical utility in safety-critical domains like medical imaging and autonomous driving.
...
Result: Empirical evaluation on large-scale segmentation models across multiple datasets (CamVid, OCTA-500, Lung Segmentation, Cityscapes) demonstrates reliable safety guarantees with substantially tighter bounds compared to state-of-the-art methods.

Conclusion: The proposed framework provides a practical and scalable solution for probabilistic verification of semantic segmentation networks, offering provable safety guarantees while addressing the limitations of previous methods in high-dimensional settings. A toolbox implementation is made available on Github.
...
TL;DR: This paper bridges generative vision-language models and dense alignment methods by using synthetic captions from VLMs for improved zero-shot segmentation.


<details>
  <summary>Details</summary>
Motivation: Generative VLMs lack spatially dense alignment between vision and language, while existing dense alignment methods need better semantic understanding. The authors aim to combine these approaches.
...
Conclusion: Synthetic captions from VLMs provide an effective and scalable solution for improving dense vision-language alignment in segmentation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods struggle with sparse-view settings due to SfM limitations and MVS-based approaches generate massive Gaussian counts with high memory costs.
...
Conclusion: Advances practicality of 3DGS for constrained-view scenarios through efficient segmentation-driven initialization.
...
Conclusion: Symbolic scene graphs show promise for grounding VLM reasoning but have current limitations, offering guidance for future hybrid VLM-symbolic approaches in video understanding.
...
TL;DR: Dr.V is a hierarchical framework for detecting hallucinations in large video models through fine-grained spatial-temporal grounding across perceptive, temporal, and cognitive levels, consisting of a benchmark dataset and satellite agent.
...
Method: Proposes Dr.V framework with two components: Dr.V-Bench (10k instances with spatial-temporal annotations) and Dr.V-Agent (detects hallucinations through hierarchical spatial-temporal grounding at perceptive, temporal, and cognitive levels).
...
TL;DR: MOT approaches outperform traditional MAT tools for long-term pig tracking, showing potential to improve livestock monitoring accuracy.
...
TL;DR: A novel Weighted Prompt Manipulation technique for zero-shot image generation from poems that dynamically adjusts attention weights and text embeddings in diffusion models to create more accurate visual interpretations.


<details>
  <summary>Details</summary>
Motivation: Poetry invites multiple interpretations based on readers' emotions and backgrounds, so the paper aims to generate customizable images for poems that audiences can modify according to their requirements.

Method: Introduces Weighted Prompt Manipulation (WPM) technique that systematically modifies attention weights and text embeddings within diffusion models, using LLMs like GPT with poetry datasets to dynamically adjust word importance for better visualizations.
...
Method: Proposes Reverse SAM Parameter Configuration Module to mitigate adverse parameters in train-free manner, and T-Visioner Module integrating Test-Time Training layers from language tasks to vision tasks for reinforcing advantageous parameters.
...
Conclusion: SAM-TTT sets new benchmark in COD by simultaneously suppressing adverse parameters while reinforcing advantageous ones through innovative parameter configuration and test-time training integration.
...
TL;DR: Brea-Depth is a novel framework that integrates airway-specific geometric priors into depth foundation models for more accurate bronchoscopic depth estimation, addressing the challenge of anatomical awareness in complex airway structures.


<details>
  <summary>Details</summary>
Motivation: Current depth foundation models lack anatomical awareness in bronchoscopy, overfitting to local textures rather than capturing global airway structure, especially under ambiguous depth cues and poor lighting conditions.

Method: Proposes a depth-aware CycleGAN to refine translation between real bronchoscopic images and airway geometries, and introduces an airway structure awareness loss to enforce depth consistency within the airway lumen while preserving smooth transitions.
...
<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods like Outlier Exposure and Mixture Outlier Exposure still struggle with learning effective class relationships and clearly distinguishing in-distribution from out-of-distribution data, especially for samples near decision boundaries.

Method: Linear interpolation technique in logit space that mixes in-distribution and out-of-distribution data to smooth logits between classes, plus enforcing consistency between logit-space mixing and input-space mixing results.
...
Conclusion: Logit-space mixing technique significantly improves OOD detection performance, particularly for challenging near-boundary out-of-distribution samples, by leveraging the distinct separation properties of class-wise distributions in logit space.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3DSSG methods rely mainly on sensor data and assume complete scene reconstructions, limiting their applicability in real-world incremental settings. There's a need to integrate richer semantic information and handle incremental scenarios.
...
Conclusion: The proposed approach provides an effective solution for incremental 3D semantic scene graph prediction by leveraging multi-modal information, making it suitable for real-world robotics and embodied AI applications without requiring complete scene reconstructions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional FER models trained only on pixels fail to generalize across datasets because facial appearance is an indirect and biased proxy for underlying affect. The authors aim to incorporate brain-informed priors to improve robustness.

Method: Uses a teacher trained on EEG topographic maps to produce frozen Valence/Arousal prototypes. The student model (ResNet-18/50) is trained with conventional loss plus two regularizers: Proto-KD (aligns features to prototypes) and D-Geo (shapes embedding geometry based on depression research findings).

Result: The method shows consistent gains from both prototypes and D-Geo components, with 5x5 grid performing best for stability. It improves robustness in both within-domain and cross-dataset evaluations without adding architectural complexity.

Conclusion: NeuroGaze-Distill provides a simple, deployable solution that enhances FER model generalization by transferring brain-informed priors through static prototypes and geometric regularization, requiring no EEG signals during deployment.
...
TL;DR: VMD method uses variation inference and multimodal knowledge distillation to leverage radiologists' domain knowledge from limited annotations and reports to improve carotid plaque vulnerability diagnosis from 3D MRI images.


<details>
  <summary>Details</summary>
Motivation: Diagnosing carotid plaque vulnerability from 3D MRI is challenging for both radiologists and conventional 3D vision networks. Radiologists use multimodal approaches with various imaging modalities and domain expertise, suggesting the need for multimodal diagnostic networks.

Method: Developed Variation inference and Multimodal knowledge Distillation (VMD) strategy that harnesses cross-modality prior knowledge from limited image annotations and radiology reports to enhance diagnostic accuracy for unannotated 3D MRI images.
...
Conclusion: The VMD method provides an effective approach to leverage radiologists' domain knowledge for automated diagnosis of carotid plaque vulnerability, demonstrating improved performance through multimodal learning.
...
TL;DR: The paper proposes a novel approach to initialize and optimize deep neural networks for image interpolation by leveraging graph theory and unrolling Douglas-Rachford iterations into an interpretable neural network, achieving state-of-the-art results with significantly fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Conventional DNNs use random initialization and SGD optimization, which risks poor local minima. The authors aim to develop a more reliable initialization and optimization method for image interpolation tasks.

Method: 1) Initialize directed graph adjacency matrix based on known interpolator 2) Learn perturbation matrices from data to augment the graph 3) Implement restoration via Douglas-Rachford iterations 4) Unroll DR iterations into lightweight interpretable neural network
...
TL;DR: Sphere-GAN is a novel saliency detection model for 360° videos that uses a Generative Adversarial Network with spherical convolutions, outperforming state-of-the-art methods in predicting saliency maps.


<details>
  <summary>Details</summary>
Motivation: The rise of immersive applications requires new approaches for processing 360° content. While saliency estimation is well-established for 2D content, there are few effective algorithms for 360° saliency detection, which is crucial for optimizing transmission and processing of immersive media.

Method: The paper introduces Sphere-GAN, a Generative Adversarial Network that employs spherical convolutions specifically designed for 360° video saliency detection.

Result: Extensive experiments on a public 360° video saliency dataset show that Sphere-GAN outperforms existing state-of-the-art models in accurately predicting saliency maps.

Conclusion: Sphere-GAN provides an effective solution for 360° video saliency estimation, demonstrating superior performance over current methods and offering potential for optimizing immersive media processing and transmission.
...
TL;DR: Proposed CLAIRE model with dual encoder architecture for land cover classification using optical and SAR imagery fusion, addressing class imbalance with RIFT loss function and adding interpretability through SLM-generated explanations.
...
Method: Dual encoder architecture extracts modality-specific features from optical and SAR imagery, fused using cross-modality attention module (CLAIRE). Uses hybrid RIFT loss (Weighted Focal + Tversky) for class imbalance. Includes SLM-generated metric-driven reasoning for interpretability.
...
Conclusion: The proposed architecture effectively addresses class imbalance and improves segmentation performance while providing transparent, expert-level explanations through SLM-generated reasoning.
...
Conclusion: Offers a foundation for LiDAR-based simulation and data augmentation with strong performance in generating controllable and temporally stable 4D LiDAR sequences from language input.
...
TL;DR: Vision-language models suffer from significant information loss when projecting visual features into language space, with 40-60% distortion in nearest neighbors and degraded retrieval performance.


<details>
  <summary>Details</summary>
Motivation: To study the under-explored information loss that occurs when projecting visual representations into language embedding space in vision-language models, and understand how this impacts model capabilities.
...
Result: Connectors substantially distort visual representation geometry (40-60% divergence in k-NN), correlating with degraded retrieval performance. Patch-level reconstruction shows high loss areas predict model struggles in visual QA tasks.

Conclusion: The projection step in VLMs causes significant information loss that directly impacts model performance, with patch-level analysis providing interpretable insights into where models fail.
...
TL;DR: SCORE is a novel framework for robust concept removal in diffusion models that formulates erasure as an adversarial independence problem, providing theoretical guarantees and outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Diffusion models pose increasing risks in privacy, fairness, and security, creating demand for methods to erase sensitive/harmful concepts while preserving generative capabilities.

Method: SCORE minimizes mutual information between target concepts and generated outputs using adversarial optimization, trajectory consistency, and saliency-driven fine-tuning.

Result: Achieves up to 12.5% higher erasure efficacy than state-of-the-art methods while maintaining comparable or superior image quality across four benchmarks.

Conclusion: SCORE sets a new standard for secure and robust concept erasure in diffusion models with provable guarantees and superior performance.
...
TL;DR: RAM++ is a two-stage framework for all-in-one image restoration that integrates semantic understanding with texture generation to achieve robust restoration across various degradation scenarios.
...
Method: Uses three key designs: 1) Adaptive Semantic-Aware Mask (AdaSAM) for pretraining with pixel-level masks on semantically rich regions, 2) Mask Attribute Conductance (MAC) for selective fine-tuning of high-contribution layers, and 3) Robust Feature Regularization (RFR) leveraging DINOv2's representations for semantically coherent restoration.
...
Conclusion: The framework successfully integrates high-level semantic understanding with low-level texture generation for content-oriented robust image restoration, demonstrating superior performance across various degradation scenarios.
...
TL;DR: This paper introduces RSKT-Seg, a novel open-vocabulary segmentation framework specifically designed for remote sensing images, addressing the domain gap between natural and RS images through specialized modules for rotation-invariant feature extraction and efficient knowledge transfer.


<details>
  <summary>Details</summary>
Motivation: Open-Vocabulary Remote Sensing Image Segmentation (OVRSIS) is underexplored due to lack of standardized benchmarks and the significant domain gap between natural images and remote sensing imagery, which limits the performance of existing OVS methods.

Method: Proposes RSKT-Seg framework with three key components: 1) Multi-Directional Cost Map Aggregation (RS-CMA) for rotation-invariant visual cues, 2) Efficient Cost Map Fusion (RS-Fusion) transformer for spatial-semantic modeling, and 3) Remote Sensing Knowledge Transfer (RS-Transfer) module for domain adaptation.

Result: RSKT-Seg outperforms strong OVS baselines by +3.8 mIoU and +5.9 mACC on the newly established OVRSISBench benchmark, while achieving 2x faster inference through efficient aggregation.
...
TL;DR: SMARLI is a novel framework for layout-to-image generation that integrates structured masking in attention computation to prevent feature entanglement and enable precise layout control in autoregressive models, achieving superior layout accuracy while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models face challenges in layout-conditioned generation due to sparse layout conditions and risk of feature entanglement between different regions and their descriptions.
...
Result: Achieves seamless integration of layout tokens with text and image tokens without compromising generation quality, enabling superior layout-aware control.

Conclusion: SMARLI effectively integrates spatial layout constraints into AR-based image generation while maintaining structural simplicity and generation efficiency of autoregressive models.
...
TL;DR: A modular computer vision pipeline using state-of-the-art models for automated animal behavior analysis in group housing, achieving 94.2% accuracy on pig behavior recognition with significant improvement over existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional manual observation methods for animal behavior analysis are time-consuming, subjective, and not scalable, creating a need for automated solutions in agricultural settings for welfare and productivity monitoring.
...
TL;DR: AvatarSync is an autoregressive framework that generates realistic talking-head animations from a single reference image using text/audio input, addressing flicker and identity drift issues through a two-stage phoneme-based approach with superior visual fidelity and efficiency.
...
Conclusion: AvatarSync provides a scalable and controllable solution for realistic talking-head animation that effectively addresses the limitations of previous approaches through its innovative phoneme-based autoregressive design.
...
TL;DR: A cross-population data augmentation framework that enables fetal pose estimation models to generalize to younger gestational ages using only older GA annotated data, improving motion tracking reliability across gestation.


<details>
  <summary>Details</summary>
Motivation: Fetal motion quantification is challenging at early gestational ages due to anatomical changes and lack of annotated early GA data, while current methods trained on third-trimester data fail to generalize to younger fetuses.

Method: Developed a fetal-specific augmentation strategy that simulates early GA intrauterine environment and fetal positioning, using cross-population data augmentation from older to younger GA cohorts.

Result: Cross-population augmentation yields reduced variability and significant improvements in pose estimation across both older GA and challenging early GA cases.

Conclusion: The framework enables more reliable pose estimation across gestation, potentially facilitating early clinical detection and intervention in 4D fetal imaging settings.
...
TL;DR: ImplMORe is a deep learning method using implicit surface representations for high-resolution multi-organ reconstruction from 3D medical images, outperforming traditional discrete approaches.


<details>
  <summary>Details</summary>
Motivation: Fine-grained surface reconstruction from 3D medical imaging provides better diagnostic support and surgical planning, but current methods are limited by resolution constraints and computational demands. Implicit representations offer compact solutions but aren't directly applicable to medical images.

Method: End-to-end deep learning approach using 3D CNN encoder to incorporate local features, multi-scale interpolation to learn features in continuous domain using occupancy functions, applied to single and multiple organ reconstructions.

Result: Outperforms discrete explicit representation-based surface reconstruction approaches, providing fine-grained surface details at resolutions higher than the input image using the totalsegmentator dataset.

Conclusion: ImplMORe successfully adapts implicit surface representations to medical imaging, enabling higher-resolution organ reconstruction with better detail than traditional discrete methods.
...
TL;DR: U-Mamba2 is a novel neural network combining Mamba2 state space models with U-Net for efficient multi-anatomy CBCT segmentation, achieving top performance in ToothFairy3 challenge with mean Dice scores of 0.792-0.852.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of dental anatomies in CBCT scans is critical for clinical applications like diagnosis and surgical planning, but remains time-consuming and challenging with existing methods.

Method: Integrates Mamba2 state space models into U-Net architecture, adds interactive click prompts with cross-attention blocks, uses self-supervised pre-training, and incorporates dental domain knowledge for structural constraints.
...
Conclusion: U-Mamba2 provides an efficient and effective solution for dental CBCT segmentation by combining modern state space models with domain-specific knowledge, achieving state-of-the-art performance while maintaining computational efficiency.
...
TL;DR: A novel trajectory-controllable unfolding framework for CASSI hyperspectral imaging that enforces smooth optimization paths from noisy estimates to high-quality reconstructions, achieving better quality and efficiency than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing deep unfolding networks for CASSI reconstruction suffer from uncontrollable reconstruction trajectories, leading to abrupt quality jumps and non-gradual refinement across stages, which limits reconstruction performance.

Method: Proposes a trajectory-controllable unfolding framework inspired by diffusion trajectories and flow matching. Uses an efficient spatial-spectral Transformer for hyperspectral reconstruction and a frequency-domain fusion module to ensure feature consistency.

Result: Experiments on simulation and real data demonstrate superior reconstruction quality and efficiency compared to prior state-of-the-art approaches.

Conclusion: The proposed trajectory-controllable unfolding framework effectively addresses the limitations of existing methods by enforcing smooth, continuous optimization paths, leading to improved hyperspectral image reconstruction in CASSI systems.
...
TL;DR: TetHeart is an end-to-end framework for reconstructing 4D heart motion from both complete and sparse CMR scans, using deep deformable tetrahedra representation and slice-adaptive feature assembly for accurate cardiac motion recovery in clinical settings.


<details>
  <summary>Details</summary>
Motivation: Existing cardiac motion reconstruction methods require complete CMR stacks, limiting their utility in intra-procedural scenarios where only sparse slice observations are available. There's a need for a unified approach that works with both full-stack acquisitions and sparse-slice observations.

Method: TetHeart uses deep deformable tetrahedra (explicit-implicit hybrid representation) to capture shape and motion. It features: (1) attentive slice-adaptive 2D-3D feature assembly for dynamic integration from arbitrary slices, (2) distillation strategy from full to sparse settings, and (3) two-stage weakly supervised motion learning requiring only ED/ES annotations.

Result: TetHeart achieves state-of-the-art accuracy and strong generalization across both pre- and intra-procedural settings. It was trained on three large public datasets and externally evaluated zero-shot on additional private interventional and public CMR datasets.

Conclusion: TetHeart is the first unified framework for 4D multi-structure heart mesh recovery from both complete and sparse CMR observations, demonstrating robust performance in clinical scenarios with extreme sparsity while requiring minimal annotations.
...
TL;DR: FS-SAM2 leverages SAM2's video segmentation capabilities for few-shot semantic segmentation by applying Low-Rank Adaptation (LoRA) to handle diverse images, achieving state-of-the-art results with high computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot segmentation methods require extensive training on large datasets. SAM2 offers powerful zero-shot segmentation capabilities but is designed for temporally connected video frames, not diverse static images.

Method: Repurposes SAM2's video modules for few-shot segmentation using Low-Rank Adaptation (LoRA) to adapt the model to diverse standard datasets while maintaining SAM2's core segmentation performance.
...
Conclusion: FS-SAM2 effectively adapts SAM2 for few-shot segmentation with minimal parameter training, demonstrating that foundational models can be efficiently repurposed for specialized tasks with proper adaptation techniques.
...
Method: RailSafeNet fuses semantic segmentation (SegFormer B3 model), object detection (YOLOv8), and a rule-based Distance Assessor that compares projected distances with the standard 1435mm rail gauge to classify risk levels from monocular video.

Result: On the RailSem19 dataset, SegFormer B3 achieved 65% IoU for rail detection, while fine-tuned YOLOv8 attained 75.6% mAP at IoU threshold 0.50 for object detection, enabling accurate real-time risk assessment.

Conclusion: RailSafeNet provides accurate, annotation-light scene understanding that can warn tram drivers before dangerous situations escalate, offering a practical solution for improving tram safety in urban environments.
...
TL;DR: Reflection-V enhances visual reasoning models by improving visual reflection through vision-centered data construction and visual attention rewards, achieving better performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models show limited visual reflection capability - their attention to visual information decreases with longer responses, hindering effective visual reasoning.

Method: Two-stage approach: 1) Construct vision-centered reasoning data using agent interaction between VLMs and reasoning LLMs for cold-start learning; 2) Use visual attention-based reward model during RL to encourage visual-based reasoning.
...
Conclusion: Reflection-V effectively enhances visual reflection capabilities in VRMs through systematic data construction and reward design, enabling better visual reasoning performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing MDD detection methods using sMRI and deep learning are limited by voxel-level features or handcrafted regional representations from predefined atlases, which cannot capture complex brain patterns effectively.

Method: Developed a unified pipeline using Vision Transformers to extract 3D region embeddings from sMRI data and Graph Neural Networks for classification. Explored two region definition strategies: atlas-based (predefined brain atlases) and cube-based (ViTs trained on 3D patches). Used cosine similarity graphs to model interregional relationships for GNN classification.
...
Conclusion: The proposed unified pipeline effectively detects MDD using sMRI data, with atlas-based methods demonstrating superior performance, highlighting the importance of domain-specific anatomical priors for accurate MDD detection.
...
TL;DR: OpenHOUSE system for hierarchical streaming video understanding that combines temporal action localization with free-form description generation, using LLMs to enrich datasets and achieving near-double performance on boundary detection.


<details>
  <summary>Details</summary>
Motivation: To address the scarcity of datasets with hierarchical temporal annotations and extend streaming action perception beyond simple classification to include free-form description generation.

Method: Proposes OpenHOUSE system with specialized streaming module for detecting boundaries between adjacent actions, using LLMs to group atomic actions into higher-level events and enrich existing datasets.

Result: The system nearly doubles the performance of direct extensions of existing methods in detecting boundaries between closely adjacent actions.

Conclusion: OpenHOUSE represents a key step toward integrating powerful generative models in streaming action perception, enabling hierarchical online video understanding with free-form descriptions.
...
TL;DR: XR-0 is a multi-anatomy X-ray foundation model that achieves state-of-the-art performance across diverse clinical tasks by leveraging self-supervised learning on 1.15M diverse X-ray images.
...
TL;DR: LoRA fine-tuning for vision models achieves comparable or better performance than full fine-tuning for RILI diagnosis from CT scans, while reducing computational costs and training time.


<details>
  <summary>Details</summary>
Motivation: To investigate the efficacy of Low-Rank Adaptation (LoRA) for fine-tuning large vision models to diagnose Radiation-Induced Lung Injury from CT scans, comparing it with traditional methods to evaluate robustness and efficiency.

Method: Used LoRA to fine-tune DinoV2 and SwinV2 models on X-ray CT scans, comparing with full fine-tuning and inference-only methods. Tested with cropped images of different sizes (50mm³ and 75mm³) centered at treatment isocenter, and explored adaptation techniques for 2D models on 3D data.
...
Conclusion: LoRA is an efficient and effective alternative to traditional fine-tuning for adapting large vision models to medical imaging tasks like RILI diagnosis, offering performance benefits with reduced computational requirements.
...
TL;DR: HoloGarment enables 360° novel view synthesis of real-world garments from 1-3 images or video by combining real video data with synthetic 3D data to create a shared garment embedding space, handling occlusions, poses, and deformations.


<details>
  <summary>Details</summary>
Motivation: Prior methods rely on synthetic 3D training data of static, unoccluded objects, which poorly generalizes to real-world clothing with complex deformations, occlusions, and pose variations.
...
Conclusion: HoloGarment effectively bridges the domain gap between synthetic and real data, enabling high-quality 360° view synthesis of real-world garments from limited input images or video.
...
<details>
  <summary>Details</summary>
Motivation: High labeling costs for video camera trap data create a bottleneck in animal behavior research, requiring data-efficient learning approaches to scale up datasets for ecology, cognition, and conservation studies.

Method: Utilized pretrained V-JEPA model with domain-adaptive pretraining (DAP) - continuing pretraining with in-domain primate behavior data without requiring labeled samples.

Result: Outperformed published state-of-the-art action recognition models by 6.1% accuracy on PanAf dataset and 6.3% mAP on ChimpACT dataset for great ape behavior recognition.

Conclusion: Domain-adaptive pretraining shows great potential for improving animal behavior recognition as it doesn't require labeled samples, making it highly scalable for conservation and research applications.
...
<details>
  <summary>Details</summary>
Motivation: To systematically organize and compare existing methods for 3D human pose estimation and mesh recovery from LiDAR data, addressing the need for standardized evaluation and clear taxonomy in this emerging field.

Method: The authors conduct a comprehensive review, create a structured taxonomy to classify methods, perform quantitative dataset comparisons, compile unified evaluation metrics, and establish benchmark tables for fair comparisons.

Result: The paper provides a systematic framework for understanding LiDAR-based 3D human analysis, including standardized metrics, dataset characteristics, and performance benchmarks that enable objective comparison of different approaches.

Conclusion: This review establishes foundational standards for LiDAR-based 3D human understanding, identifies current limitations, and outlines critical research directions to advance the field, with ongoing updates via an accompanying webpage.
...
<details>
  <summary>Details</summary>
Motivation: Existing datasets lack the dynamic complexity, multi-domain diversity, and spatial-temporal annotations needed for comprehensive 4D world modeling tasks like geometric reconstruction, future prediction, and camera-control video generation.

Method: Introduces OmniWorld dataset consisting of newly collected OmniWorld-Game data and curated public datasets across diverse domains, providing richer modality coverage, larger scale, and more realistic dynamic interactions than existing synthetic datasets.
...
Conclusion: OmniWorld serves as a powerful resource for training and evaluation, acting as a catalyst for accelerating development of general-purpose 4D world models and advancing machines' holistic understanding of the physical world.
...
TL;DR: LazyDrag introduces explicit correspondence mapping for drag-based image editing, eliminating the need for implicit point matching and test-time optimization, enabling stable inversion and better generative capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing drag-based editing methods rely on implicit point matching via attention, which compromises inversion strength and requires costly test-time optimization, limiting diffusion models' generative capabilities for high-fidelity editing.

Method: Generates explicit correspondence maps from user drag inputs to boost attention control, enabling stable full-strength inversion without test-time optimization, and supports multi-round workflows with simultaneous move and scale operations.
...
Conclusion: LazyDrag establishes new state-of-the-art performance and paves a new way for editing paradigms by unifying precise geometric control with text guidance, eliminating previous limitations in drag-based editing.
...
<details>
  <summary>Details</summary>
Motivation: Animated characters pose challenges for traditional recognition systems due to extreme diversity in appearance, motion, and deformation, requiring new approaches for character-centric understanding.
