...
Motivation: Traditional segmentation models struggle with generalization across diverse datasets, creating a need for accurate algorithms that can effectively segment anatomical structures for preoperative planning beyond breast cancer applications.

Method: Processed 120 retrospective breast MRIs through three phases: anonymization and manual segmentation, co-registration and segmentation of breast structures, and 3D visualization using ITK-SNAP. Used human-in-the-loop approach with U-Mamba model to refine segmentations and improve generalization.
...
<details>
  <summary>Details</summary>
Motivation: Manual analysis of thousands of TRISO particles in irradiated fuel compacts is cumbersome and subjective. Automated methods are needed to accelerate data analysis and reduce subjectivity in identifying irradiation-induced morphological changes like kernel swelling and buffer densification.

Method: Used convolutional neural networks (CNNs) including RU-Net (developed in this study), U-Net, ResNet, and Attention U-Net to automatically segment cross-sectional images of microscopic TRISO layers. Created a dataset with over 2,000 microscopic images and corresponding annotations.

Result: The RU-Net model performed best in terms of Intersection over Union (IoU) metric. CNN models successfully expedited analysis of TRISO particle cross sections.

Conclusion: CNN-based segmentation significantly reduces manual labor and improves objectivity in analyzing TRISO particle irradiation effects, with RU-Net showing superior performance for this specific application.
...
Method: Hierarchical pipeline using autoencoder for early warning, comparing vegetation index features with random forest vs raw whole-image deep learning with vision transformer for status estimation of fresh weight, dry mass, and tissue nutrients.

Result: 73% net detection of severely nutrient-depleted samples 9 days after transplanting at lower energy than wasted nitrogen; ViT outperformed RF on phosphorus and calcium estimation (R² 0.61 vs 0.58, 0.48 vs 0.35) but with higher energy cost.
...
<details>
  <summary>Details</summary>
Motivation: Humor understanding is a core aspect of social intelligence but remains a major challenge for Large Multimodal Models (LMMs), highlighting the need for better evaluation frameworks.

Method: Created PixelHumor benchmark with 2,800 annotated multi-panel comics to test LMMs' abilities in interpreting multimodal humor and recognizing narrative sequences through panel sequencing tasks.

Result: State-of-the-art LMMs achieve only 61% accuracy in panel sequencing, far below human performance, revealing critical limitations in visual-textual integration for narrative and humor understanding.

Conclusion: PixelHumor provides a rigorous evaluation framework that exposes significant gaps in current LMMs' multimodal reasoning capabilities and aims to drive development of more socially aware AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Current HOI methods work offline using full sequence information, but real-world applications require online processing with only current and historical data available at each time step.

Method: OnlineHOI framework based on Mamba architecture with memory mechanism to handle streaming data and efficiently integrate historical information for real-time processing.
...
Conclusion: The proposed online approach significantly outperforms offline methods in real-time scenarios, demonstrating the importance of specialized architectures for streaming HOI data.
...
<details>
  <summary>Details</summary>
Motivation: To examine the dual nature of deep learning technology - while it offers revolutionary advancements, its application in deepfake creation presents significant societal risks that need to be understood and addressed.

Method: Analysis of deepfake technology's capabilities and its real-world applications in creating indistinguishable counterfeit photos and videos, focusing on face-swapping techniques and their misuse.

Result: Deepfake technology can produce highly realistic fake media that undermines facial recognition systems, compromises personal privacy, damages reputations of public figures, and threatens national security through political manipulation and election interference.

Conclusion: The improper application of deepfake technology poses severe risks to society, including privacy violations, reputational damage, political manipulation, and threats to national security, requiring urgent attention and countermeasures.
...
<details>
  <summary>Details</summary>
Motivation: Existing studies on neural networks' simplicity bias focus on small models or synthetic tasks, leaving a gap in understanding SB in large models like CLIP and its relevance to real-world image classification tasks.

Method: The authors theoretically analyze limitations of existing complexity measures, propose a frequency-aware SB measure, validate it on CLIP models using SB-modulation methods, and examine SB-performance relationships across various classification tasks including zero-shot and fine-tuning settings.

Result: The proposed frequency-aware measure proves more informative and consistent than previous measures. Experiments reveal that stronger simplicity bias correlates better with out-of-distribution generalization than adversarial robustness, showing task-dependent benefits.

Conclusion: The study demonstrates the importance of aligning a model's inductive biases with target task characteristics, and provides a better tool for measuring simplicity bias in large vision-language models like CLIP.
...
<details>
  <summary>Details</summary>
Motivation: Current AI systems for dermoscopy often ignore crucial patient metadata (age, sex, site) and lack physical scale calibration needed for geometric analysis of lesions, limiting their diagnostic accuracy.

Method: The framework uses U-Nets for lesion and ruler segmentation, regresses pixels-per-millimeter from ruler masks, computes real-scale geometric descriptors, and employs a spectral GNN with EfficientNet-B3 features and metadata/geometry-based edges for semi-supervised classification.

Result: Achieved AUC of 0.9812 for classification, with sparser graphs (25% edges) maintaining AUC 0.9788 vs 0.9440 for image-only baseline. Ruler segmentation Dice 0.904, lesion segmentation Dice 0.908, scale regression MAE 1.5px.

Conclusion: Integrating calibrated scale, lesion geometry, and metadata in population graphs provides substantial improvements over image-only approaches, with sparse graphs enabling efficient deployment while maintaining high accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Current TIMT approaches focus only on translating all text in images without providing bounding boxes and cover limited scenarios, missing practical needs for fine-grained, layout-aware translation.

Method: Extends TIMT to PATIMT with two sub-tasks: region-specific translation and full-image translation with grounding. Uses Adaptive Image OCR Refinement Pipeline to select appropriate OCR tools based on scenario and refine text-rich image results.
...
Conclusion: The proposed PATIMT framework and PATIMTBench benchmark effectively address the limitations of traditional TIMT by enabling fine-grained, layout-preserving translation with reliable evaluation metrics.
...
Method: Uses WakeGAN for style transfer, instance-level feature similarity filtering to prioritize target-like samples, Feature-Confidence Memory Bank with KNN fusion for pseudo-label calibration, and region-mixed training.
...
<details>
  <summary>Details</summary>
Motivation: No single data source provides seamless spatiotemporal air temperature data - weather stations offer continuous monitoring but limited spatial coverage, while satellites provide broad coverage but lack temporal continuity, especially when obscured by clouds.

Method: Two-stage neural network approach: 1) Reconstructs GOES-16 surface temperature data through a neural network encoded with annual temperature cycle, using linear amplification of ERA5 data and convolutional layers for spatiotemporal variations; 2) Transforms surface temperature to air temperature using another neural network that leverages relationships with Earth surface properties, enhanced with deep ensemble learning for uncertainty estimation.
...
Conclusion: The approach successfully streamlines surface temperature reconstruction and air temperature prediction, enabling seamless high-resolution monitoring that can be extended to other satellite sources for comprehensive temperature mapping.
...
Method: Used DINOv2 (general-purpose) and MegaDescriptor (domain-specific) as backbone models. Applied K-Nearest Neighbor classifier with robust thresholding for identification. Tested triplet-learning projection head for metric learning improvements.

Result: Triplet-learning improved specialized MegaDescriptor by 0.13 points but only 0.03 for general-purpose DINOv2 on averaged BAKS and BAUS metrics. General-purpose manifold proved difficult to reshape for fine-grained tasks with stagnant validation loss.

Conclusion: General-purpose features have critical limitations for specialized, limited-data re-ID tasks. Domain-specific pre-training is crucial, as general-purpose manifolds are resistant to metric learning refinement for fine-grained tasks.
...
<details>
  <summary>Details</summary>
Motivation: Orthomosaic imagery from UAVs has stitching artifacts and heavy preprocessing that limit field deployment suitability, while raw UAV imagery may offer better performance for ecological monitoring applications.

Method: Used state-of-the-art detectors and keypoint models to compare detection performance between orthomosaic and raw UAV imagery, and evaluated crown-center annotations versus bounding-box centroids for localization accuracy.

Result: Raw imagery yields superior performance in deployment-relevant scenarios, while orthomosaics are better for cross-domain generalization. Crown-center annotations improve localization accuracy and provide precise tree positions.

Conclusion: Raw UAV imagery with crown-center annotations offers practical advantages for biodiversity and conservation monitoring, providing guidance for field deployment of UAV-based ecological monitoring systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for articulated objects assume simplified kinematic structures or require joint annotations, but mechanical assemblies like gears have motion arising from geometric coupling through meshing teeth or aligned axes, making it difficult to reason about relational motion from geometry alone.

Method: Introduce MechBench dataset with 693 synthetic gear assemblies and ground-truth motion trajectories, then propose DYNAMO - a dependency-aware neural model that predicts per-part SE(3) motion trajectories directly from segmented CAD point clouds.
...
Conclusion: MechBench and DYNAMO establish a novel systematic framework for data-driven learning of coupled mechanical motion in CAD assemblies, addressing the gap in understanding motion from geometric coupling rather than predefined joints.
...
Method: Built on YOLOv11n, Cott-ADNet enhances spatial representation through improved convolutional designs, adds a NeLU-enhanced Global Attention Mechanism for weak feature capture, and uses Dilated Receptive Field SPPF for multi-scale context modeling. Trained on 4,966 labeled images.

Result: Achieves 91.5% Precision, 89.8% Recall, 93.3% mAP50, 71.3% mAP, and 90.6% F1-Score with only 7.5 GFLOPs. Maintains stable performance under multi-scale and rotational variations.
...
<details>
  <summary>Details</summary>
Motivation: Existing surveys focus on network architectures but ignore practical challenges like large data volumes, diverse scenes, varying density, and multi-modal data in real applications.

Method: Review deep learning approaches and datasets for key point cloud processing tasks (scene completion, registration, semantic segmentation, modeling) across urban and environmental applications.

Result: Identifies gaps between current deep learning methods and practical implementation requirements, highlighting challenges in transitioning research to real-world practices.

Conclusion: Both algorithmic improvements and practical considerations are needed to bridge the gap between point cloud processing research and real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing glaucoma prognosis methods are limited by fixed-length inputs and struggle with small dataset sizes, requiring a more flexible and data-efficient approach.
...
Conclusion: The proposed framework significantly enhances model performance while maintaining compact parameter size, enabling better glaucoma prognosis with limited data.
...
<details>
  <summary>Details</summary>
Motivation: Current image tokenizers prioritize reconstruction tasks but don't consider generation errors during sampling, creating a significant discrepancy between reconstruction and generation distributions that affects generative model performance.

Method: Proposes a two-phase training: main-training with latent perturbation strategy to simulate sampling noises and improve robustness, and post-training that optimizes the tokenizer decoder with a well-trained generative model to mitigate distribution differences.

Result: Achieves 1.60 gFID with main training alone and further improves to 1.36 gFID with additional post-training using a ~400M generator. The method also works effectively with off-the-shelf discrete/continuous tokenizers and various generator types.

Conclusion: The proposed tokenizer training scheme successfully bridges the gap between reconstruction and generation distributions, significantly improving generation quality, convergence speed, and providing a better evaluation metric (pFID) that correlates tokenizer performance with generation quality.
...
<details>
  <summary>Details</summary>
Motivation: mmWave radars are robust sensors but suffer from poor angular resolution, especially in inexpensive single-chip versions. Existing learning approaches lack standardized foundational models and large datasets, requiring practitioners to train task-specific models from scratch with limited data.

Method: Collected the largest raw radar dataset (1M samples, 29 hours) and trained a Generalizable Radar Transformer (GRT) foundation model that processes raw radar data to predict 3D occupancy and semantic segmentation.

Result: GRT achieves quality typically only possible with higher-resolution sensors, generalizes across diverse settings, shows 20% logarithmic data scaling per 10x data increase, and raw data outperforms lossy representations equivalent to 10x more training data.

Conclusion: The approach demonstrates the effectiveness of foundational models for radar processing, with estimates suggesting ≈100M samples (3000 hours) would fully exploit GRT's potential, establishing a path toward standardized radar perception models.
...
<details>
  <summary>Details</summary>
Motivation: While VLMs excel in multimodal tasks, their performance under noisy conditions remains poorly understood, necessitating a systematic evaluation framework to assess robustness.

Method: Used controlled perturbations (lighting variation, motion blur, compression artifacts) and evaluated with both lexical metrics (BLEU, METEOR, ROUGE, CIDEr) and neural-based similarity measures using sentence embeddings across diverse datasets.

Result: Key findings: (1) ground-truth caption quality affects performance, (2) larger models like LLaVA excel in semantics but don't universally outperform smaller models, (3) JPEG compression and motion blur significantly degrade performance across all models.
...
<details>
  <summary>Details</summary>
Motivation: Existing WSSS methods struggle with precise object boundary localization and often focus only on discriminative regions rather than complete object coverage, creating a need for better localization accuracy.

Method: Proposes IG-CAM with three innovations: 1) Instance-Guided Refinement using ground truth masks, 2) Influence Function Integration for robust features, and 3) Multi-Scale Boundary Enhancement for precise boundaries.

Result: Achieves 82.3% mIoU before CRF and 86.6% after CRF on PASCAL VOC 2012, significantly outperforming previous methods with superior boundary accuracy and complete object coverage.

Conclusion: IG-CAM establishes a new benchmark for WSSS, providing a practical solution for scenarios lacking pixel-level annotations while maintaining computational efficiency and strong generalization.
...
<details>
  <summary>Details</summary>
Motivation: Artist-created meshes are crucial for commercial graphics pipelines but existing methods assume clean inputs or use complex multi-stage pipelines, limiting real-world applicability

Method: Reformulate 3D point cloud refinement as 2D inpainting task to leverage powerful generative models, end-to-end approach that refines input and produces high-quality meshes
...
Result: The approach provides robust diameter estimation by mitigating the effects of curvature, occlusion, and image noise in agricultural settings.

Conclusion: This computer vision method offers a scalable and reliable solution for high-throughput stalk diameter phenotyping to support crop breeding and agronomic research.
...
Method: Proposes feature disentanglement module to extract semantically specific features for multi-label settings, aligns feature distributions using predefined Neural Collapse structure, and designs regularization losses for compact clustering in latent space.

Result: Experiments on four benchmark datasets across eight diverse settings demonstrate superior performance over existing methods.

Conclusion: The approach effectively addresses multi-label federated learning challenges by leveraging Neural Collapse theory and feature disentanglement, validating its effectiveness in handling heterogeneous data distributions.
...
<details>
  <summary>Details</summary>
Motivation: Face forgery detection suffers from ecological invalidity in training data - current datasets don't capture the diverse intents, iterative processes, and complex text-image interactions that occur in real-world social media forgery scenarios.

Method: A multi-agent framework where LLM-powered agents with profile and memory modules simulate human forgery creation processes in a social environment, generating samples with nuanced text-image consistency labels. Uses Adaptive Rejection Sampling (ARS) to ensure data quality and diversity.

Result: Extensive experiments show that data generated by this simulation-driven approach brings significant performance gains to detectors of multiple architectures, demonstrating the framework's effectiveness.

Conclusion: The proposed Agent4FaceForgery framework successfully addresses the ecological validity problem in face forgery detection by simulating realistic forgery creation processes and generating high-quality training data that improves real-world detection performance.
...
<details>
  <summary>Details</summary>
Motivation: Transformer architectures don't explicitly model relational structures inherent in HOI detection, while GNNs are inherently better suited for modeling relationships between human-object pairs.
...
Result: Achieves state-of-the-art performance on HICO-DET and V-COCO benchmarks, with significant performance gains when integrated with advanced object detectors, while maintaining balance between rare and non-rare classes.
...
<details>
  <summary>Details</summary>
Motivation: Existing lighting estimation methods struggle with restoring detailed illumination textures while maintaining fast running speed and texture fidelity.

Method: Combines VQVAE for discrete feature extraction (avoiding posterior collapse) and ViT for capturing global context and dependencies, formulating lighting estimation as a multiclass classification task.
...
TL;DR: Proposes an adaptive sampling scheduler for consistency distillation that dynamically selects target timesteps, optimizes alternating sampling, and uses smoothing techniques to improve flexibility and generation quality.
...
Method: Develops an adaptive sampling scheduler with three strategies: dynamic target timestep selection based on computed importance, optimized alternating sampling along solution trajectory, and smoothing clipping with color balancing for stable high-quality generation.

Result: Experimental validation across various consistency distillation methods showed significant improvements in generative performance and demonstrated strong adaptability of the proposed method.

Conclusion: The adaptive sampling scheduler effectively overcomes limitations of existing approaches, providing flexibility and enhanced generation quality across different consistency distillation frameworks.
...
Method: Reverse-engineering localization models to identify critical keypoints, then strategically removing regions containing Top-K keypoints to disrupt LiDAR-based localization. Validated on three point-cloud registration models and Autoware platform, with physical-world testing using near-infrared absorptive materials.

Result: Removing critical keypoint regions significantly degrades registration accuracy in all three models. Hiding few critical regions causes noticeable localization drift in Autoware. Physical-world attacks successfully replicated the effects observed in simulation.

Conclusion: The proposed DisorientLiDAR framework demonstrates effective adversarial attacks on LiDAR localization, showing veracity and generality across different models and real-world scenarios, highlighting serious security vulnerabilities in autonomous driving systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing reflection removal methods work only in image domain and ignore spectral properties of reflected light, limiting their ability to effectively discern and remove reflections from captured images.

Method: Proposes Spectral Codebook to reconstruct optical spectrum of reflection image, uses spectral prior refinement modules for spatial pixel redistribution and spectral difference enhancement, and Spectrum-Aware Transformer for joint recovery in spectral and pixel domains.
...
Conclusion: Spectral learning perspective effectively addresses reflection removal by leveraging wavelength differences in spectrum, providing a more accurate approach than traditional image-domain methods.
...
<details>
  <summary>Details</summary>
Motivation: Maps are essential for autonomous driving, and their representation and production processes have evolved significantly. Understanding this evolution helps address technical challenges and integrate new map innovations into autonomous driving systems.

Method: The authors categorize map evolution into three stages (HD maps, Lite maps, Implicit maps) and provide comprehensive reviews of production workflows, technical challenges, and academic solutions for each stage.

Result: The paper systematically analyzes the progression of map technologies, highlighting challenges and solutions at each evolutionary stage, and explores how cutting-edge map representations can be integrated into end-to-end autonomous driving frameworks.

Conclusion: The evolution from HD to Lite to Implicit maps represents significant advancements in autonomous driving mapping technology, with each stage addressing previous limitations and enabling more efficient integration into complete autonomous driving systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing ARD methods suffer from degraded performance on clean examples due to divergent optimization objectives in dual-teacher frameworks and performance deterioration of robust teachers from iterative adversarial examples.
...
Result: Achieves average 3.53% improvement in adversarial defense rates across various attacks and 5.87% increase in clean sample accuracy on CIFAR-10, CIFAR-100, and Tiny-ImageNet datasets.

Conclusion: CIARD establishes a new benchmark for balancing model robustness and generalization, effectively addressing the trade-off between adversarial defense and clean accuracy in knowledge distillation.
...
Method: Two-stage approach: 1) Construct SAMM dataset using state-of-the-art image manipulations followed by contextually-plausible textual narratives; 2) Propose RamDG framework that retrieves external contextual evidence and processes through image forgery grounding and deep manipulation detection modules.
...
<details>
  <summary>Details</summary>
Motivation: Existing cross-view geo-localization methods neglect spatial and semantic information while extracting features through feature map segmentation, failing to address significant appearance variations across different viewpoints and difficulty in extracting discriminative features.

Method: MFAF method with Multi-Frequency Branch-wise Block (MFB) to capture low-frequency structural features and high-frequency edge details across multiple scales, and Frequency-aware Spatial Attention (FSA) module to adaptively focus on key regions of frequency features.
...
Conclusion: The proposed MFAF method effectively addresses viewpoint variations and background noise in cross-view geo-localization by leveraging multi-scale frequency analysis and attention mechanisms, achieving state-of-the-art performance.
...
<details>
  <summary>Details</summary>
Motivation: Underwater computer vision faces challenges like light attenuation, turbidity, and class imbalance, while AUVs have limited computational resources. Existing YOLO benchmarks focus on terrestrial domains, leaving marine performance unclear.

Method: Used two underwater datasets (Coral Disease and Fish Species) with varying training sizes. Trained YOLOv8-s, YOLOv9-s, YOLOv10-s, and YOLOv11-s with identical hyperparameters and evaluated precision, recall, mAP, inference time, and FPS. Used Grad-CAM for visualization.

Result: Accuracy saturated after YOLOv9 across both datasets, while inference speed improved significantly. YOLOv10 provided the best speed-accuracy trade-off for embedded AUV deployment.

Conclusion: Recent YOLO architectural innovations primarily target efficiency rather than accuracy for underwater imagery. The study provides the first controlled comparison and delivers an open benchmark for future marine-vision research.
...
Method: Created StereoCarla dataset using CARLA simulator with diverse camera configurations (baselines, viewpoints, sensor placements) and varied environmental conditions (lighting, weather, road geometries) to enhance training data diversity.

Result: Models trained on StereoCarla outperformed those trained on 11 existing stereo datasets across four standard evaluation benchmarks (KITTI2012, KITTI2015, Middlebury, ETH3D), and showed substantial improvements when integrated into multi-dataset training.

Conclusion: StereoCarla provides a valuable benchmark for developing robust stereo algorithms under realistic and controllable settings, significantly enhancing generalization performance for autonomous vehicle depth perception systems.
...
<details>
  <summary>Details</summary>
Motivation: Early-stage fire scenes (0-15 minutes) suffer from smoke-obscured surveillance visibility, hindering emergency response. Current smoke removal algorithms are limited by lack of large-scale real-world paired datasets.

Method: Created SmokeBench dataset containing precisely aligned image pairs captured under diverse scenes and smoke concentrations, enabling supervised learning and rigorous evaluation.

Result: Comprehensive experiments benchmarking various desmoking methods on the dataset demonstrate its value for advancing robust image desmoking in real-world fire scenarios.

Conclusion: SmokeBench provides a valuable foundation for practical image desmoking research and has been publicly released to support the development of effective smoke removal algorithms for emergency response applications.
...
Conclusion: The proposed cascaded framework successfully unifies fusion and referring image segmentation through joint optimization, demonstrating superior performance in text-guided infrared-visible image fusion with improved semantic alignment.
...
Conclusion: DeFA effectively addresses CZSL challenges through debiased feature augmentation and achieves superior compositional generalization performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing MoE approaches struggle with the asymmetry between visual (spatial completeness) and linguistic (sequential context) processing, causing language experts to lose contextual grounding and rely on parametric knowledge rather than utilizing multimodal information.

Method: Proposes AsyMoE with three expert groups: intra-modality experts for modality-specific processing, hyperbolic inter-modality experts for hierarchical cross-modal interactions, and evidence-priority language experts to suppress parametric biases and maintain contextual grounding.
...
Conclusion: AsyMoE effectively addresses the modality asymmetry problem in LVLMs through specialized expert design, significantly improving performance while reducing computational costs.
...
Method: Introduces two dynamic spatial benchmarks: locally observable maze navigation and match-2 elimination, where each action triggers structural changes requiring continuous cognitive updates. Proposes a subjective experience-based memory mechanism for cross-task experience transfer.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models (VLMs) are vulnerable to jailbreak attacks, and existing methods lack both effectiveness and efficiency. The paper reveals that incorporating weak defense into the attack pipeline can significantly improve jailbreak performance.

Method: Defense2Attack consists of three components: (1) visual optimizer embedding adversarial perturbations with affirmative semantics, (2) textual optimizer using defense-styled prompts, and (3) red-team suffix generator with reinforcement fine-tuning.

Result: The method was evaluated on four VLMs and four safety benchmarks, demonstrating superior jailbreak performance in single attempts, outperforming state-of-the-art attack methods that require multiple tries.

Conclusion: This work provides a new perspective on jailbreaking VLMs by leveraging defensive patterns to guide attack design, showing that weak defense integration can significantly enhance both effectiveness and efficiency of jailbreaks.
...
<details>
  <summary>Details</summary>
Motivation: Existing Gaussian Splatting methods use indiscriminate attribute assignment which causes gradient conflicts from dual supervision, leading to suboptimal reconstruction results and inefficient parameter usage.

Method: Dynamic activation of spherical harmonics or normals under surface reconstruction supervision, adaptive SH order adjustment based on gradient magnitudes, and task-decoupled pruning to remove unnecessary Gaussians without affecting reconstruction quality.

Result: The approach consistently outperforms state-of-the-art methods in both reconstruction quality and efficiency, achieving superior performance with significantly fewer parameters while being model-agnostic and integrable into other frameworks.

Conclusion: The proposed Gaussian management approach effectively mitigates gradient conflicts, improves representation efficiency, and provides a versatile solution that enhances reconstruction performance while reducing model size across various frameworks.
...
<details>
  <summary>Details</summary>
Motivation: To understand and model the receptive fields learned in depthwise-separable deep networks, specifically examining whether they can be represented by separable filtering operations and discrete Gaussian-based models.

Method: Applied clustering to extract 8 master key filters, computed spatial spread measures, modeled filters using difference operators applied to discrete Gaussian kernels, and performed model fitting using spatial variance matching and norm minimization.

Result: The learned filters show good qualitative similarity to idealized models, with spatial offsets close to half a grid unit, and the idealized models demonstrate good predictive properties when replacing learned filters.

Conclusion: Learned filters in depthwise-separable deep networks can be effectively approximated by discrete scale-space filters, supporting the hypothesis that they operate as separable filtering operations.
...
Method: The researchers curated a dataset of human preferences using synthetically generated image pairs, analyzed inter-task correlations between image quality attributes for both humans and LLMs, and created synthetic datasets with controlled variations for each quality attribute.

Result: Humans showed strong inter-task correlations between image quality attributes, while LLMs exhibited much weaker relationships. Humans could easily judge all quality attributes, but LLMs struggled particularly with anatomical accuracy.

Conclusion: There are significant differences in how humans and multimodal LLMs perceive and evaluate image quality, revealing limitations in current LLM-based assessment methods for certain attributes like anatomical accuracy.
...
Method: Reformulates CVOGL as recurrent localization using learnable tokens that iteratively attend to reference features. Incorporates SAM-based knowledge distillation for semantic guidance and Reference Feature Enhancement Module with hierarchical attention to focus on object-relevant regions.
...
Conclusion: Recurrent localization with iterative refinement, combined with segmentation priors and enhanced attention mechanisms, provides a more effective and parameter-efficient solution for cross-view object geo-localization.
...
<details>
  <summary>Details</summary>
Motivation: Traditional TDOM generation methods rely on complex offline photogrammetric pipelines causing delays that prevent real-time applications, and suffer from quality degradation due to inaccurate camera poses, DSM errors, and scene occlusions.

Method: Uses On-the-Fly SfM to compute pose and sparse point cloud for each new image, then integrates and optimizes new Gaussians into previously unseen or coarsely reconstructed regions through 3DGS optimization, combined with orthogonal splatting for immediate rendering after each update.

Result: Initial experiments on multiple benchmarks show A-TDOM can actively render TDOM in near real-time, with 3DGS optimization for each new image completed in seconds while maintaining acceptable rendering quality and geometric accuracy.

Conclusion: A-TDOM successfully addresses the limitations of traditional TDOM generation by providing a near real-time solution that maintains quality and accuracy, enabling practical real-time applications in urban management and related fields.
...
Result: Extensive experiments on three benchmark few-shot datasets demonstrate BATR-FST achieves superior results in both 1-shot and 5-shot scenarios, significantly improving few-shot classification performance via transformers.

Conclusion: BATR-FST effectively addresses Vision Transformers' limitations in few-shot learning by progressively improving token representations and maintaining robust inductive bias through its innovative bi-level refinement approach.
...
TL;DR: A novel Mamba-based deep learning framework that combines multi-phase contrast-enhanced CT scans to accurately subtype pancreatic tumors (PDAC vs PNETs) with 97.4% accuracy and 98.6% AUC.


<details>
  <summary>Details</summary>
Motivation: Current methods fail to effectively utilize contextual information across multiple CECT phases used in radiologists' diagnostic workflows, limiting performance in pancreatic tumor subtyping due to high heterogeneity and variability of these tumors.

Method: Proposes a dual hierarchical contrast-enhanced-aware Mamba module with spatial and temporal sampling sequences to explore intra/inter-phase contrast variations. Includes similarity-guided refinement for temporal modeling, space complementary integrator, and multi-granularity fusion module for cross-scale semantic encoding.

Result: Achieved 97.4% accuracy and 98.6% AUC on an in-house dataset of 270 clinical cases for distinguishing between pancreatic ductal adenocarcinoma (PDAC) and pancreatic neuroendocrine tumors (PNETs).

Conclusion: The proposed framework demonstrates potential as a more accurate and efficient tool for pancreatic tumor subtyping by effectively leveraging multi-phase CECT data through advanced Mamba architecture and specialized modules.
...
TL;DR: A Contextualized Representation Learning Network for HOI detection that integrates affordance-guided reasoning and contextual prompts to better capture complex human-object interactions involving tools and auxiliary entities.


<details>
  <summary>Details</summary>
Motivation: Existing two-stage HOI detection approaches face challenges due to incomplete context modeling, particularly for complex interactions involving tools and auxiliary objects that require understanding functional roles and multivariate relationships.

Method: Expands HOI detection beyond simple human-object pairs to include multivariate relationships through triplet structures <human, tool, object>. Uses affordance-guided reasoning to model functional roles of auxiliary objects and integrates learnable prompts with contextual visual features using attention mechanisms for language-image alignment.

Result: Demonstrates superior performance on both HICO-Det and V-COCO datasets in most scenarios, showing improved capability in identifying tool-dependent interactions like 'filling'.

Conclusion: The proposed contextualized representation learning approach effectively captures complex, context-dependent interactions by integrating affordance reasoning and visual-language alignment, advancing HOI detection beyond traditional human-object pair modeling.
...
Conclusion: DH-Diff effectively addresses the key challenges in synthetic anomaly generation, providing high-fidelity anomaly images and masks that enhance anomaly detection performance in manufacturing inspection applications.
...
TL;DR: SPAM is a deep learning framework that generates accurate and regular superpixels by combining image features with semantic-agnostic segmentation from pretrained models, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Traditional superpixel methods use low-level features while deep learning approaches sacrifice regularity for accuracy, leading to less interpretable segmentations. There's a need for superpixels that are both accurate and regular.

Method: Train a model to extract image features for superpixel generation, then leverage a large-scale pretrained model for semantic-agnostic segmentation at inference to ensure superpixels align with object masks. Can handle prior segmentation and interactive object focusing.
...
Conclusion: SPAM provides a versatile and robust framework for generating accurate yet regular superpixels, making it valuable for various computer vision applications.
...
<details>
  <summary>Details</summary>
Motivation: Traditional 3D asset creation is labor-intensive and requires specialized workflows, creating barriers to entry and slowing down game development pipelines.

Method: The platform integrates multiple neural modules (Part-level 3D Generation, Polygon Generation, Semantic UV, etc.) into a unified end-to-end system that transforms concept images or text into complete 3D models with optimized geometry and PBR textures.
...
Conclusion: Hunyuan3D Studio represents a significant advancement in AI-assisted workflows for game development, providing a seamless bridge from creative intent to technical asset production.
...
TL;DR: SAGA introduces selective adaptive gating to improve linear attention by addressing feature redundancy and low-rank constraints, achieving better computational efficiency and performance compared to standard linear attention methods.


<details>
  <summary>Details</summary>
Motivation: Transformer architectures have quadratic complexity with softmax attention, making them inefficient for high-resolution images. Linear attention reduces complexity but suffers from uniform compression of KV information, leading to feature redundancy and performance gaps.

Method: Proposes SAGA with input-adaptive learnable gates to selectively modulate information aggregation into KV feature maps, enhancing semantic diversity. Uses efficient Hadamard-product decomposition for gate computation with no additional memory overhead.
...
TL;DR: Continual pretraining of vision encoders (MI2 and RAD-DINO) on 3.5M chest x-rays shows that MI2 excels at finding-related tasks while RAD-DINO is better for tube-related tasks. Structured supervision improves performance, and even 30k in-domain samples can surpass open-weights foundation models.


<details>
  <summary>Details</summary>
Motivation: Medical imaging foundation models are limited by smaller datasets compared to general vision encoders like CLIP and DINOv2. The study aims to understand how data scale and pretraining paradigms affect performance in medical imaging, particularly for diverse tasks beyond radiology findings.

Method: Systematic continual pretraining of two vision encoders (MI2 representing CLIP paradigm and RAD-DINO representing DINOv2 paradigm) on up to 3.5M chest x-rays from a single institution. Evaluation covers classification (findings, lines/tubes), segmentation (lines/tubes), and radiology report generation, with constant compute and evaluation protocols.

Result: MI2 scales better for finding-related tasks, RAD-DINO performs stronger on tube-related tasks. Continual pretraining with both reports and structured labels using UniCL improves performance. Some tasks require only 30k in-domain samples to surpass open-weights foundation models.

Conclusion: Center-specific continual pretraining enables medical institutions to achieve significant performance gains using in-domain data. The approach demonstrates the value of structured supervision at scale and shows different encoder paradigms excel at different medical imaging tasks.
...
<details>
  <summary>Details</summary>
Motivation: Evaluating NeRF-generated outputs is challenging due to unique artifacts, and no single metric performs well across all datasets. Combining different perceptual metrics can overcome individual limitations.

Method: Experiment with two normalization strategies for DISTS and VMAF metrics, and two fusion strategies to evaluate correlation with subjective quality scores. Test on Synthetic and Outdoor datasets across three configurations.

Result: The fusion metrics show improved correlation with subjective scores compared to individual metrics, demonstrating robustness and generalizability across different datasets.

Conclusion: Combining DISTS and VMAF metrics through appropriate fusion strategies provides a more reliable evaluation method for NeRF-generated content quality assessment.
...
<details>
  <summary>Details</summary>
Motivation: Data collection is challenging for rare medical conditions and costly tasks. The study aims to address data scarcity in canine musculoskeletal diagnosis by leveraging LLMs' text generation capabilities to create synthetic visual training data.

Method: Developed a mapping system that segments visual documentations into 200+ labeled regions representing muscles/joints. Used guided decoding, chain-of-thought reasoning, and few-shot prompting with LLMs to generate synthetic visual documentations for patellar luxation and other diagnoses.
...
Conclusion: LLM-generated synthetic data shows strong potential for addressing data scarcity, particularly valuable for rare diseases. The methodology, though medical-domain specific, offers adaptable insights and techniques for other fields facing similar data challenges.
...
TL;DR: CCS is a label-free metric for evaluating object detectors using test-time data augmentation and spatial consistency measurements, achieving over 90% congruence with traditional metrics.


<details>
  <summary>Details</summary>
Motivation: Ground-truth annotations are rarely available in real-world deployment, making continuous monitoring and comparison of object detectors challenging.

Method: Applies test-time data augmentation to images, collects predicted bounding boxes across augmented views, computes overlaps using Intersection over Union, normalizes maximum overlaps, and averages across augmentation pairs to measure spatial consistency.
...
TL;DR: PENet is a few-shot 3D point cloud segmentation framework that uses diffusion model features to expand prototype capacity and address intra-class diversity and inter-set inconsistency challenges.
...
Conclusion: The framework successfully leverages diffusion model capabilities to enhance prototype representation and alignment, achieving superior performance in few-shot 3D point cloud semantic segmentation.
...
TL;DR: Lego-Edit is a novel instruction-based image editing framework that uses MLLM to organize model-level editing tools, achieving SOTA performance through a toolkit design and progressive reinforcement learning.
...
Method: Lego-Edit incorporates: (1) a model-level toolkit with diverse models trained on limited data and image manipulation functions for fine-grained editing composition, and (2) a three-stage progressive reinforcement learning approach using feedback on unannotated open-domain instructions to train MLLM.
...
Conclusion: Lego-Edit effectively addresses the generalization challenge in instruction-based image editing by leveraging MLLM's capabilities and a progressive learning approach, demonstrating strong performance across diverse real-world instructions.
...
TL;DR: Proposes high-order inversion method using Runge-Kutta solver for better image consistency and DDTA mechanism for precise attention control in rectified flow models.


<details>
  <summary>Details</summary>
Motivation: Rectified flow models have superior generative performance but suffer from low inversion accuracy (poor source image consistency) and entangled multimodal attention in diffusion transformers that hinders precise control.

Method: Two solutions: (1) Efficient high-order inversion method based on Runge-Kutta solver for differential equations to improve inversion accuracy, (2) Decoupled Diffusion Transformer Attention (DDTA) mechanism that disentangles text and image attention in multimodal diffusion transformers.

Result: Extensive experiments show state-of-the-art performance in image reconstruction and text-guided editing tasks, achieving superior fidelity and editability.

Conclusion: The proposed methods effectively address the two major challenges of rectified flow models, enabling better image consistency and more precise semantic control for improved generative performance.
...
TL;DR: MEJO framework addresses surgical triplet recognition challenges by disentangling task representations and rebalancing gradients, achieving state-of-the-art performance on surgical datasets.
...
Method: Proposes MLLM-Engaged Joint Optimization (MEJO) with: 1) Shared-Specific-Disentangled learning to separate task-shared and task-specific representations, 2) MLLM-powered probabilistic prompt pool for semantic augmentation, 3) Coordinated Gradient Learning to rebalance gradients from head/tail classes.
...
TL;DR: DialNav introduces a collaborative embodied dialog task where a Navigator and Guide communicate to reach a goal, with the novel requirement that the Guide must infer the Navigator's location, making communication essential.


<details>
  <summary>Details</summary>
Motivation: Prior work in embodied navigation lacked holistic evaluation and didn't require the Guide to infer the Navigator's position, limiting the necessity of communication for task success.

Method: Collected the RAIN dataset with human-human dialog paired with navigation trajectories in photorealistic environments, designed a comprehensive benchmark for both navigation and dialog evaluation, and conducted experiments with different Navigator/Guide models.

Result: The research highlights key challenges in collaborative embodied dialog navigation and provides extensive experimental analysis of different model configurations.

Conclusion: DialNav represents a significant advancement in embodied dialog tasks by making communication essential, and the publicly released dataset, code, and framework will foster future research in this area.
...
TL;DR: Cross-Layer Vision Smoothing (CLVS) improves LVLMs by maintaining sustained attention on key objects through vision memory that smooths attention distribution across layers, achieving SOTA performance on visual tasks.


<details>
  <summary>Details</summary>
Motivation: LVLMs have brief attention on key objects, and sustained focus is hypothesized to improve visual capabilities.

Method: CLVS incorporates vision memory initialized with position-unbiased attention in first layer, then iteratively updated across layers while considering previous memory. Smoothing terminates when uncertainty indicates completed visual understanding.

Result: Achieves state-of-the-art performance on four benchmarks across three LVLMs, with significant improvements in relation and attribute understanding tasks.

Conclusion: CLVS effectively enhances LVLMs' visual capabilities through sustained attention on key objects via cross-layer vision smoothing.
...
TL;DR: MSGFusion is a novel infrared and visible image fusion framework that uses structured scene graphs to guide fusion, outperforming state-of-the-art methods by better preserving details and semantic information.


<details>
  <summary>Details</summary>
Motivation: Current deep learning fusion methods rely too much on low-level visual cues and struggle with high-level semantic information. Existing text-guided approaches use unstructured descriptions without explicit modeling of entities, attributes, and spatial relationships.

Method: MSGFusion uses multimodal scene graphs derived from text and vision to explicitly represent entities, attributes, and spatial relations. It employs successive modules for scene graph representation, hierarchical aggregation, and graph-driven fusion to refine both high-level semantics and low-level details.

Result: Extensive experiments show MSGFusion significantly outperforms state-of-the-art approaches, particularly in detail preservation and structural clarity. It also delivers superior semantic consistency and generalizability in downstream tasks like object detection, semantic segmentation, and medical image fusion.
...
TL;DR: A novel generative anomaly detection method using image-to-image translation and patch similarity scoring, achieving improved lesion segmentation in chest CT and brain MRI compared to state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Early disease detection and medical condition differentiation require robust anomaly detection, but normal tissue variability in pulmonary anatomy challenges existing generative methods.

Method: Combines image-to-image translation for anomaly-free reconstruction with patch similarity scoring between observed and generated image pairs for precise anomaly localization.

Result: Achieved relative DICE score improvements of +1.9% in chest CTs and +4.4% in brain MRIs for infectious disease lesion detection and ischemic stroke segmentation.

Conclusion: The proposed approach effectively addresses normal tissue variability challenges and demonstrates strong generalizability across different medical imaging modalities and conditions.
...
<details>
  <summary>Details</summary>
Motivation: Aerial object tracking faces challenges like scale variations, dynamic backgrounds, and occlusions. Existing trackers focus on spatial cues but neglect temporal dependencies, and correlation-based Siamese trackers struggle with complex non-linear appearance changes due to their linear correlation operations.

Method: Extends SiamTPN architecture with explicit temporal modeling through temporal feature fusion and attention-based interactions to strengthen temporal consistency and enable richer feature representations.

Result: Achieves 13.7% improvement in success rate and 14.7% improvement in precision compared to baseline. Runs in real-time at 7.1 FPS on Jetson Nano, demonstrating computational efficiency suitable for embedded applications.

Conclusion: Temporal modeling is crucial for Siamese tracking frameworks. T-SiamTPN provides a strong and efficient solution for aerial object tracking with significant performance gains while maintaining real-time operation on resource-constrained devices.
...
Result: For YOLOv8m: Parameters reduced from 25.85M to 6.85M (73.51% reduction), FLOPs from 49.6G to 13.3G, MACs from 101G to 34.5G. AP50 reduced by only 2.7% to 47.9. Inference speed improved from 26 FPS to 45 FPS. With TensorRT: 68 FPS with AP50 47.6.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods treat point cloud registration validation as a classification task with limited quality classes, but regression can provide more fine-grained quantification of registration quality. The authors aim to improve accuracy and robustness, particularly for point clouds with varying spatial densities.

Method: The authors use regression instead of classification for PCR validation, extend misalignment-related features through multiscale extraction, and employ attention-based aggregation for feature processing.

Result: The method achieves accurate and robust registration error estimation on diverse datasets, performs particularly well on point clouds with heterogeneous spatial densities, and significantly improves mapping quality when used to guide downstream tasks compared to state-of-the-art classification-based methods.

Conclusion: Regression-based validation with multiscale feature extraction and attention-based aggregation provides superior fine-grained quantification of point cloud registration quality compared to classification approaches, especially benefiting applications with heterogeneous point cloud densities.
...
<details>
  <summary>Details</summary>
Motivation: Existing self-supervised methods struggle with accurate dynamic object reconstruction due to imprecise motion estimation and weak temporal consistency, especially when annotated bounding boxes are unavailable.
...
Conclusion: 4DRadar-GS effectively addresses dynamic object reconstruction challenges in autonomous driving by leveraging 4D Radar's velocity and spatial information for improved accuracy and temporal consistency.
...
TL;DR: A novel method for 3D scene understanding using object-level Gaussian representations with CLIP feature aggregation, enabling accurate open-vocabulary object retrieval and seamless 2D/3D segmentation without differentiable rendering.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting produces fuzzy representations that hinder 3D scene understanding, limiting AR/VR and robotics applications. Existing semantic learning approaches fail due to alpha blending limitations that average semantics across objects.

Method: Leverages predecomposed object-level Gaussians and represents each object through multiview CLIP feature aggregation to create comprehensive 'bags of embeddings' that holistically describe objects.
...
Conclusion: Proposes a paradigm-shifting alternative that bypasses differentiable rendering for semantics, enabling accurate object retrieval and seamless task adaptation for both 2D segmentation and 3D extraction.
...
TL;DR: A method for interactive multi-surface segmentation using user clicks and multi-modal inputs, with a novel network architecture that integrates RGB, non-RGB modalities, and user interactions to improve segmentation masks.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of segmenting multiple entangled surfaces in images simultaneously, which requires handling complex scenarios where surfaces are adjacent and heavily intertwined.

Method: A network architecture that takes RGB images, non-RGB modalities, erroneous masks, and encoded clicks as input to predict improved segmentation masks. The design ensures RGB backbone remains black-box and integrates interaction-specific information after feature extraction for faster response times.

Result: The method reduces NoC@90 by up to 1.28 clicks per surface on DeLiVER and up to 1.19 on MFNet. The RGB-only baseline also shows competitive performance in single-mask interactive segmentation scenarios.

Conclusion: The proposed multi-modal fusion strategy is effective for multi-surface interactive segmentation, demonstrating significant improvements in efficiency and maintaining strong performance even in RGB-only settings.
...
TL;DR: FEST Competition introduces a few-shot learning challenge for segmenting text lines in ancient handwritten documents using only 3 annotated images per manuscript, addressing the scarcity of annotated data in historical document analysis.


<details>
  <summary>Details</summary>
Motivation: Historical handwritten documents present unique challenges for text line segmentation due to irregular handwriting, faded ink, complex layouts, and scarcity of large annotated datasets, making fully supervised approaches impractical.

Method: The competition uses the U-DIADS-TL dataset with diverse ancient manuscripts and requires participants to develop systems that learn from only three annotated images per manuscript for training.

Result: The competition framework is established to evaluate few-shot learning approaches on realistic historical document conditions with varying layouts, degradation levels, and non-standard formatting.

Conclusion: FEST competition aims to promote robust and adaptable text line segmentation methods that require minimal manual annotation, enabling broader adoption of automated document analysis tools in historical research by humanities scholars.
...
TL;DR: Protein surface shape retrieval track with 9 teams evaluating 15 methods on 11,555 protein surfaces using electrostatic potential as key descriptor, showing best results when combining shape with electrostatic data.


<details>
  <summary>Details</summary>
Motivation: To evaluate and compare different methods for protein surface shape retrieval, particularly focusing on the importance of incorporating electrostatic potential as a complementary descriptor to molecular surface shape.

Method: Evaluation of 15 proposed methods from 9 participating teams on a dataset of 11,555 protein surfaces with calculated electrostatic potential, using multiple metrics including Accuracy, Balanced accuracy, F1 score, Precision and Recall.

Result: The best retrieval performance was achieved by methods that used electrostatic potential complementary to molecular surface shape, with this observation holding true even for classes with limited data.

Conclusion: Incorporating additional molecular surface descriptors like electrostatic potential alongside shape information significantly improves protein surface retrieval performance, demonstrating the importance of multi-descriptor approaches in bioinformatics.
...
TL;DR: WINNER addresses SIRENs' spectral bottleneck by using Gaussian noise initialization scaled to target signal's spectral centroid, achieving SOTA performance without extra parameters.


<details>
  <summary>Details</summary>
Motivation: SIRENs struggle with signals outside their frequency support due to improper initialization, causing spectral bottleneck where networks fail to represent even supported frequencies.

Method: WINNER perturbs uniformly initialized SIREN weights with Gaussian noise, adaptively scaling noise based on target signal's spectral centroid to mitigate spectral bias.

Result: Achieves state-of-the-art audio fitting and significant improvements in image and 3D shape fitting tasks compared to base SIREN.

Conclusion: WINNER provides effective target-aware initialization strategy that overcomes SIRENs' spectral limitations without adding trainable parameters, opening new avenues for adaptive neural network initialization.
...
TL;DR: Omnidirectional vision is gaining importance in embodied AI, offering complete 360-degree environmental awareness for applications like robotics and industrial inspection. The paper presents recent breakthroughs in omnidirectional generation, perception, and understanding, proposing a PANORAMA system architecture and discussing future challenges.


<details>
  <summary>Details</summary>
Motivation: Omnidirectional vision provides holistic environmental awareness that significantly enhances scene perception completeness and decision-making reliability compared to traditional pinhole vision. However, foundational research in this area has historically lagged behind, creating a need to address this gap in the embodied AI era.

Method: The paper synthesizes state-of-the-art advancements in omnidirectional vision, highlighting breakthroughs in omnidirectional generation, perception, understanding, and related datasets. It proposes an ideal panoramic system architecture called PANORAMA with four key subsystems, drawing insights from both academia and industry.

Result: The overview presents emerging trends at the intersection of panoramic vision and embodied AI, offering in-depth opinions on cross-community impacts and proposing a comprehensive roadmap for future research in building robust, general-purpose omnidirectional AI systems.

Conclusion: Omnidirectional vision represents a rapidly developing field driven by industrial demand and academic interest. The paper outlines challenges and opportunities for future research, emphasizing the need to advance foundational work in this area to keep pace with the embodied AI era's requirements.
...
TL;DR: A Dual-Stage Reweighted Mixture-of-Experts framework for detecting subtle and infrequent user action mistakes from egocentric video, combining feature extraction and multiple specialized classifiers to handle class imbalance and improve generalization.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of detecting subtle and infrequent user action mistakes in egocentric video data, which is difficult due to class imbalance and ambiguous instances.
...
Conclusion: The proposed DR-MoE framework effectively handles the challenges of subtle and infrequent mistake detection in egocentric video through its dual-stage expert fusion approach.
...
TL;DR: Modern Vision Foundation Models outperform specialized AI-generated image detectors by over 20% accuracy in real-world scenarios due to better alignment with forgery concepts learned during pre-training.
...
Method: The authors use a simple linear classifier on top of a modern Vision Foundation Model (VFM) trained on identical data, and analyze text-image similarities to understand why VLMs perform better.

Result: The VFM-based approach boosts in-the-wild accuracy by over 20% compared to specialized detectors. The performance advantage comes from VLMs learning to align synthetic images with forgery-related concepts during pre-training, but this advantage disappears on novel datasets created after the VFM's pre-training cut-off date.

Conclusion: 1) Updated Vision Foundation Models provide more effective real-world detection than static specialized detectors. 2) True generalization evaluation requires test data to be completely independent of the model's entire training history, including pre-training data exposure.
...
TL;DR: Neuromorphic drone detection system using spiking neural networks and event cameras achieves extreme energy efficiency, enabling year-long battery operation and easy deployment in remote areas.


<details>
  <summary>Details</summary>
Motivation: Small drones pose increasing threats to military and civilian infrastructure, requiring early automated detection systems that can operate efficiently in various environments.

Method: Developed a fully neuromorphic system using spiking neural networks and neuromorphic cameras (event cameras) deployed on neuromorphic chips, with multiple detection units creating virtual tripwires. Used synthetically generated data for training.

Result: System is several orders of magnitude more energy efficient than edge GPU solutions, capable of running over a year on battery power. Model relies on drone shape rather than propeller temporal characteristics.

Conclusion: The neuromorphic approach provides highly energy-efficient drone detection suitable for deployment in contested areas or locations lacking power infrastructure due to small size and low power consumption.
...
TL;DR: Dream3DAvatar is a two-stage framework that generates realistic, animation-ready 3D avatars from single images using text-controllable multi-view generation and 3D Gaussian Splat reconstruction with specialized adapters for pose consistency and facial identity preservation.


<details>
  <summary>Details</summary>
Motivation: Current single-image 3D avatar reconstruction methods struggle with ill-posedness due to limited monocular input, making it difficult to control geometry and texture in occluded regions during generation.

Method: Two-stage framework: 1) Lightweight adapter-enhanced multi-view generation with Pose-Adapter (SMPL-X renderings), ID-Adapter-G (facial features), and BLIP2 for text descriptions; 2) Feedforward Transformer with multi-view feature fusion to reconstruct 3D Gaussian Splat representations using ID-Adapter-R for facial feature fusion.
...
Conclusion: Dream3DAvatar effectively addresses the challenges of single-image 3D avatar reconstruction through its innovative two-stage approach with specialized adapters, enabling text-controllable generation of high-fidelity 3D avatars with preserved facial identity and geometric consistency.
...
TL;DR: A two-stage reinforcement learning framework that enhances both visual perception and reasoning capabilities in vision-language models, addressing the limitations of directly applying LLM RL methods to VLMs.


<details>
  <summary>Details</summary>
Motivation: RL has been effective for LLMs but directly transplanting these methods to VLMs is suboptimal because VLMs face more complex tasks requiring accurate visual perception before reasoning can occur.

Method: Two-stage RL framework: 1) dataset-level sampling to mitigate vanishing advantage, 2) first stage focuses on improving visual perception through coarse- and fine-grained understanding, 3) second stage enhances reasoning abilities.
...
Conclusion: The proposed two-stage RL framework effectively addresses the unique challenges of VLMs by jointly enhancing both perception and reasoning, leading to state-of-the-art performance on visual reasoning benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: High-resolution LVLMs process images by dividing them into local tiles, which creates excessive visual tokens leading to significant computational and memory overhead that needs to be addressed.

Method: HERO uses content-adaptive token budget allocation and function-aware token selection based on empirical findings about varying tile importance, CLS token attention patterns, and complementary granularity information in visual tokens.
...
Conclusion: The study provides both empirical insights into visual token utilization and a practical training-free solution for efficient inference in high-resolution large vision-language models.
...
Conclusion: TFANet provides a comprehensive solution for fine-grained image-text alignment in complex RIS scenarios by addressing both multimodal misalignment and semantic degradation issues.
...
<details>
  <summary>Details</summary>
Motivation: Traditional Fourier frequency fitting with pixel-wise loss functions focuses too much on local information and causes global information loss. There's a need for better Fourier-domain alignment that preserves both amplitude (luminance) and phase (spatial structure) information.

Method: Proposed LLFDisc network with cross-attention and gating mechanisms, plus a novel distribution-aware loss using closed-form KL-Divergence to fit Fourier-domain information. Also enhanced VGG perceptual loss with KL-Divergence on deep features.
...
Conclusion: The proposed LLFDisc with distribution-aware KL-Divergence loss effectively enhances frequency-aware image processing, achieving superior Fourier-domain alignment and structural fidelity compared to conventional methods.
...
TL;DR: A novel semi-supervised 3D medical image segmentation framework using dual-network architecture with cross consistency enhancement, uncertainty-aware weighting, and contrastive learning to address noisy pseudo-labels and improve segmentation performance with limited labeled data.
...
Conclusion: Synthetic data generation provides a time-saving, resource-efficient pipeline for visual assembly control that can effectively support SMEs in implementing automated quality control solutions without extensive manual data collection.
...
TL;DR: HDFF framework combines four pre-trained models for deepfake detection, achieving 0.96852 score and 20th place in competition.
...
Method: Hierarchical ensemble architecture integrating Swin-MLP, CoAtNet, EfficientNetV2, and DaViT models fine-tuned on MultiFFDI dataset, with feature concatenation and final classifier training.
...
TL;DR: Weakly and self-supervised class-agnostic motion prediction from LiDAR using foreground/background or non-ground/ground masks instead of motion annotations, with novel consistency-aware loss.


<details>
  <summary>Details</summary>
Motivation: Motion understanding is critical for autonomous driving, but motion annotations are expensive. Outdoor scenes naturally separate into mobile foregrounds and static backgrounds, allowing motion prediction to be associated with scene parsing to reduce annotation requirements.

Method: Proposes weakly supervised paradigm using fully/partially annotated foreground/background masks (1%, 0.1%) for supervision. Uses non-ground/ground masks as alternative to further reduce annotation (0.01% or zero annotations). Designs Robust Consistency-aware Chamfer Distance loss with multi-frame information and robust penalty to handle outliers.

Result: Weakly and self-supervised models outperform existing self-supervised counterparts. Weakly supervised models rival some supervised approaches, effectively balancing annotation effort and performance.

Conclusion: The proposed approaches demonstrate that motion prediction can be effectively learned with minimal or no motion annotations by leveraging scene structure cues, making autonomous driving systems more practical and scalable.
...
TL;DR: This paper introduces CRPS-D, a large-scale parking slot detection dataset with diverse real-world conditions, and SS-PSD, the first semi-supervised method for parking slot detection using teacher-student model with confidence-guided consistency and adaptive feature perturbation.


<details>
  <summary>Details</summary>
Motivation: Current parking slot detection datasets are limited in scale and lack real-world noise variations. Manual annotation is error-prone and expensive for large-scale datasets, creating a need for better datasets and methods that can leverage unlabeled data.

Method: Constructed CRPS-D dataset with diverse lighting, weather conditions, and challenging parking slot variants. Developed SS-PSD, a semi-supervised approach using teacher-student model with confidence-guided mask consistency and adaptive feature perturbation to exploit unlabeled data.

Result: SS-PSD outperforms state-of-the-art solutions on both the proposed CRPS-D dataset and existing datasets. Performance gains increase with more unlabeled data. CRPS-D is the largest dataset with highest density of parking slots, including many slanted slots.

Conclusion: The proposed CRPS-D dataset and SS-PSD semi-supervised method significantly advance parking slot detection by addressing dataset limitations and leveraging unlabeled data through innovative consistency and perturbation techniques, demonstrating superior performance over existing approaches.
...
Method: Two-stage distillation: 1) Reconstruction-guided feature distillation to align and densify features, 2) Diffusion-guided feature distillation using a lightweight diffusion network with noise adapter for adaptive noise level alignment with predefined timesteps.
...
Conclusion: MSDNet effectively addresses the limitations of existing methods by providing both high reconstruction quality and computational efficiency through multi-stage knowledge distillation from LiDAR priors to 4D radar features.
...
Method: Introduces TexTAR, a multi-task context-aware Transformer with novel data selection pipeline and 2D RoPE-style mechanism for incorporating input context. Also creates MMTAD dataset with multilingual, multi-domain document annotations.
...
Conclusion: Contextual awareness through the proposed architecture and data selection pipeline enables superior textual attribute recognition performance across diverse, real-world document types and languages.
...
TL;DR: A multi-video collaborative framework that uses spatio-temporal graphs to represent video knowledge and fuse information from multiple related videos to enhance video language model reasoning.


<details>
  <summary>Details</summary>
Motivation: Current video language models suffer from spatio-temporal incompleteness in individual videos, leading to hallucinations and inaccuracies. Multiple related videos could enhance reasoning but directly feeding them is inefficient due to redundant video tokens.
...
Conclusion: The multi-video collaborative framework with structured graph representation shows promise for advancing video language models by addressing spatio-temporal incompleteness through efficient multi-video integration.
...
TL;DR: WHU-STree is a comprehensive multi-modal street tree dataset collected across two cities, featuring 21,007 annotated trees across 50 species with synchronized point clouds and images, supporting over 10 inventory tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional field surveys for street tree inventory are time-consuming and labor-intensive, while existing MMS-acquired datasets are limited by small scale, poor annotation, or single modality, restricting comprehensive urban tree analysis.

Method: Collected cross-city data using Mobile Mapping Systems, integrating synchronized point clouds and high-resolution images, with detailed annotations of 21,007 tree instances across 50 species and 2 morphological parameters.

Result: Extensive experiments demonstrate significant potential of multi-modal data fusion and highlight cross-domain applicability as critical for practical algorithm deployment. Benchmark results for tree species classification and individual tree segmentation are provided.

Conclusion: WHU-STree addresses limitations of existing datasets and enables multiple street tree inventory tasks. Key challenges identified include multi-modal fusion, multi-task collaboration, cross-domain generalization, spatial pattern learning, and potential integration with Multi-modal Large Language Models for urban tree management.
...
Method: Using modern LLMs to automatically extract diagnostic labels from radiology reports without complex prompt engineering, creating "silver-standard" datasets at minimal cost. Training vision encoders on these datasets with vanilla CLIP training using a 3D ResNet-18 architecture.

Result: Achieved remarkable precision (>96% AUC) in label extraction, created 50k CT image-report pairs for ~$3. Vision encoder performance comparable to specialized BERT-based models. State-of-the-art results: 83.8% AUC for zero-shot diagnosis on CT-RATE, 77.3% AUC on RAD-ChestCT, and substantial improvements in cross-modal retrieval (MAP@50=53.7% for image-image, Recall@100=52.2% for report-image).

Conclusion: LLMs can facilitate more performant and scalable medical AI systems by democratizing access to large-scale supervised pre-training and fundamentally improving contrastive vision-language alignment in medical imaging applications.
...
TL;DR: This paper addresses road-obstacle segmentation for autonomous driving by highlighting the temporal nature of the problem and introducing video-based benchmarks and strong baseline methods using vision foundation models.


<details>
  <summary>Details</summary>
Motivation: Existing road-obstacle segmentation methods work on individual frames, ignoring temporal consistency and leading to inconsistent predictions between consecutive frames, which is critical for safe autonomous navigation.

Method: The authors curated and adapted four evaluation benchmarks for road-obstacle video segmentation, evaluated 11 state-of-the-art image- and video-based segmentation methods, and introduced two strong baseline methods based on vision foundation models.

Result: The approach establishes a new state-of-the-art in road-obstacle video segmentation for long-range video sequences, demonstrating improved performance over frame-by-frame methods.

Conclusion: Road-obstacle segmentation is inherently temporal, and the proposed video-based approach with foundation models provides valuable insights and direction for future research in autonomous driving safety.
...
TL;DR: Vi-SAFE is a spatial-temporal framework combining enhanced YOLOv8 with TSN for violence detection in surveillance videos, achieving 88% accuracy on RWF-2000 dataset while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Address challenges in violence detection including small-scale targets, complex environments, and real-time temporal analysis requirements for public safety surveillance.

Method: Integrates optimized YOLOv8 (with GhostNetV3 backbone, EMA attention, pruning) for human detection and Temporal Segment Network for violence classification. Models trained separately on pedestrian and violence datasets.

Result: Achieves 0.88 accuracy on RWF-2000 dataset, outperforming standalone TSN (0.77) and existing methods in both accuracy and efficiency.

Conclusion: Vi-SAFE demonstrates effective violence detection for public safety surveillance with improved accuracy and computational efficiency compared to existing approaches.
...
TL;DR: A novel detection method called End4 is proposed to identify images generated by diffusion-based inpainting models, addressing security concerns about potential misuse of these powerful generative models.


<details>
  <summary>Details</summary>
Motivation: Diffusion models have advanced image synthesis but raise concerns about malicious use. Existing approaches struggle to detect images generated by diffusion-based inpainting models, even when trained on similar inpainted images.

Method: End4 uses an end-to-end denoising diffusion approach with a denoising reconstruction model to improve latent space alignment between reconstruction and detection processes. It also employs a Scale-aware Pyramid-like Fusion Module (SPFM) to refine local image features using attention pyramid layers at different scales.

Result: The method effectively generalizes to unseen masking patterns and remains robust under various perturbations. A comprehensive benchmark with five distinct masked regions was established for evaluation.

Conclusion: End4 provides an effective solution for detecting diffusion-based inpainted images, addressing security concerns while maintaining robustness and generalization capabilities across different masking patterns.
...
TL;DR: A novel curriculum multi-task self-supervised learning framework for lightweight hyperspectral image analysis that combines masked image modeling with spatial/spectral jigsaw puzzles, achieving strong performance with models 16,000x lighter than SOTA.
...
Method: CMTSSL integrates masked image modeling with decoupled spatial and spectral jigsaw puzzle solving, using curriculum learning to progressively increase data complexity during self-supervision.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in maintaining consistent quality in vacuum thermoforming due to variations in material properties and tooling configurations

Method: Developed dataset with visual data from vacuum-formed samples under various parameters, used image augmentation, and employed k-Nearest Neighbour algorithm to map low-quality parts to high-quality counterparts for parameter adjustment
...
Conclusion: Vision-based quality control system effectively predicts and optimizes process parameters to enhance part quality in vacuum thermoforming
...
TL;DR: ResidualViT: A vision transformer architecture that reduces computational cost for temporally dense video tasks by leveraging temporal redundancy, with residual connections and token reduction, achieving 60% cost reduction and 2.5x speedup while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Temporally dense video tasks require high frame-rate processing but computing frame-level features is computationally expensive due to temporal resolution requirements.

Method: Proposes ResidualViT architecture with learnable residual connections for temporal consistency and token reduction module to discard redundant information while reusing pretrained foundation model weights. Uses lightweight distillation to approximate original model features.

Result: Achieves up to 60% reduction in computational cost and 2.5x faster inference speed across four tasks and five datasets in both zero-shot and supervised settings, while closely matching original model accuracy.
...
TL;DR: RadGame is an AI-powered gamified platform that improves radiology training through interactive localization and report-writing exercises with automated feedback, showing significant improvements over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional radiology training relies on passive case exposure or limited real-time feedback from supervising radiologists, creating scalability and immediate feedback challenges.
...
Result: Participants achieved 68% improvement in localization accuracy (vs 17% with traditional methods) and 31% improvement in report-writing accuracy (vs 4% with traditional methods) after same case exposure.
...
<details>
  <summary>Details</summary>
Motivation: Need for reliable quantification of AI-generated image realness and identification of visually inconsistent regions to improve practical use and training of generative AI models.
...
Result: Improved objective realness prediction performance and production of dense realness maps that effectively distinguish realistic vs unrealistic spatial regions.
...
Method: Proposes Style Disentangled Attention (SD-Attn) module with cross-3D attention for dynamic content-style interaction, plus style-disentangled feature selection and Style Guided Control mechanism for exclusive geometry/texture stylization.
...
Conclusion: StyleSculptor enables zero-shot style-guided 3D generation with effective style disentanglement and adjustable control, advancing practical 3D asset creation.
...
TL;DR: SR-3D is a vision-language model that bridges 2D images and 3D data through shared visual tokens, enabling flexible region prompting and accurate spatial reasoning without exhaustive multi-frame labeling.


<details>
  <summary>Details</summary>
Motivation: To connect single-view 2D images with multi-view 3D data and enable flexible region annotation without requiring exhaustive multi-frame labeling, addressing the gap between 2D and 3D representation spaces.

Method: Enrich 2D visual features with 3D positional embeddings, allowing the 3D model to leverage strong 2D priors for spatial reasoning across frames, even when objects don't appear in the same view.
...
Conclusion: SR-3D successfully bridges 2D and 3D visual understanding, enabling accurate spatial relationship inference and metric measurements even in wild videos without 3D inputs or ground-truth annotations.
