...
Motivation: Traditional art style identification relies on subjective human expertise; this work aims to provide objective, mathematical methods for style analysis.

Method: Applied persistent homology (PH) from topological data analysis to analyze artistic styles through topological features of artworks.

Result: PH can statistically differentiate between artists from different and same artistic currents, and distinguish real artist works from AI-generated imitations.

Conclusion: Persistent homology offers objective, interpretable tools for art style analysis that complement traditional subjective methods.
...
<details>
  <summary>Details</summary>
Motivation: Limited training images for animal faces, especially macaque monkeys used in neuroscience research, make generating diverse facial expressions challenging.

Method: Used StyleGAN2 with three key approaches: 1) data augmentation via motion transfer animation, 2) sample selection based on latent representations, and 3) refined loss function for subtle movements.

Result: Generated diverse facial expressions for multiple macaque individuals, outperforming models trained only on original still images, and enabled effective style-based image editing.

Conclusion: The model successfully disentangles motion components as style parameters, providing a valuable tool for macaque facial expression research.
...
TL;DR: Proposes PairHuman, the first large-scale benchmark dataset for dual-person portrait generation with 100K+ images, and introduces DHumanDiff method for enhanced facial consistency and balanced personalization.


<details>
  <summary>Details</summary>
Motivation: The absence of a benchmark dataset hinders high-quality dual-person portrait customization, which has applications in preserving emotional memories and wedding photography planning.

Method: Created PairHuman dataset with 100K+ images containing rich metadata, and developed DHumanDiff baseline method that enhances facial consistency while balancing personalized person generation and semantic-driven scene creation.

Result: Experimental results demonstrate highly customized portraits with superior visual quality tailored to human preferences.

Conclusion: The proposed dataset and method successfully address the gap in dual-person portrait generation, producing high-quality customized results that meet photographic standards.
...
TL;DR: Unsupervised autoencoder with CDF 97 wavelet transform successfully denoises neutron imaging data with mixed Gaussian-Poisson noise, outperforming traditional methods like BM3D in reconstruction error and edge preservation.


<details>
  <summary>Details</summary>
Motivation: Neutron imaging is crucial for ICF analysis but images are degraded by mixed Gaussian-Poisson noise that conventional methods struggle to remove while preserving image fidelity.

Method: Implemented unsupervised autoencoder with Cohen-Daubechies-Feauveau (CDF 97) wavelet transform in the latent space for mixed Gaussian-Poisson denoising.

Result: Network successfully denoised neutron imaging data with lower reconstruction error and superior edge preservation metrics compared to BM3D and other non-ML filtering methods.

Conclusion: This approach presents a promising advancement in neutron image noise reduction and 3D reconstruction analysis for ICF experiments.
...
TL;DR: SAM 3 is a unified model for detecting, segmenting, and tracking objects using concept prompts (noun phrases, image exemplars, or both), achieving double the accuracy of existing systems.


<details>
  <summary>Details</summary>
Motivation: To advance Promptable Concept Segmentation (PCS) by creating a scalable model that can handle diverse concept prompts and improve object detection and tracking in both images and videos.

Method: Built a scalable data engine producing 4M unique concept labels with hard negatives. Model uses an image-level detector and memory-based video tracker with shared backbone, decoupling recognition and localization using a presence head.
...
Conclusion: SAM 3 significantly advances promptable concept segmentation and is open-sourced along with the new SA-Co benchmark for community use and evaluation.
...
Method: Proposed SaFeR-CLIP framework using proximity-aware alignment that redirects unsafe concepts to their semantically closest safe alternatives, applying minimal intervention to preserve representation geometry.

Result: SaFeR-CLIP recovers up to 8.0% in zero-shot accuracy over prior methods while maintaining robust safety. Also introduced NSFW-Caps benchmark with 1,000 highly-aligned pairs for safety evaluation under distributional shift.
...
TL;DR: A three-stage framework for generating multi-view consistent SVGs from single-view input using 3D lifting, spatial memory from SAM2, and path optimization.


<details>
  <summary>Details</summary>
Motivation: SVGs are important for modern design but generating multi-view consistent SVGs from single-view input remains underexplored.

Method: Three-stage framework: 1) Lift rasterized input to 3D and render multi-view images, 2) Extend SAM2's temporal memory to spatial domain for part-level correspondences, 3) Path consolidation and structural optimization during raster-to-vector conversion.

Result: Generated SVGs show strong geometric and color consistency across views, reduced redundant paths, and preserved fine structural details.
...
TL;DR: Mesh RAG is a training-free framework that enhances autoregressive mesh generation by using retrieval-based component integration, improving quality, speed, and enabling incremental editing.


<details>
  <summary>Details</summary>
Motivation: Traditional mesh creation is manual and time-intensive, while current autoregressive models face quality-speed trade-offs and sequential dependency issues that complicate incremental editing.
...
Conclusion: Mesh RAG provides an effective plug-and-play solution that overcomes limitations of sequential dependency in autoregressive mesh generation, offering improved quality, speed, and editing capabilities.
...
TL;DR: WorldGen automatically creates large-scale, interactive 3D worlds from text prompts using LLM-driven reasoning, procedural generation, and diffusion-based 3D generation.
...
Method: Combines LLM-driven scene layout reasoning, procedural generation, diffusion-based 3D generation, and object-aware scene decomposition to transform text descriptions into interactive environments.

Result: Produces geometrically consistent, visually rich, and efficient-to-render 3D worlds that are immediately explorable in standard game engines with fine-grained control over layout, scale, and style.
...
TL;DR: REO-Instruct is the first unified benchmark for both descriptive and regression tasks in Earth Observation, focusing on forest ecological scenarios to bridge qualitative understanding and quantitative prediction of biophysical variables like above-ground biomass.


<details>
  <summary>Details</summary>
Motivation: Existing EO datasets mainly focus on semantic understanding tasks like captioning or classification, lacking benchmarks that align multimodal perception with measurable biophysical variables for scientific regression.

Method: Created REO-Instruct dataset integrating co-registered Sentinel-2 and ALOS-2 imagery with structured textual annotations generated through a hybrid human-AI pipeline, establishing a cognitively interpretable logic chain in forest ecology.

Result: Comprehensive evaluation reveals current vision language models struggle with numeric reasoning, highlighting an essential challenge for scientific VLMs in Earth Observation applications.

Conclusion: REO-Instruct provides a standardized foundation for developing and assessing next-generation geospatial models capable of both description and scientific inference, addressing the gap in scientific regression capabilities for Earth Observation.
...
TL;DR: BOP-ASK is a large-scale dataset for object interaction reasoning that addresses fine-grained spatial understanding limitations in current VLMs, including precise 3D localization, physical compatibility, affordances, and multi-step planning.


<details>
  <summary>Details</summary>
Motivation: Current spatial reasoning benchmarks test high-level relationships but ignore fine-grained spatial understanding needed for real-world applications like precise 3D localization, physical compatibility between objects, object affordances, and multi-step spatial planning.

Method: The data generation pipeline leverages 6D object poses from BOP datasets to derive fine-grained annotations including grasp poses, referred object poses, path planning trajectories, relative spatial and depth relationships, and object-to-object relationships.

Result: BOP-ASK comprises over 150k images and 33M question-answer pairs spanning six tasks (four novel). Models trained on BOP-ASK outperform baselines and exhibit emergent capabilities in precise object/grasp pose estimation, trajectory planning, and fine-grained spatial reasoning in cluttered environments.

Conclusion: BOP-ASK provides a rich resource for training and evaluating VLMs on object interaction reasoning, addressing critical weaknesses in current spatial understanding benchmarks and enabling testing of generalization through out-of-distribution benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Existing GCN models perform poorly on imperfect skeletons due to occlusions and missing parts, as they lack the ability to capture contextual information from distant joints when local context is missing.
...
Conclusion: The hybrid GCN-Mamba approach effectively addresses the limitations of GCNs in handling imperfect skeletons by better capturing and maintaining contextual information from distant joints.
...
TL;DR: Extends Gromov-Wasserstein distance to enable simultaneous matching between collections of objects, providing a framework for multiple-to-one and multiple-to-multiple object matching.


<details>
  <summary>Details</summary>
Motivation: Traditional GW distance is limited to pairwise matching between single objects, which restricts its utility in applications requiring multiple-to-one or multiple-to-multiple object matching.

Method: Introduces Joint Gromov-Wasserstein (JGW) objective that extends GW framework to simultaneous matching between collections of objects, with formulations for point cloud representations using Optimal Transport algorithms including entropic regularization.

Result: Superior performance in accuracy and computational efficiency compared to other GW variants for partial matching; effective for multiple shape matching on both synthetic and real-world datasets including geometric shapes and biomolecular complexes.

Conclusion: JGW provides a promising framework for solving complex matching problems across diverse domains like computer graphics and structural biology, with demonstrated effectiveness in multiple shape matching applications.
...
<details>
  <summary>Details</summary>
Motivation: To leverage the inductive bias from aligning generative model representations with pretrained self-supervised encoders for improved performance in inverse problems where generative models serve as priors.

Method: Apply representation alignment between diffusion/flow-based models and pretrained self-supervised visual encoders (e.g., DINOv2) to guide reconstruction at inference time, using approximate target features when ground truth is unavailable.

Result: REPA substantially enhances reconstruction fidelity and perceptual realism across multiple inverse problems (super-resolution, inpainting, deblurring), while providing efficiency gains by reducing required discretization steps without compromising performance.

Conclusion: Representation alignment with pretrained encoders provides a powerful inductive bias that consistently improves reconstruction quality and efficiency in inverse problem solving, with theoretical grounding in divergence measures and representation steering.
...
TL;DR: NFGlassNet is a novel glass surface detection method that uses flash/no-flash image pairs to exploit reflection dynamics, outperforming state-of-the-art methods.
...
Conclusion: Leveraging reflection dynamics in flash/no-flash imagery provides an effective approach for accurate glass surface detection, with the proposed modules and dataset enabling superior performance.
...
TL;DR: R-AVST is a novel dataset for audio-visual spatio-temporal reasoning with fine-grained annotations, and AVST-Zero is a reinforcement learning model that achieves competitive performance on this benchmark.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs focus on simple video scenarios and fail to capture the complex, diverse nature of real-world audio-visual events, creating a gap in multimodal understanding capabilities.

Method: Created R-AVST dataset using LLM-based key object extraction, automatic spatial annotation, and manual quality inspection. Proposed AVST-Zero model using reinforcement learning with multi-dimensional rewards, avoiding intermediate supervision.

Result: Built dataset with 5K untrimmed videos, 27K objects across 100 audio-visual event types, and 8K QA pairs. AVST-Zero demonstrated competitive performance compared to existing models in extensive experiments.
...
TL;DR: Warm Diffusion bridges hot diffusion (noise-only) and cold diffusion (blur-only) by proposing a unified Blur-Noise Mixture Diffusion Model that jointly controls blurring and noise, exploiting spectral dependencies in images.


<details>
  <summary>Details</summary>
Motivation: Hot diffusion fails to exploit image correlations between high-frequency detail and low-frequency structures, while cold diffusion neglects the role of noise in shaping the data manifold, causing out-of-manifold issues.

Method: Proposed Warm Diffusion with Blur-Noise Mixture Diffusion Model (BNMD), using a divide-and-conquer strategy that exploits spectral dependencies to disentangle denoising and deblurring processes, and analyzing Blur-to-Noise Ratio (BNR) via spectral analysis.

Result: Extensive experiments across benchmarks validate the effectiveness of the approach for image generation.

Conclusion: Warm Diffusion successfully integrates the strengths of both hot and cold diffusion paradigms, providing a unified framework that addresses their respective limitations while maintaining strong generative performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing quality assessment methods provide only single scores, which are too coarse to guide model improvement. Fine-grained evaluation along realism and plausibility dimensions is crucial for improving generative models.
...
Result: Experimental results demonstrate the high quality and significance of the dataset and the comprehensiveness of the benchmark. The approach enables targeted guidance for improving generative models.

Conclusion: Q-Real provides a comprehensive framework for fine-grained evaluation of AI-generated images, addressing limitations of coarse single-score assessments and enabling more effective model optimization.
...
TL;DR: UniModel is a unified pixel-to-pixel diffusion framework that supports both visual understanding and generation by treating all inputs/outputs as RGB pixels in a shared visual space, eliminating modality discrepancies.


<details>
  <summary>Details</summary>
Motivation: To achieve unification across models, tasks, and representations by eliminating modality gaps between text and images, creating a fully vision-native multimodal learning approach.

Method: Map text and images into shared visual space by rendering text as painted images; use Unified Diffusion Transformer with rectified flow for bidirectional pixel-to-pixel transformations; lightweight task embeddings specify direction (understanding vs generation).

Result: Demonstrates strong cross-modal alignment and emergent controllability including cycle-consistent image-caption-image loops; performs well on both text-to-image synthesis and image-to-text understanding tasks.
...
TL;DR: DeltaDeno is a zero-shot anomaly generation method that creates realistic defects without training or real anomaly samples by contrasting diffusion branches and using latent inpainting guided by accumulated denoising deltas.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly generation methods require few-shot fine-tuning with anomalous samples, which contradicts the scarcity of anomalies and tends to overfit category priors. The paper addresses the setting where no real anomaly samples or training are available.

Method: Proposes Delta-Denoising (DeltaDeno) - a training-free zero-shot method that localizes and edits defects by contrasting two diffusion branches driven by a minimal prompt pair under shared schedule. Uses accumulated denoising deltas to create localization maps for latent inpainting, with token-level prompt refinement and spatial attention bias restricted to anomaly tokens.

Result: Experiments on public datasets show DeltaDeno achieves great generation quality, realism, and consistent gains in downstream detection performance.

Conclusion: DeltaDeno provides an effective zero-shot solution for anomaly generation that works without real anomaly samples or training, addressing the fundamental scarcity issue in anomaly detection.
...
TL;DR: DGAF-VSR is a diffusion-based video super-resolution method that improves alignment and compensation between frames using feature domain processing and upscaled warping to enhance perceptual quality, fidelity, and temporal consistency.
...
Result: Achieves significant improvements: 35.82% DISTS reduction (perceptual quality), 0.20 dB PSNR gain (fidelity), and 30.37% tLPIPS reduction (temporal consistency) on synthetic and real-world datasets.

Conclusion: DGAF-VSR demonstrates superior performance over state-of-the-art methods by effectively addressing alignment and compensation challenges in diffusion-based video super-resolution.
...
TL;DR: Deep learning framework integrating semantic and shape awareness for accurate tooth segmentation from CBCT images, addressing interdental adhesion challenges through multi-label learning and morphological constraints.


<details>
  <summary>Details</summary>
Motivation: Accurate tooth segmentation from CBCT images is crucial for digital dentistry but remains challenging due to interdental adhesions causing severe anatomical shape distortion.

Method: Proposes a deep learning framework with target-tooth-centroid prompted multi-label learning to model semantic relationships between teeth, and tooth-shape-aware learning to enforce morphological constraints, unified via multi-task learning.
...
TL;DR: OmniGround is a comprehensive benchmark for Spatio-Temporal Video Grounding (STVG) addressing limitations of existing datasets through diverse videos and complex queries, with proposed PG-TAF framework achieving significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Current STVG models face limitations due to narrow benchmark scope, leading to category bias, oversimplified reasoning, and poor linguistic robustness, which don't meet real-world demands for diverse objects and complex queries.

Method: Introduced OmniGround benchmark with 3,475 videos across 81 categories using Forward-Backward-Refinement annotation pipeline. Proposed PG-TAF, a training-free two-stage framework that decomposes STVG into high-level temporal grounding and fine-grained spatio-temporal propagation.

Result: Evaluations show 10.4% performance drop on complex real-world scenes, especially with small/occluded objects and intricate spatial relations. PG-TAF achieves 25.6% and 35.6% improvements in m_tIoU and m_vIoU on OmniGround with consistent gains across four benchmarks.

Conclusion: OmniGround addresses critical gaps in STVG evaluation, and PG-TAF demonstrates effective decomposition approach for handling complex spatio-temporal video grounding tasks with substantial performance improvements.
...
TL;DR: MultiPriv is the first benchmark to evaluate individual-level privacy reasoning in VLMs, revealing significant unmeasured privacy risks beyond simple perception.


<details>
  <summary>Details</summary>
Motivation: Current privacy benchmarks are insufficient as they only evaluate privacy perception but fail to address the critical risk of privacy reasoning - VLMs' ability to infer and link distributed information to construct individual profiles.

Method: Proposed Privacy Perception and Reasoning (PPR) framework with a bilingual multimodal dataset featuring synthetic individual profiles where identifiers are linked to sensitive attributes, enabling nine challenging tasks from attribute detection to cross-image re-identification.

Result: Evaluation of 50+ VLMs reveals: (1) significant unmeasured reasoning-based privacy risks, (2) perception-level metrics are poor predictors of reasoning risks, and (3) existing safety alignments are inconsistent and ineffective against reasoning-based attacks.

Conclusion: MultiPriv exposes systemic vulnerabilities in VLMs and provides a necessary framework for developing robust, privacy-preserving models.
...
TL;DR: A novel implicit neural representation framework that jointly models dynamic MRI images and optical flow fields, using physics-inspired regularization for simultaneous reconstruction and motion estimation without prior flow estimation.


<details>
  <summary>Details</summary>
Motivation: Dynamic MRI faces challenges from limited sampling and motion artifacts. Conventional methods rely on pre-estimated optical flow, which is inaccurate under undersampling and degrades reconstruction quality.

Method: Two implicit neural representations (INRs): one for spatiotemporal image content and another for optical flow, coupled via the optical flow equation as physics-inspired regularization, plus data consistency loss with k-space measurements.
...
Conclusion: The proposed implicit joint modeling with flow-regularized constraints shows strong potential for advancing dynamic MRI reconstruction.
...
Method: Proposes FiGOP (Finger Group-of-Pictures) which pairs RGB keyframes with subsequent hand keypoints, using a lightweight temporal encoder to convert keypoints into motion embeddings integrated with RGB features.

Result: Experiments on FingerCap-40K dataset show that existing Video-MLLMs struggle with finger-level reasoning, while FiGOP-augmented models achieve consistent gains under both automated HandJudge evaluation and human studies.

Conclusion: FiGOP effectively addresses temporal sparsity in video understanding by recovering fine temporal cues without increasing RGB density, enabling better finger-level motion captioning.
...
TL;DR: Proposes a two-branch framework for point-supervised facial expression spotting using only single timestamp annotations, with Gaussian-based intensity modeling and contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing methods require costly temporal boundary annotations; this work aims to reduce annotation burden by using only point supervision while handling both macro- and micro-expressions.

Method: Two-branch framework: (1) Class-agnostic expression intensity branch with Gaussian-based instance-adaptive intensity modeling for soft pseudo-labeling, (2) Class-aware apex classification branch for macro/micro-expression distinction, plus intensity-aware contrastive loss.

Result: Extensive experiments on SAMM-LV, CAS(ME)^2, and CAS(ME)^3 datasets demonstrate the framework's effectiveness in facial expression spotting with minimal supervision.

Conclusion: The proposed point-supervised framework successfully reduces annotation requirements while maintaining performance through adaptive intensity modeling and dual-branch architecture.
...
<details>
  <summary>Details</summary>
Motivation: Current SDE-based GRPO methods for flow matching models suffer from inefficient credit assignment and incompatibility with high-order solvers, limiting their practical application despite GRPO's promise in aligning generative models with human preferences.

Method: Proposes Neighbor GRPO which generates diverse candidate trajectories by perturbing initial ODE noise conditions and optimizes using a softmax distance-based surrogate leaping policy, with symmetric anchor sampling and group-wise quasi-norm reweighting for efficiency.

Result: Extensive experiments show Neighbor GRPO significantly outperforms SDE-based counterparts in training cost, convergence speed, and generation quality while preserving deterministic ODE sampling advantages.

Conclusion: Neighbor GRPO provides an effective alternative to SDE-based GRPO that maintains deterministic sampling benefits while achieving superior performance in aligning flow matching models with human preferences.
...
TL;DR: MatPedia is a foundation model for PBR materials that uses a joint RGB-PBR representation encoded as a 5-frame sequence, enabling unified text-to-material, image-to-material, and intrinsic decomposition tasks with superior quality and diversity.


<details>
  <summary>Details</summary>
Motivation: Existing material generation methods lack unified representations that bridge natural image appearance and PBR properties, leading to fragmented pipelines and inability to leverage large-scale RGB image data.

Method: Uses a novel joint RGB-PBR representation with two interdependent latents (RGB appearance and 4 PBR maps), formulated as a 5-frame sequence using video diffusion architectures trained on MatHybrid-410K dataset.
...
Conclusion: MatPedia provides a unified framework for PBR material generation that effectively bridges the gap between RGB appearance and physical properties through joint representation learning.
...
TL;DR: Proposes a Dual Branch Degradation Extractor Network for blind super-resolution that predicts unsupervised blur and noise embeddings to handle unknown degradations.


<details>
  <summary>Details</summary>
Motivation: Existing SISR methods perform poorly when actual degradation deviates from assumed fixed degradations like bicubic downsampling, requiring better blind SR approaches.

Method: Uses a dual branch network to extract unsupervised blur and noise embeddings, treating the degradation extractor as a regularizer and adapting SR network to these embeddings.

Result: Achieves state-of-the-art performance on multiple benchmarks for blind super-resolution.

Conclusion: The proposed method effectively handles unknown degradations in blind SR by explicitly modeling blur and noise components separately.
...
TL;DR: Edge-efficient food image synthesis from raw inputs using cooking state guidance and culinary similarity metrics


<details>
  <summary>Details</summary>
Motivation: Existing image-to-image generation methods produce unrealistic results or are too resource-intensive for edge deployment in cooking applications

Method: Propose recipe and cooking state guided generator with domain-specific Culinary Image Similarity (CIS) metric for training and monitoring

Result: Outperforms baselines with 30% FID improvement on their dataset and 60% on public datasets

Conclusion: The approach enables user-preferred visual targets rather than fixed presets while ensuring temporal consistency and culinary plausibility
...
TL;DR: SeeCLIP improves open-set domain generalization by enhancing CLIP with fine-grained semantic decomposition, duplex contrastive learning, and semantic-guided diffusion to better distinguish hard unknown classes.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with the dilemma between structural risk from known classes and open-space risk from unknown classes, particularly when dealing with 'hard unknowns' that visually resemble known classes.

Method: Semantic-aware prompt enhancement decomposes images into discriminative semantic tokens; duplex contrastive learning maintains separability from known classes while preserving semantic proximity; semantic-guided diffusion synthesizes challenging pseudo-unknowns.
...
Conclusion: SeeCLIP effectively addresses the known-unknown dilemma in open-set domain generalization through fine-grained semantic enhancement and challenging sample generation.
...
<details>
  <summary>Details</summary>
Motivation: 3DGS uses many Gaussian primitives causing high storage and computational costs. Existing pruning methods use manual criteria or extra parameters, leading to suboptimal results.

Method: Models survival pressure as regularization gradient field applied to opacity, allowing optimization gradients to autonomously determine pruning. Includes opacity decay with finite opacity prior to accelerate selection.
...
Conclusion: The proposed framework provides fully learnable pruning without human intervention, effectively reducing computational overhead while maintaining rendering quality.
...
<details>
  <summary>Details</summary>
Motivation: Plant diseases cause massive economic losses ($220B annually) and threaten food security. Current ensemble diversity metrics fail to identify optimal ensemble teams for accurate disease detection.
...
TL;DR: RadioKMoE: A knowledge-guided radiomap estimation framework combining Kolmogorov-Arnold Networks (KAN) with Mixture-of-Experts (MoE) for improved wireless signal coverage mapping.
...
Method: Uses KAN to predict initial coarse coverage maps leveraging physics model approximation, then employs MoE with specialized expert networks for precise radiomap estimation using environmental information.
...
Conclusion: The proposed RadioKMoE framework effectively integrates knowledge guidance with specialized expert networks to improve radiomap estimation performance in complex wireless environments.
...
TL;DR: DReX is a vision-only model that fuses self-supervised DINOv3 and supervised ResNet-50 features using attention to predict image complexity, achieving state-of-the-art performance without language data.


<details>
  <summary>Details</summary>
Motivation: To determine whether language information is necessary for visual complexity prediction and explore if visual features alone can achieve human-aligned complexity assessment.

Method: Fuses multi-scale hierarchical features from ResNet-50 with semantically rich representations from DINOv3 ViT-S/16 through a learnable attention mechanism.
...
Conclusion: Visual features alone are sufficient for human-aligned complexity prediction, and self-supervised transformers with supervised CNNs offer complementary benefits when properly fused.
...
<details>
  <summary>Details</summary>
Motivation: Real-world depth is often multi-layered due to transmissive materials, but existing depth estimation models are passive and only estimate static depth maps for the nearest surface, unlike humans who can actively shift focus.

Method: A steerable Vision Transformer conditioned on a scalar depth preference that dynamically adapts computation to focus on intended depth, trained on a new 500k multi-layered synthetic dataset capturing diverse see-through effects.
...
TL;DR: MVLR is a unified model that restores images from various weather degradations by combining a lightweight encoder-decoder with visual-language reasoning and memory retrieval.


<details>
  <summary>Details</summary>
Motivation: Reliable visual perception under adverse weather conditions (rain, haze, snow) is challenging but essential for autonomous driving and outdoor robots.

Method: Uses a lightweight encoder-decoder backbone coupled with Visual-Language Model for chain-of-thought inference and Implicit Memory Bank storing degradation patterns. VLM-generated priors query IMB to retrieve fine-grained degradation prototypes, which are adaptively fused with visual features via dynamic cross-attention.

Result: Extensive experiments on four severe-weather benchmarks show MVLR surpasses single-branch and Mixture-of-Experts baselines in PSNR and SSIM metrics.

Conclusion: MVLR offers a practical balance between model compactness and expressiveness for real-time deployment in diverse outdoor conditions.
...
<details>
  <summary>Details</summary>
Motivation: Current VLM evaluations overlook scenarios with multiple cultural cues, despite cultural robustness being crucial for supporting diverse societies. Existing benchmarks typically feature only singular cultural concepts per image.

Method: Introduced ConfusedTourist suite with image-stacking perturbations and image-generation-based variants to test VLM stability against mixed geographical cues. Used interpretability analyses to examine attention patterns.

Result: VLMs show critical vulnerability with heavy accuracy drops under simple perturbations. Failures stem from systematic attention shifts toward distracting cues, diverting focus from intended targets. Image-generation-based perturbations cause even worse performance.
...
TL;DR: FLUID is a training-free face de-identification framework that substitutes identity in diffusion model latent space using chemical-inspired semantic displacement with novel reagent losses for attribute preservation and identity suppression.


<details>
  <summary>Details</summary>
Motivation: To achieve effective face de-identification while preserving facial attributes, overcoming limitations of existing methods that often sacrifice attribute preservation for identity removal.

Method: Direct identity substitution in pretrained diffusion model latent space using optimization with reagent losses for attribute preservation and identity suppression, with linear and geodesic editing schemes on the latent manifold.

Result: Superior trade-off between identity suppression and attribute preservation on CelebA-HQ and FFHQ datasets, outperforming state-of-the-art methods in both qualitative and quantitative metrics.

Conclusion: FLUID provides an effective training-free solution for face de-identification that maintains attribute fidelity while achieving strong identity suppression through latent space manipulation.
...
TL;DR: Proposes a method to estimate circle of confusion maps directly from RGB images for realistic lens blur in mixed reality, eliminating need for camera parameters or depth information.


<details>
  <summary>Details</summary>
Motivation: Existing methods require camera parameters and scene depth for realistic lens blur rendering, which limits accessibility for ordinary users who lack such information.

Method: Directly estimates CoC map from RGB images, infers CoC values for virtual objects using linear relationship between signed CoC map and depth, and uses neural reblurring network for realistic lens blur rendering.
...
Conclusion: Provides flexible and practical solution for real-world mixed reality applications by eliminating dependency on camera metadata and scene depth information.
...
TL;DR: RacketVision is a novel dataset and benchmark for sports analytics (table tennis, tennis, badminton) with fine-grained racket pose annotations and ball positions, enabling research on ball tracking, racket pose estimation, and trajectory prediction.
...
Method: Created a dataset covering three racket sports with fine-grained annotations for racket pose and ball positions. Evaluated baselines for three tasks: ball tracking, racket pose estimation, and trajectory prediction. Used CrossAttention mechanism for effective multi-modal fusion.

Result: Evaluation revealed that naive concatenation of racket pose features degrades performance, but CrossAttention mechanism effectively unlocks their value, leading to trajectory prediction results that surpass strong unimodal baselines.

Conclusion: RacketVision provides a versatile resource for future research in dynamic object tracking, conditional motion forecasting, and multimodal analysis in sports, with CrossAttention identified as essential for effective multi-modal fusion.
...
<details>
  <summary>Details</summary>
Motivation: To enable painless creation of realistic 3D indoor scenes using only short text input, eliminating the need for manual layout design or panoramic image guidance.

Method: Uses hierarchical language-driven agent planners to parse text prompts into detailed scene descriptions, generates 3D point clouds, implements arrangement constraints for collision-free layouts, and employs AnyReach Sampling and Interval Timestep Flow Sampling for efficient rendering.
...
Conclusion: RoomPlanner provides an effective automatic solution for 3D room generation from text, achieving high-quality results with significantly reduced generation time compared to existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing computational pipelines for whole-slide image analysis lack explicit reasoning trajectories, resulting in opaque and unjustifiable predictions that don't mirror how human pathologists dynamically zoom, refocus, and self-correct during evidence collection.

Method: PathAgent uses three modules: Navigator for iterative micro-region localization, Perceptor for morphology visual cue extraction, and Executor for integrating findings into evolving natural language trajectories. The entire sequence forms an explicit chain-of-thought.

Result: Evaluated across five datasets, PathAgent shows strong zero-shot generalization, surpassing task-specific baselines in both open-ended and constrained visual question-answering tasks. Human pathologist collaboration confirms its promise as a transparent diagnostic assistant.

Conclusion: PathAgent successfully bridges the gap between computational pipelines and human expert reasoning by providing fully interpretable predictions through explicit reasoning trajectories, demonstrating strong clinical applicability as a transparent diagnostic tool.
...
TL;DR: OmniPT is a unified pedestrian tracking framework using LVLMs that can track, reference-based track, and generate semantic understanding of tracked objects through a multi-phase training approach.
...
Method: Three-phase training: RL for bounding box format, mid-training on pedestrian datasets, SFT on tracking datasets, then RL to improve tracking and instruction following.

Result: Experimental results show the method outperforms previous approaches on tracking benchmarks.

Conclusion: OmniPT successfully bridges the gap between LVLMs and instance-level tracking tasks, demonstrating superior performance through structured training phases.
...
TL;DR: RL-AD-Net is a reinforcement learning refinement framework that improves local geometric consistency in point cloud completions by operating in the latent space of a pretrained autoencoder and using selective adjustments guided by geometric fidelity metrics.


<details>
  <summary>Details</summary>
Motivation: Existing point cloud completion models generate globally plausible shapes but often leave local geometric inconsistencies, requiring a refinement approach to improve geometric fidelity.

Method: Uses RL in latent space of pretrained point autoencoder with global feature vectors, selective adjustments by RL agent, and PointNN selector for geometric consistency evaluation. Training is category-specific.

Result: Consistently improves completions across both training-style and random cropping scenarios on ShapeNetCore-2048, outperforming baseline completion networks.

Conclusion: RL-AD-Net provides effective, lightweight, modular, and model-agnostic refinement that enhances geometric consistency without requiring retraining of completion networks.
...
TL;DR: REArtGS++ improves articulated object reconstruction using planar Gaussian splatting and temporal geometry constraints, addressing limitations in handling screw-joints and multi-part objects.
...
Method: Models decoupled screw motion for each joint without type prior, jointly optimizes part-aware Gaussians with joint parameters through part motion blending, and introduces temporal geometry constraint using planar Gaussians with Taylor first-order expansion.
...
Conclusion: REArtGS++ provides a more generalizable solution for articulated object reconstruction through improved motion modeling and temporal geometric constraints.
...
Method: Uses BBDM for 2D MRI slice synthesis, retrieves similar CT slices from database via fine-tuned model, incorporates references through ControlNet, and applies spherical linear interpolation for rare retrieval failures.
...
Conclusion: ReBrain effectively addresses the challenge of synthesizing full brain MRI volumes from sparse CT scans through retrieval-augmented diffusion framework.
...
TL;DR: DiverseVAR addresses the diversity collapse issue in Visual Autoregressive (VAR) models by suppressing pivotal components in input and amplifying them in output, restoring generative diversity without additional training.
...
Method: Suppress the pivotal component of feature maps in model input and amplify it in model output to unlock inherent generative potential while maintaining high-fidelity synthesis.

Result: Substantially enhances generative diversity with negligible performance impact, effectively restoring VAR models' generative diversity without requiring retraining.

Conclusion: DiverseVAR provides a simple yet effective solution to diversity collapse in VAR models, enabling them to achieve both efficiency and diversity without additional training costs.
...
Method: Uses traversal orders of uniform spanning trees sampled from image patch lattices, obtained via breadth-first search with rejection sampling to ensure connected partial observations appear as sequence prefixes.

Result: STAR preserves postfix completion capability while maintaining sampling performance without requiring architectural changes to conventional autoregressive models.

Conclusion: Spanning tree traversal provides a structured randomized strategy that balances performance and flexibility for autoregressive image modeling and editing.
...
TL;DR: A novel category-agnostic articulated object reconstruction framework using planar Gaussian Splatting that achieves high-fidelity part-level surface reconstruction from sparse-view RGB images of a single state.


<details>
  <summary>Details</summary>
Motivation: Existing articulated object reconstruction methods require costly multi-stage and multi-view observations, which limits their practical application in daily environments where articulated objects are ubiquitous.

Method: Uses planar Gaussian Splatting with a Gaussian information field to select optimal viewpoints, compresses 3D Gaussians to planar Gaussians for normal/depth estimation, optimizes in coarse-to-fine manner with depth smooth regularization and few-shot diffusion, and incorporates part segmentation probability via back-projection of segmentation masks.

Result: Achieves higher-fidelity part-level surface reconstruction on both synthetic and real-world data compared to existing methods, demonstrating superior performance with only sparse-view RGB images from a single state.

Conclusion: The proposed framework successfully addresses the limitations of existing methods by enabling accurate articulated object reconstruction from minimal input data, making it more practical for real-world applications.
...
TL;DR: ReCoVAD is a training-free video anomaly detection framework that uses selective frame processing inspired by human nervous system pathways to reduce computational costs while maintaining state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing training-free VAD methods rely on dense frame-level inference, which incurs high computational costs and latency. The paper questions whether dense reasoning is necessary when using powerful pre-trained models.

Method: ReCoVAD uses a dual-pathway approach: (1) Reflex pathway with lightweight CLIP-based module for fast response using dynamic memory, and (2) Conscious pathway with medium-scale vision-language model for refined analysis and periodic review by large language model.

Result: ReCoVAD achieves state-of-the-art training-free performance while processing only 28.55% and 16.04% of frames compared to previous methods on UCF-Crime and XD-Violence datasets.

Conclusion: Sparse reasoning is sufficient for effective large-model-based video anomaly detection, significantly reducing computational requirements while maintaining high performance.
...
TL;DR: PACL uses pre-trained textual models to bridge the 'affective gap' in visual emotion recognition by leveraging emotional connections between images and texts through partitioned contrastive learning strategies.


<details>
  <summary>Details</summary>
Motivation: Visual emotion recognition suffers from an 'affective gap' where pre-trained visual models lack direct association between factual features and emotional categories, while textual models naturally express emotions clearly.

Method: Proposed Partitioned Adaptive Contrastive Learning (PACL) that separates different sample types and uses distinct contrastive learning strategies for each, dynamically constructing positive/negative pairs to exploit noisy social media data.
...
Conclusion: Leveraging knowledge from pre-trained textual models effectively enhances emotional perception of visual models, demonstrating the value of cross-modal learning for emotion recognition.
...
TL;DR: ChainV is a framework that dynamically integrates visual hints into multimodal reasoning to make reasoning chains shorter and more efficient while improving accuracy.
...
Method: ChainV performs coarse visual patch selection based on previous reasoning, refines it using attention intensity to find atomic visual hints, evaluates hint reliability with consistency-based mechanism, and incorporates pixel coordinates with Bernoulli stochastic process.

Result: Significant improvements in reasoning accuracy and efficiency, especially on math-intensive benchmarks. Achieves 2.3% improvement on MathVista, reduces inference latency by 51.4%, and shortens output tokens by 24.5%.

Conclusion: ChainV effectively makes multimodal reasoning shorter and better by dynamically integrating visual hints and adaptively adjusting self-reflection levels.
...
TL;DR: PEGS integrates physical priors and event stream enhancement with 3D Gaussian Splatting for deblurred motion reconstruction and recovery, achieving superior performance in large spatiotemporal scale motion reconstruction.
...
Method: Integrates physical priors with event stream enhancement in 3D Gaussian Splatting pipeline; uses triple-level supervision with acceleration constraint, event stream guidance, and Kalman regularizer; implements motion-aware simulated annealing strategy; creates first RGB-Event paired dataset for fast rigid motion.

Result: Superior performance in reconstructing motion over large spatiotemporal scales compared to mainstream dynamic methods.

Conclusion: PEGS framework effectively addresses challenges in rigid motion reconstruction through physical priors and event stream enhancement, demonstrating state-of-the-art performance in large-scale motion recovery.
...
TL;DR: Transitioning from 1D CCT-based colorimetric mapping to 2D chromaticity space with MLP improves color accuracy for non-Planckian LED lighting while maintaining real-time performance.


<details>
  <summary>Details</summary>
Motivation: Modern LED lighting deviates substantially from Planckian illuminants, exposing limitations of traditional CCT-based interpolation methods that assume illumination lies on the Planckian locus.

Method: Replace CCT interpolation with lightweight multi-layer perceptron (MLP) using 2D chromaticity features, trained via lightbox-based calibration with representative LED sources.

Result: Reduces angular reproduction error by 22% on average in LED-lit scenes, maintains backward compatibility with traditional illuminants, supports multi-illuminant scenes, and enables real-time deployment with negligible computational overhead.

Conclusion: 2D chromaticity-based MLP approach significantly improves colorimetric accuracy for modern non-Planckian lighting while preserving computational efficiency and compatibility with existing systems.
...
<details>
  <summary>Details</summary>
Motivation: Deploying state-of-the-art deep learning image compression models on resource-constrained FPGAs is challenging due to quantization-induced performance degradation and hardware inefficiency of floating-point models.

Method: Proposed Dynamic Range-Aware Quantization (DRAQ) with activation clipping and weight regularization, followed by progressive mixed-precision search and GDN-adapted channel pruning for FPGA optimization.

Result: DRAQ reduced BD-rate overhead from 30% to 6.3%, and hardware optimizations achieved over 20% computational complexity reduction with negligible RD performance impact, creating a state-of-the-art FPGA implementation.

Conclusion: The framework successfully bridges floating-point models to efficient integer-based FPGA implementations, achieving superior quality and efficiency compared to existing FPGA-based image compression solutions.
...
TL;DR: ODTSR is a one-step diffusion transformer that achieves state-of-the-art real-world image super-resolution with balanced fidelity and controllability using a novel noise-hybrid visual stream design.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based real-world image super-resolution methods face a trade-off: multi-step methods have low fidelity due to generative diversity, while one-step methods lack control flexibility due to fidelity-specific finetuning.

Method: Proposes ODTSR based on Qwen-Image with Noise-hybrid Visual Stream (NVS) that processes low-quality images with adjustable noise and consistent noise in parallel, plus Fidelity-aware Adversarial Training (FAA) for one-step inference.
...
Conclusion: ODTSR successfully balances fidelity and controllability in real-world image super-resolution through its noise-hybrid design and adversarial training approach.
...
TL;DR: CC-DiceCE loss function improves small lesion detection in medical image segmentation by increasing recall with minimal segmentation degradation, outperforming blob loss.


<details>
  <summary>Details</summary>
Motivation: Traditional loss functions like Dice under-segment small lesions because their small volume contributes negligibly to overall loss, requiring instance-wise approaches for better per-lesion evaluation.

Method: Introduce CC-DiceCE loss based on CC-Metrics framework and compare with blob loss, both benchmarked against DiceCE baseline using nnU-Net framework for standardized evaluation.

Result: CC-DiceCE increases detection recall with minimal segmentation degradation, though with slightly more false positives, and generally outperforms blob loss across multiple datasets.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in real-time remote sensing detection due to small object prevalence and the need to balance accuracy with computational efficiency.

Method: Uses Dual-branch Feature Extraction (DFE) with local feature extraction via depthwise separable convolutions and global context capture via vision transformer with gating mechanism. Includes Multi-scale Feature Fusion (MFF) with dilated convolutions and Global and Local Aggregate Feature Pyramid Network (GLAFPN) for enhanced small object detection.

Result: Achieves competitive performance on VisDrone2019 and NWPU VHR-10 datasets in terms of mAP, model size, and other key metrics.

Conclusion: DMG-YOLO effectively addresses small object detection challenges in remote sensing imagery while maintaining real-time performance and computational efficiency.
...
TL;DR: DiffRefiner is a two-stage trajectory prediction framework that combines discriminative trajectory proposals with generative diffusion refinement to achieve state-of-the-art performance in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Current generative methods for trajectory prediction rely on denoising human-crafted anchors or random noise, leaving significant room for improvement in accuracy and flexibility.

Method: Two-stage approach: 1) Transformer-based Proposal Decoder generates coarse trajectory predictions using predefined anchors, 2) Diffusion Refiner iteratively denoises and refines these predictions with a fine-grained denoising decoder for enhanced scene compliance.
...
Conclusion: The proposed framework effectively combines discriminative and generative approaches, with ablation studies validating the effectiveness of each component in improving trajectory prediction accuracy.
...
TL;DR: UI-Styler is a novel ultrasound image translation framework that addresses domain shifts between devices by transferring texture patterns while preserving structural content and ensuring class-specific semantic alignment.


<details>
  <summary>Details</summary>
Motivation: Ultrasound images vary across acquisition devices, causing domain shifts that degrade performance of fixed black-box downstream models when reused. Existing unpaired image translation methods overlook class-specific semantic alignment, leading to misaligned content-class mappings that impair diagnostic accuracy.

Method: UI-Styler uses a pattern-matching mechanism to transfer target domain texture patterns onto source images while preserving source structural content. It employs a class-aware prompting strategy guided by pseudo labels of the target domain to enforce accurate semantic alignment with diagnostic categories.

Result: Extensive experiments on ultrasound cross-device tasks show UI-Styler consistently outperforms existing unpaired image translation methods, achieving state-of-the-art performance in distribution distance and downstream tasks like classification and segmentation.

Conclusion: UI-Styler effectively addresses domain shifts in ultrasound imaging by combining texture pattern transfer with class-aware semantic alignment, making it suitable for practical scenarios with reused inference-blackbox models.
...
TL;DR: FireScope is a VLM-based reasoning-to-generation framework that uses multimodal data and reinforcement learning to predict wildfire risk maps with interpretable reasoning traces, demonstrating improved cross-continental generalization.


<details>
  <summary>Details</summary>
Motivation: Existing wildfire risk prediction methods lack causal reasoning and multimodal understanding needed for reliable generalization across different geographical regions.

Method: Proposed FireScope framework using Vision-Language Models (VLM) with reinforcement learning and visual supervision to predict risk rasters with complementary reasoning traces, trained on Sentinel-2 imagery, climate data, and expert-defined risk rasters.
...
Conclusion: Reasoning can ground raster prediction models, improving both generalization and interpretability, demonstrating that language-based reasoning can improve generalization in visual generation across continents.
...
TL;DR: Systematic evaluation of self-supervised representations for audio-visual deepfake detection across modalities and domains reveals they capture meaningful patterns but fail to generalize across datasets.


<details>
  <summary>Details</summary>
Motivation: To explore the underexplored potential of self-supervised representations for audio-visual deepfake detection, moving beyond isolated usage in complex architectures.

Method: Systematic evaluation across modalities (audio, video, multimodal) and domains (lip movements, generic visual content), assessing detection effectiveness, interpretability, and cross-modal complementarity.

Result: Most self-supervised features capture deepfake-relevant information that is complementary, with models attending to semantically meaningful regions rather than artifacts, but none generalize reliably across datasets.

Conclusion: Self-supervised representations learn meaningful patterns for deepfake detection but face fundamental challenges in achieving robust cross-domain generalization.
...
TL;DR: Proposes LENS-Net for nighttime traffic sign recognition, combining adaptive image enhancement with multimodal CLIP-GCNN classifier, and introduces INTSD dataset with 41 sign classes captured under various Indian nighttime conditions.


<details>
  <summary>Details</summary>
Motivation: Traffic sign recognition at night is challenging due to visual noise and lack of public nighttime datasets. Existing methods struggle with low illumination and fail to effectively use multimodal cues.

Method: 1) Introduces INTSD dataset with 41 traffic sign classes from diverse Indian regions under varying lighting/weather. 2) Proposes LENS-Net: adaptive image enhancement detector for joint illumination correction and sign localization, followed by multimodal CLIP-GCNN classifier using cross-modal attention and graph-based reasoning.

Result: LENS-Net surpasses existing frameworks, with ablation studies confirming effectiveness of key components. Dataset and code made publicly available.

Conclusion: The proposed approach addresses nighttime traffic sign recognition challenges through a comprehensive dataset and a robust multimodal framework that integrates illumination correction with semantic reasoning.
...
TL;DR: PostCam enables post-capture editing of camera trajectories in dynamic scenes through a query-shared cross-attention module that fuses 6-DoF camera poses and 2D rendered frames, achieving superior camera control precision and video quality.
...
Method: Introduces a query-shared cross-attention module that integrates 6-DoF camera poses and 2D rendered video frames into a unified representation, using a two-stage training strategy for coarse control learning and visual refinement.

Result: Outperforms state-of-the-art methods by over 20% in camera control precision and view consistency, while achieving the highest video generation quality on real-world and synthetic datasets.

Conclusion: PostCam provides a framework for accurate and flexible camera motion manipulation in dynamic scenes, significantly improving both control precision and visual fidelity in novel-view video generation.
...
TL;DR: A multi-stage noise-decoupling framework for hyperspectral image denoising that decomposes complex noise into explicit and implicit components, using pre-training and adaptive high-frequency feature extraction to handle real-world noise effectively.
...
Method: Proposes a multi-stage framework that decouples noise into explicit and implicit components, uses existing noise models for pre-training, introduces high-frequency wavelet guided network for implicit noise, and employs separate pre-training with joint fine-tuning strategy.
...
Conclusion: The proposed multi-stage noise-decoupling framework successfully addresses complex real-world noise in hyperspectral images through explicit-implicit noise decomposition and adaptive feature extraction, achieving superior denoising performance.
...
Conclusion: VLA-4D provides a unified framework that jointly enables spatially-smooth and temporally-coherent robotic manipulation through its 4D-aware visual and spatiotemporal action representations.
...
TL;DR: CA-SAM introduces a lightweight Alignment Layer and continual learning strategy to efficiently adapt SAM for medical image segmentation, achieving SOTA performance while reducing computational overhead and preventing catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Heterogeneous privacy policies make joint training infeasible, requiring continual learning from data streams. SAM's strong zero-shot priors are promising but its large parameter count and computational overhead challenge practical deployment in medical segmentation.

Method: Proposes Alignment Layer - a lightweight plug-and-play module that aligns encoder-decoder feature distributions to efficiently adapt SAM. Then introduces CA-SAM, a continual learning strategy that automatically adapts appropriate Alignment Layers to mitigate catastrophic forgetting while leveraging SAM's zero-shot priors.

Result: Achieves state-of-the-art performance across nine medical segmentation datasets under continual-learning scenario. Improves accuracy while reducing computation compared to standard SAM.

Conclusion: SAM paradigm is highly promising for medical segmentation when computational efficiency and performance are balanced. CA-SAM provides an effective solution for continual learning in medical imaging while maintaining strong performance on unseen datasets.
...
TL;DR: SING3R-SLAM is a globally consistent Gaussian-based RGB SLAM framework that combines local 3D reconstructions with a unified global Gaussian representation to address drift and redundancy issues in dense 3D mapping.
...
Conclusion: SING3R-SLAM provides an efficient and versatile 3D mapping solution that enables globally consistent reconstruction suitable for multiple downstream applications.
...
<details>
  <summary>Details</summary>
Motivation: Volumetric CT poses unique challenges including extreme token scaling, geometric anisotropy, and weak/noisy clinical supervision that make standard transformer and contrastive learning approaches ineffective for medical imaging.

Method: Joint optimization of local transformer for high-resolution volumetric feature extraction and global transformer for whole-scan context modeling; combines DINO-style self-distillation with SigLIP-based vision-language alignment using paired radiology reports.

Result: Consistently outperforms prior CT foundation models across multiple benchmarks in both zero-shot and fine-tuned settings, demonstrating high performance and generalizability using only openly available CT datasets.

Conclusion: SPECTRE establishes a scalable, open, and fully transformer-based foundation model for 3D medical imaging that achieves state-of-the-art performance without relying on private data.
...
TL;DR: A distortion-aware BEV semantic segmentation framework that directly processes fisheye images using Gaussian parameterization and differentiable splatting, achieving strong performance on parking and urban driving scenarios without requiring image undistortion.
...
Conclusion: The framework produces continuous, uncertainty-aware semantic maps directly from fisheye images without requiring undistortion or perspective rectification, demonstrating strong segmentation performance in complex scenarios.
...
TL;DR: Dual-domain Adaptation Networks efficiently adapt pre-trained SR models from synthetic to real-world datasets using spatial and frequency domain adaptation strategies.


<details>
  <summary>Details</summary>
Motivation: Realistic image super-resolution faces challenges due to limited real-world LR-HR data and complex degradation patterns. Pre-trained models from synthetic datasets offer valuable prior knowledge but need adaptation to real-world scenarios.

Method: Proposes Dual-domain Adaptation Networks with: 1) Spatial-domain adaptation via selective parameter updates and low-rank adaptation, 2) Frequency domain adaptation branch that combines spectral data and spatial features to infer HR frequency maps.
...
Conclusion: The proposed method effectively adapts pre-trained SR models to real-world scenarios, improving performance while leveraging prior knowledge from synthetic datasets.
...
TL;DR: QueryOcc is a self-supervised framework that learns continuous 3D semantic occupancy using 4D spatio-temporal queries, achieving state-of-the-art performance without manual 3D annotations.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for 3D scene understanding either rely on implicit 2D rendering consistency or discretized voxel grids, which limit spatial precision and scalability. Large-scale 3D annotation is expensive, creating need for self-supervised methods.

Method: Uses query-based framework with independent 4D spatio-temporal queries sampled across adjacent frames. Supports supervision from pseudo-point clouds (vision foundation models) or raw lidar data. Implements contractive scene representation for long-range reasoning with constant memory.

Result: Surpasses previous camera-based methods by 26% in semantic RayIoU on Occ3D-nuScenes benchmark while running at 11.6 FPS.

Conclusion: Direct 4D query supervision enables strong self-supervised occupancy learning, demonstrating superior performance over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for compact, transformation-invariant models in resource-constrained environments while preserving geometric equivariance properties, particularly relevant for satellite imagery analysis and geometric vision tasks.

Method: Combines group equivariant CNNs (G-CNNs) with structured pruning that preserves equivariant properties, using e2cnn library for rotation equivariance, adaptive fine-tuning with 2% accuracy drop trigger, and dynamic INT8 quantization in a comprehensive pipeline.

Result: Achieved 29.3% parameter reduction with significant accuracy recovery across EuroSAT, CIFAR-10, and Rotated MNIST benchmarks, demonstrating substantial compression while maintaining geometric robustness.

Conclusion: The framework successfully bridges group-theoretic network design with practical deployment constraints, providing reproducible optimization for equivariant models with particular relevance to satellite imagery and geometric vision applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing blind deconvolution methods for color images either convert to grayscale or process color channels separately, ignoring important inter-channel relationships that affect deblurring quality.

Method: Develops a quaternion fidelity term using quaternion convolution kernels with four components: one for overall blur and three unconstrained kernels for RGB channels to model interdependencies, with normalized quaternion kernel to preserve image intensity.

Result: Extensive experiments on real blurred color image datasets show the method effectively removes artifacts and significantly improves deblurring performance compared to existing approaches.

Conclusion: The proposed quaternion-based blind deconvolution method demonstrates strong potential as an effective tool for color image deconvolution by properly handling inter-channel relationships.
...
TL;DR: LVLMs suffer from multi-path hallucinations. A causal intervention framework identifies and targets critical hallucination heads across image-text pathways, reducing hallucinations across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models (LVLMs) are prone to hallucination despite strong performance, but current understanding of hallucination causes is limited.

Method: Propose a comprehensive intervention framework aligned with transformer causal architecture, analyzing image-to-input-text, image-to-output-text, and text-to-text pathways. Identify and intervene on critical hallucination heads tailored to discriminative and generative formats.

Result: Experiments across multiple benchmarks show consistent reduction of hallucinations across diverse alignment types.

Conclusion: Hallucinations in LVLMs arise from interplay of multiple causal pathways, and targeted intervention on critical heads effectively reduces them.
...
TL;DR: Proposes relevance feedback mechanisms to enhance visual search with VLMs, including pseudo-relevance feedback, generative relevance feedback, attentive feedback summarizer, and explicit feedback, improving retrieval performance by 1-5% across different model sizes.


<details>
  <summary>Details</summary>
Motivation: To improve visual search performance of VLMs without requiring fine-tuning or model scaling, by adapting traditional text-based search techniques like relevance feedback to multimodal retrieval.

Method: Four feedback strategies: 1) Classical pseudo-relevance feedback using top-ranked results, 2) Generative relevance feedback using synthetic captions, 3) Attentive feedback summarizer with custom transformer for multimodal features, 4) Explicit feedback using ground-truth captions as upper bound.

Result: GRF, AFS, and explicit feedback improve MRR@5 by 3-5% for smaller VLMs and 1-3% for larger ones on Flickr30k and COCO datasets. AFS shows robustness against query drift in iterative retrieval settings.

Conclusion: Relevance feedback consistently enhances VLM-based retrieval across different model sizes, enabling interactive and adaptive visual search without requiring model fine-tuning.
...
TL;DR: A novel semantic mask-guided diffusion approach for generating synthetic LiDAR point clouds by editing real-world scans, enabling cost-effective creation of diverse edge case scenarios for autonomous driving training.


<details>
  <summary>Details</summary>
Motivation: Current methods for acquiring diverse point cloud data for autonomous driving training are limited by the difficulty of capturing real-world edge cases and the computational expense of handcrafted 3D simulations, restricting system generalization and robustness.

Method: Transform LiDAR point clouds to 2D range view images, then use semantic mask conditioning with convex hull-based masks to guide diffusion-based generation, ensuring geometric consistency and realism while enabling semantic editing of object dimensions, orientations, and locations.
...
Conclusion: This provides a cost-effective and scalable solution for generating diverse LiDAR data, improving the robustness of autonomous driving systems.
...
<details>
  <summary>Details</summary>
Motivation: Multilingual T2I models often fail to preserve cross-lingual cultural consistency, producing culturally neutral or English-biased results despite having cultural knowledge.

Method: 1) A probing method to localize culture-sensitive neurons; 2) Inference-time cultural activation to amplify identified neurons; 3) Layer-targeted cultural enhancement that updates only relevant layers.

Result: Experiments on CultureBench show consistent improvements in cultural consistency while maintaining fidelity and diversity over strong baselines.

Conclusion: The proposed methods effectively address cultural bias in multilingual T2I models by targeting specific culture-sensitive neurons and layers, achieving better cultural alignment without compromising other quality metrics.
...
Result: MolSight achieves state-of-the-art performance in stereochemical optical structure recognition, with compact parameter size enhanced by GRPO algorithm.

Conclusion: The proposed framework effectively addresses stereochemical recognition challenges and demonstrates superior performance across diverse datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing finger pose estimation methods on touchscreen devices are limited to pitch and yaw angles using capacitive images, have reduced accuracy for large-angle inputs (>45°), and cannot estimate roll angle.

Method: Uses bimodal input combining capacitive images and fingerprint patches from under-screen fingerprint sensors to enable simultaneous and accurate prediction of comprehensive finger pose parameters.

Result: Outperforms previous SOTA methods with over 21% improvement in prediction performance, 2.5x higher task completion efficiency, and 23% better user operation accuracy in 12-person user studies.

Conclusion: BiFingerPose enables reliable roll angle estimation and improved pose parameter prediction, demonstrating practical superiority for enhancing authentication security and improving interactive experiences on touchscreen devices.
...
TL;DR: SpatialGeo enhances MLLMs' spatial reasoning by combining CLIP's semantic features with geometry features through hierarchical fusion, improving accuracy in spatial tasks with reduced memory usage.


<details>
  <summary>Details</summary>
Motivation: Most MLLMs have limited spatial reasoning ability due to lossy visual embeddings from CLIP that focus on instance-level semantics, lacking spatial awareness for 3D arrangements.

Method: Proposes hierarchical fusion of CLIP semantic features and geometry features from self-supervised learning via an adapter, trained with random feature dropping to prevent over-reliance on CLIP.

Result: Improves spatial reasoning accuracy by at least 8.0% in SpatialRGPT-Bench while reducing memory cost by approximately 50% during inference.

Conclusion: SpatialGeo effectively enhances MLLMs' spatial grounding capability through geometry-semantic fusion, achieving better performance with lower computational cost.
...
TL;DR: MuM extends masked autoencoding to multiple views for 3D vision, outperforming DINOv3 and CroCo v2 on geometric tasks.


<details>
  <summary>Details</summary>
Motivation: Most self-supervised learning focuses on semantic understanding rather than geometric reasoning. CroCo showed promise for 3D understanding but needs improvement.

Method: Extends MAE to arbitrarily many views with uniform masking across views and lightweight decoder with inter-frame attention.

Result: Outperforms state-of-the-art visual encoders DINOv3 and CroCo v2 on feedforward reconstruction, dense image matching, and relative pose estimation.

Conclusion: Multi-view masked autoencoding is an effective approach for learning geometric representations, simpler and more scalable than previous methods.
...
TL;DR: NoPe-NeRF++ is a local-to-global optimization method for training Neural Radiance Fields without pose priors, combining relative pose initialization, local joint optimization, and global bundle adjustment to achieve superior pose estimation and novel view synthesis.
...
Method: The approach uses relative pose initialization with explicit feature matching, followed by local joint optimization to enhance pose estimation, and then a global optimization phase with bundle adjustment incorporating geometric consistency constraints and feature trajectories.
...
Conclusion: NoPe-NeRF++ successfully combines local and global cues with NeRF, validating the design choices of the local-to-global optimization approach for pose-free NeRF training.
...
TL;DR: The paper presents a method to improve generative image models' synthesis of transparent objects by incorporating physical optics laws, specifically using Snell's Law to synchronize pixels and generate more realistic refraction effects.


<details>
  <summary>Details</summary>
Motivation: Generative image models perform poorly at synthesizing transparent objects because they don't properly model optical phenomena like refraction, reflection, absorption, and scattering. Refraction is particularly challenging as it creates constraints between pixel colors across the image.

Method: The approach synchronizes pixels within and outside object boundaries using Snell's Law of Refraction during generation. For surfaces visible only via refraction/reflection, it recovers their appearance by synchronizing with a second generated panorama image using the same warping and merging procedure.

Result: The method generates much more optically-plausible images that respect physical constraints compared to standard generative models, producing convincing transparent objects with accurate refraction effects.

Conclusion: By incorporating physical optics laws into the generation process, particularly Snell's Law for refraction synchronization, the approach significantly improves the realism of transparent object synthesis in generative image models.
...
<details>
  <summary>Details</summary>
Motivation: Existing video painting tutorials lack interactivity and personalization, while current generative models struggle with cross-media generalization and temporal consistency, making faithful reproduction of human creative workflows difficult.

Method: Uses diffusion models with cross-medium style augmentation, embedding multiple media into conditional space, and employs reverse-painting training strategy for smooth, human-aligned generation.

Result: Achieves strong results on LPIPS, DINO, and CLIP metrics for cross-media consistency, temporal coherence, and final-image fidelity. Introduces Perceptual Distance Profile (PDP) curve to quantitatively model human artistic progression.

Conclusion: The proposed framework successfully generates multi-media painting processes with consistent texture evolution and process transfer across styles, effectively mirroring human creative workflows.
...
<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of costly and time-consuming manual labeling in skeleton-based action recognition by developing a label-efficient approach that reduces dependency on large labeled datasets.

Method: Uses graph convolutional networks (GCNs) with a novel acquisition function that optimizes data representativity, diversity, and uncertainty to select the most informative subsets for labeling. Also extends the approach with an invertible GCN to map data from ambient to latent spaces for better data distribution capture.
...
Conclusion: The proposed method successfully achieves label-efficient skeleton-based action recognition by intelligently selecting the most informative data subsets for labeling, showing superior performance over existing approaches.
...
TL;DR: DSeq-JEPA improves on I-JEPA by introducing discriminative sequential prediction, where regions are predicted in order of visual importance rather than uniformly, combining JEPA's latent prediction with GPT-style sequential reasoning.


<details>
  <summary>Details</summary>
Motivation: I-JEPA treats all image regions uniformly and independently, lacking selective attention and sequential reasoning that mimics human visual perception which processes information from most to least informative regions.
...
Conclusion: DSeq-JEPA successfully bridges predictive and autoregressive self-supervised learning, demonstrating that sequential discriminative prediction leads to better visual representations than uniform region prediction.
...
TL;DR: Proposes Unified Attention-Mamba (UAM) backbone for cell-level radiomics analysis, achieving state-of-the-art performance in cell classification (78% accuracy) and tumor segmentation (80% precision) on H&E images.


<details>
  <summary>Details</summary>
Motivation: Cell-level radiomics features provide fine-grained tumor insights but are largely unexplored, with no dedicated backbone for radiomics data. Existing methods focus on slide/patch-level analysis, missing micro-level patterns.

Method: Introduces Unified Attention-Mamba (UAM) backbone that flexibly combines Attention and Mamba modules in a single architecture, eliminating manual ratio tuning. Extends to multimodal framework for joint cell classification and image segmentation.
...
Conclusion: UAM demonstrates effectiveness as a unified and extensible multimodal foundation for radiomics-driven cancer diagnosis, highlighting the promise of cell-level radiomics analysis for enhanced diagnostic accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Gaussian representations in occupancy estimation require many primitives leading to high memory usage and slow inference, making them unsuitable for real-time applications in automated driving.

Method: Uses superquadrics with multi-layer icosphere-tessellated Gaussian approximation to enable rasterization for supervision, reducing primitive count by 84% compared to Gaussian approaches.

Result: Achieves 75% memory footprint reduction, 124% faster inference, 5.9% mIoU improvement on Occ3D dataset, and enables real-time inference without temporal labels.

Conclusion: SuperQuadricOcc is the first occupancy model to enable real-time inference while maintaining competitive performance, demonstrating superquadrics as an effective alternative to Gaussian representations for efficient scene understanding.
...
TL;DR: ATAC is a simple yet effective test-time defense method that corrects adversarial perturbations in CLIP's embedding space using augmentation-induced drift vectors and angular consistency, achieving 50% higher robustness than previous methods with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: CLIP is highly vulnerable to adversarial attacks, and adversarial fine-tuning is too costly. Existing test-time defense strategies have limited robustness, so a more effective and efficient approach is needed.

Method: ATAC operates in CLIP's embedding space by calculating augmentation-induced drift vectors to infer semantic recovery direction, then corrects embeddings based on angular consistency of these latent drifts.

Result: ATAC consistently achieves remarkably high robustness across benchmarks, surpassing previous state-of-the-art methods by nearly 50% on average, with minimal computational overhead. It maintains robustness in extreme settings and against adaptive attacks.

Conclusion: ATAC represents an efficient novel paradigm for test-time adversarial defenses in CLIP's embedding space, demonstrating that effective protection can be achieved without costly fine-tuning.
...
Method: Integrates SDF into sparse voxel rasterization with two key techniques: (1) robust geometric initialization using visual geometry model, (2) spatial smoothness loss enforcing coherent relationships across parent-child and sibling voxel groups.

Result: Achieves strong reconstruction accuracy across various benchmarks with consistently speedy convergence.

Conclusion: SVRecon successfully combines sparse voxels with SDF for high-fidelity surface reconstruction, overcoming optimization challenges through careful initialization and smoothness constraints.
...
TL;DR: Proposes Non-Parametric Probabilistic Robustness (NPPR), a practical robustness metric that learns perturbation distributions from data instead of assuming fixed distributions, providing more conservative and realistic robustness estimates.
...
Method: Develops NPPR using Gaussian Mixture Model (GMM) with MLP heads and bicubic up-sampling to learn optimized perturbation distributions directly from data, covering both input-dependent and input-independent perturbation scenarios.

Result: Experiments on CIFAR-10, CIFAR-100, and Tiny ImageNet across multiple architectures show NPPR provides up to 40% more conservative (lower) probabilistic robustness estimates compared to assuming common perturbation distributions used in state-of-the-art methods.

Conclusion: NPPR is validated as a more practical robustness metric that addresses distributional uncertainty and provides more realistic robustness evaluations compared to existing probabilistic robustness formulations.
...
TL;DR: MorphSeek is a fine-grained representation-level policy optimization method for deformable image registration that uses stochastic Gaussian policy in latent space for efficient exploration and achieves improved performance with high label efficiency.


<details>
  <summary>Details</summary>
Motivation: Deformable image registration faces challenges from high-dimensional deformation space and limited voxel-level supervision. Existing reinforcement learning methods use coarse representations that can't capture spatially variant deformations well.

Method: Reformulates DIR as continuous optimization in latent feature space using stochastic Gaussian policy head. Uses unsupervised warm-up with weakly supervised fine-tuning via Group Relative Policy Optimization with multi-trajectory sampling.

Result: Achieves consistent Dice improvements across three 3D registration benchmarks (OASIS brain MRI, LiTS liver CT, Abdomen MR-CT) while maintaining high label efficiency with minimal parameter cost and low latency.

Conclusion: MorphSeek advances representation-level policy learning for spatially coherent and data-efficient deformation optimization, providing a backbone-agnostic and optimizer-agnostic solution for scalable visual alignment.
...
TL;DR: Proposes a methodology using generative models to create diverse synthetic face images and introduces the DIF-V dataset to address biases in face verification systems.


<details>
  <summary>Details</summary>
Motivation: Existing face datasets suffer from biases related to race, gender, and demographics, limiting fairness and effectiveness of face verification systems used in applications like online banking and secure access.

Method: Integrates advanced generative models to create varied and diverse high-quality synthetic face images that represent diverse facial traits while adhering to identity card photograph standards.

Result: Created the DIF-V dataset with 27,780 images of 926 unique identities. Analysis revealed existing verification models exhibit biases toward certain genders and races, and identity style modifications negatively impact model performance.

Conclusion: This work addresses dataset inequities, enriches discussion on AI diversity and ethics, and lays foundation for developing more inclusive and reliable face verification technologies.
...
TL;DR: MCMoE is a novel multimodal AQA framework that handles missing modalities at inference by dynamically reconstructing missing information and unifying unimodal/joint representation learning in single-stage training.
...
Method: Proposes adaptive gated modality generator to reconstruct missing modalities, modality experts for unimodal knowledge, and dynamic mixing of expert knowledge for joint representations. Uses complete multimodal features and unimodal knowledge to guide training.
...
Conclusion: MCMoE effectively addresses modality missing issues in multimodal AQA through unified representation learning and dynamic modality reconstruction.
...
TL;DR: MoE-ViT introduces a Mixture-of-Experts architecture for Vision Transformers in multi-channel imaging domains, using sparse channel interactions to achieve significant efficiency gains without performance loss.


<details>
  <summary>Details</summary>
Motivation: Current Vision Transformers face computational bottlenecks in multi-channel domains due to quadratic growth in attention from channel-wise comparisons, leading to excessive FLOPs and high training costs.

Method: Proposes MoE-ViT that treats each channel as an expert and uses a lightweight router to select only the most relevant experts per patch for attention, inspired by Sparse Mixture-of-Experts philosophy.

Result: Proof-of-concept experiments on JUMP-CP and So2Sat datasets show substantial efficiency gains without sacrificing performance, sometimes even enhancing it.

Conclusion: MoE-ViT provides a practical and attractive backbone for multi-channel imaging by addressing efficiency challenges while maintaining or improving performance.
...
TL;DR: A knowledge distillation framework using teacher networks fine-tuned on small bias-free datasets to mitigate shortcut learning in medical image analysis, achieving performance comparable to bias-free training even on out-of-distribution data.


<details>
  <summary>Details</summary>
Motivation: Deep learning models in medical imaging often learn shortcuts using spurious correlations instead of clinically meaningful features, leading to poor robustness and potential patient harm in high-risk applications.

Method: Proposed knowledge distillation framework where a teacher network fine-tuned on small task-relevant data guides a student network trained on large biased datasets. Different shortcut types (diffuse vs localized) are targeted at specific network layers.

Result: Consistent improvements over traditional Empirical Risk Minimization, augmentation-based, and group-based bias-mitigation approaches across CheXpert, ISIC 2017, and SimBA datasets using various architectures. Achieved comparable performance to bias-free training even on out-of-distribution test data.

Conclusion: The approach is practically applicable to real-world medical imaging where bias annotations are limited and shortcut features are difficult to identify beforehand, demonstrating effective mitigation of shortcut learning.
...
TL;DR: The paper introduces RS-FMD, a comprehensive database of 150+ remote sensing foundation models, and REMSA, an LLM-based agent that automatically selects appropriate models from natural language queries using in-context learning and transparent justifications.
...
Result: REMSA outperforms baselines including naive agents, dense retrieval, and unstructured RAG-based LLMs on a benchmark of 75 expert-verified query scenarios producing 900 configurations. It operates entirely on public metadata without accessing private data.

Conclusion: RS-FMD and REMSA provide effective solutions for automated remote sensing foundation model selection, addressing the challenge of model selection complexity in the field.
...
Method: Dual-teacher knowledge fusion architecture for collaborative optimization of clean feature preservation and robust feature enhancement, with dynamic weight allocation based on teacher confidence and adaptive sigmoid-based weighting to mitigate teacher bias.
...
Conclusion: MMT-ARD effectively enhances adversarial robustness of multimodal large models through multi-teacher collaboration, demonstrating superior performance and scalability.
...
TL;DR: SketchVerify is a training-free framework that improves video generation by planning and verifying motion trajectories before full synthesis, using a sketch-based verification loop to ensure physical plausibility and semantic alignment.


<details>
  <summary>Details</summary>
Motivation: Existing video generation methods use single-shot motion plans limited to simple motions or iterative refinement requiring multiple expensive diffusion calls, leading to computational inefficiency and poor motion quality.

Method: Proposes a sketch-verification framework that predicts multiple candidate motion plans, renders them as lightweight video sketches over static backgrounds, and ranks them using a vision-language verifier for semantic alignment and physical plausibility through iterative refinement.
...
Conclusion: The method demonstrates that scaling trajectory candidates enhances performance and provides an effective training-free approach for generating physically plausible and instruction-consistent motions in video generation.
...
TL;DR: Introduces Illustrator's Depth, a novel depth definition for decomposing flat images into editable, ordered layers, enabling applications like image vectorization, text-to-vector-graphics generation, 3D relief creation, and depth-aware editing.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of decomposing flat images into editable, ordered layers for digital content creation, inspired by an artist's compositional process.

Method: Proposes a neural network trained on curated layered vector graphics to predict layering from raster inputs, inferring a discrete, globally consistent layer index for each pixel.
...
Conclusion: By reframing depth as a creative abstraction rather than physical quantity, Illustrator's Depth provides a new foundation for editable image decomposition with powerful downstream applications.
...
TL;DR: This paper presents a study on using vision foundation models for unsupervised domain adaptation in lidar semantic segmentation, identifying key architectural choices that enable a single pretrained backbone to handle multiple domain shifts with frozen parameters.
...
Method: Leverages vision foundation models through unsupervised image-to-lidar knowledge distillation, focusing on lidar backbone architecture optimization and using frozen pretrained backbones with trainable MLP heads.

Result: Achieves state-of-the-art results in four challenging domain adaptation settings, demonstrating that a single pretrained backbone can address multiple domain shifts effectively.

Conclusion: The optimal approach involves pretraining a single backbone once, keeping it frozen, and training only an MLP head for semantic segmentation, providing robust performance across various lidar domain shifts.
...
TL;DR: A neural network-based odometry method using GPR B-scan images for precise Euclidean distance estimation, achieving 10.2% RMSE reduction over state-of-the-art methods.
...
Method: A custom neural network that extracts multi-scale features from consecutive GPR B-scan images and analyzes similarity/difference features to determine Euclidean distance traveled.

Result: Achieved overall weighted RMSE of 0.449 m across all datasets, outperforming state-of-the-art methods with 10.2% RMSE reduction.

Conclusion: The proposed neural network-based odometry method effectively leverages GPR B-scan image features for accurate distance estimation, demonstrating superior performance over existing approaches.
...
TL;DR: CWMDT transforms standard video diffusion models into counterfactual world models by creating digital twin representations, using LLMs for temporal reasoning, and generating counterfactual videos.


<details>
  <summary>Details</summary>
Motivation: Current world models focus on factual predictions, but emerging applications need counterfactual reasoning about "what if" scenarios like object removal or property changes.

Method: Three-step framework: 1) Construct digital twins as structured text representations of scenes, 2) Use LLMs to predict temporal effects of interventions, 3) Condition video diffusion models with modified representations.

Result: State-of-the-art performance on two benchmarks, demonstrating that digital twin representations provide powerful control for video simulation.

Conclusion: Alternative representations like digital twins enable effective counterfactual world modeling, offering better control for video forward simulation.
...
TL;DR: Radar2Shape: A denoising diffusion model for 3D shape reconstruction from partially observable high-frequency radar signals using frequency correlation with multiresolution shape features.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for radar modeling fail to represent arbitrary shapes or handle real-world radar signals with limited viewing angles, while optical 3D reconstruction methods struggle when treating radar signals as camera views.

Method: Two-stage approach: first learns a regularized latent space with hierarchical shape features, then diffuses into this latent space by conditioning on radar frequencies in a coarse-to-fine manner.
...
Conclusion: Radar2Shape enables effective 3D reconstruction from radar signals and the release of benchmark datasets supports future research for safe adaptation into real-world radar systems.
...
TL;DR: A deep learning method to estimate spine age from MRI images, using the spine age gap (SAG) as a biomarker for spine health.


<details>
  <summary>Details</summary>
Motivation: The spine is vulnerable to age-related degenerations that can be identified through MRI, and there's a need for automated assessment of spine health.

Method: Used UMAP and HDBSCAN clustering to identify age-related spine degeneration patterns, then trained a deep learning model on over 18,000 MRI series with ablation studies on data size, loss functions, and spine regions.

Result: The spine age gap (SAG) was associated with disc bulges, disc osteophytes, spinal stenosis, fractures, smoking, and physically demanding work.

Conclusion: SAG may be a useful biomarker for measuring overall spine health and detecting degenerative conditions.
...
TL;DR: The paper analyzes how scaling down LLMs affects multimodal models, finding visual capabilities are disproportionately impacted. It introduces Extract+Think approach with visual extraction tuning and step-by-step reasoning to maintain performance in smaller models.
...
Method: Introduces Extract+Think approach with visual extraction tuning (explicitly training to extract instruction-relevant visual details) and step-by-step reasoning to generate answers.

Result: LLM downscaling causes sharp performance drops in both visual perception and reasoning, with perception often matching or exceeding reasoning impact. The proposed method sets new standards for efficiency and performance.

Conclusion: The Extract+Think approach effectively addresses the bottleneck of visual capability loss in downscaled multimodal models, enabling efficient systems without sacrificing performance.
...
TL;DR: Video-R4 introduces visual rumination for text-rich video reasoning, enabling iterative frame selection, zooming, and re-encoding to improve fine-grained evidence capture.


<details>
  <summary>Details</summary>
Motivation: Current video QA models fail on fine-grained textual cues due to single-pass perception, while humans naturally pause, zoom, and re-read critical regions.

Method: Multi-stage rumination learning with supervised practice (Video-R4-CoT-17k) and RL (Video-R4-RL-30k), finetuning a 7B LMM via SFT and GRPO-based RL to learn visual operations.

Result: Video-R4-7B achieves SOTA on M4-ViteVQA and generalizes to document QA, slides QA, and generic video QA.

Conclusion: Iterative rumination is an effective paradigm for pixel-grounded multimodal reasoning across diverse visual domains.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for reconstructing intensity images from event cameras yield perceptually inferior results and struggle with model capacity scaling due to the ill-posed nature of absolute brightness reconstruction from sparse events.

Method: Proposes EvDiff with a surrogate training framework that eliminates paired dataset dependence, uses a temporally consistent EvEncoder, and performs only a single forward diffusion step to reduce computational costs for high-frame-rate video generation.

Result: EvDiff generates high-quality colorful videos from monochromatic event streams, outperforming existing approaches on both pixel-level and perceptual metrics in real-world datasets.

Conclusion: The method strikes a sweet spot between fidelity and realism, enabling scalable training with large image datasets and efficient high-quality video generation from event streams.
...
TL;DR: A native 3D editing framework that directly manipulates 3D representations in a single feed-forward pass, outperforming existing methods in quality, consistency, and instruction fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing instruction-guided 3D editing methods are either too slow (optimization-based) or suffer from inconsistent geometry and degraded quality (feed-forward approaches using multi-view 2D editing).
