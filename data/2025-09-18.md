...
Motivation: To create a more reliable and interpretable uncertainty-aware decision-making system that avoids confidently incorrect outcomes and provides auditable decisions through explicit supporting evidence.

Method: Retrieves proximal exemplars in embedding space for each test instance, fuses their predictive distributions using Dempster-Shafer theory, and uses the fused belief as a per-instance adaptive thresholding mechanism.

Result: Experiments on CIFAR-10/100 with BiT and ViT backbones show higher/comparable performance with fewer confidently incorrect outcomes and sustainable review load compared to entropy thresholds. Few evidences sufficient for gains.

Conclusion: Evidence-conditioned tagging provides more reliable and interpretable alternative to fixed prediction entropy thresholds for operational uncertainty-aware decision-making.
...
Conclusion: Hybrid quantum-classical architectures offer compelling advantages in accuracy, training efficiency, and parameter scalability, particularly for complex vision tasks, with performance gains scaling with dataset complexity.
...
<details>
  <summary>Details</summary>
Motivation: Expressway traffic congestion reduces travel efficiency and regional connectivity. Existing systems suffer from low vehicle perception accuracy under occlusion and loss of long-sequence dependencies in congestion forecasting.

Method: Optimized YOLOv11 to YOLOv11-DIoU by replacing GIoU Loss with DIoU Loss, improved DeepSort by fusing Mahalanobis and cosine distances, used Greenberg model for high-density scenarios, and built GRU-Attention model for congestion warning.

Result: YOLOv11-DIoU achieved 95.7% mAP (6.5% higher than baseline) with 5.3% occlusion miss rate. DeepSort reached 93.8% MOTA (11.3% higher than SORT). GRU-Attention achieved 99.7% test accuracy with ≤1 minute time error in 10-minute advance warnings.

Conclusion: The framework provides quantitative support for expressway congestion control with promising applications in intelligent transportation systems, demonstrating high accuracy in both vehicle detection and congestion prediction.
...
<details>
  <summary>Details</summary>
Motivation: To optimize real-time cloud-based parking service quality by automating the ground truth testing process and reducing human engineering work in analyzing crowd-sourced parking spot detections.

Method: Applied machine learning and image pattern recognition techniques, specifically convolutional neural networks, to automate the analysis process of ultrasonic sensor data for parking spot classification.

Result: Achieved a 99.58% reduction in human resource time for the testing and analysis process while maintaining high performance levels in parking spot detection.

Conclusion: The automation tool successfully optimized the parking service by significantly reducing manual engineering work, with potential for future development and broader application in similar analysis automation tasks.
...
<details>
  <summary>Details</summary>
Motivation: To comprehensively understand why VLMs work effectively for OOD detection, what advantages they have over single-modal methods, and how robust their behavior is - addressing gaps in current research.

Method: Systematic empirical analysis using in-distribution and OOD prompts, characterizing key operational properties in VLM embedding space, and quantifying performance comparisons with single-modal approaches.

Result: VLMs leverage rich semantic novelty for superior OOD detection over single-modal methods, show resilience to image noise but exhibit significant sensitivity to prompt phrasing variations.

Conclusion: The study provides structured understanding of VLM strengths and vulnerabilities in OOD detection, offering empirically-grounded guidance for developing more robust and reliable future VLM designs.
...
<details>
  <summary>Details</summary>
Motivation: To develop a geometric approach for analyzing discrete metric spaces and quantitatively assessing the quality of data representations produced by dimensionality reduction methods.

Method: Utilizes abstract notions of sectional curvature to construct curvature profiles that capture metric relations between triples of points and other points in discrete metric spaces.

Result: The curvature-based analysis can estimate intrinsic dimensionality of datasets and explore large-scale geometry of empirical networks, providing a quantitative measure for evaluating dimensionality reduction effectiveness.

Conclusion: The proposed curvature-based geometric profiling offers a powerful tool for analyzing discrete metric spaces and assessing data representation quality across various applications.
...
Result: Generated land cover maps and visualizations showing urban area changes over time, enabling monitoring of urbanization patterns in Nadi.

Conclusion: The framework successfully provides technical support for land use/land cover change detection and modeling, demonstrating effective monitoring of urban development in Fiji using machine learning and remote sensing technologies.
...
<details>
  <summary>Details</summary>
Motivation: To develop a robust and scalable solution for detecting foreign object intrusions in power transmission systems that can operate in real-time on low-cost edge hardware while handling occlusions and new object types.

Method: Three-stage approach: 1) YOLOv7 segmentation for object localization, 2) ConvNeXt-based feature extractor with triplet loss for discriminative embeddings, 3) Feature-assisted IoU tracker for resilient multi-object tracking. Uses mixed-precision inference for edge optimization and supports incremental updates without retraining.

Result: High accuracy and robustness demonstrated across diverse FOI scenarios on real-world surveillance and drone video datasets. Hardware benchmarks confirm practical deployment on NVIDIA Jetson devices.

Conclusion: The framework provides an effective, scalable solution for real-time foreign object intrusion detection in power systems, with proven edge deployment capabilities and incremental learning support for practical field applications.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for instruction-based image editing either rely on limited paired reference images or imprecise zero-shot VLMs, creating a need for more reliable and interpretable evaluation.

Method: Object-centric framework that decomposes images into semantic objects, synthesizes diverse editing instructions, and integrates VLMs with object detectors, feature extractors, and human preference models for multi-faceted evaluation.

Result: Combining VLMs with object detectors yields stronger agreement with human judgments compared to VLMs alone and CLIP-based metrics. The framework successfully identifies failure modes in existing editing models.

Conclusion: EdiVal-Agent provides a scalable, fine-grained evaluation approach that can inform development of next-generation editing models, with modular design allowing future tool integration.
...
<details>
  <summary>Details</summary>
Motivation: To create a universal 3D reconstruction backbone that can handle diverse 3D vision tasks with a single feed-forward model, eliminating the need for specialized models for each task.

Method: Uses a transformer-based architecture with factored representation of multi-view scene geometry (depth maps, local ray maps, camera poses, metric scale factor). Standardizes supervision across datasets and employs flexible input augmentation for joint training.
...
Conclusion: MapAnything demonstrates the feasibility of a universal 3D reconstruction backbone that efficiently handles multiple tasks through joint training, paving the way for more unified 3D vision systems.
...
Method: Uses VMamba backbone for RGB feature extraction, Semantic-Aware Feature Fusion module, LiDAR descriptors with semantics and geometry, cross-modal semantic attention in NetVLAD, and contrastive learning with Multi-View Semantic-Geometric Matching and Semantic Consistency Loss.
...
<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting generates massive data requiring compression, but existing methods rely on simple uniform scalar quantization which may not be optimal for scene-specific characteristics.

Method: Proposed scene-adaptive lattice vector quantization (SALVQ) that optimizes lattice basis for each scene and scales lattice density for multi-rate capability.

Result: SALVQ enhances rate-distortion performance with minimal modifications to existing systems and eliminates need for separate models for different compression levels.

Conclusion: SALVQ provides an efficient quantization solution that balances vector quantization benefits with low complexity, making it easily integrable into current 3DGS compression architectures.
...
<details>
  <summary>Details</summary>
Motivation: Understanding group-level social interactions is crucial for urban planning and designing socially vibrant environments, requiring interpretation of subtle visual cues beyond traditional object detection.

Method: A modular three-stage pipeline: (1) off-the-shelf human detection and depth estimation, (2) VLM-based reasoning for pairwise social affiliation classification, (3) lightweight spatial aggregation algorithm to localize socially connected groups.

Result: The paper introduces a new dataset of 100K urban street-view images annotated with bounding boxes for individuals and social groups, combining human-created labels and MINGLE outputs for semantic richness.

Conclusion: MINGLE addresses the challenge of detecting social group regions by integrating computer vision techniques with social reasoning, providing a foundation for future research in social interaction analysis from images.
...
<details>
  <summary>Details</summary>
Motivation: Existing bias discovery methods focus on output-level demographic distributions but fail to uncover deeper representational biases and concept entanglements in text-to-image models.

Method: Leverages cross-attention attribution maps to reveal structural entanglements between demographics and semantics, quantifies spatial concept entanglement using Intersection over Union (IoU), and implements bias mitigation through energy-guided diffusion sampling that modifies latent noise space.

Result: Shows that existing fairness interventions reduce output distribution gaps but fail to disentangle concept-level coupling, while BiasMap successfully mitigates concept entanglement in image generation.

Conclusion: BiasMap provides a deeper lens into hidden representational biases and offers effective mitigation that complements existing distributional bias approaches, addressing concept-level entanglement that current methods overlook.
...
<details>
  <summary>Details</summary>
Motivation: Existing image annotation tools require uploading pre-collected datasets, which limits on-demand pipelines and introduces unnecessary steps. This is problematic in laboratory environments where real-time data acquisition from instruments like microscopes is common.

Method: Developed a Python-based graphical user interface that integrates with imaging systems, featuring Bezier splines, binary masks, and non-destructive layers for precise annotation. Uses OpenCV and high-performance libraries like Numpy for object detection and matrix operations.

Result: LivePyxel enables real-time image annotation directly from video devices, providing high-performance editing tools and wide compatibility across imaging systems.

Conclusion: LivePyxel facilitates seamless data collection and labeling, accelerating AI model development in experimental workflows by eliminating the need for pre-collected datasets and supporting real-time annotation from laboratory instruments.
...
Method: Freezes pre-trained model parameters and trains a small h-transform network to learn conditional h-transform (1.42% parameters). Adds adaptive consistency loss combining consistency loss and denoising score matching loss for faster inference.
...
Conclusion: DEFT-VTON provides an efficient solution for adapting large pre-trained models to VTO with minimal parameter training and fast inference, making it suitable for real-world deployment constraints.
...
<details>
  <summary>Details</summary>
Motivation: Synthetic data is crucial for autonomous driving scenarios but introduces domain gaps between synthetic and real domains, particularly for VRU (vulnerable road user) recognition like pedestrians.

Method: Deploy data augmentation to generate custom traffic scenarios with VRUs, create a pipeline for augmenting Cityscapes dataset with virtual pedestrians, and develop a novel generative network architecture for adversarial learning of dataset lighting conditions to improve realism.

Result: The approach was evaluated on semantic and instance segmentation tasks, though specific performance metrics are not provided in the abstract.

Conclusion: The proposed method effectively addresses domain gap issues in synthetic data for autonomous driving by enhancing realism through lighting-condition learning and virtual pedestrian augmentation.
...
<details>
  <summary>Details</summary>
Motivation: Traditional deep learning approaches for medical image processing lack interpretability, and existing Kolmogorov-Arnold networks disrupt spatial structure by flattening features, limiting their effectiveness for image data.
...
Result: Outperformed other KAN-based methods in medical image enhancement (PSNR, TV metrics on IXI dataset) and segmentation (IoU, F1 scores on BUSI, GlaS, and CVC-ClinicDB datasets for breast cancer, glands, and polyps detection).

Conclusion: FunKAN successfully bridges theoretical function approximation with medical image analysis, providing a robust and interpretable solution suitable for clinical applications while preserving spatial structure in imaging data.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal methods treat all video content uniformly instead of emphasizing hateful components, and cannot systematically capture structured information in videos, limiting detection effectiveness.

Method: Proposes a dual-stream GNN that constructs instance graphs by separating videos into instances, extracts instance-level features, uses complementary weight graphs to assign importance weights highlighting hateful instances, and combines weights with features for classification.
...
<details>
  <summary>Details</summary>
Motivation: 3D scene understanding in colonoscopy requires automated depth estimation, but existing models lack temporal consistency across video sequences, limiting 3D reconstruction applications.

Method: Uses diffusion-based depth estimation with geometric priors learned from synthetic colonoscopy sequences, plus style transfer to adapt real clinical videos to synthetic training domain.
...
Conclusion: ColonCrafter provides clinically relevant applications for colonoscopy depth estimation, though full trajectory 3D reconstruction remains challenging.
...
Method: Proposes merging redundant 3D Gaussian primitives in voxel space based on geometric similarity to reduce GPU memory usage, and uses Patch-Grid point sampling for initialization to improve rendering accuracy.

Result: The approach reduces GPU memory usage without impacting runtime performance and improves rendering quality, as demonstrated through quantitative and qualitative evaluations on public datasets.

Conclusion: The proposed method effectively addresses the computational constraints of embedded platforms while maintaining and even enhancing 3D Gaussian Splatting performance and quality.
...
<details>
  <summary>Details</summary>
Motivation: Trajectory prediction models face distribution shifts in real-world deployment, but most OOD detection research focuses on computer vision tasks rather than trajectory-level detection. Existing methods don't adequately handle the mode-dependent and time-evolving nature of prediction errors.

Method: Proposes an adaptive framework that explicitly models error modes in trajectory prediction, building on quickest change detection formulation. The method accounts for mode-dependent distributions of prediction errors that evolve over time with dataset-specific dynamics.

Result: Substantial improvements in both detection delay and false alarm rates. Significantly outperforms prior uncertainty quantification and vision-based OOD approaches in both accuracy and computational efficiency across multiple real-world datasets.

Conclusion: The framework offers a practical path toward reliable, driving-aware autonomy by providing robust OOD detection for trajectory prediction in complex driving environments.
...
Method: Uses deep learning to compare Earth observation satellite image pairs from different dates to detect forest cover changes, and proposes a visual semantic model that automatically annotates detected changes with keywords extracted from scientific documents about the Amazon region.
...
Method: Integrates Google Gemini 2.5 Flash for visual feature extraction combined with natural language processing, coordinate verification mechanisms, probabilistic Gaussian modeling for anomaly distribution, multi-layered visualization techniques, and precise prompt engineering for structured clinical information extraction.

Result: Demonstrated high performance in anomaly detection across multiple modalities (CT, MRI, X-ray, Ultrasound) with location measurement achieving 80 pixels average deviation, featuring zero-shot learning capabilities and user-friendly Gradio interface.

Conclusion: Represents a significant advancement in automated diagnostic support and radiological workflow efficiency, though requires clinical validation and multi-center evaluation prior to widespread adoption.
...
<details>
  <summary>Details</summary>
Motivation: To generalize the CLAP algorithm beyond 2D localization to handle noise and uncertainty in various applications including 3D localization and image stitching.

Method: Extends the clustering-based CLAP algorithm to 3D localization and image stitching, comparing it with traditional methods like RANSAC and Hough transforms.

Result: CLAP provides a robust alternative to traditional outlier rejection methods and demonstrates wide applicability across different fields dealing with noise.

Conclusion: The generalized CLAP framework is a versatile tool for handling noise and uncertainty in multiple domains beyond its original 2D localization application.
...
<details>
  <summary>Details</summary>
Motivation: Traditional weakly supervised registration methods require anatomical priors like segmentation masks or landmarks, which are often unavailable in practice. The authors aim to leverage visual foundation models' strong representation learning capabilities to overcome this limitation.

Method: Uses SAM's pretrained image encoder to extract structure-aware feature embeddings, adds a lightweight 3D head for local deformation adaptation, and introduces a Hierarchical Feature Consistency Loss for coarse-to-fine feature matching.

Result: Significantly outperforms state-of-the-art methods with 2.68% improvement on ACDC cardiac dataset and 6.44% improvement on abdomen CT dataset.

Conclusion: SAMIR demonstrates that leveraging visual foundation models like SAM can effectively enhance medical image registration without requiring additional weak labels, providing a practical and powerful solution for clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: Accurate deforestation identification from satellite images is essential for geographical monitoring, but centralized approaches compromise data security and privacy when combining data from multiple clients/centers.

Method: Uses Federated Learning with FLOWER and RAY frameworks to enable distributed training across edge satellite centers. Employs YOLOS-small (Vision Transformer), Faster R-CNN with ResNet50, and Faster R-CNN with MobileNetV3 models trained on public datasets.

Result: The framework provides efficient client spawning and distributed learning workload execution while maintaining data privacy and security across different satellite centers.

Conclusion: Federated Learning offers a privacy-preserving alternative to centralized training for satellite image segmentation tasks, enabling collaborative deforestation detection without compromising client data security.
...
<details>
  <summary>Details</summary>
Motivation: Conventional two-view pose estimation methods are not metric (scale-ambiguous) and struggle with wide baselines, textureless surfaces, and reflective surfaces.

Method: Leverages metric monocular depth estimator and Gaussian scene reconstructor to create metric 3D GMMs for each image, then refines initial pose by optimizing a differentiable GMM alignment objective considering geometry, color, covariance, and semantic features.

Result: Outperforms both classical and state-of-the-art learning-based methods including MASt3R on Real-Estate10K dataset.

Conclusion: Demonstrates the potential of bridging single-view perception with multi-view geometry for robust metric relative pose estimation without requiring explicit 2D correspondences.
...
<details>
  <summary>Details</summary>
Motivation: Multiplication operations in convolutional neural networks are computationally intensive and energy-consuming, hindering deployment on mobile and edge devices. The authors aim to reduce computational costs by leveraging lookup tables instead of multiplications.

Method: The authors introduce a generic and efficient lookup operation that replaces multiplication of weights and activation values. They construct lookup tables in a differentiable manner and propose training strategies for end-to-end optimization. Lookup networks are developed for image classification, super-resolution, and point cloud classification tasks.

Result: Lookup networks achieve higher efficiency in energy consumption and inference speed while maintaining competitive performance compared to traditional convolutional networks. Extensive experiments show state-of-the-art performance on both classification and regression tasks across different data types.

Conclusion: The proposed lookup operation provides an effective alternative to multiplication operations in neural networks, enabling efficient deployment on resource-constrained devices without sacrificing performance across various computer vision tasks.
...
Method: Proposes a semantic visual projector that uses SAM-generated semantic superpixels as visual words, compressing them into visual tokens. Includes semantic superpixel positional embedding for geometry awareness and semantic superpixel aggregator to preserve both fine-grained details and global context.
...
Conclusion: The semantic visual projector approach effectively addresses visual token redundancy in MLLM-SAM frameworks for RIS, providing adaptive token compression based on scene complexity while minimizing semantic loss.
...
<details>
  <summary>Details</summary>
Motivation: Existing BEV segmentation methods work well with pinhole cameras but struggle with fisheye cameras due to severe geometric distortion, ambiguous multi-view correspondences, and unstable temporal dynamics.
...
Conclusion: FishBEV successfully addresses the unique challenges of fisheye cameras in BEV segmentation through specialized architectural components that handle distortion, uncertainty, and temporal dynamics effectively.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in medical image classification for resource-limited clinical settings, particularly with limited and diverse datasets, by developing lightweight and interpretable models that can learn directly from raw data without preprocessing.

Method: Developed three spline-based Kolmogorov-Arnold Network variants: SBTAYLOR-KAN (B-splines + Taylor series), SBRBF-KAN (B-splines + Radial Basis Functions), and SBWAVELET-KAN (B-splines + Morlet wavelet transforms) to capture local and global nonlinearities. Evaluated on brain MRI, chest X-rays, tuberculosis X-rays, and skin lesion images with cross-dataset validation and data reduction analysis.

Result: SBTAYLOR-KAN achieved up to 98.93% accuracy with balanced F1-score, maintaining over 86% accuracy using only 30% training data. Outperformed other models on imbalanced skin cancer dataset (68.22% accuracy). Used only 2,872 parameters vs. ResNet50's 24.18M parameters. Grad-CAM provided interpretable visualizations of relevant image regions.

Conclusion: The spline-based KAN framework provides a lightweight, interpretable, and generalizable solution for medical image classification, effectively addressing challenges of limited datasets and data-scarce scenarios in clinical AI applications with significantly reduced computational requirements.
...
<details>
  <summary>Details</summary>
Motivation: Generative models, especially diffusion models, can be misused to cheaply replicate artists' unique styles, threatening their creative labor and intellectual property.

Method: Identifies style-sensitive cross-attention layers through activation analysis, then updates only these selected layers to create StyleProtect defense against style mimicry.

Result: Effective protection against style replication by fine-tuned diffusion models while maintaining good imperceptibility, tested on WikiArt and anime datasets.

Conclusion: StyleProtect provides an efficient and lightweight solution to safeguard artistic styles from malicious diffusion model customization without extensive modifications.
...
<details>
  <summary>Details</summary>
Motivation: Self-supervised monocular depth estimation methods struggle with uncertainty in input data (low-texture or dynamic regions), leading to reduced depth accuracy. Existing motion-aware approaches often require inference-time overhead and additional labels.
...
Conclusion: UM-Depth effectively addresses uncertainty challenges in self-supervised monocular depth estimation without adding inference-time overhead or requiring additional labels, achieving superior performance on standard benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Existing query-based RVOS methods suffer from query selection bias where static textual queries are misled by distractors with similar appearance or motion patterns.

Method: Triple Query Former (TQF) factorizes referring queries into three specialized components: appearance query, intra-frame interaction query, and inter-frame motion query. Uses motion-aware aggregation modules for intra-frame interactions and inter-frame motion alignment.
...
Conclusion: Decomposing queries into specialized components with visual guidance and motion-aware aggregation effectively addresses query selection bias and improves RVOS performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches treat GREC and GRES independently, overlook joint training benefits, and neglect instance-aware capabilities and consistency between instance-level boxes and masks in generalized visual grounding tasks.

Method: Proposes InstanceVG framework using instance queries with prior reference points to unify joint predictions of instance-level boxes and masks, ensuring consistent multi-granularity predictions across points, boxes, and masks.

Result: Extensive experiments on ten datasets across four tasks show InstanceVG achieves state-of-the-art performance, significantly surpassing existing methods in various evaluation metrics.

Conclusion: InstanceVG is the first framework to simultaneously tackle both GREC and GRES with instance-aware capabilities, demonstrating superior performance and providing consistent multi-granularity predictions in generalized visual grounding.
...
Method: Proposes Adaptive Similarity Distribution Matching (A-SDM) to rectify unmatched positive pairs and Explicit Fine-grained Alignment (EFA) module for cross-modal verification and local alignment.
...
<details>
  <summary>Details</summary>
Motivation: Current text-driven color editing lacks precision and continuous control due to language ambiguity and unknown relationships between interpolation coefficients and resulting colors.
...
Conclusion: The method achieves finer-grained, continuous and controllable color editing with good performance in color continuity and controllability.
...
<details>
  <summary>Details</summary>
Motivation: Existing safety methods for text-to-image models rely only on LLM-based prompt refinement, ignoring visual outputs which can lead to unsafe images or unnecessary modifications to safe prompts.

Method: Proposes iterative prompt refinement algorithm using Vision Language Models (VLMs) to analyze both input prompts and generated images, leveraging visual feedback for more effective refinement. Also introduces a new dataset with textual and visual safety labels using multi-modal LLM.
...
Conclusion: The method provides a practical solution for generating safer text-to-image content by incorporating visual feedback into prompt refinement, addressing limitations of text-only safety approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing RAW data processing methods either use computationally heavy ISP networks or are limited by traditional ISP pipelines' representational capacity, making them unsuitable for resource-constrained devices.

Method: Proposes Task-Aware Image Signal Processing (TA-ISP) that predicts lightweight multi-scale modulation operators (global, regional, pixel scales) to reshape image statistics instead of using dense convolutional pipelines.
...
Conclusion: TA-ISP provides an efficient solution for RAW data processing that is well-suited for deployment on resource-constrained devices, offering better performance with reduced computational requirements.
...
<details>
  <summary>Details</summary>
Motivation: Existing image deraining techniques perform poorly under nighttime illumination due to spatial heterogeneity of rain distribution and light-dependent stripe visibility, which hampers nighttime surveillance and autonomous navigation.

Method: Proposed Nighttime Deraining Location-enhanced Perceptual Network (NDLPNet) with Position Perception Module (PPM) to capture spatial positional information and density distribution of rain streaks. Also constructed a night scene rainy dataset with 900 real-world image pairs.

Result: Extensive experiments show the method outperforms state-of-the-art methods in nighttime deraining tasks on both existing datasets and the new NSR dataset.

Conclusion: The proposed NDLPNet effectively addresses nighttime deraining challenges by capturing spatial contextual information and rain density distribution, providing superior performance for low-light image restoration.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for segmenting articulatory structures in rtMRI rely primarily on visual cues, but synchronized acoustic and phonological signals provide complementary context that can enhance precision and enrich visual information.

Method: The framework uses cross-attention fusion for dynamic feature alignment between video, audio, and phonological inputs, and incorporates a contrastive learning objective to improve segmentation performance even when audio is unavailable during inference.

Result: Achieved state-of-the-art performance with Dice score of 0.95 and HD_95 of 4.20 mm on USC-75 rtMRI dataset, outperforming both unimodal and multimodal baselines. Ablation studies confirmed the contributions of cross-attention and contrastive learning.

Conclusion: The results demonstrate the value of integrative multimodal modeling for accurate vocal tract analysis, showing that combining multiple modalities through cross-attention and contrastive learning significantly improves segmentation precision and robustness.
...
<details>
  <summary>Details</summary>
Motivation: Traditional codecs and learned methods struggle with subjective quality at high compression ratios, and existing generative approaches face challenges in visual fidelity and generalization, especially with mixed natural and AI-generated content.

Method: Uses pre-optimized encoder for compressed-domain representations, integrated with pretrained diffusion models via lightweight adapter and attentive fusion module. Includes distribution renormalization for enhanced reconstruction fidelity.

Result: Outperforms existing methods in visual fidelity at low bitrates, achieves up to 79% better compression than H.266/VVC, and provides efficient adaptation to different pretrained models with minimal retraining costs.

Conclusion: The proposed framework effectively leverages diffusion priors for superior compression performance while maintaining high visual quality, offering an adaptable solution for both AI-generated and broader content types.
...
<details>
  <summary>Details</summary>
Motivation: Current CoT reasoning in VLA models for autonomous driving introduces unnecessary computational overhead in simple scenarios without improving decision quality.

Method: Pretrained on AD scenarios with QA and trajectory data, then fine-tuned with two-mode dataset (fast answering without CoT and slow thinking with CoT). Uses Adaptive Think Reward with GRPO to reward selective CoT application.

Result: Achieves PDMS of 90.3 on Navsim benchmark, surpassing vision-only baseline by 1.7 points. Reduces inference time by 14% compared to always-Think baseline while improving performance.

Conclusion: AdaThinkDrive effectively balances accuracy and efficiency through adaptive reasoning, outperforming both never-Think and always-Think approaches.
...
<details>
  <summary>Details</summary>
Motivation: While accuracy in deepfake detection is important, there's increasing demand for precise localization of manipulated regions. Current approaches using forged region annotations often neglect the complementary nature of local details and global semantic context, and naive fusion strategies amplify noise and errors.

Method: Proposes independent prediction of manipulated regions using both local and global perspectives, then employs morphological operations to fuse the outputs for noise suppression and spatial coherence enhancement.
...
Conclusion: The proposed approach effectively addresses the challenges of precise deepfake localization by leveraging complementary local and global information with intelligent fusion strategies.
...
<details>
  <summary>Details</summary>
Motivation: Existing event camera methods require predefined time windows (introducing latency) or face computational challenges preventing real-time efficiency. Current approaches convert event streams into intermediate representations like frames, voxel grids, or point clouds, which are inefficient.

Method: Proposes a lightweight causal spatial neighborhood encoder to capture local geometric relations, followed by Mamba-based state space models for scalable temporal modeling with linear complexity. Includes an adaptive controller that adjusts processing speed based on event rate.

Result: The method achieves direct processing of raw event streams without intermediate representations, enabling efficient temporal modeling with linear complexity and adaptive processing that balances window latency and inference latency.

Conclusion: The proposed Variable-Rate Spatial Event Mamba overcomes limitations of existing event processing methods by eliminating intermediate representations and providing efficient, adaptive processing with linear complexity, making it suitable for real-time high-speed vision applications.
...
<details>
  <summary>Details</summary>
Motivation: DiT-based video generation suffers from high latency due to sequential denoising, and existing acceleration methods either compromise quality or fail to properly reuse intermediate features.

Method: Proposes Block-Wise Caching (BWCache) that dynamically caches and reuses DiT block features across timesteps, with a similarity indicator to trigger reuse only when feature differences are below a threshold.

Result: Achieves up to 2.24x speedup while maintaining comparable visual quality across several video diffusion models.

Conclusion: BWCache effectively reduces computational redundancy in DiT blocks without training, enabling faster video generation while preserving visual fidelity.
...
<details>
  <summary>Details</summary>
Motivation: Hybrid spacecraft pose estimation pipelines perform well on synthetic data but suffer from synthetic-to-real domain gap. Existing unsupervised domain adaptation methods underperform when limited labeled target data is available.

Method: Supervised Domain Adaptation framework based on Learning Invariant Representation and Risk (LIRR) paradigm that jointly optimizes domain-invariant representations and task-specific risk using both labeled synthetic and limited labeled real data.

Result: Extensive experiments on SPEED+ benchmark show consistent outperformance over baselines. With only 5% labeled target data, matches or surpasses oracle performance trained on larger data fractions. Framework is lightweight, backbone-agnostic, and computationally efficient.

Conclusion: Provides a practical pathway toward robust and deployable spacecraft pose estimation in real-world space environments by effectively bridging the synthetic-to-real domain gap with minimal labeled real data requirements.
...
<details>
  <summary>Details</summary>
Motivation: Overcome limitations of existing retrieval-based UAV localization approaches including poor real-time performance, environmental sensitivity, and limited generalization in dynamic/temporal environments.

Method: Integrates semantic features from UAV and satellite imagery using semantic weighting mechanism and optimized particle filtering architecture. Uses Multi-Altitude Flight Segments dataset (MAFS) for variable altitude scenarios.

Result: 10x computational efficiency gain over feature extraction methods, global positioning errors below 10 meters, rapid 4-DoF pose estimation within seconds using low-resolution satellite maps.

Conclusion: The proposed SWA-PF method effectively addresses persistent challenges in vision-based UAV localization, providing efficient and accurate positioning in GNSS-denied environments with strong generalization capabilities.
...
Conclusion: MFM provides a simple, efficient, and generalizable strategy for unsupervised domain-adaptive semantic segmentation that tightly couples auxiliary objectives with the primary task without inference overhead.
...
<details>
  <summary>Details</summary>
Motivation: Spectral classification is valuable for various applications, but current state-of-the-art models like 1D-Justo-LiuNet suffer performance degradation with limited training data.

Method: Investigated MiniROCKET and HDC-MiniROCKET for spectral classification, which use well-engineered feature extraction without trainable parameters in that part.
...
Conclusion: MiniROCKET-based approaches provide a robust solution for spectral classification, particularly when training data is limited, making them suitable for practical applications.
...
TL;DR: Semi-MOE is a novel multi-task Mixture-of-Experts framework for semi-supervised histopathology image segmentation that uses three specialized experts and dynamic pseudo-labeling to address noisy labels from ambiguous gland boundaries.
...
Conclusion: The method shows the potential of MoE-based architectures for advancing semi-supervised segmentation, effectively addressing noisy pseudo-labels through specialized experts and adaptive loss balancing.
...
TL;DR: VHBench-10 benchmark reveals visual encoders have distinct hallucination patterns. VisionWeaver routing network dynamically combines multiple experts to reduce hallucinations in LVLMs.


<details>
  <summary>Details</summary>
Motivation: Object hallucination in Large Vision-Language Models limits real-world use. Different visual encoders have diverse training paradigms and inductive biases, leading to varied hallucination behaviors that aren't captured by existing coarse benchmarks.

Method: Introduced VHBench-10 with ~10,000 samples across 10 fine-grained hallucination categories. Proposed VisionWeaver - a Context-Aware Routing Network that uses global visual features to generate routing signals, dynamically aggregating features from multiple specialized experts.

Result: Evaluations confirmed encoders exhibit unique hallucination characteristics. VisionWeaver significantly reduces hallucinations and improves overall model performance compared to simple feature fusion approaches.

Conclusion: Fine-grained hallucination analysis is crucial. VisionWeaver's dynamic expert routing effectively mitigates object hallucination in LVLMs by leveraging complementary strengths of multiple visual encoders.
...
TL;DR: The paper challenges the assumption that uncorrelated views automatically learn meaningful representations, proposing Consistent View Alignment to explicitly structure latent space and improve downstream task performance.
...
Method: Proposes Consistent View Alignment method that aligns representations from different data views to capture complementary information without introducing false positives, using self-supervised learning.
...
Conclusion: Structured view alignment is critical for learning effective representations, and explicit alignment of complementary information from different views significantly improves representation quality.
...
TL;DR: SpecDiff is a training-free multi-level feature caching strategy that uses self-speculative future information along with historical information to accelerate diffusion models, achieving 2.7-3.2x speedup with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Existing feature caching methods rely solely on historical information, which constrains accuracy and speed performance. The authors aim to overcome the speedup-accuracy trade-off by incorporating future information through self-speculation.

Method: Proposes SpecDiff with two main components: (1) feature selection algorithm using self-speculative and historical information to determine dynamic importance scores, and (2) multi-level feature classification algorithm that categorizes tokens by importance score differences and uses multi-level feature calculation.

Result: Achieves average 2.80x, 2.74x, and 3.17x speedup on Stable Diffusion 3, 3.5, and FLUX respectively with negligible quality loss compared to RFlow on NVIDIA A800-80GB GPU.

Conclusion: SpecDiff successfully overcomes the speedup-accuracy trade-off bottleneck by merging speculative and historical information, pushing the Pareto frontier of speedup and accuracy in efficient diffusion model inference.
...
<details>
  <summary>Details</summary>
Motivation: Traditional dataset distillation methods primarily capture low-level visual features but neglect high-level semantic and structural information in images, limiting their effectiveness.
...
Result: Extensive experiments confirm the effectiveness of the method, showing improved performance over traditional dataset distillation techniques.

Conclusion: The proposed EDITS framework successfully exploits implicit textual semantics to achieve enhanced dataset distillation, providing a more comprehensive approach that captures both visual and semantic information.
...
TL;DR: LamiGauss is a novel reconstruction algorithm for X-ray Computed Laminography that uses Gaussian Splatting with a specialized detector-to-world transformation to achieve high-quality 3D reconstructions from highly sparse-view projections.
...
Method: Combines Gaussian Splatting radiative rasterization with a dedicated detector-to-world transformation model incorporating laminographic tilt angle, plus an initialization strategy that filters out common laminographic artifacts from preliminary reconstruction.

Result: Achieves superior performance using only 3% of full views compared to iterative methods optimized on full datasets, as demonstrated on both synthetic and real datasets.

Conclusion: LamiGauss enables accurate and efficient reconstruction with limited data by effectively optimizing directly from sparse projections and preventing redundant Gaussians from being allocated to false structures.
...
TL;DR: DAM4SAM enhances SAM2 with distractor-aware memory and introspection management to improve visual object tracking by reducing drift to distractors and improving redetection after occlusion.


<details>
  <summary>Details</summary>
Motivation: Memory-based video segmentation methods like SAM2 perform well on segmentation tasks but are not optimized for visual object tracking where distractors pose significant challenges.

Method: Proposes a distractor-aware drop-in memory module and introspection-based management method for SAM2, and constructs DiDi dataset for distractor analysis.

Result: Outperforms SAM2.1 on 13 benchmarks, sets new SOTA on 10 benchmarks. Integration with EfficientTAM improves performance by 11% and matches SAM2.1-L quality, while EdgeTAM integration delivers 4% boost.

Conclusion: The distractor-aware memory module effectively addresses tracking challenges with distractors and shows good generalization across different tracking architectures.
...
TL;DR: EvHand-FPV is a lightweight framework for 3D hand tracking from a single event camera, achieving 89% reduction in parameters and FLOPs while improving accuracy for XR applications.


<details>
  <summary>Details</summary>
Motivation: Frame-based hand tracking methods struggle with accuracy, latency, and energy efficiency in resource-constrained XR devices. Event cameras offer microsecond-level temporal resolution with milliwatt-level power consumption.
...
Conclusion: Demonstrates accurate and efficient egocentric event-based hand tracking suitable for on-device XR applications, addressing the limitations of traditional frame-based methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing prototype generation methods in FS-PCS suffer from randomness in sampling and distributional gaps between learnable tokens and support features, limiting segmentation performance.

Method: WARM sandwiches cross-attention between whitening and coloring transformations - whitening aligns support features to prototypical tokens before attention, then coloring restores the original distribution to attended tokens.

Result: Achieves state-of-the-art performance with significant margin on multiple FS-PCS benchmarks, demonstrating effectiveness through extensive experiments.

Conclusion: The proposed WARM module effectively resolves distribution misalignment in attention-based prototype generation, enabling robust semantic relationship capture and representative prototype creation for few-shot 3D point cloud segmentation.
...
TL;DR: SRC framework iteratively calibrates rationale-answer alignment in LVLMs through rationale fine-tuning, candidate generation, pairwise scoring, and preference fine-tuning, improving reasoning consistency and performance.
...
Method: 1) Rationale fine-tuning to require rationale before answer without explicit prompts 2) Generate diverse candidate responses 3) Pairwise scoring with R-Scorer model to evaluate rationale quality and factual consistency 4) Confidence-weighted preference curation for preference fine-tuning

Result: Significant improvements in perception, reasoning, and generalization across multiple benchmarks

Conclusion: Rationale-oriented alignment is crucial for exploring LVLMs' potential, and SRC framework effectively addresses rationale-answer misalignment issues.
...
TL;DR: AntiPure is a protective perturbation method that resists purification attacks on diffusion models, using frequency and timestep guidance to maintain adversarial noise through purification processes.
...
Conclusion: AntiPure effectively exposes purification vulnerabilities and provides robust protection against image misuse in diffusion models by maintaining persistent perturbations through purification processes.
...
TL;DR: Noise Level Guidance (NLG) is a simple, efficient method that optimizes initial noise in diffusion models without extra data, networks, or backpropagation, improving image quality and prompt adherence.


<details>
  <summary>Details</summary>
Motivation: Random Gaussian noise initialization in diffusion models causes variations in output quality and prompt adherence. Existing optimization approaches require additional resources like datasets, networks, or backpropagation, limiting practicality.

Method: Propose Noise Level Guidance (NLG) that refines initial noise by increasing its alignment with general guidance. It requires no additional training data, auxiliary networks, or backpropagation, and works with both conditional and unconditional diffusion models.

Result: Extensive experiments on five standard benchmarks show that NLG enhances output generation quality and improves adherence to input conditions while maintaining computational efficiency.

Conclusion: NLG provides a practical and scalable enhancement to diffusion models that seamlessly integrates with existing guidance methods, establishing it as an effective noise-level optimization approach.
...
TL;DR: PairTally is a new benchmark dataset for evaluating fine-grained visual counting, featuring 681 high-resolution images with two object categories that require distinguishing subtle differences. Current models struggle with intent-driven counting in complex scenes.


<details>
  <summary>Details</summary>
Motivation: Existing visual counting models lack the ability to perform fine-grained, intent-driven counting in complex scenes where users need to count specific object types based on subtle distinctions.

Method: Created PairTally dataset with 681 high-resolution images containing two object categories (both inter-category and intra-category settings), then benchmarked various state-of-the-art models including exemplar-based methods, language-prompted models, and large vision-language models.

Result: Current models struggle to reliably count what users intend, especially in fine-grained and visually ambiguous cases, despite recent advances in counting capabilities.

Conclusion: PairTally provides a new foundation for diagnosing and improving fine-grained visual counting systems, highlighting the limitations of existing approaches and the need for better intent-driven counting models.
...
<details>
  <summary>Details</summary>
Motivation: To enable efficient transfer of multimodal semantics to lightweight vision-only detectors without modifying the teacher model or requiring textual input during inference, making it suitable for real-world deployment.

Method: Uses a translation module to map student features into joint space, with training guided by dual-objective loss enforcing both local alignment and global relational consistency at object level rather than dense or global alignment.
...
Conclusion: MOCHA proves effective for transferring multimodal knowledge to compact detectors, enabling performance on par with larger models while maintaining efficiency for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Drone detection in visually complex environments is challenging due to background clutter, small object scale, and camouflage effects. Generic detectors like YOLO perform poorly in cluttered environments with low object-background separability.

Method: Enhanced YOLO-FEDER FusionNet framework integrating generic object detection with camouflage detection. Improvements include systematic training data composition (large-scale synthetic data + real samples), feature fusion strategies, and backbone design evaluation across multiple YOLO configurations.

Result: Integration of intermediate FEDER features with backbone upgrades led to significant improvements. Best configuration (YOLOv8l backbone + DWD module FEDER features) achieved up to 39.1 percentage points FNR reduction and 62.8 percentage points mAP increase at IoU 0.5 threshold.

Conclusion: The enhanced YOLO-FEDER FusionNet framework effectively addresses drone detection challenges in complex environments through systematic integration of camouflage detection techniques, improved training data, and optimized feature fusion strategies.
...
<details>
  <summary>Details</summary>
Motivation: To advance multimodal understanding and reasoning capabilities by building upon SAIL-VL with improved data quality, training efficiency, and architectural innovations for comprehensive vision-language tasks.
...
Result: State-of-the-art performance across 106 datasets, top results on challenging reasoning benchmarks (MMMU, MathVista), and SAIL-VL2-2B ranks first among open-source models under 4B parameters on OpenCompass leaderboard.

Conclusion: SAIL-VL2 demonstrates competitive multimodal capabilities and serves as an efficient, extensible foundation for the open-source community, advancing vision-language understanding from perception to complex reasoning.
...
TL;DR: PROFUSEme is a multi-modal AI model that fuses clinical, radiology, and pathology data to predict biochemical recurrence in prostate cancer patients after radical prostatectomy, achieving superior performance with C-index of 0.861 on internal validation and 0.7103 on external challenge data.


<details>
  <summary>Details</summary>
Motivation: 30% of prostate cancer patients experience biochemical recurrence after radical prostatectomy, which is associated with increased mortality. Early accurate prediction at surgery time would enable better clinical decisions and improved patient outcomes.

Method: PROFUSEme uses intermediate fusion of multi-modal embeddings (clinical, radiology, pathology data) combined with Cox Proportional Hazard regressors to learn cross-modal interactions for BCR prediction.

Result: Superior performance compared to late fusion configurations, with mean C-index of 0.861 (σ=0.112) on internal 5-fold nested cross-validation and 0.7103 on CHIMERA 2025 challenge validation leaderboard hold-out data.

Conclusion: The intermediate fusion approach effectively integrates multi-modal data for accurate biochemical recurrence prediction in prostate cancer, demonstrating strong performance on both internal and external validation datasets.
...
TL;DR: Wan-Animate is a unified framework for character animation and replacement that can animate characters from images using reference videos or replace characters in videos while maintaining environmental consistency.


<details>
  <summary>Details</summary>
Motivation: To create a unified solution for high-fidelity character animation and seamless character replacement that can precisely replicate expressions, movements, and environmental lighting conditions.
...
Conclusion: Wan-Animate provides an effective unified framework for character animation and replacement tasks, with plans to open-source the model weights and source code.
...
<details>
  <summary>Details</summary>
Motivation: Current MOT algorithms perform poorly in real-world low-quality video scenarios due to image deterioration, limiting their practical applications in challenging environments.

Method: Proposes a tri-branch architecture using vision-language models to extract global visual semantic information, with MOT-Adapter to adapt semantics for tracking tasks and Visual Semantic Fusion Module to improve feature fusion effectiveness.

Result: The method outperforms existing approaches by 8-20% in tracking performance metrics for low-quality videos while maintaining robust performance in conventional scenarios.

Conclusion: VSE-MOT successfully addresses the challenges of low-quality video tracking by leveraging visual semantic enhancement through vision-language models, demonstrating significant improvements in real-world applications.
...
TL;DR: AD-DINOv3 is a novel vision-language framework that adapts DINOv3 for zero-shot anomaly detection, addressing feature misalignment and global semantic bias through multimodal contrastive learning with lightweight adapters and an anomaly-aware calibration module.


<details>
  <summary>Details</summary>
Motivation: Traditional zero-shot anomaly detection methods rely on CLIP, but vision foundation models like DINOv3 offer strong transferable representations. However, adapting DINOv3 for anomaly detection faces challenges with domain bias and global semantic bias that cause misinterpretation of subtle anomalies.

Method: Uses DINOv3 as visual backbone to extract patch tokens and CLS token, with CLIP text encoder for normal/abnormal prompts. Introduces lightweight adapters in both modalities to bridge domain gap, and an Anomaly-Aware Calibration Module (AACM) to guide CLS token attention to anomalous regions.
...
Conclusion: AD-DINOv3 demonstrates superiority as a general zero-shot anomaly detection framework, effectively overcoming domain bias and semantic misinterpretation challenges.
...
Method: Two strategies: (1) EMA-guided pseudo supervision framework generating reliable segment-level masks, (2) class-aware cross-modal agreement loss for audio-visual embedding alignment.
...
TL;DR: Proposed CSMoE model integrates Soft MoE mechanism into remote sensing foundation models to improve computational efficiency while maintaining performance across scene classification, semantic segmentation, and image retrieval tasks.
...
Result: CSMoE achieves more than twice the computational efficiency of existing RS FMs while maintaining competitive performance across all experiments. Superior trade-off between representational capacity, accuracy, and computational efficiency.

Conclusion: The Soft MoE adaptation effectively creates computationally efficient remote sensing foundation models without sacrificing performance, demonstrating practical applicability for RS tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional histopathological diagnosis requires multiple chemical stains that are time-consuming, labor-intensive, and environmentally taxing. Virtual staining offers a faster, tissue-conserving alternative but faces challenges due to reliance on well-aligned paired data, which is difficult to obtain because chemical staining processes distort tissue structures.
...
<details>
  <summary>Details</summary>
Motivation: Digital beautification through social media filters raises concerns about facial image reliability and effectiveness of automated face analysis, particularly for manipulation detectors that distinguish genuine vs manipulated content.
...
TL;DR: The MARS2 2025 Challenge introduces a multimodal reasoning benchmark with two datasets (Lens and AdsQA) and three competition tracks to advance state-of-the-art in multimodal machine learning and LLMs.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive benchmark that brings together different approaches in multimodal machine learning and LLMs, addressing the need for real-world and specialized scenarios to broaden multimodal reasoning applications.

Method: Released two tailored datasets (Lens for general reasoning in 12 daily scenarios, AdsQA for domain-specific reasoning in advertisement videos), evaluated 40+ baselines including generalist MLLMs and task-specific models, and established three competition tracks: VG-RS, VQA-SA, and VR-Ads.
...
Conclusion: The MARS2 2025 Challenge successfully provides a platform for advancing multimodal reasoning research through comprehensive benchmarking and community engagement, with ongoing updates and future events planned.
...
TL;DR: The paper investigates why abstract images made of primitive shapes underperform compared to raster images in deep learning, introduces HAID dataset with multiple abstraction levels, and evaluates vision systems on classification, segmentation, and detection tasks.


<details>
  <summary>Details</summary>
Motivation: To understand the performance gap between abstract primitive shape images and traditional raster images in conveying visual semantic information to deep learning models, and to explore how much high-level semantic content can be captured at different abstraction levels.
...
Result: Provides comprehensive comparative study between rasterized and abstract image representations, analyzing performance across different vision tasks at varying abstraction levels.

Conclusion: Discusses whether abstract images can be considered as an effective format for conveying visual semantic information and contributing to vision tasks, based on the empirical evaluation results.
...
Conclusion: The geometric-aware teacher-student framework effectively addresses domain shift accumulation across multiple geometric spaces in BEV perception, demonstrating significant improvements in cross-domain 3D object detection.
...
TL;DR: STEP is a hybrid token-reduction framework that combines dynamic patch merging and early token pruning to significantly improve Vision Transformer efficiency for semantic segmentation while maintaining accuracy.
...
Result: Reduces token count by 2.5x, computational cost by 2.6x, throughput by 3.4x with ViT-Large backbone. Full STEP achieves 4x computational complexity reduction, 1.7x inference speed gain with <2.0% accuracy drop. 40% of tokens can be halted early.
...
TL;DR: The paper introduces Dense Video Understanding (DVU) with Gated Residual Tokenization (GRT) to enable efficient high-FPS video comprehension by reducing tokenization overhead while preserving temporal details, and proposes DIVE benchmark for dense temporal reasoning evaluation.


<details>
  <summary>Details</summary>
Motivation: Current VLLMs and benchmarks rely on low-frame-rate sampling, discarding dense temporal information needed for tasks requiring precise temporal alignment like lecture comprehension, while avoiding the high computational cost of processing every frame.
...
Result: Experiments on the proposed DIVE benchmark show GRT outperforms larger VLLM baselines and scales positively with FPS, demonstrating efficient high-FPS video understanding capabilities.

Conclusion: The work highlights the importance of dense temporal information and shows that GRT enables scalable, efficient high-FPS video understanding while addressing the computational challenges of processing high-frame-rate videos.
...
TL;DR: Cineaste benchmark for long-form movie understanding with 3,119 multiple-choice questions from 1,805 scenes across 200 movies, testing fine-grained narrative reasoning where current models struggle (63.15% top accuracy).


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on short-clip recognition or template-based questions, lacking evaluation of deep narrative comprehension and fine-grained reasoning over long-form content.

Method: Created dataset using GPT-4o to generate diverse questions from visual descriptions, captions, scene titles, and summaries, with two-stage filtering (Context-Independence and Contextual Veracity) to ensure quality.

Result: Existing MLLMs perform poorly on Cineaste, with long-range temporal reasoning identified as the primary bottleneck - top open-source model achieves only 63.15% accuracy.
...
TL;DR: GenExam is the first multidisciplinary text-to-image exam benchmark with 1,000 samples across 10 subjects, designed to rigorously test AI models' ability to integrate knowledge, reasoning, and generation through exam-style prompts.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on understanding/reasoning or world knowledge illustration, but neglect rigorous drawing exams that require integrated capabilities. There's a need for comprehensive evaluation of models' ability to handle exam-style image generation tasks.

Method: Created a benchmark with 1,000 samples across 10 subjects using exam-style prompts organized under a four-level taxonomy. Each problem includes ground-truth images and fine-grained scoring points for precise evaluation of semantic correctness and visual plausibility.
...
Conclusion: GenExam provides a rigorous assessment framework for evaluating models' integrated knowledge, reasoning, and generation capabilities, offering valuable insights for advancing toward general AGI through exam-style image generation tasks.
