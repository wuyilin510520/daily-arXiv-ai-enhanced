...
TL;DR: RMLP regularization improves DINOv2 ViT interpretability while maintaining performance, especially in medical imaging with domain shifts.


<details>
  <summary>Details</summary>
Motivation: ViTs like DINOv2 have low-informative patch tokens that reduce attention map interpretability, particularly problematic in medical imaging where domain shifts degrade both performance and transparency.

Method: Introduce Randomized-MLP (RMLP) regularization, a contrastive learning-based method that encourages semantically aligned representations during DINOv2 fine-tuning.
...
TL;DR: Sparse semantic tokens are sufficient for autonomous driving planning, outperforming traditional world modeling approaches without needing exhaustive scene reconstruction.


<details>
  <summary>Details</summary>
Motivation: Challenge the assumption that exhaustive scene modeling is necessary for autonomous driving, proposing a more efficient approach using minimal semantic tokens instead of computationally intensive future scene generation.

Method: Use perception-informed BEV representations with sparse semantic tokens, conditioning trajectory decoding on predicted future tokens without explicit reconstruction loss.

Result: Achieved 0.548 m ADE without future prediction (comparable to 0.75 m on nuScenes), improved to 0.479 m ADE with future token conditioning (12.6% improvement), and observed temporal fuzziness for adaptive planning.

Conclusion: "Token is all you need" principle enables planning through imagination rather than reaction, representing a paradigm shift from world reconstruction to understanding.
...
TL;DR: This paper introduces a holistic AI platform that combines OCR, deep learning, LLMs, and graph analytics to overcome limitations of traditional OCR systems and achieve superior invoice data extraction quality and consistency.


<details>
  <summary>Details</summary>
Motivation: Conventional OCR systems struggle with variant invoice layouts, handwritten text, and low-quality scans due to strong template dependencies that limit flexibility across different document structures. Newer approaches using deep learning and LLMs show promise but need integration.

Method: The proposed method combines multiple AI technologies: OCR for initial text recognition, deep learning models (CNNs and Transformers) for layout analysis, Large Language Models for sophisticated entity recognition and semantic comprehension, and graph analytics for contextual relationship mapping.

Result: The holistic platform achieves unprecedented extraction quality and consistency, with greater contextual sensitivity and much higher accuracy rates than older approaches through visual NER capabilities and hybrid architectures.

Conclusion: Integrating OCR, deep learning, LLMs, and graph analytics in a unified platform provides maximum scalability and minimal human intervention while overcoming the limitations of conventional OCR systems for invoice processing.
...
TL;DR: Vision-language models with in-context learning can assess additive manufacturing quality with minimal samples, achieving accuracy comparable to traditional ML while providing interpretable rationales.


<details>
  <summary>Details</summary>
Motivation: Traditional vision-based quality assessment requires expensive data collection and model training. This paper aims to eliminate the need for large application-specific datasets by leveraging VLMs' reasoning capabilities.

Method: Used in-context learning (ICL) with vision-language models (Gemini-2.5-flash and Gemma3:27b) for quality assessment in wire-laser direct energy deposition. Explored different sampling strategies for ICL to optimize performance with limited samples.

Result: ICL-assisted VLMs achieved quality classification accuracies similar to traditional machine learning models using only minimal samples. VLMs also generated human-interpretable rationales, enhancing transparency.

Conclusion: ICL-assisted VLMs can effectively address application-specific manufacturing tasks with limited data, providing both high accuracy and interpretable rationales for improved decision transparency.
...
TL;DR: EVLP is a unified multimodal generation framework that integrates textual reasoning and visual generation for long-horizon manipulation tasks, addressing inconsistencies in current multimodal planning methods.


<details>
  <summary>Details</summary>
Motivation: Current methods fail to provide unified multimodal planning, leading to inconsistencies in complex embodied long-horizon manipulation tasks that require both textual logical reasoning and visual-spatial imagination.

Method: Three key components: 1) Unified multimodal generation framework integrating semantic and spatial features, 2) Dynamic perception pretraining with bidirectional alignment using inverse/forward dynamics tasks, 3) Reinforced supervised fine-tuning with spatial logic alignment between text actions and generated images.
...
Conclusion: EVLP provides an effective solution for multimodal planning in complex embodied manipulation tasks through unified generation and reinforced alignment strategies.
...
TL;DR: MCFCN is a multi-view clustering method that learns consensus graphs end-to-end using fusion-consensus graph convolutional networks to address noise interference and cross-view consistency issues in existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view clustering methods neglect inherent topological structures, suffer from noise interference in graph structures, and have limitations in cross-view consistency and handling hard-to-distinguish samples.

Method: Proposes MCFCN with view feature fusion model and Unified Graph Structure Adapter to learn consensus graphs end-to-end, using Similarity Matrix Alignment Loss and Feature Representation Alignment Loss to optimize view-specific graphs and preserve cross-view topological consistency.

Result: MCFCN achieves state-of-the-art performance on eight multi-view benchmark datasets, verified through extensive qualitative and quantitative experiments.

Conclusion: The method effectively addresses limitations in existing multi-view clustering approaches by learning consensus representations through unified graph adaptation and alignment losses, improving clustering performance.
...
<details>
  <summary>Details</summary>
Motivation: Multi-task learning for panoptic perception increases model parameters and complexity, making deployment on on-board devices difficult. Need efficient compression method to maintain performance while reducing model size.

Method: Combines task-aware safe pruning (Taylor-based channel importance + gradient conflict penalty) with task head-agnostic distillation that transfers intermediate backbone and encoder features from teacher to student model.
...
Conclusion: Combining pruning and knowledge distillation provides effective compression solution for multi-task panoptic perception in autonomous driving systems.
...
TL;DR: Proposes FilletRec, a lightweight graph neural network using pose-invariant intrinsic geometric features for automated recognition and simplification of fillet features in CAD models, achieving state-of-the-art accuracy with high efficiency.


<details>
  <summary>Details</summary>
Motivation: Automated fillet feature recognition and simplification is critical for CAE analysis but remains challenging due to limitations of traditional rule-based methods and existing deep learning models that suffer from poor generalization and low accuracy on complex fillets.

Method: Construct a large-scale benchmark dataset for fillet recognition, then develop FilletRec - a lightweight graph neural network using pose-invariant intrinsic geometric features (like curvature) to learn fundamental geometric patterns for high-precision recognition.

Result: FilletRec surpasses state-of-the-art methods in both accuracy and generalization while using only 0.2%-5.4% of baseline model parameters, demonstrating high efficiency. The framework successfully integrates geometric simplification for complete automated workflow.

Conclusion: The proposed end-to-end data-driven framework effectively addresses fillet recognition challenges through specialized dataset creation and efficient geometric feature learning, achieving superior performance with minimal computational resources.
...
TL;DR: Foundation VLMs can perform few-shot object detection on biomedical microscopy images using in-context learning, with performance improving with more support examples up to 6 shots.
...
Method: Introduce Micro-OD benchmark with 252 images across 11 cell types, evaluate 8 VLMs under few-shot conditions, and implement hybrid FSOD pipeline combining detection head with VLM-based few-shot classifier.
...
Conclusion: In-context adaptation provides a practical path for microscopy applications, and the Micro-OD benchmark offers a reproducible testbed for advancing open-vocabulary detection in biomedical imaging.
...
### [10] [Automatic Extraction of Road Networks by using Teacher-Student Adaptive Structural Deep Belief Network and Its Application to Landslide Disaster](https://arxiv.org/abs/2511.05567)
*Shin Kamada,Takumi Ichimura*

Main category: cs.CV

TL;DR: Proposes adaptive DBN with ensemble learning for road network detection from aerial images, achieving 89% accuracy and demonstrating disaster response applications.


<details>
  <summary>Details</summary>
Motivation: Road maps contain complex features requiring high representation power for accurate detection, and there's need for rapid disaster response road detection.

Method: Adaptive structural learning of RBM/DBN with neuron/layer generation algorithms, using Teacher-Student ensemble learning model for RoadTracer.

Result: Detection accuracy improved from 40.0% to 89.0% on average across seven major cities, with successful application to landslide disaster road detection.

Conclusion: The adaptive DBN model effectively handles complex road features and enables lightweight deployment for real-time disaster response applications.
...
### [11] [Do Street View Imagery and Public Participation GIS align: Comparative Analysis of Urban Attractiveness](https://arxiv.org/abs/2511.05570)
*Milad Malekzadeh,Elias Willberg,Jussi Torkko,Silviya Korpilo,Kamyar Hasanzadeh,Olle Järv,Tuuli Toivonen*

Main category: cs.CV

TL;DR: Study compares Street View Imagery (SVI) and Public Participation GIS (PPGIS) for capturing urban perceptions, finding only partial alignment due to SVI's inability to capture non-visual experiential factors.


<details>
  <summary>Details</summary>
Motivation: To understand how different data sources reflect human experiences of urban environments and investigate the comparability between SVI-based perceived attractiveness and residents' reported experiences via PPGIS.

Method: Used participant-rated SVI data and semantic image segmentation to train a machine learning model predicting perceived attractiveness, then compared predictions to PPGIS-identified locations using strict and moderate agreement criteria.

Result: Partial alignment between datasets - moderate threshold agreement reached 67% for attractive and 77% for unattractive places, but strict threshold dropped to 27% and 29% respectively. Non-visual cues significantly contributed to mismatches.

Conclusion: SVI offers scalable visual proxy but cannot fully substitute experiential richness captured through PPGIS. Both methods are valuable but serve different purposes, requiring integrated approach for holistic urban perception capture.
...
### [12] [C3-Diff: Super-resolving Spatial Transcriptomics via Cross-modal Cross-content Contrastive Diffusion Modelling](https://arxiv.org/abs/2511.05571)
*Xiaofei Wang,Stephen Price,Chao Li*

Main category: cs.CV

TL;DR: C3-Diff is a cross-modal contrastive diffusion framework that enhances low-resolution spatial transcriptomics using histology images as guidance, achieving superior performance on multiple datasets and downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Current spatial transcriptomics platforms suffer from low resolution, limiting understanding of spatial gene expression. Super-resolution approaches that integrate histology images with gene expressions are promising but face challenges in modeling interactions between these modalities.

Method: Proposes C3-Diff framework with: 1) refined contrastive learning for modal-invariant and content-invariant features, 2) noise-based information augmentation on feature hypersphere to address low sequencing sensitivity, 3) dynamic cross-modal imputation training to mitigate data scarcity.
...
Conclusion: C3-Diff successfully enhances spatial transcriptomics resolution using histology guidance, advancing AI-enhanced biotechnology for biomedical research and clinical applications.
...
### [13] [Video Text Preservation with Synthetic Text-Rich Videos](https://arxiv.org/abs/2511.05573)
...
<details>
  <summary>Details</summary>
Motivation: Existing T2V models struggle with generating legible and coherent text in videos, often failing to render even short phrases correctly, and previous solutions are computationally expensive.

Method: Generate text-rich images using T2I diffusion model, animate them into videos using text-agnostic I2V model, then use these synthetic video-prompt pairs to fine-tune pre-trained T2V model (Wan2.1) without architectural changes.

Result: Shows improvement in short-text legibility and temporal consistency, with emerging structural priors for longer text.
...
### [14] [Elements of Active Continuous Learning and Uncertainty Self-Awareness: a Narrow Implementation for Face and Facial Expression Recognition](https://arxiv.org/abs/2511.05574)
*Stanislav Selitskiy*

Main category: cs.CV

TL;DR: A self-awareness mechanism using a supervising ANN to monitor uncertainty in a CNN ensemble for face recognition, triggering human help when uncertainty is high.


<details>
  <summary>Details</summary>
Motivation: To emulate reflection and self-correction in AI by modeling high-level intelligence concepts at the narrow ML level, enabling systems to recognize their own uncertainty.

Method: A supervising ANN observes activation patterns of a CNN ensemble for face recognition, detects high uncertainty, and triggers active learning to request human assistance.

Result: The system can identify when it's uncertain about predictions and proactively seek human help, demonstrating elements of agency and self-awareness in ML algorithms.

Conclusion: Self-awareness mechanisms can be implemented in narrow ML systems to improve reliability through uncertainty detection and human-in-the-loop correction.
...
### [15] [DiffSwap++: 3D Latent-Controlled Diffusion for Identity-Preserving Face Swapping](https://arxiv.org/abs/2511.05575)
...
Method: Proposes a diffusion pipeline that incorporates 3D facial latent features during training, conditioning the denoising process on both identity embeddings and facial landmarks to enhance geometric consistency.

Result: Extensive experiments on CelebA, FFHQ, and CelebV-Text show DiffSwap++ outperforms prior methods in preserving source identity while maintaining target pose and expression, validated through biometric-style evaluation and user study.

Conclusion: DiffSwap++ demonstrates that incorporating 3D-aware representations in diffusion models significantly improves face swapping quality and identity preservation, offering a more effective approach than previous methods.
...
### [16] [Beyond Softmax: Dual-Branch Sigmoid Architecture for Accurate Class Activation Maps](https://arxiv.org/abs/2511.05590)
...
<details>
  <summary>Details</summary>
Motivation: Class Activation Mapping (CAM) and its extensions suffer from fundamental distortions due to reliance on softmax classifiers, including additive logit shifts that bias importance scores and sign collapse that conflates excitatory and inhibitory features.

Method: Clone the classification head into a parallel branch with per-class sigmoid outputs, freeze the original softmax head, and fine-tune only the sigmoid branch with class-balanced binary supervision. This preserves both magnitude and sign of feature contributions.
...
Conclusion: The proposed dual-branch sigmoid head method effectively decouples localization from classification, fixes softmax-induced distortions in CAM methods, and integrates seamlessly with most CAM variants while incurring negligible overhead.
...
### [17] [Google-MedGemma Based Abnormality Detection in Musculoskeletal radiographs](https://arxiv.org/abs/2511.05600)
*Soumyajit Maity,Pranjal Kamboj,Sneha Maity,Rajat Singh,Sankhadeep Chatterjee*

Main category: cs.CV

TL;DR: A MedGemma-based framework for automatic abnormality detection in musculoskeletal radiographs that outperforms conventional methods using transfer learning and efficient domain adaptation.


<details>
  <summary>Details</summary>
Motivation: To improve abnormality detection in musculoskeletal radiographs by leveraging modern medical foundation models instead of traditional autoencoder and neural network pipelines.

Method: Uses MedGemma foundation model with SigLIP-derived vision encoder to encode X-ray images into embeddings, followed by a lightweight multilayer perceptron for binary classification.
...
### [18] [In-process 3D Deviation Mapping and Defect Monitoring (3D-DM2) in High Production-rate Robotic Additive Manufacturing](https://arxiv.org/abs/2511.05604)
*Subash Gautam,Alejandro Vargas-Uscategui,Peter King,Hans Lohr,Alireza Bab-Hadiashar,Ivan Cole,Ehsan Asadi*

Main category: cs.CV

TL;DR: Real-time monitoring system for detecting shape deviations in high deposition rate robotic additive manufacturing to maintain part accuracy and quality.


<details>
  <summary>Details</summary>
Motivation: Maintaining shape accuracy is challenging in open-loop high deposition rate AM systems due to process instabilities, requiring real-time detection of deviations to prevent error propagation.

Method: Developed a real-time monitoring system that acquires and reconstructs the growing part, then directly compares it with a near-net reference model to detect shape deviations during manufacturing.

Result: The system enables early identification of shape inconsistencies and allows segmentation and tracking of each deviation region for timely intervention.

Conclusion: Real-time shape deviation monitoring paves the way for timely compensation and consistent part quality in high deposition rate additive manufacturing processes.
...
### [19] [Walking the Schrödinger Bridge: A Direct Trajectory for Text-to-3D Generation](https://arxiv.org/abs/2511.05609)
*Ziying Li,Xuequan Lu,Xinkui Zhao,Guanjie Cheng,Shuiguang Deng,Jianwei Yin*

Main category: cs.CV

TL;DR: TraCe addresses artifacts in text-to-3D generation by formulating it as learning optimal transport trajectories using Schrödinger Bridge framework, enabling high-quality 3D assets with smaller CFG values.


<details>
  <summary>Details</summary>
Motivation: Current text-to-3D methods using Score Distillation Sampling (SDS) suffer from artifacts like over-saturation and over-smoothing, limiting generation quality.

Method: TraCe formulates generation as learning direct transport trajectories between rendering and target distributions using Schrödinger Bridge framework, training LoRA-adapted models on trajectory score dynamics.

Result: TraCe consistently achieves superior quality and fidelity compared to state-of-the-art text-to-3D generation techniques.

Conclusion: The Schrödinger Bridge framework provides a mathematically grounded approach for high-quality text-to-3D generation, overcoming limitations of traditional SDS methods.
...
### [20] [Pose-Aware Multi-Level Motion Parsing for Action Quality Assessment](https://arxiv.org/abs/2511.05611)
*Shuaikang Zhu,Yang Yang,Chen Sun*

Main category: cs.CV

TL;DR: A multi-level motion parsing framework for action quality assessment that uses enhanced spatial-temporal pose features, action-unit parsing, motion parsing, and condition parsing to achieve state-of-the-art performance in diving sports.


<details>
  <summary>Details</summary>
Motivation: Human pose variations are crucial for action quality assessment, where subtle spatial-temporal differences determine scoring in high-level competitions. Existing methods need better handling of nuanced pose changes and external conditions like water splash in diving.

Method: Proposes a multi-level framework: 1) Action-Unit Parser for precise action segmentation and pose representations, 2) Motion Parser for spatial-temporal feature learning of pose changes, 3) Condition Parser for external factors like water splash, and 4) Weight-Adjust Scoring Module for diverse action types.

Result: Extensive evaluations on large-scale diving sports datasets show state-of-the-art performance in both action segmentation and action scoring tasks.

Conclusion: The multi-level motion parsing framework effectively captures subtle pose variations and external conditions, providing a comprehensive solution for action quality assessment in sports competitions.
...
### [21] [Personalized Image Editing in Text-to-Image Diffusion Models via Collaborative Direct Preference Optimization](https://arxiv.org/abs/2511.05616)
*Connor Dunlop,Matthew Zheng,Kavana Venkatesh,Pinar Yanardag*

Main category: cs.CV

TL;DR: C-DPO is a personalized image editing framework for diffusion models that uses collaborative preference optimization to align edits with individual user tastes by sharing information across like-minded users.


<details>
  <summary>Details</summary>
Motivation: Current T2I diffusion models are generic and fail to adapt to individual users' nuanced aesthetic preferences, limiting their personalization capabilities.

Method: Encodes users as nodes in a dynamic preference graph, learns embeddings via lightweight graph neural network, and integrates personalized embeddings into a novel DPO objective that jointly optimizes for individual alignment and neighborhood coherence.
...
Conclusion: The framework successfully enables personalized image editing in diffusion models by leveraging collaborative signals from like-minded users through graph-based preference optimization.
...
### [22] [Convolutional Fully-Connected Capsule Network (CFC-CapsNet): A Novel and Fast Capsule Network](https://arxiv.org/abs/2511.05617)
*Pouya Shiri,Amirali Baniasadi*

Main category: cs.CV

TL;DR: CFC-CapsNet improves Capsule Networks by introducing a Convolutional Fully-Connected layer that creates fewer but more powerful capsules, achieving better accuracy, faster training/inference, and fewer parameters on complex datasets.


<details>
  <summary>Details</summary>
Motivation: CapsNet performs well on simple datasets like MNIST but struggles with complex datasets, has slow performance compared to CNNs, and uses too many parameters.

Method: Introduces a Convolutional Fully-Connected (CFC) layer as an alternative method to create capsules, producing fewer but more powerful capsules.
...
Conclusion: The CFC-CapsNet successfully addresses CapsNet's limitations and provides a more efficient and effective capsule-based classifier for complex image datasets.
...
### [23] [Grounding Foundational Vision Models with 3D Human Poses for Robust Action Recognition](https://arxiv.org/abs/2511.05622)
*Nicholas Babey,Tiffany Gu,Yiheng Li,Cristian Meo,Kevin Zhu*

Main category: cs.CV

TL;DR: A model that fuses V-JEPA 2's world dynamics with CoMotion's human pose data for improved action recognition, especially in occluded scenes.
...
Method: Propose a model architecture that grounds action recognition in physical space by fusing V-JEPA 2's contextual, predictive world dynamics and CoMotion's explicit, occlusion-tolerant human pose data.

Result: Outperforms three other baselines on InHARD and UCF-19-Y-OCC benchmarks, especially in complex, occlusive scenes.
...
### [24] [Registration-Free Monitoring of Unstructured Point Cloud Data via Intrinsic Geometrical Properties](https://arxiv.org/abs/2511.05623)
...
<details>
  <summary>Details</summary>
Motivation: Traditional preprocessing steps like registration and mesh reconstruction for point cloud data monitoring are error-prone, time-consuming, and can introduce artifacts that affect monitoring accuracy.

Method: Two feature learning methods using intrinsic geometric properties (Laplacian and geodesic distances) combined with a monitoring scheme that uses thresholding to select features most indicative of defects.

Result: Numerical experiments and case studies demonstrate the approach effectively identifies various types of defects in complex shapes.
...
### [25] [Culture in Action: Evaluating Text-to-Image Models through Social Activities](https://arxiv.org/abs/2511.05681)
*Sina Malakouti,Boqing Gong,Adriana Kovashka*

Main category: cs.CV

TL;DR: CULTIVate is a benchmark for evaluating text-to-image models on cross-cultural activities, revealing systematic biases where models perform better for global north countries than global south countries.


<details>
  <summary>Details</summary>
Motivation: Current T2I models inherit cultural biases from training data and fail to depict underrepresented regions faithfully, with existing benchmarks focusing mainly on object-centric categories rather than social activities that reflect cultural norms.

Method: Created CULTIVate benchmark with 576 prompts spanning 16 countries and 19,000+ images, using an explainable descriptor-based evaluation framework across cultural dimensions (background, attire, objects, interactions) with four metrics for cultural alignment, hallucination, exaggerated elements, and diversity.

Result: Found systematic disparities: models perform better for global north countries than global south countries, with distinct failure modes across T2I systems. Human studies confirmed that CULTIVate metrics correlate more strongly with human judgments than existing text-image metrics.

Conclusion: CULTIVate provides an effective framework for measuring cultural faithfulness in T2I models, revealing significant cultural biases that need to be addressed in model development and evaluation.
...
### [26] [VMDT: Decoding the Trustworthiness of Video Foundation Models](https://arxiv.org/abs/2511.05682)
...
<details>
  <summary>Details</summary>
Motivation: Video modality lacks comprehensive trustworthiness benchmarks despite foundation models becoming more sophisticated, creating an urgent need for systematic evaluation frameworks.

Method: Developed VMDT platform to evaluate 7 T2V and 19 V2T models across five trustworthiness dimensions through extensive testing and analysis.

Result: Open-source T2V models fail to recognize harmful queries and generate harmful videos with higher unfairness than image models. V2T models show scale-dependent patterns: unfairness and privacy risks increase with size, while hallucination and robustness improve (but remain low). Safety shows no correlation with model size.

Conclusion: There is an urgent need for more robust and trustworthy video foundation models, and VMDT provides a systematic framework for measuring and tracking progress toward this goal.
...
### [27] [Pedicle Screw Pairing and Registration for Screw Pose Estimation from Dual C-arm Images Using CAD Models](https://arxiv.org/abs/2511.05702)
*Yehyun Suh,Lin Li,Aric Plumley,Chaochao Zhou,Daniel Moyer,Kongbin Kang*

Main category: cs.CV

TL;DR: A method for pedicle screw correspondence and pose estimation from dual C-arm images that compares screw combinations and uses 2D-3D alignment with CAD models to accurately pair screws and estimate their pose.
...
Result: The correct screw combination consistently outperforms incorrect pairings across all test cases, even before registration. After registration, the correct combination further enhances alignment between projections and images, significantly reducing projection error.
...
### [28] [Long Grounded Thoughts: Distilling Compositional Visual Reasoning Chains at Scale](https://arxiv.org/abs/2511.05705)
...
TL;DR: A new framework for generating large-scale vision-centric reasoning datasets with over 1M synthetic questions, preference data, and instruction prompts that outperforms existing models and transfers well to other modalities.


<details>
  <summary>Details</summary>
Motivation: Address the lack of systematic approaches for building large-scale vision-centric reasoning datasets beyond visual math, and overcome limitations of undisclosed datasets and proprietary data synthesis methods.

Method: Two-stage synthesis framework: (1) scale generation and (2) complexity enhancement, using VLMs and reasoning LLMs to produce CoT traces that capture diverse cognitive behaviors from frontier reasoning models.

Result: Qwen2.5-VL-7B finetuned on this data outperforms all open-data baselines across vision-centric benchmarks, surpasses closed-data models like MiMo-VL-7B-RL, and shows positive transfer to text-only reasoning (MMLU-Pro) and audio reasoning (MMAU).

Conclusion: High-quality SFT data with non-linear reasoning traces is essential for effective online RL, staged offline RL matches online RL performance with less compute, and careful SFT enables substantial cross-modality transfer despite being vision-centric.
...
### [29] [Towards Better Ultrasound Video Segmentation Foundation Model: An Empirical study on SAM2 Finetuning from Data Perspective](https://arxiv.org/abs/2511.05731)
*Xing Yao,Ahana Gangopadhyay,Hsi-Ming Chang,Ravi Soni*

Main category: cs.CV

TL;DR: This paper presents a data-centric analysis of adapting SAM2 for ultrasound video segmentation, showing that data scale and temporal context are more important than model architecture, with joint training offering the best balance.


<details>
  <summary>Details</summary>
Motivation: SAM2 foundation models perform poorly on medical imaging due to domain shift, and current adaptation studies focus too much on architecture rather than data characteristics and training strategies.

Method: Comprehensive analysis of SAM2 adaptation using different training paradigms (fine-tuning, intermediate adaptation, joint training), SAM2 variants, prompting modes, and ultrasound-specific augmentations across three datasets.

Result: Data scale and temporal context are more decisive than model architecture or initialization. Joint training provides efficient compromise between modality alignment and task specialization.

Conclusion: The study provides empirical insights for developing efficient, data-aware adaptation pipelines for SAM2 in ultrasound video analysis, emphasizing the importance of data characteristics over architectural modifications.
...
### [30] [A Second-Order Attention Mechanism For Prostate Cancer Segmentation and Detection in Bi-Parametric MRI](https://arxiv.org/abs/2511.05760)
*Mateo Ortiz,Juan Olmos,Fabio Martínez*

Main category: cs.CV

TL;DR: A second-order geometric attention (SOGA) mechanism on Riemannian manifolds improves prostate cancer detection in MRI by guiding segmentation networks, achieving better performance than baseline methods on public datasets.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for detecting clinically significant prostate cancer from MRI rely heavily on extensive annotations and struggle with lesion variability across prostate zones, requiring more robust and generalized approaches.

Method: Proposed a second-order geometric attention (SOGA) mechanism modeled on Riemannian manifolds using symmetric positive definite representations, integrated into U-Net and nnU-Net backbones through skip connections.

Result: Achieved AP of 0.37 and AUC-ROC of 0.83 on PI-CAI dataset, outperforming baselines, and AP of 0.37 and AUC-ROC of 0.75 on independent Prostate158 dataset, demonstrating robust generalization.

Conclusion: The SOGA mechanism provides discriminative learned representations that improve prostate cancer detection performance and generalization across different datasets.
...
### [31] [Sign language recognition from skeletal data using graph and recurrent neural networks](https://arxiv.org/abs/2511.05772)
*B. Mederos,J. Mejía,A. Medina-Reyes,Y. Espinosa-Almeyda,J. D. Díaz-Roman,I. Rodríguez-Mederos,M. Mejía-Carreon,F. Gonzalez-Lopez*

Main category: cs.CV

TL;DR: A Graph-GRU temporal network for skeleton-based sign language gesture recognition achieves high accuracy on the AUTSL dataset by modeling spatial and temporal dependencies.


<details>
  <summary>Details</summary>
Motivation: To develop an effective approach for isolated sign language gesture recognition using pose data from videos, addressing the need for scalable sign language understanding systems.

Method: Proposes a Graph-GRU temporal network that models spatial dependencies using graph-based representations and temporal dependencies through GRU architecture, using skeleton pose data from video sequences.

Result: Achieves high accuracy on the AUTSL (Ankara University Turkish Sign Language) dataset, demonstrating the effectiveness of integrating graph-based spatial representations with temporal modeling.

Conclusion: The approach highlights the potential of pose-driven methods for sign language understanding and provides a scalable framework for sign language recognition.
...
### [32] [TCSA-UDA: Text-Driven Cross-Semantic Alignment for Unsupervised Domain Adaptation in Medical Image Segmentation](https://arxiv.org/abs/2511.05782)
*Lalit Maurya,Honghai Liu,Reyer Zwiggelaar*

Main category: cs.CV

TL;DR: TCSA-UDA is a text-driven cross-semantic alignment framework for unsupervised domain adaptation in medical image segmentation that uses domain-invariant textual class descriptions to guide visual representation learning and reduce domain shifts across imaging modalities.


<details>
  <summary>Details</summary>
Motivation: Unsupervised domain adaptation for medical image segmentation faces challenges due to substantial domain shifts across imaging modalities like CT and MRI. While vision-language representation learning shows promise, its potential in UDA segmentation tasks remains underexplored.

Method: Proposes TCSA-UDA framework with: 1) Vision-language covariance cosine loss to align image encoder features with inter-class textual semantic relations, 2) Prototype alignment module that aligns class-wise pixel-level feature distributions across domains using high-level semantic prototypes.
...
### [33] [Position-Prior-Guided Network for System Matrix Super-Resolution in Magnetic Particle Imaging](https://arxiv.org/abs/2511.05795)
...
<details>
  <summary>Details</summary>
Motivation: Current SM calibration methods are time-consuming and require repeated measurements when system parameters change. Existing deep learning SR techniques don't fully utilize physical prior knowledge like symmetric positional priors.
...
Result: Empirically validated the efficacy of incorporating positional priors through experiments.

Conclusion: Positional priors can be effectively integrated into SM calibration frameworks to improve the process.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations in current encoder-decoder architectures: (1) information loss of fine-grained details from shallow layers as data propagates deeper, and (2) inefficient integration of local details and global context between encoder and decoder stages.
...
TL;DR: LRANet++ is an end-to-end text spotting framework that uses a parameterized text shape method based on low-rank approximation for precise detection and a triple assignment detection head for fast inference, achieving state-of-the-art performance on arbitrary-shaped text spotting.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end text spotters struggle with accurate and efficient detection of arbitrary-shaped text due to unreliable detection methods. The main bottleneck is the lack of a robust and fast text detection approach.

Method: Proposes a data-driven low-rank approximation method for text shape representation using ℓ1-norm formulation to handle annotation noise, and a triple assignment detection head with deep sparse branch (training), ultra-lightweight sparse branch (inference), and dense branch (supervision).

Result: Extensive experiments on challenging benchmarks demonstrate LRANet++'s superiority over state-of-the-art methods in both accuracy and efficiency for arbitrary-shaped text spotting.

Conclusion: LRANet++ effectively addresses the text detection bottleneck through novel shape representation and detection architecture, enabling accurate and efficient end-to-end text spotting for arbitrary-shaped text.
...
TL;DR: Proposes Hilbert curve-based window and neighborhood construction for 2D local attention, achieving 4× and 18× speedups for window and slide attention respectively through improved block sparsity.


<details>
  <summary>Details</summary>
Motivation: Global self-attention has quadratic compute/memory costs in high-resolution images, and conventional local attention patterns fail to deliver significant speedups due to non-contiguous tokens in 1D sequences.

Method: Reorders image tokens along a Hilbert curve, then forms windows and neighborhoods on the reordered 1D sequence to increase block sparsity, combined with existing block-sparse kernels.
...
### [37] [TYrPPG: Uncomplicated and Enhanced Learning Capability rPPG for Remote Heart Rate Estimation](https://arxiv.org/abs/2511.05833)
...
<details>
  <summary>Details</summary>
Motivation: Existing rPPG models based on transformers have low computation efficiency, while Mamba models show promise but their SSM core is unnecessary for vision tasks. The goal is to prove Mambaout-based modules can effectively learn heart rate remotely.

Method: Proposed TYrPPG with innovative gated video understanding block (GVB) that integrates 2D-CNN and 3D-CNN based on Mambaout structure, plus comprehensive supervised loss function (CSL) and its weakly supervised variants.

Result: Experiments show TYrPPG achieves state-of-the-art performance in commonly used datasets for remote heart rate estimation.

Conclusion: TYrPPG demonstrates the feasibility and superiority of using Mambaout-based modules for remote heart rate learning, offering efficient and effective rPPG analysis.
...
### [38] [Understanding Cross Task Generalization in Handwriting-Based Alzheimer's Screening via Vision Language Adaptation](https://arxiv.org/abs/2511.05841)
*Changqing Gong,Huafeng Qin,Mounim A. El-Yacoubi*

Main category: cs.CV

TL;DR: A lightweight Cross-Layer Fusion Adapter framework repurposes CLIP for zero-shot Alzheimer's disease screening using handwriting analysis, systematically investigating cross-task generalization and identifying discriminative stroke patterns.


<details>
  <summary>Details</summary>
Motivation: Early detection of Alzheimer's disease is critical, and handwriting provides a non-invasive window into motor and cognitive decline. Existing handwriting-based AD studies haven't systematically examined task type influence on diagnostic performance and cross-task generalization.

Method: Introduces CLFA (Cross-Layer Fusion Adapter) framework that implants multi-level fusion adapters within CLIP's visual encoder to progressively align representations toward handwriting-specific medical cues, enabling prompt-free zero-shot inference.

Result: The framework enables systematic investigation of cross-task generalization, revealing which task types and writing patterns most effectively discriminate AD, with extensive analyses highlighting characteristic stroke patterns and task-level factors.

Conclusion: The approach offers both diagnostic insights for early AD identification and provides a benchmark for handwriting-based cognitive assessment using vision-language models.
...
### [39] [Enhancing Diffusion Model Guidance through Calibration and Regularization](https://arxiv.org/abs/2511.05844)
*Seyed Alireza Javid,Amirhossein Bagheri,Nuria González-Prelcic*

Main category: cs.CV

TL;DR: This paper addresses overconfident predictions in classifier-guided diffusion models by proposing calibration techniques and enhanced sampling methods that improve image generation quality without requiring diffusion model retraining.
...
Method: Two main approaches: 1) Differentiable calibration using Smooth Expected Calibration Error to improve classifier calibration, and 2) Enhanced sampling guidance methods including tilted sampling with batch-level reweighting, adaptive entropy-regularized sampling, and novel f-divergence-based sampling strategy.

Result: On ImageNet 128x128, the divergence-regularized guidance achieves FID of 2.13 using ResNet-101 classifier, improving upon existing classifier-guided diffusion methods while requiring no diffusion model retraining.

Conclusion: Principled calibration and divergence-aware sampling provide practical and effective improvements for classifier-guided diffusion, enabling better conditional image generation without model retraining.
...
### [40] [Point Cloud Segmentation of Integrated Circuits Package Substrates Surface Defects Using Causal Inference: Dataset Construction and Methodology](https://arxiv.org/abs/2511.05853)
*Bingyang Guo,Qiang Zuo,Ruiyun Yu*

Main category: cs.CV

TL;DR: Created CPS3D-Seg dataset for 3D surface defect detection in ceramic package substrates and proposed CINet method using causal inference that outperforms existing algorithms.


<details>
  <summary>Details</summary>
Motivation: Need for effective 3D segmentation of ceramic package substrates to detect subtle defects in integrated circuits, addressing the lack of public datasets and complex structure challenges.

Method: Constructed high-quality CPS3D-Seg dataset with 1300 point cloud samples and proposed CINet method using Structural Refine and Quality Assessment modules for causal inference-based 3D segmentation.

Result: CINet significantly outperforms existing algorithms in both mIoU and accuracy metrics on the CPS3D-Seg benchmark.

Conclusion: The CPS3D-Seg dataset and CINet method provide effective solutions for 3D surface defect detection in ceramic package substrates, advancing industrial inspection capabilities.
...
### [41] [CGCE: Classifier-Guided Concept Erasure in Generative Models](https://arxiv.org/abs/2511.05865)
*Viet Nguyen,Vishal M. Patel*

Main category: cs.CV

TL;DR: CGCE is a plug-and-play framework that uses lightweight classifiers to detect and refine unsafe text embeddings at inference time, enabling robust concept erasure without modifying model weights.


<details>
  <summary>Details</summary>
Motivation: Existing concept erasure methods are vulnerable to adversarial attacks and degrade model quality for safe concepts, creating a trade-off between safety and performance.

Method: Classifier-guided approach using lightweight classifiers on text embeddings to detect and refine prompts containing undesired concepts, with multi-concept erasure through classifier aggregation.

Result: Achieves state-of-the-art robustness against red-teaming attacks while maintaining high generative utility on benign prompts across T2I and T2V models.

Conclusion: CGCE provides a practical and effective solution for safe generative AI with superior balance between safety and performance.
...
### [42] [Light-Field Dataset for Disparity Based Depth Estimation](https://arxiv.org/abs/2511.05866)
*Suresh Nehra,Aupendu Kar,Jayanta Mukhopadhyay,Prabir Kumar Biswas*

Main category: cs.CV

TL;DR: A new publicly available light field image dataset with 285 real images from Lytro Illum camera and 13 synthetic images, addressing limitations of existing datasets and demonstrating focal position effects on disparity.


<details>
  <summary>Details</summary>
Motivation: To enable development of disparity-based light field depth estimation algorithms by providing suitable light field image datasets, as current datasets have shortcomings and the trade-off between angular and spatial information depends on camera focal position.

Method: Created a comprehensive dataset using Lytro Illum LF camera for real images (285) and Blender for synthetic images (13), including both real and synthetic stereo light field data captured via mechanical gantry system and simulation.

Result: Produced a publicly available dataset that demonstrates the effect of focal position on 3D point disparity and addresses limitations of existing light field datasets, with synthetic data matching real camera disparity characteristics.

Conclusion: The introduced dataset is essential for developing and testing novel light field depth estimation algorithms, providing both real and synthetic data that captures the critical trade-offs in light field imaging.
...
### [43] [MoEGCL: Mixture of Ego-Graphs Contrastive Representation Learning for Multi-View Clustering](https://arxiv.org/abs/2511.05876)
*Jian Zhu,Xin Zou,Jun Sun,Cheng Luo,Lei Liu,Lingfang Zeng,Ning Zhang,Bian Wu,Chang Tang,Lirong Dai*

Main category: cs.CV

TL;DR: MoEGCL introduces fine-grained ego-graph fusion and contrastive learning for multi-view clustering, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing GNN-based multi-view clustering methods suffer from coarse-grained graph fusion at the view level, which limits their effectiveness.

Method: Proposes Mixture of Ego-Graphs Fusion (MoEGF) using Mixture-of-Experts for sample-level fusion, and Ego Graph Contrastive Learning (EGCL) to align representations and enhance similarity within clusters.

Result: Extensive experiments show MoEGCL achieves state-of-the-art results in deep multi-view clustering tasks.

Conclusion: MoEGCL successfully addresses coarse-grained fusion limitations through fine-grained ego-graph fusion and contrastive learning, significantly improving multi-view clustering performance.
...
### [44] [Towards Frequency-Adaptive Learning for SAR Despeckling](https://arxiv.org/abs/2511.05890)
*Ziqing Ma,Chang Yang,Zhichang Guo,Yao Li*

Main category: cs.CV

TL;DR: SAR-FAH is a frequency-adaptive heterogeneous despeckling model that uses wavelet decomposition and specialized sub-networks for different frequency components to better preserve edges and textures while suppressing speckle noise in SAR images.


<details>
  <summary>Details</summary>
Motivation: Traditional deep learning methods for SAR despeckling use unified networks that fail to account for distinct speckle statistics across different spatial characteristics, leading to artifacts, blurred edges, and texture distortion.

Method: Uses wavelet decomposition to separate images into frequency sub-bands, then employs specialized sub-networks: neural ODEs for low-frequency denoising to ensure structural fidelity, and enhanced U-Net with deformable convolutions for high-frequency components rich in edges and textures.

Result: Extensive experiments on synthetic and real SAR images validate superior performance in noise removal and structural preservation compared to existing methods.

Conclusion: The frequency-adaptive heterogeneous approach effectively addresses the limitations of unified networks by leveraging statistical variations across frequencies, improving edge and texture preservation while suppressing speckle noise.
...
### [45] [Hybrid second-order gradient histogram based global low-rank sparse regression for robust face recognition](https://arxiv.org/abs/2511.05893)
*Hongxia Li,Ying Ji,Yongxin Dong,Yuehua Feng*

Main category: cs.CV

TL;DR: Proposes H2H-GLRSR model combining hybrid second-order gradient histograms with global low-rank sparse regression for robust face recognition under occlusions and illumination variations.
...
Conclusion: The proposed H2H-GLRSR model effectively handles complex facial variations and outperforms current state-of-the-art regression approaches for face recognition.
...
### [46] [Open-World 3D Scene Graph Generation for Retrieval-Augmented Reasoning](https://arxiv.org/abs/2511.05894)
...
<details>
  <summary>Details</summary>
Motivation: To address limitations of closed-vocabulary supervision and static annotations in 3D scene understanding for open-world settings.
...
Result: Demonstrated robust generalization and superior performance on 3DSSG and Replica benchmarks across four tasks: scene question answering, visual grounding, instance retrieval, and task planning.
...
### [47] [GABFusion: Rethinking Feature Fusion for Low-Bit Quantization of Multi-Task Networks](https://arxiv.org/abs/2511.05898)
*Zhaoyang Wang,Dong Wang*

Main category: cs.CV

TL;DR: GABFusion and ADA address quantization performance degradation in multi-task networks by balancing gradients and fusing features, achieving significant accuracy improvements while maintaining compatibility with existing QAT methods.
...
Method: Propose Gradient-Aware Balanced Feature Fusion (GABFusion) to dynamically balance gradient magnitudes and fuse task-specific features, and Attention Distribution Alignment (ADA) for feature-level distillation in quantized models.

Result: Achieves average mAP improvements of 3.3% on PASCAL VOC and 1.6% on COCO datasets. Under 4-bit quantization on YOLOv5, reduces accuracy gap with full-precision model to only 1.7% on VOC.

Conclusion: The proposed framework is modular, easy to integrate, and compatible with any existing QAT technique, effectively preserving performance under low-bit constraints without modifying original network architecture.
...
### [48] [Causal Tracing of Object Representations in Large Vision Language Models: Mechanistic Interpretability and Hallucination Mitigation](https://arxiv.org/abs/2511.05923)
*Qiming Li,Zekai Ye,Xiaocheng Feng,Weihong Zhong,Weitao Ma,Xiachong Feng*

Main category: cs.CV

TL;DR: FCCT framework systematically analyzes causal effects in LVLMs, revealing critical roles of MHSA in cross-modal aggregation and FFNs in hierarchical visual representation storage. Based on these insights, IRI technique enhances perception and reduces hallucination without training.


<details>
  <summary>Details</summary>
Motivation: Existing mechanistic interpretability analyses for LVLMs are insufficiently comprehensive, lacking examination of visual/textual tokens, model components, and full layer ranges, limiting insights for improving model faithfulness and downstream tasks like hallucination mitigation.

Method: Introduced Fine-grained Cross-modal Causal Tracing (FCCT) framework for systematic quantification of causal effects on visual object perception, covering all visual/textual tokens, MHSA, FFNs, and hidden states across all decoder layers.

Result: First to demonstrate that MHSAs of last token in middle layers play critical role in cross-modal information aggregation, while FFNs show three-stage hierarchical progression for visual object representation storage and transfer. IRI technique achieves state-of-the-art performance across five benchmarks.

Conclusion: FCCT provides comprehensive mechanistic insights into LVLMs, enabling development of effective training-free techniques like IRI that enhance perception and mitigate hallucination while preserving inference speed and foundational performance.
...
### [49] [CoMA: Complementary Masking and Hierarchical Dynamic Multi-Window Self-Attention in a Unified Pre-training Framework](https://arxiv.org/abs/2511.05929)
*Jiaxuan Li,Qing Xu,Xiangjian He,Ziyu Liu,Chang Xing,Zhen Chen,Daokun Zhang,Rong Qu,Chang Wen Chen*

Main category: cs.CV

TL;DR: CoMA uses complementary masking for uniform pixel sampling to improve feature learning, while DyViT employs dynamic multi-window attention for efficiency. Together they achieve MAE-level performance with 12% of pre-training epochs.


<details>
  <summary>Details</summary>
Motivation: MAE requires many pre-training epochs and ViT has inefficient parameter use due to fixed spatial resolution. Need to improve pre-training efficiency and parameter utilization.

Method: Complementary Masked Autoencoders (CoMA) with uniform pixel sampling strategy, and DyViT hierarchical vision transformer with Dynamic Multi-Window Self-Attention (DM-MSA).

Result: Matches MAE downstream performance with only 12% pre-training epochs, 10% reduction in pre-training time per epoch, and reduced parameters/FLOPs while improving fine-grained features.

Conclusion: CoMA and DyViT together provide more effective and efficient self-supervised learning with superior pre-training efficiency and better parameter utilization.
...
### [50] [AD-DAE: Unsupervised Modeling of Longitudinal Alzheimer's Disease Progression with Diffusion Auto-Encoder](https://arxiv.org/abs/2511.05934)
...
<details>
  <summary>Details</summary>
Motivation: Existing generative approaches impose constraints on distribution learning, leading to limited controllability for generating follow-up images without explicit supervision from subject-specific longitudinal images.

Method: Uses a conditionable Diffusion Auto-encoder framework that forms a compact latent space capturing high-level semantics, enabling disentanglement of progression-relevant information. Applies controlled shifts to baseline representations within a restricted subspace to isolate progression factors from identity-preserving components.
...
Conclusion: The approach successfully enables controlled movements in latent space and generates progression images from baseline images in an unsupervised manner, correlating with progression attributes without requiring subject-specific longitudinal supervision.
...
### [51] [Interaction-Centric Knowledge Infusion and Transfer for Open-Vocabulary Scene Graph Generation](https://arxiv.org/abs/2511.05935)
*Lin Li,Chuhan Zhang,Dong Zhang,Chong Sun,Chen Li,Long Chen*

Main category: cs.CV

TL;DR: ACC is an interaction-centric end-to-end framework for open-vocabulary scene graph generation that addresses limitations in distinguishing interacting vs non-interacting objects through bidirectional interaction prompts and interaction-guided query selection.


<details>
  <summary>Details</summary>
Motivation: Existing OVSGG methods struggle to distinguish between interacting and non-interacting instances of the same object category due to lack of explicit interaction modeling, causing noisy pseudo-supervision and ambiguous query matching.

Method: Proposes ACC framework with: 1) bidirectional interaction prompts for robust pseudo-supervision generation, 2) interaction-guided query selection to prioritize pairing interacting objects, and 3) interaction-consistent knowledge distillation to push relational foreground away from background.

Result: Extensive experiments on three benchmarks show ACC achieves state-of-the-art performance in open-vocabulary scene graph generation.

Conclusion: ACC demonstrates the potential of interaction-centric paradigms for real-world OVSGG applications by effectively minimizing mismatches between interacting and non-interacting objects.
...
### [52] [Global Multiple Extraction Network for Low-Resolution Facial Expression Recognition](https://arxiv.org/abs/2511.05938)
*Jingyi Shi*

Main category: cs.CV

TL;DR: GME-Net is a novel network for low-resolution facial expression recognition that uses hybrid attention-based local feature extraction and multi-scale global feature extraction to overcome detail loss and weak global modeling in existing methods.
...
Result: Extensive experiments on multiple datasets show GME-Net achieves superior performance in recognizing low-resolution facial expressions compared to existing solutions.

Conclusion: GME-Net effectively extracts expression-related discriminative features from low-resolution images through its combined local and global feature extraction approach, demonstrating improved performance over current methods.
...
### [53] [Polymap: generating high definition map based on rasterized polygons](https://arxiv.org/abs/2511.05944)
*Shiyu Gao,Hao Jiang*

Main category: cs.CV

TL;DR: Proposes a segmentation-based approach for HD map construction using instance segmentation and Potrace post-processing to improve generalizability over detection-based methods.


<details>
  <summary>Details</summary>
Motivation: Detection-based methods for online HD map construction lack robust generalizability, limiting their applicability in auto-labeling systems.

Method: Reinterprets road elements as rasterized polygons using instance segmentation with a transformer, followed by Potrace-based post-processing to vectorize map elements.
...
Conclusion: The segmentation-based approach with post-processing provides better generalizability for HD map construction compared to detection-based methods.
...
### [54] [Reperio-rPPG: Relational Temporal Graph Neural Networks for Periodicity Learning in Remote Physiological Measurement](https://arxiv.org/abs/2511.05946)
*Ba-Thinh Nguyen,Thach-Ha Ngoc Pham,Hoang-Long Duc Nguyen,Thi-Duyen Ngo,Thanh-Ha Le*

Main category: cs.CV

TL;DR: Reperio-rPPG is a novel framework that integrates Relational Convolutional Networks with Graph Transformer to capture periodic structure in remote photoplethysmography, achieving state-of-the-art performance with enhanced robustness under various conditions.


<details>
  <summary>Details</summary>
Motivation: Existing rPPG approaches often underexplore the intrinsic periodicity of physiological signals, limiting their ability to capture fine-grained temporal dynamics under real-world conditions.

Method: Proposes Reperio-rPPG framework that strategically integrates Relational Convolutional Networks with Graph Transformer, plus a tailored CutMix augmentation to enhance model generalizability.

Result: Achieves state-of-the-art performance on PURE, UBFC-rPPG, and MMPD datasets with remarkable robustness under various motion and illumination conditions.

Conclusion: The proposed framework effectively captures periodic structure in physiological signals and demonstrates superior performance and robustness in real-world scenarios.
...
### [55] [U(PM)$^2$:Unsupervised polygon matching with pre-trained models for challenging stereo images](https://arxiv.org/abs/2511.05949)
*Chang Li,Xingtao Peng*

Main category: cs.CV

TL;DR: U(PM)^2 is an unsupervised polygon matching method that combines pre-trained models with handcrafted features to address stereo image polygon matching challenges without training requirements.


<details>
  <summary>Details</summary>
Motivation: Polygon matching in stereo images is an underexplored field facing challenges like disparity discontinuity, scale variation, training requirements, and generalization issues that need to be addressed.
...
Result: Achieved state-of-the-art accuracy on ScanNet and SceneFlow datasets with competitive speed and satisfactory generalization performance at low cost without training.

Conclusion: The proposed U(PM)^2 method effectively addresses polygon matching challenges through unsupervised learning with pre-trained models, demonstrating strong performance and generalization capabilities.
...
### [56] [CSGaze: Context-aware Social Gaze Prediction](https://arxiv.org/abs/2511.05955)
...
Result: CSGaze achieves competitive performance with state-of-the-art methods on GP-Static, UCO-LAEO, and AVA-LAEO datasets. The model demonstrates robustness across diverse scenarios in open set testing.

Conclusion: Contextual cues significantly improve social gaze prediction. The attention mechanism provides explainability, and the model shows strong generalization capabilities across different scenarios.
...
### [57] [Adaptive Agent Selection and Interaction Network for Image-to-point cloud Registration](https://arxiv.org/abs/2511.05965)
*Zhixin Cheng,Xiaotian Yin,Jiacheng Deng,Bohao Liao,Yujia Chen,Xu Zhou,Baoqun Yin,Tianzhu Zhang*

Main category: cs.CV

TL;DR: A novel cross-modal image-to-point cloud registration framework using iterative agent selection and reliable agent interaction to improve robustness and accuracy under challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Existing transformer-based methods struggle with noise and fail to effectively select informative cross-modal representations, limiting registration robustness and accuracy.

Method: Proposes two modules: Iterative Agents Selection (IAS) with phase maps and reinforcement learning for reliable agent selection, and Reliable Agents Interaction (RAI) for guided cross-modal interactions to reduce mismatches.

Result: Achieves state-of-the-art performance on RGB-D Scenes v2 and 7-Scenes benchmarks, demonstrating consistent improvements in registration accuracy.

Conclusion: The proposed framework effectively addresses cross-modal registration challenges by enhancing feature awareness and reducing mismatches through intelligent agent selection and interaction.
...
### [58] [Commonality in Few: Few-Shot Multimodal Anomaly Detection via Hypergraph-Enhanced Memory](https://arxiv.org/abs/2511.05966)
*Yuxuan Lin,Hanjing Yan,Xuan Tong,Yang Chang,Huanzhen Wang,Ziheng Zhou,Shuyong Gao,Yan Wang,Wenqiang Zhang*

Main category: cs.CV

TL;DR: CIF is a few-shot multimodal industrial anomaly detection method that uses hypergraphs to extract structural commonality from limited training samples and employs memory banks for improved detection performance.


<details>
  <summary>Details</summary>
Motivation: Few-shot settings in industrial anomaly detection face challenges due to insufficient training samples that fail to cover diverse test patterns, requiring extraction of structural commonality from limited data.

Method: Uses hypergraphs to model higher-order correlations and extract structural commonality, constructs memory banks with intra-class structural prior, employs training-free hypergraph message passing to update test features, and hyperedge-guided memory search to reduce false positives.

Result: Outperforms state-of-the-art methods on MVTec 3D-AD and Eyecandies datasets in few-shot settings.

Conclusion: The proposed CIF method effectively addresses few-shot multimodal industrial anomaly detection by leveraging structural commonality through hypergraphs and memory banks.
...
### [59] [Adapted Foundation Models for Breast MRI Triaging in Contrast-Enhanced and Non-Contrast Enhanced Protocols](https://arxiv.org/abs/2511.05967)
...
TL;DR: DINOv2-based Medical Slice Transformer (MST) can rule out BI-RADS ≥4 findings in abbreviated breast MRI with 97.5% sensitivity and 19% specificity for contrast-enhanced protocols, potentially reducing radiologist workload.


<details>
  <summary>Details</summary>
Motivation: MRI has high sensitivity for breast cancer but interpretation is time-consuming. AI pre-screening could help reduce radiologist workload by automatically ruling out cases without significant findings.

Method: Retrospective study using 1,847 breast MRI exams with four abbreviated protocols (T1sub, DWI1500, DWI1500+T2w, T1sub+T2w). Performance evaluated using five-fold cross-validation, AUC analysis, and external validation on Duke dataset.

Result: T1sub+T2w achieved AUC of 0.77±0.04, with 97.5% sensitivity and 19% specificity. DWI1500+T2w had AUC of 0.74±0.04 with 17% specificity. Missed lesions were small (<10mm) non-mass enhancements. External validation AUC was 0.77.

Conclusion: MST framework can correctly triage cases without BI-RADS ≥4 findings at high sensitivity, achieving modest specificity. Further research needed before clinical implementation.
...
### [60] [DiA-gnostic VLVAE: Disentangled Alignment-Constrained Vision Language Variational AutoEncoder for Robust Radiology Reporting with Missing Modalities](https://arxiv.org/abs/2511.05968)
*Nagur Shareef Shaik,Teja Krishna Cherukuri,Adnan Masood,Dong Hye Ye*

Main category: cs.CV

TL;DR: DiA-gnostic VLVAE is a framework for robust radiology reporting that handles missing modalities and feature entanglement through disentangled alignment using a Mixture-of-Experts based Vision-Language Variational Autoencoder.


<details>
  <summary>Details</summary>
Motivation: Current automated methods struggle with missing modalities and feature entanglement in clinical data, leading to suboptimal fusion and clinically unfaithful hallucinated findings in radiology reports.
...
Conclusion: The proposed DiA framework effectively addresses challenges of missing modalities and feature entanglement in clinical data, enabling robust and clinically interpretable radiology report generation.
...
### [61] [Runtime Safety Monitoring of Deep Neural Networks for Perception: A Survey](https://arxiv.org/abs/2511.05982)
*Albert Schotschneider,Svetlana Pavlitska,J. Marius Zöllner*

Main category: cs.CV

TL;DR: This survey provides a comprehensive overview of runtime safety monitoring approaches for deep neural networks in safety-critical applications, categorizing methods into input, internal representation, and output monitoring.


<details>
  <summary>Details</summary>
Motivation: DNNs in safety-critical applications like autonomous driving are vulnerable to generalization errors, out-of-distribution inputs, and adversarial attacks that can cause hazardous failures, necessitating runtime safety monitoring.

Method: The survey categorizes existing runtime safety monitoring approaches into three main groups: monitoring inputs, internal representations, and outputs of DNNs during inference without modifying the DNN itself.

Result: The paper analyzes state-of-the-art methods in each category, identifies their strengths and limitations, and maps them to the specific safety concerns they address.

Conclusion: The survey highlights open challenges and future research directions for runtime safety monitoring of DNNs in safety-critical applications.
...
### [62] [A Dual-Mode ViT-Conditioned Diffusion Framework with an Adaptive Conditioning Bridge for Breast Cancer Segmentation](https://arxiv.org/abs/2511.05989)
*Prateek Singh,Moumita Dholey,P. K. Vinod*

Main category: cs.CV

TL;DR: A conditional Denoising Diffusion Model with Vision Transformer encoder and enhanced UNet decoder achieves state-of-the-art breast ultrasound lesion segmentation by addressing low contrast and unclear boundaries through adaptive feature fusion and topological consistency regularization.


<details>
  <summary>Details</summary>
Motivation: Breast ultrasound lesion segmentation is challenging due to low contrast, speckle noise, and unclear boundaries, while standard convolutional architectures fail to capture sufficient global context, leading to anatomically inconsistent segmentations.

Method: Flexible conditional Denoising Diffusion Model combining ViT encoder for global features with enhanced UNet decoder, featuring Adaptive Conditioning Bridge for multi-scale fusion, Topological Denoising Consistency loss for structural regularization, and dual-head architecture for efficient inference.

Result: Achieves new state-of-the-art performance with Dice scores of 0.96 on BUSI, 0.90 on BrEaST, and 0.97 on BUS-UCLM datasets, with ablation studies confirming model component effectiveness.

Conclusion: The proposed framework produces not only accurate but also anatomically plausible segmentations, effectively addressing the challenges in breast ultrasound image analysis through diffusion modeling and structural regularization.
...
### [63] [Exploring Category-level Articulated Object Pose Tracking on SE(3) Manifolds](https://arxiv.org/abs/2511.05996)
...
<details>
  <summary>Details</summary>
Motivation: Articulated object pose tracking remains underexplored compared to rigid objects due to complex kinematic constraints, creating a need for specialized tracking methods.

Method: Uses SE(3) quasi-canonicalization of point clouds, models objects with Point Pair Features to predict pose voting parameters, and incorporates semantic joint axis information for unified kinematic constraints.

Result: Demonstrates strong generalization on synthetic and real-world datasets, showing effectiveness and robustness in multi-frame articulated object pose tracking.
...
### [64] [MALeR: Improving Compositional Fidelity in Layout-Guided Generation](https://arxiv.org/abs/2511.06002)
*Shivank Saxena,Dhruv Srivastava,Makarand Tapaswi*

Main category: cs.CV

TL;DR: MALeR addresses challenges in layout-guided text-to-image generation by preventing subjects from appearing outside layouts, ensuring in-distribution generation, and using masked attribute-aware binding to prevent attribute leakage.


<details>
  <summary>Details</summary>
Motivation: Current layout-guided text-to-image methods struggle with compositional scenes, where unintended subjects appear outside layouts, images contain unnatural artifacts, and attributes bleed across subjects.

Method: Proposes MALeR with masked attribute-aware binding mechanism that prevents attribute leakage and ensures subjects only appear within given layouts while maintaining in-distribution generation.

Result: Qualitative and quantitative evaluation shows superior performance in compositional accuracy, generation consistency, and attribute binding compared to previous methods.

Conclusion: MALeR effectively generates complex compositional scenes with multiple subjects and attributes per subject, addressing key limitations in current layout-guided text-to-image generation.
...
### [65] [How Reasoning Influences Intersectional Biases in Vision Language Models](https://arxiv.org/abs/2511.06005)
*Adit Desai,Sudipta Roy,Mohna Chakraborty*

Main category: cs.CV

TL;DR: Analysis of social biases in 5 open-source Vision Language Models (VLMs) for occupation prediction, revealing systematic intersectional disparities in reasoning patterns.


<details>
  <summary>Details</summary>
Motivation: VLMs are increasingly deployed but their training data encode social biases that surface in outputs, diverging from human reasoning through statistical associations rather than contextual understanding.
...
Conclusion: There is a need to align VLM reasoning with human values prior to downstream deployment to address inherent biases.
...
### [66] [Distributed Deep Learning for Medical Image Denoising with Data Obfuscation](https://arxiv.org/abs/2511.06006)
*Sulaimon Oyeniyi Adebayo,Ayaz H. Khan*

Main category: cs.CV

TL;DR: Distributed deep learning for chest X-ray denoising using U-Net and U-Net++ architectures, achieving 60% faster training with optimized multi-GPU setup while maintaining competitive denoising performance.


<details>
  <summary>Details</summary>
Motivation: To improve medical image quality while protecting sensitive information in large-scale clinical datasets through efficient distributed deep learning approaches.

Method: Used U-Net and U-Net++ architectures with additive Gaussian noise for obfuscation, implemented under single-GPU, DataParallel, and optimized DistributedDataParallel (DDP) with Automatic Mixed Precision (AMP) training configurations.

Result: U-Net++ delivered superior denoising with better PSNR and SSIM scores, though lower LPIPS. Optimized training pipeline reduced training time by over 60% vs single-GPU and 40% vs standard DataParallel, with minor accuracy trade-off.

Conclusion: Combining architectural design, lightweight obfuscation, and advanced distributed training strategies provides practical acceleration for medical image processing pipelines in clinical and research environments.
...
### [67] [One-Shot Knowledge Transfer for Scalable Person Re-Identification](https://arxiv.org/abs/2511.06016)
*Longhua Li,Lei Qi,Xin Geng*

Main category: cs.CV

TL;DR: OSKT is a one-shot knowledge transfer method for person re-identification that creates a weight chain from teacher models, enabling instant generation of compressed models for different resource constraints without repeated computations.


<details>
  <summary>Details</summary>
Motivation: To address the computational inefficiency of conventional compression methods that require separate computations for each model size when multiple compressed models are needed for different edge computing scenarios.

Method: Proposes OSKT which consolidates teacher model knowledge into a weight chain that can be expanded to any target model size without additional computation, enabling one-time knowledge transfer.

Result: OSKT significantly outperforms state-of-the-art compression methods while eliminating the need for frequent computations for each target model.

Conclusion: The proposed OSKT approach provides an efficient solution for generating multiple compressed models for edge computing in person re-identification through one-shot knowledge transfer.
...
### [68] [MiVID: Multi-Strategic Self-Supervision for Video Frame Interpolation using Diffusion Model](https://arxiv.org/abs/2511.06019)
...
<details>
  <summary>Details</summary>
Motivation: Classical VFI methods struggle with occlusions, domain shifts, and ambiguous motion, while learning-based approaches require dense ground-truth data. There's a need for robust, accessible VFI systems that don't rely on explicit motion estimation.

Method: Combines 3D U-Net backbone with transformer-style temporal attention, trained under hybrid masking regime with cosine-based progressive masking and adaptive loss scheduling. Entirely trained on CPU using 9-frame video segments.

Result: Achieves optimal results at just 50 epochs on UCF101-7 and DAVIS-7 datasets, competitive with supervised baselines despite being self-supervised and low-resource.
...
### [69] [Towards Implicit Aggregation: Robust Image Representation for Place Recognition in the Transformer Era](https://arxiv.org/abs/2511.06024)
*Feng Lu,Tong Jin,Canming Ye,Yunpeng Liu,Xiangyuan Lan,Chun Yuan*

Main category: cs.CV

TL;DR: Proposes eliminating dedicated aggregators in transformer-based visual place recognition by using learnable aggregation tokens that implicitly aggregate information through self-attention, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current VPR methods use backbone-plus-aggregator paradigm, but this paper argues dedicated aggregators are unnecessary in transformer era as transformers can implicitly aggregate information through self-attention.

Method: Introduces learnable aggregation tokens prepended to patch tokens before transformer blocks, allowing implicit aggregation through self-attention. Proposes optimal token insertion strategy and initialization method.

Result: Outperforms state-of-the-art methods on multiple VPR datasets with higher efficiency, ranks 1st on MSLS challenge leaderboard.

Conclusion: Dedicated aggregators are unnecessary in transformer-based VPR; implicit aggregation via learnable tokens provides robust global descriptors more efficiently.
...
### [70] [S2ML: Spatio-Spectral Mutual Learning for Depth Completion](https://arxiv.org/abs/2511.06033)
*Zihui Zhao,Yifei Zhang,Zheng Wang,Yang Li,Kui Jiang,Zihan Geng,Chia-Wen Lin*

Main category: cs.CV

TL;DR: S2ML is a spatio-spectral mutual learning framework that combines spatial and frequency domain analysis for depth completion, outperforming state-of-the-art methods by leveraging complementary physical characteristics of depth images.


<details>
  <summary>Details</summary>
Motivation: Raw depth images from RGB-D cameras often have incomplete depth values due to weak reflections, boundary shadows, and artifacts, limiting their use in vision tasks. Existing methods overlook the physical characteristics and frequency distribution patterns altered by invalid depth areas.
...
Result: Outperforms state-of-the-art method CFormer by 0.828 dB on NYU-Depth V2 and 0.834 dB on SUN RGB-D datasets, demonstrating superior depth completion performance.

Conclusion: The S2ML framework effectively explores complementary physical characteristics and priors by integrating spatial and frequency domain information, achieving more accurate depth completion than existing methods.
...
### [71] [StreamSTGS: Streaming Spatial and Temporal Gaussian Grids for Real-Time Free-Viewpoint Video](https://arxiv.org/abs/2511.06046)
*Zhihui Ke,Yuyang Liu,Xiaobo Zhou,Tie Qiu*

Main category: cs.CV

TL;DR: StreamSTGS is a novel free-viewpoint video representation that enables real-time streaming by compressing 3D Gaussian attributes as 2D images and temporal features as video, achieving competitive performance with significantly reduced frame sizes.


<details>
  <summary>Details</summary>
Motivation: Current 3DGS-based FVV methods face storage challenges (up to 10MB per frame) that prevent real-time streaming, requiring a more efficient representation for practical deployment.

Method: Represents dynamic scenes using canonical 3D Gaussians, temporal features, and deformation fields; encodes Gaussian attributes as 2D images and temporal features as video; uses sliding window for local motion and transformer-guided module for global motion.

Result: Achieves competitive performance on FVV benchmarks with average PSNR increase of 1dB while reducing frame size to just 170KB (compared to 10MB in previous methods).

Conclusion: StreamSTGS enables real-time streaming of free-viewpoint video with adaptive bitrate control and significantly improved compression efficiency without compromising quality.
...
### [72] [Neodragon: Mobile Video Generation using Diffusion Transformer](https://arxiv.org/abs/2511.06055)
...
<details>
  <summary>Details</summary>
Motivation: To enable low-cost, private, and on-device text-to-video synthesis that democratizes AI-based video content creation, eliminating reliance on cloud services.

Method: Four technical contributions: (1) Text-Encoder Distillation using DT5, (2) Asymmetric Decoder Distillation, (3) MMDiT block pruning with two-stage distillation, (4) Step distillation using DMD for pyramidal flow-matching to reduce NFE requirements.

Result: Achieves 7 FPS generation speed, 81.61 VBench score, with 4.945B parameters, 3.5GB peak RAM usage, and 6.7s end-to-end latency on mobile hardware.

Conclusion: Neodragon successfully demonstrates efficient high-fidelity video synthesis on mobile devices, making AI video generation accessible without cloud dependency.
...
### [73] [LoopExpose: An Unsupervised Framework for Arbitrary-Length Exposure Correction](https://arxiv.org/abs/2511.06066)
*Ao Li,Chen Chen,Zhenyu Wang,Tao Huang,Fangfang Wu,Weisheng Dong*

Main category: cs.CV

TL;DR: LoopExpose is an unsupervised exposure correction method that uses nested loop optimization with pseudo-labels and multi-exposure fusion, achieving state-of-the-art performance without requiring labeled data.


<details>
  <summary>Details</summary>
Motivation: Supervised learning for exposure correction relies on large labeled datasets that are difficult to obtain in practice, creating a need for unsupervised methods that can work without ground truth data.

Method: Proposes a nested loop optimization framework with two levels: upper-level trains correction model using pseudo-labels from lower-level multi-exposure fusion, with feedback mechanism to refine pseudo-labels. Introduces Luminance Ranking Loss as self-supervised constraint using relative luminance ordering.
...
Conclusion: LoopExpose demonstrates that unsupervised exposure correction through nested loop optimization with pseudo-labels and self-supervised constraints can effectively address the challenge of limited labeled data while achieving competitive performance.
...
### [74] [An Artificial Intelligence-based Assistant for the Visually Impaired](https://arxiv.org/abs/2511.06080)
...
<details>
  <summary>Details</summary>
Motivation: Visually impaired individuals face challenges in object identification, text reading, and navigation that limit their independence and quality of life, with existing solutions not being effective in all situations.

Method: Uses state-of-the-art machine learning algorithms including You Only Look Once architectures and a Large Language and Vision Assistant, with methods to facilitate user interaction and access to information.

Result: The application aims to enhance user autonomy and access to information, with user feedback supporting improved perception of daily usability.

Conclusion: AIDEN successfully contributes to improving the quality of life for visually impaired individuals by leveraging AI technologies to address their daily challenges.
...
### [75] [Hybrid CNN-ViT Framework for Motion-Blurred Scene Text Restoration](https://arxiv.org/abs/2511.06087)
*Umar Rashid,Muhammad Arslan Arshad,Ghulam Ahmad,Muhammad Zeeshan Anjum,Rizwan Khan,Muhammad Akmal*

Main category: cs.CV

TL;DR: Hybrid CNN-ViT framework for motion-blurred scene text restoration, combining local feature extraction with global contextual reasoning to achieve high-quality deblurring with computational efficiency.
...
Method: Combines CNN encoder-decoder for structural detail preservation with transformer module for global awareness via self-attention. Trained on synthetic blurred TextOCR data using realistic motion-blur kernels. Optimized with composite loss (MAE, MSE, perceptual similarity, SSIM).

Result: Achieves 32.20 dB PSNR and 0.934 SSIM with 2.83M parameters and 61ms inference time, demonstrating both effectiveness and computational efficiency.

Conclusion: The CNN-ViT hybrid design proves practical for real-world motion-blurred scene-text restoration, effectively balancing performance and efficiency.
...
### [76] [DiLO: Disentangled Latent Optimization for Learning Shape and Deformation in Grouped Deforming 3D Objects](https://arxiv.org/abs/2511.06115)
*Mostofa Rafid Uddin,Jana Armouti,Umong Sain,Md Asib Rahman,Xingjian Li,Min Xu*

Main category: cs.CV

TL;DR: Proposes an unsupervised method to disentangle 3D object deformations into shape and deformation factors using latent optimization and regularization, with applications in deformation transfer, classification, and explainability.


<details>
  <summary>Details</summary>
Motivation: To parameterize grouped deforming 3D objects into disentangled shape and deformation factors without supervision, enabling various downstream applications.
...
Result: Highly effective performance on 3D human, animal, and facial expression datasets, comparable or superior to more complex existing methods.

Conclusion: The simple approach successfully achieves unsupervised disentanglement of shape and deformation factors, enabling practical applications like deformation transfer and classification.
...
### [77] [Latent Refinement via Flow Matching for Training-free Linear Inverse Problem Solving](https://arxiv.org/abs/2511.06138)
*Hossein Askari,Yadan Luo,Hongfu Sun,Fred Roosta*

Main category: cs.CV

TL;DR: LFlow is a training-free framework that uses pretrained latent flow priors to solve linear inverse problems, operating in latent space for efficiency and introducing theoretically grounded posterior covariance for better guidance.


<details>
  <summary>Details</summary>
Motivation: Current flow-based inverse solvers have limitations: they operate in pixel space (computationally expensive) and use guidance strategies with prior-agnostic posterior covariances that weaken alignment with generative trajectories.

Method: LFlow leverages flow matching to perform ODE sampling in latent space along an optimal path, and introduces a theoretically grounded posterior covariance derived from the optimal vector field for effective flow guidance.
...
Conclusion: LFlow provides an efficient and effective approach for solving linear inverse problems using latent flow priors, with improved reconstruction quality compared to existing methods.
...
### [78] [Real-Time Bundle Adjustment for Ultra-High-Resolution UAV Imagery Using Adaptive Patch-Based Feature Tracking](https://arxiv.org/abs/2511.06152)
*Selim Ahmet Iz,Francesco Nex,Norman Kerle,Henry Meissner,Ralf Berger*

Main category: cs.CV

TL;DR: A real-time bundle adjustment framework for UAV imagery that processes full-resolution images without downsampling by dividing them into patches and using localized optimization with sliding clusters.


<details>
  <summary>Details</summary>
Motivation: Real-time processing of high-resolution UAV imagery is essential for time-critical applications like disaster response, but conventional bundle adjustment methods are too slow or sacrifice detail through downsampling.

Method: Divides images into patches, dynamically tracks them using GNSS/IMU data and DSM, determines image overlaps in real-time, and performs localized bundle adjustment on sliding clusters of overlapping images.

Result: Validated on 50MP MACS datasets, the method maintains precise camera orientations and high-fidelity mapping across multiple strips, running full bundle adjustment in under 2 seconds without GPU acceleration.

Conclusion: The proposed lightweight framework enables real-time bundle adjustment on full-resolution UAV imagery, making it suitable for time-critical missions like disaster response while preserving global BA accuracy.
...
### [79] [MambaOVSR: Multiscale Fusion with Global Motion Modeling for Chinese Opera Video Super-Resolution](https://arxiv.org/abs/2511.06172)
...
<details>
  <summary>Details</summary>
Motivation: Preserve classical Chinese opera performances by addressing degradation in early filmed videos due to equipment limitations (low frame rates, resolution), and overcome challenges in applying existing STVSR methods to opera videos with large motions.

Method: MambaOVSR with three key components: Global Fusion Module for motion modeling via multiscale alternating scanning, Multiscale Synergistic Mamba Module for alignment across sequence lengths, and MambaVR block to resolve feature artifacts and positional information loss.

Result: Significantly outperforms SOTA STVSR method by average 1.86 dB in PSNR on the COVC dataset, demonstrating superior performance in handling opera videos with large motions.

Conclusion: The proposed MambaOVSR effectively addresses the challenges of Chinese opera video super-resolution, with both dataset and code to be publicly released to advance archival preservation efforts.
...
### [80] [NURBGen: High-Fidelity Text-to-CAD Generation through LLM-Driven NURBS Modeling](https://arxiv.org/abs/2511.06194)
*Muhammad Usama,Mohammad Sadil Khan,Didier Stricker,Muhammad Zeshan Afzal*

Main category: cs.CV

TL;DR: NURBGen is a framework that generates editable 3D CAD models from text using NURBS representations, achieving high geometric fidelity and dimensional accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-CAD systems produce meshes or rely on scarce design-history data, making editable 3D CAD generation from natural language challenging.

Method: Fine-tune an LLM to translate text into JSON with NURBS parameters, use hybrid representation combining NURBS with analytic primitives, and create partABC dataset with automated captioning.

Result: NURBGen surpasses prior methods in geometric fidelity and dimensional accuracy, confirmed by expert evaluations on diverse prompts.

Conclusion: The framework enables direct generation of high-fidelity 3D CAD models from text using NURBS, with code and dataset to be released publicly.
...
### [81] [Scene-Aware Urban Design: A Human-AI Recommendation Framework Using Co-Occurrence Embeddings and Vision-Language Models](https://arxiv.org/abs/2511.06201)
*Rodrigo Gallardo,Oz Fishman,Alexander Htet Kyaw*

Main category: cs.CV

TL;DR: A human-in-the-loop computer vision framework using generative AI to propose micro-scale design interventions in public spaces, enabling continuous local participation through object detection and spatial configuration analysis.
...
Method: Uses Grounding DINO and curated ADE20K dataset for urban object detection, builds co-occurrence embeddings to reveal spatial configurations, and employs vision language models to suggest complementary objects that complete urban tactics.

Result: The system provides users with five statistically likely complements to chosen anchor objects and suggests third objects to complete complex urban tactics, while keeping people in control of selection and refinement.

Conclusion: The framework successfully integrates generative AI with human oversight to enable participatory urban design interventions that are grounded in local spatial patterns and lived experiences.
...
### [82] [MoRA: Missing Modality Low-Rank Adaptation for Visual Recognition](https://arxiv.org/abs/2511.06225)
*Shu Zhao,Nilesh Ahuja,Tan Yu,Tianyi Shen,Vijaykrishnan Narayanan*

Main category: cs.CV

TL;DR: MoRA is a parameter-efficient fine-tuning method for vision-language models that handles missing modalities by modeling cross-modal interactions and maintaining modality-specific adaptations.


<details>
  <summary>Details</summary>
Motivation: Real-world scenarios often have missing modalities due to privacy, collection difficulties, or resource limitations, but existing prompt learning approaches fail to capture cross-modal relationships effectively and have computational overhead.
...
Result: MoRA achieves 5.24% average performance improvement in missing-modality scenarios, uses only 25.90% of inference time compared to SOTA, and requires only 0.11% of trainable parameters compared to full fine-tuning.

Conclusion: MoRA effectively addresses missing modality challenges in vision-language models through efficient cross-modal interaction modeling while maintaining high performance and computational efficiency.
...
### [83] [Temporal-Guided Visual Foundation Models for Event-Based Vision](https://arxiv.org/abs/2511.06238)
*Ruihao Xia,Junhong Cai,Luziwei Leng,Liuyi Wang,Chengju Liu,Ran Cheng,Yang Tang,Pan Zhou*

Main category: cs.CV

TL;DR: TGVFM integrates Visual Foundation Models with temporal context fusion to process event camera data, achieving state-of-the-art performance in vision tasks like segmentation, depth estimation, and object detection.


<details>
  <summary>Details</summary>
Motivation: Event cameras excel in challenging environments but processing their asynchronous event streams is difficult. Existing methods are specialized or resource-intensive, and the potential of image-based Visual Foundation Models for event-based vision remains underexplored.

Method: Proposes Temporal-Guided VFM (TGVFM) with three key components: Long-Range Temporal Attention for global dependencies, Dual Spatiotemporal Attention for multi-scale frame correlation, and Deep Feature Guidance Mechanism for semantic-temporal fusion. Retrains event-to-video models on real data using transformer-based VFMs.
...
Conclusion: TGVFM successfully bridges the gap between image-based Visual Foundation Models and event-based vision, enabling cross-modality transfer with temporal reasoning capabilities.
...
### [84] [Physics-Informed Image Restoration via Progressive PDE Integration](https://arxiv.org/abs/2511.06244)
*Shamika Likhite,Santiago López-Tapia,Aggelos K. Katsaggelos*

Main category: cs.CV

TL;DR: Proposes a physics-informed PDE framework for motion deblurring that integrates advection-diffusion equations into deep learning architectures to capture directional blur patterns and global spatial dependencies.


<details>
  <summary>Details</summary>
Motivation: Motion blur degrades image quality and impairs computer vision tasks. Existing deep learning methods struggle with long-range spatial dependencies in blur patterns, requiring extremely deep networks for global modeling.

Method: Progressive training framework integrating physics-informed PDE dynamics (advection-diffusion equations) into restoration architectures to model feature evolution and capture directional flow characteristics of motion blur.

Result: Achieves superior restoration quality with minimal overhead (~1% increase in inference GMACs), consistently improving perceptual quality across FFTformer, NAFNet, Restormer, and Stripformer architectures on standard benchmarks.
...
### [85] [Gait Recognition via Collaborating Discriminative and Generative Diffusion Models](https://arxiv.org/abs/2511.06245)
*Haijun Xiong,Bin Feng,Bang Wang,Xinggang Wang,Wenyu Liu*

Main category: cs.CV

TL;DR: CoD² is a novel gait recognition framework that combines diffusion models with discriminative models using multi-level conditional control to generate identity-consistent gait sequences and learn robust features.


<details>
  <summary>Details</summary>
Motivation: Gait recognition provides non-intrusive biometric identification, but generative models' potential remains underexplored compared to discriminative models' success.

Method: Proposes Multi-level Conditional Control strategy with high-level identity-aware semantic conditions from discriminative extractor and low-level visual details (appearance, motion) to guide diffusion model generation.
...
### [86] [AdaDrive: Self-Adaptive Slow-Fast System for Language-Grounded Autonomous Driving](https://arxiv.org/abs/2511.06253)
*Ruifei Zhang,Junlin Xie,Wei Zhang,Weikai Chen,Xiao Tan,Xiang Wan,Guanbin Li*

Main category: cs.CV

TL;DR: AdaDrive is an adaptive slow-fast framework that optimizes when and how LLMs are used in autonomous driving, balancing reasoning capabilities with real-time efficiency through dynamic activation and fusion strategies.


<details>
  <summary>Details</summary>
Motivation: Existing approaches either activate LLMs too frequently (causing computational overhead) or use fixed schedules (failing to adapt to dynamic driving conditions), requiring a more intelligent integration strategy.

Method: AdaDrive uses: (1) adaptive activation loss with comparative learning to determine when to invoke LLMs, and (2) adaptive fusion strategy that modulates continuous LLM influence based on scene complexity and prediction confidence.

Result: Extensive experiments show state-of-the-art performance in both driving accuracy and computational efficiency on language-grounded autonomous driving benchmarks.
...
### [87] [VLDrive: Vision-Augmented Lightweight MLLMs for Efficient Language-grounded Autonomous Driving](https://arxiv.org/abs/2511.06256)
*Ruifei Zhang,Wei Zhang,Xiao Tan,Sibei Yang,Xiang Wan,Xiaonan Luo,Guanbin Li*

Main category: cs.CV

TL;DR: VLDrive introduces a lightweight MLLM architecture for autonomous driving that reduces parameters by 81% while improving driving performance through enhanced vision components and novel attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based autonomous driving approaches face challenges with frequent collisions due to visual representation limitations and deployment hurdles from large parameter counts.

Method: VLDrive uses a lightweight MLLM architecture with cycle-consistent dynamic visual pruning, memory-enhanced feature aggregation, and distance-decoupled instruction attention for improved visual-linguistic feature learning.

Result: VLDrive achieves state-of-the-art driving performance with 81% parameter reduction (from 7B to 1.3B) and driving score improvements of 15.4%, 16.8%, and 7.6% at tiny, short, and long distances respectively in CARLA simulator evaluations.
...
### [88] [Robust Nearest Neighbour Retrieval Using Targeted Manifold Manipulation](https://arxiv.org/abs/2511.06261)
...
Method: Uses lightweight query-specific trigger patches added to query images, weakly backdooring the network to steer patch-containing inputs toward a dummy class. Similar images require only slight shifts to be classified as dummy class.
...
### [89] [A Mixture-of-Experts Framework with Log-Logistic Components for Survival Analysis on Histopathology Images](https://arxiv.org/abs/2511.06266)
...
<details>
  <summary>Details</summary>
Motivation: To develop an improved method for predicting cancer-specific survival from whole slide pathology images by capturing tissue heterogeneity and complex survival distributions.

Method: Four-component framework: Quantile Gated Patch Selection, Graph Guided Clustering, Hierarchical Context Attention, and Expert Driven Mixture of Log-logistics for survival distribution estimation.

Result: Achieved concordance indices of 0.644 (TCGA LUAD), 0.751 (TCGA KIRC), and 0.752 (TCGA BRCA), outperforming state-of-the-art methods.

Conclusion: The proposed modular framework effectively predicts cancer survival by integrating multiple components to handle tissue heterogeneity and complex survival patterns.
...
### [90] [LLM-Driven Completeness and Consistency Evaluation for Cultural Heritage Data Augmentation in Cross-Modal Retrieval](https://arxiv.org/abs/2511.06268)
*Jian Zhang,Junyi Guo,Junyi Yuan,Huanda Lu,Yanlin Zhou,Fangyu Wu,Qiufeng Wang,Dongming Lu*

Main category: cs.CV

TL;DR: C³ is a data augmentation framework that enhances cross-modal retrieval by improving the completeness and consistency of LLM-generated descriptions for cultural heritage data.
...
Method: Proposes C³ framework with: 1) completeness evaluation module using visual cues and language-model outputs, 2) Markov Decision Process to supervise Chain-of-Thought reasoning for consistency evaluation with adaptive query control.
...
Conclusion: C³ effectively addresses LLM limitations in cross-modal retrieval by ensuring both completeness and consistency of generated descriptions, demonstrating strong performance across cultural heritage and general datasets.
...
### [91] [RelightMaster: Precise Video Relighting with Multi-plane Light Images](https://arxiv.org/abs/2511.06271)
*Weikang Bian,Xiaoyu Shi,Zhaoyang Huang,Jianhong Bai,Qinghe Wang,Xintao Wang,Pengfei Wan,Kun Gai,Hongsheng Li*

Main category: cs.CV

TL;DR: RelightMaster is a novel framework for precise video relighting that addresses limitations in existing text-to-video models by introducing Multi-plane Light Image (MPLI) visual prompts and leveraging a custom RelightVideo dataset built with Unreal Engine.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video models lack fine-grained lighting control due to text's limitations in describing lighting details and insufficient pre-training on lighting-related prompts. Constructing high-quality relighting training data is also challenging due to scarce real-world controllable lighting data.

Method: 1) Built RelightVideo dataset with identical dynamic content under varying lighting conditions using Unreal Engine; 2) Introduced Multi-plane Light Image (MPLI) - a novel visual prompt representing 3D light source positions, intensities, and colors; 3) Designed Light Image Adapter that compresses MPLI via pre-trained Video VAE and injects latent light features into Video Diffusion Transformers.
...
Conclusion: The proposed framework enables accurate and controllable video relighting by combining novel visual prompts (MPLI) with a specialized dataset and adapter mechanism, overcoming limitations of text-based lighting control in existing video generation models.
...
### [92] [LaneDiffusion: Improving Centerline Graph Learning via Prior Injected BEV Feature Generation](https://arxiv.org/abs/2511.06272)
*Zijie Wang,Weiming Zhang,Wei Zhang,Xiao Tan,Hongxing Liu,Yaowei Wang,Guanbin Li*

Main category: cs.CV

TL;DR: LaneDiffusion is a novel generative approach using diffusion models for centerline graph learning in autonomous driving, outperforming existing methods on multiple metrics.
...
Method: Uses diffusion models to generate lane centerline priors at BEV feature level, integrates Lane Prior Injection Module (LPIM) and Lane Prior Diffusion Module (LPDM), then decodes vectorized centerlines and topologies from prior-injected BEV features.

Result: Significantly outperforms existing methods on nuScenes and Argoverse2 datasets, achieving improvements of 4.2-6.4% on point-level metrics and 2.1-6.8% on segment-level metrics.
...
### [93] [VideoSSR: Video Self-Supervised Reinforcement Learning](https://arxiv.org/abs/2511.06281)
*Zefeng He,Xiaoye Qu,Yafu Li,Siyuan Huang,Daizong Liu,Yu Cheng*

Main category: cs.CV

TL;DR: VideoSSR is a self-supervised reinforcement learning framework that generates verifiable training data from video intrinsic information, improving MLLM performance by over 5% across 17 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Manual annotation of high-quality video datasets is expensive, and existing datasets are becoming insufficient for advancing MLLMs. The paper explores whether videos' intrinsic information can be leveraged to self-generate verifiable training data.

Method: Introduces three self-supervised pretext tasks (Anomaly Grounding, Object Counting, Temporal Jigsaw), creates VideoSSR-30K dataset, and proposes VideoSSR framework for video self-supervised reinforcement learning.

Result: VideoSSR consistently enhances model performance across 17 benchmarks in four video domains (General Video QA, Long Video QA, Temporal Grounding, Complex Reasoning), achieving average improvements over 5%.

Conclusion: VideoSSR establishes a potent foundational framework for developing more advanced video understanding in MLLMs by effectively leveraging intrinsic video information for self-supervised learning.
...
### [94] [From ACR O-RADS 2022 to Explainable Deep Learning: Comparative Performance of Expert Radiologists, Convolutional Neural Networks, Vision Transformers, and Fusion Models in Ovarian Masses](https://arxiv.org/abs/2511.06282)
...
TL;DR: Deep learning models, especially Vision Transformers, significantly outperform radiologists using O-RADS v2022 for ovarian lesion classification, and hybrid human-AI frameworks further enhance diagnostic accuracy.
...
Method: Retrospective study of 512 adnexal mass images from 227 patients, comparing radiologist O-RADS assessments with 16 deep learning models (CNNs and ViTs) and developing hybrid models combining radiologist scores with AI predictions.

Result: Radiologists achieved AUC 0.683 and 68.0% accuracy. CNN models ranged from AUC 0.620-0.908, while ViT16-384 performed best with AUC 0.941 and 87.4% accuracy. Hybrid frameworks significantly improved CNN performance but not ViT models.

Conclusion: Deep learning models substantially outperform radiologists, and hybrid human-AI approaches provide the highest diagnostic accuracy, offering potential to standardize ultrasound interpretation and improve lesion detection.
...
### [95] [TinyChemVL: Advancing Chemical Vision-Language Models via Efficient Visual Token Reduction and Complex Reaction Tasks](https://arxiv.org/abs/2511.06283)
*Xuanle Zhao,Shuxin Zeng,Yinyuan Cai,Xiang Cheng,Duzhen Zhang,Xiuyi Chen,Bo Xu*

Main category: cs.CV

TL;DR: TinyChemVL is an efficient 4B-parameter chemical vision-language model that uses visual token reduction and reaction-level tasks to improve efficiency and reasoning capacity, outperforming existing models while using significantly fewer visual tokens.
...
Method: Proposed TinyChemVL with visual token reduction and reaction-level tasks, plus ChemRxn-V benchmark for vision-based reaction recognition and prediction. Directly predicts reaction products from molecular images requiring both recognition and reasoning.

Result: TinyChemVL achieves superior performance on both molecular and reaction tasks with faster inference and training speeds, outperforming ChemVLM while using only 1/16th of the visual tokens.

Conclusion: This work builds efficient yet powerful VLMs for chemical domains through co-designing model architecture and task complexity, demonstrating that smaller models can achieve better performance with proper design.
...
### [96] [Enhancing Multimodal Misinformation Detection by Replaying the Whole Story from Image Modality Perspective](https://arxiv.org/abs/2511.06284)
*Bing Wang,Ximing Li,Yanjun Wang,Changchun Li,Lin Yuanbo Wu,Buyu Wang,Shengsheng Wang*

Main category: cs.CV

TL;DR: RETSIMD is a multimodal misinformation detection method that focuses on text as the primary modality, using text segmentation and text-to-image generation to augment images, with graph neural networks for feature fusion.


<details>
  <summary>Details</summary>
Motivation: Text modality is more informative than images in misinformation detection since text describes the whole event while images show only partial scenes, and preliminary results show images contribute less to MMD.

Method: Split text into segments, use pre-trained text-to-image generator to create image sequences, incorporate text-image and image-label mutual information objectives, post-train generator, and use graph neural networks with heuristic image relationships for feature fusion.

Result: Extensive empirical results validate the effectiveness of RETSIMD for multimodal misinformation detection.

Conclusion: RETSIMD effectively addresses multimodal misinformation detection by leveraging text as the primary information source and using generated images and graph-based fusion to enhance detection performance.
...
### [97] [Learning-Based Vision Systems for Semi-Autonomous Forklift Operation in Industrial Warehouse Environments](https://arxiv.org/abs/2511.06295)
*Vamshika Sutar,Mahek Maheshwari,Archak Mittal*

Main category: cs.CV

TL;DR: Vision-based framework using YOLOv8 and YOLOv11 for pallet and pallet hole detection in warehouses, with hyperparameter optimization and spatial mapping for forklift automation.


<details>
  <summary>Details</summary>
Motivation: To develop robust, low-cost perception systems for forklifts and AGVs in warehouse automation, enabling safer and more economical logistics operations.

Method: Used YOLOv8 and YOLOv11 architectures with Optuna hyperparameter optimization and spatial post-processing, plus an innovative pallet hole mapping module for spatial representation.

Result: YOLOv8 achieved high detection accuracy, while YOLOv11 with optimized configurations offered superior precision and stable convergence on custom warehouse dataset.

Conclusion: Demonstrates feasibility of cost-effective, retrofittable visual perception for forklifts, providing a scalable approach to advance warehouse automation.
...
### [98] [SFFR: Spatial-Frequency Feature Reconstruction for Multispectral Aerial Object Detection](https://arxiv.org/abs/2511.06298)
*Xin Zuo,Yuchen Qu,Haibo Zhan,Jifeng Shen,Wankou Yang*

Main category: cs.CV

TL;DR: Proposes SFFR method using spatial-frequency feature reconstruction with KAN networks for multispectral object detection, featuring FCEKAN for frequency component exchange and MSGKAN for multi-scale spatial feature modeling.


<details>
  <summary>Details</summary>
Motivation: Current multispectral object detection methods focus mainly on spatial-domain fusion, while frequency-domain features remain underexplored. There's a need to leverage both spatial and frequency domains for better feature representation.

Method: SFFR method with two core modules: FCEKAN for selective frequency component exchange between RGB and IR images, and MSGKAN using multi-scale Gaussian basis functions for spatial domain feature modeling to handle scale variations from different UAV altitudes.

Result: Extensive experiments on SeaDroneSee, DroneVehicle and DVTOD datasets demonstrate superior performance in UAV multispectral object perception tasks. The modules effectively capture complementary frequency and spatial semantic features.

Conclusion: The proposed SFFR method with FCEKAN and MSGKAN modules provides significant advantages for multispectral object detection by effectively leveraging both spatial and frequency domain features, enhancing adaptability to scale variations in UAV applications.
...
### [99] [Physics-Informed Deformable Gaussian Splatting: Towards Unified Constitutive Laws for Time-Evolving Material Field](https://arxiv.org/abs/2511.06299)
...
<details>
  <summary>Details</summary>
Motivation: Pure data-driven 3DGS struggles to capture physics-driven motion patterns in dynamic scenes, necessitating physics-informed approaches for better motion representation.
...
Conclusion: Physics-informed approach enhances 3DGS for dynamic scenes by incorporating Lagrangian mechanics and optical flow supervision, achieving better motion capture and generalization.
...
### [100] [Adaptive 3D Reconstruction via Diffusion Priors and Forward Curvature-Matching Likelihood Updates](https://arxiv.org/abs/2511.06310)
*Seunghyeok Shin,Dabin Kim,Hongki Lim*

Main category: cs.CV

TL;DR: The paper proposes Forward Curvature-Matching (FCM), a novel method that dynamically determines optimal step sizes for likelihood updates in diffusion-based point cloud reconstruction, enabling high-fidelity reconstruction from single/multi-view inputs without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based methods for point cloud reconstruction suffer from inflexibility - requiring conditioning during training, fixed input views, and complete retraining for different measurements. Recent approaches use heuristic fixed step sizes that lead to slow convergence and suboptimal quality.

Method: Integrates FCM update method with diffusion sampling, using forward automatic differentiation and finite-difference curvature estimates to dynamically determine optimal step sizes for precise likelihood optimization. Supports various input modalities through simple operator substitution without retraining.

Result: Achieves superior reconstruction quality on ShapeNet and CO3D datasets at matched or lower NFEs, with higher F-score and lower CD and EMD metrics. Enables high-fidelity reconstruction from both single-view and multi-view inputs.

Conclusion: FCM method provides efficient and adaptable point cloud reconstruction, validating its practical applicability through improved quality and flexibility without requiring retraining for different measurement scenarios.
...
### [101] [Seq2Seq Models Reconstruct Visual Jigsaw Puzzles without Seeing Them](https://arxiv.org/abs/2511.06315)
*Gur Elkn,Ofir Itzhak Shahar,Ohad Ben-Shahar*

Main category: cs.CV

TL;DR: Language models can solve jigsaw puzzles without visual input by treating puzzle pieces as token sequences and reframing reassembly as sequence-to-sequence prediction.


<details>
  <summary>Details</summary>
Motivation: To explore unconventional approaches to jigsaw puzzle solving by using language models instead of traditional vision-based methods, testing their capability to solve problems beyond their native domain.

Method: Developed a specialized tokenizer to convert puzzle pieces into discrete token sequences, then used encoder-decoder transformers as "blind" solvers to reconstruct the layout through sequence-to-sequence prediction without visual input.

Result: Achieved state-of-the-art performance across multiple benchmarks, often outperforming vision-based methods despite being restricted from accessing raw visual input.

Conclusion: Language models demonstrate surprising capability to solve visual problems like jigsaw puzzles, suggesting unconventional approaches can inspire new directions in puzzle-solving research.
...
TL;DR: CINEMAE adapts text detection principles to images using Masked AutoEncoder reconstruction uncertainty for AI-generated content detection, achieving strong cross-generator generalization.
...
Method: Uses Masked AutoEncoder trained to reconstruct masked patches, computes conditional Negative Log-Likelihood to quantify local semantic anomalies, and aggregates patch-level statistics with global features through learned fusion.

Result: Trained only on Stable Diffusion v1.4, achieves over 95% accuracy on all eight unseen generators in GenImage benchmark, substantially outperforming state-of-the-art detectors.
...
### [103] [Improving Multimodal Sentiment Analysis via Modality Optimization and Dynamic Primary Modality Selection](https://arxiv.org/abs/2511.06328)
*Dingkang Yang,Mingcheng Li,Xuecheng Wu,Zhaoyu Chen,Kaixun Jiang,Keliang Liu,Peng Zhai,Lihua Zhang*

Main category: cs.CV

TL;DR: Proposes MODS framework for multimodal sentiment analysis that dynamically selects primary modalities and optimizes acoustic/visual sequences using graph-based compression to address imbalanced modality performance and redundancy issues.


<details>
  <summary>Details</summary>
Motivation: Existing approaches use fixed primary modality strategies that fail to adapt to dynamic variations in modality importance across samples, and non-language modalities suffer from sequential redundancy and noise that degrade performance.

Method: MODS framework with three components: Graph-based Dynamic Sequence Compressor (GDC) using capsule networks and graph convolution to reduce redundancy; sample-adaptive Primary Modality Selector (MSelector) for dynamic dominance determination; Primary-modality-Centric Cross-Attention (PCCA) module to enhance dominant modalities and cross-modal interaction.
...
Conclusion: The proposed MODS framework successfully addresses modality imbalance and redundancy issues in multimodal sentiment analysis through dynamic primary modality selection and sequence optimization, demonstrating improved performance over existing approaches.
...
### [104] [Label-Efficient 3D Forest Mapping: Self-Supervised and Transfer Learning for Individual, Structural, and Species Analysis](https://arxiv.org/abs/2511.06331)
*Aldino Rizaldy,Fabian Ewald Fassnacht,Ahmed Jamal Afifi,Hua Jiang,Richard Gloaguen,Pedram Ghamisi*

Main category: cs.CV

TL;DR: The paper presents a unified framework using self-supervised and transfer learning to reduce dependence on large annotated datasets for individual tree analysis from 3D point clouds, achieving significant performance improvements in instance segmentation, semantic segmentation, and tree classification.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of labor-intensive and difficult-to-scale dense annotations required for deep learning models in forestry applications, while supporting precision forestry, biodiversity conservation, and carbon mapping.

Method: Combined self-supervised learning with domain adaptation for instance segmentation, used self-supervised learning alone for semantic segmentation, and implemented hierarchical transfer learning for tree species classification.

Result: Achieved AP50 +16.98% for instance segmentation, mIoU +1.79% for semantic segmentation, and Jaccard +6.07% for tree classification of unseen species. Pretrained models reduced energy consumption and carbon emissions by ~21%.

Conclusion: The proposed unified framework with self-supervised and transfer learning effectively reduces annotation dependency and enables operational extraction of individual tree information from laser scanning point clouds, supporting forestry, biodiversity, and carbon mapping applications.
...
### [105] [BuildingWorld: A Structured 3D Building Dataset for Urban Foundation Models](https://arxiv.org/abs/2511.06337)
*Shangfeng Huang,Ruisheng Wang,Xin Wang*

Main category: cs.CV

TL;DR: BuildingWorld is a comprehensive 3D building dataset addressing the lack of architectural diversity in existing datasets, featuring 5 million LOD2 models from global regions with real/simulated LiDAR data.


<details>
  <summary>Details</summary>
Motivation: Current 3D building datasets have limited architectural diversity, undermining model generalizability across heterogeneous urban environments for applications like energy modeling and autonomous navigation.

Method: Collected ~5 million LOD2 building models from diverse global regions (North America, Europe, Asia, Africa, Oceania) with real and simulated airborne LiDAR point clouds, plus Cyber City virtual model for unlimited training data generation.
...
Conclusion: BuildingWorld bridges the diversity gap in 3D building datasets, facilitating urban-scale foundation modeling and analysis for digital twin applications.
...
### [106] [GazeVLM: A Vision-Language Model for Multi-Task Gaze Understanding](https://arxiv.org/abs/2511.06348)
...
<details>
  <summary>Details</summary>
Motivation: Prior research has modeled gaze cues, but a unified system combining visual and language modalities for comprehensive gaze understanding was still needed.
...
Result: The model achieves state-of-the-art evaluation scores on GazeFollow and VideoAttentionTarget datasets, with ablation studies showing superior performance from RGB+HHA depth fusion.

Conclusion: GazeVLM successfully demonstrates the effectiveness of Vision-Language Models for multi-task gaze understanding, introducing a new object-level gaze detection metric ($AP_{ob}$) and showing significant improvements over existing methods.
...
### [107] [AesTest: Measuring Aesthetic Intelligence from Perception to Production](https://arxiv.org/abs/2511.06360)
*Guolong Wang,Heng Huang,Zhiqiang Zhang,Wentian Li,Feilong Ma,Xin Jin*

Main category: cs.CV

TL;DR: AesTest is a comprehensive benchmark for evaluating multimodal aesthetic perception and production in MLLMs, covering 10 tasks across perception, appreciation, creation, and photography with diverse data sources and query types.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for image aesthetic assessment are narrow in scope and lack diversity needed to evaluate systematic aesthetic production capabilities in multimodal large language models.

Method: Created AesTest benchmark with curated multiple-choice questions spanning 10 tasks grounded in psychological theories, integrating data from professional editing workflows, photography tutorials, and crowdsourced preferences to cover both expert principles and real-world variation.

Result: Evaluation of instruction-tuned IAA MLLMs and general MLLMs on AesTest revealed significant challenges in building aesthetic intelligence, indicating current limitations in multimodal aesthetic capabilities.

Conclusion: AesTest addresses the gap in multimodal aesthetic evaluation and will be publicly released to support future research in developing aesthetic intelligence in MLLMs.
...
### [108] [V-Shuffle: Zero-Shot Style Transfer via Value Shuffle](https://arxiv.org/abs/2511.06365)
*Haojun Tang,Qiwei Lin,Tongda Xu,Lida Huang,Yan Wang*

Main category: cs.CV

TL;DR: V-Shuffle is a zero-shot style transfer method that uses multiple style images to prevent content leakage by shuffling value features in diffusion model attention layers and adding hybrid style regularization.
...
Method: Shuffles value features within self-attention layers of diffusion models to disrupt semantic content while preserving low-level style, and adds Hybrid Style Regularization for high-level style textures.

Result: Achieves excellent performance with multiple style images and outperforms previous state-of-the-art methods when using a single style image.

Conclusion: V-Shuffle effectively navigates the trade-off between content preservation and style fidelity by leveraging multiple style images and disrupting semantic content while preserving style representations.
...
### [109] [InfoAffect: A Dataset for Affective Analysis of Infographics](https://arxiv.org/abs/2511.06404)
*Zihang Fu,Yunchao Wang,Chenyu Huang,Guodao Sun,Ronghua Liang*

Main category: cs.CV

TL;DR: Created InfoAffect dataset with 3.5K affect-annotated infographics to explore affective dimensions, validated with high accuracy (CACI score 0.986).


<details>
  <summary>Details</summary>
Motivation: Infographics are widely used but their affective dimensions remain underexplored due to lack of data resources.

Method: Collected data from 6 domains, used preprocessing and quality control strategies, constructed affect table for annotation, employed 5 MLLMs with RRF fusion for robust affect analysis.

Result: Achieved high accuracy with Composite Affect Consistency Index (CACI) score of 0.986, indicating reliable affect annotations.

Conclusion: InfoAffect dataset successfully addresses the scarcity of affect-annotated infographic data and provides a valuable resource for affective computing research.
...
### [110] [On Modality Incomplete Infrared-Visible Object Detection: An Architecture Compatibility Perspective](https://arxiv.org/abs/2511.06406)
*Shuo Yang,Yinghui Xing,Shizhou Zhang,Zhilong Niu*

Main category: cs.CV

TL;DR: Proposes Scarf-DETR with modality-agnostic deformable attention for robust infrared-visible object detection under incomplete modality scenarios.


<details>
  <summary>Details</summary>
Motivation: Current IVOD models suffer performance declines when facing incomplete modality data, especially when dominant modality is missing.

Method: Plug-and-play Scarf Neck module with modality-agnostic deformable attention and pseudo modality dropout training strategy.

Result: Excellent performance in missing modality scenarios and superior results on standard IVOD benchmarks.

Conclusion: Scarf-DETR provides robust and flexible adaptation to any single or double modalities during training and inference.
...
### [111] [VDNeRF: Vision-only Dynamic Neural Radiance Field for Urban Scenes](https://arxiv.org/abs/2511.06408)
*Zhengyu Zou,Jingfeng Li,Hao Li,Xiaolei Hou,Jinwen Hu,Jingkun Chen,Lechao Cheng,Dingwen Zhang*

Main category: cs.CV

TL;DR: VDNeRF is a vision-only method that recovers camera trajectories and learns spatiotemporal representations for dynamic urban scenes without requiring camera pose information or expensive sensors.
...
Method: Uses two separate NeRF models: static NeRF optimizes camera poses and background, while dynamic NeRF incorporates 3D scene flow for dynamic objects. Includes training framework to resolve ambiguity between camera and object motion.

Result: Extensive evaluations show VDNeRF surpasses state-of-the-art NeRF-based pose-free methods in camera pose estimation and dynamic novel view synthesis.

Conclusion: VDNeRF enables accurate camera trajectory recovery and spatiotemporal learning for dynamic urban scenes using only visual data, without needing pose information or expensive sensors.
...
### [112] [DiffusionUavLoc: Visually Prompted Diffusion for Cross-View UAV Localization](https://arxiv.org/abs/2511.06422)
*Tao Liu,Kan Ren,Qian Chen*

Main category: cs.CV

TL;DR: DiffusionUavLoc is a cross-view UAV localization framework that uses geometric rendering to create pseudo-satellite images from UAV views and employs a text-free conditional diffusion model for robust feature learning in GNSS-denied environments.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of UAV localization in GNSS-denied environments where satellite-based methods fail, and overcome the geometric and appearance gaps between oblique UAV views and nadir satellite orthophotos in cross-view image retrieval.

Method: Uses training-free geometric rendering to synthesize pseudo-satellite images from UAV imagery as structural prompts, then employs a text-free conditional diffusion model that fuses multimodal structural cues to learn viewpoint-robust features.

Result: Achieves competitive performance for cross-view localization on University-1652 and SUES-200 datasets, particularly excelling in satellite-to-drone localization on University-1652.

Conclusion: The proposed DiffusionUavLoc framework provides an effective solution for UAV localization in GNSS-denied environments by leveraging diffusion models and geometric rendering, offering promising performance without requiring complex networks, text prompts, or extensive annotations.
...
### [113] [Diagnose Like A REAL Pathologist: An Uncertainty-Focused Approach for Trustworthy Multi-Resolution Multiple Instance Learning](https://arxiv.org/abs/2511.06433)
*Sungrae Hong,Sol Lee,Jisu Shin,Mun Yong Yi*

Main category: cs.CV

TL;DR: UFC-MIL is a novel multiple instance learning method that provides calibrated diagnostic predictions for histopathology by mimicking pathologists' examination behaviors and incorporating uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: Current multiple-resolution MIL approaches focus only on performance improvement but lack well-calibrated predictions that clinical experts can trust for reliable diagnostic results.

Method: Proposes UFC-MIL with patch-wise loss for learning latent patterns and uncertainty, attention-based architecture with neighbor patch aggregation, and calibration through patch-level uncertainty without multiple iterative inferences.
...
Conclusion: The proposed UFC-MIL successfully provides calibrated diagnostic predictions that mimic pathologists' examination behaviors, offering trustworthy AI diagnostic aid for histopathological specimen examination.
...
### [114] [Countering Multi-modal Representation Collapse through Rank-targeted Fusion](https://arxiv.org/abs/2511.06450)
*Seulgi Kim,Kiran Kokilepersaud,Mohit Prabhushankar,Ghassan AlRegib*

Main category: cs.CV

TL;DR: Proposes Rank-enhancing Token Fuser to address feature and modality collapse in multi-modal fusion using effective rank as a unifying framework, achieving state-of-the-art results in action anticipation.


<details>
  <summary>Details</summary>
Motivation: Multi-modal fusion suffers from feature collapse (dimensions lose discriminative power) and modality collapse (one modality dominates), but existing methods address these separately without a unified framework.

Method: Uses effective rank to quantify both collapses; proposes Rank-enhancing Token Fuser that selectively blends complementary features between modalities; evaluates modality combinations that mutually increase effective rank.

Result: Depth maintains representational balance when fused with RGB; R3D framework achieves up to 3.74% improvement over state-of-the-art on NTURGBD, UTKinect, and DARai datasets.

Conclusion: Effective rank provides a unified framework to address both feature and modality collapse; depth-RGB fusion avoids modality collapse while enhancing representation quality in action anticipation tasks.
...
### [115] [EIDSeg: A Pixel-Level Semantic Segmentation Dataset for Post-Earthquake Damage Assessment from Social Media Images](https://arxiv.org/abs/2511.06456)
...
<details>
  <summary>Details</summary>
Motivation: Existing remote sensing methods for post-earthquake damage assessment rely on costly aerial images, expert labeling, and produce only binary damage maps. Social media ground-level images offer valuable data but lack large pixel-level annotated datasets.

Method: Created EIDSeg dataset with 3,266 images from 9 major earthquakes (2008-2023), annotated across 5 damage classes. Developed three-phase cross-disciplinary annotation protocol enabling consistent segmentation by non-expert annotators with over 70% inter-annotator agreement.

Result: Benchmarked state-of-the-art segmentation models, identifying Encoder-only Mask Transformer (EoMT) as top performer with 80.8% mIoU. The dataset enables fine-grained damage classification of buildings (undamaged, damaged, destroyed) and roads (undamaged, damaged).

Conclusion: EIDSeg unlocks social networks' rich ground-level perspective for faster, finer-grained damage assessment in post-earthquake scenarios, overcoming limitations of traditional remote sensing methods.
...
### [116] [Inpaint360GS: Efficient Object-Aware 3D Inpainting via Gaussian Splatting for 360° Scenes](https://arxiv.org/abs/2511.06457)
*Shaoxiang Wang,Shihong Zhang,Christen Millerdurai,Rüdiger Westermann,Didier Stricker,Alain Pagani*

Main category: cs.CV

TL;DR: Inpaint360GS is a 360° scene editing framework using 3D Gaussian Splatting that enables multi-object removal and high-fidelity inpainting in complex 360° environments.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with 360° scene inpainting due to challenges in identifying target objects, handling severe occlusions in multi-object scenes, and maintaining consistent appearance across views.

Method: Proposes Inpaint360GS framework based on 3DGS that distills 2D segmentation into 3D and uses virtual camera views for contextual guidance to enable accurate object-level editing.

Result: Outperforms existing baselines and achieves state-of-the-art performance in 360° inpainting, with a new dataset created for this task.

Conclusion: Inpaint360GS provides an effective solution for multi-object removal and high-quality inpainting in complex 360° scenes, addressing key challenges in 3D scene editing.
...
### [117] [NOAH: Benchmarking Narrative Prior driven Hallucination and Omission in Video Large Language Models](https://arxiv.org/abs/2511.06475)
*Kyuho Lee,Euntae Kim,Jinwoo Choi,Buru Chang*

Main category: cs.CV

TL;DR: Video LLMs exhibit narrative prior bias causing hallucinations and omissions, so NOAH benchmark is introduced to systematically evaluate these errors through composite videos and specialized tasks.


<details>
  <summary>Details</summary>
Motivation: Video LLMs prioritize narrative coherence over visual grounding, leading to hallucinations (introducing non-existent events) and omissions (suppressing factual events), which need systematic evaluation.

Method: Created NOAH benchmark with composite videos by inserting clips from other sources into target videos, varying semantic similarity and insertion position, with captioning and three QA tasks (Existence, Temporal, Narrative).

Result: Most Video LLMs show narrative prior-induced hallucinations and omissions; error patterns vary by architecture and depend on event similarity/position; narrative reliance intensifies with fewer frames.

Conclusion: NOAH provides first standardized evaluation of narrative prior-induced errors in Video LLMs, establishing foundation for developing more reliable and trustworthy video understanding models.
...
### [118] [Zooming into Comics: Region-Aware RL Improves Fine-Grained Comic Understanding in Vision-Language Models](https://arxiv.org/abs/2511.06490)
*Yule Chen,Yufan Ren,Sabine Süsstrunk*

Main category: cs.CV

TL;DR: AI4VA-FG is the first comprehensive benchmark for evaluating Vision-Language Models on comic understanding, revealing significant performance gaps in current models and proposing novel training strategies including Region-Aware Reinforcement Learning.


<details>
  <summary>Details</summary>
Motivation: VLMs excel on natural images but struggle with comics due to stylized line art, onomatopoeia, and complex multi-panel layouts, creating a gap in visual narrative understanding.

Method: Created AI4VA-FG benchmark with dense annotations, evaluated state-of-the-art VLMs, and investigated post-training strategies including SFT-S, SFT-R, RL, and proposed Region-Aware Reinforcement Learning (RARL) for dynamic region attention.

Result: Current VLMs show substantial performance deficits on comic understanding tasks. RL and RARL applied to Qwen2.5-VL significantly improved entity recognition and storyline ordering capabilities.

Conclusion: Comic understanding remains an unsolved challenge for VLMs, but the proposed training strategies, particularly RARL, show promise for enhancing model performance in visual narrative domains.
...
### [119] [SportR: A Benchmark for Multimodal Large Language Model Reasoning in Sports](https://arxiv.org/abs/2511.06499)
...
TL;DR: SportR is a multi-sports benchmark with 5,017 images and 2,101 videos that tests multimodal models' ability to combine fine-grained visual perception with rule-based reasoning across sports.


<details>
  <summary>Details</summary>
Motivation: Current sports benchmarks lack detailed reasoning chains and precise visual grounding needed to evaluate core capabilities like nuanced visual perception, rule knowledge application, and visual evidence grounding in a multi-sport context.

Method: Created a hierarchical QA structure with progressive difficulty levels, from simple infraction identification to complex penalty prediction, including 7,118 human-authored Chain of Thought annotations for multi-step reasoning tasks, with both image/video modalities and manual bounding box annotations.

Result: State-of-the-art baseline models performed poorly on challenging tasks. While training via Supervised Fine-Tuning and Reinforcement Learning improved scores, performance remained relatively low, highlighting significant capability gaps.

Conclusion: SportR presents a major challenge for multimodal reasoning research, providing a critical resource to advance sports intelligence capabilities in AI systems.
...
### [120] [Video Dataset for Surgical Phase, Keypoint, and Instrument Recognition in Laparoscopic Surgery (PhaKIR)](https://arxiv.org/abs/2511.06549)
...
TL;DR: PhaKIR is a multi-center dataset with 8 laparoscopic cholecystectomy videos providing frame-level annotations for surgical phase recognition, instrument keypoint estimation, and instrument instance segmentation, enabling comprehensive surgical scene understanding.


<details>
  <summary>Details</summary>
Motivation: Existing surgical datasets often address isolated tasks, neglect temporal dependencies, or lack multi-center variability, limiting the development of robust computer vision systems for robotic-assisted surgery.

Method: Created a dataset of 8 complete laparoscopic cholecystectomy videos from 3 medical centers with frame-level annotations for three interconnected tasks: surgical phase recognition (485,875 frames), instrument keypoint estimation (19,435 frames), and instrument instance segmentation (19,435 frames).
...
### [121] [Spatial-Frequency Enhanced Mamba for Multi-Modal Image Fusion](https://arxiv.org/abs/2511.06593)
*Hui Sun,Long Lv,Pingping Zhang,Tongdan Tang,Feng Tian,Weibing Sun,Huchuan Lu*

Main category: cs.CV

TL;DR: SFMFusion is a novel multi-modal image fusion framework that enhances Mamba with spatial-frequency perception and uses a three-branch structure to efficiently couple image fusion with image reconstruction as an auxiliary task.


<details>
  <summary>Details</summary>
Motivation: Existing MMIF methods using CNNs have limited receptive fields while Transformers have high computational costs. Mamba shows promise for long-range dependencies but lacks spatial and frequency perception needed for MMIF. Also, image reconstruction as an auxiliary task is beneficial but challenging to implement effectively.

Method: Proposes SFMFusion with three key components: 1) Three-branch structure coupling MMIF and image reconstruction to retain complete source image contents, 2) Spatial-Frequency Enhanced Mamba Block (SFMB) for comprehensive feature extraction in both domains, 3) Dynamic Fusion Mamba Block (DFMB) for cross-branch dynamic feature fusion.

Result: Extensive experiments show the method achieves better results than most state-of-the-art methods on six MMIF datasets.

Conclusion: SFMFusion effectively addresses limitations of previous approaches by enhancing Mamba with spatial-frequency perception and efficiently leveraging image reconstruction as an auxiliary task, demonstrating superior performance in multi-modal image fusion.
...
### [122] [On Accurate and Robust Estimation of 3D and 2D Circular Center: Method and Application to Camera-Lidar Calibration](https://arxiv.org/abs/2511.06611)
*Jiajun Jiang,Xiao Hu,Wancheng Liu,Wei Jiang*

Main category: cs.CV

TL;DR: A robust framework for LiDAR-camera extrinsic calibration using circular targets, featuring improved 3D circle center estimation and 2D projected center recovery methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for LiDAR-camera calibration using circular targets struggle with accurate 3D-2D circular center correspondence due to decoupled 3D fitting and erroneous 2D ellipse-center estimation.

Method: Proposes a geometrically principled framework with: (i) robust 3D circle center estimator using conformal geometric algebra and RANSAC; (ii) chord-length variance minimization for true 2D projected center recovery, resolving dual-minima ambiguity via homography validation or quasi-RANSAC fallback.
...
Conclusion: The proposed framework provides accurate and robust LiDAR-camera extrinsic calibration with circular targets, with code to be publicly released for reproducibility.
...
### [123] [Explainable Cross-Disease Reasoning for Cardiovascular Risk Assessment from LDCT](https://arxiv.org/abs/2511.06625)
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches treat pulmonary and cardiac assessment as independent tasks, missing the physiological interplay between lung and cardiovascular health that LDCT scans naturally capture.
...
### [124] [DIAL-GS: Dynamic Instance Aware Reconstruction for Label-free Street Scenes with 4D Gaussian Splatting](https://arxiv.org/abs/2511.06632)
...
<details>
  <summary>Details</summary>
Motivation: Current supervised methods require costly annotations and lack scalability, while self-supervised methods struggle to distinguish static/dynamic elements and individual dynamic objects, limiting fine-grained editing capabilities.
...
Result: Outperforms existing self-supervised baselines in reconstruction quality and instance-level editing on urban driving scenarios.

Conclusion: Provides a concise yet powerful solution for urban scene modeling that enables dynamic-adaptive and instance-aware reconstruction without requiring labels.
...
### [125] [UniADC: A Unified Framework for Anomaly Detection and Classification](https://arxiv.org/abs/2511.06644)
...
Method: Proposes UniADC with two components: a training-free controllable inpainting network that synthesizes anomaly images by repainting normal regions using anomaly priors, and a multi-task discriminator trained on synthesized samples to align image features with anomaly-category embeddings.
...
Conclusion: UniADC effectively unifies anomaly detection and classification, enabling superior performance with minimal anomaly data through synthetic sample generation and multi-task learning.
...
### [126] [FreqGRL: Suppressing Low-Frequency Bias and Mining High-Frequency Knowledge for Cross-Domain Few-Shot Learning](https://arxiv.org/abs/2511.06648)
*Siqi Hui,Sanping Zhou,Ye deng,Wenli Huang,Jinjun Wang*

Main category: cs.CV

TL;DR: FreqGRL is a novel cross-domain few-shot learning framework that addresses data imbalance through frequency-space analysis, using low-frequency replacement and high-frequency enhancement to improve cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Address the severe imbalance between abundant source data and scarce target data in cross-domain few-shot learning, where models are biased toward source-specific knowledge in low-frequency components and struggle to learn domain-generalizable high-frequency features.

Method: Proposes FreqGRL with three modules: Low-Frequency Replacement (LFR) substitutes source low-frequency components with target ones; High-Frequency Enhancement (HFE) filters low frequencies and learns directly on high-frequency features; Global Frequency Filter (GFF) suppresses noisy frequencies and emphasizes informative ones.

Result: Extensive experiments on five standard CD-FSL benchmarks demonstrate state-of-the-art performance.

Conclusion: Frequency-space perspective effectively addresses data imbalance in CD-FSL, with FreqGRL achieving superior performance by mitigating source-specific biases and enhancing cross-domain generalization through frequency manipulation.
...
### [127] [NOVO: Bridging LLaVA and SAM with Visual-only Prompts for Reasoning Segmentation](https://arxiv.org/abs/2511.06651)
*Kyung-Yoon Yoon,Yeong-Jun Cho*

Main category: cs.CV

TL;DR: NOVO is a framework that connects vision-language models with segmentation models using visual-only prompts (coarse masks and points) instead of text tokens, achieving state-of-the-art performance in reasoning segmentation.


<details>
  <summary>Details</summary>
Motivation: To bridge vision-language models and segmentation models while preserving the pretrained capabilities of models like SAM, avoiding the limitations of text-derived token embeddings.
...
Result: Achieves state-of-the-art performance across multiple metrics and model sizes, demonstrating effectiveness and scalability in reasoning segmentation.

Conclusion: NOVO effectively bridges VLMs and segmentation models through visual-only prompts, outperforming previous approaches while maintaining compatibility with pretrained models like SAM.
...
### [128] [HiMo-CLIP: Modeling Semantic Hierarchy and Monotonicity in Vision-Language Alignment](https://arxiv.org/abs/2511.06653)
*Ruijia Wu,Ping Chen,Fei Shen,Shaoan Zhao,Qiang Hui,Huanlin Gao,Ting Lu,Zhaoxiang Liu,Fang Zhao,Kai Wang,Shiguo Lian*

Main category: cs.CV

TL;DR: HiMo-CLIP enhances CLIP-style models with hierarchical decomposition and monotonicity-aware contrastive loss to better handle complex, compositional text descriptions in image-text retrieval.


<details>
  <summary>Details</summary>
Motivation: Current contrastive vision-language models treat text as flat sequences, failing to capture semantic hierarchy and monotonicity - essential properties where richer descriptions should have stronger visual alignment.

Method: Proposes HiDe module for hierarchical decomposition via in-batch PCA to extract semantic components, and MoLo loss for joint alignment of global and component-level representations with monotonicity constraints.

Result: Outperforms strong baselines on multiple image-text retrieval benchmarks, especially with long or compositional descriptions.

Conclusion: HiMo-CLIP successfully addresses limitations of flat text processing in CLIP models by incorporating semantic hierarchy and monotonicity, producing more structured and cognitively-aligned cross-modal representations.
...
### [129] [Active Learning for Animal Re-Identification with Ambiguity-Aware Sampling](https://arxiv.org/abs/2511.06658)
*Depanshu Sani,Mehar Khurana,Saket Anand*

Main category: cs.CV

TL;DR: Proposes a novel active learning framework for animal re-identification that uses complementary clustering to identify ambiguous regions and mine informative sample pairs, achieving state-of-the-art performance with minimal annotation effort.


<details>
  <summary>Details</summary>
Motivation: Animal Re-ID faces challenges due to subtle distinguishing patterns, handling new species, and open-set nature. Existing foundation models underperform in zero-shot scenarios, while unsupervised and active learning methods are inadequate. Annotation is laborious and requires domain expertise.

Method: Uses complementary clustering methods to uncover structurally ambiguous regions in embedding space, mines informative and representative sample pairs, and integrates oracle feedback (must-link/cannot-link constraints) with constrained clustering refinement algorithm.

Result: Achieves average improvements of 10.49%, 11.19% and 3.99% mAP on 13 wildlife datasets over foundational, USL and AL methods respectively, using only 0.033% of all annotations. Also improves 11.09%, 8.2% and 2.06% for unknown individuals in open-world setting.

Conclusion: The proposed active learning framework effectively addresses animal Re-ID challenges by combining clustering-based sample selection with constrained clustering refinement, achieving state-of-the-art performance with minimal annotation effort across diverse wildlife datasets.
...
TL;DR: The paper introduces Medical Diagnosis Segmentation (MDS), a joint task combining medical image segmentation with diagnostic reasoning, and proposes the Sim4Seg framework with RVLS2M module that outperforms baselines.


<details>
  <summary>Details</summary>
Motivation: Existing medical image segmentation models rarely explore segmentation and diagnosis tasks jointly, but providing explainable diagnoses along with segmentation results is crucial for patients.

Method: Proposes Sim4Seg framework with Region-Aware Vision-Language Similarity to Mask (RVLS2M) module and test-time scaling strategy; introduces M3DS dataset with multimodal medical images, segmentation masks, and diagnostic chain-of-thought.
...
Conclusion: The Medical Diagnosis Segmentation task and Sim4Seg framework effectively combine medical segmentation with diagnostic reasoning, providing explainable diagnoses alongside segmentation results.
...
### [131] [REOcc: Camera-Radar Fusion with Radar Feature Enrichment for 3D Occupancy Prediction](https://arxiv.org/abs/2511.06666)
...
<details>
  <summary>Details</summary>
Motivation: Vision-based 3D occupancy prediction struggles in challenging environments with cameras alone. Camera-radar fusion offers complementary strengths but is limited by radar data sparsity and noise, leading to suboptimal performance.

Method: Proposes REOcc with two main components: Radar Densifier to enhance spatial density by integrating spatial information, and Radar Amplifier to improve quality by incorporating contextual information, both refining radar feature representations.

Result: Extensive experiments on Occ3D-nuScenes benchmark show significant performance gains over camera-only baseline, especially in dynamic object classes, demonstrating effective mitigation of radar sparsity and noise.

Conclusion: REOcc successfully enhances radar features to complement camera data more effectively, unlocking the full potential of camera-radar fusion for robust and reliable 3D occupancy prediction.
...
### [132] [Flexible Concept Bottleneck Model](https://arxiv.org/abs/2511.06678)
*Xingbo Du,Qiantong Dou,Lei Fan,Rui Zhang*

Main category: cs.CV

TL;DR: FCBM enables dynamic concept adaptation in concept bottleneck models without full retraining, using a hypernetwork for weight generation and sparse concept selection.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based CBMs require full retraining when new concepts are added, limiting adaptability in real-world scenarios with rapidly evolving foundation models.

Method: Uses a hypernetwork to generate prediction weights from concept embeddings, plus a modified sparsemax module with learnable temperature for dynamic concept selection.

Result: Achieves comparable accuracy to state-of-the-art baselines with similar concept counts, and generalizes well to unseen concepts with minimal fine-tuning.

Conclusion: FCBM provides strong adaptability and flexibility for concept bottleneck models, enabling dynamic concept updates without full model retraining.
...
### [133] [AnoStyler: Text-Driven Localized Anomaly Generation via Lightweight Style Transfer](https://arxiv.org/abs/2511.06687)
*Yulim So,Seokho Kang*

Main category: cs.CV

TL;DR: AnoStyler is a lightweight method for zero-shot anomaly generation using text-guided style transfer, creating realistic anomaly images from single normal images without heavy models or large datasets.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly generation methods suffer from poor visual realism, dependence on large datasets, and memory-intensive architectures, limiting practical deployment.

Method: Uses text-guided style transfer with a lightweight U-Net trained with CLIP-based losses. Generates anomaly masks and text prompts from single normal images, then stylizes normal images into anomalies localized by masks and aligned with text prompts.

Result: Outperforms existing methods on MVTec-AD and VisA datasets, generating high-quality, diverse anomaly images that enhance anomaly detection performance.

Conclusion: AnoStyler provides an effective, lightweight solution for zero-shot anomaly generation that overcomes limitations of existing approaches and improves anomaly detection.
...
### [134] [SPAN: Spatial-Projection Alignment for Monocular 3D Object Detection](https://arxiv.org/abs/2511.06702)
*Yifan Wang,Yian Zhao,Fanqi Pu,Xiaochen Yang,Yang Tang,Xi Chen,Wenming Yang*

Main category: cs.CV

TL;DR: SPAN introduces spatial-projection alignment to address geometric inconsistency in monocular 3D detection by enforcing global spatial constraints and 3D-2D projection alignment, with hierarchical learning for training stability.
...
Method: Proposes Spatial-Projection Alignment (SPAN) with: (i) Spatial Point Alignment for global spatial constraints, (ii) 3D-2D Projection Alignment for tight box alignment, and Hierarchical Task Learning for progressive training.
...
### [135] [K-Stain: Keypoint-Driven Correspondence for H&E-to-IHC Virtual Staining](https://arxiv.org/abs/2511.06709)
*Sicheng Yang,Zhaohu Xing,Haipeng Zhou,Lei Zhu*

Main category: cs.CV

TL;DR: K-Stain is a novel framework that uses keypoint-based spatial and semantic relationships to improve virtual staining of H&E images into IHC images, addressing misalignment issues in tissue slices.


<details>
  <summary>Details</summary>
Motivation: Existing virtual staining methods struggle with effective spatial information utilization due to misalignment in tissue slices, creating a need for more robust spatial correspondence indicators.

Method: K-Stain framework with three components: Hierarchical Spatial Keypoint Detector (HSKD) for keypoint identification, Keypoint-aware Enhancement Generator (KEG) for integrating keypoints during generation, and Keypoint Guided Discriminator (KGD) for improved spatial detail sensitivity.

Result: K-Stain outperforms state-of-the-art methods in both quantitative metrics and visual quality, producing more accurate and visually consistent IHC images.

Conclusion: The keypoint-based approach effectively addresses spatial misalignment issues in virtual staining, enabling more precise alignment and integration of structural details in synthesized IHC images.
...
### [136] [MirrorMamba: Towards Scalable and Robust Mirror Detection in Videos](https://arxiv.org/abs/2511.06716)
*Rui Song,Jiaying Lin,Rynson W. H. Lau*

Main category: cs.CV

TL;DR: MirrorMamba: A novel Mamba-based video mirror detection method that leverages multiple cues and achieves state-of-the-art performance with global receptive field and linear complexity.
...
Method: Proposes MirrorMamba with multiple cues (perceived depth, correspondence, optical flow), Mamba-based Multidirection Correspondence Extractor for global receptive field with linear complexity, and Mamba-based boundary enforcement decoder to resolve unclear boundaries.
...
Conclusion: MirrorMamba demonstrates superior robustness and generalizability, marking the first successful application of Mamba-based architecture in mirror detection with effective performance across both video and image domains.
...
### [137] [MRT: Learning Compact Representations with Mixed RWKV-Transformer for Extreme Image Compression](https://arxiv.org/abs/2511.06717)
*Han Liu,Hengyu Man,Xingtao Wang,Wenrui Li,Debin Zhao*

Main category: cs.CV

TL;DR: Proposes MRT framework using mixed RWKV-Transformer architecture to compress images into 1-D latent representations, achieving superior compression efficiency at very low bitrates.


<details>
  <summary>Details</summary>
Motivation: Existing methods compress images into 2-D latent spaces via CNNs or Swin Transformers, which retain substantial spatial redundancy and limit compression performance.

Method: MRT partitions images into fixed-size windows, using RWKV modules for global dependencies across windows and Transformer blocks for local redundancies within windows, plus a dedicated RWKV Compression Model (RCM) for intermediate 1-D latent features.
...
Conclusion: The mixed RWKV-Transformer architecture enables more efficient and compact 1-D representation learning, significantly improving extreme image compression performance.
...
### [138] [Relative Energy Learning for LiDAR Out-of-Distribution Detection](https://arxiv.org/abs/2511.06720)
*Zizhao Li,Zhengkang Xiang,Jiayang Ao,Joseph West,Kourosh Khoshelham*

Main category: cs.CV

TL;DR: REL is a simple yet effective framework for OOD detection in LiDAR point clouds that uses relative energy learning and synthetic outlier generation to improve reliability in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Current LiDAR OOD methods struggle with high false-positive rates and overconfident errors in safety-critical autonomous driving scenarios, failing to distinguish rare anomalies from common classes effectively.

Method: Proposes Relative Energy Learning (REL) framework using energy gap between positive and negative logits as relative scoring function, plus Point Raise data synthesis strategy that perturbs existing point clouds to generate auxiliary anomalies.

Result: REL consistently outperforms existing methods by a large margin on SemanticKITTI and Spotting the Unexpected (STU) benchmarks, demonstrating improved robustness across various scenes.
...
### [139] [AvatarTex: High-Fidelity Facial Texture Reconstruction from Single-Image Stylized Avatars](https://arxiv.org/abs/2511.06721)
...
Method: Three-stage diffusion-to-GAN pipeline: 1) Diffusion-based inpainting for missing regions, 2) GAN-based latent optimization for style/structure consistency, 3) Diffusion-based repainting for fine details. Also introduces TexHub dataset with 20,000 multi-style UV textures.
...
Conclusion: AvatarTex successfully integrates diffusion models' diversity with GANs' structured latent space for superior texture generation, and TexHub dataset will facilitate future research in this field.
...
Method: Two difficulty-aware sampling strategies: Progressive Image Semantic Masking (PISM) quantifies hardness via image degradation, and Cross-Modality Attention Balance (CMAB) assesses cross-modal interaction complexity. Hierarchical training framework with GRPO-only and SFT+GRPO paradigms.

Result: GRPO applied to difficulty-stratified samples consistently outperforms conventional SFT+GRPO pipelines across six benchmark datasets, improving model accuracy without supervised fine-tuning.

Conclusion: Strategic data sampling can eliminate the need for supervised fine-tuning while enhancing multimodal reasoning performance.
...
### [141] [Argus: Quality-Aware High-Throughput Text-to-Image Inference Serving System](https://arxiv.org/abs/2511.06724)
*Shubham Agarwal,Subrata Mitra,Saud Iqbal*

Main category: cs.CV

TL;DR: Argus is a high-throughput text-to-image inference system that dynamically selects appropriate approximation levels for each prompt to maintain quality while meeting throughput targets, achieving significant improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models are compute-bound with high inference times due to iterative denoising, creating challenges for high-throughput systems. Many prompts can be served with faster approximated models, but approximation must be carefully calibrated to avoid quality degradation.

Method: Argus intelligently switches between different approximation strategies for each prompt, selecting the right level of approximation to satisfy both throughput and quality requirements on a fixed-size cluster.
...
Conclusion: The system successfully addresses the challenge of designing high-throughput T2I inference by dynamically adapting approximation levels per prompt, demonstrating significant performance improvements while maintaining quality.
...
### [142] [Rethinking Rainy 3D Scene Reconstruction via Perspective Transforming and Brightness Tuning](https://arxiv.org/abs/2511.06734)
*Qianfeng Yang,Xiang Chen,Pengpeng Li,Qiyuan Guan,Guiyue Jin,Jiyu Jin*

Main category: cs.CV

TL;DR: Proposes OmniRain3D dataset with realistic rain effects and REVR-GSNet framework for high-fidelity 3D scene reconstruction from rain-degraded multi-view images.


<details>
  <summary>Details</summary>
Motivation: Existing datasets overlook viewpoint-dependent rain streak variation and ambient brightness reduction in real rainy scenes, leading to inaccurate 3D reconstruction results.

Method: Constructed OmniRain3D dataset with perspective heterogeneity and brightness dynamicity; developed REVR-GSNet framework integrating recursive brightness enhancement, Gaussian primitive optimization, and GS-guided rain elimination through joint alternating optimization.

Result: Extensive experiments demonstrate effectiveness of both the dataset and method in achieving high-fidelity reconstruction of clean 3D scenes from rain-degraded inputs.

Conclusion: The dataset and method provide a foundation for future research on multi-view image deraining and rainy 3D scene reconstruction.
...
### [143] [SinSEMI: A One-Shot Image Generation Model and Data-Efficient Evaluation Framework for Semiconductor Inspection Equipment](https://arxiv.org/abs/2511.06740)
*ChunLiang Wu,Xiaochun Li*

Main category: cs.CV

TL;DR: SinSEMI is a one-shot learning approach that generates diverse, realistic semiconductor optical images from a single input, addressing data scarcity in semiconductor equipment development.


<details>
  <summary>Details</summary>
Motivation: Data scarcity of raw optical images in early semiconductor equipment development hinders AI advancement in semiconductor manufacturing.

Method: Multi-scale flow-based model with LPIPS energy guidance during sampling for perceptual realism and output variety.

Result: Superior performance in visual quality, quantitative measures, and downstream tasks compared to other one-shot generation techniques.

Conclusion: SinSEMI-generated images achieve high fidelity and meaningful diversity, making them suitable as training data for semiconductor AI applications.
...
### [144] [Otter: Mitigating Background Distractions of Wide-Angle Few-Shot Action Recognition with Enhanced RWKV](https://arxiv.org/abs/2511.06741)
...
<details>
  <summary>Details</summary>
Motivation: Wide-angle videos in FSAR have background distractions that make action recognition challenging, and temporal relations are degraded by frames with similar backgrounds, impacting performance.

Method: Designs CompOund SegmenTation and Temporal REconstructing RWKV (Otter) with: 1) Compound Segmentation Module (CSM) to segment and emphasize key patches, 2) Temporal Reconstruction Module (TRM) for bidirectional scanning in temporal-enhanced prototype construction, and 3) combination of regular and temporal-enhanced prototypes.
...
### [145] [PointCubeNet: 3D Part-level Reasoning with 3x3x3 Point Cloud Blocks](https://arxiv.org/abs/2511.06744)
*Da-Yeong Kim,Yeong-Jun Cho*

Main category: cs.CV

TL;DR: PointCubeNet is a multi-modal 3D understanding framework that performs unsupervised part-level reasoning without part annotations, using global and local branches with 3x3x3 local blocks for part analysis.
...
Result: Demonstrates that understanding 3D object parts enhances overall 3D object understanding, achieving reliable and meaningful part-level reasoning without supervision.

Conclusion: First successful attempt at unsupervised 3D part-level reasoning, showing that part-level analysis improves comprehensive 3D object understanding.
...
### [146] [Image Restoration via Primal Dual Hybrid Gradient and Flow Generative Model](https://arxiv.org/abs/2511.06748)
*Ji Li,Chao Wang*

Main category: cs.CV

TL;DR: A Plug-and-Play framework that integrates flow matching generative models with primal-dual optimization to handle various data fidelity terms (ℓ₁ and ℓ₂ norms) for robust image restoration under non-Gaussian noise.


<details>
  <summary>Details</summary>
Motivation: Existing PnP methods mainly work with smooth squared ℓ₂ data fidelity for Gaussian noise, but their applicability to more general data fidelity terms and non-Gaussian noise types remains limited.

Method: Proposed a PnP algorithm based on primal-dual hybrid gradient (PDHG) that replaces the proximal operator with a time-dependent denoiser from flow matching generative models, supporting both ℓ₁ and ℓ₂ fidelity terms.

Result: The method demonstrates superior performance on image restoration tasks (denoising, super-resolution, deblurring, inpainting) with non-Gaussian noise, showing that ℓ₁ and ℓ₂ fidelity outperform conventional squared ℓ₂ loss.

Conclusion: The proposed PDHG-based PnP framework is computationally efficient, memory-friendly, and effectively handles various fidelity terms, making it robust to non-Gaussian noise in imaging inverse problems.
...
### [147] [Med-SORA: Symptom to Organ Reasoning in Abdomen CT Images](https://arxiv.org/abs/2511.06752)
*You-Kyoung Na,Yeong-Jun Cho*

Main category: cs.CV

TL;DR: Med-SORA is a framework for symptom-to-organ reasoning in abdominal CT images that addresses limitations of existing models by using RAG-based dataset construction, soft labeling with learnable organ anchors, and 2D-3D cross-attention architecture.


<details>
  <summary>Details</summary>
Motivation: Existing medical multimodal models oversimplify clinical reality with one-to-one hard labeling and lack 3D information, limiting their ability to capture complex symptom-organ relationships and full anatomical context.

Method: Proposes RAG-based dataset construction, soft labeling with learnable organ anchors to capture one-to-many symptom-organ relationships, and a 2D-3D cross-attention architecture to fuse local and global image features.

Result: Med-SORA outperforms existing medical multimodal models and enables accurate 3D clinical reasoning.

Conclusion: This is the first work to address symptom-to-organ reasoning in medical multimodal learning, providing a more clinically realistic approach to understanding symptom-image associations.
...
### [148] [CAST-LUT: Tokenizer-Guided HSV Look-Up Tables for Purple Flare Removal](https://arxiv.org/abs/2511.06764)
...
<details>
  <summary>Details</summary>
Motivation: Purple flare artifacts degrade image quality, existing traditional methods lack flexibility and rely on fixed priors, while deep learning approaches suffer from scarcity of paired training data.

Method: Two-stage architecture: 1) CAST converts RGB to HSV and encodes H/V channels into semantic tokens; 2) HSV-LUT module generates independent correction curves for H, S, V channels using dynamic 1D-LUTs.

Result: Significantly outperforms existing methods in visual effects and achieves state-of-the-art performance on all quantitative metrics.
...
### [149] [Robust and High-Fidelity 3D Gaussian Splatting: Fusing Pose Priors and Geometry Constraints for Texture-Deficient Outdoor Scenes](https://arxiv.org/abs/2511.06765)
*Meijun Guo,Yongliang Shi,Caiyun Liu,Yixiao Feng,Ming Ma,Tinghai Yan,Weining Lu,Bin Liang*

Main category: cs.CV

TL;DR: This paper improves 3D Gaussian Splatting for large outdoor scenes by combining LiDAR-IMU odometry for robust pose estimation with normal vector constraints and rank regularization for better scene representation, achieving faster optimization and superior visual quality.
...
Method: Uses LiDAR-IMU Odometry for prior poses in COLMAP triangulation, and introduces normal vector constraints with effective rank regularization for Gaussian primitive consistency, jointly optimized with photometric loss.

Result: Pose optimization requires only one-third of the time while maintaining accuracy and robustness. Scene representation significantly outperforms conventional 3DGS, especially in weak/repetitive texture scenarios with enhanced visualization.

Conclusion: The proposed approach effectively addresses pose estimation and scene representation challenges in large outdoor scenes, achieving faster optimization and superior performance compared to standard 3DGS pipelines.
...
### [150] [ConeGS: Error-Guided Densification Using Pixel Cones for Improved Reconstruction with Fewer Primitives](https://arxiv.org/abs/2511.06810)
*Bartłomiej Baranowski,Stefano Esposito,Patricia Gschoßmann,Anpei Chen,Andreas Geiger*

Main category: cs.CV

TL;DR: ConeGS improves 3D Gaussian Splatting by using image-space-informed densification to better distribute primitives, achieving higher quality with fewer Gaussians.


<details>
  <summary>Details</summary>
Motivation: 3DGS suffers from suboptimal spatial distribution of primitives due to cloning-based densification, which limits exploration and requires many primitives to cover scenes adequately.

Method: Uses iNGP as geometric proxy for depth estimation, identifies high-error pixels, inserts new Gaussians along viewing cones at predicted depths, applies opacity penalty to remove redundant Gaussians, and uses primitive budgeting.
...
Conclusion: ConeGS provides more efficient primitive placement than traditional cloning-based densification, improving 3DGS performance especially when primitive counts are limited.
...
### [151] [TiS-TSL: Image-Label Supervised Surgical Video Stereo Matching via Time-Switchable Teacher-Student Learning](https://arxiv.org/abs/2511.06817)
*Rui Wang,Ying Zhou,Hao Wang,Wenwei Zhang,Qiang Li,Zhiwei Wang*

Main category: cs.CV

TL;DR: TiS-TSL is a time-switchable teacher-student learning framework that addresses temporal inconsistency in surgical stereo matching by using a unified model with three prediction modes and a two-stage learning strategy for spatio-temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Stereo matching in minimally invasive surgery faces challenges due to limited dense disparity supervision and lack of temporal consistency in existing teacher-student learning methods, leading to unstable predictions and flickering artifacts across video frames.

Method: Proposes TiS-TSL with a unified model operating in three modes (IP, FVP, BVP) and a two-stage learning strategy: Image-to-Video stage transfers sparse image knowledge, and Video-to-Video stage refines temporal predictions using bidirectional spatio-temporal consistency to filter noisy labels and enforce coherence.

Result: Experimental results on two public datasets show TiS-TSL outperforms image-based state-of-the-art methods, improving TEPE and EPE by at least 2.11% and 4.54% respectively.

Conclusion: TiS-TSL effectively addresses temporal inconsistency in surgical stereo matching under minimal supervision, providing more stable and temporally coherent disparity predictions through its unified temporal modeling approach.
...
### [152] [Integrating Reweighted Least Squares with Plug-and-Play Diffusion Priors for Noisy Image Restoration](https://arxiv.org/abs/2511.06823)
*Ji Li,Chao Wang*

Main category: cs.CV

TL;DR: A plug-and-play image restoration framework using generative diffusion priors for robust removal of general noise types, including impulse noise, with superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing plug-and-play methods mainly use Gaussian denoisers and are limited to Gaussian noise. There's a need for effective restoration methods that can handle non-Gaussian noise like impulse noise, which remains largely unexplored.

Method: Proposes a MAP estimation framework with a generalized Gaussian scale mixture-based loss that approximates various noise distributions, leading to an ℓ_q-norm fidelity term. Uses iteratively reweighted least squares (IRLS) approach with diffusion-based denoisers for the proximal step.
...
Conclusion: The proposed framework successfully extends plug-and-play image restoration to handle general noise types beyond Gaussian noise, demonstrating robust performance with impulse noise through the integration of generative diffusion priors and adaptive fidelity terms.
...
### [153] [MUGSQA: Novel Multi-Uncertainty-Based Gaussian Splatting Quality Assessment Method, Dataset, and Benchmarks](https://arxiv.org/abs/2511.06830)
*Tianang Chen,Jian Jin,Shilv Cai,Zhuangzi Li,Weisi Lin*

Main category: cs.CV

TL;DR: Proposes a multi-distance subjective quality assessment method for 3D objects reconstructed with Gaussian Splatting, creates the MUGSQA dataset considering multiple input uncertainties, and establishes benchmarks for method robustness and quality metric evaluation.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of assessing perceptual quality of 3D objects reconstructed with different Gaussian Splatting methods, as existing methods lack standardized evaluation approaches.

Method: Developed a unified multi-distance subjective quality assessment method that mimics human viewing behavior, and constructed the MUGSQA dataset considering uncertainties in input view quantity/resolution, view distance, and initial point cloud accuracy.

Result: Created a comprehensive quality assessment dataset and established two benchmarks: one for evaluating GS method robustness under uncertainties, and another for assessing existing quality metrics.

Conclusion: The proposed framework provides standardized tools for evaluating Gaussian Splatting reconstruction quality, with the dataset and benchmark code to be publicly released.
...
### [154] [ConsistTalk: Intensity Controllable Temporally Consistent Talking Head Generation with Diffusion Noise Search](https://arxiv.org/abs/2511.06833)
...
Method: Three key components: 1) Optical flow-guided temporal module (OFT) to decouple motion from appearance, 2) Audio-to-Intensity (A2I) model for joint audio-visual motion modeling, 3) Diffusion noise initialization strategy (IC-Init) with constraints on background coherence and motion continuity.

Result: Significantly outperforms prior methods in reducing flicker, preserving identity, and delivering temporally stable, high-fidelity talking head videos with better audio-visual synchronization.

Conclusion: ConsistTalk provides an effective solution for intensity-controllable and temporally consistent talking head generation with improved visual quality and motion dynamics.
...
### [155] [NeuroBridge: Bio-Inspired Self-Supervised EEG-to-Image Decoding via Cognitive Priors and Bidirectional Semantic Alignment](https://arxiv.org/abs/2511.06836)
*Wenjiang Zhang,Sifeng Wang,Yuwei Su,Xinyu Li,Chen Zhang,Suyu Zhong*

Main category: cs.CV

TL;DR: NeuroBridge is a self-supervised architecture that improves visual neural decoding by simulating perceptual variability and using bidirectional cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Current visual neural decoding methods are limited by scarce stimulus-brain response data and semantic mismatches between neural representations and visual content.

Method: Integrates Cognitive Prior Augmentation (CPA) to simulate perceptual variability and Shared Semantic Projector (SSP) for bidirectional cross-modality alignment in a shared semantic space.

Result: Achieves 63.2% top-1 and 89.9% top-5 accuracy on 200-way zero-shot retrieval, improving by 12.3% and 10.2% respectively over previous methods.

Conclusion: NeuroBridge demonstrates effective, robust, and scalable neural visual decoding through its co-adaptive cross-modal alignment approach.
...
### [156] [PanoNav: Mapless Zero-Shot Object Navigation with Panoramic Scene Parsing and Dynamic Memory](https://arxiv.org/abs/2511.06840)
*Qunchao Jin,Yilin Wu,Changhao Chen*

Main category: cs.CV

TL;DR: PanoNav is an RGB-only, mapless zero-shot object navigation framework that uses panoramic scene parsing and memory-guided decision making to improve navigation performance without depth sensors or prebuilt maps.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot object navigation methods often rely on depth sensors or prebuilt maps, limiting MLLMs' spatial reasoning. Mapless approaches suffer from short-sighted decisions and local deadlocks due to lack of historical context.

Method: Integrates Panoramic Scene Parsing module to extract spatial information from panoramic RGB inputs, and Memory-guided Decision-Making mechanism with Dynamic Bounded Memory Queue to incorporate exploration history and avoid deadlocks.
...
Conclusion: PanoNav demonstrates that RGB-only, mapless navigation with panoramic scene parsing and memory guidance can effectively overcome limitations of existing approaches and achieve superior zero-shot object navigation performance.
...
### [157] [Aerial Image Stitching Using IMU Data from a UAV](https://arxiv.org/abs/2511.06841)
*Selim Ahmet Iz,Mustafa Unel*

Main category: cs.CV

TL;DR: A novel UAV image stitching method combining IMU data with computer vision techniques to improve accuracy and reliability over traditional feature-based approaches.


<details>
  <summary>Details</summary>
Motivation: Feature-based image stitching algorithms often suffer from errors in feature detection and matching, especially with UAV images that have large displacements, rotations, and camera pose variations.

Method: Uses IMU data to estimate UAV displacement and rotation between images, corrects perspective distortion, computes homography matrix, and applies standard image stitching algorithms for alignment and blending.

Result: The method outperforms existing feature-based image stitching algorithms in accuracy and reliability, particularly in challenging scenarios with large displacements and rotations.

Conclusion: The proposed approach effectively leverages IMU data to enhance image stitching performance and can be easily integrated into existing UAV workflows.
...
### [158] [Gaussian-Augmented Physics Simulation and System Identification with Complex Colliders](https://arxiv.org/abs/2511.06846)
...
<details>
  <summary>Details</summary>
Motivation: Existing differentiable MPM approaches are limited to simplified object-environment interactions with planar colliders and fail in challenging scenarios with non-planar surfaces, restricting their practical applications.

Method: Extends differentiable MPM with a differentiable collision handling mechanism that allows target objects to interact with complex rigid bodies while maintaining end-to-end optimization.

Result: The framework successfully enables physical property estimation with arbitrarily shaped colliders and can be interfaced with various novel view synthesis methods.

Conclusion: AS-DiffMPM provides a more versatile framework for system identification from visual observations by supporting complex object-environment interactions beyond planar surfaces.
...
### [159] [Distillation Dynamics: Towards Understanding Feature-Based Distillation in Vision Transformers](https://arxiv.org/abs/2511.06848)
...
<details>
  <summary>Details</summary>
Motivation: To understand why feature-based knowledge distillation works for CNNs but fails for Vision Transformers, and identify the root causes of this negative transfer phenomenon.

Method: Used a novel "distillation dynamics" framework combining frequency spectrum analysis, information entropy metrics, and activation magnitude tracking to analyze ViT behavior.

Result: Discovered ViTs exhibit U-shaped information processing (compression then expansion) and teacher models use distributed high-dimensional encoding that students can't replicate due to limited channel capacity.

Conclusion: Successful ViT distillation requires moving beyond naive feature mimicry to methods that respect representational constraints between teacher and student models.
...
### [160] [Ambiguity-aware Truncated Flow Matching for Ambiguous Medical Image Segmentation](https://arxiv.org/abs/2511.06857)
*Fanding Li,Xiangyu Li,Xianghe Su,Xingyu Qiu,Suyu Dong,Wei Wang,Kuanquan Wang,Gongning Luo,Shuo Li*

Main category: cs.CV

TL;DR: ATFM is a novel method for ambiguous medical image segmentation that simultaneously enhances accuracy and diversity through data-hierarchical inference, Gaussian truncation representation, and segmentation flow matching, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of simultaneously enhancing accuracy and diversity in ambiguous medical image segmentation, where existing truncated diffusion probabilistic models suffer from entangled predictions with insufficient fidelity and plausibility.

Method: Proposes three key components: 1) Data-Hierarchical Inference for disentangling accuracy and diversity at different levels, 2) Gaussian Truncation Representation for enhanced fidelity and truncation reliability, 3) Segmentation Flow Matching for improved plausibility of diverse predictions.

Result: Outperforms SOTA methods on LIDC and ISIC3 datasets, improving GED and HM-IoU by up to 12% and 7.3% respectively, while achieving more efficient inference.

Conclusion: ATFM successfully addresses the trade-off between accuracy and diversity in ambiguous medical image segmentation through its novel inference paradigm and dedicated components, demonstrating superior performance over existing methods.
...
### [161] [VAEVQ: Enhancing Discrete Visual Tokenization through Variational Modeling](https://arxiv.org/abs/2511.06863)
*Sicheng Yang,Xing Hu,Qiang Wu,Dawei Yang*

Main category: cs.CV

TL;DR: VAEVQ improves vector quantization by using VAE for smoother latent spaces, adaptive alignment strategies, and distribution consistency regularization to address issues like unstable codeword learning and underutilized codebooks.


<details>
  <summary>Details</summary>
Motivation: Traditional VQ methods suffer from non-smooth latent spaces, weak alignment between pre/post-quantization features, and poor coherence between continuous/discrete domains, leading to unstable learning and degraded performance.

Method: Three key components: (1) Variational Latent Quantization (VLQ) using VAE instead of AE, (2) Representation Coherence Strategy (RCS) for adaptive alignment modulation, (3) Distribution Consistency Regularization (DCR) for codebook-continuous distribution alignment.
...
Conclusion: VAEVQ effectively addresses VQ limitations through VAE-based quantization, adaptive alignment strategies, and distribution regularization, achieving superior performance compared to existing methods.
...
### [162] [Generating an Image From 1,000 Words: Enhancing Text-to-Image With Structured Captions](https://arxiv.org/abs/2511.06876)
...
TL;DR: FIBO is the first open-source text-to-image model trained on long structured captions with fine-grained attributes, using DimFusion for efficient long caption processing and achieving SOTA prompt alignment.


<details>
  <summary>Details</summary>
Motivation: Address the gap between sparse textual input and rich visual outputs in text-to-image models, which reduces controllability and biases toward average user preferences, limiting professional precision.

Method: Train on long structured captions with fine-grained attributes, propose DimFusion for efficient long caption processing, and introduce Text-as-a-Bottleneck Reconstruction (TaBR) evaluation protocol.

Result: FIBO achieves state-of-the-art prompt alignment among open-source models and enables disentangled control over visual factors through structured captioning.

Conclusion: Training text-to-image models on long structured captions with fine-grained attributes significantly improves controllability and expressiveness for professional use cases.
...
### [163] [A Two-Stage System for Layout-Controlled Image Generation using Large Language Models and Diffusion Models](https://arxiv.org/abs/2511.06888)
...
Method: Two-stage approach: 1) LLM generates structured layouts from object lists using task decomposition, 2) Layout-conditioned diffusion models (ControlNet vs GLIGEN) synthesize images adhering to layouts after domain-specific finetuning.

Result: Task decomposition improved object recall from 57.2% to 99.9% for complex scenes. ControlNet preserves text-based stylistic control but suffers from object hallucination, while GLIGEN provides superior layout fidelity with reduced prompt-based controllability.

Conclusion: The decoupled approach successfully generates images with specified object counts and plausible spatial arrangements, demonstrating viability for compositionally controlled synthesis.
...
### [164] [Adaptive Morph-Patch Transformer for Arotic Vessel Segmentation](https://arxiv.org/abs/2511.06897)
*Zhenxi Zhang,Fuchen Zheng,Adnan Iltaf,Yifei Han,Zhenyu Cheng,Yue Du,Bin Li,Tianyong Liu,Shoujun Zhou*

Main category: cs.CV

TL;DR: Proposes MPT, a Transformer-based model with adaptive morphology-aware patches and semantic clustering attention for improved aortic vascular segmentation, achieving SOTA results on three datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional Transformer models use fixed rectangular patches that damage complex vascular structure integrity, leading to suboptimal segmentation accuracy in aortic vascular segmentation.

Method: Introduces adaptive Morph Patch Transformer (MPT) with morphology-aware patch partitioning that aligns with vascular structures, and Semantic Clustering Attention (SCA) to aggregate features from semantically similar patches.
...
Conclusion: MPT effectively addresses the limitations of traditional Transformers in vascular segmentation by preserving structural integrity through adaptive patches and semantic feature aggregation.
...
### [165] [Classification of Microplastic Particles in Water using Polarized Light Scattering and Machine Learning Methods](https://arxiv.org/abs/2511.06901)
...
<details>
  <summary>Details</summary>
Motivation: Need for continuous, large-scale microplastic monitoring in aquatic environments, overcoming limitations of gold-standard methods and transmission-based interference issues.

Method: Reflection-based approach using linearly polarized laser light and polarization-sensitive camera to capture microplastic signals, with deep CNN for image-based classification of three polymer types.

Result: Achieved 80% mean classification accuracy; AOLP signals more robust against noise and better at distinguishing polyethylene types, while DOLP better identifies polypropylene.

Conclusion: Polarized light scattering with CNN classification is effective for in-situ microplastic identification, with AOLP and DOLP signals providing complementary strengths for different polymer types.
...
### [166] [Mono3DVG-EnSD: Enhanced Spatial-aware and Dimension-decoupled Text Encoding for Monocular 3D Visual Grounding](https://arxiv.org/abs/2511.06908)
*Yuzhen Li,Min Liu,Zhaoyang Li,Yuan Bian,Xueping Wang,Erbo Zhai,Yaonan Wang*

Main category: cs.CV

TL;DR: Mono3DVG-EnSD improves monocular 3D visual grounding by addressing over-reliance on explicit keywords and cross-dimensional interference through lexical certainty adaptation and dimension decoupling.


<details>
  <summary>Details</summary>
Motivation: Existing methods over-rely on high-certainty keywords while neglecting spatial descriptions, and suffer from cross-dimensional interference between 2D and 3D features in text-guided visual refinement.

Method: Proposes CLIP-Guided Lexical Certainty Adapter (CLIP-LCA) to mask high-certainty keywords and retain spatial descriptions, and Dimension-Decoupled Module (D2M) to separate 2D/3D textual features for dimensionally-consistent cross-modal interactions.
...
Conclusion: The proposed framework effectively addresses key limitations in monocular 3D visual grounding by enhancing spatial understanding and mitigating cross-dimensional interference, demonstrating superior performance.
...
### [167] [DTTNet: Improving Video Shadow Detection via Dark-Aware Guidance and Tokenized Temporal Modeling](https://arxiv.org/abs/2511.06925)
*Zhicheng Li,Kunyang Sun,Rui Yao,Hancheng Zhu,Fuyuan Hu,Jiaqi Zhao,Zhiwen Shao,Yong Zhou*

Main category: cs.CV

TL;DR: DTTNet is a video shadow detection method that uses vision-language matching to distinguish shadows from dark backgrounds and tokenized temporal modeling to handle dynamic shadow deformations, achieving state-of-the-art accuracy with real-time efficiency.
...
Method: Proposes Vision-language Match Module (VMM) and Dark-aware Semantic Block (DSB) to extract text-guided features for shadow-background differentiation, adaptive mask reweighting for penumbra regions, and Tokenized Temporal Block (TTB) for efficient spatiotemporal learning using temporal tokens.

Result: Achieves state-of-the-art accuracy on multiple benchmark datasets with real-time inference efficiency.

Conclusion: The proposed DTTNet effectively addresses video shadow detection challenges through vision-language integration and efficient temporal modeling, demonstrating superior performance and practical efficiency.
...
### [168] [PlantTraitNet: An Uncertainty-Aware Multimodal Framework for Global-Scale Plant Trait Inference from Citizen Science Data](https://arxiv.org/abs/2511.06943)
...
<details>
  <summary>Details</summary>
Motivation: Existing plant trait maps are limited by costly field measurements with sparse coverage. Citizen science provides over 50 million geotagged plant photos that can overcome these limitations.
...
Result: PlantTraitNet consistently outperforms existing trait maps across all evaluated traits when validated against independent vegetation survey data (sPlotOpen).
...
### [169] [From Attribution to Action: Jointly ALIGNing Predictions and Explanations](https://arxiv.org/abs/2511.06944)
...
<details>
  <summary>Details</summary>
Motivation: Existing explanation-guided learning methods rely on external annotations or heuristic-based segmentation, which can be noisy, imprecise, and difficult to scale, potentially degrading model performance.
...
Conclusion: ALIGN effectively improves both interpretability and generalizability by leveraging high-quality masks as guidance, producing accurate and interpretable models without external supervision.
...
### [170] [FoCLIP: A Feature-Space Misalignment Framework for CLIP-Based Image Manipulation and Detection](https://arxiv.org/abs/2511.06947)
*Yulin Chen,Zeyuan Wang,Tianyuan Yu,Yingmei Wei,Liang Bai*

Main category: cs.CV

TL;DR: FoCLIP is a feature-space misalignment framework that fools CLIP-based image quality metrics by optimizing images to achieve high CLIPscores despite visual unrecognizability or semantic incongruence with prompts, while also proposing a defense method based on color channel sensitivity.


<details>
  <summary>Details</summary>
Motivation: CLIP-based metrics like CLIPscore are widely used for image quality assessment but are vulnerable due to their delicate multimodal alignment, creating a need to understand and defend against such vulnerabilities.

Method: FoCLIP uses stochastic gradient descent with three components: feature alignment to reduce modality gaps, score distribution balance, and pixel-guard regularization to optimize multimodal output equilibrium between CLIPscore performance and image quality.

Result: Optimized images achieved significant CLIPscore improvement while preserving visual fidelity. Grayscale conversion was found to degrade fooling images' features, leading to a color channel sensitivity-based detection method with 91% accuracy.

Conclusion: This work establishes practical pathways for feature misalignment in CLIP-based systems and corresponding defense mechanisms, highlighting vulnerabilities in multimodal alignment and providing detection solutions.
...
### [171] [PADM: A Physics-aware Diffusion Model for Attenuation Correction](https://arxiv.org/abs/2511.06948)
...
TL;DR: PADM is a diffusion-based model that corrects attenuation artifacts in cardiac SPECT using only non-attenuation-corrected input, eliminating the need for expensive CT systems through physics-aware teacher-student distillation.


<details>
  <summary>Details</summary>
Motivation: Attenuation artifacts in cardiac SPECT compromise diagnostic accuracy. Hybrid SPECT/CT systems are expensive, have limited accessibility, and add radiation exposure, hindering widespread clinical adoption.

Method: Physics-aware Attenuation Correction Diffusion Model (PADM) uses diffusion-based generative method with explicit physics priors via teacher-student distillation mechanism, trained on CardiAC dataset with 424 patient studies.
...
Conclusion: PADM provides an effective CT-free solution for attenuation correction in cardiac SPECT, enabling artifact correction without the limitations of hybrid SPECT/CT systems.
...
### [172] [GFix: Perceptually Enhanced Gaussian Splatting Video Compression](https://arxiv.org/abs/2511.06953)
*Siyue Teng,Ge Gao,Duolikun Danier,Yuxuan Jiang,Fan Zhang,Thomas Davis,Zoe Liu,David Bull*

Main category: cs.CV

TL;DR: GFix is a content-adaptive framework that uses a single-step diffusion model to enhance perceptual quality in 3D Gaussian Splatting-based video compression, achieving significant BD-rate savings.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS-based video codecs suffer from noticeable visual artifacts and low compression ratios, motivating the need for perceptual enhancement.
...
Result: Outperforms GSVC with up to 72.1% BD-rate savings in LPIPS and 21.4% in FID, demonstrating strong perceptual quality enhancement.

Conclusion: GFix effectively addresses perceptual artifacts in 3DGS-based video compression through diffusion-based enhancement and efficient adaptation, achieving superior compression efficiency.
...
### [173] [Learning from the Right Patches: A Two-Stage Wavelet-Driven Masked Autoencoder for Histopathology Representation Learning](https://arxiv.org/abs/2511.06958)
*Raneen Younis,Louay Hamdi,Lukas Chavez,Zahra Ahmadi*

Main category: cs.CV

TL;DR: WISE-MAE introduces a wavelet-informed patch selection strategy for MAE-based self-supervised learning in digital pathology, improving representation quality by focusing on biologically relevant tissue regions.
...
Conclusion: The wavelet-informed patch selection strategy successfully brings structure and biological relevance into MAE-based learning for digital pathology, improving learned representations while maintaining computational efficiency.
...
### [174] [Exploring the "Great Unseen" in Medieval Manuscripts: Instance-Level Labeling of Legacy Image Collections with Zero-Shot Models](https://arxiv.org/abs/2511.07004)
*Christofer Meinecke,Estelle Guéville,David Joseph Wrisley*

Main category: cs.CV

TL;DR: Using modern techniques to segment and describe entire medieval manuscript pages to create better training data for computer vision and multimodal models.


<details>
  <summary>Details</summary>
Motivation: To develop a more holistic theoretical approach to medieval manuscript pages and their contents by leveraging advanced technology.

Method: Employ state-of-the-art techniques to segment and describe complete manuscript folios.

Result: Creation of richer training data specifically tailored for medieval visual content analysis.

Conclusion: Enhanced training data will improve computer vision techniques like instance segmentation and multimodal models for medieval manuscript analysis.
...
### [175] [TrueCity: Real and Simulated Urban Data for Cross-Domain 3D Scene Understanding](https://arxiv.org/abs/2511.07007)
*Duc Nguyen,Yan-Ling Lai,Qilin Zhang,Prabin Gyawali,Benedikt Schwab,Olaf Wysocki,Thomas H. Kolbe*

Main category: cs.CV

TL;DR: TrueCity is the first urban semantic segmentation benchmark with synchronized real-world and simulated point clouds, addressing the synthetic-to-real domain gap in 3D scene understanding.


<details>
  <summary>Details</summary>
Motivation: Limited real-world annotated data for 3D semantic scene understanding creates a need for synthetic data, but existing synthetic datasets fail to capture real-world complexity and sensor noise, resulting in domain gaps.

Method: Introduces TrueCity benchmark with cm-accurate annotated real-world point clouds, semantic 3D city models, and annotated simulated point clouds representing the same urban environment, with segmentation classes aligned with international standards.

Result: Extensive experiments quantify domain shift and demonstrate strategies for using synthetic data to enhance real-world 3D scene understanding, providing baseline performance metrics.

Conclusion: TrueCity enables consistent evaluation of synthetic-to-real gap and will foster development of sim-to-real gap quantification methods and generalizable data-driven models for 3D semantic segmentation.
...
### [176] [Performance Decay in Deepfake Detection: The Limitations of Training on Outdated Data](https://arxiv.org/abs/2511.07009)
*Jack Richings,Margaux Leblanc,Ian Groves,Victoria Nockles*

Main category: cs.CV

TL;DR: A simple two-stage deepfake detection method achieves 99.8% AUROC but performance decays rapidly (30% recall drop) when tested on newer generation techniques, highlighting the need for ongoing dataset curation and frame-level feature detection.
...
Result: Achieves 99.8% AUROC on current deepfakes but suffers 30% recall drop when tested on deepfakes created with generation techniques from just six months later.
...
### [177] [Certified L2-Norm Robustness of 3D Point Cloud Recognition in the Frequency Domain](https://arxiv.org/abs/2511.07029)
...
<details>
  <summary>Details</summary>
Motivation: Existing certified defenses for point cloud classification focus on point-wise perturbations but overlook subtle geometric distortions that preserve individual points while altering overall structure, creating safety risks in applications like autonomous driving.

Method: Transforms point clouds via graph Fourier transform, applies frequency-aware subsampling to generate multiple sub-point clouds based on spectral similarity, uses majority voting of standard model predictions on sub-clouds.
...
Conclusion: Spectral representations provide an effective pathway toward certifiable robustness in 3D point cloud recognition, with FreqCert establishing theoretical foundations for frequency domain certification.
...
### [178] [3D-ANC: Adaptive Neural Collapse for Robust 3D Point Cloud Recognition](https://arxiv.org/abs/2511.07040)
*Yuanmin Huang,Wenxuan Li,Mi Zhang,Xiaohan Zhang,Xiaoyu You,Min Yang*

Main category: cs.CV

TL;DR: 3D-ANC is a novel defense method that uses Neural Collapse to create disentangled feature spaces, improving 3D point cloud model robustness against adversarial attacks by 53.7% on ModelNet40.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks for 3D point cloud recognition are vulnerable to adversarial attacks, and conventional defenses struggle with evolving attack patterns due to entangled feature spaces.

Method: Combines ETF-aligned classification with adaptive training framework including representation-balanced learning (RBL) and dynamic feature direction loss (FDL) to address class imbalance and geometric similarities.

Result: Significantly improves model robustness - DGCNN's accuracy increased from 27.2% to 80.9% on ModelNet40 (53.7% absolute gain), surpassing leading baselines by 34.0%.

Conclusion: 3D-ANC effectively enhances 3D point cloud model robustness by leveraging Neural Collapse to create maximally separable feature spaces, overcoming class imbalance and geometric complexity challenges.
...
### [179] [From Pretrain to Pain: Adversarial Vulnerability of Video Foundation Models Without Task Knowledge](https://arxiv.org/abs/2511.07049)
*Hui Lu,Yi Yu,Song Xia,Yiming Yang,Deepu Rajan,Boon Poh Ng,Alex Kot,Xudong Jiang*

Main category: cs.CV

TL;DR: TVA is a temporal-aware adversarial attack method that exploits video foundation models to craft transferable perturbations for attacking downstream models without access to victim models or training data.


<details>
  <summary>Details</summary>
Motivation: The open accessibility of Video Foundation Models (VFMs) introduces security risks where adversaries can exploit these models to attack downstream applications without direct access to victim models.

Method: Proposes Transferable Video Attack (TVA) with bidirectional contrastive learning to maximize feature discrepancy and temporal consistency loss using motion cues to enhance sequential perturbation impact.

Result: Extensive experiments across 24 video-related tasks demonstrate TVA's effectiveness against downstream models and MLLMs, revealing significant security vulnerabilities.

Conclusion: TVA exposes previously underexplored security risks in video model deployment, offering a practical attack strategy that doesn't require expensive surrogate models or domain-specific data.
...
### [180] [Improving Deepfake Detection with Reinforcement Learning-Based Adaptive Data Augmentation](https://arxiv.org/abs/2511.07051)
*Yuxuan Zhou,Tao Yu,Wen Huang,Yuheng Zhang,Tao Dai,Shu-Tao Xia*

Main category: cs.CV

TL;DR: CRDA is a dynamic data augmentation framework using reinforcement learning and causal inference to progressively train deepfake detectors on multi-domain forgery features, significantly improving cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Current deepfake detectors rely on fixed augmentation strategies that cannot adapt to the evolving complexity and diversity of real-world forgery features, limiting their generalization capability.

Method: Proposes CRDA framework with reinforcement learning agent that dynamically selects augmentation actions from a configurable pool of forgery operations, guided by causal inference to suppress spurious correlations and focus on causally invariant features.
...
Conclusion: Dynamic curriculum-based augmentation guided by RL and causal inference effectively enhances deepfake detector generalization by adapting to increasingly complex forgery patterns while mitigating spurious correlations.
...
### [181] [RaLD: Generating High-Resolution 3D Radar Point Clouds with Latent Diffusion](https://arxiv.org/abs/2511.07067)
*Ruijie Zhang,Bixin Zeng,Shengpeng Wang,Fuhui Zhou,Wei Wang*

Main category: cs.CV

TL;DR: RaDL is a framework that uses latent diffusion models to generate dense 3D point clouds from sparse millimeter-wave radar data, addressing limitations of existing methods through scene-level frustum-based LiDAR autoencoding and direct radar spectrum conditioning.


<details>
  <summary>Details</summary>
Motivation: Millimeter-wave radar is robust and low-cost but produces sparse, low-resolution point clouds that limit 3D perception capabilities. Existing generative approaches use inefficient dense voxel representations and struggle with structural detail preservation.

Method: Integrates scene-level frustum-based LiDAR autoencoding, order-invariant latent representations, and direct radar spectrum conditioning within a latent diffusion model framework to enable compact and expressive generation.

Result: RaDL produces dense and accurate 3D point clouds from raw radar spectrums, demonstrating improved performance over existing methods for radar-based 3D generation.

Conclusion: The framework offers a promising solution for robust 3D perception in challenging environments by effectively leveraging latent diffusion models for radar data enhancement.
...
### [182] [ClusterMine: Robust Label-Free Visual Out-Of-Distribution Detection via Concept Mining from Text Corpora](https://arxiv.org/abs/2511.07068)
*Nikolas Adaloglou,Diana Petrusheva,Mohamed Asker,Felix Michels,Markus Kollmann*

Main category: cs.CV

TL;DR: ClusterMine enables unsupervised OOD detection by mining positive labels from text corpora without needing pre-defined ID labels, achieving state-of-the-art performance and robustness to distribution shifts.
...
Method: Proposes ClusterMine which extracts positive concepts from text corpora by combining visual-only sample consistency (via clustering) and zero-shot image-text consistency, eliminating the need for positive labels.

Result: Achieves state-of-the-art OOD detection performance without access to positive labels, scalable across CLIP models, and demonstrates superior robustness to covariate in-distribution shifts.

Conclusion: ClusterMine enables truly unsupervised OOD detection by leveraging widely available text corpora, overcoming limitations of pre-defined label dependencies and maintaining performance under distribution shifts.
...
### [183] [LeCoT: revisiting network architecture for two-view correspondence pruning](https://arxiv.org/abs/2511.07078)
*Luanyuan Dai,Xiaoyu Du,Jinhui Tang*

Main category: cs.CV

TL;DR: LeCoT is a novel two-view correspondence pruning network that uses Spatial-Channel Fusion Transformer blocks to capture global context information without extra modules, outperforming state-of-the-art methods in multiple vision tasks.


<details>
  <summary>Details</summary>
Motivation: Current methods use MLP backbones with additional modules to handle context information limitations. The authors aim to capture correspondence context naturally without extra design modules.

Method: Proposes LeCoT network with Spatial-Channel Fusion Transformer blocks that efficiently utilize spatial and channel global context. Includes prediction block that progressively refines probability sets using intermediate features to guide learning.
...
Conclusion: LeCoT provides an effective approach for two-view correspondence pruning by naturally leveraging global context information at different stages through novel transformer blocks and progressive prediction mechanisms.
...
### [184] [Pandar128 dataset for lane line detection](https://arxiv.org/abs/2511.07084)
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of large-scale public LiDAR datasets for lane detection and standardized evaluation metrics in this domain.

Method: Created Pandar128 dataset with synchronized sensors and odometry; developed SimpleLidarLane method combining BEV segmentation, clustering, and polyline fitting; proposed IAM-F1 metric for polyline-based evaluation.

Result: Dataset contains diverse real-world conditions; baseline method achieves strong performance in challenging scenarios; new metric enables principled evaluation.

Conclusion: High-quality data and modular approaches can compete with complex methods, and standardized evaluation is crucial for advancing LiDAR-based lane detection research.
...
### [185] [How Bias Binds: Measuring Hidden Associations for Bias Control in Text-to-Image Compositions](https://arxiv.org/abs/2511.07091)
*Jeng-Lin Li,Ming-Ching Chang,Wei-Chao Chen*

Main category: cs.CV

TL;DR: Text-to-image models exhibit bias in semantic bindings where object-attribute associations amplify bias, requiring new debiasing approaches that address joint contextual effects.


<details>
  <summary>Details</summary>
Motivation: Current debiasing methods fail on semantically bound prompts where multiple objects/attributes interact, as they focus on single-object contexts and neglect joint effects of semantic binding.

Method: Introduced bias adherence score to quantify bias activation in object-attribute bindings, and developed training-free context-bias control framework using token decoupling for debiasing semantic bindings.

Result: Achieved over 10% debiasing improvement in compositional generation tasks, with analysis showing bias distribution amplification through contextual associations.

Conclusion: Current bias mitigation strategies have critical limitations in semantically bound contexts, requiring reassessment to reduce bias without disrupting essential semantic relationships.
...
### [186] [GEWDiff: Geometric Enhanced Wavelet-based Diffusion Model for Hyperspectral Image Super-resolution](https://arxiv.org/abs/2511.07103)
*Sirui Wang,Jiang He,Natàlia Blasco Andreo,Xiao Xiang Zhu*

Main category: cs.CV

TL;DR: GEWDiff is a novel diffusion model for 4x hyperspectral image super-resolution that uses wavelet compression and geometric enhancement to overcome memory limitations and preserve spatial-spectral information.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral images are too memory-intensive for conventional diffusion models, lack geometric understanding of ground objects, and current diffusion models have suboptimal convergence behavior for complex data.

Method: Proposed Geometric Enhanced Wavelet-based Diffusion Model (GEWDiff) with wavelet encoder-decoder for efficient compression, geometry-enhanced diffusion process, and multi-level loss function for stable convergence.

Result: Achieved state-of-the-art results across multiple dimensions including fidelity, spectral accuracy, visual realism, and clarity in hyperspectral image super-resolution.

Conclusion: GEWDiff effectively addresses the challenges of hyperspectral image generation through wavelet compression, geometric enhancement, and multi-level loss optimization, demonstrating superior performance in 4x super-resolution tasks.
...
### [187] [HENet++: Hybrid Encoding and Multi-task Learning for 3D Perception and End-to-end Autonomous Driving](https://arxiv.org/abs/2511.07106)
*Zhongyu Xia,Zhiwei Lin,Yongtao Wang,Ming-Hsuan Yang*

Main category: cs.CV

TL;DR: HENet/HENet++ is a multi-task 3D perception framework for autonomous driving that uses hybrid image encoding (large encoder for short-term, small for long-term frames) and extracts both dense/sparse features to handle computational constraints while maintaining state-of-the-art performance across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: Address computational constraints in autonomous driving where large image encoders, high-resolution images, and long-term temporal inputs are incompatible with resource limitations, and different tasks require distinct feature representations that are difficult to handle with single models.

Method: Proposes hybrid image encoding network with large encoder for short-term frames and small encoder for long-term frames, simultaneously extracts both dense and sparse features, maintains compatibility with existing 3D feature extraction methods, and supports multimodal inputs.

Result: Achieves state-of-the-art end-to-end multi-task 3D perception results on nuScenes benchmark and lowest collision rate on nuScenes end-to-end autonomous driving benchmark.

Conclusion: The framework effectively addresses computational constraints while delivering comprehensive multi-task 3D perception for autonomous driving, reducing cumulative errors and providing suitable representations for different tasks.
...
### [188] [Sparse4DGS: 4D Gaussian Splatting for Sparse-Frame Dynamic Scene Reconstruction](https://arxiv.org/abs/2511.07122)
*Changyue Shi,Chuxiao Yang,Xinyuan Hu,Minghao Chen,Wenwen Pan,Yan Yang,Jiajun Ding,Zhou Yu,Jun Yu*

Main category: cs.CV

TL;DR: Sparse4DGS enables dynamic 4D scene reconstruction from sparse-frame inputs by focusing on texture-rich areas with specialized deformation regularization and canonical optimization techniques.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic Gaussian Splatting methods require dense-frame video sequences, but real-world scenarios often only provide sparse frames due to equipment constraints, leading to reconstruction failures especially in texture-rich areas.
...
Result: Outperforms existing dynamic and few-shot techniques on multiple datasets (NeRF-Synthetic, HyperNeRF, NeRF-DS, iPhone-4D) when using sparse frames as inputs.

Conclusion: Sparse4DGS successfully addresses the challenge of sparse-frame dynamic scene reconstruction by focusing on texture-rich regions, making 4D reconstruction feasible in practical scenarios with limited frame availability.
...
### [189] [MPJudge: Towards Perceptual Assessment of Music-Induced Paintings](https://arxiv.org/abs/2511.07137)
*Shiqi Jiang,Tianyi Liang,Changbo Wang,Chenhui Li*

Main category: cs.CV

TL;DR: Proposes a novel framework for assessing music-induced paintings by modeling perceptual coherence between music and visual art, using a new dataset and preference optimization.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on emotion recognition models which introduce noise and miss broader perceptual cues beyond just emotion similarity.

Method: Created MPD dataset with expert-annotated music-painting pairs, developed MPJudge model with music-visual fusion via modulation, and used Direct Preference Optimization for ambiguous cases.

Result: Outperforms existing approaches in experiments and more accurately identifies music-relevant regions in paintings qualitatively.

Conclusion: The proposed framework effectively addresses limitations of emotion-based methods by directly modeling perceptual coherence between music and visual art.
...
### [190] [ProcGen3D: Learning Neural Procedural Graph Representations for Image-to-3D Reconstruction](https://arxiv.org/abs/2511.07142)
*Xinyi Zhang,Daoyi Gao,Naiqi Li,Angela Dai*

Main category: cs.CV

TL;DR: ProcGen3D generates 3D content by creating procedural graph abstractions that can be decoded into complex 3D assets, using transformer-based prediction with MCTS guidance for image-faithful reconstructions.


<details>
  <summary>Details</summary>
Motivation: Inspired by procedural generators used in production 3D applications, the goal is to learn procedural graph representations for 3D assets to enable image-based 3D reconstruction with better generalization.
...
Conclusion: ProcGen3D demonstrates effective neural procedural graph generation that enables high-quality 3D reconstruction from images with strong generalization capabilities.
...
### [191] [Federated Learning for Video Violence Detection: Complementary Roles of Lightweight CNNs and Vision-Language Models for Energy-Efficient Use](https://arxiv.org/abs/2511.07171)
*Sébastien Thuau,Siba Haidar,Rachid Chelouah*

Main category: cs.CV

TL;DR: Comparative study of federated learning methods for violence detection showing 3D CNNs achieve better calibration and lower energy costs than vision-language models, while VLMs offer richer multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: Need for privacy-preserving video surveillance with low computational and environmental overhead, addressing energy and sustainability challenges of deploying large vision-language models in federated learning.

Method: Compared three federated strategies: zero-shot inference with pretrained VLMs, LoRA-based fine-tuning of LLaVA-NeXT-Video-7B, and personalized federated learning of a 65.8M-parameter 3D CNN on RWF-2000 and RLVS datasets with non-IID splits.

Result: All methods exceeded 90% accuracy in binary violence detection. 3D CNN achieved superior calibration (ROC AUC 92.59%) at half the energy cost (240 Wh vs. 570 Wh) of federated LoRA. Hierarchical category grouping boosted VLM multiclass accuracy from 65.31% to 81% on UCF-Crime.

Conclusion: Hybrid deployment strategies should default to efficient CNNs for routine inference and selectively engage VLMs for complex contextual reasoning, balancing accuracy, energy efficiency, and multimodal capabilities.
...
### [192] [LiteUpdate: A Lightweight Framework for Updating AI-Generated Image Detectors](https://arxiv.org/abs/2511.07192)
*Jiajie Lu,Zhenkan Fu,Na Zhao,Long Xing,Kejiang Chen,Weiming Zhang,Nenghai Yu*

Main category: cs.CV

TL;DR: LiteUpdate is a lightweight framework that efficiently updates AI-generated image detectors to adapt to new generators, using representative sample selection and model merging to improve detection accuracy while preventing catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Existing AI-generated image detection methods struggle to keep pace with rapidly evolving generative models, leading to significant performance degradation and the need for continuous detector updates.

Method: Uses representative sample selection with image confidence and gradient-based features to select boundary samples, and model merging that fuses weights from multiple fine-tuning trajectories (pre-trained, representative, and random updates).

Result: Substantially boosts detection performance across various detectors. On AIDE, average detection accuracy on Midjourney improved from 87.63% to 93.03% (6.16% relative increase).

Conclusion: LiteUpdate provides an efficient framework for continuously updating AI-generated image detectors, balancing adaptability to new generators with preservation of prior knowledge, significantly enhancing detection performance.
...
### [193] [Automated Estimation of Anatomical Risk Metrics for Endoscopic Sinus Surgery Using Deep Learning](https://arxiv.org/abs/2511.07199)
...
TL;DR: Automated deep learning pipeline for estimating skull base anatomical risk scores in endoscopic sinus surgery using heatmap regression of key landmarks.


<details>
  <summary>Details</summary>
Motivation: Manual measurement of anatomical risk scores (Keros, Gera, TMS) on CT/CBCT scans is time-consuming and requires standardization for preoperative assessment to minimize surgical risks like CSF leakage.

Method: Deep learning pipeline using heatmap regression to localize anatomical landmarks, comparing direct approach with specialized global-to-local learning strategy.

Result: Mean absolute errors: 0.506mm for Keros, 4.516° for Gera, and 0.802mm/0.777mm for TMS classification.

Conclusion: Automated deep learning approach successfully estimates anatomical risk scores with high accuracy, potentially reducing manual measurement burden in preoperative planning.
...
### [194] [Geometric implicit neural representations for signed distance functions](https://arxiv.org/abs/2511.07206)
*Luiz Schirmer,Tiago Novello,Vinícius da Silva,Guilherme Schardong,Daniel Perazzo,Hélio Lopes,Nuno Gonçalves,Luiz Velho*

Main category: cs.CV

TL;DR: This survey reviews geometric implicit neural representations (INRs) for signed distance functions (SDFs), focusing on incorporating differential geometry tools like normals and curvatures into loss functions for improved 3D surface reconstruction.


<details>
  <summary>Details</summary>
Motivation: To improve 3D surface reconstruction by developing neural SDFs that incorporate geometric regularization terms to ensure the INR satisfies global properties like unit gradient constraints.

Method: Uses geometric INRs with additional regularization terms in loss functions, incorporating differential geometry tools (normals, curvatures) and exploring sampling schemes from a differential geometry perspective.

Result: The review highlights significant advancements in surface reconstruction from oriented point clouds and posed images enabled by geometric INRs.

Conclusion: Geometric INRs represent a promising framework that successfully incorporates differential geometry principles to enhance the quality and accuracy of 3D surface reconstruction.
...
### [195] [Breaking the Stealth-Potency Trade-off in Clean-Image Backdoors with Generative Trigger Optimization](https://arxiv.org/abs/2511.07210)
*Binyan Xu,Fan Yang,Di Tang,Xilin Dai,Kehuan Zhang*

Main category: cs.CV

TL;DR: GCB introduces a new clean-image backdoor attack using conditional InfoGAN to find natural image features as stealthy triggers, enabling successful attacks with minimal clean accuracy drop (<1%) across multiple datasets and tasks.


<details>
  <summary>Details</summary>
Motivation: Existing clean-image backdoor attacks require high poison rates that cause noticeable drops in clean accuracy, compromising stealthiness. There's a need for more covert attacks that minimize accuracy degradation.

Method: Uses conditional InfoGAN to identify naturally occurring image features that serve as potent triggers, ensuring triggers are easily separable from benign features to enable learning from extremely small poisoned datasets.

Result: Achieves successful backdoor attacks with clean accuracy drop of less than 1%, demonstrates versatility across 6 datasets, 5 architectures, and 4 tasks including regression and segmentation, and shows resilience against most existing defenses.

Conclusion: GCB provides a more stealthy and effective clean-image backdoor attack paradigm by optimizing trigger selection through generative models, significantly reducing the trade-off between attack success and clean accuracy degradation.
...
### [196] [Omni-View: Unlocking How Generation Facilitates Understanding in Unified 3D Model based on Multiview images](https://arxiv.org/abs/2511.07222)
*JiaKui Hu,Shanshan Zhao,Qing-Guo Chen,Xuerui Qiu,Jialun Liu,Zhao Xu,Weihua Luo,Kaifu Zhang,Yanye Lu*

Main category: cs.CV

TL;DR: Omni-View extends unified multimodal understanding and generation to 3D scenes using multiview images, demonstrating that generation enhances understanding through joint modeling of scene understanding, novel view synthesis, and geometry estimation.


<details>
  <summary>Details</summary>
Motivation: To explore the principle that 'generation facilitates understanding' in 3D scenes and create a unified framework that synergistically combines understanding and generation tasks for holistic 3D scene comprehension.

Method: Three-module architecture: understanding model, texture module for appearance synthesis, and geometry module for explicit geometric constraints. Uses two-stage training strategy with spatiotemporal modeling from texture module and geometric constraints from geometry module.

Result: Achieves state-of-the-art score of 55.4 on VSI-Bench benchmark, outperforming specialized 3D understanding models while maintaining strong performance in novel view synthesis and 3D scene generation.

Conclusion: Omni-View successfully demonstrates that joint modeling of understanding and generation tasks enables synergistic interaction, enriching holistic 3D scene understanding while achieving superior performance across multiple tasks.
...
### [197] [Mapping Reduced Accessibility to WASH Facilities in Rohingya Refugee Camps with Sub-Meter Imagery](https://arxiv.org/abs/2511.07231)
*Kyeongjin Ahn,YongHun Suh,Sungwon Han,Jeasurk Yang,Hannes Taubenböck,Meeyoung Cha*

Main category: cs.CV

TL;DR: Remote sensing framework using semi-supervised segmentation to detect refugee shelters and quantify declining WASH accessibility in Rohingya camps, revealing gender disparities and recommending demand-responsive allocation strategies.


<details>
  <summary>Details</summary>
Motivation: WASH services remain a major public health concern in refugee camps, with challenges in detecting shelters due to dense spatial configuration and irregular patterns in emergent camps like Cox's Bazar.
...
Result: Declining WASH accessibility from 25 people per facility in 2022 to 29.4 in 2025, with women and girls experiencing reduced accessibility due to inadequate safety-related segregation in facilities.

Conclusion: Importance of demand-responsive allocation strategies to identify under-served populations and ensure limited infrastructure serves maximum people, highlighting value of remote sensing and machine learning for equitable resource planning in humanitarian settings.
...
### [198] [Noise & pattern: identity-anchored Tikhonov regularization for robust structural anomaly detection](https://arxiv.org/abs/2511.07233)
...
<details>
  <summary>Details</summary>
Motivation: Collecting representative anomaly examples is infeasible for industrial inspection, requiring methods that can detect subtle defects without extensive anomaly data.

Method: Self-supervised autoencoder that learns to repair corrupted inputs using structured, spatially coherent perturbations with added Gaussian noise as Tikhonov regularizer.

Result: Achieves state-of-the-art results on MVTec AD benchmark with I/P-AUROC scores of 99.9/99.4.

Conclusion: The identity-anchored regularization stabilizes reconstruction and improves both detection and segmentation accuracy for structural anomaly detection.
...
### [199] [Leveraging Text-Driven Semantic Variation for Robust OOD Segmentation](https://arxiv.org/abs/2511.07238)
*Seungheon Song,Jaekoo Lee*

Main category: cs.CV

TL;DR: A novel vision-language approach for out-of-distribution (OOD) segmentation in autonomous driving that leverages linguistic cues to detect anomalous objects, achieving state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current OOD segmentation methods for autonomous driving don't fully utilize linguistic knowledge from vision-language models, which could improve detection of anomalous objects in complex real-world scenarios.

Method: Combines vision-language model encoder with transformer decoder, uses Distance-Based OOD prompts at varying semantic distances from in-distribution classes, and employs OOD Semantic Augmentation for better OOD representations through visual-textual alignment.
...
Conclusion: Vision-language-based OOD segmentation shows strong potential to enhance safety and reliability of autonomous driving systems by effectively generalizing to unseen objects in diverse environments.
...
### [200] [4DSTR: Advancing Generative 4D Gaussians with Spatial-Temporal Rectification for High-Quality and Consistent 4D Generation](https://arxiv.org/abs/2511.07241)
*Mengmeng Liu,Jiuming Liu,Yunpeng Zhang,Jiangtao Li,Michael Ying Yang,Francesco Nex,Hao Cheng*

Main category: cs.CV

TL;DR: 4DSTR is a novel 4D generation network that uses spatial-temporal rectification to modulate 4D Gaussian Splatting, achieving superior temporal consistency and adaptation to rapid movements in video-to-4D generation.
...
Method: Proposes 4DSTR with temporal correlation to rectify deformable scales and rotations for consistency, and adaptive spatial densification/pruning strategy to handle temporal variations by dynamically managing Gaussian points based on movement.
...
Conclusion: 4DSTR effectively addresses spatial-temporal consistency and rapid variation adaptation challenges in 4D generation through spatial-temporal rectification and adaptive Gaussian point management.
...
### [201] [MVU-Eval: Towards Multi-Video Understanding Evaluation for Multimodal LLMs](https://arxiv.org/abs/2511.07250)
...
TL;DR: MVU-Eval is the first comprehensive benchmark for evaluating Multi-Video Understanding in MLLMs, addressing the gap in existing single-video benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation benchmarks for MLLMs are limited to single-video understanding, overlooking the critical need for multi-video understanding in real-world scenarios like sports analytics and autonomous driving.

Method: Created MVU-Eval benchmark with 1,824 question-answer pairs spanning 4,959 videos from diverse domains, assessing eight core competencies including fundamental perception and high-order reasoning tasks aligned with real-world applications.

Result: Evaluation of state-of-the-art open-source and closed-source models revealed significant performance discrepancies and limitations in current MLLMs' ability to perform understanding across multiple videos.

Conclusion: The benchmark addresses a significant gap in MLLM evaluation and will be publicly available to foster future research in multi-video understanding.
...
### [202] [StreamKV: Streaming Video Question-Answering with Segment-based KV Cache Retrieval and Compression](https://arxiv.org/abs/2511.07278)
*Yilong Chen,Xiang Bai,Zhibin Wang,Chengyu Bai,Yuhan Dai,Ming Lu,Shanghang Zhang*

Main category: cs.CV

TL;DR: StreamKV is a training-free framework that enhances Video-LLMs for long video processing by dynamically partitioning videos into semantic segments and performing layer-adaptive KV cache retrieval and compression.
...
Result: Extensive experiments on StreamingVQA benchmarks show StreamKV significantly outperforms existing Online Video-LLMs with superior accuracy while substantially improving memory efficiency and computational latency.

Conclusion: StreamKV effectively addresses the challenges of long video processing in Video-LLMs through dynamic semantic partitioning and unified layer-adaptive KV cache management, achieving state-of-the-art performance.
...
### [203] [Segmentation of Ischemic Stroke Lesions using Transfer Learning on Multi-sequence MRI](https://arxiv.org/abs/2511.07281)
*R. P. Chowdhury,T. Rahman*

Main category: cs.CV

TL;DR: A novel framework using Res-Unet architecture for automatic ischemic stroke lesion segmentation on MRI sequences, achieving 80.5% Dice score and 74.03% accuracy on ISLES 2015 dataset.


<details>
  <summary>Details</summary>
Motivation: Manual stroke lesion segmentation is tedious, time-consuming, and prone to inconsistency, while previous automated methods using hand-crafted features fail to capture complex lesion shapes.

Method: Used Res-Unet architecture trained twice (with and without pre-existing weights) on ISLES 2015 dataset, then integrated Majority Voting Classifier to combine results from each axis for comprehensive segmentation.

Result: Achieved Dice score of 80.5% and accuracy of 74.03% on the ISLES 2015 Brain Stroke sequence dataset across various MRI sequences (T1-weighted, T2-weighted, DWI, FLAIR).

Conclusion: The proposed framework effectively segments ischemic stroke lesions automatically, demonstrating the efficacy of the approach for medical image analysis.
...
### [204] [Glioma C6: A Novel Dataset for Training and Benchmarking Cell Segmentation](https://arxiv.org/abs/2511.07286)
...
<details>
  <summary>Details</summary>
Motivation: To provide a realistic testbed for biomedical image analysis and enhance utilization of image data for cancer cell research through morphological categorization.
...
Result: Evaluation of generalist segmentation models showed limitations on this dataset. Experiments demonstrated that training on Glioma C6 significantly enhances segmentation performance.

Conclusion: Glioma C6 dataset is valuable for developing robust and generalizable models for glioma cell segmentation and is publicly available for researchers.
...
### [205] [LMM-IQA: Image Quality Assessment for Low-Dose CT Imaging](https://arxiv.org/abs/2511.07298)
*Kagan Celik,Mehmet Ozan Unal,Metin Ertas,Isa Yildirim*

Main category: cs.CV

TL;DR: Proposes an LLM-based system for assessing low-dose CT image quality with numerical scores and textual descriptions of degradations like noise, blur, and contrast loss.


<details>
  <summary>Details</summary>
Motivation: Low-dose CT improves patient safety but suffers from increased noise, blur, and contrast loss that reduce diagnostic quality, requiring robust quality assessment for clinical use.

Method: Uses LLM-based quality assessment system with various inference strategies including zero-shot approach, metadata integration, and error feedback to generate both numerical scores and textual degradation descriptions.

Result: The system produces highly correlated scores and interpretable outputs that add value to clinical workflows, with progressive performance improvements from each inference strategy.

Conclusion: The proposed LLM-based approach provides consistent and robust quality assessment for low-dose CT images, offering both quantitative scores and interpretable textual descriptions for clinical applications.
...
### [206] [VADER: Towards Causal Video Anomaly Understanding with Relation-Aware Large Language Models](https://arxiv.org/abs/2511.07299)
*Ying Cheng,Yu-Ho Lin,Min-Hung Chen,Fu-En Yang,Shang-Hong Lai*

Main category: cs.CV

TL;DR: VADER is an LLM-driven framework for Video Anomaly Understanding that integrates object relation features with visual cues to provide detailed interpretation of anomalous events, outperforming traditional detection-only methods.


<details>
  <summary>Details</summary>
Motivation: Traditional video anomaly methods focus only on detecting and localizing anomalies without understanding deeper causal relationships and object interactions, which are crucial for comprehensive anomaly interpretation.

Method: VADER uses an Anomaly Scorer for per-frame scoring, Context-Aware Sampling to capture causal context, Relation Feature Extractor and COntrastive Relation Encoder to model object interactions, and integrates these with LLMs for reasoning.

Result: Experiments on multiple real-world benchmarks show VADER achieves strong performance across anomaly description, explanation, and causal reasoning tasks.

Conclusion: VADER advances explainable video anomaly analysis by providing causally grounded descriptions and robust question answering capabilities through integrated visual and relational reasoning.
...
### [207] [Beyond Boundaries: Leveraging Vision Foundation Models for Source-Free Object Detection](https://arxiv.org/abs/2511.07301)
*Huizai Yao,Sicheng Zhao,Pengteng Li,Yi Cui,Shuo Lu,Weiyu Guo,Yunfan Lu,Yijie Xu,Hui Xiong*

Main category: cs.CV

TL;DR: A novel SFOD framework that leverages Vision Foundation Models as external knowledge to enhance feature alignment and label quality through three modules: PGFA, PIFA, and DEPF.
...
Result: Extensive experiments on six benchmarks show state-of-the-art SFOD performance, validating improved transferability and discriminability.
...
### [208] [YoNoSplat: You Only Need One Model for Feedforward 3D Gaussian Splatting](https://arxiv.org/abs/2511.07321)
...
Method: Predicts local Gaussians and camera poses for each view, aggregates them using a novel mixing training strategy that gradually transitions from ground-truth to predicted poses, with pairwise camera-distance normalization and camera intrinsics embedding.

Result: Achieves state-of-the-art performance on standard benchmarks, reconstructing scenes from 100 views in just 2.69 seconds on an NVIDIA GH200 GPU.
...
### [209] [Garbage Vulnerable Point Monitoring using IoT and Computer Vision](https://arxiv.org/abs/2511.07325)
...
Method: Uses IoT and computer vision with street-level cameras and object detection algorithms (YOLOv8, YOLOv10, YOLO11m, RT-DETR) on dataset from Sangareddy district, India.

Result: YOLO11m achieved the highest accuracy of 92.39% and mAP@50 of 0.91 in waste detection, effectively capturing waste disposal patterns including hourly, daily, and weekly trends.

Conclusion: The object detection model is well-suited for monitoring waste dumping at GVP locations, providing comprehensive daily and nightly monitoring capabilities.
...
### [210] [Inference-Time Scaling of Diffusion Models for Infrared Data Generation](https://arxiv.org/abs/2511.07362)
*Kai A. Horstmann,Maxim Clouser,Kia Khezeli*

Main category: cs.CV

TL;DR: Infrared image generation improved using domain-adapted CLIP verifier for inference-time guidance, reducing FID scores by 10% on KAIST dataset.


<details>
  <summary>Details</summary>
Motivation: Infrared vision models suffer from limited annotated data, and training foundation generative models for infrared is challenging due to data scarcity.

Method: Fine-tuned FLUX.1-dev diffusion model on small infrared dataset using parameter-efficient techniques, then used domain-adapted CLIP verifier during inference to guide sampling towards higher quality generations.

Result: Consistent improvements in generation quality, with 10% reduction in FID scores on KAIST Multispectral Pedestrian Detection Benchmark compared to unguided baseline.

Conclusion: Inference-time guidance is a promising approach for bridging domain gaps in low-data infrared settings.
...
### [211] [Real-Time LiDAR Super-Resolution via Frequency-Aware Multi-Scale Fusion](https://arxiv.org/abs/2511.07377)
*June Moh Goo,Zichao Zeng,Jan Boehm*

Main category: cs.CV

TL;DR: FLASH is a LiDAR super-resolution framework that uses dual-domain processing with frequency-aware attention and adaptive multi-scale fusion to achieve state-of-the-art performance with single-pass efficiency.
...
Method: FLASH integrates Frequency-Aware Window Attention (combining local spatial attention with global frequency-domain analysis via FFT) and Adaptive Multi-Scale Fusion (learned position-specific feature aggregation enhanced by CBAM attention).

Result: Achieves state-of-the-art performance on KITTI across all evaluation metrics, outperforming TULIP with Monte Carlo Dropout while maintaining single-pass efficiency for real-time deployment.

Conclusion: The dual-domain approach effectively handles uncertainty through architectural design rather than computationally expensive stochastic inference, making it practical for autonomous systems with consistent superiority across all distance ranges.
...
### [212] [StreamDiffusionV2: A Streaming System for Dynamic and Interactive Video Generation](https://arxiv.org/abs/2511.07399)
...
TL;DR: StreamDiffusionV2 is a training-free pipeline for real-time video streaming that addresses latency and scalability challenges in live generative streaming by combining SLO-aware scheduling, KV cache optimization, and multi-GPU parallelization.


<details>
  <summary>Details</summary>
Motivation: Existing image-based streaming diffusion models lack temporal consistency, while offline video diffusion systems don't meet live streaming's strict latency requirements (minimal time-to-first-frame, per-frame deadlines with low jitter). Scalable multi-GPU serving for real-time streams remains unresolved.
...
Result: Without TensorRT or quantization, achieves first frame within 0.5s, 58.28 FPS with 14B-parameter model and 64.52 FPS with 1.3B-parameter model on four H100 GPUs. Supports flexible denoising steps (1-4) for ultra-low-latency and higher-quality modes.

Conclusion: StreamDiffusionV2 makes state-of-the-art generative live streaming practical and accessible, scaling seamlessly across heterogeneous GPU environments from individual creators to enterprise platforms.
...
### [213] [SpatialThinker: Reinforcing 3D Reasoning in Multimodal LLMs via Spatial Rewards](https://arxiv.org/abs/2511.07403)
*Hunar Batra,Haoqin Tu,Hardy Chen,Yuanze Lin,Cihang Xie,Ronald Clark*

Main category: cs.CV

TL;DR: SpatialThinker is a 3D-aware multimodal LLM that uses RL training with dense spatial rewards to improve spatial understanding, outperforming existing methods and GPT-4o on spatial VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with spatial understanding despite progress in vision-language tasks, often requiring explicit 3D inputs or extensive datasets. The goal is to enable robust 3D spatial understanding with limited data.

Method: Uses RL training with multi-objective dense spatial rewards, constructs scene graphs of objects and spatial relations, and includes a data synthesis pipeline creating STVQA-7K dataset.

Result: SpatialThinker-7B outperforms supervised fine-tuning and sparse RL baselines, nearly doubling base-model gains compared to sparse RL, and surpasses GPT-4o on spatial understanding and real-world VQA benchmarks.
...
### [214] [DIMO: Diverse 3D Motion Generation for Arbitrary Objects](https://arxiv.org/abs/2511.07409)
*Linzhan Mou,Jiahui Lei,Chen Wang,Lingjie Liu,Kostas Daniilidis*

Main category: cs.CV

TL;DR: DIMO generates diverse 3D motions for arbitrary objects from a single image by leveraging video model priors and learning a shared motion representation in latent space.


<details>
  <summary>Details</summary>
Motivation: To enable generation of diverse 3D motions from a single image input, overcoming limitations of existing methods that require multiple inputs or lack motion diversity.

Method: Extract motion patterns from video models, embed motions into latent vectors, train shared motion decoder to learn neural key point trajectories, drive 3D Gaussians with key points for geometry and appearance modeling.

Result: Successfully generates diverse 3D motions in single-forward pass, supports motion interpolation and language-guided motion generation.

Conclusion: DIMO provides an effective approach for generating diverse 3D motions from single images with applications in motion interpolation and language-guided generation.
...
### [215] [TwinOR: Photorealistic Digital Twins of Dynamic Operating Rooms for Embodied AI Research](https://arxiv.org/abs/2511.07412)
...
<details>
  <summary>Details</summary>
Motivation: Safety regulations and operational constraints in real operating rooms limit embodied AI systems from freely perceiving and interacting, creating a need for safe, controllable environments for continual learning and evaluation.

Method: Reconstructs static geometry from pre-scan videos and continuously models human/equipment motion through multi-view perception, then fuses static and dynamic components into an immersive 3D environment supporting controllable simulation.

Result: Achieves centimeter-level accuracy in OR geometry reconstruction while preserving dynamic interactions. Models like FoundationStereo and ORB-SLAM3 on TwinOR-synthesized data perform within reported accuracy on real indoor datasets, demonstrating sensor-level realism.

Conclusion: TwinOR enables safe, scalable, and data-efficient development of embodied AI through a real-to-sim pipeline for dynamic photorealistic digital twins, accelerating deployment from simulation to reality.
